...
export function useRulersGrid(): RulersGridContextType | null {
  ...
  if (!_rulersGridContext) {
    console.warn('🚨 [useRulersGrid] Context not initialized yet, returning null');
    return null;
  }

  if (!context) {
    console.warn('🚨 [useRulersGrid] No context found, component may be outside RulersGridSystem provider');
    return null;
  }
  return context;
}
...
export function useGridState() {
  ...
  if (!rulersGrid) {
    return {
      settings: { visual: { enabled: false, color: '#888888', opacity: 0.3, step: 25 } },
      isVisible: false,
      toggleGrid: () => {},
      setGridVisibility: () => {},
      updateGridSettings: () => {},
