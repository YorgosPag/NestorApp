...
export function useAutoSaveSceneManager(): AutoSaveSceneManagerState {
  ...
  const setLevelSceneWithAutoSave = useCallback((levelId: string, scene: SceneModel) => {
    ...
    if (autoSaveEnabled && currentFileName && !isLoadingFromFirestoreRef.current) {
      ...
      saveTimeoutRef.current = setTimeout(async () => {
        ...
        console.log(`🔄 [AutoSave] Debounced save triggered for ${currentFileName} (level: ${levelId})`);
        ...
        try {
          ...
          if (success) {
            ...
            console.log(`✅ [AutoSave] Successfully saved changes to ${currentFileName}`);
          } else {
            setSaveStatus('error');
  ...
  useEffect(() => {
    ...
    console.log(`📂 [AutoSave] Auto-loading temporarily disabled for: ${currentFileName}`);
