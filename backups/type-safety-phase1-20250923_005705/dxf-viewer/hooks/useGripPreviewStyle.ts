...
export function updateDraftGripSettingsStore(settings: { overrideGlobalSettings: boolean; settings: any }) {
  draftGripSettingsStore = settings;
  console.log('🎯 [updateDraftGripSettingsStore] Updated store:', settings);
}
...
export function getGripPreviewStyleWithOverride(): GripPreviewStyle {
  ...
  if (draftGripSettingsStore?.overrideGlobalSettings && draftGripSettingsStore.settings) {
    ...
    console.log('✅ [getGripPreviewStyleWithOverride] Using SPECIFIC grip settings from store:', specificSettings);
  ...
  console.log('🔄 [getGripPreviewStyleWithOverride] Using GENERAL grip settings (checkbox unchecked or no store)');
