...
interface UseZoomWindowResult {
  ...
  startZoomWindow: (point: { x: number; y: number }) => void;
  updateZoomWindow: (point: { x: number; y: number }) => void;
  finishZoomWindow: () => { x: number; y: number; width: number; height: number } | null;
...
export function useZoomWindow(): UseZoomWindowResult {
  ...
  const startZoomWindow = useCallback((point: { x: number; y: number }) => {
    setState(prev => {
  ...
  const updateZoomWindow = useCallback((point: { x: number; y: number }) => {
    setState(prev => {
