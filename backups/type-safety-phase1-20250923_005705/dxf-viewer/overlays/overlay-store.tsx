...
export function OverlayStoreProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (!state.currentLevelId) {
      // console.log('🔍 [OverlayStore] No currentLevelId, not loading overlays');
      return;
    }
    
    // console.log('🔍 [OverlayStore] Loading overlays for level:', state.currentLevelId);
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      snapshot.docs.forEach(doc => {
        overlays[doc.id] = { ...doc.data(), id: doc.id } as Overlay;
      });
      // console.log('🔍 [OverlayStore] Loaded', snapshot.docs.length, 'overlays:', Object.keys(overlays));
