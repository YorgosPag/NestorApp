...
export class DxfSceneBuilder {
  static buildScene(content: string): SceneModel {
    console.log('🔧 Worker: Starting proper DXF parsing...');
    ...
    console.log(`📊 Worker: Parsed ${parsedEntities.length} raw entities`);
    ...
    parsedEntities.forEach((entityData, index) => {
      ...
      if (entity) {
        ...
        if (entities.length <= 3) {
          console.log(`🎯 Entity ${entities.length}: ${entity.type} on layer "${entity.layer}"`, 
            entity.type === 'line' ? `(${entity.start.x},${entity.start.y}) -> (${entity.end.x},${entity.end.y})` : '');
        }
      }
    });
    
    console.log(`✅ Worker: Final count: ${entities.length} entities across ${Object.keys(layers).length} layers`);
    ...
    console.log('📐 Worker: Calculated bounds:', bounds);
  ...
  private static registerLayer(layers: Record<string, SceneLayer>, layerName: string): void {
    if (!layers[layerName]) {
      ...
      console.log('📋 Worker: Registered layer:', layerName);
    }
  }
