...
import type { Point2D, HoverRenderContext } from './types';
import { drawVerticesPath } from '../renderers/shared/geometry-rendering-utils';
import { validateArcEntity, validateEllipseEntity } from '../renderers/shared/entity-validation-utils';
import { renderMeasurementLabel } from '../renderers/shared/geometry-rendering-utils';
...
export function renderCircleHover({ entity, ctx, worldToScreen, options }: HoverRenderContext): void {
  ...
  // console.log(`🔍 [renderCircleHover] Circle ${entity.id} - hovered: ${options.hovered}, selected: ${options.selected}, isDashed: ${isDashed}`);
  // console.log(`🎨 [renderCircleHover] Setting strokeStyle: ${isDashed ? UI_COLORS.WHITE : UI_COLORS.WHITE}, lineWidth: ${isDashed ? '2' : '1'}, lineDash: ${isDashed ? '[5,5]' : '[]'}`);
...
export function renderRectangleHover({ entity, ctx, worldToScreen, options }: HoverRenderContext): void {
  ...
  // console.log(`🎯 [renderRectangleHover] Called for rectangle ${entity.id} - rendering DASHED lines ONLY (no measurements)`);
