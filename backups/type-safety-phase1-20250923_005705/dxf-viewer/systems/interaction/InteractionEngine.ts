...
export function useInteractionEngine({
  ...
  const handleMouseDown = useCallback((event: MouseEvent) => {
    ...
    console.log('🎯 InteractionEngine: MouseDown', { canvasPoint, worldPoint: snappedWorldPoint, tool: activeTool });
  ...
  const handleMouseUp = useCallback((event: MouseEvent) => {
    ...
    if (isClick) {
      ...
      if (isDoubleClick) {
        ...
        console.log('🎯 InteractionEngine: DoubleClick', { canvasPoint, worldPoint: snappedWorldPoint });
      } else {
        handlers.onCanvasClick?.(canvasPoint, snappedWorldPoint);
        lastClickRef.current = { time: upTime, point: canvasPoint };
        console.log('🎯 InteractionEngine: Click', { canvasPoint, worldPoint: snappedWorldPoint, tool: activeTool });
      }
    }
  ...
  const handleWheel = useCallback((event: WheelEvent) => {
    if (handlers.onWheel) {
      ...
      console.log('🎯 InteractionEngine: Wheel', { deltaY: event.deltaY, tool: activeTool });
    }
  }, [handlers, activeTool]);
  ...
  const handleKeyDown = useCallback((event: KeyboardEvent) => {
    ...
    console.log('🎯 InteractionEngine: KeyDown', { key: event.key, tool: activeTool });
