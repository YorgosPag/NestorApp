...
export function useDynamicInputState({ activeTool }: UseDynamicInputStateProps) {
  ...
  const [firstClickPoint, setFirstClickPoint] = useState<Point | null>(null);
  
  // Refs
  const hideAngleLengthFieldsRef = useRef(false);
  const drawingPhaseRef = useRef<Phase>('first-point');
  const firstClickPointRef = useRef<Point | null>(null); // Hot reload safe backup
  ...
  const setFirstClickPointSafe = useCallback((point: Point | null) => {
