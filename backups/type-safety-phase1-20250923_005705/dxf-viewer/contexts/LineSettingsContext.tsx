...
// Types
export type LineType = keyof typeof LINE_TYPE_LABELS;
export type LineCapStyle = keyof typeof LINE_CAP_LABELS;
export type LineJoinStyle = keyof typeof LINE_JOIN_LABELS;
...
export interface LineSettings {
  // Basic properties
  enabled: boolean;          // ΝΕΟ! Ενεργοποίηση/απενεργοποίηση εμφάνισης γραμμών
  lineType: LineType;
  lineWidth: number;
  color: string;
  opacity: number;

  // Advanced dash properties
  dashScale: number;
  dashOffset: number;
...
// Dash patterns based on line type
...
export function LineSettingsProvider({ children }: { children: React.ReactNode }) {
  ...
  //   debounceMs: 500,
  //   onSaved: (data) => {
  //     console.log('✅ [LineSettings] Αποθηκεύτηκαν οι γενικές ρυθμίσεις γραμμής:', data);
  //   },
  //   onError: (error) => {
  ...
  const getCurrentDashPattern = useCallback((): number[] => {
    const basePattern = DASH_PATTERNS[settings.lineType] || [];
    if (basePattern.length === 0) return [];

    // Apply dash scale
    return basePattern.map(value => value * settings.dashScale);
