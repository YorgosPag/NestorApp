...
export class OrthoSnapEngine extends BaseSnapEngine {
  ...
  initialize(entities: Entity[]): void {
    console.log('🎯 OrthoSnapEngine: Initialize with', entities.length, 'entities');
  }
  ...
  findSnapCandidates(cursorPoint: Point2D, context: SnapEngineContext): SnapEngineResult {
    ...
    if (referencePoint) {
      ...
      for (const orthoPoint of orthoPoints) {
        const distance = GeometricCalculations.calculateDistance(cursorPoint, orthoPoint.point);
  ...
  private findReferencePoint(cursorPoint: Point2D, context: SnapEngineContext): Point2D | null {
    ...
    for (const entity of context.entities) {
      ...
      for (const endpoint of endpoints) {
        const distance = GeometricCalculations.calculateDistance(cursorPoint, endpoint);
  ...
  private getOrthogonalPoints(referencePoint: Point2D, cursorPoint: Point2D, maxDistance: number): Array<{point: Point2D, type: string, entityId?: string}> {
    ...
    const horizontalDistance = GeometricCalculations.calculateDistance(cursorPoint, horizontalPoint);
    const verticalDistance = GeometricCalculations.calculateDistance(cursorPoint, verticalPoint);
    ...
    if (diagonalDistance <= maxDistance) {
      ...
      const dist45 = GeometricCalculations.calculateDistance(cursorPoint, diagonal45);
      const dist135 = GeometricCalculations.calculateDistance(cursorPoint, diagonal135);
