...
export class NearestSnapEngine extends BaseSnapEngine {
  ...
  initialize(entities: Entity[]): void {
    console.log('🎯 NearestSnapEngine: Initialize with', entities.length, 'entities');
  }
  ...
  findSnapCandidates(cursorPoint: Point2D, context: SnapEngineContext): SnapEngineResult {
    ...
    for (const entity of context.entities) {
      ...
      if (nearestPoint) {
        const distance = GeometricCalculations.calculateDistance(cursorPoint, nearestPoint);
  ...
  private getNearestPointOnEntity(entity: Entity, point: Point2D): Point2D | null {
    ...
    if (entityType === 'line') {
      ...
      if (points && points.length > 1) {
        return this.getNearestPointOnPolyline(point, points, (entity as any).closed);
      }
    }
  ...
  private getNearestPointOnPolyline(point: Point2D, points: Point2D[], closed: boolean): Point2D | null {
    ...
    for (let i = 1; i < points.length; i++) {
      ...
      const distance = GeometricCalculations.calculateDistance(point, segmentNearest);
