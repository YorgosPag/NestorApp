...
export function getSelectionColors(kind: MarqueeKind): SelectionColors {
  ...
  if (kind === 'window') {
    ...
    if (DEBUG_CANVAS_CORE) console.log('🔵 Window selection colors:', result, 'from settings:', settings.selection.window);
    return result;
  } else {
    ...
    if (DEBUG_CANVAS_CORE) console.log('🟢 Crossing selection colors:', result, 'from settings:', settings.selection.crossing);
    return result;
  }
}
