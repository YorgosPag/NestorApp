...
interface DxfCanvasProps {
  ...
  overlayEntities?: Entity[]; // 🎯 NEW: Overlay entities for unified snapping
}
...
export const DxfCanvasRefactored = forwardRef<DxfCanvasRef, DxfCanvasProps>(
  function DxfCanvasRefactored(props, ref) {
    ...
    const [gripPreview, setGripPreview] = useState<{ entityId: string; next: Partial<Entity> } | null>(null);
    ...
    const handleMouseMove = useCallback((point: Point, e?: React.MouseEvent<HTMLCanvasElement>) => {
      console.log('🖱️ [DxfCanvasRefactored] handleMouseMove called with point:', point, 'activeTool:', activeTool);
      ...
      if (isDragging) {
        flushSync(() => {
          console.log('🖱️ [DxfCanvasRefactored] Updating mouse position during dragging:', point);
      ...
      console.log('🖱️ [DxfCanvasRefactored] Updating mouse position:', point);
      updateMousePosition(point);
