...
export function useSceneManager(): SceneManagerState {
  ...
  const setLevelScene = useCallback((levelId: string, scene: SceneModel) => {
    ...
    if (!scene.entities?.length) {
      ...
      if (DEBUG_SCENE_MANAGER) console.log(`🏢 Setting scene για level ${levelId}:`, scene.entities.length, 'entities');
    }
    setLevelScenes(prev => {
  ...
  const clearLevelScene = useCallback((levelId: string) => {
    if (DEBUG_SCENE_MANAGER) console.log(`🏢 Clearing scene για level ${levelId}`);
    setLevelScenes(prev => {
