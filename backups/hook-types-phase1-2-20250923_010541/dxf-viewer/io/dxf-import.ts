import type { DxfImportResult, SceneModel, SceneBounds } from '../types/scene';
...
export class DxfImportService {
        ...
        switch (entity.type) {
          ...
          case 'polyline':
            ...
            if (polylineEntity.vertices && Array.isArray(polylineEntity.vertices)) {
              polylineEntity.vertices.forEach((vertex: any) => {
        ...
        switch (entity.type) {
          ...
          case 'polyline':
            ...
            if (polyEnt.vertices && Array.isArray(polyEnt.vertices)) {
              polyEnt.vertices.forEach((vertex: any) => {
                if (vertex.x !== undefined && vertex.y !== undefined) {
                  vertex.x += offsetX;
                  vertex.y += offsetY;
                }
  ...
  async importDxfFile(file: File, encoding?: string): Promise<DxfImportResult> {
    console.log('🔄 Starting DXF import for:', file.name, 'Size:', file.size, 'bytes', 'with encoding:', encoding || 'auto-detect');
    
    // Προσωρινά επαναφορά σε direct parsing μέχρι να διορθωθεί το worker
    if (process.env.NODE_ENV === 'development') {
      try {
        console.log('🔧 Development mode: Using direct parsing (worker has MIME type issue)');
        const result = await this.directParseFileWithEncoding(file, encoding);
        if (result.success) {
          return result;
        }
        console.log('⚠️ Direct parsing failed, falling back to worker');
      } catch (error) {
        console.log('⚠️ Direct parsing error, falling back to worker:', error);
      }
    }
  ...
  private async directParseFileWithEncoding(file: File, encoding?: string): Promise<DxfImportResult> {
    ...
    for (const currentEncoding of encodings) {
      try {
        console.log(`🔤 Trying encoding: ${currentEncoding}`);
        const content = await this.tryReadFileWithEncoding(file, currentEncoding);
        
        if (!content) {
          console.log(`❌ Failed to read with ${currentEncoding}`);
          continue;
        }
        
        // Debug: Show first few characters and bytes
        console.log(`🔍 First 200 chars with ${currentEncoding}:`, content.substring(0, 200));
        ...
        if (hasGreekText) {
          console.log(`🇬🇷 Greek text detected with ${currentEncoding}`);
        }
        if (hasGarbledChars) {
          console.log(`❌ Garbled characters detected with ${currentEncoding}: ${garbledCount} replacement chars`);
          // If UTF-8 produces garbage, skip to next encoding
          if (currentEncoding === 'UTF-8') {
            console.log(`⏭️ Skipping UTF-8 due to garbled chars, trying next encoding`);
            continue;
          }
        }
        ...
        console.log(`🔧 Direct parsing with ${currentEncoding}: Building scene from DXF content`);
        
        const scene = DxfSceneBuilder.buildScene(content);
        
        if (!scene) {
          console.log(`❌ Scene builder returned null with ${currentEncoding}`);
          continue;
        }
        
        console.log(`🎯 Scene built with ${currentEncoding}:`, {
        ...
        if (scene && scene.entities.length > 0) {
          ...
          console.log('🎯 Applied perfect bounds normalization:', perfectBounds);
        }
        
        if (!DxfSceneBuilder.validateScene(scene)) {
          console.log(`❌ Invalid scene with ${currentEncoding}:`, {
            hasEntities: !!scene?.entities,
            entityCount: scene?.entities?.length || 0,
            hasLayers: !!scene?.layers,
            layerCount: Object.keys(scene?.layers || {}).length,
            hasBounds: !!scene?.bounds,
            bounds: scene?.bounds
          });
          console.log('⏭️ Trying next encoding...');
          continue;
        }
        
        const parseTimeMs = performance.now() - startTime;
        console.log(`✅ Direct parsing with ${currentEncoding} completed in ${parseTimeMs.toFixed(2)}ms`);
        ...
        console.log(`❌ Error with ${currentEncoding}:`, error);
        continue;
      }
    }
