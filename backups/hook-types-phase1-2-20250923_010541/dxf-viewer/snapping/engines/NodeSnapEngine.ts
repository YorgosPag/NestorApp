...
export class NodeSnapEngine extends BaseSnapEngine {
  ...
  initialize(entities: Entity[]): void {
    console.log('🎯 NodeSnapEngine: Initialize with', entities.length, 'entities');
  ...
  findSnapCandidates(cursorPoint: Point2D, context: SnapEngineContext): SnapEngineResult {
    ...
    for (const node of nearbyNodes) {
      ...
      const distance = GeometricCalculations.calculateDistance(cursorPoint, node.point);
  ...
  private getAllNodePoints(entity: Entity): Array<{point: Point2D, type: string}> {
    ...
    if (entityType === 'line') {
      ...
      if ((entity as any).position) {
        nodes.push({point: (entity as any).position, type: 'Point'});
      }
    }
