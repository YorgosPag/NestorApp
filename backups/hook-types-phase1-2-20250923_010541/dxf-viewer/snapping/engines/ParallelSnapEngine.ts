...
export class ParallelSnapEngine extends BaseSnapEngine {
  ...
  initialize(entities: Entity[]): void {
    console.log('🎯 ParallelSnapEngine: Initialize with', entities.length, 'entities');
  }
  ...
  findSnapCandidates(cursorPoint: Point2D, context: SnapEngineContext): SnapEngineResult {
    ...
    for (const refLine of referenceLines) {
      ...
      for (const dist of distances) {
        ...
        const distance1 = GeometricCalculations.calculateDistance(cursorPoint, parallelPoint1);
        const distance2 = GeometricCalculations.calculateDistance(cursorPoint, parallelPoint2);
