...
export class GripInteractionManager {
  ...
  // Event callbacks
  private onGeometryUpdate?: (entityId: string, geometry: any) => void;
  ...
  startDragging(entity: EntityModel, gripIndex: number, startPosition: Point2D): boolean {
    ...
    console.log(`🎯 [GripInteractionManager] Started dragging grip ${gripIndex} of ${entity.type} ${entity.id}`);
  ...
  updateDragPosition(entity: EntityModel, currentPosition: Point2D): boolean {
    ...
    console.log(`📏 [GripInteractionManager] Updating drag position for ${entity.type} ${entity.id}`);
    
    // 🎯 NO DUPLICATE MEASUREMENTS - individual renderers handle this already
    console.log(`📏 [GripInteractionManager] ${entity.type} grip ${this.state.activeGrip.gripIndex} - measurements handled by renderer`);
  ...
  endDragging(): boolean {
    ...
    console.log(`🎯 [GripInteractionManager] Ended dragging`);
   ...
   */
  private cloneEntityGeometry(entity: EntityModel): any {
  ...
  private calculateNewGeometry(entity: EntityModel, currentPosition: Point2D): any {
