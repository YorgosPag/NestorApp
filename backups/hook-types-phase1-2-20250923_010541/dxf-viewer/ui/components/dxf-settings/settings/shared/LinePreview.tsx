...
// Helper function για dash patterns - αντιγράφουμε από EntitiesSettings
const getDashArray = (type: string, scale: number = 1) => {
  ...
  const pattern = patterns[type] || patterns.solid;
  return pattern.map(value => value * scale).join(' ');
};
              ...
              return (
                <>
                  <line
                    ...
                    strokeDasharray={getDashArray(previewSettings.lineType, previewSettings.dashScale || 1)}
                    strokeDashoffset={previewSettings.dashOffset || 0}
                    strokeLinecap={previewSettings.lineCap as any || 'butt'}
                    strokeLinejoin={previewSettings.lineJoin as any || 'miter'}
                  />
                  <line
