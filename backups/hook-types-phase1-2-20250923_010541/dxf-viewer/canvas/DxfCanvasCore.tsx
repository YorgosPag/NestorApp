...
import { MARGINS, type ViewTransform, type Point2D } from '../systems/rulers-grid/config';
...
export function DxfCanvasCore({
  ...
  const lastGridSettingsRef = useRef<any>(null);
  ...
  const grid = safeRulersGrid?.grid || { visual: { enabled: true, color: '#00FF80', opacity: 0.3, step: 25 } };
  ...
  const getRulersGridSettings = () => {
    ...
    if (!rulersGrid) {
      ...
      return storedSettings || {
        grid: { visual: { enabled: true, color: '#00FF80', opacity: 0.3, step: 25 } },
  ...
  useEffect(() => {
    ...
    if (entityRendererRef.current && scene) {
      const hasPreviewEntity = scene.entities?.some(entity => (entity as any).preview === true);
