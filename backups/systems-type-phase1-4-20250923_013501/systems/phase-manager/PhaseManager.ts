...
import type { Point2D, ViewTransform } from '../coordinates/config';
...
import { renderSquareGrip } from '../../utils/renderers/shared/geometry-rendering-utils';
...
function renderMeasurementText(
  ...
  ctx.save();
  ctx.fillStyle = '#00FF80'; // ⚡ NUCLEAR: GREEN FOR LIVE MEASUREMENTS
  ctx.font = '12px Arial';
...
export class PhaseManager {
  ...
  determinePhase(entity: EntityModel, options: RenderOptions = {}): PhaseRenderingState {
    ...
    if (options.preview || (entity as any).preview === true) {
      if (DEBUG_PHASE_MANAGER) console.log('🎯 [PhaseManager] PREVIEW PHASE determined for:', {
        entityId: entity.id,
        entityType: entity.type,
        fromOptions: !!options.preview,
        fromEntity: !!(entity as any).preview,
        phase: 'preview'
      });
    ...
    if (options.hovered || options.selected) {
      ...
      if (DEBUG_PHASE_MANAGER) console.log('🎯 [PhaseManager] INTERACTIVE PHASE determined for:', {
        entityId: entity.id,
        entityType: entity.type,
        subState,
        phase: 'interactive'
    ...
    if (DEBUG_PHASE_MANAGER) console.log('🎯 [PhaseManager] NORMAL PHASE determined for:', {
      entityId: entity.id,
      entityType: entity.type,
      phase: 'normal'
    });
  ...
  applyPhaseStyle(entity: EntityModel, state: PhaseRenderingState): void {
    ...
    switch (state.phase) {
      case 'preview':
        ...
        if (DEBUG_PHASE_MANAGER) console.log('🔍 [PhaseManager] applyPhaseStyle called for preview - DETAILED ANALYSIS:', {
          entityType: entity.type,
          entityId: entity.id,
          hasPreview: (entity as any).preview,
          hasMeasurement: (entity as any).measurement,
          hasOverlayPreview: (entity as any).isOverlayPreview,
          isOverlayEntity,
          phase: 'preview',
          allEntityProps: Object.keys(entity as any).join(', ')
        });
        
        if (isOverlayEntity) {
          const toolStyle = toolStyleStore.get();
          if (DEBUG_PHASE_MANAGER) console.log('🎨 [PhaseManager] ✅ OVERLAY DETECTED! Using ToolStyleStore colors:', {
            strokeColor: toolStyle.strokeColor,
            fillColor: toolStyle.fillColor,
            lineWidth: toolStyle.lineWidth,
            opacity: toolStyle.opacity,
            willSetStroke: toolStyle.strokeColor || '#22c55e',
            willSetFill: toolStyle.fillColor || '#ff000080'
          });
          ...
          if (DEBUG_PHASE_MANAGER) console.log('🎨 [PhaseManager] ✅ REGULAR DXF - Using LinePreviewSettings with override check:', {
            strokeColor: previewStyle.strokeColor,
            lineWidth: previewStyle.lineWidth,
            lineDash: previewStyle.lineDash,
            opacity: previewStyle.opacity,
            source: 'getLinePreviewStyleWithOverride()'
          });
      ...
      case 'normal':
        ...
        if (generalStyleForNormal.enabled) {
          ...
          if (DEBUG_PHASE_MANAGER) console.log('🎨 [PhaseManager] ✅ NORMAL - Using general settings but SOLID:', {
            strokeColor: generalStyleForNormal.strokeColor,
            lineWidth: generalStyleForNormal.lineWidth,
            opacity: generalStyleForNormal.opacity,
            lineDash: 'solid ([])'
          });
        } else {
  ...
  renderPhaseGrips(entity: EntityModel, grips: GripInfo[], state: PhaseRenderingState): void {
    ...
    if (state.phase === 'preview' && !(entity as any).showPreviewGrips) {
  ...
  private drawPhaseGrip(position: Point2D, colorState: 'cold' | 'warm' | 'hot', state: PhaseRenderingState, gripType?: string): void {
    ...
    if (gripType === 'edge') {
      ...
      if (DEBUG_PHASE_MANAGER) console.log('🎨 [PhaseManager] EDGE GRIP με χρώματα από settings:', {
        gripType,
        colorState,
        fillColor,
        position,
        gripSettings: this.gripSettings ? 'Loaded' : 'Using defaults',
        message: '*** EDGE GRIP με δυναμικά χρώματα ***'
      });
    } else {
      ...
      fillColor = colorState === 'hot' ? colors.hot
                : colorState === 'warm' ? colors.warm
                                        : colors.cold;
      if (DEBUG_PHASE_MANAGER) console.log('🎨 [PhaseManager] VERTEX GRIP με χρώματα από settings:', {
        gripType: gripType || 'vertex',
        colorState,
        fillColor,
        position,
        gripSettings: this.gripSettings ? 'Loaded' : 'Using defaults',
        message: '*** VERTEX GRIP με δυναμικά χρώματα ***'
      });
    }
  ...
  private renderLineDragMeasurements(entity: EntityModel, gripIndex: number, currentPos: Point2D): void {
    ...
    this.ctx.save();
    if (DEBUG_PHASE_MANAGER) console.log('🔴🔴🔴 [PhaseManager] renderLineDragMeasurements - DRAWING RED TEXT FOR LINE:', {
      screenCurrentPos,
      distance: distance.toFixed(2),
      canvasSize: { width: canvasWidth, height: canvasHeight },
      message: '*** RED COLOR #ff0000 USED FOR LINE DRAG MEASUREMENTS ***'
    });
  ...
  private renderCircleDragMeasurements(entity: EntityModel, gripIndex: number, currentPos: Point2D): void {
    ...
    this.ctx.save();
    if (DEBUG_PHASE_MANAGER) console.log('🔴🔴🔴 [PhaseManager] renderCircleDragMeasurements - DRAWING RED TEXT FOR CIRCLE:', {
      screenCurrentPos,
      newRadius: newRadius.toFixed(2),
      center,
      canvasSize: { width: canvasWidth, height: canvasHeight },
      message: '*** RED COLOR #ff0000 USED FOR CIRCLE DRAG MEASUREMENTS ***'
    });
    this.ctx.fillStyle = '#ff0000'; // Red for live measurements
    this.ctx.font = '12px Arial';
    this.ctx.textBaseline = 'middle';
  ...
  private renderRectangleDragMeasurements(entity: EntityModel, gripIndex: number, currentPos: Point2D): void {
    ...
    this.ctx.save();
    if (DEBUG_PHASE_MANAGER) console.log('🔴🔴🔴 [PhaseManager] renderRectangleDragMeasurements - DRAWING RED TEXT FOR RECTANGLE:', {
      screenGripPos,
      ...
      canvasSize: { width: canvasWidth, height: canvasHeight },
      message: '*** RED COLOR #ff0000 USED FOR RECTANGLE DRAG MEASUREMENTS ***'
    });
    this.ctx.fillStyle = '#ff0000'; // Red for live measurements
    this.ctx.font = '12px Arial';
  ...
  private renderEllipseDragMeasurements(entity: EntityModel, gripIndex: number, currentPos: Point2D): void {
    ...
    // Render live measurements
    this.ctx.save();
    if (DEBUG_PHASE_MANAGER) console.log('🔴🔴🔴 [PhaseManager] renderEllipseDragMeasurements - DRAWING RED TEXT FOR ELLIPSE:', {
      screenCenter,
      newMajorAxis: newMajorAxis.toFixed(2),
      newMinorAxis: newMinorAxis.toFixed(2),
      area: area.toFixed(2),
      perimeter: perimeter.toFixed(2),
      message: '*** RED COLOR #ff0000 USED FOR ELLIPSE DRAG MEASUREMENTS ***'
    });
    this.ctx.fillStyle = '#ff0000'; // Red for live measurements
    this.ctx.font = '12px Arial';
