...
function useLevelsSystemState({
  ...
  const handleError = useCallback(async (err: string | Error) => {
    ...
    if (StorageErrorHandler.isStorageError(err)) {
      console.log('🚨 Storage error detected in LevelsSystem:', errorMessage);
  ...
  const setLevelScene = useCallback((levelId: string, scene: any) => {
  ...
  const importLevelsData = useCallback(async (data: any): Promise<void> => {
    try {
      setIsLoading(true);
