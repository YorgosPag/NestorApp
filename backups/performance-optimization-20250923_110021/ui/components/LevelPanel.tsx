...
export function LevelPanel({ 
  ...
  if (DEBUG_LEVEL_PANEL) console.log('🎯 LevelPanel props:', { currentTool, onToolChange: !!onToolChange });
  ...
  const handleEditingModeChange = (mode: EditingMode) => {
    ...
    if (mode === 'editing') {
      ...
      console.log('🎯 [LevelPanel] Editing mode activated - grips enabled, grip-edit tool selected');
    } else {
  ...
  const handleLayeringActivation = () => {
    ...
    if (DEBUG_LEVEL_PANEL) console.log('🎯 [LevelPanel] Layering tool activated - showing toolbox and enabling grips');
  };
    ...
    <div className="h-full flex flex-col px-1 py-2 space-y-4">
      ...
      {Array.isArray(levels) && levels.length > 0 ? (
        <div className="space-y-2">
          {levels.map((level) => {
            if (DEBUG_LEVEL_PANEL) console.log('🎯 Rendering level:', level.id, level.name);
                ...
                <div className="flex items-center justify-between min-w-0">
                  {isEditing ? (
                    ...
                    <div className="flex-1 cursor-pointer" onClick={(e) => {
                      if (DEBUG_LEVEL_PANEL) console.log('🎯 CLICK EVENT FIRED! Level clicked:', level.id, 'currentTool:', currentTool, 'onToolChange exists:', !!onToolChange);
                      if (DEBUG_LEVEL_PANEL) console.log('🎯 Event details:', e.type, e.target);
                      setCurrentLevel(level.id);
                      // Auto-activate grip-edit tool for layer editing
                      if (currentTool !== 'grip-edit' && onToolChange) {
                        if (DEBUG_LEVEL_PANEL) console.log('🎯 Level selected, activating grip-edit tool');
                        onToolChange('grip-edit');
                      }
