...
const STATUS_COLORS_MAPPING: Record<PropertyStatus, { stroke: string; fill: string }> = {
...
const BUTTON_STATUS_COLORS: Record<PropertyStatus, string> = Object.fromEntries(
  Object.entries(STATUS_COLORS_MAPPING).map(([status, colors]) => [status, colors.stroke])
) as Record<PropertyStatus, string>;

// 🎯 EXPORT ΓΙΑ ΧΡΗΣΗ ΣΕ ΆΛΛΑ COMPONENTS - μοναδική πηγή αλήθειας
export { STATUS_COLORS_MAPPING };
...
export const OverlayToolbar: React.FC<OverlayToolbarProps> = ({
  ...
  // Debug logging
  React.useEffect(() => {
    console.log('🎛️ [OverlayToolbar] Rendered with mode:', mode);
  }, [mode]);
