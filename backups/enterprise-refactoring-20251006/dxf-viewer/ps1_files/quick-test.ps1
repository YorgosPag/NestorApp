...
try {
    ...
    if ($packageJson.dependencies.zustand) {
        ...
        Write-Error "Zustand δεν βρέθηκε στις dependencies!"
        $totalTests++; $failedTests++
    }
} catch {
    Write-Error "Σφάλμα ελέγχου package.json"
    $totalTests++; $failedTests++
}
...
if (Test-Path $configFile) {
    ...
    if ($configContent -match "USE_ZUSTAND_SETTINGS\s*=\s*true") {
        ...
        Write-ColorText "Zustand System: ΑΝΕΝΕΡΓΟ" $Yellow
        $totalTests++; $passedTests++
    }
} else {
...
try {
    ...
    Write-Success "Dev server: ΤΡΕΧΕΙ (Status: $($response.StatusCode))"
    $totalTests++; $passedTests++
} catch {
    Write-Error "Dev server: ΔΕΝ ΤΡΕΧΕΙ"
    Write-ColorText "Ξεκίνησε με: npm run dev:fast" $Blue
    $totalTests++; $failedTests++
}
...
Write-ColorText "Συνολικά tests: $totalTests" $Blue
...
if ($failedTests -eq 0) {
    $percent = "%"
    $message = "Ολα τα tests περασαν! (" + $successRate + $percent + ")"
    Write-ColorText $message $Green
} else {
    $percent = "%"
    $message = "Καποια tests απετυχαν (" + $successRate + $percent + ")"
