...
export function useBuildingForm({
  ...
  const validate = useCallback((): boolean => {
    console.log('🔍 [useBuildingForm] Validating form...');
    const newErrors: Partial<Record<keyof BuildingFormData, string>> = {};

    // Required: name
    if (!formData.name.trim()) {
      console.log('❌ [useBuildingForm] Name is required but empty!');
      newErrors.name = t('validation.nameRequired');
    }

    // Required: address
    if (!formData.address.trim()) {
      console.log('❌ [useBuildingForm] Address is required but empty!');
    ...
    setErrors(newErrors);
    console.log('🔍 [useBuildingForm] Validation errors:', newErrors);
    console.log('🔍 [useBuildingForm] formData.name:', `"${formData.name}"`);
    console.log('🔍 [useBuildingForm] formData.address:', `"${formData.address}"`);
  ...
  const handleSubmit = useCallback(
    async (e: React.FormEvent) => {
      e.preventDefault();
      console.log('🏗️ [useBuildingForm] handleSubmit called!');
      console.log('🏗️ [useBuildingForm] formData:', formData);

      if (!validate()) {
        console.log('❌ [useBuildingForm] Validation failed!');
        return;
      }
      console.log('✅ [useBuildingForm] Validation passed!');
      ...
      try {
        ...
        console.log('🏗️ [useBuildingForm] createBuilding result:', result);
        if (result.success) {
          console.log('✅ [useBuildingForm] Building created successfully!');
          toast.success(t('dialog.messages.success'));
          setFormData(INITIAL_FORM_DATA);
          onBuildingAdded?.();
          onOpenChange(false);
        } else {
          console.log('❌ [useBuildingForm] Building creation failed:', result.error);
