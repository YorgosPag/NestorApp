...
export const CanvasSection: React.FC<DXFViewerLayoutProps & { overlayMode: OverlayEditorMode, currentStatus: Status, currentKind: OverlayKind }> = (props) => {
  ...
  */
  ...
  // 🔍 DEBUG (2026-02-01): Log transform/viewport props passed to canvases
  // This helps identify if the props are DIFFERENT between DXF and Layer canvases
  React.useEffect(() => {
    const now = performance.now().toFixed(0);
    console.log(`[${now}ms][CanvasSection] PROPS: transform.offsetY=${transform.offsetY.toFixed(1)}, viewport.h=${viewport.height.toFixed(1)}, viewport.w=${viewport.width.toFixed(1)}`);
  }, [transform, viewport]);
