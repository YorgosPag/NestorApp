...
- **Registry**: `.ssot-registry.json` — 62+ modules σε 7 tiers (core, data integrity, security, business logic, enum constants, infrastructure, config consolidation, expanded coverage)
- **Baseline**: `.ssot-violations-baseline.json` — 7 αρχεία, 16 violations (2026-04-11, όλα Domain A file-download)
- **Entity audit trail**: Το module `entity-audit-trail` (Tier 3, ADR-195) απαγορεύει direct writes στο `entity_audit_trail`, inline queries της collection, και re-implementations του `useEntityAudit` hook. Canonical: `src/services/entity-audit.service.ts` + `src/hooks/useEntityAudit.ts`.
...
- **PRE-COMMIT ENFORCEMENT**: Ο SSoT ratchet hook μπλοκάρει `addDoc(`, `.add({`, `.collection().doc()` σε νέα αρχεία. Module: `addDoc-prohibition` στο `.ssot-registry.json`
- Ref: ADR-017 (Enterprise ID Generation), ADR-210 (Document ID Audit), ADR-294 (SSoT Ratchet)
...
### Missing i18n Keys Enforcement (Pre-commit Check 3.8):
- **Pre-commit hook** (`scripts/check-i18n-missing-keys.js`) — μπλοκάρει `t('key')` calls χωρίς αντίστοιχο key στα locale JSON
- Εντοπίζει namespace mismatch (π.χ. `t('common.close')` σε namespace `files` αντί `t('share.close')`)
- **Baseline**: `.i18n-missing-keys-baseline.json` (4762 legacy violations — ratchet down)
- `npm run i18n-keys:baseline` — ενημερώνει το baseline file

### ICU Interpolation Enforcement (Pre-commit Check 3.9):
- **ΑΠΑΓΟΡΕΥΕΤΑΙ** `{{variable}}` (i18next default) σε locale JSON files
- **ΥΠΟΧΡΕΩΤΙΚΟ** `{variable}` (ICU format) — project χρησιμοποιεί `i18next-icu` plugin
- **Pre-commit hook** (`scripts/check-icu-interpolation.sh`) — **RATCHET MODE**
- **Baseline**: `.icu-violations-baseline.json` (0 violations — fully cleaned 2026-04-09)
- `npm run icu:baseline` — ενημερώνει το baseline file

### Firestore companyId Enforcement (Pre-commit Check 3.10):
- **ΚΑΘΕ Firestore `query()` με `where()`** ΠΡΕΠΕΙ να περιλαμβάνει `where('companyId', '==', companyId)`
- **ΓΙΑΤΙ**: Χωρίς companyId, τα Firestore rules κάνουν deny → `permission-denied` errors στο runtime
- **Pre-commit hook** (`scripts/check-firestore-companyid.sh`) — **RATCHET MODE**
- **Baseline**: `.firestore-companyid-baseline.json` (48 violations σε 30 αρχεία, 2026-04-10)
- `npm run firestore:audit` — ελέγχει πρόοδο vs baseline
- **Ratchet**: Νέο αρχείο + violation = ΜΠΛΟΚ. Υπάρχον αρχείο + αύξηση = ΜΠΛΟΚ.

### i18n Runtime Resolver Reachability (Pre-commit Check 3.13) — ADR-279/ADR-280:
- **Ο ΚΑΝΟΝΑΣ**: Κάθε dotted i18n key που αναφέρεται σε static config (service-config.ts, individual-config.ts, modal-select label tables, dropdown-*-labels.ts) ΠΡΕΠΕΙ να είναι προσβάσιμη μέσω του `SERVICE_FORM_NAMESPACES` list στο `src/components/generic/i18n/translate-field-value.ts` — όχι απλώς να υπάρχει «κάπου» στα locale files.
- **ΓΙΑΤΙ**: Μετά το ADR-280 namespace splitting, keys μπορεί να μεταφερθούν σε νέο namespace που δεν είναι στη λίστα του runtime resolver. Το `namespace-compat.ts → LEGACY_NESTED_MAP` σώζει τα κλασικά `t()` calls αλλά **όχι** τον `translateFieldValue` που καλεί απευθείας `i18next.exists()`. Αποτέλεσμα: raw dotted keys στο UI. Συνέβη 2026-04-11 στη φόρμα Δημόσιας Υπηρεσίας → «Βασικά Στοιχεία».
- **Pre-commit hook** (`scripts/check-i18n-resolver-reachability.js`) — **RATCHET MODE**:
  - Παρσάρει το `SERVICE_FORM_NAMESPACES` από το resolver module (single source of truth)
  - Φορτώνει τα per-namespace JSON sets
  - Προσομοιώνει ακριβώς τον runtime resolver (direct hit + `contacts.` prefix strip)
  - Αν key δεν resolvάρει → ΜΠΛΟΚ
- **Baseline**: `.i18n-resolver-reachability-baseline.json` (378 violations σε 13 αρχεία, 2026-04-11)
- **ΟΤΑΝ προσθέτεις νέο split namespace** που θα διαβάζεται από service/individual form renderers → πρόσθεσέ το στο `SERVICE_FORM_NAMESPACES` **ΜΑΖΙ** με το locale file
- **Commands**:
  - `npm run resolver-reach:audit` — scan όλου του scope
  - `npm run resolver-reach:baseline` — refresh baseline μετά από legit cleanup
- **Σχέση με CHECK 3.8 / 3.12**: CHECK 3.8 ελέγχει `t('key')` calls vs any locale. CHECK 3.12 ελέγχει `label: 'key'` vs any locale. CHECK 3.13 ελέγχει runtime namespace reachability — κλείνει την τρύπα «key exists in locales but unreachable at runtime».

### Audit Value Catalogs SSoT (Pre-commit Check 3.14) — ADR-195/ADR-279:
- **Ο ΚΑΝΟΝΑΣ**: Κάθε audit-tracked field με enum values ΠΡΕΠΕΙ να δηλωθεί στο `src/config/audit-value-catalogs.ts` με pointer στο canonical i18n catalog (`{ ns, path }`). Ο audit trail renderer (`ContactHistoryTab`, `audit-timeline-entry`) μεταφράζει τα stored values **αποκλειστικά** μέσω αυτού του map — δεν επιτρέπεται διπλασιασμός στο `common:audit.values.*`.
- **ΓΙΑΤΙ**: Συνέβη 2026-04-11 στο `/contacts` — ο `category` (public service) rendered `"Κατηγορία: Δήμος → region"` (mixed). Root cause: το `options.serviceCategories` είχε 19 entries αλλά το `audit.values.*` μόνο 2, και οι template-literal `t()` calls διαφεύγουν του CHECK 3.8. Single source of truth: τα enum values ζουν **μόνο** στο canonical form option catalog.
- **Pre-commit hook** (`scripts/check-audit-value-catalogs.js`) — **ZERO TOLERANCE** (no baseline):
...
- **ΟΤΑΝ προσθέτεις νέο enum audit field** → πρόσθεσέ το στο `AUDIT_VALUE_CATALOGS` και κάνε commit. Το CHECK 3.14 validates αυτόματα. Δεν χρειάζεται καμία άλλη αλλαγή.
- **Commands**:
  - `npm run audit-values:audit` — τρέχει τον validator manually
- **Σχέση με CHECK 3.8 / 3.12 / 3.13**: CHECK 3.8/3.12 δεν πιάνουν dynamic template-literal `t()` calls σε audit value rendering. CHECK 3.14 κλείνει αυτό το κενό ως single-source validator για audit-trail enum translation.

### Firestore Index Coverage (Pre-commit Check 3.15) — ADR-195 (Phase 10 hotfix):
- **Ο ΚΑΝΟΝΑΣ**: Κάθε query που περνάει από το `firestoreQueryService` SSoT ΠΡΕΠΕΙ να έχει matching composite index στο `firestore.indexes.json` — **και** για τη default variant (με auto-injected `companyId`) **και** για τη super-admin variant (χωρίς tenant prefix, όπως κάνει `buildTenantConstraints()` όταν `ctx.isSuperAdmin === true`).
- **ΓΙΑΤΙ**: Συνέβη 2026-04-11 στο `/projects/[id]/history` + `/admin/audit-log` + contact history. Το `subscribeEntity('ENTITY_AUDIT_TRAIL', { constraints: [where entityType, where entityId, orderBy timestamp desc] })` είχε index `[companyId, entityType, entityId, timestamp desc]` — company admins δούλευαν, **super admins έπαιρναν `FAILED_PRECONDITION`** γιατί το `[entityType, entityId, timestamp desc]` (χωρίς companyId) δεν υπήρχε.
- **Pre-commit hook** (`scripts/check-firestore-index-coverage.js`) — **ZERO TOLERANCE on touch**, no baseline:
...
- **Scope**: μόνο staged files. Pre-existing backlog (39 shapes σε 30 αρχεία, 2026-04-11) καθαρίζει σταδιακά με Boy Scout rule — το CHECK θα ξυπνήσει όταν αγγίξεις το αρχείο.
- **Commands**:
  - `npm run firestore:indexes:audit` — full scan (για CI / manual audit)
- **Γιατί όχι baseline**: ένα broken index είναι production incident, όχι tech-debt κατηγορία. Zero tolerance *on touch* είναι το Google presubmit equivalent για security/correctness gates.

### Firestore Rules Test Coverage (Pre-commit Check 3.16) — ADR-298:
- **Ο ΚΑΝΟΝΑΣ**: Κάθε αλλαγή στο `firestore.rules` ΠΡΕΠΕΙ να συνοδεύεται από matching test coverage στο `tests/firestore-rules/suites/`. Κάθε top-level `match /xxx/{id}` ΠΡΕΠΕΙ να υπάρχει είτε στο `FIRESTORE_RULES_COVERAGE` είτε στο `FIRESTORE_RULES_PENDING` (sunset list).
- **ΓΙΑΤΙ**: Συνέβη 2026-04-11 στο `entity_audit_trail` — rule AND-chain είχε super_admin short-circuit σε λάθος θέση, super-admin reads έπαιρναν `permission-denied`. Το CHECK 3.15 έπιασε το missing index (Bug #2), αλλά κανένα gate δεν έπιασε το broken rule shape (Bug #1). Αυτό το CHECK κλείνει την τρύπα.
- **Pre-commit hook** (`scripts/check-firestore-rules-test-coverage.js`) — **ZERO TOLERANCE on touch**, no baseline. Τρέχει όταν staged το `firestore.rules` ή κάποιο αρχείο στο `tests/firestore-rules/`.
- **SSoT**: `tests/firestore-rules/_registry/coverage-manifest.ts` — typed TS registry, persona × operation matrix ανά collection. Το `COVERAGE` export κάθε test file ΠΡΕΠΕΙ να ταιριάζει 1:1 με το manifest και να κάνει iterate το `COVERAGE.matrix` (drift prevention).
- **Harness**: `tests/firestore-rules/_harness/` — emulator lifecycle, persona→context factory, seed helpers, matrix-driven assertions, rule-shape validator.
- **Rule shape check (Phase A scope)**: μόνο για `immutable` pattern collections — το πρώτο OR leg του `allow read` gate πρέπει να είναι `isSuperAdminOnly()`. Phase B επεκτείνει σε `tenant_direct`.
- **Scope**: staged files only. `pnpm firestore-rules:coverage:audit` για full CI scan.
...
- **Jest config**: dedicated `jest.config.firestore-rules.js` (node env, isolated from main suite). Root `jest.config.js` εξαιρεί `tests/firestore-rules` μέσω `testPathIgnorePatterns`, οπότε τα δύο jest runs δεν διασταυρώνονται.
- **CI gate** (Phase D — 2026-04-11): `.github/workflows/firestore-rules.yml` — 2 jobs (static full scan + runtime emulator via `emulators:exec`). Path-triggered, pnpm + emulator binary cache, Temurin 17. Required status check for merge σε `main`.
- **Pending list**: 86 collections (2026-04-11). Boy Scout rule: όταν αγγίζεις pending collection, μετακινείται στο manifest με full matrix.
- **Γιατί όχι baseline**: Τα tests είναι binary (υπάρχει/δεν υπάρχει). Pending list εξυπηρετεί τη σταδιακή μετάβαση, όχι drift ratchet.
- **Σχέση με CHECK 3.10 / 3.15**: CHECK 3.10 ελέγχει client queries (companyId). CHECK 3.15 ελέγχει index coverage. CHECK 3.16 είναι ο τρίτος πυλώνας — server rule behavior validation.

### Entity Audit Coverage (Pre-commit Check 3.17) — ADR-195:
- **Ο ΚΑΝΟΝΑΣ**: Κάθε αρχείο που κάνει Firestore write σε audit-tracked collection (`projects`, `contacts`, `buildings`, `properties`, `floors`, `parking`, `storage`, `purchase_orders`, `companies`) ΠΡΕΠΕΙ επίσης να καλεί `EntityAuditService.recordChange()` από το canonical SSoT module (`src/services/entity-audit.service.ts`).
- **ΓΙΑΤΙ**: Συνέβη 2026-04-11 στο `/projects` — το `/api/projects/list` POST handler έγραφε νέο project document αλλά δεν έκανε record στο audit trail, οπότε το per-project «Ιστορικό» tab ήταν άδειο ενώ το reader side ήταν πλήρως ενσωματωμένο (ADR-195 Phase 3). Το υπάρχον SSoT registry entry `entity-audit-trail` κλειδώνει τα direct writes στην `entity_audit_trail` collection (reader protection), αλλά δεν εγγυάται ότι κάθε mutation σε tracked entity γεννά audit entry. Αυτό το check κλείνει το συμμετρικό κενό στην writer πλευρά.
- **Pre-commit hook** (`scripts/check-entity-audit-coverage.js`) — **RATCHET MODE**:
...
- **Baseline**: `.entity-audit-coverage-baseline.json` (70 legacy αρχεία grandfathered, 2026-04-11)
- **ΟΤΑΝ προσθέτεις νέο handler** που γράφει σε audit-tracked collection → ΥΠΟΧΡΕΩΤΙΚΑ fetch `performedBy` + `performedByName` + `companyId` και κάλεσε `recordChange({ entityType, entityId, action, changes, performedBy, performedByName, companyId })` στην ίδια transactional ενότητα
- **Commands**:
...
- **Σχέση με entity-audit-trail SSoT**: Το registry entry απαγορεύει **direct writes** στο `entity_audit_trail` collection από οπουδήποτε εκτός του canonical service (reader-side protection). Το CHECK 3.17 απαιτεί **writes σε tracked entities** να καλούν το canonical service (writer-side coverage). Μαζί εγγυώνται ότι η audit trail είναι complete και untampered.
