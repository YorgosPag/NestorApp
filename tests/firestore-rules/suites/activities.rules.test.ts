/**
 * Firestore Rules — `activities` collection
 *
...
export const COVERAGE = FIRESTORE_RULES_COVERAGE.find(
  (c) => c.collection === 'activities',
)!;

describe('activities.rules — tenant_direct (crmDirectMatrix)', () => {
  ...
  for (const cell of COVERAGE.matrix) {
    describe(`${cell.persona} × ${cell.operation}`, () => {
      it(`should ${cell.outcome}${cell.reason ? ` (${cell.reason})` : ''}`, async () => {
        const docId = 'activity-same-tenant';

        await seedActivity(env, docId);
        ...
        const target: AssertTarget = {
          ...
          createData: {
            type: 'call',
            description: `Created Activity ${cell.persona}`,
