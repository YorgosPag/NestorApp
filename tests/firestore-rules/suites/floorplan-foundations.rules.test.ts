/**
 * Firestore Rules — `floorplan_foundations` collection (ADR-657 / ADR-436)
 *
 * ADR-657 AUTHORING-tier canary for the `'kind+params'` create-key variant.
 * Substructure footings (pad/strip/tie-beam) are structural authoring data,
 * edited only inside /dxf/viewer, so `external_user` is denied ALL FIVE ops.
...
export const COVERAGE = FIRESTORE_RULES_COVERAGE.find(
  (c) => c.collection === 'floorplan_foundations',
)!;

const DOC_ID = 'foundation-same-tenant';
const PROJECT_ID = 'proj-test';
const FLOORPLAN_ID = 'floorplan-test';

describe('floorplan_foundations.rules — AUTHORING tier (ADR-657)', () => {
  ...
  for (const cell of COVERAGE.matrix) {
    describe(`${cell.persona} × ${cell.operation}`, () => {
      it(`should ${cell.outcome}${cell.reason ? ` (${cell.reason})` : ''}`, async () => {
        await seedBimEntity(env, 'floorplan_foundations', DOC_ID, {
          overrides: { kind: 'pad', params: {} },
        });
