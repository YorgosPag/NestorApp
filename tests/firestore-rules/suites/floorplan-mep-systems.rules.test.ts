/**
 * Firestore Rules — `floorplan_mep_systems` collection (ADR-657 / ADR-408)
 *
 * ADR-657 AUTHORING-tier canary for the `'params'`-only create-key variant — an
 * MEP system is a geometry-less grouping node, so its `hasAll()` omits `kind`.
 * MEP authoring data is edited inside /dxf/viewer, so `external_user` is denied
 * ALL FIVE ops.
 *   read   — canReadBimAuthoring(companyId) = isInternalUserOfCompany(companyId)
 *   create — canCreateBimEntity(['companyId','projectId','floorplanId','params'])
...
export const COVERAGE = FIRESTORE_RULES_COVERAGE.find(
  (c) => c.collection === 'floorplan_mep_systems',
)!;

const DOC_ID = 'mepsys-same-tenant';
const PROJECT_ID = 'proj-test';
const FLOORPLAN_ID = 'floorplan-test';

describe('floorplan_mep_systems.rules — AUTHORING tier (ADR-657)', () => {
  ...
  for (const cell of COVERAGE.matrix) {
    describe(`${cell.persona} × ${cell.operation}`, () => {
      it(`should ${cell.outcome}${cell.reason ? ` (${cell.reason})` : ''}`, async () => {
        await seedBimEntity(env, 'floorplan_mep_systems', DOC_ID, {
          overrides: { params: {} },
        });
