/**
 * Firestore Rules — `floorplan_hatches` collection (ADR-657 / ADR-507)
 *
 * ADR-657 AUTHORING-tier canary for the `'data'` create-key variant — flat DXF
 * hatch fills (Revit Filled-Region) carry their geometry under `data`, not
 * `params`. Fills are drafting content edited inside /dxf/viewer, so
 * `external_user` is denied ALL FIVE ops.
 *   read   — canReadBimAuthoring(companyId) = isInternalUserOfCompany(companyId)
 *   create — canCreateBimEntity(['companyId','projectId','floorplanId','data'])
...
export const COVERAGE = FIRESTORE_RULES_COVERAGE.find(
  (c) => c.collection === 'floorplan_hatches',
)!;

const DOC_ID = 'hatch-same-tenant';
const PROJECT_ID = 'proj-test';
const FLOORPLAN_ID = 'floorplan-test';

describe('floorplan_hatches.rules — AUTHORING tier (ADR-657)', () => {
  ...
  for (const cell of COVERAGE.matrix) {
    describe(`${cell.persona} × ${cell.operation}`, () => {
      it(`should ${cell.outcome}${cell.reason ? ` (${cell.reason})` : ''}`, async () => {
        await seedBimEntity(env, 'floorplan_hatches', DOC_ID, {
          overrides: { data: {} },
        });
