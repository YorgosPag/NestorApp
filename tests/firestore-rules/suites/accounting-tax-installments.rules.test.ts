/**
 * Firestore Rules — `accounting_tax_installments` collection
 *
...
import { seedAccountingTaxInstallment } from '../_harness/seed-helpers-accounting';
import { FIRESTORE_RULES_COVERAGE } from '../_registry/coverage-manifest';
import { PERSONA_CLAIMS, SAME_TENANT_COMPANY_ID } from '../_registry/personas';
import type { RulesTestEnvironment } from '@firebase/rules-unit-testing';

export const COVERAGE = FIRESTORE_RULES_COVERAGE.find(
  (c) => c.collection === 'accounting_tax_installments',
)!;

describe('accounting_tax_installments.rules — role_dual', () => {
  ...
  for (const cell of COVERAGE.matrix) {
    describe(`${cell.persona} × ${cell.operation}`, () => {
      it(`should ${cell.outcome}${cell.reason ? ` (${cell.reason})` : ''}`, async () => {
        const docId = 'tax-installment-same-tenant';
        await seedAccountingTaxInstallment(env, docId);
