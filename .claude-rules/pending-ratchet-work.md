...
**STATUS: ACTIVE**
**Last updated:** 2026-05-27 — **ADR-378 Phase 1 DONE (Sonnet, dead code delete)**: `git rm` σε 3 αρχεία — `AISnappingEngine.ts` + `AISnappingEngine.types.ts` + `useProSnapShortcuts.ts`. Zero production references επιβεβαιωμένα με grep (μόνο 1 stale comment σε `snap-engine-utils.ts:36` + 6 doc/analysis files info-only). ai-snapping folder κενό. ΑΝΑΦΟΡΑ_2 Χ1 + ADR-378 §15 changelog updated. Pending commit (Phase 1 deletes + tracking updates) μαζί με Phase 0 bundle. Επόμενο: Phase 2 migrate measure-snap-bridge ghost (~30min Sonnet).
