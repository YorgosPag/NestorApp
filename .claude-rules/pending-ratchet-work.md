...
**STATUS: ACTIVE**
**Last updated:** 2026-05-26 — **ADR-375 Phase C.7 DONE — ADR-375 ALL PHASES COMPLETE**: 3D BIM edge overlay parity. Industry stack `Line2/LineMaterial + LineSegmentsGeometry` (Autodesk Forge/Three.js Editor/Sketchfab). `EdgesGeometry(geo, 30°)` Revit silhouette default. Screen-space `linewidth × devicePixelRatio` (Revit True Line Weight pattern). 4 NEW (resolver wrapper + overlay builder + resolution store + 2 test suites = 38/38 PASS) + 5 MODIFIED (BimToThreeConverter 4 entity types via `attachEdgesProjection` helper, wall-opening-extrude per-segment, StairToThreeConverter 4 components με ADR-377 keys treads/risers/outlines, ThreeJsSceneManager.resize one-liner). Jest infra fix: pnpm-aware transformIgnorePatterns enables Three.js ESM examples. 76/76 2D regression PASS — zero impact. local_ΑΝΑΦΟΡΑ_2.txt AΛ'7 ✅ + ADR-375 v2.5 + adr-index updated. Pending commit (bundle C.6 + C.7 + ADR-376 ribbon residual).
