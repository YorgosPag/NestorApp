...
**STATUS: ACTIVE**
**Last updated:** 2026-05-27 — **ADR-378 Phase 2 DONE (Sonnet, ghost singleton delete)**: `snapping/index.ts` έχασε το re-export γραμμή `export { snapSystem } from './pro-snap-engine'`. `git rm src/subapps/dxf-viewer/snapping/pro-snap-engine.ts`. **Surprise finding**: ZERO production consumers του `snapSystem` global υπήρχαν (ADR-378 §3.3 + §9.2.1 ήταν λάθος — claimed `measure-snap-bridge.ts` consumer αλλά αυτό το αρχείο είναι σε `src/components/shared/files/media/` και άσχετο με snap). Καμία migration χρειάστηκε — απλό delete. Μόνη remaining ref: 1 stale doc line σε `src/subapps/dxf-viewer/docs/features/snapping/ARCHITECTURE.md:23` (Phase 5 doc cleanup scope). ΑΝΑΦΟΡΑ_2 Χ2 + ADR-378 §3.3/§9.2/§15 + memory updated. Pending commit. Επόμενο: Phase 3 TextSnapEngine completion (~2h Sonnet) ή Phase 5 doc cleanup (~15min Sonnet).
