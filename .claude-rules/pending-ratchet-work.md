...
**STATUS: ACTIVE**
**Last updated:** 2026-05-26 — **ADR-376 Phase B.2 DONE**: BOQ signature-group aggregation (Revit Schedule pattern, 6/6 industry convergence). 2 new (opening-boq-grouper.ts + tests 29/29 PASS) + 3 modified (BimToBoqBridge + useOpeningPersistence + ADR-376 §7 B.2 drift fix + §11 v7 + ADR-175 §4.3 cross-ref). Pending: Phase C (draggable tag). Pending ratchet: legacy `boq_bim_<openingId>` rows από Phase A single-entry path (αν υπάρχουν production data) — cleanup TODO παρακάτω. .ssot-registry.json `opening-boq-grouper` entry pending μέχρι να καθαρίσει working tree.
