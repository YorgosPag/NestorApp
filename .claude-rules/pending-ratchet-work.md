...
**STATUS: ACTIVE**
**Last updated:** 2026-05-26 — **ADR-376 Phase C.2 DONE**: per-project Tag Style override. 4 OQs resolved με industry research (Q1 storage=per-project 3/3 Revit/ArchiCAD/AutoCAD, Q2 fields=all 6 fontSizePx/borderWidthPx/leaderStyle/pillBgColor/leaderColor/leaderVisible, Q3 UI=modal Annotate ribbon Tag Style button Palette icon, Q4 preview=debounced 200ms Figma/Photoshop). 4 NEW files (opening-tag-style-service.ts pure singleton + tests + OpeningTagStyleDialog Radix + OpeningTagStyleHost Suspense leaf) + 10 MODIFIED (OpeningTagRenderer overrides accept ResolvedOpeningTagStyle + test mock + annotate-tab-openings button + opening-command-keys action + useRibbonOpeningBridge handler + RibbonButtonIcon Palette case + EventBus event + project type field + DxfViewerContent mount + i18n el/en × 2). 31/31 tests PASS (22 service + 9 renderer regression). ADR-376 §4.11 + §7 C.2 row + §11 v10 changelog + adr-index updated. **Earlier today**: ADR-375 Phase A+B runtime verified + 5 hotfixes. ADR-376 Phase C.1 DONE (γωνιακή leader + Reset Position UX). Phase B.2 BOQ signature-group aggregation DONE. Pending: Phase C.3 (PDF schedule export). Pending ratchet: legacy `boq_bim_<openingId>` rows από Phase A single-entry path (αν υπάρχουν production data) — cleanup TODO παρακάτω. .ssot-registry.json `opening-boq-grouper` + `opening-tag-drag-controller` + `opening-tag-style` entries pending μέχρι να καθαρίσει working tree.
