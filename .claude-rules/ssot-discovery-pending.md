...
- `src/services/file-approval.service.ts` (2x, 272 righe — SSoT block?)
...
**DONE 2026-04-18 (C.5.2)**: `src/lib/layer-sync.ts` migrato via codemod AST-aware `scripts/migrate-toisostring.mjs --dir src/lib --apply` (6×`new Date().toISOString()` → `nowISO()` + import `@/lib/date-local`). File size 444→445 righe. Zero conflitti, tsc clean. Baseline post-commit: **38 violations / 31 files** (drop 115→38 dovuto anche a cleanup altri agent su 10 file 2x: useLevelsFirestoreSync, useFloorOverlays, ChequeTable/DetailDialog, PublicPOPageContent, ContractCard, PurchaseOrderList/Detail, dynamic-styles, interest-cost-helpers).
...
| 2026-04-18 | **Phase C.5.2 DONE.** `src/lib/layer-sync.ts` migrato via codemod AST-aware `scripts/migrate-toisostring.mjs --dir src/lib --apply` (6× `new Date().toISOString()` → `nowISO()` + import SSoT `@/lib/date-local`). Zero conflitti, tsc clean, file 444→445 righe. Baseline ratcheted **115→38 violations / 91→31 files** (-77 viol, -60 files). Drop aggiuntivo dovuto a cleanup paralleli altri agent (10 file 2x): `useLevelsFirestoreSync`, `useFloorOverlays`, `ChequeTable/DetailDialog`, `PublicPOPageContent`, `ContractCard`, `PurchaseOrderList/Detail`, `dynamic-styles`, `interest-cost-helpers`. Top residui: `RelationshipCRUDService.ts` (4x), `file-approval.service.ts` (2x), `format.ts` accounting (2x), `APYCertificateDetails.tsx` (2x), `file-display-name.ts` (2x). |
