...
| v2.0 | 2026-05-15 | OrgStructureTab UX overhaul — department row actions: replaced ghost buttons with dark toolbar (`bg-zinc-900`, white icons); added `Trash2` delete button with `ConfirmDialog` (destructive variant); added i18n keys `orgStructure.dept.deleteConfirmTitle/Desc` (el+en). |
| v1.9 | 2026-05-15 | UI fix — `MemberEditor` `linked` mode: replaced stub plain-text `contactId` input with `ContactSearchManager` (canonical contact picker). On contact select: auto-fills `displayName` (if empty) + syncs primary `work` email to `OrgMember.emails[]`. `Draft` state now includes `emails: OrgEmailInfo[]`. `handleSave` uses `draft.emails` instead of `member?.emails ?? []`. Added i18n key `orgStructure.member.selectContact` (el + en). Added `onCreateNew` → `TabbedAddNewContactDialog` (sheet presentation) + `contactSearchKey` remount pattern for post-creation list refresh. |
