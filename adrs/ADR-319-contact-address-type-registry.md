...
| 2026-04-23 | Giorgio Pagonis | **Unit test coverage**: 87 Jest tests across 3 modules — `address-types.ts` (scope-filtering, primary/secondary, type-guard invariants), `contactAddressLabel.ts` (custom-label rules, i18n passthrough), `address-impact-helpers.ts` (HQ extraction fallback chain, field-by-field change detection). Regression guard for 2026-04-23 public-service taxonomy addition. |
