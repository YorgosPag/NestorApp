...
| 2026-04-23 | Giorgio Pagonis | **Bugfix (GOL + SSOT)**: individual HQ pin disappeared after map drag. `applyDragResolve` assumed `formData.companyAddresses` was populated; for individuals that array is empty, so `updatedAddresses[0]` was `undefined` and `setFormData` zeroed `street`/`city`/`postalCode` — the memo in `ContactAddressMapPreview` then filtered the pin out. Split `applyDragResolve` into two branches: (a) individual — no `companyAddresses`, write HQ flat fields directly from resolved address; (b) multi-address — existing path. Also fixed ADR-277 hierarchy-warning guard in `onDragResolve`: for individuals the hierarchy lives on flat `formData.settlementId`, not on a `CompanyAddress` entry — dialog now triggers correctly for individuals with filled hierarchy. |
