...
| 2026-04-18 | Fixed signed URL bucket mismatch. Root cause: `.env.local` set `FIREBASE_STORAGE_BUCKET=pagonis-87766.firebasestorage.app` (new alias domain), but project bucket is legacy `.appspot.com`. Admin SDK JSON API resolves the alias silently (upload + `exists()` both return OK), but signed URLs embed the alias in the URL host — `storage.googleapis.com/pagonis-87766.firebasestorage.app/...` — which the GCS XML API does NOT resolve, returning `NoSuchKey`. Fix: revert `FIREBASE_STORAGE_BUCKET` and `NEXT_PUBLIC_FIREBASE_STORAGE_BUCKET` in `.env.local` to `pagonis-87766.appspot.com` (matches `.env`). Same change must be applied on Vercel env vars before the next deploy. Added `Post-save diagnostics` log in `uploadPdfToStorage` (prints `bucket.name`, `cloudStorageURI.href`, `publicUrl()`) for future bucket debugging. |
