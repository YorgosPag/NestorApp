...
| `src/hooks/inbox/useRealtimeTriageCommunications.ts` | `useState([])` + `useState(true)` unconditional; reset to `[]` on companyId change | `inboxTriageCache = createStaleCache<Communication[]>('inbox-triage')` keyed by companyId; reset seeds from cache instead of `[]` | 2026-04-16 |
| `src/components/building-management/tabs/TimelineTabContent/dashboard/useScheduleDashboard.ts` | `boqLoading: useState(true)` unconditional; ganttLoading from useConstructionGantt (already migrated) | `scheduleDashboardBoqCache = createStaleCache<BOQItem[]>('building-schedule-dashboard')` keyed by buildingId | 2026-04-16 |
| `src/components/shared/files/ArchiveView.tsx` | `useState([])` + `useState(true)` unconditional | `archiveViewCache = createStaleCache<FileRecord[]>('file-archive')` keyed by `${companyId}-${entityType}-${entityId}` | 2026-04-16 |
| `src/components/shared/files/TrashView.tsx` | `useState([])` + `useState(true)` unconditional | `trashViewCache = createStaleCache<FileRecord[]>('file-trash')` keyed by `${companyId}-${entityType}-${entityId}` | 2026-04-16 |
| `src/components/shared/files/DocumentTemplatePanel.tsx` | `useState([])` + `useState(true)` unconditional | `documentTemplatesCache = createStaleCache<DocumentTemplate[]>('file-document-templates')` keyed by companyId | 2026-04-16 |
| `src/components/admin/role-management/components/AuditTab.tsx` | `useState([])` + `isLoading: useState(true)` unconditional | `adminAuditLogCache = createStaleCache<FrontendAuditEntry[]>('admin-audit-log')` single-key; load-more path unaffected | 2026-04-16 |
| `src/components/admin/role-management/components/ProjectMembersTab.tsx` | `useState([])` + `isLoadingProjects: useState(true)` unconditional | `adminProjectsListCache = createStaleCache<ProjectSummary[]>('admin-project-members')` single-key | 2026-04-16 |
| `src/components/procurement/BOQRelatedPOs.tsx` | `useState([])` + `useState(true)` unconditional | `boqRelatedPOsCache = createStaleCache<LinkedPOInfo[]>('procurement-boq-pos')` keyed by `${projectId}-${boqItemId}` | 2026-04-16 |
| `src/components/crm/dashboard/TasksTab.tsx` | `leads: useState([])` + `leadsLoading: useState(true)` unconditional (tasks come from useRealtimeTasks, already migrated) | `crmDashboardLeadsCache = createStaleCache<Opportunity[]>('crm-dashboard-tasks')` single-key | 2026-04-16 |
