...
| 2026-04-19 | 2.0 | Claude | **Fase 6 BUG FIX — sceneFileId mai persistito dopo wizard import** — Root cause: `StepUpload.onComplete(file)` non passava `fileRecord.id` → catena rotta → `linkSceneToLevel` mai chiamato → `level.sceneFileId = null` → refresh = empty scene. Fix: (1) `DxfSaveContext.fileRecordId?: string` aggiunto; (2) `StepUpload.onComplete(file, fileId?)` passa `result.fileRecord?.id`; (3) `WizardCompleteMeta.fileId?: string` aggiunto; (4) `LevelPanel` include `fileRecordId: meta.fileId` in `saveContext`; (5) `useSceneState.handleFileImport` risolve `fileRecordId` anche da `saveContext.fileRecordId`. Dopo fix: `useAutoSaveSceneManager` wizard-path (linea 104) trova `wizardFileId` → chiama `onSceneSavedRef` → `linkSceneToLevel` aggiorna Firestore `sceneFileId` → `useLevelSceneLoader` auto-reload su refresh. |
