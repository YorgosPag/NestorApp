...
| 2026-04-16 | 1.5 | Claude | **Fase 5 IMPLEMENTATA** — "Φόρτωση από αποθήκη": nuovo `StoredFloorplanPicker.tsx` (Dialog + `useFloorplanFiles` + `DxfFirestoreService.loadFromStorage`). LevelPanel: +`setLevelScene` destructure, +`showStoragePicker` state, +bottone condizionale (visibile quando level ha entityId). Entity mapping: `floorplanType→{entityId,entityType}` internamente nel picker. i18n: 6 nuove chiavi `panels.levels.loadFromStorage` + `storagePicker.*` (el+en). LevelPanel rimane 498 linee (≤500). |
| 2026-04-16 | 1.6 | Claude | **Fase 5 REFACTOR (Google+SSoT)** — `StoredFloorplanPicker.tsx` eliminato. `FloorplanImportWizard` esteso con `mode='load'\|'import'` + `onLoad` callback. Nuovo `StepStoragePicker.tsx` come Step 6 in load mode (SSoT: stesso wizard, stesso flusso entità). `STEP_SHORTCUT_CONFIG` aggiornato con `importLabelKey`/`loadLabelKey` per labels contestuali. `stepLabels` dinamici per step 6. LevelPanel: `showStoragePicker` → `showLoadWizard`, `onLoad` inline con `DxfFirestoreService.loadFromStorage`. i18n: 9 nuove chiavi `loadTitle`, `steps.select`, `shortcuts.load*`, `storagePicker.*` in `files-media.json` (el+en). LevelPanel 477 linee. |
...
| 2026-04-19 | 2.0 | Claude | **Fase 6 BUG FIX (parte 1) — fileRecordId chain** — `StepUpload.onComplete` non passava `fileRecord.id`. Fix catena: `DxfSaveContext.fileRecordId?`, `WizardCompleteMeta.fileId?`, `LevelPanel saveContext.fileRecordId`, `useSceneState` resolve da saveContext. |
| 2026-04-19 | 2.1 | Claude | **Fase 6 BUG FIX (parte 2) — stale React closure** — `setLevelSceneWithAutoSave` catturava `currentFileName = null`. Fix: `currentFileNameRef` sync. |
| 2026-04-19 | 2.2 | Claude | **Fase 6 BUG FIX (parte 3) — 2 bug rimanenti** — (A) wizard guard usava `purpose` come flag: `purpose: meta.purpose \|\| undefined` = `undefined` quando empty string → auto-save path entrava senza `canonicalScenePath` → throw → `onSceneSavedRef` mai chiamato. Fix: guard cambiato in `injectedFileRecordIdRef.current` (sempre set dal wizard). (B) `linkSceneToLevel` usava `updateDoc` client-side su `dxf_viewer_levels` ma le regole Firestore NON hanno `allow update` (commento esplicito: "Updates go through Admin SDK"). Fix: `linkSceneToLevel` ora usa `fetch('/api/dxf-levels', PATCH)`. Importato `doc`/`updateDoc` rimosso. |
