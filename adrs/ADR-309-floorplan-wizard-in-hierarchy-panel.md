...
| 2026-04-19 | 2.1 | Claude | **Fase 6 BUG FIX (parte 2) — stale React closure** — Root cause reale: `setLevelSceneWithAutoSave` catturava `currentFileName = null` nella closure React prima che lo state update propagasse. Fix: aggiunto `currentFileNameRef` che si aggiorna sync in `setCurrentFileName`. `setLevelSceneWithAutoSave` legge `currentFileNameRef.current` invece di `currentFileName` state, rimosso `currentFileName` dai deps del `useCallback`. `setFileRecordId` usa anche ref. Risultato: `onSceneSavedRef` → `linkSceneToLevel` → `sceneFileId` in Firestore → `useLevelSceneLoader` auto-reload su F5. |
