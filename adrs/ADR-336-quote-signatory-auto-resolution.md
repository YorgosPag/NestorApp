...
| 2026-05-05 | Profession SSoT alignment + role-on-master cleanup (industry-standard parity with SAP S/4HANA Business Partner + Procore Directory). <br><br>**Changes:** <br>• `signatory/types.ts` — `SignatoryFields` extended with `escoUri`, `escoLabel`, `iscoCode` (nullable). <br>• `SignatoryProposalCard.tsx` — profession field replaced with new `ProfessionEscoField` wrapper (preserves confidence-band border + ConfidencePill, delegates input to canonical `EscoOccupationPicker`, ADR-034). Free-text fallback preserved by ESCO picker. <br>• `commit-signatory/route.ts` — Zod `SignatoryFieldsSchema` extended with the 3 ESCO fields (escoUri max 300, escoLabel max 200, iscoCode max 10). <br>• `commit-signatory-service.ts` — `CommitSignatoryFields` extended; `createIndividualContact` now writes `escoUri` + `escoLabel` + `iscoCode` on the new `IndividualContact` master, and **no longer writes `position` to the master**. <br><br>**Architectural rationale (industry-standard convergence):** <br>SAP BP models job/function as an attribute of the BP **Relationship**, not the BP master. Procore models the job title as a free-text field on the Company↔Person relationship row in the Directory, not on the Person record. Both decouple identity (master) from organizational role (edge). The previous V1 implementation duplicated the `role` onto `IndividualContact.position`, conflating master and edge. The fix: role lives **only** on `ContactRelationship.position` (already the case for the relationship side, line 195). Existing contacts created before this change retain their `position` value (no migration needed — field is optional). <br><br>**Profession SSoT:** consolidated on `IndividualContact.profession` + `escoUri/escoLabel/iscoCode` (ADR-034 ESCO occupation taxonomy). No registry exists for "ρόλος/ιδιότητα" free text — by design, mirrors Procore's free-text Job Title pattern. |
