...
| **SSoT structural duplicates** (CHECK 3.18) | ADR-314 / ADR-294 | `.ssot-discover-baseline.json` | **46 / 5 / 91** (2026-04-19) | Ratchet down only — tracks duplicateExports / antiPatterns / unprotected. Layer 1 = pre-commit smoke, Layer 2 = CI full scan (`.github/workflows/ssot-discover.yml`). |
...
| **B.6** | `obligations`, `obligation_transmittals`, `obligation_templates` | `tenant_direct` + `crmDirectMatrix` | ✅ done (2026-04-13) |
...
| 2026-04-19 | **CHECK 3.18 — SSoT Discover Ratchet added (ADR-314)**. New baseline `.ssot-discover-baseline.json` (46 duplicates / 5 anti-patterns / 91 unprotected, frozen post Phase C.5.21). Defense in Depth: Layer 1 = pre-commit smoke (`scripts/check-ssot-discover-ratchet.js` smoke mode, ~0.2s), Layer 2 = `.github/workflows/ssot-discover.yml` full scan (Ubuntu, ~1-2 min). Wraps `scripts/ssot-discover.sh` with baseline comparator. Closes presubmit gap for new duplicate symbols + scattered anti-patterns that escape CHECK 3.7 (registered-modules only). Full scan not in pre-commit because scanner takes ~4 min on Windows Git Bash — opt-in via `SSOT_DISCOVER_FULL=1 git commit …`. §2.2 row added. ADR-294 changelog + CLAUDE.md N.12 updated in same commit. |
| 2026-04-13 | **ADR-298 Phase B.6 COMPLETED** — obligations, obligation_transmittals, obligation_templates (3 collections). crmDirectMatrix() for all three — no overrideCells needed. New seed-helpers-compliance.ts module (SRP). 465 tests green (23 suites). Pending list 74→71. Coverage 20→23. |
