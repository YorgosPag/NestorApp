...
| 2026-04-17 | bugfix | 3 bug fix: (1) ctx.userId→ctx.uid in full/incremental/config routes (AuthContext ha `uid`, non `userId` — Firestore reject su undefined), (2) BackupGcsService.ensureBucketExists() auto-crea bucket se non esiste (location EUROPE-WEST1), (3) ctx.userId→ctx.uid anche in config route |
| 2026-04-17 | SSoT | GCS bucket config centralizzato: `src/config/gcs-buckets.ts` (GCP_PROJECT_ID, GCS_BACKUP_BUCKET, GCS_BACKUP_BUCKET_CONFIG). Eliminato `pagonis-87766` hardcoded da 4 file backup. Modulo registrato in `.ssot-registry.json` |
