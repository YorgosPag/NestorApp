...
- **[🔒 Mandate Conflict — η εντολή ως κατάληψη πόρου (ADR-832)](reference/adrs/ADR-832-mandate-as-resource-occupancy.md)** — `src/lib/mandate/mandate-conflict.ts`: ο **ΕΝΑΣ** κριτής για «συγκρούονται αυτές οι δύο εντολές;». Ο κανόνας είναι `ίδιος πόρος ∧ επικάλυψη χρόνου ∧ ασύμβατοι τρόποι` — κατά λέξη το `EXCLUDE USING GIST` της PostgreSQL, με **τρίτο** σκέλος τον πίνακα shared/exclusive (`lockModeFor` · `modesCompatible`, `types/listing-agreement.ts`) που η PostgreSQL δεν χρειάζεται επειδή τα κλειδώματά της είναι πάντα αποκλειστικά. 🔑 **Ο πόρος είναι `(ακίνητο × πράξη)`**, όχι το ακίνητο: αποκλειστική **πώλησης** δεν εμποδίζει εντολή **εκμίσθωσης** (άρθρο 200 §4 Ν.4072/2012 · RESO `Right To Sell` ≠ `…To Lease`). Τρέχει σε **τέσσερα** σημεία — υποβολή αιτήματος · αποδοχή **μέσα σε συναλλαγή** · `setOwnerPropertyMandate` · και η **οθόνη**, μέσω `mandate-occupancy-notice.ts`, που δείχνει *«ο Χ κρατά αποκλειστική πώλησης ως 12/03 — ελεύθερο από τότε»* **πριν** πατήσει ο άνθρωπος. ⚠️ **ΠΟΤΕ χειρόγραφο «έχει ήδη εντολή;»** (`mandate.kind !== 'self'`, `mandates.length > 0`): ήταν το ελάττωμα — έκρινε **τρία στα τέσσερα** σενάρια λάθος. ⚠️ **`bindingMandates`, ποτέ `activeMandates`**, στους καλούντες: ο χρόνος ανήκει στον κριτή, και το χρονικό φίλτρο **έκρυβε** τις χαλασμένες εντολές («άγνωστο» → «καθαρό»). ⛔ Το `mandatesExpireAt` είναι **ευρετήριο**, ποτέ κριτής.
