...
| **Φ5** | **Ο σοβάς της ΠΛΑΚΑΣ** ως finish-member (soffit `down` + ενιαίο top-cap `up`, associative, id-based self-exclusion) | 🟡 **Φ5a+Φ5b IMPLEMENTED** (UNCOMMITTED) · Φ5c (perimeter/BOQ) DEFER |
...
- **2026-07-17 (Φ5 — Ο ΣΟΒΑΣ ΤΗΣ ΠΛΑΚΑΣ, Φ5a τύποι + Φ5b soffit/top· UNCOMMITTED)** — Giorgio (C4D
  ...
  - **DEFER Φ5c (ΑΝΕΠΑΛΗΘΕΥΤΟ, χρειάζεται C4D Giorgio):** περιμετρικός σοβάς πλάκας (silhouette member,
    ADR-449 Φ4)· BOQ πλάκας (soffit+top κυρίαρχα → ΟΧΙ αντιγραφή column/beam)· «δομικό μέλος που ΠΑΤΑΕΙ
    ΠΑΝΩ στην πλάκα σβήνει το top-plaster κάτω από τη βάση του» (χρειάζεται member-base ως cover στο cap).
