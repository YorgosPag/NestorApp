...
- **2026-07-18 (Φ7c — ΓΝΗΣΙΟ 45° MITER ΕΝΣΩΜΑΤΩΜΕΝΟ ΣΤΟ ΕΝΙΑΙΟ WELDED MESH: αντικαθιστά τα Φ7b wedges)** —
  Giorgio (3Δ/C4D): «τα miter wedges που μπήκαν στις γωνίες φαίνονται σαν artifacts στην όψη». **Root cause:**
  το Φ7b wedge είναι **ξεχωριστό τριγωνικό prism** κολλημένο στη γωνία → η μία πλευρά του συμπίπτει (coincident,
  αντίθετα normals) με το square end-cap του welded body στο core-length → z-fighting/ορατό facet. Εγγενές του
  «πρόσθεσε ξεχωριστή γεωμετρία» — δεν διορθώνεται με tweak· το wedge πρέπει να **φύγει** και το miter να μπει
  **ΜΕΣΑ** στο ίδιο extrude. **Απόφαση Giorgio (ρητή): γνήσιο 45° miter σωστά υλοποιημένο, ΟΧΙ single-owner.**
  **FIX (vertex-shift του back-cap):** το `ExtrudeGeometry` κάθε όψης έχει front cap (u=0 = core) και back cap
  (u=thicknessM = outer). NEW `computeFaceMiterShifts` (pure SSoT, `structural-finish-face-profile.ts`): σαρώνει
  **ΚΑΘΕ strip-άκρο** (ΟΧΙ μόνο τα 2 group-ακρότατα) και σε κάθε **convex κάθετη γωνία** παράγει scalar shift
  `{tM, deltaM}` (m), όπου `deltaM = t_outer − t_core` (αντλεί την **κοινή** mitered κορυφή αυτούσια από `aOuter/
  bOuter`)· gate = strip-level convex (outer ΠΕΡΑ από το t-span του strip) + `isPerpCorner` cross-group (collinear/
  junction/free/concave → κανένα shift). 3Δ: NEW `applyMiterShift` (`structural-finish-3d.ts`) μετατοπίζει τα back-cap
  vertices σε κάθε shift `tM` κατά `deltaM` → το outer φτάνει τη mitered κορυφή ενώ το core μένει → **45° miter ΜΕΣΑ
  στο welded mesh** (δύο γειτονικές όψεις μοιράζονται ΑΚΡΙΒΩΣ την ίδια κορυφή).
  **OPENINGS FIX (Giorgio: «στα χείλη ανοιγμάτων ο σοβάς δεν κλείνει»):** το **strip-level** scan (mirror του πρώην
  `collectMiterWedges`) είναι το κλειδί. Στο χείλος ανοίγματος η **πρόσοψη** έχει το παράθυρο ως **τρύπα** (το χείλος
  = εσωτερικό σύνορο, ΟΧΙ group-ακρότατο) και η **λαμπάδα** (κάθετη όψη, full wall depth από `splitFootprintByOpening
  Bands`) τελειώνει εκεί. Το πρώτο draft (group-extreme μόνο) έχανε ΚΑΙ ΤΑ ΔΥΟ → η γωνία έμενε ανοιχτή (μόνο εκεί που
  υπάρχουν ανοίγματα). Το strip-level: (α) η πρόσοψη mitered-άρει το **χείλος της τρύπας** (τα hole-corner back-cap
  vertices κάθονται στο `tM` του χείλους), (β) η λαμπάδα mitered-άρει το **facade-end** της· ο perp-gate ψάχνει πλέον
  **ΟΛΑ** τα strip-άκρα (`allFaceEnds`, ΟΧΙ μόνο ακρότατα) ώστε πρόσοψη↔λαμπάδα να «δουν» η μία την άλλη. Οι δύο μισές
  τρίγωνες γεμίζουν **συμπληρωματικά** (μηδέν overlap), όπως στη γωνία κτιρίου. **PERP FIX (root, απαραίτητο):** το `outwardPerpOf` (`structural-finish-vertical-merge.ts`)
  έβγαζε **διαγώνιο** perp από `mid-outer − mid-core` σε ασύμμετρη όψη (ένα άκρο miter-out, άλλο chamfer-in) →
  skewed extrude (το πάχος εξωθούνταν λοξά)· τώρα = **καθαρό axis-normal** `(-dir.y, dir.x)` με πρόσημο core→outer.
  **OPENINGS FIX (Giorgio: «προβλήματα μόνο στα ανοίγματα»):** το αρχικό draft έκανε `computeVertexNormals` μετά
  το shift → ξανάγραφε ΟΛΑ τα normals, **και των reveal walls των τρυπών** → ανεστραμμένη σκίαση στα ανοίγματα.
  Αφαιρέθηκε: το ορατό back-cap μένει επίπεδο στο u=thicknessM (μόνο x κινείται) → κρατά το σωστό `+perp` normal·
  το μόνο stale normal είναι η κρυμμένη 25mm διαγώνια miter-έδρα. **ΑΦΑΙΡΕΘΗΚΕ το wedge machinery:** `MiterWedge`,
  `collectMiterWedges`, `endWedge`, `miterWedgeGeometry` + το wedge loop. **ΚΡΑΤΗΘΗΚΑΝ:** `mergeNonIndexed`,
  `finishMaterialKey`, merge-ανά-υλικό (Giorgio: «ένα ενιαίο σύνολο»), το hup fix, `computeMiteredOuter` SSoT.
  **Files:** MOD `structural-finish-face-profile.ts` (−wedges, +`computeFaceMiterShifts`/`FaceMiterShift`/`FaceProfile.miter[]`/
  `allFaceEnds`/`endShift`), MOD `structural-finish-3d.ts` (−`miterWedgeGeometry`/wedge loop, +`applyMiterShift` λίστα),
  MOD `structural-finish-vertical-merge.ts` (`outwardPerpOf` → axis-normal). **Tests (diagnostic-first):** rewritten
  `structural-finish-corner-overlap-diag.test.ts` (γωνία κτιρίου: outer corner → κοινή κορυφή (125,-25)· core (100,0)·
  καθαρό perp· free-end→0 · **ΧΕΙΛΟΣ ΑΝΟΙΓΜΑΤΟΣ: ΚΑΙ πρόσοψη ΚΑΙ λαμπάδα → κοινή κορυφή (1025,-25)**)·
  `face-profile.test.ts` CORNER → 1 shift ±25mm· geometry-level `silhouette-3d.test.ts` «L-γωνία → miter tip ΜΕΣΑ στο
  ενιαίο mesh, μηδέν wedge child». **266/266 finishes GREEN**, jscpd καθαρό, <500γρ,
  tsc SKIP (N.17). **ΕΚΤΟΣ ADR-040** (pure finish geometry + converter). **✅ Google-level: YES** — γνήσιος αρμός
  αντί hack, reuse `computeMiteredOuter` (μηδέν νέο miter math), root-cause perp/openings fixes, diagnostic-first.
  🔴 C4D-verify (Giorgio, φρέσκο re-export) + commit. | Opus
- **2026-07-18 (Φ7b — TRUE 45° MITER ΣΤΙΣ ΓΩΝΙΕΣ: αντικαθιστά το overlap corner-join του Φ7)** — Giorgio
  ...
  **ΕΚΤΟΣ ADR-040** (pure finish geometry + converter· 2Δ outline αναλλοίωτο — ήδη mitered). **✅ Google-level:
  ...
  regression risk, diagnostic-first. 🔴 C4D-verify (Giorgio) + commit.
  **hup FIX (ίδιο session — Giorgio C4D: `Wall_structural-finish-hup-` παρέμενε ΜΕΤΑ από φρέσκο export):**
  **DIAGNOSTIC (measure, jest mirror):** το merged top-cap σε τοίχο 4m/πάχος 15mm κάτω από πλάκα με
  outline = δομικός πυρήνας → έμενε **0.1272 m²** = ακριβώς περιμετρικό δαχτυλίδι πάχους 15mm σε ΚΑΘΕ
  τοίχο/κολόνα. **Root cause:** το cap χτίζεται στο **finished** (διεσταλμένο κατά το πάχος)
  `mergeCoresToFinishedRings` outline, αλλά ο cover (πλάκα/δοκάρι) έχει outline = δομικός **πυρήνας** →
  η `safeDifference` άφηνε το plaster frame ΕΞΩ από τον πυρήνα (= το hup· η slab-removal του Φ7 δεν το
  άγγιζε γιατί το frame προέρχεται από τους ΤΟΙΧΟΥΣ/ΚΟΛΟΝΕΣ, όχι τις πλάκες). **FIX:** NEW `dilatedCover`
  (SSoT στα `-horizontal-obstacles.ts`) — οι covers του merged cap διαστέλλονται έξω κατά το ίδιο πάχος
  (`spec.thickness·s`) → καταπίνουν το frame → **0 cap όπου πλάκα από πάνω**· εκτεθειμένο parapet (κανένας
  cover) → πλήρες cap αμετάβλητο (regression test). MOD `structural-finish-scene-horizontal.ts`
  (dilate covers στο `computeMergedStructuralTopCap`), `structural-finish-horizontal-obstacles.ts`
  (+`dilatedCover`). NEW `structural-finish-merged-cap-hup.test.ts` (slab=core→0· parapet→>0). **318/318
  finishes GREEN**, jscpd καθαρό, <500γρ. **✅ Google-level: YES** — diagnostic-first (measured 0.1272→0),
  reuse `dilatePolygonOutward` SSoT, targeted (μόνο merged cap subtraction), μηδέν regression σε parapet.
  **CONTINUITY FIX (ίδιο session — Giorgio C4D 234109: `Column_structural-finish-_87…_97` «ΚΑΙ ΠΟΛΛΑ
  ΑΛΛΑ ΚΑΘΕΤΑ ΚΟΜΜΑΤΙΑ» διακόπτουν το συνεχές δέρμα):** **DIAGNOSIS (naming-decode, ADR-668
  `buildMeshName` + `mesh3d-prepare` dedup):** το κάθετο silhouette skin ήταν ΕΝΑ group με **N child
  meshes** (ένα welded body ανά ομοεπίπεδη όψη Φ7 + ένα ανά miter wedge Φ7b). Το OBJ είναι **επίπεδο**
  format (`o <name>` ανά mesh) → κάθε child = ξεχωριστό object `Column_structural-finish-`, `_2`, …`_97`
  (dedup counter, όλα ίδιο bimId). Δηλαδή ΕΝΑ λογικό δέρμα σπασμένο σε δεκάδες objects. **Απόφαση
  Giorgio: «ένα συνεχές δέρμα, merge ανά υλικό».** **FIX:** το `buildFinishSkinFromStripGroups` μαζεύει
  ΟΛΕΣ τις γεωμετρίες (όψεις + wedges) σε **buckets ανά υλικό** (`materialId|colorOverride|classification`)
  και κάνει `mergeNonIndexed` (συνένωση non-indexed ExtrudeGeometry buffers position/normal/uv) → **ΕΝΑ
  mesh ανά υλικό** (τυπικά 1-2 objects) αντί για N. Τα wedges (πρώην ξεχωριστά `addMiterWedge` meshes)
  γίνονται πλέον `miterWedgeGeometry` και μπαίνουν στο ΙΔΙΟ merged bucket. MOD `structural-finish-3d.ts`
  (−`addMiterWedge`, +`miterWedgeGeometry`/`mergeNonIndexed`/`finishMaterialKey`, rewrite build). Tests:
  silhouette-3d «2 όψεις ίδιου υλικού → 1 merged mesh» + «2 υλικά → 2 meshes». **319/319 GREEN**, jscpd
  καθαρό, <500γρ. **✅ Google-level: YES** — big-player ενιαίο δέρμα, non-pickable derived (μηδέν επίπτωση
  σε picking/BOQ), diagnostic-first (naming-decode). 🔴 C4D-verify (Giorgio) + commit. | Opus
