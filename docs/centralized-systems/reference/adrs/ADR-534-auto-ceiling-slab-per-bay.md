...
- **2026-07-18 (Φ7c — ΓΝΗΣΙΟ 45° MITER ΕΝΣΩΜΑΤΩΜΕΝΟ ΣΤΟ ΕΝΙΑΙΟ WELDED MESH: αντικαθιστά τα Φ7b wedges)** —
  ...
  **FIX (vertex-shift του back-cap):** το `ExtrudeGeometry` κάθε όψης έχει front cap (u=0 = core) και back cap
  (u=thicknessM = outer). NEW `computeFaceMiterDeltas` (pure SSoT, `structural-finish-face-profile.ts`): ανά group
  υπολογίζει scalar `deltaLo/deltaHi` (m) στα δύο t-ακρότατα = `t_outer − t_core` (αντλεί την **κοινή** mitered
  κορυφή αυτούσια από `aOuter/bOuter`), μη-μηδενικό ΜΟΝΟ σε **convex** (outer ΠΕΡΑ από core t-span) **κάθετη**
  γωνία (`isPerpCorner` cross-group gate — window jambs/collinear/junction/free/concave → 0). 3Δ: NEW `applyMiterShift`
  (`structural-finish-3d.ts`) μετατοπίζει ΜΟΝΟ τα back-cap vertices στα t-άκρα κατά τα deltas → το outer t-άκρο
  φτάνει τη mitered κορυφή ενώ το core μένει → η πλευρική έδρα γίνεται διαγώνια = **45° miter ΜΕΣΑ στο welded
  mesh** (δύο γειτονικές όψεις μοιράζονται ΑΚΡΙΒΩΣ την ίδια mitered κορυφή· ΟΙ τρύπες/ανοίγματα δεν είναι στα
  t-άκρα → ανέγγιχτα). **PERP FIX (root, απαραίτητο):** το `outwardPerpOf` (`structural-finish-vertical-merge.ts`)
  ...
  **Files:** MOD `structural-finish-face-profile.ts` (−wedges, +`computeFaceMiterDeltas`/`FaceMiterDeltas`/`FaceProfile.miter`),
  MOD `structural-finish-3d.ts` (−`miterWedgeGeometry`/wedge loop, +`applyMiterShift`), MOD `structural-finish-vertical-merge.ts`
  (`outwardPerpOf` → axis-normal). **Tests (diagnostic-first):** rewritten `structural-finish-corner-overlap-diag.test.ts`
  (back-cap outer corner → κοινή mitered κορυφή (125,-25)· core μένει (100,0)· deltas· καθαρό perp· free-end→0)·
  `structural-finish-face-profile.test.ts` CORNER → back-cap delta ±25mm· NEW geometry-level `structural-finish-silhouette-3d.test.ts`
  «L-γωνία → miter tip ΜΕΣΑ στο ενιαίο mesh, μηδέν wedge child». **263/263 finishes GREEN**, jscpd καθαρό, <500γρ,
