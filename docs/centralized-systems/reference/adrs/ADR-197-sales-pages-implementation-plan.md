...
- **2026-07-16 (ADR-584 / N.18 — sales page-content SSoT, ΤΕΤΡΑΔΑ)**: Το jscpd σήμανε 105 διπλές γραμμές στο ζεύγος `SalesAvailablePropertiesPageContent` ↔ `SalesSoldPageContent`, αλλά ο έλεγχος αδελφών αποκάλυψε **τετράδα**: το ίδιο μοτίβο κουβαλούσαν και τα `SalesAvailableParkingPageContent` + `SalesAvailableStoragePageContent` (ασήμαντα από το jscpd επειδή είχαν διαφορετικά ονόματα πεδίων — `filteredItems`/`handleSelectItem` αντί για `filteredUnits`/`handleSelectProperty`). Το `SalesHubPageContent` ελέγχθηκε και **εξαιρέθηκε**: είναι dashboard, όχι σελίδα-λίστα. Κεντρικοποίηση **spec/slot-driven** (σχήμα `band-stack-preview-renderer.ts`, ADR-412/414) — ΟΧΙ god-component με `if (kind === 'sold')`:
  - **NEW** `src/components/sales/shared/sales-list-page-shell.tsx` — ο σκελετός: loading gate, header, dashboard, responsive φίλτρα, διακόπτης λίστας/πλέγματος. Κατέχει και τον όρο αναζήτησης (ήταν `useState`+`useEffect` αντιγραμμένο 4×)· ο callback ζει σε ref ώστε τα inline arrows των σελίδων να μην ξαναρίχνουν το effect σε κάθε render. Το chrome contract ορίζεται πάνω στο υπάρχον `ListGridHeaderProps` (`@/core/headers`) — ΟΧΙ τρίτη διάλεκτο. `renderList`/`renderGrid` είναι συναρτήσεις: χτίζεται μόνο η ενεργή προβολή.
  - **NEW** `use-sales-properties-list-page.ts` — «διαθέσιμα» και «πωλημένα» τρέχουν στον ΙΔΙΟ hook (`viewScope`), άρα μοιράζονται controller: αντιστοίχιση φίλτρων + props του `SalesSidebar` (ήταν 33+18 διπλές γραμμές).
  - **NEW** `sales-space-page.ts` — κοινά parking/storage: `salesSpaceSidebarProps()` (τα δύο sidebars έχουν ήδη ταυτόσημο contract πάνω στο `SalesSpaceSidebar`), `salesSpaceCardPricing()` (εμπορική τιμή κατισχύει· τιμή/τ.μ. μόνο με θετικό εμβαδόν), `mapCommonSpaceFilters()`.
  - **NEW** `SalesCardGrid.tsx` (καμβάς πλέγματος + empty state), `sales-stat-values.ts` (`salesMoneyValue`/`salesPerSqmValue` — ο κανόνας «μηδέν = άγνωστο, όχι €0» ήταν inline 12×· **6 tests**), `shared/index.ts` (barrel· ADR-294-aware — `SalesSpaceSidebar` και ο properties controller μένουν ΕΚΤΟΣ για να μη μπουν στο chunk κάθε σελίδας).
  - **NEW** `src/components/core/AdvancedFilters/ResponsiveFiltersPanel.tsx` — το desktop/mobile split των φίλτρων (SSoT). Boy-scout (N.0.2): αντικατέστησε και το ταυτόσημο `properties/page/PropertyPageFilters.tsx`, το οποίο **διαγράφηκε** (`UnitsPageContent` δείχνει πλέον στο SSoT).
  - **Ενοποίηση προβολής πλέγματος**: η σελίδα διαθέσιμων ακινήτων τύλιγε το grid σε `div.w-full.p-2.overflow-y-auto.flex-1 > section.grid`, ενώ οι άλλες τρεις σε `section.flex-1.grid.p-2.overflow-y-auto`. Ήταν τυχαία απόκλιση, όχι σχεδίαση — όλες περνούν στο `SalesCardGrid`. **Θέλει browser-verify.**
  - ADR-294: τα named exports και των 4 σελίδων παραμένουν άθικτα (`lazyRoutesAdr294.tsx` επαληθεύτηκε).
  - jscpd `--diff` στα 13 αρχεία: **καθαρό** (από 6 clones σε 0 μέσα στους γύρους της αλλαγής).
