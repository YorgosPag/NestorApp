...
| Ημ/νία | Αλλαγή |
|---|---|
| 2026-07-11 | **✨ 2D PLACEMENT GHOST (Giorgio: «να εμφανίζεται το φάντασμα στον κέρσορα όπως όλες οι οντότητες»).** Το εργαλείο «2D Σύμβολα» απέκτησε WYSIWYG placement ghost — μόλις ενεργοποιείται, το επιλεγμένο σύμβολο ζωγραφίζεται ημιδιάφανο στον (snapped) κέρσορα (ghost ≡ commit by identity). Full SSoT via ADR-624: νέο `useFloorplanSymbolGhostPreview` = 3-line binding του `createBridgeStorePlacementGhostHook` (ίδιο factory με fixture/panel/6 MEP· μηδέν copy-paste draw skeleton) + micro-leaf `FloorplanSymbolGhostPreviewMount` (ADR-040, self-subscribes cursor· CanvasSection δεν re-render-άρει σε mousemove). Threading mirror του mep-fixture: `canvas-layer-stack-types` + `-preview-mounts` + `CanvasLayerStack` + `CanvasSection` payload (`isAwaitingPosition`+`getGhostFootprint` από `useFloorplanSymbolTool`). Bridge correctness: το `floorplanSymbolToolBridgeStore` δημοσιεύει πλέον τα EFFECTIVE overrides (picked `assetId` merged, ίδιο με `resolveCommitOverrides`) + `getSceneUnits`, ώστε ο ghost να αντανακλά το ΤΡΕΧΟΝ επιλεγμένο σύμβολο στο σωστό μέγεθος (όχι πάντα το default). 8 αρχεία (2 νέα), 1 domain. Smoke tests PASS (18), jscpd καθαρό. Opus 4.8. |
| 2026-07-11 | **🐛 FIX — τα σύμβολα ΑΠΟΘΗΚΕΥΟΝΤΑΝ αλλά ΔΕΝ εμφανίζονταν στον 2D καμβά (missing renderer registration).** Root cause (πλήρες pipeline trace: placement→persistence→Firestore→subscription→merge→model-convert→render): ΟΛΑ σωστά ΕΚΤΟΣ από το τελευταίο στάδιο — ο `FloorplanSymbolRenderer` **ΔΕΝ ήταν registered** στο `EntityRendererComposite.renderers` Map (ούτε στα 3 declarative SSoT μητρώα ADR-550). Στο `DxfRenderer.render` → `entityComposite.render()` → `getRenderer('floorplan-symbol')` = `undefined` → `renderFallback` (τίποτα) → **αόρατα**. Το Φ1 changelog παρακάτω ΙΣΧΥΡΙΖΟΤΑΝ «Registration: EntityRendererComposite» αλλά ο κώδικας ΔΕΝ το είχε (N.0.1: code = source of truth, ADR out-of-date). Fix (4 αρχεία, additive, full SSoT): import+instantiate+`renderers.set('floorplan-symbol', …)` στο `EntityRendererComposite` + δήλωση ως 2D-only renderable στα 3 μητρώα (`BIM_RENDERABLE_TYPES`, `ENTITY_RENDER_CONTRACTS` `only2D`, `BIM_2D_ONLY_TYPES`). Coverage test `entity-render-coverage`: όλα τα floorplan-symbol invariants PASS (14/15· το μοναδικό υπόλοιπο failure = προϋπάρχον committed orphan `'leader'` από ADR-635, εκτός scope). Opus 4.8. |
