...
### Changelog
- **2026-07-10** — Bugfix: `deactivate` had `initialOverrides` in its `useCallback` deps, but the default `initialOverrides = {}` is a **fresh object every render** → `deactivate` identity churned → `useToolLifecycle`'s effect re-fired each render → `setOverrides(new {})` → **"Maximum update depth exceeded"** on every mounted-but-inactive area tool. Fixed by reading the latest initial-overrides via `initialOverridesRef` and removing `initialOverrides` from the deps → `deactivate` is now reference-stable (deps `[chain.deactivate]` only). Fixes all 3 area tools (SSoT). 10 jest pass.
