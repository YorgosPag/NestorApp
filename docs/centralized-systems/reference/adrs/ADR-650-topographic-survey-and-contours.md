...
- **v19 (2026-07-27) — M5α.2: ο «Έλεγχος ποιότητας» απαντά και στο 3Δ (markers + zoom-to + non-modal panel).**
  ...
  **Τι δεν έλεγε το αίτημα και έπρεπε να λυθεί: οι υποψήφιες ΔΕΝ ΥΠΗΡΧΑΝ στην 3Δ όψη.** Το M8β/Γ
  ...
  - **Το «μέγεθος» σε 3Δ ΔΕΝ γίνεται με πάχος γραμμής**: το `LineBasicMaterial.linewidth` το αγνοεί
    το WebGL (OpenGL core profile / ANGLE → πάντα 1 px). Η fat-line διαδρομή (`LineSegments2`) θα
    έφερνε per-resize `resolution` plumbing που το scene layer δεν έχει. Λύση: **κουκκίδες κορυφών**
    (`THREE.Points`, `sizeAttenuation: false` → σταθερό μέγεθος οθόνης, όπως το ⊙ και οι λαβές) +
    `depthTest: false` στην εστιασμένη, ώστε να **περνά μέσα από τον λόφο**. Το κυνήγι μιας γραμμής
    κρυμμένης πίσω από ράχη είναι ακριβώς η αποτυχία που το highlight υπάρχει για να προλάβει.
    **ΜΗΝ** «διορθωθεί» σε `linewidth`: δεν είναι παράλειψη, είναι περιορισμός της πλατφόρμας.
  - Οι κουκκίδες μοιράζονται **το ίδιο buffer** με την εστιασμένη γραμμή (κάθε κορυφή εμφανίζεται
    δύο φορές, μία ανά γειτονικό τμήμα· τα διπλά σχεδιάζονται ακριβώς από πάνω = αόρατα). Δεύτερη,
    αφαιρεμένων-διπλών προβολή θα ήταν δεύτερο αντίγραφο της αλυσίδας μετασχηματισμών για μηδενική
    οπτική διαφορά.
...
- **v21 (2026-07-27) — M10e IMPLEMENTED: η αυτόματη ταύτιση, με τέσσερα κανάλια και ένα συμβόλαιο.**
  **Status: ΥΛΟΠΟΙΗΜΕΝΟ & ΠΡΑΣΙΝΟ** — 78/78 στο `systems/geo-referencing`, 393/393 μαζί με
