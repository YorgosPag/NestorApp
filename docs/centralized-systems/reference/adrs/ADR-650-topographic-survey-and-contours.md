...
> ⚠️ **Το `TopoBoundary` απέκτησε ΔΕΥΤΕΡΟ ρόλο (ADR-718, 2026-07-28).** Εδώ στο M6 απαντά «τι
> **μετράνε** οι όγκοι». Με τον διακόπτη «Κοπή στο όριο» απαντά επιπλέον «τι **είναι** η
> επιφάνεια»: το `getTopoSurface()` επιστρέφει την TIN κομμένη στον ίδιο δακτύλιο, οπότε κάτοψη,
> ισοϋψείς, 3Δ, cut cap και εμβαδά συρρικνώνονται μαζί. Οι δύο ρόλοι είναι **ανεξάρτητοι** — το
> όριο μπορεί να μετρά χωρίς να κόβει. Το `boundary` παραμένει **εκτός** των definitions ακριβώς
> για τον λόγο που λέει το §M6 παρακάτω· η κοπή ζει σε **δεύτερο πάτωμα memo** και δεν
> ξανα-τριγωνοποιεί ποτέ. Λεπτομέρειες: `ADR-718-terrain-crop-to-site-boundary.md`.
...
- **2026-07-28 (v29)** — 🔴 **Ο ΟΡΙΣΜΟΣ ΤΗΣ ΕΠΙΦΑΝΕΙΑΣ ΑΠΕΚΤΗΣΕ ΦΙΛΤΡΟ → βλ. [ADR-720](./ADR-720-survey-points-without-elevation.md).**
  Ο εισαγωγέας (M2, `topo-column-mapping`) **πετούσε κάθε γραμμή CSV χωρίς υψόμετρο**. Μετρημένο σε
  πραγματικό τοπογραφικό: **7 από τις 9 κορυφές του οικοπέδου** ήταν σημεία του CSV (ταύτιση 2,2–6,4 mm)
  και **καμία** δεν είχε υψόμετρο ⇒ ο εισαγωγέας διέγραφε **το ίδιο το οικόπεδο** και κρατούσε τα σημεία
  των όμορων. Πλέον `TopoPoint.z` είναι **προαιρετικό**: το σημείο μπαίνει ως **δισδιάστατο** (ορατό,
  snappable) και **εξαιρείται από το TIN** — στο ΕΝΑ σημείο φιλτραρίσματος, την κορυφή του `buildTin`,
  ώστε και τα οκτώ παράγωγα να το κληρονομήσουν χωρίς αλλαγή (ίδιο σκεπτικό με την κοπή του ADR-718).
  **Η τριγωνοποίηση και οι όγκοι δεν αλλάζουν** — οι ίδιοι κόμβοι όπως πριν. Αλλάζει το τι **επιβιώνει**
  της εισαγωγής. `Breakline.vertices` έγινε `TopoPoint3D[]` (ασυνέχεια πάντα με Ζ). Δείτε το ADR-720 §3.3
  για τους **έξι σιωπηλούς καταναλωτές** που διάβαζαν `p.z` σαν να ήταν πάντα αριθμός.
...
- **2026-07-27 (v31)** — **§M10g BLUEPRINT: το πλαίσιο συντεταγμένων γίνεται ΔΕΔΟΜΕΝΟ, όχι υπόθεση.**
  ...
  **Status: §M10g — BLUEPRINT (υλοποιήθηκε στο v32· βλ. εκεί τις τρεις τεκμηριωμένες αποκλίσεις).**

- **2026-07-28 (v32)** — **§M10g IMPLEMENTED: το πλαίσιο έγινε δεδομένο, και ο μετακινημένος
  βορράς ακολουθεί.** *(Εντολή Giorgio: υλοποίηση του v31 blueprint, «όπως οι μεγάλοι — και αν
  γίνεται, εξυπνότερα».)*
  ...
  | Αρχείο | Ρόλος |
  |---|---|
  | `systems/geo-referencing/project-frame.ts` | **ΝΕΟ** — η ταυτότητα του πλαισίου: `ProjectFrameStamp`, `stampOfGeoReference` / `geoReferenceOfStamp`, `isSameFrame`, `isValidFrameStamp`, **`frameDelta(from,to)`**. Καθαρό· το delta συντίθεται από τον πυρήνα `inverseRigidMap`/`mapX`/`mapY` του `geo-transform` — **ούτε ένα νέο `cos`** (N.18) |
  | `geo-reference-store.ts` | +`getActiveProjectFrame()` — ο δίδυμος του `getActiveWorldToDisplayProjector`: «σε ποιο πλαίσιο ψήνω τώρα». Η **μία** είσοδος που διαβάζει store |
  | `systems/topography/topo-baked-groups.ts` | **ΝΕΟ** — ο δηλωτικός πίνακας: ομάδα → layers → **προσανατολισμός glyphs** (`paper`/`world`) |
  | `systems/topography/topo-baked-frame-store.ts` | **ΝΕΟ** — η σφραγίδα, **ανά `levelId` και ανά ομάδα**· fail-closed απολύμανση στο restore |
  | `systems/topography/topo-bake-commit.ts` | **ΝΕΟ** — η **ΜΙΑ ραφή** του ψησίματος: dev assertion → `completeEntities` → σφραγίδα, με το `group` **υποχρεωτικό** |
  | `systems/topography/persistence/topo-frame-reconcile.ts` | **ΝΕΟ** — ο **ΕΝΑΣ** reconciler: καθαρός πυρήνας (`reconcileBakedFramesInScene`) + ο ένας ιδιοκτήτης του lifecycle (`reconcileTopoFrame`) |
  | `persistence/regenerate-topo.ts` | έγινε **καθαρό** (`rebuildTopoDerivedScene`: scene in → scene out)· το `regenerateTopoContours` **διαγράφηκε** — δεύτερος δρόμος commit θα ήταν δεύτερος ιδιοκτήτης |
  | `topo-persistence-types` · `topo-state-io` | πεδίο `bakedFrames` (signature + doc fields + hydrate)· απόν ⇒ legacy |
  | `hooks/data/useTopoPersistence.ts` | και οι **τρεις** στιγμές (load / αλλαγή επιπέδου / αλλαγή γεωαναφοράς) καλούν τον reconciler· +συνδρομή του store σφραγίδων στο save |
  | `useTopoGrid` · `useTopoPointLabels` · `useNorthArrow` | περνούν από τη ραφή, δηλώνοντας ομάδα |
  | `systems/topography/topo-frame-status-store.ts` + `ui/panels/topography/TopoFrameNotice.tsx` | το **ορατό** μισό του fail-closed (i18n el+en `topography.frame.*`) |
  | `config/geometry-constants.ts` | +`SCENE_COORD_BAND_MM` — η ζώνη ADR-635 έπαψε να είναι literal σε δύο σπίτια |

  ### 2. Οι ΤΡΕΙΣ αποκλίσεις από το blueprint (και γιατί η καθεμιά είναι διόρθωση)

  **α) Σφραγίδα ΠΕΡΙΕΧΟΜΕΝΟΥ αντί για μονότονο `version`.** Το v31 §5.2 ζητούσε counter στο
  `setGeoReference`. Ένας runtime counter όμως **μηδενίζεται σε κάθε session**: μια σφραγίδα που τον
  κουβαλά και επιστρέφει από το Firestore αύριο συγκρίνεται με counter που ξεκίνησε από το 0 —
  δηλαδή παίρνει **σιωπηλά λάθος απόφαση** ακριβώς εκεί που το §M10g απαγορεύει τη σιωπή. Η
  ταυτότητα ενός πλαισίου **ΕΙΝΑΙ** το περιεχόμενό του (origin + στροφή): σταθερή μεταξύ sessions,
  και ταυτόχρονα **αυτή που χρειάζεται ο υπολογισμός του delta** — ένας αριθμός-ετικέτα δεν θα
  αρκούσε ποτέ για να μετακινήσει οντότητα. Ο counter δεν γράφτηκε καθόλου (θα ήταν dead code).

  **β) Σφραγίδα ανά `levelId`, όχι ανά έργο.** Το τοπογραφικό είναι SITE-level, αλλά τα **ψημένα**
  προϊόντα είναι οντότητες σκηνής και ζουν στο επίπεδο όπου τα έψησε ο χρήστης. Με μία σφραγίδα ανά
  ομάδα για όλο το έργο εμφανιζόταν **σιωπηλή απώλεια**: ψήσιμο καννάβου σε ισόγειο ΚΑΙ όροφο →
  αλλαγή γεωαναφοράς → ο reconciler τρέχει στο ενεργό επίπεδο, ενημερώνει τη μοναδική σφραγίδα, και
  ο κάναβος του άλλου ορόφου μένει **οριστικά** στο παλιό πλαίσιο επειδή πλέον η σφραγίδα λέει «όλα
  εντάξει». Με κλειδί ανά επίπεδο η διόρθωση είναι **οκνηρή αλλά πλήρης** — κάθε επίπεδο
  συμφιλιώνεται όταν το επισκεφθείς (Revit: ένα link επανασυμφιλιώνεται όταν ανοίξεις τη view του).

  **γ) Ο προσανατολισμός των glyphs ΔΗΛΩΝΕΤΑΙ ανά ομάδα.** Το blueprint έλεγε «rigid transform» —
  αλλά τα δύο είδη κειμένου δεν συμπεριφέρονται το ίδιο. Η ετικέτα «407650» και το «Χ407723.10»
  είναι **υπόμνημα του χαρτιού** (§M10f κανόνας 3: τα glyphs σχεδιάζονται οριζόντια), οπότε ένας
  reconciler που τα έστριβε θα **απέκλινε από τον ίδιο του τον παραγωγό** — ένα φρέσκο ψήσιμο θα τα
  ξανάγραφε οριζόντια. Το «Β» του βορρά αντίθετα διαβάζεται κατά μήκος του βέλους, δηλαδή είναι
  **μέγεθος του κόσμου**, και αν δεν στρίψει το σύμβολο λέει ψέματα. Ένας reconciler που **μαντεύει**
  ποιο ισχύει είναι ακριβώς η κλάση του §M10f· γι' αυτό η απάντηση ζει **δίπλα στα layers**, στον
  `topo-baked-groups`, και ο reconciler τη διαβάζει.

  ### 3. Πού πήγαμε ΠΙΟ ΠΕΡΑ από τους μεγάλους (§M10g.4, υλοποιημένο)

  1. **Το όριο ως συμβόλαιο με δόντια** — και **χωρίς μαγικό κατώφλι**. Ένα σταθερό «±1e6 mm» θα
     χτυπούσε ψευδώς σε υπαρκτό μεγάλο οικόπεδο (2 χλμ. είναι νόμιμο σχέδιο). Η διάγνωση όμως δεν
     χρειάζεται κατώφλι: με ενεργό πλαίσιο origin `O`, η **προβεβλημένη** γεωμετρία κάθεται κοντά
     στο 0 και η **αξήμωτη** κοντά στο `O` (4·10⁸ mm). Φράγμα στο `|O|/2` **ανά άξονα** → τάξεις
     μεγέθους περιθώριο, μηδέν ψευδώς θετικά, καμία αυθαίρετη σταθερά. Χωρίς γεωαναφορά ο έλεγχος
     **παραλείπεται**: δεν υπάρχει γέφυρα να ξεχαστεί. Ο Revit εδώ μόνο προειδοποιεί, εκ των υστέρων.
  2. **Αυτο-ίαση**: η σφραγίδα δίνει ντετερμινιστική ανίχνευση + διόρθωση στο load (delta-move).
  3. **Fail-closed ΜΕ φωνή**: legacy/άγνωστο πλαίσιο ⇒ κανένα άγγιγμα **και** ρητό μήνυμα στον
     διάλογο της γεωαναφοράς — τη στιγμή που ο χρήστης πειράζει το πλαίσιο. Επίσης fail-closed όταν
     η ομάδα περιέχει τύπο εκτός του SSoT της στροφής: **μισή** μετακίνηση θα διέλυε τη γεωμετρία.
  4. **Καμία παράκαμψη της ραφής**: capability anchor που διαβάζει τις πηγές και απαγορεύει σε
     οποιοδήποτε module της τοπογραφίας να καλέσει `completeEntities` εκτός allowlist — ο τέταρτος
     παραγωγός **δεν μπορεί** να γεννηθεί χωρίς σφραγίδα.

  ### 4. Τεστ (τα άγκιστρα, όχι τα δείγματα)

  - `geo-referencing/__tests__/project-frame.test.ts` (17) — το **αναλλοίωτο**: το πραγματικό ΕΓΣΑ
    σημείο μιας ψημένης οντότητας δεν αλλάζει επειδή άλλαξε η γεωαναφορά· συνθετικότητα δύο delta·
    ανοχή ταυτότητας **με αρνητικό μάρτυρα** (1 mm ⇒ ΑΛΛΟ πλαίσιο)· fail-closed είσοδοι.
  - `topography/__tests__/topo-frame-reconcile.test.ts` (13) — **ο μετακινημένος βορράς επιβιώνει**
    (+ αρνητικός μάρτυρας: ένα regenerate θα τον ξανάβαζε στο άγκιστρο)· idempotency με έλεγχο
    **ταυτότητας αντικειμένου**· legacy fail-closed **με αρνητικό μάρτυρα**· paper vs world glyph.
  - `topography/__tests__/topo-bake-commit.test.ts` (4) — ωμές ΕΓΣΑ ⇒ σκάει και **δεν μένει
    σφραγίδα**· νόμιμο 2-χλμ. οικόπεδο **δεν** είναι ψευδώς θετικό.
  - `topography/__tests__/topo-bake-seam-coverage.test.ts` (10) — το capability anchor της ραφής
    (+ αρνητικός μάρτυρας ότι το allowlist δεν είναι μπαγιάτικο) + απολύμανση του store.
  - `persistence/__tests__/regenerate-topo.test.ts` — ξαναγράφτηκε ώστε να οδηγεί **από τον
    reconciler**: μένει έλεγχος συμπεριφοράς, όχι συνάρτησης που άλλαξε σπίτι.

  **Πύλες:** topography + geo-referencing **58 suites / 516 tests πράσινα** · services + panels +
  app 42/553 · hooks/data + config 55/632 · `jscpd --diff` στα 16 αρχεία **καθαρό** · eslint καθαρό
  · `tsc` **δεν** τρέχει από πράκτορα (N.17). Registry: +`project-coordinate-frame`,
  +`topo-baked-frame-stamp` στο `.ssot-registry.json` (N.12). Boy-scout: αφαιρέθηκε νεκρό import
  `getPointLabelOptions` από το `useTopoPointLabels` (προϋπήρχε στο HEAD).

  **Εκκρεμεί:** ζωντανή οπτική επιβεβαίωση στον browser (`localhost:3000/dxf/viewer`) — άλλαξε
...
- **2026-07-29 (v34)** — **§M10g: το ψήσιμο έγινε ΜΙΑ ενέργεια χρήστη = ΜΙΑ αναίρεση (ADR-729).**

  Το capability anchor του §M10g **επιβάλλει** ότι κάθε παραγωγός ψημένων προϊόντων περνά από το
  `completeEntities` (ADR-057 SSoT) — allowlist εισαγωγών στο `topo-bake-seam-coverage.test.ts`.
  Αυτό ήταν **σωστό** και δεν άλλαξε. Αυτό που άλλαξε είναι η **εγγύηση πίσω από τη ραφή**:

  Μέχρι τώρα η ραφή παρήγαγε **187 εγγραφές ιστορικού** ανά ψήσιμο (1 διαγραφή + 186 δημιουργίες)
  έναντι ταβανιού 100 ⇒ το ψήσιμο ήταν **μη αναστρέψιμο** και **σάρωνε το προηγούμενο ιστορικό της
  συνεδρίας**. Το `topo-bake-commit` τεκμηρίωνε τον περιορισμό ως αναπόφευκτο: «τύλιγμα και των δύο
  θα απαιτούσε **παράκαμψη** της SSoT». **Το δίλημμα ήταν ψευδές.** Η
  `CommandHistory.runAsSingleUndo` (ADR-729) τυλίγει **γύρω** από το `completeEntities` αντί να το
  παρακάμψει — η διαδρομή που επιβάλλει το anchor μένει **ακέραιη**, καμία εισαγωγή δεν άλλαξε, και
  το ψήσιμο γίνεται **ΕΝΑ** βήμα αναίρεσης (αντίστροφο ξετύλιγμα: πρώτα φεύγουν τα φρέσκα, μετά
  επιστρέφουν τα αντικατασταθέντα από τα snapshots).

  **Η εγγύηση που προστίθεται στο §M10g**: ο παραγωγός δεν χρειάζεται να ξέρει τίποτα για την
  αναίρεση. Όπως κληρονομεί τη σφραγίδα πλαισίου και την ξανα-παρατήρηση του σήματος επειδή περνά
  από τη ΜΙΑ ραφή, έτσι κληρονομεί και την **ατομικότητα** — ο τέταρτος παραγωγός θα τη λάβει χωρίς
  να τη ζητήσει.

  **Status: §M10g — IMPLEMENTED & GREEN (62 suites / 594 tests, αμετάβλητα).**

- **2026-08-10 — §M10h: το «Αυτόματο κούμπωμα» έγραφε τη γεωαναφορά ΧΩΡΙΣ ΝΑ ΜΕΤΡΗΘΕΙ ΠΟΤΕ.**

  **Το εύρημα, σε πραγματικό έργο** (Ο.Τ. Γ753 Ευόσμου, `proj_04a6b4bb`). Ο ιδιοκτήτης εισήγαγε 84
  κορυφές και πάτησε «Γεωαναφορά». Το σχέδιο προσγειώθηκε **23,185 m** δίπλα από τον χάρτη.

  | | X (E) | Y (N) | στροφή |
  |---|---|---|---|
  | αποθηκευμένο `basePoint` | 407.372,1915 | 4.503.587,0312 | 0 |
  | σωστό (`sourceOrigin`) | 407.395,3762 | 4.503.586,8872 | 0 |
  | **σφάλμα** | **−23,1847 m** | **+0,1441 m** | 0 |

  **Ο ένοχος ταυτοποιήθηκε με αναπαραγωγή, όχι με εικασία.** Το `northRotation` ήταν **ακριβώς 0**,
  που μόνο το `fromOnePointPair` παράγει· από τους δύο παραγωγούς του, το `identity-restore` θα
  έδινε **ακριβώς** το `sourceOrigin`. Τρέχοντας το `autoAlignByRobustCenters` πάνω στα **ίδια**
  δεδομένα (1.459 οντότητες, 84 κορυφές) προέκυψε `407.372,6891 / 4.503.587,1594` — **0,50 m** από
  το αποθηκευμένο, έναντι σφάλματος **23,18 m**. Το υπόλοιπο εξηγείται πλήρως από την προσέγγιση
  bbox της αναπαραγωγής. **Ο ένοχος είναι το «Αυτόματο κούμπωμα».**

  🔴 **Η ΒΛΑΒΗ ΔΕΝ ΕΙΝΑΙ ΟΤΙ Η ΕΚΤΙΜΗΣΗ ΗΤΑΝ ΚΑΚΗ — ΕΙΝΑΙ ΟΤΙ ΚΑΝΕΙΣ ΔΕΝ ΤΗ ΜΕΤΡΗΣΕ.** Το
  `onAutoAlign` καλούσε `setGeoReference` **και** `persistProjectGeoReference` **αμέσως**, και
  εμφάνιζε `status.autoDone` = **«ολοκληρώθηκε»**. Στο **ίδιο** υποσύστημα κάθονταν αχρησιμοποίητοι
  ο `scoreGeoReference` και το `applyAcceptanceGates`, που μετρημένα απαντούν **0 inliers από 84,
  απαιτούνται 26** — ενώ το `identity-restore`, **δωρεάν και διαθέσιμο**, σκοράρει **84/84 με RMS
  0,43 mm** (μέγιστη απόσταση 0,7 mm). *Μήνυμα επιτυχίας πάνω σε αποτέλεσμα που σκοράρει μηδέν* —
  η χειρότερη μορφή του «πράσινο που σημαίνει «κανείς δεν κοίταξε»».

  🔑 **Παραβίαζε το ΓΡΑΠΤΟ συμβόλαιο του ίδιου του υποσυστήματος.** Το `geo-auto-match.ts` δηλώνει
  κεφαλαία: *«NEVER AUTO-APPLIES … the engineer presses «Εφαρμογή» … a human sees the evidence
  **before the project's base point changes**»*. Η διόρθωση δεν εισάγει **καμία νέα πολιτική** —
  εφαρμόζει την υπάρχουσα στο μονοπάτι που δεν είχε ποτέ καλωδιωθεί.

  **Η λύση**: `proposeRobustCenterAlignment()` — **ο πέμπτος τρόπος**, στο ίδιο σπίτι, κρινόμενος
  από τον **ίδιο** κριτή, παρουσιαζόμενος στην **ίδια** κάρτα απόδειξης. Δεν γράφει τίποτα.

  ⚠️ **ΔΕΝ μπήκε στο cascade, και ο λόγος είναι κανόνας**: κάθε κλάδος εκεί δοκιμάζεται επειδή
  **μπορεί να είναι σωστός**, ενώ μια εκτίμηση κεντροειδούς ουσιαστικά ποτέ δεν περνά τις πύλες.
  Θα ήταν φρουρός **που δεν μπορεί να πυροδοτήσει** (ADR-749 §5). Είναι **χωριστή είσοδος** γιατί
  απαντά **άλλο ερώτημα**: «δώσε μου πρόχειρη αφετηρία που θα διορθώσω», όχι «πες μου πού πάει».

  ⚠️ **`geo !== null` ΚΑΙ `failure !== null` ΣΥΝΥΠΑΡΧΟΥΝ, επίτηδες.** Απορρίφθηκε το «μπλόκαρέ το»:
  ο ίδιος ο σχεδιασμός του M10 δηλώνει ότι το auto-align είναι *«a CONVENIENCE first estimate …
  the user refines it»*. Ο μηχανικός **δικαιούται** πρόχειρη αφετηρία — **εν γνώσει του**. Η κάρτα
  δείχνει την άρνηση **δίπλα** στο «Εφαρμογή», ποτέ αντί για αυτό.

  ⚠️ **Η ασυμμετρία είναι το αντικυκλικό**: η **ΕΚΤΙΜΗΣΗ** διαβάζει bbox κέντρα οντοτήτων
  (`sceneEntityCenters`), η **ΕΠΑΛΗΘΕΥΣΗ** διαβάζει `ctx.allXY` — τα ίδια υποψήφια σημεία με κάθε
  άλλον κλάδο. *Μια εικασία που της επιτρέπεται να διαλέξει τα δικά της τεκμήρια δεν μπορεί να
  αποτύχει.* Άγκυρα `ζ4` + μετάλλαξη Μ2.

  **Boy Scout**: το `buildMatchContext` εξήχθη τη στιγμή που απέκτησε δεύτερο καλούντα — το σύνολο
  σημείων, το ευρετήριο και το εφικτό μέγιστο **ΕΙΝΑΙ** το «ίδιος κριτής»· δεύτερο αντίγραφο θα
  άφηνε τις δύο εισόδους να κρίνουν άλλα σύνολα αναφέροντας και οι δύο «inliers» (N.18 / 3.28).

  **Άγκυρες**: `ζ1`-`ζ6` στο `geo-auto-match.test.ts` (η `ζ1` αναπαράγει το σχήμα του πραγματικού
  σφάλματος· η `ζ2` δείχνει ότι στην **ίδια** είσοδο το cascade απαντά ακριβώς — αυτό είναι που
  κάνει τη σιωπηλή εκτίμηση **κατηγορητήριο**, όχι περιέργεια) + `Κ1`-`Κ4` στο
  `TopoGeoMatchResultCard.test.tsx` (η κάρτα δείχνει **ΚΑΙ** την άρνηση **ΚΑΙ** το «Εφαρμογή»).
  **3/3 μεταλλάξεις κόκκινες** (Μ1 σιωπή=3· Μ2 δικά του σημεία=3· Μ3 στο cascade=15), **Μ0 πράσινο**.

  ⚠️ Το `status.autoDone` **διαγράφηκε** από τα δύο locale — δεν υπάρχει πια «ολοκληρώθηκε» να πει.

  **Status: §M10h — IMPLEMENTED & GREEN (124 tests geo-referencing + 4 κάρτας).**

- **2026-08-10 — §M10h.1: ο ΚΡΙΤΗΣ έλεγε «0,0 εκ.» εκεί που δεν είχε μετρήσει τίποτα.**

  Η κάρτα που εισήγαγε το §M10h έδειχνε **«Μέση απόκλιση: 0,0 εκ.»** για την **απορριπτέα**
  πρόταση (0 από 84 σημεία) και **0,5 εκ.** για την **τέλεια** (84 από 84) — δηλαδή η λίστα
  απόδειξης κατέτασσε **ανάποδα**, δίπλα στο κουμπί που ξαναγράφει το `Project.basePoint`. Αιτία:
  `rmsMm: tally.inliers > 0 ? … : 0` στο `scoreGeoReference`. **RMS πάνω σε μηδέν inliers είναι
  0/0** — δεν είναι μικρό υπόλοιπο, είναι **μη ορισμένο**, και το σχόλιο του πεδίου το παραδεχόταν
  γραπτώς («`0` when there are no inliers»). Τεκμηριωμένη ψευδής τιμή είναι ψευδής τιμή.

  🔑 **Το στρώμα επιλέχθηκε με ΑΠΟΓΡΑΦΗ καταναλωτών, όχι με γούστο**: το `applyAcceptanceGates`
  **δεν μπορεί** να δει την ψευδή τιμή, γιατί `required = max(8, ⌈0,3·matchable⌉) ≥ 8 > 0` ⇒ η
  πύλη **πλήθους** απαντά πάντα πρώτη· και το `FailureLog.record` την παραλείπει **ρητά**
  (`score.inliers === 0`). Άρα η αλλαγή στη μηχανή **δεν μπορεί** να μετακινήσει ετυμηγορία πύλης.

  `GeoMatchScore.rmsMm` / `GeoMatchResult.rmsMm`: `number` → **`number | null`**. Το `GateInput`
  δέχεται `null` **fail-closed** (άρνηση, ποτέ «ταιριάζει») — ⚠️ **χωρίς** νέα κατάσταση
  `GateFailure`, που θα ήταν φρουρός ανίκανος να πυροδοτήσει (ADR-749 §5).

  **Άγκυρες**: `null ≠ 0` στον κριτή· **σειρά πυλών** + fail-closed στο συμβόλαιο· `Κ5`/`Κ6` στην
  κάρτα. **2/2 μεταλλάξεις κόκκινες, Μ0 πράσινο.** Πλήρης αιτιολόγηση: **ADR-782 §16**.
