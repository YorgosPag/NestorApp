...
- **v22 (2026-07-27) — M10e ΚΛΕΙΣΙΜΟ, μέρος Α: ένα false positive που έλεγε ψέματα με σιγουριά,
  ...
  **Το `topography.geoRef.match.running` παραμένει νεκρό** (επαληθευμένο: μηδέν καταναλωτές σε
  ...
  ### Ανοιχτά μετά το v22
  - **Α. SSoT registry** για `core/spatial` + `systems/geo-referencing` — **περιμένει απόφαση
    Giorgio** (κακό ERE ταιριάζει σιωπηλά με ΤΙΠΟΤΑ· βλ. `registry-golden-regex.test.js`).
  - **Γ. Απόδοση** — απόφαση Giorgio μεταξύ αλγοριθμικής επιτάχυνσης και worker.
