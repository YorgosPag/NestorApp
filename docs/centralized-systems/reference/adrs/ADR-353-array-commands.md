...
**Status:** ✅ COMPLETE — Phases A + B + C shipped. Array system feature complete (2026-05-16)
**Date:** 2026-05-15
...
| **C** | C3 | Path ribbon + path picker + reverse button (Phase C SHIPS — feature complete) | ✅ done | 11 new/modify | ~75 min | 2026-05-16 |
...
| 2026-05-16 | **Session C3 COMPLETE — Phase C SHIPS — Array system feature complete.** Files (new, 2): `systems/array/path-pick-controller.ts` (single-shot interactive path-entity re-pick from the contextual ribbon — `enterPathPickMode`/`exitPathPickMode`/`isPickingPath`/`getPickingPathArrayId`/`applyPathPick`; reads `getHoveredEntity()` from HoverStore at click time; validates against `PATH_ENTITY_TYPES` set of 7 types; issues `UpdateArrayParamsCommand`); `hooks/tools/useArrayPathTool.ts` (single-shot tool activation: validate pre-selection → arm awaiting-path state → status-bar prompt `arrayTool.pickPath` → next canvas click reads HoverStore → if valid curve → `CreateArrayCommand('path', defaults)` → replace selection with new ArrayEntity → contextual path tab auto-opens → revert tool to 'select'; if invalid type → show `arrayTool.invalidPathType` hint, keep waiting). Files (modified, 9): `systems/array/ArrayStore.ts` (+`pickingPathArrayId` state + setter/clearer); `ui/ribbon/hooks/bridge/array-command-keys.ts` (+path combo keys: `pathCount`/`pathSpacing`; +string key `pathMethod`; +toggles: `pathAlignItems`/`pathReversed`; +action: `pathPickPath`; new `ArrayRibbonStringComboKey` type + `isArrayRibbonStringKey` predicate); `ui/ribbon/data/contextual-array-tab.ts` (added `CONTEXTUAL_ARRAY_PATH_TAB` + `ARRAY_PATH_CONTEXTUAL_TRIGGER`; path tab = Geometry {method combobox, count, spacing} + Options {alignItems toggle, reversed toggle, pick-path action} + shared Actions panel; `ARRAY_ACTIONS_PANEL` factory now accepts `'path'`); `app/ribbon-contextual-config.ts` (added path case in `resolveContextualTrigger` + `CONTEXTUAL_ARRAY_PATH_TAB` in `RIBBON_CONTEXTUAL_TABS`); `ui/ribbon/data/home-tab-modify.ts` (`array.path` + `array2.path` `comingSoon: false` in both Modify and Modify Edit panels); `ui/ribbon/hooks/useRibbonArrayBridge.ts` (extended bridge — `readPathParams`/`readPathField`/`patchPathParam`/`readPathToggleField`/`patchPathToggle`/`readPathStringField`/`patchPathStringParam`; string-key path (pathMethod) dispatched before `parseFloat` guard in `onComboboxChange`); `ui/ribbon/hooks/useArrayRibbonActions.ts` (intercept `pathPickPath` action — toggle path-pick mode, surfaces `arrayTool.pickPath` hint); `hooks/tools/useModifyTools.ts` (mount `useArrayPathTool`, shared `replaceWithArrayId`); `hooks/canvas/canvas-click-types.ts` (+`arrayPathIsActive`, `handleArrayPathClick`, `handleArrayPathEntityRepick`); `hooks/canvas/useCanvasClickHandler.ts` (+path pick path at PRIORITY 1.605 — `handleArrayPathEntityRepick` then `handleArrayPathClick` after polar center checks); `hooks/canvas/useCanvasKeyboardShortcuts.ts` (+`handleArrayPathEscape`/`arrayPathIsActive` ESC handler); `components/dxf-layout/CanvasSection.tsx` (import `applyPathPick`/`getPickingPathArrayId`; destructure `arrayPathTool` from `useModifyTools`; `handleArrayPathEntityRepick` callback; wire to `useCanvasClickHandler` + `useCanvasKeyboardShortcuts`). i18n: el+en `dxf-viewer-shell.json` (`ribbon.commands.arrayEditor.{pathCount,pathMethod,pathSpacing,pathAlignItems,pathReversed,pathPickPath}`) + `tool-hints.json` (`arrayTool.{pickPath,invalidPathType}`). Architectural decision D1: path-entity identification at click time uses `getHoveredEntity()` from HoverStore (the existing hover pipeline already resolves the entity under cursor via snap/hover system) — avoids implementing custom hit-testing and is consistent with the existing `handleArrayPolarCenterRepick` pattern where the snap-aware world point is already computed by the caller. Architectural decision D2: `pathMethod` is a string-valued field ('divide'|'measure') — added a new `ArrayRibbonStringComboKey` type and `isArrayRibbonStringKey` predicate so the bridge can dispatch string values before the `parseFloat` guard used for numeric fields. Tests: C2 162 passing (unchanged). Acceptance: ✅ array.path `comingSoon: false` → button live; ✅ activate array-path → prompts "click path entity"; ✅ click invalid entity → `invalidPathType` hint shown, pick continues; ✅ click valid curve → `CreateArrayCommand('path')` → selection switches to new ArrayEntity → `array-path-selected` trigger → CONTEXTUAL_ARRAY_PATH_TAB auto-opens; ✅ ribbon method/count/spacing/align/reversed live-edit via `UpdateArrayParamsCommand`; ✅ `pathPickPath` button re-arms path re-pick on existing array; ✅ ESC cancels both modes; ✅ pre-commit hook passes (all files ≤500 lines, no hardcoded strings). |
| 2026-05-16 | **Phase C COMPLETE — Array system feature complete.** Path/Polar/Rectangular all shipped. arc-length sampler covers LINE/POLYLINE/LWPOLYLINE/ARC/CIRCLE/ELLIPSE/SPLINE. Contextual tabs for all 3 kinds. Interactive pickers, live ribbon editing, ESC cancellation, Explode, Edit Source, all wired. |
| 2026-05-15 | **Session C1 COMPLETE — Arc-length sampler analytical.** Files (new, 5 + 1 test): `systems/array/path-arc-length-sampler.ts` (dispatcher SSoT — `getPathSamplerStrategy`, `isPathEntity`, `pathTotalLength`, `samplePath`; `PathSample` + `PathSamplerStrategy<T>` interfaces; strategy pattern via `STRATEGIES: ReadonlyArray<PathSamplerStrategy>`); `path-sampler-strategies/line-strategy.ts` (lerp + atan2 tangent; reversed = swap start/end); `path-sampler-strategies/polyline-strategy.ts` (handles POLYLINE + LWPOLYLINE; cumulative-length walk; closed segment included; reversed = reversed vertex copy; bulge deferred to C2); `path-sampler-strategies/arc-strategy.ts` (ArcEntity degrees + counterclockwise flag; sweep = `positiveSweep(end−start)` for CCW / `positiveSweep(start−end)` for CW; reversed flips actualCCW + effectiveStart; tangent = angleDeg ± 90 per direction); `path-sampler-strategies/circle-strategy.ts` (TAU * radius; starts at angle 0 east; reversed = CW dir tangent −90); `__tests__/path-arc-length-sampler.test.ts` (51 tests, all passing — all ADR acceptance criteria covered). Decisions: (1) Strategy pattern with bivariant method shorthand — `PathSamplerStrategy<T extends Entity>` assignable to `PathSamplerStrategy<Entity>` array without cast. (2) CW arc sweep direction: `positiveSweep(startAngle − endAngle)` is the correct "long way round" formula; CW arc from 0°→90° = 270° sweep (confirmed by test). (3) Circle starts at angle 0 (east) — arbitrary but deterministic; C3 ribbon can add `startAngle` offset if needed. (4) No import of `calculateDistance`/`degToRad` from geometry-utils — strategies use inline `Math.hypot`/`Math.atan2`/`* DEG_TO_RAD` to avoid importing canvas-rendering modules into a pure-math SSoT file (no React, no canvas). (5) `as never` cast in dispatcher `pathTotalLength`/`samplePath` — unavoidable after the lookup; justified because `getPathSamplerStrategy` guarantees the strategy is the correct one for the entity. Next session (C2): ELLIPSE/SPLINE numerical + POLYLINE bulge arc-segments + `path-transform.ts` consumer. Gotchas: CW arc test expectation: `arc(0,0,1,0,270,false)` has sweep=90° (not 270°) — CW from 0° to 270° is the SHORT arc (0°→−90°). To get 270° CW sweep use `arc(0,0,1,0,90,false)` (long arc going CW 0°→350°→90°). |
| 2026-05-15 | **Session B2 COMPLETE — Phase B SHIPS — Polar Array end-to-end.** Files (new, 2): `systems/array/polar-center-pick-controller.ts` (single-shot interactive centre re-pick from the contextual ribbon — `enterCenterPickMode`/`exitCenterPickMode`/`isPickingCenter`/`getPickingCenterArrayId`/`applyCenterPick`; resets `radius` to 0 on apply so the renderer auto-derives from the new centre); `hooks/tools/useArrayPolarTool.ts` (single-shot tool activation: validate pre-selection → arm awaiting-centre state → status-bar prompt `arrayTool.pickCenter` → next snap-aware canvas click → CreateArrayCommand polar with defaults count 6 / fillAngle 360 / startAngle 0 / rotateItems true / radius 0 → replace selection with new ArrayEntity → revert tool to 'select'). Files (modified, 11): `systems/array/ArrayStore.ts` (+`pickingCenterArrayId` state + setter/clearer); `systems/array/array-grip-handlers.ts` (+`getPolarGripPositions` & `applyPolarGripDrag` — 3 grips: `polar-center` translate, `polar-radius` radial drag of item 0, `polar-fill-angle` arc-end drag for partial arcs only; `polarLayout` helper centralises radius-auto + last-item angle math); `ui/ribbon/data/contextual-array-tab.ts` (rewritten — exports `CONTEXTUAL_ARRAY_RECT_TAB`/`CONTEXTUAL_ARRAY_POLAR_TAB` and `ARRAY_RECT_CONTEXTUAL_TRIGGER`/`ARRAY_POLAR_CONTEXTUAL_TRIGGER`; deprecated alias `CONTEXTUAL_ARRAY_TAB`/`ARRAY_CONTEXTUAL_TRIGGER` kept for backward-compat; polar tab = Geometry { polarCount, polarFillAngle, polarStartAngle, polarRadius } + Options { polarRotateItems toggle, polarPickCenter action } + shared Actions panel via `ARRAY_ACTIONS_PANEL(variant)` factory); `ui/ribbon/hooks/bridge/array-command-keys.ts` (+polar combo keys + `polarRotateItems` toggle key + `polarPickCenter` action key + `isArrayRibbonToggleKey` predicate; deprecated `ArrayRibbonKey` alias); `ui/ribbon/hooks/useRibbonArrayBridge.ts` (extended bridge — discriminated read/write paths for rect+polar via `readPolarParams`/`patchPolarParam`/`patchPolarToggle`; toggles wired via `getToggleState`/`onToggle`; rect helpers preserved); `ui/ribbon/hooks/useArrayRibbonActions.ts` (intercept `polarPickCenter` action — toggles centre-pick mode, surfaces status-bar `arrayTool.pickCenter` hint); `app/DxfViewerContent.tsx` (`activeContextualTrigger` discriminates by `params.kind` → polar trigger when polar; `ribbonContextualTabs` adds the polar tab); `hooks/canvas/canvas-click-types.ts` (+`arrayPolarIsActive` + `handleArrayPolarClick`); `hooks/canvas/useCanvasClickHandler.ts` (+PRIORITY 1.605 — re-pick centre via `applyCenterPick` then polar tool centre click; also imports `useCommandHistory` for the centre re-pick command dispatch); `hooks/canvas/useCanvasKeyboardShortcuts.ts` (+ESC handler for polar tool); `hooks/tools/useModifyTools.ts` (mount `useArrayPolarTool`, share `replaceWithArrayId` selection helper across rect+polar tools); `components/dxf-layout/CanvasSection.tsx` (wire arrayPolarTool to canvas-click + keyboard-shortcut hooks); `ui/ribbon/data/home-tab-modify.ts` (`array.polar` `comingSoon: false` in both Modify panel and Modify Edit panel). i18n: el+en `dxf-viewer-shell.json` (`ribbon.panels.arrayOptions`, `ribbon.commands.arrayEditor.{polarCount,polarFillAngle,polarStartAngle,polarRadius,polarRotateItems,polarPickCenter,centerPickPrompt}`) + `tool-hints.json` (`arrayTool.pickCenter`). Decisions: (1) Two contextual tabs (rect/polar) routed by `params.kind` instead of one tab with hidden panels — keeps each tab declaration self-contained and avoids a runtime "if polar then…" inside the tab schema; deprecated `ARRAY_CONTEXTUAL_TRIGGER` alias kept so external callers still compile. (2) Centre re-pick is a *separate* state from the polar-tool awaiting-centre state — re-pick targets an existing array (UpdateArrayParamsCommand) while the tool's awaiting state targets a fresh CreateArrayCommand. Both consume the same canvas click but never simultaneously (the re-pick path returns early). (3) `polar-fill-angle` grip is suppressed for full-circle arrays (Math.abs(fillAngle)===360) and for count<2 — geometry has no meaningful endpoint to drag. (4) `polar-radius` grip lives at item 0's position (startAngle direction) — matches AutoCAD's "outer radial handle" convention. (5) `polar-center` drag resets `radius` to 0 (auto-derive) so the array stays attached to the source group's natural radius after the centre moves; explicit radius preserved by the radius combobox / radial grip path. (6) `replaceWithArrayId` selection helper extracted in useModifyTools so rect + polar tools share a single SSoT for "create array → swap selection to it → contextual tab opens". (7) `polarRotateItems` toggle re-uses the existing ribbon `'toggle'` widget type (TextEditor pattern) — no new widget code. Acceptance: ✅ Polar button (Modify ▾) → array-polar tool active → status bar prompts centre pick → click → polar array created with industry defaults → contextual polar tab auto-opens → comboboxes/toggle live-edit via UpdateArrayParamsCommand merged drag → grips drag radius/fillAngle/centre → "Pick Centre" ribbon button re-arms centre pick on existing arrays → Explode breaks polar into N independent entities → ESC cancels both modes. Gotchas: (a) `getPolarGripPositions` / `applyPolarGripDrag` are pure math — actual grip *renderer* and click→grip-drag plumbing are owned by the existing entity-grip system; this commit makes the math available, the renderer wire-up is the natural follow-up if the existing rect-grip dispatcher needs an explicit polar branch. (b) Radius-auto preserves AutoCAD parity: `radius=0` in PolarParams is the sentinel for "derive from source bbox center distance"; combobox option `value:'0'` displayed as `auto`. Next session (C1): Path arc-length sampler + path-tool. |
| 2026-07-15 | **Image (entourage) array support + SSoT de-clone in `array-entity-transform.ts`.** Context: entourage objects (trees/chairs/cars — all `ImageEntity`, ADR-654) placed along a path via the existing generic Array Path tool were **translated but not rotated** — `applyTransformToEntity` had no `case 'image'`, so images fell to `translateEntityFallback` which ignores `rotateDeg`. Result: `alignItems` (tangent-follow) silently no-op on images → cars on a curved lot / chairs on a curved row not oriented. Fix (general capability, NO type-gate — big-player parity with AutoCAD ARRAYPATH / C4D Cloner, Giorgio 2026-07-15): route `image` through the transform dispatch. Rather than add a 4th sibling twin, **extracted the shared per-type transforms into two SSoT helpers** (N.18 anti-clone, jscpd `3.28` diff was flagging 3 sibling clones): `transformPositioned` (shared by `text`/`mtext`/`image` — move `position` anchor + `accumulateRotationDeg`) and `transformBoxRect` (shared by `rectangle`/`rect` — move `x,y` origin + `corner1/2` + rotation), plus `accumulateRotationDeg(current, delta)` helper. Verified (grep/read, not assumed): (1) image bbox is already rotation-aware (`entity-bounds-ssot.ts` `image → viaBoundsCalculator` → `calculateImageBounds`) so `measure` spacing + array selection bounds are correct; (2) `computePathTransforms` places the source **bbox center** at each sample point (`translateX = s.position.x - cx`), and `ImageRenderer` rotates the rect **around `position` (bottom-left)** exactly like `RectangleEntity` (`createRectangleVertices`) — so the image's visual center lands on the path and rotates about itself (geometry proven correct); (3) expander deep-clones via `{...item, id}` and `transformPositioned` spreads `...e`, so `url`/`intrinsicWidth/Height`/`dxfImageExport` ride along → valid `ImageEntity` clones. Files: `systems/array/array-entity-transform.ts` (image case + 2 helpers, net −24 lines, 5 twins → 2 SSoT), `systems/array/__tests__/array-entity-transform.test.ts` (+4 image tests: translate-preserves-url/size/intrinsic, rotation-accumulate = the §3 gap, rotate-position-around-pivot, identity-clone). Tests: 18/18 pass. jscpd:diff: clean (0 new clones — reduced pre-existing). No UI/command added — reuses the generic Modify→Array path (Giorgio chose "reuse plain generic" discoverability). Superior-to-big-players entourage features (scatter/jitter, tangent align-offset angle) proposed as an optional next phase, pending Giorgio's go-ahead. |
| 2026-07-15 | **🐛 FIX (M2 browser-verify blocker) — `array-path` tool «pick path» click was a silent no-op.** Symptom (Giorgio): pre-select source → «Κατά διαδρομή» → hover+click on the polyline did nothing. Root: `useArrayPathTool.handleArrayPathClick` reads the target via the native `HoverStore` (`getHoveredEntity()`), but the 2D hover hit-test only runs when `activeTool === 'select' || entityPickingActive` (`systems/cursor/mouse-handler-move.ts:276`) — and `array-path` was **missing** from the `entityPickingActive` OR-expression in `CanvasSection.tsx`, so `getHoveredEntity()` stayed `null` and the `if (!hoveredId) return;` guard swallowed every click. Same class as the existing dim / match-properties / wall-on-entity picking tools. Fix: one additive boolean term `|| activeTool === 'array-path'`. `array-polar` needs none (it picks a center `worldPoint`, not an entity via hover). ADR-040-safe: no new subscription (cardinal rule 1 / CHECK 6C clear); CHECK 6B touch → co-staged ADR-040 + ADR-353. Pre-existing Phase-C wiring gap, unrelated to M1/M2 logic — surfaced only now because M2 is the first end-to-end browser test of path arrays. |
| 2026-07-15 | **M2 — ribbon UI + i18n exposing the M1 scatter/align/distribution controls.** Contextual path tab (`CONTEXTUAL_ARRAY_PATH_TAB`) now surfaces every M1 `PathParams` extra — zero new command/widget/store, pure reuse of the existing combobox/toggle bridge (mirrors how `pathCount`/`pathMethod` were wired in Session C3): (a) **`alignOffsetDeg`** as a number combobox in the Options panel next to the Align toggle (0/45/90/-90/180°). (b) New **"Scatter" panel** (`ribbon.panels.arrayScatter` = «Διασπορά») grouping `rotationJitterDeg` (°), `scaleJitterPct` (%), `offsetJitter` (units), `sourceDistribution` (string combo group/sequential/random — mirrors `pathMethod`), and `seed`. All read/patch flow through the existing `UpdateArrayParamsCommand` via the bridge (live preview + single-undo merge) — no new command. Blank-display avoided: undefined M1 fields display their effective `?? 0` / `?? 'group'` default so the combobox shows the real state. New command keys added to `ARRAY_RIBBON_KEYS` (`pathAlignOffset`/`pathRotationJitter`/`pathScaleJitter`/`pathOffsetJitter`/`pathSeed` numeric, `pathDistribution` string) + the `isArrayRibbonKey`/`isArrayRibbonStringKey` guards. Files: `ui/ribbon/hooks/bridge/array-command-keys.ts` (+6 keys +guard entries), `ui/ribbon/hooks/useRibbonArrayBridge.ts` (`readPathField`/`patchPathParam`/`readPathStringField`/`patchPathStringParam` extended), `ui/ribbon/data/contextual-array-tab.ts` (alignOffset combo + Scatter panel), NEW `ui/ribbon/data/contextual-array-tab-options.ts` (combobox option sets extracted here to keep the tab file <500 — N.7.1). i18n el+en FIRST (N.11): `ribbon.commands.arrayEditor.{alignOffset,rotationJitter,scaleJitter,offsetJitter,seed,distribution}` + `ribbon.panels.arrayScatter`. Tests: NEW `array-command-keys-m2.test.ts` (guard-routing regression, 3 green) + M1 array/command suites re-run green (192/192 in the array+UpdateArrayParams slice). jscpd:diff clean. All touched files <500 lines. **ADR-353 path array now FEATURE COMPLETE end-to-end (math + UI).** |
| 2026-07-15 | **M1 — "magical" path-array extras (math + SSoT, Giorgio approved 2026-07-15).** Beyond AutoCAD ARRAYPATH: (a) **align-offset angle** (`alignOffsetDeg`) — constant angle added on top of the tangent (AutoCAD "Base angle"), no-op unless `alignItems`; chairs perpendicular to a curved row / cars across a parking lane. (b) **Seeded scatter/jitter** (Lumion/Enscape/C4D Cloner parity) — per-item ± `rotationJitterDeg`, ± `offsetJitter` along the path NORMAL, ± `scaleJitterPct` uniform scale, all driven by a deterministic PRNG (`seed`) so the layout is STABLE across re-render/reload/undo (Google-level idempotent). (c) **Source distribution** (`sourceDistribution: 'group' | 'sequential' | 'random'`) — C4D-Cloner variety: 'group' (default, legacy = whole source group per point), 'sequential' (one source per point, cycling), 'random' (seeded pick) for natural forest variety; the picked source's OWN bbox center is re-based onto the sample point. SSoT reuse (N.18): scale jitter composes the **`scaleEntity` SSoT (ADR-348)** — zero new per-type scale math (mirrors `scaleArray`); tangent/arc-length reuse the existing path sampler. New file: `utils/seeded-prng.ts` (mulberry32, MIT, pure/deterministic — the ONE seeded-PRNG SSoT, none existed). All new `PathParams`/`ItemTransform` fields are **optional → zero regression** on legacy arrays (verified: rect/polar/existing-path suites green). Files: `systems/array/types.ts` (+6 optional PathParams + `ItemTransform.scale`), `utils/seeded-prng.ts` (new), `array-entity-transform.ts` (scale compose via scaleEntity; extracted `dispatchTranslateRotate`), `path-transform.ts` (`buildItemTransform` — align-offset + jitter), `array-expander.ts` (`resolveDistribution`/`expandGrouped`/`expandPerPoint`). Tests: +new `utils/__tests__/seeded-prng.test.ts` + extended path-transform/array-expander/array-entity-transform suites → array+command suites 635/635 green (the 1 failing `move-entity-geometry-coverage` test is a PRE-EXISTING `image`∉move-geometry gap in another agent's entourage domain — unrelated to array, not touched). jscpd:diff clean. **M2 (ribbon UI + i18n exposing these controls) is the next phase — NOT yet implemented.** No UI wired yet → params reachable only programmatically until M2. |
| 2026-07-16 | **SSoT extraction — `array-tool-core.ts` (ADR-584 / N.18 clone ratchet).** CHECK 3.28 flagged `useArrayPathTool` ↔ `useArrayPolarTool` as sibling clones (86 duplicated lines); the audit found `useArrayTool` (rect) carried the SAME activation guard, so all THREE hooks were rebased on one core instead of just the flagged pair. Extracted: `ArrayToolProps` (identical wiring in all 3), `showArrayHint`/`clearArrayHint`, `resolveArrayScene`, `findArrayEntity`, `collectSourceTypes`, `collectArraySources` (the empty/nested-array Q19 guard, previously in triplicate), `commitArrayCommand` (build → execute → reselect tail), `useArraySourcePick` (the polar/path arming FSM over the ADR-589 `useEdgeTriggeredLifecycle` SSoT), `resolvePendingSourcesInScene` + `resolvePendingArraySources`. Hooks: path 179→110, polar 213→112, rect 169→123 lines — each now holds only its own params + pick semantics. The three state machines are deliberately NOT merged (rect = single-shot, polar = awaits a world point, path = awaits a hovered curve) — same doctrine as the sibling `corner-tool-core.ts` (fillet/chamfer). Behaviour preserved byte-for-byte, incl. the path tool's «invalid type → stay armed» ordering and the «no scene manager → return without exiting» policy. NEW `__tests__/array-tool-core.test.ts` — 23 tests over the pure core (nested-array guard, stale-source rejection, command tail, arming FSM edges); the guards had no test while they lived in triplicate. Verified: CHECK 3.28 diff clean on all 5 files, full scan 3006→2978 clones (−28), 183/183 array system tests + 23/23 new GREEN. tsc skipped per N.17 (Giorgio / CI CHECK 3.29). |
