...
| Date | Change | Author |
|------|--------|--------|
| 2026-05-24 | **ADR-373 metadata-only enrichment — ZERO storage path impact.** The 5 new ISO 19650 fields (`disciplineCode`, `documentSeries`, `revisionCode`, `cdeState`, `buildingCode`) + `iso19650Source` audit live exclusively in the Firestore `files` document. `buildStoragePath()` SSoT untouched. 100% canonical pipeline (this ADR) preserved. See [ADR-373](./ADR-373-iso19650-metadata-enrichment.md). | Claude (Opus 4.7) |
| 2026-05-14 | Fix (part 3): `scenePathCacheRef` in `useAutoSaveSceneManager` was keyed by `fileName` instead of `fileId`. Two files with the same `originalFilename` would contaminate each other's cache entry: the second file's auto-save would read the first file's `canonicalScenePath` from the cache, find `fileId && canonicalScenePath` both set → skip the resolution block → write the wrong path into `processedData.processedDataPath` in Firestore. Next load fails with "No valid scene found in any Storage path". Fix: cache keyed by `fileId` (unique enterprise ID). Read: `scenePathCacheRef.current.get(fileId)` (only when `fileId` is known). Write: `scenePathCacheRef.current.set(fileId, canonicalScenePath)`. File: `src/subapps/dxf-viewer/hooks/scene/useAutoSaveSceneManager.ts`. | Claude Code |
| 2026-05-14 | Fix (part 2): Tier 2 loader crash-recovery for `.scene.json` in `processedDataPath`. The 2026-05-14 auto-save bug could write a plain-JSON `.scene.json` path into `processedData.processedDataPath`. Tier 2 previously assumed all `processedDataPath` values are gzip-compressed and called `pako.ungzip()` unconditionally — which throws on plain JSON, silently skipping valid scene data. Fix: if `processedPath.endsWith('.scene.json')`, decode as plain JSON (same as Tier 1) instead of decompressing. This recovers files already corrupted by the bug. File: `src/subapps/dxf-viewer/services/dxf-firestore-storage.impl.ts`. | Claude Code |
| 2026-05-14 | Fix: wrong `canonicalScenePath` written to Firestore when fileId is injected but path is missing. Root cause: `useAutoSaveSceneManager` called `findExistingFileRecord(companyId, fileName)` to resolve the missing path — but this name-based query can return a *different* file with the same `originalFilename`, causing `processedDataPath` in Firestore to point to that file's storage path. The injected `fileId` (correct) and the derived `canonicalScenePath` (wrong) then diverge → Storage write goes to the wrong path → next session load fails ("No valid scene found in any Storage path"). Fix: split the resolution block into two branches. (a) `fileId` known + path missing → `DxfFirestoreService.getFileStoragePath(fileId)` fetches the storagePath for *that specific document* via direct by-ID Firestore `getDoc`, derives canonical path from it. (b) `fileId` unknown → keep the existing `findExistingFileRecord` fallback. New static method `getFileStoragePath(fileId)` added to `DxfFirestoreService`. Files changed: `src/subapps/dxf-viewer/services/dxf-firestore.service.ts`, `src/subapps/dxf-viewer/hooks/scene/useAutoSaveSceneManager.ts`. | Claude Code |
| 2026-05-11 | Fix autosave fail on line completion — `canonicalScenePath is required` throw at `dxf-firestore-storage.impl.ts:171`. Root cause: `useLevelSceneLoader` set `currentFileName` but never injected `fileRecordId` nor `canonicalScenePath` after loading a scene from Storage, forcing every auto-save to either re-derive via `findExistingFileRecord` (1 extra Firestore read per debounce window) or throw when the lookup missed. Fix propagates `storagePath` end-to-end: (a) `DxfFileRecord` type gains optional `storagePath: string` field (SSoT extension, backward compatible — 2 production callers `BuildingFloorplanService.loadFloorplan` + `useLevelSceneLoader` unaffected). (b) `loadFromStorageImpl` + `getFileImpl` populate `storagePath` from the metadata they already fetch (zero extra reads). (c) `useLevelSceneLoader.loadScene` calls `sceneManager.setFileRecordId(sceneFileId)` + `sceneManager.setSaveContext({ canonicalScenePath: DxfFirestoreService.deriveScenePath(fileRecord.storagePath) })` immediately after `setLevelScene`, so the first auto-save tick already has the canonical path. Eliminates the throw AND the per-save Firestore lookup. Files: `src/subapps/dxf-viewer/services/dxf-firestore.types.ts`, `src/subapps/dxf-viewer/services/dxf-firestore-storage.impl.ts`, `src/subapps/dxf-viewer/systems/levels/hooks/useLevelSceneLoader.ts`. | Claude Code |
| 2026-04-17 | Phase 8 test coverage (Batch 30.2) — +4 test suites, 24 scenarios green for the full rename cascade stack. (a) `src/app/api/files/__tests__/propagate-entity-rename.route.test.ts` — 9 scenarios: 200 happy + null-email edge, 400 invalid entityType, 400 empty entityId, 400 whitespace newEntityLabel, 404 entity missing, 403 cross-tenant, 403 missing companyId, 500 propagator throws. Asserts `EntityFileDisplayPropagator.propagate()` is invoked with `ctx`-derived fields (`companyId`, `performedBy`, `performedByName`). (b) `src/services/filesystem/__tests__/file-mutation-gateway-propagate.test.ts` — 5 scenarios for `propagateEntityLabelRenameWithPolicy`: success → 1 `RealtimeService.dispatch('FILE_UPDATED')` per `updatedFile`; empty array, undefined, `success:false`, and `apiClient.request` throw → no dispatch. (c) `src/services/property/__tests__/property-mutation-gateway.test.ts` — 6 scenarios for `updatePropertyWithPolicy` cascade hook: name-change fires, name-same/whitespace-only/undefined/`success:false` skip, propagator reject isolated by `safeFireAndForget`. (d) `src/components/shared/files/hooks/__tests__/useEntityFiles-realtime.test.tsx` — 4 scenarios for `FILE_UPDATED` subscriber: matching fileId patched, unmatched id no-op, status string narrowed to typed enum, unmount unsubscribes (no leak). (e) Pre-commit hook: new CHECK 5B area `File Display Cascade` — `run_area_tests` triggers when staged files match `^src/(services/filesystem\|services/property\|app/api/files/propagate\|components/shared/files/hooks/useEntityFiles)` and runs the 5 suites (~3-5s). Closes Google Presubmit gap identified in the Batch 30 follow-up audit. | Claude Code |
