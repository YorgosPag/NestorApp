...
| Date | Changes |
|------|---------|
| 2026-06-07 | **Replace flow — BIM + BOQ cleanup (full replace) + warning/race fix** (Opus 4.8, GOL+SSOT). Root cause: the wizard pre-flight wipe (`floorplan-floor-wipe.service`) deleted overlays/levels/backgrounds/files/storage but **never** the 20 floor-scoped BIM collections (ADR-420) nor their `bim-auto` BOQ — so a replaced floorplan left orphan walls/columns/openings/slabs that still rendered on the new plan (verified on `flr_ea148848`). Plus a race: a slow `fetchPreview` left `wipeRequired=false` → upload skipped the confirm dialog and wiped silently (the screenshot stuck on «Έλεγχος υπάρχουσας κάτοψης…»). **Fix:** (1) NEW `bim-floor-wipe.service.ts` — `wipeBimForFloor`/`countBimForFloor` iterate `FLOOR_SCOPED_BIM_COLLECTIONS` (new SSoT in `firestore-collections.ts`) by `floorId`, delete only `source:'bim-auto'` BOQ (manual BOQ preserved), best-effort per-entity `deleted` audit (`EntityAuditService`, supported `AuditEntityType`s). (2) `floorplan-floor-wipe.service` gains `WipeAllForFloorOptions.wipeBim` + BIM/BOQ counts in preview; extracted shared `firestore-batch-delete.ts` (was duplicated 3×). (3) `wipe-floor` route accepts `wipeBim` + threads pre-resolved performer name. (4) `StepUpload` confirm dialog now offers **two** choices — «Διατήρηση BIM» (default, respects ADR-420 re-import) vs «Διαγραφή όλων (BIM + μετρήσεις)» — only when the floor has BIM; preview banner + dialog show BIM/BOQ counts; race fixed (no upload until preview resolves; failed preview → zero-state). i18n el+en `files-media.floorplanImport.wip*`. Tests: `bim-floor-wipe.service.test.ts` 6/6. **Files**: `src/config/firestore-collections.ts`, `src/services/floorplan-background/{firestore-batch-delete,bim-floor-wipe.service,floorplan-floor-wipe.service,floorplan-cascade-delete.service}.ts`, `src/app/api/floorplans/wipe-floor/route.ts`, `src/features/floorplan-import/{hooks/useFloorplanSmartUpload,components/StepUpload}.tsx`, i18n el/en `files-media.json`. See ADR-420. |
| 2026-05-17 | **Phase 9 follow-up — re-import viewport + FileRecord race fix**. (1) `DxfViewerContent` fit-to-view now keys on `fileRecordId` instead of `prevSceneRef` so the viewport resets on wizard re-import (wipe + re-upload on the same level). (2) `FloorplanImportWizard.onComplete` propagates `meta.fileId` as `saveContext.fileRecordId` to prevent `linkSceneToLevel` racing against cadFiles processor. (3) `floorplan-floor-wipe.service.ts`: wipe clears `sceneFileId/sceneFileName → null` (content clear) instead of deleting `dxf_viewer_levels` docs — keeps `currentLevelId` alive across the wipe → re-import cycle. |
...
| 2026-05-12 | 🛠️ **backgroundId made optional in floorplan_overlays API (Phase 9 schema consolidation)** — `backgroundId` was `z.string().min(1).max(128)` (required non-empty) in `CreateFloorplanOverlaySchema` and `UpsertFloorplanOverlaySchema`. Blocked saves from DXF-only levels with no background image. **Fix**: schema changed to `.nullish()` (accepts string, null, or omitted); `FloorplanOverlayDocument.backgroundId` typed `string | null | undefined`; POST+PUT handlers use conditional spread so null/undefined is never written to Firestore. Firestore rules immutability check already null-safe (`null == null = true` in rules). **Files**: `floorplan-overlays.schemas.ts` (2 schemas), `floorplan-overlays.types.ts`, `floorplan-overlays.handlers.ts` (POST + PUT). |
| 2026-05-09 | 🚀 **DXF Viewer mousemove lag fix — LayerCanvas useSyncExternalStore decoupling** (Sonnet 4.6, GOL+SSOT) — **Root cause**: `LayerCanvas` subscribed to `ImmediatePositionStore` via `useCursorPosition()` (`useSyncExternalStore`). Every mousemove → `setPosition()` → React 18 synchronous re-render of `LayerCanvas` (bypasses `React.memo` — internal trigger). `LayerCanvas` has heavy hook landscape (`useCentralizedMouseHandlers`, `useLayerCanvasRenderer`, `useLayerHitTest`, …) → 5-20ms per render × 60fps = 200-364ms violation. **Secondary**: `cursor.position` in `renderLayers` useCallback deps (`layer-canvas-hooks.ts`) — callback recreated each mousemove → `useEffect` re-registered with `UnifiedFrameScheduler` each frame. **Tertiary**: `cursor.position` in dirty `useEffect` deps — redundant (ImmediatePositionStore.setPosition already calls `markSystemsDirty`). **Fix A** (`layer-canvas-hooks.ts`): Remove `cursor.position` from `renderLayers` useCallback deps — function reads `ImmediatePositionStore.getPosition()` directly at render time, dep is unnecessary. **Fix B** (`layer-canvas-hooks.ts`): Remove `cursor.position` from dirty effect deps — already covered by `markSystemsDirty`. **Fix C** (`layer-canvas-hooks.ts` + `LayerCanvas.tsx`): Remove `cursorPosition` prop from `LayerHitTestParams`/`useLayerHitTest`; `handleLayerSelection` (click-only, not mousemove) reads `ImmediatePositionStore.getPosition()` directly in the callback body. Remove `useCursorPosition()` import+call from `LayerCanvas.tsx`. **Fix D** (`CanvasLayerStack.tsx`): Memoize `handleDxfMouseMove` with `useCallback` — prevents `DxfCanvas` (React.memo) from receiving new prop reference each render. `CrosshairOverlay` retains `useCursorPosition()` intentionally — it is a small component whose re-render on cursor move is expected and negligible. **Snap system**: confirmed O(log n) via spatial index (`querySnap`) — not a performance factor. **Expected outcome**: `LayerCanvas` no longer re-renders on mousemove; RAF cycle unaffected; crosshair canvas2D path via `directRenderCallback` unchanged. |
