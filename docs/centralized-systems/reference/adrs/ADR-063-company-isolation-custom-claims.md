...
- 🔴 **ADR-787** — Η πολυ-οργανισμική πλατφόρμα, **απόφαση Ε-5** (2026-08-22).
  **Το token κρατά ΤΑΥΤΟΤΗΤΑ, ΠΟΤΕ ΚΑΤΑΛΟΓΟ ΧΩΡΩΝ.** Όταν ένας άνθρωπος γίνει μέλος πολλών
  χώρων (αρχή Α2), ο πειρασμός είναι να μπει η **λίστα** των χώρων στα custom claims.
  ⛔ **Απαγορεύεται, με αριθμό**: το όριο των custom claims είναι **1.000 bytes** και ένα
  αναγνωριστικό χώρου είναι **41 χαρακτήρες** (`comp_` + UUID) ⇒ ταβάνι **~20 χώροι**, και η
  **21η πρόσκληση σπάει τη σύνδεση του ανθρώπου**. Η ιδιότητα μέλους ελέγχεται
  **στον διακομιστή, σε κάθε αίτημα**· ο χώρος ταξιδεύει **στη διεύθυνση** ως *αίτημα*, ποτέ
  ως *άδεια*.
  ⚠️ Η γνωστή **καθυστέρηση ανανέωσης** των claims (έως 1 ώρα) **ΔΕΝ** είναι ο λόγος — είναι
  ήδη λυμένη από το `src/lib/auth/set-claims-with-mirror.ts` +
  `src/auth/contexts/auth-context/use-claims-refresh.ts` (ανανέωση σε δευτερόλεπτα). Ο **μόνος**
  φραγμός είναι το **μέγεθος**.
