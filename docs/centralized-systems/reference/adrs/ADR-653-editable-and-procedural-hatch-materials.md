...
**Status:** Φ8 ✅ + Φ9 ✅ IMPLEMENTED · Φ7 ΜΠΛΟΚΑΡΙΣΜΕΝΗ (χρειάζεται CC0 αρχεία εικόνας — ο agent δεν κατεβάζει)
...
|---|---|---|---|
| **Φ7** ⛔ | **Εμπλουτισμός καταλόγου** — +N νέα CC0 builtin υλικά (φωτογραφικά). Νέα slugs + πραγματικό tile size + κατάλογος + i18n. Παίρνει **αυτόματα** και το 3D (ίδιο registry). **ΜΠΛΟΚΑΡΙΣΜΕΝΗ:** κάθε υλικό χρειάζεται πραγματικό αρχείο υφής (`public/textures/<slug>/albedo.jpg` για dev + Firebase Storage `bim-texture-library/<slug>/` για prod — τα `.jpg` είναι `.gitignored`)· ο agent δεν κατεβάζει/δημιουργεί φωτο. Ξεμπλοκάρει όταν ο Giorgio δώσει εικόνες ή λίστα downloads. | `bim-texture-registry.ts`, `data/material-image-catalog.ts`, `locales/{el,en}/dxf-viewer-shell.json`, `public/textures/<slug>/albedo.jpg` | ~10 λεπτά/υλικό |
