...
| **3b** | `StairGeometryService`: kind `u-shape` + `gamma` + auto-landing (G16) + cut split (G14) + tread numbering (G21) | 4 | L | 500 | 3a |
...
| 2026-05-16 | **Phase 3b partial — `buildCutLineForFlights` (G14) + `buildTreadLabels` (G21)**. 2 file: (1) **EXTEND** `stair-geometry-shared.ts` — `buildCutLineForFlights(treads, flightSplit, flightDirections, width, cutPlaneHeight): Segment3D | undefined` fixes latent bug in Phase 3a `buildCutLine`: l-shape always passed `u1` even when cut crossed inside flight 2 (wrong perpendicular direction). Multi-flight aware: iterates flight→tread in order, uses `flightDirections[f]` for each flight, returns perpendicular segment at first tread ≥ cutPlaneHeight. Returns `undefined` when no tread crosses (single-band stair below cut plane). (2) **NEW** `systems/stairs/stair-geometry-labels.ts` (~69 LOC) — `buildTreadLabels(treads, flightSplit, display, everyN, restartPerFlight, treadNumberStart): readonly StairTreadLabel[] | undefined` pure fn, SSoT for tread numbering across all stair kinds. Display modes: `'none'` → `undefined`; `'all'` → one label per tread; `'nth'` → every N (default 5). Local index resets per-flight when `restartPerFlight=true` (independent numbering per flight), otherwise global index. Text = `String(treadNumberStart + localIndex)`. Position = centroid3D of tread polygon. No side effects, no React state, RAF-safe. **Commit eseguito da Giorgio** (§7.0). |
...
- Aggiornare `.ssot-registry.json` con **8 nuovi moduli**: `StairGeometryService` *(Phase 3a DONE 2026-05-16 — `computeStairGeometry` dispatch `straight` + `l-shape` operativi; kinds `u-shape`/`gamma` (Phase 3b), `spiral`/`helical` (4a), `elliptical`/`winder` (4b), `triangular-fan`/`triangular-outline`/`sketch` (4c) ancora throw-sentinel; `computeWalkline` re-export pubblico)*, `stair-validator`, `geometry-offset-utils` *(Phase 2b DONE 2026-05-16 — `offsetPolyline` xy-plane miter+bevel)*, `geometry-curve-utils` *(Phase 2a spiral+helix DONE 2026-05-16; Phase 2b ellipse DONE 2026-05-16 — `ellipseArcLength` + `ellipseSample`)*, `stair-grips`, `stair-transforms`, `stair-presets-service`, `stair-schedule-exporter`.
