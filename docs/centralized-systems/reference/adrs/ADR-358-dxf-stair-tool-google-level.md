...
| **2a** | Geometry helpers math: `spiralSample`, `helixSample` + unit tests (accuracy vs analytical, tolerance 1e-6) | 3 | M | 350 | 1 |
| **2b** | Geometry helpers math: `ellipseArcLength`, `offsetPolyline` (convex+concave) + unit tests | 3 | M | 350 | 1 |
| **3a** | `StairGeometryService`: kind `straight` + `l-shape` + walkline + tread placement | 3 | M | 400 | 2a-2b |
...
- Aggiornare `.ssot-registry.json` con **8 nuovi moduli**: `StairGeometryService`, `stair-validator`, `geometry-offset-utils`, `geometry-curve-utils`, `stair-grips`, `stair-transforms`, `stair-presets-service`, `stair-schedule-exporter`.
