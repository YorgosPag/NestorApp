...
| **7b2** | Floating Advanced Properties panel + materials/finishes UI per-stair+per-tread override (G13, Q19) + `cutPlaneHeight` toggle (Q21) + tread numbering UI (Q28 — `treadLabelDisplay` all/nth/none + `treadLabelEveryN` + `restartPerFlight`) + `nosingSide` selector (Q34) + Inline Dynamic Input rise/tread/width Tab cycling (Phase 5a carryover) + Multi-flight `turnDirection` toggle per L/U/Γ flight 2/3 (Phase 5b carryover) | 7 | L | 700 | 7b1 |
...
- **(g) Floating Advanced Properties panel** + materials/finishes UI per-stair + per-tread override (G13, Q19) + `cutPlaneHeight` toggle (Q21) + tread numbering UI (Q28) + `nosingSide` selector (Q34) — originale Phase 7b scope §7.2, da fare in Phase 7b2.
