...
- **Status**: 🟢 PARTIAL IMPLEMENTED (Phase 3 — πιλότος frame/casing user-library. **ΒΗΜΑ 1 = ορθογώνιο** + **ΒΗΜΑ 2 = realistic swept `section` outline** υλοποιήθηκαν 2026-07-18. ΒΗΜΑ 3 = interactive outline editor + επόμενα εξαρτήματα handle/hinge/glazing = εκκρεμούν)
...
- **2026-07-18 (Phase 3 — IMPLEMENTATION, πιλότος frame/casing user-library, ΒΗΜΑ 1 = ορθογώνιο)** — υλοποιήθηκε με
  orchestrator (6 implementer agents + 4 recognition agents, έγκριση Giorgio). **Code = source of truth.** Παραδοτέα:
  - **enterprise-id (N.6):** νέο prefix `OPENING_FRAME_PRESET='frmpst'` + `generateOpeningFramePresetId()` στα 4 touch points
    (`enterprise-id-prefixes.ts`/`-class.ts`/`-convenience.ts`/`.service.ts` facade) + collection const `OPENING_FRAME_PRESETS`
    (`src/config/firestore-collections.ts`, subcollection `companies/{cid}/opening_frame_presets`) + `.ssot-registry.json`
    (append στο `bim-id-prefix` module description) + tests (`bim-ids.test.ts`, `bim-collections.test.ts`).
  - **Data layer:** `OpeningFrameProfilePresetDoc extends ScopedLibraryDoc` + `frameProfilePresetDocToProfile` mapper
    (`bim/types/opening-frame-profile.ts`)· `opening-frame-profile-store.ts` (Zustand + `subscribeWithSelector`,
    sync accessors `getUserFrameProfileById`/`listUserFrameProfiles` — **η σύγχρονη λύση** για τον σύγχρονο resolver, mirror
    `bim-family-type-store`)· `opening-frame-profile-library-service.ts` (πάνω στο `createSubcollectionScopedLibrary`,
    hand-rolled `saveProfile`/`updateProfile`/`deleteProfile` + Zod + tenant/scope stamp, mirror `bim-family-type-service`)·
    `useOpeningFrameProfileLibrary.ts` hook.
  - **Merge SSoT (N.18):** `opening-frame-profile-lookup.ts` — `resolveFrameProfileById`/`listMergedFrameProfiles`/
    `listMergedFrameProfileManufacturers` (builtin ∪ user-store, **builtin wins σε σύγκρουση id**). Ο resolver
    (`resolve-opening-frame-profile.ts`, 2 call sites) και ο ribbon bridge (`opening-frame-profile-bridge.ts`) καταναλώνουν
    **αυτό** — καμία διπλή merge λογική. `useFrameProfileCatalog.ts` = thin hook που delegate-άρει στο lookup.
  - **UI:** ribbon widget `opening-frame-profile-library-widget.tsx` (+parts) «Αποθήκευση ως δικό μου» + «Duplicate & edit»
    (default scope=company §4 Q5· derive→`origin:'derived'`+`derivedFrom`) στο υπάρχον `opening-frame-profile` panel
    (`contextual-opening-tab.ts` + `RibbonPanel.tsx` switch)· διαχειριστής βιβλιοθήκης `FrameProfilesLibraryPanel.tsx`
    (+`FrameProfileCard.tsx`) list/rename/delete (mirror `MaterialsLibraryPanel`) + sidebar registration
    (`panel-types.ts`/`PanelTabs.tsx`/`usePanelContentRenderer.tsx`, panel id `frame-profiles`).
  - **i18n (N.11):** el+en `dxf-viewer-shell.json` (`ribbon.commands.openingEditor.frameProfile.*` widget keys) +
    `dxf-viewer-panels.json` (`panels.frameProfiles.*` manager keys).
  - **Tests:** lookup-merge + resolver-integration + store round-trip (jest). jscpd καθαρό σε όλα τα diffs (N.18).
  - **Αποφάσεις agent (Revit-grade, τεκμηριωμένες):** (α) reconcile ονομασίας collection §2.2 `opening_<x>_presets` vs §2.9
    `opening_frame_profiles` → επιλέχθηκε **`opening_frame_presets`** (ταιριάζει με prefix `frmpst`/preset terminology).
    (β) Hook = one-shot `list()` **όχι** `subscribe()` — το `ScopedLibraryService.subscribe()` **πετά** σε subcollection
    topology (μιμείται `useBimFamilyTypes`, όχι `useMaterialLibrary`). (γ) `useFrameProfileCatalog` κρατήθηκε (ADR §2.9-3)
    αλλά delegate-άρει το merge στο lookup SSoT αντί να το αναπαράγει (N.18). (δ) registry: append στο υπάρχον module
    αντί για νέο (allowlist ήδη καλύπτει τα paths).
  - **ΕΚΚΡΕΜΕΙ (ΒΗΜΑ 2, §4 Q4):** editable realistic `section?: Vec2[]` outline (extrude spec §2.4) — ο 3D `frameBars`
    παραμένει ορθογώνιο box (faceWidth×depth). Επόμενα εξαρτήματα (handle/hinge/glazing) + composite set (§2.5) +
    Στρώμα 2 custom-mesh (§2.6) = μελλοντικά dimensions.
- **2026-07-18 (Phase 3 — IMPLEMENTATION, ΒΗΜΑ 2 = realistic swept `section`)** — υλοποιήθηκε (Plan Mode, έγκριση Giorgio,
  shared working tree). **Code = source of truth.** Το προαιρετικό `section` outline σαρώνεται (swept/extruded) κατά μήκος
  κάθε μέλους κάσας — Revit *Profile Family* / ArchiCAD *Complex Profile* / C4D constant-cross-section. **Zero-regression:**
  χωρίς `section` → ακριβώς το ίδιο ορθογώνιο box όπως πριν (byte-identical positions/counts/order — το `opening-mesh.test.ts`
  το κλειδώνει). Παραδοτέα:
  - **Data model (`bim/types/opening-frame-profile.ts`):** νέος SSoT τύπος `FrameSectionPoint {x,y}` (mm, `(face×depth)` plane,
    origin=centerline — δεν υπάρχει canonical 2D-point στο dxf-viewer)· `section?: readonly FrameSectionPoint[]` σε
    `OpeningFrameProfile` + `OpeningFrameProfilePresetDoc`· `frameProfilePresetDocToProfile` μεταφέρει το `section` (spread
    μόνο όταν υπάρχει → κανένα explicit `undefined`).
  - **Resolver (`resolve-opening-frame-profile.ts`):** `ResolvedFrameProfile.section?` + fold «last wins» (catalog/preset layer
    ή per-instance override· section-less layer ΔΕΝ καθαρίζει προηγούμενο· legacy fallback → undefined).
  - **Per-instance override:** `OpeningParams.frameProfileOverrides.section?` (`opening-types.ts`) + Zod `FrameSectionSchema`
    (`opening.schemas.ts`, ≥3 finite vertices, **exported SSoT** — reused ΚΑΙ από το library service, όχι διπλό schema).
  - **Persistence (`opening-frame-profile-library-service.ts`):** `FrameProfileFieldsSchema += section` + ρητό `section` στο
    `saveProfile` payload (το `saveProfile` απαριθμεί πεδία — **δεν** ήταν generic spread· `pruneUndefined` κόβει το undefined
    → zero regression). `updateProfile` το μεταφέρει μέσω `...patch.profile`.
  - **3D geometry:** νέο `bim-3d/converters/opening-frame-section-geometry.ts` — `frameBarLayout()` = **SSoT σκελετός**
    (jamb/head/sill center+orientation+length) που καταναλώνουν ΚΑΙ το box path ΚΑΙ το extrude path (καμία διπλή θέση)·
    `buildFrameMemberGeometry()`/`buildFrameProfileMembers()` = swept extrude μοτίβο `beam-ishape-geometry`. `buildClosedShape()`
    προστέθηκε στο `bim-three-shape-helpers.ts` (ring→Shape SSoT). Στο `opening-mesh.ts`: dispatch box-vs-extrude για την κάσα
    (leaf/panel + hardware πάντα box)· `finalizeMemberMesh()` = SSoT tail (basis-apply + shadows + BIM stamp) κοινό box+extrude.
  - **Catalog:** builtin `GENERIC-70x70-rebate-frame` (L-shaped πατούρα section) — ορατό demo στον profile picker χωρίς UI editor.
  - **Tests (jest):** section-propagation + mapper round-trip (`resolve-opening-frame-profile.test.ts`)· pure swept geometry +
    layout SSoT (`opening-frame-section-geometry.test.ts`)· extrude-vs-box + count/order invariance (`opening-mesh.test.ts`)·
    **write-path persistence** — save/list/update round-trip του `section` + Zod reject + absent-when-unset, Firestore fully
    mocked (`opening-frame-profile-library-service.test.ts`, mirror `stair-presets-service.test.ts`). Όλα πράσινα· jscpd καθαρό (N.18).
  - **ΕΚΚΡΕΜΕΙ (ΒΗΜΑ 3):** interactive vertex/outline editor UI (preset/override-driven section μόνο προς το παρόν)·
    επόμενα εξαρτήματα handle/hinge/glazing (cylinder/lathe → πλήρες `ComponentShapeSpec` union §2.4).
