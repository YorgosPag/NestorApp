...
## 4. Ανοιχτές ερωτήσεις για Giorgio (πριν Phase 3 / κατά τη διάρκεια)

- **Q1 — Στρώμα 2 τώρα ή μετά τον πιλότο;** Ο Giorgio είπε «και τα δύο από την αρχή». Πρόταση: ο **πιλότος frame
  = μόνο Στρώμα 1** (το frame δεν χρειάζεται custom mesh), και το Στρώμα 2 σχεδιάζεται/υλοποιείται στο **δεύτερο**
  dimension (π.χ. χειρολαβές) όπου έχει πραγματικό νόημα το «ανεβάζω δικό μου 3D». → επιβεβαίωση.
- **Q2 — Geometry-σέβεται-ποσότητες;** Θέλει η γεωμετρία να ζωγραφίζει N μεντεσέδες όταν το override λέει N
  (σήμερα hardcoded 2/3); Ξεχωριστό/μεγαλύτερο scope (§2.4) — να μπει στο ίδιο dimension ή αργότερα;
- **Q3 — Enterprise-id:** νέα per-type prefixes (`frmpst`/`hwpst`/…, σύσταση) ή reuse του `bpst` ενιαία (§2.2);
- **Q4 — Frame realistic cross-section στον πιλότο;** Να μπει το editable `section` outline **τώρα** στον πιλότο
  (πλήρες «ρεαλιστικά εξαρχής») ή πρώτα library+save+derive με ορθογώνιο και το `section` σε επόμενο βήμα;
- **Q5 — Scope default:** company επιβεβαιωμένα κύριο· να είναι το user προεπιλογή στο «Αποθήκευση ως δικό μου» ή company;
...
- **2026-07-18 (Phase 2 — DESIGN)** — ADR δημιουργήθηκε από το research report (6 read-only research agents, Phase 1).
  ...
  Ανοιχτά Q1-Q5 (§4) για επιβεβαίωση πριν/κατά Phase 3. **ΚΑΜΙΑ υλοποίηση κώδικα σε αυτή τη φάση.** — *Ο agent του
  Phase 3 (πιλότος frame) θα προσαρτήσει εδώ την implementation entry, code = source of truth.*
