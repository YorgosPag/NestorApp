...
| Date | Author | Change |
|------|--------|--------|
| 2026-07-25 | Claude Code (Opus 5) | **Η 4η πράξη του κύκλου ζωής επιτέλους κεντρικοποιήθηκε — βλ. ADR-697.** Το §2 αυτού του ADR αποφάσισε «ΕΝΑΣ κεντρικοποιημένος μηχανισμός, όχι copy-paste ανά entity» και τηρήθηκε για `softDelete` / `restoreFromTrash` / `permanentDelete`. Η **ανάγνωση** του κάδου όμως ήταν **πέντε αντίγραφα**: `api/{buildings,projects,properties,parking,storages}/trash/route.ts`, 79 γρ. έκαστο (**391 γρ. συνολικά**), σχεδόν byte-identical — διέφεραν μόνο σε collection, response key, sortField, permission και ουσιαστικό (399t × 4 cross-pairs, jscpd/CHECK 3.28). **Νέα**: (a) `listTrashed(db, entityType, companyId)` στο `soft-delete-engine.ts` — η 4η πράξη ζει πλέον δίπλα στις άλλες τρεις· sort στη μνήμη **σκόπιμα**, γιατί composite index `companyId+status+name` δεν υπάρχει και ένα `orderBy` θα έκανε κάθε κάδο να πετάει `FAILED_PRECONDITION`. (b) `SOFT_DELETE_CONFIG[x].trashList` — το per-entity συμβόλαιο ως δεδομένα· **optional, και το `contact` δεν το έχει**: η απουσία *είναι* η δήλωση ότι ο κάδος επαφών φιλτράρεται client-side. (c) `createTrashListRoute(entityType)` στο `src/lib/api/trash-list-route.ts` — **σύνθεση** πάνω στο `defineRoute` (ADR-602, που **καμία** trash route δεν χρησιμοποιούσε), όχι επανυλοποίηση· τα 5 routes γίνονται **17γραμμα declarations**. (d) `resolveTenantScope` στο νέο `src/lib/auth/tenant-scope.ts` — το super-admin `?companyId=` list scoping ήταν ξαναγραμμένο σε **31 αρχεία** χωρίς κανένα SSoT, και είναι **απόφαση ασφαλείας** (χαμένο `isSuperAdmin &&` = cross-tenant leak που διαβάζεται σαν κανονικός κώδικας). **Wire contract ΠΑΓΩΜΕΝΟ**: το per-entity response key (`buildings`/`parkingSpots`/…) και το `{error, details}` 500 envelope αναπαράγονται από config, γιατί το `EntityTrashSpec.selectItems` ξετυλίγει ακριβώς αυτά — ενιαίο envelope = ξεχωριστό versioned rollout. Αλλαγή **μόνο σε logs**: το properties route έχανε το prefix `[Properties/Trash]`. **Αποδεδειγμένη ισοδυναμία**: η διακλάδωση `if (isSuperAdmin && queryCompanyId) … else …` ήταν **νεκρή** (και οι δύο κλάδοι ταυτόσημοι) → κατέρρευσε. **Boy Scout (N.0.2)**: `TRASHED_STATUS` αντικαθιστά 6× το literal `"deleted"` μέσα στο engine· `loadLifecycleTarget()` εξάγει τον πρόλογο (collection lookup → existence check → tenant guard) που ήταν γραμμένος **3×** στο ίδιο αρχείο (129t + 73t, πιάστηκε από `jscpd:diff` στον 1ο γύρο) — η διαφορά έγινε **παράμετρος**, όχι rewrite: μόνο το `softDelete` περνά `isSuperAdmin`, restore/permanent-delete σκόπιμα δεν κάνουν ποτέ bypass· το `ApiError` έρχεται πλέον από `@/lib/api/api-error-types` αντί από `ApiErrorHandler`, που το re-exports αλλά σέρνει όλο το `next/server`. **Επαλήθευση: 32 νέα tests** (14 tenant-scope ασφαλείας + 18 trash-list) σε κώδικα που είχε **μηδέν**, incl. **anchor εξαντλητικότητας** που διαβάζει τα route αρχεία από το filesystem και απαιτεί το σύνολο των `createTrashListRoute('X')` να ισούται με το config → config χωρίς route ❌, route χωρίς config ❌, χειρόγραφη trash route ❌. 215/215 σε 11 σχετικά suites· `jscpd:diff` καθαρό (2ος γύρος)· registry golden 96/96· registry module `trash-list-route` (370 modules) με μετρημένα 0 false positives. **`usePropertiesTrashState` παραμένει σκόπιμα εξαιρεμένο** (0 clones — επιβεβαιώθηκε η απόφαση της 2026-07-16, δεν είναι εκκρεμότητα). **Χωρίς tsc (N.17).** |
| 2026-07-17 | Claude Code | **Ο client έφτασε τον server.** Η αρχική απόφαση («ΕΝΑΣ κεντρικοποιημένος μηχανισμός, όχι copy-paste ανά entity») είχε υλοποιηθεί στον server (`soft-delete-engine`) και στο service layer (`TrashService`), αλλά **ποτέ στα hooks/UI**: Buildings/Parking/Projects/Storages είχαν το καθένα δικό του αντίγραφο. Νέα SSoT: `hooks/trash/useEntityTrashState.ts` (η μηχανή· τα 4 hooks γίνονται thin bindings με `EntityTrashSpec`) + `components/shared/trash/EntityTrashDialogs.tsx` (soft-delete + permanent-delete μαζί — είναι **μία** ροή· κωδικοποιεί ότι ο ADR-226 guard κρατά το confirm disabled όσο ελέγχει, και ότι το permanent confirm είναι disabled σε κενή επιλογή). Η μηχανή επιστρέφει το `permanentDelete` **έτοιμο σχηματισμένο** ώστε καμία σελίδα να μην ξαναχτίζει το ίδιο literal. Συνοδά: `DistributionCard`, `useEntityCreateForm`, `useListPageHeaderState`. Οδηγός: CHECK 3.28 (jscpd/N.18) — 9 clones → 0. |
...
| 2026-04-08 | Claude Code | BUGFIX: TrashActionsBar restore/delete buttons were always disabled — required multi-select but no checkbox mechanism exists in trash view. Added `activeContactId` prop as fallback for the viewed contact. `effectiveIds` pattern derives once, used everywhere. |
| 2026-04-08 | Claude Code | BUGFIX: Trash restore succeeded (API 200) but UI didn't update — `handleContactsDeleted` only cleared `selectedContact` if in `selectedContactIds` array, but `activeContactId` fallback bypasses that array. Created dedicated `handleTrashActionComplete` in `useContactsTrashState` that ALWAYS clears selection + refreshes. Also added refresh on 409 error (stale client data recovery). |
| 2026-04-08 | Claude Code | REFACTOR: Google SSoT pattern — `contactDetailProps` derived once from `showTrash` state, spread in all 3 `ContactDetails` instances (desktop, mobile list, grid). Eliminated 3× repeated `showTrash ? undefined : ...` ternaries. |
| 2026-04-08 | Claude Code | BUGFIX: `{{count}}` rendered literally in trash strings — ICU plugin (`i18next-icu`) requires `{count}` (single braces), not `{{count}}` (i18next default). Fixed 6 keys in contacts-lifecycle + trash locale files (el/en/pseudo). |
| 2026-04-09 | Claude Code | FEATURE: Archive View for Files (Google Drive pattern) — `ArchiveView.tsx` (346 γρ.), `file-record-lifecycle.ts` (42 γρ. lifecycle service), archive/unarchive via `lifecycleState` field, batch operations support, audit trail with display names, i18n keys (el/en), EntityFilesToolbar toggle. Extends soft-delete pattern to file management. |
| 2026-04-15 | Claude Code | FEATURE: Trash UI for Storage & Parking — trash list endpoints (api/storages/trash, api/parking/trash), state hooks (useStoragesTrashState, useParkingTrashState), trash badge in StoragesHeader + ParkingsHeader, TrashActionsBar + permanent-delete dialog wired in StoragePageContent + ParkingPageContent. Both entities now have full trash view parity with Buildings/Projects/Properties/Contacts. |
| 2026-04-20 | Claude Code | BUGFIX: `SharedPropertiesProvider` Firestore snapshot included soft-deleted properties. Trashed units appeared in Sales sidebar (`/sales/available`) and anywhere consuming `useSharedProperties`. Root cause: live SSoT provider used `firestoreQueryService.subscribe` without a `status !== 'deleted'` filter, while the REST `/api/properties` route already filtered deletions. Fix: filter deleted docs in the provider snapshot callback — aligns provider with API contract and matches the pattern used in `useFirestoreBuildings`/`useFirestoreStorages`. |
| 2026-04-20 | Claude Code | BUGFIX: Permanent-delete button in properties trash view did nothing. Root cause: `usePropertiesTrashState` was missing `pendingPermanentDeleteIds` state + `handleConfirmPermanentDelete`/`handleCancelPermanentDelete` actions, and `UnitsPageContent` destructured the dialog flag as unused (`_showPermanentDeleteDialog`) with no `<DeleteConfirmDialog>` rendered. Fix: (a) hook now mirrors parking/storage pattern (SSoT) + adds ADR-226 `useDeletionGuard('property')` pre-check for single-item deletes (property is the entity with most blocking deps: accounting_invoices, opportunities, communications, contact_links, boq_items, obligations); (b) bulk path uses `Promise.allSettled` so partial failures surface via `permanentDeletePartial` notification instead of aborting the batch; (c) `UnitsPageContent` renders `<DeleteConfirmDialog>` + deletion-guard `BlockedDialog`; (d) new i18n keys `permanentDeleteSuccess_one`, `permanentDeleteFailed`, `permanentDeletePartial` in `trash.json` (el/en/pseudo). |
| 2026-04-20 | Claude Code | FEATURE: Property `permanentDelete` now covers full Firestore + Storage footprint (ADR-226 Phase 10). `executeDeletion` engine gains (a) `useCollectionGroup` cascade branch for subcollections (DXF overlay polygons κάτω από `dxf_overlay_levels/{levelId}/items`), (b) `storageCleanup` step — best-effort `bucket.deleteFiles({ prefix, force: true })` με template `companies/{companyId}/entities/property/{entityId}/` για φωτογραφίες, κατόψεις, share PDFs. Νέες cascade entries για property: `files`, `shares`, overlay items. Νέα modules `deletion-storage-cleanup.ts` / `deletion-link-guard.ts` / `deletion-common.ts` (SRP split αναγκαίος για 500-line limit στο `deletion-guard.ts`). Storage cleanup failures δεν μπλοκάρουν τη διαγραφή — καταγράφονται στο `_storage_cleanup` audit entry. `entity_audit_trail` παραμένει εκτός cascade (permanent audit log). `soft-delete-engine.ts` δεν αλλάζει — συνεχίζει να delegatει στο `executeDeletion`. |
| 2026-04-21 | Claude Code | BUGFIX: Contact permanent-delete did not cascade-delete owned file records in `files` collection. `deletion-registry.ts` contact config was missing `FILES` cascade entry (present in `property` config). Fix: added `{ collection: COLLECTIONS.FILES, foreignKey: 'entityId', label: 'Αρχεία επαφής', queryType: 'equals' }` to contact `cascadeDependencies`. 18 orphan records manually cleaned from Firestore. |
| 2026-04-21 | Claude Code | FEATURE: Google-style trash banner for contact Documents tab. When contact is in trash, Documents tab shows amber banner "Η επαφή βρίσκεται στον κάδο..." + auto-opens trash sub-tab of EntityFilesManager (existing TrashView). Flow: ContactDetails `readOnly` → `isContactTrashed` → UnifiedContactTabbedSection → RendererContext → files renderer. EntityFilesManager gains `defaultActiveTab` prop. 6 files touched: EntityFilesManager, contactRenderersCore, UnifiedContactTabbedSection, ContactDetails, el/en contacts-lifecycle.json. |
| 2026-04-21 | Claude Code | BUGFIX: Contact restore cascade — contact restore did not cascade to owned trashed files. Fix: `cascadeRestoreContactFiles()` in `contacts/[contactId]/restore/route.ts` — Admin SDK batch, queries `files` where `lifecycleState=trashed`, restores to `active` + clears all trash fields (`trashedAt`, `purgeAt`, `deletedAt` via `FieldValue.delete()`). Response includes `filesRestored` count. Mirror pattern of trash cascade (same index coverage). |
| 2026-04-22 | Claude Code | BUGFIX: Project soft-delete produced **two** `entity_audit_trail` entries per trash action (`soft_deleted` from engine @ +407ms + `deleted` from route handler @ +423ms, both `source: "service"`). Root cause: `project-mutations.service.ts#handleDeleteProject` called `EntityAuditService.recordChange({ action: 'deleted' })` AFTER `softDelete()` engine already wrote the canonical `soft_deleted` entry — violating SSoT (ADR-281 engine owns soft-delete audit) AND semantic split (`'deleted'` is reserved for permanent delete via `deletion-guard.ts`, `'soft_deleted'` for trash). Fix: removed the duplicate `recordChange` block in `project-mutations.service.ts` (lines 219-237); route now only writes legacy auth log via `logAuditEvent`. Pattern now matches sibling routes (contacts/buildings/storages/properties/parking) which all delegate entity audit to the engine. **SSoT invariant (reinforced): API routes MUST NOT call `EntityAuditService.recordChange` for soft-delete/restore/permanent-delete — the engine (`soft-delete-engine.ts`) and `deletion-guard.ts` are the sole writers.** Verified: audit trail now emits exactly one `soft_deleted` entry per trash action; History tab no longer shows ghost `deleted` row; semantic separation intact (`soft_deleted` = trash, `deleted` = hard delete). |
| 2026-04-22 | Claude Code | BUGFIX: Projects trash UI — after restore/permanent-delete from trash container, the details panel kept rendering the acted-upon project as if still selected (card gone from trash list, details sidebar stuck showing it). Root cause: `useProjectsTrashState.handleTrashActionComplete` only called `forceDataRefresh()` + `fetchTrashedProjects()` but never cleared `selectedProject` — which is a single-select state shared between active list and trash list (`TrashActionsBar` derives `selectedIds` from `selectedProject?.id`). Fix: hook now accepts `setSelectedProject` prop and clears it in both `handleToggleTrash` (view switch) and `handleTrashActionComplete` (post-action). Pattern matches `usePropertiesTrashState` which does `setSelectedProperties([])` in the same two hooks. Files: `src/hooks/useProjectsTrashState.ts`, `src/components/projects/projects-page-content.tsx`. Google-level: proactive state cleanup, no UI ghost, SSoT cleanup pattern replicated from properties. |
| 2026-05-03 | Claude Code | BUGFIX: Trash retention was using archive compliance values (10-20 years) instead of short restore-window values. `RETENTION_BY_CATEGORY` (single map used for both trash and archive) replaced by two maps: `TRASH_RETENTION_BY_CATEGORY` (30-90 days) and `ARCHIVE_RETENTION_BY_CATEGORY` (compliance-grade, 3-20 years). Consumers updated: `file-record-lifecycle.ts` + `contacts/[contactId]/route.ts`. Example: floorplans in trash now purge after 90 days, not 10 years. |
| 2026-04-21 | Claude Code | BUGFIX: Contact trash cascade — contact DELETE did not cascade to owned files. Files remained `lifecycleState: 'active'`, `isDeleted: false` (orphaned). Fix: `cascadeContactFilesToTrash()` in `contacts/[contactId]/route.ts` — Admin SDK batch, queries `files` where `companyId + entityType='contact' + entityId + lifecycleState='active'`, respects holds (skip + warn), per-category retention (RETENTION_BY_CATEGORY), chunks of 500 for Firestore batch limit. Response now includes `filesCascaded` + `filesSkipped`. Existing composite index (companyId+entityType+entityId+domain+category+lifecycleState+createdAt) covers the query. |
| 2026-07-16 | Claude Code (Opus 4.8) | **SSoT: the hook layer finally honours this ADR's own decision.** §2 mandated "ENAS kentrikopoiimenos mixanismos, oxi copy-paste ana entity" — the server (`soft-delete-engine`) and the service layer (`TrashService`) complied; the **client hooks never did**. `useBuildingsTrashState` / `useParkingTrashState` / `useProjectsTrashState` / `useStoragesTrashState` each carried their own copy of the identical state machine: **12 clones / 122 duplicated lines (21.5% of the quartet)**, measured with jscpd. Files 8+9 of §Implementation (`useSoftDelete.ts`, `useTrashView.ts`) were **never created** — the copies filled the gap instead. New engine `src/hooks/trash/useEntityTrashState.ts`; each hook is now a ~25-line binding owning an `EntityTrashSpec` (entity kind, trash route, response-key selector, optional realtime `refreshOn`) and re-exporting under its domain names (`trashedBuildings`, `handleRestoreParkingSpots`, …), so the **consumers' read API is unchanged**. **Prop API unified** — `onBeforeToggle` / `onRestoreComplete` / `onItemDeleted` / `setSelectedProject` were four names for one idea and collapse to `clearSelection`; 4 consumers updated. **Behaviour aligned, deliberately:** the selection is now cleared on view switch **and after every completed action** for all four. Buildings previously skipped it after permanent-delete and Parking skipped it entirely — that is the exact UI-ghost defect this changelog fixed for Contacts (2026-04-08) and Projects (2026-04-22); the divergence was copy drift, not intent, so the SSoT states the rule once instead of parameterising the inconsistency. **Genuine divergences kept as spec/options**, not flattened: Projects notify via the project notification SSoT (not the generic trash toast), Storages subscribe to `STORAGE_DELETED`, Buildings expose `onBuildingMovedToTrash`. **`usePropertiesTrashState` deliberately excluded** (0 clones against the quartet): different machine — `Promise.allSettled` partial-failure reporting + ADR-226 deletion guard + no auto-fetch. **`useContactsTrashState` excluded**: no fetch at all, count derived client-side. **Verified: 12 clones → 0; 55 NEW tests written against the PRE-merge hooks (they had none) and kept byte-identical across the merge → GREEN before and after, so "no behaviour change" is demonstrated, not claimed; mutation-verified ×2** (parking rebound to the buildings route/kind → 3 tests failed; engine idempotency guard removed → 1 failed; both reverted). Registry module `entity-trash-state` (pattern `setPendingPermanentDeleteIds`, properties allowlisted) → 0 violations, **no baseline needed**. Registry golden 90/90. **No tsc (N.17).** |
| 2026-09-19 | Claude Code (Opus 5) | **Θέσεις & αποθήκες: το `status` είναι πλέον ΜΟΝΟ κύκλος ζωής** (ADR-777 §8.60.20). Ήταν ανάμεικτο (εμπορικό + φυσικό + κάδος)· τώρα `RecordLifecycleStatus` = `ACTIVE_RECORD_STATUS` · `TRASHED_STATUS` (`lib/firestore/trashed-status.ts`). `defaultRestoreStatus` θέσης/αποθήκης `available` → `ACTIVE_RECORD_STATUS`· το PATCH/CREATE **αρνούνται** `status` από σώμα (`z.undefined()` — ένα `status: 'deleted'` δεν μπορεί πια να παρακάμψει τον κάδο)· οι λίστες ρωτούν `isTrashed` αντί για ωμό `'deleted'`· ο αθροιστής αναφορών χώρων **δεν μετρά πια τον κάδο**. Παλιά έγγραφα στον κάδο με ανάμεικτο `previousStatus` τα καλύπτει η συμπλήρωση (`planSpaceStatusBackfill`). |
| 2026-09-22 | Claude Code (Opus 5) | **Μπάρα κάδου: ΜΙΑ διάταξη, ΜΙΑ ροή επαναφοράς** (βρέθηκε από CHECK 3.28 στη διόρθωση πληθυντικών ICU, ADR-867). Επαφές (`contacts/trash/TrashActionsBar`) και ακίνητα (`properties/trash/PropertyTrashActionsBar`) είχαν δίδυμη ολόκληρη τη μπάρα (JSX + ροή επαναφοράς) δίπλα στη γενική `shared/trash/TrashActionsBar`. Τώρα: `shared/trash/EntityTrashActionsBar` (κανόνας στόχου `effectiveTrashIds` — επιλεγμένα > ενεργό) + `useTrashBarRestore` (log → υπηρεσία → μήνυμα επιτυχίας/αποτυχίας → ανανέωση **και σε αποτυχία**, 409) πάνω στη γενική μπάρα, που δέχεται `labels` ως **μερική** υπέρβαση. Κάθε οντότητα δηλώνει μόνο υπηρεσία + κείμενα. **Συνέπεια διατύπωσης**: μετρητής, «Επαναφορά επιλεγμένων», «Οριστική διαγραφή» έρχονται από το namespace `trash` για **όλες** τις οντότητες (ανά οντότητα μόνο «πίσω» + «προειδοποίηση»)· 5 ορφανά κλειδιά ανά γλώσσα αφαιρέθηκαν από `contacts-lifecycle`/`properties-viewer`. Το πλαίσιο προειδοποίησης παίρνει παντού περίγραμμα `--text-warning` (ήταν έτσι σε 2 από τις 3). ⚠️ Η μεταφορά επαφών/ακινήτων στη μηχανή `useEntityTrashState` μένει εκκρεμής (`.claude-rules/pending-ratchet-work.md`). Άγκυρα `shared/trash/__tests__/entity-trash-actions-bar.test.tsx` |
