...
| 2026-07-05 | **SSoT consolidation — ΕΝΑΣ canonical Object-Snap-Tracking resolver (Revit/AutoCAD-OTRACK style).** Root-cause: το «assembly» του tracking (acquired ⊕ ambient ⊕ polar → `composeTrackingSnap`, μαζί με το read του `ambientAlignmentConfigStore` για radius/maxMembers + το `polarTrackingStore` polar config) ήταν γραμμένο **δύο φορές, byte-πανομοιότυπα**: μία στον `systems/tracking/resolve-alignment-tracking.ts` (drawing + rotation, store-only + `segmentBase`) και μία στον `hooks/dimensions/dim-alignment-tracking.ts::resolveDimAlignmentTracking` (dim/action, `refPoints` + configurable tolerance). Οι μόνες διαφορές ήταν **ποια optional inputs** εξέθετε ο καθένας — κλασικό SSoT violation (μια αλλαγή στο ambient/compose wiring απαιτούσε edit σε 2 σημεία). **Fix:** ο `resolveAlignmentTracking` (systems/tracking, χαμηλότερο hooks-agnostic layer) γίνεται **ο ΕΝΑΣ** resolver — το `AlignmentTrackingInput` αποκτά optional `refPoints?` (ρητά transient anchors, `acquiredAt:0`, tag `'ref-point'`, prepended ΠΡΩΤΑ στα store points) + `matchTolerancePx?` (default `DEFAULT_ALIGN_TOLERANCE_PX=3`). Το assembly ζει ΜΙΑ φορά· κάθε context περνά per-context inputs: σχεδίαση→`segmentBase`, περιστροφή→pivot (store), διάσταση/ACTION→`refPoints`+8px pull. Ο `resolveDimAlignmentTracking` γίνεται **thin adapter** (~10 γρ.) με το εργονομικό `(cursor, refPoints, input)` signature — ΜΗΔΕΝ διπλότυπο logic. Τα `resolveActionAlignmentTracking` / `resolveDimActionEndpoint` / `paintDimActionTracking` / `paintActionAlignmentTracking` παραμένουν (θεμιτές domain συνθέσεις/paints, ΟΧΙ διπλότυπα). **Καμία συμπεριφορική αλλαγή** (byte-identical: default tol 3px, anchor order `[...refAnchors,...store]`→empty για drawing⇒store-only, ambient gating caller-side, `sourceSnapType` = label μόνο, δεν καταναλώνεται από το `resolveTrackingSnap`). Consumers αμετάβλητοι (optional additions). **+3ο προϋπάρχον διπλότυπο (3D):** το `bim-3d/placement/use-bim3d-wall-placement.ts` έκανε το ΙΔΙΟ inline assembly (collect ambient + `TrackingPointStore.getPoints()` + `composeTrackingSnap` με polar-store config) σε scene-units domain — τώρα καλεί κι αυτό τον canonical `resolveAlignmentTracking` περνώντας `scale = 1/scenePerPx` (byte-identical: `worldPerPixel(1/s)=s`, `pixelsToWorld(px,1/s)=px·s`, επιβεβαιωμένο από τους ορισμούς του `viewport-scale.ts`), `polarEnabled:false`, `sceneEntities = cfg.enabled ? entities : null`. Ο κοινός πυρήνας `composeTrackingSnap` (ADR-543 2D↔3D) παραμένει· αυτό που εξαλείφθηκε ήταν το γύρω-του assembly wrapper, πλέον σε **ΕΝΑ** σημείο για 2D+3D. Tests: νέο `systems/tracking/__tests__/resolve-alignment-tracking.test.ts` (4 ✅ — refPoints participate, default 3px reject, 8px pull, empty⇒null) + πράσινα `dim-alignment-tracking-tolerance`, `ambient-tracking-compose`, `tracking-resolver`, `body-drag-alignment-tracking`, `rotation-tracking-overlay`, `ambient-alignment-source` (37/37 σύνολο). Files: `systems/tracking/resolve-alignment-tracking.ts`, `hooks/dimensions/dim-alignment-tracking.ts`, `systems/tracking/__tests__/resolve-alignment-tracking.test.ts` (NEW). Βλ. ADR-562 Φ9. |
...
| 2026-06-21 | **Ambient alignment — zoom-adaptive search radius (Giorgio browser feedback «να προσαρμόζεται στο zoom»).** Search radius changed from a FIXED 4m world distance → SCREEN-relative: `ambient-alignment-config-store` `radiusMm`→`radiusPx` (default 400, key `dxf:ambient.radiusPx`), hover handler computes `radiusWorld = radiusPx / max(liveScale, 0.001)` (= px × worldPerPixel). So the "members near my cursor on screen" feel stays constant at every zoom (1:500 → ~52m world, 1:20 → ~2m world); `maxMembers=6` cap bounds the candidate set. Files: `systems/tracking/ambient-alignment-config-store.ts`, `hooks/drawing/drawing-hover-handler.ts`. (`getSceneUnitsScale` retained — still used for the display-unit tooltip mm conversion.) |
| 2026-06-21 | **Ambient alignment — "magic" distance snap + display-unit tooltip (Giorgio browser feedback).** (1) **Adaptive distance snap** (AutoCAD PolarSnap / Revit temp-dim, always-on): NEW pure `systems/tracking/adaptive-distance-snap.ts` — `adaptiveDistanceStep(worldPerPixel)` rounds to the nice 1/2/5/10×10ᵏ sequence sized so each step ≈25px on screen (grows zoomed-out, shrinks zoomed-in) + `quantizeAlongPath(point,anchor,dir,step)` rounds the slide distance along a PROJECTION track (intersections already fixed). Wired in `drawing-hover-handler.ts` after `resolveTrackingSnap` → quantized `trackingPoint` drives previewPt + label + overlay. 11 jest. (2) **Display-unit tooltip**: the yellow `angle° / distance` label now formats the distance via `formatDisplayValue(distMm, displayUnitState.getUnit())` + unit label (mm→world via `getSceneUnitsScale()`) → shows e.g. `0° / 2.30 m` instead of raw mm, honoring the status-bar unit SSoT. Files: `systems/tracking/adaptive-distance-snap.ts` (NEW)+test, `hooks/drawing/drawing-hover-handler.ts` (imports `displayUnitState`+`formatDisplayValue`+`DISPLAY_UNIT_LABELS`). |
| 2026-06-21 | **Ambient alignment — same-session refinements (Giorgio browser feedback).** (1) **Scope generalized columns → ALL structural members** (Giorgio: «να διαβάζει ΚΑΙ τοίχους ή άλλα δομικά»): `ambient-alignment-source.ts` filter `isColumnEntity` → `STRUCTURAL_MEMBER_TYPES` Set `{column,wall,beam,slab,foundation}`; reuse-only (`getBimCharacteristicPoints` is already a generic BIM dispatcher — zero new geometry). Renamed `maxColumns`→`maxMembers` (config store + ctx), `AMBIENT_SOURCE_TYPE` `'ambient-column'`→`'ambient-member'`. New jest: wall fixture proves non-column structural participates (13/13 ✅). (2) **Thinner alignment line**: `tracking-paint.ts` `paintAlignmentPaths` lineWidth `1`→`0.5` (affects both ambient + acquired — same SSoT renderer). Files: `systems/tracking/ambient-alignment-source.ts`, `ambient-alignment-config-store.ts`, `hooks/drawing/drawing-hover-handler.ts`, `canvas-v2/preview-canvas/tracking-paint.ts`, tracking jest ×2. |
| 2026-06-21 | **DEAD DUPLICATE POLAR/CONSTRAINTS SUBSYSTEM DELETED — single polar SSoT consolidation (Giorgio: «full enterprise + full SSoT, like Revit»).** The DXF viewer carried TWO independent ortho/polar implementations: the LIVE one (`systems/constraints/polar-utils.ts` `applyPolar`, this ADR — 3 consumers: `bim-ortho-reference`, `drawing-hover-handler`, `useDrawingHandlers`) and a DEAD legacy `ConstraintsSystem` React subsystem (own baseAngle/ortho/polar engine) with **zero live consumers** — its only entry was `utils/dynamicSystemImports.ts` (`DynamicConstraintsSystem`), itself imported by nobody. Verified by transitive grep (knip cannot help: `src/subapps/dxf-viewer/**` is in knip.json `ignore`, so CHECK 3.22 never covered this code and `.deadcode-baseline.json` is unaffected by the deletion). **14 files deleted**: `ConstraintsSystem.tsx`, `constraints-ortho-polar.ts` (the duplicate polar engine), 9 `useConstraint*`/`useOrtho*`/`usePolar*` hooks, `useConstraintsSystemState.ts`, barrel `index.ts`, `utils.ts` (backward-compat shim), and `utils/dynamicSystemImports.ts` (whole file dead — `DynamicToolbars/RulersGrid/ConstraintsSystem` all unconsumed). **Honesty finding**: `constraints-ortho-polar.ts` was NOT "0 consumers" as the handoff claimed — it had exactly one importer, the protected `utils.ts`, via a DEAD `OrthoUtils/PolarUtils` re-export tail that no live consumer used. The sole live consumer of `utils.ts` (`snapping/engines/ExtensionSnapEngine.ts`) imported only `CoordinateUtils`, whose real home is `constraints-geometry.ts`. Full-SSoT fix: redirected `ExtensionSnapEngine` to import `CoordinateUtils` directly from `constraints-geometry.ts`, then deleted both the shim (`utils.ts`) and the dead engine (`constraints-ortho-polar.ts`). **Survivors (live subgraph, untouched)**: `polar-utils.ts`, `polar-tracking-store.ts`, `cad-toggle-state.ts`, `constraints-geometry.ts`, `config.ts` + their 2 tests (`cad-toggle-state.test.ts`, `polar-utils.test.ts`, 32/32 ✅). `applyPolar` is now the ONE polar implementation. Note: the handoff referenced "ADR-065" for the dead subsystem, but `ADR-065.md` is actually about inline-id-generation — the "ADR-065 Phase 5" markers in `constraints-geometry.ts` comments are an internal split label, not a real ADR doc; there was no dedicated ConstraintsSystem ADR to mark Superseded, so this consolidation is recorded here in ADR-357 (the living polar SSoT). ✅ Google-level: YES — single source of truth (one polar engine, `CoordinateUtils` one home), zero dangling references (verified), live tests green, no production path touched (deleted code was unreachable). |
...
| 2026-06-30 | **Anti-collision των cursor tooltips (polar Φ1 ↔ object-snap-tracking Φ4) — Giorgio screenshot bug.** Όταν POLAR (F10) + AutoAlign ήταν ταυτόχρονα ΟΝ, δύο cursor-adjacent tooltips ζωγραφίζονταν σχεδόν στο ίδιο σημείο: το polar label (`paintPolarTrackingLine`, πορτοκαλί, `cursor+(14,−8)`) και το tracking distance label (`paintTooltip`, γκρι, `cursor+(14,−12)`) → διαφορά 4px → **επικάθονταν** («155° / 0,090 m» μπερδεμένο). **Fix (SSoT, μηδέν magic numbers):** NEW `CURSOR_LABEL_SLOTS` στο `overlay-text-style.ts` (ο ήδη δηλωμένος SSoT ΟΛΩΝ των overlay labels) — `above {dx:14,dy:−10}` / `below {dx:14,dy:16}` / `belowFar {dx:14,dy:42}`. Το polar tooltip πιάνει το `above`, το tracking tooltip το `below` → **στοιβάζονται** (26px gap, καθαρά) αντί να συγκρούονται. Η μη-επικάλυψη = τεκμηριωμένο συμβόλαιο, όχι σύμπτωση. **ΟΧΙ από το line-HUD** (ADR-508 §line-hud): προϋπήρχε· απλώς έγινε ορατό. **Boy-Scout (N.0.2, διαταγή Giorgio):** βρέθηκε ΤΡΙΤΟ cursor tooltip με inline `+14,+18` — το 🔴 opening-conflict tooltip (`PreviewRenderer.drawGhostConflictTooltip`, ADR-508)· έπεφτε 2px από το tracking (λανθάνουσα 2η σύγκρουση) → μπήκε κι αυτό στο SSoT (`belowFar`). Πλέον το `dx:14` ζει ΜΙΑ φορά (ήταν 3× inline). **MOD:** `overlay-text-style.ts` (+slots), `polar-tracking-line-paint.ts`, `tracking-paint.ts`, `PreviewRenderer.ts`. 36/36 preview-canvas tests GREEN. ✅ Google-level: YES — ένας ορισμός slots, και τα ΤΡΙΑ cursor tooltips τον διαβάζουν. 🔴 browser-verify + commit (Giorgio). |
| 2026-07-01 | **Contextual tab της επιλεγμένης γραμμής → AutoCAD-grade split σε ενότητες (πλήρη λεπτομέρεια: ADR-510 Φ4).** Το «Quick Style» tab (5 comboboxes σε 1 panel) στοιβαζόταν κάθετα → scroll. Σπάστηκε σε **3 panels οριζόντια** (Γενικά: Χρώμα/Επίπεδο/Διαφάνεια · Εμφάνιση: Τύπος/Πάχος/Κλίμακα+Πλάτος · Γεωμετρία **line-only**: Μήκος/Γωνία + Αρχή/Τέλος/Δ, όλα επεξεργάσιμα). Reuse του υπάρχοντος bridge/editable-numeric-combobox/`geometry-vector-utils` SSoT· NEW `systems/properties/line-geometry-edit.ts`. 71 jest GREEN. 🔴 browser-verify + commit. |
| 2026-07-04 | **Ambient alignment — scope generalized structural members → ΚΑΙ απλή γεωμετρία CAD (Giorgio browser bug: κατά τη σχεδίαση ορθογωνίου με το εργαλείο Γραμμή, το κάθετο ίχνος από το δεξί άκρο της κάτω γραμμής ΔΕΝ εμφανιζόταν).** Root cause: το `ambient-alignment-source` παρήγαγε anchors ΜΟΝΟ από `STRUCTURAL_MEMBER_TYPES` (column/wall/beam/slab/foundation) — μια **απλή γραμμή** (`type:'line'`) απορριπτόταν στο type-filter (`ambient-alignment-source.ts` παλιά γρ. 92), άρα το άκρο της δεν γινόταν ποτέ ambient anchor· ο μόνος τρόπος ήταν το AutoCAD OTRACK hover-acquire (1000ms pause, `TrackingPointStore`). Η οριζόντια που έβλεπε ο χρήστης ήταν το ortho/polar rubber-band από το `lastRefPt` του τρέχοντος τμήματος, όχι ambient anchor. **Fix (Revit-grade, full SSoT):** NEW `PLAIN_GEOMETRY_TYPES` Set `{line,polyline,lwpolyline,rectangle,arc}` + NEW helper `ambientPointsForEntity(e)` — ΕΝΑ σημείο αποφασίζει την πηγή char-points ανά type: BIM → `getBimCharacteristicPoints` (ως πριν)· plain → `GeometricCalculations.getEntityEndpoints`+`getEntityMidpoints` (**ΤΟ ΙΔΙΟ** SSoT extractor που καταναλώνουν τα Endpoint/Midpoint snap engines — μηδέν νέα γεωμετρία, μηδέν παράλληλη μηχανή). Το `nearestMembersWithinRadius` unified: `footprintBounds(points)`+`distanceToFootprintBounds` για proximity και στις δύο περιπτώσεις (τα midpoints/center είναι εντός του corners-bbox → identical bounds για BIM). Το axis-gating (`pushAxisGatedAnchors`) αμετάβλητο. Μια γραμμή δίνει 2 endpoints + 1 midpoint = 3 σημεία → ικανοποιεί το `footprintBounds` ≥3-verts guard. **Files:** `systems/tracking/ambient-alignment-source.ts` (+`GeometricCalculations` import), `__tests__/ambient-alignment-source.test.ts` (malformed `{type:'line'}` fixture → proper `lineFrom()` + 2 νέα tests: line endpoint aligned-column emits / off-axis line → []; 9/9 ✅). ⚠️ Pre-commit CHECK 6D: το `ambient-alignment-source.ts` δεν είναι στη λίστα micro-leaf, αλλά stage ADR-357 (this). ✅ Google-level: YES — ένας extractor SSoT (reuse-only), unified proximity, ένα σημείο-απόφασης πηγής ανά type, transient anchors (καμία μόλυνση store). 🔴 browser-verify + commit (Giorgio). |
| 2026-07-04 | **Status-bar SNAP ↔ POLAR menu centralization — κοινό editable numeric combobox (Giorgio: «το Snap να πάρει το μενού του Polar», κεντρικοποίηση κώδικα).** Πριν: το SNAP-step πεδίο ήταν σκέτο `StatusBarNumericField` (μόνο πληκτρολόγηση, καμία λίστα presets) ενώ το POLAR increment ήταν σκέτο Radix `Select` (μόνο επιλογή, καμία πληκτρολόγηση) → δύο διαφορετικά UI για το ίδιο μοτίβο «αριθμός με presets». **Fix (SSoT):** NEW `statusbar/StatusBarEditableCombobox.tsx` — ΕΝΑ type-to-enter numeric combobox (input + preset dropdown ▾) που καταναλώνουν **και** το SNAP (mm) **και** το POLAR increment (°). Η filter/commit λογική **επαναχρησιμοποιείται** από τον υπάρχοντα SSoT `ui/ribbon/components/buttons/ribbon-combobox-numeric.ts` (`filterNumericDraft`/`commitNumericDraft`/`ResolvedNumericConfig`, ADR-345 §4.5) — μηδέν re-implementation. Commit policy Revit: typing local, Enter/blur/preset-pick commit. **SNAP presets:** `Ελεύθερο(0)/10/25/50/100/500/1000 mm` — το `0 = Ελεύθερο` κάνει ρητά ορατή την προϋπάρχουσα δυνατότητα «SNAP on χωρίς βήμα» (`cad-toggle-state`: step 0 ⇒ no quantization). **POLAR increment:** τώρα πληκτρολογήσιμο (0–360°) + οι ίδιες quick-pick τιμές `5…90°`. **Απόφαση (Giorgio ναι/όχι):** το «Επιπλέον Γωνίες» section ΔΕΝ αντιγράφτηκε στο SNAP — γωνία = διακριτή ακτινική διεύθυνση (σύνολο έχει νόημα), βήμα SNAP = περιοδικό πλέγμα (μία τιμή, τα πολλαπλάσια βγαίνουν αυτόματα)· ούτε οι μεγάλοι το κάνουν (AutoCAD Snap = ένα X/Y spacing χωρίς additional list, Polar = additional angles). **i18n:** `cadDock.statusBar.snapStepFree` el+en. **Files:** `statusbar/StatusBarEditableCombobox.tsx` (NEW), `statusbar/CadStatusBar.tsx` (SNAP+POLAR wiring, POLAR_INCREMENT_PRESETS), `i18n/locales/{el,en}/dxf-viewer-panels.json`. `StatusBarNumericField` παραμένει (live consumer: `LinetypeScaleControl`). ✅ Google-level: YES — ένα combobox SSoT για δύο μενού, filter/commit λογική reuse-only (ADR-345), zero νέα numeric-parsing μηχανή, i18n καθαρό. 🔴 browser-verify + commit (Giorgio). |
| 2026-07-04 | **Object Snap Tracking — segment-base «clean corner» (Giorgio browser bug: σχεδιάζοντας ορθογώνιο, ευθυγράμμιση με την ΑΡΧΗ της πάνω γραμμής κλείδωνε στο 445,03 αντί 445,00 — κεκλιμένη γραμμή 179,4° αντί 180°).** Root cause (πλήρες trace): με **POLAR on** το default increment είναι **15°** (`polar-tracking-store.DEFAULT_INCREMENT`), άρα ο `tracking-resolver` χτίζει **24 paths/anchor**. Έστω κι 1 anchor → 24 paths > **`MAX_INTERSECTION_PATHS=16`** → το intersection scan **παρακάμπτεται πάντα** (γρ. 104) → πέφτει σε `findClosestProjection`. Η προβολή στο **κάθετο** ίχνος της αρχής A δίνει `(A.x, cursor.y)` — σωστό `x`, αλλά `y=cursor.y` **αντί για `C.y`** (C = σημείο εκκίνησης τρέχοντος τμήματος) → η γραμμή βγαίνει κεκλιμένη και το μήκος = `hypot(445, Δy)` = 445,03. Η καθαρή γωνία (`τομή` κάθετου-A × οριζόντιου-ορθο-C) δεν υπολογιζόταν ποτέ. **Fix (Revit/AutoCAD OTRACK model, full SSoT):** το **σημείο εκκίνησης του τρέχοντος τμήματος (rubber-band base) είναι ΠΑΝΤΑ tracking origin**. NEW `segmentBase?: Point2D|null` παράμετρος στο `resolveTrackingSnap` → NEW `buildBasePaths` (reuse `buildAlignmentPaths`, ίδιο angle-set) + NEW `findClosestBaseIntersection` (base-rays × anchor-paths, **O(n)**, **εξαιρείται** από το flood-cap) + `closerIntersection` (διαλέγει το κοντινότερο base vs anchor intersection). Έτσι η γωνία `base × ίχνος-A` σχηματίζεται ανεξάρτητα από πλήθος anchors/scene density — kind='intersection' → **χωρίς** adaptive-quantize → ακριβές `(A.x, C.y)`, μήκος 445,00. Threaded: `ambient-tracking-compose` (`ComposeTrackingOptions.segmentBase`), `resolve-alignment-tracking` (`AlignmentTrackingInput.segmentBase`), preview `drawing-hover-handler` (περνάει `lastRefPt`). **WYSIWYG parity (κρίσιμο):** το **commit path** (`useDrawingHandlers` onDrawingPoint) καλούσε `resolveTrackingSnap` με **acquired-only** (μηδέν ambient, μηδέν base, μηδέν quantize) → το κλικ ΔΕΝ κλείδωνε στην ambient-ευθυγράμμιση (το ορθογώνιο δεν έκλεινε). Ενοποιήθηκε στο **ΙΔΙΟ** SSoT `resolveAlignmentTracking` (acquired ⊕ ambient ⊕ segment-base + quantize) → preview ≡ commit· αφαιρέθηκαν dead imports `polarTrackingStore`/`resolveTrackingSnap`/`pixelsToWorld`. **Απορρίφθηκε** το εύκολο «ανέβασε το `MAX_INTERSECTION_PATHS`» — band-aid: με πυκνή κάτοψη ή μικρό polar step ξανασπάει + O(n²) κόστος. **Files:** `systems/tracking/tracking-resolver.ts`, `ambient-tracking-compose.ts`, `resolve-alignment-tracking.ts`, `hooks/drawing/drawing-hover-handler.ts`, `hooks/drawing/useDrawingHandlers.ts`, `__tests__/tracking-resolver.test.ts` (+3: clean-corner με POLAR-flood / tilted χωρίς base / null-base parity· 42/42 tracking-suite ✅). rotation-tracking-overlay & 3D wall-place: αμετάβλητα (segmentBase optional → null). ⚠️ Pre-commit 6D: stage ADR-357 (this). ✅ Google-level: YES — ένα tracking brain, base=first-class origin (O(n), εκτός cap), preview≡commit ενοποιημένο, zero νέα γεωμετρία. 🔴 browser-verify + commit (Giorgio). |
| 2026-07-05 | **§heads-up-direct-distance-entry — AutoCAD «πληκτρολόγησε→Enter→τοποθέτηση» στο Δαχτυλίδι Εντολών (πλήρη λεπτομέρεια: ADR-513).** Το Q4 (Direct Distance Entry sempre attivo) υλοποιήθηκε πλήρως στο tool-agnostic `RadialCommandRing`: με το δαχτυλίδι ενεργό & κανένα popup, ψηφίο/`.`/`,`/`-` ανοίγει **αυτόματα** το «Μήκος» seeded (NEW pure `isHeadsUpNumericKey`)· Enter → lock + synthetic `mousedown→mouseup` στον καμβά → τοποθέτηση κατά τη διεύθυνση κέρσορα μέσω του ΚΑΝΟΝΙΚΟΥ click pipeline (`applyLengthAngleLock` → preview≡commit)· Μήκος one-shot (`RingFieldDef.clearOnPlace`→`unlockLength`, η Γωνία επιμένει). Κόμμα/τελεία/math = reuse `evalExpr` (μηδέν νέος parser· το legacy `line-keyboard-handler` DOM overlay είναι πλέον dead για τη γραμμή — ADR-513 §line-parity). **Parity**: Γραμμή+Τοίχος (2D+3D) + **Δοκός** (NEW `beam-ring-config` + `applyLengthAngleLock` preview/commit — βλ. ADR-363). Orchestrator: 2 παράλληλοι subagents (ring/beam), disjoint files. jest ring 180/180 + beam 30/30 GREEN. tsc SKIP (N.17). Owner τεκμηρίωσης: **ADR-513**. ⚠️ 3D beam ring εκτός scope. 🔴 browser-verify + commit (Giorgio). | Claude Opus 4.8 |
| 2026-07-07 | **Status-bar toggles για τις οπτικές ενδείξεις της «Γραμμής» + αφαίρεση «CAD Mode \| F-keys» (Giorgio: «να ανάβουν/σβήνουν βάζοντας πλήκτρα toggle κάτω»).** Μετά από SSoT audit (grep) των 4 κατηγοριών ενδείξεων του line-tool ghost, προστέθηκαν **2 νέα status-bar toggles** + **1 sub-toggle μέσα στο OSNAP popover**, χωρίς διπλότυπα: (κατ. 2) **«ΜΗΚΟΣ/ΓΩΝΙΑ»** — λευκό HUD μήκους/γωνίας (ADR-508 §line-hud)· (κατ. 3) **«ΤΟΞΟ ΦΟΡΑΣ»** — κόκκινο/πράσινο direction-arc (ADR-508 §polyline-parity)· (κατ. 1β) **«Αποστάσεις»** — κυανές listening dimensions (ADR-508 §line-cyan), μέσα στο OSNAP popover γιατί σχετίζεται με OSNAP. **Κατ. 1α (σύμβολα έλξης) & κατ. 4 (λευκά ίχνη ευθυγράμμισης) ΔΕΝ έφτιαξαν νέα κουμπιά** — ελέγχονται ήδη από OSNAP (+per-type popover) και **AutoAlign** αντίστοιχα (full SSoT, μηδέν διπλότυπο). **Αρχιτεκτονική (reuse):** 3 νέα flags `dimHudOn`/`dirArcOn`/`listeningDimOn` στο ήδη υπάρχον δίδυμο `cad-toggle-state.ts` (non-React getters για τα gates) + `useCadToggles.ts` (React subscriber + Firestore persistence via `cadTogglesSchema` `.default(true)`) — **ακριβώς** το pattern του `dynInput`, μηδέν νέος μηχανισμός. Defaults ON (σημερινή συμπεριφορά). **Gates (single-point, non-React, διαβάζουν `cadToggleState` getters όπως ήδη το αρχείο διαβάζει `wallPreviewStore`):** HUD → `drawing-hover-overlays.ts` (`if (lineHud && isDimHudOn())` — μόνο line/polyline· ο τοίχος έχει ξεχωριστό `wallHud`)· τόξο → `drawing-hover-overlays.ts` (line/polyline όρος `&& isDirArcOn()`, τα BIM μέλη κρατούν πάντα το τόξο)· listening dims → `line-preview-helpers.ts:resolveLineFaceDims` (`if (!isListeningDimOn()) return null` — καλύπτει State A stub + State B με 1 γραμμή). **Removal:** το span `cadDock.statusBar.modeInfo` («CAD Mode \| F-keys») + το i18n key (el/en) — grep επιβεβαίωσε μοναδική χρήση. **UI:** 2× `CadToggleRow` (status-bar-only, χωρίς F-key όπως AutoAlign/ΔΥΝ) στο `CadStatusBar.tsx`· sub-toggle Button στο `ProSnapToolbar.tsx` (νέα optional props `listeningDimOn`/`onToggleListeningDim`, ΟΧΙ μέλος `ExtendedSnapType`). **i18n:** `dimHud`/`dirArc`/`listeningDim` (+Desc) el+en. **Files:** `systems/constraints/cad-toggle-state.ts`, `hooks/common/useCadToggles.ts`, `services/user-settings/user-settings-schema.ts`, `statusbar/CadStatusBar.tsx`, `ui/components/ProSnapToolbar.tsx`, `hooks/drawing/drawing-hover-overlays.ts`, `hooks/drawing/line-preview-helpers.ts`, `i18n/locales/{el,en}/dxf-viewer-panels.json`. tsc SKIP (N.17). 🔴 browser-verify + commit (Giorgio). | Claude Opus 4.8 |
| 2026-07-07 | **DDE buffer → `DirectDistanceEntry` SSoT (flagged item F, ADR-397/513 σύγκλιση).** Το `hooks/canvas/useCanvasKeyboardShortcuts.ts` κρατούσε hand-rolled `ddeBufferRef = useRef('')` + inline digit accumulation (`/^[\d.]$/`, `+= e.key`, `slice`) για το Phase 3 Direct Distance Entry — **δεν** χρησιμοποιούσε την `DirectDistanceEntry` class (ADR-344 buffer SSoT) ούτε το per-keystroke SSoT, και **δεν δεχόταν κόμμα**. **Fix (full SSoT):** το per-keystroke pure `applyTypedAngleKey` (typed-angle-entry.ts) **γενικεύτηκε** σε neutral `applyTypedNumericKey(dde, key, { allowNegative })` — η input-λογική («ψηφία/`.`/`,`/`-`/Backspace/Enter → buffer/commit») είναι domain-neutral· το `applyTypedAngleKey` έμεινε **thin alias** (allowNegative default = signed γωνία → οι 2 rotation consumers grip-hotgrip/useRotationTool byte-identical, μηδέν άγγιγμα). Το LINE DDE migrated: `ddeRef = useRef(new DirectDistanceEntry())`, ψηφία/Backspace μέσω `applyTypedNumericKey(..., { allowNegative:false })` (απόσταση πάντα θετική — κατεύθυνση από κέρσορα, AutoCAD DDE), Enter διαβάζει `snapshot()` (κενό buffer → chain-finish αμετάβλητο), reset σε tool-change/tempPoints-empty. **Κέρδος:** κόμμα-parity δωρεάν (45,5 ≡ 45.5) + ένα buffer SSoT για γραμμή & περιστροφή. **Tests:** `typed-angle-entry.test.ts` +6 (neutral + allowNegative:false απορρίπτει «-» + alias byte-identical), 26 GREEN (typed-angle+DDE) + 17 rotation GREEN. tsc SKIP (N.17). 🔴 browser-verify + commit (Giorgio). | Claude Opus 4.8 |
