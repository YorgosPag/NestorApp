...
| 2026-03-25 | Fix: ESCO skill enforcement + merge semantics — skills without URI are validated against ESCO (>1 match → reject). Skills now MERGE with existing (not replace), deduplicated by URI. Extracted `enforceEscoOccupation()` + `enforceEscoSkill()` helpers into shared module. |
| 2026-03-25 | Fix: Anti-hallucination retry — when AI claims write without tool calls, now RETRIES with forced tool instruction instead of returning generic error. Max 1 retry per loop (hasRetried flag). Defense-in-depth Layer 3. |
| 2026-03-25 | Feat: `attach_file_to_contact` tool — Admin sends photo/document via Telegram, AI links to contact. 3 purposes: profile_photo (sets photoURL), gallery_photo (appends multiplePhotoURLs), document (links FileRecord). Media attachments now flow from webhook→pipeline→agentic loop. FileRecord promoted from PENDING/ingestion→READY/admin. New `attachment-handler.ts`. |
| 2026-03-27 | Feat: `read_document` tool — On-demand document reading via OpenAI Vision API. Agent can read ANY document at any time (new uploads or existing attachments). Two modes: full content extraction (question=null) or focused Q&A (question provided). Fetches FileRecord → validates companyId → downloads file → Vision API (Responses endpoint, supports images+PDFs). Reuses `downloadAndValidateFile()`, `extractOutputText()` from existing infrastructure. New `document-reader-handler.ts`. |
| 2026-03-27 | Fix: Anti-fabrication guardrail now trusts tool results as data sources. Values extracted by `read_document` (or any tool) are legitimate — not hallucination. `isFabricatedContactValue()` accepts `trustedSources[]` from prior tool results. `checkFabricationGuardrail()` collects all tool result texts (prior iterations + current batch) as trusted context. Fixes false positive where email/phone from PDF were blocked despite user explicitly requesting "read the receipt and update the contact". |
| 2026-03-27 | Fix: Guardrail A (write-claim without tools) skips file-only messages. New `isDocumentPreviewOnly` flag in `AgenticContext`, set by path executor when user sends file without text. Describing a document analysis IS the correct response — no tools needed. Fixes false positive where "Η ανάλυση ολοκληρώθηκε" triggered forced retry, causing agent to call wrong tools (create_complaint_task, send_email) and fail. |
| 2026-03-27 | Fix: System prompt teaches agent to use `read_document` for follow-up questions about documents. Phase 1 preview is a SUMMARY — agent must call `read_document(fileRecordId)` to extract full details (phone, email, address, ΑΦΜ). Without this, agent said "δεν βρέθηκαν στοιχεία" instead of re-reading the PDF. |
| 2026-03-27 | Fix: `manage_relationship` handler — remove inverse document creation. Handler was creating 2 Firestore docs per relationship (forward + inverse with `bidirectionalRef`), but UI already queries both directions (`sourceContactId OR targetContactId`). This caused duplicates on one contact and missing relationships on the other. Now creates single doc per relationship, aligned with UI adapter pattern. Also improved UI adapter dedup to use canonical pair key (`sorted IDs + type`) as safety net for legacy bidirectional data. |
| 2026-03-29 | **MIGRATION: Vercel AI SDK** — Replaced custom agentic loop with Vercel AI SDK `generateText()` + `maxSteps`. New files: `vercel-ai-engine.ts` (drop-in replacement), `vercel-ai-tool-adapter.ts` (tool converter with guardrails via `jsonSchema()`). Legacy files preserved for rollback: `agentic-loop.ts`, `agentic-openai-client.ts`, `agentic-tool-runner.ts`. Toggle via import in `agentic-path-executor.ts`. All guardrails (A, B, anti-fabrication, anti-hallucination, FIND-T) preserved. Same model (GPT-4o-mini), same cost, same pipeline flow. Packages: `ai@6.0.141`, `@ai-sdk/openai@3.0.48` (both MIT). |
