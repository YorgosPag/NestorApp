...
- **Α15 — Είδος `spatial_tour`: προσωπικός σύνδεσμος θέασης περιήγησης** (ADR-884 Κ3β · Φ0.12). Ένας σύνδεσμος = **ένας**
  παραλήπτης με όνομα και λήξη. Ο resolver δίνει στον παραλήπτη **μόνο** τη ρίζα της περιήγησης· η κρίση και τα μέσα έρχονται
  από την **πύλη θέασης** του ADR-884 (`view-session`, βάση `link`), που απαιτεί το κουπόνι επίσκεψης **του ίδιου** συνδέσμου
  και ξαναελέγχει ότι είναι ενεργός και δείχνει αυτή την περιήγηση. `entityCollection` = το εταιρικό διαμέρισμα περιηγήσεων.
- **Α16 — Κριτής δημιουργίας ΑΝΑ ΕΙΔΟΣ** (`server/sharing/share-create-authority.ts`, `Record<ResolvableShareKind, …>`). Μέχρι
  το Α15 η απάντηση ήταν μία («ανήκει στον μισθωτή σου;»). Για επαφή/αρχείο/βιτρίνα αρκεί — ο σύνδεσμος **δείχνει**. Ο σύνδεσμος
  περιήγησης όμως **δίνει πρόσβαση**, και το «ποιος βλέπει» είναι απόφαση του **υπευθύνου** (ADR-884 §12 Δ3): ρωτά
  `locateManagedTour` πάνω στη ρίζα που γράφει το ίδιο το έγγραφο της περιήγησης. Χωρίς όψη ρόλου ⇒ άρνηση (fail-closed). Νέο
  είδος **δεν μεταγλωττίζεται** χωρίς κριτή.
- **Α17 — Πολιτική συνδέσμου ανά είδος** (`SHARE_KIND_LINK_POLICY` στο `share-resolve-contract.ts`): `password` · `labelRequired`.
  **Μία** δήλωση, τρεις αναγνώστες: ο διάλογος (κρύβει τον κωδικό, απαιτεί «για ποιον» — `ShareLinkCreatePanel` ·
  `ActiveShareLinksList` · `LinkTokenFields`), η δημιουργία (`validateAgainstLinkPolicy`) **και** η αλλαγή ρυθμίσεων
  (`share-update`: `password-not-allowed` · `label-required`) — ένα πεδίο που η οθόνη κρύβει αλλά ο διακομιστής δέχεται είναι πόρτα
  στην οθόνη, όχι στα δεδομένα. Για `spatial_tour`: **κανένας κωδικός** (κοινό μυστικό που δεν ανακαλείται ανά άνθρωπο), **υποχρεωτικό** όνομα.
- **Α18 — Ένας πυρήνας κουπονιού επίσκεψης** (`server/access-grant/access-grant.ts`): το κουπόνι Α6 έγινε καταναλωτής του, με
  **αμετάβλητο** API· δεύτερος καταναλωτής το κουπόνι θέασης του ADR-884 Φ0.4. Ίδιο μυστικό (`SHARE_ACCESS_SECRET`), ίδια λήξη 15′,
  **διαχωρισμός με τον υπογεγραμμένο σκοπό** (άγκυρα: ίδιο πλήθος πεδίων, άλλος σκοπός ⇒ τίποτα).
...
- ⏳ Ίχνος θέασης ανά σύνδεσμο («ποιος σύνδεσμος άνοιξε πότε») — σήμερα μόνο μετρητής + `lastAccessedAt`. Για το `spatial_tour`
  αρκεί, επειδή ο σύνδεσμος είναι **ανά παραλήπτη** με όνομα (Α15): «ο κ. Π. — τράπεζα, 3 ανοίγματα, τελευταίο χθες».
- ⏳ **Σύνδεσμοι για προσωπική θεματοφυλακή** (ADR-884 §9 Ε14): σήμερα οι κοινοποιήσεις είναι εμβέλειας μισθωτή — η περιήγηση ιδιώτη
  απαντά `visibility-unsupported` στο `link-only`. Ίδια ρίζα με τα «αρχεία σε προσωπική φύλαξη» παρακάτω.
...
| Ημερομηνία | Αλλαγή |
|---|---|
| 2026-09-26 | **Α15–Α18 (ADR-884 Κ3β).** Είδος `spatial_tour` (resolver · `ResolvedShareDataByKind` · `RESOLVABLE_SHARE_KINDS` · σελίδα `/shared/[token]` → `SharedTourPageContent`)· κριτής δημιουργίας ανά είδος (`share-create-authority.ts`, το `POST /api/shares` περνά την όψη ρόλου)· πολιτική συνδέσμου ανά είδος (`SHARE_KIND_LINK_POLICY` → διάλογος + δημιουργία + αλλαγή ρυθμίσεων)· `ShareMetaField` δέχεται `null` (επιφάνεια χωρίς σακούλα meta)· ο πυρήνας κουπονιού εξήχθη (`server/access-grant/access-grant.ts`, 106 τεστ κοινοποίησης αμετάβλητα)· `useShareLinks` πάνω στο κοινό `hooks/useReconciledResource.ts`. Άγκυρες: `share-create-authority.test.ts` (υπεύθυνος ✓ · μέλος χωρίς δυνατότητα · χωρίς όψη ρόλου · άλλος μισθωτής · πλαστό id · χωρίς όνομα · με κωδικό · αλλαγή: κωδικός/σβήσιμο ονόματος)· άγκυρα ένωσης 8 → 9. |
