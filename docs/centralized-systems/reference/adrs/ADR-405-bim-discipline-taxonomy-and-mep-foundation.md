...
| 2026-06-02 | 0.3 — DONE (Step 1 + §4 υλοποιημένα) | **§1-§3 Taxonomy SSoT:** NEW `bim/discipline/bim-discipline.ts` (`Discipline` union + `DISCIPLINE_BY_CATEGORY` total map + `CATEGORIES_BY_DISCIPLINE` inverse + `MODEL_DISCIPLINES` + `resolveEntityDiscipline` priority explicit>type>layer>null). `AecLayerCategory` → alias `= Discipline` (zero churn). `BaseEntity.discipline?` optional override. `STRUCTURAL_BIM_CATEGORIES` → `MODEL_BIM_CATEGORIES` (+ deprecated alias). **§4 Visibility filter:** `resolveIsEntityVisible` +5η source `disciplineVisibility` (ANY-hides-wins, annotation-exempt, explicit>type derivation)· store `disciplineVisibility` state + `setDisciplineVisibility` action (debounced+persisted)· `BimRenderSettings`/`resolveBimSettings` persistence· threaded `discipline`+`disciplineVisibility` σε ΟΛΑ τα 2D renderers (7) + 3D `BimSceneLayer` (resolveEntity + hosted openings) + envelope 2D/3D = πλήρες 2D⟷3D parity. **UI (συνύπαρξη — Revit «View Discipline» + «Μόνο DXF» = quick isolate):** NEW `DisciplineVisibilityToggle` multi-toggle (5 model disciplines) + `DISCIPLINE_PANEL` + RibbonPanel dispatch + i18n el/en. **Tests:** 74/74 PASS (mapping + 5η source + store action), tsc clean. ⚠️ 12 pre-existing failures στο `BimSceneLayer-visibility-resolver-3d.test.ts` (param-less fixtures από ADR-401/404 prior commits — ΟΧΙ regression αυτού του ADR, αποδείχθηκε με stash-compare). 🔴 browser verify εκκρεμεί. | Claude (Opus 4.8) |
