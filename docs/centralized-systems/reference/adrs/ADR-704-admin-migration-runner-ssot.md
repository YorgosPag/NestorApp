...
| 2026-09-17 | **`category` προαιρετική, τυπωμένη ΜΟΝΟ ως `'SENSITIVE'`** (ADR-862 Φ0 Β14 · CHECK 3.78 / ADR-855) — `MIGRATION_RATE_LIMIT_CATEGORY` στο `admin-migration-runner.ts`. Η διαδρομή που φτιάχνεται από το εργοστάσιο **έκρυβε** τη βαθμίδα ρυθμού από τον αναγνώστη του `route.ts` ⇒ νέα διαδρομή μπλόκαρε στο 3.78. Η δήλωση είναι πλέον ορατή, και ο τύπος κάνει **αδύνατη** τη δήλωση που διαφωνεί με το `withSensitiveRateLimit` (καμία δεύτερη τιμή δεν μεταγλωττίζεται). Πρώτος χρήστης: `backfill-project-members`. Οι 4 υπάρχουσες μένουν στη baseline. | Claude Opus 5 + Giorgio |
