...
**Status**: 🟢 IN PROGRESS — αποφάσεις LOCKED (2026-06-11), υλοποίηση σε slices. **Slice 0 + Slice 1 + Slice 2 + Slice 3 + Slice JOIN DONE** (Slice 1 rules+indexes DEPLOYED· Slice 3 + Slice JOIN εκκρεμούν browser-verify+commit). v1 (associative grid hosting + εσχάρα + follow-on-move + corner-fill γωνιών) ΟΛΟΚΛΗΡΩΘΗΚΕ. Σχέδιο: §10.
**Date**: 2026-06-11
...
- **Slice 4 — BOQ/IFC net volume (safeUnion):** η επικάλυψη στους κόμβους (`w × w` block ανά διασταύρωση, εγγενής στη μονολιθική αναπαράσταση με διακριτές λωρίδες) κάνει **double-count όγκου** στην επιμέτρηση. Λύση: boolean **union** των footprints των hosted strips (reuse `bim/walls/safe-polygon-boolean.ts::safeUnion`, polygon-clipping MIT, crash-proof) → καθαρό εμβαδό/όγκο. Offline (BOQ-time), **καμία** αλλαγή στη γεωμετρία/όψη — οι λωρίδες παραμένουν επικαλυπτόμενες (Revit/Tekla way: μοντέλο με επικαλυπτόμενα μέλη, καθαρές ποσότητες μέσω boolean). Επόμενο.
- **Slice 5 — Foundation Justification + έκκεντρα πέδιλα + συνδετήριες (tie-beams):** σήμερα οι λωρίδες είναι **κεντρικές (concentric)** πάνω στον άξονα (`w/2` εκατέρωθεν) = δομικά ιδανικό default (φορτίο από κεντροβαρικό, μηδενική εκκεντρότητα). ΑΛΛΑ σε **όριο οικοπέδου / υπάρχον κτίριο** ο μηχανικός χρειάζεται **έκκεντρο** πέδιλο (ανάπτυξη μόνο προς τα μέσα, αλλιώς το `w/2` overhang περνά το όριο). Revit-grade λύση = παράμετρος **Justification/Alignment** ανά πέδιλο (`center` default | `inner-face` περιμετρικά σε όριο | `outer-face`/custom offset) — υλοποιήσιμη μέσω του ΥΠΑΡΧΟΝΤΟΣ `GuideBinding.extend` SSoT (signed offset του coordinate σχετικά με τον άξονα = ακριβώς ο μηχανισμός εκκεντρότητας). Η εκκεντρότητα δημιουργεί ροπή → απαιτεί **συνδετήριες/strap beams** για ισορροπία (το `tie-beam` kind υπάρχει ήδη). Σχέση §8 (κεντρικά vs έκκεντρα + strap beams). Μελλοντικό — μετά απόφαση Giorgio.
