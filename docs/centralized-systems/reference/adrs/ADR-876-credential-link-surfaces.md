...
| **Σ22** | ⚠️ **ΑΠΟΦΑΣΗ GIORGIO**: ο server πάνω στον **emulator** στέλνει **πραγματικό** email μέσω Mailgun («Email sent successfully via Mailgun» — στο ανύπαρκτο `vendor+…@golden.local`). Κανένας φραγμός emulator στις ~δεκάδες διαδρομές Mailgun | **ΔΕΝ** διορθώθηκε: διατομεακό (πλατφόρμα email), εκτός ADR-876. Πρόταση: ένα SSoT «sink» στον adapter όταν `FIRESTORE_EMULATOR_HOST` |
