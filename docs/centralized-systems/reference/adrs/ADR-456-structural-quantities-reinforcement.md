...
- **2026-07-19 (item 2)** — **Extra-hoops decouple ολοκληρώθηκε + guarantee/schedule regression locks (συνέχεια του από κάτω, §3.1-3.3 του handoff).** Έκλεισε το τελευταίο coupled site: το `extraHoopMm` στο `column-reinforcement-compute.ts` **δεν** μετρά πλέον το adaptive-πυκνό display polyline (`closedPolylineLengthMm(extraStirrupPathsMm)`) — διαβάζει το **αναλυτικό** (arc-aware) μήκος ανά hoop. **(1)** NEW πεδίο `ColumnRebarLayout.extraStirrupCenterlineLengthsMm?: readonly number[]` (index-aligned με `extraStirrupPathsMm`, «geometric truth» των ποσοτήτων, decoupled από tessellation). **(2)** Το γεμίζουν οι δύο builders: **wall** (`column-wall-reinforcement.ts` — ο `boundaryHoop` επιστρέφει πλέον `{pathMm, centerlineLengthMm}` με `roundedPathCenterlineLengthMm(corners, r)`)· **multihoop** (`column-multihoop-layout.ts` — `legs.slice(1).map(l => l.centerlineLengthMm)`, rect closed-form ανά σκέλος). **(3)** Ο compute κάνει `extraStirrupCenterlineLengthsMm.reduce(...) ?? extraStirrupPathsMm.reduce(measure) ?? 0` — **fallback** σε μέτρηση μόνο όταν ο builder δεν το παρέχει (adaptive → <0,1%, back-compat). **Tests:** §3.2 guarantee (`rounded-path-centerline-length.test.ts` +3): το reported μήκος μη-ορθογωνικής διάταξης = analytic (ΑΝΕΞΑΡΤΗΤΟ tessellation) + αυστηρά > display polyline· wall extra hoops carry analytic lengths. §3.3 NEW `schedule/__tests__/schedule-preset-mappers.test.ts` (+4): `mapColumn` Γ/τοίχωμα → βάρος χάλυβα **≠** ορθογωνικό ισοδύναμο· ορθογωνική → label+βάρος (μηδέν regression). **Gates:** batched jest **GREEN** — rounded-path 7, schedule 4, multihoop+multishape 29, stirrup-types+rebar-layout 37, beam 18· `jscpd` **0 clones** (4 source αρχεία). ΣΗΜ.: το `column-rebar-layout.ts` διασπάστηκε (file-size N.7.1) σε NEW `column-stirrup-path-geometry.ts` από **παράλληλο agent** (κοινό working tree) — τα path/arc primitives re-exported από το layout module· οι imports μου παραμένουν έγκυροι. 🔴 tsc (Giorgio) + browser/C4D verify + commit.
- **2026-07-19** — **Ρεαλιστική εμφάνιση οπλισμού + αληθινές (arc-aware, decoupled) ποσότητες (αίτημα Giorgio μετά από C4D export verify).** Τρία μέρη: **(1) Στρογγυλές ράβδες:** `ROD_RADIAL_SEGMENTS` 6→**12** (`rebar-3d-shared.ts`) — τα κολωνοσίδερα είναι κυκλικά, όχι εξαγωνικά (Revit «fine»)· καθαρά cross-section, ΜΗΔΕΝ επίδραση σε μήκος/βάρος. **(2) Ομαλές καμπύλες (chord-tolerance):** το `buildRoundedStirrupPath` πυκνώνει **προσαρμοστικά** τα τόξα (`STIRRUP_ARC_MAX_CHORD_MM=4`, floor `STIRRUP_BEND_ARC_SEGMENTS=6`, cap 48) → λείες καμπύλες κάθε ακτίνας, όπως οι μεγάλοι CAD. **(3) Αληθινές ποσότητες — big-players split «εμφάνιση ≠ ποσότητα»:** NEW `roundedPathCenterlineLengthMm(corners, r)` = **αναλυτικό** μήκος άξονα (Σ ευθύγραμμα + Σ τόξα r·|da|), **ΑΝΕΞΑΡΤΗΤΟ** από το tessellation → το smoothing ΔΕΝ αλλάζει ΠΟΤΕ το βάρος χάλυβα. Αντικατέστησε το `closedPolylineLengthMm(displayPath)` (χορδές, υπο-εκτίμηση) στα coupled sites: **perimeter** (Γ/Τ/Π/N-gon), **wall** κύριο ring, **beam** stirrups. Το rect closed-form (`stirrupCenterlinePerimeterMm`) = ειδική περίπτωση του ίδιου μοντέλου. **BOQ bug fix:** το `mapColumn` (`schedule-preset-mappers.ts`) καλούσε το compute **χωρίς `section`** → κάθε μη-ορθογωνική κολώνα ανέφερε ΟΡΘΟΓΩΝΙΚΟ βάρος (παραπλανητικό)· τώρα περνά `resolveColumnReinforcementSection(p)` (ορθογωνική → ίδιο rect fast-path, μηδέν regression). **SSoT dedup (N.18):** NEW `buildRingStirrupParts` + `rebarDims` + `cornerDirs`/`normalizeToPi`/`cornerTurnAngleRad` helpers, κοινοί σε rect+perimeter engines. Tests: NEW `rounded-path-centerline-length.test.ts` (4) + adaptive assertions. Gates: layout+reinforcement+schedule **~460 jest GREEN** (batched — το PC OOM-άρει σε μαζικό run), `jscpd:diff` **καθαρό** (6 αρχεία). **ΕΚΚΡΕΜΕΙ (deferred):** extra hoops (wall boundary + multihoop extra legs, `column-reinforcement-compute.ts:205`) ακόμα μετρούν το adaptive-πυκνό display polyline → ακρίβεια <0,1% (ΟΧΙ παραπλανητικό, αλλά όχι πλήρως analytically-decoupled — χρειάζεται layout field). 🔴 tsc (Giorgio) + full test run + browser/C4D verify + commit.
