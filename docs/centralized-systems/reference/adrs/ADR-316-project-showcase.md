...
| Date | Change |
|------|--------|
| 2026-07-25 | **ADR-701 — το `snapshot-builder.ts` (F1) ΜΕΤΑΝΑΣΤΕΥΣΕ ΕΠΙΤΕΛΟΥΣ στο factory.** Ήταν η **μόνη** από τις πέντε επιφάνειες που **δεν καλούσε καθόλου** το `createShowcaseSnapshotBuilder`: ξανάγραφε χειρόγραφα και τα πέντε βήματα (doc fetch → not-found → tenant check → mapping → branding → wrap) και δήλωνε δικές του `ProjectNotFoundError`/`TenantMismatchError` που **δεν κληρονομούσαν** από τις πυρηνικές — άρα `instanceof ShowcaseEntityNotFoundError` ήταν `false` για ακριβώς μία επιφάνεια. **Αόρατο και στα δύο εργαλεία**: το CHECK 3.18 δεν είχε import να ελέγξει, το jscpd δεν έβλεπε clone επειδή τα ονόματα μεταβλητών διέφεραν· το βρήκε μόνο `diff` των τεσσάρων αδελφών. 🔴 **Η παγίδα**: το χειρόγραφο έκανε branding από το **δικό του** id (`propertyData: { projectId }`), ενώ το default του factory διαβάζει `raw.projectId` — foreign key που ένα project doc **δεν φέρει**. Σκέτη μετάβαση θα έσβηνε σιωπηλά το branding σε **κάθε PDF έργου**· λύθηκε με ρητό `resolveBranding` override + νέο `entityId` στο `BrandingResolutionParams`, καρφωμένο με test στα ίδια τα ορίσματα της κλήσης. Το `labels.ts` (F1) δηλώνει πλέον `PROJECT_SPEC_ROWS` και ο `ProjectShowcaseSpecLabels` **παράγεται** από τη λίστα. Wire contract παγωμένο. Λεπτομέρειες: ADR-701. |
