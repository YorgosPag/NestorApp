...
### Q-11: Στυλ σχεδίασης τοίχων

**Απόφαση**: **Και τα δύο, σταδιακά** (Επιλογή Γ)

| Φάση | Στυλ τοίχων | Λεπτομέρεια |
|------|-------------|-------------|
| **Phase 1** | Μονή γραμμή (σκίτσο) | Δωμάτιο = closed polyline (ορθογώνιο). Δουλεύει αμέσως με existing tools, δεν χρειάζεται geometry engine. |
| **Phase 2** | Διπλή γραμμή με πάχος | Τοίχος = wall object με configurable πάχος (π.χ. 25cm εξωτερικός, 10cm εσωτερικός). Απαιτεί geometry engine (Section 6.5) για intersections γωνιών, T-junctions. |
| **Μετάβαση** | "Μετατροπή σε τεχνικό σχέδιο" | Η AI μετατρέπει αυτόματα μονές γραμμές σε διπλές — ο χρήστης σκιτσάρει γρήγορα, μετά λέει "κάνε το τεχνικό". |

**Λογική**: Ακολουθεί τη φυσική ροή εργασίας μηχανικού — πρώτα concept (σκίτσο), μετά τεχνικό σχέδιο.

---

### Q-12: Error Handling AI

**Απόφαση**: **Η AI είναι βοηθητική, ποτέ εμπόδιο.** 4 σενάρια αποτυχίας:

| Σενάριο | Αντίδραση συστήματος |
|---------|---------------------|
| **Timeout** (>60s) | Μήνυμα "Η εντολή πήρε πολύ χρόνο — δοκίμασε ξανά ή απλοποίησε" |
| **Ασαφής εντολή** | Η AI ρωτάει πίσω: "Τι ακριβώς θέλεις; Γραμμή, κύκλο, δωμάτιο;" |
| **Λάθος αποτέλεσμα** | Ο χρήστης λέει "Αναίρεσε" → undo λειτουργεί πάντα |
| **API down** (outage) | Μήνυμα "Η AI δεν είναι διαθέσιμη". Ο DXF Viewer λειτουργεί κανονικά — τα χειροκίνητα εργαλεία δουλεύουν πάντα. |

**Αρχή**: Αν πέσει η AI, δεν πέφτει ο viewer.

---

### Q-12b: AI Learning & Feedback System

**Απόφαση**: Ενιαίο feedback system + ξεχωριστό knowledge base ανά τομέα.

**Πώς "μαθαίνει" η AI** (3 μηχανισμοί):

| Μηχανισμός | Περιγραφή | Εφαρμογή |
|-----------|-----------|----------|
| **System Prompt** | Οδηγίες που δίνουμε στην AI σε κάθε κλήση | Ήδη υπάρχει — βελτιώνεται συνεχόμενα |
| **Feedback (👍/👎)** | Ο χρήστης αξιολογεί τις απαντήσεις AI | Υπάρχει στο Telegram — αποθηκεύεται στο Firestore → αναλύουμε → βελτιώνουμε prompts |
| **Knowledge Base (RAG)** | Αρχεία γνώσης που η AI διαβάζει ως context πριν απαντήσει | Νέο — ξεχωριστά αρχεία ανά τομέα |

**ΣΗΜΑΝΤΙΚΟ**: Τα LLMs δεν "μαθαίνουν" από τη χρήση. Δεν αλλάζουν. Η "μάθηση" γίνεται μέσω:
- Βελτίωση system prompts (από εμάς, βάσει feedback)
- Εμπλουτισμό knowledge base (νέα αρχεία γνώσης)
- Ανάλυση feedback patterns (ποιες εντολές αποτυγχάνουν συχνά)

**Αρχιτεκτονική**:
- **Feedback**: Ενιαίο σύστημα για ΟΛΗ την εφαρμογή (Telegram, Email, DXF)
- **Knowledge Base**: Ξεχωριστό ανά τομέα

**Planned file structure**:
```
src/services/ai-pipeline/knowledge/
├── general/           — Γενική γνώση εφαρμογής
├── dxf-drawing/       — CAD tools, entity types, drawing rules
├── building-code/     — ΝΟΚ, κανονισμοί
└── feedback/          — Αποθήκευση & ανάλυση feedback (👍/👎 + σχόλια)
```

---

### Q-13: Budget AI — Smart Model Routing

**Απόφαση**: **Smart routing** — αυτόματη επιλογή μοντέλου ανάλογα με πολυπλοκότητα εντολής.

| Τύπος εντολής | Μοντέλο | Κόστος/εντολή | Ποσοστό |
|---------------|---------|---------------|---------|
| **Απλή** (γραμμή, κύκλος, ορθογώνιο, undo, μέτρηση) | gpt-4o-mini | ~$0.001 | ~80% |
| **Σύνθετη** (ανάλυση, κάτοψη, vision, building code) | gpt-4o | ~$0.02-0.05 | ~20% |

**Εκτιμώμενο κόστος** (100 εντολές/ημέρα): ~$15/μήνα

**Routing logic** (server-side):
- Ο server αποφασίζει μοντέλο βάσει: τύπου εντολής, ύπαρξης εικόνας (vision), αριθμού entities στο canvas
- Ο χρήστης δεν βλέπει / δεν επιλέγει μοντέλο — είναι αυτόματο

---

### Q-14: Multi-level (Όροφοι)

**Απόφαση**: **Σταδιακά** — Phase 1 τρέχον level μόνο, Phase 2 level awareness.

| Φάση | Συμπεριφορά |
|------|-------------|
| **Phase 1** | Η AI σχεδιάζει στο τρέχον ενεργό level. Ο χρήστης αλλάζει level χειροκίνητα. |
| **Phase 2** | Level awareness — η AI καταλαβαίνει: "σχεδίασε στον 2ο όροφο", "αντίγραψε κάτοψη σε νέο level", "τι υπάρχει στο ισόγειο;" |

---

### Q-15: Αλληλεπίδραση με Imported DXF

**Απόφαση**: Η AI τροποποιεί **ΟΛΑ** τα entities (imported + δημιουργημένα), με ασφαλιστικές δικλείδες.

| Κανόνας | Περιγραφή |
|---------|-----------|
| **Πλήρης πρόσβαση** | Η AI μπορεί να τροποποιήσει, μετακινήσει, διαγράψει οτιδήποτε |
| **Πάντα undo-able** | Κάθε ενέργεια αναιρείται (Command Pattern) |
| **Επιβεβαίωση σε bulk** | Αν η AI πρόκειται να αλλάξει πολλά imported entities → ζητάει επιβεβαίωση πρώτα ("Θα αλλάξω 15 imported entities — σίγουρα;") |

---

### Q-15b: Μελλοντική Ανεξαρτησία AI — Δικό μας Μοντέλο

**Απόφαση**: **ΜΕΛΛΟΥΣΑ ΕΡΓΑΣΙΑ** — Σταδιακή απεξάρτηση από paid AI providers.

**Στόχος**: Δικό μας fine-tuned μοντέλο σε δικό μας server, **ελάχιστο ή μηδενικό κόστος API**.

**Στρατηγική 3 βημάτων**:

| Βήμα | Πότε | Τι κάνουμε | Κόστος AI |
|------|------|-----------|-----------|
| **1. Τρέχουσα κατάσταση** | Τώρα | OpenAI μέσω Vercel AI SDK + μαζεύουμε δεδομένα (εντολές, feedback, αποτελέσματα) | ~$15/μήνα API |
| **2. Fine-tune open-source** | 6-12 μήνες | Εκπαιδεύουμε Llama/Mistral (MIT, δωρεάν) με τα δικά μας δεδομένα CAD/αρχιτεκτονικής | $200-2000 εφάπαξ |
| **3. Self-hosted** | 12-24 μήνες | Τρέχουμε δικό μας μοντέλο σε δικό μας server (cloud GPU ή dedicated) | **~€50-100/μήνα server** (ΟΧΙ API fees) |

**Γιατί είναι εφικτό**:
- Τα open-source μοντέλα (Llama 4, Mistral) είναι **δωρεάν** (MIT license)
- Το **Vercel AI SDK** υποστηρίζει τοπικά μοντέλα μέσω Ollama — αλλαγή = 1 γραμμή κώδικα
- Τα δεδομένα εκπαίδευσης **τα μαζεύουμε ΗΔΗ** (feedback, εντολές, knowledge base)

**Φθηνότερη/δωρεάν λύση**: Self-hosted server με open-source μοντέλο → **μηδέν API fees**, μόνο κόστος server.

**Προϋποθέσεις** (τα χτίζουμε ΤΩΡΑ):
- ✅ Vercel AI SDK (provider-agnostic) — αποφασίστηκε Q-13 / Section 6.6
- ✅ Feedback collection (👍/👎) — αποφασίστηκε Q-12b
- ✅ Knowledge base (RAG) — αποφασίστηκε Q-12b
- ⏳ Συλλογή training data (εντολές χρηστών + σωστές απαντήσεις)

> **ΣΗΜΑΝΤΙΚΟ**: Δεν χρειάζεται $100M+ — αυτό κάνουν μόνο Google/Meta/OpenAI. Εμείς κάνουμε **fine-tuning** πάνω σε υπάρχον μοντέλο, που κοστίζει ελάχιστα.

---

### Q-16: Canvas State Format — Τι "βλέπει" η AI

**Απόφαση**: **3 επίπεδα context**, αυτόματο routing με 3 απλούς κανόνες στον server.

| Επίπεδο | Τι στέλνεται | Πότε (κανόνας) | Tokens |
|---------|-------------|----------------|--------|
| **Minimal** | Summary: "12 entities (5 γραμμές, 3 ορθογώνια...)" | Νέα σχεδίαση, δεν αναφέρεται σε υπάρχοντα | ~100 |
| **Standard** | Summary + ορατά entities (position, type, dimensions) | Αναφέρεται σε υπάρχον ("αυτό", "μετακίνησε", "παράλληλη") | ~500-1000 |
| **Full** | Summary + screenshot (Vision API) | Ζητάει οπτική ανάλυση ("τι βλέπεις;", "ανάλυσε") | ~2000+ |

**Routing**: Deterministic κώδικας (3 κανόνες), ΟΧΙ απόφαση AI. Ο server αποφασίζει, ο χρήστης δεν ασχολείται.

**90%+ των εντολών** θα πάει Minimal ή Standard.

---

### Q-17: Snap Integration

**Απόφαση**: **ΟΧΙ snap** στα AI-generated points.

| Τρόπος σχεδίασης | Snap |
|-------------------|------|
| **Χειροκίνητα** (ποντίκι) | ✅ Ναι — ο χρήστης χρειάζεται βοήθεια ακρίβειας |
| **AI-generated** | ❌ Όχι — η AI δίνει ακριβείς συντεταγμένες server-side μέσω geometry engine |

**Λογική**: "Ορθογώνιο 4x3m" → ο server υπολογίζει ακριβώς 4000mm × 3000mm. Δεν χρειάζεται snap — τα νούμερα είναι ήδη σωστά.

---

### Q-18: Auth & Permissions

**Απόφαση**: **Feature flag τώρα**, role-based αργότερα.

| Φάση | Μηχανισμός | Πρόσβαση |
|------|-----------|----------|
| **Αρχικά** | Feature flag (on/off στα settings) | Μόνο ο admin (Γιώργος) |
| **Αργότερα** | Role-based permissions | Admin + εξουσιοδοτημένοι μηχανικοί |

---

### Q-19: Offline / Degraded Mode

**Απόφαση**: **Η AI είναι προαιρετική λειτουργία, ΟΧΙ εξάρτηση.**

Αν πέσει το AI API:
- ✅ Χειροκίνητα εργαλεία σχεδίασης → δουλεύουν πάντα
- ✅ Import/Export DXF → δουλεύει πάντα
- ✅ Layers, levels, undo/redo → δουλεύουν πάντα
- ⚠️ AI Chat Panel → "Η AI δεν είναι διαθέσιμη αυτή τη στιγμή"

---

### Q-20: Server Timeout — Streaming Responses

**Απόφαση**: **Streaming** — progressive entity-by-entity rendering.

| Προσέγγιση | Περιγραφή |
|-----------|-----------|
| ~~Batch (ΟΛΑ μαζί)~~ | ~~Ο server τελειώνει ΟΛΗ την εντολή → στέλνει μαζί~~ — Κίνδυνος timeout, κακό UX |
| **Streaming (σταδιακά)** | Κάθε entity στέλνεται αμέσως μόλις δημιουργηθεί → εμφανίζεται στο canvas |

**Πλεονεκτήματα**:
- Κάθε chunk < 60s → δεν χτυπά Vercel timeout
- Ο χρήστης βλέπει σταδιακά το σχέδιο (progressive rendering)
- Μπορεί να πατήσει "Cancel" αν κάτι πάει λάθος, χωρίς να περιμένει

---

### Q-21: DXF Export

**Απόφαση**: **Ναι, αλλά ΟΧΙ στο Phase 1.**

| Φάση | Export |
|------|--------|
| **Phase 1** | Όχι — εστιάζουμε στη σχεδίαση μέσω AI |
| **Phase 2-3** | Ναι — export σε DXF αρχείο ώστε να ανοίξει σε AutoCAD/BricsCAD |

**Λόγος**: Το DXF export απαιτεί σωστή μετατροπή entities σε DXF format (layers, linetypes, blocks). Ξεχωριστή εργασία.
...
> **ΑΠΟΦΑΣΗ (ενημέρωση μετά C-02)**: Phase 1 = **OpenAI SDK μόνο**. Δεν εισάγουμε Vercel AI SDK τώρα — δεν χρειάζεται. Αν αργότερα χρειαστεί αλλαγή provider, τότε εισάγουμε Vercel AI SDK (μικρή αλλαγή). Η αρχιτεκτονική μας (server-side tool calling) δεν εξαρτάται από το SDK — η μετάβαση είναι εύκολη.
...
|---|---------|---------|--------|
| **Q-11** | **Στυλ σχεδίασης τοίχων**: Και τα δύο σταδιακά — μονή γραμμή (Phase 1), διπλή με πάχος (Phase 2), αυτόματη μετατροπή | Section 0 → Q-11 | ✅ ΑΠΑΝΤΗΘΗΚΕ |
| **Q-12** | **Error handling AI**: 4 σενάρια — timeout→μήνυμα+retry, ασαφής→AI ρωτάει πίσω, λάθος→undo, API down→viewer λειτουργεί κανονικά χωρίς AI | — | ✅ ΑΠΑΝΤΗΘΗΚΕ |
| **Q-13** | **Budget AI κλήσεων**: Smart routing — απλές εντολές→gpt-4o-mini (80%), vision/σύνθετα→gpt-4o (20%) | Section 6.6 | ✅ ΑΠΑΝΤΗΘΗΚΕ |
| **Q-14** | **Multi-level (όροφοι)**: Phase 1→AI σχεδιάζει στο τρέχον level μόνο. Phase 2→level awareness (αλλαγή, αντιγραφή κάτοψης σε νέο level). | Section 2.1 | ✅ ΑΠΑΝΤΗΘΗΚΕ |
| **Q-15** | **Αλληλεπίδραση με imported DXF**: AI τροποποιεί ΟΛΑ (imported + δικά μας), πάντα undo-able, επιβεβαίωση σε bulk αλλαγές imported | UC-DXF-011 | ✅ ΑΠΑΝΤΗΘΗΚΕ |
| **Q-16** | **Canvas state format**: 3 επίπεδα (Minimal/Standard/Full), αυτόματο routing με 3 απλούς κανόνες server-side | Section 6.7 | ✅ ΑΠΑΝΤΗΘΗΚΕ |
| **Q-17** | **Snap integration**: ΟΧΙ snap στα AI-generated points — η AI δίνει ακριβείς συντεταγμένες, snap μόνο για χειροκίνητη σχεδίαση | Section 6.3 | ✅ ΑΠΑΝΤΗΘΗΚΕ |
| **Q-18** | **Auth & permissions**: Feature flag τώρα (admin on/off), role-based αργότερα όταν μπουν μηχανικοί | Q-01 | ✅ ΑΠΑΝΤΗΘΗΚΕ |
| **Q-19** | **Offline/degraded mode**: Ο DXF Viewer λειτουργεί 100% κανονικά χωρίς AI — η AI είναι προαιρετική, ΟΧΙ εξάρτηση | Section 6.6 | ✅ ΑΠΑΝΤΗΘΗΚΕ |
| **Q-20** | **Server timeout**: Streaming responses — entity-by-entity progressive rendering, κάθε chunk <60s | Section 6.7 | ✅ ΑΠΑΝΤΗΘΗΚΕ |
| **Q-21** | **DXF Export**: Ναι αλλά ΟΧΙ Phase 1 — export σε DXF αρχείο στο Phase 2-3 | UC-DXF-016 | ✅ ΑΠΑΝΤΗΘΗΚΕ |
| **Q-22** | **Layer Architecture**: Ιεραρχικά layers με "/" separator + tree view UI. Naming: "Τοίχοι/Εξωτερικοί" → parent "Τοίχοι", child "Εξωτερικοί". Toggle/lock parent → επηρεάζει όλα τα children. Μέγιστο βάθος 2 επίπεδα. Default layers σε κάθε νέο project. Η AI τοποθετεί entities στο σωστό layer αυτόματα. Ο χρήστης δημιουργεί ελεύθερα δικά του layers. | Q-08, ADR-186 #4 | ✅ ΑΠΑΝΤΗΘΗΚΕ |
| **Q-23** | **Real-time Constraint Monitoring**: Η AI δεν παρακολουθεί κάθε κίνηση (ακριβό). Αντ' αυτού: Επίπεδο 1 (Phase 2) = ο Geometry Engine ελέγχει κανόνες ΝΟΚ τοπικά σε κάθε drop (δωρεάν, <10ms) — π.χ. "ο τοίχος είναι στα 2.1m, ελάχιστη δ=2.5m". Επίπεδο 2 (Phase 3+) = ο χρήστης ρωτάει AI για σύνθετη συμβουλή ("τι προτείνεις;"). Ο Geometry Engine κάνει μαθηματικά, η AI μπαίνει μόνο για ερμηνείες. | ADR-186, Q-09 | ✅ ΑΠΑΝΤΗΘΗΚΕ |

### Αντιφάσεις που χρειάζουν διευκρίνιση

| # | Αντίφαση | Πού βρέθηκε | Πρόταση |
|---|----------|------------|---------|
| **C-01** | **Μονάδες: m vs mm** — 3 στρώματα: χρήστης μιλάει σε m, AI επεξεργάζεται σε mm integers, canvas εμφανίζει σε m. Αυτόματη μετατροπή, ο χρήστης δεν βλέπει ποτέ mm. | Q-07 vs 6.7 | ✅ ΛΥΘΗΚΕ |
| **C-02** | **Vercel AI SDK**: ΟΧΙ τώρα. Phase 1 = OpenAI SDK μόνο. Vercel AI SDK εισάγεται αργότερα ΜΟΝΟ αν χρειαστεί αλλαγή provider. | Section 6.6 vs ADR-171 | ✅ ΛΥΘΗΚΕ |
...
| 2026-02-18 | **Bug fix #6: AI ρωτούσε αντί να σχεδιάζει** — Σε φυσική γλώσσα χωρίς ρητές συντεταγμένες (π.χ. "γραμμή μήκους 20m") η AI ρωτούσε "πού θέλεις;" αντί να σχεδιάσει. FIX: System prompt αναδιατυπώθηκε — AI αποφασίζει αυτόνομα θέση (default 0,0 ή κοντά σε υπάρχοντα). Προστέθηκαν explicit examples φυσικής γλώσσας. | Claude |
| 2026-02-18 | **Bug fix #7: draw_polyline missing from VALID_TOOL_NAMES** — Το `draw_polyline` tool προστέθηκε στο Phase 1b αλλά η `VALID_TOOL_NAMES` set στο API route δεν ενημερώθηκε → polyline tool calls αγνοούνταν σιωπηλά. Fix: Προσθήκη `'draw_polyline'` στο validation set. | Claude |
| 2026-02-18 | **Enhancement: Multi-tool calls** — System prompt ενθαρρύνει πλέον πολλαπλά tool calls σε μία απάντηση (π.χ. "2 παράλληλες γραμμές" → 2× draw_line). | Claude |
| 2026-02-18 | **Bug fix #8: Colors overridden by toolbar** — `applyCompletionStyles()` (ADR-056) αντικαθιστούσε ΠΑΝΤΑ το χρώμα entity με αυτό του toolbar. AI λέει `#FF0000` (κόκκινο) → γίνεται πράσινο. Fix: Αποθήκευση AI color πριν completion, restore μετά + re-emit drawing:complete. | Claude |
| 2026-02-18 | **Enhancement: Geometry formulas** — System prompt περιέχει πλέον cos/sin τιμές (30°/45°/60°), τύπους κανονικών πολυγώνων (πεντάγωνο/εξάγωνο), diameter→radius. Η AI μπορεί τώρα να υπολογίσει γωνίες και πολύγωνα. | Claude |
| 2026-02-18 | **Enhancement: Mini agentic loop** — API route μετατράπηκε από single-call σε mini loop (max 3 iterations). Όταν OpenAI γυρίσει tool_calls, στέλνουμε simulated results πίσω → η AI συνεχίζει και καλεί περισσότερα tools. Λύνει "2 παράλληλες γραμμές = 1 μόνο". | Claude |
| 2026-02-18 | **Bug fix #9: React batching — μόνο τελευταίο entity επιβιώνει** — `completeEntity()` σε loop: κάθε `getScene()` επέστρεφε stale state → μόνο το τελευταίο entity καταγράφηκε. ROOT CAUSE: React batches `setScene()` calls. FIX: Batch insert — `getScene()` ΜΙΑ φορά, add ALL entities σε array, `setScene()` ΜΙΑ φορά. Αφαίρεση import `completeEntity`. | Claude |
| 2026-02-18 | **Enhancement: draw_shapes compound tool** — Νέο 6ο tool (`draw_shapes`) που δέχεται array shapes σε ΜΙΑ κλήση. Αποφεύγει τα parallel tool call limitations του gpt-4o-mini. System prompt αναδιατυπώθηκε: ΠΑΝΤΑ draw_shapes για 2+ σχήματα. | Claude |
| 2026-02-18 | **Enhancement: Prompt hardening** — Sanitization layer names (prompt injection protection), DATA delimiters, αύξηση limits (50 entities, 2000 chars), cos/sin precision 6 decimals, validation rules section, spatial awareness rules. | Claude |
| 2026-02-18 | **Phase 1c: draw_regular_polygon tool** — Νέο 7ο tool — ο ΚΩΔΙΚΑΣ υπολογίζει ακριβείς κορυφές κανονικών πολυγώνων μέσω `Math.cos/Math.sin` (full JS precision). Λύνει bug πενταγώνου με άνισες πλευρές (AI υπολόγιζε χειροκίνητα). Προστέθηκε Π shape example στο prompt. 8 tools συνολικά. | Claude |
| 2026-02-18 | **Bug fix #10: Stale closure — entities δημιουργούνταν αλλά δεν εμφανίζονταν** — Μετά `await fetch()` στο `useDxfAiChat`, React μπορεί να re-render, αλλά η async function κρατούσε παλιές αναφορές `getScene`/`setScene` στο closure. Ο executor διάβαζε stale scene data → entities χανόντουσαν. ROOT CAUSE: `getLevelScene` (useSceneManager) ήταν `useCallback([levelScenes])` — η closure κλείδωνε τα παλιά scenes. FIX: (1) Refs σε `useDxfAiChat` για getScene/setScene/levelId/messages (πάντα latest). (2) `getLevelScene` τώρα διαβάζει μέσω `levelScenesRef.current` αντί closure variable. | Claude |
