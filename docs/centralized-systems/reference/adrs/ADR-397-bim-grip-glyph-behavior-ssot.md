...
## 15. Changelog
- **2026-07-06 (Opus 4.8) — Σ3 FIX: το πληκτρολογημένο typed angle κλειδώνει ΜΟΝΟ με Enter, ΠΟΤΕ με κλικ (Giorgio, console diagnostic).** Ο Giorgio ανέφερε «μόλις πατήσω το 4 (χωρίς Enter) κλειδώνει». **Console diagnostic (`[ROT-DIAG]` σε 5 σημεία) απέδειξε την αιτία:** το ψηφίο «4» μπαίνει σωστά στο buffer (`typedRotate.deg=4`, μόνο PREVIEW), αλλά ένα **πραγματικό terminal `mouseup`** (`dispatchDiscreteEvent → onGripMouseUp → runGripMouseUp`) καλεί `commitFreeRotate`, το οποίο —με την αρχική Σ3 σημασιολογία «κλικ == Enter» (`if (typedRotateDeg != null) commitTypedRotate`)— οριστικοποιεί τη **μερική** τιμή 4°, αφού το `typedRotate.deg` γίνεται non-null **από το 1ο ψηφίο**. **FIX:** αφαιρέθηκε το «κλικ == Enter» για την πληκτρολόγηση. Το keyboard typed angle **δεν** περνά πλέον ως commit-on-click· αντ' αυτού το `handleMouseUp` στέλνει `keyboardAngleEntryActive = typedRotate?.deg != null` και το `commitFreeRotate` κάνει **early-return** (no-op) όσο πληκτρολογείς → το φάντασμα δείχνει τη γωνία ζωντανά ως PREVIEW, αλλά **μόνο το Enter** (`runHotGripKeyDown` → `commitTypedRotate`) οριστικοποιεί (ESC/Backspace ακυρώνει/επαναφέρει). Ισχύει είτε Δυν.Εισ. ON είτε OFF (keyboard path). **Το «Δαχτυλίδι Εντολών» (ADR-513 §rotation-ring) μένει ανεπηρέαστο:** commit-on-click = ΜΟΝΟ η δική του `RotationRingStore.getLockedDeg()` (ολοκληρωμένη γωνία από το popup Enter → synthetic click)· `keyboardAngleEntryActive` false τότε → κανονικό commit. **MOD:** `grip-hotgrip-actions.ts` (commitFreeRotate early-return + `HotGripActionCtx.keyboardAngleEntryActive`), `grip-mouse-handlers.types.ts` (`GripMouseUpCtx` field + `typedRotateDeg` semantics = ring-only), `useUnifiedGripInteraction.ts` (handleMouseUp: `typedRotateDeg: RotationRingStore.getLockedDeg()` + `keyboardAngleEntryActive`). 1150 grip+dynamic-input jest GREEN. Λεπτομέρειες: **ADR-513 §rotation-ring**. 🔴 browser-verify + commit.
- **2026-07-06 (Opus 4.8) — Σ3 typed-angle: single-slice «Γωνία» ring feed (ADR-513 §rotation-ring, cross-link).** Ο typed rotation angle μηχανισμός (Σ3 — `typedAngleDeg` οδηγεί ghost+τόξα μέσω `buildRotateReferencePreview` + commit μέσω `commitTypedRotate`/`rotateDeltaForAngleDeg`) αποκτά **δεύτερη πηγή εισόδου** πέρα από το keyboard `DirectDistanceEntry`: το «Δαχτυλίδι Εντολών» ΠΕΡΙΣΤΡΟΦΗΣ (όλος ο δίσκος = 1 πλήκτρο «Γωνία», Δυν.Εισ. ON). **Μηδέν αλλαγή στη Σ3 μηχανή** — γέφυρα `RotationRingStore` (bridge, `systems/dynamic-input/rotation-ring-store.ts`)· το `useUnifiedGripInteraction` τροφοδοτεί το ΙΔΙΟ `typedAngleDeg` (`typedRotate?.deg ?? ringLockedDeg`) σε preview+commit· το keyboard DDE **νικά** όταν υπάρχουν και τα δύο. Enter στο ring → synthetic canvas click → `commitFreeRotate`→`commitTypedRotate` (ΕΝΑ commit path). Λεπτομέρειες + files: **ADR-513 §rotation-ring**. 192 dynamic-input jest GREEN. 🔴 browser-verify + commit.
- **2026-07-05 (Opus 4.8) — Πορτοκαλί POLAR γραμμή ΚΑΙ στο 2-click ROTATE tool (ADR-572 §8.3, parity με hot-grip).** Το hot-grip rotation έδειχνε ήδη την πορτοκαλί POLAR γραμμή (εγγραφή 2026-07-01 παρακάτω, `rotation-tracking-overlay`)· το **toolbar 2-click ROTATE** (`useRotationTool` + preview `useRotationPreview`) είχε μόνο το angle-lock core (χωρίς οπτική γραμμή). **Fix (χειρουργικό, 1 αρχείο κώδικα, μηδέν νέα μηχανή):** στο `useRotationPreview.draw` (angle phase, μέσα στο `useCanvasGhostPreview` RAF harness — leaf, ΟΧΙ orchestrator) recompute του polar snap 1:1 από τον live cursor με το ΙΔΙΟ SSoT `resolveOrthoPolarStep` που τρέχει το commit (`handleRotationMouseMove`) → paint μέσω του ΙΔΙΟΥ `paintPolarTrackingLine` (σχεδίαση & hot-grip). **Polar-only** (όχι AutoAlign λευκές — το 2-click commit δεν κάνει alignment, θα έσπαγε το preview≡commit). `stepped` ως cursor-anchor = ακριβώς το committed target· η γωνία της γραμμής (`polarResult.snappedAngle`) ταυτίζεται με το `currentAngle` (το `applyAlongAxisStepSnap` αλλάζει απόσταση, όχι γωνία). Label distance mm = `sceneDistanceToMeters(polarResult.distance, units) * 1000` (ίδιο chain με hot-grip). Zero νέα subscription (μόνο `cadToggleState.isX()` getters + pure resolver μέσα στο RAF draw — CHECK 6C ασφαλές). No-op όταν F10 off. Co-staged: **ADR-040** + **ADR-572**. ✅ Google-level: YES — reuse ολόκληρου του SSoT painter (μηδέν διπλότυπο), preview≡commit εγγυημένο, no-op-safe. 🔴 browser-verify (2-click ROTATE → F10 ON → πορτοκαλί γραμμή pivot→cursor· F10 OFF → καμία) + commit.
