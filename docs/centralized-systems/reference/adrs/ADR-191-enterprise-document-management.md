...
| Date | Change |
|------|--------|
| 2026-09-18 | 📒 **ADR-866 Φ0 βήμα 2β.4 — ΔΡΑΣΤΗΡΙΟΤΗΤΑ ΑΡΧΕΙΟΥ ανά κάτοχο** (ADR-866 §2.6.11 · Ε-Φ0-3/4/5). Το `file_audit_log` αποκτά **προσωπικό διαμέρισμα** `file_audit_log_personal` κατά το πρότυπο του `audit-ledger.ts` (`FILE_AUDIT_COLLECTION satisfies CustodyPartition` στο `lib/files/file-custody.ts`): **ένα** σύστημα (λεξιλόγιο `FileAuditAction` ανέγγιχτο · σχήμα · αναγνώστης · `AuditLogPanel`), δύο βιβλία· το προσωπικό το διαβάζει **μόνο** ο κάτοχος (ούτε super admin). **Γραφέας διακομιστή** `recordFileAudit` δέχεται `CustodyScope` (οι εταιρικοί καλούντες αμετάβλητοι). **Κάδος · επαναφορά · μετονομασία** μέσω του **ενός** γραφέα `services/file-record/file-activity-commit.ts`: για προσωπικό αρχείο η αλλαγή **και** η γραμμή της σε **μία ατομική δέσμη**, και οι κανόνες ζευγαρώνουν τις δύο πλευρές (`personalActivityPaired` · `personalActivityMatchesChange`, `get`/`getAfter`) ⇒ η δραστηριότητα ούτε πλαστογραφείται ούτε παραλείπεται· για εταιρικό ό,τι ίσχυε (δηλωμένα — ο κανόνας `files` δίνει στον δημιουργό κάδο «χωρίς claims»). Κλείνουν οι **4** δηλωμένες σιωπές (`logForCustody` διαγράφηκε · `recordPurgeAudit` · `recordTrace` · `FilePreviewPanel`). 🔴 Βρέθηκαν και διορθώθηκαν: η **εταιρική** γραμμή εκκαθάρισης γραφόταν **χωρίς** `companyId` (αόρατη σε όλους) · ο **κάδος δεν ήταν ιδεμπότητος** (10/15 ζωντανές γραμμές `delete` ήταν ζεύγη, με μετάθεση `purgeAt`) · η **εξαγωγή ΓΚΠΔ** διάβαζε μόνο το εταιρικό βιβλίο (και με `limit(500)`). ΓΚΠΔ διαγραφή: προσωπικό βιβλίο **σβήνεται** (πλην αρχείων σε δέσμευση), εταιρικό ανωνυμοποιείται. Νέος δείκτης `file_audit_log_personal (userId, fileId, timestamp ↓)`. |
| 2026-09-17 | 🗂️ **ADR-866 Φ0 βήμα 2β.3β — ΕΚΔΟΣΕΙΣ ανά κάτοχο** (ADR-866 §2.6.10 · Ε-Φ0-1 «εκδόσεις ΝΑΙ, φάσεις CDE ΟΧΙ»). Ο **ΕΝΑΣ** γραφέας δοχείου δέχεται πλέον **και** το διαμέρισμα `files_personal`: ο δράστης (`ContainerActor`) κουβαλά **κάτοχο** (`custody: CustodyScope`) αντί `companyId`, και το διαμέρισμα του αιτήματος **παράγεται** από αυτόν (`COLLECTIONS[FILE_COLLECTION[kind]]` στο σημείο κλήσης) ⇒ δομικά αδύνατο να κριθεί το ένα διαμέρισμα και να γραφτεί το άλλο. Ο ιδιώτης παίρνει καθεστώς `versions-only` με **δικό του** λόγο (`personal-custody`) στην ίδια ένωση του ADR-862 §5.3.7: `supersede` ✅, οι πράξεις φάσης ⇒ ονομασμένη άρνηση. Η **στοίβα εκδόσεων** (`readVersionStack`) και ο **προβιβασμός** (`promoteVersion`) ζητούν κάτοχο και **δεν διασχίζουν** διαμερίσματα· ο διάδοχος γεννιέται από τον **ίδιο** builder με το overload **ανθρώπου** (κανένα `companyId`, κανένα πεδίο CDE, ρίζα Storage `people/`). Οι τρεις διαδρομές (`versions` · `versions/promote` · `cde`) πέρασαν στην πόρτα διαμερίσματος `withFileCustodyAuth` με **κοινό** PEP (`resolveContainerFile`), και το `VersionHistory` ξεκλειδώθηκε για προσωπικά αρχεία — κατεβάζοντας πλέον με `id` αντί `downloadUrl` (η υποσυλλογή `versions` είχε καταργηθεί· το σχόλιο που δικαιολογούσε την εφεδρεία ήταν **μπαγιάτικο**, και η εφεδρεία φυλάει μισθωτή αλλά **όχι** δοχείο/ρίζα). Νέος δείκτης `files_personal (supersededByFileId, userId)`. **Καμία** αλλαγή στο `firestore.rules`. Άγκυρα **Α34** (15 tests) + **Α2.ε** + emulator **Π6** — μεταλλάξεις **9/9 κόκκινες**, emulator **104/104**. |
| 2026-09-17 | 🗂️ **ADR-866 Φ0 βήμα 2β.3α — ΛΗΨΗ · ΕΚΚΑΘΑΡΙΣΗ · ΓΚΠΔ ανά κάτοχο** (ADR-866 §2.6.9). Λήψη μέσω διακομιστή με `?custody=` (πόρτα `api/files/_shared/file-custody-route.ts`: εταιρεία = ίδιο `withAuth`, άνθρωπος = μόνο `uid`)· προσωπικό αρχείο κρίνεται από κάτοχο εγγράφου **και** ρίζα Storage `people/{uid}`. `file-purge.job` σαρώνει `files` **και** `files_personal`· `purgeFileRecord` με υποχρεωτικό `custody`· `api/files/purge` = προσαρμογέας του job (ήταν αντίγραφο). ΓΚΠΔ διαγραφή/εξαγωγή: κοινός σαρωτής `file-subject-scan.ts` + πόρτα `gdpr-subject-route.ts` (δέχεται πολίτη). Νέοι δείκτες `files (isDeleted,purgeAt)` — έλειπε — και `files_personal` ×2. |
| 2026-09-17 | 🗂️ **ADR-866 Φ0 βήμα 2β.2 — ΓΕΝΝΗΣΗ · ΛΙΣΤΑ · ΚΑΔΟΣ ανά κάτοχο.** `EntityFilesManager` δέχεται `custody: FileCustody` (17 σημεία απόδοσης). Το χωνί `buildPendingFileRecordData` γεννά εταιρικό (`companyId` + `cdeReadReach`) **ή** προσωπικό (`userId`, κανένα πεδίο CDE) αρχείο. Κάθε πράξη μόνο-`fileId` παίρνει **υποχρεωτικό** διαμέρισμα (`CustodyKind`), που το αποδεικνύει το ίδιο το έγγραφο (`lib/files/file-custody`). Για προσωπικό κάτοχο κρύβονται οι ενέργειες γραφείου (AI, διαβάθμιση, αρχειοθέτηση, ZIP, εκδόσεις, κοινοποίηση, σχόλια, έγκριση, ISO 19650). Κοινά πλέον: `useLifecycleFileList` + `LifecycleListFrame` (κάδος/αρχειοθήκη) και `services/upload/utils/resumable-upload.ts`. Βλ. ADR-866 §2.6.8 · §7 · §12. |
| 2026-09-17 | 🗂️ **ADR-866 Φ0 βήμα 2β.1 — ΔΕΥΤΕΡΟ ΔΙΑΜΕΡΙΣΜΑ `files_personal`.** Ίδιο `FileRecord`, κάτοχος `userId` αντί `companyId` (`lib/workspace/custody-scope.ts`). Νέο `COLLECTIONS.FILES_PERSONAL` + `tenant-config` `mode:'userId'` + κανόνας `match /files_personal` (μόνο ο κάτοχος, ούτε super_admin· χωρίς πεδία CDE από πελάτη). Η λίστα πεδίων θεματοφυλακής και η φρουρά έγιναν **καθολικές** (`cdeCustodyKeys()` · `cdeCustodyUnchanged()`) — ο κανόνας `files` ανέγγιχτος στη συμπεριφορά. Προσωπικά αρχεία = **εκδόσεις ναι, φάσεις CDE όχι** (ADR-866 Ε-Φ0-1). Επόμενα: γέννηση/λίστα/κάδος (2β.2), εκδόσεις (2β.3). Βλ. ADR-866 §2.6.7 · §5.2 · §12. |
...
| 2026-04-20 | **BUG FIX — PDF classification `classification_failed`** — `OpenAIAnalysisProvider.ts` inviava `input_file.file_data` come base64 grezzo (senza prefisso `data:`) per file non-immagine (PDF, TXT, CSV). OpenAI Responses API richiede `data:{mimeType};base64,{b64}`. Fix: riga 237 ora usa `buildFileDataBuffer(input.content, input.mimeType)` identico al path `input_image`. |
| 2026-04-10 | **Phase 4.3 — Shared Preview SSoT Consolidation** — Κεντρικοποίηση του file preview rendering σε ένα μόνο component (`FilePreviewRenderer`) που χρησιμοποιείται **και** από τον authenticated file manager (`FilePreviewPanel`) **και** από την public share page (`SharedFilePageContent`). Νέο SSoT registry `src/lib/file-types/preview-registry.ts` εκθέτει `getPreviewType()` + `getFileCategory()` + `getFileCategoryI18nKey()`. Υποστηριζόμενα previews (ίδια σε public/private): PDF (pdfjs canvas), εικόνες (zoom/rotate), video, audio, DOCX (client-side `docx-preview`), unsupported fallback. **Διαγράφηκε** το `src/app/api/shared/[token]/file/route.ts` (Office Online Viewer proxy hack) — το DOCX rendering γίνεται πλέον client-side, δουλεύει σε localhost και production ομοιόμορφα. Αρχεία: `src/lib/file-types/preview-registry.ts` (NEW), `src/components/shared/files/preview/FilePreviewRenderer.tsx` (NEW), `src/components/file-manager/FilePreviewPanel.tsx` (refactor), `src/components/shared/pages/SharedFilePageContent.tsx` (refactor). |
| 2026-05-03 | **BUG FIX — Thumbnail falsely deleted by orphan cleanup** — `onStorageFinalize` (orphan-cleanup.ts) extracted fileId as `fileName.split('.')[0]`. For thumbnail files (`file_xxx_thumb.webp`) this yielded `file_xxx_thumb` instead of `file_xxx` → `findFileOwner()` found no claim → thumbnail deleted seconds after upload. Fix: added early `return` when `fileName.includes('_thumb.')` — covers both `.webp` (images/PDFs) and `.png` (DXF) companion thumbnails. |
| 2026-04-21 | **ADR-296 — File-Type Classification SSoT Unification** — Μετά από 31 commits μέσα στην ημέρα (~10 fix για drift μεταξύ τριών παράλληλων MIME lists σε `route.ts` / `useFileClassification.ts` / `preview-registry.ts`), ενοποίηση σε νέο SSoT module `src/config/file-types/classification-registry.ts`. Ο νέος registry ορίζει μία εγγραφή ανά file family (pdf, docx, xlsx, dxf, svg, txt, xml, html, …) με `mimeTypes` + `extensions` + `classifiable` + `previewType` + `category` + προαιρετικό `extractor` + προαιρετικό `mediaDocumentType`. Public API: `isAIClassifiable`, `getPreviewType`, `getFileCategory`, `getMediaDocumentType`, `getExtractorKind`, `specForFile`. Τα τρία legacy modules έγιναν thin adapters (preview-registry από 164 → 28 LOC, re-export). Extractor dispatch στο `classify-background.ts` πέρασε σε `runExtractor(kind: ExtractorKind)` switch με TS exhaustive check. Νέα pre-commit enforcement: (1) CHECK 3.19 ratchet στο `.ssot-registry.json` (module `file-type-classification`) μπλοκάρει re-declare των `CLASSIFIABLE_TYPES`/`IMAGE_MIME_TYPES`/`DOCX_MIME`/κτλ, (2) Jest suite `extractor-registry.test.ts` εγγυάται κάλυψη για κάθε `ExtractorKind` + κάθε `*-extractor.ts` wired, (3) Jest suite `FilePreviewRenderer.invariant.test.ts` εγγυάται ότι κάθε `PreviewType` literal έχει branch στον renderer (catches bug pattern από commit `1d522810`). 80 tests, ~8s full run. Behaviour change: zero. |
