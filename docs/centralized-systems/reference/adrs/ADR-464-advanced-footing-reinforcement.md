...
**Status:** 🟡 Slices 0-1-1b-2-3 COMMITTED 2026-06-16 (Opus). Slices 4-5 DONE 2026-06-16 (Opus, UNCOMMITTED — browser-verify + commit). Slice 4 = tributary load takedown (grid half-spacing + AreaLoadSettings + «Υπολογισμός Φορτίων»)· Slice 5 = raft bearing parity + detail-sheet design summary. Slice 0 = doc/types/soil-setting· Slice 1 = loads model + bearing engine (EC7) + bearingInadequate diagnostic· Slice 1b = foundation panel UI (σ_allow + φορτία + bearing readouts, browser-testable)· Slice 2 = flexure engine (EC2 §9.8.2) + ενοποιημένος top-mesh κανόνας (suggester) + padEccentricHogging warning· Slice 3 = punching (EC2 §6.4) + one-way shear (EC2 §6.2.2) + column-dims resolve μέσω organism FK + punchingInadequate/oneWayShearInadequate diagnostics (318 structural jest GREEN). Slices 4-5 PENDING.
...
| **2** | Flexure + ενοποιημένος top-mesh κανόνας + `padEccentricHogging` + auto top-mesh | 🟢 **DONE** (UNCOMMITTED) |
| **3** | Punching + one-way shear + diagnostics | 🟢 **DONE** (UNCOMMITTED) |
| **4** | Tributary load takedown (`load-takedown` grid half-spacing + `AreaLoadSettings` G/Q + `ComputeTakedownLoadsCommand`/`useStructuralLoadTakedown` + ribbon «Υπολογισμός Φορτίων» + area-load panel fields + `AppliedMemberLoad.source` manual-guard) | 🟢 **DONE** (UNCOMMITTED) |
| **5** | Raft bearing parity (`raft-bearing` uniform pressure + runner loop) + detail-sheet design summary (`footing-detail-design-summary` checks table) | 🟢 **DONE** (UNCOMMITTED) |
...
## 5. Changelog
- **2026-06-17 (Opus) — Slice 4 browser-verify fix:** Το `useBuildingStoreyCount` διάβαζε `ProjectHierarchy.selectedBuilding.floors` (κενό στο standalone DXF route → storeyCount=0 → takedown πάντα 0). Διορθώθηκε στο σωστό SSoT `useFloorsByBuilding(activeLevel.buildingId)` (ίδια πηγή με τα storey-aware θερμικά/ADR-448). + toast feedback `bim:structural-loads-computed` (success «N πέδιλα» / info διαγνωστικό όταν 0 — χωρίς feedback ήταν σιωπηλό). **Verified browser:** N=596 kN auto, p_max=489 kPa, αξιοποίηση 245% (πέδιλο 1.5×1.5 ανεπαρκές → ορθά flagged). UNCOMMITTED.
- **2026-06-16 (Opus) — Slice 5 (Raft Parity + Design Summary):** Νέο `footing-design/raft-bearing.ts` (`computeRaftBearing`: μέση πίεση εδαφόπλακας = (area-load resultant + ίδιο βάρος)/A vs σ_allow, uniform M=0· reuse `computeBasePressure`/`makeDesignCheck`/`buildSlabFoundationSectionContext`/`areaLoadResultant`· αδρανές χωρίς σ_allow/area-loads). `runFootingDesignChecks` +optional `RaftLoadContext` 4ο arg → loop `isFoundationSlabEntity` → `bearingInadequate` (boy-scout `pushBearingInadequate` SSoT, κοινό pad+raft). `useStructuralOrganism` περνά storey-count (`useBuildingStoreyCount`) + area loads. **Detail summary:** νέο `detail-sheet/footing-detail-design-summary.ts` (πίνακας ελέγχων Έδραση/Διάτρηση/Τέμνουσα + util% + OK/! χρωματισμένο + γραμμή «άνω σχάρα» όταν hogging· mirror schedule layout). Wired στο `footing-detail-sheet` (optional `design` → κάτω τμήμα ζώνης οπλισμού) + `FootingDesignSummaryLabels` + `FoundationDetailHost` (computeFootingDesign όταν σ_allow). Raft punching/κάμψη = DEFER. i18n el/en. 10 νέα jest (raft 5 + summary 3 + …). UNCOMMITTED.
- **2026-06-16 (Opus) — Slice 4 (Tributary Load Takedown):** Νέο `loads/load-takedown.ts` (pure SSoT: `computeGridTributaryAreas` grid half-spacing ανά άξονα [edge=mirror, μεμονωμένος=`DEFAULT_BAY_SPAN_M`, irregular Voronoi=DEFER] + `areaLoadResultant` [A×όροφοι×load, κοινό με raft] + `computeMemberTakedown` source='takedown' concentric + `toAppliedTakedownLoad` Firestore-safe). `footing-design/footing-load-takedown.ts` (entity-aware: tributary ανά κολώνα → πέδιλο μέσω FK `footingId` + ίδιο βάρος κολώνας × όροφοι· `isTakedownWritable` gate). `AppliedMemberLoad.source` (`manual`/`takedown`) — ο takedown ΠΟΤΕ δεν αντικαθιστά χειροκίνητο (panel σφραγίζει `manual`). `AreaLoadSettings` (`deadAreaLoadKpa`/`liveAreaLoadKpa` building-level στο `structural-settings` + store setters + `raw()` omit-when-invalid + service round-trip). `ComputeTakedownLoadsCommand` (undoable, mirror AutoReinforce) + `useStructuralLoadTakedown` hook (+ `useBuildingStoreyCount` counted-storeys SSoT) + ribbon «Υπολογισμός Φορτίων» (Ανάλυση) + event map `bim:compute-loads-requested`/`bim:structural-loads-computed` + callbacks + organism re-derive. Panel building-level area-load combos (G/Q) στο «Φορτία & Έδραση». i18n el/en. 15 νέα jest. **Tributary απόφαση: grid half-spacing (Revit-grade), Giorgio-εγκεκριμένη.** UNCOMMITTED.
- **2026-06-16 (Opus) — Slice 3 (Punching + One-way Shear):** Νέο `footing-shear.ts` (SSoT v_Rd,c EC2 §6.2.2 `concreteShearResistanceMpa` [C_Rd,c·k·(100ρl·fck)^⅓ ≥ v_min, k≤2, ρl≤2%] — κοινό one-way & punching· + `computeFootingOneWayShear` στην κρίσιμη διατομή d) + `footing-punching.ts` (EC2 §6.4: u1=2(cx+cy)+2π·2d, soil-relief V_Ed,red capped στο ίχνος, β=1.15 §6.4.3(6), `computeFootingPunching`). Orchestrator → `{bearing,flexure,punching,oneWayShear}`. **Column dims resolve** μέσω explicit FK `ColumnParams.footingId` (organism explicit-FK-wins· `resolveSupportingColumnDims` στο input-builder· `entities?` optional → UI bearing readout αμετάβλητο)· ρl από stored/suggested footing reinforcement· concreteGrade=DEFAULT (DEFER per-footing input). Diagnostics `punchingInadequate`/`oneWayShearInadequate` (errors) + i18n el/en. Boy-scout SSoT `makeDesignCheck` (3 checks). Αδρανές χωρίς attached κολώνα (advisory). β πλήρες από M/W1 + eccentric punching = DEFER. 318 structural jest GREEN (12 νέα). UNCOMMITTED.
- **2026-06-16 (Opus) — Slice 2 (Flexure + Top-Mesh + Eccentric-Hogging):** Νέο `footing-design/footing-flexure.ts` (EC2 §9.8.2: M=p·a²/2 στην παρειά κολώνας → As=M/(z·fyd) κάτω ανά διεύθυνση· hogging όταν αποκόλληση e>kern· `FlexureResult`). Εξαγωγή **SSoT κατανομής πίεσης** `computeBasePressure` από το `footing-bearing` (το μοιράζονται bearing SLS + flexure ULS, `BearingResult extends BasePressure`, +export `KERN_RATIO`, μηδέν διπλότυπο N.0.2). Orchestrator `computeFootingDesign` → `{ bearing, flexure }`· `FootingDesignInput.coverMm` (από code limits). **Ενοποιημένος top-mesh κανόνας** στον suggester pad branch (`padNeedsTopMesh`): `thickness ≥ padTopMeshMinThicknessMm` (επιδερμικός, EC2 §9.7/§7.3.3) **ή** `eccentricityRatio > padTopMeshKernRatio` (kern) → `topMesh` = κάτω σχάρα (mirror raft). 2 νέα `FootingReinforcementLimits` πεδία (600mm / 1/6, 2 providers). `PadSectionContext.eccentricityRatio?` (από `appliedLoad` SLS στο `section-context`). Auto-reinforce συμπεριλαμβάνει topMesh αυτόματα (μέσω `buildReinforcePatch`→suggester). Diagnostic `padEccentricHogging` (warning) στον `footing-design-checks` + i18n el/en. **Default 1.5×1.5×0.5 κεντρικό → καμία άνω σχάρα (μηδέν regression, verified jest)**. Column dims=0 → πρόβολος συντηρητικά πλήρης (real dims = Slice 3 via organism FK). 306 structural jest GREEN (16 νέα). UNCOMMITTED.
