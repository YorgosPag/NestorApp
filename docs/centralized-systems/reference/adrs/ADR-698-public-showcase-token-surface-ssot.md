...
| 2026-09-25 | **ADR-884 Φ0.12 (Κ4) — η επίλυση διακριτικού πέρασε στη ΜΙΑ πύλη κοινοποίησης.** Το `lookupPublicShowcaseShare` είναι πλέον λεπτός προσαρμογέας πάνω στο `server/sharing/share-gate.ts`: αναζήτηση με `tokenHash` (μεταβατικά και `token`) σε `shares` **και** `file_shares`, λήξη + όριο, και **κουπόνι κωδικού** — το API σέρβιρε showcase με κωδικό **χωρίς να τον ζητά** (401 πλέον). Επιστρέφει ονομασμένη ετυμηγορία (`PublicShowcaseLookup`) + `publicShowcaseRefusalResponse` (404/401/410/503). Τα factories δέχονται `shareEntityType` αντί για hook `resolveShare`· το `incrementCounter` καταργήθηκε (μετρητής σε **συναλλαγή** πριν τη ροή του PDF, όχι fire-and-forget μετά). **Η ιδιοκτησία μετανάστευσε κι αυτή** — το §4 «property genuinely differs» ισχύει πλέον μόνο για κεφαλίδα/όνομα αρχείου/payload. Νεκρό `loadShareByToken` διαγράφηκε. Κανένα ωμό διακριτικό σε log. Tests: `public-share-lookup.test.ts` (node). |
| 2026-09-25 | **Ένα προοίμιο (CHECK 3.28).** Κενό token → 400 · χωρίς Admin SDK → 503 · άρνηση πύλης → η κατάστασή της: ζει πλέον **μία** φορά στο `openPublicShowcaseShare` (`api/public-share-lookup.ts`)· τα `create-public-payload-route` / `create-public-pdf-route` το καλούν αντί να το αντιγράφουν. Το τοπικό `jsonError` του payload route αντικαταστάθηκε από το κοινό του `shared-pdf-proxy-helpers`. Μαζί: `createShowcasePdfPreSubmit` (`components/sharing/showcase-pdf-pre-submit.ts`) — το `preSubmit` «PDF πριν τον σύνδεσμο» που έγραφαν κατά λέξη `BuildingDetails` και `project-details`· `useDelegatedSave` / `useInlineEditSession` / `useShowcaseDialog` για τις σελίδες λεπτομερειών κτιρίου · θέσης στάθμευσης · αποθήκης. |
