...
| | |
|---|---|
| **Status** | 🔵 PROPOSED (Milestones 1 + 1.5 + 2 + 3 + 4 υλοποιημένα) |
| **Date** | 2026-07-13 |
...
- **Υποψήφια migration στους νέους SSoT** (baselined ως γνωστές παραβάσεις του ratchet, ΟΧΙ regression):
  (α) `bim-family-type-service.ts` + `stair-presets-service.ts` χτίζουν χειροκίνητα τα ίδια user/company/
  project buckets → θα μπορούσαν να συνθέσουν τον `ScopedLibraryService`. (β) `utils/geometry/GeometryUtils.ts`
  → `entityToSegments()` (μερικό: μόνο line/polyline/arc) → thin adapter πάνω στο `entityToPolylines`.
  Και τα δύο εκτός scope του M1–M4 (Boy-Scout όταν αγγιχτούν).
