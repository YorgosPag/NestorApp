...
- **2026-06-16** — **Phase 4c DONE — 3Δ render datum SSoT για τα floor-relative hang-down/centred solids** (uncommitted). **Σύμπτωμα (Giorgio):** στη στάθμη **Θεμελίωση** ένα δοκάρι ορατό στο 2Δ ήταν **αόρατο στο 3Δ**, ενώ τα πέδιλα φαίνονταν. **Διάγνωση (Firestore + cut-plane math, ΟΧΙ μαντεψιά):** τα `beamToMesh`/`slabToMesh`/`buildMultiLayerSlabSolid` τοποθετούσαν το **floor-relative** top (`params.topElevation`/`levelElevation`) με datum το **building base (world 0)** — **έλειπε το `floorElevationMm`** — ενώ `columnToMesh`/`wallToMesh` ΚΑΙ όλοι οι MEP point-converters το προσθέτουν. Σε ισόγειο (`floorElevationMm=0`) αόρατο· στη θεμελίωση (FFL world −1000mm) το δοκάρι έμπαινε 1m πιο ψηλά (world +0.5…+1m) και το **View-Range cut plane** (`computeCutPlaneWorldY` = `(floorElevationMm+cutPlaneMm)·0.001` = (−1000+1000)·0.001 = **world 0**) το έκοβε· τα πέδιλα (απόλυτο `topElevationMm`, world ~−1m) ήταν κάτω από το plane → ορατά. **FULL SSoT fix:** NEW `hangDownMeshY`/`floorBaseMeshY`/`centeredMeshY` στο `bim-3d/converters/bim-three-shape-helpers.ts` (ΕΝΑ κατακόρυφο-datum SSoT για top/bottom/centre anchors). **MOD** `bim-three-structural-converters.ts` (beam+slab → `hangDownMeshY`· slab signature +`floorElevationMm`), `slab-multilayer-solid-3d.ts` (+`floorElevationMm` → `hangDownMeshY`), `BimSceneLayer.ts` (`syncSlabs` περνά `ctx.floorElevationMm`), `bim-three-point-converters.ts` (οι ΗΔΗ-σωστοί MEP converters δρομολογήθηκαν μέσα από τα ίδια helpers → καθαρό SSoT, **μηδέν αλλαγή συμπεριφοράς**). **Safe by construction:** σε ισόγειο `floorElevationMm=0` → ταυτόσημο με το legacy formula (μηδέν regression)· το slab preview path (`bim3d-preview-rebuild`) καλεί με default 0 (active floor) → αμετάβλητο. **Tests:** NEW `bim-vertical-datum.test.ts` (7 pure-helper: foundation −1000 → 1m lower & ≤ cut plane· legacy invariants) + `units-canonical-mm.test.ts` (+2 floor-relative slab/column integration). 12 GREEN· ΟΛΟ το converter `__tests__` = 301 passed (2 pre-existing wall-tilt failures ΑΣΧΕΤΑ — επιβεβαιώθηκε με stash). Env caveat ΜΕΡΟΣ-4 (fetch stub) για τα converter suites. 🔴 browser-verify (δοκάρι/πλάκα στη θεμελίωση & σε άνω ορόφους εμφανίζονται 3Δ στο σωστό ύψος) + commit (shared tree· git add ΜΟΝΟ δικά μου). (Opus 4.8)
