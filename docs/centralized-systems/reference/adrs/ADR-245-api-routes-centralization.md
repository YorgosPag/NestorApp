...
| 2026-07-29 | **Νεκρή δικλείδα 503 — 17/22 αρχεία μεταφέρθηκαν στο `requireAdminFirestore()`.** Αφαιρέθηκαν 23 σημεία `if (!db) throw new ApiError(503, …)` που **δεν εκτελούνταν ποτέ** (το `getAdminFirestore()` επιστρέφει `Firestore`, όχι nullable, και **πετάει** σε αποτυχία αρχικοποίησης ⇒ ο πελάτης έπαιρνε 500 ενώ η διπλανή γραμμή υποσχόταν 503). **5 αρχεία δεν μπόρεσαν** — βλ. γραμμή από κάτω. Παράπλευρα de-duplications που επέβαλε το CHECK 3.28: `fetchParkingSpotsWhere` (parking — ο αποκλεισμός soft-deleted ήταν αντιγραμμένος σε 2 διαδρομές φίλτρου), `requireMilestoneOfBuilding` (milestones — ο έλεγχος ιδιοκτησίας 403 ήταν αντιγραμμένος σε update+delete), `requirePropertyInScope` (properties — η σειρά 404-πριν-από-tenant-scope ήταν αντιγραμμένη σε PATCH+DELETE). Στο `parking/route.ts` μεταφέρθηκαν και οι 7 υπόλοιπες κλήσεις: μία διαδρομή με 503 και επτά με 500 στο ίδιο αρχείο είναι χειρότερη από την ομοιόμορφη ασυνέπεια. | Claude Code |
| 2026-07-29 | **⚠️ Όριο του CHECK 3.28 στο ίδιο έργο** — 5 αρχεία (`construction-baselines/route`, `construction-phases/route`, `construction-resource-assignments/route`, `showcase/generate/route`, `properties/[id]/route`) μπλοκάρονται από **προϋπάρχοντες** εσωτερικούς κλώνους, άσχετους με την αλλαγή. Η οδηγία του backlog «η απάντηση είναι χωριστά commits» **δεν ισχύει γι' αυτά**: ο κλώνος είναι *εντός* του αρχείου, οπότε μπλοκάρουν και μόνα τους. Στο `properties/[id]/route.ts` ο εναπομείνας κλώνος (58 tokens) είναι **η υπογραφή του Next.js route handler** (`withStandardRateLimit(withAuth<T>(async (request, ctx, _cache) => …`) — δεν εξάγεται χωρίς HOF που θα έκρυβε ποιο HTTP verb κάνει τι. Νόμιμη περίπτωση `SKIP_JSCPD_DIFF=1` κατά N.18, **με δικαιολογία**. | Claude Code |
| 2026-07-29 | **`parking/route.ts` — CHECK 4 (302/300 γραμμές) ⇒ νέο `parking/_helpers.ts`.** Το ίδιο το de-duplication της γραμμής από πάνω (`fetchParkingSpotsWhere`) έσπρωξε το route πάνω από το όριο των 300 γραμμών του API route. Το κόψιμο **δεν** ήταν μηχανικό: μεταφέρθηκαν οι **τρεις** αναγνώστες Firestore, και στην πορεία ενοποιήθηκε το τριαδικό `isSuperAdmin ? όλη-η-συλλογή : where(companyId)` (ADR-232) που ήταν αντιγραμμένο σε **κτήρια + θέσεις** μέσα στην ίδια συνάρτηση, μαζί με τον αποκλεισμό soft-deleted (ADR-281) που ήταν σε **τρία** σημεία. Το route έμεινε 260 γραμμές και κρατά μόνο HTTP/auth/validation. Ο δομικός τύπος `ParkingDocLike` (`{ id; data() }`) αποφεύγει εξάρτηση του helper από το `firebase-admin`. | Claude Code |
| 2026-07-29 | **Τα 3 `construction-*` routes ξεμπλόκαραν με ΠΡΑΓΜΑΤΙΚΟ de-duplication (απόφαση Giorgio), όχι με `SKIP_JSCPD_DIFF`.** Το CHECK 3.28 ανέφερε **20 κλώνους** και μπλόκαρε τα αρχεία **ακόμα και μόνα τους**, γιατί ο κλώνος ήταν *εντός* του κάθε αρχείου — ανάμεσα στα HTTP verbs. Τρία νέα SSoT: (α) **`lib/api/building-scoped-route.ts`** (`runBuildingScoped`) — το τετράπτυχο «όριο ρυθμού → auth+δικαίωμα → `requireAdminFirestore()` → `requireBuildingInTenant()`» ήταν γραμμένο **10 φορές**· ⚠️ **δεν** τυλίγει το verb (το `export async function POST` μένει ορατό), άρα δεν παραβιάζει τον περιορισμό της γραμμής παρακάτω για το `properties/[id]/route.ts`. (β) **`lib/api/firestore-doc-guards.ts`** — `requireDocOfBuilding` (7 αντίγραφα· η σειρά **404 πριν από 403** είναι συμβόλαιο ασφαλείας: αντεστραμμένη διαρρέει την ύπαρξη id σε ξένο tenant) + `buildAllowedUpdates` (2 αντίγραφα· χωρίς τη λευκή λίστα ένα PATCH γράφει `companyId`). (γ) **`lib/api/construction-doc-mappers.ts`** — ο πεδίο-προς-πεδίο mapper φάσεων/εργασιών ήταν γραμμένος 2 φορές, στο GET **και** στο POST του baseline· **δεν είναι ίδια μορφή, είναι ίδια απάντηση**: το στιγμιότυπο πρέπει να παγώνει ό,τι ακριβώς σερβίρει το GET, αλλιώς το «baseline vs actual» συγκρίνει δύο σχήματα. Παράπλευρα: έφυγαν οι **3 τελευταίες νεκρές δικλείδες** `if (!adminDb)` (επιβίωναν στα GET), και το `construction-phases/_helpers.ts` σταμάτησε να ξανακάνει `requireBuildingInTenant` — ήταν **δεύτερη ανάγνωση Firestore για την ίδια απάντηση**. Μηνύματα σφάλματος αμετάβλητα by design (`ownerLabel` κρατά το γενικό `'Document'` των phases). | Claude Code |
| 2026-07-29 | **ΚΛΕΙΣΤΟ — και τα 22 αρχεία της νεκρής 503 μεταφέρθηκαν· `properties/[id]` + `showcase/generate` σε ΕΝΑ πρωτόγονο.** Οι τελευταίοι δύο κλώνοι ήταν πάλι **εντός** αρχείου: `PATCH ≡ DELETE` (58 tokens) και `POST ≡ DELETE` (75). Νέο **`lib/api/guarded-route.ts` (`runGuarded`)** = το μοναδικό πρωτόγονο «όριο ρυθμού → auth → δικαίωμα → `adminDb`»· το `buildingScopedRoute` ξαναγράφτηκε **πάνω του** και κρατά μόνο ό,τι το κάνει route κτηρίου. Νέο **`lib/api/entity-id-route.ts`**: `entityIdRoute` (id = τελευταίο τμήμα URL) + `segmentIdRoute` (id = `params` του Next). ⚠️ **ΔΙΟΡΘΩΣΗ της προηγούμενης γραμμής**, που έλεγε «ΜΗΝ το εξαγάγεις σε HOF — θα έκρυβε ποιο verb κάνει τι»: **ίσχυε για HOF που τυλίγει το verb**. Εδώ το verb μένει ρητό (`export const PATCH = entityIdRoute<…>({…})`) και φεύγει **μόνο** η αλυσίδα εξουσιοδότησης, που είναι ίδια *εξ ορισμού*. Το `SKIP_JSCPD_DIFF` **δεν χρειάστηκε πουθενά**. ⚠️ **Αλλαγή συμπεριφοράς, ρητή**: στο `showcase/generate` ο έλεγχος κενού id τρέχει πλέον **μετά** το auth (πριν έτρεχε πριν) — σκόπιμο, ένας ανώνυμος καλών δεν πρέπει να ξεχωρίζει «κακό id» από «δεν δικαιούσαι»· στην πράξη το Next δεν ταιριάζει καν τη διαδρομή με κενό id. Καταχωρήθηκαν 2 modules στο `.ssot-registry.json` (`guarded-route-composition`, `construction-doc-mappers`) — patterns που πιάνουν **0 αρχεία**, άρα ούτε allowlist ούτε baseline. | Claude Code |
