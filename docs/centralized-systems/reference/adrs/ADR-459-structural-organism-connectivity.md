...
**Status:** ACTIVE (Phase 0 + 1 + 2 + 4a + 4b + 4c + 4d + 4e + 4f implemented 2026-06-15· Phase 6 proactive + cross-level 2026-06-17· **Phase 7 Αυτόματος Σχεδιασμός Θεμελίωσης 2026-06-17· Phase 7 cross-level footing rendering all-floors + drift resilience 2026-06-17 (v8.1)· cross-level autosave-origin fix 2026-06-17 (v8.2)· Revit-grade footing in-place update + atomic undo 2026-06-17 (v8.3)· **3Δ rotate/resize→footing follow via kind→event emit SSoT 2026-06-17 (v8.4)· **Phase 8 PROACTIVE auto-reinforce οργανισμού 2026-06-17 (v9)· **Phase 9 Slice 1 PROACTIVE real-time φορτία 2026-06-17 (v10)· **Atomic undo σε διαγραφή — SSoT geometry-edit classifier 2026-06-19 (v11)· **Structural-relevance gate — μη-δομικό move/create δεν τρέχει recompute 2026-07-04 (v17)· **Wall-retrim relevance gate — first-move toast root fix 2026-07-04 (v18)· **SINGLE-PATH relevance router — μία πηγή αλήθειας (big-player) 2026-07-04 (v19)**)
...
- **2026-07-04 (v19, Opus 4.8):** **SINGLE-PATH relevance — μία πηγή αλήθειας αντί για διάσπαρτο gate (big-player refactor, Giorgio order).**
  **Κίνητρο (Giorgio):** «πρέπει να υπάρχει ΕΝΑ και μοναδικό μονοπάτι — τι θα έκαναν οι μεγάλοι παίκτες;» Τα v17/v18 έβαλαν τον ΙΔΙΟ gate (`eventTouchesStructuralMember`) **σε κάθε** structural subscriber ξεχωριστά → **fragile**: αρκεί ΕΝΑΣ να τον ξεχάσει και το bug επιστρέφει (ακριβώς αυτό συνέβη — v17 κάλυψε τα 4 proactive hooks αλλά ξέχασε τον wall-retrim → v18). Οι μεγάλοι παίκτες (Revit/AutoCAD/C4D) κρίνουν τη σχετικότητα **στην πηγή, μία φορά** (dependency-graph invalidation), όχι από κάθε παραλήπτη.
  **Fix (SINGLE-PATH):** NEW `hooks/useStructuralRelevanceRouter.ts` — το **ΕΝΑ** chokepoint: ακούει τα δύο generic events (`bim:entities-moved` + `drawing:entity-created`), φιλτράρει με τον SSoT `isStructuralMemberEntity`, και εκπέμπει **μία φορά** το νέο σημασιολογικό `bim:structural-geometry-changed { members, sourceEvent }` **μόνο** αν αγγίζει δομικό μέλος. ΟΛΟΙ οι structural reactors (`useProactiveStructuralLoads`/`-OrganismReinforce`/`-MemberSizing`/`useAutoFoundationDesign`/`useStructuralOrganism`/`useWallRetrimEffect`) ακούν πλέον **ΑΥΤΟ** — αντικαταστάθηκαν τα generic events στις λίστες τους. **Μηδέν per-subscriber gate → αδύνατο να ξεχαστεί.**
  **Καθαρισμός (superseded):** αφαιρέθηκε ο gate από `useGroupedStructuralReaction.schedule` (μένει μόνο η atomic-undo grouping ταξινόμηση)· **διαγράφηκε** το `hooks/structural-relevant-trigger.ts` + test του (v17 predicate, superseded — ο router χρησιμοποιεί απευθείας το βαθύτερο SSoT `isStructuralMemberEntity`)· ο wall-retrim έχασε το δικό του gate (v18). `bim:structural-geometry-changed` προστέθηκε στο `GEOMETRY_EDIT_TRIGGERS` (πάντα πηγάζει από move/create → grouped undo).
  **Tests:** NEW `useStructuralRelevanceRouter.test.ts` (6: line/column/mixed/non-array/create) + τα relevance tests (loads, wall-retrim) ξαναγράφτηκαν **end-to-end** (mount router + reactor, emit generic → verify). **Όλα GREEN.** Το generic `bim:entities-moved` μένει για τους μη-δομικούς consumers (persistence, grips, column-adjacency-notification).
- **2026-07-04 (v18, Opus 4.8):** **Wall-retrim relevance gate — ο πραγματικός ένοχος του «first-move» toast (bug fix, instrumentation-verified).** *(SUPERSEDED από v19 — ο per-subscriber gate αντικαταστάθηκε από το single-path router.)*
  **Σύμπτωμα (Giorgio):** μετά τον v17 gate, το toast «45 μέλη έλαβαν αυτόματο φορτίο» εμφανιζόταν **ΑΚΟΜΑ** — αλλά **ΜΟΝΟ στην πρώτη μετακίνηση γραμμής μετά από hard refresh** (μετά ποτέ, μέχρι το επόμενο refresh).
  **Ρίζα (stack-trace, ΟΧΙ υπόθεση):** ο v17 gate μπλόκαρε σωστά το **άμεσο** `bim:entities-moved` [line]. ΟΜΩΣ ο `useWallRetrimEffect` (`hooks/tools/useSpecialTools-wall-retrim.ts`) άκουγε το **generic** `bim:entities-moved` για **ΚΑΘΕ** entity → σε μετακίνηση γραμμής έτρεχε `recomputeWallTrims` (200ms debounce) → την **1η φορά** (cold walls: το recompute διέφερε από το persisted trim) οι τοίχοι «άλλαζαν» → **re-emit** structural event → ο proactive load-takedown (σωστά, αφού walls=δομικά) έτρεχε → toast. Idempotent → μετά την 1η φορά οι τοίχοι ήταν ήδη τακτοποιημένοι → μηδέν re-emit → μηδέν toast.
  **Fix (SSoT reuse):** εφαρμόστηκε ο **ΙΔΙΟΣ** `eventTouchesStructuralMember` gate στη συνδρομή του wall-retrim στο `bim:entities-moved` (τα `bim:wall-params-updated`/`bim:column-params-updated` μένουν άθικτα — ήδη structural-scoped). Οι τοίχοι εξαρτώνται ΜΟΝΟ από τοίχους/κολόνες → η μετακίνηση γραμμής δεν τους αγγίζει πλέον (correctness + μηδέν spurious 200ms recompute). NEW `hooks/tools/__tests__/wall-retrim-relevance.test.ts` (4 assertions: γραμμή=no-op, τοίχος/κολόνα=τρέχει, wall-params=τρέχει πάντα). **20/20 jest GREEN.** Verified: instrumentation stack-trace → `mouse-up → body-drag commit → reconcileAssociativeGeometry emit → useWallRetrimEffect → recomputeWallTrims re-emit → proactive loads`.
- **2026-07-04 (v17, Opus 4.8):** **Structural-relevance gate — μετακίνηση/δημιουργία ΜΗ-δομικού entity δεν πυροδοτεί πλέον δομοστατικό recompute (bug fix).**
  **Σύμπτωμα (Giorgio):** σέρνεις & αφήνεις μια απλή **γραμμή DXF** → εμφανίζονται toasts «N μέλη έλαβαν αυτόματο φορτίο» / «Κανένα μέλος δεν χρειαζόταν οπλισμό» → στο παρασκήνιο έτρεχε **full load-takedown + οπλισμός + θεμελίωση** σε όλο το κτίριο.
  **Ρίζα:** οι 4 proactive hooks (`useProactiveStructuralLoads` / `useProactiveOrganismReinforce` / `useProactiveMemberSizing` / `useAutoFoundationDesign`) ακούν τα **generic** events `bim:entities-moved` + `drawing:entity-created` (εκπέμπονται από `reconcileAssociativeGeometry` / `SnapshotTransformCommand` για **κάθε** τύπο entity) και αντιδρούσαν **ανεξαίρετα** — κανείς δεν εξέταζε τον τύπο του μετακινημένου/δημιουργημένου entity. Ο `isGeometryEditTrigger` gate αφορά μόνο το atomic-undo grouping, όχι το αν αξίζει καθόλου το recompute.
  **Fix (SSoT, ένα σημείο → 4 hooks):** NEW `hooks/structural-relevant-trigger.ts` (`eventTouchesStructuralMember`) εξετάζει το payload των δύο generic events και επιτρέπει recompute ΜΟΝΟ αν ≥1 εμπλεκόμενο entity είναι **δομικό μέλος**· εφαρμόζεται στο `useGroupedStructuralReaction.schedule` (πριν το `coalescer.schedule()`). Κάθε άλλο event (type-scoped `*-params-updated` / `*-delete-requested`, batch `*-from-grid`/`-perimeter`, derived `structural-loads-computed`) περνά ως έχει. **Safe default:** άγνωστο payload shape → δεν κόβεται.
  **Νέα SSoT** `types/structural-entity-types.ts` (`STRUCTURAL_MEMBER_TYPES` = column/beam/wall/slab/stair/foundation + `isStructuralMemberType`/`isStructuralMemberEntity`). **Boy-Scout:** το ADR-470 private `isStructuralEntity` (`useStructuralComponentOverride.ts`) κάνει πλέον delegate σε αυτό (εξάλειψη μελλοντικού διπλότυπου). NEW `__tests__/structural-relevant-trigger.test.ts` (16 predicate assertions) + NEW `__tests__/useProactiveStructuralLoads-relevance.test.ts` (**wiring end-to-end**: mount του πραγματικού hook + emit `bim:entities-moved` → γραμμή=κανένα `runStructuralLoadTakedown`, κολόνα=1 κλήση, μεικτή=1). **19/19 jest GREEN** (+ το grouping test άθικτο). Zero behavior change στα πραγματικά δομικά moves/creates. **Σημ.:** και τα δύο gestures μετακίνησης (select→drag ΚΑΙ body-drag «κλικ+παρατεταμένο στο σώμα») δεσμεύονται μέσω της **ΙΔΙΑΣ** `MoveEntityCommand` (`useEntityBodyDragCommit`) → ίδιο `bim:entities-moved` → ο ένας gate τα καλύπτει και τα δύο.
