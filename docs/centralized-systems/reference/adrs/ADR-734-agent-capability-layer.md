...
**Κατάσταση:** ACCEPTED — **Φάσεις 1-2 υλοποιημένες** (2026-07-30, §8.1 / §8.2)· **Φάσεις 3α + 3β υλοποιημένες** (2026-07-31, §8.3 / §8.4)· **tenant isolation επιλογή Β υλοποιημένη** (2026-07-31, §7.1.1 — ο πάγος του §9 έληξε)· Φάσεις 4-5 εκκρεμείς
...
#### 7.1.1 ✅ Η επιλογή Β υλοποιήθηκε (2026-07-31) — και η επιλογή Α δεν αρκούσε

Ο πάγος του §9 έληξε με την παράδοση των Φάσεων 3α και 3β. Η μέτρηση που τον
ξεπάγωσε: **μηδέν** καταναλωτές του `getById` εκτός του service, **ένας**
καταναλωτής των mutations (`useBOQItems.ts`) — ο οποίος **ήδη έχει** το
`companyId` ως παράμετρο. Η καταγραφή έλεγε «breaking change σε δοκιμασμένο
service + όλοι οι εξωτερικοί καταναλωτές»· αυτό ίσχυε **όσο υποθέταμε** ότι
υπάρχουν εξωτερικοί καταναλωτές.

**Δύο πράγματα που αποκάλυψε το grep και δεν ήταν στην αρχική περιγραφή:**

🔴 **α) Το κενό δεν ήταν το `getById`. Ήταν οκτώ μέθοδοι.** Καμία μέθοδος που
διευθυνσιοδοτεί με id δεν έπαιρνε tenant: `getById`, `update`, `delete`,
`bulkDelete`, `duplicate`, `updateStatus`, `transition`, `reopenToDraft`.
Χειρότερα, τα `delete()` και `updateStatus()` του repository **έγραφαν χωρίς να
διαβάσουν ποτέ το έγγραφο** — ένα id αρκούσε για `deleteDoc`. Προσθήκη
`companyId` **μόνο** στο `getById` θα παρήγαγε service με φυλασσόμενη ανάγνωση
και αφύλακτη εγγραφή: χειρότερο από την προηγούμενη κατάσταση, γιατί θα έμοιαζε
λυμένο.

🔴 **β) Ο πραγματικός κίνδυνος ήταν το Admin SDK, όχι το client.** Το client
μονοπάτι έχει από κάτω του τα Firestore rules (`firestore.rules:2955/2989/3015` —
`belongsToCompany(resource.data.companyId)` σε read/update/delete), οπότε
cross-tenant πρόσβαση από κανονικό χρήστη **αποτύγχανε ήδη**. Το
`BOQAdminReadService` της Φάσης 3α **παρακάμπτει τους κανόνες εξ ορισμού** — και
είναι ακριβώς η διαδρομή του πράκτορα και του `/api/mcp`. Εκεί ο έλεγχος του
`fetchOwnedBoqItem` ήταν η **μόνη** άμυνα, μαζί με ένα regex στο
`.ssot-registry.json`: μια σύμβαση, όχι το σύστημα τύπων.

**Τι ισχύει τώρα:**

```ts
getById(companyId: string, id: string): Promise<BOQItem | null>;   // ✅ και στις 8
```

- Ο έλεγχος ζει στο `services/measurements/boq-tenant-ownership.ts` — **ένα** SSoT,
  δύο καταναλωτές (client repository + admin service), ώστε τα δύο SDK να μη
  διατηρούν δύο αντίγραφα του ίδιου ελέγχου πρόσβασης.
- Στο client repository υπάρχει **μία πόρτα**, το ιδιωτικό `readOwned()`: κάθε
  μέθοδος με `id`, ανάγνωσης ή εγγραφής, περνά από εκεί πρώτα.
- Το `IBOQRepository`/`IBOQService` μοιράζονται πλέον κοινή βάση `IBOQItemAccess`:
  το CHECK 3.28 εντόπισε ότι οι 11 κοινές υπογραφές ήταν δομικό διπλότυπο 29
  γραμμών — δηλαδή κάθε αλλαγή συμβολαίου έπρεπε να γραφτεί δύο φορές σωστά.
- Το `boq-tenant-guard.ts` **δεν καταργήθηκε**: παύει να είναι η άμυνα και μένει
  ως (1) μεταφραστής `null → NOT_FOUND` και (2) **δεύτερη ζώνη**. Ο μεταγλωττιστής
  εγγυάται ότι το `companyId` *περνιέται*, όχι ότι κάθε υλοποίηση του
  `IBOQReadService` το *τιμά*.

**Επαλήθευση:** νέο `__tests__/boq-tenant-isolation.test.ts` (20 tests) που ελέγχει
πλήρη στοίβα service → repository → SDK. Κάθε test εγγραφής απαιτεί **και** την
επιστρεφόμενη τιμή **και** ότι το `deleteDoc`/`updateDoc`/`setDoc` δεν κλήθηκε
ποτέ — μόνο το πρώτο θα περνούσε και με σιωπηλά αποτυχημένη εγγραφή.

🔴 **Τι έπιασε η σκόπιμη μετάλλαξη (7 μεταλλάξεις):** η αφαίρεση του ελέγχου από
το `repository.delete()` **επέζησε** στην πρώτη εκτέλεση — γιατί το
`service.delete()` κόβει νωρίτερα με δικό του `getById`, άρα το repository ήταν
φυλασσόμενο μόνο *κατά σύμπτωση*. Η `FirestoreBOQRepository` όμως εξάγεται και
μπορεί να κληθεί απευθείας. Προστέθηκε block tests που χτυπά το repository χωρίς
το service από πάνω· **7/7 μεταλλάξεις σκοτωμένες**. Ένα στρώμα που φυλάσσεται
μόνο επειδή τυχαίνει να καλείται από άλλο φυλασσόμενο στρώμα δεν είναι
φυλασσόμενο — είναι τυχερό.
...
- ~~❌ **Δεν** τροποποιούμε `boqService`, `cost-engine`, `boq-repository` στις Φάσεις 1-3.~~
  ✅ **Ο πάγος έληξε 2026-07-31** με την παράδοση των Φάσεων 3α και 3β. Το
  `boqService` και το `boq-repository` τροποποιήθηκαν για την **επιλογή Β** του
  §7.1.1 (tenant στην υπογραφή). Το `cost-engine.ts` **δεν αγγίχθηκε** — ο πάγος
  του παραμένει σε ισχύ: είναι η πηγή των αριθμών και καμία δουλειά ταυτότητας δεν
  τον χρειάζεται.
...
| Ημ/νία | Αλλαγή |
|---|---|
| 2026-07-31 | **Tenant isolation — η επιλογή Β ΥΛΟΠΟΙΗΘΗΚΕ· ο πάγος του §9 έληξε** (§7.1.1). Το `companyId` μπήκε στην υπογραφή **και των 8** id-addressed μεθόδων (`getById`, `update`, `delete`, `bulkDelete`, `duplicate`, `updateStatus`, `transition`, `reopenToDraft`). 🔴 **Το grep διόρθωσε δύο υποθέσεις της ίδιας της καταγραφής:** (α) το κενό **δεν** ήταν το `getById` αλλά οκτώ μέθοδοι — τα `delete()`/`updateStatus()` του repository **έγραφαν χωρίς καμία ανάγνωση**, δηλαδή προσθήκη tenant μόνο στο `getById` θα έδινε φυλασσόμενη ανάγνωση με αφύλακτη εγγραφή· (β) ο πραγματικός κίνδυνος ήταν το **Admin SDK** (παρακάμπτει τα Firestore rules, είναι η διαδρομή του πράκτορα/MCP), όχι το client μονοπάτι που τα rules ήδη κάλυπταν. Επίσης: «όλοι οι εξωτερικοί καταναλωτές» = **μηδέν** για το `getById`, **ένας** για τα mutations, ο οποίος **ήδη είχε** το `companyId`. Νέο SSoT `boq-tenant-ownership.ts` (ένας έλεγχος, δύο SDK)· μία πόρτα `readOwned()` στο repository· κοινή βάση `IBOQItemAccess` αφού το **CHECK 3.28** εντόπισε 29 γραμμές διπλότυπων υπογραφών μεταξύ των δύο interfaces. Ο `boq-tenant-guard` **μένει** ως μεταφραστής + δεύτερη ζώνη: ο μεταγλωττιστής εγγυάται ότι το `companyId` περνιέται, όχι ότι κάθε υλοποίηση το τιμά. **20 νέα tests** (πλήρης στοίβα, κάθε test εγγραφής απαιτεί «καμία κλήση `deleteDoc`/`updateDoc`/`setDoc`»), 324 πράσινα συνολικά. **7 σκόπιμες μεταλλάξεις — 7 σκοτωμένες**, αλλά **η μία επέζησε στην πρώτη εκτέλεση**: το `repository.delete()` ήταν φυλασσόμενο μόνο επειδή το `service.delete()` κόβει νωρίτερα, ενώ η κλάση **εξάγεται** και καλείται απευθείας. Προστέθηκαν tests που χτυπούν το repository χωρίς το service από πάνω. |
