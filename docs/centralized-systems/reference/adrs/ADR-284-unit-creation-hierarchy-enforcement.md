...
- **2026-04-06**: **SSoT EXTENSION — "No projects yet" CTA reused in Building Management**. Το `noProjects` empty state εξήχθη από το `PropertyHierarchyEmptyStates.tsx` σε νέο shared component `src/components/shared/empty-states/NoProjectsEmptyState.tsx` με ουδέτερες i18n keys (`common-empty-states.noProjects.{title,cta,description.forUnit,description.forBuilding}`). Το `GeneralTabContent.tsx` (Building Management → General tab, create mode) το εμφανίζει όταν `projectsCount === 0`, με CTA που ανοίγει το canonical `ProjectQuickCreateSheet` (ίδιο slide-out από δεξιά που χρησιμοποιείται ήδη από το properties flow). Μετά τη δημιουργία του project, το EntityLinkCard auto-selects το νέο `projectId`. Zero duplication — ένα component, ένα slide-out, δύο entry points (unit creation + building creation). 6 αρχεία: 1 NEW component + 2 i18n + 1 refactor (`PropertyHierarchyEmptyStates` χρησιμοποιεί το shared) + 1 integration (`GeneralTabContent`) + 2 ADR updates (238, 284).
...
- **2026-04-05**: **Batch 7.8 FIX — Hide Building/Floor cards for Family B in detail view** (Γιώργος QA). Bug: `PropertyDetailsContent.tsx` top-row grid (PropertyEntityLinks "Σύνδεση με κτίριο" + FloorSelectField "Επίπεδα ακινήτου" + LinkedSpacesCard) rendered για όλα τα property types — παραβίαση Family B κανόνα (villa/detached_house = direct Project parent, no Building/Floor). Fix: imported `isStandaloneUnitType` από `usePropertyCreateValidation` (SSoT discriminator), added `isStandalone` memo από `effectiveType`, extended render guard: `{!isReadOnly && !isCreatingNewUnit && !isStandalone && (...)}`. Now villas/detached houses σε view/edit mode εμφανίζουν ΜΟΝΟ PropertyFieldsBlock (χωρίς top-row κάρτες Building/Floor). 1 file: `PropertyDetailsContent.tsx` (+3 lines). Zero behavior change για Family A units.
- **2026-04-05**: **Batch 7.9 REFACTOR — PropertyType SSoT centralization (ADR-145)** (Γιώργος request). Ο τύπος ακινήτου ήταν διάσπαρτος σε **9 αρχεία** με 3 bug categories: (α) hyphen/underscore mismatch σε `public-property-filters` (`apartment-2br` vs `apartment_2br`), (β) incomplete `UNIT_TYPES_FOR_FILTER` στο property-tab-constants (9 από 14 types — έλειπαν penthouse/loft/villa/detached_house/hall), (γ) shadow `PropertyType` definition στο `property-status-core.ts` (6 αντί 14 values). **Νέο SSoT module**: `src/constants/property-types.ts` (leaf, zero deps) με `PROPERTY_TYPES` array + derived `PropertyTypeCanonical` type + `STANDALONE_UNIT_TYPES` discriminator + `PROPERTY_TYPE_I18N_KEYS`. **9 files migrated** (δες ADR-145). **Επίσης**: Αφαιρέθηκαν `apartment_2br`/`apartment_3br` από το active dropdown (Γιώργος request), μεταφέρθηκαν στο `DEPRECATED_PROPERTY_TYPES` για backward-compat με παλιά Firestore records. Το `isStandaloneUnitType` discriminator (χρησιμοποιείται από Batch 7.8 guard στο `PropertyDetailsContent.tsx`) τώρα ζει στο SSoT — `usePropertyCreateValidation` + `property-creation-policy` κάνουν re-export. Zero behavior change για Family A/B discrimination.
