...
| 2026-07-26 | **Η συνέχεια έφτασε και στην ΕΠΙΜΕΤΡΗΣΗ (→ ADR-712).** Μέχρι τώρα το `baseDropMm` ζούσε μόνο στο 3Δ render: το τμήμα από την άνω παρειά του πεδίλου ως τη nominal βάση («κοντόστυλο») δεν το χρέωνε κανείς. Πλέον παράγει δική του γραμμή με άρθρο **θεμελίων** (`OIK-2.07`), γιατί το ΝΕΤ ΟΙΚ τιμολογεί χωριστά θεμέλια από ανωδομή → λάθος απόδοση = σφάλμα **τιμής**. Το transport (`column-continuity-boq-source`) **δεν** παραβιάζει το §7: χτίζει τον χάρτη on-demand μέσα στην κλήση (μηδέν store)· από store διαβάζονται μόνο τα **δεδομένα** των πεδίλων (ADR-459 Φ0). Το §7.1 κριτήριο, που ο επιμετρητικός καταναλωτής δεν μπορεί να εφαρμόσει (βλέπει έναν όροφο), απαντήθηκε ένα επίπεδο ψηλότερα — `activeLevelBearsOnFoundation`, fail-closed. |
