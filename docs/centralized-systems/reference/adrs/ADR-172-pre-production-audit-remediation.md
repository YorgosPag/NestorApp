...
| 2026-03-24 | Enforcement | **3-Level Automated Enforcement System**: (1) ESLint `no-explicit-any` upgraded to "error" for ALL code (was "warn" in services/api), added `no-restricted-syntax` ban on `as any`; (2) Pre-commit hook enhanced — now runs ESLint on staged .ts/.tsx files + custom project rules; (3) New `scripts/validate-project-rules.sh` — 5 custom rules: no hardcoded storage paths, no inline UUID, no addDoc(), no @ts-ignore, purge must include Storage deletion. npm scripts: `validate:project`, `validate:all` |
