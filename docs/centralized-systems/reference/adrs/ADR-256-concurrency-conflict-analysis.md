...
| Date | Change | Author |
|------|--------|--------|
| 2026-09-15 | **Company profile (`accounting_settings/{companyId}`) = LWW ανά ΠΕΔΙΟ με μάσκα πεδίων — ΟΧΙ etag / `_v`** (ADR-841 §7 Α23, Φ3.2 Γ3). **Πρόβλημα**: το `PUT /api/accounting/setup` έκανε `docRef.set(ολόκληρο)` χωρίς συναλλαγή ⇒ μια οθόνη προφίλ ανοιχτή **πριν** από την «Υιοθέτηση επωνυμίας ΓΕΜΗ» ξανάγραφε **σιωπηλά** την παλιά επωνυμία (και ό,τι άλλο είχε αλλάξει στο μεταξύ). **Απόφαση** — συνεπής με τη γραμμή 2026-04-13 (σιωπηλό LWW, κανένα ConflictDialog), αλλά σε **λεπτότερο κόκκο**: last-writer-wins **ανά ιδιότητα**, όχι ανά έγγραφο. Ο client στέλνει `fields` = τα πεδία που **ο ίδιος** άλλαξε σε σχέση με το προφίλ που φόρτωσε (`changedProfileFields`, `dequal`)· ο server σε **μία** `runTransaction` διαβάζει το αποθηκευμένο, συγχωνεύει **μόνο** αυτά (`mergeProfileFields`) και γράφει. Χωρίς μάσκα ⇒ πλήρης αντικατάσταση (`*`)· άγνωστο πεδίο ⇒ 400· καμία αλλαγή ⇒ καμία γραφή (ιδεμποτής). Αλλαγή νομικής μορφής (`entityType` στη μάσκα) ⇒ γράφονται μορφή + πεδία της νέας μορφής, σβήνονται της παλιάς (`oneof`), η επωνυμία **δεν** ξαναγράφεται. Το σύνολο πεδίων είναι **εξαντλητικό από κατασκευή** (mapped type `FIELD_SCOPE` ⇒ νέο πεδίο στον τύπο χωρίς δήλωση = σφάλμα τύπου). **Γιατί όχι `withVersionCheck`/`_v`**: etag ολόκληρου εγγράφου θα αρνιόταν αλλαγή τηλεφώνου επειδή η **επωνυμία** άλλαξε από την υιοθέτηση — ακριβώς ο θόρυβος που απέρριψε η 2026-04-13· εδώ άσχετες αλλαγές **δεν συγκρούονται ποτέ**, και η ίδια αλλαγή στο ίδιο πεδίο λύνεται LWW με ίχνος (ADR-440). **Πηγές**: [Figma multiplayer](https://www.figma.com/blog/how-figmas-multiplayer-technology-works/) («two clients changing unrelated properties on the same object won't conflict») · [AIP-134](https://google.aip.dev/134) (`*` = full replacement) · [AIP-161](https://google.aip.dev/161) (`INVALID_ARGUMENT` για ανύπαρκτο πεδίο σε εγγραφή) · [Stripe Update account](https://docs.stripe.com/api/accounts/update) («Any parameters not provided are left unchanged») · [proto3 oneof](https://protobuf.dev/programming-guides/proto3/). **Αρχεία**: `subapps/accounting/services/setup/company-profile-field-mask.ts` · `services/repository/accounting-repo-company-setup.ts` · `app/api/accounting/setup/route.ts` · `hooks/useCompanySetup.ts` (commits `f1796083`, `63cb2616`). **Όριο**: αν η φόρτωση του προφίλ απέτυχε ενώ υπάρχει έγγραφο, ο client δεν έχει βάση ⇒ πλήρης γραφή (προϋπήρχε· η οθόνη δείχνει σφάλμα). | Claude Code |
