...
- **2026-07-10** (v2 **Phase C** — full straight-stair wall parity, Opus 4.8) — Giorgio: «φέρε το ΠΛΗΡΕΣ σύστημα λαβών του ευθύγραμμου τοίχου στην ευθύγραμμη σκάλα, ίδια ακριβώς συμπεριφορά, μηδέν διπλότυπα». The STRAIGHT variant now emits the IDENTICAL grip set to the straight wall via the **shared `axis-box-grips` SSoT** (ADR-363): 8 shape handles (`stair-width`/`stair-length` + 4 corners + the 2 opposite mid-edges **`stair-width-far`/`stair-length-start`**, NEW kinds) + the axis-quarter rotation handle (`stair-direction`) + the centre 4-arrow MOVE cross (`stair-base`, no longer filtered). Decision (Giorgio 2026-07-10): resize is **full wall parity** — opposite-face-fixed asymmetric via `applyAxisBoxGripDrag` (supersedes the v2 §Q2 symmetric-width choice for straight). **Files**: (1) NEW `bim/stairs/stair-rect-adapter.ts` (`STAIR_ROLE_TO_KIND` + `stairAxisBoxParams` [identity scale — stair values are ALREADY scene units, NOT mm like the wall] + `stairParamsFromPatch` [tread-snap + role-based re-anchoring] + `applyRectStairGrip`). (2) `stair-grips.ts` — `getStairGrips` straight branch → self-contained `pushStraightGrips` (axis-box emission + move cross, early-return, no filter); non-straight pre-push unchanged; old 4-corner helper kept as `pushStraightCornerGrips` (split-variant degenerate fallback). (3) `stair-grip-transforms.ts` — `applyStairGripDrag` routes straight resize through `applyRectStairGrip`; `rotateDirection`→pivot-aware **`rotateStair`** (unifies legacy spin-about-base + wall-parity orbit-a-picked-pivot via shared `rotateAxisPointsAboutPivot`); `StairGripDragInput.pivot?`; new `stair-width-far`/`stair-length-start` cases; dead `moveCorner` deleted (straight now axis-box); `moveCornerDispatch`→multi-flight only. (4) `stair-grip-math.ts` — SSoT helpers `recomputeRunSteps` + `projectCursorAxial` + `twoFlightSplit` (N.0.2 Boy-Scout de-dup, jscpd clean). (5) `hooks/grip-kinds.ts` — +2 `StairGripKind` members. (6) `hooks/grips/wall-hot-grip-fsm.ts` — `HOT_GRIP_OP_REGISTRY` += `stair-base:'move'` + `stair-direction:'rotate'` (**critical wiring** — without it the move-popup + rotation flow never arm). (7) `bim/grips/move-glyph-frame.ts` — `resolveMoveGlyphFrame` reads `basePoint`+`direction` (**critical wiring** — returned null for the stair → move cross + rotation-reference axis broke). (8) `grip-parametric-commits.ts` — `commitStairGripDrag` reads `BimRotateHotGripStore` pivot/anchor for `stair-direction` (mirror `commitWallGripDrag`). Emission/glyph/rotation/move = shared SSoT; corner tests 14-18 pass UNCHANGED (axis-box recenter ≡ old `moveCorner`); edge tests 12/13 updated to wall-parity delta semantics. **41/41 tests PASS** (7 new: 10-grip layout, axis-quarter rotation, 2 mid-edges, pivot-orbit, no-pivot legacy, stair move-glyph frame ×2), jscpd:diff clean, files ≤500.
