...
|---|---|---|---|
| v1.8 | 2026-09-09 | Claude (Opus 5) | 🔴 **ΤΟ ΔΕΥΤΕΡΟ UV SET (`uv2`) ΑΦΑΙΡΕΘΗΚΕ — ΕΚΑΝΕ **ΚΑΘΕ** ΕΞΑΓΟΜΕΝΟ glTF ΑΚΥΡΟ.** Το Δ2 έγραφε από την v1.0 `uv` **και** `uv2` *(αντίγραφο)*, «required by `aoMap`». Ήταν αληθές σε three **< r152**· ο επίσημος οδηγός μετάβασης **r151→r152** το ανέτρεψε **δύο φορές**, και τα δύο **σιωπηλά για εμάς**: **(1)** *«`uv`, `uv2`, `uv3`, `uv4` are now `uv`, `uv1`, `uv2`, `uv3`»* — το όνομα `uv2` **μετακινήθηκε στο ΤΡΙΤΟ set**· **(2)** *«`aoMap` and `lightMap` **no longer use `uv2`** — set `material.lightMap.channel`»*, και ο `pbr-material-builder` **ποτέ** δεν όρισε `channel` ⇒ προεπιλογή `0` ⇒ το `aoMap` διάβαζε **ήδη** το `uv`. ⇒ Το `uv2` ήταν **αντίγραφο που κανείς δεν διάβαζε**, με **διπλάσια μνήμη ανά geometry** για **μηδέν** πληροφορία — και **δηλητηρίαζε την εξαγωγή**: ο `GLTFExporter` r170 χαρτογραφεί `uv2 → TEXCOORD_2`, αφήνοντας **τρύπα στο TEXCOORD_1**, ενώ το glTF 2.0 απαιτεί ρητά τα `TEXCOORD_n` να **ξεκινούν στο 0 χωρίς κενά**. 🔬 **ΜΕΤΡΗΜΕΝΟ ΖΩΝΤΑΝΑ, ΟΧΙ ΘΕΩΡΗΤΙΚΟ** *(ADR-845 Ο-15)*: πραγματικό ανέβασμα **122.636 bytes** απορρίφθηκε από τον ψήστη με *«glTF-Validator reported **13** error(s)»*· μέσα στο αρχείο μετρήθηκε `13x [POSITION, NORMAL, TEXCOORD_0, **TEXCOORD_2**]`. **Αλλαγή:** αφαιρέθηκαν οι **5** εγγραφές `uv2` — **όλες σε ΕΝΑ αρχείο**, το `bim-uv-helpers.ts` *(`ensureWorldUvs` · `setPlanarWorldUvs` · `setBoxWorldUvs` · `setPlanarTileUvs` · `setSlopeAlignedTileUvs`)*· τα άλλα **11** αρχεία είχαν **μόνο σχόλια** που πλέον έλεγαν ψέματα και διορθώθηκαν. ⚠️ **ΜΗΔΕΝ οπτική αλλαγή**: το `aoMap` διάβαζε ήδη `uv`. 🏆 **ΦΥΛΑΓΕΤΑΙ ΑΠΟ ΑΓΚΥΡΑ ΠΟΥ ΕΚΤΕΛΕΙ ΤΟΝ ΕΠΙΣΗΜΟ ΚΡΙΤΗ**: `export/core/mesh3d/__tests__/mesh3d-gltf-validity.test.ts` — περνά **αληθινή** γεωμετρία από τα helpers μας στον **πραγματικό** `serialiseGlb` και μετά στον **ίδιο** `gltf-validator` που τρέχει ο ψήστης της δημοσίευσης. ⛔ Ο κανόνας του κλάδου *(validator σε CI πάνω σε δεσμευμένα `.glb`)* **δεν θα το έπιανε ποτέ**: δεν έχουμε δεσμευμένα `.glb` — έχουμε **εξαγωγέα**, και ο εξαγωγέας έσπασε. **Μετάλλαξη**: `[0,2]` → `[0,1]`, κόκκινο→πράσινο. **Παλινδρόμηση**: 3.491/3.491 tests σε 368 σουίτες (`bim-3d` + `export`). ⛔ **ΜΗΝ το ξαναβάλεις ως `uv2`**: αν χρειαστεί **πραγματικά** δεύτερο set *(π.χ. lightmap atlas — δηλαδή UV **διαφορετικά**, όχι αντίγραφο)*, το όνομα είναι **`uv1`** και απαιτεί ρητό `texture.channel = 1`. |
| v1.7 | 2026-07-24 | Claude (Opus 5) | **§2D — «άγνωστο id» ΔΕΝ σημαίνει πια σκυρόδεμα. Βλ. ADR-693 Άξονας Α/Γ.** Το v1.4 είχε ήδη καταγράψει ότι «τα `bmat_*` καταρρέουν σε concrete στο `resolveMaterialKey`» και το έκλεισε για το **3D** (user-material-registry) — αλλά το **2D swatch** μονοπάτι έμεινε ανοιχτό. Μετρημένο (Giorgio, browser 2026-07-24): δύο swatches της μπάρας «Υλικά όψης» εμφανίζονταν ως **broken image**. NEW `resolveMaterialKeyOrNull` στο `material-catalog-defs` (ΕΝΑΣ prefix matcher· ο `resolveMaterialKey` = wrapper `?? DEFAULT`, μηδέν αλλαγή για τους υπάρχοντες καλούντες)· `slugForMaterialId` + `resolveThumbnailTextureSet` + `preloadThumbnailTextures` επιστρέφουν πλέον `null` για id εκτός καταλόγου, αντί για την υφή σκυροδέματος. `MaterialSwatch`: `onError` → flat colour chip (ποτέ broken-image icon) και το flat fallback διαβάζει το ΠΡΑΓΜΑΤΙΚΟ `appearance.baseColorHex` του υλικού πριν πέσει σε χρώμα καταλόγου. `material-thumbnail-sphere`: επικύρωση του readback (`'data:,'` από χαμένο GL context περνούσε για επιτυχία και cache-αριζόταν μόνιμα) + φραγμένη ανάκτηση context. ⚠️ Ο `resolveMaterialKey` **διατηρεί** σκόπιμα το concrete fallback. |
| v1.6 | 2026-06-12 | Claude (Opus 4.8) | **ADR-447 — default concrete textures για δομικά/θεμελίωση.** Στο `MATERIAL_TEXTURE_MAP` (bim-texture-registry): `elem-beam` 'wood'→**'concrete'** (RC δοκάρι) + `elem-foundation`/`-pad`/`-strip`/`-tie-beam`→**'concrete'** (όπως ήδη `elem-column`/`elem-slab`). Στο realistic (ADR-446 faceMode) όλα τα δομικά δείχνουν υφή σκυροδέματος χωρίς ο χρήστης να δηλώνει υλικό. Per-category χρώματα (ADR-445) αμετάβλητα. Βλ. ADR-447. |
