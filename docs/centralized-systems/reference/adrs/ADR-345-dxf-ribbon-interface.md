...
| 2026-05-12 | CSS `.dxf-ribbon-tab[data-active="true"]` — aggiunto `position: relative; z-index: 1` + pseudo-elemento `::after` 1px che copre la `border-bottom` di `.dxf-ribbon-tab-bar` sotto il chip attivo. Risultato: la tab attiva fluisce visivamente nel body del ribbon senza linea di separazione (pattern AutoCAD/Office 365). |
