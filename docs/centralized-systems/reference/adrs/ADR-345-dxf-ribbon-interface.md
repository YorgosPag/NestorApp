...
| Data | Modifica |
|------|----------|
| 2026-06-23 | **Hatch contextual tab — (Α) κουμπί «Επιλογή γραμμοσκίασης» + (Β) widget «Γραμμοσκιάσεις ορόφου»** (ADR-507, λεπτομέρειες εκεί). Ribbon-side προσθήκες: NEW action key `hatch.action.selectExisting` (simple button στο panel `hatch-actions`, icon `hatch`) → δρομολογείται στο `useRibbonHatchBridge.onAction` μέσω `isHatchActionKey` (δεν matchάρει το central close-interceptor)· NEW `widgetId:'hatch-list'` (panel `hatch-info`) → dispatch στο `RibbonPanel.tsx` σε `RibbonHatchListWidget` (leaf, mirror `RibbonMepCircuitPickerWidget`, `DropdownMenu`). ✅ Google-level: YES — reuse υπάρχοντος action-routing + widget-dispatcher pattern, μηδέν νέος μηχανισμός. **Files**: MOD `hatch-command-keys.ts`, `useRibbonHatchBridge.ts`, `contextual-hatch-tab.ts`, `RibbonPanel.tsx`· NEW `RibbonHatchListWidget.tsx`. 🔴 browser-verify + commit. |
