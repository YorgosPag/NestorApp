...
- ⏳ Wiring concreto controlli Text Editor (font combobox, bold/italic/underline toggle, layer dropdown, find/replace) → **Fase 5.5** separata. Richiede creare `RibbonCombobox` + `RibbonToggleButton` components (ADR §4.4-4.5 non ancora implementati)
- ⏳ Floating panel sidebar `text-properties` tab → resterà attivo finché Fase 5.5 non wirea i controlli. Disabilitazione → Fase 5.5 close.
