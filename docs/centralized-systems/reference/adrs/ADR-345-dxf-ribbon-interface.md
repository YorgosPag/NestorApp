...
| Data | Modifica |
|------|----------|
| 2026-07-08 | **«Διάστιχο» MTEXT-only visibility** (πλήρες B+A στο **ADR-557**). Ribbon-side: το `RibbonLineSpacingWidget` (panel «Παράγραφος») κάνει πλέον leaf-subscribe στο νέο **derived** `useTextToolbarStore.isMultiLine` και **`return null`** σε single-line επιλογή (AutoCAD/Revit/Word parity: το διάστιχο έχει νόημα μόνο σε ≥2 γραμμές· η «Στοίχιση παραγράφου» στο ίδιο panel **μένει** ορατή γιατί single-line TEXT έχει 9-point attachment). Επειδή ο ribbon visibility είναι **panel-level μόνο** (`RibbonPanel`→`useRibbonPanelVisibility`) και το widget μοιράζεται panel με τη στοίχιση, χρησιμοποιήθηκε widget-level null-render + καθολικό `.dxf-ribbon-panel-row:empty { display:none }` (`ribbon-tokens.css`) → η now-empty γραμμή collapse-άρει, μηδέν phantom gap, μηδέν νέο panel/separator. Η πηγή αλήθειας για το line count = `textLineCount` SSoT, υπολογισμένη στο `useTextToolbarSelectionSync` (populate meta). ✅ Google-level: YES — reuse υπάρχοντος leaf/populate pattern + SSoT line-count, μηδέν νέος μηχανισμός visibility. **Files**: MOD `RibbonLineSpacingWidget.tsx`, `ribbon-tokens.css` (+ text-toolbar store/sync & command — ADR-557). 🔴 browser-verify + commit. |
...
| 2026-07-20 | **Κλικ σε κάρτα επιπέδου → `'select'` αντί `'grip-edit'`** — Αναφορά Giorgio: πλοηγώντας στα «Επίπεδα Έργου» άναβε **μόνο του** το «Σχεδίαση Επιπέδων» (`layering`) στο ribbon· ο χρήστης θέλει να ανάβει το «Επιλογή». Αλυσίδα: `LevelPanel.tsx` `onSelect` → `onToolChange('grip-edit')` → `usePanelContentRenderer` emit `'level-panel:tool-change'` → `useDxfViewerEffects:302` `handleToolChange('grip-edit')` → **bridge effect** `useDxfViewerEffects:272-278` (`activeTool==='grip-edit' && overlayMode!=='edit'`, και `overlayMode` default = `'select'`) → `handleToolChange('layering')`. Δηλαδή το bridge, φτιαγμένο για έξοδο από πραγματικό overlay-edit, πυροδοτούνταν σε **κάθε** κλικ κάρτας. **Fix (1 γραμμή, `LevelPanel.tsx:328`)**: το κλικ κάρτας ζητά πλέον `'select'`. Δεν είναι μόνο προτίμηση — συμφωνεί με το entry του 2026-05-28 εδώ πάνω: το `grip-edit` δείχνει λαβές **μη-διαδραστικές** (`isGripMode = 'select' \|\| 'layering'`), άρα το `'select'` δίνει τις ίδιες λαβές **λειτουργικές**. Το `handleLayeringActivation` (listener του `'level-panel:layering-activate'`) μένει ως έχει — ανοίγει toolbox + `showGrips`, **δεν** αλλάζει tool. Αμετάβλητο και το `handleEditingModeChange` (`LevelPanel.tsx:189`): εκεί το `grip-edit` είναι **ρητή** επιλογή του χρήστη στο toolbox, όχι παρενέργεια πλοήγησης. Το bridge effect δεν πειράχτηκε (χειρουργικό fix στη ρίζα, όχι στο σύμπτωμα). tsc SKIP (N.17). ✅ Google-level: YES — διορθώθηκε ο **αιτιώδης** κόμβος (ποιο tool ζητά η πλοήγηση), όχι το τελικό σύμπτωμα· καμία νέα κατάσταση, καμία διπλή διαδρομή. |
