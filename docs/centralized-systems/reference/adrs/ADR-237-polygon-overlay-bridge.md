...
| Πεδίο | Τιμή |
|-------|------|
| **Firestore Collection** | `dxf_overlay_levels/{levelId}/items` |
...
**Τα `dxf_overlay_levels` παραμένουν Single Source of Truth (SSoT)** για polygon geometry. Δεν αντιγράφονται vertices σε Unit documents.
...
│  Firestore:         │ ────────→ │                      │
│  dxf_overlay_levels │  read-    │  useFloorOverlays()  │
...
1. **Ένα Firestore collection** — `dxf_overlay_levels` παραμένει SSoT
...
- ❌ `dxf_viewer_levels` Firestore documents δεν αποθηκεύουν `floorId`
...
| Αποθήκευση `floorId` στα `dxf_viewer_levels` Firestore documents | `LevelsSystem.tsx` |
...
| Sub-query: `dxf_overlay_levels/{levelId}/items` | onSnapshot() |
...
- ⛔ **ΜΗΝ αντιγράφεις** polygon vertices σε Unit documents — SSoT = `dxf_overlay_levels`
...
| 2026-04-05 | **ADR-289**: Overlay polygon item writes (`dxf_overlay_levels/{levelId}/items/{overlayId}` subcollection) migrated to centralized `/api/dxf-overlay-items` endpoint. Direct client-side `setDoc`/`updateDoc`/`deleteDoc` removed from `overlay-store.tsx` (add/update/remove/restore). `onSnapshot` readers unchanged. Tenant scoping + audit now fully server-side. | Claude Code |
| 2026-04-16 | **Overlay entity dropdown fix**: `Level` interface got `buildingId?: string`. `linkLevelToFloor(levelId, floorId, buildingId?)` ora salva anche il `buildingId` in Firestore. `OverlayProperties` deriva `buildingId` da `Level.buildingId` (fallback quando `selectedBuilding = null`). Hint UI aggiunto quando il livello non è collegato a un piano. Fix risolve dropdown vuoto per property/parking/storage. | Claude Code |
