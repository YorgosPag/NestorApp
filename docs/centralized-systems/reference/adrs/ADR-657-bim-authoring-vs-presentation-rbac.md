...
> 🔴 **ΔΙΟΡΘΩΣΗ 2026-09-08 (ADR-841 §7 Α21.14): ΤΟ SOFT WARNING ΔΕΝ ΓΡΑΦΤΗΚΕ ΠΟΤΕ, ΚΑΙ ΤΟ
> `rulesRange` ΑΦΑΙΡΕΘΗΚΕ.**
>
> Η κεφαλίδα του `check-storage-rules-test-coverage.js` έγραφε αυτολεξεί *«a non-blocking
> `--verbose` warning is emitted if a range no longer contains its block's parsed start
> line»* — **και καμία τέτοια γραμμή δεν υπήρχε στον κώδικα**: το `entry.rulesRange`
> παρσαριζόταν και δεν διαβαζόταν ποτέ ξανά. Δηλαδή το πεδίο ήταν ανεπικύρωτο **δίπλα σε
> πρόταση που έλεγε ότι επικυρώνεται**.
>
> 📊 **Το κόστος, μετρημένο δύο φορές ανεξάρτητα**: `storage.rules` **11/11 λάθος** *(η ίδια
> η καταγραφή μέσα στο manifest, 2026-08-24)*· `firestore.rules` **124/127 λάθος (98%)**,
> χειρότερη απόκλιση **1853 γραμμές** *(2026-09-08)*. Ενδιάμεσα, μια συνεδρία διόρθωσε
> **και τις 125** εγγραφές παραγωγικά — και **μία μέρα μετά** η μέτρηση ξαναέδινε 98%.
>
> ⇒ Το συμπέρασμα του §3.3 **επιβεβαιώθηκε και ολοκληρώθηκε**: δεν αρκεί να υποβιβάσεις
> έναν αριθμό γραμμής σε τεκμηρίωση — **πρέπει να πάψει να υπάρχει**, γιατί όσο υπάρχει
> διαβάζεται ως αλήθεια. Η θέση παράγεται πλέον κατ' απαίτηση (`--map`) και για τις **δύο**
> πύλες, από κοινό `scripts/_shared/rules-location-map.js`.
>
> 🏆 Ίδια πρακτική με τα εργαλεία που το `CLAUDE.md` ονομάζει πρότυπο: **PHPStan baseline**
> *(«No line numbers», συνειδητή απόφαση)* και **ESLint bulk suppressions** *(v9.24+,
> `{αρχείο: {κανόνας: {count}}}`)*.
...
| 2026-07-15 | **Fail-closed claims shipped** (`auth-context.ts` + `require-project-for-page.ts`) μετά από καθαρό audit (`scripts/audit-missing-auth-claims.js`). **SECURITY INCIDENT κατά το audit**: 2 από 5 χρήστες είχαν κενά claims και άρα, λόγω του fail-open bug, ήταν **de-facto company_admin** στην κύρια εταιρεία. (1) `grigoris.pagonis` → backfill σε super_admin (κατ' εντολή, ίδια δικαιώματα με τον ιδιοκτήτη). (2) `mugeshraotech@gmail.com` → **άγνωστος λογαριασμός** (πιθανή μη-εξουσιοδοτημένη πρόσβαση ~1 μήνα, 16 Ιουν–15 Ιουλ)· **DISABLED**, κρατήθηκε για διερεύνηση (entity_audit_trail / writes με createdBy=`eMAVv04g…`). Audit πλέον: 0 enabled χρήστες χωρίς claims → safe. **N.18 στο commit-gate:** το CHECK 3.28 εντόπισε clone στα δύο credential paths του `buildRequestContext` (Bearer / __session) — κεντρικοποιήθηκε το κοινό decode→claims→context σε `contextFromDecodedToken()`. |
