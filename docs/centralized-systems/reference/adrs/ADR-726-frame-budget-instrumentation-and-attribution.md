...
✅ **ΕΠΑΛΗΘΕΥΤΗΚΕ ΜΕ ΜΕΤΡΗΣΗ (Φ5, production, 2026-07-30):** `frame:dxf-canvas` p50 **1,2ms**
(από min 32,7ms δάπεδο), καρέ p50 16,7ms = **60 FPS**, το pan ΛΥΜΕΝΟ. Η ουρά που έμεινε
(p90 74,6ms στο wheel-zoom) είναι το θέμα της **Φ3.1** παρακάτω — όχι αναίρεση της Φ3.
...
#### Φ3.1 — Gesture-aware acceptance: ΚΑΝΕΝΑ transform-driven rebuild μέσα στη χειρονομία (2026-07-30)

**Το εύρημα της Φ5 (production, βλ. παρακάτω):** το pan λύθηκε, αλλά στο wheel-zoom το
`frame:dxf-canvas` (295 εκτελέσεις) έδωσε p50 **1,2ms** (το blit δουλεύει) αλλά
**p90 74,6ms · p95 111,7 · p99 167,9 · max 184,9ms — 60 ακριβές εκτελέσεις (20%) >33ms**.
Αιτία (κώδικας, όχι υπόθεση): `dxf-bitmap-cache-anchor.ts` → `maxMagnification(dpr) =
max(1.25, dpr)`. Στη dpr=1 οθόνη του μηχανήματος το k ξεπερνά το 1,25 σε 1-2 wheel notches ⇒
το `isAnchoredBlitAcceptable` απορρίπτει ⇒ **full re-raster 2.909 οντοτήτων ΜΕΣΑ στη
χειρονομία** — τα 75-185ms spikes. Zoom-out: το raster παύει να καλύπτει το viewport (τρύπα) ⇒
ίδιο αποτέλεσμα από το άλλο κριτήριο.

**Η απόφαση (μοτίβο Google Maps / Figma — §9.3):** τα κριτήρια ΠΟΙΟΤΗΤΑΣ (θολούρα / τρύπα)
είναι κριτήρια **ηρεμίας**, όχι χειρονομίας. Όσο ο χρήστης κινεί την οθόνη, το raster
σερβίρεται ΟΠΩΣ ΕΙΝΑΙ — θολό ή/και ελλιπές (ο browser ζωγραφίζει background στα κενά) — και
κανένα transform-driven rebuild δεν τρέχει. Rebuild μόνο στο idle, από τον ΥΠΑΡΧΟΝΤΑ μηχανισμό
(`rerasterDue` + `RASTER_IDLE` 120ms), που αποκαθιστά τα ακριβή pixels και ξανακεντράρει το
overscan. Σε CAD όρους: ό,τι κάνει το Figma στο pinch-zoom και το Google Maps στα tiles —
προβολή κατά τη χειρονομία, πιστότητα στην ηρεμία.

**Υλοποίηση (ΜΗΔΕΝ νέα συστήματα — SSOT audit επιβεβαίωσε ότι όλα προϋπήρχαν):**
- «Είμαστε σε χειρονομία;» = `isNavigationGesture()` — το ΥΠΑΡΧΟΝ SSoT
  `systems/navigation/NavigationGestureStore.ts` (ADR-728 Φ1ε: συνάγεται από το transform SSoT,
  ρητό `endNavigationGesture()` μόνο-για-τερματισμό στο pan mouseup). Δεύτερος καταναλωτής του
  store μετά τον snap-scheduler — ακριβώς ο λόγος που χτίστηκε ως singleton.
- Η πύλη ζει στο `DxfBitmapCache.canServe()` (τον CALLER), ΟΧΙ στα pure functions: μέσα σε
  χειρονομία μια προβολή σερβίρεται αρκεί να είναι **αριθμητικά έγκυρη** — νέο pure predicate
  `isAnchoredBlitUsable(rect, k)` στο anchor module (finite k>0, finite dx/dy), το οποίο το
  `isAnchoredBlitAcceptable` πλέον καλεί ως πρώτο βήμα (validity ∧ quality). Το anchor module
  δεν έμαθε τίποτα για χειρονομίες.
- **ΔΕΝ αναστέλλονται:** δομική ακύρωση (scene/viewport/dpr/annotation/BIM/toggles — ελέγχεται
  ΠΡΙΝ το `canServe` στο `isDirty`, αλλιώς stale pixels μετά από layer toggle κατά το pan),
  `invalidate()`, `rerasterDue` (το idle rebuild τρέχει 120ms μετά την τελευταία μετατόπιση,
  δηλαδή σε πρακτική ηρεμία, ακόμα κι αν το gesture idle παράθυρο των 220ms δεν έληξε), και το
  πρώτο χτίσιμο (`anchor === null` ⇒ δεν υπάρχει τι να προβληθεί).

**Τίμημα (δηλωμένο):** στο γρήγορο zoom το σχέδιο μένει θολό/ελλιπές για όσο κρατά η χειρονομία
+ ~120ms — η ίδια ανταλλαγή που δήλωσε η Φ3, απλώς εφαρμοσμένη ΚΑΙ μέσα στη χειρονομία, όπου
πριν «έσπαγε» με τον πιο ακριβό δυνατό τρόπο (rebuild στο καρέ). Χειρότερη περίπτωση αμετάβλητη:
ένα rebuild στην ηρεμία.

**Tests:** νέο describe «gesture-aware acceptance» στο `dxf-bitmap-cache-anchored-raster.test.ts`
(αναστολή ποιοτικών κριτηρίων, ΜΗ αναστολή δομικών/invalidate/rerasterDue/πρώτου χτισίματος,
επαναφορά αυστηρών κριτηρίων στο τέλος) + validity tests του `isAnchoredBlitUsable` στο
`dxf-bitmap-cache-anchor.test.ts`. **Mutation-verified** (αφαίρεση πύλης / πύλη που αναστέλλει
και τα δομικά / validity που επιστρέφει πάντα true ⇒ κόκκινα).

✅ **ΕΠΑΛΗΘΕΥΤΗΚΕ ΜΕ ΜΕΤΡΗΣΗ (production, ίδια μέρα, δύο ανεξάρτητα πειράματα):**
1. **Ανθρώπινο wheel-zoom-heavy pan/zoom** (Giorgio, 313 εκτελέσεις `frame:dxf-canvas`):
   p50 1,1 / p75 1,7 / **p90 9,9ms (από 74,6)** / p95 83,9 / max 142,9 · >33ms **9,9%** (από 20%)
   · >70ms 23 (7,3%). Καρέ συνολικά: p50 16,7ms = 60 FPS.
2. **Το αποφασιστικό: συνθετικό ΣΥΝΕΧΕΣ zoom 5s** (125 wheel events ανά 40ms, καμία παύση,
   tab ορατό): 82 εκτελέσεις, p50 1,3 / p90 3,3 / max 96,3ms — **ακριβώς ΕΝΑ καρέ >70ms**,
   το σχεδιασμένο idle re-raster ΜΕΤΑ το τέλος της χειρονομίας. ⇒ **ΜΗΔΕΝ transform-driven
   rebuild μέσα στη χειρονομία** — χθες το ίδιο σενάριο πλήρωνε rebuild ανά 1-2 notches.

Τα 23 >70ms του ανθρώπινου (1) είναι τα **σχεδιασμένα rest-time re-rasters** — ένα μετά από
κάθε παύση ριπής zoom (πολλές ριπές ⇒ πολλά νόμιμα rebuilds σε ηρεμία), όπως αποδεικνύει το (2):
όταν η χειρονομία δεν διακόπτεται, το ακριβό καρέ είναι ένα και συμβαίνει στην ηρεμία.

### Φ4 — `fillText` cache (§4.Ε) και three.js teardown σε 2D (§4.Δ)

### Φ5 — Επαναμέτρηση σε **production build** · ✅ **ΕΓΙΝΕ (2026-07-30) — 60 FPS median, το pan ΛΥΜΕΝΟ**

**Πρωτόκολλο:** production build (`next build`, hashed chunks — επιβεβαιωμένο), `47_ergasia.dxf`
2.909 στοιχεία, tab ορατό+εστιασμένο (`hasFocus:true` ✓), 3.050 καρέ πραγματικού pan/zoom από
τον Giorgio, όργανο = `__dxfPerf` (flag `dxf-perf-trace`) + ανεξάρτητος rAF-interval recorder +
LoAF. Πλήρες αναπαράξιμο πρωτόκολλο + παγίδες build του μηχανήματος: HANDOFF 2026-07-30 §4.

| Δείκτης | Τιμή |
|---|---|
| Καρέ p50 / p75 | **16,7ms / 16,7ms = 60 FPS** |
| Καρέ p90 / p99 | 49,8ms / 83,4ms |
| Καρέ >33ms / >70ms | 11,5% / **1,8%** |
| `frame:TOTAL` (δικός μας rAF κώδικας) | **p50 0,0ms · p95 1,4ms** — πρακτικά δωρεάν |
| `frame:snap-detection` | **απόν** (η αναστολή ADR-728 Φ1 δουλεύει σε prod) |
| Όλα τα 13 canvas frame stages εκτός dxf-canvas | ≤0,2ms |
| ⚠️ `frame:INTERVAL` avg/max | δηλητηριασμένα από 1 idle gap 37,8s πριν τη χειρονομία — μόνο percentiles μετράνε (§5.2) |

**Σύγκριση: πριν 3 μέρες ~12 FPS σταθερά · τώρα 60 FPS median σε production.** Τα παλιά
