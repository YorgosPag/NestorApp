...
| 2026-06-13 | **Company doc audit-field parity (`_lastModified*`)** — read-only έλεγχος άδειας βάσης (Giorgio) ανέδειξε ότι το materialized `companies/{comp_*}` doc είχε μόνο `createdBy`/`createdAt`, ενώ ΟΛΑ τα entity docs (contact/project/building/floor μέσω `buildCommonFields` στο `entity-creation.service`) φέρουν `_lastModifiedBy`/`_lastModifiedByName`/`_lastModifiedAt`. FIX (`company-document.service.ts`): NEW `resolveActorDisplayName(db, uid)` helper (null για system/unknown, mirror του audit convention) → `ensureCompanyDocument` stampάρει τα 3 fields στο `docData` + περνά `performedByName: actorName` στο audit recordChange (αντί null)· DRY: η μία resolution του actor name τροφοδοτεί ΚΑΙ το company-name last-resort fallback ΚΑΙ το audit stamp (αφαιρέθηκε η διπλή gated user-lookup)· `repairCompanyDocument` stampάρει επίσης `_lastModified*` + `performedByName` στο update. `CompanyDocument` type +3 optional fields· `getCompanyDocument` τα surfaceάρει. IDE diagnostics καθαρά (2 αρχεία). **ΣΗΜΕΙΩΣΗ — lazy materialization ΔΕΝ είναι bug:** το doc υλοποιείται on-demand (phantom→real) by-design· eager provision-at-registration = ADR-439 Phase 3 (provision-tenant), DEFERRED (product-απόφαση Giorgio). 🔴 browser-verify (φτιάξε νέο tenant → companies doc φέρει `_lastModifiedBy`/`_lastModifiedByName`) + commit (Giorgio). | Opus |
| 2026-03-13 | **Phase 4: Legacy Company ID Migration** — New endpoint `POST /api/admin/migrate-company-id` (GET=dry-run, POST=execute). Migrates `pzNUy8ksddGCtcQMqumR` → `comp_xxx` across: (1) Company document creation at new ID, (2) Firebase custom claims for all users, (3) `companyId` field in 17 collections (projects, buildings, contacts, units, files, conversations, messages, tasks, obligations, leads, opportunities, activities, communications, ai_pipeline_queue, floors, parking_spots, storage_units), (4) `contactId` in navigation_companies, (5) RBAC subcollections (audit_logs, projects/members, units/grants) — copy + delete, (6) Old company document cleanup. | Claude Code (Anthropic AI) |
| 2026-03-17 | **cadFiles → files Dual-Write**: `DxfFirestoreService.saveToStorage()` now dual-writes to both `cadFiles` (legacy) and `files` (enterprise FileRecord). `CAD_FILES` collection marked `@deprecated`. `DxfSaveContext` interface for optional entity context injection. Non-blocking — cadFiles remains primary. | Claude Code (Anthropic AI) |
