...
## Changelog
- **2026-07-04 — v1.1 — false-positive trigger fix (Giorgio browser bug: «περιέστρεψα μια ΓΡΑΜΜΗ
  και βγήκε "Εντοπίστηκε κούφωμα"»).** Το `DxfSymbolDetectHost` άκουγε **κάθε**
  `drawing:entity-created {tool:'wall'}` ως «νέος τοίχος → σκάναρε σύμβολα». Όμως το event αυτό
  είναι **υπερφορτωμένο**: το εκπέμπει και ο **wall-retrim persistence re-save**
  (`add-wall-to-scene.recomputeWallTrims` + neighbour re-emit), ο οποίος πυροδοτείται από
  `useWallRetrimEffect` σε **`bim:entities-moved`** — event που εκπέμπεται για move/rotate
  **ΟΠΟΙΟΥΔΗΠΟΤΕ** entity (μέσω `SnapshotTransformCommand → reconcileAssociativeGeometry`),
  ακόμη κι άσχετης γραμμής. Αλυσίδα: rotate γραμμής → `bim:entities-moved` → (200ms) retrim →
  `drawing:entity-created{tool:'wall'}` για κοντινό τοίχο → detector → dialog. **Fix (χειρουργικό,
  semantic de-overload):** NEW optional `origin?: 'create' | 'retrim'` στο payload του
  `drawing:entity-created` (`drawing-event-map.ts`). Τα δύο retrim/persistence re-emits στο
  `add-wall-to-scene.ts` (neighbours + `recomputeWallTrims`) περνούν `origin:'retrim'`· ο host
  κάνει early-return σε `origin==='retrim'` → detection **μόνο** σε πραγματικά νέο τοίχο (create/undefined).
  Το `useWallPersistence` αγνοεί το `origin` (φιλτράρει μόνο `tool`/`type`) → τα retrim re-saves
  συνεχίζουν κανονικά. **Files:** `systems/events/drawing-event-map.ts`, `bim/walls/add-wall-to-scene.ts`,
  `app/DxfSymbolDetectHost.tsx`. ✅ Google-level: YES — semantic split του υπερφορτωμένου event,
  ένας consumer φιλτράρει (detector), persistence contract αμετάβλητο, backward-compat (undefined=create).
  🔴 browser-verify (rotate γραμμή → κανένα dialog· δημιουργία τοίχου με σύμβολο → dialog) + commit.
