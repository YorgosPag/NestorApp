...
## Changelog
- **2026-06-22 (FULL SSoT body-fill — Giorgio order, UNCOMMITTED)** — Ο Giorgio παρατήρησε ότι το φόντο του **τοίχου** είχε διαφορετική διαφάνεια από της **κολώνας** + απαίτησε «ΟΛΑ τα BIM να χρησιμοποιούν τον ΙΔΙΟ κώδικα, FULL SSoT». **ΡΙΖΑ:** μόνο ο `WallRenderer` εφάρμοζε το `adaptFillTintForCanvas` (ADR-509)· οι υπόλοιποι 30+ BIM renderers (κολώνα/δοκάρι/πλάκα/…) ζωγράφιζαν body fill **raw** → σε μαύρο φόντο ο τοίχος boost-αριζόταν (0.18→~0.6) ενώ η κολώνα έμενε 0.22 → ασύμβατη διαφάνεια. **FIX:** NEW `bim/utils/bim-body-fill.ts` (`resolveBimBodyFill(category, cutState, objectStyles, fallbackFill, bgHex?)`) ενοποιεί σε ΕΝΑ κώδικα: `resolveVgFillTint` (ADR-375 V/G) → palette fallback → `adaptFillTintForCanvas` (ADR-509). **Έκταση (Giorgio SSoT audit → «όλοι οι 6 τώρα»):** ΟΛΟΙ οι BIM renderers με το `resolveVgFillTint(...) ?? KIND_FILL` pattern υιοθέτησαν τον helper → το pattern ζει ΜΟΝΟ στο `bim-body-fill`: `WallRenderer`/`ColumnRenderer`/`BeamRenderer`/`SlabRenderer`/`SlabOpeningRenderer` → `resolveBimBodyFill`. `StairRenderer` (per-tread-type fallback glass/default σε ξεχωριστό `stair-render-structure-style.ts`, δεν ταιριάζει στο single-fallback signature) → NEW `adaptBimBodyFill(resolvedFill)` entry του ΙΔΙΟΥ module (V/G tint resolved upstream, adaptive κοινό) → μηδέν διάσπαρτη κλήση `adaptFillTintForCanvas`. + `WALL_CATEGORY_FILL` base alpha → 0.22 σε όλες τις κατηγορίες (= `KIND_FILL` κολώνας) ώστε ίδια διαφάνεια ΚΑΙ σε ανοιχτό φόντο. ⇒ ΟΛΑ τα δομικά BIM body fills == ίδια διαφάνεια σε κάθε φόντο, ΕΝΑΣ κώδικας. ⚠️ ADR-040 critical (entity renderers) → CHECK 6D: stage ADR-040+375+509. Beam/Slab/SlabOpening/Stair 37/37 jest GREEN· Wall 19/19· ColumnRenderer-hatch 16 failures ΠΡΟΫΠΑΡΧΟΝΤΑ (committed `e7df7574` ADR-507 Φ7 poché, stale tests — verified με stash, μηδέν regression από εδώ). DEFER: Foundation/Roof/ThermalSpace/FloorFinish/WallCovering (δικιά τους παλέτα ΧΩΡΙΣ `resolveVgFillTint` → ξεχωριστή εξέταση). 🔴 browser-verify (όλα ίδια διαφάνεια) + commit.
