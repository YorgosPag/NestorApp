...
## Changelog
- **2026-08-04 (max-contrast ink — ΔΙΟΡΘΩΣΗ ΚΑΤΩΦΛΙΟΥ, ADR-739 §38)** — Το `inkForBackgroundRgb` (ο ΕΝΑΣ «στόχος/endpoint» που μοιράζονται `maxContrastInk`, `adaptColorToBackground` και `computeAdaptedFillTint`) έκοβε στο `srgbRelativeLuminance(bg) < 0.5` — το διαισθητικό «μισό». **Μετρήσιμα λάθος**: αντίθεση με λευκό = `1,05/(L+0,05)`, με μαύρο = `(L+0,05)/0,05` ⇒ ισοπαλία στο `L = √(1,05×0,05) − 0,05 ≈ **0,179**`. Σε ΟΛΗ τη ζώνη `0,179 < L < 0,5` επέστρεφε **λευκό ενώ το μαύρο έδινε περισσότερη αντίθεση** (`#868686`: 3,64 vs **5,77** · `#808080`: 3,95 vs **5,32**). 🔑 **Γιατί έμεινε αόρατο**: **κανένα** από τα 9 προκαθορισμένα θέματα καμβά δεν πέφτει στη ζώνη (πλησιέστερο `cinema4d #5b5b5b`, `L≈0,105`) — άρα η ζωντανή συμπεριφορά των τοίχων/γεμισμάτων **δεν άλλαξε**. Έγινε ορατό μόλις το **αυτόματο μελάνι κελιού** (ADR-739 §38) άρχισε να ρωτά το ίδιο SSoT και για **γεμίσματα κελιών**, που είναι αυθαίρετα χρώματα του χρήστη και πέφτουν εκεί μέσα συνέχεια (όπως και το `custom` φόντο καμβά). **FIX**: ρητή σύγκριση `contrastRatioRgb(λευκό, bg) >= contrastRatioRgb(μαύρο, bg)` — η σταθερά `0,179` **ΔΕΝ γράφεται** (θα ήταν τρίτη διατύπωση της ίδιας εξίσωσης δίπλα στο `contrastRatioRgb` που την υλοποιεί ήδη), και η `maxContrastInk` κάνει πλέον **κυριολεκτικά** ό,τι λέει το όνομά της. ⚠️ **Η έρευνα βρήκε δεύτερη σχολή** (Myndex/APCA: flip στο `Y≈0,38`, «perceptual middle» — πιο κοντά στο παλιό 0,5 παρά στο σωστό WCAG). **ΔΕΝ την ακολουθούμε**: το έργο κρίνει **κάθε** οντότητα με λόγο αντίθεσης WCAG (`MIN_ENTITY_CONTRAST`, `MIN_FILL_CONTRAST`, CHECK 3.32) — ένα μελάνι που μεγιστοποιεί άλλη μετρική από αυτήν με την οποία μετριέται θα ήταν δύο απαντήσεις στην ίδια ερώτηση. Έλεγχοι: `adaptive-entity-color|wall-render-palette|WallRenderer|stair-render-structure-style` **4 σουίτες / 64 tests** ✅ (καμία αλλαγή αναμενομένου) + νέο `bim/table/__tests__/table-ink.test.ts` που σαρώνει **ΟΛΟ** το φάσμα του γκρι και απαιτεί ο επιλεγμένος να είναι ο νικητής της αντίθεσης.
