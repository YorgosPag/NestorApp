...
**Status:** 🟢 Φάση 1 (parser + stair extractor + BIM mapper + scene builder + import service) **IMPLEMENTED + UNIT-VERIFIED** (18 jest GREEN) + **REAL-FILE-VERIFIED** στο `ΣΚΑΛΑ.tek` (FESPA 9.1.0.46) 2026-06-25. 🟢 Φάση 2 (UI wiring: ribbon κουμπί «Εισαγωγή Τέκτονα» + native file picker + scene-load + stair first-save) **CODE-COMPLETE** (tsc clean) — 🔴 browser-verify εκκρεμεί. 🟢 Φάση 3 (**EXPORT** σκαλών `<stair>` type 21 → `.tek`, ο αντίστροφος του import) **CODE-COMPLETE + UNIT-VERIFIED** (νέα tek-stair-export suite + ενημερωμένα tek-export/adapter suites — όλα GREEN) 2026-06-25 — 🔴 Τέκτων round-trip verify (Giorgio) εκκρεμεί. 🟢 Φάση 4 (Wizard import wiring: `.tek` δεκτό από «Εισαγωγή Κάτοψης (Wizard)» — reuse Φ1+Φ2 pipeline· 4 jest GREEN) **CODE-COMPLETE** — 🔴 browser-verify εκκρεμεί. 🟢 Φάση 5a (2Δ primitives import: `<line>` type 4 + `<arc>` type 5 + `<text>` type 3 → `LineEntity`/`ArcEntity`/`CircleEntity`/`TextEntity`· αντιστροφή export· σωστά χρώματα RGB· inline `<s>` κείμενο· 29 jest GREEN) **CODE-COMPLETE** — 🔴 browser-verify εκκρεμεί. 🟢 Φάση 5 (**Wizard import PERSISTENCE**: το `.tek` ανεβαίνει από την ΙΔΙΑ διαδρομή με το DXF → canonical `fileId` → `linkSceneToLevel` + scene-blob auto-save → **επιβιώνει hard-refresh**· λύνει το Φ4 storage-DEFER· 5 jest GREEN + jscpd clean) **CODE-COMPLETE** — 🔴 browser-verify εκκρεμεί. 🟢 Φάση 6 (**ΕΥΘΕΙΑ σκάλα fix**: sentinel-`(0,0)` filter + `landings`-based straight/winder classification + centerline-from-rails placement + generator-geometry suppression «αντικατάσταση όχι overlay»· 114 tek jest GREEN + jscpd clean) **CODE-COMPLETE** — 🔴 browser-verify εκκρεμεί. UNCOMMITTED. Commit μόνο Giorgio.
...
### Αποφάσεις (Φ4a)
- **Storage:** ~~μηδέν... Generic file-upload storage path = DEFER~~ → **ΛΥΘΗΚΕ στη Φ5** (2026-07-09): το `.tek` ανεβαίνει τώρα από τον ίδιο legacy uploader με το DXF (FileRecord + Storage) ώστε η κάτοψη να σώζεται/συνδέεται στο επίπεδο και να επιβιώνει σε refresh. Οι σκάλες συνεχίζουν να persist-άρουν και μέσω `StairPersistenceHost` (belt-and-suspenders).
...
## Changelog
- **2026-07-11 — Φ6c (2 σειρές «ψωμιών» — διπλότυπες τριγωνικές + ορθογώνιες σε λάθος ύψος· browser-found, screenshots Giorgio):** μετά το Φ6b, τα μπετά κάθε σκαλοπατιού ανέβηκαν αλλά εμφανίστηκαν **δύο σειρές**: ορθογώνιες πλάκες (σωστό σχήμα) 150mm **πολύ ψηλά** + τριγωνικές (διπλότυπες, 2 τρίγωνα = 1 ορθογώνιο) στο σωστό ύψος. **Αιτία:** ο Τέκτων εξάγει τον όγκο μπετού κάθε σκαλοπατιού ως **τριγωνοποιημένο 3Δ mesh** — για κάθε πάτημα μία ΟΡΘΟΓΩΝΙΑ άνω όψη (4 κορυφές, στο ύψος πάνω παρειάς) + 2 ΤΡΙΓΩΝΙΚΕΣ όψεις (ίδια βάση, 1 πάχος χαμηλότερα = «κάθισμα»)· εισαγόμενες ως slabs → 2 σειρές (data-verified: 100 planes → 34 τρίγωνα @26,203,379…, 66 quads @0/26/176/203…· kept 49 = 32 τρίγωνα + 17 quads @176,353…3000). **FIX (Giorgio 2026-07-11):** νέο `io/tek/tek-stair-plane-refine.ts` (`refineStairMeshPlanes`) — για πλάκες που είναι stair-generated (elev1≈0 + κέντρο εντός footprint σκάλας): πετά τις τριγωνικές· κατεβάζει τις ορθογώνιες κατά το πάχος μπετού (διάμεσος του κενού ορθογώνιας↔πλησιέστερης τριγωνικής από κάτω = 150mm) στο ύψος καθίσματος. Wiring στον `tek-scene-builder` πριν το `tekPlaneToSlabEntity`. Κανονικές δομικές πλάκες (elev1 ορισμένο ή εκτός σκάλας) **αμετάβλητες**. Verify: 49 → **17 ορθογώνιες πλάκες, 0 τριγωνικές** @26,203,379…2850 (μία καθαρή σειρά ανά σκαλοπάτι). Tests: νέο `tek-stair-plane-refine.test.ts` (4)· **120 tek jest GREEN** + jscpd clean. 🔴 browser-verify (Giorgio). UNCOMMITTED.
- **2026-07-11 — Φ6b (τα «ψωμιά» μπετού κάθε σκαλοπατιού στο z=0· browser-found, screenshots Giorgio):** μετά το Φ6 fix η σκάλα βγήκε ευθεία, αλλά οι **49 πλάκες** (`<plane>` type 10 — οι οριζόντιες όψεις μπετού κάθε σκαλοπατιού· οι κατακόρυφες κόβονται ως zeroArea) εμφανίζονταν όλες **επίπεδες στο z=0** αντί στο ύψος του κάθε σκαλοπατιού. **Root cause:** ο Τέκτων αφήνει `<elev1>0</elev1>` σε πλάκες που παράγει από 3Δ αντικείμενα, κρατώντας το πραγματικό ύψος ΜΟΝΟ στα `<point3d>` `pointZ` (π.χ. 2.6735, 2.85 m)· ο `extractPlaneRecords` διάβαζε μόνο `pointX/pointY` (έριχνε το Z) και ο `tekPlaneToSlabEntity` έπαιρνε στάθμη από `elev1`=0 → όλες στο 0. **FIX (ADR-531 Φ5b.4 slab path, χειρουργικό — μόνο η στάθμη):** `+TekPlaneRecord.baseElevationM` (optional) = `min(pointZ)` των κορυφών· `extractPlaneRecords` το υπολογίζει· `tekPlaneToSlabEntity` χρησιμοποιεί `elev1` όταν ορίζεται, αλλιώς (elev1≈0) το `baseElevationM` → κάθε «ψωμί» κάθεται στο ύψος του σκαλοπατιού (verify: 49 πλάκες κατανεμημένες 26→3000 mm ανά ρίχτι, όχι στο 0). Κανονικές πλάκες με elev1 ορισμένο = αμετάβλητες. Tests: `tek-plane-to-slab.test.ts` +2 (fallback από polygon Z· elev1 υπερισχύει)· **116 tek jest GREEN** + jscpd clean. 🔴 browser-verify (Giorgio). UNCOMMITTED.
- **2026-07-10 — Φ6 (ΕΥΘΕΙΑ σκάλα: ψεύτικη στροφή ~30° + διπλότυπα· browser-found, screenshots Giorgio):** εισαγωγή `.tek` με **ευθύγραμμη** σκάλα (`ΣΚΑΛΑ-2.tek`: πλάτος 1.2m, 16 πατήματα, ΔΥψος 3m, `landings=0`) εμφανιζόταν **στριφτή ~30°** (winder), σπασμένη σε 2 κλάδους στο 3D, με τεράστια labels + διπλότυπες 2Δ γραμμές. **3 root causes στον `tek-stair-to-bim.ts`:** (1) **sentinel `(0,0)`** (padding των `<point2d>` του Τέκτονα) ΔΕΝ φιλτραριζόταν — απόσταση ~11m < κατώφλι 20m του `dropOutliers` → `derivePlacement` έβγαζε heading προς την αρχή αξόνων → τεράστιο `turnDeg`. (2) `resolveVariant` βασιζόταν **μόνο** στο θορυβώδες `turnDeg`, αγνοώντας το authoritative `rec.landings` (=0 ⇒ ευθεία) που ήδη εξαγόταν. (3) `pickWalkline` διάλεγε τη **μεγαλύτερη** polyline = λίστα ακμής (x=6.55), όχι το κέντρο (x=7.15) → `basePoint` μετατοπισμένο κατά μισό πλάτος. **FIX:** `+isSentinel`/`cleanPolyline` (drop `(0,0)` πριν outliers)· `+deriveCenterline` = μέσος όρος των **δύο εξωτερικών ακμών** (`mostDetailed`+`pickRails`+`averageRails`) → πραγματικό κέντρο, διατηρεί καμπυλότητα για winder· `resolveVariant(turnDeg, stepCount, landings)` → straight όταν `landings===0 && |turnDeg|<20°`. **Αντικατάσταση (όχι overlay):** ο Τέκτων αποθηκεύει τη σκάλα ΚΑΙ ως semantic `<stair>` ΚΑΙ ως ~37 `<line>` (ακμές βαθμίδων) + `<text>` (αριθμοί) — ο `tek-scene-builder.ts` (`+dropGeneratorGeometry`/`inflate`/`isInside`) πετά πλέον τα 2Δ primitives **εντός** του footprint κάθε σκάλας (2% margin, μέσω SSoT `calculateBimEntity2DBounds`+`primitiveBounds`) → μένει μόνο το native BIM μοντέλο· primitives εκτός σκάλας ανέπαφα + warning πλήθους. Tests: `tek-stair-to-bim.test.ts` +ρεαλιστικό multi-polyline straight case (κέντρο basePoint, straight variant, σωστές διαστάσεις)· νέο `tek-scene-builder-suppression.test.ts` (εντός→drop, εκτός→keep, warning). **114 tek jest GREEN** (καμία regression) + jscpd clean. 🔴 browser-verify (Giorgio: `ΣΚΑΛΑ-2.tek` → ευθεία, χωρίς διπλότυπα) + commit. UNCOMMITTED. **Εκτός scope:** κλίμακα κειμένου annotations εκτός σκάλας· λεπτομέρεια waist/μπετού κάτω από κάθε βαθμίδα (επαλήθευση μετά το fix).
- **2026-07-09 — Φ5 (Wizard import PERSISTENCE· «η κάτοψη .TEK χάνεται σε refresh»):** Ο Giorgio ανέφερε ότι κάτοψη εισαγμένη με το «Εισαγωγή Κάτοψης (Wizard)» ζωγραφιζόταν αλλά **χανόταν μετά την ανανέωση** (δεν σωζόταν σε βάση/Storage). **Root cause (grep-verified):** το Φ4 έκανε ρητά DEFER το storage — το `.tek` short-circuit στο `useFloorplanSmartUpload.uploadSmart` επέστρεφε `{success:true, format:'tek'}` **χωρίς `fileId`** → `resolvedFileRecordId=undefined` στο `handleFileImport` → το `linkSceneToLevel` **παρακαμπτόταν** (`useSceneState.ts:178`) → καμία εγγραφή `sceneFileId` στο `dxf_viewer_levels` + κανένα scene blob στο Storage → η scene ζούσε ΜΟΝΟ in-memory → hard-refresh = κενή κάτοψη (`useLevelSceneLoader` δεν βρίσκει `sceneFileId`). **FIX (option A — ίδιο SSoT με DXF, industry-standard: archive source + persist derived + link):** το `.tek` περνά τώρα από τον **ίδιο** legacy uploader με το DXF (`legacy.uploadFloorplan` → `uploadFileWithPolicy` → FileRecord + Storage) → ρέει canonical `fileId` → πυροδοτείται `linkSceneToLevel` + auto-save του derived scene blob → επιβιώνει refresh. Αλλαγές: (1) NEW `io/tek/tek-filename.ts` — pure `isTekFileName` (εξαγωγή από το heavy `tek-import.ts` ώστε ο generic `useFloorplanUpload` να ΜΗΝ σέρνει τον tek importer· `tek-import.ts` re-export = SSoT, zero διπλότυπο). (2) `useFloorplanUpload.ts` — validation δέχεται `.tek` (`isTekFileName`)· server DXF/PDF processing skip για tek (parse γίνεται client-side). (3) `useFloorplanSmartUpload.ts` tek branch → `legacy.uploadFloorplan` → επιστρέφει `fileId` (πρώην χωρίς). Downstream αμετάβλητο (`handleFileImport`/`linkSceneToLevel`/auto-save δουλεύουν μόλις υπάρχει `fileId`). Το `importTekFile` παραμένει η client-side parse SSoT. Tests: `useFloorplanSmartUpload.tek.test.tsx` (5 jest GREEN: tek→fileId, failure propagation, dxf no-regression)· tek-import 6/6 GREEN (re-export μη-regression)· jscpd clean. 🔴 Giorgio browser-verify (import .tek → refresh → κάτοψη παραμένει) + commit. UNCOMMITTED.
- **2026-06-25 — Φ5a text font (browser-found):** ο Giorgio ανέφερε «η γραμματοσειρά αντικαθίσταται με άλλο είδος». Αιτία: ο text renderer (`TextRenderer.ctx.font`/`fillText`) διαβάζει font ΜΟΝΟ από `textStyle.fontFamily` (default `'arial'`)· το import μου δεν περνούσε τη γραμματοσειρά του αρχείου. FIX: `extractTextRecords` διαβάζει `<ttfont><name>` (π.χ. "Arial") → `TekTextRecord.fontFamily` → `tekTextToEntity` θέτει `TextEntity.fontFamily`. Επειδή το `textStyle` παράγεται από `textNode` (που το import δεν έχει), επεκτάθηκε ο `dxf-scene-entity-converter` (text case) ώστε flat `entity.fontFamily` → `textStyle.fontFamily` όταν λείπει (additive· DXF text παίρνει font από textNode → αμετάβλητο). 29 tek jest GREEN. 🔴 browser-verify (Giorgio).
- **2026-06-25 — Φ5a text import (browser-found):** ο Giorgio ανέφερε «τα κείμενα δεν εμφανίζονται». **Ανατροπή προηγούμενης υπόθεσης:** το `<text>` (type 3) `<s>N</s>` ΔΕΝ είναι external string-pool index — είναι το **inline κείμενο** (επιβεβ. σε πολλά αρχεία: `<s>ΚΟΥΖΙΝΑ</s>`, `<s>Ε = 70.77 τμ</s>`· στο `ΚΑΤΟΨΗ` ψηφία `<s>1</s>..<s>9</s>`). Υλοποίηση: `+extractTextRecords` (type 3· `+readXMatrix` SSoT helper), `+tekTextToEntity` → `TextEntity` (content από `<s>`, θέση από xmatrix x20/x21 μέσω `tekMetersToScene`, alignment από `hallign`, rotation από u-άξονα Y-flipped, color μέσω `tekColorToHex`). `+TekTextRecord`/`TekXMatrix` types, `texts` στο `TekSceneParseResult`/scene-builder/`primitiveBounds`, `stats.textCount`. **Ύψος:** από το xmatrix vertical scale — baked-scale mode (font=0, scale~0.006) × `TEK_OUTLINE_GLYPH_EM=40` (καλιμπραρισμένο από `ΚΑΤΟΨΗ` → ~0.25m label)· native mode (scale≈1) → scale. ⚠️ tunable — Φ5b refine για native-ttfont room labels. 29 tek jest GREEN. 🔴 browser-verify (Giorgio· έλεγξε ΚΑΙ μέγεθος κειμένου).
- **2026-06-25 — Φ5a color fix (browser-found):** ο Giorgio ανέφερε «κάποια χρώματα μαύρα/αόρατα στον σκούρο καμβά». **2 root causes:** (1) ο mapper παρήγαγε χρώμα **χωρίς `#`** → όσα είχαν επαρκές contrast επιστρέφονταν αυτούσια από το `adaptEntityColorForCanvas` (ADR-509) → `ctx.strokeStyle='00805C'` **άκυρο CSS** → μαύρο/αόρατο (όσα χρειάζονταν adaptation περνούσαν από `mixHex` με `#` → φαίνονταν· γι' αυτό «κάποια»). (2) **BGR swap λάθος** — ο export γράφει RGB **αυτούσιο** (`colorHex6`, μόνο strip `#`). FIX: νέο `tekColorToHex(raw) = '#' + colorHex6(raw)` (reuse export SSoT, ακριβής αντιστροφή· `colorBgr`→`color` raw RGB). 26 tek jest GREEN. 🔴 browser-verify (Giorgio).
