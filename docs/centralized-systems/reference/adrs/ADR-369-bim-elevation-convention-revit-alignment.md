...
| Date | Author | Change |
|------|--------|--------|
| 2026-07-27 | Giorgio + Claude (Opus 5) | **ADR-650 §M10e — τα `basePoint`/`northRotation` αποκτούν ΑΥΤΟΜΑΤΟ παραγωγό, χωρίς νέα διαδρομή αποθήκευσης.** Το `autoMatchToSurvey()` (`systems/geo-referencing/geo-auto-match.ts`) προτείνει το ζεύγος «Project Base Point + Angle to True North» ταυτίζοντας το εισαγμένο DXF με τα σημεία τοπογραφικού. **Καμία αλλαγή στο schema αυτού του ADR**: το αποτέλεσμα είναι ο **υπάρχων** runtime `GeoReference{originWorld, rotationDeg}` και γράφεται μέσω των **υπαρχόντων** `setGeoReference` + `persistProjectGeoReference` → `Project.basePoint` (μέτρα) / `northRotation`, με τη μετατροπή mm↔m να μένει στο `geo-reference-schema.ts` boundary. **Δύο δεσμεύσεις που αφορούν άμεσα αυτό το ADR:** (α) **ΠΟΤΕ κλίμακα** — ο estimator υπολογίζει `scaleEstimate` και **αρνείται να τον εφαρμόσει**· `\|s−1\| > 0,002` ⇒ αναφορά `unit-mismatch` και **κανένα** geo. Rigid μόνο, όπως Revit Shared Coordinates / Civil 3D real-world coords: μια γεωαναφορά με κλίμακα θα άλλαζε σιωπηλά κάθε εμβαδόν, όγκο και γραμμή του BOQ. (β) **ΠΟΤΕ auto-apply** — επιστρέφεται **πρόταση** με κάρτα απόδειξης (μέθοδος, layer, inliers/δυνατοί, RMS σε εκ., στροφή)· ο μηχανικός πατά «Εφαρμογή». Δες **ADR-650 §M10e v21**. |
