...
- **Status**: 🚧 IN_IMPLEMENTATION — Phase A1-A5 + B + D + F + G + H + 0.4+A.1+0.3 + Q2.1 + Q2.2 + Q2.3 + Q2.4 + Q3 + **Q8.2** ✅ complete 2026-05-21. Phase C (Firestore migration) N/A. Q&A 10/10 complete.
- **Date**: 2026-05-20
...
8. **Q8 — IFC export readiness;** ✅ **ANSWERED 2026-05-20**: **Επιλογή Γ — Full IFC Export τώρα (writer + schema)**. IFC4 (ISO 16739-1) compliant exporter shipped από Phase 1. Schema IFC-ready από day 1. Writer functional + UI export button. Validation με BIMvision/Solibri.
   ...
   **Persistence**: `entity.pset` field (already in schema) → Firestore update.
   ...
   ### Q8.3 — IfcExporter Core + Spatial Hierarchy (~12h)

   **Goal**: Foundation service που ξέρει να χτίζει IFC4 file skeleton.

   **Files:**
   - `src/services/ifc/ifc-exporter.service.ts` (NEW) — main orchestrator
   - `src/services/ifc/ifc-spatial-hierarchy.ts` (NEW) — Project→Site→Building→Storey chain
   - `src/services/ifc/ifc-units.ts` (NEW) — IfcSIUnit + IfcGeometricRepresentationContext
   - `package.json`: `web-ifc` (MIT license ✅, ThatOpen Engineering)
