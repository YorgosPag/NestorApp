...
- **2026-07-16 — 🔴 RC3/RC4 ΞΑΝΑ, στον δίδυμο writer: `AssignOpeningTypeCommand` (εντοπίστηκε μέσω ADR-663 tsc burn-down).** Τα RC3/RC4 (2026-07-09) διορθώθηκαν **μόνο** στον `UpdateOpeningParamsCommand`. Ο `AssignOpeningTypeCommand` — ο δεύτερος writer που πατάει `params` + DERIVED geometry, μέσω family-type assign/clear/override — κρατούσε **αντιγραμμένο** `resolveHostWall()` και είχε **και τα δύο bugs ζωντανά**:
  - **(RC3′)** `validate(): if (!this.next.params.wallId) return 'Opening params.wallId is required'` → **κάθε** family-type assign/clear σε self-hosted κούφωμα απορριπτόταν **σιωπηλά** (ο caller κάνει `if (validate() !== null) return`).
  - **(RC4′)** `applyState()` δεν είχε self-hosted κλάδο → `resolveHostWall(undefined)` = null → soft-orphan: **μόνο validation, ΟΧΙ geometry**. Ένα type-driven `width`/`height` άφηνε `outline/position/rotation` **stale**.
  - **Root cause (γιατί ξανασυνέβη):** η επιλογή host-κλάδου ήταν **copy-paste ανά command**, όχι SSoT. Ο ένας καταναλωτής ενημερώθηκε για το ADR-615, ο δίδυμος έμεινε πίσω — και τίποτα δεν το έπιασε (ο `wallId?: string` έκανε το λάθος να φαίνεται σαν type noise αντί για bug).
  - **Fix (SSoT, N.0.2/N.18):** νέο `core/commands/entity-commands/opening-derived-state.ts` — `resolveOpeningHostWall()` + `computeOpeningDerivedState()` (self-host / wall-host / soft-orphan branch **μία φορά**) + `applyOpeningDerivedPatch()` (γράφει `params`/`kind`/`ifcType`/`geometry`/`validation`, με `extraPatch` για το command-specific `typeId`/`typeOverrides`) + `validateOpeningHostRef()`. **Και οι δύο** commands περνούν πλέον από εκεί· μηδέν αντιγραμμένο `resolveHostWall`. Ένας νέος opening writer δεν μπορεί πια να «ξεχάσει» τον self-hosted κλάδο.
  - **Tests:** νέο `AssignOpeningTypeCommand.test.ts` (6 tests) — wall-hosted recompute, **self-hosted validate() δεκτό** (RC3′ regression), **self-hosted geometry re-derived** (RC4′ regression), self-hosted undo, hostless rejection, soft-orphan διατήρηση geometry. 549/550 πράσινα στα entity-commands + cascade + validators (το μόνο fail = προϋπάρχον `move-entity-geometry-coverage`, άσχετο· βλ. ADR-663 §4). `jscpd:diff` καθαρό στα 15 αρχεία.
- **2026-07-16 — `wallId?: string` consumers ευθυγραμμίστηκαν (13 σημεία, ADR-663 burn-down).** Το `wallId` είναι **σκόπιμα** optional (self-hosted discriminator), αλλά 13 καταναλωτές το διάβαζαν σαν να ήταν πάντα παρόν — δηλαδή **αγνοούσαν τα self-hosted κουφώματα**, όχι «λάθος τύπος». Όλοι περνούν πλέον από το υπάρχον `isWallHostedOpening()` type-predicate SSoT (γραμμένο ακριβώς γι' αυτό, ADR-615): group-by-wall (`bim-readonly-render`, `dxf-renderer-frame-builders`, `bim-to-tek` — ένα self-hosted θα έμπαινε σε bucket με κλειδί `undefined`), set-membership (`envelope-boq-sync`, `envelope-element-applicator`, `bim-cascade-resolver` — self-hosted δεν κρέμεται σε τοίχο ⇒ κανένα reveal/cascade), IFC (`ifc-opening-serializer` δεν γράφει `IfcRelVoidsElement`, `ifc-covering-serializer` δεν έχει τοίχο να επενδύσει), host-lookup (`useFloorplanBimEntities`, `ifc-bim-scene-loader`, `opening-type-resolution` — `null` host = **φυσιολογικό** για self-hosted, όχι σφάλμα), και `EnvelopeOverlay` (self-hosted δεν τρυπά band μόνωσης).
