...
- **2026-06-27** — **β fix: text grip occlusion** (Giorgio: «στο κείμενο δεν υπάρχει λαβή»). Το flat text-
  plane (Mesh) **δεν** εξαιρούνταν από το grip depth-occluder prepass (μόνο Line/Points/Sprite) → σε πλάγια
  γωνία θέασης το coplanar quad έκοβε τη ΔΙΚΗ του κεντρική λαβή (depth-gradient/pixel > 5mm bias) από draw+pick.
  FIX: `mesh.userData.isDxfAnnotation = true` (`dxf-text-3d.ts`) + `isNonSolidPrimitive` το εξαιρεί
  (`grip-3d-depth-occluder.ts`, ADR-535) — ίδιος κανόνας με wireframes/labels. ⚠️ stage **ADR-535** μαζί
  (CHECK 6B). Browser-verify (click/hover text → επιλογή/glow/center grip + move) + commit εκκρεμούν.
