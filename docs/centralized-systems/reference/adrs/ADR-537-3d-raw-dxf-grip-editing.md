...
- **2026-06-27 — wall-placement overlay parity (mustard fix + 2D-parity snap + dynamic input) (uncommitted).**
  ...
  +tests (`WallPlacementGhost.test.ts` NEW, `use-bim3d-wall-placement.test.ts` updated). 41 jest GREEN
  στα σχετικά suites. **🔴 Browser-verify (Giorgio):** (α) ghost μπλε flat, ΟΧΙ μουσταρδί· (β) έλξη =
  επίπεδο glyph «Γωνία/Μέσο τοίχου», ΟΧΙ κύβος· (γ) μετά το 1ο κλικ → radial ring (Μήκος/Γωνία/Πάχος/
  Ύψος) όπως 2D, ΕΝΑ μόνο. **Ανοικτά (χρειάζονται browser-repro ΠΡΙΝ αλλαγή — handoff):** ghost «μακριά
  από κέρσορα» (το audit ΔΕΝ βρήκε δομικό offset στον projector) + «crosshair δεν φαίνεται». **Follow-up:**
  ColumnPlacementGhost + MEP/furniture ghosts έχουν την ΙΔΙΑ λανθάνουσα μουσταρδί ρίζα (MeshStandard,
  μη-εγγεγραμμένα) → ίδιο fix (shared `PlacementGhostOverlay` SSoT) — 8+ αρχεία, ζητά έγκριση (N.8).
  Stage μαζί: ADR-537 (+ADR-040/542/513· CHECK 6B/6D).
