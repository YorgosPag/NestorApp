...
- **2026-07-06 — Φ2: ίχνη ευθυγράμμισης + λευκές ενδείξεις (HUD) στο 3D grip drag (parity με 2D, full SSoT).**
  ...
  **Full SSoT — μηδέν νέα μηχανή:**
  ...
  - **Κεντρικοποίηση (follow-up, μηδέν διπλότυπο — audit Giorgio):** 3 κοινά SSoT εξήχθησαν ώστε τα δύο νέα
    passes να μην αντιγράφουν λογική, + migrate των **προϋπαρχόντων** consumers:
    · `cameraSceneUnitsPerPixel` (`viewport/coordinate-transforms.ts`) — ο τύπος `getPixelWorldSize·1000·mmToSceneUnits`
      ήταν inline **4×** (wall-placement, wall-HUD, grip-HUD, grip-tracking) → ΜΙΑ πηγή·
    · `paintComposedTrackingThroughProjector` + `composedTrackingLabel` (`hooks/dimensions/dim-alignment-tracking.ts`)
      — projector-based paint των ιχνών (paths+intersections+tooltip) + label· ο 2D `paintGripAlignmentTracking`,
      το 3D grip-tracking pass ΚΑΙ ο wall-placement label builder καλούν πλέον το ίδιο·
    · `makeWallHud3DProjector` (`overlay-dispatch/wall-hud-3d-projector.ts`, νέο) — η κατασκευή του `WallHudProjector`
      (scene→plan-mm→px + screen scale + projected aligned-dim) ήταν αντιγραμμένη σε `use-wall-hud-pass` ⇄ `use-grip-hud-pass`
      → ΕΝΑΣ builder, δύο consumers·
    · `resolveDraggedDxfGrip` + `useDxfGhostGripsActive` (`grips/dxf-grip-drag-access.ts`, νέο) — το «resolve του
      dragged raw-DXF grip + της οντότητάς του» (drag singleton + grips + `findDxfEntityInScope`, με τα guards) ήταν
      αντιγραμμένο **3×** (`use-grip-pass` ghost, grip-tracking, grip-HUD)· ο activation gate (`dxfGhostEntityIds.length>0`)
      **2×** → ΜΙΑ πηγή, migrate και του προϋπάρχοντος `use-grip-pass`. Tests: 396 πράσινα μετά το migrate.
