...
## Changelog
- **2026-06-10 (Opus 4.8) — Coordination Phase 1 BROWSER-VERIFIED + markers UNIFIED (child ADR-435, live session with Giorgio).** Clash Slice 1b was tested end-to-end and, on Giorgio's SSoT direction ("use the 2D code in 3D"), the 2D and 3D markers were unified into **ONE ⊙ glyph**: extracted `ClashMarkerGlyph` (SVG) + `ClashMarkerLayer` (imperative, zero-lag positioning), **removed the Three.js `ClashMarkerOverlay` + `use-bim3d-clash-markers`**, and the 3D markers are now the SAME DOM ⊙ projected via `camera.project()` (CSS2D). Both overlays reproject via a LOW-priority `UnifiedFrameScheduler` subsystem (frame-synced → zero-lag pan/zoom/orbit); the 2D overlay moved off the wiped shared preview canvas to DOM (persists when idle — a latent bug in every ghost overlay), and is hidden in 3D. Panel → draggable `FloatingPanel` (SSoT). Engine: `detectClashes` dedups entities by id (level scene can list an id twice → inflated/duplicate clashes); 2D focus box sized in metres (scale-correct). **Known issues (deferred):** segment↔fitting false positives (connectivity filter), upstream duplicate-entity data. 24 coordination jest green. See **ADR-435 §5b + changelog**.
