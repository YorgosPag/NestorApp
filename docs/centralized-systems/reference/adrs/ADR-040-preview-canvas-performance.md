...
Παράλληλα με το 2-click Move tool, τώρα και το **grip drag** (center/vertex/edge/quadrant λαβές) ακολουθεί τη σύμβαση «ανεστραμμένου φαντάσματος» AutoCAD/Revit (ADR-049): το **ένα** entity που σέρνεται ζωγραφίζεται ως **συμπαγές αντίγραφο στο πραγματικό του χρώμα** πάνω στο PreviewCanvas (ακολουθεί τον κέρσορα), ενώ το πρωτότυπο στη θέση εκκίνησης γίνεται **dimmed ghost** στον main canvas. **SSoT:** νέο thin `rendering/ghost/ghost-solid-color.ts` (`resolveGhostSolidColor`) που **delegate-άρει στον κανονικό `resolveEntityColorHex`** (`systems/selection/select-similar-by-color.ts` → `resolveEntityStyle`, ο ίδιος effective-rendered-hex resolver που χρησιμοποιούν Select-Similar + column-batch-fill + DXF export) → ΑΚΡΙΒΕΣ canvas χρώμα για ByLayer/ByBlock/ACI/TrueColor/BIM, όχι προσέγγιση. Τρέφει **και** `useMovePreview` **και** `useGripGhostPreview` (ΕΝΑΣ κανόνας χρώματος· ο παλιός inline simplified `getLayer` cascade στο `useMovePreview` ήταν μίνι-διπλότυπο εκείνου του resolver και αφαιρέθηκε).
