...
### 2026-06-10 — Clash Detective markers + report panel mounted (ADR-435 Slice 1b, CHECK 6B/6D)

**Status**: IMPLEMENTED 2026-06-10. Η Coordination Phase 1 απέκτησε 3Δ markers + DOM report panel, **όλα low-frequency leaves** (αλλάζουν μόνο σε Detect/Clear/click — ΠΟΤΕ per-frame). **Καμία αρχιτεκτονική αλλαγή** στα shell/scene-host αρχεία: το `CanvasLayerStack` απλώς mount-άρει το self-contained `<ClashReportPanel/>` (sibling του `AutoAreaResultPanel`, subscribe στο low-freq `useClashReport` μέσα του — **μηδέν νέο `useSyncExternalStore` στο shell**, CHECK 6C respected), και το `BimViewport3D` mount-άρει το `useBim3DClashMarkers` driver. Τα 3Δ markers (`ClashMarkerOverlay`) είναι scene-leaf· το screen-constant scaling τους **δεν προσθέτει render loop**: τρέχει ως HIGH-priority `UnifiedFrameScheduler` subsystem (`bim-3d-clash-markers`) που είναι `isDirty` ΜΟΝΟ όταν `hasMarkers() && manager.isSceneDirty()` → piggyback στα ήδη-dirty frames (μηδέν extra renders, mirror του gizmo overlay `updateScale`). Ο 2Δ overlay leaf (`useClashOverlayPreview`) απλώς πέρασε στο severity-colour SSoT (Boy-Scout) — **καμία αλλαγή σε bitmap cache-key** (μηδέν hover/selection state). Co-staged για CHECK 6B/6D (`BimViewport3D.tsx`, `CanvasLayerStack.tsx`, `useClashOverlayPreview.ts`). Λεπτομέρεια στο ADR-435 §5b.
