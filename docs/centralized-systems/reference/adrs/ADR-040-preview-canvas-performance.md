...
**Giorgio (2ο στιγμιότυπο):** σβήσιμο του λευκού-φόντο readout pill· οι μοίρες ως χρωματιστό κείμενο (🔴/🟢, 2 δεκαδικά) δεξιά του προσήμου· διακεκομμένη γραμμή στο σημείο έναρξης μέτρησης (0° άξονας αναφοράς). **ADR-040 touch (`useGripGhostPreview.ts`, CHECK 6D):** αφαιρέθηκε το `drawDimPill(formatMoveAngle(...))` rotation readout (το μόνο white-bg label· το POLAR tooltip είναι γκρι/no-bg, ξένη uncommitted δουλειά — άθικτο) + νεκρό `ROTATE_READOUT_OFFSET_PX` (CHECK 3.22). Όλη η νέα ζωγραφιά μένει στο pure `rotation-direction-arc.ts` (χρωματιστή ετικέτα `formatAngleLocale(_,2)` + dashed baseline) — μηδέν αλλαγή σε cache-key/scheduler/store subscriptions. 10 jest. 🟡 UNCOMMITTED.

### 2026-07-01 — Ένδειξη ΦΟΡΑΣ περιστροφής: χρωματισμένο τόξο + βελάκι (ADR-397 §15, CHECK 6D)
**Ζητούμενο (Giorgio):** κατά το hot-grip rotate (αφού οριστεί κέντρο), τόξο φοράς από τον άξονα αναφοράς προς τον κέρσορα, 🟢 πράσινο + `(+)` όταν `rotateSweepDeg > 0` (CCW) / 🔴 κόκκινο + `(−)` όταν `< 0` (CW), live signed μοίρες (από το υπάρχον readout pill). **NEW pure `hooks/tools/rotation-direction-arc.ts`** (geometry + color + paint) — **ADR-040 micro-leaf safe:** zero React / zero stores / zero DOM-state, καθαρές screen-space πράξεις, καμία συνδρομή σε high-freq store. **ADR-040 touch (`useGripGhostPreview.ts`, CHECK 6D):** μία προσθήκη `paintRotationDirectionArc(...)` στο rotation branch ΜΕΤΑ το readout pill — καμία αλλαγή σε bitmap cache-key / scheduler / micro-leaf δομή / `useSyncExternalStore`. SSoT reuse: χρώμα από `resolveGhostStatusColor`, sweep από `rotateSweepDeg`, προβολή από `CoordinateTransforms.worldToScreen`. 9 jest. Βλ. **ADR-397 §15**. 🟡 UNCOMMITTED.
