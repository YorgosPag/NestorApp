...
## 2026-06-21: 🐛 FIX — preview ghost «πάγωνε» στο wheel-zoom χωρίς mousemove (world-locked transform refresh, CHECK 6B/6D stage)

**ADR-040-compliant — leaf ακούει το ίδιο zero-lag transform SSoT με τον main canvas.** Με ενεργό φάντασμα δημιουργίας (κολώνα/τοίχος/δοκάρι/…), zoom με το ροδάκι **χωρίς κίνηση κέρσορα** άφηνε το ghost παγωμένο: τα δύο preview paths ξανασχεδίαζαν μόνο σε cursor event, ενώ το `updateImmediateTransform` (από `useViewportManager.setTransform`) άλλαζε σιωπηλά το transform. **FIX (Revit/AutoCAD world-locked):** το cached preview entity είναι σε **world coords** → re-paint με νέο live transform = σωστή κλίμακα/θέση, **μηδέν re-snap**. (1) NEW `PreviewRenderer.refreshTransform(transform, viewport)` (no-op χωρίς cached ghost). (2) `PreviewCanvas` κάνει `subscribeTransform(() => refreshTransform(getImmediateTransform(), viewportRef.current))` — το **ΙΔΙΟ** `ImmediateTransformStore` `fullListeners` που οδηγεί τον main canvas (zero-lag). (3) `useCanvasGhostPreview` αντικαθιστά τον React-prop `transform` redraw-trigger (1-2 frame lag) με live `subscribeTransform` (RAF-coalesced). **Καμία `useSyncExternalStore` σε orchestrator** (CHECK 6C safe — η subscription ζει σε leaf canvas/ghost-harness). **Bitmap cache ΑΘΙΚΤΟ** (preview canvas ≠ DXF bitmap cache, rule 3 N/A). Staged για CHECK 6B/6D. 11 jest. Βλ. ADR-398 §4 changelog.
