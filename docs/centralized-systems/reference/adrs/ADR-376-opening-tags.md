...
- **2026-05-25** (v5.1 — Phase A polish) — Layers panel UI toggle: new `AnnotationsSection.tsx` component wired στο `LevelPanel` ως ξεχωριστή section κάτω από το `LayersSection`. **[SUPERSEDED 2026-07-18: το toggle μετακινήθηκε στο contextual «Ιδιότητες Ανοίγματος» (ribbon widget `OpeningTagLayerToggle`)· το `AnnotationsSection.tsx` + τα `panels.annotations.*` keys διαγράφηκαν. Το state SSoT (`opening-tag-layer.ts`) αμετάβλητο. Βλ. ADR-363 changelog 2026-07-18.]** `Switch` (variant=`status`) εναλλάσσει global εμφάνιση opening tags μέσω SSoT setters. i18n keys προστέθηκαν σε `dxf-viewer-panels.{el,en}.json`. +9 SSoT unit tests (`__tests__/opening-tag-layer.test.ts`). `.ssot-registry.json` νέο entry `opening-tag-layer` (Tier 3, forbids inline `__system_opening_tags__` literal εκτός SSoT). Πλέον ο user μπορεί να ενεργοποιεί/απενεργοποιεί τα tags χωρίς console. Pending Phase A items closed: 1 (Layers panel UI button DONE), 1 left (manual smoke test με αληθινό DXF — requires browser session).
