...
### Φ5b — ο κύκλος έκλεισε (ADR-736, 2026-07-30)

Το Φ5b **έγραφε** `IMAGE`/`IMAGEDEF` + raster σε zip (μοτίβο AutoCAD *eTransmit*) αλλά **κανείς
δεν τα ξαναδιάβαζε**: μια εξαγωγή του ΝΕΣΤΟΡΑ που ξανα-εισαγόταν έχανε τα υπόβαθρά της σιωπηλά.
Το **ADR-736** πρόσθεσε την ανάγνωση (`IMAGE`→`ImageEntity`) **και** τον αναγνώστη zip
(`export/core/zip-unpack.ts`, sibling του `zip-pack.ts`, μηδέν εξάρτηση) ⇒ το πακέτο που παράγει
το Φ5b είναι πλέον **έγκυρη είσοδος** για τον ίδιο τον ΝΕΣΤΟΡΑ.
...
| Ημ/νία | Αλλαγή |
|---|---|
| 2026-07-14 | **Own-scan texture enrichment (+12 υλικά)** — η βιβλιοθήκη υλικών εικόνας (`material-image-catalog.ts`, το starter library της §Φ2) εμπλουτίστηκε με **12 ιδιόκτητα σκαναρίσματα** (`images_6`, own IP του Giorgio — ΟΧΙ Poly Haven) που καλύπτουν πραγματικά κενά: **υφάσματα** (λινό, τουίντ, τιρκουάζ, καρό), **μοκέτες/χαλί** (γκρι/ανθρακί/τερακότα), **ψάθα** (για outdoor έπιπλα), **νερό πισίνας/ρηχό**, **τερazzo**. Νέα infra: `ADR-413 TEXTURE_SET_DEFS` +12 slugs (albedo-only, `license: 'owner'` — νέα τιμή enum) + `isOwnScanTexture()` helper· `download-bim-textures` **προσπερνά** τα own-scans (δεν είναι στον provider — η αναπαραγωγή τους είναι το git-committed public tile + το Storage copy)· νέο `scripts/build-own-textures.js` (ντετερμινιστικό: images_6 stem → κεντρικό crop 1024² → `public/textures/<slug>/albedo.jpg`, SSoT προέλευσης). `material-image-catalog` +4 κατηγορίες (textile/carpet/wicker/water) +12 `matimg-*` rows (tile size **derived** από `TEXTURE_SET_DEFS`, μηδέν διπλότυπο). i18n `hatchImageFill.materials.*` +12 (el+en). Ο picker (flat swatch grid) τα δείχνει αυτόματα. **Reproducibility:** τα 12 tiles (~1.3MB, own IP, μη-κατεβάσιμα) git-committed (`add -f`) + Storage upload (`upload-bim-textures.js`). Tests: `material-image-catalog` 12→**14** (30-count + own-scan presence)· resolver 6/6. jscpd:diff καθαρό. |
