...
- **Geometry-derived ποσότητες** (`bim/stairs/stair-boq-quantities.ts`), ΟΧΙ από `qto`: το `StairEntity.qto` **δεν γεμίζει ποτέ** στον κώδικα (μόνο type) — επιβεβαιωμένο 2026-05-29· ο bridge είναι ο SSoT (§4.6 / G5). Formulas (από `StairParams` mm):
  ...
  - concrete (m³) = waist slab (`κεκλιμένο μήκος · width · πάχος πλάκας`) + σφήνες σκαλιών· **πάχος πλάκας = `params.waistThickness ?? 150mm`** (✅ G1 follow-up 2026-05-29: επεξεργάσιμο πεδίο `StairParams.waistThickness` + ribbon combobox «Πάχος Πλάκας» στο Geometry panel· `DEFAULT_WAIST_SLAB_THICKNESS_MM`=150 ως SSoT fallback)· **0 για `steel-grating`/`glass-tread`**.
...
| **G5** ✅ DONE | **Πεδίο `qto` = νεκρός κώδικας** (ΟΧΙ «μόνο stair το γεμίζει» — επαλήθευση 2026-05-29: `makeBimQto` 0 callers, καμία ανάθεση `qto:{}` πουθενά, οι factories το spread-άρουν μόνο αν περαστεί ⇒ ποτέ). Μοναδικός reader: Schedule combined preset (default view) → 3 στήλες (Ποσότητα/Μονάδα/ΑΤΟΕ) **πάντα κενές** = λανθάνον bug, ΟΧΙ πραγματικός consumer. **Fix (Option A):** repoint combined preset σε geometry SSoT (`deriveAtoeQuantity`+`resolveAtoeMapping`, stair→`computeStairBoqQuantities`) **+** delete `qto`/`BimQuantityTakeoff`/`StairQTO`/`makeBimQto` + factory inputs. Bridge = μοναδικός quantity SSoT, geometry-derived. | Νεκρός κώδικας + σπασμένο default Schedule view | `bim-base.ts`, `stair-types.ts`, `schedule-presets.ts`, `bim-to-atoe-mapping.ts`, bridge, 5 factories |
...
6. **G5 / G6**: **G5 ✅ DONE 2026-05-29** (Option A — καθάρισμα + φτιάξιμο). Επαληθεύτηκε ότι το `qto` field **δεν γράφεται ποτέ** (η αρχική premise «μόνο stair το γεμίζει» ήταν λάθος): `makeBimQto` 0 callers, καμία `qto:{}` ανάθεση, factories spread μόνο-αν-περαστεί. Ο μόνος reader ήταν το Schedule **combined preset** (default view) που γι' αυτό έδειχνε 3 **πάντα κενές** στήλες. Λύση: (α) το `mapCombined` αντλεί πλέον primaryQuantity/unit/ΑΤΟΕ από geometry + ΑΤΟΕ SSoT (νέο `deriveAtoeQuantity` στο `bim-to-atoe-mapping.ts`, stair→`computeStairBoqQuantities` concrete m³→cladding m² fallback), ίδιο SSoT με τον bridge (boy-scout: ο bridge delegate-άρει πλέον στο `deriveAtoeQuantity`)· (β) διαγραφή νεκρού `qto`/`TQto` generic + `BimQuantityTakeoff` + `StairQTO` + `makeBimQto` + qto input στα 5 factories + Firestore StairDoc passthrough + 2 readonly hydration paths. Ο bridge παραμένει ο μοναδικός quantity SSoT (geometry-derived). **G6 ✅ DONE 2026-05-29**: `computeWallGeometry` δέχεται optional `openings` → net area (gross − Σ width×height, clamp ≥0)· `useWallPersistence` recompute net boqGeometry από in-memory scene openings (mirror slab) + `bim:opening-persisted` live re-feed. SSoT helper, και τα 2 bridge paths (single + multi-layer) auto-net. Geometry layer pattern ίδιο με `computeSlabGeometry`.
...
| Date | Change |
|------|--------|
| 2026-05-29 | **G5 ✅ DONE — `qto` dead-code cleanup + Schedule combined fix (Option A, pending commit).** Phase-1 επαλήθευση ανέτρεψε την premise «μόνο stair γεμίζει το qto»: το `qto` field **δεν γράφεται ΠΟΥΘΕΝΑ** (`makeBimQto` 0 callers· καμία `qto:{}` ανάθεση· οι 5 factories spread-άρουν `...(input.qto && {qto})` μόνο-αν-περαστεί ⇒ ποτέ). Μοναδικός reader: `schedule-presets.ts mapCombined` (το **default** Schedule view) → οι 3 στήλες Ποσότητα/Μονάδα/ΑΤΟΕ έβγαιναν **πάντα κενές** = λανθάνον bug, ΟΧΙ πραγματικός consumer (το «dual-path» ήταν παρανόηση). **Fix:** (1) νέο pure `deriveAtoeQuantity(unit, geometry)` SSoT στο `bim-to-atoe-mapping.ts` (pcs→1, m2→area, m3→volume)· ο `BimToBoqBridge` delegate-άρει σε αυτό (boy-scout, N.0.2 — αφαιρέθηκε το ιδιωτικό `deriveQuantity`). (2) `mapCombined` αντλεί primaryQuantity/unit/ΑΤΟΕ από `resolveAtoeMapping`+`deriveAtoeQuantity` (stair→`computeStairBoqQuantities`: concrete m³, fallback cladding m²· slab-opening→κενό). (3) Διαγραφή νεκρού: `TQto` generic + `qto?` field (`bim-base.ts`), `BimQuantityTakeoff`, `makeBimQto`, `StairQTO` (`stair-types.ts`) + StairDoc `qto` + create/update/entityToSaveInput passthrough + `stair-doc-hydration`/`bim-readonly-hydration` paths + `qto` input στα 5 factories (wall/slab/column/beam/opening) + exports (`bim/index.ts`, `types/entities.ts`). MOD: 14 (3 fix + 11 delete) + 1 readonly hydration. Tests: +5 `deriveAtoeQuantity` + 1 combined geometry-derived value test· stair-firestore `qto:undefined` literal removed, combined-preset test renamed. Affected suites: bim-to-atoe-mapping / schedule-builder / stair-firestore-service (89) + BimToBoqBridge / stair-boq-sync / stair-boq-quantities (62) = **151/151 PASS**. tsc clean. |
| 2026-05-29 | **WAIST PARAM PROMOTION ✅ DONE** (G1 follow-up, pending commit). Το πάχος της κεκλιμένης πλάκας σκάλας (waist slab) ήταν hardcoded `WAIST_SLAB_THICKNESS_MM = 150` στο `stair-boq-quantities.ts` → προήχθη σε **επεξεργάσιμο πεδίο `StairParams.waistThickness?: number` (mm)**. `computeStairBoqQuantities` χρησιμοποιεί `params.waistThickness ?? DEFAULT_WAIST_SLAB_THICKNESS_MM` (η σταθερά μετονομάστηκε + exported ως SSoT default). Ribbon: νέο combobox «Πάχος Πλάκας (mm)» στο Geometry panel (options 120-250mm, default 150)· νέο command key `STAIR_RIBBON_KEYS.params.waistThickness` + read/patch helpers (plain mm, **όχι** scene-scaled — BOQ-only, mirror `storyHeight`· clamp [80,400]). Persist/hydration αυτόματα (optional field στο `params`, firestore service γράφει `params` ως έχει, hydration δεν χρειάζεται default — undefined → BOQ fallback). i18n el+en. MOD: `stair-types.ts`, `stair-boq-quantities.ts`, `stair-command-keys.ts`, `stair-param-helpers.ts`, `contextual-stair-tab.ts`, `dxf-viewer-shell.json` (el+en). Tests: +5 stair-boq-quantities (default fallback / custom scale / monotonic / non-concrete ignore / negative clamp) → 14/14 PASS. tsc clean. |
