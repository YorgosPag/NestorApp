...
| 2026-05-11 | 🔧 Telegram alert payload enrichment — `src/lib/telemetry/telegram-alert-service.ts`: message limit 200→1500 char (Firestore index-creation URL ora visibile per intero), stack 150→1500 char con 8 frame (era 2), metadata details senza cap a 5 entries, env+commit SHA footer (`VERCEL_ENV` / `VERCEL_GIT_COMMIT_SHA`), auto-split su 4000 char con paginazione `(n/m)` quando supera limite Telegram (4096). `src/lib/firebaseAdmin.ts safeFirestoreOperation`: ora propaga `error.stack` e `error.code` al logger.error. Root cause: errori `FAILED_PRECONDITION` arrivano su Telegram troncati e l'URL `console.firebase.google.com/...` per creare l'index si perde, obbligando audit manuale codebase. |
