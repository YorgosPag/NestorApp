...
| Date       | Change |
|------------|--------|
| 2026-07-25 | **ADR-701 — τα `snapshot-builder.ts` + `labels.ts` (Phase 1) έγιναν δηλώσεις.** Ο builder καλούσε **ήδη** το `createShowcaseSnapshotBuilder`, κι όμως κουβαλούσε δικά του `pickString`/`pickNumber` (4η κόπια στο δέντρο) και χειρόγραφο project-name loader — γιατί το `loadRelations` ζητούσε **async closure**· δέχεται πλέον δεδομένα (`foreignKeyField`/`collection`/`resultKey`/`nameFields`), και η σειρά `['name','title']` του project lookup διατηρήθηκε ως **δεδομένο**. Το `labels.ts` δηλώνει `BUILDING_SPEC_ROWS` (18 γραμμές) και ο `BuildingShowcaseSpecLabels` **παράγεται** από τη λίστα — δεν μπορούν να ξεσυγχρονιστούν· οι 23 διατυπώσεις `[el,en]` ζουν μία φορά στο `SHOWCASE_SPEC_FALLBACKS`. Οι τρεις `translateXxx` έγιναν `createEnumLabelTranslator(...)`, το catalog preamble και τα blocks chrome/email/description/media μετακόμισαν στο `showcase-core/labels-catalog.ts`. Οι **πίνακες ετικετών μένουν εδώ** — είναι δεδομένα, και η επανάληψη δεδομένων δεν είναι διπλασιασμός. Wire contract παγωμένο. Λεπτομέρειες: ADR-701. |
| 2026-07-25 | **ADR-699 — το `building-showcase.resolver.ts` (Phase 1) ΔΙΑΓΡΑΦΗΚΕ.** Ήταν το «καλό» παράδειγμα — 47 γρ., καλούσε ήδη το `createShowcaseShareResolver` — κι όμως ό,τι του έδινε ήταν το **ίδιο object literal** με τα storage/parking, με μετονομασμένα κλειδιά· τα δύο τελευταία ξανάγραφαν και ολόκληρο το `validateCreateInput` για **μία boolean** διαφορά (`requiresPdfPath`). Το factory δέχεται πλέον **δεδομένα** αντί για `buildResolvedData` hook, και οι πέντε επιφάνειες είναι δηλώσεις 8 γραμμών σε **ένα** αρχείο (`sharing/resolvers/showcase-surfaces.resolvers.ts`). Το `BuildingShowcaseResolvedData` παραμένει εξαγόμενο ως alias — **κανένας consumer δεν άλλαξε**. Νέο SSoT `sharing/resolver-core/` για τα κοινά **όλων** των resolvers (projection / base validation / tenant ownership / entity read), που εξάλειψε και το 128t clone `contact ↔ project`. 59 tests· anchor που διαβάζει το `ShareEntityType` union. Λεπτομέρειες: ADR-699. |
| 2026-07-25 | **ADR-698 — το public token surface έγινε δήλωση.** Το `GET /api/building-showcase/[token]` (104 γρ.) και το `.../pdf` (85 γρ.) καλούσαν **ήδη** τα factories του ADR-321, κι όμως ήταν σχεδόν πανομοιότυπα με τα project/parking/storage αδέρφια τους: **446t** clones building↔project pdf (το μεγαλύτερο cross-file cluster σε όλο το `src/`) + **322t/320t/233t** στην payload τετράδα (jscpd/CHECK 3.28). Αιτία: τα factories δέχονταν **hooks** και όλες οι υλοποιήσεις ήταν ο ίδιος κώδικας με άλλο ουσιαστικό — **over-parameterised factory**. Τώρα: `createUnifiedPublicShowcasePayloadRoute` / `createUnifiedPublicShowcasePdfRoute` δέχονται **δεδομένα**· τα κοινά μηχανικά (`lookupPublicShowcaseShare`, `loadShowcaseMediaBuckets`, `createPublicTokenRouteExport`) ζουν μία φορά· το `BuildingShowcaseMedia` γίνεται alias του ΕΝΟΣ `ShowcaseMediaItem` (ήταν 4 πανομοιότυπα interfaces) — **κανένας consumer δεν άλλαξε**. Τα δύο routes: **104→44** και **85→37** γρ. Wire contract παγωμένο· το payload κάνει **whitelist** (`entityKey` + `company`) αντί spread, ώστε νέο snapshot πεδίο να μην μπορεί να φτάσει σε ανώνυμη απόκριση. Query πλέον στο **ρητά indexed** `[token + isActive]` (δεν υπάρχει `[token+entityType+isActive]` index — το παλιό triple επιβίωνε σε zigzag merge join) + `limit(2)` για ανίχνευση διπλού token. **Πλήρες σκεπτικό, αποκλίσεις property surface, και 2 εκκρεμότητες (σπασμένο test στο main, κενό rate-limit): ADR-698.** |
