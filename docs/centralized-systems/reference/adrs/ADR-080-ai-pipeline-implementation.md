...
| 2026-02-11 | **ADR-174: WhatsApp Channel**: PipelineChannel.WHATSAPP, WhatsAppChannelAdapter, dispatchWhatsApp in reply dispatcher. WhatsApp messages → agentic path (ADR-171) for immediate auto-reply. Admin vs customer prompt split. Interactive Reply Buttons (suggestions + 👍/👎 feedback). Instant "⏳ Επεξεργάζομαι..." acknowledgment |
| 2026-02-11 | **ADR-174 Phase 2+3: Messenger + Instagram Channels**: PipelineChannel types updated (INSTAGRAM added). MessengerChannelAdapter + InstagramChannelAdapter. dispatchMessenger + dispatchInstagram in reply dispatcher. Both channels → agentic path for immediate auto-reply. Messenger: Quick Replies (suggestions + feedback). Instagram: text-only (no interactive elements). 12 new files, 9 modified. Same 5-file webhook pattern as WhatsApp |
| 2026-02-11 | **ADR-174 Phase 2 OPERATIONAL**: Messenger webhook verified, FB Page "Nestor App" connected (984661054730180), subscriptions active (messages, messaging_postbacks, message_deliveries), env vars set. End-to-end test passed: AI reply + Quick Replies + feedback buttons working via Messenger |
| 2026-02-11 | **Multi-Channel Super Admin Detection (ADR-145)**: Admin αναγνώριση σε WhatsApp, Messenger, Instagram adapters. `AdminCommandMeta.resolvedVia` expanded: +messenger_psid +instagram_igsid. Firestore registry updated με Messenger PSID. Giorgos τώρα αναγνωρίζεται ως super admin σε Telegram + Messenger |
| 2026-02-11 | **ADR-174 Phase 3 OPERATIONAL**: Instagram webhook setup complete (giorgio_pagoni IGSID: 17841403201916682), Meta app published (Live mode). **BUG FIX**: Instagram client was using `graph.facebook.com` (Messenger endpoint) — fixed to `graph.instagram.com`. **State Machine FIX**: Agentic path was skipping UNDERSTOOD state (acked→proposed invalid) — added missing transition (acked→understood→proposed). Privacy Policy + Terms + Data Deletion pages created for Meta compliance |
| 2026-02-11 | **Instagram dedicated app account (nestor_app)**: Ξεχωριστός Instagram Business account (nestor_app) ως webhook receiver. INSTAGRAM_ACCESS_TOKEN ενημερώθηκε. End-to-end test: giorgio_pagoni → nestor_app DM → ack "⏳ Επεξεργάζομαι..." → AI agentic reply → απάντηση ληφθεί ✅ |
| 2026-02-11 | **Interactive elements parity across ALL channels**: Instagram Quick Replies (suggestions + feedback). Negative feedback categories (❌📊❓🐢) σε WhatsApp, Messenger, Instagram — ίδια ροή με Telegram (ADR-173). Callback prefix `fbc_` → `negativeCategory` στο `ai_agent_feedback` |
| 2026-02-11 | **Instagram: Text-based feedback (Quick Replies don't work)**: Instagram DM API silently ignores `quick_replies` field. Replaced with text-based emoji/number detection: "Απάντησε 👍 ή 👎" prompt → handler detects emoji reactions + numbered category responses (1-4) via `FeedbackService.getLatestFeedbackForChannel()` (30-min window). Removed `sendInstagramQuickReplies()`. Composite Firestore index added: `ai_agent_feedback(channelSenderId, createdAt desc)` |
| 2026-03-25 | **Refactor: Split pipeline-orchestrator.ts (1448→343 lines)**: Extracted 5 modules for N.7.1 SRP compliance: `intent-mapping.ts` (AI result mapping), `multi-intent-steps.ts` (ADR-131 multi-module steps), `agentic-path-executor.ts` (ADR-171 agentic loop), `post-reply-actions.ts` (channel keyboards/CRM/feedback), `approval-resume.ts` (UC-009 resume). Added `pipeline-types.ts` for shared types. Zero public API changes. |
| 2026-03-25 | **QA Session 4 Fixes (FIND-J, FIND-K, FIND-I)**: (1) FIND-J: Attachment fallback — when media download fails, lightweight stubs passed to AI so it knows media was sent (`downloadFailed` flag on IntakeAttachment). (2) FIND-K: AI-created contacts now indexed in `searchDocuments` collection for Global Search via new `search-indexer.ts`. (3) FIND-I: `fieldType: 'website'` in append_contact_info → writes to `websites[]` array (not socialMedia). Prompt hardened: contactId must come from search, website routing rule. |
| 2026-03-26 | **QA Session 6 Fixes (FIND-T/U/V/W) + Entity-Aware Types**: (1) FIND-T: Guardrail 3 in agentic-tool-runner — blocks tools requiring contactId if no prior search_text/firestore_query was called. Prevents AI from fabricating document IDs. (2) FIND-U: Explicit prompt rule — vatNumber stores directly, NO lookup_doy_code (only taxOffice needs DOY lookup). (3) FIND-V: Explicit rule — industry/sector are free-text company fields, NO ESCO search needed. (4) FIND-W: Added `companyName` to SEARCH_FIELDS + prompt rule to use search_text for name searches (partial/fuzzy matching). (5) **Entity-Aware Type Resolution**: contact-handler now resolves phone/email/website types per entity type. Individual: mobile/home/work. Company: main/department/sales. Service: main/secretariat/helpdesk. Fixes incorrect type values on company contacts (e.g., phone "work"→"main", email "personal"→"general", website "personal"→"corporate"). |
| 2026-03-26 | **QA Session 7 Fixes (FIND-N/U/Y/Z) + Bidirectional Relationships + KAD Tool**: (1) **FIND-N (CRITICAL)**: Guardrail B bug — `.find()` matched ANY blocked call in history, even if later retry succeeded. Fix: check only LAST write tool call. Also: success results now include `_status: "OK"` flag for unambiguous AI interpretation. (2) **FIND-U**: Strengthened prompt rule — vatNumber update must NOT auto-set taxOffice. (3) **FIND-Z**: Server guardrail blocks 4-digit years in registrationNumber (ΓΕΜΗ ≠ foundedDate). Prompt rule: no foundedDate field exists. (4) **FIND-Y**: Prompt rule for entity context disambiguation in multi-entity conversations. (5) **Bidirectional Relationships**: `handleAdd()` now creates inverse relationship doc (client↔vendor, employee↔manager, symmetric→same). `handleList()` queries both directions. `handleRemove()` deactivates both sides. `bidirectionalRef` field links paired docs. (6) **manage_activities Tool**: New KAD activity management tool (add/list/remove/set_primary). Validates against 10521 Greek NACE Rev.2 codes. New `ActivityHandler` registered in executor. 63 test suites, 1026 tests pass. |
