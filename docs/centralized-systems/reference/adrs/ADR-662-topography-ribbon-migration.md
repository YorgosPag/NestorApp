...
## Changelog
- **2026-07-28 — §13 ο φύλακας τύπου έγινε ΠΡΑΓΜΑΤΙΚΟΣ (καμία αλλαγή συμπεριφοράς).** Το
  `topoSurfaceGripsOf` δεχόταν `Entity` και οι δύο καταναλωτές έφταναν σε αυτό μέσα από
  **ανέλεγκτο** cast: ο ορατός renderer έγραφε `entity as unknown as TopoSurfaceEntity`, το
  interaction registry το ίδιο με άλλη διατύπωση. Δηλαδή ο έλεγχος «είναι όντως επιφάνεια;»
  υπήρχε **δύο φορές γραμμένος με το χέρι** και **καμία** από τις δύο φορές δεν τον έκανε ο
  μεταγλωττιστής — αν το κλειδί του registry ξέφευγε ποτέ, θα διαβάζαμε `footprint` από γραμμή.
  Η υπογραφή είναι πλέον το ελάχιστο δομικό σχήμα (`{ type: string }`), ακριβώς όπως στο
  `isTopoSurfaceEntity`, ώστε να δέχεται **και** `EntityModel` **και** `DxfEntityUnion` χωρίς
  cast από κανέναν — και ο έλεγχος γίνεται **μία** φορά, από τον **ίδιο** φύλακα που ήδη
  χρησιμοποιεί ο renderer. Το «ορατό ≡ πιάσιμο» του §13 ισχύει τώρα και στο επίπεδο των τύπων,
  όχι μόνο στη γεωμετρία.
...
- **2026-07-27 — §13 ΛΑΒΕΣ ΣΤΗΝ ΕΠΙΦΑΝΕΙΑ (αλλαγή συμβολαίου).** Η επιφάνεια ήταν ρητά grip-less και **καρφωμένο σε test**· άλλαξε η **απόφαση**, όχι μια αστοχία. Έρευνα (§13.1): κανείς μεγάλος δεν δίνει draggable λαβές σε **παραγόμενο** όριο (Civil 3D: καμία λαβή στο Surface Border· Revit: πίσω από `Modify Sub Elements` mode· ArchiCAD: authored boundary, δεν μεταφέρεται). **Εμείς κρατάμε τον δρόμο της επιστροφής** (κάθε κορυφή footprint = κόμβος TIN) → μοντέλο Revit **χωρίς το mode**.
  - **NEW**: `systems/grip/closed-ring-grips.ts` (2ο μισό του `entity-closed-rings`· ο **hatch έγινε λεπτός adapter** πάνω του — ο διπλός βρόχος έφυγε, N.18), `systems/topography/topo-survey-point-resolve.ts`, `systems/topography/topo-surface-grips.ts`, `core/commands/entity-commands/MoveTopoSurveyPointCommand.ts` (**πρώτη undoable μεταβολή του `TopoPointStore`**), `hooks/grips/grip-topo-surface-commits.ts`. **MOD**: `geo-transform.ts` (+`unproject`, ίδιος `forwardRigidMap` πυρήνας), grip registry/dispatch/producers, `TopoSurfaceRenderer.getGrips`, `apply-entity-preview.ts`.
  - **ΑΠΟΡΡΙΦΘΗΚΑΝ ρητά** (§13.4): «promote σε `TopoBoundary`» (είναι όριο **ΟΓΚΟΥ** — θα άλλαζε σιωπηλά τα χωματουργικά, σφάλμα **τιμής**) και λαβές **μέσου ακμής** (θα επινοούσαν **υψόμετρο** και θα το παρουσίαζαν ως μετρημένο).
  - **Καμία λαβή χωρίς προορισμό** (μάθημα ADR-654): κορυφή από **γραμμή ασυνέχειας** δεν παίρνει λαβή. Ταυτότητα κόμβου **με συντεταγμένες** (εξαγόμενο `localVertexKey`), ποτέ με δείκτη — ο TIN κάνει dedup.
  - **ADR-587 αυτο-διορθούμενο συμβόλαιο: λειτούργησε.** Τα anchors απαίτησαν συνειδητή μετακίνηση του `topo-surface` σε 5 golden sets (grip-producer, grip-kinds 37→38, parametric dispatch 29→30, mode-aware seam, **ghost preview** — το τελευταίο το είχε προδιαγράψει ονομαστικά το ίδιο το pin). Τεστ: 28 νέα + 336 anchors + 3362 στο ευρύ δίχτυ, όλα πράσινα. `jscpd:diff` καθαρό.
  - 🔴 **Καμία ζωντανή επαλήθευση στον browser** — απαιτούν χειροκίνητο έλεγχο: σύρσιμο λαβής σε **geo-referenced** έργο (παγίδα τριών πλαισίων §13.3), Shift/ortho σε **στραμμένη** geo-reference, undo/redo της νέας γραφής στον `TopoPointStore`.
