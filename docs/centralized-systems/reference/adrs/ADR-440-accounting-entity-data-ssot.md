...
- **2026-09-15 (ίχνος ΜΕΣΑ στη συναλλαγή — ADR-841 §7 Α23, Φ3.2 Γ1–Γ3)** — Κλείνει το DEFER του §4 «optimistic concurrency στο `saveCompanySetup` (read-then-write)» **και** το κενό «το ίχνος γράφεται μετά την αποθήκευση».
  - **Γ1** — νέος τύπος `COMPANY_LEGAL_NAME_CHANGED` (`types/accounting-audit.ts`) + καθαρό `services/audit/company-legal-name-audit.ts` (`legalNameChangeAudit({from,to,source:'gemi-adoption'|'profile',registry?})`, ακριβής σύγκριση). Η «Υιοθέτηση επωνυμίας ΓΕΜΗ» (`services/company-registry/legal-name-adoption.service.ts`) γράφει `businessName` **και** την εγγραφή ίχνους στην **ίδια** Firestore συναλλαγή. `accountingAuditEntryOf` (`accounting-audit-service.ts`) = ο ΕΝΑΣ κατασκευαστής εγγραφής· ο `logAccountingEvent` τον χρησιμοποιεί.
  - **Γ2** — το `saveCompanySetup` του audited wrapper καταγράφει και αλλαγή επωνυμίας (source `profile`), όχι μόνο ιδιοκτησία. ⚠️ Τότε ακόμα **μετά** την αποθήκευση.
  - **Γ3** — `IAccountingRepository.saveCompanySetup(data, { fields?, auditOf? })` ⇒ **μία** `runTransaction` (`services/repository/accounting-repo-company-setup.ts`): `tx.get` → μάσκα πεδίων (ADR-256 2026-09-15) → `tx.set` → `auditOf(before, after)` → εγγραφές ίχνους στην **ίδια** δέσμευση. Επιστρέφει `{ before, after }` της συναλλαγής ⇒ η διαδρομή `PUT /api/accounting/setup` **δεν** κάνει πια δεύτερη ανάγνωση για το «προηγούμενο» (έφυγε το race στο `propagateCompanyRename`). Ο wrapper δίνει `auditOf` = `diffCompanyOwnership` + `legalNameChangeAudit`, κρινόμενα από το **συγχωνευμένο** «μετά» (όχι από τη φόρμα) και συνθέτει το `auditOf` του καλούντα.
  - **SSoT**: `appendAuditEntryInTransaction` (`services/repository/accounting-repo-audit.ts`) = το ΕΝΑ σημείο εγγραφής ίχνους μέσα σε συναλλαγή· το χρησιμοποιούν **και** η υιοθέτηση **και** το `saveCompanySetup`. Το `auditLogDocumentOf` έγινε ιδιωτικό.
  - **Γιατί**: αλλαγή χωρίς ίχνος ή ίχνος χωρίς αλλαγή είναι δομικά αδύνατα (ατομικότητα) — GDPR άρθ. 5(1)(στ) ακεραιότητα, 5(2) λογοδοσία, 32· βάση 6(1)(γ) όπου υπάρχει νόμιμη υποχρέωση τήρησης, αλλιώς 6(1)(στ) με ελάχιστο περιεχόμενο — ακριβές άρθρο/χρόνος διατήρησης ανοιχτά για δικηγόρο (ADR-841 §7 Α23).
  - **Άγκυρες**: `repository/__tests__/firestore-accounting-repository.company-setup.test.ts` (FakeFirestore: μπαγιάτικη οθόνη δεν επαναφέρει επωνυμία · μέτοχοι επιζούν · ίχνος στην ίδια δέσμευση · `interfere` ⇒ επανάληψη) · `services/__tests__/audited-repository-wrapper.company-setup.test.ts` (Ο1–Ο6) · `services/company-registry/__tests__/legal-name-adoption.test.ts` (Υ1–Υ4). Μεταλλάξεις Γ1 6/6 · Γ2 7/7 · Γ3 26/26. Commits `35927ce3`, `f1796083`, `63cb2616`.
