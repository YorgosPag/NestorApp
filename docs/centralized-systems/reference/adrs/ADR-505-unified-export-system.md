...
- **Refactor 5 renderers → thin consumers** (behavior-preserving): `column-/linear-member-/footing-/slab-rebar-2d.ts` + `structural-finish-outline-2d.ts`. _(2026-07-08 phase-m: και ο **ctx draw loop** των 4 rebar renderers εξήχθη σε ΕΝΑ SSoT `bim/renderers/draw-rebar-plan-geometry.ts` — βλ. changelog «m».)_
...
### Phase E — συμπαγές γέμισμα (βαμμένη επιφάνεια) + per-category layers (Full SSoT)
Ο Giorgio ζήτησε, εκτός από το περίγραμμα, **συμπαγές γέμισμα** για σοβάδες + κολώνες + δοκάρια (+πλάκες/πέδιλα), **κάθε κατηγορία σε δικό της layer** + το γέμισμα σε **ξεχωριστό layer** ανά κατηγορία (Revit-grade). Δύο κλειδωμένες αποφάσεις:
1. **Γεωμετρία γεμίσματος = DXF `3DFACE` (κατακόρυφη όψη 3Δ)** — ΟΧΙ `SOLID` 2Δ plan poché (Giorgio 2026-06-20). Κάθε footprint+ύψος → πλευρικές όψεις (quad ανά ακμή) + πάνω/κάτω καπάκια → shaded όγκος που ταιριάζει με το body extrusion. `3DFACE` = R12, 4 κορυφές, μηδέν boundary-loops/handles → ασφαλές σε bare DXF (ίδιος λόγος που το true HATCH «κολλούσε» στον AutoCAD)· ο Τέκτονας το αγνοεί (διαβάζει μόνο LINE/TEXT/CIRCLE).
2. **Re-layering ΟΛΩΝ των BIM** (Giorgio 2026-06-20) σε per-category layers: `COLUMNS`/`BEAMS`/`SLABS`/`FOOTINGS`/`WALLS`/`OPENINGS`/`MEP`/`STAIRS`/`RAILINGS`/`ROOF`/`FURNITURE`/`SYMBOLS` (όχι μόνο τα δομικά που γεμίζουμε).

- **NEW `export/core/solid-fill-geometry.ts`** — `buildPrismFaces(ring, baseZMm, heightMm)` → `Fill3DFace[]` (πλευρές + καπάκια). Καπάκια μέσω **SSoT `THREE.ShapeUtils.triangulateShape`** (concave-safe, ίδιο με `column-piece-geometry.ts`)· κορυφές x/y σε scene units, zMm σε mm (ίδια σύμβαση με rebar Z). Pure, zero entity-deps.
- **NEW `export/core/dxf-category-layers.ts`** — SSoT entity-type→layer: `resolveDxfBodyLayer` (outline) + `resolveDxfFillLayer` (fill, μόνο column/beam/slab/foundation) + `CATEGORY_LAYER_DEFS` + `usedCategoryLayerDefs`. Χρώματα = `BIM_CATEGORY_LINE_COLORS` (ADR-445, μηδέν διπλό palette)· ASCII-only ονόματα (bare-DXF AutoCAD safe).
- **Carrier = υπάρχον `HatchEntity`** (`patternType:'solid'`) + προ-υπολογισμένα `dxfFaces` (μηδέν νέος global τύπος). Ο **writer** `case 'hatch'` → `emit3DFace` ανά face (μηδέν triangulation στον writer → μένει pure serialization).
- **Collector**: (α) σοβάς fill ανά band → `FINISH_FILL` (base z=0, ίδιο με το outline extrude· χρώμα flat υλικό), gated 'plaster'· (β) **body fill** `collectBodyFillEntities` (reuse `extractEntityFootprintRing`+`extractHeightMm` — exported από `bim-to-dxf-primitives`, μηδέν re-impl) → `<CAT>_FILL`, gated **'core'** (Revit: κρύψε core → σβήνει το γέμισμα)· χρώμα = resolved χρώμα στοιχείου.
- **`bim-to-dxf-primitives`**: `makeClosedLwpolyline` re-layers κάθε BIM body μέσω `resolveDxfBodyLayer` (άγνωστο → κρατά αρχικό).
