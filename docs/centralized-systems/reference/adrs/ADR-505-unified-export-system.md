...
**Status:** 🟡 DXF πλήρες (active + zip + single-merged) · IFC/PDF μέσω delegation στα canonical engines — UNCOMMITTED 2026-06-20 · 🔴 browser-verify + commit
**Date:** 2026-06-20
...
- **2026-06-20 (h)** — Browser-verify #6: «γιατί τα BIM δεν είναι 3Δ στο AutoCAD;» — εξάγαμε 2Δ footprint. Fix: **pseudo-3D extrusion** (generic από `geometry.bbox` z extent: elevation=min.z, thickness=max.z−min.z· `extractExtrusion`)· ο writer εκπέμπει DXF group 39 (thickness) + 30 (elevation) στο POLYLINE → το AutoCAD το σηκώνει σε 3Δ πρίσμα. ΜΟΝΟ σε polyline mode (Τέκτονας/lines=2Δ). Καλύπτει wall/column/slab/beam/foundation + όποιο BIM έχει 3Δ bbox. 54 jest. Re-verify pending.
