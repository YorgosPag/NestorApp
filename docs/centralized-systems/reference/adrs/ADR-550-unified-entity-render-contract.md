...
- NEW option `PlacementGhostOverlay(..., orderIndependent)`. Όταν `true`, το ghost render-άρεται **λείο translucent** (opacity 0.45, όπως το 2D) με **depth-prime + stencil one-write σε ΕΝΑ `renderer.render(root)`**: (1) ανά mesh προστίθεται **depth-prime twin** (`colorWrite:false, depthWrite:true`, OPAQUE queue → three το σχεδιάζει ΠΡΩΤΟ) που γράφει το **κοντινότερο** βάθος ΚΑΙ μηδενίζει το stencil της περιοχής (`Always→Replace 0`, self-clearing)· (2) το colour material έχει **`depthFunc: EqualDepth`** (κρατά μόνο το κοντινότερο layer) **+ stencil `NotEqual(1)→Replace 1`** (κλειδώνει το pixel μετά το πρώτο fragment) → **κάθε pixel βάφεται ΜΙΑ φορά** ακόμη και για **συνεπίπεδες** όψεις. Αυτό το τελευταίο ήταν κρίσιμο: το depth-prime **μόνο** άφηνε τον **μηρό** (waist slab) αδιαφανή («λευκή σφήνα») γιατί τα tread/riser finishes **seat flush** (συνεπίπεδα, ADR-685 `stair-waist-slabs`) στο **ίδιο βάθος** → περνούσαν όλα το EqualDepth → ξανα-άθροιζαν· το stencil guard το κόβει. Απορρίφθηκε το ενδιάμεσο `alphaHash`/screen-door (Giorgio «όλο κουκίδες»). Οι twins μοιράζονται geometry (borrowed → δεν disposed). Default `false` → convex placement ghosts κρατούν το φθηνό single-material blend (καμία αλλαγή).
...
**Ιστορικό αστοχιών (ίδια συνεδρία, διαφάνεια):** (1η) υπόθεση «unclipped ghost υπό section» → section-clip fix· **λάθος στόχος** (το λευκό μένει και χωρίς τομή — Giorgio isolate). Ο section-clip κρατήθηκε ως έγκυρο latent fix. (2η) `alphaHash` για anti-accumulation → **κουκίδες** χωρίς TAA (Giorgio). (3η) depth-prime EqualDepth → λείο ΓΙΑ ΤΑ ΠΑΤΗΜΑΤΑ, αλλά ο **μηρός** έμεινε «λευκή σφήνα» (συνεπίπεδα flush finishes περνούν όλα το EqualDepth). (Τελικό) + stencil one-write → λείο παντού. Μάθημα: κάθε βήμα χρειάστηκε ground-truth από Giorgio (isolate/κίνηση/transient) — οι υποθέσεις χωρίς repro απέτυχαν επανειλημμένα.
