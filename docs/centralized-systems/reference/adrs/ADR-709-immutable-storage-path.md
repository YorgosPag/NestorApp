...
| Ημερομηνία | Αλλαγή |
|---|---|
| 2026-09-17 | **Δεύτερη ΡΙΖΑ κατόχου, ΙΔΙΟ σχήμα (ADR-866 §5.2 · Φ0 βήμα 2α).** Ο ιδιώτης δεν έχει και δεν επιτρέπεται να αποκτήσει `companyId` (ADR-787 Ε-3 §3) ⇒ `people/{userId}/entities/…/files/{fileId}.{ext}` δίπλα στο `companies/{companyId}/…`. **Η αρχή «ένα σχήμα» ΔΕΝ αλλάζει**: η ρίζα κωδικοποιεί **αμετάβλητη ταυτότητα κατόχου**, ακριβώς όπως το `companyId`· αλλαγή κατόχου = **μετακίνηση** μεταξύ ριζών (πρότυπο Google Drive: μετακίνηση σε κοινόχρηστο δίσκο αλλάζει κάτοχο), ποτέ μετονομασία. `StoragePathParams = CustodyScope & συντεταγμένες` (`lib/workspace/custody-scope.ts`) — οι εταιρικοί καλούντες είναι ήδη το μέλος `{ companyId }`, **ανέγγιχτοι**. **Μία** ρίζα (`buildCustodyStorageRoot`) τροφοδοτεί builder **και** `buildEntityStoragePrefix`/`buildCategoryStoragePrefix` (η αρχή αυτού του ADR: writer και sweeper διαβάζουν την ίδια πηγή). `parseStoragePath`: **δύο** ρίζες, άγνωστη ⇒ `null`· legacy `projects/` **μόνο** κάτω από `companies/` (η `people/` γεννήθηκε κανονική). Επικύρωση «ακριβώς ένας κάτοχος». Αναγνώστες: migration normalize-storage-paths (προσωπικό ⇒ ήδη κανονικό) · `file-path-tree` (τμήμα `people`). Storage rule `canonical_personal` (`isOwner`, ούτε super_admin) + σουίτα emulator. 7 νέα tests, μετάλλαξη ×2 κόκκινη. |
