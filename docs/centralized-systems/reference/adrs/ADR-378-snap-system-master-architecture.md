...
| 2026-07-11 | Bugfix | **Open-polyline direction-change vertices were un-snappable (ENDPOINT osnap only saw the two extreme vertices).** Symptom (Γιώργος, imported `KADOS.ΓΡΑΜΜΟΣΚΙΑΣΕΙΣ-AUTOCAD.dxf` → 1ος Όροφος `polyline_1`, 5 vertices): hovering the corner vertices between segments showed no attraction marker and would not snap; the two endpoints worked. Root cause (`snapping/shared/GeometricCalculations.getEntityEndpoints`): for `closed:false` polylines the extractor pushed **only `vertices[0]` + `vertices[last]`** into the ENDPOINT spatial index, so the intermediate corners (the direction-change points — each an endpoint of an internal segment) were dropped. This treated an open polyline like a bare 2-endpoint `line`, contradicting AutoCAD/Revit where **every** polyline vertex is an ENDPOINT osnap target (MIDPOINT already covered every edge mid via `getEntityMidpoints`). **Fix (FULL-SSoT, simplifies):** the `isClosed` branch collapses — both open and closed polylines now `endpoints.push(...vertices)` (the `closed` flag only adds the closing *edge*, covered by MIDPOINT, never a vertex). Covers `polyline` + `lwpolyline`; no new engine. `imported-dxf-snap-visibility.test.ts` +4 cases (intermediate corner snaps ×2, first/last no-regression, closed all-vertices) — 11/11 GREEN. Opus 4.8. 🔴 PENDING: browser-verify (hover polyline corner → ■ marker + attraction) + commit (CHECK 6D → stage ADR-378). |
