...
- **2026-07-28 — Η ΚΛΕΙΣΤΗ ΠΟΛΥΓΡΑΜΜΗ ΜΠΑΙΝΕΙ ΣΤΟΝ ΕΝΑ DISPATCHER (§measure-facts)**.
  Ζητούμενο Giorgio: επιλέγοντας κλειστή πολυγραμμή, το **αριστερό Properties palette** να
  δείχνει **Περίμετρο** και **Εμβαδόν**.
  - **Ο dispatcher μετονομάστηκε**: `entity-area-facts.ts` / `entityAreaFacts` →
    **`entity-measurement-facts.ts` / `entityMeasurementFacts`**. Το συμβόλαιο έπαψε να είναι
    «τι **εμβαδόν** δίνει» και έγινε «τι **μετριέται**» όταν μπήκε το `perimeterMm`, οπότε το
    όνομα ακολούθησε. **Ο εναλλακτικός δρόμος — αδελφό `entity-perimeter-facts.ts` —
    απορρίφθηκε**: θα έσπαγε την υπόσχεση «νέος τύπος ⇒ ένα αρχείο» (ο επόμενος τύπος θα
    έπρεπε να θυμηθεί **δύο** dispatchers· κλασικό sibling drift, N.18). Καταναλωτές: 4 (3
    κώδικα + 1 σχόλιο) — φθηνή μετονομασία, μόνιμο κέρδος.
  - **ΜΗΔΕΝ νέα μαθηματικά.** Η πολυγραμμή διαβάζει το **υπάρχον**
    `computePolygonAreaMetrics(vertices, closed)` (`rendering/entities/shared/measurement-label.ts`)
    — το ΙΔΙΟ που ζωγραφίζει ήδη ο `PolylineRenderer` (`:231`) και το hover overlay. Άρα το
    panel και ο καμβάς **δεν μπορούν** να διαφωνήσουν.
  - ⚠️ **ΟΧΙ μέσω `entityClosedRings`.** Το SSoT των rings **αποκλείει ρητά** την πολυγραμμή
    («θα διπλασιάζαμε τις έλξεις της» — OSNAP). Ο dispatcher διαβάζει τα `vertices` απευθείας,
    όπως ο κλάδος της τοπογραφικής διαβάζει τρίγωνα TIN: **σκόπιμα διαφορετική γεωμετρία ανά τύπο**.
  - **ΚΑΝΟΝΑΣ AutoCAD για ΑΝΟΙΧΤΗ πολυγραμμή** (έρευνα + απόφαση Giorgio 2026-07-27): το
    **εμβαδόν** υπολογίζεται **σαν να έκλεινε** (νοητό τμήμα τέλος→αρχή), η **περίμετρος**
    **ΑΓΝΟΕΙ** το κλείσιμο. Ασύμμετρο **επίτηδες** — «καθάρισμα» προς συνέπεια σπάει τη
    συμμόρφωση. Δεν χρειάστηκε κλάδος: το `calculatePerimeter(vertices, closed)` το έκανε ήδη.
  - **ΝΕΟ ΠΕΔΙΟ `closedBoundary`** — ο διαχωρισμός των **δύο** ερωτημάτων: το panel δείχνει τον
    αριθμό (AutoCAD parity) αλλά το εργαλείο «Ετικέτα Εμβαδού» απαιτεί **αληθινά** κλειστό
    σχήμα. `hasMeasurableArea` = `facts?.closedBoundary === true`. **Συνέπεια που πρέπει να
    ξέρεις:** η κλειστή πολυγραμμή γίνεται πλέον **επιλέξιμη** από το `pickTopEntityAt` του
    εργαλείου ετικέτας — επιθυμητό, αλλά διευρύνει τους υποψηφίους του pick.
  - **`perimeterMm: null` για γραμμοσκίαση + τοπογραφική** — **σκόπιμα, όχι παράλειψη**: το όριό
    τους είναι «outer − islands» / TIN hull, οπότε ένας μοναδικός αριθμός θα ήταν **εφεύρεση**.
    Ούτε το AutoCAD δίνει περίμετρο σε hatch. `null` ≠ `0`.
  - **UI**: νέα ομάδα «**Μετρήσεις**» (τελευταία) στο `LINE_PROPERTY_GROUPS`, με το **υπάρχον**
    `control: 'readout'` — το ίδιο που χρησιμοποιεί ήδη η ομάδα `info` της γραμμοσκίασης
    (`hatch-property-fields.ts:182`). Read-only, όπως τα grayed-out Area/Length του AutoCAD.
    Αυτο-κρύβεται όταν η επιλογή δεν μετριέται· **απλή ΓΡΑΜΜΗ δεν τη δείχνει** (το μήκος της ζει
    ήδη στη «Γεωμετρία» — δύο θέσεις για τον ίδιο αριθμό = διπλότυπο UI).
  - **Γιατί το `line-measurement-readout.ts` και όχι το bridge**: το `useRibbonLineToolBridge`
    λύνει την επιλογή στην **ωμή** `SceneModel` (`levelManager.getLevelScene`), ενώ το
    `LinePropertiesTab` τη λύνει στη σκηνή που **δείχνει**. Οι δύο **διαφωνούν όποτε παίζουν
    containers** (INSERT→`BlockEntity`) — και οι μετρήσεις οφείλουν να περιγράφουν αυτό που ο
    χρήστης ΒΛΕΠΕΙ. Δεύτερος λόγος: το bridge είναι 486 γραμμές (όριο N.7.1 = 500). Το tab
    αλυσιδώνει `readout ?? bridge`· το bridge early-return-άρει τα readout keys (μοτίβο
    `newLineType`) ώστε να μη διαρρεύσουν ποτέ στο color fallthrough.
  - **Ορολογία**: «**Περίμετρος**» / «**Εμβαδόν**» — οι **υπάρχοντες** όροι του repo
    (`dxf-viewer-panels.json:623-624`, `dxf-schedule.json:163`), όχι νέο λεξιλόγιο.
  - **Μονάδες**: canonical mm/mm² (ADR-462), ίδια σύμβαση με το `computeHatchAreaMm2`· η
    μετατροπή γίνεται **μία φορά στην άκρη του UI** (`formatLengthForDisplay`/`formatAreaForDisplay`).
  - **Τεστ**: 9 νέα (`entity-measurement-facts.test.ts`) — γνωστό ορθογώνιο 4000×3000 ⇒
    περίμετρος 14000 / εμβαδόν 12·10⁶· ανοιχτή ⇒ περίμετρος **11000** με **ίδιο** εμβαδόν·
    `closedBoundary` ↔ `hasMeasurableArea`. Σουίτες measure+panels: **73 πράσινα**. `jscpd:diff`: καθαρό.
