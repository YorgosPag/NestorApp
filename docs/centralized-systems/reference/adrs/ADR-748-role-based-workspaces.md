...
| **Ε14.ι** | 🔒 **ΤΟ ΔΟΧΕΙΟ ΔΕΝ ΠΑΡΑΣΥΡΕΙ ΤΟ ΠΕΡΙΕΧΟΜΕΝΟ.** Γονιός κρύβεται **μόνο** αν δεν ανήκει ούτε ο ίδιος **ούτε κανένα παιδί** του· όταν κρατιέται λόγω παιδιών, κρατά **μόνο τα ορατά**. Ό,τι φεύγει —γονιός **και** παιδιά— **μετριέται** (Α-3) |
...
> ⚠️ **ΕΚΚΡΕΜΕΙ ΝΕΑ ΖΩΝΤΑΝΗ ΕΠΑΛΗΘΕΥΣΗ** μετά τις δύο διορθώσεις: αναμένεται
> **7 κρυμμένα** στο dashboard (αντί 8) και επιστροφή των «Νομικά Έγγραφα» + «Υποχρεώσεις»
> στο sidebar. Ο δείκτης του header θα πέσει από το 17. **Επίσης ανεπαλήθευτα**: η επιβίωση
> σε refresh και οι υπόλοιπες πέντε θέσεις του διακόπτη.
