...
| | |
|---|---|
| **Κατάσταση** | 🟢 **ΦΑΣΕΙΣ 1, 2 & 3 ΥΛΟΠΟΙΗΜΕΝΕΣ** (2026-08-02) — μητρώο δουλειών · φίλτρο ribbon + διακόπτης ειδικότητας · **ζωντανός υπολογισμός + φίλτρο sidebar/dashboard + ορατός διακόπτης δουλειάς**. Φάση 0 (ευρήματα + Ε-1…Ε-7) έκλεισε. ✅ **Η Φάση 3 ΕΚΛΕΙΣΕ — επαληθεύτηκε ζωντανά σε ΤΕΣΣΕΡΙΣ γύρους** (16:39 · 16:58 · 17:13 · 17:30 — §14.6 … §14.6.3), με **όλες τις έξι** θέσεις του διακόπτη και **έξι στις έξι** ταυτόσημους αριθμούς κώδικα/οθόνης. Η οθόνη βρήκε **τρία** ελαττώματα που τα tests δεν έβλεπαν (Ε14.θ/Ε14.ι · Ε14.ια · Ε14.ιβ). **56 anchor tests**, 255 πράσινα στο `src/config`. ✅ **ΦΑΣΗ 3.6 ΥΛΟΠΟΙΗΜΕΝΗ** (§14.7): ο δείκτης σε **τρία** επίπεδα — κεφαλίδα · **μέσα σε κάθε δοχείο** · **«Αποκάλυψη κρυμμένων»** *(πρότυπο Revit, επεκταμένο εκεί που ο ίδιος ο Revit σταματά — Υ-11)*· νέα **Ε14.ιγ** *(ο κανόνας θέσης)*· **74 πράσινα** στα δύο αρχεία config. ✅ **ΚΑΙ Η ΦΑΣΗ 3.6 ΕΠΑΛΗΘΕΥΤΗΚΕ ΖΩΝΤΑΝΑ** (21:48→22:10, §14.7.7) — και τα **τρία** επίπεδα, με **τέταρτη** ανεξάρτητη σύμπτωση κώδικα/ανθρώπου· η οθόνη βρήκε **δύο** ελαττώματα διάταξης *(το ένα λανθάνον: μπάρα που θα κυλούσε εκτός οθόνης ⇒ **sticky**)*. Μένουν **4 μικρά** αδοκίμαστα (§14.7.7). Μετά: Φάση 3.5 (η πρώτη μέρα). |
| **Ημερομηνία** | 2026-08-02 |
| **Συγγραφείς** | Claude Opus 5 + Γιώργος Παγώνης |
| **Σχετικά** | 🔴 **ADR-787 (Η ΠΟΛΥ-ΟΡΓΑΝΙΣΜΙΚΗ ΠΛΑΤΦΟΡΜΑ — εκεί απέκτησαν ιδιοκτήτη τα Σ-1/Σ-2/Σ-3 του §0.2)**, ADR-180 (dashboard tiles), ADR-244 (role management console), ADR-283 (project roles SSoT), ADR-345 (ribbon), ADR-443/444 (discipline tabs), ADR-662 (topography tab), ADR-657 (BIM authoring/presentation RBAC), ADR-703 (role-predicate SSoT), ADR-702 (tenant query scope) |
...
>
> 🔗 **ΤΟ ΟΡΑΜΑ ΕΧΕΙ ΠΛΕΟΝ ΔΙΚΟ ΤΟΥ ΕΓΓΡΑΦΟ: [ADR-787](./ADR-787-multi-organization-platform.md)**
> *(2026-08-21)*. Αυτό το ADR κρατά το ερώτημα **«τι βλέπω»**· το ADR-787 κρατά το ερώτημα
> **«ποιος ανοίγει χώρο και ποιος μπαίνει στον ξένο»**. Τα τρία σενάρια του **§0.2** ήταν
> ονομασμένα και **αδέσποτα** επί 19 μέρες — και το όραμα ξαναγράφτηκε στο ADR-777 §11 και
> ξανασυζητήθηκε από την αρχή στις 2026-08-21, **επειδή κανένα έγγραφο δεν ήταν ο τόπος του**.
> ⚠️ Νέα απόφαση για **ταυτότητα / χώρους / πρόσβαση σε ξένο χώρο** γράφεται **εκεί**, όχι εδώ.
...
`DEFAULT_RIBBON_TAB_ORDER` = **15 μόνιμα tabs**, ήδη χωρισμένα ανά μελέτη:

> 🔴 **ΔΙΟΡΘΩΣΗ 2026-08-02 (Φάση 2): είναι 15, όχι 16.** Ο πίνακας αμέσως από κάτω
> **πάντα** απαρίθμησε 15 *(1 home + 1 structural + 1 architecture + 6 ΗΛΜ +
> 1 topography + 5 κοινά)* — το «16» ήταν **λάθος άθροιση** που αντιγράφηκε σε
> **πέντε** σημεία του ADR και στον handoff. Το εντόπισε το anchor test της
> Φάσης 2 **την πρώτη φορά που έτρεξε**. Κανόνας **N.12**: *άνοιξε το αρχείο, μην
> αντιγράψεις τον αριθμό.* ⚠️ Τα πλήθη **ανά θέση** διακόπτη (7/7/12/7/5) ήταν
> **σωστά** — μόνο το άθροισμα ήταν λάθος.
...
`src/server/auth/identity-record.ts` *(πρώην `pending-registration.ts` — ADR-853 Φ1)* (**ADR-660 — self-registration hardening**)
⚠️ **Δείκτης ενημερωμένος 2026-08-23**: έλεγε `src/app/api/auth/complete-registration/route.ts`, που
**διαγράφηκε** (ADR-660 §5.13 — ήταν αδρομολόγητο, 401άριζε σε ολόκληρο τον πληθυσμό του, και ο μόνος
κλάδος του που έγραφε κάτι **υποβάθμιζε** τον καλούντα). Η **συμπεριφορά** που περιγράφεται παρακάτω
δεν άλλαξε — μετακόμισε στο SSoT, που το καλεί το `POST /api/auth/session`:
...
### 2.9 🔴 Οι τρεις πηγές δικαιωμάτων — στον browser φτάνει **ΜΙΑ** *(μετρημένο 2026-08-02, Φάση 3)*

Το **Π-15** διόρθωσε το Ε1.ζ: οι πηγές είναι **τρεις**, όχι δύο. Η Φάση 3 μέτρησε το επόμενο
ερώτημα — *πόσες από αυτές φτάνουν εκεί που γίνεται το φιλτράρισμα;*

`src/app/api/admin/set-user-claims/claims-handler.ts:159-164`:

```ts
const rolePermissions  = PREDEFINED_ROLES[globalRole]?.permissions ?? [];
const mergedPermissions = new Set([...rolePermissions, ...permissions]);
...
| Πηγή | Πού ζει | Φτάνει στον client; |
|---|---|---|
| `GlobalRole` | custom claims | ✅ |
| `ProjectRole` | `/projects/{pid}/members/{uid}` | ❌ **ποτέ** |
| `PermissionSets` | `members/{uid}.permissionSetIds` | ❌ **ποτέ** *(κι εκεί ζει ΟΛΟ το νομικό)* |

🔴 **Συνέπεια**: φίλτρο που κρύβει ό,τι δεν βεβαιώνουν τα claims θα έκρυβε το «Σχέδιο» από
τον Νίκο της εταιρείας Β — που είναι `external_user` στην εταιρεία αλλά **μελετητής στο
έργο** (Ε-5.1). **Άδεια οθόνη**, δηλαδή το ελάττωμα του ACC (§6.11.1), γραμμένο από εμάς.

### 2.10 🔴 Ο `super_admin` έχει **κενή** λίστα permissions *(μετρημένο 2026-08-02, Φάση 3)*

`src/lib/auth/roles.ts:55-62`:

```ts
super_admin: { permissions: [], level: 0, isBypass: true }   // ← ΚΕΝΗ
```

Όλη η δύναμή του ζει στο `isBypass`, και στα claims του γράφεται **μόνο** `admin_access`.
Υπολογισμός δουλειών **χωρίς** έλεγχο bypass θα έδινε στον ιδιοκτήτη της εφαρμογής **μόνο τη
Διαχείριση** και θα του έκρυβε τις άλλες πέντε. Ο έλεγχος γίνεται με το υπάρχον
`isRoleBypass()` — **ποτέ** χειρόγραφο `globalRole === 'super_admin'` *(θα ήταν έβδομο
λεξιλόγιο ρόλων)*. Anchor test **Μ-1** το φυλάει, με ρητή δοκιμή «χωρίς bypass ⇒ μόνο Διαχείριση».

---

## 3. Το πραγματικό εμπόδιο — **ΕΝΝΕΑ** λεξιλόγια ρόλων

> 🔴 **ΔΙΟΡΘΩΘΗΚΕ 2026-08-24 (ADR-798 §3.1): ήταν γραμμένο «τέσσερα» — είναι ΕΝΝΕΑ.**
> Τα τέσσερα ήταν όσα βρήκε η σάρωση της 2026-08-02, που κοίταξε **μόνο** τα λεξιλόγια
> *εξουσιοδότησης*. Τα υπόλοιπα πέντε ζουν σε `types/contacts/`, σε σελίδα onboarding και σε
> subapp — δηλαδή **εκτός** της περιοχής που ρωτήθηκε. **Πέμπτη φορά «μπαγιάτικος αριθμός σε
> έγγραφο»** μετά τα Π-5 · Π-16 · `ssot-discover` · `.jscpd-baseline` (κανόνας N.12).
> ⚠️ **Οι αριθμοί των γραμμών 1-4 ΔΕΝ άλλαξαν** — προστέθηκαν οι 5-9 *(«never renumber»)*.
...
| 5 | `PersonaType` — `src/types/contacts/personas.ts:25` (ADR-121) | 9: `construction_worker`, `engineer`, `accountant`, `lawyer`, `property_owner`, `client`, `supplier`, `notary`, `real_estate_agent` | Ποια **επιπλέον πεδία** δείχνει η φόρμα **επαφής** — ⚠️ όχι λογαριασμού |
| 6 | `EngineerSpecialty` — `src/types/contacts/personas.ts:60` | 7: `civil`, `architect`, `mechanical`, `electrical`, `surveyor`, `chemical`, `mining` | Ποια **ειδικότητα ΤΕΕ** — δεύτερο επίπεδο του #5 |
| 7 | `UserType` — `src/auth/types/auth.types.ts:25` | 3: `citizen`, `professional`, `technical` | Πόσο **προχωρημένο εργαλείο** δείχνω — 🔴 **αυτο-δηλωμένο σε `localStorage`**, βλ. **Π-19** |
| 8 | `DeptKey` — `src/app/(app)/onboarding/organization/page.tsx:14` | 4: `accounting`, `sales`, `engineering`, `legal` | Ποια **τμήματα** έχει ο **νέος οργανισμός** — άξονας οργανισμού, όχι ανθρώπου |
| 9 | `TradeCode` — `src/subapps/procurement/types/trade.ts:35` (ADR-267) | 32 | Τι **συνεργείο** είναι ο υπεργολάβος — σχέση με **προμήθεια** |

🔴 **Κανένα από τα εννέα δεν απαντά «τι ΕΙΝΑΙ αυτός ο άνθρωπος»** — το κενό που ανέλαβε το
**ADR-798**. Και η θεραπεία **δεν** είναι δέκατο λεξιλόγιο: η απάντηση είναι **ESCO/ISCO-08**
*(**ADR-132**)*, που υπάρχει ήδη πλήρες πάνω στο `Contact` και του λείπει **μόνο η γέφυρα** προς
τον λογαριασμό *(το `Contact` **δεν έχει `uid`**)*.
...
| Θέση διακόπτη | Ribbon tabs | Από 15 → |
|---|---|---|
...
| Όλα *(προεπιλογή, = σημερινή συμπεριφορά)* | όλα, ακριβώς όπως σήμερα | **15** |

✅ **ΥΛΟΠΟΙΗΘΗΚΕ (Φάση 2, 2026-08-02).** Η κατανομή **δεν** ζει ως «λίστα tabs ανά
θέση» *(αυτό θα ήταν δεύτερη λίστα tabs δίπλα στο `ribbon-default-tabs.ts`)*: κάθε
tab δηλώνει **μία φορά** πού ανήκει, στο `RIBBON_TAB_SPECIALTIES`
(`ui/ribbon/data/ribbon-tab-specialties.ts`), και τα πλήθη **παράγονται**. Νέο tab
χωρίς ανάθεση ⇒ **κόκκινο anchor test** — το **Υ-4** σε λειτουργία.

#### 5.1.1 ✅ Η ΖΩΝΤΑΝΗ ΕΠΑΛΗΘΕΥΣΗ *(Γιώργος, στιγμιότυπα 2026-08-02 15:38 & 15:39)*

> Πράσινα tests **δεν** αποδεικνύουν ότι κάτι δουλεύει στην οθόνη. Αυτό εδώ το αποδεικνύει.

| Θέση διακόπτη | Καρτέλες **μετρημένες στην οθόνη** | Δείκτης | Αναμενόμενο |
|---|---|---|---|
| **Όλα** | Αρχική · Δομικά · Αρχιτεκτονικά · Ηλεκτρολογικά · Ύδρευση · Αποχέτευση · Θέρμανση · Κλιματισμός · Πυρόσβεση & Αέριο · Τοπογραφικό · Εισαγωγή · Ανάλυση · Προβολή · Επισημείωση · Ρυθμίσεις = **15** | *(κανένας — σωστά)* | **15** ✅ |
| **Αρχιτεκτονικά** | Αρχική · Αρχιτεκτονικά · Εισαγωγή · Ανάλυση · Προβολή · Επισημείωση · Ρυθμίσεις = **7** | **«8 κρυμμένες»** | **7** · 15−7=**8** ✅ |

Επιβεβαιώθηκαν επίσης: το dropdown δείχνει **και τις έξι** θέσεις μεταφρασμένες με ✓ στην
ενεργή· ο διακόπτης παραμένει **προσιτός** και στις δύο θέσεις (ζει στη γραμμή καρτελών,
όχι μέσα σε καρτέλα)· ο δείκτης «Χ κρυμμένες» εμφανίζεται **μόνο** όταν όντως κρύβεται κάτι.

🔴 **Δεν επαληθεύτηκαν ακόμη ζωντανά**: η θέση **ΗΛΜ** (αναμενόμενο **12**) και η **επιβίωση
σε refresh**. Καλύπτονται από tests, αλλά test ≠ οθόνη.
...
### 6.12 ΠΕΜΠΤΗ ΕΡΕΥΝΑ *(2026-08-02, για τη Φάση 1)* — **τα εγκάρσια και τα ευαίσθητα**

> Πέμπτο ερώτημα, από τις δύο ανοιχτές αποφάσεις του §14.4: **(α)** πού ανήκει κάτι που
> ανήκει παντού *(οι αναφορές)* και **(β)** επιτρέπεται να κρύψεις κάτι που **δεν
> προστατεύεται** *(τα νομικά)*.

#### 6.12.1 Τα εγκάρσια — **ομοφωνία τεσσάρων**

| Ποιος | Τι κάνει με τις αναφορές |
|---|---|
| **ACC** | *«The reports tool is **accessible to all** project administrators and team members… **however, the reports you can view are based on your access to project modules**… the cards displayed depend on your access to project services.»* |
| **Procore** | **Δύο στρώματα**: δικαίωμα να *φτιάχνεις* αναφορές · δικαίωμα στα *δεδομένα* — *«determined by your **existing tool permissions**… **no tool-level admin access is required**»*· *«if you have access to view the item **in the tool**, you can view all associated data»* |
| **Salesforce** | Το tab «Reports» ανήκει σε **πολλά** Lightning Apps· το **περιεχόμενο** φιλτράρεται από **folder sharing**, όχι από το tab |
| **SAP Fiori** | Ο *space* αντιστοιχεί σε **ρόλο**, αλλά τα αναλυτικά πλακίδια έρχονται από **catalogs** που κληρονομούν τα δικαιώματα του **αντικειμένου** |

🔑 **Ο κανόνας: η αναφορά ΔΕΝ ταξινομείται — κληρονομεί την ορατότητα της πηγής της.**
Ειρωνεία που αξίζει να καταγραφεί: ο **ACC**, που τα **δικαιώματα** τα **αντιγράφει**
(§6.9.1, το ελάττωμα του Υ-5), τις **αναφορές** τις **κληρονομεί** σωστά. Παίρνουμε το
σωστό από κάθε πλατφόρμα, όχι την πλατφόρμα ολόκληρη.

#### 6.12.2 Τα ευαίσθητα — **κρύβουν, αλλά ΠΟΤΕ αντί για**

| Ποιος | Τι λέει |
|---|---|
| **Procore** | *«If a user has **'None'** permissions to a tool, it **will not be visible** in their tool menu.»* — η απόκρυψη είναι **συνέπεια** πραγματικού δικαιώματος |
| **Salesforce** | **Δύο ξεχωριστές** ρυθμίσεις, ρητά τεκμηριωμένες: *Tab Settings* (ορατότητα) ≠ *Object Permissions* (πρόσβαση). *«Hiding a tab does **not** restrict access to the underlying records.»* Ο διαχειριστής ρυθμίζει **και τα δύο** |
| **ACC** | *«**Individual-based** permissions… for exception or **sensitive folders, such as commercial or contract documentation**»*· *«Inviting a member… does **not** automatically grant access to project files»* |
| **OWASP A01:2021** | Το **#1** ρίσκο, σε **94%** των εφαρμογών. Ρητό αντι-πρότυπο: *«reliance on **client-side** access check enforcement»*. Κανόνας: *«Except for public resources, **deny by default**»* |
| **Saltzer & Schroeder (1975)** | *«A conservative design must be based on arguments **why objects should be accessible**, rather than why they should not.»* |

🔑 **Η απόκρυψη ΑΚΟΛΟΥΘΕΙ το δικαίωμα. Δεν το προσποιείται ποτέ.** Το `legal_viewer` με MFA
που **ήδη έχουμε** είναι ακριβώς η πρακτική των μεγάλων — το λάθος δεν είναι στο μοντέλο,
είναι ότι το μενού το προσπέρασε.

#### 6.12.3 Πηγές πέμπτης έρευνας

- Procore — [Project 360 Reporting permissions](https://support.procore.com/products/online/user-guide/project-level/reports/permissions) · [What are permissions and how do they work](https://support.procore.com/faq/what-are-permissions-in-procore-and-how-do-they-work)
- ACC — [Reports](https://learnacc.autodesk.com/acc-reports) · [Best Practice: Permission Set up](https://resources.imaginit.com/autodesk-construction-cloud/acc-best-practice-part-2-permission-set-up) · [Folder permission levels](https://www.autodesk.com/support/technical/article/caas/sfdcarticles/sfdcarticles/Differences-in-folder-permission-levels-in-ACC-Docs.html)
- Salesforce — [Access to Report Folders](https://help.salesforce.com/s/articleView?id=analytics.access_report_folders.htm&language=en_US&type=5) · [Tab Settings](https://help.salesforce.com/s/articleView?id=platform.permissions_tab_settings_ref.htm&language=en_US&type=5)
- SAP Fiori — [Launchpad spaces & pages](https://www.sap.com/design-system/fiori-design-web/v1-145/foundations/integration-and-services/sap-fiori-launchpad/launchpad)
- Ασφάλεια — [OWASP Top 10:2021 A01](https://owasp.org/Top10/2021/A01_2021-Broken_Access_Control/) · [OWASP Authorization Cheat Sheet](https://cheatsheetseries.owasp.org/cheatsheets/Authorization_Cheat_Sheet.html) · [Saltzer & Schroeder](https://www.cs.virginia.edu/~evans/cs551/saltzer/)
- UI — [NN/g — Empty states](https://www.nngroup.com/articles/empty-state-interface-design/) · [Smashing — Hidden vs. Disabled](https://www.smashingmagazine.com/2024/05/hidden-vs-disabled-ux/)

---

### 6.13 ΕΚΤΗ ΕΡΕΥΝΑ *(2026-08-02, για τη **Φάση 3.6**)* — **πώς δηλώνεις ότι κάτι λείπει**

> **Το ερώτημα**: όχι «πώς κρύβεις» *(αυτό απαντήθηκε στην πρώτη έρευνα)* αλλά **«τι λες στον
> χρήστη για ό,τι έκρυψες»** — και ειδικά για τα **13** υπο-στοιχεία που κλαδεύονται μέσα σε
> **ΟΡΑΤΟΥΣ** γονείς και δεν μετριούνται πουθενά μετά τη διόρθωση των 17:13.

#### 6.13.1 🔴 Το εύρημα που καθόρισε τη λύση: **και οι δύο κορυφαίοι έχουν το ΙΔΙΟ κενό**

| Εργαλείο | Τι κάνει | Το **μετρημένο** όριό του |
|---|---|---|
| **Revit** — `Reveal Hidden Elements` | Τα κρυμμένα εμφανίζονται **στη θέση τους** σε ματζέντα, τα ορατά γκριζάρουν, και **ολόκληρη** η περιοχή σχεδίασης παίρνει **έγχρωμο περίγραμμα** όσο ο τρόπος είναι ενεργός | 🔴 Η τεκμηρίωση της Autodesk το γράφει **ρητά**: αποκαλύπτει ό,τι κρύφτηκε *by Category · by Element · Temporary Hide/Isolate* — **«but not by Filter»** |
| **VS Code** — δέντρο με φίλτρο | **Δύο** τρόποι: *highlight* *(κρατά τα πάντα και βάζει **badge με πλήθος** στους γονείς που περιέχουν ταίριασμα)* και *filter* *(τα αφαιρεί)* | 🔴 Στον τρόπο **filter** δεν υπάρχει **καμία** ένδειξη ποιοι κόμβοι δεν λήφθηκαν υπόψη — **ανοιχτό ζήτημα** στο δικό τους repo |
| **ArchiCAD** — Layer Combinations / Renovation Filter | Εναλλαγή από το Quick Options Bar· η κατάσταση φαίνεται ως **όνομα συνδυασμού** | Κανένα **πλήθος**. Ο χρήστης που δεν βρίσκει στοιχείο καταλήγει στο άρθρο υποστήριξης *«Missing elements in Archicad»* — δηλαδή **η τεκμηρίωση αντικαθιστά το UI** |
| **Cinema 4D** — Solo / Object Manager | **Πορτοκαλί** κουμπί όσο το solo είναι ενεργό· «φανάρια» ορατότητας ανά αντικείμενο | Ο δείκτης είναι **δυαδικός** *(κρύβω / δεν κρύβω)*, ποτέ ποσοτικός |
| **Figma** — Layers panel | Το κρυμμένο layer **δεν φεύγει ποτέ** από τη λίστα: γκριζάρει με κλειστό μάτι | Δεν έχει καθόλου έννοια «φίλτρο ρόλου/δουλειάς» — δεν απαντά το ερώτημά μας |
| **NN/g · Baymard** — filtering UX | «Applied filters» σε **σύνοψη**· **αριθμητικός δείκτης** στον γονιό που έχει επιλογή από μέσα του· *embrace redundancy* | Γενικό πρότυπο e-commerce — χρήσιμο ως **σχήμα**, όχι ως έτοιμη λύση πλοήγησης |

> 🔑 **Το συμπέρασμα δεν ήταν αναμενόμενο**: το πρότυπο «τρόπος αποκάλυψης» **υπάρχει** και στους
> δύο κορυφαίους — και **και οι δύο τον αφήνουν τυφλό ακριβώς στο φιλτράρισμα**, που είναι η
> **μόνη** περίπτωση εδώ. Ο ένας το τεκμηριώνει ως περιορισμό, ο άλλος το έχει ως ανοιχτό bug.
> **Δεν χρειάστηκε να εφεύρουμε μηχανισμό — χρειάστηκε να μην αντιγράψουμε το κενό τους.**

#### 6.13.2 Πηγές έκτης έρευνας

- Revit — [Reveal and Unhide Hidden Elements (Autodesk)](https://help.autodesk.com/cloudhelp/2022/ENU/Revit-DocumentPresent/files/GUID-B61C4920-0F8D-4243-9AD5-C75EC5BF0C53.htm) · [Reveal Hidden Elements — Real World Revit](https://revit.miraheze.org/wiki/Reveal_Hidden_Elements) · [Troubleshooting visibility issues](https://www.click2bim.com/blog/revit/troubleshooting-visibility-issues/)
- VS Code — [Lists And Trees (highlight vs filter, badge στους γονείς)](https://github.com/Microsoft/vscode/wiki/Lists-And-Trees) · [Explorer: resolve all folders before filtering (#66971)](https://github.com/microsoft/vscode/issues/66971) · [Tree find control](https://dev.to/nickytonline/vs-code-tip-of-the-week-tree-find-control-fg7)
- ArchiCAD — [Layer Combinations](https://help.graphisoft.com/AC/26/INT/_AC26_Help/025_Attributes/025_Attributes-6.htm) · [Missing elements in Archicad](https://support.graphisoft.com/hc/en-us/articles/32361065292305-Missing-elements-in-Archicad)
- Cinema 4D — [Viewport Solo Mode (Maxon)](https://help.maxon.net/c4d/en-us/Content/html/52991.html)
...
- UX — [Baymard — Display «Applied Filters» in an Overview](https://baymard.com/blog/how-to-design-applied-filters) · [Pencil & Paper — Enterprise filtering patterns](https://www.pencilandpaper.io/articles/ux-pattern-analysis-enterprise-filtering)

---

### 6.14 ΕΒΔΟΜΗ ΕΡΕΥΝΑ *(2026-08-02, για τη **Φάση 3.5α**)* — **ρωτάς ή προτείνεις;**

> Έβδομο ερώτημα, από το **Ε7.γ**: *«μία προαιρετική ερώτηση — τι δουλειά κάνεις;»*.
> Πού ζει, τι γίνεται στην παράλειψη, και **αν** πρέπει καν να ρωτηθεί.
> ⚠️ Η έρευνα **ανέτρεψε το ίδιο το ερώτημα** — για **τρίτη** φορά σε αυτό το ADR.

#### 6.14.1 Τι κάνουν πραγματικά οι τέσσερις

| Παίκτης | Ρωτά; | Πού | Προεπιλογή χωρίς απάντηση |
|---|---|---|---|
| **Revit ≤2021** | Στην **εγκατάσταση**, όχι στο προϊόν | installer / deployment `Revit.ini` | *«Autodesk Revit: **All**»* |
| **Revit 2022+** | 🔴 **ΝΑΙ** — modal *«Hello There!»* (**Ask Profile**): ειδικότητα **+** ρόλος εργασίας, με **λίστα προτεινόμενων αλλαγών UI για review** και **accept/decline**. Εμφανίζεται **μία φορά**, ελεγχόμενο από `%AppData%\…\Revit\Profile\Profile.xml` | modal 1ης εκκίνησης· **διόρθωση αλλού**: `File → Options → User Interface → Configure` | — |
| **ArchiCAD** | **ΟΧΙ ξεχωριστά** — ο επιλογέας Work Environment ζει **μέσα** στο «Start ArchiCAD», κάτω από το *«What would you like to do?»* | ροή έναρξης | **Last Profile Used** |
| **Cinema 4D** | **ΟΧΙ** | — | **Standard** layout · μόνιμος επιλογέας πάνω δεξιά · `Save as Startup Layout` |

🔑 **2 στους 3 δεν ρωτούν καθόλου.**

#### 6.14.2 🔴 Ο ένας που ρωτά, **το πληρώνει** — τεκμηριωμένο, όχι εικασία

Για το «Hello There!» υπάρχουν **και τα δύο**: αίτημα στο **Autodesk Ideas** *«Revit 2022 – Disable
"Hello There!" Prompt on First Run»* **και επίσημο άρθρο υποστήριξης της Autodesk «How to disable
Hello There window in Revit»**.

> **Ίδιο σχήμα με το §6.11.1**: όταν το προϊόν χρειάζεται άρθρο υποστήριξης για να εξηγήσει —
> ή να απενεργοποιήσει — μια οθόνη, **η οθόνη είναι το σφάλμα**.

⚠️ **Και η αιτία είναι εντοπίσιμη**: ο Revit **ρωτά σε ένα μέρος και σε βάζει να διορθώσεις σε
άλλο**. Το modal ζει στην εκκίνηση· η διόρθωση στο `File → Options → User Interface → Configure`.
Ο χρήστης δεν μαθαίνει ποτέ **πού ζει η απόφαση** — άρα την επόμενη φορά δεν την βρίσκει.

#### 6.14.3 🔴 Το **Ε7.δ** στηρίζεται σε **κομμένη παράθεση**

Το §6.11.2 παραθέτει *«if you do not select a discipline, an appropriate default is used»* και το
ερμήνευσε ως «**η δουλειά με τα περισσότερα δικαιώματα**». Η **πλήρης** πρόταση της Autodesk:

> *«If you do not select a discipline, the default is as follows: **Autodesk Revit: All**;
> Revit Architecture: Architectural; Revit MEP: Mechanical.»*

Το «appropriate» δεν σημαίνει «υπολογισμένο από τον ρόλο» — σημαίνει **«ό,τι σου δίνει η άδειά
σου»**: το **πλήρες** προϊόν → **All**· οι **περιορισμένες** SKU → η ειδικότητα **του προϊόντος**.
Στον Νέστορα η «άδεια» είναι τα permissions ⇒ η προεπιλογή είναι **`JOB_ALL`** — που **ήδη ισχύει**.

#### 6.14.4 🔴 Οι πηγές του §6.11.4 είναι **μεροληπτικές**

Το «+30–50% activation» προέρχεται από **appcues.com** και **saasui.design** — εταιρείες που
**πουλάνε εργαλεία onboarding**. Ανεξάρτητη μέτρηση λέει το αντίθετο:

> **NN/g**: *οι χρήστες που **πέρασαν** το onboarding εκτέλεσαν τις εργασίες **εξίσου επιτυχώς**
> με όσους το **προσπέρασαν**.*

#### 6.14.5 Ο κανόνας που χωρίζει τις δύο οικογένειες — **NN/g**

| Έννοια | Ποιος ελέγχει | Ετυμηγορία NN/g |
|---|---|---|
| **Customization** | ο **χρήστης** επιλέγει ρητά | ✅ το συνιστώμενο *(C4D layouts, ArchiCAD profiles, ο δικός μας διακόπτης)* |
| **Personalization** | το **σύστημα** προβλέπει | ⚠️ *«Personalization is **over-rated**»* |

Και ο μηχανισμός της αποτυχίας έχει όνομα: **active user paradox** — *οι χρήστες «don't want to
spend time setting up complex personalization features»*. Η Amazon πετυχαίνει επειδή απαιτεί
***«no extra work on the users»***: **προτείνει** από δεδομένα, **δεν ρωτά** και **δεν κρύβει**.

#### 6.14.6 🔑 Το αποφασιστικό: **ο Revit ρωτά επειδή δεν ξέρει. Εμείς ξέρουμε.**

Ένα Revit, όλοι οι κλάδοι, **καμία** πληροφορία ρόλου ⇒ η ερώτηση είναι ο **μόνος** τρόπος. Ο
Νέστωρ έχει **13 ρόλους με ρητά permissions** και το `pickDefaultJob()` **υπολογίζει ζωντανά**.

> **Ερώτηση για κάτι που ήδη ξέρεις είναι το active user paradox σε καθαρή μορφή:**
> ζητάς από τον χρήστη δουλειά για να μάθεις ό,τι έχεις ήδη μετρήσει.

#### 6.14.7 Πηγές έβδομης έρευνας

- Revit — [Select the Discipline for a Revit Installation](https://help.autodesk.com/cloudhelp/2016/ENU/Revit-Installation/files/GUID-9BEBC25A-680A-435C-AC53-15FACEFAD787.htm) · [Your User Profile (Ask Profile dialog)](https://help.autodesk.com/cloudhelp/2022/ENU/Revit-GetStarted/files/GUID-18B2CD71-6923-49DC-B900-3364A8D1A389.htm) · [Autodesk Ideas — Disable «Hello There!» Prompt on First Run](https://forums.autodesk.com/t5/revit-ideas/revit-2022-disable-quot-hello-there-quot-prompt-on-first-run/idi-p/10730561) · [How to disable Hello There window in Revit](https://www.autodesk.com/support/technical/article/caas/sfdcarticles/sfdcarticles/How-to-disable-Hello-There-window-in-Revit.html) · [Turn off 2022 User Profile prompt (Revit Forum)](https://www.revitforum.org/node/442943)
- ArchiCAD — [Start ArchiCAD dialog](https://help.graphisoft.com/AC/18/INT/AC18Help/01_Configuration/01_Configuration-2.htm) · [Default Work Environment Profiles](https://helpcenter.graphisoft.com/user-guide/127854/)
- Cinema 4D — [New Layouts: Cinema 4D R25 and newer (Maxon)](https://support.maxon.net/hc/en-us/articles/4406397183634-New-Layouts-Cinema-4D-R25-and-newer-Hot-Corners-Managers-and-New-Icon-Placement) · [How to choose default startup scene (2024.1+)](https://support.maxon.net/hc/en-us/articles/10816546845468-How-to-choose-default-startup-scene-in-Cinema-4D-2024-1-and-newer)
- NN/g — [Personalization is Over-Rated](https://www.nngroup.com/articles/personalization-is-over-rated/) · [Personalization vs Customization](https://www.nngroup.com/videos/personalization-customization/) · [Customization Features (report)](https://www.nngroup.com/reports/customization-features/)
- Figma — [Figma's initial onboarding](https://useronboarding.academy/user-onboarding-inspirations/figma-initial-onboarding)
...
| **Υ-11** | 🔴 **Ο τρόπος αποκάλυψης είναι ΤΥΦΛΟΣ ΣΤΟ ΦΙΛΤΡΟ — και στους δύο κορυφαίους** (§6.13.1). Το `Reveal Hidden Elements` του **Revit** δηλώνει ρητά *«but not by Filter»*· το δέντρο του **VS Code** σε κατάσταση *filter* δεν λέει **τίποτα** για τους κόμβους που αφαίρεσε *(ανοιχτό ζήτημα στο repo τους)*. Δηλαδή: το εργαλείο που υπάρχει **ακριβώς** για να μη χαθεί τίποτα σιωπηλά, δεν καλύπτει τη μόνη περίπτωση όπου κάτι όντως χάνεται σιωπηλά | **Η «Αποκάλυψη κρυμμένων» καλύπτει το φιλτράρισμα — γιατί γι' αυτό υπάρχει.** Τα κρυμμένα επιστρέφουν **στη θέση τους**, υποβαθμισμένα *(Α-2)* και **πλήρως λειτουργικά** *(η δουλειά είναι ετικέτα, ποτέ πύλη — §14.5)*, με **μόνιμη ένδειξη κατάστασης** και έξοδο με **ένα** κλικ. Και **τρία** επίπεδα αντί για ένα: ο δείκτης της κεφαλίδας *(τι λείπει από την οθόνη)* · ο δείκτης **μέσα σε κάθε δοχείο** *(τι λείπει από **εδώ**)* · η αποκάλυψη *(**πού** ακριβώς ήταν)* |
| **Υ-12** | 🔴 **Ο Revit ρωτά σε ένα μέρος και σε βάζει να διορθώσεις σε άλλο** (§6.14.2). Το modal «Hello There!» ζει στην πρώτη εκκίνηση· η διόρθωση στο `File → Options → User Interface → Configure`. Ο χρήστης **δεν μαθαίνει ποτέ πού ζει η απόφαση** — γι' αυτό υπάρχουν **και** αίτημα απενεργοποίησης **και** άρθρο υποστήριξης. Ο **C4D** το αποφεύγει σιωπώντας εντελώς, αλλά τότε **δεν προσφέρει τίποτα** | **Η πρόταση ζει ΠΑΝΩ ΣΤΟ ΙΔΙΟ ΤΟ ΧΕΙΡΙΣΤΗΡΙΟ** που θα την εκτελέσει *(`JobSwitch`)* ⇒ ο χρήστης **μαθαίνει πού ζει η απόφαση την ώρα που την παίρνει**. Και **τρία** ακόμη: **(α)** ο αριθμός «Χ από Υ» βγαίνει από τις **ίδιες** καθαρές συναρτήσεις που κάνουν το πραγματικό φιλτράρισμα ⇒ **δεν μπορεί να αποκλίνει** *(δεύτερος μετρητής = το ελάττωμα των 17:13)*· **(β)** η αποδοχή είναι **μόνιμα** αναστρέψιμη — ο Revit εφαρμόζει και σε αφήνει, εμάς η επαναφορά μένει ορατή *(δείκτης Φάσης 3.6)*· **(γ)** **σιωπή όπου δεν ξέρουμε**: πρόταση μόνο πάνω σε `granted`, ποτέ σε `unknown` — «δεν το κρύβω» **≠** «το προτείνω» |
...
| Ε2.ε | ⚠️ **Το σημερινό `complete-registration`** (που δίνει `external_user` + έναν `companyId`) πρέπει να αναθεωρηθεί: σήμερα ο λογαριασμός **γεννιέται μέσα** σε εταιρεία. — 🔄 **ΕΝΗΜΕΡΩΣΗ 2026-08-23 (ADR-660 §5.13)**: το endpoint **διαγράφηκε** (αδρομολόγητο· 401 σε ολόκληρο τον πληθυσμό του). Το σημείο αναθεώρησης **μετακινείται** στο `src/server/auth/identity-record.ts` *(πρώην `pending-registration.ts` — ADR-853 Φ1)* — που ήδη **δεν** χορηγεί `external_user` ούτε `companyId`. ⚠️ Το ζήτημα του Ε2.ε **δεν έκλεισε**: παραμένει «πού γεννιέται ο λογαριασμός», και ο ιδιοκτήτης του είναι πλέον το **ADR-787**. |
...
| **Ε5.ι** | 🔴 **Η ΑΠΟΦΑΣΗ ΕΙΝΑΙ ΤΡΙΩΝ ΚΑΤΑΣΤΑΣΕΩΝ, ΟΧΙ BOOLEAN** *(Φάση 3, 2026-08-02)*: `granted` · `unknown` · `none`. Το `unknown` σημαίνει **«η πηγή που θα απαντούσε δεν είναι διαθέσιμη εδώ»** (§2.9). 🔒 **Κρύβεται ΜΟΝΟ το `none`· το `unknown` ΠΟΤΕ.** Το Ε5.ζ *(fail-closed)* ισχύει ακέραιο στην **απόφαση** — καμία δουλειά δεν επινοείται· δεν ισχύει στο **φίλτρο**, γιατί fail-closed σε φίλτρο **UX** κρύβει από τον χρήστη πράγματα που **δικαιούται** (Ε5.η: UX, όχι ασφάλεια· Ε14.ζ: το φίλτρο δεν προσποιείται ότι αφαιρεί πρόσβαση). Πρότυπο: τα **διακριτά** `deny:*` του `decideAssetPackAccess()`, ποτέ σκέτο `false`. 🔑 Και **ξεκλειδώνει το Υ-6**: κάθε απόφαση φέρει τον λόγο της ⇒ *«βλέπεις το Σχέδιο επειδή έχεις `dxf:files:view`»* vs *«δεν το ξέρω — ο ρόλος σου στο έργο δεν φτάνει εδώ»*. **Ούτε ο ACC ούτε η Figma διακρίνουν «δεν δικαιούσαι» από «δεν ξέρω».** |
...
| Ε7.ια | 🔴 **ΚΑΜΙΑ ΣΕΛΙΔΑ ΕΠΙΛΟΓΗΣ ΣΕ ΚΑΘΕ ΕΙΣΟΔΟ** *(ερώτημα Γιώργου, 2026-08-02)*. **Πρώτη φορά ποτέ** ⇒ η **μία** προαιρετική ερώτηση του Ε7.γ. **Κάθε επόμενο login** ⇒ **τίποτα**: μπαίνει κατευθείαν στην **τελευταία ενεργή** δουλειά (Ε5.β). **Μία μόνο δουλειά** ⇒ δεν υπάρχει καν επιλογή. Η αλλαγή γίνεται **πάντα** από το μόνιμο χειριστήριο (Ε6.β), **χωρίς επανασύνδεση** (Ε1.β). ⚠️ Η παλιά διατύπωση του §10 *(«οθόνη επιλογής χώρου μετά το login»)* **ήταν παραπλανητική** — διορθώθηκε στη Φάση 4 |
| Ε7.ιβ | ⚠️ **ΡΟΛΟΣ ≠ ΔΟΥΛΕΙΑ ≠ ΕΙΔΙΚΟΤΗΤΑ** — τρία διαφορετικά πράγματα που η καθημερινή γλώσσα ενώνει σε ένα *(«ειδικότητα-ρόλος»)*: ο **ρόλος** = τι δικαιούσαι *(ορίζεται από τη συμμετοχή)*· η **δουλειά** = τι κάνεις τώρα *(υπολογίζεται, Ε5.α)*· η **ειδικότητα** = πώς σχεδιάζεις *(διακόπτης μέσα στο «Σχέδιο», τον γυρνά **ο χρήστης**, Α-1)*. Ο ρόλος `architect` **προτείνει** «Αρχιτεκτονικά»· **δεν τον κλειδώνει** εκεί (Α-5, Ε4.β, Ε1.ε) |
| **Ε7.γ′** | 🔴 **ΑΝΑΤΡΟΠΗ ΤΟΥ Ε7.γ *(έβδομη έρευνα, §6.14 — 2026-08-02)*: ΠΡΟΤΑΣΗ, ΟΧΙ ΕΡΩΤΗΣΗ.** Το Ε7.γ ζητούσε «μία προαιρετική ερώτηση». Η έρευνα στους **ίδιους** παίκτες το ανέτρεψε: **2 στους 3 CAD δεν ρωτούν καθόλου**, και ο ένας που ρωτά *(Revit 2022 «Hello There!»)* έχει **αίτημα απενεργοποίησης + άρθρο υποστήριξης**. Το **NN/g** το ονομάζει *active user paradox*. **Και το αποφασιστικό**: ο Revit ρωτά επειδή **δεν ξέρει** — εμείς **υπολογίζουμε** από 13 ρόλους (§6.14.6). ⇒ **Καμία ερώτηση, κανένα modal.** Μία **μη-μπλοκαριστική πρόταση**, πάνω στο ίδιο το χειριστήριο, με **μετρημένο** αποτέλεσμα πριν την αποδοχή *(πρότυπο Revit «review before accept»)* και **μόνιμη** αναίρεση *(Α-3)*. ⚠️ **ΣΙΩΠΗ ΟΠΟΥ ΔΕΝ ΞΕΡΟΥΜΕ**: η πρόταση απαιτεί `decision === 'granted'` — **ποτέ** `unknown`. Το `unknown` δεν **κρύβεται** (Ε14.ζ), αλλά δεν **προτείνεται**: αλλιώς ο τυφλός υπολογισμός του Π-15 θα πρότεινε την πρώτη κατά `JOB_ORDER` ως «γνώση» |
| **Ε7.δ′** | 🔴 **ΔΙΟΡΘΩΣΗ ΤΟΥ Ε7.δ — στηριζόταν σε ΚΟΜΜΕΝΗ ΠΑΡΑΘΕΣΗ** *(§6.14.3)*. Η πλήρης πρόταση της Autodesk είναι *«…the default is as follows: **Autodesk Revit: All**; Revit Architecture: Architectural; Revit MEP: Mechanical»*. Το «appropriate default» σημαίνει **«ό,τι σου δίνει η άδειά σου»**, **όχι** «η δουλειά με τα περισσότερα δικαιώματα». ⇒ Η προεπιλογή είναι **`JOB_ALL`** — που **ήδη ίσχυε** (`ActiveJobContext`), άρα **καμία αλλαγή κώδικα**: το Ε7.δ ήταν λάθος **στην αιτιολόγηση**, όχι στο αποτέλεσμα. Το `pickDefaultJob()` παραμένει σωστό και χρήσιμο — αλλά ως **περιεχόμενο της πρότασης**, όχι ως σιωπηλή εφαρμογή |
...
| **1** ✅ | Μητρώο **δουλειών** (SSoT· ορολογία Ε6.στ — **όχι** `Workspace`): ένα αρχείο, δεδομένα, μηδέν λογική | `src/config/jobs-registry.ts` | **ΕΓΙΝΕ 2026-08-02.** Το θεμέλιο. 33 anchor tests· στο `.ssot-registry.json` |
| **2** ✅ | Φίλτρο ribbon στον DXF viewer **+ ο ορατός διακόπτης ειδικότητας** (Ε6.β) | 7 αρχεία + 2 locale | **ΕΓΙΝΕ 2026-08-02.** Ανάθεση **ανά καρτέλα** (Υ-4 gate)· διακόπτης στη **γραμμή καρτελών** — όχι σε γρανάζι status bar, που είναι η αδυναμία **Υ-2** του AutoCAD· δείκτης «Χ κρυμμένες» με επαναφορά ενός κλικ (Α-3). Προεπιλογή «Όλα» ⇒ **καμία αλλαγή για υπάρχοντα χρήστη** μέχρι να γυρίσει ο ίδιος τον διακόπτη. Μηδέν αναφορές auth στο subapp — ο διακόπτης είναι **επιλογή χρήστη**, όχι δικαίωμα (Ε7.ιβ) |
| **3** ✅ | **Ο ζωντανός υπολογισμός** (`jobs-access.ts`) + φίλτρο **sidebar** + πλακίδια **dashboard** της ενεργής δουλειάς + δείκτης «Χ κρυμμένα» **+ ο ορατός διακόπτης δουλειάς** | 6 νέα + 6 τροποποιημένα + 2 locale | **ΕΓΙΝΕ 2026-08-02.** ⚠️ **Ο διακόπτης δουλειάς μετακινήθηκε ΕΔΩ από τη Φάση 4** *(απόφαση Γιώργου)*: χωρίς αυτόν η φάση είναι **ανεπαλήθευτη** — ο ιδιοκτήτης είναι `super_admin` (Ε4.ε) και θα έβλεπε «καμία αλλαγή». Δεν είναι πρόωρο κομμάτι της Φάσης 4: το **Ε6.β** έχει ήδη κλειδώσει ότι η δουλειά ζει **ΕΞΩ** από το μονοπάτι *οργανισμός › έργο* ⇒ το μονοπάτι χτίζεται **δίπλα** του, δεν το αντικαθιστά. Προεπιλογή **«Όλες οι δουλειές»** ⇒ **καμία αλλαγή** για υπάρχοντα χρήστη μέχρι να γυρίσει ο ίδιος τον διακόπτη (ίδιο με τη Φάση 2, Α-3) |
| **3.5α** ✅ | **Η πρόταση δουλειάς** (Ε7.γ′/Ε7.δ′ — §14.8). **ΟΧΙ ερώτηση**: μη-μπλοκαριστική πρόταση πάνω στο χειριστήριο, με μετρημένο αποτέλεσμα και μόνιμη αναίρεση | 3 νέα + 4 τροπ. + 2 locale | **ΕΓΙΝΕ 2026-08-02.** 🔴 Η **έβδομη έρευνα** (§6.14) ανέτρεψε **δύο** κλειδωμένες αποφάσεις: το Ε7.γ *(ρωτάμε)* και την **αιτιολόγηση** του Ε7.δ *(κομμένη παράθεση)*. Μηδέν επαφή με auth |
| **3.5β** | **Η πρώτη μέρα — το υπόλοιπο** (Ε-7): προσωπικός χώρος από την εγγραφή · προσγείωση προσκεκλημένου στο έργο του · κενές οθόνες κατά NN/g. ⚠️ **Εδώ τροποποιείται το ADR-660** (Ε7.ζ/Ε7.θ) και **εδώ πεθαίνει το Π-11** | tenancy + claims + ADR-660 | 🔴 **ΔΕΝ είναι «~3 αρχεία»** *(μετρημένο 2026-08-02)*: το `src/config/tenant.ts` έχει **hardcoded ΕΝΑΝ** `TENANT_COMPANY_ID` και το `bootstrap-company` είναι `super_admin`-only πάνω στον **ίδιο** έναν ⇒ «προσωπικός χώρος» σημαίνει **δημιουργία tenant**. Προαπαιτούμενο: το **§11.6** (`getActiveCompanyId()`) **πρώτο**. ⚠️ Και οι **κενές οθόνες** ανήκουν **εδώ**: μετρήθηκε ότι σήμερα **δεν έχουν αντικείμενο** — τα `COMMON_SIDEBAR_ROUTES` (6) + `COMMON_DASHBOARD_TILES` (3) εγγυώνται ότι **καμία** δουλειά δεν αδειάζει **κανένα** δοχείο· οι πραγματικές κενές οθόνες είναι σελίδες **χωρίς δεδομένα** |
| **4** | **Ο επιλογέας της τριάδας** (Ε-6): μονοπάτι *οργανισμός › έργο* + ~~χειριστήριο δουλειάς~~ *(**έγινε στη Φάση 3**· εδώ **προάγεται** — αλυσίδα αιτίασης Ε6.ε, κρυμμένες δουλειές Ε5.β)* + Ctrl+K + όψη ανά οργανισμό. ⚠️ **Προαγωγή** του υπάρχοντος `switchWorkspace()` και **ενοποίηση** με το `CompanySwitcher` (Ε6.ζ/Ε6.η) — **όχι** νέος μηχανισμός | ~4 αρχεία | Έχει νόημα μόνο αφού υπάρχουν δουλειές να διαλέξεις. Εδώ πεθαίνει το Π-8 |
...
> 🔑 **ΜΕΤΑΚΟΜΙΣΕ 2026-08-24 — ΑΥΤΟ ΤΟ ΕΓΓΡΑΦΟ ΔΕΝ ΚΑΤΕΧΕΙ ΠΛΕΟΝ ΚΑΝΕΝΑ ΑΠΟ ΤΑ ΤΕΣΣΕΡΑ.**
> Τα επίπεδα **γεννήθηκαν** εδώ *(2026-08-02)* και έμειναν **αδέσποτα**: το ADR-787 λύνει από τις
> 21/08 τα 2-3-4 **χωρίς ποτέ να τα κληρονομήσει ονομαστικά**, και το επίπεδο 1 δεν το πήρε
> κανείς. Ο πίνακας μένει εδώ ως **ιστορικό**· η **αυθεντία** είναι πλέον αλλού:
>
> | # | Επίπεδο | **Ιδιοκτήτης από 24/08** |
> |---|---|---|
> | **1** | **Άνθρωπος** *(Person)* | 🆕 **ADR-798** — *ταυτότητα προσώπου ξεχωριστά από εξουσιοδότηση* |
> | **2** | **Οργανισμός** *(Tenant)* | **ADR-787 §0.2** |
> | **3** | **Συμμετοχή** *(Membership)* | **ADR-787 §0.2** *(→ Κ-2, §5.1)* |
> | **4** | **Χώρος εργασίας** *(Workspace)* | **ADR-787 §0.2** — ⚠️ *το «τι βλέπω» παραμένει εδώ, §5-§8* |
>
> ⚠️ **Η απόδειξη ότι η αδέσποτη γνώση ΗΔΗ κόστισε** ζει στο **§11.6** παρακάτω: δύο έγγραφα
> έλυσαν το **ίδιο** πρόβλημα, χωριστά, με **άλλο όνομα**.
...
### 11.6 ✅ ΚΛΕΙΣΤΟ — υλοποιήθηκε ως `resolveEffectiveCompanyId`, με **άλλο όνομα**

> ✅ **ΕΚΛΕΙΣΕ 2026-08-24.** Η απόφαση **έγινε** — αλλά **όχι με το όνομα που ζήτησε αυτό το
> έγγραφο**, και αυτό είναι το μάθημα:
>
> | Όνομα | Εμφανίσεις στο `src/` *(μετρημένο 2026-08-24)* |
> |---|---|
> | `getActiveCompanyId` — **αυτό που ζήτησε το §11.6** | 🔴 **0** |
> | `resolveEffectiveCompanyId` — **αυτό που γράφτηκε** | ✅ **25** |
>
> 🔑 **Δύο έγγραφα έλυσαν το ίδιο πρόβλημα, χωριστά, με άλλο όνομα** — και κανένα δεν το έμαθε
> από το άλλο. Ο **μόνος** λόγος που δεν έγινε **τρίτη** συνάρτηση είναι ότι το **ADR-787 §5.3**
> έφτιαξε πύλη *(**CHECK 3.58**, «η αρχή του χώρου»)*: ένα κλειστό σύνολο δηλώσεων που
> **μπλοκάρει κάθε νέο απαντητή ενεργού χώρου**, ακόμα κι αν είναι σωστός, ώσπου να τον δει
> άνθρωπος. **Ο κανόνας που ζητούσε αυτή η παράγραφος υπάρχει πλέον ως ΕΚΤΕΛΕΣΙΜΟΣ ΦΡΟΥΡΟΣ,
> όχι ως πρόταση σε έγγραφο** — και είναι ακριβώς η διαφορά που ονομάζει το **CHECK 3.36**:
> *ένα anchor χωρίς gate είναι σχόλιο*.
>
> ➡️ **Η αυθεντία είναι πλέον το ADR-787 §5.3.** Ό,τι αφορά «ποιος απαντά σε ποιον χώρο ενεργώ»
> γράφεται **εκεί**. Το κείμενο παρακάτω μένει ως ιστορικό της απόφασης.

**Το αρχικό κείμενο (2026-08-02):**
...
| Π-13 | 🔴 **Ολόκληρο το νομικό υποσύστημα είναι σχεδιασμένο αλλά ΑΝΕΠΙΒΛΗΤΟ.** Τα **7** `legal:*` δεν ανήκουν σε κανέναν από τους 13 ρόλους — **εκ σχεδιασμού**: ζουν σε `legal_viewer`/`legal_manager` με **`requiresMfaEnrolled: true`**. Όμως **καμία επιβολή**: rules tenant-scoped χωρίς ρόλο, `/api/contracts` με `withAuth` **χωρίς permission**. Και το ίδιο το `/legal-documents` είναι **404** (καμία `page.tsx`). **Ξεχωριστή απόφαση ασφαλείας** — ΔΕΝ λύνεται με ετικέτα δουλειάς (Ε14.στ) | `permission-sets.ts`, `firestore.rules`, §14.4/2 |
| Π-14 | 🔴 **Το `reports:reports:view` το έχουν μόνο 4 ρόλοι** (`project_manager`, `site_manager`, `accountant`, `viewer`) — **όχι** ο `architect`, ο `engineer`, ο `sales_agent`, ο `company_admin`. **Δεύτερο Π-7**: φίλτρο στον γονιό `/reports` θα έκρυβε τις αναφορές από τον διαχειριστή της εταιρείας | `roles.ts`, Ε14.γ |
| Π-15 | 🔴 **Το Ε1.ζ λέει «ΔΥΟ πηγές» δικαιωμάτων — είναι ΤΡΕΙΣ**: `GlobalRole` + `ProjectRole` + **`PermissionSets`** (`lib/auth/permission-sets.ts`, ζωντανά: κονσόλα ADR-244 + API route + **υποχρεωτική αιτιολογία**). Η τρίτη δεν μετρήθηκε ποτέ — κι όμως εκεί ζει ΟΛΟ το νομικό υποσύστημα. Ζωντανός υπολογισμός Ε-5 που θα την αγνοούσε θα έβγαζε **λάθος** λίστα δουλειών. ✅ **Έκτο «το σωστό μοτίβο υπάρχει ήδη»**: το Ε5.δ ζητά «ρητή εξαίρεση με υποχρεωτικό `reason` + χορηγό» — η κονσόλα **ήδη** απαιτεί αιτιολογία· λείπει μόνο το `expiresAt` | `permission-sets.ts`, Ε1.ζ |
| Π-16 | **Το «16 ribbon tabs» ήταν λάθος άθροιση — είναι 15.** Γραμμένο σε **5** σημεία του ADR + στον handoff, ενώ ο πίνακας του §2.5 **πάντα** απαρίθμησε 15. Το βρήκε το anchor test της Φάσης 2 με το που έτρεξε. Κλασικό «μπαγιάτικος αριθμός σε έγγραφο» (N.12) — **τρίτη φορά** στο έργο, μετά τα Π-5 και το §12/ssot-discover | §2.5, §5.1, §14.0, §14.3 |
| Π-17 | ⚠️ **Το legacy λεξιλόγιο ρόλων (Π-2) διαρρέει σε ζωντανούς ελέγχους.** `app-sidebar.tsx:39` έλεγχε `globalRole === 'admin'` και το `ConditionalAppShell.tsx:133-139` ελέγχει `'admin'`/`'agent'`/`'sales'`/`'viewer'`/`'guest'` — **καμία** από αυτές τις τιμές δεν υπάρχει στα `GLOBAL_ROLES` (`super_admin`\|`company_admin`\|`internal_user`\|`external_user`). Είναι **νεκροί κλάδοι** που το `globalRole?: string` (χαλαρός τύπος στο `auth.types.ts:44`) αφήνει να περάσουν. Ο πρώτος **δεν μεταφέρθηκε** στο `useEffectivePermissions`· ο δεύτερος **δεν αγγίχτηκε** (αφορά route preloading, όχι δικαιώματα). **ΔΕΝ διορθώνεται εδώ** — το §10 απαγορεύει αλλαγές ρόλων στις Φάσεις 1-4 | `app-sidebar.tsx`, `ConditionalAppShell.tsx`, §2.9 |
| Π-18 | ⚠️ **Ο όρος `Job` είναι ήδη πιασμένος στο subapp**: `bim-3d/animation/RenderQueueStore.ts` χρησιμοποιεί `activeJobId` / `setActiveJob` / `JobRecord` για **εργασίες απόδοσης βίντεο**. Διαφορετικός τομέας, χωρίς καθολικό export ⇒ **δεν** συγκρούεται σήμερα, αλλά είναι ο **δεύτερος** υποψήφιος για «έκτο λεξιλόγιο» μετά το `Workspace` (Π-9). Ο κώδικας της δουλειάς ζει σε `config/jobs-*` και `contexts/ActiveJobContext` — **μην** εισαγάγεις `Job*` τύπους μέσα στο `bim-3d/animation` ή αντίστροφα | `RenderQueueStore.ts`, Ε6.στ |
...
| Π-19 | 🔴 **Η τριάδα `citizen` \| `professional` \| `technical` ζει σε 17 σημεία, με ΤΡΙΑ ονόματα — και το ένα από αυτά σημαίνει ΑΛΛΟ ΠΡΑΓΜΑ** *(κλειστή απογραφή 2026-08-24)*. **4 ονομασμένοι τύποι**: `UserType` *(`auth/types/auth.types.ts:25`)* · `UserType` **ξανά** *(`services/analytics-bridge-types.ts:12`)* · `UserTypeVariant` *(`styles/design-tokens/components/user-type.ts:22`)* · 🔴 **`UserRole`** *(`subapps/geo-canvas/systems/polygon-system/types/polygon-system.types.ts:18`)* — **το ίδιο όνομα με το legacy λεξιλόγιο του Π-2** *(`admin`/`authenticated`/`public`)*, με **εντελώς άλλη σημασία**. **+5 ανώνυμες ενσωματωμένες επαναλήψεις** της ίδιας ένωσης *(`error-tracker-types.ts:21` · `error-tracker-integrations.ts:61,62` · `GeoCanvasContent.tsx:83` · `GeoCanvasPanels.tsx:129`)* **+8 σημεία χρόνου εκτέλεσης** *(`UserTypeContext.tsx:122` **πίνακας κυριολεκτικών** που επαναλαμβάνει την ένωση· `GeoCanvasContent.tsx:75`· `polygon-config.ts:315,316,320,321,325,326`)*. 🔴 **Η πηγή είναι `localStorage`, αυτο-δηλωμένη, μηδέν έλεγχος** — το **§4** την απαγορεύει ρητά ως πηγή ταυτότητας. ⚠️ **ΔΕΝ διορθώνεται εδώ**· κλείνει μαζί με τον φρουρό του **ADR-798** | `auth.types.ts`, `polygon-system.types.ts`, ADR-798 §3.1 |
| Π-20 | 🔴 **`ProjectRole` = ΔΥΟ εξαγόμενοι τύποι με το ΙΔΙΟ όνομα και ΑΛΛΟ νόημα** *(μετρημένο 2026-08-24)*: `lib/auth/types.ts:33` = **9** τιμές, απαντά *«τι **δικαιούμαι** σε αυτό το έργο»*· `types/entity-associations.ts:52` = **7** τιμές, απαντά *«ποιος **ΕΙΝΑΙ** ο αρχιτέκτονας του έργου»* *(ADR-283)*. Είναι οι γραμμές **2** και **4** του §3 — **δεν** το είχε δει κανείς ως **ομωνυμία**. Ο καταναλωτής `config/project-role-labels.ts:26` εισάγει το **δεύτερο**· ο `config/profession-bridge.config.ts:22` επίσης. **Δικαίωμα και ιδιότητα με ένα όνομα**: ένα `import { ProjectRole }` από λάθος μονοπάτι μεταγλωττίζεται μια χαρά και σημαίνει το αντίθετο. ⚠️ **ΔΕΝ διορθώνεται εδώ** — μετονομασία αγγίζει και τα δύο δέντρα· είναι το σχήμα που έκλεισε το **CHECK 3.59** για το `Point3D` | `lib/auth/types.ts`, `entity-associations.ts`, §3 |
| Π-21 | ✅ **ΕΚΛΕΙΣΕ 2026-08-24 (ADR-798 Φάση 1)** — 🔴 **13 σχόλια κώδικα σε 11 αρχεία απέδιδαν το ESCO σε ADR-034, που είναι ΤΡΙΠΛΟ και ΚΑΝΕΝΑ του αρχείο δεν αναφέρει τη λέξη ESCO** *(0·0·0, μετρημένο)*. ⚠️ **Η πρώτη καταγραφή έλεγε «τρία» — ήταν λάθος**: μέτρησε μόνο τα αρχεία που ήταν ήδη ανοιχτά. Η κλειστή σάρωση *(`ADR-034` **ΚΑΙ** `esco|isco|profession` στην ίδια γραμμή)* έδωσε **13**. **Έκτο «λάθος αριθμός σε έγγραφο»**, γραμμένο **μέσα** στην καταγραφή που το φαινόμενο καταγγέλλει — *«τα σημεία που ήδη κοιτούσα» δεν είναι μέτρηση*. Διορθώθηκαν **και τα 13**, εναπομείναντα **0**· οι **νόμιμες** αναφορές ADR-034 *(gantt · spatial/validation bounds · z-index)* **άθικτες**. Ο πραγματικός ιδιοκτήτης είναι το **ADR-132**, που δηλώνει το `esco-types.ts` στα Canonical Locations του. ⚠️ **Το λάθος διαδόθηκε**: το handoff **και** το εγκεκριμένο σχέδιο της Φάσης 0 το αντέγραψαν από το σχόλιο. Κλάση `collided-cross-home` του **CHECK 3.49** — *ο αριθμός δεν προσδιορίζει έγγραφο*, οπότε ο επόμενος ανοίγει **ένα από τρία λάθος αρχεία** και συμπεραίνει ότι η τεκμηρίωση λείπει. **Διόρθωση = τρία σχόλια**, στη Φάση 1 του ADR-798 | `esco-types.ts`, ADR-132, ADR-798 §3.2 |
...
| Ημερομηνία | Αλλαγή |
|---|---|
| 2026-09-22 | 🔗 **ΟΙ ΓΟΝΕΙΣ ΤΗΣ ΣΤΗΛΗΣ ΕΓΙΝΑΝ ΟΜΑΔΕΣ ΧΩΡΙΣ ΔΙΕΥΘΥΝΣΗ — η ταξινόμηση μιλά ΚΛΕΙΔΙ ΚΟΜΒΟΥ** *(ADR-871 §10.6, Υ13/Υ14)*. Ο γονιός-ομάδα αποδιδόταν **πάντα** ως κουμπί, άρα το href του ήταν ταυτότητα με ρούχα διεύθυνσης (Primer/Carbon/Atlassian: ομάδα = κουμπί χωρίς `href`). Κλειδί κόμβου = `href` για σύνδεσμο, `id` για ομάδα (`navNodeKey`, `src/config/navigation-node.ts`)· τα `id` είναι **κλειστή ένωση χωρίς `/`** (`OfficeGroupId`). **Ένα-προς-ένα, μηδέν αλλαγή ταξινόμησης**: `JOBS.clients.sidebar` `'/spaces','/sales','/crm'` → `'spaces','sales','crm'` · `JOBS.finance.sidebar` `'/accounting'` → `'accounting'` · `COMMON_SIDEBAR_ROUTES` → **`COMMON_SIDEBAR_NODES`** (`'/settings'` → `'settings'`) · `REPORT_SOURCES` `{kind:'route'}` → **`{kind:'node', key}`** · `REPORTS_PARENT_ROUTE` → **`REPORTS_GROUP_ID = 'reports'`** · `isRouteVisibleForJob` → **`isNodeVisibleForJob`** · `hiddenHrefs` → **`hiddenKeys`** · `hiddenSubItems` με κλειδί το `id` της ομάδας. `LEGAL_DOCUMENTS_STATUS`: `route/hasPage` → `groupId: 'legal'` + `deadRoute` — ο γονιός «Νομικά» **δεν ισχυρίζεται πια** ότι το `/legal-documents` είναι σελίδα· το κενό επιβολής των `legal:*` **μένει** ανοιχτό. **Νέος κανόνας** (Υ19): ορατή ομάδα που **άδειασε** από το φίλτρο πηγαίνει στα **κρυμμένα** (ένας κανόνας, `withGroupItems`) — ποτέ κενό κουμπί, ποτέ σιωπηλή απώλεια από τον δείκτη. **Αριθμοί**: «Οικονομικά» **9 κλάδοι αμετάβλητοι**· υπο-στοιχεία **13 → 12** επειδή το «Debug» (`environments: ['development']`, ανεπίβλητο μέχρι τώρα) λείπει πλέον από την παραγωγή — τρίτη φορά που ο ένας αριθμός κινείται και ο άλλος όχι. Μετάλλαξη «κλειδί δουλειάς σε ανύπαρκτο κόμβο» ⇒ κόκκινο (`office-navigation-integrity.test.ts` Κ4) |
| 2026-08-24 | 🔗 **ΤΟ `pickDefaultJob()` ΑΠΕΚΤΗΣΕ ΔΕΥΤΕΡΟ ΚΡΙΤΗ — ΤΟ ΔΗΛΩΜΕΝΟ ΕΠΑΓΓΕΛΜΑ** *(ADR-798 Φάση 3· ο κανόνας ισοβαθμίας του **Ε7.δ** αλλάζει, η **προεπιλογή** του **Ε7.δ′** ΟΧΙ)*. 🔴 **Το κενό ήταν στο ίδιο το Ε7.δ**: *«σε ισοβαθμία, η σταθερή σειρά του μητρώου»* σημαίνει ότι ένας **τοπογράφος** και ένας **δικηγόρος** με **ταυτόσημα** permissions έπαιρναν **ταυτόσημη** απάντηση, βγαλμένη από τη **σειρά του πίνακα** — δηλαδή από **τίποτα**. Το `pickDefaultJob(input, tiebreak?)` δέχεται πλέον **προαιρετικό** δεύτερο όρισμα ⇒ **κανένας υπάρχων καλών δεν αλλάζει** και η συμπεριφορά χωρίς αυτό είναι **γραμμή προς γραμμή** η προηγούμενη *(άγκυρα Ε-4)*. 🔒 **Ο ΧΡΥΣΟΣ ΚΑΝΟΝΑΣ ΤΟΥ §5 ΜΕΝΕΙ ΑΚΕΡΑΙΟΣ**: το επάγγελμα **ΣΠΑΕΙ ΙΣΟΒΑΘΜΙΑ, ΠΟΤΕ ΔΕΝ ΠΡΟΣΘΕΤΕΙ ΔΟΥΛΕΙΑ** — το `available` υπολογίζεται **πριν** και **χωρίς** αυτό, και το `jobs-access.ts` **δεν εισάγει** τον πίνακα συγγένειας *(αυτή η άγνοια **είναι** η εγγύηση)*. Αυτο-δηλωμένη ιδιότητα που διεύρυνε το σύνολο θα ήταν **κλιμάκωση προνομίων με ένα dropdown** — και το **NIST SP 800-63** το λέει ρητά: IAL1 ⇒ **εξατομίκευση**, εξουσιοδότηση ⇒ **επαληθευμένα** γνωρίσματα. ⚠️ Το **Σ-7** *(«δεν το κρύβω» ≠ «το προτείνω»)* **επιβιώνει**: επάγγελμα που δείχνει σε δουλειά χωρίς **μετρημένο** δικαίωμα οδηγεί πάλι σε **σιωπή** *(άγκυρα Ε-5)*. Το «ποια δουλειά υποδεικνύει ποιο επάγγελμα» ζει **εκτός** αυτού του ADR — **ADR-798 §12** · `config/isco-job-affinity.ts`. Άγκυρες: `job-suggestion.test.ts` **15 → 33**, **11/11 μεταλλάξεις κόκκινες**. |
| 2026-08-24 | 🔗 **ΤΟ §11.3 ΒΡΗΚΕ ΙΔΙΟΚΤΗΤΕΣ, ΤΟ §11.6 ΕΚΛΕΙΣΕ, ΤΟ §3 ΗΤΑΝ ΜΠΑΓΙΑΤΙΚΟ** *(ADR-798 Φάση 0, μηδέν κώδικας)*. **(α) §11.3**: τα επίπεδα **2-3-4** μετακόμισαν στο **ADR-787 §0.2** *(που τα έλυνε από τις 21/08 **χωρίς ποτέ να τα κληρονομήσει ονομαστικά** — μετρημένο: `748 §11`/`§11.3`/`§11.6`/`getActiveCompanyId` σε **3.030** γρ. ⇒ **0**)*· το επίπεδο **1 (Άνθρωπος)** πήγε στο **ADR-798**, και ο λόγος του διαχωρισμού είναι **νόμος μοντέλου** *(Party-Role: το επάγγελμα είναι **ιδιότητα**, η συμμετοχή **σχέση** — η σύζευξη αναπαράγει το λάθος του Slack)*, όχι εύρος. **(β) §11.6 ΕΚΛΕΙΣΕ**: υλοποιήθηκε — αλλά **με άλλο όνομα**. `getActiveCompanyId` *(αυτό που ζήτησε)* = **0** εμφανίσεις· `resolveEffectiveCompanyId` *(αυτό που γράφτηκε)* = **25**. **Δύο έγγραφα έλυσαν το ίδιο, χωριστά** — και ο μόνος λόγος που δεν έγινε **τρίτη** συνάρτηση είναι η πύλη **CHECK 3.58** *(ADR-787 §5.3)*: *ένα anchor χωρίς gate είναι σχόλιο* **(CHECK 3.36)**. **(γ) §3**: **τέσσερα → ΕΝΝΕΑ** λεξιλόγια — τα τέσσερα ήταν όσα βρήκε σάρωση που κοίταξε **μόνο** την εξουσιοδότηση· τα άλλα πέντε ζουν σε `types/contacts/` · onboarding · subapp. **Πέμπτο «μπαγιάτικος αριθμός σε έγγραφο»** μετά τα Π-5 · Π-16 · `ssot-discover` · `.jscpd-baseline` (N.12). **Οι αριθμοί 1-4 ΔΕΝ άλλαξαν** *(«never renumber»)*. **(δ) §12**: **Π-19** *(η τριάδα `citizen`/`professional`/`technical` σε **17** σημεία / **3 ονόματα**, με το `UserRole` του geo-canvas να **συγκρούεται σημασιολογικά** με το legacy του Π-2)* · **Π-20** *(`ProjectRole` = **ΔΥΟ** εξαγόμενοι τύποι, **ίδιο όνομα**, **δικαίωμα** έναντι **ιδιότητας** — οι γραμμές 2 και 4 του §3, που κανείς δεν είχε δει ως **ομωνυμία**)* · **Π-21** *(**13** σχόλια κώδικα σε **11** αρχεία απέδιδαν το ESCO σε **ADR-034**, που είναι **τριπλό** και **κανένα** του αρχείο δεν αναφέρει τη λέξη ESCO — ο ιδιοκτήτης είναι το **ADR-132**· το λάθος είχε ήδη διαδοθεί σε handoff και σχέδιο. ⚠️ **Η πρώτη καταγραφή έλεγε «τρία» και ήταν λάθος** — έκτο «λάθος αριθμός σε έγγραφο», γραμμένο μέσα στην καταγραφή που το καταγγέλλει· **ΕΚΛΕΙΣΕ την ίδια μέρα**, ADR-798 Φάση 1)*. |
| 2026-08-02 | **🟢 ΦΑΣΗ 3.5α ΥΛΟΠΟΙΗΜΕΝΗ — «ΠΡΟΤΑΣΗ, ΟΧΙ ΕΡΩΤΗΣΗ» (νέο §14.8).** **Έβδομη έρευνα** (νέο **§6.14**, 14 πηγές· ερώτημα: *«ρωτάς ή προτείνεις;»*). 🔴 **Η έρευνα ανέτρεψε ΔΥΟ κλειδωμένες αποφάσεις αυτού του ίδιου ADR — τρίτη φορά που η έρευνα ανατρέπει την πρόταση.** **(1) Ε7.γ′**: το Ε7.γ ζητούσε «μία προαιρετική **ερώτηση**»· μετρήθηκε ότι **2 στους 3 CAD δεν ρωτούν καθόλου** *(ArchiCAD: ο επιλογέας ζει **μέσα** στο «Start ArchiCAD»· C4D: **Standard** + μόνιμος επιλογέας)*, και ο **ένας** που ρωτά — το modal *«Hello There!»* του **Revit 2022** *(ειδικότητα + ρόλος, με review/accept/decline)* — έχει **αίτημα απενεργοποίησης στο Autodesk Ideas ΚΑΙ επίσημο άρθρο υποστήριξης «How to disable»**. **Ίδιο σχήμα με το §6.11.1**: όταν το προϊόν χρειάζεται άρθρο για να απενεργοποιήσει μια οθόνη, **η οθόνη είναι το σφάλμα**. Το **NN/g** δίνει τον μηχανισμό — *«Personalization is over-rated»*, **active user paradox**, **customization > personalization**, η Amazon πετυχαίνει επειδή απαιτεί *«no extra work on the users»*. **Και το αποφασιστικό (§6.14.6): ο Revit ρωτά επειδή ΔΕΝ ΞΕΡΕΙ** *(ένα προϊόν, όλοι οι κλάδοι)*· ο Νέστωρ **υπολογίζει** από **13 ρόλους με ρητά permissions** ⇒ ερώτηση για κάτι που ήδη ξέρεις **είναι** το active user paradox. **(2) Ε7.δ′**: το **Ε7.δ στηριζόταν σε ΚΟΜΜΕΝΗ ΠΑΡΑΘΕΣΗ** — η πλήρης πρόταση της Autodesk είναι *«…the default is as follows: **Autodesk Revit: All**; Revit Architecture: Architectural; Revit MEP: Mechanical»*, δηλαδή «appropriate» = **«ό,τι σου δίνει η άδειά σου»**, **όχι** «η δουλειά με τα περισσότερα δικαιώματα» ⇒ προεπιλογή **`JOB_ALL`**, που **ήδη ίσχυε**: το Ε7.δ ήταν λάθος **στην αιτιολόγηση**, όχι στο αποτέλεσμα. ⚠️ **Και οι πηγές του §6.11.4 («+30–50% activation») είναι appcues.com / saasui.design — εταιρείες που ΠΟΥΛΑΝΕ onboarding**· το ανεξάρτητο **NN/g** μέτρησε ότι όσοι **πέρασαν** το onboarding τα πήγαν **εξίσου καλά** με όσους το **προσπέρασαν**. **Νέο Υ-12**: ο Revit **ρωτά σε ένα μέρος και σε βάζει να διορθώσεις σε άλλο** *(`File → Options → User Interface → Configure`)* — αυτή **είναι** η αιτία του άρθρου· εμείς προτείνουμε **πάνω στο ίδιο το χειριστήριο** ⇒ ο χρήστης **μαθαίνει πού ζει η απόφαση την ώρα που την παίρνει**, **συν** *(α)* αριθμός από τις **ίδιες** συναρτήσεις με το πραγματικό φίλτρο ⇒ **δεν μπορεί να αποκλίνει**, *(β)* **μόνιμα** αναστρέψιμη αποδοχή *(ο Revit εφαρμόζει και σε αφήνει)*, *(γ)* **σιωπή σε `unknown`**. 🔑 **Το ελάττωμα που βρέθηκε ΓΡΑΦΟΝΤΑΣ ΤΟΝ ΕΛΕΓΧΟ**: το πρώτο anchor ήταν **υπό συνθήκη** *(`if (outcome !== null)`)* ⇒ θα ήταν **πράσινο χωρίς να ελέγχει τίποτα*** — και κοιτάζοντάς το φάνηκε ότι με **μηδέν** μετρημένα permissions το `pickDefaultJob` επιστρέφει δουλειά **από `unknown`**, δηλαδή θα προτείναμε **μαντεψιά** και θα κατέρριπτε το ίδιο το επιχείρημα «δεν ρωτάμε επειδή ξέρουμε». ⇒ **νέος κανόνας: «δεν το κρύβω» ≠ «το προτείνω»** *(Σ-7, ζεύγος ελέγχων ώστε να μη γίνει «σιωπή για πάντα»)*. **Νέα**: `src/config/job-suggestion.ts` *(SSoT, μηδέν React)* · `job-suggestion.test.ts` *(**15** anchors)* · `components/job/JobSuggestion.tsx`. **Τροπ.**: `useJobFilteredNavigation.ts` *(**εξήχθη το `buildJobMenus`** — **ένα** σημείο που χτίζει τα τρία δέντρα, δύο καταναλωτές· αλλιώς η πρόταση θα μετρούσε **άλλο** δέντρο από αυτό που βάφει η οθόνη)* · `JobSwitch.tsx` · `safe-storage.ts` · el/en `navigation.json` · παραγόμενα i18n types. **Επίσης**: το §10 σπάει σε **3.5α** *(έγινε)* και **3.5β** — και το 3.5β **δεν είναι «~3 αρχεία»**: μετρήθηκε ότι το `tenant.ts` έχει **hardcoded ΕΝΑΝ** `TENANT_COMPANY_ID` ⇒ «προσωπικός χώρος» = **δημιουργία tenant**, με προαπαιτούμενο το §11.6. **Και οι κενές οθόνες (Ε7.ε) μεταφέρθηκαν στο 3.5β**: μετρήθηκε ότι σήμερα **δεν έχουν αντικείμενο** — τα `COMMON_SIDEBAR_ROUTES` (6) + `COMMON_DASHBOARD_TILES` (3) εγγυώνται ότι **καμία** δουλειά δεν αδειάζει **κανένα** δοχείο. Πύλες: `src/config` **2.024** πράσινα · `jscpd:diff` καθαρό · i18n 64/64 + 56/56 + audit 0/0. 🔴 **ΑΝΕΠΑΛΗΘΕΥΤΟ ΖΩΝΤΑΝΑ** |
| 2026-08-02 | **🟢 ΦΑΣΗ 3.6 ΥΛΟΠΟΙΗΜΕΝΗ — ο δείκτης μέσα στο δοχείο, σε ΤΡΙΑ επίπεδα (νέο §14.7).** **Έκτη έρευνα** (νέο **§6.13**, 14 πηγές· ερώτημα: *«τι λες στον χρήστη για ό,τι έκρυψες»*). 🔴 **Το εύρημα που καθόρισε τη λύση ήταν ότι οι μεγάλοι έχουν ΟΛΟΙ το ίδιο κενό**: το `Reveal Hidden Elements` του **Revit** τεκμηριώνεται ρητά ως *«but not by **Filter**»*, και το δέντρο του **VS Code** σε κατάσταση *filter* δεν δίνει **καμία** ένδειξη για τους κόμβους που αφαίρεσε *(ανοιχτό ζήτημα στο δικό τους repo)*. Δηλαδή **το εργαλείο που υπάρχει ακριβώς για να μη χαθεί τίποτα σιωπηλά δεν καλύπτει τη μόνη περίπτωση όπου κάτι όντως χάνεται σιωπηλά** ⇒ νέο **Υ-11**: δεν χρειάστηκε να εφεύρουμε μηχανισμό, χρειάστηκε να **μην αντιγράψουμε το κενό τους**. **Νέα Ε14.ιγ** *(ο κανόνας θέσης)*: ο **θετικός** αριθμός *(«9 υπο-ενότητες» = πόσα **έχει**)* ζει στο **ΚΛΕΙΣΤΟ** δοχείο ως υπότιτλος χωρίς ρήμα· ο **αρνητικός** *(«+7 κρυμμένα»)* **ΜΟΝΟ μέσα στο ΑΝΟΙΧΤΟ**, ως γραμμή **με ρήμα και ενέργεια** — έτσι η σύγκρουση νοήματος λύνεται με **δομή**, όχι με χρώμα. **Τρία επίπεδα**: κεφαλίδα *(τι λείπει από την οθόνη — **ο αριθμός ΔΕΝ άλλαξε**, μόνο απέκτησε ανάλυση στο tooltip)* · **μέσα σε κάθε δοχείο** *(τι λείπει από **εδώ**)* · **«Αποκάλυψη κρυμμένων»** *(**πού ακριβώς** ήταν — υποβαθμισμένα στη θέση τους, **λειτουργικά**, με μόνιμη ένδειξη κατάστασης και έξοδο με ένα κλικ)*. 🔑 **Η δομική αλλαγή**: το φίλτρο **έπαψε να πετάει ό,τι κόβει** — το `JobFilterResult` μεταφέρει πλέον `hidden` + `hiddenSubItems`, και το `hiddenCount` **έγινε `hidden.length`** παραγόμενο σε **ΕΝΑ** σημείο· η αναλλοίωτη του Μ-7 έπαψε να είναι σύμβαση που τηρείς και έγινε **ιδιότητα που ισχύει** — το ελάττωμα των 17:13 *(δύο μονοπάτια, δύο μονάδες)* **δεν είναι πλέον εκφράσιμο**. **Νέα**: `src/config/jobs-visibility.ts` *(η **ορατότητα**, εξαγμένη — το `jobs-access.ts` ήταν **497/500** και το σύνορο ήταν **ήδη** χαραγμένο ως κεφαλίδα ενότητας μέσα του· **497 → 225** γρ.)* · `sidebar-hidden-row.tsx` · `sidebar-reveal-banner.tsx` · νέο module `jobs-visibility` στο `.ssot-registry.json` *(και τα patterns του `jobs-access` **περιορίστηκαν** — αλλιώς το CHECK 3.7 θα μπλόκαρε τη μετακόμιση των ονομάτων)*. **Νέα anchors Μ-8/Μ-9/Μ-10** *(18 tests, νέο αρχείο)*: τα κρυμμένα είναι **ανακτήσιμα** *(`visible ∪ hidden` = είσοδος)* · οι **δύο αριθμοί δεν ενώνονται ΠΟΤΕ** · το `hiddenHrefs` καλύπτει **και τα δύο** επίπεδα. 🔑 **Ο κώδικας επιβεβαίωσε ανεξάρτητα τον άνθρωπο**: `hiddenCount = 9` **και** `hiddenSubItemCount = **13**` — αναπαράγει **ακριβώς** το 22 των 17:13, σπασμένο στα δύο συστατικά του. **74 πράσινα** στα δύο αρχεία· `registry-golden` 102/102· `i18n-shell-slice` 64/64. ⚠️ Το **CHECK 3.28** έπιασε δίδυμο που **η ίδια αυτή αλλαγή** έσπρωξε πάνω από το κατώφλι *(το σώμα «εικονίδιο · τίτλος · σήμα», δύο φορές στο `sidebar-menu-item.tsx`)* ⇒ εξαγωγή σε `SidebarItemLabel`, **χωρίς** χαλάρωση κατωφλίου (N.18). ✅ **ΚΑΙ ΤΑ ΤΡΙΑ ΕΠΙΠΕΔΑ ΕΠΑΛΗΘΕΥΤΗΚΑΝ ΖΩΝΤΑΝΑ (21:48→22:10, νέο §14.7.7)** — tooltip **«9 κλάδοι · 13 υπο-στοιχεία»** · **`+7 κρυμμένα`** μέσα στις Αναφορές με **4+7=11** μετρημένο **με το μάτι** · **7** στοιχεία επέστρεψαν υποβαθμισμένα στο κύριο μενού *(= το «7+1+1» του §14.6.3)* · αλλαγή δουλειάς έσβησε την αποκάλυψη μόνη της. **Τέταρτη ανεξάρτητη σύμπτωση κώδικα–ανθρώπου.** 🔴 **Και η οθόνη βρήκε ΔΥΟ ελαττώματα — τέταρτη φορά**: *(1)* η επεξήγηση της μπάρας έσπαγε σε **4 σειρές** στριμώχνοντας το «Τέλος» ⇒ δύο σειρές + κοντύτερο κείμενο· *(2)* **λανθάνον**: το `SidebarContent` **κυλά**, άρα σε μακρύ μενού η μπάρα θα έβγαινε από το οπτικό πεδίο ⇒ **η κατάσταση θα γινόταν κρυφή**, ακριβώς αυτό που ο τρόπος ήρθε να αποτρέψει ⇒ **`sticky` με αδιαφανές φόντο**. ⚠️ **Το #2 κανένα test δεν θα το έπιανε ποτέ** — δεν είναι λογικό σφάλμα, είναι **παράλειψη μετάφρασης του προτύπου**: το Revit δεν βάζει «μια ένδειξη κάπου», βάζει ένδειξη **που δεν μπορείς να μη δεις**· αντιγράψαμε το *τι* και παραλίγο να χάσουμε το *γιατί*. ⚠️ Μένουν 4 μικρά αδοκίμαστα (§14.7.7). |
| 2026-08-02 | **🟢 ΦΑΣΗ 3 ΕΚΛΕΙΣΕ — ΤΡΙΤΟΣ ΓΥΡΟΣ ΣΤΗΝ ΟΘΟΝΗ (17:13, νέο §14.6.2).** Το sidebar ήρθε **ακριβώς** το αναμενόμενο *(Αρχική · Ακίνητα · Έργα · Διοικητική Σύνοψη · Λογιστικό \| Αρχεία · Νομικά Έγγραφα \| Ρυθμίσεις)* — το **Ε14.ια** δουλεύει. 🔴 **Το τελευταίο εύρημα ήταν μια ΕΡΩΤΗΣΗ, όχι ένα σφάλμα**: ο Γιώργος ρώτησε «είναι σωστό αυτό;» βλέποντας δείκτη **22** ενώ από το μενού έλειπαν **9**. Ο αριθμός ήταν **αριθμητικά αληθής και επικοινωνιακά λάθος** — γέννησε αμφιβολία αντί για πληροφορία, δηλαδή απέτυχε στον σκοπό του (Α-3). **Αιτία**: ασυνέπεια μέσα στην **ίδια** συνάρτηση — ο κανόνας **Ε14.ιβ** είχε γραφτεί ως σχόλιο και εφαρμοστεί **μόνο** στο μονοπάτι του κλάδου που φεύγει (`hiddenCount += 1`), ενώ το μονοπάτι του **ορατού** γονιού πρόσθετε ακόμη `subItems.length - keptSubItems.length` ⇒ τα 13 κλειστά υπο-στοιχεία *(7 στο `/reports`, 6 admin στο `/settings`)* φούσκωναν τον δείκτη. **Δύο απαντήσεις για μία ερώτηση — τρίτη φορά το ίδιο σχήμα σε αυτό το ADR** (μετά τα Ε14.θ και Ε14.ια). **Διόρθωση**: ο μετρητής μετρά **στοιχείο του ίδιου επιπέδου με την είσοδο** — κλάδος με 11 παιδιά που φεύγει μετρά **1**, ορατός γονιός που έχασε 7 παιδιά μετρά **0**. Το Ε14.ιβ ξαναγράφτηκε ως **συμμετρικό** και η αναλλοίωτη έγινε ελέγξιμη σε μία γραμμή. **Νέο anchor Μ-7** πάνω στη **ζωντανή** πλοήγηση: `hiddenCount === items.length - visible.length` για **κάθε** δουλειά και **κάθε** από τα τρία μενού · **συν** έλεγχος ότι το κλάδεμα των παιδιών **συμβαίνει** *(αλλιώς η ισότητα θα ήταν πράσινη και σε κώδικα που σταμάτησε να κλαδεύει — ακριβώς το ελάττωμα των 16:58)* · **συν** το **ένα** καρφωμένο **9**, το στιγμιότυπο του ανθρώπου. **255 πράσινα** στο `src/config` *(από 246· +9 tests)*. 🔑 **Η ανεξάρτητη επιβεβαίωση**: ο κώδικας υπολόγισε **9** και ο άνθρωπος μέτρησε **9** στην οθόνη, χωρίς να δει ο ένας τον άλλον. **Μετρήθηκαν και οι έξι δουλειές** *(§14.6.2: Σχέδιο 7 · Εργοτάξιο 8 · Πελάτες 6 · Οικονομικά 9 · Προμήθειες 9 · Διαχείριση 10)*, και ο παλιός πίνακας του §14.6.1 σημάνθηκε **ιστορικός** αντί να διαγραφεί. ⚠️ **Ανοιχτό**: τα 13 υπο-στοιχεία δεν μετριούνται πλέον **πουθενά** — υπερασπίσιμο *(κλειστό accordion δεν έχασε τίποτα οπτικά)*, αλλά πλήρης συμμόρφωση με το Α-3 θέλει **δικό του δείκτη μέσα σε κάθε ανοιγμένο γονιό** *(«−7 εδώ», NN/g)*. **Απόφαση Γιώργου (17:35)**: **να λυθεί όπως το λύνουν οι μεγάλοι** *(Revit · ArchiCAD · Cinema 4D · Figma)* — με έρευνα, χωρίς εκπτώσεις, και **καλύτερα** από αυτούς αν υπάρχει τρόπος. Μεταφέρεται σε **νέα συνεδρία** ως **Φάση 3.6**. ✅ **ΤΕΤΑΡΤΟΣ ΓΥΡΟΣ (17:30-17:31, νέο §14.6.3): ΚΑΙ ΟΙ ΕΞΙ ΘΕΣΕΙΣ ΤΟΥ ΔΙΑΚΟΠΤΗ ΖΩΝΤΑΝΑ** — Σχέδιο **7** · Εργοτάξιο **8** · Πελάτες **6** · Οικονομικά **9** · Προμήθειες **9** · Διαχείριση **10**, **έξι στις έξι ταυτόσημα** με τον υπολογισμό, χωρίς να δει ο ένας τον άλλον· επιβεβαιώθηκε οπτικά και η **σύνθεση** κάθε sidebar. 🔑 **Και τα ίδια στιγμιότυπα έδωσαν το κρίσιμο στοιχείο για το ανοιχτό ερώτημα**: τα πλακίδια **ήδη** δείχνουν ανά-δοχείο πλήθος *(«9 υπο-ενότητες», «11 υπο-ενότητες»)* — το πρότυπο υπάρχει, **αλλά** μετρά «πόσα **έχει**», όχι «πόσα **έχασε**»· ίδια οπτική θέση με δύο νοήματα είναι το σχήμα ελαττώματος που κυνηγήθηκε ήδη τρεις φορές εδώ. |
| 2026-08-02 | **🟢 ΦΑΣΗ 3 ΥΛΟΠΟΙΗΜΕΝΗ — ο ζωντανός υπολογισμός μπήκε σε κώδικα.** **Νέα**: `src/config/jobs-access.ts` *(καθαρή συνάρτηση, μηδέν I/O, πρότυπο `decideAssetPackAccess`)* · `hooks/useEffectivePermissions.ts` · `hooks/useJobFilteredNavigation.ts` · `contexts/ActiveJobContext.tsx` · `components/header/JobSwitch.tsx` · `components/job/HiddenItemsBadge.tsx`. **Τροποποιημένα**: `app-sidebar` *(φίλτρο μίας έκφρασης· ο inline υπολογισμός permissions **μετακόμισε** — ήταν ο μόνος του είδους του και η φάση πρόσθετε δύο καταναλωτές, N.0.2)* · `DashboardHome` *(τα 12 πλακίδια παύουν να είναι hardcoded)* · `app-header` · `ConditionalAppShell` · `navigation.ts` · `safe-storage.ts` *(`ACTIVE_JOB` — **ένα** κλειδί· Ε5.β)*. **32 anchor tests**, 231 πράσινα στο `src/config`, 102 registry-golden, 85 i18n· `jscpd:diff` καθαρό. Δύο νέα modules στο `.ssot-registry.json`. 🔴 **ΔΥΟ ΜΕΤΡΗΣΕΙΣ ΑΝΕΤΡΕΨΑΝ ΤΟΝ ΣΧΕΔΙΑΣΜΟ** *(πέμπτη και έκτη φορά συνολικά)*: **(1)** νέο **§2.9** — από τις **τρεις** πηγές δικαιωμάτων (Π-15) στον browser φτάνει **ΜΙΑ** (`claims-handler.ts:159-164`)· άρα fail-closed φίλτρο θα έκρυβε το «Σχέδιο» από τον `external_user` που είναι **μελετητής στο έργο** = άδεια οθόνη = το ελάττωμα του ACC ⇒ νέα **Ε5.ι**: η απόφαση είναι **τριών καταστάσεων** (`granted`/`unknown`/`none`) και **κρύβεται μόνο το `none`**· **(2)** νέο **§2.10** — ο `super_admin` έχει **κενή** λίστα permissions (`roles.ts:58`, όλα στο `isBypass`) ⇒ υπολογισμός χωρίς έλεγχο bypass θα έδινε στον ιδιοκτήτη **μόνο τη Διαχείριση**· anchor **Μ-1** το φυλάει. **Απόφαση Γιώργου**: ο **διακόπτης δουλειάς μετακινήθηκε από τη Φάση 4 στη Φάση 3** — χωρίς αυτόν η φάση είναι **ανεπαλήθευτη** (Ε4.ε)· δεν είναι πρόωρο κομμάτι της Φάσης 4, γιατί το **Ε6.β** έχει ήδη κλειδώσει ότι η δουλειά ζει **ΕΞΩ** από το μονοπάτι *οργανισμός › έργο*. Νέα παράπλευρα **Π-17** *(το legacy λεξιλόγιο ρόλων διαρρέει σε ζωντανούς ελέγχους — `globalRole === 'admin'` δεν υπάρχει στα `GLOBAL_ROLES`)* και **Π-18** *(`Job` ήδη πιασμένο στο `RenderQueueStore` του subapp)*. **⚠️ Δεν αγγίχτηκε κανένα permission, ρόλος ή Firestore rule** (§10). **✅ ΖΩΝΤΑΝΗ ΕΠΑΛΗΘΕΥΣΗ (16:39, νέο §14.6)**: διακόπτης + δείκτης ορατά· dashboard **8 κρυμμένα** ακριβώς όπως υπολογίζεται· το `/reports` παρέμεινε (Ε14.α). 🔴 **Και η οθόνη βρήκε ελάττωμα που τα 32 tests δεν έβλεπαν — δεύτερη φορά στο ίδιο ADR**: το `/obligations`, δηλωμένο **κοινό σε όλες**, εξαφανίστηκε επειδή ζει ως **παιδί** του αταξινόμητου `/legal-documents`. Δύο **γενικές** διορθώσεις, μηδέν ειδικές περιπτώσεις: **Ε14.θ** *(αταξινόμητο ⇒ **ορατό** — το κύριο φίλτρο ήταν ασυνεπές με το ήδη σωστό φίλτρο αναφορών)* και **Ε14.ι** *(το δοχείο δεν παρασύρει το περιεχόμενο· ό,τι φεύγει μετριέται)*. Συνέπεια: το `/legal-documents` **μένει ορατό** — είναι 404 **και** ανεπίβλητο (Π-13), άρα απόκρυψη θα έκρυβε **το σφάλμα** (Ε14.στ, OWASP A01). Ο ίδιος κανόνας ενοποιήθηκε και στα πλακίδια, αλλιώς η ίδια οθόνη θα έλεγε δύο διαφορετικά πράγματα για το ίδιο στοιχείο. 🔴 **ΚΑΙ ΔΕΥΤΕΡΟΣ ΓΥΡΟΣ ΣΤΗΝ ΟΘΟΝΗ (16:58, §14.6.1)**: dashboard **7 κρυμμένα** / **5** πλακίδια **ακριβώς** όπως προβλέφθηκε, «Υποχρεώσεις» επέστρεψαν, **refresh επιβιώνει** — αλλά τα «Χώροι · Πωλήσεις · CRM» **έμειναν** στα Οικονομικά: οι δύο νέοι κανόνες **αλληλοακυρώθηκαν**, γιατί κάθε γονιός με υπο-μενού σωζόταν από τα αταξινόμητα παιδιά του ⇒ το φίλτρο έγινε **διακοσμητικό**. Νέα **Ε14.ια** *(το παιδί χωρίς ετικέτα **κληρονομεί τον γονιό** — η ίδια αρχή με τις αναφορές, γενικευμένη)* και **Ε14.ιβ** *(ο δείκτης μετρά **ένα ανά κλάδο**, όχι τα κλειστά υπο-στοιχεία — αριθμός που δείχνει δεκάδες παύει να σημαίνει κάτι)*. **47 anchor tests**, 246 πράσινα στο `src/config`· προστέθηκε test που τρέχει στην **ΠΡΑΓΜΑΤΙΚΗ** πλοήγηση, όχι σε πλασματικά items — αυτό θα είχε πιάσει το ελάττωμα πριν φτάσει στην οθόνη. ⚠️ **Εκκρεμεί τρίτος γύρος** + οι άλλες πέντε δουλειές. |
| 2026-08-02 | **🟢 ΦΑΣΕΙΣ 1 & 2 ΥΛΟΠΟΙΗΜΕΝΕΣ — ο πρώτος κώδικας.** **Φάση 1**: `src/config/jobs-registry.ts` — οι **έξι** δουλειές ως καθαρά δεδομένα (ταυτότητα · i18n keys · permissions που τις γεννούν · ετικέτες ορατότητας · `REPORT_SOURCES`), 33 anchor tests. **Φάση 2**: `ribbon-tab-specialties.ts` (ανάθεση **ανά καρτέλα**, ολική) + διακόπτης στη γραμμή καρτελών + φίλτρο **μίας** έκφρασης στο `RibbonRoot`, 45 anchor tests· 72 suites / 857 υπάρχοντα ribbon tests **πράσινα**. Και τα δύο modules στο `.ssot-registry.json`. **Πέμπτη έρευνα** (νέο **§6.12**, 12 πηγές) για τις **δύο ανοιχτές αποφάσεις** — και **ανέτρεψε την πρότασή μου δύο φορές**: **(1)** `/reports` ⇒ **κληρονομιά από την πηγή**, όχι ετικέτες (η ετικέτα ήταν **αντιγραφή ταξινόμησης** = το ελάττωμα Υ-5 που το ίδιο το ADR απαγορεύει)· **(2)** `/legal-documents` ⇒ **(Γ) εκτός μητρώου**, γιατί η μέτρηση έδειξε ότι **δεν υπάρχει σελίδα** (404) και ότι τα 7 `legal:*` είναι **σχεδιασμένα με MFA αλλά ανεπίβλητα** — ετικέτα δουλειάς θα ήταν απόκρυψη χωρίς προστασία (OWASP A01). Νέα **Ε14.α–Ε14.η**. Νέα παράπλευρα **Π-13** (νομικό ανεπίβλητο) · **Π-14** (`reports:reports:view` σε 4 μόνο ρόλους) · **Π-15** (🔴 **οι πηγές δικαιωμάτων είναι ΤΡΕΙΣ, όχι δύο** — το Ε1.ζ αγνοεί τα `PermissionSets`, όπου ζει ΟΛΟ το νομικό· **έκτο** «το σωστό μοτίβο υπάρχει ήδη») · **Π-16** (**τα ribbon tabs είναι 15, όχι 16** — λάθος άθροιση σε 5 σημεία, το βρήκε το anchor test με το που έτρεξε· τα πλήθη ανά θέση 7/7/12/7/5 ήταν σωστά). **⚠️ Δεν αγγίχτηκε κανένα permission, κανένας ρόλος, κανένα Firestore rule** (§10). |
| 2026-08-02 | **Ε7.ια + Ε7.ιβ** — διευκρίνιση μετά από ερώτημα Γιώργου *(«θα υπάρξει νέα σελίδα που θα κατευθύνει τον χρήστη μετά το login;»)*. **ΟΧΙ**: μία προαιρετική ερώτηση **μία φορά στη ζωή** του λογαριασμού· κάθε επόμενη είσοδο **κατευθείαν** στην τελευταία ενεργή δουλειά· η αλλαγή **πάντα** από το μόνιμο χειριστήριο, χωρίς επανασύνδεση. Η **παλιά** διατύπωση του §10 *(«οθόνη επιλογής χώρου μετά το login»)* ήταν **παραπλανητική**. **Ε7.ιβ**: **ρόλος ≠ δουλειά ≠ ειδικότητα** — τρία πράγματα που η καθημερινή γλώσσα ενώνει σε ένα· ο ρόλος **προτείνει**, δεν κλειδώνει (Α-1/Α-5). Επίσης: η **Φάση 2 επεκτάθηκε** σε *φίλτρο **+ ορατός διακόπτης ειδικότητας*** — χωρίς αυτόν η δουλειά είναι **ανεπαλήθευτη** στην οθόνη (ο `super_admin` βλέπει και τα 16 tabs εξ ορισμού, Ε4.ε· και ο DXF viewer έχει **0** αναφορές auth, §2.4). |
...
### 14.3 Ribbon — 15 tabs: **ένας** χώρος, **έξι** θέσεις διακόπτη

Ο DXF viewer ανήκει **μόνο** στη δουλειά **«Σχέδιο»**. Στις **άλλες πέντε** δεν εμφανίζεται
καθόλου. Η κατανομή των **15** tabs στις θέσεις του διακόπτη ειδικότητας είναι **ήδη γραμμένη
στο §5.1** και δεν επαναλαμβάνεται εδώ. Περίληψη: **7 / 7 / 12 / 7 / 5 / 15** tabs ανά θέση
...
`compliance`+`export` ⇒ Διαχείριση.

> ### ✅ ΑΠΟΦΑΣΗ ΓΙΩΡΓΟΥ (2026-08-02): **ΚΛΗΡΟΝΟΜΙΑ ΑΠΟ ΤΗΝ ΠΗΓΗ** — ούτε (Α) ούτε (Β)
>
> Η **πέμπτη έρευνα** (§6.12) βρήκε ομόφωνο μοτίβο σε **τέσσερις** πλατφόρμες: η αναφορά
> **δεν ταξινομείται** — **κληρονομεί την ορατότητα της πηγής της**. Η αρχική πρόταση
> «ετικέτα ανά υπο-στοιχείο» **αποσύρθηκε**: ήταν **αντιγραφή ταξινόμησης**, δηλαδή το
> ελάττωμα που το **Ε5.γ/Υ-5** ήδη απαγορεύει *(αν το `/sales` αλλάξει δουλειά, η ετικέτα
> του `/reports/sales` μένει πίσω και αποκλίνει σιωπηλά)*.

| # | Κλειδωμένο |
|---|---|
| Ε14.α | Ο **γονιός** `/reports` μένει **ορατός σε όλες** τις δουλειές· δεν παίρνει ετικέτα ούτε permission |
| Ε14.β | Τα **10** υπο-στοιχεία δηλώνουν **πηγή** (`REPORT_SOURCES`) και κληρονομούν ζωντανά. Οκτώ από διαδρομή που ήδη ταξινομείται· δύο (`compliance`, `export`) από **δουλειά**, γιατί δεν έχουν διαδρομή-πηγή |
| Ε14.γ | 🔴 **Μετρημένος λόγος να ΜΗΝ φιλτράρεται ο γονιός**: το `reports:reports:view` το έχουν **μόνο** `project_manager`, `site_manager`, `accountant`, `viewer`. **ΔΕΝ** το έχουν `architect`, `engineer`, `sales_agent`, `company_admin` ⇒ φίλτρο στον γονιό θα έκρυβε τις αναφορές **από τον διαχειριστή της εταιρείας**. **Δεύτερο Π-7**, ίδιο σχήμα *(⇒ Π-14)* |
| Ε14.δ | Αν όλα φιλτραριστούν ⇒ **κενή κατάσταση κατά NN/g**, ποτέ εξαφανισμένο μενού (Α-3, Ε7.ε) |

**(2) Το `/legal-documents`** — η αρχική διατύπωση *(«ρητά αφαιρεμένο permission ⇒ ο
υπολογισμός δεν έχει τι να ρωτήσει»)* ήταν **ελλιπής**. Η μέτρηση της Φάσης 1 βρήκε **τέσσερα**:

| # | Μετρημένο 2026-08-02 |
|---|---|
| α | 🔴 **Η ΣΕΛΙΔΑ ΔΕΝ ΥΠΑΡΧΕΙ.** `find src/app -name page.tsx \| grep -i legal` → **κενό**· κανένα rewrite/redirect/middleware. Το στοιχείο μενού δίνει **404** — και φέρει σχόλιο *«Increased priority for visibility»*. Ζωντανό είναι **μόνο** το υπο-στοιχείο `/obligations` |
| β | Κανένας από τους **13** ρόλους δεν έχει κανένα από τα **7** `legal:*` — **εκ σχεδιασμού**: ζουν σε add-on πακέτα `legal_viewer`/`legal_manager` (`lib/auth/permission-sets.ts`), **και τα δύο `requiresMfaEnrolled: true`** |
| γ | **Η επιβολή όμως δεν υπάρχει πουθενά**: τα rules των `obligations` είναι tenant-scoped (`companyId`) **χωρίς διάκριση ρόλου**· το `/api/contracts/route.ts` καλεί `withAuth(...)` **χωρίς permission**. Ο server προστατεύει τον **οργανισμό**, όχι τον **ρόλο** |
| δ | Το «accessible to all users» ήταν **αναγκαστική συνέπεια** του (β): αφού κανένας *ρόλος* δεν έδινε το δικαίωμα, το μενού «φαινόταν χαλασμένο» και «διορθώθηκε» ανοίγοντάς το σε όλους. **Η διόρθωση ακύρωσε τον σχεδιασμό ασφαλείας** |

> ### ✅ ΑΠΟΦΑΣΗ ΓΙΩΡΓΟΥ (2026-08-02): **(Γ) — εκτός μητρώου**
>
> Το `/legal-documents` **δεν μπαίνει**: δεν είναι στοιχείο, είναι **σφάλμα**. Το μητρώο
> περιγράφει **ό,τι υπάρχει**. Στο μητρώο μπαίνει το **`/obligations`** ως **κοινό σε όλες**.

| # | Κλειδωμένο |
|---|---|
| Ε14.ε | Το `/legal-documents` **δεν** εμφανίζεται σε καμία δουλειά ούτε στα κοινά. Καταγράφεται στο `LEGAL_DOCUMENTS_STATUS` ως **ανοιχτό σφάλμα** — anchor test το επιβάλλει |
| Ε14.στ | 🔑 **Το φίλτρο UX ΔΕΝ χρησιμοποιείται ΠΟΤΕ ως υποκατάστατο ασφάλειας.** Ετικέτα «Πελάτες» θα έκρυβε το κουμπί ενώ ο εργάτης και ο προμηθευτής της ίδιας εταιρείας θα συνέχιζαν να **διαβάζουν τα συμβόλαια** — κατά λέξη το αντι-πρότυπο **OWASP A01:2021** *(«reliance on client-side access check enforcement»)*. Θα έκανε το πρόβλημα **αόρατο** |
| Ε14.ζ | Ο χρυσός κανόνας του §5 ισχύει **και ανάποδα**: όχι μόνο «το φίλτρο δεν δίνει πρόσβαση», αλλά και **«το φίλτρο δεν προσποιείται ότι την αφαιρεί»** |
| Ε14.η | Όταν επιβληθεί το `legal:documents:view`, η απόκρυψη έρχεται **δωρεάν** — πρότυπο **Procore**: permission «None» ⇒ το εργαλείο φεύγει **μόνο του** από το μενού. **Η απόκρυψη ΑΚΟΛΟΥΘΕΙ το δικαίωμα, δεν το προσποιείται** *(⇒ Π-13)* |
...
### 14.6 ✅ Η ΖΩΝΤΑΝΗ ΕΠΑΛΗΘΕΥΣΗ ΤΗΣ ΦΑΣΗΣ 3 *(Γιώργος, στιγμιότυπο 2026-08-02 16:39)*

> Και **πάλι** η οθόνη βρήκε κάτι που τα 32 tests δεν έβλεπαν. Δεύτερη φορά στο ίδιο ADR.

**Τι επιβεβαιώθηκε** με ενεργή δουλειά **«Οικονομικά»**:

| Μετρημένο στην οθόνη | Αναμενόμενο | |
|---|---|---|
| Διακόπτης «Οικονομικά» + δείκτης «17 κρυμμένα» στο header | ορατός, μόνιμος | ✅ |
| Tooltip: *«Τι δουλειά κάνεις τώρα. Δεν αλλάζει δικαιώματα — μόνο τι εμφανίζεται.»* | ο χρυσός κανόνας **στην οθόνη**, όχι μόνο στο ADR | ✅ |
| Dashboard: **8 κρυμμένα**, ορατά **3 + 1** πλακίδια | 8 πλακίδια → 3 *(Ακίνητα · Έργα κοινά + Λογιστικό)* ⇒ 5· 4 εργαλεία → 1 ⇒ 3· **5+3=8** | ✅ |
| «Διοικητική Σύνοψη» (`/reports`) **παραμένει** ορατή | **Ε14.α** — ο εγκάρσιος γονιός δεν φιλτράρεται ποτέ | ✅ |
| Sidebar: Αρχική · Ακίνητα · Έργα · Αναφορές · Λογιστικό · Αρχεία · Ρυθμίσεις | τα κοινά + το `/accounting` της δουλειάς | ✅ |

#### 🔴 ΤΟ ΕΛΑΤΤΩΜΑ ΠΟΥ ΒΡΗΚΕ Η ΟΘΟΝΗ: **το δοχείο παρέσυρε το περιεχόμενο**

Το **`/obligations`** — που το μητρώο δηλώνει **ΚΟΙΝΟ σε όλες** τις δουλειές — **εξαφανίστηκε**.
Αιτία: ζει **αποκλειστικά ως παιδί** του `/legal-documents` (`smart-navigation-factory.ts:605-617`),
που δεν ανήκει σε καμία δουλειά. Το φίλτρο έκοψε τον γονιό και πήρε μαζί το κοινό παιδί.

**Δύο διορθώσεις, και οι δύο γενικές — καμία ειδική περίπτωση:**

| # | Κλειδωμένο |
|---|---|
| **Ε14.θ** | 🔒 **ΑΤΑΞΙΝΟΜΗΤΟ ⇒ ΟΡΑΤΟ.** Διαδρομή ή πλακίδιο που **καμία** δουλειά δεν διεκδικεί **δεν κρύβεται**. «Δεν το ξέρω ⇒ κρύψ' το» είναι fail-closed σε φίλτρο **UX** — το ίδιο λάθος που απαγορεύει το **Ε5.ι**. Το `isReportSubItemVisibleForJob` το έκανε ήδη σωστά· το κύριο φίλτρο ήταν **ασυνεπές**. Η προστασία από «όλα αταξινόμητα ⇒ φίλτρο-φάντασμα» **δεν** είναι αυτός ο κανόνας — είναι το anchor **Μ-5** (πύλη Υ-4) |
| **Ε14.ι** | 🔒 **ΤΟ ΔΟΧΕΙΟ ΔΕΝ ΠΑΡΑΣΥΡΕΙ ΤΟ ΠΕΡΙΕΧΟΜΕΝΟ — αλλά ΜΟΝΟ ρητά κοινό παιδί το σώζει.** Γονιός που ανήκει αλλού κρατιέται **μόνο** αν έχει παιδί **ρητά κοινό/εγκάρσιο**, και τότε κρατά **μόνο αυτό** |
| **Ε14.ια** | 🔴 **ΤΟ ΠΑΙΔΙ ΧΩΡΙΣ ΔΙΚΗ ΤΟΥ ΕΤΙΚΕΤΑ ΔΕΝ ΕΙΝΑΙ «ΑΤΑΞΙΝΟΜΗΤΟ» — ΚΛΗΡΟΝΟΜΕΙ ΤΟΝ ΓΟΝΙΟ ΤΟΥ.** *(Δεύτερη ζωντανή μέτρηση, 16:58.)* Η πρώτη διατύπωση του Ε14.θ+Ε14.ι **αλληλοακυρώθηκε**: τα «Χώροι · Πωλήσεις · CRM» έμειναν όρθια στα Οικονομικά επειδή τα **4+5+11** παιδιά τους δεν έχουν ετικέτα ⇒ «ορατά» ⇒ ο γονιός σωζόταν ως δοχείο ⇒ **κάθε γονιός με υπο-μενού επιβίωνε πάντα** και το φίλτρο έγινε διακοσμητικό. Είναι η **ίδια** αρχή με τις αναφορές (Ε14.β), γενικευμένη: η **πηγή** ενός υπο-στοιχείου είναι ο **γονιός** του. Παιδί με **δική** του ετικέτα κρίνεται μόνο του *(τα `/admin/*` μέσα στο `/crm` και στο `/settings` — §14.1/11, §14.1/15)* |
| **Ε14.ιβ** | 🔒 **Ο ΔΕΙΚΤΗΣ ΜΕΤΡΑ ΕΝΑ ΑΝΑ ΚΛΑΔΟ — ΚΑΙ ΣΤΑ ΔΥΟ ΕΠΙΠΕΔΑ.** Ο χρήστης έχασε **ένα** στοιχείο από το μενού, όχι δεκατρία. Δείκτης που δείχνει δεκάδες παύει να σημαίνει κάτι (Α-3: ο αριθμός πρέπει να είναι **αναγνώσιμος**, όχι απλώς αληθής). ⚠️ **Διορθώθηκε 17:13** *(§14.6.2)*: ο κανόνας είχε γραφτεί εδώ αλλά εφαρμοζόταν **μόνο** στους κλάδους που φεύγουν· τα κλαδεμένα παιδιά **ΟΡΑΤΩΝ** γονέων προσθέτονταν στον ίδιο μετρητή ⇒ **22 αντί για 9**. Η μονάδα μέτρησης είναι **στοιχείο του ίδιου επιπέδου με την είσοδο**: ένας κλάδος με 11 παιδιά που φεύγει μετρά **1**· ένας ορατός γονιός που έχασε 7 παιδιά μετρά **0**, γιατί ο χρήστης τον βλέπει στη θέση του. Η αναλλοίωτη είναι ελέγξιμη σε μία γραμμή — `hiddenCount === items.length - visible.length` *(anchor **Μ-7**)* |

**Και η συνέπεια που το `/legal-documents` επιβάλλει**: μένει **ορατό**. Είναι **404** και
**ανεπίβλητο** (Π-13) — απόκρυψή του θα έκρυβε **το σφάλμα**, όχι τα δεδομένα, ενώ ο εργάτης και ο
προμηθευτής θα συνέχιζαν να διαβάζουν τα συμβόλαια από το API. Κατά λέξη το **Ε14.στ**.
🔑 **Η απόκρυψη ΑΚΟΛΟΥΘΕΙ το δικαίωμα** — όταν επιβληθεί το `legal:documents:view`, έρχεται δωρεάν.

#### 14.6.1 ✅ Δεύτερος γύρος *(στιγμιότυπο 16:58)* — δύο σωστά, ένα λάθος

| Μετρημένο | |
|---|---|
| Dashboard **«7 κρυμμένα»**, **5** πλακίδια (3 + 2 με τα «Νομικά») | ✅ **ακριβώς** το προβλεπόμενο |
| «Νομικά Έγγραφα» + «Υποχρεώσεις» επέστρεψαν στο sidebar | ✅ Ε14.θ/Ε14.ι δουλεύουν |
| **Η επιλογή επιβιώνει σε refresh** *(επιβεβαίωση Γιώργου)* | ✅ Ε5.β |
| 🔴 «Χώροι · Πωλήσεις · CRM» **έμειναν** στα Οικονομικά· δείκτης 17→**22** | ❌ ⇒ **Ε14.ια** |

**Οι αναμενόμενοι αριθμοί μετά τη διόρθωση** *(μετρημένοι στη ζωντανή πλοήγηση, όχι εικασία)*:

| Δουλειά | Κρυμμένα | Κύριο μενού |
|---|---|---|
| Σχέδιο | 22 | `/` · `/properties` · `/projects` · `/buildings` · `/reports` *(+ `/geo/canvas`, `/dxf/viewer`)* |
| Εργοτάξιο | 22 | + `/construction/portfolio` |
| Πελάτες | 19 | + `/contacts` · `/spaces` · `/sales` · `/crm` |
| **Οικονομικά** | **22** | `/` · `/properties` · `/projects` · `/reports` · `/accounting` |
| Προμήθειες | 24 | + `/procurement` |
| Διαχείριση | 17 | `/` · `/properties` · `/projects` · `/reports` |

> ⚠️ **ΟΙ ΑΡΙΘΜΟΙ ΑΥΤΟΥ ΤΟΥ ΠΙΝΑΚΑ ΕΙΝΑΙ ΙΣΤΟΡΙΚΟΙ** — μετρήθηκαν με τον **παλιό** μετρητή, που
> προσέθετε και τα κλειστά υπο-στοιχεία. Οι ισχύοντες είναι στο **§14.6.2**. Το **περιεχόμενο**
> της τρίτης στήλης παραμένει σωστό· μόνο η δεύτερη άλλαξε.

#### 14.6.2 ✅ Τρίτος γύρος *(στιγμιότυπο 17:13)* — το sidebar σωστό, ο **αριθμός** λάθος

Το sidebar ήρθε **ακριβώς** το αναμενόμενο: *Αρχική · Ακίνητα · Έργα · Διοικητική Σύνοψη ·
Λογιστικό* | *Αρχεία · Νομικά Έγγραφα* | *Ρυθμίσεις*. Τα «Χώροι · Πωλήσεις · CRM» έφυγαν — το
**Ε14.ια** δουλεύει.

🔴 **Και τότε ο Γιώργος ρώτησε «είναι σωστό αυτό;» για τον δείκτη: έλεγε 22, ενώ έλειπαν 9.**
Η ερώτηση **είναι** το εύρημα: ένας δείκτης που γεννά αμφιβολία αντί για πληροφορία έχει
αποτύχει στον σκοπό του, ακόμη κι όταν ο αριθμός του είναι αριθμητικά αληθής.

| Σύνθεση του 22 *(μετρημένη, δουλειά «Οικονομικά»)* | |
|---|---|
| Κλάδοι πρώτου επιπέδου που **λείπουν από το μενού** | **9** |
| Υπο-στοιχεία μέσα σε **ΟΡΑΤΟΥΣ** γονείς — 7 στο `/reports`, 6 admin στο `/settings` | 13 |

**Η αιτία ήταν ασυνέπεια στην ίδια συνάρτηση**: ο κανόνας «ένα ανά κλάδο» *(Ε14.ιβ)* είχε
γραφτεί ως σχόλιο **και εφαρμοστεί μόνο** στο μονοπάτι όπου ο κλάδος φεύγει ολόκληρος
(`hiddenCount += 1`), ενώ το μονοπάτι του ορατού γονιού έκανε ακόμη
`hiddenCount += subItems.length - keptSubItems.length`. Δύο απαντήσεις για **μία** ερώτηση —
το ίδιο σχήμα ελαττώματος με τα Ε14.θ/Ε14.ια, τρίτη φορά στο ίδιο ADR.

#### 14.6.3 ✅ ΤΕΤΑΡΤΟΣ ΓΥΡΟΣ *(17:30-17:31)* — **και οι ΕΞΙ θέσεις του διακόπτη, ζωντανά**

Ο Γιώργος πέρασε **όλες** τις θέσεις με hot-reload πάνω στη διόρθωση. **Καμία απόκλιση**:

| Δουλειά | Κώδικας *(κύριο + εργαλεία + ρυθμίσεις)* | **Οθόνη** | Dashboard |
|---|---|---|---|
| Σχέδιο | 7 *(7+0+0)* | **7** ✅ | 5 |
| Εργοτάξιο | 8 *(6+1+1)* | **8** ✅ | 7 |
| Πελάτες | 6 *(4+1+1)* | **6** ✅ | 4 |
| **Οικονομικά** | 9 *(7+1+1)* | **9** ✅ | 7 |
| Προμήθειες | 9 *(7+1+1)* | **9** ✅ | 8 |
| Διαχείριση | 10 *(8+1+1)* | **10** ✅ | 8 |

> 🔑 **Η ανεξάρτητη επιβεβαίωση**: ο υπολογισμός του κώδικα και η καταμέτρηση του **ανθρώπου**
> στην οθόνη συμφώνησαν **έξι στις έξι**, χωρίς να δει ο ένας τον άλλον. Αυτό είναι το κριτήριο
> του Α-3 — ο αριθμός πρέπει να είναι **επαληθεύσιμος με το μάτι**. Η αρχική ερώτηση
> *«είναι σωστό αυτό;»* δεν έχει πια νόημα: ο χρήστης μετρά και βρίσκει.

**Και το sidebar κάθε δουλειάς, ζωντανά** *(η τρίτη στήλη του §14.6.1 επιβεβαιώθηκε οπτικά)*:
Σχέδιο → *Ακίνητα · Έργα · Κτίρια · Διοικητική Σύνοψη* + **Geo-Canvas** & **DXF Viewer** στα
εργαλεία · Εργοτάξιο → **+ Portfolio Κατασκευής** · Πελάτες → *Επαφές · Χώροι · Πωλήσεις · CRM* ·
Προμήθειες → **+ Προμήθειες** · Διαχείριση → μόνο τα κοινά.

🔑 **ΕΥΡΗΜΑ ΓΙΑ ΤΟ ΑΝΟΙΧΤΟ ΕΡΩΤΗΜΑ, από τα ίδια στιγμιότυπα**: τα πλακίδια του dashboard
**δείχνουν ήδη** ανά-δοχείο πλήθος — «**9 υπο-ενότητες**» *(Λογιστικό)*, «**11 υπο-ενότητες**»
*(CRM)*, «**4 υπο-ενότητες**» *(Χώροι, Πωλήσεις)*. Άρα το πρότυπο «αριθμός **μέσα** στο δοχείο»
**υπάρχει ήδη στην εφαρμογή** και δεν θα ήταν νέα γλώσσα — αλλά προσοχή: εκεί μετρά **πόσα
έχει**, όχι **πόσα έχασε**. Δύο διαφορετικά νοήματα στην ίδια οπτική θέση είναι ακριβώς το
είδος ασυνέπειας που αυτό το ADR κυνηγά τρεις φορές. Το ερώτημα παραμένει ανοιχτό.

**Anchor Μ-7** *(νέο)*: η αναλλοίωτη γράφεται σε μία γραμμή και δεν σαπίζει —
`hiddenCount === items.length - visible.length` για **κάθε** δουλειά και **κάθε** από τα τρία
μενού, πάνω στη **ζωντανή** πλοήγηση· συν έναν έλεγχο ότι το κλάδεμα των παιδιών **συμβαίνει**
*(αλλιώς η ισότητα θα ήταν πράσινη και σε κώδικα που σταμάτησε να κλαδεύει — το ελάττωμα των
16:58)*, συν το **ένα** καρφωμένο νούμερο **9** που είναι το στιγμιότυπο του ανθρώπου.

~~⚠️ **Ανοιχτό — δεν αποφασίστηκε**: τα **13** υπο-στοιχεία δεν μετριούνται πλέον
**πουθενά**.~~ ✅ **ΑΠΑΝΤΗΘΗΚΕ ⇒ §14.7 (Φάση 3.6)** — με **τρία** επίπεδα, όχι ένα.

> ✅ **ΤΙΠΟΤΑ ΔΕΝ ΕΚΚΡΕΜΕΙ ΖΩΝΤΑΝΑ ΓΙΑ ΤΗ ΦΑΣΗ 3** — και οι έξι θέσεις πέρασαν (§14.6.3).

---

### 14.7 🟢 ΦΑΣΗ 3.6 — **Ο ΔΕΙΚΤΗΣ ΜΕΣΑ ΣΤΟ ΔΟΧΕΙΟ** *(υλοποιήθηκε 2026-08-02)*

**Εντολή Γιώργου (17:35)**: *«όπως το κάνουν οι μεγάλοι — Revit · ArchiCAD · Cinema 4D ·
Figma. Καμία έκπτωση. Αν δεν είσαι σίγουρος, έρευνα· και αν μπορείς, βρες κάτι πιο έξυπνο
από τους ίδιους.»* ⇒ **έκτη έρευνα, §6.13**.

#### 14.7.1 Το πρόβλημα, ακριβώς

Μετά τη διόρθωση του §14.6.2 ο δείκτης μετρά **μόνο κλάδους πρώτου επιπέδου** *(9 στα
«Οικονομικά»)*. Τα **13** υπο-στοιχεία μέσα σε **ΟΡΑΤΟΥΣ** γονείς *(7 στο `/reports`, 6 admin
στο `/settings`)* έπαψαν να αναφέρονται **οπουδήποτε**. Υπερασπίσιμο — ένα **κλειστό**
accordion δεν «έχασε» τίποτα οπτικά — αλλά το **Α-3** *(ποτέ σιωπηλή απόκρυψη)* δεν
ικανοποιείται πλήρως.

> 🔑 **Ο κώδικας επιβεβαίωσε ανεξάρτητα την ανθρώπινη μέτρηση**: το anchor **Μ-9** επιστρέφει
> `hiddenCount = 9` **και** `hiddenSubItemCount = 13` — δηλαδή αναπαράγει **ακριβώς** το 22 που
> ο Γιώργος είδε στην οθόνη στις 17:13, σπασμένο στα δύο συστατικά του. Δεν είναι εκτίμηση.

#### 14.7.2 🔒 Ε14.ιγ — Ο ΚΑΝΟΝΑΣ ΘΕΣΗΣ *(η σύγκρουση νοήματος, λυμένη πριν γραφτεί κώδικας)*

Τα πλακίδια του dashboard δείχνουν **ήδη** αριθμό μέσα σε δοχείο: «9 υπο-ενότητες»,
«11 υπο-ενότητες». Εκείνος μετρά **πόσα έχει**· ο νέος **πόσα λείπουν**. Ίδια οπτική θέση με
δύο νοήματα = **ακριβώς** το σχήμα ελαττώματος που πιάστηκε τρεις φορές εδώ.

> 🔒 **Ε14.ιγ** — Ο **θετικός** αριθμός *(«πόσα έχει»)* ζει στο **ΚΛΕΙΣΤΟ** δοχείο, ως
> **υπότιτλος, χωρίς ρήμα**. Ο **αρνητικός** *(«πόσα λείπουν»)* ζει **ΜΟΝΟ μέσα στο ΑΝΟΙΧΤΟ**
> δοχείο, ως **γραμμή με ρήμα και ενέργεια**. Δεν συνυπάρχουν ποτέ στο ίδιο καρέ, και ο
> δεύτερος **πάντα κάνει κάτι** — άρα δεν διαβάζεται ποτέ ως πλήθος περιεχομένου.
>
> ⚠️ Η διάκριση **δεν** αφήνεται στο χρώμα. Χρώμα είναι σύμβαση που ξεχνιέται· **θέση + ρήμα**
> είναι δομή.

#### 14.7.3 Τα τρία επίπεδα

| # | Τι απαντά | Πού ζει | Πηγή δεδομένου |
|---|---|---|---|
| **1** | «Τι λείπει από **την οθόνη μου**» | Δείκτης κεφαλίδας *(υπάρχει από τη Φάση 3)*. **Ο αριθμός ΔΕΝ άλλαξε** — άλλαξε μόνο το tooltip, που πλέον δίνει την **ανάλυση** *«9 κλάδοι · 13 υπο-στοιχεία»* | `hiddenCount` |
| **2** | «Τι λείπει από **εδώ**» | Τελευταία γραμμή του **ανοιγμένου** υπο-μενού: **`+7 κρυμμένα · Εμφάνιση`**. Κουμπί, όχι ετικέτα *(Υ-2)* | `hiddenSubItems.get(href)` |
| **3** | «**Πού ακριβώς** ήταν» | **«Αποκάλυψη κρυμμένων»**: τα δέντρα επιστρέφουν **αφιλτράριστα**, τα κρυμμένα **υποβαθμισμένα στη θέση τους** *(Α-2)* και **λειτουργικά**, με **μόνιμη ένδειξη κατάστασης** + έξοδο με ένα κλικ | `hiddenHrefs` |

**Γιατί το Επίπεδο 3 και όχι μόνο τα 1+2**: ένας αριθμός λέει **πόσα**· η θέση λέει **τι**. Το
`/reports` που έχασε 7 αναφορές και το `/settings` που έχασε 6 διαχειριστικά είναι **δύο
τελείως διαφορετικές** πληροφορίες, και κανένα άθροισμα δεν τις διακρίνει. Το Revit το έλυσε
με τον ίδιο τρόπο πριν από δεκαετίες — απλώς **δεν το επέκτεινε στο φιλτράρισμα** (Υ-11).

#### 14.7.4 Ο κώδικας — **τι άλλαξε δομικά**

🔑 **Το φίλτρο έπαψε να πετάει ό,τι κόβει.** Μέχρι τη Φάση 3 κρατούσε **μόνο έναν αριθμό**·
τώρα το `JobFilterResult` μεταφέρει `hidden` *(οι κλάδοι που έφυγαν, **με τα παιδιά τους
άθικτα**)* και `hiddenSubItems` *(τα κλαδεμένα παιδιά ορατών γονέων, με κλειδί τον γονιό)*.

> 🔒 **Και το `hiddenCount` έγινε `hidden.length`, παραγόμενο σε ΕΝΑ σημείο (`buildResult`).**
> Το ελάττωμα των 17:13 ήταν **δύο μονοπάτια που αύξαναν τον ίδιο μετρητή με διαφορετική
> μονάδα**. Τώρα η απόκλιση **δεν είναι εκφράσιμη**: η αναλλοίωτη του Μ-7 έπαψε να είναι
> σύμβαση που τηρείς και έγινε ιδιότητα που **ισχύει**.

| Αρχείο | Τι |
|---|---|
| `src/config/jobs-visibility.ts` | **ΝΕΟ** — η **ορατότητα**, εξαγμένη από το `jobs-access.ts` *(497/500 γρ.)*. Το σύνορο ήταν **ήδη** χαραγμένο ως κεφαλίδα ενότητας μέσα στο αρχείο· η εξαγωγή το έκανε δομικό. Εξάρτηση **μονόδρομη**: ορατότητα → απόφαση → μητρώο. Νέο `summarizeHidden()` |
| `src/config/jobs-access.ts` | **497 → 225 γρ.** Κρατά **μόνο** την απόφαση *(τρεις καταστάσεις, bypass, `pickDefaultJob`)* |
| `contexts/ActiveJobContext.tsx` | `isRevealingHidden` — **δεν αποθηκεύεται** *(τρόπος λειτουργίας, όχι προτίμηση)*· σβήνει σε αλλαγή δουλειάς· ψεύδεται ποτέ σε `JOB_ALL` |
| `hooks/useJobFilteredNavigation.ts` | Επιστρέφει `hiddenSubItemCount` + `reveal` *(ένα αντικείμενο για τα επίπεδα 2 & 3)* |
| `components/sidebar/sidebar-hidden-row.tsx` · `sidebar-reveal-banner.tsx` | **ΝΕΑ** — ξεχωριστά γιατί τα χρειάζονται **δύο** μονοπάτια render το καθένα |
| `sidebar-menu-item.tsx` · `sidebar-menu-section.tsx` · `app-sidebar.tsx` | Υποβάθμιση + γραμμή δείκτη σε **ΚΑΙ ΤΑ ΤΡΙΑ** μονοπάτια *(ανοιχτό · popover μαζεμένου · απλό στοιχείο)* |
| `job/HiddenItemsBadge.tsx` · `header/JobSwitch.tsx` | Η ανάλυση στο tooltip. 🔒 **Ο αριθμός του σήματος ΔΕΝ αλλάζει** |
| `.ssot-registry.json` | Νέο module `jobs-visibility`· τα patterns του `jobs-access` **περιορίστηκαν** στην απόφαση — αλλιώς το CHECK 3.7 θα μπλόκαρε τη μετακόμιση |

#### 14.7.5 Τα anchors — **Μ-8 · Μ-9 · Μ-10** *(νέο αρχείο `jobs-visibility.test.ts`, 18 tests)*

| # | Η αναλλοίωτη | Γιατί |
|---|---|---|
| **Μ-8** | `visible ∪ hidden` = η είσοδος, **χωρίς επικάλυψη και χωρίς απώλεια**, για κάθε δουλειά × κάθε μενού | Το Επίπεδο 3 είναι αδύνατο αν τα κρυμμένα δεν είναι **ανακτήσιμα**. Δεν αρκεί να ξέρεις πόσα — πρέπει να τα βάλεις **πίσω** |
| **Μ-9** | 🔴 `hiddenCount === 9` **ΚΑΙ** `hiddenSubItemCount === 13` — **χωριστά** | Το ελάττωμα των 17:13 ήταν η **ένωσή** τους (22). Η Φάση 3.6 φέρνει τον δεύτερο αριθμό πίσω στην οθόνη ⇒ φέρνει και τον πειρασμό. **Το Μ-9 κοκκινίζει την ημέρα που θα συμβεί** |
| **Μ-10** | Το `hiddenHrefs` περιέχει **ΚΑΙ** κλάδους **ΚΑΙ** υπο-στοιχεία· ο ορατός γονιός **δεν** μπαίνει | Αν κάλυπτε μόνο το ένα επίπεδο, η «Αποκάλυψη» θα έδειχνε κάποια στοιχεία κανονικά — δηλαδή θα **έλεγε ψέματα** για το τι έλειπε |

**Σύνολο**: **74 πράσινα** στα δύο αρχεία *(56 + 18)*. `jscpd:diff` καθαρό · `registry-golden`
102/102 · `i18n-shell-slice` 64/64.

> ⚠️ **Το CHECK 3.28 έπιασε δίδυμο που η ΙΔΙΑ αυτή αλλαγή έσπρωξε πάνω από το κατώφλι**: το σώμα
> «εικονίδιο · τίτλος · σήμα» αποδιδόταν **δύο φορές** μέσα στο `sidebar-menu-item.tsx` και ήταν
> οριακά κάτω από τα 50 tokens. Εξήχθη σε `SidebarItemLabel`. **Δεν χαλάρωσε κατώφλι** (N.18) —
> αλλιώς η επόμενη αλλαγή στην ετικέτα θα εφαρμοζόταν στο ένα από τα δύο, σιωπηλά.

#### 14.7.6 ⚠️ ΤΙ **ΔΕΝ** ΕΠΑΛΗΘΕΥΤΗΚΕ ΑΚΟΜΗ

🔴 **Τίποτα από τα τρία επίπεδα δεν έχει δει οθόνη.** Αυτό το ADR έχει καταγράψει **τρεις**
φορές ότι πράσινα tests δεν αποδεικνύουν συμπεριφορά *(16:39 · 16:58 · 17:13)*, και **και τις
τρεις** επειδή τα tests δοκίμαζαν στημένα δέντρα ή —όπως εδώ— **καθαρές συναρτήσεις**, όχι την
πραγματική απόδοση. Τα Μ-8…Μ-10 χαρακτηρίζουν **το υλικό**· δεν λένε τίποτα για το αν η γραμμή
εμφανίζεται, αν το popover του μαζεμένου sidebar τη δείχνει, ή αν η υποβάθμιση διαβάζεται.

**Ζητούμενη ζωντανή επαλήθευση** *(δουλειά «Οικονομικά»)* — ✅ **ΕΓΙΝΕ, βλ. §14.7.7**:
1. Άνοιγμα του **«Διοικητική Σύνοψη»** ⇒ αναμένεται **`+7 κρυμμένα · Εμφάνιση`** στο τέλος.
2. Άνοιγμα των **«Ρυθμίσεις»** ⇒ αναμένεται **`+6 κρυμμένα`**.
3. Κλικ στο «Εμφάνιση» ⇒ **μόνιμη ένδειξη** πάνω από το μενού· τα 9 κρυμμένα **επιστρέφουν
   στη θέση τους** υποβαθμισμένα· «Τέλος» τα ξανακρύβει.
4. Αλλαγή δουλειάς **ενώ** η αποκάλυψη είναι ανοιχτή ⇒ πρέπει να **σβήσει μόνη της**.
5. Δείκτης κεφαλίδας: **παραμένει 9** και στις δύο καταστάσεις· tooltip **«9 κλάδοι · 13
   υπο-στοιχεία»**.

#### 14.7.7 ✅ Η ΖΩΝΤΑΝΗ ΕΠΑΛΗΘΕΥΣΗ ΤΗΣ ΦΑΣΗΣ 3.6 *(Γιώργος, στιγμιότυπα 21:48 → 22:10)*

| Μετρημένο στην οθόνη | Αναμενόμενο | |
|---|---|---|
| Δείκτης κεφαλίδας **«9 κρυμμένα»**, δουλειά «Οικονομικά» | αμετάβλητος από τη Φάση 3 | ✅ |
| Tooltip: **«Εμφάνιση όλων» / «9 κλάδοι · 13 υπο-στοιχεία»** | η ανάλυση του §14.6.2, **στην οθόνη για πρώτη φορά** | ✅ |
| «Διοικητική Σύνοψη» ανοιχτή ⇒ **`👁 +7 κρυμμένα   Εμφάνιση`** στο τέλος | **Επίπεδο 2** | ✅ |
| «Όλες οι δουλειές» ⇒ **11** υπο-αναφορές· «Οικονομικά» ⇒ **4** ορατά **+7** | **4 + 7 = 11** *(η αναλλοίωτη Μ-8, μετρημένη με το μάτι)* | ✅ |
| «Όλες οι δουλειές» ⇒ **κανένας** δείκτης πουθενά | `JOB_ALL` ⇒ τίποτα κρυμμένο· «0 κρυμμένα» θα ήταν θόρυβος | ✅ |
| Κλικ «Εμφάνιση» ⇒ μπάρα **«Αποκάλυψη κρυμμένων»** + **7** στοιχεία επέστρεψαν **υποβαθμισμένα στη θέση τους** *(Επαφές · Κτίρια · Portfolio Κατασκευής · Χώροι · Προμήθειες · Πωλήσεις · CRM)* | **Επίπεδο 3**· το §14.6.3 αναλύει το **9** ως **7**+1+1 ⇒ **7** στο κύριο μενού | ✅ |
| …και οι **7** υπο-αναφορές επέστρεψαν κι αυτές, υποβαθμισμένες, **μέσα** στο δοχείο | η αποκάλυψη πιάνει **και τα δύο** επίπεδα *(anchor Μ-10)* | ✅ |
| Το `+7 κρυμμένα` **εξαφανίστηκε** κατά την αποκάλυψη | σχεδιασμένο: τα στοιχεία είναι πλέον ορατά, ο δείκτης θα ήταν αντιφατικός | ✅ |
| Δείκτης κεφαλίδας **παρέμεινε 9** κατά την αποκάλυψη | συμπεριφορά Revit: η αποκάλυψη **δείχνει**, δεν ξε-κρύβει | ✅ |
| Αλλαγή δουλειάς **με ανοιχτή** την αποκάλυψη ⇒ έσβησε **μόνη της** | Ε14.ιγ / διπλό δίχτυ | ✅ |

> 🔑 **ΤΕΤΑΡΤΗ ανεξάρτητη σύμπτωση κώδικα–ανθρώπου σε αυτό το ADR.** Ο κώδικας υπολόγισε
> `hiddenSubItemCount = 13` **πριν** το δει άνθρωπος· ο άνθρωπος μέτρησε **7** μέσα στις
> Αναφορές και **7** στο κύριο μενού. Και οι τρεις αριθμοί προβλέφθηκαν από τα anchors.

##### 🔴 ΚΑΙ Η ΟΘΟΝΗ ΒΡΗΚΕ **ΔΥΟ** ΕΛΑΤΤΩΜΑΤΑ — τέταρτη φορά σε αυτό το ADR

| # | Ελάττωμα | Διόρθωση |
|---|---|---|
| **1** *(ορατό, 21:56)* | Η επεξήγηση της μπάρας ήταν **στην ίδια γραμμή** με το κουμπί εξόδου· σε πλάτος sidebar έσπασε σε **τέσσερις** σειρές και **στρίμωξε το «Τέλος»** | **Δύο σειρές**: *τίτλος + «Τέλος»* πάνω, επεξήγηση κάτω· και το κείμενο κόπηκε σε «Στη θέση τους, υποβαθμισμένα.» |
| **2** *(λανθάνον — δεν είχε χτυπήσει ακόμη)* | Το `SidebarContent` **κυλά**. Σε μακρύ μενού η μπάρα θα έβγαινε από το οπτικό πεδίο ⇒ **η κατάσταση θα γινόταν κρυφή** — ακριβώς αυτό που ο τρόπος ήρθε να αποτρέψει, και ο λόγος που το Revit βάζει το περίγραμμα **μόνιμα** γύρω από **όλη** την περιοχή | **`sticky top-0`** με **αδιαφανές** `bg-sidebar` *(διαφανές φόντο ⇒ το περιεχόμενο που κυλά φαίνεται από πίσω και η ένδειξη γίνεται δυσανάγνωστη **ακριβώς όταν χρειάζεται**)* |

> ⚠️ Το **#2** είναι το πιο διδακτικό: **κανένα** test δεν θα το έπιανε ποτέ, γιατί δεν είναι
> λογικό σφάλμα — είναι **παράλειψη μετάφρασης του προτύπου**. Το Revit δεν βάζει «μια ένδειξη
> κάπου»· βάζει ένδειξη **που δεν μπορείς να μη δεις**. Αντιγράψαμε το *τι* και παραλίγο να
> χάσουμε το *γιατί*.

##### ⚠️ Τι **δεν** δοκιμάστηκε ακόμη *(μικρά, μη αποκλειστικά)*

1. **«Ρυθμίσεις» ⇒ `+6 κρυμμένα`** — το δεύτερο δοχείο του §14.6.2 δεν ανοίχτηκε ποτέ.
2. **Κλικ «Τέλος»** — η έξοδος επαληθεύτηκε **έμμεσα** *(μέσω αλλαγής δουλειάς)*, όχι με το ίδιο
   το κουμπί.
3. **Το `sticky`** — η μπάρα δεν δοκιμάστηκε με **κυλισμένο** μενού· η διόρθωση #2 είναι
   **αθέατη** μέχρι να συμβεί αυτό.
4. Η αλλαγή δουλειάς έγινε προς **«Όλες»**, όπου ισχύει **και** ο δεύτερος φρουρός
   *(`activeJob !== JOB_ALL`)*. Αλλαγή **δουλειά → δουλειά** *(π.χ. Οικονομικά → Σχέδιο)* θα
   απομόνωνε τον πρώτο φρουρό *(`setActiveJob` ⇒ `setRevealingHidden(false)`)*.

### 14.8 🟢 ΦΑΣΗ 3.5α — **Η ΠΡΟΤΑΣΗ, ΟΧΙ Η ΕΡΩΤΗΣΗ** *(υλοποιήθηκε 2026-08-02)*

> 📘 Πλήρες σκεπτικό: **§6.14** *(έβδομη έρευνα)* · **Ε7.γ′** · **Ε7.δ′** · **Υ-12**.

#### 14.8.1 Τι άλλαξε σε σχέση με το αρχικό Ε7.γ

| Το Ε7.γ ζητούσε | Τι υλοποιήθηκε | Γιατί |
|---|---|---|
| «μία προαιρετική **ερώτηση**» | **πρόταση** που δεν ρωτά τίποτα | 2/3 CAD δεν ρωτούν· ο ένας που ρωτά έχει άρθρο «How to disable» (§6.14.2) |
| «μετά το login» | **ποτέ μόνη της** — ανοίγει **μόνο με κλικ** | *active user paradox* (NN/g): αυτόματο άνοιγμα = «Hello There!» με άλλο ρούχο |
| «προτείνει αρχική δουλειά» | προτείνει **και δείχνει το μετρημένο αποτέλεσμα πριν** | πρότυπο Revit: *«recommended UI changes… for your review before you accept or decline»* |
| *(τίποτα)* | **σιωπή σε `unknown`** | «δεν το κρύβω» ≠ «το προτείνω» — αλλιώς ο τυφλός υπολογισμός του Π-15 προτείνει την πρώτη κατά `JOB_ORDER` |

#### 14.8.2 Οι **πέντε** συνθήκες *(όλες, αλλιώς σιωπή)* — `job-suggestion.ts`

1. Δεν έχει απορριφθεί *(Α-3: μία άρνηση αρκεί)*.
2. `activeJob === JOB_ALL` — **δεύτερη γνώμη σε παρμένη απόφαση είναι ενόχληση**.
3. **≥2** διαθέσιμες δουλειές *(Ε7.ια: «μία δουλειά ⇒ δεν υπάρχει καν επιλογή»)*.
4. Η προτεινόμενη είναι **`granted`**, ποτέ `unknown` *(Ε7.γ′)*.
5. 🔑 **Το φίλτρο κρύβει ≥1 στοιχείο** — μια πρόταση που δεν αλλάζει τίποτα **είναι** ο θόρυβος
   για τον οποίο ζητούν να κλείσουν το «Hello There!». Απαντιέται **μόνο** με πραγματικό
   φιλτράρισμα, γι' αυτό μπαίνει τελευταία.

#### 14.8.3 🔒 Γιατί ο αριθμός δεν μπορεί να αποκλίνει

Το «Χ από Υ» βγαίνει από **`filterItemsByJob` + `summarizeHidden`** — τις **ίδιες** συναρτήσεις που
τρέχει το `useJobFilteredNavigation` **αφού** δεχτεί ο χρήστης — πάνω στα **ίδια** δέντρα
*(`buildJobMenus`, **ένα** σημείο, δύο καταναλωτές)*. Ο έλεγχος **Σ-5** το επιβάλλει: ξανατρέχει το
πραγματικό φίλτρο και απαιτεί **ίδιο** αριθμό. Δεύτερος μετρητής θα ήταν το ελάττωμα των 17:13.

#### 14.8.4 Αρχεία · πύλες

`config/job-suggestion.ts` *(ΝΕΟ, SSoT, μηδέν React)* · `config/__tests__/job-suggestion.test.ts`
*(ΝΕΟ, **15** anchors Σ-1…Σ-7)* · `components/job/JobSuggestion.tsx` *(ΝΕΟ)* ·
`hooks/useJobFilteredNavigation.ts` *(εξήχθη το **`buildJobMenus`** + `useJobSuggestion`)* ·
`components/header/JobSwitch.tsx` · `lib/storage/safe-storage.ts` · el/en `navigation.json` ·
παραγόμενα i18n types.

**Πύλες**: `src/config` **2.024** tests πράσινα · `jscpd:diff` καθαρό · `i18n-shell-slice` 64/64 ·
`i18n-types-freshness` 56/56 *(μετά από `generate:i18n-types`)* · `i18n:audit` **0/0**.

⚠️ **Γνωστό όριο, δηλωμένο**: η απόρριψη ζει σε **localStorage ανά uid** ⇒ **ανά συσκευή**.
Συνεπές, όχι παράλειψη — **όλο** το Ε5.β είναι σήμερα per-device (`ACTIVE_JOB`). Μετακίνηση σε
λογαριασμό = **Φάση 4**, μαζί με το υπόλοιπο Ε5.β.

🔴 **ΑΝΕΠΑΛΗΘΕΥΤΟ ΖΩΝΤΑΝΑ.** Πράσινα tests δεν είναι απόδειξη ότι δουλεύει στην οθόνη — η Φάση 3.6
το απέδειξε **δύο** φορές, με ελαττώματα που **καμία** δοκιμή δεν μπορούσε να πιάσει (§14.7.6).
