...
| 2026-05-04 | ✅ **Phase 5 COMPLETED (MVP)** — Framework Agreements surface at `/procurement/agreements`. Files: 23 (12 NEW + 11 MODIFY). Greenfield collection `framework_agreements/`. Architecture: (1) `FrameworkAgreement` entity = `{ id: 'fwa_*', companyId, agreementNumber (unique per company), title, vendorContactId (FK contacts/supplier), status (draft|active|expired|terminated), validFrom/Until, applicableProjectIds/MaterialIds/AtoeCategoryCodes (null=all), currency, totalCommitment, discountType (flat|volume_breakpoints), flatDiscountPercent OR volumeBreakpoints[] (sorted ASC), isDeleted, audit fields }`; (2) Server-side validation: agreementNumber length+uniqueness via `where(companyId,==).where(agreementNumber,==).where(isDeleted,==false)`, validity range (until>from), discount % bounds 0-100, breakpoint thresholds non-negative; (3) Soft-delete pattern with `EntityAuditService` audit trail; (4) `FrameworkAgreementNumberConflictError` → 409, `FrameworkAgreementValidationError` → 400; (5) UI: list grid (4 cols), search + status filter, create/edit dialog with conditional flat % vs `BreakpointsEditor` (extracted helper component), AlertDialog soft-delete confirmation. NEW files: types, service, 2 API routes, hook, 5 components (Card/List/Filters/FormDialog/BreakpointsEditor). MODIFY: enterprise-id (prefix `fwa` + generator), firestore-collections, domain-constants (ENTITY_TYPES.FRAMEWORK_AGREEMENT), audit-trail.AuditEntityType (+'framework_agreement'), firestore.rules (`/framework_agreements/{agreementId}` read tenant-scoped, writes Admin-only), firestore.indexes.json (3 composite: createdAt-desc, +status, +vendorContactId), coverage-manifest.ts (PENDING entry), 2× procurement.json (~50 keys `hub.frameworkAgreements.*` incl. status, discount.type, form), `FrameworkAgreementsCard` (placeholder → real `activeCount`), barrel export. Decisions applied: contacts SSoT preserved (vendor FK only); auto-apply on PO totals deferred to Phase 5.5 (future Cloud Function trigger). |
| 2026-05-04 | ✅ **S5-GAP-3 COMPLETED** — PurchaseOrdersCard + cross-project PO list page. Files: 5 (2 NEW + 3 MODIFY). (1) `procurement/purchase-orders/page.tsx` NEW — lista cross-project `usePurchaseOrders()` + `PurchaseOrderList` + `ProcurementSubNav`; `onViewPO(poId)` risolve `projectId` dalla PO in memoria → `getPoDetailUrl(po.projectId, po.id)`; `onCreateNew` → `/procurement/new`; `onDuplicate` → `getPoDetailUrl + ?duplicate=1`. (2) `PurchaseOrdersCard.tsx` NEW in `hub/cards/` — `Package`/indigo-600, stat: `allCount`, pattern identico a RfqCard/QuotesCard. (3) `ProcurementSubNav.tsx` MODIFY — aggiunto tab "Παραγγελίες" (`Package`, indigo-600) dopo Quotes, prima di Vendors. Tab order: Hub → RFQ → Quotes → Παραγγελίες → Vendors → Materials → Agreements → Analytics. (4) `HubLanding.tsx` MODIFY — importa `PurchaseOrdersCard`, grid `xl:grid-cols-3 → xl:grid-cols-4` (7 card = 4+3), `PurchaseOrdersCard` inserita dopo `QuotesCard`. (5) `el+en/procurement.json` MODIFY — 4 nuove keys `hub.purchaseOrders.{title,description,count,noOrders}`. Pattern: cross-project read-only list per management overview (§2.1 eccezione tollerata). |
| 2026-05-04 | ✅ **S5-GAP-2 COMPLETED** — RfqCard + QuotesCard aggiunte alla Hub landing. HubLanding ora 6 card (grid xl:grid-cols-3): RFQ (ClipboardList/teal, stat: active count) + Quotes (FileText/orange, stat: submitted+under_review count) + 4 master-data cards esistenti. i18n: 8 nuove keys `hub.rfq.*` + `hub.quotes.*` (el+en). Ordine card = ordine tab SubNav. |
| 2026-05-04 | ✅ **S5-GAP COMPLETED** — Navigation gap fixed. Files: 4 (2 NEW + 2 MODIFY). (1) `procurement/rfqs/page.tsx` NEW — lista cross-project `useRfqs()` + `RfqList` + `ProcurementSubNav`; bottone "+ Νέο RFQ" già built-in in `RfqList.onCreateRfq` (default → `/procurement/rfqs/new`). (2) `procurement/quotes/page.tsx` NEW — lista cross-project `useQuotes()` + `QuoteList` + `ProcurementSubNav`; `onCreateNew` → `/procurement/quotes/scan`. (3) `ProcurementSubNav.tsx` MODIFY — aggiunto tab "RFQ" (`ClipboardList`, teal-600) + tab "Προσφορές" (`FileText`, orange-600) dopo Hub, prima di Vendors. Tab order: Hub → RFQ → Quotes → Vendors → Materials → Agreements → Analytics. (4) `el+en/procurement.json` MODIFY — aggiunto `nav.rfqs: "RFQ"`. Pattern: SAP/Procore cross-project list con project-scoped detail pages. |
...
**MVP scope (shipped):** CRUD entity + reference-only registry. No auto-apply on PO totals.

- Greenfield collection `framework_agreements/`
- Schema: `agreementNumber` (unique per company), `title`, `vendorContactId`, `status` (`draft|active|expired|terminated`), `validFrom/Until`, `currency`, `totalCommitment`, `discountType` (`flat|volume_breakpoints`), `flatDiscountPercent` OR `volumeBreakpoints[]`, scope filters (`applicableProjectIds/MaterialIds/AtoeCategoryCodes` — null = all)
- Soft-delete + audit trail (consistent with PO/Material patterns)
- Surface: `/procurement/agreements` (replaces placeholder), Hub card shows active count
...
### Phase S5-GAP — Navigation Gap (✅ COMPLETED 2026-05-04)
