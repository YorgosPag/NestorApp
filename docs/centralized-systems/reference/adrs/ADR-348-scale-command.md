...
| 2026-07-17 | 1.7 | **Drift καταγράφηκε (N.0.1).** Αυτό το ADR σιωπούσε για το ότι το `ScaleEntityCommand` **extends `SnapshotTransformCommand`** (ADR-507 §8 — in-place spine: snapshot/restore/follower-cascade)· το ίδιο το αρχείο το επικαλείται στο docblock του από την εξαγωγή της base. |
| 2026-07-17 | 1.8 | **`copyMode` ΑΦΑΙΡΕΘΗΚΕ (ADR-507 §8).** Το scale-with-copy (v0.3 «Copy mode included in Phase 1») δεν είναι πια ctor flag: είναι `CloneWithTransformCommand` (Revit `ElementTransformUtils.CopyElements` — clone με το transform ψημένο μέσα), μέσω `createScaleCommand({copy: true})`. Το `ScaleEntityCommand` είναι πλέον **καθαρά in-place** (χωρίς `execute/undo/redo` overrides — κληρονομεί τη base)· το `computeUpdates` delegate-άρει στο `buildScalePatch` SSoT και το `ScaleParams` μετακόμισε στο `transform-patch-builders.ts` (re-export για back-compat). Διορθώθηκαν 2 bugs που έφερνε το flag: το copy δεν έκανε BIM identity/broadcast (clone εξαφανιζόταν στο Firestore) και το redo mint-αρε φρέσκο id (orphan doc) — το `MirrorEntityCommand` ήταν το μόνο από τα τρία που τα έκανε σωστά, γι' αυτό ΑΥΤΟ έγινε ο κοινός μηχανισμός. **UI intent αμετάβλητο:** `ScaleToolStore.copyMode` + ribbon/keyboard `C` toggle μένουν ως έχουν — απλώς καταλήγουν σε `{copy}` στο factory. Σημείωση: το scale **σκόπιμα** αγνοεί parametric BIM (`isScalableEntityType`, ADR-646 #3), οπότε scale-copy τοίχου κλωνοποιεί αμετασχημάτιστα — προϋπάρχον, εκτός scope. Βλ. **ADR-507 §8** (2026-07-17). |
| 2026-07-27 | 1.9 | 🔴 **Το `scaleDimension` κλιμάκωνε ΜΟΝΟ τα deprecated πεδία — οι διαστάσεις έμεναν πίσω ×1000 (ADR-716 Φ7, ζωντανά μετρημένο).** Το μοντέλο διάστασης μετανάστευσε στα canonical `defPoints` / `textMidpoint` / `measurementValue`, αλλά ο κλάδος scale εξακολουθούσε να μετασχηματίζει **μόνο** τα `@deprecated startPoint` / `endPoint` / `textPosition`. Οι builders (`linear-aligned-builder`, `radial-builder`, `ordinate-builder`, …) διαβάζουν τα **canonical**, άρα το canonical-mm pass (ADR-462, ×1000 για τοπογραφικό σε μέτρα) μετακινούσε τους καθρέφτες και **άφηνε πίσω τη σχεδιαζόμενη γεωμετρία**. **Μέτρηση στο `47_ergasia.dxf`: και στις 9 διαστάσεις ο λόγος `startPoint.x / defPoints[0].x` ήταν ΑΚΡΙΒΩΣ 1000** — σχεδιάζονταν 1000× μικρές, παρκαρισμένες κοντά στην αρχή των αξόνων, έξω από το φύλλο· και το `measurementValue` έλεγε `12` ενώ η γεωμετρία μετρούσε `12000` mm. **Αόρατο σε mm αρχεία** (mmFactor = 1) — γι' αυτό επιβίωσε: ενεργοποιείται μόνο στην κατηγορία που ξεκλείδωσε το ADR-716. **FIX:** ο κλάδος βγήκε σε **NEW `systems/scale/scale-dimension.ts`** (N.7.1 — το `scale-entity-transform.ts` ήταν στις 447/500 γρ.· EXTRACT, ποτέ trim) και καλύπτει πλέον `defPoints` (κάθε σημείο, όλες οι 11 variants), `textMidpoint`, `datum` (ordinate), `leaderLength` (radius/diameter), `manualBreaks` (DIMBREAK, και τα 4 σύνολα) και `measurementValue` — **με τον κανόνα «σημείο ή μήκος κλιμακώνεται, ΓΩΝΙΑ ποτέ»**: τα `angular2L`/`angular3P` μετρούν μοίρες και μένουν άθικτα· σε **μη-ομοιόμορφη** κλίμακα το μήκος δεν ανακτάται από έναν συντελεστή, οπότε η cache **πέφτει** (`undefined`) και οι builders το ξαναϋπολογίζουν από τα φρέσκα `defPoints` (ο DXF writer ήδη μεταχειρίζεται το κενό ως `0` = «ξαναϋπολόγισέ το»). Οι deprecated καθρέφτες ενημερώνονται μαζί ώστε να μην αποκλίνουν. **Tests:** NEW `__tests__/scale-entity-dimension.test.ts` — **42 τεστ, παραμετρικά σε ΚΑΘΕ variant** (το bug δεν αφορούσε ένα σχέδιο ή μία variant, αλλά **ποια πεδία γνωρίζει** ο κλάδος)· ελέγχουν το **merge** `{...e, ...scaleEntity(e,…)}` όπως ο πραγματικός caller, γιατί το γυμνό partial συγχέει το «αφέθηκε επίτηδες» με το «σβήστηκε». Πράσινα: 42 νέα · 1.230/1.230 σε 101 σουίτες dimension+scale · 151/151 στην πύλη ADR-716 · `jscpd:diff` καθαρό. **Ζωντανή επαλήθευση μετά τη διόρθωση:** λόγοι = **1**, όλα τα `defPoints` εντός σχεδίου, και τα 9 `measurementValue` **ταυτόσημα** με το μετρημένο μήκος. ✅ Google-level: YES — η διόρθωση έγινε στο SSoT, όχι στον καταναλωτή. |
