...
| 2026-07-16 | **De-duplication (N.18 / ADR-584 CHECK 3.28).** jscpd εντόπισε 94 διπλές γραμμές `ExtendToolStore` ↔ `TrimToolStore` + 3 clones `useExtendTool` ↔ `useTrimTool`. NEW `stores/createEdgeToolStore.ts` — SSoT factory για το κοινό edge-tool store shape (state machine, quick/standard mode, EDGEMODE/PROJECTMODE toggles, fence-drag preview, warning aggregation, closure registries)· delegate στο `createExternalStore` primitive (ίδιο δόγμα με `createToolBridgeStore`). NEW `hooks/tools/edge-tool-shared.ts` — `EDGE_TOOL_KEYWORDS` (τα 4 keyword sets ήταν identical), `resolveEdgePickTarget()` (κοινό pick preamble, positional args όπως το sibling `resolveCornerTarget`), `undoLastEdgeToolCommand()`. `ExtendToolStore.ts` 211→48 γραμμές (μόνο το spec του). **BREAKING (internal):** `boundaryEdgeIds`→`edgeIds` + `setBoundaryEdgeIds`→`setEdgeIds` — ενοποιημένο με το TRIM `cuttingEdgeIds`, όπως ήδη έκαναν και τα δύο hooks που το προωθούσαν downstream ως `selectedEdgeIds`. Τα 4 κοινά enums (`Phase`/`Mode`/`EdgeMode`/`ProjectMode`) ήταν byte-identical σε `extend-types.ts`/`trim-types.ts` → ένας ορισμός ως `EdgeTool*` στο factory. Συμπεριφορά αμετάβλητη· 74/74 tests GREEN· CHECK 3.28 diff καθαρό. tsc SKIP (N.17). |
