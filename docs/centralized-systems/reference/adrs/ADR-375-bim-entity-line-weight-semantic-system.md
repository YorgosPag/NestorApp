...
|--------|------|--------|----------|
| ✅ **Phase A DONE** · ✅ **Phase B.1 DONE** · ✅ **Phase B.2 DONE** · ✅ **Phase B.3 DONE** · ✅ **Phase B.4 DONE 2026-07-03 (Fit-to-paper AUTO drawing scale)** · 🩹 **Phase B.4 CEILING + persisted user-lock 2026-08-01 (ADR-739 §20.8 — ο αυτόματος υπολογισμός σιωπά όταν η έκταση είναι οικοπέδου· η επιλογή του χρήστη επιβιώνει reload)** · 🩹 **Phase B Runtime Wiring HOTFIX 2026-05-26** · ✅ **Phase C.1 DONE 2026-05-26** · ✅ **Phase C.2 DONE 2026-05-26** · ✅ **Phase C.4 DONE 2026-05-26** · 🩹 **Phase C.4 V/G Visibility Runtime Wiring HOTFIX v2.6 2026-05-26** · 🎨 **Phase C.4 V/G Color Picker UX swap v2.7 2026-05-26** · 🩹 **Phase C.4 V/G > Layer Priority Stack Fix v2.8 2026-05-26** · ✅ **Phase C.5 DONE 2026-05-26** · ✅ **Phase C.6 DONE 2026-05-26** · ✅ **Phase C.7 DONE 2026-05-26 (3D parity)** · ✅ **Phase C.8 DONE 2026-06-02 (Hide BIM / DXF-only toggle)** · 🎨 **Phase C.4 V/G floating palette v2.15 2026-06-02** · 🧹 **View tab compaction v2.17 2026-06-02 (7 BIM panels → 2 dense)** · 🪟 **Pen Table → FloatingPanel SSoT v2.18 2026-06-02** · 🎨 **Phase C.9 DONE 2026-06-08 (Revit-grade default line colors per category)** · 🔌 **Phase C.7 EDGE MODE 2026-06-12 (ADR-446: `attachEdgesProjection`/`buildEdgeOverlay` διαβάζουν τον EDGES axis — `none`=skip / `visible`=occlude(depthTest on) / `all`=x-ray(depthTest off)· `occlude?` option default true=back-compat)** | 2026-05-25 / 2026-06-12 | Giorgio Pagonis + Claude (Sonnet 4.6 / Opus 4.7 / Opus 4.8) | **Full Revit Clone — Enterprise — Unified SSoT with ADR-358** |
...
#### Phase B.4 — Fit-to-paper AUTO drawing scale ✅ DONE 2026-07-03

**Problem**: Ο default `drawingScale = 100` (1:100) έκανε τα annotation (κείμενα διαστάσεων, βέλη) να φαίνονται **δυσανάλογα μεγάλα** σε μικρά σχέδια. Το ύψος κειμένου = `DIMTXT(2.5mm) × drawingScale` → στα 1:100 = 250mm πραγματικό ύψος, τεράστιο για κάτοψη λίγων μέτρων. Ο χρήστης έπρεπε να μαντέψει χειροκίνητα το σωστό 1:N.

**Λύση (industry standard — AutoCAD/Revit plot scale)**: αυτόματη επιλογή τυποποιημένης κλίμακας ώστε το bounding box της σκηνής να χωράει σε φύλλο **A3** (fit-to-paper).

**Τύπος** (`systems/dimensions/auto-drawing-scale.ts`, pure SSoT):
```
bounds(mm) → w,h            (createCombinedBounds / scene.bounds — canonical mm, ADR-462)
A3 usable = 400 × 277 mm    (420×297 − 10mm περιθώριο)   [FIT_TO_PAPER_A3_USABLE_MM]
sceneLong = max(w,h), sceneShort = min(w,h)              ← orientation-agnostic
raw = max( sceneLong/400 , sceneShort/277 )
drawingScale = niceScaleAtLeast(raw)   → 1-2-5 ladder (…50,100,200,500,1000…), clamp[1,10000]
```

Παραδείγματα: 20×12m → **1:50** (κείμενο 125mm) · 5×3m → **1:20** (50mm) · 60×40m → **1:200** (500mm).

##### Files

| # | File | Type | Purpose |
|---|------|------|---------|
| 1 | `systems/dimensions/auto-drawing-scale.ts` | NEW | Pure SSoT: `computeFitToPaperScale(bounds, paper?)` + `niceScaleAtLeast(raw)` |
| 2 | `systems/dimensions/__tests__/auto-drawing-scale.test.ts` | NEW | 10 tests (snap ladder, A3/A4, orientation, degenerate) |
| 3 | `config/bim-render-settings-types.ts` | MODIFIED | `FIT_TO_PAPER_A3_USABLE_MM = {long:400, short:277}` |
| 4 | `state/bim-render-settings-store-types.ts` | MODIFIED | `drawingScaleUserSet` flag + `applyAutoDrawingScale` action |
| 5 | `state/bim-render-settings-store.ts` | MODIFIED | `applyAutoDrawingScale` impl· `setDrawingScale` sets user-lock |
| 6 | `hooks/canvas/useViewportAutoFit.ts` | MODIFIED | AUTO on initial-content / re-import fit (not on persisted restore) |
| 7 | `hooks/canvas/useFitToView.ts` | MODIFIED | `annotation-fit-to-paper` handler → FORCE re-fit from combined bounds |
| 8 | `systems/events/drawing-event-map.ts` | MODIFIED | `annotation-fit-to-paper` event (no payload) |
| 9 | `ui/ribbon/components/DrawingScaleWidget.tsx` | MODIFIED | «Αυτόματη προσαρμογή» item → emits the event |
| 10 | `i18n/locales/{el,en}/dxf-viewer-shell.json` | MODIFIED | `drawingScale.fitToPaper` + `fitToPaperAriaLabel` |

##### User-override semantics (Revit annotation-scale rule)

- `drawingScaleUserSet` = **PERSISTED ανά επίπεδο** (ADR-739 §20.8, 2026-08-01), `false` όταν απών.
  ⚠️ **Ήταν runtime session flag μέχρι τότε** — και αυτό ακύρωνε σιωπηλά ολόκληρο τον κανόνα
  παρακάτω: το `buildRaw` έγραφε την **τιμή** χωρίς το **κλείδωμα**, οπότε σε κάθε reload η σημαία
  γύριζε `false` και το επόμενο auto pass ήταν ελεύθερο να αντικαταστήσει μια χειροκίνητη 1:100 με
  μια bounds-derived 1:5000. Ο resolver χρησιμοποιεί **αυστηρό `=== true`** ⇒ προϋπάρχοντα έγγραφα
  (που δεν έχουν το πεδίο) μένουν σε AUTO ακριβώς όπως πριν.
- AUTO στο load (`applyAutoDrawingScale`) εφαρμόζει **μόνο** όσο `!drawingScaleUserSet` → δεν πατάει χειροκίνητη επιλογή.
- Χειροκίνητο `setDrawingScale` (widget input/preset) → κλειδώνει (`userSet=true`).
- Το κουμπί «Αυτόματη προσαρμογή» → `force:true` (override πάντα)· ΔΕΝ κλειδώνει (auto μένει live).

##### ⚠️ Ταβάνι στον ΑΥΤΟΜΑΤΟ δρόμο (ADR-739 §20.8, 2026-08-01)

Το `computeFitToPaperScale` **παραμένει αυτούσιο** — είναι η *ειλικρινής* απάντηση και τη χρησιμοποιεί
το **ρητό** κουμπί «Αυτόματη προσαρμογή». Δίπλα του μπήκε το `computeAutoDrawingScale`, που είναι ό,τι
επιτρέπεται να εφαρμοστεί **σιωπηλά**, και το καλεί **μόνο** το `useViewportAutoFit`:

- `MAX_AUTO_DRAWING_SCALE` = τελευταίο στοιχείο του `DRAWING_SCALE_PRESETS` (**500**) — **παράγωγο**,
  ώστε να μη ξεκολλήσει από το widget αν κάποιος επεκτείνει τη σκάλα.
- Πάνω από αυτό ⇒ `null` = «καμία γνώμη», το **ίδιο** συμβόλαιο που ήδη ίσχυε για εκφυλισμένη σκηνή.
- **Γιατί**: σε επίπεδο με τοπογραφικό 1,9 km το fit-to-A3 έδινε 1:5000, που είναι αριθμητικά σωστό
  και πρακτικά άχρηστο — κάθε σημείωση γεννιόταν σε μέγεθος οικοπέδου (πίνακας 120 sheet-mm = **600 m**
  μετρημένο ζωντανά· κείμενο 2,5 mm = **12,5 m**). Η μέτρηση της κατανομής έδειξε ότι το 90 % της
  γεωμετρίας ζει σε 228×106 m: το «χώρεσε τα ΠΑΝΤΑ σε ένα A3» ήταν το λάθος ερώτημα.

##### Reuse (κανένας νέος μηχανισμός)

- `createCombinedBounds` / `scene.bounds` — bounds SSoT (ADR-010/462, canonical mm).
- `useViewportAutoFit` — trigger SSoT (ADR-399: initial / re-import / navigation).
- `getBoundsDimensions` — Bounds → {w,h} SSoT.
- `DRAWING_SCALE_MIN/MAX` — shared clamp window.

##### Out of scope

- **Line-weight ασυμμετρία**: το πάχος γραμμών διάστασης είναι screen-fixed 1px (ByLayer sentinel, ADR-562) ενώ το κείμενο είναι world-scaled. Ξεχωριστό ζήτημα, δεν αγγίχτηκε εδώ.
- Auto-fit που να προσαρμόζει το ΙΔΙΟ το paper size — σταθερό A3 προς το παρόν.
...
|------|---------|--------|--------|
| 2026-08-01 | **2.26 — Φ.B.4 διόρθωση: ο αυτόματος υπολογισμός παύει να είναι σιωπηλή αυθεντία (ADR-739 §20.8)** | **Surface (Giorgio):** ο πίνακας του ADR-739 γεννιέται τεράστιος στο τοπογραφικό. **RECOGNITION (μετρημένο ζωντανά, code=SoT):** `drawingScale = 5000` διαβασμένο **από το ίδιο το widget**· πίνακας 120 sheet-mm μετρημένος **599,85 m** με τον δείκτη συντεταγμένων της εφαρμογής (πρόβλεψη 600 — συμφωνία 0,03 %· **όχι** 650 όπως αναφέρθηκε). Η αριθμητική είναι σωστή **παντού**· λάθος είναι το **ερώτημα**: το Φ.B.4 επέλεγε 1:N ώστε να χωρέσει η **ένωση κάθε οντότητας** σε ένα A3, ενώ η κατανομή είναι **διτροπική** (5–95 εκατοστημόριο = 228×106 m, πλήρη όρια 817×1915 m). Έρευνα AutoCAD/Revit/ArchiCAD/Vectorworks: **κανείς δεν παράγει την κλίμακα σημειώσεων από την έκταση περιεχομένου** — είναι πάντα επιλογή του χρήστη δεμένη σε δοχείο (view/layer/viewport). **Απόφαση Giorgio:** «να μαντεύει, αλλά ο χρήστης να μπορεί να την αλλάζει». **Λύση (2 αλλαγές, καμία στο μοντέλο):** (α) το `drawingScaleUserSet` γίνεται **persisted** ανά επίπεδο — ήταν session-only, που **ακύρωνε σιωπηλά** τον ίδιο τον κανόνα override του v2.22 σε κάθε reload (`buildRaw` έγραφε τιμή χωρίς κλείδωμα)· resolver με αυστηρό `=== true` ⇒ παλιά έγγραφα μένουν AUTO· (β) νέο `computeAutoDrawingScale` = ό,τι επιτρέπεται **σιωπηλά**, με ταβάνι `MAX_AUTO_DRAWING_SCALE` **παράγωγο** από το `DRAWING_SCALE_PRESETS` (500)· πάνω από αυτό ⇒ `null` = «καμία γνώμη» (**υπάρχον** συμβόλαιο ⇒ μηδέν αλλαγές σε καλούντες). Το `computeFitToPaperScale` **αυτούσιο** για το ρητό κουμπί. **Καλύπτει κάθε καταναλωτή του `paperHeightToModel`** (κείμενο 12,5 m → 25 cm, 4 διαδρομές διαστάσεων, scale-bar, βόρειο βέλος). **Files (4 MOD + 2 test MOD):** `systems/dimensions/auto-drawing-scale.ts`, `hooks/canvas/useViewportAutoFit.ts`, `config/bim-render-settings-types.ts`, `state/bim-render-settings-store{,-types}.ts`. **Tests:** 1580/1580 (101 suites) + 509/509 anchors + **4/4 μεταλλάξεις απομονωμένες** + jscpd καθαρό. **Browser-verified:** επίπεδο σε AUTO @1:100, φόρτωση χωρίς persisted viewport ⇒ widget **1:100**, έγγραφο Firestore **αμετάβλητο**, πίνακας **12,25 m** μετρημένος. **Out of scope (δηλωμένο):** επίπεδο που όντως μιξάρει τοπογραφικό + κτίριο θέλει φύλλα/viewports (ADR-739 §3 4ο backend)· υπάρχοντα επίπεδα με 1:5000 δεν αγγίζονται. | Claude (Opus 5) |
| 2026-07-27 | **2.25 — Phase C.7 fat-line: κοινό primitive + διόρθωση μονάδων HiDPI** | **Αφορμή:** ADR-650 M8β/Γ v2 — το 3Δ layer τοπογραφικού review χρειάστηκε fat line, δηλαδή **δεύτερος** χρήστης του `LineSegments2`+`LineMaterial`+resolution-subscription block. Χειρόγραφο αντίγραφο = sibling clone (N.18). **NEW `bim-3d/lines/scene-fat-line.ts`** (`createSceneFatLineMaterial` + `createFatLineMesh`) και ο `bim-3d-edge-overlay-builder` **μετατράπηκε** να το χρησιμοποιεί — χωρίς τη μετατροπή θα ήταν τρίτο αντίγραφο, όχι κεντρικοποίηση. Ο 2Δ αδελφός (`WebglLineLayerManager`, ADR-639 Στ.5) **μένει έξω τεκμηριωμένα**: δική του πηγή resolution ανά rAF tick + LOD instanceCount. **🐛 ΔΙΟΡΘΩΣΗ (ενεργή αλλαγή συμπεριφοράς σε HiDPI):** ο vertex shader κάνει `offset *= linewidth; offset /= resolution.y` ⇒ `linewidth` και `resolution` είναι στην **ΙΔΙΑ** μονάδα. Το v2.20 έδινε `resolution` σε **CSS px** και `linewidth` σε **device px** (`opts.lineWidthPx * dpr`) ⇒ **κάθε ακμή BIM ήταν `dpr×` πιο χοντρή σε οθόνη HiDPI** — αόρατο σε dpr 1 (όπου ρυθμίστηκε το v2.20), διπλάσιο σε dpr 2. Το `bimEdgeResolutionStore` δημοσιεύει πλέον **και** `pixelRatio` (από `bimPixelRatio()`, ο ίδιος clamp `min(dpr,2)` που παίρνει ο renderer) και το primitive παράγει **και τα δύο** από **ένα** snapshot. **Δεύτερο κενό:** το `applyDevicePixelRatioSync` (αλλαγή οθόνης χωρίς resize) δεν ενημέρωνε **καθόλου** το store, και επειδή το CSS μέγεθος δεν αλλάζει ο identity guard θα κατάπινε ένα σκέτο `setSize` → όλη η 3Δ όψη κρατούσε τα πάχη της **προηγούμενης** οθόνης. **Files (4 MOD + 1 NEW):** NEW `bim-3d/lines/scene-fat-line.ts` (+ test 10), MOD `bim-3d/edges/bim-3d-edge-overlay-builder.ts` (−dead `Line2` import), `bim-3d/edges/bim-edge-resolution-store.ts` (+`pixelRatio`), `bim-3d/scene/scene-manager-resize.ts` (2 writers). **Tests:** 50 suites / 419 πράσινα (edges + converters + section + topo)· `jscpd --diff` καθαρό. ΟΧΙ tsc (N.17). 🔴 browser-verify + commit (Giorgio). Πλήρες σκεπτικό: **ADR-650 v23**. | Claude (Opus 5) |
| 2026-07-11 | **2.24 — V/G «DXF Σχέδιο» master row (Revit «Imported Categories») + category filter field** | **Surface (Giorgio):** (α) στο panel «Ορατότητα & Γραφικά» εμφανίζονταν raw i18n keys (`ribbon.commands.objectStyles.categories.*`) για 6 κατηγορίες + το namespace `textFindReplace` δεν φορτωνόταν καθόλου· (β) «θέλω πεδίο φίλτρου να μην ψάχνω μέσα σε τόσες οντότητες»· (γ) «είναι σωστό να μπουν οι διάφορες οντότητες DXF στις κατηγορίες; τι θα έκαναν οι μεγάλοι;». **Απόφαση (Revit-grade, locked via AskUserQuestion):** ΟΧΙ per-entity-type rows (LINE/ARC/…) — οι μεγάλοι (Revit «Imported Categories», AutoCAD Layers) ελέγχουν το εισαγόμενο CAD ως **ΕΝΑ** entry + ανά **layer**, ποτέ ανά τύπο οντότητας. Άξονες: BIM=categories, DXF=layers. **Υλοποίηση:** (1) **missing keys fix** — 6 κατηγορίες (`mep-radiator/boiler/water-heater/underfloor`, `fuel`, `foundation`) + namespace `textFindReplace` registered (lazy-config/config/namespace-loaders). (2) **Filter field** — accent-insensitive (`normalizeForSearch` SSoT, ADR-217) live-filter των κατηγοριών + «καμία κατηγορία» μήνυμα. (3) **«DXF Σχέδιο» master row** — ΜΙΑ γραμμή V/G που ελέγχει ΟΛΕΣ τις raw DXF οντότητες (`resolveEntityBimCategory === null`): visibility on/off + ενιαίο projection-colour override + ενιαίο **lineweight (mm) override** (AutoCAD per-import Lineweight· 0=unset «—»)· per-view persist (debounced). Απόφαση weight-unit: mm (AutoCAD layer model) αντί pen-index (Revit Pen Table εξαρτάται από scale-column/dpi context που ο DxfRenderer δεν συγκεντρώνει) → reuse `lineweightToPx` SSoT· control = υπάρχον `BimLineweightSelect` + νέο `allowUnset` prop («—»=0). Renderer seams: `DxfRenderer.isEntityLayerSkipped` (visibility gate, κοινό στα 3 passes) + `DxfRenderer.resolveStyleForRender` (colour+weight override, τοπικά — print/WYSIWYG άθικτα, βλ. ADR-040 changelog 2026-07-11). **Files:** `bim-render-settings-types.ts` (+`DxfImportStyle`{visible,projectionColor,projectionLineweightMm}+default+raw/resolved/resolver), `bim-render-settings-store{,-types}.ts` (+`setDxfImportVisibility`/`setDxfImportColor`/`setDxfImportLineweight`+buildRaw+loadForLevel), `DxfRenderer.ts` (2 seams +`lineweightToPx` import), `BimStyleSelects.tsx` (`BimLineweightSelect` +`allowUnset`/`disabled`), `VisibilityGraphicsPanel.tsx` (filter + DXF row), i18n el+en, ADR-040 changelog, store test (+8). **Out of scope (fast-follow):** line-pattern override στη γραμμή DXF (σπάνιο για imports· visibility+colour+weight = πλήρες Revit/AutoCAD-grade override). ΟΧΙ tsc (N.17). 🔴 browser-verify (import DXF → toggle «DXF Σχέδιο» κρύβει μόνο DXF· colour+weight override· «—»=χωρίς override· filter· persist ανά όροφο) + commit (Giorgio). | Claude (Opus 4.8) |
| 2026-07-03 | **2.22 — Phase B.4: Fit-to-paper AUTO drawing scale** | **Surface (Giorgio):** τα κείμενα διαστάσεων εμφανίζονται δυσανάλογα μεγάλα σε σχέση με τις οντότητες DXF & την κλίμακα. **RECOGNITION (code=SoT):** το ύψος κειμένου (`dim-text-renderer.ts`) = `paperHeightToModel(DIMTXT=2.5mm, dimscale, sceneUnits) × view.scale`, όπου `dimscale` πέφτει στο `drawingScale` SSoT (default **1:100**) → 250mm πραγματικό ύψος, τεράστιο για μικρές κατόψεις. Το κείμενο είναι σωστά world-scaled (Revit/AutoCAD annotation scale) — το πρόβλημα ήταν ο σταθερός default 1:100 χωρίς auto-fit. **Λύση (locked με Giorgio via AskUserQuestion — Fit-to-paper + snap σε τυποποιημένη + A3 + auto-on-load ΚΑΙ κουμπί):** νέο pure SSoT `computeFitToPaperScale(bounds, paper=A3-usable-400×277)` που επιλέγει τυποποιημένη 1-2-5 κλίμακα ώστε το bounding box (canonical mm, ADR-462) να χωράει σε A3: `raw = max(long/400, short/277)` → `niceScaleAtLeast` (…50,100,200,500,1000…, clamp[1,10000]). Orientation-agnostic. Παραδείγματα: 20×12m→1:50 (κείμενο 125mm), 5×3m→1:20, 60×40m→1:200. **Wiring:** (α) `useViewportAutoFit` — AUTO στο initial-content/re-import fit (ΟΧΙ σε persisted-viewport restore)· (β) store `applyAutoDrawingScale(scale, {force?})` + runtime flag `drawingScaleUserSet` (Revit override rule: το χειροκίνητο `setDrawingScale` κλειδώνει το auto για τη session· το auto σέβεται το κλείδωμα)· (γ) κουμπί «Αυτόματη προσαρμογή» στο `DrawingScaleWidget` → `annotation-fit-to-paper` event → `useFitToView` handler → `force:true` re-fit από combined bounds. **Reuse (κανένας νέος μηχανισμός):** `createCombinedBounds`/`scene.bounds` (bounds SSoT), `useViewportAutoFit` (trigger SSoT ADR-399), `getBoundsDimensions`, `DRAWING_SCALE_MIN/MAX`. **Out of scope:** ασυμμετρία πάχους γραμμής (screen-fixed 1px ByLayer vs world-scaled text) — ξεχωριστό. **Files (2 NEW + 8 MOD):** NEW `systems/dimensions/auto-drawing-scale.ts` + test (10 tests)· MOD `config/bim-render-settings-types.ts` (FIT_TO_PAPER_A3_USABLE_MM), `state/bim-render-settings-store{,-types}.ts`, `hooks/canvas/useViewportAutoFit.ts`, `hooks/canvas/useFitToView.ts`, `systems/events/drawing-event-map.ts`, `ui/ribbon/components/DrawingScaleWidget.tsx`, i18n el+en. **Tests:** 27/27 PASS (10 νέα + annotation-scale + store regression). 🔴 browser-verify (import DXF → auto 1:N· widget «Αυτόματη προσαρμογή»· χειροκίνητο override δεν πατιέται σε re-import). | Claude (Opus 4.8) |
| 2026-06-13 | **2.21 — Server schema strip → INFINITE PATCH LOOP root-cause fix (settingsVersion + όλα τα top-level `BimRenderSettings` πεδία)** | **Surface (Giorgio):** πλημμύρα `PATCH /api/dxf-levels` στο terminal — ο ίδιος `levelId` με `_v` να ανεβαίνει ασταμάτητα (>42.000) σε ~800ms/write χωρίς ο χρήστης να αγγίζει τίποτα. **RECOGNITION (code=SoT + Firestore doc read):** το πραγματικό doc `dxf_viewer_levels/lvl_b997…` είχε `bimRenderSettings` = **ΑΚΡΙΒΩΣ** `{drawingScale, viewRange, objectStyles}` — **κανένα** `settingsVersion`, `visualStyle`, ή toggle. **Root cause (ίδια οικογένεια με v2.13):** το nested `BimRenderSettingsSchema` (dxf-levels.schemas.ts) είναι `z.object` **ΧΩΡΙΣ** `.passthrough()` → η Zod **σβήνει σιωπηλά** κάθε μη-δηλωμένο key. Το schema δήλωνε μόνο `drawingScale/viewRange/objectStyles` (το v2.13 διόρθωσε μόνο τα *nested* objectStyle πεδία), οπότε κάθε νέο ADR-field (settingsVersion ADR-445, visualStyle 446, disciplineVisibility 405, colorBySystem 408, showHeatLoad 422, showFinishSkin 449, cutPlaneActive 452) κοβόταν server-side. **Ο βρόχος:** client γράφει `settingsVersion:2` → server το κόβει → `loadForLevel` migration βλέπει un-versioned doc → `changed:true` → heal-save v2 → server κόβει → Firestore snapshot echo → `useBimRenderSettingsSync` → migrate → heal → **∞**. Κάθε heal bump-άρει `_v` + γράφει Firestore (production cost/quota). **Fix (1 αρχείο):** προσθήκη ΟΛΩΝ των top-level `BimRenderSettings` πεδίων στο `BimRenderSettingsSchema` (`settingsVersion` int· `disciplineVisibility` record(bool)· `colorBySystem` bool· `visualStyle` enum 8 presets· `realisticMaterials`/`showHeatLoad`/`showFinishSkin`/`cutPlaneActive` bool) + SSoT-contract σχόλιο «κάθε νέο ADR-field μπαίνει ΚΑΙ εδώ». Το «μολυσμένο» doc αυτοθεραπεύεται στο 1ο load μετά το fix (τελευταίο heal γράφει v2 → migrate `changed:false` → κλείδωμα· μηδέν backfill). **Files (2 MOD):** `dxf-levels.schemas.ts` (+10 πεδία +contract note), `dxf-levels.schemas.test.ts` (+4 regression tests: settingsVersion preserved, visualStyle preserved+invalid-rejected, all toggles round-trip). **Tests:** 17/17 PASS. ΕΚΤΟΣ ADR-040. 🔴 browser-verify (hard-refresh → ο καταιγισμός PATCH σταματά εντός ~1-2 writes) + commit (git add ΜΟΝΟ δικά μου). | Claude (Opus 4.8) |
| 2026-06-12 | **2.20 — Phase C.7 "Shaded with Edges" 3Δ ορατές ακμές (depth + draw-order + uniform colour) — BROWSER-VERIFIED** | **Surface (Giorgio):** στο 3Δ canvas οι όψεις εμφανίζονταν **flat-shaded χωρίς σκούρες ακμές** — έλειπε η «μολυβιά» (Revit «Shaded with Edges»). **RECOGNITION (code=SoT):** το edge overlay system (v2.5) χτίζει σωστά τα `LineSegments2` overlays (θετικό width, wired σε όλους τους converters incl. foundation). **Διάγνωση με browser (το αποφασιστικό βήμα):** προσωρινό κόκκινο/χοντρό overlay με `depthTest:false` → εμφανίστηκε παντού → overlays render OK· με `depthTest:true` → εξαφάνιση. Άρα το πρόβλημα ήταν αμιγώς **depth-buffer**, ΟΧΙ ύπαρξη/resolution (η «resolution (1,1)» αποκλείστηκε: θα έδινε γιγάντιες γραμμές, όχι απουσία). **Δύο αιτίες μαζί:** (1) **draw-order overdraw** — το overlay έχει `depthWrite:false` και κανένα `renderOrder` → οι όψεις ζωγραφίζονταν *μετά* και σκέπαζαν τις ακμές· (2) **depth rejection** — το screen-space expansion του `LineSegments2` σπρώχνει το βάθος της γραμμής ελάχιστα **πίσω** από τη δική της όψη → `depthTest:true` την απορρίπτει. **Fix (Revit-grade, FULL SSoT):** (α) `overlay.renderOrder = 1` (`EDGE_OVERLAY_RENDER_ORDER`) → ακμές ζωγραφίζονται ΜΕΤΑ τις όψεις (default 0)· (β) αρνητικό `polygonOffset` στο edge `LineMaterial` (`EDGE_POLYGON_OFFSET_FACTOR=-2`, `UNITS=-4`) → τραβά τις γραμμές ίσα-ίσα μπροστά ώστε να κερδίζουν τις δικές τους όψεις, κρατώντας `depthTest:true` (only-visible, χωρίς x-ray)· (γ) θετικό `polygonOffset` (1,1) στο face-material factory `MaterialCatalog3D.buildMat` (`FACE_POLYGON_OFFSET_*`, belt-and-suspenders, ΕΝΑ SSoT σημείο για όλα τα 3Δ materials)· (δ) **uniform silhouette colour** `BIM_3D_EDGE_COLOR='#1a1a1a'` στο `attachEdgesProjection` — η Revit «Shaded with Edges» χρησιμοποιεί ΕΝΑ ομοιόμορφο σκούρο χρώμα ακμών, ΟΧΙ τα per-category 2Δ projection colours (π.χ. v2.19 κολώνα μπλε-γκρι #5b6478)· width/pattern/visibility μένουν resolver-driven (pen table + V/G eye + element/layer overrides ενεργά στο 3Δ). **Files (3 MOD + 1 NEW test):** `bim-3d/edges/bim-3d-edge-overlay-builder.ts` (renderOrder + edge polygonOffset + consts/docstrings, λεπτό width `opts.lineWidthPx*dpr`, `depthTest:true`), `bim-3d/converters/bim-three-edges.ts` (uniform `BIM_3D_EDGE_COLOR`), `bim-3d/materials/MaterialCatalog3D.ts` (buildMat +polygonOffset), NEW `bim-3d/materials/__tests__/MaterialCatalog3D-shaded-edges.test.ts`· +renderOrder assertion (overlay-builder test) + updated 3Δ uniform-colour assertion (bim-three-edges test). **Tests:** 74/74 PASS (zero regression). **✅ BROWSER-VERIFIED (Giorgio):** λεπτές μαύρες διακριτικές ακμές, only-visible. 🔴 commit (Giorgio· shared tree· git add ΜΟΝΟ δικά μου). | Claude (Opus 4.8) |
| 2026-06-08 | **2.19 — Phase C.9: Revit-grade προκαθορισμένα ΧΡΩΜΑΤΑ γραμμής ανά κατηγορία** | **Surface (Giorgio):** «θέλω προκαθορισμένα χρώματα ανά BIM κατηγορία όπως οι μεγάλοι παίκτες (Revit) — εξωτ/εσωτ τοίχο, κολώνα, τοιχίο, πλάκα, πόρτα, παράθυρο· FULL ENTERPRISE + FULL SSOT, μηδέν hardcoded χρώματα στους renderers». **Έρευνα (ειλικρινής):** η Revit στο default Architectural template είναι κυρίως **μαύρο + ιεραρχία πάχους**· εσωτ/εξωτ τοίχος ΔΕΝ είναι default χρωματική διάκριση (Wall Type/Function/View Filters), κολώνα vs τοιχίο = διαφορετικά categories (εδώ ενοποιημένα στο `column` με `ColumnKind`). Η ζητούμενη παλέτα = θεμιτή house-style επέκταση (locked via AskUserQuestion, «Παλέτα A — Εξευγενισμένη»). **Λύση (μέχρι τώρα τα `projectionColor`/`cutColor` ήταν κενά):** νέα SSoT σταθερά **`BIM_CATEGORY_LINE_COLORS`** (7 αποχρώσεις) + γέμισμα `DEFAULT_OBJECT_STYLES` — parent `wall`=#2b2f36 (εξωτ.) + subcategory `wall:interior`=#6b7280· parent `column`=#5b6478 + `column:shear-wall`=#2f3a4a· `slab`=#6e6358· `opening` door-*=#c97c2f / window-*=#2d72b8. Ο resolver (`resolveSubcategoryStyle`) **ήδη** εφαρμόζει τα χρώματα → **2D outline + 3D edge overlays** (`resolve3DEdgeStyle`) παίρνουν τα ίδια χρώματα από ΕΝΑ SSoT (zero duplication). **Minimal-surface subcategories** (κρατά το `common-edges` semantics, 2 νέα keys): `WallRenderer` περνά `wallFootprintSubcategory(category)`· `ColumnRenderer` περνά `shear-wall` για `isWallColumnKind`. **Πλήρης κεντρικοποίηση (FULL SSOT):** οι hardcoded παλέτες έφυγαν από τους renderers — `CATEGORY_FILL`→NEW `bim/walls/wall-render-palette.ts`, inline slab consts→NEW `bim/slabs/slab-render-palette.ts`· τα line colors οδηγούνται 100% από τον resolver (fill tints = 2D-only config). **3D:** `BimToThreeConverter` + `bim-three-structural-converters` περνούν το ίδιο function/kind subcat στο `attachEdgesProjection`. **Bitmap-cache:** static defaults → καμία stale (το `bimSettingsHash` hash-άρει μόνο runtime user-overrides). **i18n:** `interior`/`shear-wall` keys (el+en). **Files**: bim-object-styles.ts (+BIM_CATEGORY_LINE_COLORS + 4 cat colors), bim-subcategories.ts (+2 keys/types/taxonomy/wired), WallRenderer/ColumnRenderer/SlabRenderer.ts, NEW wall-render-palette.ts + slab-render-palette.ts, BimToThreeConverter.ts, bim-three-structural-converters.ts, i18n el+en, +color/wiring tests. **Tests**: object-styles + resolver + subcategories + Wall/Column wiring + 3D edges = πράσινα (zero regression). 🔴 browser verify (2D εξωτ./εσωτ./κολώνα/τοιχίο/πλάκα/πόρτα/παράθυρο + 3D ακμές + V/G panel νέες υποκατηγορίες). | Claude (Opus 4.8) |
| 2026-06-02 | **2.18 — Pen Table panel → FloatingPanel SSoT (mirror V/G v2.15)** | **Surface (Giorgio):** «τον "Πίνακα Πενών" θέλω να τον κάνεις FloatingPanel όπως το panel της Ορατότητας — FULL SSOT + SSOT floating». Ο Pen Table άνοιγε ως Radix `DropdownMenu` (έκλεινε σε κάθε outside-click· δύσχρηστο για επεξεργασία καμβά). **Αλλαγή:** port στο κεντρικό **`FloatingPanel`** compound (`@/components/ui/floating`, πάνω στο `useDraggable` SSoT) — ίδια μεταχείριση με το V/G panel (v2.15): draggable palette που παραμένει ανοιχτό, toggle από ribbon trigger (`aria-expanded`), default θέση πάνω-δεξιά (`getClientPosition`), `560×600` `max-h-[70vh] overflow-y-auto`, `FloatingPanel.Header` με `Table2` icon + `panelTitle`. Αφαιρέθηκαν τα Radix imports (`DropdownMenu*`)· το `DropdownMenuSeparator` → semantic `border-t ${colors.border.default}` `role="separator"`. Trigger badge (16×6 + overrideCount), preset tabs, 16×6 grid (`BimLineweightSelect` SSoT), reset — **ΑΜΕΤΑΒΛΗΤΑ**· μηδέν αλλαγή σε store (`useBimPenTableStore`)/render path. **Files (3 modified)**: `panels/PenTablePanel.tsx` (DropdownMenu → FloatingPanel), i18n el+en (+`ribbon.commands.penTable.panelTitle`). tsc clean. **GOL/SSoT**: ✅ ένα floating system (μηδέν διπλό), idempotent, zero hardcoded. 🔴 browser verify (open/drag/stays-open/preset tabs/cell edit/right-click reset/reset-all). | Claude (Opus 4.8) |
| 2026-06-02 | **2.17 — View tab compaction: 7 BIM panels → 2 dense panels (Revit/AutoCAD grouping)** | **Surface (Giorgio):** η καρτέλα «Προβολή» είχε «απλωθεί πολύ» — 14 panels, εκ των οποίων **7 BIM panels** (`viewRange, objectStyles, penTable, visibilityGraphics, hideBim, discipline, viewTemplates`) ήταν **το καθένα 1 panel × 1 row × 1 widget**, σπαταλώντας πλάτος (κάθε panel = label + divider για ΕΝΑ κουμπί). Ζητούμενο: «όπως οι μεγάλοι παίκτες — FULL ENTERPRISE + FULL SSOT». **Απόφαση (RECOGNITION):** το ribbon panel system (`RibbonPanelDef.rows[] / RibbonRow.buttons[] / isInFlyout` + `RibbonPanel.tsx` `rowSize()`) **ήδη υποστηρίζει multi-row/multi-button/flyout** → **μηδέν αλλαγή σε `RibbonPanel.tsx` ή `ribbon-types.ts`** (καθαρό SSoT reuse, καμία διπλή υποδομή). **Αλλαγές:** τα 7 widget buttons (ίδια `widgetId`/`command`, τα widget components ΑΜΕΤΑΒΛΗΤΑ — render-άρουν το δικό τους label μέσω `ribbon.commands.X.label`) αναδιοργανώθηκαν σε **2 dense grouped panels**: **`BIM_GRAPHICS_PANEL`** «Ορατότητα/Γραφικά» (row1: visibility-graphics + discipline-visibility· row2: hide-bim) + **`BIM_STYLES_PANEL`** «Στυλ & Πρότυπα» (row1: object-styles + pen-table· row2: view-range + view-templates). View tab: **14 → 9 panels**. **Files (4 modified)**: `data/view-tab-bim-settings.ts` (7 single-widget panel exports → 2 multi-row panel exports + extracted `RibbonButton` consts), `data/ribbon-default-tabs.ts` (import 7→2, view tab panel list), i18n el+en (+`ribbon.panels.bimGraphics` + `ribbon.panels.bimStyles`· τα 7 παλιά panel-label keys παραμένουν αβλαβή). tsc clean. 🔴 browser verify (View tab: 2 πυκνά BIM panels, κάθε widget ανοίγει/λειτουργεί όπως πριν). | Claude (Opus 4.8) |
| 2026-06-02 | **2.16 — Color picker UX: horizontal layout + stable width + SSoT draggable + infinite-loop fix** | **Surface (Giorgio, μετά το v2.15 floating V/G):** (α) ο color picker dialog ήταν μακρόστενος κάθετος → απαιτούσε scrolling, δύσχρηστο. (β) «το color picker & το V/G panel είναι και τα δύο floating αλλά ΔΕΝ χρησιμοποιούν τον ίδιο floating κώδικα — θέλω full SSoT». (γ) εναλλαγή HEX→RGB→HSL άλλαζε το πλάτος (κουραστικό). (δ) **runtime crash: «Maximum update depth exceeded»** (infinite loop) στο `useComposedRefs`/`safelyDetachRef` όταν άνοιγε ο picker. **Αλλαγές:** **(1) Horizontal layout** — NEW `orientation?: 'vertical'\|'horizontal'` prop στο `EnterpriseColorPicker` (default vertical = backward-compat)· σε horizontal τα sections μπαίνουν σε 2 στήλες (αριστερά: color area + sliders + mode/field + eyedropper· δεξιά: swatches + contrast). `EnterpriseColorDialog` default `horizontal` (override-able) + πλατύτερο `PANEL_MAX_WIDTH_XL` (NEW token `max-w-[840px]`). Τέρμα το scroll. **(2) Full SSoT draggable** — αφαιρέθηκε το **τοπικό διπλότυπο `useDraggable`** (~52 γραμμές) από `EnterpriseColorDialog`· χρησιμοποιεί πλέον το κεντρικό `@/hooks/useDraggable` (το ΙΔΙΟ που χρησιμοποιεί το `FloatingPanel` του V/G panel). Drag-handle = header (`data-drag-handle="true"`). ADR-001/N.12. **(3) Σταθερό πλάτος ανά mode** — η ζώνη ελέγχων (`modeFieldSection`) πήρε fixed `w-48` (192px, ίσο με τον color area)· RGB/HSL χρησιμοποιούν `grid-cols-4` (=`minmax(0,1fr)`) number inputs που πλέον δεν διευρύνουν το panel. **(4) 🐛 Infinite-loop fix (root cause)** — το κεντρικό `useDraggable` επιστρέφει `setPosition` ως **μη-memoized** function (νέα κάθε render)· το open-sync `useEffect` είχε `[isOpen, resetPosition]` με `resetPosition=useCallback(…, [setPosition])` → άλλαζε κάθε render → effect κάθε render → `setPosition(new {0,0})` → re-render → loop (το παλιό τοπικό hook είχε σταθερό `resetPosition` με `[]`). Fix: effect deps `[isOpen]` μόνο (sync-on-open), inline `setPosition({0,0})`, eslint-disable τεκμηριωμένο. **Files (4 modified)**: `EnterpriseColorPicker.tsx` (orientation + section refactor + w-48), `EnterpriseColorDialog.tsx` (−local useDraggable / +central SSoT / horizontal default / loop fix), `color/types.ts` (+orientation), `panel-tokens.ts` (+PANEL_MAX_WIDTH_XL). tsc clean. 🔴 browser verify (open=no-crash, horizontal=no-scroll, drag, HEX/RGB/HSL=ίδιο πλάτος). | Claude (Opus 4.8) |
| 2026-06-02 | **2.15 — Phase C.4 V/G floating palette + native-`<select>`→SSoT sweep (3 panels)** | **Surface (Giorgio UX directive, 2 γύροι)**: (α) ο πίνακας Ορατότητα/Γραφικά (View tab) ήταν μικρό Radix `DropdownMenu` (`text-xs`, `minWidth 640px`) που έκλεινε σε κάθε outside-click — αδύνατο να επεξεργαστείς τον καμβά ενώ μένει ανοιχτός. (β) «τα drop-down που χρησιμοποιεί είναι SSoT; νομίζω όχι» → επιβεβαιώθηκε: ήταν native HTML `<select>` (παραβίαση ADR-001 — canonical = `@/components/ui/select`). (γ) «οι στήλες Τομή/Προβ. Γραμμή είναι πολύ στενές, κρύβονται τα περιεχόμενα» → φάρδυναν τα pattern columns + panel. **Αλλαγές (full SSoT + GOL):** **(1) FloatingPanel port** — από `DropdownMenu` στο κεντρικό **`FloatingPanel`** compound (`@/components/ui/floating`, πάνω στο `useDraggable` SSoT): draggable, παραμένει ανοιχτός (Revit/AutoCAD palette), toggle από ribbon trigger (`aria-expanded`), `text-sm`, panel **880×640** `max-h-[70vh] overflow-y-auto`, default πάνω-δεξιά client-side (`getClientPosition`). `GRID_TEMPLATE` 150/32/58/86/**140**/58/86/**140**px (pattern columns φαρδιά για ελληνικά labels «Διακεκομμένη ×0.5»/«Γραμμή-Τελεία»). Ο παλιός Radix focus-trap workaround (v2.9) δεν χρειάζεται — `FloatingPanel` = `aria-modal="false"`, ο nested `UnifiedColorPicker` ανοίγει χωρίς recursion. **(2) NEW SSoT helper `ui/ribbon/components/BimStyleSelects.tsx`** — `BimPenSelect` (1–16), `BimPatternSelect` (i18n line patterns), `BimLineweightSelect` (ISO mm + "modified" amber highlight + right-click reset), όλα πάνω στο canonical `@/components/ui/select` (Radix). Centralized `PEN_OPTIONS`/`PATTERN_OPTIONS`/`LINEWEIGHT_MM_OPTIONS`. **(3) native-`<select>` sweep σε 3 panels**: `VisibilityGraphicsPanel` (4 selects → BimPen×2 + BimPattern×2), `ObjectStylesPanel` (2 → BimPen×2), `PenTablePanel` (96 cells → BimLineweightSelect). Handlers ξαναγράφτηκαν να δέχονται `number` αντί `string` (η μετατροπή ζει στο SSoT helper). Boy-Scout: αφαιρέθηκαν unused `BIM_LINE_PATTERNS`/local `PEN_OPTIONS`/`LINEWEIGHT_ISO_VALUES` imports. **Files (6 modified + 1 new)**: NEW `BimStyleSelects.tsx`; MOD `VisibilityGraphicsPanel.tsx` + `ObjectStylesPanel.tsx` + `PenTablePanel.tsx` + i18n el+en (+`closeAriaLabel` +`panelTitle`). Μηδέν αλλαγή σε store/resolver/render path. tsc clean. **GOL**: ✅ proactive (ADR-001 SSoT), idempotent, single source (ένα helper), zero hardcoded selects. 🔴 browser verify (open/drag/stays-open/color-picker/pattern labels readable/pen+mm dropdowns/reset). | Claude (Opus 4.8) |
