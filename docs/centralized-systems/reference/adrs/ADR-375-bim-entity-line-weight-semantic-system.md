...
| 2026-05-27 | **2.12 — Phase C.4 V/G Wall Body Fill Tint** | **Surface (Giorgio runtime report after v2.11 fixed the revert race)**: «Στα τετράγωνα προεπισκόπησης παραμένει το επιλεγμένο χρώμα, αλλά δεν χρωματίζεται ο τοίχος με βάση το επιλεγμένο χρώμα». Store ✅, V/G swatch ✅, canvas wall fill ❌. **Root cause**: `WallRenderer.drawFootprint` έβαζε `this.ctx.fillStyle = CATEGORY_FILL[cat]` — μια **hardcoded** translucent tint constant ανά WallCategory (exterior/interior/partition/parapet/fence). Edges (stroke) έπαιρναν ήδη το V/G `_edgeColor` (✅ από v2.4–v2.8) και η hatch (drawMaterialHatch) έπαιρνε το V/G `_hatchColor` ως cut-pattern subcategory (✅). Αλλά για walls **χωρίς material** (δηλ. `wall.params.dna` undefined ή zoom < 0.001), η μόνη ορατή χρωματική επιφάνεια είναι το body fill — και αυτό αγνοούσε εντελώς το V/G. Άρα ο user έβλεπε χρώμα μόνο στις 1-2 px άκρες του outline, ενώ το «σώμα» του τοίχου παρέμενε στην ουδέτερη hardcoded απόχρωση. **Fix**: tint body fill με το resolved `_edgeColor` (V/G category color) όταν είναι set, αλλιώς fallback στο CATEGORY_FILL hardcoded default. (a) `WallRenderer.ts`: νέο private helper `hexToRgba(hex, alpha)` (validates `^#[0-9a-fA-F]{6}$`) + constant `VG_FILL_ALPHA = 0.2` (Revit "cut pattern background" convention — αρκετά διαφανές για να μη σπάει layering με openings/hatches). Reorder στο `drawFootprint`: resolver call moved BEFORE `fillStyle` assignment, then `fillStyle = _vgFill ?? CATEGORY_FILL[cat]`. Όλα τα υπόλοιπα untouched (edges, hatch, openings cutout). **Industry alignment**: Revit V/G «Override Graphics in View» όταν user σετάρει category color, το "Cut Pattern → Background" αυτόματα adopt-άρει εκείνη την tint εκτός αν user explicitly override-άρει — αυτό ακριβώς το pattern. **Files (1 modified)**: `WallRenderer.ts` (+13/-1 — helper + const + 1-line reorder + tint apply). **Tests**: 21/21 relevant suites PASS (WallRenderer-subcategory-wiring + bim-renderers-vg-visibility). Pre-existing test infra issue σε `WallRenderer-with-openings.test.ts` (Firebase auth mock missing — same import chain πριν, NOT caused by v2.12). TSC clean. **Next**: ίδιο pattern μπορεί να χρειαστεί σε Slab/Column/Beam/Stair renderers (όλοι έχουν αντίστοιχα CATEGORY_FILL hardcoded constants). Pending Giorgio confirmation στο wall first — θα ακολουθήσει sweep αν χρειάζεται. | Claude (Sonnet 4.6) |
