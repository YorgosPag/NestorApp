...
| 2026-05-27 | **2.10 — Phase C.4 V/G Color Pick Revert HOTFIX (commit-on-pick UX)** | **Surface (Giorgio runtime report)**: «Κάνω κλικ στο V/G eye → ανοίγει popup → κλικ στο swatch τοιχώματος → ανοίγει color picker → επιλέγω χρώμα → preview swatch αλλάζει αλλά αμέσως μετά επανέρχεται στο αρχικό». **Root cause**: `EnterpriseColorDialog` έχει buffered Apply/Cancel UX (Photoshop pattern) — όταν dialog κλείνει χωρίς Apply (X button), `handleCancel` καλεί `onChange(originalValue)` που rollback-άρει το store. V/G panel δεν έχει affordance για Apply (single swatch interaction, όχι form) και ο user expectation είναι Revit V/G commit-on-pick: pick = done. **Fix**: split UX semantics ανά `showFooter` prop. (a) `EnterpriseColorDialog.tsx` `handleCancel`: όταν `showFooter=true` → revert to originalValue (existing Photoshop UX, DXF Settings tabs unaffected); όταν `showFooter=false` → commit latest `localColor` + fire `onChangeEnd` (Revit V/G commit-on-pick). Cancels pending RAF either way για να μην overshoot stale state. (b) `VisibilityGraphicsPanel.tsx` περνά `showModalFooter={false}` σε όλα τα 24 UnifiedColorPicker instances (12 categories × 2 colors). Footer ξεκάρφωτο σε swatch-only context + X button πλέον commit. **Files (2 modified)**: `EnterpriseColorDialog.tsx` (+5/-2 — handleCancel signature change, deps updated) + `VisibilityGraphicsPanel.tsx` (+2 props × 2 cells). 25/25 V/G runtime tests PASS (zero regression — focus + visibility behavior untouched). TSC clean. DXF Settings tabs (Crosshair / Cursor / Text / Grid / Selection / Ruler colors) διατηρούν Apply/Cancel buffered UX γιατί ακόμη χρησιμοποιούν default `showFooter=true`. | Claude (Sonnet 4.6) |
