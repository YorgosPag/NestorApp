...
| 2026-07-18 | **Οι δύο painters απέκτησαν καταναλωτή ΕΚΤΟΣ της οικογένειας transform tools (ADR-189 §3.13).** Το `drawMoveBasePointMarker` (κόκκινο ＋) + `drawRubberBandLine` (χρυσή διακεκομμένη) καλούνται πλέον και από το `useParallelGuideAnchorPreview` — τη δυναμική γραμμή του «Παράλληλου οδηγού» — μετά από ρητή απαίτηση Giorgio για **ταυτόσημη** εικόνα με τη Μετακίνηση. **Συνέπεια στο συμβόλαιο του glyph:** το ＋ δεν σημαίνει πλέον στενά «σημείο βάσης μετακίνησης» αλλά **«παγωμένο anchor τρέχουσας 2-βηματικής χειρονομίας»**· η διάκριση από το ⊙ της περιστροφής (δαχτυλίδι+σταυρός) παραμένει. ΜΗΝ «διορθώσεις» την επαναχρησιμοποίηση ούτε μετονομάσεις τους painters σε move-specific — η κοινή χρήση είναι ο μηχανισμός που εγγυάται την οπτική ταυτότητα (καμία τιμή χρώματος/dash δεν επαναλαμβάνεται στον καταναλωτή). |
