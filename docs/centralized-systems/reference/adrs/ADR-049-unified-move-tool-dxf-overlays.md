...
- **Solid colour SSoT**: `rendering/ghost/ghost-solid-color.ts` → `resolveGhostSolidColor(entity)`
  **delegates to the canonical `resolveEntityColorHex`** (`systems/selection/select-similar-by-color.ts`
  → `resolveEntityStyle`), the SAME effective-rendered-hex resolver the canvas + "Select Similar by
  colour" + DXF export use. So the moving copy matches EXACTLY what the eye sees for every colour mode
  (ByLayer/ByBlock/ACI/TrueColor/BIM identity) — not an approximation. Shared by `useMovePreview` +
  `useGripGhostPreview` — one colour rule, zero duplicated cascade.
