...
> 🔀 **ΠΟΥ ΖΕΙ ΣΗΜΕΡΑ ΑΥΤΟ ΤΟ DOC-COMMENT (ADR-852 Φ1, 2026-09-12).** Η απαρίθμηση μετακόμισε στο
> **`src/constants/quantity-specs.ts`** (`QUANTITY_SPECS` → `type QuantitySpec`), επειδή το ερώτημα
> «τι ποσότητα είναι αυτός ο αριθμός;» απέκτησε **δεύτερο** καταναλωτή **έξω** από την κορδέλα: το
> ιστορικό αλλαγών. Το `ribbon-types.ts` το **επανεξάγει** ως `RibbonQuantityKind` — **καμία**
> διαδρομή import δεν άλλαξε, και τα 188 `numericInput.quantityKind` της §7.2 ισχύουν αυτούσια.
> Είναι πλέον **ρίζα λεξιλογίου** στο `.domain-vocabulary.json` (CHECK 3.73): δεύτερη απαρίθμηση
> χωρίς αναφορά στη ρίζα **μπλοκάρει**.
