...
| 2026-04-04 | PHASE 5: Profession Bridge — αυτόματη γέφυρα project role → ESCO profession. Όταν επαφή χωρίς επάγγελμα αντιστοιχίζεται σε Έργο ως μηχανικός, το σύστημα auto-fills profession/escoLabel/iscoCode/escoUri. Config: profession-bridge.config.ts, Service: profession-bridge.service.ts, Hook: useEntityAssociations.ts addLink callback. |
| 2026-04-08 | FIX: Added missing `FolderKanban` icon to centralized `IconMapping.ts` for the `projectParticipation` section (icon: `folder-kanban`). Was causing 30+ WARN logs per page load. Also memoized `createIndividualFormTabsFromConfig` with `useMemo` for significant re-render reduction. |
