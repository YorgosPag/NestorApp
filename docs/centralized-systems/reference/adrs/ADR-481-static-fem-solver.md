...
## Changelog
- **2026-06-19 (ADR-504 Φ2 S5, UNCOMMITTED):** **Συνεχής δοκός — μηδέν αλλαγή solver.** Ο analytical builder (ADR-480 S5) σπάει πλέον τον δοκό σε υπο-μέλη στις εσωτερικές στηρίξεις· ο solver τα καταναλώνει **διάφανα** (per-member stiffness από `member-section-properties` ανά `member.entityId` → κάθε υπο-μέλος παίρνει τη διατομή του δοκού· per-member end-forces/diagrams ανά μοναδικό `member.id`). Αποτέλεσμα: πραγματικός συνεχής φορέας → sagging+hogging envelope. Καμία τροποποίηση solver/diagram code· 87 analytical jest GREEN (μηδέν regression). 🔴 tsc(Giorgio)+commit.
...
- **2026-06-18 (Opus, UNCOMMITTED) — ADR-488 (ζωντανός solver):** ο `useProactiveStructuralAnalysis` έπαψε να είναι αμιγώς dormant/explicit. Πλέον γίνεται **proactive** (re-solve σε κάθε κίνηση, coalesced/silent) όταν ο μηχανικός παρατηρεί στατικά (engaged latch `isAnalysisEngaged`). Η απόφαση κόστους «ΟΧΙ eager» **διατηρείται** εκτός engaged (dormant). Λεπτομέρειες → ADR-488.
