...
## 6. Changelog
- **2026-06-19 (ADR-504 Φ2 S5, UNCOMMITTED)** — **Continuous-beam FEM subdivision.** NEW `analytical/beam-interior-supports.ts` (`beamInteriorSupports` — εσωτερικές στηρίξεις δοκού ως κλάσμα `t∈(0,1)`· reuse `projectColumnFootprintOnAxis`+`beamSupportColumnIds`, ίδιο SSoT προβολής με `deriveBeamSpanModel`· end-support = footprint καλύπτει άκρο). `analytical-model-builder`: η `appendBeam` εισάγει ενδιάμεσους analytical κόμβους στα `t` και σπάει το beam member σε **υπο-μέλη** `i→s1→…→j` (κοινό `entityId`, μοναδικά `memberId` `b#k`)· η `mergeFramingEdges` ενώνει τον interior κόμβο με την κορυφή της στηρίζουσας κολώνας (η αντίδραση κατεβαίνει κολώνα→πέδιλο) αντί για nearest-endpoint· `buildMembers` → `id=memberId`. Αμφιέρειστο/πρόβολος αμετάβλητα (1 member, `id===entityId` → μηδέν regression). Αποτέλεσμα: ο solver (ADR-481) βλέπει πραγματικό συνεχή δοκό → sagging στα ανοίγματα + hogging πάνω από τις εσωτερικές στηρίξεις. +9 jest (`beam-fem-subdivision.test.ts`), 87 analytical GREEN. 🔴 tsc(Giorgio)+browser-verify+commit.
