...
- **v1.2 (2026-06-12)** — **Test alignment με το C.9 wall-interior default (test-only, ο κώδικας ήταν σωστός).** Δύο pre-existing failing tests στο `state/__tests__/bim-render-settings-subcategory.test.ts` βασίζονταν σε ξεπερασμένη παραδοχή: ότι ο **τοίχος ΔΕΝ έχει** default subcategories. Από το v1.1 (C.9) το `DEFAULT_OBJECT_STYLES.wall` ΕΧΕΙ default `interior` subcategory → `resetCategorySubcategories('wall')`/`resetAllSubcategories()` σωστά **επαναφέρουν** το `{interior}` (μέσω `withDefaultSubcategories`, SSoT=`DEFAULT_OBJECT_STYLES`), ΟΧΙ `undefined`. Fix: (1) το «WITHOUT defaults» test χρησιμοποιεί πλέον `slab` (πραγματικά χωρίς default subcategories)· (2) ΝΕΟ test ότι ο τοίχος μετά από reset → `subcategories === DEFAULT_OBJECT_STYLES.wall.subcategories` (επαναφορά default `interior`, ad-hoc keys φεύγουν)· (3) το `resetAllSubcategories` test ελέγχει slab→undefined + wall→default interior. 11/11 PASS. ΚΑΜΙΑ αλλαγή σε reset logic ή `DEFAULT_OBJECT_STYLES`. | Claude (Opus 4.8)
