...
### Phase 5: Alerts & Reports ✅ IMPLEMENTED
- [x] Overdue installment alerts (daily cron → notification docs, dedup via tags)
- [x] Payment report per project (aggregate view — ZERO N+1 queries)
- [x] Export to Excel (ExcelJS — 2 sheets: Detail + Summary, styled headers, auto-filter)
...
| 2026-03-16 | **Phase 5 IMPLEMENTED (Alerts & Reports)**: 8 new files + 6 modified (~1100 lines). Types (`payment-export/types.ts` — PaymentReportRow, PaymentReportData, PaymentReportSummary), Report Service (`payment-report.service.ts` — getProjectReport ZERO N+1, getOverdueUnits), Alert Service (`overdue-alert.service.ts` — scanAndNotify with tag-based dedup), Cron route (`/api/cron/overdue-alerts` — daily Vercel Cron, verifyCronAuthorization), Report API (`/api/projects/[projectId]/payment-report` — withAuth + withStandardRateLimit), Excel Exporter (`payment-excel-exporter.ts` — ExcelJS 2 sheets: "Κατάσταση Πληρωμών" detail + "Σύνοψη" aggregates, styled headers, auto-filter, overdue highlighting), Hook (`usePaymentReport.ts` — fetch + dynamic import Excel), Dialog (`PaymentReportDialog.tsx` — summary cards + scrollable table + export button). PaymentTabContent: "Αναφορά" button in header. vercel.json: overdue-alerts cron. i18n: 12 report + 3 alert keys (EL+EN). Email reminders: deferred (μελλοντικό). | Claude Code |
| 2026-03-16 | **SPEC-234D Enhancement: Installment CRUD UI**: 2 new files + 6 modified (~700 lines). Service: `addInstallment()` now supports `insertAtIndex` with splice + re-index. New API route: `POST/PATCH/DELETE /api/units/[id]/payment-plan/installments`. Hook: `addInstallment()`, `updateInstallment()`, `removeInstallment()` methods added to `usePaymentPlan`. New dialog: `EditInstallmentDialog.tsx` (dual-mode add/edit, type select via Radix, delete with AlertDialog confirmation). `InstallmentSchedule`: pencil edit icon per row + "+ Προσθήκη Δόσης" footer button. `PaymentTabContent`: wires dialog + hook mutations. Business rules enforced: negotiation/draft → full CRUD, active → notes only, completed/cancelled → no edit, paidAmount>0 → no delete. i18n: 12 new keys (EL+EN). | Claude Code |
| 2026-03-31 | **SRP Split (SPEC-244D)**: `payment-plan.service.ts` (1280 γρ.) split σε 4 modules: `payment-plan.service.ts` (facade, 258 γρ.), `payment-plan-core.ts` (helpers+reads, 158 γρ.), `payment-plan-installments.service.ts` (installment CRUD+status, 444 γρ.), `payment-plan-recording.service.ts` (payment recording+loans+summary, 288 γρ.). API παραμένει ίδιο: `PaymentPlanService.xxx()` delegates. ADR-244 Phase 3: `buyerContactId`/`buyerName` αφαιρέθηκαν, `ownerContactId`/`ownerName` now required. | Claude Code |
| 2026-07-17 | **Client mutation-action SSoT (ADR-584 / N.18)**: Το jscpd σήμανε 6 self-clones στο `usePaymentPlan`, αλλά ο έλεγχος αδελφών αποκάλυψε **τετράδα**: `usePaymentPlan` ↔ `useChequeRegistry` ↔ `useLoanTracking` (16 clones) **+ `useLegalContracts`** (ADR-230, +3 clones) — **19 clones / 229 διπλές γραμμές (28.17%)**, δηλαδή **23 mutation callbacks** του ΙΔΙΟΥ archetype. Το `useInterestCalculator` ελέγχθηκε και **εξαιρέθηκε**: **+0 clones** — το `fetchJson` του είναι παραλλαγή (default Content-Type, `res.text()` στο σφάλμα → άλλη σημασιολογία), όχι αντίγραφο. **NEW** `src/lib/mutations/gateway-action.ts` — `runGatewayAction(invoke, { run, background?, blocked? })`: κατέχει τις 4 αποφάσεις που ήταν αντιγραμμένες 23× (scope guard → gateway → refetch → mapping σε `ActionResult` ώστε ένα throw να μη διαφεύγει σε click handler). Το `background` είναι **label**, όχι flag → «fire-and-forget χωρίς label» είναι μη αναπαραστάσιμο. Τα hooks κρατούν τα δικά τους `useCallback` → οι ταυτότητες των actions μένουν σταθερές (consumers τα βάζουν σε effect deps). **NEW** `src/lib/api/fetch-json.ts` (`fetchJson` + `jsonRequest`) — ήταν **5 ταυτόσημα αντίγραφα** (4 hooks + `mutateJson` στο gateway)· σημασιολογία αμετάβλητη· **ΔΕΝ** αντικαθιστά το `apiClient` (Bearer/retry/unwrap) — άλλο auth model (session cookie). **NEW** `src/hooks/api/useApiList.ts` — cheque+loan μόνο (ένα URL → ένα `{success,data[]}`)· payment-plan (2 routes) και legal (REST+Firestore sub) κρατούν δικό τους fetcher, ΟΧΙ god-hook με flags. `CreateSplitPlansInput` ήταν δηλωμένο **δύο φορές** (hook + gateway) → μετακινήθηκε στο `types/payment-plan.ts`. **🔴 ΜΙΑ ΣΚΟΠΙΜΗ ΑΛΛΑΓΗ ΣΥΜΠΕΡΙΦΟΡΑΣ**: το `deletePlan` ήταν το μόνο από τα 9 actions **χωρίς guard** — έφτανε στο gateway με `propertyId!` = literal null. Copy-paste παράλειψη (τα 8 αδέλφια του, μαζί με το `updatePlan` ακριβώς από πάνω, αρνούνται πρώτα)· κανένα ADR/caller δεν θέλει null-scoped delete → **έκλεισε**, δεν κωδικοποιήθηκε ως option. Το `'No unit selected'` του loan κρατήθηκε ως spec (διαφορά κειμένου, όχι drift μηχανής). **Verification: 19→0 clones** μετρημένο· `jscpd:diff` καθαρό **ΧΩΡΙΣ** `SKIP_JSCPD_DIFF`· **182 tests** (`hooks/__tests__/gateway-action-bindings.test.tsx` 170 + `lib/mutations/__tests__/gateway-action.test.ts` 12) — γραμμένα **ΠΡΙΝ** το merge σε ατέστωτα hooks, **GREEN πριν (169) ΚΑΙ μετά**, ίδιες assertions πλην της μίας σκόπιμης· **mutation-verified ×2** (λάθος gateway στο binding = 5 κόκκινα· αφαίρεση guard στη μηχανή = 21 κόκκινα· reverted)· registry golden **90/90**· `ssot:flat` 357 modules. ΟΧΙ tsc (N.17). | Claude Code |
