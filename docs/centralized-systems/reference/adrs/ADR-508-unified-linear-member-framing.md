...
- **2026-07-09 (§length-angle-hud-global — το status-bar toggle «ΜΗΚΟΣ/ΓΩΝΙΑ» έγινε ΚΑΘΟΛΙΚΟ)** — owner τεκμηρίωσης UI: **ADR-357**
  ...
  - **SCOPE (κλειδωμένο)** — ΚΡΥΒΟΝΤΑΙ: (α) **ΜΗΚΟΣ** = aligned dims, distance labels, edge lengths, Ø διάμετρος, πλάτος/βάθος· (β) **ΓΩΝΙΑ** = ∠θ / angle labels. **ΔΕΝ κρύβονται ΠΟΤΕ**: εμβαδόν/περίμετρος (`Ε:`/`Περ:`, `buildAreaPerimeterLabelLines`, `paintPolygonAreaLabel`), πάχος·ύψος `specLabel`/`heightSpecLabel` (BIM ταυτότητα), αριθμός πλευρών πολυγώνου (N).
  - **ΑΡΧΙΤΕΚΤΟΝΙΚΟΣ ΚΑΝΟΝΑΣ (ΚΡΙΣΙΜΟΣ):** το gate μπαίνει ΣΤΑ **CALL SITES** των preview/HUD painters (και στους preview-only mid-level cores `paintWallHudCore`/`paint*ColumnHud`/`paintGhostFaceDimensions`, που τρέχουν μόνο για ghosts/HUD, **ΠΟΤΕ** για committed entities), **ΠΟΤΕ** μέσα στους low-level shared painters (`renderDistanceLabel`, `paintAlignedOverlayDimension`, `drawOverlayLabel`, `renderInfoLabel`) — αυτοί μοιράζονται με committed dimensions και gate εκεί θα τις έκρυβε. Ένα SSoT predicate, πολλοί gated call sites.
  - **Gated call sites:**
    - **§line-hud** `drawing-hover-overlays.ts` (`if (lineHud && isLengthAngleHudVisible())`).
    - **§wall-hud** `canvas-v2/preview-canvas/wall-hud-paint.ts` (`paintWallHudCore`): gate στην aligned διάσταση μήκους **+** στη ∠γωνία· το `specLabel` (πάχος·ύψος) **ΜΕΝΕΙ**. Καλύπτει 2D preview **και** 3D (`bim-3d/viewport/overlay-dispatch/use-wall-hud-pass.ts` + grip-drag `use-grip-hud-pass.ts` → ΙΔΙΟ `paintWallHudCore` με 3D projector).
    - **§column-hud** `canvas-v2/preview-canvas/column-hud-paint.ts`: gate σε πλάτος/βάθος (rect), Ø διάμετρο (circular/polygon), μήκη ακμών (profile) **+** ∠γωνία (`paintFrameAngleHeight`)· το ύψος (`heightSpecLabel`) **και** το N (πλευρές πολυγώνου) **ΜΕΝΟΥΝ**.
    - **§polygon-hud / §dxf-preview** `canvas-v2/preview-canvas/preview-entity-renderers.ts`: gate σε line μήκος+γωνία, circle ακτίνα/διάμετρο, polyline edge lengths, rectangle πλάτος/ύψος, angle-measurement σκέλη + ∠θ· το εμβαδόν/περίμετρος (`buildAreaPerimeterLabelLines`/`paintPolygonAreaLabel`) **ΜΕΝΕΙ** πάντα.
    - **§dim (listening/clearance)** `canvas-v2/preview-canvas/ghost-face-dim-paint.ts` (`paintGhostFaceDimensions`): early-return `if (!isLengthAngleHudVisible()) return` — οι κυανές listening dims (μήκος/γωνία) μπαίνουν κάτω από τον καθολικό toggle. Αλληλεπίδραση με το υπάρχον §line-cyan `listeningDimOn`: τα δύο gates είναι ΑΝΕΞΑΡΤΗΤΑ (AND) — αρκεί ένα OFF για να κρυφτούν οι listening dims.
  - **i18n:** `cadDock.statusBar.dimHudDesc` (el + en, `dxf-viewer-panels.json`) ενημερώθηκε ώστε να περιγράφει το καθολικό behavior (χωρίς hardcoded string στον κώδικα — μόνο locale JSON).
- **2026-07-07 (§line-indicator-toggles — status-bar toggles για HUD/τόξο/listening-dims της «Γραμμής»)** — owner τεκμηρίωσης UI: **ADR-357**
  ...
  - **Gates (single-point, non-React getters):** (§line-hud) `dimHudOn` → **ΑΡΧΙΚΑ** μόνο `drawing-hover-overlays.ts` (`if (lineHud && isDimHudOn())`, μόνο line/polyline — ο τοίχος έχει ξεχωριστό `wallHud`). **ΠΛΕΟΝ ΚΑΘΟΛΙΚΟ** — βλ. changelog **2026-07-09 (§length-angle-hud-global)** παρακάτω: το `dimHudOn` κρύβει μήκος+γωνία σε ΟΛΕΣ τις οντότητες (DXF+BIM, draw+grip-drag, 2D+3D) μέσω του νέου SSoT predicate `isLengthAngleHudVisible()`· (§polyline-parity) `dirArcOn` → `drawing-hover-overlays.ts` (line/polyline όρος `&& isDirArcOn()`, BIM μέλη πάντα ON)· (§line-cyan) `listeningDimOn` → `line-preview-helpers.ts:resolveLineFaceDims` (`if (!isListeningDimOn()) return null` — καλύπτει stub State A + State B).
