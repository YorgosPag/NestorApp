...
## Changelog
- **2026-06-09 (Opus 4.8)** — **Slice 2 IMPLEMENTED (preview + commit)**. 1:1 mirror of the ADR-426 water Slice 2: new `drainage-proposal-store` (low-freq pub-sub, ADR-040-safe — set on Generate, reset on Accept/Reject), pure `build-drainage-commit` (`DrainageNetworkProposal → MepSegmentEntity[] + MepSystemEntity[]` via the SSoT `completeMepSegmentFromTwoClicks` with `{classification, diameter, slopePercent}` + DISTINCT per-endpoint z so the descending gravity geometry wins, skip-on-invalid count), brown ghost leaf (`useDrainageProposalGhostPreview` + `canvas-layer-stack-drainage-proposal-ghost`, reusing `MepSegmentGhostRenderer`) — colour from the SSoT `resolveSegmentClassificationColor('sanitary-drainage')` + `hexToRgba` (NO literal hex, unlike the water ghost's tech debt), and a Generate/Accept/Reject ribbon bridge (`useRibbonDrainageAutoBridge` + `drainage-auto-command-keys`) wired through `useDxfBimBridges`/`useRibbonCommands`. Accept = one atomic `CompoundCommand` (`CreateMepSegmentsCommand` + a `CreateMepSystemCommand` rooted at the φρεάτιο outlet); the auto-fitting reconciler grows the junction afterwards. EventBus `bim:drainage-generated|empty|committed` → toasts. i18n el+en (`ribbon.commands.bim.drainageAuto*`, `ribbon.commands.drainage.networkName`, `drainage.*` toasts). 7 new files + ~9 additive shared edits. Tests: new `build-drainage-commit.test.ts` (commit + store) → 14 drainage green; 15 water regression green. Outside ADR-040 except the ghost leaf mount (STAGE ADR-040 — CHECK 6B/6D). tsc deferred (another agent's tsc running — N.17 serialization).
