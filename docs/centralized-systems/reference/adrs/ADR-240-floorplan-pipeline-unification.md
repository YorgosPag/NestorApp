...
| Date | Change |
|------|--------|
| 2026-09-22 | 🐛 **Auto-process ζητά μόνο ό,τι ο διακομιστής μπορεί** (ADR-866 §2.10.9.2 Β6, μετρημένο ζωντανά). Το `useFloorplanAutoProcess` έστελνε **κάθε** `ready` χωρίς `processedData` ⇒ PNG προσωπικού φακέλου = `404` σε κάθε όψη του ακροατή· σε ακύρωση έχανε τα αδοκίμαστα· και η διαδρομή κλείδωνε `processingStatus: 'processing'` **πριν** ελέγξει είδος (400 χωρίς απελευθέρωση ⇒ μόνιμο κλείδωμα· 0 περιπτώσεις στην παραγωγή). **Νέο SSoT** `services/floorplans/floorplan-processability.ts`: `floorplanProcessKindOf` (ο `getFileType` αναθέτει) + `isAutoProcessableFloorplan` (είδος · διαμέρισμα `company` · έτοιμο · bytes · χωρίς αποτέλεσμα). `route.ts`: βήμα 6 πριν το 5b. Άγκυρες ADR-866 Α40.1-Α40.4 (6/6 μεταλλάξεις κόκκινες). Επόμενο σκαλί (**όχι** εδώ): έναρξη από τον διακομιστή στο γεγονός `ready`, όπως Autodesk Docs. |
| 2026-07-11 | 🐛 **Server crash fix — `/api/floorplans/process` 500 «createContext is not a function».** Η server route επεξεργασίας DXF (RSC/`app-route` bundle) δυναμικά κάνει import το `dxf-scene-builder`. Μέσω της αλυσίδας `dxf-scene-builder → types/entities → entity-bounds → entity-bounds-ssot → bim/geometry/scale-bar-geometry → config/display-length-format` (και ξεχωριστά `rendering/entities/shared/distance-label-utils`), δύο **pure** modules εισήγαγαν το `FormatterRegistry` από τον **barrel** `formatting/index.ts`, που re-export-άρει τον React hook `useFormatter` → **`react-i18next`** → `React.createContext(...)` at import-time. Στο vendored RSC React του app-route το `createContext` είναι `undefined` → crash (500, η εισαγωγή DXF έσπαγε τελείως). **Fix (ADR-373 server-safe imports):** deep-import του non-React `FormatterRegistry` από `formatting/FormatterRegistry` + του τύπου `Precision` από `config/number-format-config`, αντί του barrel — καμία αλλαγή στο public API του barrel, functionally identical (18/18 `display-length-format` tests PASS). **Regression guard:** νέο `src/subapps/dxf-viewer/__tests__/server-import-safety.test.ts` — static walk του runtime import-graph από τα server DXF entry points· αποτυγχάνει αν οποιοδήποτε reachable module εισάγει React-context lib (`react-i18next`). Επιβεβαιωμένο με import-graph tracer: `react-i18next` αφαιρέθηκε πλήρως από το graph (16→15 bare packages). **Files:** `config/display-length-format.ts`, `rendering/entities/shared/distance-label-utils.ts`, NEW test. ✅ Google-level: YES — root-cause (client/server import boundary), SSoT-preserving deep import, regression pinned στατικά. |
...
| 2026-05-14 | Fix public floorplan scene access: (1) `useFloorplanSceneLoader` PATH B no longer requires `auth.currentUser` — sends optional Bearer token, API handles anon/public check. (2) Stale FileRecord `file_23328bd6-...` (Storage files missing) marked `isDeleted=true/lifecycleState=archived` so `getFilesByEntity` returns the current file. (3) `scene-fetcher.ts`: added debug-level path diagnostics + explicit warn on Storage miss. |
| 2026-05-12 | Bug fix (Phase 6.F): removed incorrect early-return in `useAutoSaveSceneManager` that skipped all saves when `injectedFileRecordIdRef` or `fileIdCacheRef` had a value — i.e. for every scene loaded from Firestore/Storage. Text entities created by `CreateTextCommand` were never persisted to `.scene.json`. Also fixed `floorplan-dxf-renderer.ts` + `thumbnail-generator.ts` to extract flat text from `textNode` AST and handle `mtext` type (properties page canvas render). Fixed `hit-test-entity-tests.ts`: `hitTestText` guard required flat `text` field — new entities only have `textNode` → unselectable. Now extracts text from `textNode.paragraphs` for bounding-box hit-test. Fixed `text-spline-renderers.ts` `renderTextHover`: replaced `isTextEntity` type guard (required flat `text`) with direct type check + `textNode` fallback extraction. Fixed `useDxfSceneConversion.ts`: `mtext` entities from `CreateTextCommand` had no `case` → fell to `default` → dropped from `DxfScene` entirely (invisible, unhittable, unhoverble). Added `case 'mtext':` fallthrough normalising them to `type: 'text'` with `flatText` extracted from `textNode`. Fixed `Bounds.calculateTextBounds` (ROOT CAUSE of click/hover failure): non-rotated bounds extended `[position.y, position.y + height]` (upward) but `TextRenderer` uses `textBaseline='top'` → actual text occupies `[position.y - height, position.y]` (downward in world-Y). For new texts (height=12 from DEFAULT_FONT_SIZE fallback) the wrong bounds covered only the top 3/12 units → clicks on the lower 9/12 missed. Fixed to `[position.y - height - tolerance, position.y + tolerance]`. Same fix for rotated corners: `{y: -estimatedHeight}` instead of `{y: +estimatedHeight}`. Fixed `TextRenderer.renderTextContent`: hover was invisible because the 35% opacity yellow pre-pass was drawn under full-opacity white text (white covers yellow). Now when `isHovered=true`, `fillStyle` is set to `HOVER_HIGHLIGHT.ENTITY.glowColor` (#FFFF00) — text turns yellow on hover, matching AutoCAD behaviour. Removed the now-redundant `renderTextGlowPrePass`. |
