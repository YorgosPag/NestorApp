...
### 🔬 Επόμενη φάση (Φ C.22) — το **μέγεθος** των TTF δεν είναι το em

Τεκμηριωμένο, **όχι** υλοποιημένο εδώ. Η Autodesk γράφει ρητά για TrueType: *«the value specified
for text height might not represent the height of uppercase letters… The height specified
represents the height of a capital letter **plus an accent area** reserved for accent marks»*
(AutoCAD Help, «About Setting Text Height»). Δηλαδή το AutoCAD κλιμακώνει ώστε ο **ascender** της
γραμματοσειράς να ισούται με το ύψος κειμένου.

Εμείς περνάμε το ύψος **αυτούσιο** ως μέγεθος em (`buildUIFont(screenHeight, …)` και
`drawGlyphRunToCanvas`: `s = targetHeight / GLYPH_REFERENCE_SIZE`, όπου το `GLYPH_REFERENCE_SIZE`
είναι το em του opentype). Για Arial `ascender/unitsPerEm = 1854/2048 ≈ 0,905` ⇒ το κείμενό μας
βγαίνει **~10% μικρότερο** από του AutoCAD, και επειδή το πλάτος ακολουθεί το ύψος, **αλλάζουν και
τα σημεία αναδίπλωσης**. Δεν μπήκε στο Φ C.21 επειδή αγγίζει **κάθε** κείμενο, **κάθε** κουτί και
**κάθε** test της στοίβας — αξίζει δική του φάση με δική της baseline, όχι λαθρεπιβίβαση.
...
MTEXT παραμένει `type:'text'` · **Φ C.22: ascender-based κλίμακα TTF** (πάνω).
🔴 **browser-verify (Giorgio):** re-import `47_ergasia.dxf` → διάστιχο σαν AutoCAD, έντονο **μόνο**
στον τίτλο, υπογράμμιση **μόνο** εκεί που δηλώθηκε, κεντραρισμένες παράγραφοι στη θέση τους, και
**καμία κενή γραμμή** ανάμεσα στις παραγράφους του πίνακα ΦΕΚ.
