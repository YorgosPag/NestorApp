...
**Status:** 🟢 Φάση 1 (τοίχοι) + 🟢 Φάση 2 (κουφώματα→nested `<open>`) **BROWSER-VERIFIED στον Τέκτονα** 2026-06-21 (2Δ+3Δ θέση/μεγέθη σωστά) — UNCOMMITTED, 🔴 commit. 🟡 Φάση 2b (έπιπλα→`<plane>` κουτιά) + 🟡 Φάση A (στέγη→native `<autoroof>` κεκλιμένη, decoded από `ΔΙΑΦΟΡΑ.tek`) **κώδικας έτοιμος** 2026-06-22, 🔴 browser-verify+commit. Commit μόνο Giorgio.
...
- `<point><record>` = footprint κορυφή `pX/pY` (μέτρα) + **`<angle>` = κλίση της ακμής σε RADIANS**
  (δείγμα 0.366519 = 21°· 0 = αέτωμα/επίπεδο). Μία εγγραφή ανά κορυφή (ακμή i = κορυφή i→i+1).
- `<v3list>` = τα **«νερά»** (faces): κάθε `<onev3list>` = ένα κεκλιμένο face, λίστα `<v3>` με `pvX/pvY/pvZ`
  σε **world μέτρα**, per-vertex z (γείσο z=base → κορφιάς z=base+rise). Επίπεδη → degenerate (1 vertex / κενό).
