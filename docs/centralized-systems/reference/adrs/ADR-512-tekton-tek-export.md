...
**Status:** 🟢 Φάση 1 (τοίχοι) + 🟢 Φάση 2 (κουφώματα→nested `<open>`) **BROWSER-VERIFIED στον Τέκτονα** 2026-06-21 (2Δ+3Δ θέση/μεγέθη σωστά) — UNCOMMITTED, 🔴 commit. 🟡 Φάση 2b (έπιπλα→`<plane>` κουτιά) + 🟡 Φάση A (στέγη→`<plane>` MVP flat) **κώδικας έτοιμος** 2026-06-22, 🔴 browser-verify+commit. Commit μόνο Giorgio.
...
- `export/core/tek/bim-to-tek.ts` — `collectTekWalls`: straight walls → records + **group openings ανά `wallId` → nested `<open>`** (Φ2)· curved/ορφανά→skip+warning· returns `openingCount`. **`collectTekPlanes`** (Φ2b+Φ-A): `isTekPlaneEntity` (= `isFurnitureEntity ∪ isRoofEntity`) → `extractEntityFootprintRing`+`extractHeightMm` (γενικοί export extractors) → `<plane>` record (width=ύψος, color=entity.color με fallback). `baseElevationMm`: έπιπλο→`mountingElevationMm`, στέγη→`basePivotZ`.
...
## Reuse
`mmToSceneUnits`/`sceneUnitsToMeters` (scene-units SSoT)· `resolveExportEntities`/`resolveExportFloors`· `buildFloorFilename`/`triggerExportDownload`/`createStoredZip`· `isWallEntity`/`isOpeningEntity`/**`isFurnitureEntity`**/**`isRoofEntity`** (Φ-A)+params· `computeOpeningGeometry` (Φ2)· **`extractEntityFootprintRing`+`extractHeightMm`** (Φ2b+Φ-A· γενικοί export extractors, ίδιοι με DXF/IFC — στέγη έχει `geometry.footprint`+`thickness` → δουλεύει χωρίς νέο extractor).

## Tests
39 jest tek-core (Φ1 15 + Φ2 12 + Φ2b 9 + **Φ-A 3**) + adapter: geometry (canvas→meters, wall+opening xmatrix decoded, decode-parity, **footprint scene→μέτρα+elev**)· writer (num/escape/color/record-fill/inject + open-record/wrap + **plane-record/points fill**)· mapper (straight→record, curved→skip, opening→nested `<open>`, style/handing/orphan + **furniture→plane 2×2m, rot 90° rotated rect, mounting→pointZ, non-furniture skip, scene-unit invariance** + **στέγη→plane flat 4×4m+width+pointZ=basePivotZ, color από entity/fallback, έπιπλο+στέγη μαζί**)· adapter (scope filter, wall+plane injection). **134 export-suite GREEN**, tsc clean (δικά μου).
...
- 🔴 **Φ-A στέγη browser-verify** (Τέκτων): η στέγη εξάγεται **flat `<plane>`** (footprint @ `basePivotZ`, πάχος=`thickness`) = MVP/προσέγγιση. (1) θέση/μέγεθος/στάθμη σωστά; (2) **PROPER κεκλιμένη** → `<autoroof>` native element (marker υπάρχει στο skeleton) ή sloped `<plane>` (pointZ ανά κορυφή + `h1/h2/elev1/elev2`). **Χρειάζεται ΔΕΙΓΜΑ `.tek` με μία στέγη** για decode του `<autoroof>` → ξεχωριστή φάση.
...
## Changelog
- **2026-06-22 (Φ-A)** — **Φάση A: ΣΤΕΓΗ → `<plane>` (MVP flat)**. Η στέγη έχει cached `geometry.footprint` (Polygon3D) + `params.thickness` → δουλεύει **αμέσως** με τους γενικούς extractors `extractEntityFootprintRing`+`extractHeightMm` (μηδέν νέος extractor, ΙΔΙΟΣ με DXF/IFC/έπιπλα). NEW predicate `isTekPlaneEntity` (= `isFurnitureEntity ∪ isRoofEntity`) αντικατέστησε το hardcoded `isFurnitureEntity` filter → νέος plane-τύπος = +ένα guard. `baseElevationMm`: στέγη→`params.basePivotZ` (στάθμη γείσου → pointZ). `toTekPlane` color = `entity.color` (SSoT) με fallback `BC80FC` (rename `DEFAULT_FURNITURE_COLOR`→`DEFAULT_PLANE_COLOR`). **2 αρχεία** (bim-to-tek + test)· 3 NEW jest (39 tek-core, 134 export-suite GREEN), type-safe (🟢 small, tsc skip). **ΣΣοΤ:** μηδέν διπλότυπο — η στέγη πέρασε από την ΥΠΑΡΧΟΥΣΑ plane pipeline, καμία νέα γεωμετρία/store. 🔴 browser-verify (flat θέση/μέγεθος) + **PROPER κεκλιμένη=`<autoroof>` χρειάζεται δείγμα** (βλ. DEFER) + commit.
