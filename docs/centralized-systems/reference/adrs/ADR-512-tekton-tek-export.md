...
- Reuse SSoT `computeFurnitureGeometry().footprint` (rotated rectangle, ΗΔΗ χειρίζεται γωνία) → scene→μέτρα.
  **Μηδέν re-derive** άξονα/γωνίας — ίδιο pattern με κουφώματα/`computeOpeningGeometry`.
...
- `export/core/tek/tek-geometry.ts` — `mmToMeters` (reuse `sceneUnitsToMeters`), **`buildXMatrix`** (γενικό column-major SSoT primitive)· `buildWallXMatrix` + **`buildOpeningPlacement`** (Φ2)· **`furnitureFootprintMeters`** (Φ2b· footprint scene→μέτρα + Z=elevation).
...
- `export/core/tek/bim-to-tek.ts` — `collectTekWalls`: straight walls → records + **group openings ανά `wallId` → nested `<open>`** (Φ2)· curved/ορφανά→skip+warning· returns `openingCount`. **`collectTekPlanes`** (Φ2b): `isFurnitureEntity` → `computeFurnitureGeometry` → `<plane>` record (width=ύψος).
...
## Reuse
`mmToSceneUnits`/`sceneUnitsToMeters` (scene-units SSoT)· `resolveExportEntities`/`resolveExportFloors`· `buildFloorFilename`/`triggerExportDownload`/`createStoredZip`· `isWallEntity`/`isOpeningEntity`/**`isFurnitureEntity`**+params· `computeOpeningGeometry` (Φ2)· **`computeFurnitureGeometry`** (Φ2b· footprint+rotation SSoT).
...
## Changelog
- **2026-06-22** — **Φάση 2b: ΕΠΙΠΛΑ ως `<plane>` κουτιά**. Decode `<plane>` (type 10) από `ΠΛΑΚΑ.tek`: `<point3d>`=footprint πολύγωνο σε world μέτρα, `<width>`=πάχος εξώθησης, `pointZ`=στάθμη. Object route (`<type>2072</type>`) απορρίφθηκε ως library-based (σταθερές διαστάσεις). NEW `PLANE_RECORD_TEMPLATE`/`PLANE_POINT_TEMPLATE` (auto-gen), `TekPlane`/`TekPlanePoint`, `furnitureFootprintMeters`, `buildPlaneRecordXml`/`buildPlanePointsXml`, `collectTekPlanes`· `<!--TEK_PLANE_RECORDS-->` marker στο `<plane>`· `injectTekEntities` 3ο `planesXml` param. FULL SSoT reuse `computeFurnitureGeometry().footprint` (rotated rectangle — μηδέν re-derive γωνίας, ίδιο pattern με `computeOpeningGeometry`)· width=ύψος επίπλου=εξώθηση. 9 NEW jest (36 tek-core σύνολο, 131 export-suite GREEN), tsc clean (δικά μου). 🔴 browser-verify (width vs h1/h2· CCW· Z) + commit.
