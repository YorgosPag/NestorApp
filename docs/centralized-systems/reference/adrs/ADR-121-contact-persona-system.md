...
| **Σχετικά** | 🆕 **ADR-798** *(2026-08-24)* — τι **ΔΕΝ** απαντά αυτό το έγγραφο, παρακάτω |

> 🔗 **ΣΧΕΤΙΚΑ — ADR-798 «Ο Άνθρωπος»** *(2026-08-24)*
>
> ⚠️ **Το `PersonaType` απαντά «ποια επιπλέον πεδία δείχνει η φόρμα **ΕΠΑΦΗΣ**» — ΟΧΙ «τι είναι
> ο κάτοχος του λογαριασμού».** Είναι το λεξιλόγιο **#5** από τα **εννέα** που μέτρησε το
> ADR-798 §3.1, και **κανένα από τα εννέα** δεν απαντά το δεύτερο ερώτημα.
>
> 🔴 **ΜΗΝ επεκτείνεις το `PersonaType` για να καλύψεις επάγγελμα χρήστη.** Η απάντηση εκεί είναι
> το **ESCO/ISCO-08** *(**ADR-132**)*, που καλύπτει **κάθε** επάγγελμα με μηχανικά αναγνώσιμο URI
> — μαζί με όσα δεν έχει σκεφτεί ακόμη κανείς *(π.χ. ο διακοσμητής = ISCO **3432**, που
> προηγούμενο audit είχε γράψει ως «κενό» της απαρίθμησης)*. Νέα τιμή εδώ θα ήταν **δέκατο
> λεξιλόγιο** *(ADR-749)*.
