...
1. ✅ **ΕΚΛΕΙΣΕ → [ADR-792](./ADR-792-bim-point-vocabulary.md)** — και ήταν **διπλάσιο**:
   όχι ένα ζεύγος ομώνυμων αλλά **τέσσερα**, δηλαδή ολόκληρο **διπλό λεξιλόγιο**:
   `Point3D` (216 vs 49) · `Polygon3D` (**54 vs 56**, το ένα **αντικείμενο** `{vertices}` και
   το άλλο **σκέτος πίνακας**) · `Polyline3D` (13 vs 14) · `BoundingBox3D` (45 vs 1) — συν
   **ΕΞΙ** `Point2D`, όχι πέντε (το `utils/precision-positioning.ts` είναι `type`, άρα ήταν
   δομικά τυφλό στο `^export interface`).

   🔑 **Ποιο μετονομάστηκε, και γιατί ΟΧΙ αυτό με τους λιγότερους καταναλωτές**: μετρήθηκε ότι
   **77% των 216** καταναλωτών του `bim-base.Point3D` **δεν διαβάζουν ΠΟΤΕ `.z`**, και **46%**
   των αναγνώσεων που υπάρχουν είναι `z ?? 0`. Το όνομα έλεγε ψέματα. Rhino · Revit · ArchiCAD ·
   three.js · Cinema 4D **και** JTS συμφωνούν σε έναν κανόνα: *το όνομα δηλώνει τι **εγγυάται***.
   Πλέον `BimPoint` / `BimPolygon` / `BimPolyline` / `BimBounds`, και το `Point3D` έχει
   **ακριβώς μία** ρίζα. **268 αρχεία · 1.635 σύμβολα · μηδέν ψευδώνυμα.**

   🔴 Το κόστος πληρωνόταν σε **τέσσερα** σημεία, όχι δύο: τα δύο transforms με ψευδώνυμα, το
   `stair-types.ts:10` με **προειδοποίηση σε πρόζα**, και το `stair-railing-host.ts` που
   ανακάτευε τα δύο λεξιλόγια **χωρίς καν ψευδώνυμο** — εκεί το ψέμα είχε γεννήσει **τρία
   νεκρά `?? 0`** σε μία συνάρτηση.

   🛡️ Φρουρός: **CHECK 3.59** — ένα όνομα, μία ρίζα· κλειστό σύνολο δηλώσεων με λόγο.
...
4. ✅ **ΕΚΛΕΙΣΕ → [ADR-791](./ADR-791-one-offset-engine.md)** — και ήταν **ΤΕΣΣΕΡΑ**, όχι δύο, με **δύο γεωμετρικά ΛΑΘΟΣ** (−50% και −29% σε ορθή γωνία). Το αρχικό εύρημα:
   ...
   **παρέκαμψε**, χωρίς να δει ότι είναι δεύτερη.
   ...
   Εδώ ανήκαν και τα **2 εναπομείναντα** lift του φρουρού, και το δηλωμένο όριο `Β5` της
   §8.6 — **όλα έκλεισαν** στο ADR-791. **Φρουρός `planar-point-lift`: 2 → 0.**
...
- **RATCHET με baseline 0** — **12 → 2** (Φάση Δ, §8.3) → **2 → 0** (ADR-791, το ανοιχτό #4). Με μηδενική baseline ο μηχανισμός **είναι** πλέον μηδενικής ανοχής στην πράξη: κάθε νέα παραβίαση, σε νέο ή υπάρχον αρχείο, μπλοκάρει. Δεν χρειάστηκε αλλαγή μηχανισμού — άλλαξε ο **αριθμός**, όχι ο κανόνας
