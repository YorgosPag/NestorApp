...
3. ✅ **ΕΚΛΕΙΣΕ → [ADR-793](./ADR-793-bounds-dimension-vocabulary.md)** — και το ερώτημα ήταν
   **λάθος διατυπωμένο**. Δεν υπήρχε σφάλμα τιμής: ο μοναδικός αναγνώστης του z
   (`entity-world-aabb.ts:94`) εξυπηρετεί τέσσερα είδη, και **και τα τέσσερα** γράφουν
   **γνήσιο z σε μέτρα** — το `polygonBbox` δεν φτάνει ποτέ εκεί.

   🔑 **Το πραγματικό εύρημα ήταν μεγαλύτερο: ΕΝΑ όνομα για ΔΥΟ πράγματα, με ΤΡΕΙΣ μονάδες.**
   Από τους παραγωγούς `BimBounds`: **7** γράφουν z σε **μέτρα**, **1** (`railing`) σε
   **ΧΙΛΙΟΣΤΑ** — σφάλμα **1000×**, αόρατο μόνο επειδή κανείς δεν διάβαζε το z του
   κιγκλιδώματος — και **11 σημεία** γράφουν `z ≡ 0` επειδή είναι **ίχνη**, με το πραγματικό
   ύψος στο **αδελφό πεδίο του ίδιου αντικειμένου**. Τη διαφορά τη γνώριζε **ένα σχόλιο** και
   μια χειρόγραφη αλυσίδα `if`.

   Πλέον `PlanBounds` (ίχνος, **χωρίς z**) και `SolidBounds` (`minZm`/`maxZm`, **μονάδα στο
   όνομα**). Το `polygonBbox` **διαγράφηκε**. 🛡️ Φρουρός: **5ο pattern** στον
   `planar-point-lift` — όχι νέα πύλη.

   🔴 **ΔΙΟΡΘΩΣΗ ΜΟΝΟΠΑΤΙΟΥ**: αυτή η παράγραφος έγραφε `wall-covering-strip-geometry.ts` στο
   `bim/geometry/`. Το αρχείο ζει στο **`bim/wall-coverings/`**. Η `stripBounds` **υπήρχε**
   (γρ. 181) και διαγράφηκε — αλλά όποιος την έψαχνε εκεί που έλεγε το κείμενο θα συμπέραινε
   ότι είχε ήδη φύγει.
