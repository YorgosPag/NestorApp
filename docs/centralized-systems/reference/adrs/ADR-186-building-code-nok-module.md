...
> **Status**: PARTIAL — Phase 1 Engines + Phase 2 CRUD Form (2026-05-03)
> **Date**: 2026-02-17 (initial), 2026-05-02 (Phase 1 implementation), 2026-05-03 (Phase 2 implementation)
...
## 8b. Phase 2 Implementation Plan (consolidated post-kickoff) — **IMPLEMENTED 2026-05-03**
...
## 10. Phase 2 Implementation Notes (2026-05-03)
...
| Layer | File |
|-------|------|
| Types | `src/types/project-building-code.ts` — `ProjectBuildingCodePhase2`, `BuildingCodeProvenance`, `ValidationIssue`, `BuildingCodeValidationResult` |
| Constants | `src/services/building-code/constants/validation.constants.ts` — `PHASE2_VALIDATION_LIMITS` (sd / coverage / height / frontages, hard+soft) |
| Validation | `src/services/building-code/validation/validate-phase2.ts` — `validateBuildingCodePhase2`, `expectedFrontagesForPlotType` |
| i18n | `src/i18n/locales/{el,en,pseudo}/buildingCode.json` — 3 locale files, root `buildingCode.*` namespace |
| Hook | `src/hooks/useProjectBuildingCode.ts` (~190 lines) + `src/hooks/useProjectBuildingCode.helpers.ts` (~150 lines) — pure provenance/zone-fill mutations split for SRP |
| Components | `src/components/projects/building-code/BuildingCodeTab.tsx` (container) + `BuildingCodeForm.tsx` + `PlotTypeSelector.tsx` + `FrontagesCounter.tsx` + `ZoneSelector.tsx` + `ProvenanceBadge.tsx` + `ResetToDefaultButton.tsx` + `BuildingCodeValidationDisplay.tsx` |

**8 files modified** (4 substantive + 4 i18n plumbing):

| File | Change |
|------|--------|
| `src/services/building-code/types/site.types.ts` | `PlotType` union extended **3 → 6** (`mesaio` / `goniako` / `disgoniaio` ⭐ / `four_sided` ⭐ / `diamperes` / `custom` ⭐). JSDoc updated. Engines (setback-calculator) only positive-check `'mesaio'` and `'diamperes'` — the 3 new types fall through to the generic multi-frontage logic, no engine changes required. |
| `src/types/project.ts` | Added `buildingCode?: ProjectBuildingCodePhase2 \| null` |
| `src/services/projects-client.service.ts` | `ProjectUpdatePayload.buildingCode?` added — `updateProjectClient()` now accepts and forwards the full payload via PATCH `/api/projects/[id]` |
| `src/config/audit-tracked-fields.ts` | `buildingCode` added to `PROJECT_TRACKED_FIELDS_RAW`. Server-side audit (`EntityAuditService.recordChange()` in `project-mutations.service.ts` step 8) automatically logs every change — no client-side audit code needed. |
| `src/config/project-tabs-config.ts` | New tab entry `building-code` at order **16.5** (between `history` 16 and legacy `building-data` 99). Coexistence per Q1 — old tab kept untouched. |
| `src/constants/domains/project-building-persona-labels.ts` | `BUILDING_CODE` keys added to `PROJECT_TAB_LABELS` + `PROJECT_TAB_DESCRIPTIONS` |
| `src/components/generic/mappings/projectMappings.ts` + `index.ts` | `BuildingCodeTab` registered in `PROJECT_COMPONENT_MAPPING` (both modern domain-scoped and legacy master) |
| `src/i18n/{namespace-loaders.ts, lazy-config.ts, config.ts, namespace-manifest.json}` + `building-tabs.json` (el/en) | New `buildingCode` namespace registered, preloaded with critical bundle, governed entry in manifest. `tabs.labels.buildingCode` + `tabs.descriptions.buildingCode` added to building-tabs locale files. |

### 10.2 Architectural decisions (Google-level checklist N.7.2)

| # | Question | Phase 2 answer |
|---|----------|----------------|
| 1 | Proactive or reactive? | **Proactive** — provenance set at the moment of edit/zone-select, never reconciled retroactively |
| 2 | Race condition possible? | **No** — single writer (`updateProjectClient` → API), optimistic local sync only after success |
| 3 | Idempotent? | **Yes** — saving the same draft twice produces the same Firestore document |
| 4 | Belt-and-suspenders? | **Yes** — client-side `validateBuildingCodePhase2` blocks save on errors AND server passes payload through Zod `ProjectUpdateSchema.passthrough()` + `stripUndefinedDeep` |
| 5 | Single Source of Truth? | **Yes** — `project.buildingCode` is the unique persistence location; `useProjectDetail` reconciles via `RealtimeService.PROJECT_UPDATED` |
| 6 | Fire-and-forget or await? | **Await** — `save()` awaits PATCH and toasts on result |
| 7 | Lifecycle owner? | **Explicit** — `useProjectBuildingCode` hook owns draft state, validation, save, and provenance for the lifetime of the tab |

### 10.3 What is intentionally NOT in Phase 2

The Phase 1 engines are ported but **never called** by Phase 2 UI:
...
Per Q1, the new `building-code` tab runs **alongside** the existing `building-data` tab. Migration / hide of the legacy tab is a future ticket (Phase 3.5) — not part of Phase 2 scope. Users can verify both tabs render simultaneously in any project detail view.

### 10.5 Validation thresholds (re-listed for runbook)
...
Hard violation → `errors[]` → save button disabled, inline `AlertCircle`.
Soft violation → `warnings[]` → save still allowed, inline `AlertTriangle`.

### 10.6 Audit trail integration

Server-side `EntityAuditService.recordChange()` in `project-mutations.service.ts` (step 8) already diffs `cleanData` against the old project document using `PROJECT_TRACKED_FIELDS`. Adding `buildingCode` to that registry was sufficient — every PATCH that touches the field produces an audit entry visible in the project's "Ιστορικό" tab. No client-side audit code is required.

Sub-field expansion (per-`sd`/`coverage`/`maxHeight` audit rows instead of one `buildingCode` row) can be added in a future ticket via a `TrackedFieldDef` collection definition; for Phase 2 the JSON-diff display is sufficient.
