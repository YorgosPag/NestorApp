...
|---|---|---|---|
| `xline-mode-store` | 3 | ~~accessi diretti a `_xlineMode`~~ → **ΞΕΠΕΡΑΣΜΕΝΟ**: η `_xlineMode` δεν υπάρχει (ο store πήγε σε `createExternalStore`, WAVE 2.6). Πλέον απαγορεύεται η **ΔΙΠΛΑΣΙΑΣΗ** της κατάστασης — δες §Changelog 2026-08-04 | API pubblica `getMode()` / `setMode()` / `getXLineModeState()` / `subscribe()` |
...
| Date | Change |
|---|---|
| 2026-08-04 | 🔴 **Οι 3 φρουροί του `xline-mode-store` φύλαγαν idiom ΠΟΥ ΔΕΝ ΥΠΑΡΧΕΙ** (ADR-749 §6, Opus 5). Τα patterns ήταν `xlineMode\\.mode(?!Store)` / `\\.angleValue` / `\\.offsetDistance` — **0 ευρήματα, και ούτε καν χωρίς το lookahead**. Η αιτία βρέθηκε **μέσα σε αυτό το ADR**: το §9 (γρ. 1051) τα σχεδίασε για την **ιδιωτική μεταβλητή `_xlineMode`**, που **εξαφανίστηκε** όταν ο store μετακόμισε σε `createExternalStore` (WAVE 2.6, `xline-mode-store.ts:74`). Ο φρουρός έμεινε πίσω από την αναδιάρθρωση — **τρεις μήνες μονίμως πράσινος**. **Νέα patterns (5), όλα μετρημένα σε 0 ευρήματα εκτός store πριν επιλεγούν**: `useState<XLineMode` · `useRef<XLineMode` · `createExternalStore<XLineModeState>` · `type XLineMode\\s*=` · `dxf:xlineMode\\.lastUsed`. 🔑 **Η αλλαγή σημασίας**: τα παλιά φύλαγαν την **ΑΝΑΓΝΩΣΗ** της κατάστασης· τα νέα φυλάνε τη **ΔΙΠΛΑΣΙΑΣΗ** (δεύτερο αντίγραφο σε React state/ref, δεύτερος store, παράκαμψη του localStorage κλειδιού ⇒ μνήμη και δίσκος αποκλίνουν, ξαναδηλωμένη ένωση που θα αποκλίνει σιωπηλά). ⚠️ **ΣΥΝΕΠΕΙΑ ΠΟΥ ΕΠΡΕΠΕ ΝΑ ΑΛΛΑΞΕΙ ΜΑΖΙ: το allowlist ΣΤΕΝΕΨΕ από 4 αρχεία σε 1.** Οι τρεις επιφάνειες (`StatusBarXLineModeSlot`, `XLineToolContextMenu`, `useKeyboardShortcuts`) εξαιρούνταν επειδή **νόμιμα διάβαζαν** την κατάσταση — αλλά με φρουρούς διπλασιασμού είναι ακριβώς τα **ΣΗΜΕΙΑ ΚΙΝΔΥΝΟΥ**: εκεί θα γεννηθεί το δεύτερο `useState`. Αν έμεναν στο allowlist, ο νέος φρουρός θα ήταν **τυφλός ακριβώς εκεί που τον χρειαζόμαστε**. **Επαλήθευση**: 0 νέες παραβιάσεις (baseline αμετάβλητο ως προς αυτό το module)· η πύλη περνά και στις 3 επιφάνειες **χωρίς** allowlist· golden **162 πράσινα** με `shouldMatch`/`shouldSkip` (η `shouldSkip` περιέχει τις παγίδες `type XLineModeState =`, `useState<number \\| null>`, `createExternalStore<SnapModeState>`)· μετάλλαξη ζωντανά (3 παραβιάσεις σε ένα αρχείο ⇒ ΜΠΛΟΚ). |
| 2026-05-19 | **Phase 11 follow-up — Spatial index + hit-test wiring**: `rendering/hitTesting/Bounds.ts` — `calculateXLineBounds()` (large square ±10000 around `basePoint`) + `calculateRayBounds()` (one-directional nominal extent από `basePoint` along `direction`). Χωρίς αυτές τις cases το `BoundsCalculator.calculateEntityBounds()` επέστρεφε null → XLINE/RAY ΔΕΝ εισάγονταν στο spatial index → unselectable + no hover στο canvas. `rendering/hitTesting/hit-test-entity-tests.ts` — `case 'xline'` + `case 'ray'` στο `performDetailedHitTest` dispatch, χρησιμοποιώντας τα υπάρχοντα `pointToInfiniteLineDistance` / `pointToRayDistance` από το Phase 5 utility module. Closes silent gap μεταξύ Phase 4.b (renderer registration) και Phase 11 (grip editing) — τώρα τα XLINE/RAY entities είναι fully interactive στο canvas. |
...
| 2026-05-24 | **i18n hotfix**: `tools.xline.*` + `tools.ray.*` + `tools.constructionLayerScaffold.*` nested keys added to `el/dxf-viewer-shell.json` + `en/dxf-viewer-shell.json` (inside `tools` root object). Root cause: ADR-280 namespace-compat routes `tools.*` → `dxf-viewer-shell` but keys existed only in legacy `dxf-viewer.json`. Runtime lookup failed → raw keys rendered in `ConstructionLayerScaffoldDialog` modal + `StatusBarXLineModeSlot`. Button overflow was a side-effect of long raw key strings. Fix: keys now present in both legacy + canonical shell namespace. |
| 2026-05-19 | **Phase 14 DONE — Selection visual feedback for XLINE/RAY**. Root cause: `XLineRenderer.getGrips` + `RayRenderer.getGrips` returned `[]` → no visual indicator when selected (hover glow worked, selection grips did not). Fix: `XLineRenderer.getGrips` now returns a single grip at `basePoint` (type `'center'`); same for `RayRenderer.getGrips`. This gives the standard blue square grip at the anchor point when entity is selected. Also fixed `DxfRenderer.renderEntityUnified`: `renderOptions` now includes `selected: isSelected` so PhaseManager correctly handles selected+hovered combos (prevents hover-glow from overriding selection). Files modified (3): `XLineRenderer.ts`, `RayRenderer.ts`, `DxfRenderer.ts`. |
| 2026-07-16 | **De-duplication — XLine/Ray parametric core (ADR-584 / N.18 jscpd)**. Root cause: Phases 6.a/6.b and 6.5.a/6.5.b implemented XLINE and RAY as independent twins. jscpd flagged 5 structural clones / 868 tokens between `intersection-calculators.ts` and `ray-intersection-calculators.ts` — the circle/arc/ellipse quadratic solvers were byte-identical, differing **only** in the parameter-acceptance test (XLine `\|t\| <= XLINE_MAX_T`, Ray `t >= -XLINE_EPSILON`). Both entities parametrise identically as `basePoint + t·direction`. Fix: new SSoT `snapping/engines/parametric-line-intersection-core.ts` (242 lines) owning `XLINE_EPSILON`/`XLINE_MAX_T`/`cross2D`/`isAngleInRange`/`getPolylineVertices` plus the shared solvers `parametricLine/Segments/Polyline/Rectangle/Circle/Arc/EllipseIntersection`, each taking a `TAccept` predicate (`acceptXlineT` / `acceptRayT`). The 6 `xline*` + 6 `ray*` functions became one-line wrappers. Private helpers `xlineSegmentPoint` + `raySegmentsIntersection` (the same function twice, both below the 50-token jscpd threshold) collapsed into `parametricSegmentsIntersection`. **Zero public signature changes** — `intersection-calculators.ts` remains the barrel and re-exports the moved primitives; all 180 tests in 15 suites pass unmodified. **Bonus**: removed a real circular import (`ray-intersection-calculators` ↔ `intersection-calculators`) that had been surviving only on function hoisting — both files now depend one-way on the core. Deliberately NOT unified: `xlineXlineIntersection`/`rayRayIntersection`/`rayXlineIntersection` clamp the parameter on different lines (one, both, or only the ray), so they do not reduce to a single predicate; merging them would have silently changed behaviour at extreme coordinates. Files: +1 new (`parametric-line-intersection-core.ts`), `intersection-calculators.ts` 407→274 lines, `ray-intersection-calculators.ts` 196→80 lines. Also corrected a stale comment citing "ADR-583" (Annotation Symbol Library — unrelated) for the `raySegmentsIntersection` consolidation. |
