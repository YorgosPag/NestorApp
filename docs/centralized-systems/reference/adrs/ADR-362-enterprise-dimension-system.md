...
- **2026-05-17 (Phase B2 DONE — Angular geometry builders + shared helper extract)** — Group B Phase B2 implementata.
  - **NEW** `src/subapps/dxf-viewer/systems/dimensions/builders/shared-geometry-helpers.ts` (68 LOC) — `rotateVector`, `intersectLines` (ε=1e-12), `perpendicularOf`, `computeTextAnchor`, `computeTextRotation` estratti da `linear-aligned-builder.ts`. Importati da entrambi B1 e B2 builders — zero duplicazione, ADR-065 SSoT pattern.
  - **NEW** `src/subapps/dxf-viewer/systems/dimensions/builders/angular-builder.ts` (255 LOC) — `buildAngular2LGeometry` + `buildAngular3PGeometry`. defPoints semantic: Angular2L=`[line1.a, line1.b, line2.a, line2.b, arcPoint]`; Angular3P=`[vertex, ray1End, ray2End, arcPoint]`. Arc dim line: centro=vertex, raggio=distance(vertex,arcPoint). `arcStartAngle` → ray1, `arcEndAngle` = `arcStartAngle + signedSweep` (segno CCW/CW). `measurementValue` = |signedSweep| in radians. Ext lines: da endpoint offset DIMEXO → punto tangente arc overshoot DIMEXE (soppresse se endpoint già oltre arc). Arrow direction: tangente OUTWARD dall'arc. `suppressExtLine1/2` rispettati. DIMTIH/DIMTOH per text rotation. Degenerate throw se rays collineari.
  - **MOD** `src/subapps/dxf-viewer/systems/dimensions/builders/linear-aligned-builder.ts` — import math helpers da `./shared-geometry-helpers.ts` (rimosse definizioni locali di `rotateVector`, `intersectLines`, `perpendicularOf`, `computeTextAnchor`, `computeTextRotation`). Nessun cambio al comportamento.
  - **MOD** `src/subapps/dxf-viewer/systems/dimensions/dim-geometry-builder.ts` — aggiornati commenti per B1/B2 scope + `DimGeometry` discriminated union type (ora `LinearDimGeometry | AngularDimGeometry`); switch case `angular2L`/`angular3P` → `buildAngular2LGeometry`/`buildAngular3PGeometry`. Radial variants (`radius`,`diameter`,`arcLength`,`joggedRadius`) + ordinate/baseline/continued → throw Phase B3.
  - **Next**: Phase B3 — Radial builders (radius/diameter/arcLength/joggedRadius) + ordinate/baseline/continued.
