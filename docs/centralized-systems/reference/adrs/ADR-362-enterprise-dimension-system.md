...
- **2026-06-24 (Round 20 — gap (2) CLOSED: associativity for `intersection` / `nearest` snap modes — Revit/AutoCAD DIMASSOC=2)**
  ...
  - **🔴 Pending (Giorgio)**: browser-verify (create a dim on an intersection / nearest point → move the host → the dim follows) + commit (stage ADR-362; ADR-040 too since `useDrawingHandlers`/`drawing-handler-utils` are on the canvas-critical pick path — CHECK 6B/6D).
