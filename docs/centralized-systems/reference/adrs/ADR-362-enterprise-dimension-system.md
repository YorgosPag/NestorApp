...
- **2026-06-23 (Round 14 — effective dimscale resolved ONCE via the `drawingScale` SSoT; mm/cm dimensions no longer microscopic)**
  ...
  - ⚠️ **Pre-existing (not from this change)**: `dim-style-importer.test.ts` R12 test ("rescue dimscale=1 when declared-mm-but-bounds-metres") fails on the committed baseline (verified via stash) — the importer-side rescue is now effectively superseded by the renderer-side heal; flagged for a separate cleanup (remove the importer rescue + its test, or keep as import-time belt-and-suspenders).
