...
- **2026-06-24 (Round 21 — gap (B) real-time dim follow DURING a drag + gap (A) SSoT re-characterization)**
  ...
  - **gap (A) — extension lines "don't touch" the intersection/endpoint — SSoT audit corrected the prior hypothesis (N.0.2 / honesty):** the Round-20 handoff guessed the OSNAP indicator is **missing** during dim creation because the dim-hover path early-returns in `drawing-hover-handler.ts:100-118`. **That is wrong.** Full-pipeline trace: dim tools are `category:'drawing'` (`tool-definitions.ts`) ⇒ `isInDrawingMode()` true ⇒ `mouse-handler-move.ts:299-302` runs `requestSnapDetection(...)` for dim tools too (no dim exclusion there) ⇒ the snap-scheduler publishes `setFullSnapResult` ⇒ `SnapIndicatorSubscriber` renders the OSNAP glyph. **The indicator already feeds during dim creation** — the early-return only skips the dim-specific *preview* compute, not the indicator. Building a 2nd indicator feed would be a duplicate (SSoT violation), so it was NOT done. The remaining real cause is one of: (a1) **DIMEXO gap = by-design** — `buildExtLine` starts the witness line at `origin + dir*DIMEXO` (default `0.625` mm-paper × dimscale), the standard AutoCAD/Revit witness gap; (a2) **snap priority** — the engine returning `perpendicular` at the click (captured verbatim by `resolveDimPickContext`), placing `extOrigin` at the perpendicular foot. **🔴 Pending Giorgio's in-browser confirmation of which (a1/a2) he sees** before any change (a1 ⇒ DIMEXO→0 / toggle; a2 ⇒ endpoint/intersection priority in the snap engine — a shared canvas-critical file, out of the dimension-only scope of this session).
  - **🔴 Pending (Giorgio)**: browser-verify (B) + commit. Stage **ADR-362 + ADR-040** (`canvas-layer-stack-leaves.tsx` is a canvas-critical micro-leaf file — CHECK 6B/6D). New files: `dim-association-ghost-paint.ts`, `useDimAssociationGhostPreview.ts`, `dim-association-ghost-paint.test.ts`; MOD `canvas-layer-stack-leaves.tsx`.
