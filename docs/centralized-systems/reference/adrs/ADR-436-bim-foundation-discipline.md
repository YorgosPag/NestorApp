...
| Date | Author | Change |
|------|--------|--------|
| 2026-06-24 | Opus | **Pad → πλήρης rect-column grip parity (→ ADR-517):** Ο Giorgio ζήτησε «ίδιες λαβές με την ορθογώνια κολόνα». Κενό εκπομπής (όχι υποδομής — όλα τα SSoT ήδη συνδεδεμένα από Slice 1b): το pad έδειχνε **7** grips (rotation + 2 edges + 4 corners), η κολόνα **10** (center MOVE + rotation + **4** edges + 4 corners). **Fix (4 αρχεία, μηδέν διπλότυπα):** `grip-kinds.ts` (`FoundationGripKind += foundation-edge-w/-edge-s`)· `foundation-grips-pad-frame.ts` (`export computeCentroidWorld`· νέες `edgeWHandleWorld`/`edgeSHandleWorld` καθρέφτης `rectColumnGrips`· `rotationHandleWorld` → `(0,−length/4)` midway όπως column rect — ΚΑΙ αποφεύγει σύμπτωση με νέο `edge-s` στο `−length/2`· νέος `FOUNDATION_EDGE_MAP` mirror `COLUMN_EDGE_MAP`)· `foundation-grips.ts` (εκπομπή `foundation-center` gripIndex 0 + `edge-w`/`edge-s`, ίδια σειρά/δείκτες με rect column· `applyFoundationGripDrag` width/length/edge-w/edge-s ενοποιημένα σε **ένα** `applyRectEdgeDrag` μέσω edge map, DRY). Ο σταυρός 4-αυτόνομων-βελακιών + directional move-by-value δουλεύει αυτόματα (`foundation-center` ήδη σε `grip-glyph-registry` + `HOT_GRIP_OP_REGISTRY`· `resolveMoveGlyphFrame` υποστηρίζει pad). 42/42 foundation-grips jest + 27/27 regression (preview/grid) GREEN. Εύρος = μόνο pad (Giorgio)· strip/tie-beam αμετάβλητο. ΕΚΤΟΣ ADR-040· CHECK 6D → stage ADR-436+517. 🔴 browser-verify + commit. |
| 2026-06-12 | Opus | **Justified-strip grip alignment fix (Giorgio: «τα χερούλια της πεδιλοδοκού δεν ταυτίζονται με τα μέσα και τις άκρες»):** Τα grips της πεδιλοδοκού (`strip`/`tie-beam`) βγαίνουν από `getAxisBoxGrips(start/end/width)` στον **raw** άξονα (location line), αλλά ο renderer (`buildBandFootprint`) σχεδιάζει το σώμα **έκκεντρα** όταν `justification = left/right` (ADR-441 Slice 5a — μετατόπιση centerline κατά `sign·hw`, ώστε μία παρειά να πέφτει στον άξονα). → για ΚΑΘΕ περιμετρική λωρίδα κανάβου τα χερούλια κάθονταν μισό πλάτος μακριά από το σχεδιασμένο σώμα. `center` (εσωτερικές/χειροκίνητες) ήταν σωστά (j=0). **Fix (SSoT, 2 αρχεία):** NEW exports στο `geometry/foundation-geometry.ts` — `stripJustifiedAxis(params)` (justified centerline = ο άξονας που πραγματικά σχεδιάζεται· εξάγει το ΥΠΑΡΧΟΝ shift+canonicalization που έκανε inline το `buildBandFootprint`, που τώρα το reuse-άρει → μηδέν duplication) + `unjustifyStripAxis(effAxis, width, justification, sceneUnits)` (inverse). `foundation-grips.ts`: το `lineAxisBoxParams` τρέφει το engine με τον **justified** άξονα (grips στο σώμα)· το `lineFromAxisPatch` κάνει **un-justify** το patch πίσω στον raw άξονα (με το ΝΕΟ width) → το entity κρατά `start`/`end` = location line + `justification` ξεχωριστά, ο renderer re-justify-άρει → round-trip exact. `center` → identity (μηδέν regression· όλα τα παλιά strip tests πράσινα). **Tests:** +6 (`foundation-grips.test.ts`: right/left corners == footprint vertices· face-on-axis· center identity· width/length drag preserve justification) = 55/55· `foundation-geometry` 100%. tsc 0 (δικά μου). ΕΚΤΟΣ ADR-040. ΜΗΝ adr-index (shared tree· ADR-441 το επεξεργάζεται άλλος agent — δεν το άγγιξα). 🔴 browser-verify (επίλεξε περιμετρική πεδιλοδοκό κανάβου → χερούλια πάνω στις παρειές/μέσα/άκρες· σύρε width/length/corner → σωστά) + commit. |
| 2026-06-11 | Opus | **Window/Crossing marquee selection fix (πέδιλα/πεδιλοδοκοί/συνδετήρες δεν επιλέγονταν με κουτί):** Το click-select (spatial-index `Bounds.ts:case 'foundation'`) και το Ctrl+A (`entities.map(e=>e.id)`, χωρίς bounds) επέλεγαν foundations, **αλλά** το window/crossing marquee (`UnifiedEntitySelection.findEntitiesInMarquee` → `calculateEntityBounds`) τα πετούσε σιωπηλά: το `'foundation'` έλειπε από **2 switch** → `default → null` → `entityIntersectsBounds`/`isEntityFullyInsideBounds` = `false`. Ίδια κατηγορία bug με ADR-406/408 (MEP). Fix (2 αρχεία): `systems/selection/shared/selection-duplicate-utils.ts` += `case 'foundation'` (→ `calculateBimEntity2DBounds`)· `bim/utils/bim-bounds.ts` += `case 'foundation'` (geometry.bbox group). +1 test case (`bim-bounds.test.ts` it.each → 14/14 pass). ΕΚΤΟΣ ADR-040. ΜΗΝ adr-index (shared tree). 🔴 browser-verify (window+crossing πάνω σε θεμελίωση) + commit. |
| 2026-06-11 | Opus | **Foundation audit 400 fix (console-noise root cause):** ο `foundation-audit-client` έστελνε `entityType:'foundation'` στο `/api/audit-trail/record`, αλλά το `foundation` έλειπε από το `AuditEntityType` union ΚΑΙ από `VALID_ENTITY_TYPES`/`ENTITY_COLLECTION_MAP` του route → **κάθε** foundation audit POST επέστρεφε `400 Invalid entityType`, σπαμάροντας την κονσόλα σε ριπές κάθε φορά που έτρεχε ο grid reconciler, ΚΑΙ το audit trail της θεμελίωσης δεν καταγραφόταν ποτέ. Ίδια κατηγορία bug με `mep-fitting`/`mep-fixture`. Fix (2 αρχεία): `types/audit-trail.ts` += `'foundation'` στο union· `route.ts` += `foundation: FLOORPLAN_FOUNDATIONS` στο `ENTITY_COLLECTION_MAP`. **SSoT hardening (βλ. ADR-195):** το `VALID_ENTITY_TYPES` παράγεται πλέον από `Object.keys(ENTITY_COLLECTION_MAP)` (+`satisfies Partial<Record<AuditEntityType,string>>`) → αδύνατο να ξανα-desync-άρει allow-list vs map. Διορθώνει το «AuditEntityType union δεν θέλει foundation» που είχε αφεθεί λάθος στο Slice 1-persist. 🔴 browser-verify (foundation audit → History tab) + commit. |
| 2026-06-11 | Opus | **Strip / tie-beam → 7-grip wall parity μέσω κοινού `axis-box-grips` SSoT (pending browser-verify + commit):** Ο πεδιλοδοκός/συνδετήρια έδειχνε **3 λαβές** (start / end / line-width, ΧΩΡΙΣ rotation)· τώρα δείχνει **7** όπως ο τοίχος/δοκός (4 γωνίες + width edge + length edge + rotation). NEW `bim/grips/axis-box-grips.ts` = ο **axis-anchored** consumer του `rect-grip-engine` (αδελφός του `centred-box-grips.ts`)· το line foundation εκπέμπει πλέον τα 7 grips μέσω `getAxisBoxGrips` + routάρει corner/edge/rotation μέσω `applyAxisBoxGripDrag`. NEW kinds `foundation-corner-{start,end}-{pos,neg}` + `foundation-line-length`· `foundation-rotation` reused για το line (anchor-relative swept angle περί midpoint/pivot — ο hot-grip pivot δουλεύει χωρίς αλλαγή στο commit). **Αφαιρέθηκαν** τα dead `lineWidthHandleWorld`/`resizeLineWidth`/`lineUnitAxis`/`lineAxisMidpoint` (το axis↔frame math ζει στο κοινό SSoT). **Semantics change:** width = opposite-face-fixed (πρώην symmetric ×2). `useGripDimAnnotation` += line `w=`/`l=` labels. 47 foundation + 14 axis-box jest, tsc clean. ⚠️ Browser verify (πεδιλοδοκός: 4 γωνίες + 2 edges + rotation). Mirror: ADR-363 Slice E (wall/beam). ΕΚΤΟΣ adr-index (shared tree). |
| 2026-06-11 | Opus | **WYSIWYG placement preview (strip / tie-beam) — πραγματικός `FoundationRenderer` αντί για πράσινη μπάντα (pending browser-verify + commit):** Κατά το 2-click placement η rubber-band έδειχνε ένα πράσινο περίγραμμα band (`PolylineEntity` BRIGHT_GREEN), ΟΧΙ το πραγματικό πέδιλο. Τώρα το `hooks/drawing/foundation-preview-helpers.ts` `generateFoundationPreview` επιστρέφει **πλήρες `FoundationEntity`** μέσω του SSoT `buildFoundationEntity` (ίδιος builder με το commit) με flag `wysiwygPreview: true` → το PreviewCanvas το renderάρει μέσω του **πραγματικού `FoundationRenderer`/`EntityRendererComposite`** (kind fill + RC hatch + dashed hidden-line + centerline) → το preview ΕΙΝΑΙ το τελικό. Η γεωμετρία υπολογιζόταν ήδη σωστά — άλλαξε μόνο το rendering. **1 MOD** (`foundation-preview-helpers.ts`· καθαρίστηκαν unused imports computeFoundationGeometry/UI_COLORS/LINEWEIGHT). Ο μηχανισμός είναι entity-type-agnostic SSoT — λεπτομέρεια ADR-040 changelog (NEW `canvas-v2/preview-canvas/bim-preview-render.ts` + `PreviewRenderer` dispatch· stage CHECK 6B/6D). ΕΚΤΟΣ adr-index (shared tree). Mirror: ADR-363 (wall). |
