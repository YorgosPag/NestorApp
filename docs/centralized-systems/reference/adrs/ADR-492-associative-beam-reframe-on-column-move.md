...
| 2026-06-27 | **Απορρόφηση στο universal reconcile (ADR-540):** ο beam-reframe γενικεύτηκε ώστε να τρέχει και μετά από `Update*ParamsCommand` (όχι μόνο transforms). Τα `reframeBeamsAndEmit` + `reframeBeamsAndEmitAfterRestore` αφαιρέθηκαν — απορροφήθηκαν στο `reconcileAssociativeGeometry` (`bim/cascade/associative-geometry-reconcile.ts`) μαζί με το wall→openings cascade. Εδώ έμειναν το pure `cascadeBeamReframe` (building block) + το `emitRestoredEntities` (undo race-guard). Βλ. **ADR-540**. UNCOMMITTED. |
