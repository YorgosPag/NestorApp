...
| 2026-04-09 | **ICU Interpolation Enforcement** — New pre-commit check 3.8 (`check-icu-interpolation.sh`): blocks `{{var}}` in locale JSON files (must be `{var}` for i18next-icu). Bulk fix: 464 replacements across 60 files (el+en+pseudo). Baseline: 0 violations. CLAUDE.md N.11 updated. |
