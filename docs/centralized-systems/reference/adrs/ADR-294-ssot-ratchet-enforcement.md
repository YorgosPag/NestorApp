...
| Date | Change |
|------|--------|
| 2026-07-16 | **`openai-provider` ratchet gap closed — raw `fetch` is now guarded, not just `createOpenAI(`** (Opus 4.8, UNCOMMITTED). **The gap:** the module's single pattern `createOpenAI\s*\(` was blind to hand-rolled clients that skip the AI SDK entirely and `fetch` the REST API directly. `src/subapps/dxf-viewer/ai-assistant/dxf-openai-call.ts` documented the loophole **in a source comment** — *"Does NOT call `createOpenAI` → does not touch the `openai-provider` SSoT ratchet (that ratchet forbids inline `createOpenAI(`, not raw fetch)"*. A ratchet a developer can read the escape hatch off of is not a ratchet. **Measured blast radius (2026-07-16, ADR-584 jscpd audit + full grep):** **11** raw-`fetch` Responses-API call sites and **4** Chat Completions call sites — the jscpd report had surfaced only 5 of them, because jscpd's 50-token floor hides per-site duplication that is individually small and collectively total. jscpd shows symptoms, not extent. **New pattern:** `fetch\([^)]*(/responses|/chat/completions)` (ERE — no `(?:)`, per the v3.0 lookahead-silent-match-nothing lesson). **Allowlist:** `src/services/ai/openai-provider.ts` + `src/services/ai/openai-responses/`. **Golden fixture added** (`openai-provider`, first Tier-2 entry in the sample): `shouldMatch` covers all four hand-rolled shapes (`createOpenAI({`, template-literal `${baseUrl}/responses`, `${BASE_URL}/chat/completions`, absolute `https://api.openai.com/v1/responses`); `shouldSkip` covers SSoT usage (`executeResponsesRequest` / `getOpenAIProvider`) plus three real false-positive traps — the Telegram reply-builder modules that merely *end in* `/responses` (`import … from './responses'`, `await import('./message/responses')`) and the `${baseUrl}/models` health check. `test:registry-golden` **86 → 90 GREEN**. **New SSoT module:** `src/services/ai/openai-responses/` (wire-types · extract-output-text · execute-request · vision-content · download-file · index) — see ADR-373 changelog for the promotion rationale. **Migrated (3 of 11):** `subapps/accounting/services/external/openai-document-analyzer.ts` (386→~275), `subapps/procurement/services/external/openai-quote-analyzer.ts` (343→~245), `services/ai-analysis/providers/OpenAIAnalysisProvider.ts` (459→~340). **Remaining 8 Responses + 4 Chat Completions sites are tracked by the ratchet baseline** (only-decrease) **and listed in `.claude-rules/pending-ratchet-work.md`** — per N.0.2 a >1h / 4+-file duplicate is Giorgio's priority call, not a unilateral scope expansion. **Deliberately NOT merged:** Chat Completions is a *different wire protocol* (`choices[].message` + `tool_calls`, not `output[].content[]`) — folding it into the Responses SSoT would be fake centralization. It stays with `agentic-openai-client.ts`. **Correction to the prior session's map:** `agentic-openai-client.ts` is labelled `LEGACY — DISABLED` in its header but is **live** — imported by `agentic-loop.ts` (`callChatCompletions`) and `agentic-tool-runner.ts` (`extractTextContent`). Code is the source of truth (N.0.1); it was NOT deleted. **Verification (❌ no tsc, N.17):** 114 characterization tests written **first** against the unmodified clients (39 accounting / 36 procurement / 39 provider — all three had **zero** direct coverage) and still **114 GREEN unchanged** after migration = behaviour-preserving proof; `test:ai-pipeline:all` **1179 GREEN** (N.10, `vision-helpers` touched); `jscpd:diff` **clean** on all 11 files (N.18 — first run caught a surviving 62-token `buildVisionContent` twin, fixed via `beginVisionContent()` rather than skipped). **Known defect preserved, NOT silently fixed:** all migrated clients leak the abort timer — `clearTimeout` runs only after a *successful* `fetch`, so a network rejection leaves the timer pending until timeout (`dxf-openai-call.ts` gets this right with `clearTimeout` in `catch`). Characterized as-is and surfaced to Giorgio; a `finally` fix is a one-line behaviour change awaiting his call. |
| 2026-07-08 | **TIER A duplicate-audit — 3 new tier-3 guard modules (scalar-clamp / normalize-angle-deg / safe-storage) + scalar-math clamp SSoT promotion** (Opus 4.8, UNCOMMITTED). Locks the DXF-viewer TIER A centralization (final duplicate audit 2026-07-08): (1) **scalar-clamp** — the clamp family (`clamp`/`clamp01`/`clamp255`) was promoted to the neutral dependency-free SSoT `src/subapps/dxf-viewer/utils/scalar-math.ts` (so config/bim/bim-3d/snapping consumers no longer import the rendering layer — dependency inversion fixed); `rendering/entities/shared/geometry-utils.ts` became an `import + re-export` shim (this is the `rendering/entities/` change that CHECK 6D flags — a mechanical clamp re-export, no drawing-behaviour change). ~61 inline clamp sites across the subapp were swept onto the SSoT. (2) **normalize-angle-deg** — `normalizeAngleDeg` in `rendering/entities/shared/geometry-angle-utils.ts` is the sole owner of degree→[0,360) wrapping (~18 double-modulo `((x%360)+360)%360` inline sites eliminated). (3) **safe-storage** — SSR-safe localStorage goes through `utils/storage-utils.ts` (`storageGet/Set/Remove/Has`, ADR-092); 3 services re-declaring `function safeStorage()` migrated. **Patterns (collision-safe subset):** scalar-clamp = `Math.max(0, Math.min(255,` + `function/const clamp01`/`clamp255`; normalize = `((.*% 360) + 360) % 360` + `function normalizeAngleDeg`; safe-storage = `function safeStorage`. **⚠️ SCOPE DECISION (mirrors `point-translate-helpers`):** CHECK 3.7 is repo-wide with no path scoping and scalar-math is a dxf-only util → the generic clamp shapes `Math.max(0, Math.min(1,` (forward), `Math.min(1, Math.max(0,` (reverse), `function clamp(`, `const clamp = (` are deliberately NOT guarded — they legitimately occur in non-dxf domains (report-engine / address-editor / floorplan-background / procurement clamps that cannot import the dxf util). Those forms stay enforced by canonical + code review + jscpd (N.18/ADR-584), exactly like the ADR-090 inline-literal discipline. **Allowlists:** scalar-clamp = scalar-math.ts + geometry-utils shim + heat-load-color.ts (NaN-guard clamp01 wrapper) + studio-background-texture.ts (rounds-before-clamp clamp255) + 1 non-dxf `services/floorplan-background/floorplan-background.service.ts` (its own clamp01, cross-domain exemption); normalize = geometry-angle-utils.ts + radial-ring-logic.ts (pieSectorPath self-use); safe-storage = storage-utils.ts. **Verification (❌ no tsc, N.17):** +3 golden fixtures → `test:registry-golden` **84 GREEN**, `test:ssot-suite`/`check-ssot-imports.test.js` **139 GREEN**; end-to-end probe (8 synthetic violations → BLOCK exit 1); 10 allowlisted/survivor files → exit 0; `npm run ssot:baseline` + regenerated `.ssot-registry-flat.txt` (was stale — missing ~11 modules incl. `create-external-store`). **Baseline delta = +1 UNRELATED:** `systems/group/ActiveGroupStore.ts` (`new Set<ActiveGroupListener>` → module `create-external-store`, from commit 21294edf 2026-07-07 which postdates the 2026-07-06 baseline; grandfathered, flagged for a future create-external-store version-signal migration). Modified: `.ssot-registry.json` (+3 modules), `scripts/__tests__/fixtures/registry-golden-fixtures.js` (+3 fixtures), `.ssot-violations-baseline.json`. |
...
| 2026-04-19 | **Registry Golden Regex Tests — Google-level enforcement-tool validation** — 40 tests / 3 groups in `scripts/__tests__/registry-golden-regex.test.js`. Closes the "test the real tool, not an approximation" gap identified while reviewing ADR-314 test coverage. **Group 1 — ERE syntax validity** spawns real `grep -E -f patternFile` against every one of the ~225 `forbiddenPatterns` entries in `.ssot-registry.json` and fails on any regex that returns status 2 (invalid regex). Windows quirk: patterns are written to a temp file + passed via `-f`, not as argv, because `spawnSync` arg-escaping on Windows mangles backslash-heavy ERE patterns (every `\(` turns into `(`) — discovered during initial test run as false failures on 13 modules, resolved. **Group 2 — Golden semantic matching** exercises a cross-tier sample of 12 modules (core: firestore-collections, enterprise-id, domain-constants, addDoc-prohibition, intent-badge-utils, tenant-company-id, soft-delete-config, notification-events, storage-path-construction, entity-creation-manual; Tier 3: intl-formatting, date-local) with hand-written `shouldMatch` / `shouldSkip` fixture pairs in `scripts/__tests__/fixtures/registry-golden-fixtures.js`. Each of the 22 patterns under those modules must fire on its should-match fixture and NOT fire on its should-skip fixture. Patterns are loaded only from `.ssot-registry.json` — tests never hardcode a regex, so registry remains SSoT. **Group 3 — Fixture coverage** sanity: sample spans ≥2 tiers, every sampled module exists in registry, no empty fixtures. **Finding surfaced during test development**: `gcs-buckets[0]` pattern `pagonis-87766(?!\.)(?!-backups)` uses PCRE lookaheads — GNU `grep -E` silently accepts them as literal text and matches nothing → that ratchet module is dead code. Documented inline in the ERE-validity test body with remediation path (switch enforcement to `grep -P` or rewrite via ERE alternation). Not gated — pre-existing registry debt tracked separately. **Runtime**: 49s (dominated by spawnSync cost on Windows for 225 `grep` invocations; Linux CI ~5s). **Why this layer matters**: prior to v3.0 (ADR-294 changelog 2026-04-09), all 225 registry patterns used `(?:...)` non-capturing groups that GNU ERE silently ignored — baseline was artificially 139 violations when the patterns actually matched nothing. Golden tests catch that class of regression at presubmit. New files: `scripts/__tests__/registry-golden-regex.test.js`, `scripts/__tests__/fixtures/registry-golden-fixtures.js`. NPM: `test:registry-golden` + `test:ssot-suite` (runs both this suite and the CHECK 3.18 ratchet suite — 97 tests total). |
| 2026-04-19 | **CHECK 3.18 Google-level Jest test suite (ADR-314)** — 57 tests / 9 groups covering every pure function (parseSummary, stripAnsi, compare, parseArgs), file I/O with tempdir isolation (loadBaseline, writeBaseline — dependency-injected path arg + `SSOT_DISCOVER_BASELINE_FILE` env override), env-driven resolvers (getBaselineFile, getScannerPath), CLI integration via `spawnSync` (smoke / --help / missing baseline / corrupt JSON / unknown arg), in-process coverage of runScanner + runFull + runSmoke + printHelp + main via `process.exit` stub and a fake-scanner bash helper (`fake-scanner-ok.sh`, `fake-scanner-fail.sh`), plus a regression snapshot that loads the real scanner output (`ssot-discover-output.txt`) and asserts `parseSummary` resolves all five metrics (protects against Summary-label rename in `scripts/ssot-discover.sh`). Coverage (measured on `scripts/check-ssot-discover-ratchet.js`): **96.82% statements, 92.30% branches, 100% functions, 96.69% lines** — exceeds Google presubmit target of 95%/90%. Runtime: ~3.5s, zero real scanner spawns. Refactor to enable testing: `loadBaseline`/`writeBaseline` accept an explicit `filePath` arg (default via `getBaselineFile()`), scanner/baseline paths are env-overridable, and the main entry point is guarded with `require.main === module` so Jest can import internals without triggering `main()`. New files: `scripts/__tests__/check-ssot-discover-ratchet.test.js`, `scripts/__tests__/fixtures/{ssot-discover-output.txt,ssot-discover-output.minimal.txt,ssot-discover-output.ansi.txt,baseline-valid.json,baseline-corrupt.json,baseline-missing-field.json,baseline-non-numeric.json,baseline-null-field.json,fake-scanner-ok.sh,fake-scanner-fail.sh}`. NPM: `test:ssot-discover`. |
...
| 2026-04-19 | **gcs-buckets dormant ratchet fixed — golden test + registry module now enforced** — Closes the finding surfaced by the 2026-04-19 Registry Golden Regex Tests entry: `gcs-buckets[0]` used PCRE lookaheads `pagonis-87766(?!\.)(?!-backups)` which GNU `grep -E` silently accepts as literal text → pattern matched nothing → ratchet for the `gcs-buckets` module was effectively dead code. **Fix approach A** (minimal blast, no scanner migration to `grep -P`): rewrote pattern to ERE-valid `['"]pagonis-87766['"]` — matches hardcoded project-ID literals in `'...'` / `"..."` quotes, naturally skips subdomain forms (`'pagonis-87766.firebaseapp.com'`, `.firebasestorage.app`) and backup-bucket literal (`'pagonis-87766-backups'`) because the closing quote in the character class cannot match `.` or `-`. Pattern 2 (template-built backup bucket `${...PROJECT_ID...}-backups`) was already ERE-valid — unchanged. **Golden fixture added**: new `gcs-buckets` entry in `scripts/__tests__/fixtures/registry-golden-fixtures.js` — `shouldMatch` blob covers both patterns (`'pagonis-87766'` + `"pagonis-87766"` + two template variants `${FIREBASE_PROJECT_ID}-backups` / `${GCP_PROJECT_ID}-backups`); `shouldSkip` blob covers SSoT imports + the 3 subdomain/backup-literal false-positive traps above. Test suite: `test:registry-golden` 40 → **44 tests**, `test:ssot-suite` 97 → **101 tests**, runtime ~30s Windows. **Baseline impact**: 1 JSDoc comment in `src/services/backup/backup-manifest.types.ts:35` contained `(e.g. 'pagonis-87766')` — the scanner comment-line filter (`^\s*(//|\*|#)`) does NOT match JSDoc opener `/**`, so the new pattern correctly surfaced this as a violation. Changed JSDoc quote style `'pagonis-87766'` → `` `pagonis-87766` `` (backtick — pattern's character class contains only `'` and `"`). Baseline returns to **0 violations / 0 files**. **Why approach A over B (`grep -E → grep -P`)**: switching the scanner to PCRE would unblock lookaheads but expand blast radius to all 225 registry patterns (some could behave differently under PCRE — e.g. `\b` word boundaries, numbered backreferences). Approach A is surgical: one module's pattern rewritten, no scanner change, golden tests prove both firing + skipping semantics. **Files touched**: `.ssot-registry.json` (module `gcs-buckets` patterns), `scripts/__tests__/fixtures/registry-golden-fixtures.js` (+fixture entry + header comment update), `scripts/__tests__/registry-golden-regex.test.js` (inline caveat updated — dormant → fixed, remediation note), `src/services/backup/backup-manifest.types.ts` (JSDoc quote style). No push — ratchet enforcement verified via `ssot:baseline` + `test:ssot-suite` only. |
