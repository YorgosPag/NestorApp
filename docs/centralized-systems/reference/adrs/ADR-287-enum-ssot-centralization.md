...
- **2026-04-17 (Batch 25)**: Missing-data warnings για systems + finishes — σηματοδότηση ελλιπών καταχωρήσεων σε residential types.
  - **Rationale**: Greek market reality — «κανείς δεν αγοράζει κατοικία χωρίς να ξέρει αν έχει θέρμανση, ψύξη. Κάθε σπίτι διαθέτει σίγουρα φινιρίσματα: έχει κουφώματα, υαλοπίνακες, δάπεδα (πλακάκι/μάρμαρο/μοκέτα)». Residential τύποι (studio, apartment_1br, apartment, maisonette, penthouse, loft, detached_house, villa) πρέπει να εμφανίζουν warning όταν τα παραπάνω fields είναι κενά. Εξαίρεση: commercial (shop, office, hall, storage) — features παραμένουν ελεύθερες (interior + security features V1 χωρίς missing checks — legitimate να είναι κενά).
  - **Distinction** `''` vs `'none'`: Κρίσιμο UX point — το κενό string σημαίνει «δεν απαντήθηκε ακόμα» (missing data), ενώ το `'none'` σημαίνει «ρητή απουσία» (explicit none). Τα νέα missing rules πυροδοτούνται μόνο όταν η τιμή είναι κενή μετά το `normalize()` (null). Τα υπάρχοντα rules για `'none'` (π.χ. `heatingNoneResidential` implausible) παραμένουν ενεργά.
  - **Modified** `src/constants/systems-plausibility.ts`: +2 reason codes `heatingMissingResidential` + `coolingMissingResidential` (unusual verdict), insert μετά από `heatingNoneResidential` (implausible) και πριν από `coolingOversizedTinyUnit`. Priority order updated: 1) heatingNoneNewBuild → 2) heatingNoneResidential → 3) heatingMissingResidential → 4) coolingMissingResidential → 5) coolingOversizedTinyUnit → 6) coolingNoneLargeUnit. Reuse `RESIDENTIAL_TYPES` set (already existed από Batch 24).
  - **Modified** `src/constants/finishes-plausibility.ts`: Breaking API change — added `propertyType` argument σε `AssessFinishesPlausibilityArgs` (και στο `FinishesAssessment`). Added local `RESIDENTIAL_TYPES` set (intentional mirror του systems — zero cross-leaf coupling per ADR layering). +3 reason codes `glazingMissingResidential` + `flooringMissingResidential` + `framesMissingResidential`. **Removed** deprecated `glazingMissingFinished` + `flooringEmptyFinished` — superseded by wider residential gate (no condition pre-requisite: missing σε κατοικία = warning ανεξαρτήτως condition). Removed unused `FINISHED_CONDITIONS` set.
  - **Modified** `src/components/properties/shared/SystemsPlausibilityWarning.tsx`: +2 cases in `reasonKey()` switch.
  - **Modified** `src/components/properties/shared/FinishesPlausibilityWarning.tsx`: +3 cases in `reasonKey()`, −2 deprecated cases. Added `propertyType` prop (inherited via `AssessFinishesPlausibilityArgs`). New `typeLabel` derivation via `PROPERTY_TYPE_I18N_KEYS` (analog to `SystemsPlausibilityWarning`). ICU placeholder `{type}` added στο reason template interpolation.
  - **Wired** `PropertyFieldsDetailCardsRow2.tsx`: Pass `propertyType={formData.type}` to `<FinishesPlausibilityWarning>` (matches same pattern ήδη used by `<SystemsPlausibilityWarning>`).
  - **i18n**: `src/i18n/locales/{el,en}/properties.json` — 2 new keys σε `alerts.systemsPlausibility.reasons.*`, 3 new keys σε `alerts.finishesPlausibility.reasons.*`, removed 2 deprecated (`glazingMissingFinished`, `flooringEmptyFinished`). Pure Greek (zero English words), ICU single-brace `{type}` placeholder. Net +3 keys per locale.
  - **Tests**: `systems-plausibility.test.ts` 13 tests (από 8) — added `heatingMissingResidential`, `coolingMissingResidential`, commercial (shop) escape hatch, priority check (implausible wins over missing). `finishes-plausibility.test.ts` 13 tests (από 9) — added 3 missing-residential, commercial + storage escape hatches, priority check, removed 2 deprecated tests. All 25 passing.
  - **Google pattern**: Market-realistic data completeness hints. Silent suggestion, non-blocking save (consistent με Batch 21/24 plausibility philosophy). Escape hatch για commercial/auxiliary (shop, office, hall, storage) αποτρέπει false positives όπου η απουσία είναι legitimate (π.χ. warehouse χωρίς flooring info).
  - **Out of scope για Batch 25**: interior-features + security-features missing checks (V1 decision — legitimate to be empty per Giorgio). `heatingMissing` + `coolingMissing` trigger σε κάθε residential ανεξαρτήτως condition (το πρώιμο-draft design εξέταζε gate σε `condition ∈ {new, excellent, good}` — απορρίφθηκε: κάθε κατοικία, ακόμα και renovation-pending, αναμένεται να έχει declared systems).
  - **2026-04-17 (Batch 25 amendment)**: Fix blocker στο creation flow + extend scope σε condition + energyClass missing checks.
    - **Bug fix** `finishes-plausibility.ts`: Το `allEmpty` early-exit επέστρεφε `insufficientData` όταν όλα τα finishes fields ήταν κενά, μπλοκάροντας όλα τα missing warnings στη νέα προσθήκη ακινήτου (όπου κατά κανόνα ΟΛΑ τα fields είναι κενά αρχικά). Restriction: `allEmpty && !isResidential` → insufficientData. Residential πάντα αξιολογείται — missing rules πρέπει να σκάσουν ακόμα και σε brand-new property. Added test `creation scenario — fires missing warning on fresh residential`.
    - **Scope extension** `condition-plausibility.ts`: Breaking API — added `propertyType` arg σε `AssessConditionPlausibilityArgs` + local `RESIDENTIAL_TYPES` set (mirror systems/finishes). +2 reasons `conditionMissingResidential` + `energyClassMissingResidential` (unusual). Reorganized gate order: implausible cross-field rules (steps 1-2) μόνο όταν valid condition → step 3 `conditionMissingResidential` fires για residential χωρίς valid condition → steps 4-5 εξακολουθούν cross-field με energy class set → step 6 `energyClassMissingResidential` residential χωρίς κλάση. Non-residential με missing condition συνεχίζει να επιστρέφει `insufficientData` (legitimate data-absence σε commercial/storage).
    - **Rationale (energyClass)**: «Κάθε κατοικία στην Ελλάδα υποχρεούται σε Πιστοποιητικό Ενεργειακής Απόδοσης (ΠΕΑ) — η απουσία καταχώρισης είναι σαφής data-quality gap, όχι legitimate option».
    - **Rationale (condition)**: Χωρίς condition οι cross-field checks (newWithoutHeating, needsRenovationButReady, newButLowEnergy, needsRenovationHighEnergy) δεν μπορούν να τρέξουν — σιωπηλά. Με `conditionMissingResidential` ο χρήστης παίρνει explicit UX hint αντί για silent skip.
    - **Modified** `ConditionPlausibilityWarning.tsx`: +2 cases in `reasonKey()`, +`propertyType` prop, derived `typeLabel` via `PROPERTY_TYPE_I18N_KEYS`, `{type}` ICU placeholder στο reason template interpolation.
    - **Wired** `PropertyFieldsDetailCards.tsx`: Pass `propertyType={formData.type}` to `<ConditionPlausibilityWarning>`.
    - **i18n**: `src/i18n/locales/{el,en}/properties.json` — +2 keys σε `alerts.conditionPlausibility.reasons.*` (`conditionMissingResidential`, `energyClassMissingResidential`). Pure Greek, ICU single-brace `{type}`.
    - **Tests**: `condition-plausibility.test.ts` 13 tests (από 8) — added `conditionMissingResidential`, `energyClassMissingResidential`, creation scenario, commercial escape hatch, priority check. `finishes-plausibility.test.ts` +1 test (creation scenario on fresh residential). Total 39 passing across 3 plausibility suites.
    - **Discovery**: Bug βρέθηκε από Giorgio testing real creation flow («Δημιούργησα ένα νέο ακίνητο, αλλά δεν με προειδοποίησε για τα τελειώματα»). Batch 25 αρχικό design είχε blind spot — tests δεν κάλυπταν το "all-empty residential" scenario.
- **2026-04-17 (Batch 26)**: Clearable Select pattern — SSoT helper για deselect σε Radix Select dropdowns (ανακτησιμότητα κενής κατάστασης μετά από user choice).
  - **Problem**: Real-usage testing (Γιώργος) — «Στα dropdown heating/cooling/frames/glazing/condition/energy, αφού ο χρήστης διαλέξει τιμή, ΔΕΝ μπορεί να γυρίσει σε "μη επιλεγμένο". Λείπει η επιλογή «Επιλέξτε...» που να επιτρέπει deselect». Τα Batch 25/25-amendment missing warnings (`heatingMissingResidential`, `coolingMissingResidential`, `conditionMissingResidential`, `energyClassMissingResidential`, `glazingMissingResidential`, `framesMissingResidential`, `flooringMissingResidential`) γίνονταν unreachable μετά την πρώτη επιλογή — ο χρήστης δεν είχε τρόπο να πει «δεν ξέρω ακόμα, ξανά missing». UX one-way door.
  - **Root cause**: Radix Select (canonical ADR-001) απαγορεύει `<SelectItem value="" />` — ο μόνος χώρος για placeholder είναι το `SelectValue placeholder` prop, το οποίο εμφανίζεται μόνο όταν `value === ''`. Μετά την πρώτη selection, το placeholder κρύβεται οριστικά. Pattern `__clear__` sentinel ήδη υπήρχε inline σε `src/components/properties/shared/NewUnitHierarchySection.tsx` + SSoT constant `SELECT_CLEAR_VALUE` σε `src/config/domain-constants.ts`, αλλά όχι reusable ως helper component — κάθε site replicated το pattern manually.
  - **Created**: `src/components/ui/clearable-select.tsx` (85 γραμμές) — pure wrapper πάνω από canonical `@/components/ui/select`. Props: `value`, `onValueChange (receives '' for clear)`, `placeholder`, `clearLabel`, `disabled`, `size`, `id`, `triggerClassName`, `children (SelectItem list)`. Εσωτερικά χρησιμοποιεί `SELECT_CLEAR_VALUE` + `isSelectClearValue()` από `domain-constants.ts` (zero duplication). `<SelectItem value={SELECT_CLEAR_VALUE}>` εμφανίζεται πρώτο στο dropdown με italic muted-foreground styling — visually distinct από τα κανονικά options. `handleChange` intercepts το sentinel και καλεί `onValueChange('')` → upstream state καθαρίζει → missing warning ενεργοποιείται ξανά.
  - **Wired**: 6 dropdown sites σε 2 files:
    - `PropertyFieldsDetailCards.tsx`: `condition` + `energyClass` Selects (replace 2 Radix Select blocks, drop `Select/SelectContent/SelectTrigger/SelectValue` direct imports).
    - `PropertyFieldsDetailCardsRow2.tsx`: `heatingType` + `coolingType` + `windowFrames` + `glazing` Selects. Για τα 2 level-aware finishes fields (frames/glazing), ο cast `value as FrameType` / `value as GlazingType` αντικαταστάθηκε με `value ? (value as FrameType) : undefined` — type-honest: το κενό string δεν είναι μέλος του enum, οπότε σε clear γράφεται `undefined` στο `LevelData.finishes` (field είναι optional `FrameType?` / `GlazingType?` στο `types/property.ts`).
  - **Refactored**: `NewUnitHierarchySection.tsx` — το inline `__clear__` pattern (floor dropdown) μετανάστευσε σε `ClearableSelect`. Αφαιρέθηκε το `value={selection.floorId || '__clear__'}` hack + η manual `<SelectItem value="__clear__">—</SelectItem>`. `handleFloorChange` απλοποιήθηκε: `value === ''` → clear, else → normal. SSoT consolidation: ένα helper, zero duplicate sentinel handling.
  - **i18n**: `src/i18n/locales/{el,en}/properties.json` — νέο `fields.clearSelection.{heating,cooling,frames,glazing,condition,energyClass,floor}` namespace. 7 νέες keys per locale (net +7). Pure Greek (zero English words): «Επιλέξτε θέρμανση / ψύξη / κουφώματα / υαλοπίνακες / κατάσταση / ενεργειακή κλάση / Χωρίς όροφο».
  - **Tests**: `src/components/ui/__tests__/clearable-select.test.tsx` — 5 unit tests (jest-mock of `../select`) covering: sentinel→'' conversion, normal-value passthrough, value='' accepted by Select root, clearLabel rendered on sentinel item, disabled propagation. All 5 pass.
  - **Type safety**: `FrameType`/`GlazingType` casts fixed (union-safe pattern `value ? value as T : undefined`). Zero `as any`. Zero `@ts-ignore`.
  - **ADR-001 relationship**: `ClearableSelect` = SSoT extension του canonical `@/components/ui/select`. ΔΕΝ αντικαθιστά το Radix Select — είναι thin wrapper πάνω του, χρησιμοποιείται μόνο όταν απαιτείται user-triggered clear. Συμβατό με ADR-001 migrate-on-touch strategy.
  - **SSoT**: Ένα pattern (clear via sentinel), ένας helper. Μελλοντικά sites που χρειάζονται clearable dropdown → direct import `ClearableSelect`, zero replication. Δεύτερη φάση refactor (optional): τα υπόλοιπα inline patterns σε property filters / admin filters να μεταναστεύσουν στο helper.
  - **Out of scope για Batch 26**: Multi-select clear (arrays like orientations/flooring/interiorFeatures δεν είναι Radix Select — είναι Button toggles, ήδη toggleable). Conditional clear gating (π.χ. μην επιτρέψεις clear αν υπάρχει dependent field) — V1 πάντα επιτρέπει clear. Visual separator μεταξύ sentinel και options (V1 relies σε italic muted style για distinction — adequate).
  - **Discovery**: Giorgio real-usage feedback — «πως γυρίζω πίσω στο "δεν ξέρω" αφού διάλεξα;». Highlights το importance του round-tripping σε Google-level forms: κάθε selection πρέπει να είναι αναιρέσιμη χωρίς reload/reset ολόκληρης φόρμας.
  - **Batch 26 amendment (i18n root collision)**: Οι αρχικές keys υπό `fields.clearSelection.*` στο `properties.json` επέστρεφαν raw key path στο runtime UI («fields.clearSelection.heating» αντί για «Επιλέξτε θέρμανση»). Root cause: `src/i18n/namespace-compat.ts:260` (ADR-280) remaps το root `fields` του `properties` namespace στο `properties-detail.json`. Οι νέες keys έπρεπε να ζουν στο `properties-detail.json` για να ικανοποιήσουν το compat contract — αλλά αυτό το αρχείο modifier-owned από άλλο agent session («NEVER checkout other agent files» rule). Fix: μετακίνηση σε top-level root `clearSelection.*` (όχι στο remap map → παραμένει σε `properties.json`). Updated 3 call sites (`PropertyFieldsDetailCards`, `PropertyFieldsDetailCardsRow2`, `NewUnitHierarchySection`) + el/en locale restructure. Takeaway: πριν από κάθε νέα i18n key σε `properties.json`, cross-check το `LEGACY_NAMESPACE_ROOT_MAP.properties` στο `namespace-compat.ts` — το precommit check «i18n keys exist in locale files» ΔΕΝ κοιτά split-namespace correctness, οπότε το bug slipped.
- **2026-04-17 (Batch 27)**: operationalStatus-aware missing gating — σιωπή warnings «ελλιπών δεδομένων» όταν το ακίνητο βρίσκεται σε pre-completion state (draft / under-construction).
  - **Rationale**: Giorgio UX feedback — «όταν η λειτουργική κατάσταση είναι ημιτελές, ισχύουν οι ίδιοι έλεγχοι; Τι θα έκανε η Google;». Τα Batch 25 missing-residential warnings (7 reason codes: `conditionMissingResidential`, `energyClassMissingResidential`, `heatingMissingResidential`, `coolingMissingResidential`, `glazingMissingResidential`, `flooringMissingResidential`, `framesMissingResidential`) σκάνε ακόμα και σε ακίνητα που κατασκευάζονται. Αποτέλεσμα: UX noise — ο χρήστης δεν μπορεί νόμιμα να καταχωρήσει heating/cooling/ΠΕΑ αν το unit είναι υπό κατασκευή (εξοπλισμός δεν έχει εγκατασταθεί, πιστοποιητικό εκδίδεται στην αποπεράτωση).
  - **Google pattern — Progressive disclosure**: Material Design + Workspace forms suppress "required-field" warnings μέχρι το state να ασφαλίζει completeness ("claimed" / "verified" level σε Maps business data). Missing warnings εμφανίζονται μόνο όταν η απουσία είναι πραγματική data-quality gap, όχι legitimate construction-phase absence.
  - **Decision matrix** (per-reason, per-status):
    - `draft` + `under-construction` = **pre-completion** → suppress missing-warnings
    - `inspection` + `maintenance` + `ready` = **post-completion** → warnings ενεργά
    - **Cross-field declarative warnings** (user δήλωσε ρητά αντιφατικές τιμές — π.χ. `heating=none` + `condition=new`) → **ΠΑΡΑΜΕΝΟΥΝ active** regardless of operationalStatus. Είναι declarative incoherence, όχι missing data.
    - **Orientation missing** → **δεν αλλάζει**. Orientation = geometry foundation-level (γνωστή από τη θεμελίωση), δεν εξαρτάται από την πρόοδο κατασκευής.
    - **Interior features missing** → **δεν υπάρχει**. Gate ήδη via `length === 0 → insufficientData`.
  - **Per-helper gate changes**:

    | helper | suppressed σε pre-completion | παραμένουν active |
    |---|---|---|
    | `condition-plausibility` | `conditionMissingResidential`, `energyClassMissingResidential` | `needsRenovationButReady`, `newWithoutHeating`, `newButLowEnergy`, `needsRenovationHighEnergy` |
    | `systems-plausibility` | `heatingMissingResidential`, `coolingMissingResidential` | `heatingNoneNewBuild`, `heatingNoneResidential`, `coolingOversizedTinyUnit`, `coolingNoneLargeUnit` |
    | `finishes-plausibility` | `glazingMissingResidential`, `flooringMissingResidential`, `framesMissingResidential` | `glazingSingleHighEnergy`, `carpetWithUnderfloor`, `glazingTripleLowEnergy` |
    | `interior-features-plausibility` | — (no missing reasons) | όλα τα declarative reasons |
    | `orientation-plausibility` | — (geometry invariant) | `missingResidential`, `tooMany`, κλπ |

  - **Created** `src/constants/operational-statuses.ts`: Νέο subset `PRE_COMPLETION_OPERATIONAL_STATUSES = ['draft', 'under-construction']` + type guard `isPreCompletionOperationalStatus(value)`. Καθαρό SSoT — κάθε helper καταναλώνει τον guard, zero duplication.
  - **Modified** `src/constants/condition-plausibility.ts`: Import `isPreCompletionOperationalStatus`. Gates `conditionMissingResidential` (step 3) και `energyClassMissingResidential` (step 6) με `&& !isPreCompletion`. Zero API breaking — η function ήδη δεχόταν `operationalStatus` από Batch 25.
  - **Modified** `src/constants/systems-plausibility.ts`: Breaking API (soft) — added optional `operationalStatus` σε `AssessSystemsPlausibilityArgs` + στο `SystemsAssessment`. Gates `heatingMissingResidential` (step 4) και `coolingMissingResidential` (step 5) με `&& !isPreCompletion`. Cross-field rules (heatingNone*, cooling*) ανέγγιχτα. `buildAssessment` signature +1 arg (operationalStatus).
  - **Modified** `src/constants/finishes-plausibility.ts`: Breaking API (soft) — added optional `operationalStatus`. Gates τα 3 missing-residential reasons (steps 4-6) με `&& !isPreCompletion`. Το `allEmpty` early-exit επεκτάθηκε: `allEmpty && (!isResidential || isPreCompletion)` → `insufficientData` (evita ψεύτικο warning σε creation υπό-κατασκευή).
  - **Modified components**: `SystemsPlausibilityWarning.tsx` + `FinishesPlausibilityWarning.tsx` — destructure + forward `operationalStatus` prop (inherited από το args interface).
  - **Wired** `PropertyFieldsDetailCardsRow2.tsx`: Pass `operationalStatus={formData.operationalStatus}` σε `<SystemsPlausibilityWarning>` + `<FinishesPlausibilityWarning>`. Το `ConditionPlausibilityWarning` το λάμβανε ήδη από Batch 25.
  - **i18n**: **ZERO νέες chiavi**. Καθαρά gating — reason emitted ή null. Κανένας νέος label, καμία αλλαγή locale files. Rispetta pure-Greek rule.
  - **Tests**: +7 test σε `condition-plausibility.test.ts`, +6 σε `systems-plausibility.test.ts`, +8 σε `finishes-plausibility.test.ts`. Total +21 tests. Coverage: pre-completion suppression (under-construction + draft), regression cross-field still fires, post-completion (inspection/maintenance) still fires missing warnings.
  - **SSoT**: Ένας subset array + ένας guard σε `operational-statuses.ts`. 3 helper χρησιμοποιούν το ίδιο guard. Μελλοντική αλλαγή policy (π.χ. `inspection` να θεωρείται επίσης pre-completion) → single edit στο subset array, zero helper changes.
  - **Google pattern — final decision**: Hybrid per-reason gate (Opzione D). Missing-reasons OFF σε pre-completion, declarative cross-field reasons ON πάντα. Orientation αμετάβλητο (geometry invariant). Interior features αμετάβλητο (gate μέσω length).
  - **Out of scope για Batch 27**: Soft-severity downgrade (π.χ. «Να συμπληρωθεί πριν την παράδοση» λεκτικό) — απερρίφθη, silence είναι καθαρότερη UX. Per-reason status matrix (κάθε reason decides standalone) — overkill για V1, το subset guard arκεί. Status-dependent i18n labels (π.χ. «ακίνητο υπό κατασκευή — παραλείπεται έλεγχος φινιρισμάτων» banner) — θα προστεθεί σε μελλοντικό batch αν ζητηθεί.
  - **Discovery**: Giorgio real-usage testing — «όταν η λειτουργική κατάσταση ενός ακινήτου είναι ημιτελές, ισχύουν οι ίδιοι έλεγχοι και οι ίδιες ειδοποιήσεις στον χρήστη; Τι θα έκανε η Google σε αυτήν την περίπτωση;». Highlights: Google-level forms δεν προειδοποιούν για απουσία δεδομένων που νόμιμα δεν μπορούν ακόμα να υπάρχουν.
- **2026-04-17 (Batch 29)**: Property photo pipeline bugfix — entity-polymorphic photo upload + live fetch subscription.
  ...
  - **New hook** `src/components/generic/photo-system/hooks/usePhotosTabFetch.ts` — live Firestore subscription on files collection filtered by entityType/entityId/domain/category/status=ready/lifecycleState=active. Query contract identical to `usePropertyMediaCounts`. 2 new composite indexes (default + super_admin) for CHECK 3.15 coverage.
  - **Contract tests** `src/components/generic/photo-system/config/__tests__/photos-tab-config.test.ts` lock canonical write+read per tab.
  - **Impact Batch 28**: `usePropertyMediaCounts` now receives matching records — completion meter photos bucket unblocks after next upload. Legacy test FileRecords with CONTACT tag stay orphan (test data, no backfill needed).
  - **Cross-reference**: Full details in ADR-293 Phase 7.

- **2026-04-17 (Batch 28)**: Property completion meter — Google-style listing profile strength με per-type weighted matrix + media thresholds + operationalStatus gating.
  - **Rationale**: Giorgio request — «ποσοστό συμπλήρωσης πεδίων ακινήτου» στο UI, πάνω από τα cards των field groups. Data-entry guidance σε real-time — ο χρήστης βλέπει score κι ακριβώς ποια critical fields λείπουν με click-to-jump.
  - **Google pattern references**: My Business completion score (weighted fields + photo threshold + claimed/verified state), LinkedIn All-Star profile (fixed per-field weights, coaching suggestions), Spitogatos/Idealista listing quality (media-heavy, floorplan differentiator, per-type expectations).
  - **Created** SSoT leaf modules:
    - `src/constants/field-completion-weights.ts` — `FIELD_WEIGHTS: Record<PropertyTypeCanonical, readonly FieldWeightEntry[]>` + `FieldKey` union (17 keys: type, areaGross/Net, bedrooms, bathrooms, orientations, condition, energyClass, heating, cooling, windowFrames, glazing, flooring, interior/securityFeatures, floorplan, photos). Per-type matrix με 3 weight tiers: `2` critical, `1` normal, `0.5` optional. Absence = **exempt** (skipped from denominator). Helpers `getFieldWeightsForType`, `isFieldScorable`.
    - `src/constants/media-completion-thresholds.ts` — `MEDIA_THRESHOLDS: Record<PropertyTypeCanonical, MediaThreshold>` με per-type photo thresholds (min/optimal/bonusCap) + floorplan boolean. Scoring curve: below-min linear ramp to 0.5, min→optimal linear to 1.0, beyond optimal flat 1.0 (diminishing returns anti photo-spam). Helpers `computePhotoScore`, `computeFloorplanScore` (multi-level aware).
    - `src/constants/property-completion.ts` — main `assessPropertyCompleteness()` pure function. Returns `{ shouldHide, score, percentage, weightTotal, weightEarned, breakdown[], missingCritical[], missing[], exemptFields[], bucketColor }`. Deterministic, no side effects, safe για malformed input (fallback σε apartment matrix).
  - **operationalStatus gating** (Batch 27 parity):
    - `draft` → `shouldHide = true`. UI πλήρως απόκρυβει meter (progressive disclosure).
    - `under-construction` → `PRE_COMPLETION_EXEMPT_FIELDS` (finishes/systems/energy/interiorFeatures) exempt από denominator. Photos weight × 0.5 (shell photos valgono meno). Floorplan, orientation, layout, area, condition, security features contano πλήρως.
    - `inspection`/`maintenance`/`ready` → full scoring, zero exemptions.
  - **Weight matrix (17 fields × 12 types)**: studio/apartment_1br (light residential) — type/areaGross critical, bedrooms weight 0.5/1, finishes normal. apartment/maisonette — bedrooms critical (2×). penthouse/loft — interiorFeatures upgraded. detached_house/villa (standalone ADR-284 Family B) — orientations critical (foundation geometry). shop — reduced finishes weight, skip orientations/bedrooms. office — orientation skip, bathrooms/heating/cooling normal. hall/storage — only identity + area + condition + photos + floorplan + securityFeatures (skip orientations/rooms/finishes/systems/ΠΕΑ).
  - **Media thresholds matrix**: studio 3/5/10, apartment_1br 4/6/10, apartment 5/8/12, maisonette/penthouse 6/10/14, loft 5/8/12, detached_house 7/10/15, villa 8/12/20, shop/office 3/5/10, hall 2/4/8, storage 1/2/4.
  - **Bucket colors**: `< 40%` red, `40–70%` amber, `> 70%` green.
  - **Created** hook layer:
    - `src/hooks/properties/usePropertyMediaCounts.ts` — real-time `firestoreQueryService.subscribe<FILES>` με per-domain/category constraints + lifecycleState ACTIVE filter. Photos: `domain='sales', category='photos'`. Floorplan: `domain='construction', category='floorplans'`. CompanyId auto-injected από service (CHECK 3.10 compliant). Skip όταν `propertyId === '__new__'` ή null. Videos out-of-scope V1 (VideosTabContent stub hardcoded `[1,2,3]`).
    - `src/hooks/properties/usePropertyCompletion.ts` — composition hook. Combina `usePropertyMediaCounts` + `assessPropertyCompleteness` + localStorage dismissal state (key `property-completion-meter:dismissed:v1:${userId}`, SSR-safe). Upgrade path σε `EnterpriseUserPreferencesService.customSettings` διαθέσιμος αν ζητηθεί cross-device sync.
  - **Created** UI components:
    - `src/features/property-details/components/PropertyCompletionMeter.tsx` — semantic `<section aria-label>` με Progress bar, percentage, bucket-colored label, expand/dismiss buttons. Draft → null. Dismissed+collapsed → 1-line «Εμφάνιση πληρότητας» link (Google undismiss pattern).
    - `src/features/property-details/components/PropertyCompletionBreakdown.tsx` — collapsible missing-fields list. Sorted by weight desc. Each row = `<Button>` με field label + weight badge. Click → `scrollIntoView({behavior: 'smooth'})` στην card via static `FIELD_TO_CARD_ANCHOR` map. Media fields (photos/floorplan) live σε άλλα tabs → V1 button disabled (silent no-op, Google UX).
  - **Wired**: `PropertyFieldsBlock.tsx` inserimento `<PropertyCompletionMeter>` prima di `<PropertyFieldsEditForm>` (sopra le cards dei campi, sotto NewUnitHierarchySection σε creation mode). Conditional `!isCreatingNewUnit && !isReadOnly`. Legge `formData` live (unsaved edits) + `effectiveLevels` per multi-level floorplan scoring.
  - **Wired**: Card ID anchors για click-to-jump — `PropertyFieldsDetailCards.tsx` (`field-layout`, `field-orientation`, `field-condition-energy`) + `PropertyFieldsDetailCardsRow2.tsx` (`field-systems`, `field-finishes`, `field-features`). `tabIndex={-1}` ώστε να δέχονται programmatic focus χωρίς tab-order disruption. Identity/area fields (type, areaGross/Net) + media (floorplan, photos) deferred σε V2 (live εκτός info tab).
  - **i18n**: +28 chiavi per locale στο `src/i18n/locales/{el,en}/properties.json` στο νέο top-level namespace `completion.*` (cross-checked με `LEGACY_NAMESPACE_ROOT_MAP.properties` — Batch 26 amendment learning: evitato `fields.*` που remap-άρει σε `properties-detail.json`). Pure Greek, ICU single-brace `{count}` placeholder. Subsections: `bucket.{red,amber,green}`, `breakdown.{heading,allComplete,weightCritical/Normal/Optional}`, `fields.*` (17 field labels).
  - **Tests**: `src/constants/__tests__/property-completion.test.ts` — ~30 unit tests. Coverage: empty creation scenarios (residential low score, villa orientations critical missing, storage reduced denominator), fully-populated types (100% apartment, 100% storage, ~90% με min photos), operationalStatus gating (draft hide, under-construction exemptions, photos halved, inspection full), media scoring curve (missing/partial/complete statuses, diminishing returns, multi-level floorplan partial), bucket boundaries, missingCritical ordering, per-type exemption integrity, edge cases (unknown type fallback, determinism, negative photo clamp).
  - **SSoT**: 3 leaf modules + 2 thin hooks + 2 UI components. Νέο weight adjustment → single edit στο `FIELD_WEIGHTS` matrix. Νέα media threshold → `MEDIA_THRESHOLDS`. Νέο field → add to `FIELD_KEYS` + extend matrix + extend `resolveFieldCredit` + add i18n label. operationalStatus policy change → `PRE_COMPLETION_EXEMPT_FIELDS` / `PRE_COMPLETION_PHOTO_FACTOR`.
  - **Design choices**:
    - Pure function separation from hook composition — ~30 deterministic unit tests on core logic without React/Firestore mocks.
    - localStorage for dismissal (V1 per-device) — avoids Firestore round-trip overhead για trivial UI toggle. Upgrade path documented.
    - Photo scoring curve per Spitogatos/Idealista market benchmarks (luxury units 8–12 photos baseline). Diminishing returns cap evitar photo-spam inflation.
    - Click-to-jump μόνο για cards εντός info tab — cross-tab navigation (photos/floorplan tabs) defer σε V2 (απαιτεί tab routing API).
    - `tabIndex={-1}` στις cards ώστε focus() λειτουργεί χωρίς να εμφανιστούν στο Tab order.
  - **Out of scope για V1**:
    - Videos scoring (VideosTabContent stub). Hook signature ready; ενεργοποίηση όταν videos tab wired.
    - Granular field IDs εντός card (π.χ. `field-bedrooms` για bedroom-specific focus). Card-level scroll είναι sufficient Google UX.
    - Identity/area field jumps (type/areaGross/areaNet anchors in PropertyFieldsEditForm identity card). V2 candidate.
    - Cross-device dismissal sync via `EnterpriseUserPreferencesService`. Upgrade path documented.
    - Undo toast post-dismiss με snapshot restore. Google pattern: explicit reshow link suffices.
    - A/B test thresholds per-region (Greek vs EU market nuances). V1 uses single Greek-market baseline.
    - Penalty για over-photo-spam (>bonusCap). V1 flat 1.0; V2 could down-weight to detect artificial inflation.
  - **Discovery**: Giorgio real-usage brainstorm — «Θέλω στοιχείο στο πάνω μέρος που δείχνει πόσο έχει συμπληρωθεί το ακίνητο, σαν LinkedIn/Google My Business». Confermata evaluation modal N.8 (~15 files, 4 domini) → Plan Mode sequenziale (non orchestrator) per coerenza stile Batch 21/24/27. Giorgio: «ΝΑ ΚΑΝΕΙΣ ΑΥΤΟ ΠΟΥ ΘΑ ΕΚΑΝΕ Η ΓΟΟΓΛΕ» — video skip V1 (stub), click-jump include, meter dentro PropertyFieldsBlock.
