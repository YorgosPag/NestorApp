...
| Ημ/νία | Αλλαγή |
|---|---|
| 2026-07-26 | **ADR-713 — το όριο ΟΓΚΟΥ αυτού του ADR παρέμεινε ΑΜΕΤΑΒΛΗΤΟ.** Προστέθηκε **δεύτερο, διακριτό** όριο για την **όψη**: η στάθμη τελειωμένου εδάφους. Πάνω της επένδυση+σοβάς· κάτω της στεγανωτική επάλειψη (νέα άρθρα `OIK-10.20/21/22` m²). Το κάτω άκρο της θαμμένης ζώνης **είναι** η άνω παρειά του πεδίλου — δηλαδή το όριο του §2.1 — και στο default (στάθμη = FFL) η θαμμένη ζώνη **ταυτίζεται** με το κοντόστυλο του `OIK-2.07`. Κανένα από τα δύο όρια δεν μετακινεί το άλλο· `geometry.volume`, `stubVolumeM3` και `baseDropMm` επαληθεύτηκαν αμετάβλητα με tests. |
