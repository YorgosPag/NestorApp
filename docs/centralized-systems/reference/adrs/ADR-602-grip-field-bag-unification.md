...
| **4** | 617 reads → `gripKindOf()`· batched disjoint (routing/forwarding/preview/renderers) | ~60 | **orchestrator** (disjoint batches) | 🟡 |
...
## 4. Επόμενο βήμα
Stages 1+2+3 ✅ DONE (uncommitted). **Επόμενο = Giorgio: tsc + commit + browser-verify** (Bug 1/2 perf ghost:
water-heater grip drag + mep-segment live ghost· S3 = pure additive, μηδέν behavior — αλλά μπαίνει στο ίδιο
commit ελέγχου) → μετά **Stage 4** (orchestrator, disjoint batches): τα **617 reads** `grip.xGripKind` →
`gripKindOf(grip, '<on>')`, batched (routing-dispatch/forwarding/preview/renderers) + το `gripKind` forward
στα 2 **3D commit bridges** (`toUnifiedGrip`/`toRawDxfUnifiedGrip`, deferred από S2/S3, §3.2) + τυχόν
consolidation του twin bridge-scaffold. Δες §3 πίνακα.
