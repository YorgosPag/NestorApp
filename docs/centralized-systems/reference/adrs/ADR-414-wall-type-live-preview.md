...
- **2026-06-04 (e)** — **Alt+click pivot fix** (Opus): το Alt+click «δεν δούλευε» στο preview γιατί (1) τα bands είναι λεπτή φέτα σε άδειο background → strict geometry pick αστοχούσε σιωπηλά, (2) καμία οπτική ένδειξη → η μετατόπιση pivot ανεπαίσθητη. Νέο SSoT `preview-pivot.ts`: `resolvePreviewPivot` (raycast bands → **fallback σε camera-facing plane** μέσα από το κέντρο, ώστε Alt+click οπουδήποτε να ορίζει pivot) + `PreviewPivotMarker` (crosshair `depthTest:false` που αναβοσβήνει 900ms στο νέο pivot, καθρέφτης του POI cross του κύριου viewport ADR-366 §A.6.Q5). Boy-Scout: αφαιρέθηκε το copy-pasted `setPivotAt` από wall + slab renderers. Εκτός ADR-040.
