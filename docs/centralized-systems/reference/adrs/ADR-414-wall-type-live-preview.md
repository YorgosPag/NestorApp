...
## Changelog
- **2026-07-16** — **Wall/slab preview de-duplication (N.18 / ADR-584, Opus 4.8).** Ένα full `jscpd:check` έδειξε ότι το **μεγαλύτερο clone ΟΛΟΥ του repo** (101 γρ. / 627 tokens) ήταν `WallTypePreviewRenderer` ↔ `SlabTypePreviewRenderer` (+45L/425T το `fitCamera`, +60L/370T τα δύο panels): το slab preview είχε φτιαχτεί με **αντιγραφή** του wall preview. Νέα SSoT: **`bim-3d/preview/band-stack-preview-renderer.ts`** (`BandStackPreviewRenderer<TDna,TBand>` — κατέχει WebGL context, φώτα, 3/4 κάμερα, το exact 8-corner `fitCamera`, το highlight outline, το band picking, το Alt+click pivot, resize, dispose) + **`ui/ribbon/components/BandStackPreviewPanel.tsx`** (κατέχει mount/dispose, prop→renderer effects, texture-version swap, pointer→NDC→pick, markup). **ΟΧΙ god-shell:** ό,τι διαφέρει πραγματικά δηλώνεται σε ένα μικρό **`BandStackPreviewSpec`** (`viewDir`, `fallbackThicknessM`, `buildBands`, `boxOf`, `halfExtents`) — για τον τοίχο: στρώσεις κατά **Z**, stub 1.2×1.0 m, `VIEW_DIR=(1.5,1.05,0.85)`, fallback 0.25 m (**ακριβώς ό,τι τεκμηριώνει το (b) παρακάτω — αμετάβλητο**). `WallTypePreviewRenderer` / `WallTypePreviewPanel` **κρατούν το API τους** (κλάση `new X(container)` / ίδια props) → **κανένα dialog δεν άγγιξε**. Όλες οι συμπεριφορές των (e)-(i) παρακάτω (pick μόνο band meshes, idempotent `setHighlight`, outline scale 1.0 + `depthTest:false` + `renderOrder 998` στο `bandGroup`, `setBoxWorldUvs`, `resolvePreviewPivot`) ζουν πλέον **μία φορά** στη βάση αντί για δύο. **Tests:** το `fitCamera` solve βγήκε σε νέο **`band-stack-fit.ts`** (`solveFitDistance`, pure — χωρίς WebGL) και τα specs έγιναν export → **51 νέα tests** (`__tests__/band-stack-fit.test.ts` = property tests με πραγματική κάμερα: όλες οι 8 γωνίες μέσα στο frustum + tightness + ακραία aspects· `__tests__/band-stack-preview-spec.test.ts` = regression lock από τους **προ-refactor** renderers, incl. το `VIEW_DIR=(1.5,1.05,0.85)` του (b) και «τα bands αθροίζουν στο DNA total thickness»). **Mutation-verified** (σκόπιμο bug → 2 tests έσπασαν → revert). **Verified: `jscpd:diff` 7 αρχεία = 0 clones· `jscpd:check` 3041→3035· 56/56 GREEN.** **ΟΧΙ tsc (N.17).** Εκτός ADR-040 (render-on-demand). ⚠️ **browser verify αξίζει ακόμα** — τα tests καλύπτουν geometry/fit, όχι pixels (υλικά/outline/UVs). Πλήρες entry: **ADR-584**.
...
- **2026-06-04 (i)** — **Rigid orbit-around-pivot, ΧΩΡΙΣ άλμα (SSOT με 3D)** (Opus): Ο Giorgio: το Alt+click πηδούσε το σχέδιο στο κέντρο (και στο preview)· θέλει «να μένει στη θέση του». `PreviewOrbitControls`: OrbitControls rotate OFF· custom Alt+left drag καλεί το κοινό `orbitCameraAroundPivot` (SSoT, `viewport/orbit-around-pivot.ts` — βλ. ADR-366 §A.6.Q5 v4)· `enablePan=false` όσο κρατιέται Alt (καθαρό boolean, όχι button-enum cast)· `setPivot` αποθηκεύει `customPivot` ΧΩΡΙΣ recenter. Το σημείο μένει καρφωμένο στην οθόνη. tsc 0. 🔴 browser verify + commit.
- **2026-06-04 (h)** — **Highlight flicker on hover fix** (Opus): Ο Giorgio: το περίγραμμα αναβόσβηνε συνεχώς όσο ο κέρσορας κινούνταν μέσα σε μία στρώση. **ΠΡΑΓΜΑΤΙΚΗ ΡΙΖΑ (regression από το (g))**: μεταφέροντας το outline μέσα στο `bandGroup`, το `pickLayerAt`/`setPivotAt` (που έκαναν `intersectObjects(bandGroup.children)`) άρχισαν να **στοχεύουν και το outline LineSegments**. Τα Line έχουν `Raycaster.params.Line.threshold` ~1 μονάδα (≈όλο το ~1m preview) → ο pick ταλαντωνόταν band↔outline (=null) ανά κίνηση → `picked` άλλαζε → ξανασχεδιασμός. **Fix:** το pick στοχεύει **ΜΟΝΟ τα band meshes** (`this.bands.map(b=>b.mesh)`), ποτέ outline/marker — και στις δύο συναρτήσεις, και στους δύο renderers. **+** idempotent `setHighlight` guard (`highlightedLayerId`, reset στο `clearOutline`) ως δεύτερη γραμμή άμυνας. tsc 0. 🔴 browser verify + commit.
- **2026-06-04 (g)** — **Highlight περίγραμμα στρώσης εφάπτεται ακριβώς** (Opus): Ο Giorgio: το γαλάζιο περίγραμμα της επιλεγμένης στρώσης δεν ταυτιζόταν με τα όριά της. ΡΙΖΑ: `outline.scale.setScalar(1.015)` το μεγάλωνε ~1.5% γύρω από το κέντρο → ξεχείλιζε (κυρίως στις φαρδιές διαστάσεις). **Fix:** scale 1.0 (ακριβές box edges) + `LineBasicMaterial depthTest:false, transparent` + `renderOrder 998` (καθαρό on-top, μηδέν z-fighting) + το outline μπαίνει στο **ίδιο parent** (`bandGroup`) με τα bands (shared coords, belt-and-suspenders). Wall + slab. tsc 0. 🔴 browser verify + commit.
