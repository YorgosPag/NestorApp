...
> αμετάβλητο). Αιτία: το κατώφλι των 3 px έκρυβε **44/49** γραμμοσκιάσεις σε πραγματικό τοπογραφικό.
> **Απόφαση 15** εξηγεί γιατί η ασυμμετρία είναι η προδιαγραφή και όχι παράβλεψη.
...
#### Το περιστατικό (Giorgio, 2026-07-28 — μετρημένο, όχι αναφορά χρήστη)

Τοπογραφικό ΕΓΣΑ87 (`47_ergasia.dxf`, 50 HATCH, ANSI31). Ο χρήστης: *«θέλω να έρχονται οι
γραμμοσκιάσεις όπως στο AutoCAD — δεν έχουν αυτά τα δοντάκια»*. Η **πρώτη** υποψία (ο import χάνει
το μοτίβο) **καταρρίφθηκε στα δεδομένα**: το round-trip βγάζει `53=304.2, 45=0.2068, 46=0.1405`
— **byte-ίδιο** με το AutoCAD (το `inlinePattern` του ADR-644 #7d δουλεύει). Η μέτρηση:

| Μέγεθος | Τιμή |
|---|---|
| Πραγματική απόσταση γραμμών | **140–210 mm** world |
| Zoom «όλο το σχέδιο» (587 m σε ~1200 px) | `scale ≈ 0,00204 px/mm` |
| Προβολή στην οθόνη | **0,23 – 0,43 px** |
| Κατώφλι Απόφασης 7 | **3 px** |

⇒ **~10× κάτω** ⇒ **44 από τις 49** γραμμοσκιάσεις κατέρρεαν σε tint. **Το LOD δούλευε ακριβώς
όπως σχεδιάστηκε — και αυτό ήταν το πρόβλημα.**
...
- **2026-07-28** — **Φ3.2 ΥΛΟΠΟΙΗΘΗΚΕ — ΤΟ LOD ΤΗΣ ΟΘΟΝΗΣ ΕΚΡΥΒΕ 44/49 ΓΡΑΜΜΟΣΚΙΑΣΕΙΣ.**
