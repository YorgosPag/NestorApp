...
| Πεδίο | Τιμή |
|---|---|
| **Status** | 🟢 **APPROVED** 2026-05-18 — Groups 1+2+3 + Boy-Scout Group 4 (2026-06-03). **Φ2 Μηχανισμοί 1/2/4 (§10.10–§10.13) + Φ3 μεταναστεύσεις Κ2/Κ2-β + Μηχ. 3 ratchet (§10.14) + closeout των 2 τελευταίων ωμών ιδιοκτητών (§10.14.Η) ολοκληρώθηκαν 2026-07-25.** Ζώνη Α άδεια στη διαδρομή ESC· ο Μηχ. 1 βγάζει `ok` σε αδρανή viewer έναντι επικυρωμένου θετικού control. **Το σάρωμα των 4 patterns δίνει πλέον ΜΟΝΟ τις 8 νόμιμες γραμμές — μηδέν ωμοί ιδιοκτήτες ESC στον viewer.** 🟢 **Η σκάλα group (275→408) ΕΠΑΛΗΘΕΥΤΗΚΕ ΖΩΝΤΑΝΑ** (2026-07-25, §10.14.Η.5): 1ο ESC → `group/exit-active-group`, 2ο → deselect, 3ο → null — με επικυρωμένο όργανο. #9 και έμμεσα #6 επίσης. 🟢 **2026-07-26 (§10.14.Η.5.β): #11 και #7 ΠΑΤΗΘΗΚΑΝ** (`tools/zoom-window`· `toolbar/zoom-controls` — με το `allowWhenEditable` **μετρημένο** μέσω `focusAt = input`), + bonus `ribbon/split-dropdown`. **#10 = ΔΟΜΙΚΑ ΑΝΕΦΙΚΤΟ** (κανένα tool δεν ορίζει `dropdownOptions` ⇒ ο split-button κλάδος του `ToolButton` είναι νεκρός κώδικας) — μην το ξανακυνηγήσεις. **Η ρίζα των 2 σημείων του §Η βρέθηκε**: `BimCommentDetailsPanel` + `Floating3DPanel` ήταν ορφανά (νεκρά αρχεία στο CHECK 3.30)· 3 features χτίστηκαν σε αποσυνδεδεμένο δοχείο με πράσινα tests. Διορθώθηκε (καρτέλες στον ζωντανό `Bim3DFloatingTab`, νεκρό δίδυμο διαγράφηκε, master-detail). 🔴 **2026-07-26 (§10.14.Η.5.γ): το lightbox ΔΕΝ είναι «αμέτρητο» — είναι ΜΗ ΠΡΟΣΒΑΣΙΜΟ.** Η αλυσίδα συνημμένων είναι νεκρή σε **3** ανεξάρτητα σημεία (`createComment` γράφει σκληρά `attachments: []`· το `updateComment` δεν καλείται από πουθενά· ο `CommentAttachmentUploader` δεν εισάγεται από πουθενά) ⇒ κανένα σχόλιο δεν αποκτά ποτέ συνημμένο ⇒ το slot δεν αποδίδεται σε **καμία** κατάσταση. Ίδιο αρχέτυπο με #10, αλλά **wiring που δεν έγινε ποτέ**, όχι νεκρή σχεδίαση. **Απόφαση Giorgio: ζωντάνεμα** → `HANDOFFS/2026-07-26_comment-attachments-wiring_handoff.md`. 🟢 **2026-07-26 — ΤΟ LIGHTBOX ΠΑΤΗΘΗΚΕ**: ESC με ανοιχτό lightbox + επιλεγμένη κολώνα 3D ⇒ **`consumedBy = 'bim-3d/comment-attachment-lightbox'`**, dialog 1→0, και **η 3D επιλογή επιβίωσε** ⇒ το `canvas/fallback-deselect` (400) δεν κατανάλωσε. Απαίτησε πρώτα διόρθωση κατάρρευσης του `BimCommentDetailsPanel` (άπειρος βρόχος `useSyncExternalStore`· ADR-366 §12). ⚠️ Το «θετικό control» του οργάνου δίνει `shadow-owner` **μόνο σε αδρανή κατάσταση** — ο bus ακούει σε window **capture** και προηγείται· μέτρηση με ιδιοκτήτη = **χωρίς** control. ✅ Ο φραγμός των authenticated routes **δεν αναπαράγεται** (200 σε 0,9–2,2s). 🟢 **2026-07-26 (§10.14.Η.5.δ): ΤΟ @-MENTION ΠΑΤΗΘΗΚΕ ΟΛΟΚΛΗΡΟ** — `aria-activedescendant`, ↓/↑ με wrap, Enter χωρίς νέα γραμμή, Escape που κρατά το κείμενο, **splice στον caret** με ουρά ανέπαφη, και dedup **επιβεβαιωμένο στη βάση** (3 mentions → 1 id). Μαζί πατήθηκαν και τα ← / → του lightbox. 🔴 **Δύο νέα ελαττώματα πλήκτρων**: (1) το **Tab είναι νεκρό μέσα στο modal** — `use3DShortcuts.ts:112` κάνει `preventDefault + stopPropagation` και ο μόνος του φύλακας είναι `isTypingInFormField`, **δεν ρωτά αν υπάρχει ανοιχτό modal** ⇒ τα κουμπιά πλοήγησης εικόνας είναι απρόσιτα από πληκτρολόγιο (APG)· (2) **το focus δεν επιστρέφει στη μικρογραφία** μετά το κλείσιμο (μηδέν κλήσεις `.focus()`, `activeElement=BODY`, ίδιο και με το κουμπί «Κλείσιμο»). 🟡 Ο Μηχ. 1 βγάζει `SHADOW-OWNER` στον **ίδιο του τον θεσμοθετημένο Κ3** (Escape λίστας mention) ⇒ θόρυβος. **Καμία διόρθωση δεν έγινε — μόνο μετρήσεις.** 🟢 **2026-07-26 (§10.15): ΚΑΙ ΤΑ 4 ΔΙΟΡΘΩΘΗΚΑΝ.** Νέο SSoT keyboard scope (**ADR-711**) τροφοδοτούμενο από τα κοινά Radix primitives· ο φύλακας των global accelerators έγινε **δομικός** (`addGlobalShortcutListener`, 7 listeners). Επαναμετρημένα ζωντανά: **Ε1** Tab ζωντανό σε modal σε **2D και 3D** (ίχνος 4/4, `dp=false`)· **Ε4** `ox/oy` αμετάβλητα με τα βέλη· **Ε2** focus επιστρέφει στον opener με 1 κλήση `.focus()`· **Ε3** ο Κ3 δηλώνεται και ο αδήλωτος εξακολουθεί να πιάνεται. 🟡 Εκκρεμεί **μόνο** η ζωντανή σιωπή της λίστας @-mention (HMR του παράλληλου agent μηδένισε 2× το project context). |
| **Date** | 2026-05-18 |
...
| Αρχείο | T1 | T2 | T3 |
|---|---|---|---|
| `src/lib/ui/inline-rename-keyboard.ts` (ως 2026-09-18: `ui/utils/`) (+3 καταναλωτές) | ✅ μόνο μέσω `onKeyDown` σε εστιασμένο `<input>` | ✅ κανένας καταναλωτής μέσα σε Radix layer | ✅ revert τοπικής τιμής / blur |
...
###### Η.5.δ Τέταρτη συνεδρία (2026-07-26) — @-mention **ΠΑΤΗΘΗΚΕ**· δύο ελαττώματα πλήκτρων εντοπίστηκαν

Αντικείμενο: οι **δευτερεύουσες** συμπεριφορές που ήταν πράσινες μόνο σε jest. Δημιουργήθηκε νέο
δοκιμαστικό σχόλιο με **δύο** συνημμένα (`probe-1.png` κόκκινο / `probe-2.png` μπλε — διακριτά
επίτηδες, ώστε η εικόνα να είναι **ανεξάρτητος μάρτυρας** και όχι μόνο ο τίτλος).

**🟢 Ό,τι επαληθεύτηκε ζωντανά**

| Μέτρηση | Απόδειξη |
|---|---|
| **← / → lightbox** | `Εικόνα 1 από 2: probe-1.png` ⇄ `Εικόνα 2 από 2: probe-2.png`· **και** το `alt` της εικόνας **και** η εναλλαγή των κουμπιών `Επόμενη`/`Προηγούμενη` (δύο ανεξάρτητοι μάρτυρες). Μετρήθηκε 2 φορές. |
| **ESC → κλείσιμο** | `consumedBy = 'bim-3d/comment-attachment-lightbox'`, `verdict ok`, `[role=dialog]` 1→0. |
| **@-mention picker** | Εμφανίζεται μετά από **37,8 s** (μετρημένο· το §3 του handoff έλεγε 19,5 s — **αργεί ακόμη περισσότερο**), 1 υποψήφιος. |
| **`aria-activedescendant`** | `_r_89_-mention-option-0` **στο textarea** ενώ το DOM focus μένει εκεί ⇒ το μοντέλο GitHub `combobox-nav` δουλεύει ζωντανά, όχι μόνο σε jsdom. |
| **↓ / ↑** | Καταναλώνονται από το textarea· wrap στον εαυτό τους με 1 υποψήφιο· **καμία νέα γραμμή**, **καμία διαρροή στον καμβά** (`ox` αμετάβλητο ⇒ ο φύλακας `isTypingInFormField` του `use3DShortcuts` κρατά). |
| **Enter** | Εισήγαγε `@VPR AGRO SPARES ` (17 χαρ.), `hasNewline === false`, picker κλειστός, caret 17. |
| **Escape στη λίστα** | listbox 1→0, `aria-controls`/`aria-activedescendant` καθαρά, **κείμενο διατηρήθηκε**, focus στο textarea. |
| **Splice στον caret** | Με caret στη **μέση** (17) και `@V` εκεί, το Enter αντικατέστησε **επιτόπου**: `@VPR… @VPR… και @VPR… ` — η **ουρά ανέπαφη**. Η περίπτωση «δύο mentions» δεν αλλοιώνεται. |
| **Ctrl+Enter + dedup** | Ανεξάρτητος μάρτυρας **στη βάση**: `bim_comments/…/replies` → `mentionedUserIds: ["eMAVv04g…"]` — **ένα** id για **τρία** mentions του ίδιου χρήστη. |

**🔴 Ελάττωμα 1 — το Tab είναι νεκρό μέσα στο modal (δικαιοδοσία §10.2, όχι Radix)**

Το `useEscapeHandler` δίνει στο lightbox το ESC, αλλά **κανείς δεν ρώτησε ποιος κατέχει το Tab**.
Ιχνηλατήθηκε σε 4 σημεία (window/document × capture/bubble):

- **3D**: το keydown `Tab` **σταματά στο window capture** — δεν φτάνει ποτέ σε document.
  Πηγή: `bim-3d/shortcuts/use3DShortcuts.ts:112` (window **capture**) → `preventDefault()` +
  `stopPropagation()` όταν ο dispatcher επιστρέψει `handled` (Tab → `ACTION_FOCUS_NEXT_3D`).
- **2D**: προπαγανδίζεται και στα 4 σημεία αλλά φτάνει **ήδη `defaultPrevented`** (`cycleSnap`,
  `config/keyboard-shortcuts.ts:774`).

Ο **μόνος** φύλακας του `use3DShortcuts` είναι `isTypingInFormField(document.activeElement)` —
**καμία ερώτηση «υπάρχει ανοιχτό modal;»**. Μέσα στο lightbox το focus κάθεται σε `<button>`,
όχι σε form field, άρα ο φύλακας δεν πιάνει. Αντίθεση που κλειδώνει τη διάγνωση: τα **βέλη**
προπαγανδίζονται (μόνο `preventDefault`, `handled=false`) γι' αυτό και φτάνουν στον React handler
του lightbox· το **Tab** κόβεται (`handled=true`).

⇒ Το dialog έχει **2 ορατά, ενεργά tabbable κουμπιά** (`Κλείσιμο`, `Επόμενη εικόνα`) και το Tab
δεν μπορεί να πάει από το ένα στο άλλο. Το «Επόμενη/Προηγούμενη εικόνα» είναι **απρόσιτο από
πληκτρολόγιο**. Παραβίαση WAI-ARIA APG (dialog: Tab κυκλώνει τα εστιάσιμα του διαλόγου).
Το σχόλιο του `CommentAttachmentLightbox.tsx:12-14` («Radix … Gives focus trap») είναι **σωστό
για τον Radix και άσχετο με το πραγματικό εμπόδιο** — ο Radix δεν βλέπει ποτέ το πλήκτρο.

⚠️ **Όριο της μέτρησης, δηλωμένο**: η **άμεση** παρατήρηση «το focus δεν μετακινήθηκε» έγινε με
`document.hasFocus() === false` (το παράθυρο Chrome ήταν σε φόντο), κατάσταση στην οποία ο Chrome
**δεν εκτελεί καθόλου** διαδοχική πλοήγηση focus — άρα από μόνη της **δεν αποδεικνύει** τίποτα.
Αυτό που αποδεικνύει το εύρημα είναι το **ίχνος** (`preventDefault` στο keydown `Tab`), που είναι
ανεξάρτητο του OS focus: κατά προδιαγραφή ακυρώνει τη διαδοχική πλοήγηση. Θετικό control σε
καθαρό iframe (χωρίς listeners της εφαρμογής) με `hasFocus === true`: το Tab **μετακίνησε**
`b1 → b2` ⇒ το όργανο μετράει.

**🔴 Ελάττωμα 2 — το focus ΔΕΝ επιστρέφει στη μικρογραφία**

Μετρήθηκε με patch στο `HTMLElement.prototype.focus`: κατά το άνοιγμα καταγράφεται **μία** κλήση
(`BUTTON[Κλείσιμο]` — ο Radix εστιάζει σωστά), κατά το κλείσιμο **καμία απολύτως**. Το
`document.activeElement` καταλήγει `BODY` ενώ η μικρογραφία παραμένει `isConnected`.
**Ίδιο αποτέλεσμα και με το κουμπί «Κλείσιμο»** ⇒ δεν είναι ελάττωμα της διαδρομής ESC, είναι
της επαναφοράς focus συνολικά. Η μέτρηση είναι **έγκυρη παρά** το `hasFocus=false`, γιατί η
προγραμματική `.focus()` λειτουργεί ανεξάρτητα (αποδείχθηκε επανειλημμένα στην ίδια συνεδρία).
⇒ Ο ισχυρισμός «focus restore to the trigger thumbnail» του `CommentAttachmentLightbox.tsx:12-13`
**δεν ισχύει στην πράξη**.

**🟡 Παρατήρηση 3 — ο audit βγάζει `SHADOW-OWNER` στον ίδιο του τον θεσμοθετημένο Κ3**

Το Escape που κλείνει τη λίστα mention δίνει `verdict: 'shadow-owner'`, `consumedBy: null`,
`focusAt: textarea…`. Είναι ο **νόμιμος** τοπικός Κ3 handler του `CommentReplyInput` (§10.5 Κ3),
όχι παραβίαση — αλλά ο Μηχ. 1 τυπώνει `console.error` σε **κάθε** χρήση. Ο ίδιος ο Giorgio το
είδε ως σφάλμα κονσόλας μέσα στη συνεδρία. Ο audit δεν διακρίνει «τοπικός Κ3 σε εστιασμένο
editable» από «ωμός ιδιοκτήτης» ⇒ **θόρυβος που εκπαιδεύει στην αγνόηση του Μηχ. 1**.

**Παράπλευρο, μετρημένο**: τα ← / → μέσα στο lightbox μετακινούν **και** το 2D viewport
(`ox` 542→462→542, και 30→−50 σε δεύτερη μέτρηση = ±80 px ανά πάτημα). Το modal δεν απομονώνει
το υπόβαθρο — ο lightbox κάνει `preventDefault()` χωρίς `stopPropagation()`, και ο global
pan handler δεν ελέγχει `defaultPrevented`.

**Όργανα — διορθώσεις που κόστισαν και σε αυτή τη συνεδρία**

1. Το `computer` tool δέχεται **screenshot coords** (`page × 1568/innerWidth`). Επιβεβαιώθηκε
   **με μέτρηση**, όχι με το μάτι: `hover(1000,400)` → `clientX/Y = (1531,612)`. Η οπτική εκτίμηση
   θέσης δρομέα από screenshot είναι **αναξιόπιστη** — μην τη χρησιμοποιείς για βαθμονόμηση.
2. Πριν από κάθε κλικ: `document.elementFromPoint(px,py)` — επιστρέφει **ποιο** στοιχείο θα δεχτεί
   το κλικ. Ισχυρότερο από τυφλή κλιμάκωση· καταργεί τον κίνδυνο του §5 (αδέσποτο κλικ).
3. Ο Radix Select **δεν** ανοίγει με προγραμματικό `Enter` αξιόπιστα· ανοίγει με πλήρη ακολουθία
   `pointerover→pointerdown→mousedown→pointerup→mouseup→click`.
4. **Συνύπαρξη με άλλον πράκτορα**: κάθε αποθήκευση σε `src/i18n/locales/**` προκαλεί πλήρη
   επαναφόρτωση Turbopack (~κάθε 30-45 s) που **σβήνει όλα τα `window.__*`**. Αντίμετρο που
   δούλεψε: `Object.defineProperty(Location.prototype,'reload',{value(){}, configurable:true})`
   αμέσως μετά το `navigate` (όσο `readyState === 'loading'`).
5. Το «ΕΡΓΟ Α φαίνεται στο `document.body.innerText`» **δεν** σημαίνει «επιλέχθηκε» — μπορεί να
   το γράφει το ίδιο το dialog. Έλεγχος: **breadcrumb** ή, καλύτερα, ότι φορτώνουν σχόλια.

---

## §10.15 — Ιδιοκτησία πληκτρολογίου σε modal + επαναφορά focus (ΔΙΟΡΘΩΣΗ, 2026-07-26)

Οι μετρήσεις του §10.14.Η.5.δ **δεν επαναλαμβάνονται εδώ** — δείξε σε αυτό. Εδώ είναι η
διόρθωση των τεσσάρων ελαττωμάτων και η **ζωντανή επαναμέτρηση**.

### Α. Η κοινή ρίζα Ε1/Ε4 — μία ερώτηση που κανείς δεν έκανε

Οι 43 window-level keydown listeners του viewer ρωτούσαν **μόνο** «γράφει ο χρήστης σε πεδίο;».
Μέσα σε modal το focus κάθεται σε `<button>`, άρα ο φύλακας δεν έπιανε: το `use3DShortcuts`
κατανάλωνε το `Tab` με `preventDefault + stopPropagation`, και το `useKeyboardShortcuts`
μετακινούσε το viewport ±80px με τα βέλη πίσω από το ανοιχτό lightbox.

**Απορρίφθηκε το `inert`** (ήταν η υποψήφια λύση του handoff §5.2): αφαιρεί focusability και
hit-testing **του υποδέντρου**, αλλά **δεν** σταματά listeners σε `window`/`document`. Το dialog
ζει σε portal **εκτός** του inert υποδέντρου ⇒ το keydown φτάνει άθικτο στο window capture.
Χρήσιμο συμπλήρωμα, **όχι** λύση — μην το ξανακυνηγήσεις.

Καμία βιβλιοθήκη δεν δίνει «αναστολή global shortcuts» δωρεάν· ο κλάδος (TanStack Hotkeys,
react-hotkeys-hook `HotkeysProvider`, modal command state των Revit/AutoCAD) το λύνει με **scope
stack**. Υλοποιήθηκε ντόπια, χωρίς νέα εξάρτηση (N.5 δεν ενεργοποιείται): **ADR-711**.

### Β. Το SSoT εύρημα που καθόρισε το μέγεθος

Ο φύλακας «γράφω σε πεδίο;» υπήρχε σε **δέκα και πλέον αντίγραφα** με **τέσσερα ονόματα**
(`isTypingInFormField`, `isEditableFocus`, `isInputFocused`, + inline παραλλαγές). Το `ssot:discover`
είναι name-based ⇒ **τυφλό** σε αυτό. Η δεύτερη ερώτηση («κατέχει modal το πληκτρολόγιο;») ανήκει
**ακριβώς εκεί**: `@/lib/a11y/keyboard-scope.ts` (ADR-711). Το
`bim-3d/ui/is-typing-in-form-field.ts` έγινε λεπτό re-export.

⚠️ **Ο bus ΔΕΝ αλλάζει συμπεριφορά**: το `EscapeCommandBus` καταναλώνει **μόνο** `isEditableTarget`
και **ποτέ** `shouldGlobalShortcutYield` — ο bus οφείλει να δουλεύει ΜΕΣΑ στα modals, εκεί ζει το
slot `MODAL_DIALOG`. Φύλακας modal στον bus θα σκότωνε το ίδιο το ESC των διαλόγων.

### Γ. Ε3 — ο Κ3 δηλώνεται, δεν σιωπά

Ο Μηχ. 1 έκρινε `shadow-owner` κάθε ESC χωρίς slot του bus — άρα και τον **θεσμοθετημένο Κ3**
του §10.5 (λίστα @-mention μέσα στο textarea). Νέο export `noteLocalEscapeOwner(nativeEvent, id)`:
ο Κ3 δηλώνεται, το εύρημα αναφέρει `consumedBy: 'local:<id>'`, το verdict γίνεται `ok`.

**Γιατί δήλωση και όχι σιωπή**: η φθηνή εναλλακτική («μη κρίνεις shadow-owner σε editable focus»)
θα ευλογούσε σιωπηλά **κάθε** ωμό αδήλωτο ιδιοκτήτη στην ίδια κατάσταση — τύφλωση ακριβώς εκεί που
χρειάζεσαι τον έλεγχο. Ο αδήλωτος **εξακολουθεί** να βγαίνει `shadow-owner`, και ο `starved` δεν
καλύπτεται από δήλωση (test-καρφωμένο).

### Δ. ΖΩΝΤΑΝΗ ΕΠΑΝΑΜΕΤΡΗΣΗ (2026-07-26)

Βαθμονόμηση: **K = 1,0** αυτή τη συνεδρία (`hover(1000,400)` → `clientX = 1000`, επαληθευμένο και
στο x = 2327). 🔴 **Ο τύπος `1568/innerWidth` του παλιού §2.1 έδινε 0,653 — ΛΑΘΟΣ.** Βαθμονόμησε
κάθε συνεδρία· μην αντιγράψεις τον λόγο.

| # | Μέτρηση | Αποτέλεσμα |
|---|---|---|
| **Ε1 (2D)** | modal ανοιχτό, `hasFocus=true`, πραγματικό Tab | Ίχνος **και στα 4** σημεία, `dp=false` στο window capture ⇒ **κανείς global δεν κατανάλωσε**. Focus `BUTTON[Κλείσιμο] → BUTTON[👁 Ορατά]`. 🟢 |
| **Ε1 (3D)** | ίδιο, σε 3D mode — **η αρχική ρίζα** (`use3DShortcuts`) | Ίχνος **και στα 4** σημεία (πριν: κοβόταν στο window capture), `dp=false`, focus `👁 Ορατά → 🔒 Κλειδωμένα`, **εντός** διαλόγου. 🟢 |
| **Ε2** | άνοιγμα με focus σε κουμπί, κλείσιμο | `document.activeElement === opener` και **1** κλήση `.focus()` πάνω του. Πριν: **μηδέν** κλήσεις, `activeElement = BODY`. 🟢 |
| **Ε4** | 3 πατήματα ← / → με modal ανοιχτό | `ox=615, oy=44` **αμετάβλητα** (πριν: ±80px/πάτημα)· `dp=false` σε όλα τα σημεία. 🟢 |
| **Ε3** | δήλωση Κ3 | jest: δηλωμένος ⇒ `ok`· **αδήλωτος ⇒ ακόμη `shadow-owner`**· `starved` δεν καλύπτεται. **Ζωντανά επιβεβαιώθηκε το θετικό control**: `__escapeAudit.last()` έδωσε `shadow-owner` για το ESC που έκλεισε τον Διαχειριστή Στρώσεων — αδήλωτος ιδιοκτήτης, σωστά επισημασμένος. 🟡 |

🟡 **Ε3 — ΤΙ ΔΕΝ ΜΕΤΡΗΘΗΚΕ ΖΩΝΤΑΝΑ (ρητά, όχι σιωπή)**: ότι το ESC της λίστας @-mention δεν
τυπώνει πλέον. Η διαδρομή απαιτεί επιλεγμένο έργο, και το `ProjectHierarchyContext`
**μηδενίστηκε δύο φορές** από HMR remount του παράλληλου agent (ADR-706) — μετρημένο: τα
`window.__*` **επιβίωσαν** (άρα remount, όχι document reload), αλλά ο πίνακας γύριζε στα
«Επίπεδα» και το breadcrumb χανόταν. Εκκρεμεί όταν σταματήσει ο παράλληλος agent.

### Ε. Νέο ζωντανό εύρημα (εκτός εύρους — καταγράφεται, δεν διορθώνεται)

Το `AdminLayerManagerDialog` βασίζεται στο **ενσωματωμένο ESC του Radix** και δεν δηλώνει slot
`ESC_PRIORITY.MODAL_DIALOG` ⇒ ο Μηχ. 1 το βγάζει σωστά `shadow-owner`. Είναι υπαρκτός Κ2 εκτός
SSoT, **προϋπάρχων**. Δεν αγγίχθηκε — ανήκει σε δικό του πέρασμα.

### ΣΤ. Τι απαιτεί ζωντανή μέτρηση για πάντα (jsdom τυφλό)

- Η **διαδοχική πλοήγηση focus** με Tab (το jsdom δεν την εκτελεί).
- Η **μετακίνηση `ox`/`oy`** του viewport (δεν αποδίδεται canvas).
- Το **κληρονομημένο `contenteditable`** (`isContentEditable` δεν υλοποιείται στο jsdom).
- ⚠️ **Μετρημένο κατά τη συγγραφή**: το `FocusScope` του Radix εκπέμπει το `AUTOFOCUS_ON_UNMOUNT`
  μέσα σε `setTimeout(…, 0)` ⇒ η επαναφορά focus **δεν είναι σύγχρονη**. Σύγχρονο assert βγάζει
  πάντα `body` και διαβάζεται λανθασμένα ως «δεν δουλεύει». Κάθε test χρησιμοποιεί `await waitFor`.

### Ζ. §10.15.β — Η ΔΗΛΩΣΗ Κ3 ΕΓΙΝΕ **ΚΛΑΣΗ**, ΟΧΙ ΠΕΡΙΠΤΩΣΗ (2026-08-10)

Το §10.15/Ε3 έδωσε στον Κ3 **τρόπο να δηλωθεί** (`noteLocalEscapeOwner`). Επί τρεις μήνες είχε
**έναν** χρήστη σε ολόκληρο το αποθετήριο (`bim-3d/comments/CommentReplyInput.tsx:137`), ενώ τα
Radix layers — ο **πολυπληθέστερος** νόμιμος ιδιοκτήτης — δεν είχαν κανέναν.

🔴 **Το προηγούμενο handoff ΣΥΝΗΓΑΓΕ ότι «κάθε Radix dialog τυπώνει SHADOW-OWNER» από ΕΝΑ
περιστατικό. Μετρήθηκε πριν αλλάξει κοινή UI ολόκληρης της εφαρμογής**, και η συναγωγή ήταν σωστή
αλλά **στενή**: όχι δύο αρχεία, **οκτώ**. Και τα οκτώ δίνουν `verdict = 'shadow-owner'` με
`defaultPrevented = true` σε αδρανή bus:

`dialog` · `popover` · `sheet` · `alert-dialog` · `dropdown-menu` · `context-menu` · `select` ·
`tooltip` (+ το `multi-combobox`, που χρησιμοποιεί το **ωμό** primitive και δεν κληρονομεί τίποτα).

Κλείνοντας μόνο τα δύο που ονόμαζε το handoff, η κλάση θα έμενε ανοιχτή — και ο έλεγχος θα
συνέχιζε να εκπαιδεύει στην αγνόηση, που είναι **ακριβώς** ο τρόπος με τον οποίο πεθαίνει ο Μηχ. 1.

**Η θεραπεία είναι ΕΝΑ module**: `components/ui/radix-escape-ownership.ts` — `withRadixEscapeOwner(id, callerHandler)`.

| Απόφαση | Γιατί |
|---|---|
| **Δήλωση**, όχι σιωπή | Το «μη κρίνεις shadow-owner όταν υπάρχει ανοιχτό modal» θα ευλογούσε **σιωπηλά** και κάθε ωμό αδήλωτο ιδιοκτήτη στην ίδια κατάσταση — τύφλωση ακριβώς εκεί που χρειάζεται ο έλεγχος |
| **Ένα** module, όχι εννιά αντίγραφα | Μια απάντηση γραμμένη εννιά φορές είναι εννιά απαντήσεις που αποκλίνουν την ημέρα που αλλάξει η μία. Δευτερευόντως: **ένα μόνο** αρχείο του `components/ui` φτάνει στο υποσύστημα του viewer |
| **Συνθέτει**, δεν παρακάμπτει | Ο καλών εξακολουθεί να μπορεί να κάνει `preventDefault()` και να **ακυρώσει το κλείσιμο** (άγκυρα `Κ4`). Η δήλωση είναι παρατήρηση, όχι απόφαση |

⚠️ **Η άγκυρα `Κ0` είναι η απόδειξη ζωής και δεν είναι διακοσμητική**: **παράγει** την κατάσταση
`shadow-owner` με τα ίδια εργαλεία που την κρίνουν, και η `Κ0β` δείχνει ότι η δήλωση τη θεραπεύει.
Χωρίς αυτές, μια αλλαγή σε κοινή UI ολόκληρης της εφαρμογής θα στηριζόταν σε **συναγωγή**
(ADR-749 §5 — φρουρός χωρίς απόδειξη ζωής).

⚠️ **Παγίδα που πληρώθηκε γράφοντας τις άγκυρες**: η πρώτη γραφή του βοηθού έριχνε το συμβάν με
`window.dispatchEvent`. Στόχος είναι τότε το **ίδιο το `window`**, άρα η διαδρομή διάδοσης είναι
**μόνο** αυτό και οι ακροατές του `document` — δηλαδή **ο Radix** — δεν τρέχουν καθόλου. Η μέτρηση
απάντησε «καμία βλάβη»: το σχήμα «0 = κανείς δεν κοίταξε», μέσα στο test που το κυνηγά.

⚠️ **Το `context-menu` δηλώθηκε αλλά ΔΕΝ αγκυρώθηκε**: το `ContextMenu.Root` του Radix δεν δέχεται
ελεγχόμενο `open`, οπότε δεν ανοίγει χωρίς πραγματικό δεξί κλικ. Δηλωμένο κενό, με όνομα.

**Άγκυρες**: `components/ui/__tests__/radix-escape-ownership.test.tsx` (`Κ0`·`Κ0β`·`Κ1`·`Κ2`·`Κ2β`×5·`Κ3`·`Κ4`)
⇒ **11 πράσινα**. `src/components` ⇒ **1.093 πράσινα / 86 suites** (η 87η αποτυγχάνει από
προϋπάρχον `fetch is not defined` του `@firebase/auth`, άσχετο με αυτή την αλλαγή).

### Η. §10.15.γ — ΕΝΑ SSoT ιδιοκτησίας Escape για ΟΛΗ την εφαρμογή· ο αόπλος bus δεν λιμοκτονεί (2026-09-11)

**Αφορμή (ADR-332 D27 βήμα 2, ADR-241, μετρημένα ζωντανά)**: (α) σε πλήρη οθόνη **ένα** Esc με Select / μενού /
διάλογο ανοιχτό έκλεινε **και τα δύο** — σε 4 καταναλωτές **και στον DXF**, όπου ο bus κλήθηκε, κανένα slot δεν
κατανάλωσε και το πάτημα πέρασε στον ωμό listener του `useFullscreen`· (β) το «1 Issue» του Next overlay ήταν
`starved` σε σελίδες **εκτός** viewer (Κτίρια: `GanttPortals` → `dxf-viewer/ui/color` → `eyedropper` → bus) — η σεντινέλα
εγκαθίσταται στο import, ο listener του bus **ποτέ**, άρα **κάθε** Esc κρινόταν λιμοκτονία, ακόμη και με δηλωμένο ιδιοκτήτη.

| Απόφαση | Γιατί |
|---|---|
| **`@/lib/a11y/escape-layers`** — LIFO στοίβα + **ένας** listener `document` bubble + `claimEscape` / `escapeClaimOf` | Εκτός viewer δεν υπάρχει κλίμακα προτεραιοτήτων — υπάρχει **σειρά ανοίγματος** (top layer: ο νεότερος νικά). Η σειρά ιδιοκτησίας προκύπτει **από κατασκευής**: slot bus (window capture + `stopImmediatePropagation`) → Radix (document capture + `preventDefault`) → ιδιοκτήτης στο στοιχείο → **στοίβα** = τελευταία λύση, «κάτω από `TOOL_RESET`» χωρίς slot. **Δεν είναι δεύτερος bus.** |
| Η στοίβα **δηλώνει** ό,τι χειρίζεται (`preventDefault` + `claimEscape`) | Για να παραιτούνται οι εξωτερικότεροι· και για να μην κρίνει ο Μηχ. 1 `shadow-owner` κάθε έξοδο από πλήρη οθόνη στον DXF (άγκυρα **B2**). |
| **Ένα** SSoT δηλώσεων: `withRadixEscapeOwner` → `claimEscape`· `noteLocalEscapeOwner` → `claimEscape`· η `judge` διαβάζει `localOwner ?? escapeClaimOf(e)` | Ως τώρα η δήλωση ζούσε σε ιδιωτικό `WeakMap` του ελέγχου, και το `components/ui` εισήγαγε από το subapp. Η φορά της εξάρτησης είναι πλέον **subapp → lib**. Ο πρώτος δηλών νικά (ο εσώτερος δηλώνει νωρίτερα — A-M7). |
| Νέα κρίση **`unarmed`** — πρώτη στη σειρά, **στο ιστορικό, όχι στην κονσόλα** | «Ο bus δεν ακούει εδώ» ≠ «ο bus λιμοκτόνησε». «Άκουγε» αποδεικνύεται με **δύο** τρόπους: η σημαία οπλισμού (`noteBusListenerArmed`, από το `installListener`/`removeListener`, καταγεγραμμένη **τη στιγμή του πατήματος**) **ή** το ότι ο bus **κλήθηκε** για το πάτημα — ο δεύτερος είναι ισχυρότερος (ένας bus που κλήθηκε, άκουγε). ⚠️ Η σεντινέλα **μένει** πρώτη στο window capture (εγκατάσταση στο import) — καταγράφεται η **κατάσταση**, δεν μετακινείται ο φρουρός. ⚠️ Η πρώτη εκδοχή έκρινε μόνο τη σημαία, και οι άγκυρες των Radix wrappers (που προσομοιώνουν την **κλήση**) χρειάστηκαν νέο δημόσιο σύμβολο — το CHECK 3.62 το μπλόκαρε ως μεγέθυνση της δημόσιας επιφάνειας· σωστά: το λάθος ήταν στο κριτήριο, όχι στην επιφάνεια. |
| `useEscapeKey` = λεπτό περιτύλιγμα της στοίβας, handler σε ref | Οι 4 καλούντες (πλήρης οθόνη, NotificationDrawer, lightbox συνημμένων, BaseModal του DXF) παραιτούνται πλέον από κάθε εσώτερο ιδιοκτήτη· νέο closure ανά render **δεν** ανεβάζει τη στρώση στην κορυφή (A5). |

**Άγκυρες**: `lib/a11y/__tests__/escape-layers` (A1–A4, 12) · `hooks/__tests__/useEscapeKey` (A5, 4) ·
`escape-dev-audit` (+B1/B2 + «κλήθηκε ⇒ άκουγε», 17) · `radix-escape-ownership` (11, αμετάβλητες) ·
`core/containers/__tests__/fullscreen-surface-behavior` (A6/A7).
**Μεταλλάξεις**: A 8/8 · A2 3/3 · B 5/5. ⚠️ Δύο επέζησαν στην πρώτη γραφή των αγκυρών (A-M7 «ο τελευταίος σβήνει τον
πρώτο» · A5-M1 «handler στα deps», επειδή το test περνούσε νέο closure σε **όλες** τις στρώσεις) — οι άγκυρες
διορθώθηκαν, όχι ο κώδικας.

## 11. Changelog

| Date | Change | Author |
|---|---|---|
| 2026-09-11 | **§10.15.γ — ένα SSoT ιδιοκτησίας Escape για όλη την εφαρμογή · κρίση `unarmed`.** `@/lib/a11y/escape-layers` (LIFO στοίβα + claim) ως **τελευταία λύση** κάτω από bus και Radix — όχι δεύτερος bus· `useEscapeKey` πάνω της· `withRadixEscapeOwner` και `noteLocalEscapeOwner` δηλώνουν στο lib (φεύγει η εισαγωγή `components/ui → subapp`)· ο έλεγχος διαβάζει τις δηλώσεις από εκεί. `unarmed` όταν ο listener του bus δεν ήταν οπλισμένος τη στιγμή του πατήματος (το «1 Issue» σε σελίδες εκτός viewer) — ιστορικό ναι, κονσόλα όχι. Κλείνει το διπλό Escape της πλήρους οθόνης (ADR-241, 4 καταναλωτές + DXF). Μεταλλάξεις A 8/8 · A2 3/3 · B 5/5. | Claude Opus 5 + Γιώργος |
| 2026-08-10 | **§10.15.β — Η δήλωση Κ3 έγινε ΚΛΑΣΗ.** Μετρήθηκε (πριν αλλάξει κοινή UI) ότι **οκτώ** Radix wrappers παράγουν `shadow-owner`, όχι δύο· το handoff το είχε **συναγάγει** από ένα περιστατικό. Νέο SSoT `components/ui/radix-escape-ownership.ts` (`withRadixEscapeOwner`) — **ένα** module, **εννιά** σημεία κλήσης, καμία αντιγραμμένη λογική. Η δήλωση **συνθέτει** τον handler του καλούντα και δεν του κλέβει το `preventDefault()` (`Κ4`). `Κ0`/`Κ0β` = απόδειξη ζωής (παράγουν και θεραπεύουν την κατάσταση). ⚠️ Το `context-menu` δηλώθηκε αλλά **δεν αγκυρώθηκε** (το Radix Root δεν δέχεται ελεγχόμενο `open`). ⚠️ Παγίδα: `window.dispatchEvent` **δεν** φτάνει στους ακροατές του `document` ⇒ η πρώτη μέτρηση έλεγε ψευδώς «καθαρό». 11 άγκυρες· `src/components` 1.093 πράσινα. | Claude Opus 5 + Γιώργος |
| 2026-07-26 | **§10.15 — ΤΑ 4 ΕΛΑΤΤΩΜΑΤΑ ΔΙΟΡΘΩΘΗΚΑΝ ΚΑΙ ΕΠΑΝΑΜΕΤΡΗΘΗΚΑΝ ΖΩΝΤΑΝΑ.** Νέο SSoT `@/lib/a11y/keyboard-scope` (**ADR-711**): ένα scope stack + ένα predicate, τροφοδοτούμενο από `dialog.tsx` / `alert-dialog` / `sheet` (**170 αρχεία, ΕΝΑ σημείο**). **Ε1** — το `Tab` ζει ξανά μέσα σε κάθε modal, μετρημένο σε **2D και 3D**: ίχνος και στα 4 σημεία με `dp=false` στο window capture, focus κυκλώνει **εντός** διαλόγου. **Ε4** — 3 πατήματα βελών με modal ανοιχτό: `ox/oy` **αμετάβλητα** (πριν ±80px). **Ε2** — επαναφορά focus στον opener με **1** κλήση `.focus()` (πριν: μηδέν, `BODY`)· ρίζα: το modal `DialogContent` του Radix εστιάζει **μόνο** `triggerRef`, που είναι `null` σε **161/170** αρχεία. **Ε3** — `noteLocalEscapeOwner()`: ο Κ3 του §10.5 **δηλώνεται** αντί να σιωπά· αδήλωτος ωμός ιδιοκτήτης **εξακολουθεί** να βγαίνει `shadow-owner` (επιβεβαιωμένο **και ζωντανά**). Το `inert` **απορρίφθηκε μετρημένα** (δεν σταματά window listeners· το portal είναι εκτός υποδέντρου). 7 listeners μετανάστευσαν στο `addGlobalShortcutListener` — ο φύλακας έγινε **δομικός**. 🟡 Το ζωντανό μισό του Ε3 (σιωπή στη λίστα @-mention) **εκκρεμεί**: το `ProjectHierarchyContext` μηδενίστηκε 2× από HMR remount του παράλληλου agent. |
...
| 2026-07-26 | **🟢 §10.14.Η.5.γ — ΤΟ LIGHTBOX ΠΑΤΗΘΗΚΕ. Ο ισχυρισμός «ο bus κερδίζει το `canvas/fallback-deselect` στο 400» είναι πλέον ΜΕΤΡΗΜΕΝΟΣ, όχι συμπερασμένος από jest.** Σενάριο ακριβώς όπως προδιαγράφηκε: σχόλιο **με εικόνα** → κλικ μικρογραφίας → lightbox ανοιχτό (`aria-label="Εικόνα 1 από 1: adr366-attachment-test.png"`) → **ΜΕ επιλεγμένη κολώνα 3D** (gizmo ορατό, ribbon «Ιδιότητες Κολώνας») → ESC. **Αποτέλεσμα: `consumedBy === 'bim-3d/comment-attachment-lightbox'`, verdict `ok`.** Δύο ανεξάρτητα σκέλη απόδειξης, όχι ένα: (1) `[role="dialog"]` **1 → 0** ⇒ ο ιδιοκτήτης όντως εκτέλεσε· (2) **η 3D επιλογή ΕΠΙΒΙΩΣΕ** (`Ιδιότητες Κολώνας` ακόμα παρόν) ⇒ το `canvas/fallback-deselect` (400) **δεν** κατανάλωσε. Δεύτερο ESC στην ίδια κατάσταση (lightbox κλειστό, κολώνα ακόμα επιλεγμένη) → `bim3d/edit-gizmo-teardown`, δηλαδή η αλυσίδα προτεραιότητας κατεβαίνει σωστά. ⚠️ **ΔΙΟΡΘΩΣΗ ΤΟΥ ΟΡΓΑΝΟΥ — σημαντικό για κάθε επόμενη μέτρηση**: το «θετικό control» του §1 (listener στο `window` που κάνει `preventDefault`) **ΔΕΝ παράγει `shadow-owner` όταν υπάρχει πραγματικός ιδιοκτήτης**, γιατί ο bus ακούει σε **window capture** και τρέχει **πριν** τον bubble listener του control. Δίνει `shadow-owner` μόνο σε αδρανή κατάσταση. Άρα: **το control επικυρώνει το όργανο μόνο όταν κανείς δεν καταναλώνει· η μέτρηση σε κατάσταση με ιδιοκτήτη πρέπει να γίνεται ΧΩΡΙΣ control** (έγινε: καθαρή επανάληψη, ίδιο αποτέλεσμα). 🔴 **Η μέτρηση ήταν αδύνατη μέχρι να διορθωθεί ένα τρίτο, ανεξάρτητο νεκρό σημείο**: το `BimCommentDetailsPanel` — ο **μόνος** γονέας του lightbox — έριχνε **ολόκληρο τον DxfViewer** (`Maximum update depth exceeded` → error boundary) στο **πρώτο κλικ σε οποιοδήποτε σχόλιο**, λόγω `useBimCommentsStore((s) => s.replies[id] ?? [])`: φρέσκο `[]` ανά κλήση ⇒ το `useSyncExternalStore` το έβλεπε πάντα ως αλλαγή. Διορθώθηκε στο store (`NO_REPLIES` + `selectRepliesFor()`) με 5 regression tests· πλήρης καταγραφή στο **ADR-366 §12, εγγραφή 2026-07-26**. Δηλαδή το §Η.5.γ είχε **τρεις** διαδοχικούς φραγμούς: νεκρό wiring συνημμένων → κατάρρευση του γονέα → και μόνο μετά, η μέτρηση. ✅ **Ο φραγμός των authenticated routes ΔΕΝ αναπαράγεται**: `/api/companies` → **200 σε 2,2s**, `/api/projects/by-company/…` → **200 σε 893ms** (με auth, από το page context). Τα 60s timeouts της προηγούμενης συνεδρίας ήταν παροδικά· να μη μεταφερθούν ως ισχύον εμπόδιο. Tests: 15 suites / 135 tests πράσινα. ΟΧΙ tsc (N.17). | Opus 5 |
| 2026-07-26 | **§10.14.Η.5.γ (συνέχεια) — ΤΟ ΕΜΠΟΔΙΟ ΑΡΘΗΚΕ: το lightbox είναι πλέον ΠΡΟΣΒΑΣΙΜΟ. Η μέτρηση ΔΕΝ έγινε ακόμα.** Η τριπλά νεκρή αλυσίδα συνημμένων κουμπώθηκε (πλήρης καταγραφή: **ADR-366 §12, εγγραφή 2026-07-26**): ο `CommentAttachmentUploader` μπήκε στη φόρμα «Νέο Σχόλιο», το `CreateCommentInput` απέκτησε `attachments` (+ προαιρετικό `id`), και τα αρχεία ανεβαίνουν **πριν** το `createComment` ώστε το σχόλιο να γεννιέται πλήρες με ένα write. Άρα το `lightboxIndex` **μπορεί πλέον να γίνει non-null** και το `CommentAttachmentLightbox` να αποδοθεί — δηλαδή ο λόγος για τον οποίο το σενάριο ήταν αμέτρητο **έπαψε να ισχύει**. ⚠️ **ΤΙΜΙΟΤΗΤΑ**: αυτό είναι **ξεμπλοκάρισμα, όχι μέτρηση**. Το «ESC με ανοιχτό lightbox + επιλεγμένο 3D στοιχείο ⇒ `consumedBy === 'bim-3d/comment-attachment-lightbox'` (ΟΧΙ `canvas/fallback-deselect`)» **παραμένει απάτητο** — απαιτεί περπάτημα οθόνης με το όργανο του `HANDOFFS/2026-07-26_esc-live-verification-final-mile_handoff.md` §1, και **ο φραγμός των authenticated routes (60s timeouts) δεν έχει διερευνηθεί**. Μην καταγραφεί ως επαληθευμένο πριν πατηθεί. | Claude Opus 5 |
...
| 2026-07-26 | **§10.14.Η.5.δ — ΤΕΤΑΡΤΗ ΣΥΝΕΔΡΙΑ ΜΕΤΡΗΣΕΩΝ: το @-mention πατήθηκε ολόκληρο· δύο ελαττώματα πλήκτρων εντοπίστηκαν.** 🟢 Ζωντανά: ← / → lightbox (με 2 συνημμένα, διπλός μάρτυρας: `alt` + εναλλαγή κουμπιών)· `aria-activedescendant` στο textarea· ↓/↑ με wrap και **χωρίς** διαρροή στον καμβά· Enter εισάγει (μηδέν νέα γραμμή)· Escape κλείνει τη λίστα **κρατώντας** το κείμενο· **splice στον caret** με ουρά ανέπαφη· Ctrl+Enter με **dedup επιβεβαιωμένο στη βάση** (3 mentions → 1 `mentionedUserIds`). Ο picker αργεί **37,8 s** (χειρότερα από τα 19,5 s του §3). 🔴 **Tab νεκρό μέσα στο modal**: `use3DShortcuts.ts:112` (window capture) κάνει `preventDefault + stopPropagation` στο Tab· μοναδικός φύλακας `isTypingInFormField` — **καμία ερώτηση για ανοιχτό modal**· σε 2D το ίδιο μέσω `cycleSnap`. Τα 2 κουμπιά πλοήγησης εικόνας είναι **απρόσιτα από πληκτρολόγιο** (παραβίαση APG). 🔴 **Το focus ΔΕΝ επιστρέφει στη μικρογραφία** — patch στο `HTMLElement.prototype.focus`: **μηδέν** κλήσεις κατά το κλείσιμο, `activeElement = BODY`· ίδιο και με το κουμπί «Κλείσιμο» ⇒ δεν φταίει το ESC. 🟡 Ο Μηχ. 1 τυπώνει `SHADOW-OWNER` σε **κάθε** Escape της λίστας mention — δηλαδή στον ίδιο του τον θεσμοθετημένο Κ3. ⚠️ Παράπλευρο: ← / → μετακινούν **και** το 2D viewport (±80 px). 🔧 Όργανα: `computer` = screenshot coords (μετρημένο με `hover`, ΟΧΙ με το μάτι)· `elementFromPoint` self-check πριν από κάθε κλικ· `hasFocus=false` ακυρώνει κάθε άμεση μέτρηση μετακίνησης focus (θετικό control σε καθαρό iframe)· patch στο `Location.prototype.reload` για επιβίωση από τα HMR reloads του παράλληλου πράκτορα. | Claude Opus 5 + Γιώργος Παγώνης |
| 2026-09-18 | **Μετακίνηση SSoT — `inline-rename-keyboard.ts` βγαίνει από το viewer** (`dxf-viewer/ui/utils/` → `src/lib/ui/`). Αφορμή: ο πρώτος καταναλωτής **έξω** από το subapp (επιτόπου μετονομασία φακέλου, ADR-866 Φ1.2) έγραφε ωμό `event.key === 'Escape'` και το CHECK 3.7 τον έπιασε. Αντί για νέα γραμμή allowlist, το **ίδιο** SSoT έγινε κοινό: 4 καταναλωτές του viewer (`EntityCard` · `FrameProfileCard` · `TableNameBox` · `useSliderValueEditing`) + `PropertyDossierTitle`. Η γραμμή allowlist στο `.ssot-registry.json` άλλαξε διαδρομή — **ίδιος αριθμός** γραμμών, καμία νέα εξαίρεση. |
