...
| Field | Value |
|---|---|
| Status | 🟢 **Φ1 + Φ7 + Φ8 DONE (uncommitted)** — Φ1 standalone path-based `RailingEntity` end-to-end (types+Zod, enterprise-id, 2-click 2Δ tool + renderer, persist+audit, units-safe 3Δ converter, discipline visibility, ΑΤΟΕ BOQ). **Φ7:** η σκάλα γεννά **αυτόματα** persisted hosted κάγκελο (posts+balusters+rail, σλοπαρισμένα + Baluster Per Tread) μέσω derived-cascade lifecycle (mirror ADR-632). **Φ8:** per-material/per-component βαφή (χρώμα/υλικό/υφή). Φ3–Φ6 (slab-edge hosting/handrail-separation/infill/Type-UI) εκκρεμούν· retire γυμνού stair tube = επόμενο υπο-βήμα. **Φ7c:** Revit «Baluster Per Tread» seated-on-tread — κάθε κάθετο πατάει στην κορυφή του **πραγματικού σκαλιού** (stepped base, `buildTreadAnchors`) και το ύψος του φτάνει LIVE τη λεία κουπαστή (GAP 0), landings με ball-rule spacing. Ground-truth από Firestore (gamma). (2026-07-23, Opus 4.8) |
| Date | 2026-06-02 |
...
| **Φ9** | **Selection editing UI** — επιλογή κάγκελου ανοίγει contextual ribbon tab «Ιδιότητες Κιγκλιδώματος» + αριστερό Properties palette (Revit-parity με τοίχο/σκάλα), οδηγούμενα από ΕΝΑ κοινό railing param schema (keys/options/read/patch, SSoT ribbon↔panel). | Παραμετρική επεξεργασία μετά τη σχεδίαση |
...
- **v0.9 (2026-07-24, Opus 4.8) — 🟢 Φ9: επιλογή κάγκελου → contextual ribbon tab «Ιδιότητες Κιγκλιδώματος» + αριστερό Properties palette (Revit-parity με τοίχο/σκάλα).** RECOGNITION (code=SoT): το `railing` έπεφτε στο τελικό fallback του `BimPropertiesRouter` (έδειχνε **λάθος** το wall empty-state «Επίλεξε έναν τοίχο») και το `resolveContextualTrigger` επέστρεφε ρητά `null` («no per-selection editor tab»). Το `UpdateRailingParamsCommand` υπήρχε ήδη από Φ1 (recompute geometry+validation atomically) — μηδέν νέα command. **SSoT-first (N.18): ΕΝΑ κοινό railing param schema τροφοδοτεί ΚΑΙ το ribbon ΚΑΙ το panel** (μηδέν sibling-clone, το κλασικό λάθος «κεντρικοποιείς το Α, γράφεις Β+Γ δίδυμα»):
  - **NEW `bim/railings/railing-param-keys.ts`** — commandKeys (`RAILING_RIBBON_KEYS` string+numeric) + guards (`isRailingRibbonKey`/`isRailingRibbonStringKey`) + option ladders (predefinedType/shape/justification/on-off/infill + raw-mm literal ladders) + `RAILING_LABEL_KEYS` (κοινά labels ribbon↔panel).
  - **NEW `railing-param-access.ts`** — `readRailingField(key,params)` + `patchRailingField(key,params,value)` (pure, deep-immutable· round-profile height sync· **booleans ως on/off comboboxes** → ΕΝΑ uniform combobox path, ίδιο consumable από `BimPropertyRow`). Read/write ΔΕΝ αποκλίνουν ποτέ (ADR-449 bug class).
  - **NEW `railing-property-groups.ts`** — `BimPropertyGroup[]` descriptors (geometry/baluster/posts/rails/infill) για το panel.
  - **Ribbon:** NEW `contextual-railing-tab.ts` (`RAILING_CONTEXTUAL_TRIGGER='railing-selected'`, 5 panels: Γεωμετρία/Κάγκελα/Ορθοστάτες/Κουπαστές/Πλήρωση) + NEW `use-ribbon-railing-bridge.ts` (mirror stair· shared helpers `useResolveSelectedEntity`/`useActiveSceneManager`/`useInertBridgeExtras`/`useStableBridge`). **Plain-mm handshake, ΟΧΙ scene-unit scale:** επιβεβαιώθηκε (audit) ότι το `quantityKind:'model-length'` κάνει display↔mm conversion ΜΟΝΟ στο `RibbonCombobox` (`ribbon-display-unit.ts`)· ο bridge πάντα μιλά plain mm (τα `RailingParams` αποθηκεύουν ήδη mm — μηδέν διπλό conversion). Wiring: barrel, `contextual-tabs-registry`, `ENTITY_CONTEXTUAL_TRIGGER` map, `useDxfBimBridges`, `useRibbonCommands-{dispatch,types}`, `useRibbonCommands`, `useDxfViewerRibbon` (full chain — αλλιώς dead prop → silent no-op).
  - **Panel:** NEW `RailingPropertiesTab`/`RailingAdvancedPanel` (mirror stair· χρησιμοποιεί το SSoT `BimPropertyRow`· **ΟΧΙ persistence** — δεν υπάρχει railing store slot στο `bim-persistence-state-store` ακόμα) + `useSelectedRailing` + `useRailingParamsDispatcher` (→ `UpdateRailingParamsCommand`) + branch στο `BimPropertiesRouter` (μετά τη σκάλα).
  - **i18n:** el+en `ribbon.tabs.railingProperties` + `ribbon.panels.railing{Geometry,Baluster,Posts,Rails,Infill}` + `ribbon.commands.railingEditor.*` + top-level `railingAdvancedPanel`.
  - **Coverage:** `railing` αφαιρέθηκε από `NO_SELECTION_TAB_TYPES` + golden pin· route-count 37→38. Tests coverage+dispatch+quantity-kind **38/38 ΠΡΑΣΙΝΑ**, **jscpd 0 clones**. 🔴 browser verify (Giorgio). Commit = Giorgio.
- **v0.8 (2026-07-23, Opus 4.8) — 🐛 Φ7c: «τα κάγκελα δεν πατάνε στη σκάλα & η κουπαστή δεν ακολουθεί τις κορυφές» → Revit «Baluster Per Tread» seated-on-tread.** GROUND-TRUTH ΠΡΩΤΑ (**ανάγνωση Firestore**, εντολή Giorgio): τα 2 persisted railings (`floorplan_railings`) + η σκάλα (`floorplan_stairs`, **gamma [5,5,5]**, totalRise 3000). Μετρήθηκε ο **πραγματικός** CatmullRom tube πάνω στο baked `resolvedPath`: rail-underside − baluster-top **≤5mm** → η geometry ΔΕΝ «αιωρεί» ενδο-railing. Το πραγματικό ελάττωμα φάνηκε από την ανακατασκευή της gamma: **tread tops = ΣΚΑΛΟΠΑΤΙΑ** (0,175,350,525,700,1050…) αλλά τα balusters κάθονταν στο **ΛΕΙΟ walkline** (ράμπα 0→875→…) με **σταθερό βήμα κατά μήκος ΟΛΟΥ του path** (μαζί με τα 2 πλατύσκαλα) → base π.χ. z=496 ενώ το σκαλί κάτω στο 350 → «δεν πατάνε στη σκάλα». **Fix (Revit-grade):**
  - **Host (`stair-railing-host.ts`):** NEW `buildTreadAnchors` — για ΚΑΘΕ πραγματικό tread (`treadsBelowCut`+`treadsAboveCut`) βρίσκει το σημείο του tread πάνω στη **railing line** (`projectOntoPath(centroid)`) και το φορτώνει με το **tread-top z (stepped, mm)**. Bake `perTreadAnchors` = ένα σημείο ανά σκαλί (όχι scalar count). Δουλεύει ίδια σε straight/gamma/winder/spiral (διαβάζει πραγματικά treads).
  - **Engine (`railing-geometry.ts`):** NEW `treadBalusters` — base στο tread top (stepped, «πατάει στη σκάλα»), ύψος υπολογισμένο **LIVE** ώστε η κορυφή να φτάνει τη λεία κουπαστή (`projectOntoPath(resolvedPath)`.z + railOffset). NEW `landingBalusters` — γεμίζει τα **flat segments** (Δz≈0) με ball-rule spacing. NEW SSoT `nearestOnPath` (κοινό projection loop· `projectOntoPath`+`nearestSegmentAngleDeg` το χρησιμοποιούν — jscpd extract). Έφυγε το `perTreadBalusterCount`/`treadCount`.
  - **Snapshot (types+Zod):** `perTreadAnchors` = πλέον tread anchors (positions+stepped z)· `treadCount` αφαιρέθηκε. Coordinator: re-bake gate σε **αλλαγή anchor count** → pre-Φ7c docs (even-spacing) μεταναστεύουν σε tread-seated στο επόμενο cascade run της σκάλας.
  - **Ground-truth ΜΕΤΑ (gamma):** 15 flight balusters με base **ΑΚΡΙΒΩΣ** στα tread tops + 22 landing balusters, **GAP baluster-top − rail-underside = 0mm για ΟΛΑ**. Τα flight ύψη ποικίλλουν (stepped base → λεία κουπαστή) = σωστή Revit συμπεριφορά. Tests 44/44, **jscpd 0**. ⚠️ Τα ήδη-persisted railings self-heal με **επεξεργασία/άγγιγμα της σκάλας** (re-bake)· pure reload δεν τρέχει cascade. ⚠️ TODO: η gamma «mixed» resolvedPath segment (flight+landing μαζί) δίνει μεγαλύτερη διακύμανση ύψους από το ιδανικό — βελτίωση με καθαρό flight/landing split. Commit = Giorgio.
