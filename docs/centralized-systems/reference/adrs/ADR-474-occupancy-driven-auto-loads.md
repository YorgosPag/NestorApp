...
## 3γ. Occupancy SSoT inheritance από `Building.category` (2026-06-17)

Full-SSoT απαίτηση Giorgio: **μηδέν διπλασιασμός** της «χρήσης κτιρίου». Το `Building.category`
(`residential|commercial|mixed|industrial`) **υπάρχει ήδη** → η structural occupancy **κληρονομεί**
από εκεί, αντί να αποθηκεύεται ξεχωριστά.

- NEW SSoT mapping `OCCUPANCY_BY_BUILDING_CATEGORY` + `resolveOccupancyFromBuildingCategory`
  (residential→A 2.0· commercial→D 5.0· mixed→B 3.0· industrial→E 7.5).
- NEW hook `useBuildingOccupancy` (mirror `useBuildingStoreyCount`: active level → buildingId →
  `useBuildingById` → category → occupancy).
- **Precedence (Revit):** `structuralSettings.occupancy` (deliberate override) **→**
  `building.category` (κληρονομιά) **→** `DEFAULT_OCCUPANCY` (residential). Wired στους 2 takedown
  hooks: `occupancy: settings.occupancy ?? occupancyRef.current`.
- **Συνέπεια:** ρητή τιμή kPa/occupancy αποθηκεύεται ΜΟΝΟ ως σκόπιμο override — αλλιώς τα φορτία
  ακολουθούν αυτόματα τη δηλωμένη χρήση/κατηγορία (όπως η Revit). Μηδέν stale διπλότυπο.
...
- **2026-06-17 (occupancy SSoT inheritance, UNCOMMITTED):** structural occupancy κληρονομεί από
  `Building.category` (NEW `OCCUPANCY_BY_BUILDING_CATEGORY` + `resolveOccupancyFromBuildingCategory` +
  hook `useBuildingOccupancy`). Precedence: structuralSettings override → category → default. Μηδέν
  διπλασιασμός «χρήσης κτιρίου». Βλ. §3γ.
