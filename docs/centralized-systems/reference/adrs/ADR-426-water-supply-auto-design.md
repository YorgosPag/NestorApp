...
## Changelog
- **2026-06-09 (Opus 4.8)** — **Cold/hot pairing now wall-aware (Routing Brain Slice 3C).** The hot offset spine is obstacle-aware: where the offset would land on a wall it is locally A\*-detoured by the shared core (`buildOffsetPairing` gains `opts.obstacles`, threaded through `buildOffsetHotNetwork` + the orchestrator). The `obstacles.length===0` pairing gate is removed — pairing now runs even with walls (no-walls ⇒ byte-identical). +1 test (wall detours the offset trunk, never crossing it, `totalLU` intact). 91/91 mep-design green. ΕΚΤΟΣ ADR-040. See **ADR-429 §5 (Slice 3C)**.
- **2026-06-09 (Opus 4.8)** — **Cold/hot parallel pairing IMPLEMENTED.** Where the cold + hot spines coincide (shared corridor, fixtures fed by both) the hot run overlapped the cold one. Revit/MagiCAD/4M-FINE-grade fix: **cold = reference (untouched), hot = constant lateral offset** by `discipline.hotSpineOffsetMm` (80, already defined). Because cold & hot have **different sources + different fixtures** (WC = cold only), the hot is routed independently then offset onto its OWN spine — new thin wrapper `water/pair-cold-hot.ts` (`buildOffsetHotNetwork`) over the **shared SSoT core** `routing/offset-pairing.ts` (`buildOffsetPairing`, generalised out of the heating 3B geometry — Boy-Scout N.0.2, zero copy-paste). Gate in `design-water-supply.ts`: pairing only for `service==='hot'` + no walls + ≥1 trunk; `totalLU` / `servedConnectors` / source metadata preserved. NEW `water/pair-cold-hot.ts` + `__tests__/pair-cold-hot.test.ts` (6) + `routing/offset-pairing.ts` + its test (6); MOD `design-water-supply.ts`, `water/index.ts`, `heating/pair-supply-return.ts` (refactored to a thin wrapper, zero-regression). 88/88 mep-design tests green. ΕΚΤΟΣ ADR-040 (headless). See **ADR-429 §5**.
