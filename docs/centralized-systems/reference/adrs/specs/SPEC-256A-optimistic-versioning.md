...
| Date | Change | Author |
|------|--------|--------|
| 2026-04-13 | **Phase 2: Silent last-write-wins (Google pattern)**. After repeated self-409 incidents (stale props, phantom server bumps from audit-trigger writes, ConflictDialog state leaks across entity swaps) the SPEC-256A v1 Phase-1 dialog-based UX was judged net-negative for a solo-user operator app. Decision: **the dialog is gone.** `useVersionedSave` now catches 409 and silently retries once without `_v` (last-write-wins), matching Gmail / Calendar / Contacts behavior. Public API trimmed to `{ version, save, setVersion }` — `isConflicted`, `conflictData`, `forceSave`, `resetConflict`, and `onConflict` are removed. `ConflictDialog.tsx` deleted along with all `versioning.*` i18n keys (el/en/pseudo). `GeneralProjectTab`, `GeneralTabContent/buildings`, `ParkingGeneralTab`, `StorageGeneralTab` updated: the two auto-save consumers simply drop the `isConflicted` short-circuit; the two manual-save consumers got inline silent retry (catch 409 → delete `_v` → retry once). The `_v` field is still sent on the first attempt so the server can audit the version trajectory, and `EntityAuditService` remains the durable who-changed-what record. **Why**: a conflict dialog that a solo user cannot meaningfully resolve (no side-by-side diff UX) only adds friction; Google's equivalent products never show one for solo records. **Rejected**: CRDT/OT (dependency bloat, no multi-user requirement); banner (still noise for races that are not genuine concurrent edits). | Claude Code |
| 2026-04-13 | BUGFIX + UX: **Solo-user self-409 race fixed** in `useVersionedSave`. Root cause: render-phase sync block unconditionally mirrored `initialVersion` → `versionRef`, so after a successful auto-save (ref 5→6) a lagging parent prop (_v=5) rolled the ref back to 5 on the next render, and the very next auto-save fired with a stale `_v` → 409 against the user's own prior write. Fix: (1) forward-only version sync — prop version only adopted if it's higher than the tracked ref; (2) new `entityId` config — when identity changes, ref resets to the new prop outright; same entity never rolls backward. Call-sites updated: `GeneralProjectTab`, `GeneralTabContent` (buildings). Also: `ConflictDialog` now shows explicit per-action consequence cards (keep-mine vs discard-and-reload vs cancel) because users could not tell the two destructive actions apart from button labels alone. New i18n keys: `versioning.reloadDescription`, `versioning.forceSaveDescription`, `versioning.cancelDescription` + reworded `conflictMessage`/`reload`/`forceSave` in el/en/pseudo. | Claude Code |
