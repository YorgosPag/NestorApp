...
| Metadata | Value |
|----------|-------|
| **Status** | ✅ Phase 1 + P2.1/P2.2/P2.3/P2.4/P2.5/P2.6/P2.7/P2.8 IMPLEMENTED 2026-05-24. Phase 2 COMPLETE. |
...
|-----|-------|----------------|
| 🔴 **ADR-787** Πολυ-οργανισμική πλατφόρμα | **Καταναλωτής — ΔΙΑΒΑΣΕ ΠΡΙΝ ΑΓΓΙΞΕΙΣ ΤΟ `cdeState`** | ✅ **ΦΡΟΥΡΟΣ από 2026-09-17 (ADR-862 Φ0)** — δες Changelog. *Ιστορικό:* το `cdeState` ήταν **μεταδεδομένο και ΔΕΝ φρουρούσε τίποτα** (μετρημένο 2026-08-21: μόνο AI enricher · οθόνη μεταδεδομένων · κατασκευαστής δέντρου · backfill — **καμία** χρήση σε έλεγχο πρόσβασης). Το ADR-787 (αρχή **Α5**, κενό **Κ-4**) αποφασίζει ότι **γίνεται φρουρός**: `WIP` = μόνο η ομάδα που το φτιάχνει· `SHARED` = όλες οι ομάδες του έργου. ⚠️ **ΜΗΝ αλλάξεις το λεξιλόγιο των τεσσάρων καταστάσεων** — είναι το ISO 19650 και το ADR-787 στηρίζεται πάνω του αυτούσιο. |
...
| Date | Change | Author |
|------|--------|--------|
| 2026-09-17 | 🔒 **ADR-862 Φ0 Β11 — το `cdeState` ΦΡΟΥΡΕΙ και την ΑΝΑΓΝΩΣΗ.** (1) Το `cdeState` το γράφει **μόνο** ο γραφέας `services/iso19650/container-transitions.ts` (5 πράξεις)· το dropdown του P2.1 αφαιρέθηκε (Β3)· οι κανόνες το παγώνουν για πελάτη (Β4)· το **CHECK 3.87** μπλοκάρει δεύτερο γραφέα στον διακομιστή. (2) **Το δηλωμένο όριο «συνάδελφος διαβάζει μεταδεδομένα ξένου WIP με client SDK» ΕΚΛΕΙΣΕ**: νέα προβολή `cdeReadReach` (`tenant`/`author`, `lib/auth/container-read-reach.ts`), κανόνες `allow get` (γραφείο ∧ (`tenant` ∨ δημιουργός)) + `allow list` (φίλτρο `cdeReadReach == tenant` ∨ `createdBy == uid`, εφαρμοσμένο αυτόματα από το `firestoreQueryService`). 🔶 **Όριο που μένει**: τα WIP **ομάδας** (όχι του δημιουργού) τα κρίνει μόνο ο διακομιστής — η ομάδα δεν μπαίνει σε claim· σήμερα 0 μέλη έργων. (3) Το λεξιλόγιο των τεσσάρων καταστάσεων **αμετάβλητο**. | Claude (Opus 5) + Γιώργος Παγώνης |
| 2026-07-17 | **`callOpenAiVisionEnricher` migrated onto the Responses SSoT (ADR-294 Φάση 1).** The hand-rolled `AbortController` + `setTimeout` + raw `fetch('/responses')` is gone; the enricher now calls `requestVisionJson()` from the new `@/services/ai-pipeline/shared/vision-json-request`, which resolves model/baseUrl from `AI_ANALYSIS_DEFAULTS` and passes `maxRetries: 0` (the enricher never retried). Non-OK → `null` and any throw → `null` are unchanged, so the budget/derivation decision tree is untouched. **§D5/§D9 nearly regressed and is now documented at the source:** the new shared module was first written with `import 'server-only'`, which broke the client build — this enricher is on a static import chain that reaches client components (`file-record.service` → `useFileDownload` → `PhotoPreviewModal`) and relies on its runtime `typeof window` guard instead. `vision-json-request.ts` now carries an explicit warning comment naming that chain, so the next author does not re-add it. The `vision-helpers` import stays (rather than switching to the canonical barrel) because this module's tests `jest.mock` that shim wholesale — the shim re-exports the SSoT verbatim, so it is the same source of truth reached through the seam the tests own. Tests: 16/16 green, unchanged. | Opus 4.8 |
| 2026-07-16 | **§D5 `vision-helpers.ts` promoted → `@/services/ai/openai-responses/` (now a re-export shim).** **Why it moved:** the header of `tools/handlers/vision-helpers.ts` declared it *"SSoT for OpenAI Responses-API vision pipeline"* and it was — for the five ai-pipeline consumers. But **three clients outside ai-pipeline** (accounting document analyzer, procurement quote analyzer, ai-analysis provider) each re-implemented `extractOutputText` byte-for-byte rather than reach into `tools/handlers/` for it. A helper buried two levels inside a *handlers* folder does not read as repo-wide canon; the ADR-584 jscpd audit surfaced the resulting clones (664 + 361 tokens). Promotion to a top-level module makes the SSoT findable, which is the actual fix. Same conversion `ai-pipeline/greek-text-utils.ts` went through (ADR-314 Phase D.3a). **New module** (`src/services/ai/openai-responses/`): `wire-types.ts` (the five re-declared interfaces) · `extract-output-text.ts` · `execute-request.ts` (`executeResponsesRequest` — auth+timeout+retry+error-unwrap) · `vision-content.ts` (`isImageMime` · `toBase64DataUri` · `beginVisionContent`) · `download-file.ts` (`downloadFile` + `DEFAULT_MAX_FILE_SIZE_BYTES`) · `index.ts` barrel. **⚠️ §D5 client-safety invariant PRESERVED and now load-bearing for more code:** every module behind the barrel is `server-only`-free (pure `fetch`/`Buffer`/JSON, no secret reads — the caller passes resolved config in), so the Turbopack static-import chain that reaches client bundles stays unpoisoned. The shim documents this inline: **do NOT re-export anything `server-only` from `vision-helpers.ts`.** **Shim keeps the blast radius at zero:** all five consumers (`iso19650-enricher`, `document-preview-service`, `invoice-entity-extractor`, `document-reader-handler`, `contact-document-classifier`) and the `jest.mock('@/services/ai-pipeline/tools/handlers/vision-helpers')` in `tools/__tests__/handlers/iso19650-enricher.test.ts` are **untouched**. `VisionContent` survives as a deprecated alias of `ResponsesContent`. **Deliberately NOT merged:** `downloadFile` (plain `fetch`, 4MB cap, returns `null`) vs the analyzers' admin-bucket read are *different transports with different auth* — the prior session's map assumed they were interchangeable, they are not. The bucket idiom (public-URL → prefix-strip → decode → download), which was verbatim-duplicated in both analyzers, went to a **separate server-only** module `@/lib/firebaseAdmin-storage.ts` (`downloadAdminObjectByPublicUrl`) — deliberately NOT under `src/lib/storage/`, which is browser localStorage and would have dragged admin credentials toward the client graph. **Verification (❌ no tsc, N.17):** `test:ai-pipeline:all` **1179 GREEN** (N.10 — mandatory, `vision-helpers` touched) · 114 characterization tests GREEN unchanged across the three migrated clients · `jscpd:diff` clean. See ADR-294 changelog for the ratchet gap this exposed and closed. | Claude (Opus 4.8) |
| 2026-05-24 | **✅ P2.5 IMPLEMENTED — ISO 19650 Cost Dashboard.** 10 αρχεία (2 NEW + 7 EDIT + 1 ADR). (1) `src/config/firestore-collections.ts` +`ISO19650_COST_LOG: 'iso19650_cost_log'`. (2) Enterprise ID: `enterprise-id-prefixes.ts` +`ISO19650_COST_LOG: 'iso19650_cost'`, `enterprise-id-class.ts` +`generateIso19650CostLogId()`, `enterprise-id-convenience.ts` +export, `enterprise-id.service.ts` +re-export. (3) `src/services/iso19650/iso19650-cost-log-service.ts` NEW (85 LOC) — Admin SDK write service. `logIso19650EnrichmentCost({ companyId, fileId, costUsd, model, disciplineCode })`: dynamic import pattern (keeps Admin SDK off client bundle), `setDoc` με `generateIso19650CostLogId()`, try/catch never-throws. (4) `src/services/file-record-post-finalize-hooks.ts`: στο `runIso19650WithSlot()`, μετά το `applyIso19650Enrichment`, fire-and-forget log αν `filledBy === 'ai' && aiCostUsd > 0`. (5) `src/app/api/admin/iso19650/backfill/route.ts`: ίδιο cost log per file μέσα στον enrichment loop. (6) `src/app/api/admin/iso19650/costs/route.ts` NEW — GET ?companyId=xxx[&month=YYYY-MM] → `{ totalCostUsd, fileCount, byDiscipline, byMonth, records }`. Auth: super_admin + withSensitiveRateLimit + `admin:migrations:execute`. In-memory aggregation, max 500 records. (7) `firestore.rules` +`/iso19650_cost_log/{docId}` → `allow read, write: if false` (Admin SDK bypasses). Architecture: N.7.2 ✓ (proactive lifecycle, fire-and-forget non-blocking, try/catch belt-and-suspenders, SSoT service). | Claude (Sonnet 4.6) |
| 2026-05-24 | **✅ P2.4 IMPLEMENTED — Distributed Token Bucket (ADR-373 P2.4).** 6 αρχεία: (1) `src/services/iso19650/enrichment-slot-service.ts` NEW (85 LOC) — Firestore-backed per-company token bucket. `acquireSlot(companyId)`: atomic `runTransaction`, resets stale docs (>5min TTL), returns false αν `activeSlots >= 5 (MAX_SLOTS)`, fail-open αν service down. `releaseSlot(companyId)`: transaction με `Math.max(0, ...)` floor. Admin SDK only — no client SDK. (2) `src/config/firestore-collections.ts` +`ISO19650_ENRICHMENT_SLOTS: 'iso19650_enrichment_slots'`. (3) `src/services/file-record-post-finalize-hooks.ts`: `PostFinalizeContext` +`companyId?: string`, new `runIso19650WithSlot()` async helper (extract pattern — acquireSlot → enrich → finally releaseSlot), `triggerIso19650Enrichment` refactored (28→8 lines). (4) `src/services/file-record.service.ts`: `FinalizedSnapshot` +`companyId?: string`, context pass-through. (5) `src/app/api/admin/iso19650/backfill/route.ts`: import slot service, `acquireSlot(companyId)` before each file (fail-open `.catch(() => true)`), `releaseSlot(companyId)` in `finally`. (6) `firestore.rules` +`/iso19650_enrichment_slots/{companyId}` → `allow read, write: if false` (Admin SDK bypasses). Architecture: Google-level N.7.2 ✓ (atomic transaction, fail-open, floor-0, TTL self-healing). | Claude (Sonnet 4.6) |
| 2026-05-24 | **✅ P2.3 IMPLEMENTED — ISO 19650 Backfill Admin Route.** 1 NEW file: `src/app/api/admin/iso19650/backfill/route.ts` (254 LOC). Pattern: GET ?companyId=xxx → dry-run count, POST { companyId, limit? } → execute (default 5, max 20 files/call, caller loops until hasMore=false). Architecture: Admin SDK query with `companyId` where clause (CHECK 3.10 ✓), in-memory filter for `!disciplineCode` (includes null + missing field — covers pre-Phase1 AND Phase1 files), dynamic import of `enrichFileWithIso19650Metadata` (server-only chain, ADR-373 hotfix pattern), `applyEnrichment` via Admin SDK `updateDoc`, `maxDuration=60`, budget cap delegated to enricher ($0.01/file). Auth: super_admin only + withSensitiveRateLimit + `admin:migrations:execute` permission. Idempotent: files with existing disciplineCode skipped in query filter. | Claude (Sonnet 4.6) |
| 2026-05-24 | **✅ P2.2 IMPLEMENTED — ISO 19650 Virtual Folders.** 7 αρχεία: (1-2) `i18n/locales/{el,en}/iso19650.json` + `"virtualFolders"` section (5 keys: viewLabel, viewTooltip, unclassified, noSeries, noCdeState). (3) `src/components/file-manager/company-file-tree-iso19650-builder.tsx` NEW (195 LOC) — SRP split από το builders file (N.7.1). `buildTreeByISO19650(files, t, companyName, translateDisplayName?)`: 3-pass bucket algorithm (discipline→series→cdeState→files), unclassified fallback, CDE state icons (Clock/Share2/CheckCircle2/Archive), sorted (disciplines alphabetically, series numerically, CDE states fixed order WIP→SHARED→PUBLISHED→SUPERSEDED). No circular dep: `import type` για types από builders. (4) `src/components/file-manager/company-file-tree-builders.tsx`: `GroupingMode` += `'iso19650'`, re-exports `buildTreeByISO19650` from split file, size 402 LOC. (5) `src/components/file-manager/CompanyFileTree.tsx`: +`buildTreeByISO19650` import, +`'iso19650'` namespace σε `useTranslation`, +case `groupingMode === 'iso19650'`. (6) `src/components/file-manager/useFileManagerState.ts`: `ViewMode` += `'iso19650-tree'`, `useTranslation` +`'iso19650'` namespace. (7) `src/components/file-manager/FileManagerToolbar.tsx`: +`Layers` icon import, +ISO 19650 button (4th view mode toggle). (8) `src/components/file-manager/FileManagerPageContent.tsx`: +`viewMode === 'iso19650-tree'` → `<CompanyFileTree groupingMode="iso19650">` render case. ADR-293 αμετάβλητο (zero storage path changes). | Claude (Sonnet 4.6) |
