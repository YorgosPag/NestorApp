...
- Priority overrides **beam↔column** (πάντα στήλες > δοκάρια). **Wall↔wall cross:** ΥΛΟΠΟΙΗΘΗΚΕ priority model (`joinPriority`, βλ. §7).
...
## 7. Wall↔Wall Cross-Junction Extension (priority) — 2026-07-02

### 7.1 Πρόβλημα

Ο χρήστης έσυρε τοίχο ώστε να **περνά ΔΙΑΜΕΣΟΥ** άλλου τοίχου (διασταύρωση **Χ**: το σημείο τομής ΕΣΩΤΕΡΙΚΟ και στους δύο άξονες). Τα solids επικαλύπτονταν στη ΜΕΣΗ → (α) βρώμικη κάτοψη (γραμμές του ενός μέσα στο σώμα του άλλου), (β) **διπλομετρημένος** όγκος τοιχοποιίας. Το `wall-trims.ts` ρητά παρατούσε τα cross (`cross → skip`) γιατί δεν είναι endpoint-junction (miter/bevel άκρου δεν εφαρμόζεται σε body-body overlap).

«Τι κάνουν οι μεγάλοι;» → Revit «Join Geometry» / ArchiCAD priority-based connections: **priority winner** μένει ακέραιος, ο μικρότερης προτεραιότητας **κόβεται** στην κοινή περιοχή (net volume, μετριέται μία φορά). Σε καθαρό Τ = ο ένας butt στην παρειά (ήδη από `wall-trims`)· σε Χ = boolean cleanup priority-driven· miter **μόνο** σε γωνία L.

### 7.2 Απόφαση & αρχιτεκτονική (reuse του §3 SSoT)

**Ίδιος μηχανισμός με το wall↔column** (`computeMemberCutbackOutline`/`computeMemberCutbackRetentionRatio`) — ο νικητής-τοίχος γίνεται **cutter** (όπως η κολόνα), ο loser κόβεται. Μηδέν νέα boolean geometry. DERIVED, ΠΟΤΕ persisted.

- **Priority model:** NEW `WallParams.joinPriority?: number` (type + Zod `WallParamsBaseSchema` + Boy-Scout: τα ήδη-δηλωμένα `startJoin`/`endJoin` έλειπαν από το Zod → προστέθηκαν). Absent ⇒ **category default** `WALL_JOIN_PRIORITY_BY_CATEGORY` (exterior 100 > interior 80 > parapet 60 > partition 40 > fence 20). **Higher wins**· tie ⇒ deterministic (μικρότερο id). User override: `RibbonWallJoinWidget` «Προτεραιότητα Ένωσης» (auto/low/medium/high/highest) + i18n el/en.
- **Pure core:** NEW `bim/walls/wall-cross-cutback.ts` — `resolveWallJoinPriority`, `resolveWallCrossWinnerId`, `isWallCrossPair` (γνήσιο Χ = t,u interior, ίδιο κριτήριο με `wall-trims.classifyPair` → corner/T εξαιρούνται, ανήκουν στο `wall-trims`), `computeWallCrossCutters(walls) → Map<loserId, cutterFootprints[]>`. 14 jest.
- **2Δ κάτοψη:** NEW post-pass `hooks/canvas/dxf-scene-wall-wall-cutback.ts` `applyWallWallCrossCutback2D`, wired **μετά** το `applyWallColumnCutback2D` (συνθέτει το ίδιο `displayFootprint` — κόβει τα ήδη-κομμένα-από-κολόνα κομμάτια περαιτέρω). `WallRenderer` αμετάβλητος.
- **BOQ:** `wall-boq-feed.ts` `wallNetCoreGeometry` → cutters = κολόνες **+** winner-walls (ένα combined `computeMemberCutbackRetentionRatio`) → net area/volume ρέει αυτόματα στο `BimToBoqBridge`.
- **3Δ:** `wallToMesh` νέο param `wallCrossFootprints` → concat με `columnFootprintsM` στο ίδιο `buildWallCoreBody` cutback seam (flat single-layer path). `bim-scene-attach-syncs.ts` υπολογίζει το cross-cutter map μία φορά. **Bonus:** επιβεβαιώθηκε ότι το wall↔column 3Δ cutback ΥΠΗΡΧΕ ήδη εκεί (ο ίδιος seam).

### 7.3 DEFER (wall↔wall v1)

- 3Δ cross-cut σε walls **με ανοίγματα / πολυστρωματικό DNA / attached profile** → group paths, κρατούν πλήρες footprint (ίδιο όριο με το wall↔column 3Δ). Flat single-layer (τυπική περίπτωση, tilted ok) κόβεται.
- Axis-split (ο loser μένει ενιαία οντότητα με notch, δεν σπάει σε 2 entities — Revit-correct: location-line editable).
- Partial-height crossing (κατακόρυφη επικάλυψη = ύψος τοίχου, mirror beam v1).

---

## 6. Changelog

- **2026-07-02 (γ)** — **§7 Wall↔Wall cross-junction priority cutback (Opus, UNCOMMITTED).** Τοίχος διαμέσου τοίχου (Χ) → επικάλυψη 2Δ+3Δ + διπλομέτρηση. Reuse του §3 SSoT με **priority winner** (`joinPriority`, absent → category default· higher wins, tie → id). NEW: `wall-cross-cutback.ts` (pure, 14 jest) + `dxf-scene-wall-wall-cutback.ts` (2Δ post-pass) + `joinPriority` σε type/Zod (+ Boy-Scout startJoin/endJoin στο Zod) + `wall-boq-feed` combined cutters (net volume) + `wallToMesh` `wallCrossFootprints` (3Δ) + `RibbonWallJoinWidget` priority dropdown + i18n el/en. 105 jest GREEN (cross 14 + boq + trims + member + schemas), μηδέν regression. tsc SKIP (N.17). ⚠️ 1 pre-existing wall-tilt-pieces-3d fail (stash-verified άσχετο). ADR-363 §Phase 1D `cross → skip` note. 🔴 browser-verify (drag τοίχο διαμέσου: καθαρός σταυρός 2Δ+3Δ, net BOQ, άλλαξε priority → αλλάζει ο χαμένος) + commit.
- **2026-07-02 (β)** — **§diagonal-corner-seat follow-up: decouple ΣΤΕΡΕΟ (deep) από ΣΟΒΑ (center) — ο σοβάς δεν διαπερνά πια την εγκοπή L/Γ (Πράκτορας 2, Giorgio screenshot 122649).** Μετά το (α), το σώμα του δοκαριού έδραζε σωστά, αλλά ο **σοβάς** (`beamFinishOutline`, ΚΟΙΝΟ `extendBeamOutlineIntoFramingColumns`) κληρονόμησε τη **βαθιά** επέκταση → προεξείχε στην **ΚΟΙΛΗ** εγκοπή της L/Γ (πράσινο poke) + ασύνδετος σοβάς (κόκκινα). **Fix:** το `deep` έγινε **opt-in param** (default `false` = «ως το κέντρο», ADR-529/493 byte-for-byte). Το **στερεό** cutback (2Δ `buildBeamCutbackDisplay` + 3Δ `beamCarveOutline`) περνά `deep=true` → πλήρης έδραση· ο **ΣΟΒΑΣ** μένει `deep=false` (center) → αρκετή area επικάλυψη για ΕΝΑ ενιαίο caps/silhouette **χωρίς** να μπαίνει στην εγκοπή. Probe (Firestore `beam_dc020e5f`+L): FINISH notchPoke=**false**, merged cap **1 poly/0 holes**· SOLID deep seat `(-3667,15665)→(-4014,15665)→(-4014,15711)`. 225 jest (cutback+member+finishes+preview-parity) GREEN. tsc SKIP (N.17). 🔴 browser-verify + commit (stage ADR-458 + 4 code). ⚠️ 2 pre-existing 3Δ wall failures (columnToMesh baseOffset / wall-tilt, άλλου πράκτορα) — stash-verified άσχετα.
- **2026-07-02** — **§diagonal-corner-seat: ΛΟΞΟ δοκάρι σε ΕΠΙΠΕΔΗ/γωνιακή παρειά κολόνας → πλήρης έδραση και στις δύο παρειές (Πράκτορας 2, Giorgio screenshots 105516/112116).** **Σύμπτωμα:** διαγώνιο δοκάρι που καρφώνεται στη ΝΔ (convex) γωνία L-κολόνας εφάπτεται μόνο στη γωνία — η μία μισή πλευρά της απόληξης κάθεται στη νότια παρειά, η άλλη μισή αφήνει τριγωνικό κενό (~46mm) + ο σοβάς δεν κλείνει. **Ρίζα:** το `framingInwardExtent` (ADR-493 pre-pass) επεκτείνει το carve-outline «ως το footprint-midpoint» (ADR-529, σωστό για receding/κυκλικές & κάθετα)· για **λοξό** δοκάρι όμως η **καθυστερημένη γωνία** της απόληξης μένει έξω από την επίπεδη παρειά ακόμη και μετά το center → μερική έδραση, το `safeDifference` αφήνει tab. **Fix (Revit «join derived»):** η επέκταση = **`max(centerAlong, lagging-corner reach)`** — όσο ώστε ΚΑΙ οι δύο γωνίες της απόληξης να ΜΠΟΥΝ στην κολόνα (per-corner ray-entry, γωνίες inset προς τον άξονα για αποφυγή collinear-face ασάφειας)· `max` με centerAlong → receding/κάθετα ΔΕΝ ρηχύνουν (ADR-529 preserved, tests πράσινα). Overshoot beam (γωνία ήδη μέσα) → 0 (μηδέν regression). Η βαθύτερη επέκταση σε **κοίλη** εγκοπή L/Γ αφήνει μικροσκοπικό spurious sliver → NEW `computeBeamCutbackOutline` = wrapper του generic + **απόρριψη κομματιών < 2%** του μεγαλύτερου (γνήσια mid-span splits συγκρίσιμα → μένουν). `lineEdgeT` διευρύνθηκε σε `{x,y}` (reuse για corner ray). Ισχύει σε **2Δ ΚΑΙ 3Δ** (κοινό `buildBeamCutbackDisplay`/`computeBeamCutbackOutline`) + στον σοβά (`beamFinishOutline` reuse). Firestore-verified (`beam_dc020e5f` + L-`col_1aac5dd7`): display outline `(-3667,15665)→(-4014,15665)→(-4014,15711)` = καθαρός wrap νότια+δυτική παρειά, 1 ring, μηδέν sliver. 31 jest (cutback+member+preview-parity) GREEN + 1637 συνολικά. tsc SKIP (N.17). 🔴 browser-verify + commit (stage ADR-458). ⚠️ shared με άλλον πράκτορα (ADR-458 ΦB/ΦC wall cutback uncommitted) — stage ΜΟΝΟ τα δικά μου. Ο τοίχος τελειώνει στην παρειά της κολόνας και στο 3Δ (μηδέν εμβύθιση στο σώμα της κολόνας), parity με 2Δ/BOQ. Mirror του beam 3Δ cutback: στον flat solid path (`buildWallCoreBody`, legacy extrude) → `computeMemberCutbackOutline(ring, columnFootprintsM)` → πολλά `buildShape` → **ένα** `extrudeShapesAndRotate`· `null` → ίσιο single-shape extrude (byte-for-byte)· `[]` → τοίχος εξ ολοκλήρου μέσα σε κολόνα (δεν σχεδιάζεται). Τα column footprints κλιμακώνονται με ΤΟ ΙΔΙΟ `sceneToM` (ADR-462, κοινός χώρος μέτρων). Συντίθεται με το υπάρχον `pullBackStraightWallEndsFromColumns` (end-butt) — μηδέν σύγκρουση (pulled-back endpoint έξω από κολόνα → cutback identity εκεί· mid-wall → cutback κάνει τη δουλειά). **SSoT dedup (Boy Scout, N.0.2):** ο `buildWallFootprintRing` υπήρχε **διπλός** (`bim-three-shape-helpers.ts` 3Δ + ο νέος pure στη ΦA)· ενοποιήθηκε — ζει στο pure `wall-geometry.ts` (χωρίς THREE dep), ο 3Δ helper τον import+re-export (backward-compat). **DEFER v1:** faced (Polygon-Mode paint) core + openings/DNA/profile/curved paths κρατούν πλήρες footprint (uncut· mid-wall σε αυτούς = occlusion από το solid κολόνας, όπως πριν)· ο flat single-layer τοίχος (η τυπική περίπτωση) κόβεται πραγματικά. Πυρήνας = ο ήδη tested `computeMemberCutbackOutline` (12 jest) → 3Δ wiring browser-verified (mirror beam, ίδια σύμβαση). ⚠️ CHECK 6D (ADR-040) — stage ADR-458 + ADR-040. 🔴 browser-verify (3Δ: τοίχος τελειώνει στην παρειά, cut-plane/transparency δείχνει καθαρή τομή) + commit #3.
