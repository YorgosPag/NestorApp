...
### Phase 7: Knowledge Base — Διαδικασίες & Δικαιολογητικά
- Δημιουργία Firestore collection ή config αρχείο με τυπικές διαδικασίες:
  - **Οριστικό συμβόλαιο:** Τοπογραφικό, οικοδομική άδεια, βεβαίωση μηχανικού (Ν.4495/2017), ΠΕΑ, φορολογική ενημερότητα, κτηματολογικό φύλλο, ΕΝΦΙΑ, κλπ
  - **Προσύμφωνο:** Ταυτότητα, ΑΦΜ, εκκαθαριστικό, βεβαίωση ΕΝΦΙΑ
  - **Δάνειο:** Εκκαθαριστικό, βεβαίωση εργοδότη, μισθοδοτικές καταστάσεις, εκτίμηση ακινήτου
  - **Μεταβίβαση:** Πιστοποιητικό μη οφειλής ΤΑΠ, πιστοποιητικό κτηματολογίου
- Ο AI διαβάζει αυτό το knowledge base και απαντάει: "Για το οριστικό συμβόλαιο χρειάζεστε: 1. ... 2. ... 3. ..."
- Αν το έγγραφο υπάρχει ήδη στο σύστημα → ο AI το στέλνει ως συνημμένο
- Αν δεν υπάρχει → ο AI ενημερώνει τι χρειάζεται και από πού εκδίδεται
- **SSoT:** Ένα σημείο αλήθειας για δικαιολογητικά — δεν hardcode-άρεται στο prompt
...
| 2026-03-23 | **SPEC-257A IMPLEMENTED** — Auto unit-level contact links on reservation/sale + batch deactivation on cancellation |
| 2026-03-23 | **SPEC-257B IMPLEMENTED** — Unit-level scoping in executor: scopeLevel field, linkedUnitIds in ContactMeta, filter injection for buyer/owner/tenant |
| 2026-03-23 | **SPEC-257C IMPLEMENTED** — Selective payment visibility: buyer sees remainingAmount+nextInstallment, owner sees all+finalPrice, tenant sees nothing |
| 2026-03-23 | **SPEC-257D IMPLEMENTED** — Complaint triage: dedicated `create_complaint_task` tool (bypasses admin-only restriction), severity→priority mapping, server-side admin Telegram notification for urgent, unit ownership validation, TASKS added to CUSTOMER_COLLECTIONS (unit-scoped read) |
| 2026-03-23 | **SPEC-257E IMPLEMENTED** — Append-only contact updates: dedicated `append_contact_info` tool, server-side phone/email validation (SSoT), duplicate detection, self-only access (ctx.contactMeta.contactId), static label→type maps, shared CONTACT_UPDATE_PROMPT |
| 2026-03-23 | **SPEC-257F IMPLEMENTED** — Photo/floorplan/document delivery: dedicated `deliver_file_to_chat` tool, entity-reference pattern (AI provides sourceType+entityId, server resolves URL), 3 source types (unit_photo/file/floorplan), Telegram native sendPhoto/sendDocument, Email attachment via Mailgun, text fallback for WA/Messenger/IG, DXF blocked, `sendChannelMediaReply()` in channel-reply-dispatcher, FILE_DELIVERY_PROMPT shared const, FILES+FLOORPLANS added to CUSTOMER_COLLECTIONS |
| 2026-03-23 | **SPEC-257G IMPLEMENTED** — Knowledge base for legal procedures: SSoT config `legal-procedures-kb.ts` (4 procedures, 22 required documents, 7 source types), dedicated `search_knowledge_base` tool with keyword matching + document availability check against `files` collection, KNOWLEDGE_BASE_PROMPT shared const, `searchProcedures()` helper with relevance scoring |
