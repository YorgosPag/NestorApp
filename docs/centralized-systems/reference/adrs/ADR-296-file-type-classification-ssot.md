...
| Date | Change |
|------|--------|
| 2026-07-17 | **Registry extended to own extension→MIME resolution — the fourth hand-maintained MIME list is gone** (Opus 4.8, UNCOMMITTED). **Why it came back:** ADR-296 centralized *classification / preview / extractor* decisions but never answered "what MIME is this file?", so two copies grew back exactly as predicted in §1 — `openai-document-analyzer.guessMimeFromUrl` and `UploadDocumentDialog.guessMimeType`, both resolving by substring `includes('.ext')` in check order. Consequence: **`report.png.pdf` was sent to OpenAI as `image/png`** (`.png` was probed first), and neither stripped the query string, so a signed URL ending `?token=a.b.c` resolved on `.c`. **The fix:** `canonicalMimeForFilename` / `canonicalMimeForUrl` + `filenameFromUrl` in the registry; both call sites now import them. Resolution is **last-extension only**, query/fragment stripped, percent-decoded. **New spec field `canonicalMimeByExt`** — the non-obvious part: `mimeTypes[0]` is an *array position*, not a per-extension answer, so a naive helper would have resolved `.rar` → `application/zip`, `.xls` → the OpenXML sheet type, `.ppt` → presentationml. Multi-MIME specs (`xlsx`, `pptx`, `xml`, `dxf`, `archive`) now declare the canonical MIME per extension, **validated at module load** in the style of the existing duplicate-MIME/duplicate-extension throws: a multi-MIME spec missing an entry, mapping a MIME it doesn't claim, or mapping an extension it doesn't own **throws at import**. Single-MIME specs are inferred (no ceremony). **Enforcement:** 2 new `forbiddenPatterns` on the `file-type-classification` module (ADR-294) block re-introduction; zero remaining violations, so the ratchet is unaffected. **Tests:** +18 (registry now 72 GREEN) — invariants (every multi-MIME spec maps every extension; every mapped MIME is one the spec claims; **every canonical MIME round-trips back to its own spec**), the three array-order traps, URL parsing incl. query/fragment/percent-escapes/malformed-`%` fallback. **Consumers:** `openai-document-analyzer.ts` (ADR-ACC-005), `UploadDocumentDialog.tsx`. **Verification (❌ no tsc, N.17):** 72/72 registry · 78/78 accounting+procurement · 691 GREEN across all registry consumers · `jscpd:diff` clean. (2 pre-existing suite failures in `useFileListActions` / `rfq-service` — `fetch is not defined` from `@firebase/auth`, unrelated: they import nothing touched here.) |
