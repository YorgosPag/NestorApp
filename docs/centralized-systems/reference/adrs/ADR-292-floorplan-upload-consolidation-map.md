...
| 2026-04-09 | **Route Split — SRP Compliance**: `src/app/api/floorplans/process/route.ts` split from 498→202 γρ. Extracted `floorplan-process.service.ts` (151 γρ.) + `floorplan-process.types.ts` (51 γρ.) — handler→service separation per Google SRP. | Claude Code |
