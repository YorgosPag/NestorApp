...
| 2026-08-04 | **ADR-745 Φ3α — ο `LandownerEntry` απέκτησε στάδιο κτήσης** (`acquisitionStatus?: 'prospective'\|'under_contract'\|'secured'\|'withdrawn'`, `src/types/ownership-table.ts`). **Προαιρετικό επίτηδες**: απουσία σημαίνει «δεν δηλώθηκε», **ΟΧΙ** `prospective` — τα υπάρχοντα έργα γράφτηκαν πριν το πεδίο και σιωπηλό default θα δήλωνε «σε συζήτηση» για οικόπεδο που ίσως χτίζεται ήδη (πρβλ. ADR-287, τιμή enum που «ποτέ δεν ταίριαζε με πραγματικά δεδομένα»). 🔑 **Ανά πρόσωπο, όχι ανά έργο**: οικόπεδο τριών αδελφών από ⅓ όπου **δύο** υπέγραψαν και ο **τρίτος** όχι — μία κατάσταση σε επίπεδο έργου **δεν μπορεί να πει την αλήθεια**· η κατάσταση του έργου είναι **παράγωγη**. Το πρότυπο υπήρχε ήδη στον **αδελφό** `PropertyOwnerEntry`, που κρατά `role` + `paymentPlanId` **ανά εγγραφή**. **Σύγκλιση των δύο τύπων ΑΠΟΡΡΙΦΘΗΚΕ με μέτρηση**: 26 αρχεία σε ρίσκο, τα **21 στο sales domain** (SellDialog, PaymentTab, CreatePaymentPlanWizard…) που δεν έχουν κανέναν λόγο να γνωρίζουν κτήση οικοπέδου — έναντι 4 νέων + 6 επεκτάσεων για σκέτη επέκταση. Το ίδιο το σχόλιο της §5.1 έλεγε «extended, όχι merge». ⚠️ **Διορθώθηκε ανακρίβεια που προϋπήρχε**: το docblock του `LandownerEntry` έγραφε «σενάριο **αντιπαροχής**» ενώ το δείγμα Γ753 του ADR-745 είναι **αγορά** με την ίδια δομή. `audit-tracked-fields.trackSubFields` += `acquisitionStatus` (αλλιώς σιωπηλό κενό ιστορικού)· `firestore-schema-map` ενημερώθηκε γιατί τροφοδοτεί το **AI pipeline**, δεν είναι τεκμηρίωση. Λεπτομέρειες: **ADR-745 changelog 2026-08-04 (ιβ)**. |
