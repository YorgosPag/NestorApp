...
| Date | Entry |
|------|-------|
| 2026-05-29 | **✅ FOLLOW-UP PASS 4 — `NAVIGATION_ENTITIES` SSoT + Sales tab icons colored (flat → vivid). CORRECTS Pass 3's false claim.** Giorgio (runtime `/sales/available-properties`): μπριζέλα + «Πώληση» καρτέλα icons αχνά. **Pass 3 wrongly asserted** that `NavigationBreadcrumb`/`DxfBreadcrumb` "already pull vivid colors from `NAVIGATION_ENTITIES`" — they did NOT. `entity-config.ts` (`NAVIGATION_ENTITIES`, the SSoT both components consume for breadcrumb-path icon colors) actually had **11× `text-primary`** (dark navy = flat) + **2× `text-muted-foreground`** (grey). **Fix A — `entity-config.ts` 13 entities → vivid theme-aware tokens** (same tiers as launchpad/ModuleBreadcrumb, zero new tokens): company/contactCompany/area/file→`--text-info`; building/storage→`--hue-indigo`; property/phone/contactIndividual/contact/communication→`--hue-teal`; procurement→`--text-warning`; vat→`--text-success`. Already-vivid entries (project/price/opportunity→`--text-success`; floor/parking/quote/contactService/task→`--text-warning`; location/email→`text-destructive`) preserved. **Fix B — Sales detail tab icons** (`SalesSidebar.tsx` `BASE_SALES_TABS`/`LEGAL_TAB`/`PAYMENT_TAB`, the «Πώληση» καρτέλα strip): `<tab.icon>` had **no color class** → inherited `TabsTrigger` muted/grey. Added `color` field per tab + `cn(iconSizes.sm, tab.color)`: sale-info→`--text-success`, unit-summary→`--hue-indigo`, documents→`--text-info`, photos→`--status-purple`, videos→`--hue-pink`, history→`--text-warning`, legal→`text-destructive`, payments→`--hue-teal`. **2 files.** Verified: ratchet `--all` 0/0 (arbitrary-value classes, not shade utilities), tsc clean. |
| 2026-05-29 | **✅ FOLLOW-UP PASS 3 — Breadcrumb module icons colored (flat navy/grey → vivid per-section).** Giorgio: breadcrumb icons (μπριζέλα/segment icons) rendered flat — not section-appropriate colors. **Root:** `ModuleBreadcrumb.tsx` `SEGMENT_CONFIG` had ~17 entries with `color: 'text-primary'` (dark navy foreground = looks black/flat, not colored) and `text-muted-foreground` (grey). The icon `<Icon className={cn('h-3.5 w-3.5', item.color)}>` faithfully applied a non-vivid color. (The other two breadcrumb components — `NavigationBreadcrumb` + `DxfBreadcrumb` — already pull vivid colors from `NAVIGATION_ENTITIES` SSoT which uses `--text-info`/`--status-purple`/`--text-success`/`--text-warning`, so they were left untouched.) **Fix:** re-mapped only the flat entries to vivid theme-aware tokens (reuse same tiers as launchpad tiles, zero new tokens): crm/dashboard/tasks/journal/documents/reports/email-analytics/ai-inbox/operator-inbox→`--text-info`; calendar→`--status-purple`; communications/teams/account/profile→`--hue-teal`; spaces/assets→`--hue-indigo`; setup/preferences→`--hue-pink`. Already-vivid entries (sales/contacts/accounting/invoices/vat/bank/financial-intelligence→`--text-success`; leads/pipeline/notifications/privacy/procurement/quotes/rfqs→`--text-warning`; obligations/efka/security/admin→`text-destructive`) preserved. 1 file. Verified: ratchet `--all` 0/0, tsc clean. |
