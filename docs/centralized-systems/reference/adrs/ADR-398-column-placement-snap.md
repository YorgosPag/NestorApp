...
**Status:** 🟢 Column→Beam axis snap + ghost coloring DONE · 🟢 bugfix snap-indicator suppression DONE — UNCOMMITTED 2026-06-20 · 🔴 browser-verify + commit
...
## 6. Changelog
- **2026-06-20 (bugfix: snap indicators suppressed during column placement, UNCOMMITTED)** — Giorgio report: με εργαλείο «Κολώνα» + snaps ON, hover πάνω σε **δοκάρι/κολώνα** έκρυβε ΟΛΕΣ τις γλυφές + ετικέτες (Γωνία/Μέσο/Κέντρο στήλης & δοκαριού). **Ρίζα (regression του ίδιου ADR):** στο `snap-scheduler.runSnapDetection`, η `applyColumnPlacementContext` → `writeBeamAxisSnap` έκανε `setFullSnapResult(null)` + short-circuit όταν status=`beam` → ο `SnapIndicatorOverlay` (διαβάζει `getFullSnapResult`) δεν είχε τίποτα. **Ο short-circuit ήταν περιττός:** το `NearestSnapEngine` (`bim-beam`, §3.1) ΗΔΗ δίνει beam-axis snap με ετικέτα μέσω `findSnapPoint`, και το `BimCharacteristicSnapEngine` δίνει corner/mid/center για κολώνα ΚΑΙ δοκάρι. **Fix (ΕΝΑ pipeline, thin reader):** (1) αφαίρεση `applyColumnPlacementContext`/`writeBeamAxisSnap`/`setFullSnapResult(null)` — το ενιαίο snap τρέχει ΠΑΝΤΑ· (2) NEW pure `resolveColumnGhostStatusFromSnap(worldPos, entities, snapEntityId)` παράγει το ghost status από το ΑΠΟΤΕΛΕΣΜΑ του snap (`snapResult.snapPoint.entityId` → δοκάρι 🟢 / κολώνα 🔴) + light `findColumnOverlap` fallback (precedence **overlap > beam > neutral**)· (3) commit (`mouse-handler-up`) ευθυγραμμίστηκε στο ίδιο `drawCorner ?? findSnapPoint` (beam-axis από NearestSnapEngine → commit ≡ ghost)· center-anchor μένει μέσω `getColumnGhostStatus()==='beam'`. **DELETE διπλότυπα:** `findColumnBeamAxisSnap`, `resolveColumnPlacementContext`, `ColumnBeamAxisSnap`, `ColumnPlacementContext` (η beam-axis projection ζει πια ΜΟΝΟ στο NearestSnapEngine). `ColumnPlacementGhostStatusStore` + `useColumnGhostPreview` + center-anchor αμετάβλητα. Tests: rewrite `column-placement-snap-context.test.ts` → 10 jest (ghost-status από snap: beam/overlap/footprint-fallback/precedence/neutral)· `beam-axis-projection` 6 GREEN· `snapping/` GREEN. 🔴 tsc(Giorgio) + browser-verify + commit.
