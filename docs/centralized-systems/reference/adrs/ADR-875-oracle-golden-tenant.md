...
| 2026-09-24 | **Εξωτερική αλλαγή από το [ADR-876 §5](./ADR-876-credential-link-surfaces.md#57-χρησμός-351-adr-875--απόφαση-giorgio)**: η δημόσια πόρτα `/vendor/quote/[token]` του `GOLDEN_PUBLIC_TEMPLATES` είναι πλέον **μόνο ανακατεύθυνση** στο `/vendor/quote#t=…` (το διαπιστευτήριο στο fragment — ο server δεν το βλέπει ποτέ, άρα ούτε το SSR). Η σπορά (`emulator-seed-golden.ts`) διαβάζει το token από το fragment. `golden-catalog.js` και `.i18n-ssr-oracle-baseline.json` **ΔΕΝ** αγγίχτηκαν — αναπροσαρμογή της πόρτας + ξανασπορά = απόφαση Giorgio, **μετά** το push Φ2.2+Φ2.4. |
...
### 14.8 🔴 ΕΥΡΗΜΑ — η άγκυρα Ι7 του CHECK 3.60 έχασε την προϋπόθεσή της με τη σπορά του §13 (2026-09-23)

`scripts/__tests__/workspace-scope-gate.test.js` **Ι7** («ο ισομορφισμός δουλεύει στην ΠΡΑΓΜΑΤΙΚΗ baseline του 3.51»)
απαιτεί `moved > 100`. Μετρημένο στο HEAD `4279ed42`: **moved = 19**. Η baseline της σποράς (§13) έχει **586 / 724**
ταυτότητες **ήδη** με πρόθεμα `/o/alpha-techniki/…` (οι persona γεμίζουν το `[workspace]`)· ό,τι έχει ήδη πρόθεμα
**δεν μετακινείται**. Η άγκυρα βαθμονομήθηκε στην ανώνυμη εποχή, όπου κάθε `/o/**` ήταν `/o/ssr-probe/…`.
⚠️ **Δεν χαλαρώθηκε ο πήχης.** Θεραπεία (απόφαση): να μετρά η Ι7 τις ταυτότητες **χωρίς** πρόθεμα (138) ή να
κρατά τον δικό της σταθερό κατάλογο-δείγμα — όχι «> 19». Ανεξάρτητο από το ADR-876 (η baseline είναι ίδια με το HEAD).
