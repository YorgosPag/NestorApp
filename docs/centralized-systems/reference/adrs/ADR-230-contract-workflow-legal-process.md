...
| 2026-04-22 | **Fix v1 (client hook)**: `LegalTabContent.handleCreate` consumava `unit.project` (denormalizzato, stale). Migrato a `usePropertyHierarchyValidation`. Regressione residua: il hook client fa `subscribeDoc('BUILDINGS', …)` e le rules Firestore rifiutano la read quando il building doc non ha `companyId` persistito (falso negativo). | Claude |
| 2026-04-22 | **UX clarity — ContractCard live-fallback**: ContractCard mostrava "Μη ανατεθ." per professional assegnati DOPO la firma (snapshot del contratto è intenzionalmente immutabile per `signed`/`completed` — legal-contract.service.ts:312). Sintomo: ProfessionalsCard mostra Georgios Pagonis (live), ContractCard mostra "Μη ανατεθ." → utente confuso. Fix: ContractCard ora accetta prop opzionale `associations` (da `useEntityContactLinks`). Se lo snapshot è null ma esiste un'associazione live per quel ruolo → mostra il nome live in italic + badge giallo "τρέχουσα ανάθεση" con tooltip che spiega "assegnato dopo la firma, non fa parte del snapshot legale". Snapshot (= legal historical record) resta single source of truth; associations è esplicitamente segnalato come overlay informativo. Chiavi i18n `liveNotInSnapshot` + `liveNotInSnapshotTooltip` in `common-sales.json` (EL+EN). | Claude |
| 2026-04-22 | **Fix v2 (SSoT server-side, definitivo)**: Nuovo hook `usePropertyHierarchyApi` (`src/hooks/sales/usePropertyHierarchyApi.ts`) che consuma `/api/properties/[id]/hierarchy` (Admin SDK, bypass rules). `LegalTabContent` ora usa questo hook invece di `usePropertyHierarchyValidation`: `resolvedProjectId = hierarchy.project.id ?? unit.project`, `resolvedBuildingId = hierarchy.building.id ?? unit.buildingId`, errori ordinati Company→Project→Building→Floor. `usePropertyHierarchyValidation` rimane per `SellDialog`/`ReserveDialog` (real-time subscribeDoc, nessun bug riportato). Nessuna modifica server-side. Eventi `RealtimeService` (`BUILDING_PROJECT_LINKED`, `CASCADE_PROPAGATED`…) triggerano refetch del hook API. | Claude |
| 2026-07-17 | **Client mutation-action SSoT (ADR-584 / N.18)** — τεκμηρίωση: **ADR-234 changelog 2026-07-17**. Τα 4 actions του `useLegalContracts` (`createContract` / `transitionStatus` / `updateContract` / `overrideProfessional`) έγραφαν το ίδιο try/catch→`{success,error}` archetype με τα 3 finance hooks του ADR-234 — **+3 clones** στο jscpd, γι' αυτό μπήκαν στην ίδια κεντρικοποίηση. Καλούν πλέον `runGatewayAction` (`src/lib/mutations/gateway-action.ts`)· το τοπικό `fetchJson` πήγε στο `src/lib/api/fetch-json.ts`. **Καμία αλλαγή συμπεριφοράς**: το hook είναι scoped από `contractId`, ΟΧΙ από `propertyId`, άρα **παραμένει χωρίς guard** — το `blocked` απλώς παραλείπεται (spec, όχι drift). Ο Firestore subscription των brokerage agreements + το `currentPhase` μένουν άθικτα (δεν είναι μέρος του archetype). Καλύπτεται από `src/hooks/__tests__/gateway-action-bindings.test.tsx` (τα 4 actions asserted by name — πρώτα tests που απέκτησε ποτέ το hook). | Claude Code |
