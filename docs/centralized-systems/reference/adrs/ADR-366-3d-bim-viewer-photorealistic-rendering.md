...
- **Performance**: per frame κόστος = N×(2 BIM scene passes color/depth off + 1 cap quad) όπου N=active planes (1-6). Box mode worst case = 12 scene passes + 6 cap quads. Acceptable για moderate scenes· future optimization Phase 7.0b candidate = custom shader για back+front σε ένα pass.
...
DEFERRED μετά Phase 7.0a: (b) hatched per-material cut Phase 7.1+ (ADR-363 ShaderType), (c) 2D Live Section Panel GenArc port Phase 7.0B (4 PORT_WITH_ADAPTATION files), (d) selection-aware emphasis intersect Phase 7.0+ TBD.
