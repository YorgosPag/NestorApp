...
|---|---|---|
| 2026-07-27 | **N.7.1 splits στον πυρήνα κάμερας/σκηνής + το `view-focus-bus` γίνεται καταναλωτής του SSoT (Opus 5).** Τρία αρχεία είχαν περάσει το όριο των 500 γραμμών καθώς προσγειώθηκε το ADR-650 M5α.2· κανένα δεν «κουρεύτηκε» — **εξήχθη** συνεκτική ευθύνη σε κάθε περίπτωση, με μηδενική αλλαγή συμπεριφοράς: (1) **NEW `bim-3d/viewport/viewport-camera-nav-math.ts`** — καθαρά μαθηματικά πλοήγησης που **δεν** αλλάζουν προβολή ούτε κάνουν fit: `readCameraZoom`/`applyCameraZoom` (η per-projection σημασία του «zoom»: `camera.zoom` σε ortho, απόσταση σε perspective), `computeScreenPanOffset`, `applyRollAroundForward`, `applyOrthoFrustum`. Ο `viewport-camera.ts` (531→~495 γρ.) κρατά μόνο τα λεπτά, stateful wrappers του. (2) **NEW `bim-3d/scene/scene-manager-camera.ts`** — το framing/orbit-pivot façade του `ThreeJsSceneManager` (515→490 γρ.) πάνω σε **structural host view** (μόνο public μέλη: `bimLayer`/`viewport`/`dxfConverter`/`renderer`), οπότε ο manager περνά `this` — κανένα deps-object literal να μένει πίσω από τον κώδικα. Ο `disposed`/`initialCameraFitDone` μανδαλωτής μένει στην κλάση, εκεί που ζει η κατάσταση. (3) **NEW `bim-3d/converters/dxf-overlay-entity-color.ts`** — ο καταρράκτης χρώματος ByLayer/ACI/trueColor βγήκε από τον `DxfToThreeConverter` (510→454 γρ.)· είναι ο **3Δ** resolver (int για `LineSegments` material) και τεκμηριώνεται ρητά ως **διαφορετικό ερώτημα** από τον 2Δ `resolveEntityColorHex` — να μην συγχωνευθούν. 48/48 tests του converter + 15/15 viewport-zoom πράσινα. (4) 🚫 **CHECK 3.7 (SSoT ratchet) έπιασε νέο `new Set<FocusListener>()`** στο ολοκαίνουργιο `view-focus-bus.ts`: το bus γεννήθηκε ως αντιγραφή του `clash-focus-bus` (grandfathered baseline), άρα κληρονόμησε χειρόγραφο pub/sub. Ξαναγράφτηκε πάνω στο `createExternalStore` (payload-carrying μορφή: το request **είναι** η τιμή του store, thin wrapper για το domain-shaped `subscribe`). **Το μάθημα**: «αντιγράφω το διπλανό αρχείο» μεταφέρει και το χρέος του — το baseline προστατεύει το παλιό αρχείο, όχι το νέο. | Opus 5 |
| 2026-07-26 | **ADR-711 / ADR-364 §10.15 — το `CommentReplyInput` δηλώνει πλέον τον Κ3 του.** Ο κλάδος `Escape` του `handleMentionKey` καλεί `noteLocalEscapeOwner(e.nativeEvent, 'comment-reply/mention-list')` πριν κλείσει τη λίστα @-mention. Πριν, ο Μηχ. 1 του ADR-364 τύπωνε `SHADOW-OWNER` σε **κάθε** κλείσιμο της λίστας — θόρυβος πάνω στον ίδιο του τον **θεσμοθετημένο** ιδιοκτήτη (§10.5 Κ3), που εκπαιδεύει στην αγνόηση του ελέγχου. Η δήλωση **δεν σιωπά** τον έλεγχο: αδήλωτος ωμός ιδιοκτήτης εξακολουθεί να βγαίνει `shadow-owner` (καρφωμένο με test θετικού control) και ο `starved` δεν καλύπτεται. Καμία αλλαγή συμπεριφοράς για τον χρήστη. Επιπλέον, ο lightbox και όλα τα dialogs των σχολίων κληρονομούν **δωρεάν** από το `dialog.tsx`: ζωντανό `Tab` μέσα στο modal και επαναφορά focus στο στοιχείο που τα άνοιξε. |
| 2026-07-26 | **C.2.Q6 ΣΥΝΗΜΜΕΝΑ ΣΧΟΛΙΩΝ — ΖΩΝΤΑΝΕΜΑ ΤΗΣ ΝΕΚΡΗΣ ΑΛΥΣΙΔΑΣ (Giorgio· Opus 5).** Το Φ9/C.2 είχε φτιάξει και τα τρία κομμάτια αλλά **κανένα δεν ήταν συνδεδεμένο** (grep-verified σε όλο το `src/`): `createComment` έγραφε σκληρά `attachments: []` και το `CreateCommentInput` δεν είχε καν πεδίο· ο `updateComment` ήταν ο **μόνος** γραφέας attachments και **δεν καλούνταν από πουθενά**· ο `CommentAttachmentUploader.tsx` **δεν εισαγόταν από πουθενά, ούτε από test**. Άρα κανένα σχόλιο δεν αποκτούσε ποτέ συνημμένο ⇒ το `lightboxIndex` δεν γινόταν ποτέ non-null ⇒ το `CommentAttachmentLightbox` **δεν αποδιδόταν ποτέ** (γι' αυτό ήταν αμέτρητο και το ESC σενάριο του ADR-364 §10.14.Η.5.β). Κλασικό «optional wiring never done» — ο ίδιος ο uploader το ομολογούσε στο header του («upload happens at comment-submit time — caller's responsibility») και **κανείς δεν έγινε ποτέ ο caller**. **🔴 ΑΝΑΤΡΟΠΗ ΤΗΣ ΚΑΤΑΓΕΓΡΑΜΜΕΝΗΣ ΣΕΙΡΑΣ**: η εγγραφή 2026-05-24 προέβλεπε **two-step (createComment → upload → updateComment patch)**. Υιοθετείται το αντίστροφο — **upload → ΕΝΑ `createComment` με τα attachments** (ο caller δεσμεύει το `commentId` με `generateBimCommentId()`, ΙΔΙΑ πηγή id, N.6· νέο προαιρετικό `CreateCommentInput.id`). Λόγος = **ασυμμετρία αποτυχίας**: το two-step, αν σκάσει μετά το create, αφήνει σχόλιο που **μόνιμα και σιωπηλά** ισχυρίζεται ότι δεν έχει συνημμένα (ζημιά ακεραιότητας, μη ανακτήσιμη) + παράθυρο όπου το subscription το δείχνει χωρίς εικόνες· το one-step, αν σκάσει μετά το upload, αφήνει **μόνο bytes** — και αυτά τα μαζεύει το ADR-694 (mark-only + sweeper, ωρίμανση 7 ημερών). Προστέθηκε γι' αυτό custody κανόνας `bim-comment-attachments` (`COLLECTIONS.BIM_COMMENTS` στο `functions/src/config`) ώστε τα ορφανά να είναι **ανακτήσιμα** αντί για αθάνατα `unknown`. **ΑΠΟΚΛΙΣΗ ΑΠΟ ΤΟ C.2.Q6 path**: το research όριζε `bim_comments/{commentId}/{fileId}_{filename}` — απορρίπτεται το `_{filename}`, γιατί βάζει **user-supplied string μέσα στο storage path**, ακριβώς ό,τι απαγορεύει το `image-asset-upload` SSoT. Ισχύον: `companies/{companyId}/bim-comment-attachments/{commentId}/{attachmentId}[-thumb].{ext}` — ντετερμινιστικό (⇒ idempotent, retry αντικαθιστά) και με το `{commentId}` ως **ΦΑΚΕΛΟ**: 5 συνημμένα × 2 objects ανά σχόλιο σημαίνει ότι ο ιδιοκτήτης δεν χωράει στο basename — το ίδιο ακριβώς λάθος που γέννησε το ADR-694 (`…/albedo.jpg` → owner «albedo»). **SSoT**: 4ος καταναλωτής του `image-asset-upload` (ADR-651 Φ.Ε), **όχι** 4ο δίδυμο· `dataUrlToBlob` SSoT για το data-URL thumbnail· path builder δίπλα στα αδέρφια του (`storage-path-bim.ts`). **ΤΟ ΚΕΝΟ ΠΟΥ ΕΛΕΙΠΕ ΕΝΤΕΛΩΣ**: δεν υπήρχε **κανένα** block στο `storage.rules` γι' αυτό το path (grep: μηδέν) — δηλαδή το wiring όπως σχεδιαζόταν θα έσκαγε σε `permission-denied`. Προστέθηκε `@pathId: bim_comment_attachments` (write από απλό μηχανικό, ≤5 MB, `image/(png|jpe?g)` — ΟΧΙ `image/.*`, ώστε να μην περνά SVG) + **πλήρες suite** στο CHECK 3.19 (`bim-comment-attachments.storage.test.ts`, 12 cells, όχι PENDING) + client guard `ALLOWED_EXTS` ώστε client και rules να συμφωνούν για το webp. **+ ΦΡΑΓΜΑ ΕΡΓΟΥ (N.0.2)**: το `NewCommentForm` δεν έφραζε υποβολή χωρίς επιλεγμένο έργο ⇒ το `subscribeByProject` κάνει early-return σε κενό `projectId` και το σχόλιο **δεν εμφανιζόταν πουθενά** (3 τέτοια ορφανά ήδη στη βάση). Πριν το wiring κόστιζε ένα άχρηστο doc· **μετά** θα κόστιζε και ανεβασμένες εικόνες — γι' αυτό διορθώθηκε εδώ αντί να μείνει «δικό του fix»: `hasProject` στη μοναδική πύλη υποβολής + ορατή εξήγηση (`comments.noProjectSelected`, el+en). **Αρχεία**: 5 NEW (`comment-attachment-upload.service.ts`, `use-new-comment-submit.ts`, tests του service, storage-rules suite, custody tests) + 8 MOD. **Tests**: custody 18/18 · comments **25/25** (6 νέα: 2 objects ανά συνημμένο, thumbnail πάντα `.jpg`, κενή λίστα ⇒ μηδέν I/O, **άκυρο 2ο αρχείο ⇒ ΚΑΝΕΝΑ ανέβασμα** — εκεί κρέμεται η απόφαση σειράς, webp/size απόρριψη) · CHECK 3.19 OK (19 blocks / 8 coverage / 11 pending) · jscpd:diff καθαρό (N.18). ✅ **Το storage-rules suite ΕΤΡΕΞΕ** (2026-07-26, βλ. επόμενη εγγραφή): **16/16** για το `bim_comment_attachments` σε πραγματικό emulator. ΟΧΙ tsc (N.17). Uncommitted (commit = Giorgio). | Opus 5 |
| 2026-07-26 | **C.2.Q6 — ΟΙ ΔΥΟ ΕΠΑΛΗΘΕΥΣΕΙΣ ΕΓΙΝΑΝ. Η αλυσίδα συνημμένων είναι ΖΩΝΤΑΝΗ end-to-end (Giorgio· Opus 5).** **(Α) storage rules — από «δηλωμένη» σε «αποδεδειγμένη» κάλυψη.** Ο λόγος που δεν είχε τρέξει ποτέ δεν ήταν το περιβάλλον: το `firebase-tools` κάνει `JSON.parse` **ανά chunk** του stdout του rules-runtime, τα 25 compiler warnings του `storage.rules` φούσκωναν την απάντηση στα ~25 KB, και το Windows pipe κόβει στα **8192 bytes** ⇒ αποτυχία parse **ακριβώς στη θέση 8192** ⇒ κρέμασμα χωρίς κανένα μήνυμα. Πλήρης μηχανισμός + το αναλλοίωτο «το `storage.rules` οφείλει να μεταγλωττίζεται χωρίς warnings»: **ADR-301 §5**. Το suite μας βγήκε **10/12**: τα δύο `write → allow` cells απέτυχαν επειδή δεν έδινε `contentType` και το default του harness (`application/octet-stream`) απορρίπτεται **επίτηδες** από αυτό το path — δηλαδή το suite ήταν υπο-προδιαγεγραμμένο, όχι ο κανόνας λάθος. Προστέθηκε `ATTACHMENT_CONTENT_TYPE` (μοτίβο `topo-surfaces`) **+ 4 pins content-type**: JPEG **allow** (καρφώνει ότι το δεύτερο σκέλος του `image/(png\|jpe?g)` είναι ζωντανό — μονοχαρακτηρικό typo θα το σκότωνε σιωπηλά), SVG/WebP/octet-stream **deny** ⇒ η απόφαση «ΟΧΙ `image/.*`» αυτής της εγγραφής είναι πλέον **δοκιμασμένη άμυνα stored-XSS**, όχι μόνο γραμμένη πρόθεση ⇒ **16/16**· σύνολο σουίτας **8 suites / 111 tests** πράσινα. **(Β) ESC στο lightbox — ΠΑΤΗΘΗΚΕ** (λεπτομέρεια: ADR-364 §10.14.Η.5.γ). Η μέτρηση απαίτησε **τρίτη** διόρθωση, που είναι και το πιο σοβαρό εύρημα της συνεδρίας: 🔴 **το `BimCommentDetailsPanel` έριχνε ΟΛΟΝ τον DxfViewer στο πρώτο κλικ σε οποιοδήποτε σχόλιο.** Ο selector `useBimCommentsStore((s) => s.replies[commentId] ?? [])` επέστρεφε **νέο `[]` σε κάθε κλήση**· ό,τι επιστρέφει selector μέσα στο hook γίνεται snapshot του `useSyncExternalStore` και συγκρίνεται με `Object.is` ⇒ κάθε κλήση έμοιαζε με αλλαγή ⇒ `forceStoreRerender` σε βρόχο ⇒ `Maximum update depth exceeded` ⇒ error boundary. **Ήταν ο τελευταίος νεκρός κρίκος της ίδιας αλυσίδας**: ακόμα κι όταν το σχόλιο αποκτούσε επιτέλους συνημμένο, το panel που φιλοξενεί το lightbox κατέρρεε πριν αποδοθεί. Διόρθωση ως SSoT στο store (`NO_REPLIES` παγωμένο + `selectRepliesFor()`, ίδια σύμβαση με τους υπάρχοντες `selectXxx`) + **5 regression tests** που καρφώνουν την αναφορική σταθερότητα με `Object.is` — ένα unit test δεν βλέπει άπειρο βρόχο, βλέπει όμως την αιτία του. **Ζωντανή απόδειξη ολόκληρης της αλυσίδας**: σχόλιο δημιουργήθηκε με PNG από τη φόρμα, η μικρογραφία απέκτησε πραγματικό `firebasestorage.googleapis.com` URL (⇒ path builder + upload + rules + πεδίο `attachments` όλα σωστά), το master-detail άνοιξε, το lightbox απέδωσε «Εικόνα 1 από 1». **Tests**: 15 suites / 135 tests (stores+comments+escape-bus+shortcuts) πράσινα· `jscpd:diff` καθαρό (N.18)· CHECK 3.19 OK. ΟΧΙ tsc (N.17). Δήλωση: **✅ Google-level: ΝΑΙ** — και τα δύο ερωτήματα απαντήθηκαν με **μέτρηση**, οι αιτίες διορθώθηκαν στη ρίζα, και καθεμιά απέκτησε gate (storage suite εκτελέσιμο με μία εντολή· content-type pins· selector stability tests). | Opus 5 |
| 2026-07-15 | **ViewCube HOME → home iso + ZOOM-TO-FIT (Giorgio· Opus 4.8).** Το σπιτάκι του ViewCube έκανε μόνο re-orient (`canonicalViewService.snapHome()` → iso-ne), κρατώντας την τρέχουσα απόσταση — το σχέδιο μπορούσε να μείνει εκτός οθόνης. Τώρα «όταν πατάω το σπιτάκι, να εμφανίζεται πάντα όλο το σχέδιο»: νέα `ViewportCamera.frameHome(min,max)` που κάνει **home orientation + fit-extents σε ΜΙΑ animation** (AutoCAD/Revit «Home»). `onHome` (scene-setup) παίρνει τα combined BIM∪DXF bounds μέσω νέου dep `getSceneFramingBounds` (`computeSceneFramingBounds`, scene-manager-construct)· κενά bounds → fallback στο παλιό re-orient. **N.18 de-dup (jscpd):** εξήχθη `animatePerspectiveTo()` (κοινό tween των iso-snap/home/home-fit) + `ensurePerspectiveActive()` (κοινό ortho→perspective handoff — καθάρισε προϋπάρχον clone snap↔goHome) + `areFiniteBounds(min,max)` SSoT στο `finite-bounds.ts` (ADR-537 guard, μοιράζεται με `frameBounds`/`isFiniteBox3`). 5 MOD src + 1 MOD test· canonical-views 12/12 GREEN· jscpd καθαρό. ΟΧΙ tsc (N.17). Uncommitted (commit = Giorgio). | Opus |
| 2026-07-13 | **Phase 3 (`DxfToThreeConverter`) — VIEW-DEPENDENT FRUSTUM CULLING + SCREEN-SIZE TEXT LOD (ADR-645 Φάση C· Opus 4.8).** Στο multi-floor «Όλοι οι όροφοι», ό,τι είναι εκτός κάμερας ή πολύ μικρό δεν σχεδιάζεται (Revit/Navisworks declutter). **Frustum = three-native, ΟΧΙ νέος μηχανισμός:** ο underlay pass κάνει ήδη `renderer.render(root, camera)` (per-object cull)· δύο αλλαγές το ενεργοποιούν ντετερμινιστικά — `geo.computeBoundingSphere()` μία φορά στα line color-buckets (`buildLineGroup`) + νέο `AtlasTextMeshBuilder.finalize()` που γυρίζει `frustumCulled=true` όταν ο streamed/inline build ολοκληρωθεί (bounds τελικά· ΠΟΤΕ mid-stream). Off-screen όροφος = μηδέν draw. **Screen-size text LOD = per-fragment shader discard (Figma/Revit-grade, zero CPU/frame, zero rebuild):** ΝΕΟ `glyph-atlas-text-lod.ts` (`applyTextLodMaterial`) κάνει `onBeforeCompile` inject στο stock `MeshBasicMaterial` (καμία νέα εξάρτηση)· κάθε glyph baked `aEmVec` (vertical extent BL→TL, rotation-aware)· vertex shader προβάλλει `vertex` vs `vertex+aEmVec` σε NDC → pixel-delta = on-screen ύψος· κάτω από `TEXT_LABEL_MIN_PX`=8 (SSoT `dxf-import-thresholds.ts`) → `discard`. **Projection-agnostic** (perspective+ortho, absorbs group scale)· το viewport uniform το τροφοδοτεί το `mesh.onBeforeRender` (fires μόνο για non-culled). Pure ref `projectedEmPixelHeight` (GLSL mirror, unit-tested). Streaming/cancellation + selectable/hoverable **άθικτα**. **Deviation:** vertical-height LOD (όχι overlap-declutter) — ίδιο readability-floor μοντέλο με το 2Δ line LOD (`computeInstanceCount`). 1 NEW src + 3 MOD (mesh/converter/config) + 1 NEW test + 1 MOD test· 60 suites/442 tests GREEN· jscpd καθαρό. Boy-scout: typo «ADR-644 Φ.B»→«ADR-645 Φ.B». Λεπτομέρειες: ADR-645 §3 Φ.C + §8. ✅ **browser-verified 2026-07-13** (Giorgio, 40MB «Όλοι οι όροφοι»: «λειτουργεί σωστά, ξεκούραστος ο χειρισμός»). Uncommitted (commit = Giorgio). | Opus |
| 2026-07-12 | **Phase 3 (`DxfToThreeConverter`) — SHARED GLYPH ATLAS text (ADR-645 Φάση B· Opus 4.8).** Ο per-text `CanvasTexture` (χιλιάδες textures στο multi-floor «Όλοι οι όροφοι» = GPU-memory blowout + upload stalls) αντικαταστάθηκε από **ΕΝΑ shared `GlyphAtlas`** + **merged BufferGeometry ανά όροφο** (ένα draw call/όροφο). **NEW:** `glyph-atlas.ts` (raster atlas 2048², shelf-pack `packRect`, lazy cell ανά (face,char), glyphs **λευκά**→alpha μέσω ADR-557 `paintTextRun`/`getGlyphRun` — καμία νέα εξάρτηση, κανένα δεύτερο font mechanism), `glyph-atlas-text-layout.ts` (pure `layoutTextGlyphs`: em-box anchor `resolveTextEmBox` = ΙΔΙΟ με το hover halo· lines/widthFactor/oblique/tracking/rotation **per corner**· αλγεβρικά ίδιο με το παλιό `orientTextPlane`), `glyph-atlas-text-mesh.ts` (`AtlasTextMeshBuilder`: `MeshBasicMaterial({map,vertexColors})` — white coverage × vertex tint· streamed `flush`+`setDrawRange`· incremental `boundingBox` μόνο στα γραμμένα glyphs· NaN-guard), `dxf-text-font-resolution.ts` (extract `resolveTextFont`, pure, SSoT reuse). Ο converter χτίζει **per-floor builders** (`makeFloorBuilders`)· atlas **converter-owned + persistent** (cells cache· `disposeRoot` dispose μόνο geometry+material, ΟΧΙ το shared texture). **Selectable/hoverable ΑΝΕΠΗΡΕΑΣΤΟ** — pick (`dxf-wireframe-hit-test`, plan-space) + glow (`dxf-entity-outline`, em-box) είναι entity-driven, ΠΟΤΕ raycast του mesh. **ΔΙΑΓΡΑΦΗΚΕ** `dxf-text-3d.ts` (+2 tests). **Deviation:** χάνεται cross-glyph kerning (§7 MSDF). 4 NEW src + 1 MOD + 3 NEW test (22 νέα, 73 GREEN)· jscpd καθαρό. Λεπτομέρειες: ADR-645 §8. 🔴 browser-verify (40MB «Όλοι οι όροφοι»: labels crisp, 1 atlas, responsive· selection/hover text). | Opus |
| 2026-07-12 | **Phase 3 (`DxfToThreeConverter`) — INCREMENTAL SCENE STREAMING, freeze fix (ADR-645 Φάση A· Opus 4.8).** Το 2Δ→3Δ σε μεγάλο DXF (40MB, 468 text × ορόφους) **πάγωνε** τον browser: ο `DxfToThreeConverter` έχτιζε **σύγχρονα, all-at-once** μέσα σε React commit effect, με το `468× buildDxfTextMesh` (canvas+glyph engine+`CanvasTexture` upload ανά text) ως hotspot. **FIX (big-player-grade streaming, μηδέν δεύτερο rAF loop):** ΝΕΟ SSoT `bim-3d/scene/incremental-scene-builder.ts` (time-sliced runner πάνω στο `UnifiedFrameScheduler` one-shot rAF — frame budget ~8ms, progress, clean cancel· pure+DI→jest). Ο converter σπάει σε **lines-first** (φθηνά line color-buckets → group στη σκηνή ΑΜΕΣΩΣ → `getBounds()` ⇒ camera-fit χωρίς αναμονή) + **text-streamed** (budgeted batches, view-priority bigger-first μέσω `getEntityBBox` area SSoT, `markSceneDirty` per batch). Gate `INCREMENTAL_3D_MIN_ENTITIES`=40 (μικρά scenes → σύγχρονο inline, κανένα loader flash). Multi-floor: text ΟΛΩΝ των ορόφων σε ΕΝΑ runner. Cancellation στο `disposeRoot()` (re-sync/floor-switch/unmount) → race-free. ΝΕΟ progress SSoT `Dxf3dStreamProgressStore` (zero React state) + overlay leaf `Dxf3dStreamProgressLeaf` (Forge-style «loading %», i18n el+en `viewport.streaming.*`). `ThreeJsSceneManager` περνά `onSceneDirty` στον converter. 5 NEW/MOD src + 1 test (56/56 GREEN) + i18n. **Εκκρεμεί:** Φάση B (shared glyph atlas — multi-floor scale), Φάση C (frustum/LOD). Λεπτομέρειες: ADR-645 §8. 🔴 browser-verify (40MB αρχείο: wireframe+fit ΑΜΕΣΩΣ, labels γεμίζουν, responsive). | Opus |
| 2026-06-30 | **§B.2.Q1 — 2D BIM hover info → status bar (επέκταση 3D μηχανισμού στο 2D· Giorgio· UNCOMMITTED, Opus 4.8).** Ο Giorgio: «όταν κάνω hover/click σε BIM οντότητα στο **2D**, εμφανίζεται floating label (`QuickPropertiesHoverPopover`: ΕΠΙΠΕΔΟ/ΧΡΩΜΑ/ΤΥΠΟΣ ΓΡΑΜΜΗΣ) — δεν το θέλω· βάλε την πληροφορία στο **status bar δεξιά από τις συντεταγμένες**, με τον **ίδιο** κώδικα που ήδη υπάρχει στον 3D καμβά». **Έρευνα:** οι `*PersistenceHost` είναι mounted **πάντα** (2D too) → `Bim3DEntitiesStore` ζωντανό στο 2D· το 2D hovered id == BIM id verbatim· ο `DxfEntity.type` φέρει τον BIM τύπο. **Υλοποίηση (FULL SSoT reuse, μηδέν διπλότυπο):** (1) ο `QuickPropertiesHoverPopover` επιστρέφει `null` για **BIM** οντότητες (`isBimEntityType(entity.type)`) — plain DXF (line/circle) κρατά το popover· **Boy-Scout N.0.2:** NEW `isBimEntityType(type)` στο `types/entities.ts` (η λίστα BIM types ΜΙΑ φορά) + ο `isBimEntity` delegate → μηδέν δεύτερη hardcoded λίστα. (2) NEW `resolveBimHoverLinesById(bimId, t)` στο `resolve-bim3d-hover-lines.ts` — id-only lookup (το 2D hover δεν φέρει type) σε walls→columns→beams→slabs, reuse των ΙΔΙΩΝ `bim-entity-formatter` formatters. (3) το `StatusBarBim3DHoverLeaf` **ενοποιήθηκε** σε `StatusBarBimHoverLeaf` (rename): ίδιο markup «Τύπος · διάσταση · κατηγορία», δύο low-freq πηγές — 3D (`QuickProperties3DStore`→`resolveBim3DHoverLines`, αμετάβλητο) ή 2D (`QuickPropertiesStore` hovered id, gated `activeTool==='select'`→`resolveBimHoverLinesById`). `ToolbarStatusBar` περνά `activeTool`. ADR-040 micro-leaf (subscribe μόνο σε 2 low-freq stores). Κάλυψη = wall/column/beam/slab (όπως 3D)· άλλοι BIM τύποι: card suppressed, status κενό (follow-up formatters). 5 αρχεία (1 NEW + 3 MOD + 1 RENAME) + ADR-363/366. Reuse i18n `bim3d` keys (μηδέν νέα κλειδιά). 22 axis-box + 32 isBimEntity-dep jest GREEN. 🔴 browser-verify (hover τοίχο/κολόνα 2D: όχι κάρτα, status bar δείχνει· line κρατά κάρτα) + commit (stage ADR-366· CHECK 6D δεν θίγεται — HoverStore/canvas-draw αμετάβλητα). | Opus |
| 2026-06-29 | **§B.2.Q1 follow-up — 3D status-bar coordinates X/Y/Z real-time (Giorgio· UNCOMMITTED, Opus 4.8).** Ο Giorgio: στο 3D οι συντεταγμένες του status bar **δεν ενημερώνονται** + λείπει ο **κάθετος** άξονας· θέλει **3 συντεταγμένες real-time**. **Ρίζα:** το `ToolbarCoordinatesDisplay` διαβάζει το **2D** world channel (`getImmediateWorldPosition`), που τον γράφει μόνο ο 2D mouse handler → στο 3D μένει stale + είναι εξ ορισμού 2 άξονες. **Υλοποίηση (FULL SSoT reuse, μηδέν νέα μαθηματικά):** NEW zero-React `Bim3DCursorReadoutStore` (3D counterpart του `ImmediatePositionStore.worldPosition`· ξεχωριστό 3D domain όπως `Selection3DStore`/`QuickProperties3DStore`) + NEW writer `bim3d-cursor-readout-writer.ts` που κάνει **reuse** `worldToDxfPlan` (ADR-366 §4 → {x=ανατολή, y=βορράς, z=υψόμετρο} mm) + `raycastFloorPoint`/`resolveActiveFloorElevationMm` (ADR-403, fallback). **Z = πραγματικό σημείο πάνω στη γεωμετρία** (surface height — αλλάζει καθώς σαρώνεις πάνω/κάτω σε στοιχείο)· σε **κενό** → προβολή στο ενεργό δάπεδο. **Ιστορικό:** αρχικά υλοποιήθηκε ως work-plane projection (Giorgio επιλογή «CAD default»), αλλά στο ισόγειο το δάπεδο=z0 → **Z έδειχνε πάντα 0** (Giorgio runtime report)· διορθώθηκε σε surface-hit. Writer **τρέφεται από το RAF slot του `bim3d-pointer-scheduler`** (reuse του ΥΠΑΡΧΟΝΤΟΣ `hit.worldPoint` από τον unified BVH raycast — **μηδέν δεύτερο raycast**, ~20fps throttle = parity με το 2D `worldPosition`)· clear σε leave/unmount. `ToolbarCoordinatesDisplay` έγινε **mode-aware** (ΕΝΑ readout SSoT): 3D readout ενεργό → `X/Y/Z`, αλλιώς 2D `X/Y` όπως πριν· ίδιο imperative textContent (zero React re-render) + ίδιο `formatCoordinateForDisplay` (canonical-mm → display unit, ADR-462). 2D ανέγγιχτο (3D store μένει null). 6 αρχεία (2 NEW + 4 MOD) + 2 NEW test (store 7/7· scheduler writer-mock 12/12). 🔴 browser-verify (Z αλλάζει πάνω σε τοίχο/δοκό) + commit (stage ADR-366· CHECK 6D → use-bim3d-pointer-handlers + bim3d-pointer-scheduler). | Opus |
| 2026-06-29 | **§B.2.Q1 — hover info: floating card → status-bar inline (Giorgio screenshot· UNCOMMITTED, Opus 4.8).** Ο Giorgio: «δεν θέλω να εμφανίζεται το floating label στο hover· αν γίνεται, στο status bar δεξιά από τις συντεταγμένες». **Υλοποίηση (FULL SSoT reuse, μηδέν διπλότυπο):** η κάρτα-φάντασμα `QuickProperties3DHoverPopover` **διαγράφηκε** (+ αφαίρεση mount/import από `BimViewport3D.tsx`)· η ίδια πληροφορία διαβάζεται πλέον **inline** στο `ToolbarStatusBar` μέσω NEW micro-leaf `StatusBarBim3DHoverLeaf` (ADR-040: subscribe ΜΟΝΟ στο `QuickProperties3DStore` low-freq + read `Bim3DEntitiesStore` once), δεξιά από το coordinates block. Μορφή (Giorgio): πλήρης μονόγραμμη «Τύπος · διάσταση · κατηγορία» (π.χ. `Τοίχος · 402.35mm × 3.00m · Εξωτερικός`). **SSoT:** ίδιο store (`QuickProperties3DStore`, ο writer στο `use-bim3d-pointer-handlers` αμετάβλητος) + ίδιος pure formatter (`bim-entity-formatter`)· η lookup-λογική `resolveLines` (που ζούσε μέσα στο popover) εξήχθη σε NEW shared `resolve-bim3d-hover-lines.ts` ώστε να μην μείνει διπλό switch. 2D view ανέγγιχτο (store μένει cleared → leaf renders nothing). 6 αρχεία (2 NEW + 3 MOD + 1 DELETE). 🔴 browser-verify + commit (stage ADR-366· CHECK 6D → BimViewport3D). | Opus |
| 2026-06-29 | **§4 coordinate-transforms — `getPixelWorldSize` ortho support (fix «τεράστια νούμερα μετακίνησης» στο Top View· UNCOMMITTED, Opus 4.8).** **Συμπτώμα (Giorgio screenshot):** DXF Viewer → 3D → ViewCube **Top** → επιλογή οντότητας → μετακίνηση → η ετικέτα απόστασης (`TempMoveReadoutOverlay`) γιγαντώνεται και σκεπάζει όλη την οθόνη. **Ρίζα (επαληθευμένη):** το `getPixelWorldSize` (`viewport/coordinate-transforms.ts`, PORT_AS_IS GenArc) επέστρεφε **placeholder `1`** (≈1 m/px) για **κάθε** μη-perspective κάμερα — δηλ. για την **OrthographicCamera** των canonical views (Top/Front/Side). Το constant-px annotation scaling των overlays (`worldH = getPixelWorldSize × textPx`) έβγαζε sprite **~160m×40m** ενώ το ορθογραφικό frustum είναι ~30m → label τεράστιο· επιπλέον τα snap/hit-test tolerances (`×1000 mm/px`) διάβαζαν ~1000 mm/px σε ortho. **FIX (FULL SSoT, μηδέν νέος μηχανισμός):** ortho κλάδος `visibleHeight = (top − bottom) / zoom; return visibleHeight / heightPx` (distance-agnostic, σωστό για ορθογραφική) + floor `clientHeight` στο 1 (anti div-by-zero). Διορθώνει **ταυτόχρονα** ΟΛΟΥΣ τους καταναλωτές σε ortho: `Temp{MoveReadout,WallMoveDim,SnapLabel,OpeningDim}Overlay`, `use-wall-hud-pass`, `use-bim3d-wall-placement`, `dxf-wireframe-hit-test`, `bim3d-edit-drag-snap`. **Boy-Scout (N.0.2):** η ίδια ortho/persp px-world math ήταν duplicated στο `viewport-camera.ts pan()` → αντικαταστάθηκε με κλήση στο SSoT `getPixelWorldSize` (αποδεδειγμένα ταυτόσημος τύπος, removed ~8γρ. διπλότυπο). **Tests:** `coordinate-transforms.test.ts` — το παλιό `toBe(1)` αντικαταστάθηκε με ortho frustum/zoom/distance-invariance + clientHeight-floor assertions. ⚠️ unrelated με το προηγ. wheel-zoom fix. 🔴 browser-verify (Top View move readout κανονικού μεγέθους) + commit (Giorgio· stage ADR-366). | Opus |
