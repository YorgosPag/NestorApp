...
|-------|--------|-------|-------|--------|
| **C.1.a — Logic Foundation** | 🚧 IN PROGRESS 2026-05-25 | Pure builders (`TurntablePathBuilder`, `WaypointPathBuilder`, `keyframe-interpolator`) + Zustand `AnimationStore` + Firestore `bim-animations.service` + `animation-types.ts` + 8 easing presets registry + SSoT extensions (enterprise-id `anm_bim_*`/`rnj_bim_*`, RBAC `bim_animations:*`, audit type `bim_animation`, Firestore collections + rules) + tests (5 suites) | ~12 | ~5-6h |
| **C.1.b — UX / Timeline** | ⏸ PENDING | `TimelineEditor.tsx` (single-track strip + scrubber + diamonds + properties panel) + `WaypointDragHandle.tsx` (3D viewport handles) + `RibbonAnimationContextualTab.tsx` (ADR-345 ribbon integration) + bezier advanced editor + i18n keys (~42×2 locales) + `ViewMode3DStore.animationToolActive` flag | ~6 | ~7-9h |
| **C.1.c — Rendering / Queue** | ⏸ PENDING | `MP4Exporter.ts` (WebCodecs H.264 Main L3.1 + `mp4-muxer`) + `RenderQueueStore.ts` (Zustand FIFO + concurrent=1) + `RenderQueuePanel.tsx` (Floating3DPanel "Renders" tab) + checkpoint/resume logic + notification keys + project_assets integration | ~5 | ~6-8h |
