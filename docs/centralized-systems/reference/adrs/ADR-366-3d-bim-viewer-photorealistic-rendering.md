...
|---|---|---|
| 2026-06-29 | **§B.2.Q1 follow-up — 3D status-bar coordinates X/Y/Z real-time (Giorgio· UNCOMMITTED, Opus 4.8).** Ο Giorgio: στο 3D οι συντεταγμένες του status bar **δεν ενημερώνονται** + λείπει ο **κάθετος** άξονας· θέλει **3 συντεταγμένες real-time**. **Ρίζα:** το `ToolbarCoordinatesDisplay` διαβάζει το **2D** world channel (`getImmediateWorldPosition`), που τον γράφει μόνο ο 2D mouse handler → στο 3D μένει stale + είναι εξ ορισμού 2 άξονες. **Υλοποίηση (FULL SSoT reuse, μηδέν νέα μαθηματικά):** NEW zero-React `Bim3DCursorReadoutStore` (3D counterpart του `ImmediatePositionStore.worldPosition`· ξεχωριστό 3D domain όπως `Selection3DStore`/`QuickProperties3DStore`) + NEW writer `bim3d-cursor-readout-writer.ts` που κάνει **reuse** `raycastFloorPoint` (ADR-403) + `resolveActiveFloorElevationMm` + `worldToDxfPlan` (ADR-366 §4 → {x=ανατολή, y=βορράς, z=υψόμετρο} mm). Z = **προβολή στο ενεργό work-plane δαπέδου** (Giorgio: CAD default — σταθερό/real-time, όπως AutoCAD/Revit). Writer καλείται **synchronous** στο 3D mousemove (φθηνό 1 ray↔plane, no BVH → 1:1 με τον κέρσορα· clear σε leave/unmount). `ToolbarCoordinatesDisplay` έγινε **mode-aware** (ΕΝΑ readout SSoT): 3D readout ενεργό → `X/Y/Z`, αλλιώς 2D `X/Y` όπως πριν· ίδιο imperative textContent (zero React re-render) + ίδιο `formatCoordinateForDisplay` (canonical-mm → display unit, ADR-462). 2D ανέγγιχτο (3D store μένει null). 5 αρχεία (2 NEW + 3 MOD). 🔴 browser-verify + commit (stage ADR-366· CHECK 6D → use-bim3d-pointer-handlers). | Opus |
