...
- **2026-06-23 — Round 7 — Default text height = `drawingScale` SSoT (Revit annotation scale), replacing the inconsistent unit heuristic**.
  ...
  - **Centralisation (2nd consumer, no duplicate)**: `dim-text-renderer.ts` computed the **same** paper→model conversion inline (`dimtxt × effectiveDimscale × unitFactor`). Refactored to `paperHeightToModel(dimtxt, effectiveDimscale, units) × transform.scale` — **byte-equivalent**, so `paperHeightToModel` is now a genuine SSoT with two consumers (ribbon Text + dimension renderer), not a one-off helper. The renderer's `effectiveDimscale` (imported-DXF DIMSTYLE rescue) stays local — it is a different concern (imported metadata) from the ribbon annotation scale. `dim-text-renderer-scene-units.test.ts` still green (6 tests).
