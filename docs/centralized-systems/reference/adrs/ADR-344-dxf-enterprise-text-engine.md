...
| `\\` / `\{` / `\}` | `\\` → `\` | **Escaped literals** — ο ίδιος ο χαρακτήρας, όχι εντολή |
| `^I` / `^J` / `^M` / `^ ` | `^I` | **Caret notation** — control char = `^` + γράμμα με ASCII `letter−64` |

#### Δύο προσθήκες που ο πίνακας **δεν είχε** μέχρι το 2026-07-30

Το «Authoritative» ίσχυε για τα **backslash** codes· δύο οικογένειες έλειπαν εντελώς, και **και οι
δύο υπάρχουν στα αρχεία που μας δίνει ο Giorgio**:

1. **Caret notation (`^`), υποστηρίζεται από ADR-635 Φ C.20.** Ένας control χαρακτήρας
   αποθηκεύεται ως `^` + το γράμμα με ASCII `letter − 64`. Το AutoCAD γράφει **`^I` για κάθε TAB**
   μέσα σε MTEXT — **149 από αυτά** στο δείγμα που το αποκάλυψε. Πριν τη Φ C.20 τίποτα δεν
   αναγνώριζε το `^`, οπότε οι δύο χαρακτήρες περνούσαν ως απλό κείμενο και **ζωγραφίζονταν
   κυριολεκτικά** («*I*I» στον καμβά) ενώ κάθε στηλοθετημένη στήλη κατέρρεε.

   | ακολουθία | σημασία | τι κάνει ο tokenizer |
   |---|---|---|
   | `^I` | TAB | `{ kind:'text', value:'\t' }` |
   | `^J` | LF | `{ kind:'lineBreak' }` |
   | `^M` | CR | **καταναλώνεται** (ζευγαρώνει με `^J`, μηδέν οπτικό αποτέλεσμα) |
   | `^ ` | escaped literal caret | `{ kind:'text', value:'^' }` |
   | οτιδήποτε άλλο | — | κρατά το `^` ως κείμενο και προχωρά **ΕΝΑ** χαρακτήρα |

   ⚠️ Το τελευταίο είναι σχεδίαση, όχι παράλειψη: ένα συνηθισμένο `^` σε πρόζα μένει άθικτο **και
   ο βρόχος δεν μπορεί να κολλήσει**. Καταναλώνονται **μόνο** οι τρεις κωδικοί που το AutoCAD
   πράγματι γράφει.

2. **Escaped literals `\\`, `\{`, `\}` (ADR-737, ΒΛΑΒΗ Ε).** Μέχρι το 2026-07-30 έπεφταν στο
   `default: return null` του `readBackslashToken` και ο χαρακτήρας **εξαφανιζόταν**. 🔴 Και ήταν
   **ασυμμετρία στο ΔΙΚΟ μας round-trip**: η `escapeText()` του `mtext-serializer` **ΗΔΗ**
   παρήγαγε `\\` / `\{` / `\}` — δηλαδή ο εξαγωγέας μας έγραφε αρχείο που ο εισαγωγέας μας
   **αποδεδειγμένα διάβαζε λάθος**, χάνοντας χαρακτήρες σε **κάθε** κύκλο export→import.
   Ταυτίζεται με ezdxf `tools/text.py`: `if char in "\\{}": chars.append(char)`.

#### Κατάσταση υλοποίησης ανά κωδικό — **η γραμμή στον πίνακα ΔΕΝ σημαίνει «ζωγραφίζεται»**

| Κωδικός | Tokenizer | AST | Export | **Render** |
|---|---|---|---|---|
| `\S` (stack) | ✅ | ✅ | ✅ **πλήρες στυλ από ADR-737 §15** — μέχρι τότε ο `serializeParagraph` έκανε `continue` και **δεν εξέπεμπε** `height`/`color`/`fontFamily` της στοίβας | ✅ στο layout· **το flat string** το απέδιδε λάθος μέχρι το ADR-737 (ΒΛΑΒΗ Γ) |
| `\C` / `\c` (χρώμα) | ✅ | ✅ | ✅ **αμφίδρομο από ADR-737 §17** — τα `\C256;`/`\C0;` ξαναδιαβάζονταν ως `ACI 256`/`ACI 0` («κληρονομιά» → «χρώμα»)· ένα ζεύγος `parseAciColorInt`/`encodeAciColorInt` δίπλα-δίπλα | ✅ |
| `\A` (vertical align) | ✅ (πάντα) | ✅ **από ADR-737** | ✅ **από ADR-737** (+ **και μέσα σε στοίβα** από ADR-737 §14) | ✅ **από ADR-737 §14** — render **και** explode· **no-op σε γραμμή ενιαίου ύψους** (τύπος ezdxf) |
| `^I` / `^J` / `^M` | ✅ | ✅ | ✅ | ✅ |
| `\\` `\{` `\}` | ✅ **από ADR-737** | ✅ | ✅ | ✅ |

⚠️ **`\A` — προειδοποίηση για τον επόμενο.** Ο tokenizer παρήγαγε **πάντα** σωστά
`{ kind:'alignment' }`, αλλά η `applyStyleToken` του parser **δεν είχε case** ⇒ `default: break` ⇒
το token καταναλωνόταν στο κενό. **Μετρημένο: 49 εμφανίσεις** στο `47_ergasia.dxf`, **όλες
σιωπηλά**. Το ADR-737 πρόσθεσε `TextRunStyle.verticalAlign?: 0|1|2` (διατήρηση + εκπομπή), και το
**ADR-737 §14 (2026-07-31) έκλεισε και την οπτική εφαρμογή**: SSoT `bim/text/text-vertical-align.ts`
(τύπος ezdxf `vertical_cell_shift`, μετατόπιση ως προς **το ψηλότερο κομμάτι της γραμμής**),
καταναλωνόμενο πανομοιότυπα από render **και** explode.

⚠️ **Δύο πράγματα που ΔΕΝ πρέπει να «απλοποιηθούν»:**
1. **`\A` = no-op σε γραμμή ενιαίου ύψους.** Είναι ο τύπος, όχι παράλειψη — μια «βελτίωση» που το
   κάνει να μετακινεί πάντα, μετακινεί **κάθε κείμενο του σχεδίου**. Καρφωμένο με αρνητικό pin.
2. **`verticalAlign === undefined` ⇒ no-op στη ΖΩΓΡΑΦΙΚΗ, ενώ `?? 0` παραμένει σωστό στο EXPORT.**
   Οι δύο πλευρές έχουν **διαφορετική** σωστή σύμβαση για το «απόν»: στο export ο απών κωδικός
   σημαίνει προεπιλογή, στη ζωγραφική σημαίνει «κανένα αρχείο δεν το ζήτησε».
...
### Φ-3D — πρότυπα κλάδου για in-place επεξεργασία κειμένου σε 3D προβολή

- AutoCAD `MTEXTFIXED` (System Variable, **initial value 2**): https://help.autodesk.com/view/ACD/2024/ENU/?guid=GUID-614588C8-2375-4D1B-A645-A9376D170B02
- AutoCAD In-Place Text Editor: https://help.autodesk.com/view/ACD/2025/ENU/?guid=GUID-25708CA1-3985-411C-9CD4-20C294B13119
- Revit Model Text (select → «Edit Text» tool): https://help.autodesk.com/view/RVT/2025/ENU/?guid=GUID-29383BC3-332D-4021-864D-AE173B7F9E63
- three.js CSS3DRenderer (**η απορριφθείσα εναλλακτική**): https://threejs.org/docs/pages/CSS3DRenderer.html
- Cinema 4D MoText — επεξεργασία στο Attributes panel: https://novedge.com/blogs/design-news/cinema-4d-tip-mastering-3d-typography-techniques-in-cinema-4d
- Figma/Miro-style canvas — overlay συγχρονισμένο με το transform: https://www.freecodecamp.org/news/how-to-create-a-figma-miro-style-canvas-with-react-and-typescript/

---

## Changelog

- **2026-07-31 (β) — 🎨 Ο χάρτης ACI ↔ `DxfColor` έγινε ΑΜΦΙΔΡΟΜΟΣ (ADR-737 §11-6 → §17, Opus 5):** Το `text-toolbar.types.ts` είχε ήδη ζεύγος για το TrueColor (`parseTrueColorInt`/`encodeTrueColorInt`) **και** τα sentinels `DXF_COLOR_BY_LAYER`/`DXF_COLOR_BY_BLOCK` — έλειπε **μόνο** το αντίστοιχο ζεύγος του ACI, και ακριβώς εκεί ζούσε η βλάβη: ο serializer έγραφε σωστά `\C256;`/`\C0;` (ίδιοι αριθμοί με `ezdxf.colors`) ενώ ο parser τους περνούσε **αυτούσιους** σε `{kind:'ACI', index:256}` ⇒ «**κληρονομώ**» γινόταν «**χρώμα υπ' αριθμόν 256**» μετά από έναν κύκλο export→import. Προσιτό από **κλείσιμο ομάδας `{…}`** — το συνηθέστερο ιδίωμα του AutoCAD. 🔴 **Έζησε αόρατη επειδή το `resolveRunColorHex` γυρίζει `undefined` και για τα δύο** (το 256 είναι εκτός του έγκυρου 1..255) ⇒ **ίδια ζωγραφική, διαφορετικό AST**. **Fix:** `ACI_BY_BLOCK=0` / `ACI_BY_LAYER=256` + `parseAciColorInt` / `encodeAciColorInt` δίπλα-δίπλα· το `TrueColor` **εξαιρείται στον τύπο** (`Exclude<DxfColor,…>`), όχι σε runtime έλεγχο. **Files:** `types/text-toolbar.types.ts`, `parser/mtext-parser.ts`, `serializer/mtext-serializer.ts` + NEW `__tests__/mtext-color-roundtrip.test.ts` (18). **Πλήρης αφήγηση/μετρήσεις:** ADR-737 §17.
- **2026-07-31 — 🐛 Ο serializer της Φάσης 1 δεν εξέπεμπε το στυλ της στοίβας `\S` (ADR-737 §11-5 → §15, Opus 5):** Ο `serializeParagraph` έκανε `continue` για κάθε `TextStack` και **παρακάμπτε τον `serializeStyleDiff`** ⇒ `height`/`color`/`fontFamily` της στοίβας **δεν γράφονταν ποτέ**. Το §14 δεν το είχε λύσει — το είχε **ξεπεράσει** ανοίγοντας δεύτερο κλάδο diff μόνο για το `\A`, και ακριβώς αυτός ο δεύτερος κλάδος **ήταν** το σφάλμα (δύο διαδρομές diff = δύο λεξιλόγια που αποκλίνουν στην επόμενη προσθήκη πεδίου). **Fix με αφαίρεση:** το `TextStack.style` είναι `Pick<TextRunStyle, …>` **4 από 12** πεδία ⇒ η στοίβα είναι **μερική δήλωση**, όχι εξαίρεση· προβάλλεται σε πλήρες στυλ **κληρονομώντας** την τρέχουσα κατάσταση και περνά από τον **ίδιο** διαφοριστή με τα κανονικά runs. ⚠️ **Κληρονομιά και όχι προεπιλογή**, γιατί το `\f` γράφει γραμματοσειρά **μαζί με** bold/italic — πεδία που η στοίβα δεν κουβαλά (αρνητικό pin). Ο πίνακας «κατάσταση ανά κωδικό» ενημερώθηκε (`\S` export· νέα γραμμή `\C`/`\c` με το **ανοιχτό** §11-6: `ByLayer`/`ByBlock` δεν επιβιώνουν του round-trip). **Files:** `text-engine/serializer/mtext-serializer.ts` + 2 νέα test αρχεία. **Πλήρης αφήγηση/μετρήσεις:** ADR-737 §15.
- **2026-07-30 — 📘 Appendix B: το «Authoritative» δεν ήταν (caret notation + escaped literals + κατάσταση υλοποίησης, Opus 5):** Doc-only. Ο πίνακας inline codes αυτοχαρακτηριζόταν *«Authoritative»* και **δεν ανέφερε δύο ολόκληρες οικογένειες** που **υπάρχουν στα πραγματικά αρχεία του Giorgio**: (α) **caret notation** (`^I` TAB, `^J` LF, `^M` CR, `^ ` literal caret) — ο κώδικας τα υποστηρίζει από **ADR-635 Φ C.20**, όπου μετρήθηκαν **149 `^I`** σε ένα δείγμα και πριν τη διόρθωση ζωγραφίζονταν **κυριολεκτικά** («*I*I») ενώ οι στηλοθετημένες στήλες κατέρρεαν· (β) **escaped literals** `\\` / `\{` / `\}` (**ADR-737** ΒΛΑΒΗ Ε) — έπεφταν στο `default: return null` και ο χαρακτήρας **εξαφανιζόταν**, ενώ 🔴 η **δική μας** `escapeText()` τα **ΗΔΗ παρήγαγε** ⇒ ασυμμετρία στο ίδιο μας το round-trip (μονόδρομη διαρροή χαρακτήρων σε κάθε κύκλο export→import). **Προστέθηκε επίσης πίνακας «κατάσταση ανά κωδικό» (tokenizer / AST / export / render)**, γιατί η ύπαρξη γραμμής στο Appendix B **δεν σημαίνει ότι ο κωδικός ζωγραφίζεται**: το `\A` tokenize-άρεται **από την πρώτη μέρα**, αλλά η `applyStyleToken` δεν είχε case (`default: break`) ⇒ **49 εμφανίσεις** στο `47_ergasia.dxf` καταναλώνονταν στο κενό· το ADR-737 το **διατηρεί και το εξάγει**, αλλά ⏳ **κανένας renderer δεν το διαβάζει ακόμη** — μην το χαρακτηρίσετε «υποστηριζόμενο» χωρίς αυτή τη διάκριση. **Files:** μόνο αυτό το ADR (Appendix B). **Πηγή αριθμών:** ADR-635 Φ C.20 + ADR-737 §5-6.
- **2026-07-28 (b) — 🐛 ΤΟ ΚΑΝΟΝΙΚΟ ΠΕΔΙΟ ΕΧΑΝΕ ΑΠΟ ΤΟΝ ΚΑΘΡΕΦΤΗ: η επεξεργασία κειμένου δεν έπιανε σε ΕΙΣΑΓΜΕΝΑ DXF** (Giorgio: «γράφω μέσα στο πεδίο, ο καμβάς κρατάει πάντα το αρχικό κείμενο»).
  - **Προϋπάρχον σφάλμα, όχι της Φ-3D** — απλώς έγινε ορατό τώρα που η επεξεργασία μπήκε και στο 3D.
  - **Η αιτία**: `projectSceneTextToDxf` (`bim/text/project-scene-text.ts`) — από όπου περνά **ΟΛΟΚΛΗΡΟ** το render pipeline, 2D και 3D — διάβαζε `e.text ?? (e.textNode ? extractFlatText(e.textNode) : '')`. Δηλαδή **ο flat καθρέφτης νικούσε το canonical AST**. Κάθε text command (`ReplaceTextNodeCommand` κ.λπ.) γράφει **μόνο** `textNode`· το flat `.text` το γεμίζει ο import **μία φορά** (`dxf-text-converters`: `plainText = extractFlatText(textNode)`) και μετά κανείς δεν το ξαναγγίζει. Άρα το AST διορθωνόταν σωστά, ο renderer κοίταζε αλλού, και **κανένα σφάλμα δεν έπεφτε πουθενά**.
  - **⚠️ Γιατί επιβίωσε τόσο καιρό**: το `CreateTextCommand` γράφει **μόνο** `textNode`, χωρίς flat `text`. Άρα κείμενο φτιαγμένο **μέσα** στην εφαρμογή (ribbon) είχε `e.text === undefined` → το `??` έπεφτε στο AST → **δούλευε κανονικά**. Έσπαγε **αποκλειστικά** σε κείμενο από **DXF import** — που είναι το μόνο που έχει γεμάτο τον καθρέφτη. Κλασικό «αόρατο όσο ο καθρέφτης είναι συγχρονισμένος».
  - **Fix 1 — ο renderer διαβάζει το canonical**: `projectSceneTextToDxf` → `resolveEntityText(e)` (`utils/text-node-utils.ts`), το SSoT που **ήδη** χρησιμοποιούσαν hit-test / bounds / click (`textNode ? extractFlatText(textNode) : (text ?? '')`). Δύο συναρτήσεις απαντούσαν το ΙΔΙΟ ερώτημα με **αντίθετη προτεραιότητα** — τώρα μία.
  - **Fix 2 — ο writer συντηρεί τον καθρέφτη** (belt-and-suspenders, N.7.2 #4): νέα `mirrorFlatText()` στο `dxf-text-command-base.ts` — κάθε patch που αγγίζει `textNode` γράφει και το παράγωγο `text`. Εφαρμόζεται σε `execute()` **και** `undo()` (αναίρεση που επανέφερε μόνο το AST θα παρήγαγε το ίδιο σφάλμα ανάποδα). Καλύπτει κάθε καταναλωτή που διαβάζει το scene `.text` απευθείας — export DXF/TEK, clip, snap — και όποιον γραφτεί αύριο.
  - **Γιατί ΚΑΙ τα δύο**: το Fix 1 κάνει το AST τη μόνη αλήθεια για την απόδοση· το Fix 2 εξασφαλίζει ότι ο καθρέφτης δεν λέει ψέματα σε όσους δεν έχουν μεταναστεύσει. Καθρέφτης που δεν συντηρείται είναι χειρότερος από ανύπαρκτο: μοιάζει έγκυρος.
  - **Fix 3 — «ημιτελές AST» ≠ «κενό κείμενο»** (το βρήκε το regression: 5 κόκκινα στο ghost preview). Μερικά μονοπάτια φτιάχνουν textNode **μόνο** για να δηλώσουν στοίχιση — `{ attachment: 'BR' }`, χωρίς `paragraphs`. Με τον έλεγχο στο σκέτο `entity.textNode`, αυτά διάβαζαν «κενό» και το ghost έχανε το κείμενό του (πριν το Fix 1 δεν φαινόταν: το `e.text ??` δεν άγγιζε ποτέ το AST). Το `resolveEntityText` ελέγχει πλέον **`node?.paragraphs`**: AST χωρίς δομή απαντά «δεν ξέρω» → flat fallback· AST **με** `paragraphs` αλλά κενό περιεχόμενο είναι **γνήσια** απάντηση (ο χρήστης έσβησε τα πάντα) → κενό, χωρίς επαναφορά του καθρέφτη. Επίσης `extractFlatText` → `(paragraphs ?? [])`: καλείται από hit-test/bounds/render, όπου ένα throw ρίχνει ολόκληρο τον καμβά για μία κακοσχηματισμένη οντότητα.
  - ✅ **Google-level: YES** — ένα ερώτημα («ποιο είναι το κείμενο;»), μία απάντηση (`resolveEntityText`)· ο καθρέφτης συντηρείται στο σημείο εγγραφής, όχι σε κάθε αναγνώστη.
  - **Tests**: `bim/text/__tests__/project-scene-text-canonical.test.ts` (6 — επεξεργασία, σβήσιμο σε κενό, legacy χωρίς AST, **ημιτελές AST**, πολυγραμμικό), `core/commands/text/__tests__/flat-text-mirror.test.ts` (3 — execute, **undo**, σβήσιμο).
  - **Files**: `bim/text/project-scene-text.ts` (MOD), `core/commands/text/dxf-text-command-base.ts` (MOD), `utils/text-node-utils.ts` (MOD — `paragraphs` guard).

- **2026-07-28 — Φ-3D: in-place επεξεργασία κειμένου ΚΑΙ στον 3D καμβά** (Giorgio: «γιατί στον καμβά 3Δ δεν μπορώ να κάνω επεξεργασία με διπλό κλικ όπως στο 2Δ;» + «υλοποίηση full enterprise + full SSoT, με τη λύση των μεγάλων παικτών ή καλύτερη»).
  - **Το εύρημα**: δεν ήταν bug — δεν είχε υλοποιηθεί ποτέ. Ο μοναδικός handler διπλού κλικ σε ΟΛΟ το `bim-3d/` ήταν `onDoubleClick={(e) => e.stopPropagation()}` (`BimViewport3D.tsx:399`), δηλαδή το γεγονός καταπινόταν ρητά.
  - ⚠️ **Διόρθωση παρανόησης που κόστισε λάθος εκτίμηση εύρους**: μια πρώτη ανάγνωση («το `mtext` δεν υπάρχει στο 3D», βασισμένη σε grep του string `'mtext'` μέσα στο `bim-3d/`) ήταν **λάθος**. Ο `convertTextEntity` (`hooks/canvas/dxf-text-entity-converter.ts`) **κανονικοποιεί το MTEXT σε `type:'text'`** πριν φτάσει στο `DxfScene` («DxfEntityUnion has no `mtext` variant»), άρα το MTEXT **ήδη** αποδιδόταν από τον glyph atlas και **ήδη** επιλεγόταν από το `distanceToDxfEntityMm`. Ο διαχωρισμός TEXT/MTEXT επιβιώνει **μόνο** στο `SceneModel` — που είναι ακριβώς το επίπεδο όπου κοιτά ο editor. Μηδέν αλλαγή σε converter / hit-test.
  - **Έρευνα προτύπων του κλάδου** (πηγές στο §11): **AutoCAD** `MTEXTFIXED`, **προεπιλογή 2** — ο In-Place editor κάθεται στη *θέση* του κειμένου, αλλά κείμενο δυσανάγνωστο («πολύ μικρό, πολύ μεγάλο, ή **περιστραμμένο**») εμφανίζεται **οριζόντια και σε ευανάγνωστο μέγεθος**. **Revit** Model Text: select → «Edit Text» tool, όχι in-place WYSIWYG σε 3D. **Cinema 4D** MoText: επεξεργασία στο Attributes panel. **Blender**: in-viewport, με την **πλοήγηση ελεύθερη** όσο γράφεις. **Figma/Miro**: ο editor συγχρονίζεται με το transform του καμβά.
  - **ΑΠΟΡΡΙΨΗ CSS3DRenderer** (τεκμηριωμένη): προοπτικά παραμορφωμένο `contenteditable` γίνεται δυσανάγνωστο σε πλάγια γωνία — ακριβώς τη στιγμή που το διορθώνεις — και τα caret/selection hit-boxes του browser σπάνε κάτω από `matrix3d`. Η AutoCAD, στο ίδιο ακριβώς domain, το έχει ήδη απαντήσει με το MTEXTFIXED=2.
  - **ΑΠΟΦΑΣΗ**: **MTEXTFIXED=2 σημασιολογία** (θέση από την προβολή· μέγεθος + προσανατολισμός σταθερά σε screen-space) **+ ελεύθερη κάμερα κατά Blender** (ο editor *ακολουθεί* την προβολή, δεν παγώνει τα OrbitControls). **Πάνω από τους μεγάλους**: viewport clamping — ο editor δεν φεύγει ΠΟΤΕ εκτός οθόνης όσο γράφεις, ούτε όταν το κείμενο περάσει πίσω από την κάμερα.
  - **ΕΝΑ SSoT, δύο resolvers**: ο κοινός πυρήνας βγήκε από το 2D hook σε **`ui/text-toolbar/text-edit-session.ts`** (NEW, καθαρό: `resolveTextEditTarget` = ο κανόνας «ακριβώς 1 επιλογή, TEXT ή MTEXT»· `buildTextEditCommand` = diff → ΕΝΑ undo βήμα). Διαφέρει **μόνο** η αγκύρωση: `text-editor-anchor-2d.ts` (NEW) vs `bim-3d/text/text-edit-anchor-3d.ts` (NEW).
  - **Η 3D αγκύρωση δεν εισάγει καμία νέα μαθηματική**: `findDxfEntityInScope` (ADR-537 δ — οντότητα + υψόμετρο ορόφου + σκηνή) → **`resolveTextEmBox`** → `dxfSceneUnitToMm` (ADR-537 γ) → `dxfPlanToWorld` → `worldToScreen`. Το `resolveTextEmBox` είναι **το ίδιο** κουτί που διαβάζει το `glyph-atlas-text-layout.ts:143` για να στήσει τα quads των glyphs → ο editor αγκυρώνεται δομικά εκεί που ζωγραφίζονται τα γράμματα. Άγκυρο = **το κέντρο των γραμμάτων**, όχι το `position` (σημείο προσάρτησης 9 σημείων — θα κρεμόταν δίπλα στο κείμενο σε κάθε στοίχιση εκτός MC).
  - **⚠️ Ο σωστός ΟΡΟΦΟΣ, όχι ο ενεργός**: με «Όλοι οι όροφοι» (`floor3DScope: 'all'`) βλέπεις — και άρα διπλοκλικάρεις — κείμενο άλλου level. Το commit λύνει το level ανά οντότητα με `resolveEntityLevelId` και χτίζει τα services πάνω σ' αυτό (νέα εξαγωγή **`buildDxfTextServices`**). Χωρίς αυτό, η διόρθωση θα γραφόταν στον ενεργό όροφο **σιωπηλά** — δύο όροφοι κατεστραμμένοι, κανένα σφάλμα πουθενά. Καρφωμένο με δικό του test.
  - **Κέρδος και στο 2D (Figma parity, δωρεάν)**: η θέση διαβαζόταν **μία φορά** από στιγμιότυπο `transformRef.current` και έμενε καρφωμένη → pan/zoom με ανοιχτό editor τον άφηνε πίσω. Πλέον διαβάζεται στον χρόνο του tick από το `ImmediateTransformStore` (ADR-040: getter, όχι snapshot). Ισχύει και για το πεδίο **δημιουργίας** (`useTextCreationTool`), που είχε **αντιγράψει** το `worldToScreen` του διπλού κλικ — το σχόλιο «Same Y-flip as useTextDoubleClickEditor.computeAnchorRect» ομολογούσε το δίδυμο· πλέον καλεί τον κοινό resolver.
  - **N.18**: εξήχθη το **`subscribeToCameraMoves`** από το `useCameraProjectedMarkers` (camera-signature diffing + LOW-priority `UnifiedFrameScheduler` slot) ώστε ο editor να το **χρησιμοποιήσει** αντί να το ξαναγράψει. `jscpd:diff` σε 11 αρχεία: καθαρό.
  - **Μηδέν γραμμές στον orchestrator**: το `BimViewport3D.tsx` είναι στις **499/500** γραμμές (N.7.1). Το leaf δένει **μόνο του** native `dblclick` στον renderer canvas (το React `stopPropagation` δεν σταματά τη διάδοση στο DOM — ο ίδιος λόγος για τον οποίο το OrbitControls συνεχίζει να λαμβάνει γεγονότα), με `addEventListener` + `AbortController` όπως ο raw-DXF grip editor του ADR-537. Το mount μπήκε στο `BimViewport3DProjectedOverlays` — τον φάκελο των camera-projected DOM overlays.
  - ✅ **Google-level: YES** — ΕΝΑ `text-edit-session` για τον κανόνα επιλογής + το undo grain· ΕΝΑ `TextEditorAnchorLayer` για την τοποθέτηση (2D+3D)· ΕΝΑ `subscribeToCameraMoves` για το camera tick· η 3D αγκύρωση διαβάζει το ΙΔΙΟ em-box με τον glyph atlas· ο όροφος λύνεται ανά οντότητα (fail-closed: χωρίς level → δεν ανοίγει)· ADR-040 καθαρό (μηδέν `useSyncExternalStore`, imperative positioning, event-time reads).
  - **Tests**: `text-edit-session.test.ts` (18), `text-edit-anchor-3d.test.ts` (8 — υψόμετρο ορόφου, μονάδες σκηνής m≡mm, πίσω από κάμερα, στοίχιση), `use-bim3d-text-double-click-editor.test.tsx` (7 — **το commit γράφει στον όροφο ΤΗΣ ΟΝΤΟΤΗΤΑΣ**). Παλινδρόμηση: 316 σουίτες / 2789 tests πράσινα.
  - **Files**: `ui/text-toolbar/text-edit-session.ts` (NEW), `ui/text-toolbar/TextEditorAnchorLayer.tsx` (NEW), `ui/text-toolbar/text-editor-anchor-2d.ts` (NEW), `bim-3d/text/text-edit-anchor-3d.ts` (NEW), `bim-3d/text/use-bim3d-text-double-click-editor.ts` (NEW), `bim-3d/text/BimViewport3DTextEditorLeaf.tsx` (NEW), `ui/text-toolbar/TextEditorOverlay.tsx` (MOD — έχασε το self-positioning), `ui/text-toolbar/hooks/useTextDoubleClickEditor.ts` (MOD — 2D wrapper), `ui/text-toolbar/hooks/useDxfTextServices.ts` (MOD — `buildDxfTextServices`), `hooks/canvas/useTextCreationTool.ts` (MOD — κοινός anchor), `components/dxf-layout/CanvasSection.tsx` (MOD — `anchorRect`→`anchor`), `bim-3d/coordination/use-camera-projected-markers.ts` (MOD — εξαγωγή tick), `bim-3d/viewport/BimViewport3DProjectedOverlays.tsx` (MOD — mount + `canvasEl`), `bim-3d/viewport/BimViewport3D.tsx` (MOD — 2 γραμμές, μηδέν καθαρή αύξηση).

- **2026-07-08 — Ribbon color-picker unification Φ2 (text `ColorPickerPopover` → floating SSoT shell)** (Giorgio: «ο color picker του Contextual Tab «Επεξεργαστής Κειμένου» να αντικατασταθεί με τον picker των Ρυθμίσεων DXF — floating, μαζεμένος, να μην κρύβεται από τη βάση της οθόνης· πανομοιότυπος κώδικας, μία πηγή αλήθειας»).
  - **Πρόβλημα**: το `ColorPickerPopover` άνοιγε με Radix `Popover` (αγκιστρωμένο κάτω από το κουμπί) → κοβόταν από τη βάση της οθόνης. Οι «Ρυθμίσεις DXF» χρησιμοποιούσαν το floating `EnterpriseColorDialog` (portal + draggable + focus-trap) → δεν κοβόταν.
  - **SSoT εξαγωγή**: το floating **κέλυφος** (portal + backdrop + `useDraggable` + `FocusScope` + ARIA dialog + header/X + shared `lastDialogPosition`) εξήχθη από το `EnterpriseColorDialog` σε νέο **`ui/color/ColorDialogShell.tsx`**. Το `EnterpriseColorDialog` το χρησιμοποιεί (public API **αμετάβλητο** → μηδέν αλλαγή στους 20+ callers: settings/rulers/grid/cursor/crosshair/line/grip/background/MEP/finish-paint/polygon-material/UnifiedColorPicker) κρατώντας μόνο hex state + RAF throttle + Apply/Cancel footer.
  - **`ColorPickerPopover`**: Radix `Popover`→`ColorDialogShell` (`dimBackdrop={false}` για live σύγκριση χρώματος πάνω στο σχέδιο). Το tab «Αληθινό χρώμα» πλέον `orientation="horizontal"` + `maxWidthClass=PANEL_MAX_WIDTH_XL` → **2 στήλες, πανομοιότυπο layout με τις Ρυθμίσεις DXF** (πριν ήταν `vertical`/`w-[320px]`). Τα 3 tabs (True/ACI/ByLayer-ByBlock) + eyedropper **αμετάβλητα**. Component API αμετάβλητο → οι consumers `FormattingPanel` / `RibbonColorSwatchWidget` **δεν αγγίχθηκαν**. Πλέον floating/draggable/μαζεμένος όπως των ρυθμίσεων.
  - **Footer SSoT**: το footer «Ακύρωση / Εφαρμογή» μεταφέρθηκε από το `EnterpriseColorDialog` **μέσα στο `ColorDialogShell`** (built-in `showFooter`/`onCancel`/`onApply`, i18n `dxf-viewer-panels:colorPicker.cancel/apply`) → ΕΝΑ footer markup για ρυθμίσεις **και** κείμενο. Ο text `ColorPickerPopover` πλέον έχει «Ακύρωση/Εφαρμογή» (πριν δεν είχε): η επιλογή εφαρμόζεται live στο κείμενο (WYSIWYG)· «Ακύρωση»/X/Escape επαναφέρει το αρχικό χρώμα (tracked `originalValue` on-open· αν ήταν «Ανάμεικτο»/null κρατιέται η live επιλογή), «Εφαρμογή» κλείνει.
  - **Backdrop**: το default `dimBackdrop` άλλαξε `true`→**`false`** στο `EnterpriseColorDialog` (Giorgio: «στις Ρυθμίσεις DXF υπάρχει πέπλο που σκουραίνει την οθόνη — δεν το θέλω»). Πλέον ΚΑΝΕΝΑΣ color picker (ρυθμίσεις + κείμενο) δεν σκοτεινιάζει το σχέδιο → live WYSIWYG σύγκριση χρώματος παντού. `types.ts` doc ενημερώθηκε.
  - ✅ Google-level: YES — ΕΝΑ floating shell (`ColorDialogShell`) για ΚΑΘΕ color picker (ρυθμίσεις + κείμενο)· μηδέν διπλότυπο portal/draggable/focus-trap· κοινή `lastDialogPosition` (κάθε picker ανοίγει εκεί που το άφησες)· κοινό no-backdrop behaviour.
  - **Files**: `ui/color/ColorDialogShell.tsx` (NEW — SSoT floating shell), `ui/color/EnterpriseColorDialog.tsx` (MOD — χρησιμοποιεί το shell + `dimBackdrop` default→false, API αμετάβλητο), `ui/color/types.ts` (MOD — doc), `ui/text-toolbar/controls/ColorPickerPopover.tsx` (MOD — Popover→shell, horizontal 2-column).
  - 🔴 browser-verify (Επεξεργαστής Κειμένου → «Χρώμα» → floating draggable dialog, όχι clipped· Ρυθμίσεις DXF picker αμετάβλητος).
...
- **2026-06-23 — Round 7 — Default text height = `drawingScale` SSoT (Revit annotation scale), replacing the inconsistent unit heuristic**.
  - **Symptom (Giorgio)**: Ribbon "Text" in a ~10m × 10m floorplan (stored in **mm**, fit-to-view ≈ 1:58) produced microscopic text — invisible until zoomed to ≈ 3:1 / 4:1. Root: the drawing is legitimate **model-space-in-mm** (the common Greek/European convention), not paper-space.
  - **Root cause (the Round-6 fix was incomplete)**: the unit-system heuristic `m→100, cm→10, mm/in/ft→1` assumed `mm = paper space`. For model-space mm it left `dimscale = 1` → `2.5 × 1 × 1 = 2.5mm` = 0.025% of a 10000mm drawing (sub-pixel at fit-to-view). The three branches were also physically **inconsistent**: 250mm (m) / 25mm (cm) / 2.5mm (mm) for the same paper default.
  - **Fix — single SSoT**: the default annotation height now reads the canonical **`drawingScale` denominator** (`useDrawingScaleStore.getState().drawingScale`, ADR-375 Revit annotation-scale, default **1:100**, set in the View ribbon, decoupled from zoom) instead of the DimStyle-registry heuristic. `getDimStyleRegistry()` is removed from this path. Height = `paperHeightToModel(DEFAULT_HEIGHT, drawingScale, units)` → **250mm physical at 1:100 in ANY unit system** (mm/cm/m). Read at click time (event-time `getState`, ADR-040) so it tracks the live scale.
  - **Fix — NEW SSoT helper `utils/annotation-scale.ts`**: `paperHeightToModel(paperMm, drawingScale, units)` = `paperMm × scale × mmToSceneUnits(units)` (reuses the existing `scene-units` math; non-finite/≤0 scale → 1, defensive). Single conversion shared by the tool + future Dim tools.
  - **Fix — full SSoT for the paper default**: `2.5` literals in `makeEmptyTextNode` / `onCommit` replaced by `TEXT_SIZE_LIMITS.DEFAULT_HEIGHT`.
  - **Centralisation (2nd consumer, no duplicate)**: `dim-text-renderer.ts` computed the **same** paper→model conversion inline (`dimtxt × effectiveDimscale × unitFactor`). Refactored to `paperHeightToModel(dimtxt, …, units) × transform.scale` — so `paperHeightToModel` is now a genuine SSoT with two consumers (ribbon Text + dimension renderer). **Follow-up (ADR-362 Round 14)**: the dimension annotation-scale resolution itself was also centralised — `resolveEffectiveDimscale` (same `drawingScale` SSoT family) now heals `style.dimscale` once in `DimensionRenderer`, so the metre-only rescue is gone and mm/cm dimensions are legible too. See ADR-362 R14.
  - **Tests**: `annotation-scale.test.ts` (NEW — 4 tests, incl. the "same physical height across all units" invariant). `useTextCreationTool-scene-units.test.tsx` (MOD — mock `drawing-scale-store@100` instead of the registry; CASES now assert 250mm physical: mm→250, cm→25, m→0.25, in/ft proportional; back-compat default → 250). 12 tests pass.
  - ✅ Google-level: YES — single source of truth (one `drawingScale` SSoT, no parallel heuristic), user-controllable (View ribbon widget), unit-independent physical size, idempotent one-shot creation, event-time read (no stale snapshot), reuses `mmToSceneUnits`.
  - **Files**: `utils/annotation-scale.ts` (NEW), `utils/__tests__/annotation-scale.test.ts` (NEW), `hooks/canvas/useTextCreationTool.ts` (MOD), `hooks/canvas/__tests__/useTextCreationTool-scene-units.test.tsx` (MOD), `rendering/entities/dimension/dim-text-renderer.ts` (MOD — consume the SSoT helper).
  - 🔴 browser-verify: ribbon Text in a mm floorplan at 1:58 fit → text legible at fit-to-view; changing the View-ribbon Drawing Scale changes the next text's size.
...
- **2026-05-20 — Round 6 — Belt-and-suspenders zero-height run patch (dimscale-aware, unit-fallback)**.
  ...
  - **Fix — `makeEmptyTextNode(units, dimscale)`**: gains optional `dimscale` param (default 1). Height = `2.5 * max(1, dimscale) * mmToSceneUnits(units)`.
  - **Fix — `handleCanvasClick` (unit-aware dimscale fallback)**: reads `rawDimscale = activeStyle?.dimscale ?? 1` from `getDimStyleRegistry()`. When `rawDimscale ≤ 1`, applies unit-system heuristic: `m → 100, cm → 10, mm/in/ft → 1`. This ensures a meters DXF with any DIMSCALE setting produces visible text: `2.5 × 100 × 0.001 = 0.25m = 250mm` — legible at 1:100 building scale. When `rawDimscale > 1` (explicit, e.g. 50, 100), uses it directly (imported DXF DIMSCALE wins).
  - **Fix — `onCommit`**: `patchZeroHeightRuns(next, state.initial...height)` — any committed run with `height: 0` is patched to the dimscale-aware value computed at click time. StackedRuns skipped. Non-zero heights preserved.
  - **Tests**: `getDimStyleRegistry` mocked (no styles → `rawDimscale=1` → fallback active). CASES expected values updated: `cm` → 2.5 (was 0.25), `m` → 0.25 (was 0.0025). R6 patch test: expected 0.25 for m-scene. All 8 tests pass. `dim-text-renderer-scene-units.test.ts`: all 6 tests pass.
  - ✅ Google-level: YES — belt-and-suspenders independent of TipTap internals; unit-aware fallback ensures visible text in any scene regardless of DIMSCALE setting; SSoT (reads DimStyle registry, same as DimensionRenderer); idempotent.
  - **Files**: `hooks/canvas/useTextCreationTool.ts` (MOD — unit-aware dimscale fallback, `patchZeroHeightRuns` helper, `onCommit` patch), `hooks/canvas/__tests__/useTextCreationTool-scene-units.test.tsx` (MOD — registry mock + updated expected values + 2 R6 tests).
