...
- **2026-07-28 (b) — 🐛 ΤΟ ΚΑΝΟΝΙΚΟ ΠΕΔΙΟ ΕΧΑΝΕ ΑΠΟ ΤΟΝ ΚΑΘΡΕΦΤΗ: η επεξεργασία κειμένου δεν έπιανε σε ΕΙΣΑΓΜΕΝΑ DXF** (Giorgio: «γράφω μέσα στο πεδίο, ο καμβάς κρατάει πάντα το αρχικό κείμενο»).
  ...
  - **Tests**: `bim/text/__tests__/project-scene-text-canonical.test.ts` (5 — επεξεργασία, σβήσιμο σε κενό, legacy χωρίς AST, πολυγραμμικό), `core/commands/text/__tests__/flat-text-mirror.test.ts` (3 — execute, **undo**, σβήσιμο).
  - **Files**: `bim/text/project-scene-text.ts` (MOD), `core/commands/text/dxf-text-command-base.ts` (MOD).
