...
- **Z4** (περβάζια κουφωμάτων) — **2D κάτοψη: inset frame** (λωρίδες) μέσα στο `outline` της τρύπας· **3D: lining frame** εξωθημένο καθ' ύψος ανοίγματος (sill→head).
- **SSoT**: το render διαβάζει per-element `slab.params.envelopeLayer` / `opening.params.revealInsulation` (γραμμένα από τον P7B applicator) — **render = pure read**, ΟΧΙ re-classify. Inset SSoT: `insetClosedPolygon` (`polygon-utils.ts`, winding-agnostic). Visibility: ίδια V/G κατηγορία `envelope` (ADR-382), μηδέν νέα palette.
