...
| 2026-09-12 | 🔴 **Η μηχανή απέκτησε μνήμη — και ήταν ΟΡΟΣ ΧΡΗΣΗΣ, όχι βελτιστοποίηση** (ADR-332 D27 Ζ5). Η πολιτική του Nominatim λέει ρητά *«Results **must** be cached on your side. Clients sending repeatedly the same query may be classified as faulty and **blocked**»*. Το `geocodeWithVerdict` δεν είχε **καμία** μνήμη: κάθε κλήση ξεκινούσε τη σκάλα των **8 παραλλαγών** (`VARIANT_I18N_KEYS`) με `sleep(NOMINATIM_DELAY_MS)` πριν από κάθε μία — μετρημένο ζωντανά **61,4″** για **μία** αποθήκευση επαφής. ⚠️ Μνήμη **υπήρχε** στο `lib/geocoding/geocoding-service.ts`, αλλά είναι περιτύλιγμα **πελάτη** γύρω από το `/api/geocoding`: η γραφή θέσεων καλεί τη μηχανή **μέσα στη διεργασία** και δεν το έβλεπε ποτέ. **Νέο**: `app/api/geocoding/geocoding-cache.ts` — cache-aside πάνω στο υπάρχον SSoT `EnterpriseAPICache` + **in-flight dedup**, με πολιτική **ανά ετυμηγορία**: `hit` μεγάλη διάρκεια · `absent` **σύντομη** (αρνητική μνήμη, πρότυπο DNS negative TTL — χωρίς αυτήν η διεύθυνση που **δεν** λύνεται πλήρωνε και τις 8 παραλλαγές σε κάθε αποθήκευση) · `unavailable` **ποτέ** (άγνοια ≠ γνώση· ένα παροδικό 429 δεν γίνεται μόνιμη αλήθεια). Το κλειδί χτίζεται **μόνο** από πεδία διεύθυνσης — καμία ταυτότητα μισθωτή, γιατί η γεωκωδικοποίηση δημόσιας διεύθυνσης δεν είναι δεδομένο πελάτη· κανονικοποιεί κεφαλαία/κενά αλλά **ΟΧΙ τόνους** (η μηχανή έχει δική της παραλλαγή χωρίς τόνους ⇒ άλλο ερώτημα, άλλη απάντηση). Επειδή **πληκτρολόγηση** (`/api/geocoding`) και **αποθήκευση** (`address-place-writeback`) καταλήγουν στην **ίδια** συνάρτηση, η μνήμη τις εξυπηρετεί **μαζί**. Άγκυρες Ε1–Ε6 (η Ε6 αποδεικνύει την **καλωδίωση**, όχι μόνο το module· η Ε5 κρατά το κλειδί ίσο με τα πεδία του `sanitizeQuery`)· **μεταλλάξεις 6/6**. ⚠️ Δηλωμένο όριο: το `EnterpriseAPICache` δεν έχει φράγμα μεγέθους — οι εγγραφές λήγουν, δεν εκδιώκονται. |
