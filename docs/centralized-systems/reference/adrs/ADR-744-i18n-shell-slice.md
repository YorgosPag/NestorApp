...
### 14.5 🔶 ΔΗΛΩΜΕΝΟ ΟΡΙΟ — ο generator είναι ΤΥΦΛΟΣ σε aliased `t`

**Μετρημένο**: `scripts/lib/i18n-shell-slice/key-extract.js:349` κάνει
`callee.text === 't'` — **ακριβής σύγκριση**. Άρα **κάθε** `tParking` · `tBuilding` ·
`tUnits` · `tStorage` είναι αόρατο στον generator, και το `parking` namespace **δεν μπήκε**
στο slice παρόλο που το component το ζητά.

**Γιατί είναι ανεκτό εδώ, μετρημένα**: το badge κατάστασης απαιτεί `result.status`, δηλαδή
**αποτελέσματα** από το `useGlobalSearch` — που απαιτούν query ≥ `MIN_QUERY_LENGTH`. Στο
SSR δεν υπάρχουν αποτελέσματα, άρα το `SearchResultItem` **δεν εκτελείται στο πρώτο καρέ**.
Επιβεβαιώνεται από το ίδιο το log που ξεκίνησε τη δουλειά: βάφτηκαν τα **4** κλειδιά της
κενής κατάστασης, **κανένα** `entityTypes`. Το async `useEffect` προλαβαίνει.

⚠️ **ΔΕΝ είναι λυμένο — είναι μετρημένο ότι δεν χτυπά σήμερα.** Ένα aliased `t` σε
**κέλυφος** που βάφει στο πρώτο καρέ θα έδινε ωμό κλειδί **χωρίς καμία πύλη να μιλήσει**.
Είναι η ίδια κλάση με τα δύο τυφλά σημεία του §12.3.

### 14.6 🔴 ΠΡΟΫΠΑΡΧΟΝ ΚΟΚΚΙΝΟ — ανοιχτό, ΔΕΝ αγγίχθηκε

Το `scripts/__tests__/i18n-shell-slice.test.js:572` απαιτεί `bytes < 220.000`. **Μετρημένο**:
στο `f400ae45` — **πριν** από οποιαδήποτε αλλαγή αυτής της δουλειάς — το slice ήταν ήδη
**227.512 bytes**, δηλαδή το test ήταν **ήδη κόκκινο** και κανείς δεν το έτρεχε
(ίδιο σχήμα με τα 11 tests του ADR-587 §6.1).

🔑 **Το test μετρά ΑΛΛΟ πράγμα από αυτό που δηλώνει**: το σχόλιό του γράφει *«This ceiling
exists to catch the migration ledger **GROWING**»* — αλλά το κατώφλι είναι πάνω στα
**συνολικά** bytes. Το ledger εδώ **δεν** μεγάλωσε· μεγάλωσαν τα **key-sliced** κλειδιά, που
είναι ακριβώς η **σωστή** κατεύθυνση του ADR. Έτσι όπως είναι γραμμένο, **η πύλη μπλοκάρει
τη θεραπεία** — το ίδιο αντι-πρότυπο που τα CHECK 3.44 και 3.53 απορρίπτουν ρητά.

⚠️ **ΜΗΝ το λύσεις ανεβάζοντας το 220.000**: θα ξαναπαλιώσει στην επόμενη νόμιμη προσθήκη
κλειδιού, και το τρίτο νούμερο θα αντιγραφεί σε handoff όπως το «91 unprotected» του N.12.
Η ερώτηση που **αξίζει** να φυλάγεται είναι το μέγεθος του **ledger**, όχι του αρχείου.
**Απόφαση Giorgio.**
