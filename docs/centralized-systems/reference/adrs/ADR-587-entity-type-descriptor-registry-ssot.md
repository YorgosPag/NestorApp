...
| Πεδίο | Τιμή |
|---|---|
| **Status** | 🟢 **Φ1 + Φ2 + Φ2c + Φ3a + Φ3b-1 IMPLEMENTED (UNCOMMITTED)** — 2026-07-08, Opus 4.8. Φ1 = declarative foundation (`EntityTypeDescriptor` + `category`). Φ2 = `dxfExportType` (απορρόφηση `ENTITY_TYPE_MAPPING`) + roadmap correction (§5.1: tool/ribbon = ToolType-keyed). Φ2c = `dxfWrappedField` (απορρόφηση `DXF_WRAPPED_SUBENTITY_FIELD`). Φ3a = entity-keyed selection→contextual-tab resolver dedup (23 `if (type==='x')` → ΕΝΑ `ENTITY_CONTEXTUAL_TRIGGER` map). **Φ3b-1 = command-keys guard factory (Seam 1)** — ~111 predicate-boilerplate guards σε 40 `*-command-keys.ts` → ΕΝΑ `makeKeySetGuard` factory (adapter· named exports αμετάβλητα· μηδέν αλλαγή συμπεριφοράς). Design + roadmap εγκεκριμένα (Giorgio). Φ2b / Φ3b-2 (tool-trigger Seam 2) / Φ4+ αναμονή. Μηδέν αλλαγή συμπεριφοράς. jest: descriptor 14/14 + trigger 11/11 + bridge command-keys 172/172 (19 suites) πράσινα· jscpd καθαρό (43 files). |
| **Date** | 2026-07-08 |
...
| Φ2b | Tool→entity back-link (ToolType-keyed) | `TOOL_DEFINITIONS.ToolInfo` +`createsEntityType?` (ο σύνδεσμος tool→οντότητα ζει στο tool registry, ΟΧΙ στον entity descriptor· μία οντότητα→πολλά tools). Coverage: κάθε `createsEntityType` είναι valid RenderableEntityType. | 🟡 Χαμηλό |
...
| Φ3b-2 | Tool-trigger dedup (Seam 2, ToolType-keyed) | `resolveActiveTrigger` (tool path, ~65 branches) → table lookup (`Map<ToolType, trigger>`) + escape-hatch order-sensitive (prefix rules, region-tool predicates, sticky line-modify). Ζει στο tool layer (§5.1)· τα hook subscriptions ανέγγιχτα (Rules of Hooks). | 🟡 Μεσαίο |
| Φ4 | TIER-3 bridge dispatch | hooks-array σε ΕΝΑ aggregator + `bridgeKey`-driven dispatch (τα ~210 branches → 1 lookup). Hook calls μένουν. | 🟡 Μεσαίο |
