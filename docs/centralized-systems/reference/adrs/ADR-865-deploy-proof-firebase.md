...
|---|---|---|
| `tree` | ζωντανό = δέντρο | καμία (αν το μητρώο δεν το ξέρει ⇒ ρωτιούνται τα **GitHub Deployments**, §11.10: γραμμή παραγωγής **ή** ⚠️ ανάπτυξη **εκτός εργαλείου**) |
...
🏆 **Πού ξεπερνάμε** *(από **ισχυρισμό** σε **γεγονός**, 2026-09-20)*: το CodePipeline **δεν** αντικαθιστά
αυτόν που **κρατά** την έγκριση (μόνο το χρονικό όριο τον βγάζει). Εμείς τον αντικαθιστούμε **αμέσως** —
**μετρημένο**: το επόμενο πήρε κουμπί στο **ίδιο δευτερόλεπτο** με την ακύρωση (18:00:00Z, πίνακας
παρακάτω) — αλλά **μόνο** όσο περιμένει. Επιπλέον ο φρουρός κάνει το παράθυρο «εγκρίθηκε ακριβώς τη
στιγμή της ακύρωσης» **ακίνδυνο**, και αυτό **μετρήθηκε στην παραγωγή**: έγκριση τρεξίματος που δεν ήταν
πια κορυφή ⇒ αυτοακύρωση σε **14,1″**, **χωρίς** να τρέξει καν `--dry-run`.
...
**Μετρημένο στην παραγωγή — Η ΑΝΤΙΚΑΤΑΣΤΑΣΗ** *(2026-09-20, άσκηση χωρίς καμία εγγραφή)*

Το φορτίο ήταν **δύο `workflow_dispatch` με `firebase_dry_run`** (ζητούν έγκριση, `--dry-run`, χωρίς
`release`/`notify`) και **ένα push** ως κορυφή ⇒ **καμία** εγγραφή σε Firebase, **καμία** κυκλοφορία.
Έτσι αναπαράχθηκε το **ακριβές** σύμπτωμα των `35429150868`/`35431491440`: κάτοχος με κουμπί, ουρά χωρίς.

| Στιγμή (UTC) | Τρέξιμο | Μέτρηση |
|---|---|---|
| 17:56:23 | **Α** `35527396356` (dispatch) | deployment `6555862522` → `waiting` ⇒ **κρατά** το `firebase-production` |
| 17:57:40 | **Γ** `35527465255` (dispatch) | apply `waiting`, αλλά deployment `6555874192` **χωρίς status** και `pending_deployments` = **0** ⇒ **καμία** ειδοποίηση, **κανένα** κουμπί |
| 17:58:56 | **Β** `35527571234` (push `af32dc60`) | η κορυφή· πλάνο `none` (17:59:51) ⇒ **δεν** ζητά δική της έγκριση |
| 17:59:58.2 | Β · διαδοχή | *«✓ κορυφή του main: af32dc60 — αυτό το τρέξιμο έχει δικαίωμα να συνεχίσει»* |
| **18:00:00.028** | Β · διαδοχή | *«⏭️ αντικαταστάθηκε #165 @aee8f601 — περίμενε έγκριση για παλαιότερο δέντρο ⇒ ακυρώθηκε»* |
| **18:00:00** | **Α** / **Γ** | `6555862522` → `error` **και** `6555874192` → `waiting` ⇒ 🔑 **η ακύρωση ελευθερώνει το group στο ΙΔΙΟ δευτερόλεπτο** |
| 18:00:15 | **Α** | run `cancelled` — **όχι** `failure`. Jobs `release`/`notify`: `cancelled` ⇒ **κανένα** Telegram «FALLITO» |
| 18:02:11 | **Γ** | εγκρίθηκε **επίτηδες** ενώ **δεν** είναι πια κορυφή (`aee8f601` vs `af32dc60`) |
| 18:02:28.9 | Γ · apply | φρουρός: *«⏭️ ΑΝΤΙΚΑΤΑΣΤΑΘΗΚΕ: το τρέξιμο είναι στο aee8f601, η κορυφή του main στο af32dc60 — δεν αναπτύσσει, δεν κυκλοφορεί»* |
| 18:02:43 | Γ · apply | `cancelled` ⇒ **14,1″**, μέσα στη χάρη `CANCEL_GRACE_MS` = 120″ (καμία έξοδος 3). **Κανένα** deploy — ούτε `--dry-run`: ο φρουρός κόβει **πριν** το βήμα |
| 18:00:16 · 18:02:48 · 18:13:26 | CI Health (ADR-757) | **έτρεξε** (`workflow_run`) μετά από **κάθε** ακύρωση — runs `35527640521` · `35527785572` · `35528360271`, όλα `success` — και έγραψε **0 σχόλια** στο issue #5 (τελευταίο σχόλιο: 2026-08-04). 🔑 Σιωπή επειδή **κοίταξε**, όχι επειδή δεν κλήθηκε: τα `cancelled` **δεν** είναι «έσπασε» |
| 18:13:47 | **Β** | ολοκληρώθηκε: build ✓ · **Κυκλοφορία στο Netcup ✓** · Ειδοποίηση ✓ ⇒ η άσκηση **δεν** εμπόδισε την κανονική κυκλοφορία της κορυφής |

⚠️ **Λεπτομέρεια που δεν φαινόταν πουθενά**: το deployment του μπλοκαρισμένου γεννιέται **μαζί με το job**
(17:57:40), όχι όταν δοθεί το κουμπί. «`pending_deployments` = 0» σημαίνει «**ο κριτής δεν ρωτήθηκε**»,
όχι «δεν υπάρχει deployment». Επίσης το ακυρωμένο deployment παίρνει state **`error`** ενώ το **run**
είναι `cancelled` — γι' αυτό ο CI Health πρέπει να κρίνει το **run**, όπως ήδη κάνει.

⚠️ **Δηλωμένα όρια**:
1. ~~**ΑΜΕΤΡΗΤΟ στην παραγωγή**: η ίδια η **αντικατάσταση**~~ ⇒ ✅ **ΜΕΤΡΗΘΗΚΕ 2026-09-20** (άσκηση
   `35527396356` / `35527465255` / `35527571234`, πίνακας παρακάτω). Και τα **δύο** σκέλη: (α) η ακύρωση του
   κατόχου **ελευθερώνει** το group — deployment `6555874192` πήρε `waiting` στο **ίδιο δευτερόλεπτο**
   (18:00:00Z) με το `error` του κατόχου `6555862522`· (β) η αυτοακύρωση σκότωσε το βήμα σε **14,1″**
   (φρουρός 18:02:28.9Z → job cancelled 18:02:43Z), δηλαδή **δεν** πλησίασε τη χάρη των 2′ (καμία έξοδος 3).
2. ✅ **ΜΕΤΡΗΜΕΝΟ 2026-09-20**: push στην κορυφή αντικαθιστά και **δοκιμή deployer** (`workflow_dispatch`)
   που περιμένει έγκριση — το `#165 @aee8f601` ήταν dispatch, ακυρώθηκε από το push `af32dc60`. Ο
   `supersededHolders` κρίνει με το `pending_deployments`, **όχι** με το event· ο `KEEPER_EVENT = 'push'`
   αφορά **ποιος σκουπίζει**, όχι **ποιος σκουπίζεται**.
...
5. ~~Εγγραφή κανόνων Firestore αμέτρητη~~ ⇒ ✅ **μετρήθηκε** (run `35450352067`, πίνακας παραπάνω).
   Για τους **δείκτες** το όριο **χωρίζεται σε δύο** — και το πρώτο έκλεισε **χωρίς καμία εγγραφή**
   *(2026-09-20)*:
   - **Εξουσιοδότηση** ⇒ ✅ **ΜΕΤΡΗΜΕΝΗ ΖΩΝΤΑΝΑ**. `gcloud iam roles describe nestorFirebaseRulesDeployer
     --project=pagonis-87766` (μόνο ανάγνωση) ⇒ **17/17**, με `datastore.schemas.create` · `.update` ·
     `.get` · `.list` **και** `datastore.operations.get`. 🔑 Η πρωτογενής πηγή (Google Cloud, *Firestore
     IAM*) λέει ότι τα `datastore.schemas.*` είναι το **νέο όνομα** των `datastore.indexes.*` (*«You can
     still use `datastore.indexes` as an alias for `datastore.schemas`»*): `schemas.create` = **Create an
     index**, `schemas.update` = **Update an index**, `schemas.list` = **List the indexes**. Το
     `operations.get` είναι ακριβώς ό,τι χρειάζεται το `--wait` για το LRO του χτισίματος. **Καμία**
     `schemas.delete` ⇒ ο deployer **δεν μπορεί** να σβήσει δείκτη, όπως υπόσχεται ο §11.
   - **Εκτέλεση** ⇒ ⏳ **αμέτρητη**: κανένα push δεν έχει διαφοροποιήσει δείκτη ακόμη, άρα δεν έχει
     εκτελεστεί `firestore:indexes` μέσα από τη γραμμή, ούτε η αναμονή `CREATING → READY`.
     ❌ **ΜΗΝ** την κλείσεις με **τεχνητό** δείκτη: δείκτης εκτός αρχείου **δεν σβήνεται ποτέ**
     αυτόματα (runbook §2) ⇒ μόνιμη μόλυνση της παραγωγής για μία μέτρηση. Κλείνει με την **πρώτη
     πραγματική** ανάγκη — ή από την εκστρατεία στην **τυφλή ζώνη** του CHECK 3.15 (**55** δυναμικά
     call-sites που η πύλη δηλώνει «unanalyzable»· τα δύο περιστατικά που γέννησαν το ADR ήταν
     **ακριβώς** αυτό το σχήμα).

### 11.10 Η πράξη της γραμμής **διαβάζεται** — απόδοση από τα GitHub Deployments *(2026-09-21)*

**Η αφορμή, μετρημένη** (handoff 2026-09-19 Βήμα 1). Δύο ερωτήματα, που αποδείχθηκαν **διαφορετικά**:

| Ερώτημα | Μέτρηση | Ετυμηγορία |
|---|---|---|
| «Windows ≠ CI στη σημείωση storage» | Το run-μάρτυρας `35426414076` (06:22Z) έτρεξε **πριν** το Ε2 (`37be5d1d`, 11:44Z). Η αναπαραγωγή **υπάρχει** (`firestore-deploy-pipeline.test.js` Ε2 · δίσκος CRLF / blob LF) και **δαγκώνει**: μετάλλαξη `renderingOf` → μόνο ωμά ⇒ **3** κόκκινα | ✅ **ήδη κλειστό** (§11.8 Ε2) — ο handoff είχε μπαγιάτικη μαρτυρία |
| «διαβάζει η απόδοση τα GitHub Deployments;» | `firebase-drift.yml` run `35586911393` (2026-09-21, **μετά** το Ε2): rules **και** storage *«το τοπικό μητρώο δεν το κατέγραψε (γραμμή παραγωγής ⇒ GitHub Deployments, ή ανάπτυξη εκτός εργαλείου)»*. `grep` σε `scripts/`: **κανείς** δεν καλούσε `/deployments` | 🔴 **ΟΧΙ** — η σημείωση **υποσχόταν** πηγή που δεν ρωτιόταν ⇒ κάθε ανάπτυξη του CI «μη καταγεγραμμένη» **για πάντα**, αδιάκριτη από Console/CLI |

Η σημερινή σημείωση ήταν **αληθής**: το CI ανέπτυξε **νέο** περιεχόμενο (`eca0e195` avatars · `7057264b` κανόνες) που το
μητρώο δεν ξέρει εκ σχεδιασμού. Δεν έφταιγε η κανονικοποίηση· έλειπε ο **αναγνώστης**.

**Η απόφαση — διπλή απόδειξη, ποτέ μία.** Ένα GitHub Deployment (το GitHub το γεννά αυτόματα για κάθε job με
`environment: firebase-production`, statuses `waiting → queued → in_progress → success|failure|error`) εξηγεί ένα
release **μόνο** όταν:

1. ο χρόνος του release (`updateTime`, Rules API) ∈ **παράθυρο εκτέλεσης** `[in_progress, τερματική]` — η αναμονή
   έγκρισης **δεν** μετρά· ανοιχτό παράθυρο όσο τρέχει (η επαλήθευση **μέσα** στο apply)· **και**
2. η μεταγλώττιση της πηγής **@sha** του Deployment = ζωντανό (`world.wireMatchesAt`, **η ίδια** ταύτιση με το
   `attributeFromHistory` — εξήχθη, δεν αντιγράφηκε).

| Γιατί και τα δύο | Αν έλειπε |
|---|---|
| Χρόνος χωρίς περιεχόμενο | αλλαγή από Console **μέσα** στο παράθυρο θα χρεωνόταν στη γραμμή |
| Περιεχόμενο χωρίς χρόνο | **μεταγενέστερο** Deployment με ίδια bytes (το πλάνο αναπτύσσει **μόνο** ό,τι διαφέρει) θα έπαιρνε την πράξη άλλου |

**Πού ξεπερνάμε.** Το Argo CD κρατά `status.history` (revision + χρόνος) και το **πιστεύει**· το Terraform κρατά
`serial`/`lineage` στο state. Εδώ το αρχείο της πράξης **διασταυρώνεται με τον πάροχο** (χρόνος release) **και** με
το git (περιεχόμενο @sha): ιστορικό που ψεύδεται **δεν** περνά.

**Τρεις απαντήσεις, πάντα ορατές** (`drift.withDeployment`) — **ποτέ** αλλαγή `sync`/`health`/κωδικού εξόδου:

| Απάντηση | Σημείωση |
|---|---|
| γραμμή παραγωγής | *«αναπτύχθηκε από τη γραμμή παραγωγής @sha (GitHub Deployment #id · κατάσταση · έναρξη → τέλος)»* + `verdict.deployment` |
| **εκτός εργαλείου** | ρωτήθηκε, δεν βρέθηκε ⇒ `outOfBand: true` — **πρώτη φορά** διακρίνεται από τη γραμμή |
| δεν ρωτήθηκε | *«GitHub Deployments δεν ρωτήθηκαν: <λόγος>»* (χωρίς token · 403 · δίκτυο) — **ποτέ** ψευδής κατηγορία «εκτός» |

Εφαρμόζεται **μόνο** σε ό,τι δεν εξήγησε το μητρώο (`unrecorded`, νέα σημαία του `judgeRules`) ή σε `history`.
**Ένα** αίτημα λίστας (`per_page=20`, μόνο όσα γεννήθηκαν **πριν** από το release) + ένα statuses ανά υποψήφιο, μέσω
του **ενός** πελάτη `scripts/lib/ci/github-api.js`. Δικαίωμα: `deployments: read` (**ποτέ** write) στα τρία jobs
επαλήθευσης (`firebase-plan` · `firebase-apply` · `drift`) — τα φυλάει άγκυρα, γιατί χωρίς αυτό η απόδοση **δεν
σπάει, σβήνει** σε «δεν ρωτήθηκαν». Τοπικά: `GITHUB_TOKEN=$(gh auth token) GITHUB_REPOSITORY=YorgosPag/NestorApp`.

**Μετρημένο 2026-09-21, πραγματική παραγωγή** (`firestore:verify --project pagonis-87766`, μόνο ανάγνωση):

```
✓ Synced · Healthy  firestore:rules  release 15:02:06Z · αναπτύχθηκε από τη γραμμή παραγωγής @7057264b (GitHub Deployment #6542247921 · success · 15:00:47Z → 15:02:12Z)
✓ Synced · Healthy  storage          release 08:36:53Z · αναπτύχθηκε από τη γραμμή παραγωγής @eca0e195 (GitHub Deployment #6538927993 · success · 08:35:47Z → 08:36:58Z)
```

jest `firestore-deploy-{gate,drift,pipeline,succession,provenance}`: **159/159**. **8/8 μεταλλάξεις σκοτωμένες**
(επαναφορά md5 OK): έναρξη = αναμονή · παράθυρο πάντα ανοιχτό · χωρίς ταύτιση περιεχομένου · «δεν ρωτήθηκε» =
«δεν βρέθηκε» · όλες οι ετυμηγορίες · σφάλμα GitHub ⇒ «εκτός» · χωρίς φίλτρο `before` · `unrecorded` ποτέ.
`jscpd:diff`: 0 κλώνοι · CHECK 3.47 / 3.54 ✓.

⚠️ **Δηλωμένα όρια**: (1) Οι **δείκτες** δεν έχουν χρόνο release ανά ορισμό ⇒ **δεν** αποδίδονται (μένουν
Sync/Health). (2) Φράγμα 20 Deployments: release παλαιότερο από τα 20 τελευταία ⇒ «εκτός εργαλείου» —
ψευδώς θετικό **ορατό**, όχι σιωπή· με ~1 ανάπτυξη/μέρα καλύπτει εβδομάδες. (3) Η σημαία `outOfBand` **δεν**
κοκκινίζει τίποτα ακόμη: το περιεχόμενο = δέντρο, άρα η παραγωγή είναι σωστή· είναι ερώτημα **ελέγχου**, όχι
διαθεσιμότητας. Αν αποφασιστεί ειδοποίηση, το κανάλι είναι το ένα του §11.4.
...
| Ημερομηνία | Αλλαγή |
|---|---|
| 2026-09-21 | ✅ **§11.10 — Η ΠΡΑΞΗ ΤΗΣ ΓΡΑΜΜΗΣ ΔΙΑΒΑΖΕΤΑΙ: απόδοση από τα GitHub Deployments** (handoff 2026-09-19 Βήμα 1). 🔑 **Το «Windows ≠ CI» ήταν ήδη κλειστό** (Ε2, `37be5d1d`): το run-μάρτυρας `35426414076` έτρεξε **πριν** από αυτό· η αναπαραγωγή CRLF/LF υπάρχει και δαγκώνει (μετάλλαξη ⇒ 3 κόκκινα). 🔴 **Το πραγματικό κενό**: η σημείωση *«γραμμή παραγωγής ⇒ GitHub Deployments»* υποσχόταν πηγή που **κανείς** δεν ρωτούσε (drift run `35586911393`, μετά το Ε2: rules **και** storage). Νέο `scripts/lib/firestore-deploy/deployments.js` (παράθυρο εκτέλεσης · επιλογή Deployment) · `world.wireMatchesAt` (**εξαγωγή** από `attributeFromHistory`) · `drift.withDeployment` + σημαία `unrecorded` · `verify-live.attributeDeployments` · `deployments: read` + `GITHUB_TOKEN` σε `firebase-plan`/`firebase-apply`/`drift`. Διπλή απόδειξη: χρόνος release ∈ `[in_progress, τερματική]` **και** περιεχόμενο @sha = ζωντανό. Μετρημένο στην παραγωγή: rules ⇒ #6542247921 @7057264b · storage ⇒ #6538927993 @eca0e195. jest **159/159** · **8/8** μεταλλάξεις · `jscpd:diff` 0. ⚠️ Εύρημα **εκτός** εύρους: `firestore:indexes` **OutOfSync** (ζωντανά 478 · HEAD 489, 11 λείπουν — `8d7ffca3` κ.ά.) — εκκρεμεί ανάπτυξη μέσω της γραμμής. |
| 2026-09-20 | ✅ **§11.9 — Η ΑΝΤΙΚΑΤΑΣΤΑΣΗ ΜΕΤΡΗΘΗΚΕ ΣΤΗΝ ΠΑΡΑΓΩΓΗ· το όριο 1 ΕΚΛΕΙΣΕ** (πίνακας §11.9). Άσκηση **χωρίς καμία εγγραφή** (Google DiRT / AWS REL12 game day): δύο `workflow_dispatch` με `firebase_dry_run` + **ένα** push ως κορυφή. Α `35527396356` πήρε κουμπί 17:56:23Z (deployment `6555862522` → `waiting`) ⇒ κράτησε το group· Γ `35527465255` έμεινε **χωρίς** κουμπί (deployment `6555874192` **δημιουργήθηκε** 17:57:40Z αλλά **χωρίς status**, `pending_deployments` = 0) — το **ακριβές** σύμπτωμα των `35429150868`/`35431491440`. Push Β `af32dc60` ⇒ run `35527571234`, πλάνο `none` (17:59:51Z), διαδοχή: *«✓ κορυφή του main: af32dc60»* (17:59:58.2Z) ⇒ *«⏭️ αντικαταστάθηκε #165 @aee8f601 — περίμενε έγκριση για παλαιότερο δέντρο ⇒ ακυρώθηκε»* (**18:00:00.028Z**). 🔑 **Στο ίδιο δευτερόλεπτο** (18:00:00Z) το `6555862522` πήγε `error` **και** το `6555874192` πήγε `waiting` ⇒ **η ακύρωση ελευθερώνει αμέσως το group και το νεότερο παίρνει κουμπί** — ο ισχυρισμός του §5 έγινε **γεγονός**. Το Α έκλεισε `cancelled` (**όχι** `failure`) 18:00:15Z με `release`/`notify` `cancelled` ⇒ **κανένα** Telegram «FALLITO»· CI Health (ADR-757) **έτρεξε** μετά από **κάθε** ακύρωση (runs `35527640521` 18:00:16Z · `35527785572` 18:02:48Z · `35528360271` 18:13:26Z, όλα `success`) και έγραψε **0 σχόλια** στο issue #5 — σιωπή επειδή **κοίταξε**. Το Β έκλεισε 18:13:47Z με **Κυκλοφορία στο Netcup ✓** και Ειδοποίηση ✓ ⇒ η άσκηση δεν εμπόδισε την κανονική κυκλοφορία. Δεύτερο σκέλος: το Γ εγκρίθηκε **επίτηδες** (18:02:11Z) ενώ δεν ήταν πια κορυφή ⇒ φρουρός *«⏭️ ΑΝΤΙΚΑΤΑΣΤΑΘΗΚΕ: … aee8f601 … κορυφή … af32dc60»* (18:02:28.9Z) ⇒ job `cancelled` 18:02:43Z = **14,1″**, μέσα στη χάρη των 120″, **κανένα** deploy ούτε `--dry-run`. ⚠️ **Δύο ευρήματα**: (α) το deployment του μπλοκαρισμένου γεννιέται **μαζί με το job**, άρα `pending_deployments` = 0 σημαίνει «ο κριτής δεν ρωτήθηκε», όχι «δεν υπάρχει deployment»· (β) το ακυρωμένο **deployment** παίρνει `error` ενώ το **run** είναι `cancelled` — ο CI Health κρίνει το run, σωστά. ✅ Έκλεισε **και** το όριο 2 (dispatch αντικαθίσταται: ο `KEEPER_EVENT='push'` ορίζει **ποιος σκουπίζει**, όχι ποιος σκουπίζεται). ⏳ **Αμέτρητη** μένει η **εκτέλεση** εγγραφής **δεικτών** (όριο 5): το φορτίο ήταν επίτηδες **χωρίς** εγγραφή. ✅ Την ίδια μέρα έκλεισε όμως το **πρώτο σκέλος** του ορίου 5, η **εξουσιοδότηση**, **χωρίς καμία εγγραφή**: `gcloud iam roles describe nestorFirebaseRulesDeployer` (ανάγνωση) ⇒ **17/17** με `datastore.schemas.create/update/get/list` + `datastore.operations.get`, και η πρωτογενής πηγή (Google Cloud *Firestore IAM*) βεβαιώνει ότι `datastore.schemas.*` **είναι** το νέο όνομα των `datastore.indexes.*` (*«You can still use `datastore.indexes` as an alias»*) — `create` = *Create an index*, `update` = *Update an index*. Καμία `schemas.delete` ⇒ ο deployer **δεν μπορεί** να σβήσει δείκτη. Άρα το όριο 5 **δεν** κρύβει πια ερώτημα δικαιωμάτων· μένει **μόνο** η εκτέλεση. |
| 2026-09-20 | 🔴 **§11.9 — ΣΧΟΛΙΟ ΔΕΝ ΦΤΑΝΕΙ ΣΤΟ ΠΛΑΝΟ· δύο όργανα, δύο απαντήσεις** (μετρημένο, push `aee8f601` ⇒ run `35526661461`). Το `firestore.rules` άλλαξε **μόνο** σε σχόλια (κεφαλίδα ανάπτυξης, 15 γραμμές). Το **CHECK 3.86** στο pre-commit **και** στο pre-push είπε *«⏳ deploy-pending · firestore:rules … στο push θα ζητηθεί η ΕΓΚΡΙΣΗ σου»* — η **γραμμή** απάντησε `✓ Synced · Healthy firestore:rules` ⇒ **`πλάνο: none`**, apply **skipped**. Ρίζα: το `firebase.json` δηλώνει το **παραγόμενο** `firestore.rules.compiled` και το `wireOf()` (`model.js:184`) μεταγλωττίζει πρώτα (`build-firestore-rules.js` **αφαιρεί σχόλια**, όριο 256KB) ⇒ η γραμμή κρίνει **σημασιολογία**, η πύλη **sha256 της πηγής**. Η απόκλιση είναι **προς την ασφαλή πλευρά** (ο hook προειδοποιεί χωρίς λόγο, ποτέ το αντίστροφο) ⇒ **δηλωμένη**, όχι ελάττωμα· αυθεντία είναι η **γραμμή**. Καταγράφηκε στο runbook `docs/deployment/firebase-pipeline.md` §2.1 (`af32dc60`) μαζί με τη διαδικασία της άσκησης. |
| 2026-09-19 | ✅ **§11.9 — Η ΔΙΑΔΟΧΗ ΣΤΟ ΠΛΑΝΟ `none` (μετρημένο)**. Push `7057264b..f9b7ed1e` ⇒ run `35452086164` **success**. Πλάνο `105920892665`: **3/3 Synced·Healthy** ⇒ `none` (οι κανόνες του `35450352067` είναι πλέον ζωντανοί). Διαδοχή `105921016007` (15:32:57→15:33:41Z): *«✓ κορυφή του main: f9b7ed1e»*. `firebase-queue-watch` και `firebase-apply` **skipped**, όπως ορίζει το `if` ⇒ **κανένα** αίτημα έγκρισης, **κανένα** Telegram «ζητείται έγκριση». Release `105922766145`: φρουρός *«✓ κορυφή»* ⇒ Coolify *«Deployment request queued»* (`oqhoxr6vcid9zb5ogcm4qgrf`, 15:45:34Z) · notify ✓. ⇒ Η διαδοχή **δεν** καθυστερεί ούτε εμποδίζει κυκλοφορία που δεν χρειάζεται Firebase (κόστος: ένα job ~44″ παράλληλα με το build). **Οι λειτουργίες μόνο-ανάγνωσης στο ΠΡΑΓΜΑΤΙΚΟ API** (τοπικά, `gh auth token`, κανένα `claim/guard/watch` που θα μπορούσε να ακυρώσει): `tip` στο `f9b7ed1e` ⇒ `fresh=true` (θετικός μάρτυρας) · `tip` στο `7057264b` ⇒ `fresh=false` (αρνητικός) · `remind` ⇒ *«✓ καμία έγκριση δεν περιμένει ≥ 8 ώρες»*, έξοδος 0. ⇒ οι διαδρομές `git/ref/heads/main` · `actions/workflows/docker-build.yml/runs` · `…/pending_deployments` απαντούν με το σχήμα που περιμένει ο κώδικας. ⏳ Η **αντικατάσταση** μένει αμέτρητη (§11.9 όριο 1): θέλει push **με** αλλαγή Firebase και δεύτερο push **πριν** την έγκριση. |
