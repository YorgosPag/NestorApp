...
| Ημερομηνία | Τι |
|---|---|
| **2026-09-21** | 🧪 **Η άγκυρα Γ1 (`auth-action-mail.test.ts`) κοκκίνιζε από ΜΟΡΦΗ, όχι από γλώσσα.** Ήρθε `'Set a new password — Nestor App'` αντί `'… — Nestor'`: το θέμα ήταν **ήδη αγγλικό**, δηλαδή η δηλωμένη γλώσσα **κέρδιζε** σωστά. 🔍 **Ρίζα**: το test γράφτηκε στο `0fc9fa73` (2026-09-11) με την παλιά υπογραφή ανά γλώσσα· μία μέρα μετά το **`91063ffb`** (ADR-857) ενοποίησε σκόπιμα την υπογραφή του `brandedSubject` στο `PRODUCT_NAME`. ✅ Το test ρωτά πλέον το SSoT (`` `Set a new password — ${PRODUCT_NAME}` ``, σύμβαση του `email-texts.test.ts`) — ο κώδικας **δεν** άλλαξε. 🔬 Μετάλλαξη «η δηλωμένη γλώσσα αγνοείται» στο `recipientLanguage` ⇒ **Γ1 κόκκινο** (μετρητής 1, επαναφορά ταυτόσημη). |
