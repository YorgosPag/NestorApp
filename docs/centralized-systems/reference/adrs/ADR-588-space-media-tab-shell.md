...
## Changelog
- **2026-09-19 (Phase 6 — «the listener owns the list», ADR-866 §2.10.8 Β5)** — Re-running the dossier upload live showed the Phase 5 fix working (types offered, file written to the right scope, right tab) **but invisible until a manual refresh** (4/4 uploads). Measured with the Firestore SDK itself: scoped tabs send **one** canonical query (Phase 5 dropped the domain/category narrowing), so the post-upload `getDocs` was answered from the **already-live** floorplan listener's view — `fromCache: false`, yet ~100 ms before the watch stream delivered the acknowledged write (3 docs at `ready`, 4 docs 97 ms later). Cure: `fileListIsLive(displayStyle, scopes)` in `upload-scope.ts` — a tab **with scopes** is always live (the shell's `EntityFilesManager` wires `realtime` from it, no hand-written `displayStyle` test); `useEntityFiles` lets the listener **own** the list (`refetch` + bus events do not read while it lives; listener failure ⇒ one-time read as before). Parking/storage unchanged (no scopes). Also: the «Upload file» command now **opens** the zone instead of toggling it (Π4). Anchors Α39.1/Α39.2/Α39.4 (ADR-866 §7).
- **2026-09-19 (Phase 5 — «what a tab offers is what it reads», ADR-866 §2.10 Β1)** — Live verification of the property dossier in production found upload **impossible**: (a) no `property_dossier` key in `UPLOAD_ENTRY_POINTS` ⇒ empty selector under «select document type first» (dead end); (b) the **class** root — a tab **read** by its own `domain`/`category` while an upload **wrote** the entry point's, two independent truths ⇒ either a dead end or an upload that succeeds **invisibly**. Shell changes (Parking/Storage **unchanged** — they declare nothing new): `MediaTabConfig.entryPointScope { offer, read, purposeAuthority }` + `emptyKey`; new pure `media-tab-scope.ts` (`mediaTabPurpose` — the one `${prefix}-${key}` rule · `mediaTabUploadDefaults` · `mediaTabOfferedEntryPoints` · `mediaTabScopePolicy`), consumed by `EntityMediaFilesTab` **and** anchor Α38.1. Below the shell: `components/shared/files/utils/upload-scope.ts` (write resolver **extracted** from `useFileUpload`; read scopes **derived** from it; one read filter for fetch **and** realtime) · `selectOfferedEntryPoints` (moved out of `UploadEntryPointSelector`) · explicit «no document type» state. 🔴 **Measured, not fixed here** (in `pending-ratchet-work.md`): Parking/Storage **Documents** and **Videos** tabs have **zero** catalog entry points (closed set, shrink-only, in Α38.1); the property tabs of the office (`features/properties-sidebar` Photos/Videos, `sales`) offer «construction progress» types that write `construction` ⇒ invisible upload (static). Cure = the 7 hand-written sites declare `entryPointScope`.
- **2026-09-18 (Phase 4 — custody from the binding, ADR-866 Φ1.2)** — The shell stopped hard-wiring `custody={{ companyId }}`. `EntityMediaBinding` gained a **required** `custodySource: { kind: 'session-company' } | { kind: 'personal', custody: { userId } }`; `parkingMediaBinding`/`storageMediaBinding` declare `SESSION_COMPANY_CUSTODY` (behaviour **unchanged** — same session company, same `companyName`), and the new `propertyDossierMediaBinding` declares the dossier owner. `EntityMediaFilesTab` now has two branches over **one** render point (`ResolvedMediaFiles`): the personal branch **never** calls `useEntityFilesTabSession`/`useCompanyId` (a personal space never widens to the employee's company). Third consumer: the property dossier tabs (Floorplan · Documents · Photos · Videos) reuse the **same four** `*_MEDIA_CONFIG`. Two blockers found on the way, both of which would have crashed the first `(me)` page with files: `EntityFilesManager` called `useWorkspace()` in its body (throws outside `(app)`) — the “Belongs to” label moved into `EntityFilesToolbar` as `ActiveWorkspaceLabel`, rendered **only** for company custody; and `NotificationProvider` was missing from `(me)`. Anchors: ADR-866 Α37.8 (shell) · Α37.9 (no workspace provider). The 7 manual call sites (`pending-ratchet-work.md`) remain: their prerequisite is met, but they need a children slot in the shell first (>1h, 3 domains).
- **2026-09-17 (Phase 3 — building · project · property file tabs)** — Triggered by CHECK 3.28 blocking the ADR-866 Φ0 βήμα 2β commit: the `companyId` → `custody` prop rename touched 26 sibling tabs at once and exposed **27 pre-existing clones** (verified: 0 clones among the 132 new/substantively-changed files; all 27 among the 1-line-rename files). Same doctrine as Phase 1, extended beyond spaces. New SSoTs in `components/shared/files/`: **`useEntityFilesTabSession`** (auth uid + company + `useCompanyDisplayName`; an explicitly passed `companyId` — even `undefined` — never silently falls back to the session company, so super_admin cross-tenant resolution stays fail-closed · 3 jest tests, mutation `in`→`??` ⇒ 1 red) and **`EntityFilesTabPlaceholder`**. Per-family bodies: `building-management/tabs/building-files-tab.tsx` (`useBuildingFilesTab` + `BuildingMediaTab`), `projects/project-files-tab.tsx` (`useProjectFilesTab` + `ProjectMediaTab` + `ProjectFilesTabPlaceholder`), `properties-sidebar/components/property-files-tab.tsx` (`usePropertyFilesTab` returning `identity` + `fallback`, message keys as literals). Photos/Videos tabs of building + project are now thin `kind` bindings; `FloorFloorplanInline` is a binding over `SpaceFloorplanInline` (+ optional `purpose`); `EntityMediaFilesTab` adopts the session hook. **Bug class removed**: 7 tabs carried a hand-copied company-name `useEffect` **without a cancellation guard** (fast entity switch could write the previous company's name) — all now go through the one guarded hook (closes the Phase 1 note «`FloorFloorplanInline` can adopt `useCompanyDisplayName` on next touch»). Contacts: `disabledOnly` + `RendererFn` promoted from a private copy in `contactRenderersTyped` to exported SSoT in `contactRenderersCore` (also used by `communication` / `taxOffice`); `address`/`addresses` share one renderer. Behaviour notes: property `FloorPlanTab` label `Μονάδα {id}` literal → i18n `unitFallbackLabel` (same text); property «select» placeholders `div` → `section`; project placeholders `div` → `section` (same padding token). jscpd:diff on all 22 touched files: **27 → 0**. ⚠️ No render tests exist for these tabs (none before either).
