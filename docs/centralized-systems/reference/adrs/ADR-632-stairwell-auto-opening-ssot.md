...
- **Status**: In progress (Phase 0–4.1 DONE· Φ5 pending)
...
| **5** | deterministic-stable id ✅ · stair-create→command + edge-touching opening + 3D/UX (badge/override/lock) + ADR finalize | 🔶 PARTIAL |
...
- ~~**id churn σε undo→redo**~~ **(RESOLVED Φ5, 2026-07-11):** ο coordinator `applyCreate` δίνει
  ...
  openings κρατούν random id (backward compat — `idOverride?` optional στο `buildSlabOpeningEntity`).
