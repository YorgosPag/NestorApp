...
- **Status**: DONE (Phase 0–5 complete· Φ5: deterministic-id + edge-touching + 3D/UX + lock/override + stair-create→undoable command DONE)
...
- **FULL tread set (below + above cut plane):** το `buildStairInput` διαβάζει
  `treadsBelowCut ∪ treadsAboveCut`, **ΟΧΙ** το `StairGeometry.treads` (= legacy alias
  του `treadsBelowCut`, δηλ. μόνο η 2D-section φέτα κάτω από το cut plane 1200mm). Τα
  headroom-παραβατικά σκαλοπάτια είναι τα ΨΗΛΑ (κοντά στην πλάκα, πάνω από το cut) —
  διαβάζοντας μόνο το alias, έλειπαν → η τρύπα έβγαινε σε λάθος (κάτω) φέτα της σκάλας.
  Mirror του 3D SSoT (`StairToThreeConverter` spread below+above).
...
| **5** | deterministic-stable id ✅ · edge-touching opening ✅ · **3D punch (inherited free) ✅ · UX warnings panel ✅ · lock/override (managed detach) ✅ · stair-create→undoable command ✅** | ✅ DONE |
| **CL-1** | **Cross-level absolute-Z SSoT** — `floorElevationMm` offset στους input builders (default 0 = same-level byte-for-byte) + 2-level pairing jest | ✅ DONE |
| **CL-2** | Multi-level gather + building-wide planner (`stairwell-cross-level-plan.ts`) + cross-level opening writer (`stairwell-opening-cross-level-writer.ts`, mirror `foundation-cross-level-writer`) | ✅ DONE |
| **CL-3** | Owner hook `useCrossLevelStairwellOpenings` (gather all-floor → plan → apply· mount @ shell) + persistence scope πλάκας | ✅ DONE |
...
με τον τύπο, ώστε να συγκλίνει ΚΑΙ στο undo όπου το entity μόλις αφαιρέθηκε). Καλείται από το ΕΝΑ
undoable create SSoT `CreateBimEntityCommand` **execute/redo/undo** — πλάκα πάνω από σκάλα (ή, μετά το
Φ5, **σκάλα κάτω από πλάκα**) ανοίγει το opening· undo το σβήνει (idempotent re-run πάνω στην επαναφερμένη
σκηνή → orphan delete)· redo το ξαναφτιάχνει.

> **Φ5 update (2026-07-11):** αρχικά ο reconcile καλούνταν από **δύο** creation call-sites — το command
> (slab, undoable) **και** χειροκίνητα το `addStairToScene` (stair, raw `setLevelScene`, μη-undoable). Με το
> Φ5 το `addStairToScene` έγινε thin wrapper πάνω στο `appendEntityToScene` → `CreateBimEntityCommand`, οπότε
> η σκάλα περνά πλέον από το **ίδιο** command· η χειροκίνητη κλήση αφαιρέθηκε → **ένα** call-site, μηδέν
> διπλός cascade, stair create undoable (βλ. §8 «Γνωστά κενά» → RESOLVED + §9 changelog).
...
### Γνωστά κενά (honesty)
- ~~**σκάλα create = μη-undoable**~~ **(RESOLVED Φ5, 2026-07-11):** το `addStairToScene` έγινε **thin
  wrapper** πάνω στο generic `appendEntityToScene` SSoT (μοτίβο `add-column-to-scene`): κρατά ΜΟΝΟ το
  stair-specific floorId/buildingId enrichment (ADR-358 Φ9C) και delegate-άρει στο undoable
  `CreateBimEntityCommand`. Πλέον Ctrl+Z μετά το σχεδίασμα σκάλας αναιρεί τη σκάλα **και** (μέσω του
  command's undo → create-time reconcile) το auto «well» opening της· redo τα ξαναφτιάχνει (Revit-grade:
  κάθε create = ένα undoable transaction, ADR-390). Ο **χειροκίνητος** `reconcileAssociativeGeometryOnCreate`
  (πρώην `add-stair-to-scene.ts:76`) αφαιρέθηκε — ο cascade έρχεται πλέον ΑΠΟΚΛΕΙΣΤΙΚΑ από το
  execute/redo/undo του command (**μηδέν διπλός cascade**). Το `emitBimEntityCreated` (τώρα deferred
  μέσα στο command) + το `bim:stair-delete-requested` στο undo διατηρούν πλήρη persistence parity
  (first-save + Firestore deleteDoc — όχι zombie). Η σκάλα ΔΕΝ ανήκει στο `STRUCTURAL_OVERLAP_TYPES`
  (ADR-567), οπότε ο overlap guard του generic κάνει no-op (σκάλα κάτω από πλάκα by design δεν μπλοκάρεται).
  Signature αμετάβλητη → οι δύο callers στο `useSpecialTools.ts` (`'stair'` / `'stair-from-region'`)
  δεν άλλαξαν. +11 jest (`bim/stairs/__tests__/add-stair-to-scene.test.ts` — create/undo/redo/opening-follows/
  no-op/floor-stamp), jscpd diff clean.
...
## 8b. Αρχιτεκτονική cross-level (σκάλα ορόφου Ν ↔ πλάκα-οροφή ορόφου Ν+1)

**Πρόβλημα (browser repro Giorgio):** ο auto «well» opening δούλευε ΜΟΝΟ όταν σκάλα +
πλάκα ήταν στον ΙΔΙΟ όροφο. Στο πραγματικό Revit/ArchiCAD workflow η σκάλα ζει στον
όροφο Ν και η πλάκα-οροφή που την «καπακώνει» είναι το δάπεδο του ορόφου Ν+1 →
**διαφορετικές σκηνές** → ο `cascadeStairwellOpenings(sceneManager)` (single-level,
διαβάζει `getEntities()` ΕΝΟΣ level) δεν τις έβλεπε μαζί → καμία τρύπα.

**SSoT audit (N.8, plan mode) — τρία ευρήματα που καθόρισαν τη λύση (reuse-first, μηδέν 2ος engine):**

1. **Ο pure planner είναι ΗΔΗ datum-agnostic.** `planStairwellOpenings` / `findSlabsAboveStair`
   / `evaluateStairHeadroom` δουλεύουν καθαρά σε **absolute mm** (x/y σε κοινό building origin —
   ίδια υπόθεση με το ETICS `resolveSlabsAboveForLevel`). Καμία single-level υπόθεση. Άρα ζευγαρώνουν
   σωστά cross-level **αρκεί** να τραφούν με absolute-Z inputs από πολλούς ορόφους. **Δεν αγγίζεται.**
2. **Z-datum:** η in-scene γεωμετρία είναι **level-relative (0-based ανά όροφο)** — ο 3D stacker
   (`multi-floor-3d-source` → `BimSceneLayer.syncMultiFloor`) προσθέτει `floorElevationMm` ανά όροφο.
   Το same-level δούλευε γιατί σκάλα + πλάκα μοιράζονται τον ίδιο (μη-προστιθέμενο) offset (ακυρώνεται)·
   cross-level διαφέρουν → πρέπει να προστεθεί το FFL **και στις δύο** πλευρές (ίδιο pattern με το
   foundation `activeFloorElevationMm`, ADR-459).
3. **Cross-level READ/WRITE υπάρχουν ήδη ως precedents:** `useEnvelopeFloorSlabs` +
   `envelope-floor-slabs-store` (μαζεύει όλων των ορόφων τις πλάκες + elevations) για READ·
   `foundation-cross-level-writer` (γράφει entity στη σκηνή+Firestore scope **άλλου** ορόφου, derived
   origin `'system-reconcile'`, optimistic store) για WRITE. `resolveFloorElevationMm`
   (`building-foundation-level`) = canonical FFL mm ανά όροφο.

**Απόφαση μοντέλου (Giorgio: «όπως οι μεγάλοι — Revit/Maxon/Figma»):** **derived-write**, ΟΧΙ
cross-level undoable command. Το «well» opening είναι **associative/derived** στοιχείο (regenerate-
on-host-change), ανήκει στον engine — mirror του cross-level foundation (ADR-459). Ζει στη σκηνή +
Firestore scope της **ΠΛΑΚΑΣ** (πάνω όροφος)· heals σε κάθε reconcile· ΔΕΝ μπαίνει στο undo stack
της σκάλας (Ctrl+Z σκάλας → ο idempotent re-run του planner σβήνει το orphan opening).

**Φάσεις:** **CL-1 (DONE)** — absolute-Z SSoT: optional `floorElevationMm` offset στους input
builders (`buildStairwellPlanStairs` / `buildStairwellSlabCandidates`), εφαρμοσμένο ομοιόμορφα σε
base/top/nosings + slab top/underside. Default `0` → same-level/single-level byte-for-byte αμετάβλητο
(fast path, όλα τα υπάρχοντα tests πράσινα). +4 jest (offset units + 2-level pairing που περνά ΜΟΝΟ σε
κοινό absolute datum + same-level regression).

**CL-2 + CL-3 (DONE) — building-wide cross-level engine (ξεχωριστές σκηνές).** Απόφαση: **cross-level
ONLY** (big-player, Revit/ArchiCAD: ένας owner ανά associative σχέση). Το ίδιο-όροφο ζεύγος το κατέχει
ήδη ο command-time single-level `cascadeStairwellOpenings` (undoable, μέσα στο undo stack). Ο building-
wide αναλαμβάνει ΜΟΝΟ ζεύγη σκάλα/πλάκα σε **διαφορετικούς** ορόφους → κανένας διπλός owner, μηδέν
race με το command path.

- **`bim/stairs/stairwell-cross-level-plan.ts`** (pure) — `planCrossLevelStairwellOpenings(entries)`:
  για κάθε όροφο-στόχο (πλάκα) ζευγαρώνει τις πλάκες του με τις σκάλες **ΟΛΩΝ ΤΩΝ ΑΛΛΩΝ** ορόφων σε
  κοινό απόλυτο datum (CL-1 FFL offset ανά όροφο), τρέχει τον **ίδιο** pure planner `planStairwellOpenings`
  (μηδέν 2ος engine) και επιστρέφει writer-actions ανά όροφο. «Existing» = ΜΟΝΟ τα cross-level managed
  openings της σκηνής (autoStairId που ΔΕΝ ανήκει σε σκάλα ίδιου ορόφου → same-level = command-owned,
  δεν αγγίζεται· orphaned → delete, ΑΚΟΜΗ κι όταν έφυγαν όλες οι σκάλες). Idempotent.
- **`bim/stairs/stairwell-opening-cross-level-writer.ts`** — mirror `foundation-cross-level-writer`
  (ADR-459): `SlabOpeningFirestoreService` με το scope του ορόφου **της πλάκας** (`resolveBimPersistenceScope`)
  + upsert-by-id scene mutation origin `'system-reconcile'` (idempotent, δεν πυροδοτεί autosave). Firestore
  = fire-and-forget· `saveSlabOpening` = setDoc overwrite (neverUpdate, deterministic id).
- **`hooks/data/useCrossLevelStairwellOpenings.ts`** (owner hook @ shell, δίπλα στο `useFoundationLevelSync`)
  — μαζεύει σκάλες+πλάκες+openings **όλων** των ορόφων (live `Bim3DEntitiesStore` active · `getLevelScene`
  visited · one-shot `loadFileV2` unvisited· mirror `useEnvelopeFloorSlabs`), τρέχει τον planner, εφαρμόζει
  μέσω writer. **Delete-safety gate `allResolved`:** το apply τρέχει ΜΟΝΟ όταν ΟΛΟΙ οι όροφοι έχουν φορτωθεί
  (αλλιώς, όσο φορτώνει η σκηνή της σκάλας, ο planner θα έσβηνε το opening ως ψευδο-orphan).
- **Reuse-first (N.0.2/N.18 — μηδέν clone):** (α) extract shared `materializeStairwellAutoOpening` /
  `rebuildStairwellOpeningOutline` στον `stairwell-opening-coordinator` (single-level apply + cross-level
  writer). (β) **νέος SSoT `systems/levels/mutate-level-scene.ts` (`mutateLevelSceneEntities`)** — ο
  stairwell writer ΚΑΙ ο **foundation** writer (`foundation-cross-level-writer`, ADR-459) είχαν
  **verbatim-ίδιο** `mutate*Scene` helper (get→replace entities→set `'system-reconcile'`)· εξήχθη σε ένα
  και τα δύο τον καλούν (boy-scout dedup ενός pre-existing twin). Ο pure planner + input builders αμετάβλητοι.
  jscpd diff clean (6 αρχεία), foundation 16/16 + cross-level 11/11 regression green.

## 9. Changelog

- **2026-07-11** — **Phase CL-2 + CL-3 DONE (building-wide cross-level engine, ξεχωριστές σκηνές).**
  SSoT audit (N.8, plan mode) πριν από κώδικα — επιβεβαιώθηκε (grep): ο pure planner + input builders
  (CL-1) είναι datum-agnostic (μηδέν 2ος engine)· gather precedents `useEnvelopeFloorSlabs`/
  `useFloors3DAggregator`· write precedent `foundation-cross-level-writer`· scope `resolveBimPersistenceScope`·
  FFL `resolveFloorElevationMm`· slab-opening Firestore `createSlabOpeningFirestoreService` — όλα υπαρκτά,
  **καμία** cross-level stairwell υλοποίηση. **Απόφαση Giorgio («όπως οι μεγάλοι»): cross-level ONLY** —
  ένας owner ανά associative σχέση (Revit/ArchiCAD)· same-level = command-owned (undoable), cross-level =
  derived-write engine (mirror foundation ADR-459). **Νέα:** pure `stairwell-cross-level-plan.ts`
  (`planCrossLevelStairwellOpenings` — building-wide diff, absolute Z, cross-level pairing + orphan cleanup,
  reuse του `planStairwellOpenings`) + `stairwell-opening-cross-level-writer.ts` (scene upsert
  `'system-reconcile'` + Firestore scope πλάκας, fire-and-forget) + owner hook
  `useCrossLevelStairwellOpenings` (gather all-floor → plan → apply, `allResolved` delete-safety gate,
  mount @ `DxfViewerContent` δίπλα στο `useFoundationLevelSync`). **Reuse (N.0.2/N.18):** extract shared
  `materializeStairwellAutoOpening` + `rebuildStairwellOpeningOutline` στον coordinator (τα μοιράζονται
  single-level apply + cross-level writer· μηδέν sibling clone). +7 jest (`stairwell-cross-level-plan.test.ts`
  — cross-level pairing / same-level skip / λάθος-datum no-pair / idempotent / orphan delete / same-level
  managed untouched / single-floor no-op), **173/173** stairwell+slab-opening regression green, jscpd diff
  clean. Βλ. §8b. **Εκκρεμεί:** browser re-verify Giorgio (σκάλα ισόγειο, πλάκα 1ος = ξεχωριστές σκηνές).
- **2026-07-11** — **Phase CL-1 DONE (cross-level absolute-Z SSoT).** SSoT audit (N.8, plan mode)
  πριν από κώδικα: ο pure planner είναι ΗΔΗ datum-agnostic (μηδέν 2ος engine)· η in-scene γεωμετρία
  είναι level-relative (0-based ανά όροφο, ο 3D stacker προσθέτει `floorElevationMm`)· cross-level
  READ/WRITE precedents υπάρχουν (`useEnvelopeFloorSlabs`, `foundation-cross-level-writer`). Απόφαση
  Giorgio («όπως οι μεγάλοι»): derived-write μοντέλο (associative opening, ανήκει στον engine, ζει στο
  scope της πλάκας — mirror foundation ADR-459), ΟΧΙ cross-level undoable command. **CL-1 = absolute-Z
  θεμέλιο:** optional `floorElevationMm` offset στο `StairwellInputOptions` + `buildStairwellSlabCandidates`,
  εφαρμοσμένο ομοιόμορφα σε base/top/nosings + slab top/underside (`stairwell-opening-inputs.ts`, ΟΧΙ σε
  winder-agent περιοχή). Default `0` → same-level/single-level byte-for-byte αμετάβλητο (fast path). +4 jest
  (offset units ×2, 2-level pairing που περνά ΜΟΝΟ σε κοινό absolute datum, same-level regression), 72/72
  stairwell regression green, jscpd diff clean. Βλ. §8b. **CL-2/CL-3 DONE** (βλ. παραπάνω entry).
- **2026-07-11** — **Bugfix (τρύπα σε λάθος περιοχή)** — browser repro (Giorgio): σκάλα + πλάκα-οροφή
  στον ΙΔΙΟ όροφο → η auto «well» τρύπα εμφανιζόταν αλλά **μετατοπισμένη**, όχι πάνω από τα παραβατικά
  σκαλοπάτια. **Ρίζα (SSoT audit):** το `buildStairInput` (`stairwell-opening-inputs.ts`) διάβαζε
  `stair.geometry.treads`, που είναι **legacy alias = `treadsBelowCut`** (γρ. 504-505 `stair-types.ts`·
  γρ. 220-223 `stair-geometry-generators.ts` → `treads: split.below`) — μόνο η 2D-section φέτα ΚΑΤΩ από
  το cut plane (default 1200mm). Τα headroom-παραβατικά σκαλοπάτια είναι τα ΨΗΛΑ (πάνω από το cut, κοντά
  στην πλάκα) → **έλειπαν εντελώς** από nosings+outline → τρύπα από λάθος (κάτω) σκαλοπάτια. Το `bbox`
  (full-stair) περνούσε το coarse overlap gate, γι' αυτό εμφανιζόταν *κάποια* τρύπα. **Fix (reuse-first,
  mirror 3D SSoT):** ο 3D `StairToThreeConverter` ήδη κάνει `[...treadsBelowCut, ...treadsAboveCut]` — το
  ίδιο τώρα και το `buildStairInput` (`belowCut ?? treads` + `aboveCut ?? []` → backward-compat για legacy
  fixtures). 1 production αρχείο (`bim/stairs/stairwell-opening-inputs.ts`, ΟΧΙ στην winder-agent περιοχή
  `bim/geometry/stairs/`). +3 jest (`stairwell-opening-inputs.test.ts` — above-cut inclusion / legacy alias /
  below-only), 79/79 stairwell regression green, jscpd diff clean. **Δεν σχετίζεται με το Φ5 undoable** —
  pre-existing bug Φ1/Φ3.
- **2026-07-11** — Phase 5 (**complete**) — **stair-create → undoable `CreateBimEntityCommand`** (το τελευταίο
  Φ5 κενό). SSoT audit (N.8, plan mode) πριν από κώδικα: ο undoable μηχανισμός **υπήρχε ήδη**
  (`appendEntityToScene` → `CreateBimEntityCommand`, με create-time reconcile στα execute/redo/undo από Φ4.1)·
  η σκάλα απλώς δεν τον χρησιμοποιούσε (ιστορικό ADR-619 raw-`setLevelScene` path). **Λύση = reuse-first, όχι
  νέο path:** το `add-stair-to-scene.ts` έγινε **thin wrapper** (μοτίβο `add-column-to-scene`) — κρατά ΜΟΝΟ το
  floorId/buildingId enrichment (ADR-358 Φ9C) και delegate-άρει στο `appendEntityToScene`. Αφαιρέθηκαν: raw
  `setLevelScene`, sync `emitBimEntityCreated`, **χειροκίνητος `reconcileAssociativeGeometryOnCreate`** (ο
  cascade έρχεται πλέον ΑΠΟΚΛΕΙΣΤΙΚΑ από το command → **μηδέν διπλός cascade**). Αποτέλεσμα: Ctrl+Z μετά το
  σχεδίασμα σκάλας αναιρεί σκάλα **+** auto «well» opening· redo τα επαναφέρει (symmetric, ADR-390). Persistence
  parity διατηρημένη (deferred `drawing:entity-created` first-save· `bim:stair-delete-requested` deleteDoc στο
  undo — `'stair'` case υπήρχε ήδη στο lifecycle SSoT· `use-stair-persistence` ακούει και τα δύο). Overlap guard
  του generic = no-op για σκάλα (όχι στο `STRUCTURAL_OVERLAP_TYPES`, ADR-567). Signature αμετάβλητη → οι δύο
  callers (`useSpecialTools` `'stair'`/`'stair-from-region'`) δεν άλλαξαν. Docstrings update
  (`associative-geometry-reconcile` call-sites — η σκάλα πλέον μέσω command, όχι direct). **+11 jest** (create/
  undo/redo/opening-follows/no-op/floor-stamp), stairwell-create-trigger + engine regression green, jscpd diff
  clean. **Φ5 = πλήρως DONE → ADR-632 DONE.**
