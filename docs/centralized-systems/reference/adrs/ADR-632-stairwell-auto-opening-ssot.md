...
| **CL-2** | Multi-level gather producer (mirror `useEnvelopeFloorSlabs`) + cross-level opening writer (mirror `foundation-cross-level-writer`) | ⏳ TODO |
| **CL-3** | Trigger wiring cross-level (create/edit/delete reconcile) + persistence scope πλάκας | ⏳ TODO |
...
κοινό absolute datum + same-level regression). **CL-2** — multi-level gather producer + cross-level
opening writer. **CL-3** — trigger wiring cross-level (create/edit/delete) + persistence scope πλάκας.
...
- **2026-07-11** — **Phase CL-1 DONE (cross-level absolute-Z SSoT).** SSoT audit (N.8, plan mode)
  ...
  stairwell regression green, jscpd diff clean. Βλ. §8b. **Εκκρεμεί:** CL-2 (gather+writer), CL-3 (wiring).
