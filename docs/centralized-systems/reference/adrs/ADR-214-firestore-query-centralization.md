...
| Metadata | Value |
|----------|-------|
| **Status** | ✅ COMPLETED — All 11 Phases Done |
| **Date** | 2026-03-12 |
...
| 2026-03-13 | **Phase 9 COMPLETE**: Enterprise Config Services Migration — 6 services migrated (7 files modified + 2 infrastructure). **EnterpriseCompanySettingsService**: 4 queries (2× `getById`, 1× `getAll`, 1× `update`), kept 1× raw `setDoc` (merge). **EnterpriseFileSystemService**: 1 query → `getAll`, kept 1× raw `setDoc` (merge). **EnterpriseLayerStyleService**: 3× `getAll` + 2× `create` + 1× `update` — fully migrated, zero raw imports remaining. **EnterprisePolygonStyleService**: 1× `getAll` + 1× `update` + 1× `create` — fully migrated. **EnterpriseUserPreferencesService**: 1× `getById` + 1× `getAll` + 1× `update`, kept 1× raw `setDoc` (merge). **EnterpriseSecurityService**: 3× `getAll` (security_roles, email_domain_policies, country_security_policies) — **ARCHITECTURE**: Removed `initialize(firestore)`/`ensureInitialized()`/`getDb()` pattern; `firestoreQueryService` uses global `db` singleton. **UserRoleContext.tsx**: Removed `securityService.initialize(db)` block + `securityInitialized` state. **Infrastructure**: 3 new collection keys (SECURITY_ROLES, EMAIL_DOMAIN_POLICIES, COUNTRY_SECURITY_POLICIES) in `firestore-collections.ts` + `tenant-config.ts` (mode: 'none'). All queries use `tenantOverride: 'skip'` since services manage manual `tenantId`/`environment` constraints. |
| 2026-03-13 | **Phase 9 FIX**: 9 TS errors fixed. **EnterpriseCompanySettingsService.ts**: `companyDoc.id` type guard (unknown → string). **EnterpriseSecurityService.ts**: 8 date field errors — replaced manual `as Record<string, unknown> & { toDate? }` casts with centralized `normalizeToDate()` from `@/lib/date-local` (ADR-218). |
| 2026-03-13 | **Phase 10 COMPLETE**: DXF Viewer Migration (Pragmatic scope). **overlay-store.tsx**: `addDoc` → enterprise ID (`generateOverlayId()`) + `setDoc` (ADR-210 compliance). **dxf-firestore.service.ts**: 3× `getDoc` → `firestoreQueryService.getById('CAD_FILES', ...)` (getFileMetadata, fileExists, getFile). Write methods (setDoc + Storage) untouched. **enterprise-id.service.ts**: 2 new prefixes (OVERLAY: 'ovrl', LEVEL: 'lvl') + generators. **tenant-config.ts**: 2 new entries (CAD_FILES, DXF_OVERLAY_LEVELS: mode 'none'). **SKIPPED**: LevelsSystem.tsx (collection name mismatch `'dxf_viewer_levels'` ≠ COLLECTIONS `'dxfViewerLevels'` + writeBatch dependency), ReadOnlyLayerViewer.tsx (already migrated Phase 7). |
| 2026-03-13 | **Phase 11 COMPLETE — ADR CLOSED**: AuthorizedQueryService deprecated + dead code removed. **query-middleware.ts**: Added `@deprecated` JSDoc to `AuthorizedQueryService` class + `QueryServiceFactory`. Deleted 3 legacy convenience functions (`readProjects`, `readUserContacts`, `logQueryContext`) — 0 consumers, dead code. Kept `AuthorizationError` + `QueryExecutionError` (re-exported by FirestoreQueryService). Kept interfaces (`AuthenticationContext`, `SessionMetadata`, etc.). **firestore-query.service.ts**: Updated re-export comment — canonical import point for error types. **SPEC-214-11**: Status → SUPERSEDED (FirestoreQueryService absorbed all functionality). **ADR-214 Status → COMPLETED**. |
| 2026-09-21 | **Η πύλη ταυτότητας ακολουθεί τη συλλογή.** 🔴 Ζωντανή βλάβη: `[UserNotificationSettingsService] Subscription error … MissingTenantError` σε άνθρωπο χωρίς εταιρεία / στο `/o/me`. Ρίζα: **κάθε** μέθοδος του service (`getAll` · `subscribe` · `subscribeDoc` · `create` · `batchGet`) περνούσε από το `requireAuthContext()`, που πετά `MissingTenantError` **πριν** κοιτάξει τι ζητήθηκε — άρα και οι συλλογές **ανθρώπου** (`mode: 'userId'`) ζητούσαν οργανισμό, ενώ οι κανόνες τις δένουν μόνο με `request.auth.uid`. Συνέπεια πέρα από τον θόρυβο: ο αυτόνομος (ADR-809) έβλεπε **σιωπηλά άδειο** καμπανάκι (το `useFirestoreNotifications` καταπίνει το σφάλμα ως σχεδιασμένη κατάσταση) και η ζώνη ώρας του έπεφτε στη συσκευή. Θεραπεία: `auth-context.ts` → `requireUserContext()` (ίδια υλοποίηση `resolveAuthContext(requirement)`, χωρίς την απαίτηση εταιρείας)· `firestore-query.service.ts` → `contextFor(key, override)`: `userId` ⇒ `requireUserContext`, αλλιώς `requireAuthContext` (το `subscribeToCollection` παίρνει πλέον την πύλη ως παράμετρο· το `subscribeSubcollection` κρατά την απαίτηση οργανισμού). Οι συλλογές `companyId`/`tenantId`/`none` **αμετάβλητες**. Jest: `user-scoped-context.test.ts` (Κ1-Κ3) + `firestore-query-context.test.ts` (Κ4-Κ6) — φάκελος 62/62 πράσινα· μετάλλαξη (πάντα `requireAuthContext`) ⇒ Κ4+Κ5 κόκκινα. ⏳ Ζωντανή επαλήθευση (Giorgio): ο αυτόνομος να βλέπει τις ειδοποιήσεις του |
