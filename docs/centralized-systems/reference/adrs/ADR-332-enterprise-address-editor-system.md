...
### D24 — «το κύριο, αλλιώς το πρώτο» απέκτησε μία διατύπωση — και ο αριθμός στο backlog ήταν **δεκαπλάσια μικρός**

**ΕΝ ΕΞΕΛΙΞΕΙ 2026-09-05 — το SSoT υπάρχει και εκτελείται· η εκστρατεία ΔΕΝ έκλεισε**

#### Το εύρημα

Το `pending-ratchet-work.md` κατέγραψε *«αντιγραμμένο σε **τρία** domains»*. Η μέτρηση
της 05/09 βρήκε **~30 σημεία** — το «τρία» ήταν όσα είχε **μπροστά της** εκείνη η
συνεδρία, όχι όσα υπάρχουν. *(Ίδιο σχήμα με τους μπαγιάτικους αριθμούς που ονομάζει ο
N.12: **άνοιξε και μέτρα**.)*

#### 🔴 Και τα ~30 ΔΕΝ είναι το ίδιο μοτίβο — **τρεις** ερωτήσεις, όχι μία

Το backlog έγραφε ότι *«η σωστή αφαίρεση είναι **γενική**»*. **Είναι λάθος**, και μια
«γενική» αφαίρεση που τα κατάπινε όλα θα άλλαζε συμπεριφορά **σιωπηλά**:

| # | Η ερώτηση | Η μορφή | Ανήκει στο SSoT; |
|---|---|---|---|
...
Το **#2 δεν αποκτά δική του συνάρτηση — συντίθεται**: `primaryOrFirst(list.filter(…))`. Ένα
όρισμα-κατηγόρημα θα ήταν παράμετρος που ο καλών **μπορεί να ξεχάσει**· ένα
φιλτραρισμένο όρισμα είναι **αδύνατο** να ξεχαστεί.

⛔ Το **#3 ΔΕΝ μεταναστεύει ΠΟΤΕ**. Ένα `find(isPrimary)` χωρίς fallback λέει *«αν δεν
όρισε κανείς κύριο, **δεν ξέρω**»* — και υπάρχουν σημεία που αυτό είναι το σωστό
*(`apply-project-value.ts`: η πινακίδα δεν μαντεύει διεύθυνση)*.

#### Τι άλλαξε

Νέο `lib/primary-entry.ts` *(leaf — καθαρή συνάρτηση, μηδέν εξαρτήσεις)*, με **4**
καταναλωτές: τρεις μεταναστευμένοι *(`building-update.handler` · `hierarchy-resolver` ·
`properties/[id]/hierarchy`)* **και το υπάρχον SSoT** `lib/contacts/primary-email.ts`, που
πλέον **καλεί** τον κοινό αντί να τον επαναλαμβάνει — αυτό ακριβώς κλειδώνει το ΜΕΡΟΣ Δ
της άγκυρας *(«το `primaryEmailOf` συμφωνεί με τον εξαγωγέα ειδοποιήσεων σε ΚΑΘΕ
περίπτωση»)*: δύο SSoT που απαντούν την ίδια ερώτηση είναι δύο ευκαιρίες να αποκλίνουν. Δύο αποφάσεις γραμμένες στο ίδιο το αρχείο:

- **`isPrimary === true`, ποτέ truthy** — είναι η **υπάρχουσα** αυθεντία *(`primaryEmailOf`,
  ADR-777 §8.33)*, όχι νέα αυστηρότητα: ωμά έγγραφα από παλιές φόρμες κουβαλούν
  `'false'` · `1` · `'no'`, τιμές που ένα truthy `if` θα δεχόταν ως *«αυτό είναι το κύριο»*.
- **`isPrimary?: unknown` και όχι `boolean`** — ένα `boolean` θα ανάγκαζε τους καλούντες
  σε `as`, δηλαδή θα μετέφερε το ψέμα από τη σύγκριση στον μετασχηματισμό τύπου.
  *(Το `building-update.handler.ts` το έκανε ήδη ακριβώς έτσι.)*

Άγκυρα `lib/__tests__/primary-entry.test.ts` — **16/16**, με **αντι-παράδειγμα** *(ΜΕΡΟΣ Γ:
η γραφή όπως ήταν πριν, που επέστρεφε `''` — χωρίς αυτό το ΜΕΡΟΣ Β δεν αποδεικνύει
ότι κάτι διορθώθηκε)*.

#### 🔶 Δηλωμένα ανοιχτά — **η εκστρατεία ΔΕΝ έκλεισε**

1. 🔴 **Η ΕΡΩΤΗΣΗ #1 ΕΧΕΙ **ΔΥΟ ΜΟΡΦΕΣ**, ΚΑΙ Η ΔΕΥΤΕΡΗ ΚΟΥΒΑΛΑΕΙ ΤΟ ΣΦΑΛΜΑ**
   *(μετρημένο 05/09, μετά την πρώτη καταμέτρηση)*:

   | Μορφή | Γραφή | Σημεία | Αρχεία |
   |---|---|---|---|
   | **#1α ωμή** | `find(isPrimary) ?? list[0]` | **11** | 7 |
   | **#1β προβεβλημένη** | `find(isPrimary)?.f ?? list[0]?.f` | **4** | 3 |
   | **#3 αυστηρή** *(δεν μεταναστεύει **ποτέ**)* | `find(isPrimary)` χωρίς fallback | **11** | — |

   ⇒ **μεταναστεύσιμα: 15**, όχι «~25». ⚠️ Και τα **11 αυστηρά** δεν είναι χρέος:
   μετρώνται εδώ ώστε η επόμενη σάρωση να μην τα ξαναμετρήσει ως δουλειά.

   🔴 **ΚΑΙ ΤΑ 4 ΤΗΣ #1β ΕΧΟΥΝ ΤΟ ΙΔΙΟ ΣΦΑΛΜΑ ΠΟΥ ΜΟΛΙΣ ΔΙΟΡΘΩΘΗΚΕ**
   *(`messaging-handler.ts:124` · `contact-linker.ts:216,217` ·
   `sales-accounting-helpers.ts:107`)*: η **προβολή γίνεται ΜΕΤΑ** το fallback, άρα μια
   εγγραφή `{ email: '' }` δίνει `''` — και το `?? null` **δεν το πιάνει**, γιατί το `''`
   δεν είναι nullish. Είναι **αυτούσιο** το σφάλμα που ο `hierarchy-resolver` μόλις
   έπαψε να έχει, ζωντανό σε **τρία ακόμη αρχεία**, δύο από αυτά σε **διαδρομή
   μηνυμάτων**.
   🔑 Γι' αυτό η #1β **δεν είναι στιλιστική παραλλαγή της #1α**: το `primaryOrFirst`
   επιστρέφει **εγγραφή**, οπότε το φιλτράρισμα του κενού μπαίνει **πριν** — που είναι
   ακριβώς ο κανόνας που το ίδιο το SSoT γράφει *(«ΦΙΛΤΡΑΡΕ ΠΡΙΝ»)*.

   ⚠️ **Ο ισχυρισμός «τα περισσότερα κάτω από `ai-pipeline`» ΔΕΝ επαληθεύτηκε**: το
   `ai-pipeline` έχει **59** αναφορές `isPrimary`, αλλά **ένα** μεταναστεύσιμο σημείο
   *(`messaging-handler.ts:124`)*. Ο N.10 **δεν** είναι ο φραγμός που νομίζαμε.

   ⛔ **Καμία μετανάστευση δεν είναι μηχανική**: κάθε σημείο θέλει την κρίση του πίνακα
   παραπάνω *(#1 → μεταναστεύει · #2 → συντίθεται · #3 → **μένει**)*.
2. **Καμία καταχώρηση στο `.ssot-registry.json`** ακόμη *(N.12)* — άρα **τίποτα δεν
   εμποδίζει το 8ο αντίγραφο**. ⚠️ Η καταχώρηση απαιτεί **αναγέννηση baseline**
   για τα 35 ζωντανά σημεία *(84s πλήρης σάρωση)*, που ο N.12 ονομάζει **ρητά
   «ποτέ από πράκτορα»** — θέλει απόφαση Giorgio.
...
🔶 **Μένουν ~25 σημεία** της #1 αμετανάστευτα — τα περισσότερα κάτω από `services/ai-pipeline/`,
όπου **κάθε** άγγιγμα ενεργοποιεί τον N.10 (77 σουίτες). Ίδιος λόγος με το ADR-777 §8.33.
