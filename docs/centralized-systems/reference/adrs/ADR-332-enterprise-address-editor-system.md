...
- ⛔ **Δεν καλωδιώθηκαν οι δύο οθόνες επαφών** — παραπάνω.
...
| 3 | νέα διεύθυνση → σύρσιμο → «Ακύρωση» → Αποθήκευση | ⚠️ **μισό**: η πινέζα επιστρέφει (0 μ.), το κείμενο μένει · η «Αποθήκευση» με αμφίσημο κείμενο («Σαμοθράκης 20», χωρίς πόλη) άνοιξε «Πιθανές Τοποθεσίες» (5 στην Αττική) και **δεν** αποθήκευσε — σχεδιασμένο ή σφάλμα: **ανοιχτό** | `_v` 5 — τίποτα |
...
| 7 | κτίριο χωρίς έργο | ⛔ **δεν έγινε**: και τα 7 κτίρια της εταιρείας έχουν `projectId` | — |
...
**Ανοιχτά από την επαλήθευση — διορθώνονται ΠΡΙΝ το Β-ΙΙ** *(απόφαση Giorgio)*:

| # | Ελάττωμα | Πού | Γιατί μετρά |
|---|---|---|---|
| Β9 | ο διάλογος συρσίματος δείχνει ψεύτικη «Χώρα — → Ελλάδα» | `utils/address/administrative-hierarchy.ts:221` (`hierarchyToResolvedAddress` δεν μεταφέρει `country`) · «Greece» ⇄ «Ελλάδα» = ίδια χώρα, άλλη γραφή | ο διάλογος είναι η **υπόσχεση** για το τι αλλάζει |
| Β10 | «Παλιές συντεταγμένες» + «Ανανέωση χάρτη» αμέσως μετά από αποθήκευση πινέζας ανθρώπου | `useAddressMapGeocoding.ts:138-170`: ωμή σύγκριση πεδίων (και το `trim` του διακομιστή μετρά) · αγνοεί το `source: 'dragged'` · η «Ανανέωση» δείχνει το σημείο της **μηχανής** | **δεύτερος κριτής θέσης στον πελάτη**, ακριβώς αυτό που απαγορεύει το Β-Ι. Δεν γράφει στη βάση: το `onGeocodingComplete` δεν έχει καταναλωτή |
| Β11 | το επίπεδο κάτοπτρο `address` δεν ξαναγράφεται όταν αλλάζει μόνο το `trim` | «Σαμοθράκης␣␣16» ως την επόμενη αλλαγή κειμένου | μικρό |
| Β12 | ο χάρτης επεξεργασίας δείχνει τη θέση **πριν** την αποθήκευση | `useAddressMapGeocoding.ts:288-297`: το `dragPositions` γεμίζει μόνο για **νέα** ids και `position = dragPos ?? geocoded`. Μετρημένο: τα props του `AddressMap` είχαν τη νέα θέση, η πινέζα την παλιά (ταύτιση 1e-12) | αντιφάσκει με το **Β5** (ο πελάτης υιοθετεί ό,τι έγραψε ο διακομιστής) |
| Β13 | ο διάλογος συρσίματος αργεί 24–38 δευτ. | η διαδρομή `reverse` κάνει έως **3 διαδοχικά** Overpass (`overpass-housenumber.ts`) · το `reverseGeocodeDetailed` καλεί `fetch` **χωρίς** `AbortSignal` · η κεφαλίδα της διαδρομής υπόσχεται έλεγχο «Greek bounding box» που ο κώδικας **δεν** κάνει | αν κρεμάσει ο διακομιστής, ο διάλογος **δεν ανοίγει ποτέ**, αντίθετα με το Β6 |

Μικρότερα: η πινέζα προσθήκης γράφει «Εργοτάξιο» ενώ η φόρμα λέει «Είσοδος» (`pendingPinAddress` → `type: 'site'`). Εκτός πεδίου: hydration mismatch στα Radix ids του `JobSwitch` / `ShellUtilities`.
