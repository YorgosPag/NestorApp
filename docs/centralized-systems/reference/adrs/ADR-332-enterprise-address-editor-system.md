...
Επαληθεύτηκε **στη βάση** (MCP firestore): η «ALFA ΚΑΤΑΣΚΕΥΑΣΤΙΚΗ Α.Ε.» — **πραγματικό**
δεδομένο — έχει ολότελα κενή έδρα σε **δύο** σημεία (`addresses[0]` **και**
`customFields.companyAddresses[0]`, με `country:"GR"`, `type:"work"`). **Δεν την πρόσθεσε χρήστης:
τη συνθέτει ο κώδικας και τη γράφει αυτούσια.**
...
| C | Παλαιά μολυσμένα έγγραφα | Δικλείδα στο form→domain όριο ⇒ καθαρίζουν στο **επόμενο save**, χωρίς μετάπτωση. |
...
#### Verification
**62 νέα tests** (13 session-storage + 27 blankness/δικλείδα + 6 draft promotion + 16 φίλτρα),
**286/286 σε 23 suites**. **Mutation-verified ×3**: αφαίρεση του `removeItem` → 2 πτώσεις·
αφαίρεση του prune από τον saver → 3 πτώσεις· επαναφορά του παλιού `addBranch` → 6 πτώσεις· και
τα τρία reverted. Το round-trip μέρος περνά από τον **πραγματικό** saver και ελέγχει **και τους
...
- 🔴 **Η ALFA είναι ΠΡΑΓΜΑΤΙΚΗ εγγραφή**: η κενή της έδρα θα φύγει **σιωπηλά** στο επόμενο save.
  Αναμενόμενο και επιθυμητό, αλλά καταγράφεται ρητά.
