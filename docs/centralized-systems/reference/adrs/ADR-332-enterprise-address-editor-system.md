...
| Date | Changes |
|------|---------|
| 2026-09-15 | ✅ **D28 Γ.2 — σύντομη μορφή της απάντησης του παρόχου**. Το ζωντανό περπάτημα στη βιτρίνα **επιβεβαίωσε** τη D28 και έδειξε ετικέτα **12 κομματιών** (`display_name` αυτούσιο). Νέο `formatResolvedAddressLine` στο SSoT `utils/address/address-line.ts`: «Οδός Αριθμός, στενότερη Περιοχή, Τ.Κ.» (γειτονιά πριν από πόλη, πρόθεμα βαθμίδας αφαιρεμένο, παύλα διατηρημένη, αριθμός μόνο αν τον έδωσε ο πάροχος). Καταναλωτής: `usePlaceResolver.toResolvedPlace`. Άγκυρα `resolved-address-line.test.ts`. |
| 2026-09-15 | ✅ **D28 — «Ο Τ.Κ. αποδεικνύει ό,τι αφαιρέσαμε»** (Plan Mode, Α+Β+Γ). 🔴 **Περιστατικό**: «Εντοπισμός στον χάρτη» της βιτρίνας για «Σαμοθράκης 16, 56334, Θεσσαλονίκη» ⇒ **404**, μετρημένο ζωντανά: με `city` αποτύγχαναν **όλες** οι παραλλαγές (ο Τ.Κ. ανήκει στον Δήμο Κορδελιού-Ευόσμου· ο Nominatim διαβάζει τη «Θεσσαλονίκη» ως δήμο), χωρίς πόλη λυνόταν σωστά. **Α** — νέα βαθμίδα **9** (οδός+αριθμός+Τ.Κ., χωρίς τοπωνύμιο) μετά τις 1–6 και πριν τις 7–8, με **πύλη απόδειξης**: δεκτός μόνο υποψήφιος με τον **ίδιο** Τ.Κ. (4 οδοί «Σαμοθράκης» στη Θεσσαλονίκη)· `reasoning.relaxation` λέει τι δεν ρωτήθηκε· σκάλα ως **δηλωμένη λίστα** (`geocoding-ladder.ts`) + πελάτης Nominatim σε `geocoding-nominatim-client.ts` (μηχανή 488→311 γρ.)· μνήμη `geocoding:v2`. **Β** — νέα κρίση **`broader`** («δηλώσατε την ευρύτερη περιοχή», όχι «λάθος»)· η σύγκριση πεδίων ήταν **γραμμένη δύο φορές** και είχε **αποκλίνει** (Τ.Κ. «546 24»⇄«54624»: `match` στον διακομιστή, «Ασυμφωνία» στο badge) ⇒ **μία** `lib/geocoding/field-match.ts`· ταύτιση ονομάτων με **ζεύγη πτώσεων** (`samePlaceName`), ποτέ «περιέχει» (Χαλάνδρι≠Χάλανδρα). **Γ** — κοινό `ResolvedPlaceConfirmation` (εξήχθη από `OwnerPropertyPlaceField`)· η βιτρίνα πλέον **λέει** τι βρέθηκε και με ποια χαλάρωση. Tests: νέες σουίτες `geocoding-engine-postcode-anchor` · `field-match` · `place-name-declension` · `ResolvedPlaceConfirmation`· οι 7 σουίτες της μηχανής πράσινες **χωρίς αλλαγή**. |
| 2026-09-14 | ✅ **D27 Ζ8 ΕΚΛΕΙΣΕ — «η προεπιλογή δεν είναι δήλωση του ανθρώπου»** (Plan Mode). 🔴 **Το εύρημα ήρθε ΠΡΙΝ από την εργασία που είχε προγραμματιστεί** (ξαναπέρασμα ALFA): στην **ίδια οθόνη**, η **ίδια** έδρα έδειχνε 🔺«Παλιά» στην κάρτα λίστας (`ContactListCard:43`) και 🔄«Σχετικά πρόσφατη» στην καρτέλα (`SharedAddressActionCard:107`). **Αριθμητικός αποκλεισμός, όχι εικασία**: `verifiedAt` = **1,01 ημέρας**, και η κλίμακα ηλικίας δίνει `stale` **μόνο** πάνω από 30 ημέρες ⇒ η μόνη άλλη διαδρομή είναι `positionTextVerdict === 'differs'` (`computeFreshness.ts:70`). **ΔΥΟ ανεξάρτητες ασυμμετρίες, μετρημένες** *(η δεύτερη ήταν άγνωστη — τη βρήκε ο εξαντλητικός βρόχος)*: **(α) `country`** — ο **αναγνώστης** `address-info-builder.ts:189` **επινοεί** `'GR'` (`?? CONTACT_ADDRESS_DEFAULT_COUNTRY`) ενώ ο **γραφέας** `:231` γράφει μόνο αν υπάρχει, άρα το `resolvedFor` δεν το έχει **ποτέ**· **(β) `neighborhood`** — **δύο διαφορετικές ΣΤΗΛΕΣ ΠΗΓΗΣ** της ίδιας εγγραφής: ο πίνακας ADR-772 στέλνει το L7 «Κοινότητα» σε `neighborhood` (έργα) αλλά σε `community` (`AddressInfo`), ενώ η ταχυδρομική συνοικία ζει στο `neighborhood` **μόνο** εκεί ⇒ μετρημένο «Κέντρο» vs «Δημοτική Κοινότητα Τριανδρίας». **Ορθογώνιες**: η καθεμία απαιτεί τη δική της διόρθωση. **Η αρχή**: *η ταυτότητα συγκρίνεται σε **κανονική μορφή**, και η κανονική μορφή περιλαμβάνει τη **δηλωμένη σημασία της απουσίας** — εφαρμοσμένη και στις **ΔΥΟ** πλευρές* (Unicode TR15). ⚠️ **Καμία νέα σημασιολογία**: το «απόν ≡ GR» ήταν **ήδη** δηλωμένο **δύο φορές** — `isGreekAddressCountry` (*«Η κενή χώρα μετράει ως ελληνική»*) **και** `geocoding-engine.ts:361` (`if (!params.country || cc === 'gr')`, δηλαδή **η ίδια η μηχανή**)· ο κριτής απλώς δεν το ήξερε. **Λύση**: `IDENTITY_CANONICAL` + `identityForComparison` στο `address-position-rules.ts` (μόνο ο κριτής) **+** νέο SSoT `utils/contacts/address-info-position-view.ts` (η **τρίτη** διάλεκτος αποκτά μετάφραση προς τη γλώσσα του γραφέα, μέσω του **ίδιου** `projectAddressVocabulary` — μηδέν νέα αντιστοίχιση) **+** το `ContactListCard` τη χρησιμοποιεί. 🔴 **Το `toQuery` ΔΕΝ αγγίχθηκε, και ο λόγος μετρήθηκε**: το `geocoding-engine.ts:126-146` γράφει `countrycodes` **μόνο `if (countryCode)`** ⇒ κανονικοποίηση εκεί θα άρχιζε να στέλνει `countrycodes=gr` σε **κάθε** επαφή χωρίς ρητή χώρα — **αλλαγή πραγματικής αναζήτησης**, όχι κοσμητική (και θα έσπαγε 3 `toEqual` του Ζ6). ❌ **Απορρίφθηκε «ο αναγνώστης να μην επινοεί»**: το ρεύμα των **έργων** κάνει το ίδιο (`address-helpers.ts:273,323` + Zod `min(1)`) και σήμερα είναι **συμμετρικό/σωστό** ⇒ η αφαίρεση θα δημιουργούσε **νέα** απόκλιση και θα ακύρωνε σκόπιμο test (`address-info-builder.test.ts:104-107`). ❌ **Απορρίφθηκε αλλαγή του κοινού πίνακα** (ADR-772): τον μοιράζονται έργα + φόρμα ιεραρχίας, τον φυλά η **3.44**, και είναι **σωστός για προβολή** — λάθος ήταν να περνά ωμό `AddressInfo` σε κριτή **ταυτότητας**. **Άγκυρες**: `address-mirror-identity-parity` **Ζ8-Κ0…Κ7 (19)** *(εξαντλητικός βρόχος πάνω στον **ίδιο** τον `ADDRESS_IDENTITY_FIELDS` ⇒ μελλοντικό ασύμμετρο πεδίο κοκκινίζει **μόνο του**)* + `ContactListCard.freshness` **Ζ8-Λ1/Λ2 (2)**. **Μεταλλάξεις 9/9** — αλλά **όχι με την πρώτη**: 🔬 **η Μ6 ΕΠΕΖΗΣΕ** *(«η κάρτα ξαναρωτά με ωμό `AddressInfo`»)* και αποκάλυψε ότι η άγκυρα καλούσε **η ίδια** τον προσαρμογέα ⇒ φύλαγε τον **προσαρμογέα**, όχι τον **καλούντα** — *«anchor χωρίς κάλυψη του σημείου κλήσης είναι σχόλιο»* (ADR-587 §6.1)· γεννήθηκε η Ζ8-Λ1 και η Μ6 **πέθανε**. 🔬 **Δεύτερο δικό μου σφάλμα, πιασμένο από μέτρηση**: το σκέλος «country» του εξαντλητικού βρόχου έτρεχε **ΚΕΝΟ** (0 ms — το δείγμα δεν δήλωνε χώρα, το `if (!proof[field]) return` το έκανε να **σιωπά**) ⇒ fixture με **και τα εννέα** πεδία + **φρουρός Ζ8-Κ0** που κάνει τη σιωπή **αδύνατη** να επιστρέψει. ⚪ **Ειλικρίνεια**: η **Μ8β** *(«πάντα differs», χωρίς ισχυρισμούς εδάφους)* **πέθανε** ενώ προέβλεψα ότι θα επιζούσε — η υπόθεσή μου ήταν στενή: τα Κ4/Κ6 έχουν **δικούς τους** απόλυτους ισχυρισμούς. Η προστασία είναι **πλεοναστική**, δηλαδή **ισχυρότερη** απ' όσο ισχυρίστηκα. **297 tests πράσινα** στις γειτονικές σουίτες, καμία παλινδρόμηση. Μεταλλάξεις σε **απομονωμένο `git worktree`** (§9· jest ⇒ **χωρίς** dev server, άρα καμία από τις δύο παγίδες Turbopack/`.env.local`), επαναφορά **επαληθευμένη**, κοινά `node_modules` **ζωντανά**. ✅ **ΖΩΝΤΑΝΗ ΕΠΑΛΗΘΕΥΣΗ (Chrome, μετά από ξανα-μεταγλώττιση 182″ — «πράσινο αμέσως μετά από edit δεν είναι απόδειξη»)**: **καμία** «Παλιά» πουθενά· η ALFA λέει «Σχετικά πρόσφατη» **και στις δύο** επιφάνειες — και αποκαλύφθηκε **δεύτερο θύμα** που κανείς δεν ήξερε, η **STAVROULA PAGONI** ⇒ ο πληθυσμός ήταν **2 στις 11** επαφές, **μεγαλώνοντας με κάθε αποθήκευση**. 🏆 **Ζ9 ΔΗΛΩΜΕΝΟ (όχι τώρα)**: **προέλευση ανά πεδίο** — Google `hasInferredComponents`/`inferred`, HERE `fieldScore`, Mapbox `match_code` το κάνουν **όλοι**, ακριβώς για να μη μολύνουν οι προεπιλογές τη σύγκριση· πελάτες του: το `partialMatch` και το εύρημα «το `regionalUnit` **δεν φτάνει ποτέ** στον πάροχο». Πύλες: 3.28 · 3.47 · 3.53 · 3.54 · 3.70 ✅. ΟΧΙ tsc (N.17). |
| 2026-09-12 | ✅ **D27 Ζ2 + Ζ4β + Ζ4γ ΕΚΛΕΙΣΑΝ — Φάση Β′ «η ετικέτα δηλώνει το επίπεδό της»** (Plan Mode). 🔴 **Η προκείμενη ανατράπηκε από μέτρηση**: το handoff έλεγε «λείπει κλήση, όχι μηχανή» — η μηχανή ήταν **χαλασμένη**, **3.067/9.294 (33,0%)** ονόματα οικισμών έδεναν σε **ΑΛΛΟΝ** οικισμό *(σάρωση `.find()` σε σειρά αρχείου, η ακριβής αντιστοίχιση χωρίς προτεραιότητα· «Καλλιθέα»/«Καλλίστη» έχουν **τον ίδιο** Τ.Κ. 69100)*. Η κλήση θα **διέδιδε** το σφάλμα σε 3 ακόμη διαδρομές. 🏆 **Το εύρημα**: το Nominatim στέλνει **ολόκληρη** τη διοικητική αλυσίδα με τα **προθέματά** της και ο route **δεν διάβαζε** τα κλειδιά `municipality`/`county` — μετρημένο ζωντανά σε **14 πόλεις**: δήμος **14/14 ΑΚΡΙΒΩΣ**, περιφέρεια **14/14**, **και στους αστικούς πυρήνες όπου η γεωμετρία σιωπά**. **Το πρόθεμα δηλώνει τη βαθμίδα** *(το μητρώο κρατά L2–L7 μαζί με το πρόθεμα)*, και ο `stripGreekAdminPrefix` το **πετούσε**. Νέος πυρήνας `lib/places/admin-identity` + `admin-name-index` *(καθαρός, εγχυόμενοι αναγνώστες, τρέχει **και** στον διακομιστή)*: ευρετήριο με **κλειδί το όνομα** ⇒ λάθος δέσιμο **δομικά αδύνατο** (84,3% μονοσήμαντα)· νικητής = **«αυτός που εξηγεί όλους τους άλλους»** *(«Δήμος Ηρακλείου» = **2** — Κρήτη **και** Αττική — λύνεται από το `state`)*· διφορούμενο ⇒ **καμία επιλογή**· **`unknown` ≠ `absent`**. Ανοχή **ΜΟΝΟ μέσα σε εμβέλεια** *(χωρίς αυτήν: «Χαλάνδρι»→**ΔΗΜΟΣ ΧΙΟΥ**, «Ιλίσια»→**Κρήτη**· 9/9 συνοικίες Αθήνας/Θεσσαλονίκης **δεν υπάρχουν** στο μητρώο ⇒ η μοναδικότητα είναι **παραπλανητική**)*. **Και τα ΤΕΣΣΕΡΑ** σημεία γραφής *(+έργα +κτίρια)* παύουν να μηδενίζουν άνευ όρων· η ταυτότητα ταξιδεύει από **ένα** σύνορο. Ζ4β: **ένας ιδιοκτήτης ανά πεδίο** (βρέθηκε **δεύτερος** διεκδικητής του `region`). 🔬 **Τρία δικά μου σφάλματα, πιασμένα από εργαλείο**: άγκυρα με **αδύνατη είσοδο** · η παγίδα **τελικού σίγμα** *(30,6% του μητρώου· 14/14 σιωπηλά μηδενικά στο εργαλείο μέτρησής μου)* · **δίδυμο** από τον N.18 ⇒ **μία** μηχανή. ⚠️ **Διόρθωση §4**: τα αποτυπώματα είναι **δίσκοι**, μόνο το `interior` αποδεικνύει· L7 **0/6.062** ✅ σωστό, αλλά **L5 266/333 (79,9%)** — **7/10** πραγματικά σημεία αποδεικνύουν δήμο, **ποτέ** λάθος· η γεωμετρία μένει **επιβεβαιωτής** *(`containingEntityOfPoint` υπάρχει ήδη· **μηδέν** εγγραφές `settlement:`)*. 📐 **Δομικό όριο ρητά**: το `companyAddress` κρατά **ΜΟΝΟ ΔΥΟ** ταυτότητες, τα άλλα έξι `NOT_STORED`. **70 σουίτες / 790 πράσινα · 5/5 μεταλλάξεις** — και μία **6η που ΕΠΕΖΗΣΕ** *(ο κανόνας του `neighbourhood` ήταν **σχόλιο, όχι συμβόλαιο**)* ⇒ άγκυρα με το μετρημένο σχήμα «Λαδάδικα». Πύλες 3.28 · 3.44 · 3.70 ✅. ΟΧΙ tsc (N.17). ✅ **ΖΩΝΤΑΝΗ ΕΠΑΛΗΘΕΥΣΗ (Chrome + Firestore)**: σύρσιμο υποκαταστήματος ALFA ⇒ ο διάλογος έδειξε «Πόλη: Νέα Μαγνησία → **Σταυρούπολις**» (αρχαΐζουσα μορφή **μητρώου**) + «Συνοικία → Ομόνοια», και το Firestore κρατά **`municipalityId: municipality:0711`** (ΔΗΜΟΣ ΠΑΥΛΟΥ ΜΕΛΑ — **ξανα-παράχθηκε** από 0704 ΔΕΛΤΑ) · **`settlementId: settlement:0711010101`** · `regionName` **κεφαλαία** (μητρώο) δίπλα σε `region` **πεζά** (μηχανή) ⇒ Ζ4β. 🔬 **Και η ζωντανή μέτρηση βρήκε κενό που κανένα test δεν ρωτούσε** (όπως στη Φάση Α): τα **ενδιάμεσα** L6/L7 γράφονταν **κενά** ενώ είναι πρόγονοι **αποδεδειγμένου** οικισμού ⇒ διορθώθηκε (`provedLevelsOfPlace`) + άγκυρα, και επαληθεύτηκε **πλήρης αλυσίδα 8/8**. **77 σουίτες / 858 πράσινα**. ⚠️ Η έδρα **δεν** διορθώνεται αναδρομικά (κανείς δεν έσυρε την πινέζα της) — σωστό. ⚠️ Το σύρσιμο ενεργοποιείται **μόνο σε επεξεργασία**. |
...
- ⚠️ **Εύρημα Ζ6 — μετά από αποτυχία του γραφέα η θέση ισχυρίζεται «geocoded / exact» για ΑΛΛΟ κείμενο.** Στο Λ7 η έδρα λέει «Εγνατία **102**» αλλά η θέση είναι της «Εγνατία **100**» με `source:'geocoded'`, `accuracy:'exact'` — κανένα σημάδι «δεν λύθηκε για το τρέχον κείμενο». Ξαναλύνεται **μόνο** στην επόμενη αλλαγή διεύθυνσης (dirty diff, ADR-323)· χωρίς αυτήν μένει έτσι. «Άγνοια ≠ γνώση» κρατιέται για το **σβήσιμο**, όχι για την **ετικέτα**.
  ...
  ✅ **Ζ6 ΕΚΛΕΙΣΕ — «ο ισχυρισμός ακρίβειας γίνεται ΕΛΕΓΞΙΜΟΣ» (2026-09-12, Plan Mode).**

  **Η αρχή**: *η θέση κουβαλά **το κείμενο για το οποίο λύθηκε**.* Τότε η ερώτηση «ισχύει;» απαντιέται
  από τα **ίδια τα αποθηκευμένα δεδομένα** — χωρίς δίκτυο και **χωρίς δεύτερο κριτή**.

  🔬 **Μετρημένο ζωντανά πριν από κάθε γραμμή** (Firestore + πάροχος, ALFA `addr_30739b3e…`):

  | | Αποθηκευμένο | Πάροχος **τότε** |
  |---|---|---|
  | Κείμενο | Εγνατία **102** · Τ.Κ. **54002** | — |
  | Σημείο `40.6345573 / 22.9461936` | `exact` · `0.91` · `osmType: way` | **αντίστροφη ⇒ Εγνατία 100, Τ.Κ. 546 23** |
  | «Εγνατία 102» **εμπρός** | — | `40.6345089 / 22.9464481` — **~22 m** — `partialMatch: **true**`, `postalCode: **mismatch**` |

  Το `exact` ήταν **αληθές για την 100**. Η αντίστροφη το είπε ρητά — δεν συνάχθηκε από τον Τ.Κ.

  🔑 **Η κλάση ήταν ΔΥΟ διαδρομές προς την ίδια ψευδή πρόταση**, όχι το δείγμα:
  - **(Α) χρονική** — το `keepStored` κρατά τα **παλιά** μεταδεδομένα δίπλα στο **νέο** κείμενο σε
    `budget-exhausted`/`geocoder-unavailable`. Η **πράξη** είναι σωστή (άγνοια δεν σβήνει θέση)·
    η **σιωπή** δεν ήταν.
  - **(Β) στιγμιαία** — ο πάροχος δήλωνε **ήδη** `partialMatch`, και το `GeocodeHit` δεν το κουβαλούσε
    ⇒ **χανόταν στη μεταφορά**: κατά λέξη το σχήμα του ελαττώματος που γέννησε το `address-position.ts`.

  **Η λύση**: `geocodingMetadata.resolvedFor` = **το ίδιο το ερώτημα** (`toQuery`), και
  `positionTextVerdict()` που ρωτά τον **υπάρχοντα** `addressIdentityChanged` — μηδέν νέος τύπος,
  μηδέν νέα σύγκριση, **σεβασμός του Β10** (καμία δεύτερη κρίση στον πελάτη). Τέσσερις ετυμηγορίες:
  `matches` · `differs` · **`unverifiable`** *(ισχυρισμός χωρίς απόδειξη — κάθε εγγραφή πριν το Ζ6·
  **άγνοια, όχι κατηγορία**, ίδιο ιδίωμα με το `unknown` του `ProvedAdminLevel`)* · `not-geocoded`.

  🔴 **Το όργανο υπήρχε και ήταν ΔΟΜΙΚΑ ΝΕΚΡΟ.** Το `AddressFreshness.staleReason` δήλωνε **τρεις**
  λόγους και ο δείκτης είχε μεταφράσεις **και για τους τρεις** (el+en) — σε παραγωγή παραγόταν
  **μόνο** ο `time-elapsed`. Το `field-changed` ζούσε ως fixture της demo και ως **εφήμερη** φάση του
  editor. Πλέον το `computeFreshness` δέχεται **τη διεύθυνση** (όχι σκέτο `verifiedAt`), ώστε η ερώτηση
  να μην *μπορεί* να ξεχαστεί, και «λύθηκε για άλλο κείμενο» **υπερισχύει της ηλικίας**.

  🔴 **Δεύτερο κενό ορατότητας, μετρημένο**: στις **επαφές** η `SharedAddressActionCard` καλούνταν
  **χωρίς** `source`/`verifiedAt`/`hasCoordinates` ⇒ τα τρία badges **δεν ήταν καν στη σελίδα** εκεί
  όπου ζούσε το δείγμα (μόνο η καρτέλα Τοποθεσιών τα περνούσε). Τα τρία props — **κομμάτια του ίδιου
  πράγματος** — ενοποιήθηκαν σε ένα `position`, οπότε η «μισή» παράδοση έπαψε να είναι εκφράσιμη.
  Η φωνή στις επαφές μπήκε ως **τρίτη κατάσταση** του υπάρχοντος υποσελίδου (`useContactDriftFooter`),
  με **μία** ενέργεια — την **υπάρχουσα** `relocateAddressPin` (`intent.relocate`): ένα «Κράτα» εδώ θα
  ήταν τοπική απόκρυψη πρότασης που παραμένει ψευδής στη βάση. *(Και γι' αυτό **δεν** είναι δίδυμο του
  `AddressPositionDriftNotice` — επιβεβαιωμένο με `jscpd:diff`, CHECK 3.28 καθαρή.)*

  **Απόδειξη**: άγκυρες Ζ6-Α1…Α4β · Β1/Β1β · **Β2** *(άκρη-σε-άκρη μέσα από τον **πραγματικό**
  μετατροπέα `geocodeAddress`)* · Σ1/Σ1β/Σ2 · Ο1…Ο1δ · Ο2 · Ο3…Ο3γ **+ Ο3α…Ο3α4**.
  **Μεταλλάξεις 17/17.** Πύλες **3.28 ✅ 3.44 ✅ 3.53 ✅ 3.70 ✅ 3.71 ✅**. ΟΧΙ tsc (N.17).

  🔬 **ΖΩΝΤΑΝΗ ΕΠΑΛΗΘΕΥΣΗ (Chrome + Firestore, ALFA, 2026-09-13)** — και **βρήκε ελάττωμα που
  κανένα από τα 699 πράσινα δεν έβλεπε**:

  | Βήμα | Αποτέλεσμα |
  |---|---|
  | Παλιά εγγραφή (χωρίς απόδειξη) | ✅ «Σχετικά πρόσφατη», **καμία** ειδοποίηση — `unverifiable` σιωπά |
  | Αποθήκευση με αλλαγμένο αριθμό | ✅ `resolvedFor` **7 πεδία** γραμμένο σε **αμφότερα** τα δοχεία (`addresses[]` **και** `customFields.companyAddresses[]`), `verifiedAt` φρέσκο |
  | Μερική αντιστοίχιση | ✅ `partialMatch: true` **αποθηκευμένο** — ο Τ.Κ. «54002» όντως δεν ταιριάζει (σωστός 546 23)· ο editor το έδειχνε ζωντανά και **χανόταν** |
  | Κείμενο ≠ απόδειξη | ✅ badge **«Παλιά»** + «Η θέση στον χάρτη είχε υπολογιστεί για διαφορετική διεύθυνση…» + **«Υπολογισμός θέσης από τη διεύθυνση»** · οι άλλες δύο κάρτες **σιωπούν** |
  | «Υπολογισμός θέσης» | ✅ **μία** εγγραφή· `resolvedFor` ίσο με το τρέχον κείμενο· νέα θέση· φρέσκο `verifiedAt` |

  🔴 **ΤΟ ΕΥΡΗΜΑ ΤΗΣ ΟΘΟΝΗΣ — ΔΥΟ ΛΕΞΙΛΟΓΙΑ.** Με `resolvedFor.number` **και** `number` ίσα με
  «104», η οθόνη **επέμενε** «Παλιά». Αιτία: η `CompanyAddress` λέει
  **`municipalityName`/`regionalUnitName`**, ενώ ο γραφέας — και άρα το `resolvedFor` που
  **αυτός** γράφει — λέει **`municipality`/`regionalUnit`**. Κρίνοντας την **ωμή** εγγραφή, τα
  διοικητικά πεδία διαβάζονταν **κενά** ⇒ `differs` σε **κάθε** διεύθυνση επαφής, για πάντα:
  φρουρός με σωστό κριτήριο που καταγγέλλει τα πάντα. Η μετάφραση **υπήρχε ήδη**
  (`contactAddressPositionView`) και η κεφαλίδα της το είχε **γράψει**: *«ΜΙΑ συνάρτηση, δύο
  πλευρές… δύο αντιγραφές θα διαφωνούσαν»*. Το Ζ6 έγινε **τρίτη πλευρά που δεν ρώτησε**.
  Θεραπεία: κλήση της **υπάρχουσας** όψης στα **τρία** σημεία (υποσέλιδο · κάρτα έδρας ·
  κάρτα υποκαταστήματος) — καμία τέταρτη μετάφραση ονομάτων.

  ⚠️ **ΤΡΙΑ ΣΧΗΜΑΤΑ ΨΕΥΔΟΠΡΑΣΙΝΟΥ, ΜΕΤΡΗΜΕΝΑ ΣΤΟΝ ΙΔΙΟ ΓΥΡΟ** *(οι μεταλλάξεις τα ξεσκέπασαν·
  καμία δεν πέθανε με την πρώτη)*:
  1. **Το όργανο mocked** — `AddressFreshnessIndicator: () => null` στο placement suite ⇒ η
     άγκυρα φύλαγε το υποσέλιδο και ήταν **τυφλή στο badge**, τη μισή διαδρομή.
  2. **Ο λόγος μη διακρίσιμος** — το κοινό fixture είχε `verifiedAt: 7` (1970) ⇒ το badge έλεγε
     «stale» **λόγω ηλικίας**· ο έλεγχος ήταν πράσινος **ανεξάρτητα** από το Ζ6.
  3. **Η μισή εφαρμογή αόρατη** — όλες οι άγκυρες έβαζαν το δείγμα σε **υποκατάστημα**
     (`CompanyAddressesSection`), ενώ η **έδρα** περνά από `AddressesSectionWithFullscreen`:
     **δύο** σημεία κλήσης της ίδιας κάρτας, και η μετάλλαξη της έδρας **επέζησε δύο φορές**.
  4. *(συνέπεια του 3)* **Το αρνητικό δείγμα δεν ξεχωρίζει τίποτα** — ένα *drifted* fixture
     βγάζει `differs` **και** με τις δύο εκδοχές· μόνο το **ευθυγραμμισμένο** διακρίνει
     «ρωτά την όψη» από «ρωτά την ωμή».

  🔑 **Η γενίκευση**: *ένας έλεγχος πρέπει να μπορεί να **κοκκινίσει για τον δηλωμένο του λόγο**
  — και σε **κάθε** διαδρομή όπου ο λόγος υπάρχει.* Αλλιώς είναι σχόλιο, όχι άγκυρα.

  🔴 **ΠΕΡΙΣΤΑΤΙΚΟ ΚΟΙΝΟΥ ΔΕΝΤΡΟΥ — mutation testing ΔΕΝ ανήκει εκεί.** Το commit `18ba6ecb`
  απαθανάτισε **μετάλλαξη** (`staleReason: 'time-elapsed'` αντί `'field-changed'`): ένας peer
  σταδιοποιούσε ακριβώς μέσα στο παράθυρο των ~40″ που το script κρατά το αρχείο αλλαγμένο.
  **Και πέρασε ολόκληρη την πύλη πράσινη** — καμία από τις 53 δεν τρέχει την Ζ6-Ο1. Δηλαδή
  υπήρξε κατάσταση του δέντρου που **κανένας φρουρός δεν μπορούσε να διακρίνει από τη σωστή**.
  Διορθώθηκε με `70e42041`. Σε δεύτερο γύρο η **επαναφορά απέτυχε** (`Permission denied` —
  άγνωστος κάτοχος: editor, file watcher ή antivirus) και το αρχείο έμεινε μεταλλαγμένο ώσπου
  το έπιασε `diff`. ⇒ **Κανόνας: μεταλλάξεις σε `git worktree`, ποτέ στο κοινό δέντρο**· και ο
  σταδιοποιών επαληθεύει **υπογραφή αρχείου**, όχι diff, πριν από κάθε `git add`.

  ⚠️ **ΔΥΟ άγκυρες επέζησαν της πρώτης μετάλλαξης, και οι δύο επειδή μετρούσαν ΑΛΛΟ απ' ό,τι νόμιζαν:**
  (α) η **Α4** έβαζε αποθηκευμένο κείμενο **που συμφωνούσε** με την απόδειξή του, άρα ένα
  `resolvedFor: toQuery(stored)` περνούσε αθόρυβα — χρειάστηκε η **Α4β** (δεύτερη σιωπή της μηχανής
  πάνω σε **ήδη αποκλίνουσα** εγγραφή), αλλιώς η ψευδής πρόταση θα γινόταν **αυτο-επικυρωμένη**·
  (β) η **Σ2** περνούσε από το route **νέας** επαφής, όπου `stored === null` ⇒ ο γραφέας
  **γεωκωδικοποιεί** και παράγει νέο `resolvedFor` που τύχαινε να ισούται με το σταλμένο ⇒ πράσινη
  ακόμη και με το πεδίο σβησμένο από το σχήμα. Ξαναγράφτηκε να χτυπά **το σχήμα**.
  *(Τρίτη φορά στο D27 το ίδιο μάθημα: «έλεγξε ότι η άγκυρά σου μετρά ΑΥΤΟ που νομίζεις».)*

  🔎 **Παράπλευρο εύρημα, ΑΝΟΙΧΤΟ**: το `toQuery` στέλνει `regionalUnit`/`municipality`, αλλά το
  `GeocodingRequestBody` της μηχανής ξέρει `county`/`municipality` ⇒ το **`regionalUnit` δεν φτάνει
  ποτέ στον πάροχο** (`grep regionalUnit src/app/api/geocoding/` → **0**). Άλλη κλάση (λεξιλόγιο
  ερωτήματος), χωριστό βήμα.
...
- ⚠️ **Εύρημα Ζ4 (λεξιλόγιο)**: μετά από «Ναι, ενημέρωσε» το υποκατάστημα έχει `country:'Ελλάδα'` ενώ όλες οι άλλες `'GR'`, και `region:'Περιφέρεια Κεντρικής Μακεδονίας'` (πεζά, της μηχανής) **δίπλα** σε `regionName:''` — `components/contacts/dynamic/contact-address-drag.ts:49-50` περνά **ωμά** `dragged.region` / `dragged.country`. Επίσης η ιεραρχία του υποκαταστήματος μένει **κενή** (καθαρίστηκε, δεν ξαναλύθηκε — δηλωμένο ως «καθαρή» στο Β-ΙΙ, αλλά το υποκατάστημα χάνει δήμο).
  ...
  **Άγκυρες** (`__tests__/contact-address-drag.test.ts`, **κόκκινες πριν**): «Ελλάδα» από τη μηχανή ⇒ `'GR'` · **άγνωστη χώρα δεν χάνεται** · **Ζ4δ** «η αυθεντική εγγραφή και το παράγωγο συμφωνούν στη χώρα» (πριν: `undefined` vs `'GR'` — δύο αποφάσεις για την κενή χώρα σε δύο αρχεία, μέσα στο ίδιο έγγραφο).
  ...
  ✅ **Ζ2 + Ζ4β + Ζ4γ ΕΚΛΕΙΣΑΝ — Φάση Β′ «η ετικέτα δηλώνει το επίπεδό της» (2026-09-12, Plan Mode).**

  🔴 **Η ΠΡΟΚΕΙΜΕΝΗ ΤΟΥ HANDOFF ΑΝΑΤΡΑΠΗΚΕ ΑΠΟ ΜΕΤΡΗΣΗ.** Το σχέδιο έλεγε *«δεν λείπει μηχανή,
  λείπει κλήση: η έδρα ταυτοποιεί, το υποκατάστημα όχι»*. Η μηχανή που θα καλούσαμε ήταν
  **χαλασμένη**: το `makeNameMatcher` + `settlements.find(...)` σάρωνε σε **σειρά αρχείου** και η
  **ακριβής** αντιστοίχιση **δεν είχε προτεραιότητα** ⇒ **3.067 από 9.294** ονόματα οικισμών
  *(**33,0%**)* έδεναν σε **ΑΛΛΟΝ** οικισμό. Η κλήση θα είχε **διαδώσει** το σφάλμα σε τρεις
  ακόμη διαδρομές. Το δείγμα: «Καλλιθέα» και «Καλλίστη» έχουν **τον ίδιο** Τ.Κ. 69100 και η
  δεύτερη είναι **131 θέσεις πιο πάνω** ⇒ διεύθυνση στην **Καλλιθέα Ροδόπης** αποθηκευόταν με
  δήμο **ΚΟΜΟΤΗΝΗΣ** αντί **ΜΑΡΩΝΕΙΑΣ - ΣΑΠΩΝ**, και το όνομα στην οθόνη γινόταν σιωπηλά
  «Καλλίστη». **Λάθος ταυτότητα είναι χειρότερη από καμία** — η κενή ιεραρχία του Ζ4γ ήταν το
  *λιγότερο* κακό.

  🏆 **ΤΟ ΕΥΡΗΜΑ ΠΟΥ ΑΛΛΑΞΕ ΤΗ ΛΥΣΗ: το Nominatim στέλνει ΟΛΟΚΛΗΡΗ τη διοικητική αλυσίδα, με
  τα προθέματά της, και ο route ΔΕΝ ΔΙΑΒΑΖΕ ΤΑ ΚΛΕΙΔΙΑ.** Το `NominatimReverseAddress` δήλωνε
  **δέκα** πεδία, χωρίς `municipality` και `county`. Μετρημένο ζωντανά σε **14** ελληνικές
  πόλεις: `state` «Περιφέρεια …» **14/14 ακριβώς** · `municipality` «**Δήμος** …» **14/14
  ακριβώς** · `county` 12/14 *(2 λένε «Μητροπολιτική», η ΕΛΣΤΑΤ «Περιφερειακή» ⇒ σωστά «απόν»)*.
  Ο **δήμος** — η μία από τις **δύο** ταυτότητες που κρατά το `companyAddress` — ήταν **πάντα
  στην απάντηση** και **ποτέ** στη βάση. **Και βγαίνει ΚΑΙ στους αστικούς πυρήνες**, όπου η
  γεωμετρία σιωπά *(Σύνταγμα · κέντρο Ηρακλείου · Νέα Μαγνησία: κανένα εσωτερικό κάλυμμα)*.

  🔑 **Η αρχή**: το **πρόθεμα δηλώνει τη βαθμίδα** *(το μητρώο κρατά τα επίπεδα 2–7 **μαζί** με
  το πρόθεμά τους: `L5 «ΔΗΜΟΣ ΘΕΣΣΑΛΟΝΙΚΗΣ»` · `L6 «ΔΗΜΟΤΙΚΗ ΕΝΟΤΗΤΑ …»`)*, και ο
  `stripGreekAdminPrefix` **το πετούσε**, σπρώχνοντας κάθε ετικέτα στο επίπεδο 8 όπου αυτή η
  μορφή **δεν υπάρχει καν**. ⚠️ **Εμπιστεύσου το πρόθεμα, ΟΧΙ το όνομα του κλειδιού** —
  μετρημένο στον **Πύργο**: `municipality: null`, `city: «Δήμος Πύργου»`.

  **Νικητής = *«αυτός που εξηγεί όλους τους άλλους»***, ο **ίδιος** κανόνας με το
  `deepestContainingEntity` της γεωμετρίας: «Δήμος Ηρακλείου» δίνει **ΔΥΟ** εγγραφές *(Κρήτη
  **και** Ηράκλειο Αττικής)* και τις ξεχωρίζει το `state` της **ίδιας** απάντησης. Διφορούμενο
  ⇒ **`ambiguous`, καμία επιλογή**. Και **`unknown` ≠ `absent`**: «δεν διαβάστηκε η ιεραρχία»
  δεν δικαιολογεί σβήσιμο.

  **Ο πυρήνας** *(καθαρός, εγχυόμενοι αναγνώστες κατά το πρότυπο `CoverageResolvers`, τρέχει
  **και** στον διακομιστή)*: `lib/places/admin-identity.ts` + `lib/places/admin-name-index.ts` —
  ευρετήριο με **κλειδί το όνομα**, που κάνει το λάθος δέσιμο **δομικά αδύνατο** *(84,3% των
  ονομάτων λύνονται μονοσήμαντα)*. Βαθμίδες: **ακριβές** ▸ **Τ.Κ. ανάμεσα σε ακριβώς ομώνυμους**
  ▸ **πτώση** ▸ **τυπογραφικό** — οι δύο τελευταίες **ΜΟΝΟ μέσα σε αποδεδειγμένη εμβέλεια**,
  όπου η εγγύηση είναι **δομική** *(η εμβέλεια **είναι** πρόγονος ⇒ καμία ανοχή δεν μπορεί να
  διαφθείρει ρηχότερη ταυτότητα)*.

  🔴 **Γιατί η εμβέλεια είναι υποχρεωτική, μετρημένο**: ανοχή απόστασης-1 **χωρίς** εμβέλεια
  δίνει «Χαλάνδρι» → «Χάλανδρα» **ΔΗΜΟΣ ΧΙΟΥ** · «Παγκράτι» → **ΔΗΜΟΣ ΣΟΥΛΙΟΥ** · «Ιλίσια» →
  **ΔΗΜΟΣ ΑΡΧΑΝΩΝ** (Κρήτη). **Η μοναδικότητα ΔΕΝ είναι ορθότητα**: 9/9 συνοικίες
  Αθήνας/Θεσσαλονίκης *(Κολωνάκι · Εξάρχεια · Λαδάδικα …)* **δεν υπάρχουν** στο μητρώο, οπότε
  δεν υπάρχει σωστή απάντηση για να ανταγωνιστεί τη μία λάθος. Μέσα σε δήμο: **0,4%** αμφισημία
  αντί 18–27%, και **93,9%** των 1.368 ομάδων ομωνύμων λύνονται *(«Καλλιθέα» ×41 → μία)*.

  **Και τα ΤΕΣΣΕΡΑ σημεία γραφής** *(όχι δύο)* παύουν να μηδενίζουν άνευ όρων: επαφή έδρα ·
  επαφή υποκατάστημα · **έργα** (`applyDraggedPin`) · **κτίρια** (`BuildingAddressesEditor`,
  που μηδένιζε **ολόκληρο** το σετ ΕΛΣΤΑΤ **και έχανε τη χώρα**). Η ταυτότητα ταξιδεύει από **το
  ένα σύνορο** (`reverseResultToAddress`) μέσα στο `DraggedAddressText`, άρα **δεν** χρειάστηκε
  μηχανή σε κανένα σημείο γραφής — και οι δύο διάλογοι συρσίματος διορθώθηκαν **δωρεάν**
  *(ίδιο `toProposal` = ίδια συνάρτηση γραφής)*.

  **Ζ4β**: ο κανόνας «**ένας ιδιοκτήτης ανά πεδίο**» — `regionName` = **αποδεδειγμένο** όνομα
  *(κανονικό, νικά στην ανάγνωση)*, `region` = **ελεύθερο** κείμενο, γραμμένο **πάντα** ώστε να
  μη μένει μπαγιάτικο. 🔴 Βρέθηκε **δεύτερος διεκδικητής**: το `hierarchyToHqBranch` έγραφε
  `region: addr.regionName` **έξω** από τον πίνακα, οπότε κανείς δεν ξεχώριζε «όνομα ιεραρχίας»
  από «ετικέτα μηχανής» — και το `region` ήταν **αθάνατο** *(ο πίνακας γράφει μόνο `slot[0]`)*.

  **Ζ2**: η «Πόλη» ακολουθεί την **ταυτότητα** *(όνομα μητρώου όταν αποδεικνύεται οικισμός)*, και
  το `neighbourhood` **έφυγε από την αλυσίδα της πόλης** — ήταν **δύο** ασυντόνιστοι ιδιοκτήτες
  *(route **και** `contact-pin-drop`)*, και γι' αυτό «Λαδάδικα» γινόταν Πόλη.

  🔬 **ΤΡΙΑ ΔΙΚΑ ΜΟΥ ΣΦΑΛΜΑΤΑ, ΠΙΑΣΜΕΝΑ ΑΠΟ ΕΡΓΑΛΕΙΟ — γραμμένα επίτηδες:**
  1. **Άγκυρα με αδύνατη είσοδο**: έγραψα κριτήριο που τάιζε «Δημοτική Ενότητα Θεσαλονίκης» στον
     hook του **πελάτη** — αλλά το `cleanPlaceName` του route **σβήνει το πρόθεμα**, άρα ο
     πελάτης **δεν το βλέπει ποτέ**. Ίδιο σχήμα με την «ταυτολογία» του §4 του handoff.
  2. **Η παγίδα του τελικού σίγμα**: το `toLowerCase()` δίνει `ς` *(U+03C2)*, το `nn` του αρχείου
     γράφει `σ` σε **6.187** ονόματα *(**30,6%** αποκλίνουν)*. Στο εργαλείο μέτρησής μου ο
     πίνακας προθεμάτων είχε `ς` και τα δεδομένα `σ` ⇒ **14/14 «αδήλωτη βαθμίδα»**, σιωπηλό μηδέν.
     **Το δίπλωμα εφαρμόζεται ΚΑΙ στους πίνακες σταθερών.**
  3. **Δίδυμο, πιασμένο από τον N.18 (CHECK 3.28)**: η νέα `overwriteAdminHierarchy` αντέγραψε τον
     βρόχο των οκτώ επιπέδων του `projectAddressVocabulary` *(7 γρ./51 tokens)* ⇒ εξήχθη **μία**
     μηχανή με τη μεταχείριση της έλλειψης ως παράμετρο. Το ίδιο εργαλείο αποκάλυψε ότι ο route
     δήλωνε **δικό του** interface απάντησης, ταυτόσημο με του πελάτη — **χρέος που προϋπήρχε**.

  ⚠️ **ΔΙΟΡΘΩΣΗ ΤΟΥ §4 ΤΟΥ HANDOFF ΚΑΙ ΤΗΣ ΔΙΚΗΣ ΜΟΥ ΜΕΤΡΗΣΗΣ**: τα αποτυπώματα **δεν** είναι
  πολύγωνα — είναι `center`/`outerKm`/`innerKm` **+ προαιρετικό `interior` = κάλυμμα πολλών
  δίσκων**, και **μόνο το `interior` αποδεικνύει**. ✅ Το §4 έχει δίκιο για το **L7** *(0/6.062)*·
  🔴 δεν μέτρησε το **L5**, όπου `interior` υπάρχει σε **266/333 (79,9%)**. Η πρώτη μου μέτρηση
  («95,2%») μετρούσε `innerKm` — **λάθος πεδίο, ίδιο σχήμα σφάλματος**. Μη-ταυτολογική δοκιμή σε
  **10 πραγματικά** σημεία: **7/10** αποδεικνύουν δήμο *(5 στο L5, +2 μέσω L6)*, **ποτέ** λάθος.
  ⇒ Η γεωμετρία μένει **επιβεβαιωτής**: `containingEntityOfPoint` **υπάρχει ήδη σε παραγωγή**
  και **δεν** μπορεί ποτέ να αποδείξει οικισμό *(μηδέν εγγραφές `settlement:`)* — οι δύο
  αποδείκτες αντιστοιχούν **ακριβώς** στις δύο αποθηκευμένες ταυτότητες. Δεν είναι σύμπτωση.

  📐 **ΔΟΜΙΚΟ ΟΡΙΟ, ΓΡΑΜΜΕΝΟ ΡΗΤΑ** *(`administrative-hierarchy-vocabulary.ts`)*: το δοχείο
  `companyAddress` κρατά **ΜΟΝΟ ΔΥΟ** ταυτότητες — `settlementId` (L8) και `municipalityId`
  (L5)· τα **L7/L6/L4/L3/L2/L1 είναι `NOT_STORED`**. Άρα η αρχή «ετικέτα → ταυτότητα»
  υλοποιείται στην πράξη ως **«ετικέτα → όνομα μητρώου + ΔΥΟ ταυτότητες»**. Το `projectAddress`
  κρατά **επτά**, και ο πίνακας δίνει σε κάθε δοχείο ό,τι μπορεί να κρατήσει.

  **Απόδειξη**: άγκυρες **κόκκινες πριν** *(«Καλλιθέα»→«Καλλίστη» ζωντανά στον πραγματικό hook
  με το πραγματικό μητρώο)* → **70 σουίτες / 790 πράσινα** → **5/5 μεταλλάξεις σκοτωμένες**.
  ⚠️ Η **έκτη** μετάλλαξη *(«ξαναβάλε το `neighbourhood` στην αλυσίδα της πόλης»)* **ΕΠΕΖΗΣΕ**:
  ο κανόνας ήταν **σχόλιο, όχι συμβόλαιο**. Προστέθηκε άγκυρα με το **μετρημένο** σχήμα των
  Λαδάδικων, και η μετάλλαξη σκοτώθηκε. **Πύλες**: 3.28 ✅ · 3.44 ✅ · 3.70 ✅.

  ✅ **ΖΩΝΤΑΝΗ ΕΠΑΛΗΘΕΥΣΗ — ΠΕΡΠΑΤΗΜΑ ΣΤΟΝ CHROME ΜΕ ΕΛΕΓΧΟ FIRESTORE** *(2026-09-12)*.

  **Ο route, σε 5 πραγματικά σημεία**: «Λαδάδικα» ⇒ `city: «Θεσσαλονίκη»` *(όνομα μητρώου,
  δύο σίγμα)* · `neighborhood: «Λαδάδικα»` · `settlement:0701010001` + `municipality:0701` —
  **ακριβώς** το id που το Ζ2 ανέφερε ως *«το έχουμε και δεν το χρησιμοποιούμε»*. Σύνταγμα
  και κέντρο Ηρακλείου, **όπου η γεωμετρία δεν αποδεικνύει τίποτα**, πήραν κανονικά δήμο.

  **Το περπάτημα** — ALFA, σύρσιμο του υποκαταστήματος «Πόντου, Νέα Μαγνησία» → «Ναι,
  ενημέρωσε» → Αποθήκευση. Ο διάλογος έδειξε **«Πόλη: Νέα Μαγνησία → Σταυρούπολις»**
  *(αρχαΐζουσα μορφή **μητρώου**, όχι το «Σταυρούπολη» της μηχανής)* και **«Περιοχή/Συνοικία:
  — → Ομόνοια»**. Στο Firestore, `companyAddresses[2]`:

  | πεδίο | τιμή | τι αποδεικνύει |
  |---|---|---|
  | `municipalityId` | **`municipality:0711`** | **Ζ4γ** — γράφτηκε· πριν ήταν `null` |
  | `municipalityName` | ΔΗΜΟΣ ΠΑΥΛΟΥ ΜΕΛΑ | ξανα-**παράχθηκε** για τη ΝΕΑ θέση *(ήταν 0704 ΔΕΛΤΑ)* |
  | `settlementId` | **`settlement:0711010101`** | **Ζ2** — ταυτότητα οικισμού |
  | `city` | **Σταυρούπολις** | **Ζ2** — όνομα **μητρώου**, όχι ετικέτα μηχανής |
  | `neighborhood` | **Ομόνοια** | **Ζ2** — η συνοικία **δεν** κατέλαβε την Πόλη |
  | `regionName` | ΠΕΡΙΦΕΡΕΙΑ ΚΕΝΤΡΙΚΗΣ ΜΑΚΕΔΟΝΙΑΣ *(κεφαλαία = μητρώο)* | **Ζ4β** — αποδεδειγμένο όνομα στο **κανονικό** πεδίο |
  | `region` | Περιφέρεια Κεντρικής Μακεδονίας *(πεζά = μηχανή)* | **Ζ4β** — ωμή ετικέτα στο **ελεύθερο** πεδίο, **χωρίς** να μπορεί να νικήσει |

  🔬 **ΚΑΙ Η ΖΩΝΤΑΝΗ ΜΕΤΡΗΣΗ ΒΡΗΚΕ ΚΕΝΟ ΠΟΥ ΚΑΝΕΝΑ TEST ΔΕΝ ΡΩΤΟΥΣΕ** *(όπως και στη
  Φάση Α)*: τα `municipalUnitName` και `communityName` γράφτηκαν **κενά**, ενώ το μητρώο ξέρει
  «ΔΗΜΟΤΙΚΗ ΕΝΟΤΗΤΑ ΣΤΑΥΡΟΥΠΟΛΕΩΣ» και «Δημοτική Κοινότητα Σταυρουπόλεως» ως **προγόνους του
  αποδεδειγμένου οικισμού** — δηλαδή **αποδεδειγμένους**. Αιτία: η αλυσίδα της ετικέτας
  απέδειξε ως τον **δήμο** και ο οικισμός αποδείχθηκε **μετά**, μέσα σε εκείνη την εμβέλεια,
  οπότε τα **ενδιάμεσα** επίπεδα δεν συμπληρώνονταν από κανέναν. **Παραβίαζε την ίδια μας την
  αρχή** *(«γράψε ό,τι αποδείχθηκε»)*. Διορθώθηκε *(`provedLevelsOfPlace`)* + άγκυρα, και
  επαληθεύτηκε ζωντανά: **πλήρης αλυσίδα 8/8** — `L1 ΒΟΡΕΙΑ ΕΛΛΑΔΑ … L6 ΔΗΜΟΤΙΚΗ ΕΝΟΤΗΤΑ
  ΣΤΑΥΡΟΥΠΟΛΕΩΣ · L7 Δημοτική Κοινότητα Σταυρουπόλεως · L8 Σταυρούπολις`.

  ⚠️ **Η έδρα ΔΕΝ διορθώθηκε αναδρομικά** και είναι ορθό: το `settlement` των επίπεδων πεδίων
  παραμένει «Δημοτική Ενότητα Θεσαλονίκης» επειδή **κανείς δεν έσυρε** την πινέζα της. Τα
  υπάρχοντα δεδομένα δεν ξαναγράφονται σιωπηλά· διορθώνονται στην **επόμενη** πράξη του
  ανθρώπου. *(Το ίδιο και το `hqAddressCountry: «Ελλάδα»`, γραμμένο πριν τη Φάση Α.)*

  ⚠️ **Το σύρσιμο ενεργοποιείται ΜΟΝΟ σε κατάσταση επεξεργασίας**
  *(`AddressesSectionWithFullscreen:384` — `onPinDrop={isEditing && setFormData ? … : undefined}`)*.
  Χρήσιμο για την επόμενη συνεδρία: σε **προβολή** οι πινέζες έχουν `cursor: pointer` και το
  σύρσιμο **μετακινεί τον χάρτη**· σε επεξεργασία εμφανίζεται το μήνυμα «Σύρετε την πινέζα…».

  ⚠️ **Συντεταγμένες Chrome**: το στιγμιότυπο **δεν** είναι ο χώρος συντεταγμένων του
  εργαλείου. Βαθμονόμηση από τα δεδομένα: `συντεταγμένη εργαλείου = getBoundingClientRect ×
  0,653` *(= 1568/2400)*. Τα κλικ με «συντεταγμένες στιγμιότυπου» αστοχούν — δες §10 του handoff.

  **Καμία δέσμευση** — ο Giorgio κάνει commit.
...
- ⚠️ **Εύρημα Ζ7 (διάταξη) — σε παράθυρο ~1200 CSS px τα πεδία Οδός / Αριθμός / Τ.Κ. του `AddressEditor` έχουν ωφέλιμο πλάτος ΜΗΔΕΝ.** Μετρημένο: πλάτος 26 px, padding 12+12 ⇒ `clientWidth − padding = 0`· ο χάρτης 215 px, η πινέζα εκτός κάδρου. Ο άνθρωπος **δεν βλέπει τι γράφει**. Επιβεβαιώνει την «ανεπιβεβαίωτη» παρατήρηση της 1ης συνεδρίας για τη στενή στήλη. Στα 2180 CSS px: Οδός 333 px, κανονικά.
  ...
  ✅ **Ζ7 ΕΚΛΕΙΣΕ — «το σχόλιο δεν τρώει τον χώρο του δεδομένου» (2026-09-13, Plan Mode).**

  🔴 **Η ΑΙΤΙΑ ΔΕΝ ΗΤΑΝ ΤΟ ΣΤΕΝΕΜΑ — ΗΤΑΝ ΟΤΙ Η ΠΡΟΤΕΡΑΙΟΤΗΤΑ ΣΥΡΡΙΚΝΩΣΗΣ ΗΤΑΝ ΑΝΑΠΟΔΗ.**
  Μετρημένο σε στήλη 346 px: `fieldset 122,8 = input 26,5 + badge 90,3 + gap 6`, μείον padding
  24 ⇒ **ωφέλιμο 0**. Κλείνει ακριβώς. Το `<Input>` είχε `flex-shrink: 1` (υποχωρούσε ως το
  μηδέν)· το `<AddressFieldBadge>` `min-content` (**ποτέ**). Δηλαδή **το σχόλιο για το δεδομένο
  έτρωγε 3,4× τον χώρο του ίδιου του δεδομένου** — και με το πλατύτερο σήμα
  («Δεν συμπληρώθηκε», **147 px**) η αναλογία ήταν ακόμη χειρότερη.

  🔴 **ΤΟ ΕΥΡΗΜΑ ΠΟΥ ΑΛΛΑΞΕ ΤΗΝ ΠΡΟΤΕΡΑΙΟΤΗΤΑ — ΤΟ ΕΥΡΟΣ ΗΤΑΝ ΤΡΙΠΛΑΣΙΟ.**
  Η φόρμα ζει σε **έξι** σημεία, όχι σε ένα· και **όλα** περνούν `formOptions.hideGrid: true`
  (`AddressesSectionWithFullscreen:309` · `CompanyAddressesSection:176` · `LocationInlineForm:206`
  · `FrontageAddressCreateDialog:142` · `BuildingAddressesEditor:272`). Δύο από αυτά έχουν στενή
  στήλη από `lg:grid-cols-2`, αλλά το **`FrontageAddressCreateDialog`** ζει σε
  `DialogContent sm:max-w-md` — **ΣΤΑΘΕΡΟ** πλάτος 448 px. **Μετρημένο στα 416 px περιεχομένου:
  Τ.Κ. = 0, Αριθμός = 8.** Δηλαδή εκείνη η οθόνη ήταν σπασμένη σε **ΚΑΘΕ** οθόνη, ανεξάρτητα
  από το παράθυρο, και κανείς δεν το είχε δει.

  🔴 **ΓΙΑΤΙ ΚΑΝΕΝΑ ΑΠΟ ΤΑ 703 ΠΡΑΣΙΝΑ ΔΕΝ ΤΟ ΕΒΛΕΠΕ.** Τα tests των **δύο** πραγματικά στενών
  σημείων (`ProjectLocationsTab.proximity.test.tsx`, `FrontageAddressCreateDialog.proximity.test.tsx`)
  κάνουν `jest.mock` σε **ΟΛΟΚΛΗΡΟ** το `@/components/shared/addresses/editor` ⇒ ο editor
  επιστρέφει `null`· το `AddressesSectionWithFullscreen.placement` mock-άρει το
  `AddressWithHierarchy` ως `() => null`. ⚠️ **Και δεν θα μπορούσαν**: το **jsdom δεν έχει
  διάταξη**. Ένα jest test εδώ ελέγχει μόνο **ονόματα κλάσεων** — θα περνούσε με σωστές κλάσεις
  και σπασμένη διάταξη. *Ο δηλωμένος λόγος («ο άνθρωπος βλέπει») μετριέται ΜΟΝΟ σε browser.*

  **Η θεραπεία** — δύο κανόνες, καθαρό flexbox, **μηδέν** container query / breakpoint / πακέτο:
  1. **Σκληρό δάπεδο**: `flex: 1 0 calc(<δάπεδο>ch + χρώμιο)` — `flex-shrink: 0`, το πεδίο **δεν
     υποχωρεί ποτέ** κάτω από όσους χαρακτήρες κρατά.
  2. **Τύλιγμα**: `flex-wrap: wrap` σε **γραμμή και ομάδα** — το σήμα πέφτει από κάτω, τα πεδία
     τυλίγονται σε νέα σειρά. Ο χώρος αποφασίζει, όχι το πλάτος του **παραθύρου**.

  ⚠️ **Το `flex-shrink: 0` είναι η καρδιά.** Με `1 1` (δοκιμασμένο) το τύλιγμα δεν πυροδοτείται
  και το αποτέλεσμα βγαίνει **μη μονότονο**: στήλη 560 px ⇒ Οδός **27 px**, στήλη 440 px ⇒ **96 px**.
  *Στένευε η οθόνη και μεγάλωνε το πεδίο.* Περνούσε στα άκρα, έσπαγε στη μέση.

  **Δύο νέα SSoT** *(ο κανόνας ήταν γραμμένος **πέντε** φορές — τέσσερα χειρόγραφα αντίγραφα στο
  `AddressWithHierarchy` + ένα στο `AddressFormFieldRow`, όλα με το **ίδιο** ελάττωμα)*:
  | Τι | Πού |
  |---|---|
  | `ADDRESS_FIELD_MIN_CHARS` — δάπεδο **ανά πεδίο, σε χαρακτήρες**, με τον λόγο δίπλα | `editor/address-field-widths.ts` |
  | `GREEK_POSTAL_CODE_DISPLAY_LENGTH` — **μία** δήλωση για `maxLength` **και** δάπεδο | ό.π. *(ήταν ωμό `6` στο JSX)* |
  | `AddressFieldControlRow` + `addressFieldGroup/Cell/CellWide` | `editor/components/` |

  🔑 **Σε `ch`, όχι σε px** — η ερώτηση δεν είναι «πόσα pixel;» αλλά **«χωράει αυτό που κρατά το
  πεδίο;»**, και η μονάδα της είναι ο **χαρακτήρας**. Ένα δάπεδο σε px σπάει σε άλλη
  γραμματοσειρά, άλλο μέγεθος, άλλο zoom· το `ch` τα ακολουθεί μόνο του.

  🏆 **ΝΕΑ ΠΥΛΗ — CHECK 3.82, «ΜΠΟΡΕΙ ΑΥΤΟ ΤΟ ΠΕΔΙΟ ΝΑ ΔΕΙΞΕΙ Ο,ΤΙ ΓΡΑΦΕΤΑΙ;»** *(απόφαση Giorgio)*.
  Playwright, πρότυπο CHECK 3.77: harness `/test-harness/address-field-width` που αποδίδει την
  **αληθινή** φόρμα (χωρίς ούτε ένα mock) σε **τέσσερα δηλωμένα πλάτη**, και spec που κρίνει σε
  **χαρακτήρες** διαβάζοντας **το ίδιο μητρώο** που διαβάζει ο κώδικας. `npm run test:address-field-width`.
  Κριτήρια: **Κ1** κανένα πεδίο κάτω από το δάπεδό του · **Κ2** τυλίγεται, δεν ξεχειλίζει *(ο
  πρόγονος έχει `overflow-x: clip` ⇒ ξεχείλισμα = σιωπηλή εξαφάνιση)* · **Κ3** η μέτρηση **έγινε**
  · **Κ4** μετρήθηκε το **ΧΕΙΡΟΤΕΡΟ** σήμα · **Κ5** το άνετο δεν παλινδρόμησε.
  *Material/Carbon/GOV.UK λένε **πού** πάει η ένδειξη κατάστασης· **κανένα** design system δεν
  επαληθεύει ότι το πεδίο έμεινε αναγνώσιμο — το αφήνουν στο μάτι του designer.*

  🔴 **Η ΠΥΛΗ ΕΠΙΑΣΕ ΔΥΟ ΠΡΑΓΜΑΤΙΚΑ ΛΑΘΗ ΣΤΗΝ ΙΔΙΑ ΤΗ ΘΕΡΑΠΕΙΑ** — γι' αυτό υπάρχει:
  1. **Δάπεδο σε κουτί αντί για ωφέλιμο**: `flex-basis` μετρά **border-box**, το δάπεδο δηλώνει
     **κείμενο**. Με σκέτο `1 0 var(--min)` ο Αριθμός και ο Τ.Κ. έδιναν **4,9 χαρακτήρες** σε
     στήλη 216 px. Θεραπεία: `calc(... + var(--address-field-chrome))` (2rem = το μεγαλύτερο
     `INPUT_PADDING.default`, ώστε να ισχύει **και στα δύο** breakpoints).
  2. **Ωμό i18n κλειδί στη μέτρηση**: το `(bare)` layout είναι **σκόπιμα χωρίς γλώσσα**
     (ADR-775 §13) και το project έτρεχε `locale: 'en-US'` ⇒ το σήμα έδειχνε
     `editor.field.badge.notProvided` (**30** χαρακτήρες αντί 16) — η πύλη μετρούσε πλάτος που
     **κανείς δεν βλέπει**, και μάλιστα ψευδώς αυστηρό. Δύο άμυνες: `locale: 'el-GR'` *(η
     παραγωγή είναι ελληνόγλωσση, και τα ελληνικά είναι το **χειρότερο** σενάριο μήκους)* **και**
     άρνηση δημοσίευσης μέτρησης όσο το κλειδί είναι ανεπίλυτο, μέσω του **υπάρχοντος**
     `i18n/unresolved-key` (ADR-798 §13). Ο `I18nProvider` μπήκε στη **σελίδα**, όχι στο layout —
     αλλιώς θα άλλαζαν οι συνθήκες των 43 visual tests του DXF.

  **Απόδειξη — ωφέλιμο πλάτος σε χαρακτήρες, πριν → μετά** *(ίδιο όργανο, ίδια πλάτη)*:
  | πλάτος | Οδός | Αριθμός | Τ.Κ. |
  |---|---|---|---|
  | **dialog 416** *(σταθερό!)* | 12,5 → **30,6** | **0 → 15,9** | **0 → 15,9** |
  | στήλη 346 | 6,4 → **24,5** | **0 → 12,7** | **0 → 12,7** |
  | στήλη 216 | **0 → 25,0** | **0 → 25,0** | **0 → 25,0** |
  | στήλη 805 *(έλεγχος)* | 44,7 | 12,7 | 12,7 |

  **Κανένα μηδέν. Μηδέν οριζόντιο ξεχείλισμα.** Πύλη **5/5** · `jscpd:diff` ✅ (καμία νέα
  αντιγραφή ⇒ η κεντρικοποίηση είναι πραγματική).

  **Ζωντανά, και στις ΤΡΕΙΣ διαδρομές** (Chrome, πραγματικές οθόνες):
  | διαδρομή | αποτέλεσμα |
  |---|---|
  | **Επαφές** (ALFA, στήλη 700 px) | `street 167 · number 88 · postalCode 88 · neighborhood 138`, ξεχείλισμα **0** — «Εγνατία», «104», «540 02» πλήρως ορατά |
  | **Έργα** (`ProjectLocationsTab`, 700 px) | `street 381 · number 149 · postalCode 147 · neighborhood 664`, ξεχείλισμα **0** |
  | **Dialog όψης** (`sm:max-w-md`, το **μονίμως** σπασμένο) | `street 219 · number **114** · postalCode **114**` *(ήταν 8 και **0**)*, ξεχείλισμα **0** |

  🔬 **ΜΕΤΑΛΛΑΞΕΙΣ — 6/8, ΣΕ `git worktree` ΚΑΙ ΠΟΤΕ ΣΤΟ ΚΟΙΝΟ ΔΕΝΤΡΟ** (§11 #3, εγκρίθηκε).
  Οι **Μ1** (`flex-shrink: 0→1`) και **Μ2** (`flex-wrap: wrap→nowrap`) **επέζησαν — και είναι
  δηλωμένες ΙΣΟΔΥΝΑΜΕΣ**, όχι κενό. Μετρήθηκε γιατί, αντί να υποτεθεί (πεδίο «Αριθμός»,
  στενή στήλη, δάπεδο 45 px):

  | αφαιρώντας | ωφέλιμο |
  |---|---|
  | τίποτα | 96 px |
  | μόνο `flex-shrink: 0` | **96 px** *(το τύλιγμα καλύπτει)* |
  | μόνο το τύλιγμα | **57 px** *(το σκληρό δάπεδο καλύπτει)* |
  | **και τα δύο** | **16 px** ⛔ |

  ⇒ **Γνήσιο belt-and-suspenders** (N.7.2 #4): καθένας μηχανισμός **μόνος του** αρκεί, μαζί
  δίνουν περιθώριο. Το αποδεικνύει η **συνδυασμένη Μ8**, που **σκοτώνει**.

  ⚠️ **ΚΑΙ ΜΙΑ ΠΑΓΙΔΑ ΤΟΥ ΓΥΡΟΥ, ΓΡΑΜΜΕΝΗ ΓΙΑ ΝΑ ΜΗΝ ΞΑΝΑΣΥΜΒΕΙ**: ο πρώτος γύρος έδωσε
  «Μ1/Μ2 επέζησαν» για **λάθος** λόγο — το script άλλαζε το CSS module και καλούσε την πύλη
  **ΑΜΕΣΩΣ**, οπότε το Playwright έκρινε **παλιό bundle**. Είναι το μάθημα του 3.77
  **αντεστραμμένο**: *«**πράσινη** πύλη αμέσως μετά από edit δεν είναι απόδειξη»*. Θεραπεία:
  `curl` που **μπλοκάρει** ώσπου να τελειώσει το recompile, πριν από κάθε εκτέλεση.

  🔴 **ΤΡΙΑ ΕΛΑΤΤΩΜΑΤΑ ΤΗΣ ΙΔΙΑΣ ΤΗΣ ΔΟΥΛΕΙΑΣ, ΠΙΑΣΜΕΝΑ ΑΠΟ ΦΡΟΥΡΟΥΣ ΚΑΙ ΟΧΙ ΑΠΟ ΚΡΙΣΗ** —
  γι' αυτό υπάρχουν οι πύλες:
  1. **Δάπεδο σε κουτί αντί για ωφέλιμο** *(το Κ1 της ίδιας της πύλης)*.
  2. **Μη ντετερμινιστική άγκυρα** *(το Κ3, σε **άλλη** συνεδρία: 5/5 σε ζεστή σελίδα, **4/5**
     σε κρύα)*. Η θεραπεία δεν ήταν να χαλαρώσει το Κ3 — ήταν να μη δημοσιεύει το **όργανο**
     ελλιπή μέτρηση: απαιτεί πλέον **πληρότητα** (ίσος αριθμός πεδίων σε κάθε περίπτωση)
     **και** σταθερότητα, με παράθυρο **45 s** αντί 3 s.
  3. **Η ίδια η πύλη ήταν `unexecuted`** — το **CHECK 3.54** μπλόκαρε το commit: υπήρχε
     project και npm script, αλλά **κανένα workflow δεν την έτρεχε**. *«Ένα anchor χωρίς gate
     δεν είναι anchor — είναι σχόλιο»* (ADR-587 §6.1). Λύθηκε με
     `.github/workflows/address-field-width-gate.yml` (tier 2) + εγγραφή στο
     `.ci-gate-tiers.json` (3.37) και στο `.gate-inventory.json` (3.66).
...
### D28 — Ο Τ.Κ. αποδεικνύει ό,τι αφαιρέσαμε· «ευρύτερη περιοχή» δεν είναι «λάθος» *(2026-09-15)*
**RESOLVED — Plan Mode, τρεις φάσεις**

#### Το περιστατικό, μετρημένο
«Εντοπισμός στον χάρτη» στην κάρτα βιτρίνας για **«Σαμοθράκης 16, 56334, Θεσσαλονίκη»** ⇒ ο χάρτης έμενε στην Ομόνοια.
Ζωντανά στο `/api/geocoding` (2026-09-14):

| Ερώτημα | Αποτέλεσμα |
|---|---|
| οδός + αριθμός + Τ.Κ. + **πόλη** | ❌ 404 — **όλες** οι παραλλαγές |
| οδός + Τ.Κ. + πόλη (χωρίς αριθμό) | ❌ 404 |
| οδός + αριθμός + Τ.Κ. (**χωρίς πόλη**) | ✅ Ελευθέριο-Κορδελιό · `street: match` · `postalCode: match` |

Ο Τ.Κ. 56334 ανήκει στον **Δήμο Κορδελιού-Ευόσμου**· η «Θεσσαλονίκη» είναι η **μητροπολιτική** περιοχή, και ο Nominatim τη διαβάζει ως δήμο — τα δομημένα πεδία του είναι αυστηρό AND. Στη Θεσσαλονίκη υπάρχουν **τέσσερις** οδοί «Σαμοθράκης»: μόνο ο Τ.Κ. τις ξεχωρίζει.

#### Πρακτική αγοράς (έρευνα)
Google Address Validation: `replaced`/`inferred`/`unconfirmed` ανά πεδίο + `possibleNextAction`. Mapbox v6: `match_code` ανά πεδίο, **όχι** αποτυχία. Pelias: σκάλα χαλάρωσης — εξάντλησε το συγκεκριμένο, μετά ρίξε το λιγότερο αξιόπιστο. HERE/Esri: βαθμολογία ανά πεδίο. Rightmove: ο Τ.Κ. ως αλήθεια αναφοράς για την πινέζα. Airbnb: ο άνθρωπος επιβεβαιώνει στον χάρτη.

#### Α — Βαθμίδα 9 με πύλη απόδειξης (διακομιστής)
- **Σειρά**: `1–6` πλήρης διεύθυνση → **`9`** οδός+αριθμός+Τ.Κ. χωρίς τοπωνύμιο → `7–8` χαλάρωση χώρας. Ο **αριθμός είναι ταυτότητα** (αποθηκεύεται στο `geocodingMetadata.variantUsed`), η **θέση είναι σειρά** — ποτέ επαναρίθμηση. Διευθύνσεις που λύνονται σήμερα δεν πληρώνουν ούτε ένα αίτημα παραπάνω (άγκυρα Α5).
- 🔑 **Το βήμα πέρα από το Pelias**: εκείνο ρίχνει πεδίο τυφλά. Εδώ υποψήφιος γίνεται δεκτός **μόνο** αν φέρει τον **ίδιο** Τ.Κ. σε κανονική μορφή (D16)· απών Τ.Κ. = καμία απόδειξη. Απάντηση που απορρίφθηκε ολόκληρη καταγράφεται `no-results`, όχι `success`.
- `reasoning.relaxation = { dropped, anchor: 'postalCode' }` — το `replaced` της Google: τι **δεν** ρωτήθηκε.
- Η βαθμίδα παραλείπεται όταν δεν δηλώθηκε `city`/`neighborhood` (θα ξαναρωτούσε ό,τι μόλις απέτυχε — πολιτική OSMF).
- **Αναδόμηση**: η σκάλα ήταν μία συνάρτηση ~110 γραμμών με οκτώ σχεδόν ταυτόσημα μπλοκ ⇒ **δηλωμένη λίστα** `app/api/geocoding/geocoding-ladder.ts` + βρόχος στη μηχανή· URL/κλήση/καταγραφή στο `geocoding-nominatim-client.ts`. Ασφαλής επειδή **κάθε** συνθήκη εφαρμογής εξαρτάται μόνο από το ερώτημα. Μηχανή 488→311 γραμμές.
- Μνήμη `geocoding:v1` → **`v2`**: αλλιώς το αρνητικό `absent` (10′) θα έκρυβε τη διόρθωση.

#### Β — `broader`: μία σύγκριση, δύο πλευρές
- Η σύγκριση πεδίων ήταν **διπλή** — `buildFieldMatches` (διακομιστής) και `useAddressFieldStatus.computeStatus` (πελάτης) — και είχε **αποκλίνει**: ο Τ.Κ. «546 24»⇄«54624» ήταν `match` στον διακομιστή και **«Ασυμφωνία»** στο badge. ⇒ **ένα** `lib/geocoding/field-match.ts` (`compareAddressField`), δύο καταναλωτές.
- Νέο `FieldMatchKind = 'broader'` για `city`/`neighborhood`: η δηλωμένη τιμή ονομάζει **ευρύτερη** περιοχή της **ίδιας** απάντησης (`county`/`region`…). **Δεν** ανεβάζει `partialMatch` — είναι συνεπής δήλωση. Badge «Ευρύτερη περιοχή» (`info`, όχι `warning`).
- ⚠️ Κρίνεται από **ονόματα**: το εμπρός geocoding δεν παράγει αποδεδειγμένη ιεραρχία (`admin` μόνο στο reverse).
- `utils/address/place-name.ts` → `samePlaceName`: πρόθεμα βαθμίδας + δίπλωμα + **κλειστός πίνακας ζευγών πτώσεων** (-η⇄-ης, -α⇄-ων, -ος⇄-ου…), λέξη-προς-λέξη, ίδιο πλήθος λέξεων.
  - ⛔ **Απορρίφθηκε**: το `greekStemMatch` του ai-pipeline («περιέχει» σε ρίζες) — η ανεκτικότητα που ο D27 μέτρησε να βγάζει δήμο Χίου για το Χαλάνδρι.
  - ⛔ **Απορρίφθηκε**: «κόψε όποια κατάληξη και σύγκρινε ρίζες» — κάνει **Χαλάνδρι = Χάλανδρα**. Τα **ζεύγη** το αποκλείουν (`-ι⇄-α` δεν είναι ζεύγος).
  - Ελάχιστη ρίζα **3**, μετρημένη: με 4 η **Βόλος⇄Βόλου** έβγαινε «άλλος τόπος».
- Άγκυρα-παρονομαστής: «Καλαμαριά» μέσα στη Θεσσαλονίκη **μένει** `mismatch`.

#### Γ — Ο άνθρωπος μαθαίνει τι βρέθηκε (βιτρίνα + προσφορά)
...
#### Γ.2 — Σύντομη μορφή της απάντησης *(2026-09-15, μετά το ζωντανό περπάτημα)*
- Το περπάτημα στη βιτρίνα επιβεβαίωσε τη διόρθωση (χάρτης στο Ελευθέριο-Κορδελιό, γραμμή «ευρύτερη περιοχή») και έδειξε ότι η ετικέτα ήταν το `display_name` αυτούσιο: **12** κομμάτια («…Δήμος … Μητροπολιτική Ενότητα … Αποκεντρωμένη Διοίκηση … Ελλάδα»).
- Νέο `formatResolvedAddressLine` στο `utils/address/address-line.ts` (το SSoT γραμμής διεύθυνσης): **«Οδός Αριθμός, στενότερη Περιοχή, Τ.Κ.»** — γειτονιά πριν από πόλη, πρόθεμα βαθμίδας αφαιρεμένο (`stripGreekAdminPrefix`), Τ.Κ. στη γραφή ΕΛΤΑ. Εφεδρεία στο πλήρες κείμενο όταν δεν υπάρχει ούτε οδός ούτε περιοχή.
- ⛔ Όχι `cleanPlaceName`: αναδιπλώνει την παύλα (δίπλωμα σύγκρισης) — «Ελευθέριο-Κορδελιό» θα γινόταν «Ελευθέριο Κορδελιό» στην οθόνη επαλήθευσης.
- ⚠️ Ο αριθμός μπαίνει **μόνο αν τον επέστρεψε ο πάροχος** — ο δηλωμένος-ανεπιβεβαίωτος λέγεται χωριστά.

#### Δ — Από «βρέθηκε» σε «αποθηκεύτηκε»: η διεύθυνση ως ρητό κλικ *(2026-09-15)*
**Περιστατικό (Giorgio, 08:55)**: στην κάρτα ο «Εντοπισμός» δούλευε, η «Αποθήκευση κάρτας» έδινε *«Επιλέξτε περιοχή για κάθε κατάστημα»* και το «Χρησιμοποίησε αυτόν τον τόπο» ήταν κλειστό. Αιτία, στον κώδικα: ο εντοπισμός **κεντράρει** (`focus`), δεν παράγει χειρονομία· το `pending` του `PlaceChooser` μένει `null` μέχρι κλικ σε κτίριο/πινέζα/σχήμα — και **καμία πρόταση** δεν το έλεγε. Το μήνυμα λάθους ζούσε μόνο δίπλα στο κουμπί.

**Πρακτική αγοράς**: Airbnb — η διεύθυνση τοποθετεί πινέζα, ο οικοδεσπότης **επιβεβαιώνει** («is the pin in the right spot?»)· Google Business Profile — διεύθυνση ⇒ «Adjust» πινέζας μόνο αν χρειάζεται ⇒ Save· GOV.UK — σφάλμα **δίπλα στο πεδίο** + μεταφορά focus.

**SSoT audit (grep)**: 🔑 η χειρονομία `typed-address` **υπήρχε** σε τύπο, zod, επικύρωση και διακομιστή (`verifyPlaceClaim` γεωκωδικοποιεί ο ίδιος, προέλευση `geocoded`) και το SPEC-777A §21.4 την έχει **επικυρωμένη** ως σκαλοπάτι 1 — αλλά **κανένας** πελάτης δεν τη χρησιμοποιούσε. Βοηθός «κύλιση + focus» **δεν** υπήρχε (μόνο `focusFirstElement` στο `utils/accessibility`).

- **Ρητό κλικ, όχι σιωπηλή επιλογή**: νέο `components/geo/PlaceAddressOffer.tsx` μέσα στον `PlaceChooser` — «Χρησιμοποίησε τη διεύθυνση που βρέθηκε» ⇒ `claim({ gesture: 'typed-address', query })`. Εμφανίζεται **μόνο** όταν υπάρχουν **και** `focus` **και** `addressQuery` (νέο prop, περνά από `PlaceIdentityField`). Επαναχρησιμοποιεί **όλη** τη μηχανή καταχώρησης (κατάσταση, ερώτηση διπλοτύπου, μηνύματα).
- ⛔ **Απορρίφθηκε `dropped-pin` από το σημείο του `focus`** (πρόταση του handoff): θα έγραφε `manual` για θέση που δεν έβαλε άνθρωπος και θα έστελνε συντεταγμένες από τον πελάτη (§14.4 κανόνας 2).
- **Ένας κριτής ακρίβειας** `addressClaimAdmissible(accuracy, target)` στο `lib/places/place-claim.ts`: γη ⇒ `exact`/`interpolated` · κτίριο ⇒ μόνο `exact` (αλλιώς ένα «κτίριο» ανά ιδιοκτήτη του ίδιου δρόμου — η γεωκωδικοποιημένη γη δεν έχει περίγραμμα για τον έλεγχο διπλοτύπου) · `approximate`/`center` ποτέ. Η οθόνη τον ρωτά για να **προσφέρει**· ο διακομιστής (`resolvePlace`, που κατέχει τον στόχο) για να **δεχτεί** ⇒ νέα άρνηση `address-too-coarse`. Αδρή ακρίβεια στην οθόνη ⇒ **πρόταση** χωρίς κουμπί, όχι σιωπή.
- **Διόρθωση λανθάνοντος σφάλματος**: το «Όχι, είναι άλλος» ξανάστελνε το `pending` — για διεύθυνση ήταν `null`, άρα **δεν έστελνε τίποτα**. Πλέον κρατιέται η **υποβληθείσα** χειρονομία.
- **Σφάλμα δίπλα στο πεδίο (βιτρίνα)**: `ShowcaseCardSection` κρατά `saveAttempted` και κρίνει **ζωντανά** (σβήνει μόλις υπάρξει τόπος)· `ShowcaseLocationEditor` λέει `cardPlaceMissingHere` κάτω από την «Περιοχή» (`role="alert"`, `aria-describedby`)· η «Αποθήκευση» μεταφέρει τον άνθρωπο εκεί με νέο `revealAndFocus` στο `utils/accessibility` (τίτλος `tabIndex={-1}`).
- **Φόρμα προσφοράς**: `OwnerPropertyPlaceField` περνά `addressQuery={placeQuery}` — κάθε πλήκτρο σβήνει το σημείο, άρα κείμενο και `focus` δεν διαφωνούν. Στόχος `building` ⇒ κουμπί μόνο σε ακριβή διεύθυνση.
- 🔒 **CHECK 3.34**: τα κλειδιά μπήκαν πρώτα στο `search-results` ⇒ **15.916 > 15.200** (εγγυημένο κέλυφος). Θεραπεία = μετακόμιση: `property-market:offer.form.placeAddressOffer.*` (οικογένεια του `ResolvedPlaceConfirmation`) + `next/dynamic` στον `PlaceChooser`. Ακόμη και **μία** γραμμή άρνησης στο `search-results` το κρατούσε κόκκινο (**15.311**) ⇒ **κανένα** νέο κλειδί στο κέλυφος: η άρνηση `address-too-coarse` λέγεται από την ίδια την προσφορά (`refused`, πρόταση «κατά προσέγγιση», χωρίς κουμπί) και το `IdentityStatus` σιωπά γι' αυτήν — ίδιο σχήμα με την ερώτηση διπλοτύπου.
- 🧪 Άγκυρες: `lib/places/__tests__/place-claim-address.test.ts` (πίνακας 4×2) · `services/places/__tests__/public-place-write.address-claim.test.ts` Δ1-Δ6 (άρνηση **χωρίς εγγραφή**, παρονομαστής πινέζας) · `components/geo/__tests__/PlaceChooser.address-offer.test.tsx` Π1-Π7 (Π6: άρνηση διακομιστή ⇒ πρόταση χωρίς κουμπί) · `ShowcaseLocationEditor.locate.test.tsx` +3. Jest: 12 σουίτες **86/86** (όλο το `components/geo` + `services/places`). Πύλες: 3.28 `jscpd:diff` ✅ · 3.34 slice ✅ (κάρτα 12.701/13.070, `search-results` 15.163 αμετάβλητο) · `generate:i18n-types` ✅. ⚠️ Όχι επαληθευμένα: tsc (N.17) · ζωντανό περπάτημα.
...
- Οι προτάσεις `offer.form.placeHouseNumber.*` είναι σε **β΄ ενικό** («Έγραψες»), ενώ η βιτρίνα μιλά στον πληθυντικό ευγενείας. Θέμα ύφους του κοινού πλαισίου· δεν άλλαξε εδώ.
- Σύνολο δεδομένων Τ.Κ.→δήμος (ΕΛΤΑ) ως ανεξάρτητη αυθεντία: το `administrative-hierarchy.json` έχει Τ.Κ. σε 949 οικισμούς μόνο και **όχι** τον 56334 — ξεχωριστό έργο.
