...
| 2026-05-14 | **FIX: Parallel guide Y-axis direction**: Y-axis sign calculation was inverted (parallels created opposite side from cursor). Changed `handleGuideParallel()` sign logic: Y-axis now uses `(ctx.worldPoint.y >= guide.offset ? -1 : 1)` instead of `1 : -1`. Root cause: coordinate semantics — Y-axis comparison requires negation for correct directional inference. X-axis (vertical guides) unaffected. Tested both X and Y guides. |
