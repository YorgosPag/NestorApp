...
που ήδη έδειχνε ο κλειδωμένος σταυρός (κανένα άλμα). **Παγώνει** στο κλικ.

**Οπτική ταυτότητα (απαίτηση Giorgio)**: η δυναμική γραμμή είναι **ταυτόσημη** με του
εργαλείου «Μετακίνηση» — κόκκινο ＋ (`drawMoveBasePointMarker`, ADR-049) + χρυσή
διακεκομμένη (`drawRubberBandLine`). Η ταυτότητα προκύπτει από **κοινό κώδικα**, όχι από
...
**Μονάδα εμφάνισης στο πεδίο πληκτρολόγησης (FIX 2026-07-18)**: με επιλεγμένη μονάδα εμφάνισης
«m», το λευκό HUD και οι ετικέτες αποστάσεων έδειχναν σωστά `0,150 m`, αλλά το ίδιο το πεδίο
πληκτρολόγησης στον καμβά (`CanvasNumericInputOverlay`) έγραφε σταθερά `mm` — και ο αριθμός
που πληκτρολογούσε ο χρήστης ερμηνευόταν όντως ως scene units, ασυμφωνία ×1000 μεταξύ αυτού
που έβλεπε και αυτού που έγραφε. **Fix**: `CanvasNumericInputStore` απέκτησε ιδιωτική
`_typedToSceneUnits(typed)` = `fromDisplay(typed, displayUnitState.getUnit()) *
immediateSceneScale.getMmToScene()` (display → mm → scene), εφαρμοσμένη ΚΑΙ στο
`getPendingDistance()` (φάντασμα) ΚΑΙ στο `confirm()` (commit) — αν εφαρμοζόταν σε ένα μόνο
από τα δύο, φάντασμα και commit θα διαφωνούσαν ×1000. Το κατώφλι `SCENE_EPSILON = 0.001`
ελέγχεται **μετά** τη μετατροπή (`0.0005` σε μέτρα = 0,5 mm, υπαρκτή απόσταση όχι θόρυβος).
`CanvasNumericInputOverlay`: η ετικέτα μονάδας ήταν hardcoded `<span>mm</span>` → έγινε
`DISPLAY_UNIT_LABELS[displayUnit]` μέσω `useDisplayUnit()`. **Γιατί στο store και όχι στον
caller**: ο buffer είναι το μοναδικό σημείο όπου μπαίνει τιμή σε μονάδα χρήστη — γενικού
σκοπού, όχι ειδικό για τον Παράλληλο Οδηγό· κάθε μελλοντικός καταναλωτής θα κληρονομούσε
σιωπηλά το ίδιο σφάλμα αν η μετατροπή ζούσε στη ροή του §3.13. **Πρότυπο που ακολουθήθηκε**:
το SNAP step του `CadStatusBar` (ADR-677) — `toDisplay` για εμφάνιση, `fromDisplay` πριν
αποθηκευτεί, canonical στο domain, μετατροπή ΜΟΝΟ στο UI boundary.

**ΓΝΩΣΤΟ ΘΕΜΑ (εκτός εύρους αυτής της διόρθωσης, ΜΗΝ το επιδιορθώσεις σιωπηλά)**: το
`systems/guides/guide-annotations-renderer.ts:131` περνά τη διαφορά `guide.offset` κατευθείαν
στο `formatLengthForDisplay(worldDistance)`, που περιμένει **mm**. Τα `guide.offset` όμως
είναι **scene units** (δημιουργούνται από world coordinates, π.χ. `addGuide('X', origin.x)`).
Αυτό είναι σωστό μόνο όσο scene units === mm. Σε σχέδιο με scene units σε μέτρα, οι ετικέτες
αποστάσεων μεταξύ οδηγών θα δείχνουν λάθος τιμή κατά ×1000. Λανθάνον, προϋπάρχον.
...
| B121 | **Entity → Guide** ❌ ΑΦΑΙΡΕΘΗΚΕ (2026-07-17) | Υλοποιήθηκε 2026-05-11, αφαιρέθηκε 2026-07-17 κατ' εντολή Γιώργου: το toast «Δημιουργία οδηγών στα σημεία της οντότητας;» έβγαινε σε **κάθε** σχεδίαση (γραμμή, τόξο, κύκλο…) και ήταν ενοχλητικό. Οι οδηγοί από οντότητα παραμένουν διαθέσιμοι **χειροκίνητα** μέσω B8 (Guide from Entity) και B37 (Guide from selection). **ΜΗΝ το επαναφέρεις ως αυτόματο prompt.** | ❌ Καταργήθηκε |
...
| 2026-05-11 | ~~**B121: Entity → Guide notification**~~ ❌ **ΑΦΑΙΡΕΘΗΚΕ 2026-07-17 (βλ. εκεί).** Γενίκευση του B36 σε όλες τις οντότητες. `EventBus.on('drawing:complete', ...)` listener στο `useEntityCompleteGuideListener` → `handleEntityComplete(entity, tool)` → notification "Create guides at entity points?" → X/Y guides στα entity key-points. SSOT extractor `extract-entity-key-points.ts` (line: start+end / circle+ellipse: center / arc: center+endpoints / polyline+spline: vertices / rectangle: 4 corners / text+point: position / dimension: 2 endpoints). Measurement tools εξαιρούνται (skip via `MEASURE_TOOLS_FOR_GUIDES` set εξαγόμενο από `useDrawingHandlers.ts`). `promptCreateGuidesAtPoints()` helper εξάγει το common X/Y guide creation logic μεταξύ B36 και B121 (DRY/SSOT). i18n key: `guides.entityToGuide`. 7 files modified |
| 2026-05-14 | ~~**FIX: Parallel guide Y-axis direction**~~ ⚠️ **ΑΝΑΚΛΗΘΗΚΕ 2026-07-17 — ΗΤΑΝ ΛΑΘΟΣ ΔΙΑΓΝΩΣΗ.** Η καταχώρηση ισχυριζόταν ότι «coordinate semantics — Y-axis comparison requires negation» και γύρισε το πρόσημο σε `(worldPoint.y >= offset ? -1 : 1)`. **Δεν υπάρχει τέτοια σημασιολογία**: η σύγκριση (`findNearestGuide`) και η τοποθέτηση (`CreateParallelGuideCommand`: `reference.offset + offsetDistance`) γίνονται ΑΜΦΟΤΕΡΕΣ στον ίδιο χώρο (world/offset), άρα η φορά του Y στην οθόνη απλοποιείται και **δεν** χρειάζεται αντιστροφή. Το flip έκανε τον Y **ντετερμινιστικά λάθος**. Το πραγματικό σύμπτωμα («άλλοτε σωστά, άλλοτε ανάποδα») προερχόταν από τον θόρυβο των 30px (βλ. 2026-07-17). ΜΗΝ επαναφέρεις την αντιστροφή — το `guide-parallel-side.test.ts` θα κοκκινίσει. |
| 2026-06-11 | **FIX §3.5 — Silent guide *line* snap, marker only at the discrete crossing (Giorgio)**: το `case 'guide'` ═-glyph (changelog 2026-02-21) ακολουθούσε τον κέρσορα σε κάθε pixel κατά μήκος του οδηγού («πολύ κουραστικό, καμία ωφέλεια»). Ευθυγράμμιση κώδικα↔ADR (§3.5: marker μόνο στις τομές). `GuideSnapEngine.findSnapCandidates` → tracking μόνο του *πλησιέστερου in-range* ορθογώνιου οδηγού/άξονα· τομή κατακόρυφου+οριζόντιου οδηγού → ΕΝΑ `INTERSECTION` (✕) candidate (priority `GUIDE-0.5`, distance=hypot — κερδίζει τις σιωπηλές line υποψηφιότητες, κάτω από real geometry)· μονός άξονας → σιωπηλό line snap. `SnapIndicatorOverlay`: suppress `type === 'guide'` (όπως `'grid'`) → το snap ΔΟΥΛΕΥΕΙ, χωρίς floating glyph. XZ/midpoint/fractal παραμένουν σιωπηλά line snaps. 2 files. DEFER: μόνιμα ✕ σε ΟΛΕΣ τις τομές όταν κέρσορας μακριά (§3.5 «ανενεργό»)· crossings με midline/fractal. |
| 2026-06-12 | **Δύο context-aware συμπεριφορές οδηγών (Giorgio· DONE· 22 jest· 🔴 verify+commit):** (Α) **Σχεδιασμός = έλξη ΜΟΝΟ στις διασταυρώσεις**: ο Giorgio «όταν σχεδιάζω γραμμή/τοίχο, με ενδιαφέρουν μόνο οι τομές των οδηγών». NEW `systems/cursor/SnapDrawingModeStore.ts` (event-time `set/isSnapDrawingMode`, plain store· γράφεται από `mouse-handler-move`+`mouse-handler-up` με `isInDrawingMode` πριν το `findSnapPoint`)· ο `GuideSnapEngine` διαβάζει `isSnapDrawingMode()` → στο σχεδιασμό καταστέλλει single-guide line (X/Y/XZ) + midpoint + fractal, **το ✕ INTERSECTION εκπέμπεται ΠΑΝΤΑ**. Εκτός σχεδιασμού πλήρες snapping αμετάβλητο. (Β) **Μετακίνηση οδηγού = ο σταυρός κολλάει στη γραμμή**: ο Giorgio «όταν σέρνω οριζόντιο οδηγό πάνω-κάτω, ο σταυρός κολλημένος πάνω του· δεξιά-αριστερά γλιστράει κατά μήκος χωρίς να ξεκολλάει». MOD `useCanvasMouse.handleContainerMouseMove` guide-drag branch: μετά το `moveGuideById`, override `setImmediatePosition` με το locked world point projected στη ΝΕΑ θέση του οδηγού (Y→`{raw.x, offset}`· X→`{offset, raw.y}`· XZ→`projectPointOnSegment` πάνω στη μετακινημένη διαγώνιο). Reuse του ίδιου crosshair-override pattern με το grip-step snap-to-grid. NEW `GuideSnapEngine.intersection-only.test`(5)+`SnapDrawingModeStore.test`(2). ✅ Google-level: YES — event-time SSoT, context-aware, μηδέν regression εκτός σχεδιασμού, ✕ ανέπαφο. 🔴 verify (σχεδίασε γραμμή: έλξη μόνο σε τομές οδηγών, όχι κατά μήκος· σύρε οριζόντιο οδηγό: σταυρός κολλημένος στη γραμμή, γλιστράει δεξιά-αριστερά) + commit. DEFER: τομές διαγώνιου×ορθογώνιου οδηγού· numeric placement πάνω σε τομή. |
| 2026-06-12 | **Guide hover-lock: το κέντρο του σταυρού κολλάει στη γραμμή μόλις φωτιστεί (Giorgio· DONE· 6 jest· 🔴 verify+commit):** Giorgio «όταν κάνω κλικ, το κέντρο του κέρσορα απέχει από τη γραμμή· πρέπει να κολλάει αυτόματα στο σώμα της γραμμής όταν έχει φωτιστεί». Ρίζα: ο σταυρός οδηγείται από `ImmediatePositionStore` = **raw screenPos**· μόνο το snap-indicator glyph πήγαινε στη γραμμή, όχι το κέντρο του σταυρού. Fix (hover-lock, mirror του grip-hover `GripSnapStore` lock): SSoT `projectPointOntoGuide(guide, pt)` + `GUIDE_HIT_TOLERANCE_PX=30` + `isGuideEditTool(tool)` (set = `needsToolHighlight`) στο `guide-types.ts`. `mouse-handler-move` setImmediatePosition block: όταν `isGuideEditTool(activeTool)` + `findNearestGuide` εντός `GUIDE_HIT_TOLERANCE_PX/scale` → `worldToScreen(projectPointOntoGuide(...))` → ο σταυρός κουμπώνει στη γραμμή· αλλιώς raw. **Ίδιο threshold με το highlight → φωτίζεται ⟺ κολλάει.** Boy-scout: `useGuideWorkflowComputed` hardcoded `30`→`GUIDE_HIT_TOLERANCE_PX` SSoT. NEW `guide-line-projection.test`(6). ΕΚΤΟΣ placement tools (guide-x/z/xz δημιουργούν νέο→δεν κλειδώνουν). 🔴 verify (εργαλείο μετακίνησης/διαγραφής/παράλληλου οδηγού: πλησίασε οδηγό→φωτίζεται→ο σταυρός κουμπώνει πάνω στη γραμμή· κλικ=ακριβώς στη γραμμή) + commit. |
| 2026-06-12 | **Guide-move OSNAP: ο οδηγός κουμπώνει στο snap point (endpoint οντότητας) στο release (Giorgio· DONE· 6 jest· 🔴 verify+commit):** Giorgio «σύρω οδηγό προς το άκρο οντότητας, εμφανίζεται σημάδι έλξης (✛), αλλά όταν αφήνω ο οδηγός δεν ταυτίζεται με αυτό». Ρίζα: ΚΑΙ το live mousemove (`useCanvasMouse`) ΚΑΙ το commit mouseup (`useCanvasContainerHandlers`) υπολόγιζαν τη θέση από **raw cursor delta**, αγνοώντας το active snap — το γενικό snap pipeline (`mouse-handler-move`) εμφανίζει το marker αλλά δεν τάιζε τη μετακίνηση. Fix (SSoT): NEW `systems/guides/guide-drag-snap.ts` `resolveGuideDrag(axis, raw, startMouse, originalOffset)` διαβάζει `getImmediateSnap()` (το snap που ήδη υπολογίζει το pipeline κάθε frame)· αν engaged → η γραμμή περνά ΑΠΟ το snap point (X→offset=snap.x, Y→offset=snap.y)· αλλιώς free delta tracking. Χρησιμοποιείται και στα ΔΥΟ σημεία → live==commit (WYSIWYG). Στο live, ο σταυρός κλειδώνει στο snap point (όχι απλώς projected) για visual confirmation. NEW `guide-drag-snap.test`(6). XZ διαγώνιος = delta-based (DEFER snap). 🔴 verify (OSNAP on· σύρε οριζόντιο/κατακόρυφο οδηγό προς endpoint οντότητας→✛· άσε→ο οδηγός περνά ΑΚΡΙΒΩΣ από το endpoint) + commit. |
| 2026-07-17 | **FIX: Παράλληλος οδηγός — η πλευρά ακολουθεί τον κέρσορα (Giorgio· DONE· 8 jest· 🔴 verify+commit).** Giorgio: «κλικ στον οδηγό αναφοράς, μετακίνηση κέρσορα στη μία πλευρά — άλλοτε τον βάζει εκεί που είναι ο κέρσορας, άλλοτε αντίθετα». **ΔΥΟ ανεξάρτητες ρίζες.** (Α) **Θόρυβος 30px**: το εργαλείο ήταν 1-click — το ΙΔΙΟ κλικ που διάλεγε την αναφορά υπολόγιζε και την πλευρά. Όμως το κλικ πρέπει να πέσει μέσα στην ανοχή `30/scale` γύρω από τη γραμμή, άρα η πλευρά κρινόταν από το αν έπεσε 2px πάνω ή κάτω = **τύχη**· η μετακίνηση του κέρσορα αγνοούνταν πλήρως. (Β) **Ανεστραμμένο πρόσημο σε Y + XZ**: ο Y από το άκυρο flip της 2026-05-14 (βλ. εκεί)· ο XZ επειδή το cross product `cx*dy - cy*dx` έχει **αντίθετο** πρόσημο από την προβολή στο κάθετο `n = (-dy, dx)` που χρησιμοποιεί η `CreateParallelGuideCommand` (`c·n = -cross/len`). Ο X ήταν πάντα σωστός — γι' αυτό «δούλευε» σε κατακόρυφους. **Fix (SSoT):** NEW `systems/guides/guide-parallel-side.ts` → `resolveParallelSide(guide, worldPoint)`, μία πηγή για την πλευρά, συμμετρική σε X/Y (καμία αντιστροφή) + διορθωμένο XZ. Ροή → **2-step**: `handleGuideParallel` κάνει ΜΟΝΟ `onParallelRefSelected`· η πλευρά διαβάζεται **event-time** στο Enter μέσω `getRealtimeWorldCursor()` (ADR-040 κανόνας 2) — `CanvasNumericInputStore.activate` δέχεται πλέον `signResolver: () => 1\|-1` αντί για παγωμένο `sign`. Παρενέργεια: τα ghost previews (`useGuideWorkflowComputed`) + ο step hint ήταν **νεκρός κώδικας** (το `parallelRefGuideId` δεν τίθετο ΠΟΤΕ) — τώρα ζωντανεύουν· +step 1 στο `tool-hints.json` (el+en). RM νεκρό `onParallelSideChosen`/`handleParallelSideChosen`. Boy-scout (N.0.2): οι 3 rotate handlers είχαν 3× πανομοιότυπο prompt-dialog block (2 jscpd clones) → NEW `promptRotateAngle()` helper. NEW `guide-parallel-side.test`(8) — **round-trip μέσα από την πραγματική εντολή**, όχι σκέτο πρόσημο: ένα test που κλειδώνει το πρόσημο απομονωμένο περνά και με ανεστραμμένη σύμβαση, γι' αυτό ακριβώς το bug επέζησε 2 μήνες. ✅ Google-level: YES — event-time SSoT, μηδέν παγωμένη κατάσταση, η σύμβαση κλειδωμένη σε test. 🔴 verify (G→P· κλικ σε ΟΡΙΖΟΝΤΙΟ οδηγό· κέρσορας πάνω→πληκτρολόγησε 5+Enter→ο οδηγός ΠΑΝΩ· επανάλαβε με κέρσορα κάτω→ΚΑΤΩ· ίδιο σε κατακόρυφο + διαγώνιο) + commit. |
...
| 2026-07-18 | **§3.13 FIX: μονάδα εμφάνισης mismatch στο πεδίο πληκτρολόγησης (Giorgio· DONE· 151 jest).** Giorgio (screenshot): με μονάδα εμφάνισης «m», το λευκό HUD + οι ετικέτες αποστάσεων έδειχναν σωστά `0,150 m`, αλλά το πεδίο πληκτρολόγησης στον καμβά έγραφε σταθερά `mm` — ασυμφωνία ×1000 μεταξύ αυτού που φαινόταν και αυτού που ερμηνευόταν. **Fix**: NEW ιδιωτική `CanvasNumericInputStore._typedToSceneUnits(typed)` = `fromDisplay(typed, displayUnitState.getUnit()) * immediateSceneScale.getMmToScene()` (display → mm → scene), εφαρμοσμένη ΚΑΙ στο `getPendingDistance()` (φάντασμα) ΚΑΙ στο `confirm()` (commit) — μία μόνο εφαρμογή θα άφηνε φάντασμα≠commit ×1000. `SCENE_EPSILON` ελέγχεται ΜΕΤΑ τη μετατροπή. `CanvasNumericInputOverlay`: hardcoded `<span>mm</span>` → `DISPLAY_UNIT_LABELS[displayUnit]` μέσω `useDisplayUnit()`. Το fix ζει στο **store**, όχι στη ροή του §3.13 — ο buffer είναι το μοναδικό σημείο εισόδου τιμής σε μονάδα χρήστη, γενικού σκοπού. **Πρότυπο**: SNAP step του `CadStatusBar` (ADR-677) — `toDisplay`/`fromDisplay` στο UI boundary, canonical στο domain. NEW describe «μονάδα εμφάνισης (ADR-677)» στο `systems/canvas-numeric-input/__tests__/CanvasNumericInputStore.test.ts` (mm ταυτοτικό, m ×1000, cm ×10, φάντασμα==commit, κατώφλι-μετά-τη-μετατροπή) + integration test με μονάδα «m» στο `hooks/guides/__tests__/guide-parallel-commit-workflow.test.ts`. **ΓΝΩΣΤΟ ΘΕΜΑ, εκτός εύρους** (βλ. §3.13 πιο πάνω): `systems/guides/guide-annotations-renderer.ts:131` περνά `guide.offset` (scene units) κατευθείαν σε `formatLengthForDisplay` (περιμένει mm) — σωστό μόνο όσο scene units≡mm. |
...
5. **Parallel workflow** — ⚠️ ΙΣΤΟΡΙΚΟ: ήταν 3-step (κλικ αναφορά → κλικ πλευρά → διάλογος απόστασης). **Η ΤΡΕΧΟΥΣΑ ροή είναι 2-step** (2026-07-17): Step 1 κλικ → επιλογή αναφοράς· Step 2 → ο κέρσορας δείχνει την πλευρά και η απόσταση πληκτρολογείται στον καμβά (`CanvasNumericInputStore`). Η πλευρά υπολογίζεται **event-time στο Enter** από το `resolveParallelSide()` (SSoT) — ΟΧΙ στο κλικ. Βλ. changelog 2026-07-17.
