...
- **Status**: v3 IMPLEMENTED — **μηδέν ανοιχτό code-side θέμα**. Τριών-tier retention/delivery **DONE**· code-side bug που έσβηνε σιωπηλά τα `expiresAt`/`timestamp` (§0) **FIXED** (`isPlainObject` guard)· dedup **opt-in ανά call site** (§2.5) **DONE**· rollback αισιόδοξης σφράγισης (§2.5.1) **DONE**· stateful helpers εκτός barrel (§2.5.2) **DONE**· `floorplans/process` → `data_updated` **DONE**· GCP-side TTL policy **ΑΚΟΜΑ ΟΧΙ enabled — η μοναδική εναπομένουσα ενέργεια, GCP-side** (verified 2026-07-20)
