...
- **Status**: v2 IMPLEMENTED — τριών-tier retention/delivery/dedup **DONE**· ο κρυφός code-side bug που έσβηνε σιωπηλά τα `expiresAt`/`timestamp` (§0) **FIXED** σήμερα (`isPlainObject` guard)· GCP-side TTL policy **ΑΚΟΜΑ ΟΧΙ enabled — αυτή είναι πλέον η μοναδική εναπομένουσα ενέργεια** (verified 2026-07-20)
...
- ✅ Ίδιος χρήστης/στόχος/διαδρομή μέσα σε 5 λεπτά → μία γραμμή αντί για N (**με σημαντική εξαίρεση παρακάτω**).
...
- 🔴 **Ανοιχτό ζήτημα, ΟΧΙ λυμένο πρόβλημα: το dedup key αγνοεί το περιεχόμενο σε 3 endpoints.**
  Το `buildAuditDedupKey()` χτίζεται από `companyId|actorId|action|targetId|path` (§2.5). Σε
  `api/search`, `financial-intelligence/portfolio` και `admin/role-management/users` όλα αυτά τα
  components είναι **σταθερά ανά χρήστη** — δεν αλλάζουν ανά κλήση. Το `global_search`/`portfolio`/
  `ctx.companyId` ως `targetId` και το σταθερό `path` δεν κωδικοποιούν *ποια* αναζήτηση ή *ποιο* φόρτωμα
  έγινε· αυτή η διαφοροποίηση ζει αποκλειστικά στο `newValue`/`metadata.reason` (π.χ. το ίδιο το search
  query, το resultCount, το πλήθος project), που **δεν** συμμετέχει στο κλειδί. Άρα μέσα στο 5λεπτο
  παράθυρο, ένας χρήστης που κάνει 20 διαφορετικές αναζήτησεις (ή φορτώνει 20 διαφορετικά dashboards)
  παράγει **μία** γραμμή, όχι 20. Για το global search αυτό δεν είναι καταστολή διπλοτύπων — είναι
  **πραγματική απώλεια τηλεμετρίας** (ποιος έψαξε τι, χάνεται). Σημειώνεται εδώ ως γνωστό ανοιχτό θέμα,
  όχι ως λυμένο.
...
- ⚠️ **Το `floorplans/process` έχει λάθος ετικέτα.** Είναι `POST` route που **μεταλλάσσει** δεδομένα
  (Firestore update στο βήμα processing), αλλά καταγράφεται ως `data_accessed` (§3, γραμμή `access`
  tier). Προϋπάρχον mislabel — δεν εισήχθη τώρα — αλλά τώρα έχει συνέπειες: κληρονομεί 1 μήνα retention
  αντί για 12 (compliance) και lossy async delivery, ενώ πρόκειται για write operation που θα έπρεπε
  ίσως να επιβιώνει σαν business record.
