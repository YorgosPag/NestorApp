...
**(β) GCP-side — το TTL policy δεν ενεργοποιήθηκε ποτέ.** *(ΔΙΟΡΘΩΘΗΚΕ την ίδια μέρα — βλ. παρακάτω)*
...
από τις 2026-06-10 μέχρι τις 2026-07-20.
...
### Και τα δύο σφάλματα έκλεισαν — 2026-07-20

Το code-side (§0.α) με το `isPlainObject` guard· το GCP-side (§0.β) με εκτέλεση της `ttls update` **και για
τα δύο collection groups**, την ίδια μέρα:

```bash
$ gcloud firestore fields ttls list --project=pagonis-87766
--- audit_logs/fields/expiresAt         → ttlConfig.state: ACTIVE
--- system_audit_logs/fields/expiresAt  → ttlConfig.state: ACTIVE
```

**Η σειρά είχε σημασία και ήταν σωστή**: πρώτα το code fix, μετά το TTL enable. Το αντίστροφο θα ήταν
θεατρικό — το Firestore θα έψαχνε ένα πεδίο που δεν υπήρχε σε κανένα document (§«Γιατί έχει σημασία»).

⚠️ **Τα documents της περιόδου 2026-06-10 → 2026-07-20 ΔΕΝ θα διαγραφούν ποτέ από το TTL.** Γράφτηκαν
χωρίς `expiresAt` (§0.α) και το Firestore TTL **αγνοεί** έγγραφα που δεν έχουν το πεδίο — δεν τα θεωρεί
ληγμένα, τα θεωρεί εκτός πολιτικής. Ο αυτοκαθαρισμός πιάνει **μόνο** ό,τι γράφεται από το fix και μετά.
Το υπάρχον υπόλοιπο είναι δοκιμαστικά δεδομένα (pre-production, μοναδικός χρήστης) και απαιτεί χειροκίνητο
recursive delete αν ενοχλεί — **δεν** είναι εκκρεμότητα του ADR.
...
Ο μηχανισμός καταστολής: in-memory `Map<string, number>` (κλειδί → τελευταία εγγραφή σε epoch ms),
module-level. `windowMs === 0` επιστρέφει πάντα `false`. Bounded στα 5.000 entries: όταν γεμίσει, πρώτα
eviction των ληγμένων· αν παραμένει υπερμεγέθης, πλήρες `clear()`.

**Το `clear()` σβήνει και φρέσκα κλειδιά — σκόπιμα.** Η συνέπεια είναι «γράφονται μερικές επιπλέον
γραμμές», δηλαδή η ίδια κατεύθυνση αστοχίας με το opt-in: ποτέ «χάνω σιωπηλά». Πραγματικό LRU θα
απαιτούσε χρόνο *τελευταίας* χρήσης — η `Map` κρατά σειρά *πρώτης* εισαγωγής, άρα «σβήσε τα παλιότερα»
θα ήταν λάθος με τη σημερινή μορφή. Δεν αξίζει η πολυπλοκότητα για cache καταστολής τηλεμετρίας.

### 2.5.1 Αισιόδοξη σφράγιση + rollback (v3)

Η `shouldSuppressDuplicate` σφραγίζει το κλειδί **σύγχρονα, πριν** επιχειρηθεί το write. Αυτό είναι
απαραίτητο: είναι ο μόνος τρόπος να μη γλιστρήσουν δύο ταυτόχρονα requests και τα δύο μέσα από τον
έλεγχο στο ίδιο tick. Το τίμημα ήταν ότι ένα write που **αποτυγχάνει** άφηνε το κλειδί σφραγισμένο για
όλο το παράθυρο: **μία χαμένη γραμμή γινόταν πέντε λεπτά τυφλότητας**.

Λύση — belt-and-suspenders, όχι αλλαγή στρατηγικής:

```ts
async function persistAuditEntry(...): Promise<boolean>   // true = έφτασε στο Firestore
export function releaseDedupKey(key: string): void        // λύσε τη σφραγίδα
```

Η `persistAuditEntry` **δεν πετάει ποτέ** (ένα audit σφάλμα δεν ρίχνει το request που το προκάλεσε), γι'
αυτό η αποτυχία επιστρέφεται ως **τιμή**: και οι δύο έξοδοι αποτυχίας (materialize failure, outer catch)
δίνουν `false`. Το `logAuditEvent` κρατά το `sealedDedupKey` εκτός του dedup block και καλεί
`releaseDedupKey` όταν το write δεν πέτυχε — και στο blocking και στο async μονοπάτι.

⚠️ Γνωστό, αποδεκτό όριο: αν το παράθυρο έχει λήξει και **νεότερη** κλήση ξανασφράγισε το ίδιο κλειδί,
το release μιας παλιάς αποτυχίας σβήνει εκείνη τη φρέσκια σφραγίδα ⇒ μία επιπλέον γραμμή. Ξανά: γέρνει
προς «γράφω παραπάνω», ποτέ προς «χάνω σιωπηλά».

### 2.5.2 Τα stateful helpers ΔΕΝ είναι δημόσια (v3)

Τα `buildAuditDedupKey`, `shouldSuppressDuplicate`, `releaseDedupKey` **αφαιρέθηκαν** από τα barrels
(`audit.ts`, `index.ts`). Μοιράζονται module-level κατάσταση και το `shouldSuppressDuplicate` **τη
μεταβάλλει σε κάθε κλήση**: εξωτερικός καλών θα σφράγιζε κλειδιά χωρίς να γράψει τίποτα, τυφλώνοντας το
πραγματικό audit path για 5 λεπτά. Μοναδικός νόμιμος καταναλωτής: το `audit-core.ts` (import απευθείας
από `'./audit-policy'`). Επαληθεύτηκε με grep ότι **κανείς** δεν τα εισήγαγε από τα barrels.
...
- ✅ Η βάση **αυτοκαθαρίζεται** — το TTL είναι `ACTIVE` και στα δύο collection groups (§0)· ο όγκος audit
  logs σταθεροποιείται σε rolling παράθυρο 24/12/1 μηνών ανάλογα με το tier, αντί για flat 12.
...
- ✅ **Το TTL policy είναι ΕΝΕΡΓΟ στο GCP (§0).** Και τα δύο σκέλη έκλεισαν 2026-07-20: το `expiresAt`
  γράφεται πραγματικά σε κάθε νέο document (§0.α) **και** υπάρχει πλέον πολιτική που το διαβάζει
  (`audit_logs` + `system_audit_logs`, `state: ACTIVE`). Η διατήρηση σε production δεν είναι πλέον άπειρη.
- ⚠️ **Τα documents πριν το fix ΔΕΝ καθαρίζονται.** Ό,τι γράφτηκε 2026-06-10 → 2026-07-20 δεν έχει
  `expiresAt`, και το TTL αγνοεί έγγραφα χωρίς το πεδίο — δεν τα θεωρεί ληγμένα. Θα μείνουν εκεί για
  πάντα εκτός αν διαγραφούν χειροκίνητα. Δοκιμαστικά δεδομένα, οπότε αδιάφορο· καταγράφεται ώστε να μην
  θεωρηθεί ότι «το TTL θα τα φάει».
...
- ✅ **ΔΙΟΡΘΩΘΗΚΕ (v3) — ήταν: «το `shouldSuppressDuplicate` μαρκάρει το κλειδί ΠΡΙΝ το write».** Η
  σφράγιση παραμένει σύγχρονη-πριν (χρειάζεται ως guard έναντι ταυτόχρονων requests), αλλά πλέον
  **ακυρώνεται** όταν το write δεν πετύχει: `persistAuditEntry` επιστρέφει `boolean` και το
  `logAuditEvent` καλεί `releaseDedupKey` σε αποτυχία (§2.5.1). Μία χαμένη γραμμή δεν γίνεται πλέον
  πέντε λεπτά τυφλότητας.
- ⚠️ **Το rollback δεν καλύπτει hard crash.** Αν το instance πεθάνει (SIGKILL, OOM) ενώ το
  fire-and-forget write είναι in-flight, δεν τρέχει κανένα `releaseDedupKey` — αλλά ούτε επιβιώνει το
  in-memory Map, οπότε η σφραγίδα χάνεται μαζί του. Το κενό αφορά **μόνο** το σενάριο «το write απέτυχε
  αλλά η διεργασία ζει», που είναι ακριβώς αυτό που καλύπτεται. Δεν υπάρχει γνωστό υπόλοιπο.
...
- **2026-07-20** — **v3: dedup opt-in ανά call site** — κλείνει το ανοιχτό 🔴 ζήτημα που το v2 είχε
  ...
  - Call sites: `dedupable: true` σε `financial-intelligence/portfolio`, `admin/role-management/users`,
    ...
    **σκόπιμα ΧΩΡΙΣ** σε `api/search` (κάθε αναζήτηση = διακριτό γεγονός), με σχόλιο στο call site
    ...
    ⚠️ Το `search/route.ts` είναι **300/300 γραμμές** (όριο API route, `check-file-sizes.js`). Γι' αυτό
    το σχόλιο εκεί είναι **μονόγραμμο** και δείχνει στο §2.5 αντί να επαναλαμβάνει το σκεπτικό. Η
    επόμενη προσθήκη σε εκείνο το αρχείο θα μπλοκαριστεί — χρειάζεται extract, όχι trim.
  ...
  (Opus)
- **2026-07-20** — **v3 (συνέχεια): rollback αισιόδοξης σφράγισης + καθάρισμα δημόσιας επιφάνειας.**
  Κλείνουν τα #4 και #5 του handoff — το ADR-438 δεν έχει πλέον ανοιχτό code-side θέμα.
  - `persistAuditEntry` → `Promise<boolean>` (και οι δύο έξοδοι αποτυχίας δίνουν `false`)· νέα
    `releaseDedupKey(key)` στο `audit-policy.ts`· το `logAuditEvent` κρατά `sealedDedupKey` και το
    λύνει σε αποτυχία, **και στα δύο** delivery paths (§2.5.1).
  - Αφαιρέθηκαν από τα barrels (`audit.ts`, `index.ts`) τα stateful `buildAuditDedupKey` /
    `shouldSuppressDuplicate` (§2.5.2)· η `releaseDedupKey` δεν προστέθηκε ποτέ εκεί. Grep επιβεβαίωσε
    μηδέν εξωτερικούς καταναλωτές — καθαρή μείωση δημόσιας επιφάνειας, χωρίς call-site αλλαγές.
  - Τεκμηριώθηκε το ολικό `clear()` του `evictStaleDedupEntries` ως **συνειδητή** επιλογή, με το
    σκεπτικό «γιατί όχι LRU» (η `Map` κρατά σειρά πρώτης, όχι τελευταίας, εισαγωγής) — §2.5.
  - Tests: +7 (**54 συνολικά**). 3 unit για `releaseDedupKey`· **2 για το όριο των 5.000 entries**
    (φραγμένη ανάπτυξη + «τα ληγμένα φεύγουν πρώτα, τα φρέσκα επιβιώνουν») — ήταν το μοναδικό ρίσκο
    που ο ίδιος ο κώδικας δήλωνε ως ρίσκο και **δεν** είχε test· 2 observable write-count μέσω του
    public API (αποτυχημένο write ⇒ η επόμενη πρόσβαση γράφεται· επιτυχημένο ⇒ η σφραγίδα κρατάει —
    ο έλεγχος-καθρέφτης, ώστε ένα «release πάντα» να μην περνάει για διόρθωση).
    Επαληθεύτηκε ότι δαγκώνουν: με το rollback αναιρεμένο → **1 κόκκινο**. (Opus)
- **2026-07-20** — 🎯 **GCP TTL ENABLED — το ADR-438 κλείνει.** Εκτελέστηκε η ενέργεια που ήταν ανοιχτή
  από τις 2026-06-10 και που το v1 δήλωνε ψευδώς ως «pending merge»:
  ```bash
  gcloud firestore fields ttls update expiresAt --collection-group=audit_logs        --enable-ttl --project=pagonis-87766
  gcloud firestore fields ttls update expiresAt --collection-group=system_audit_logs --enable-ttl --project=pagonis-87766
  ```
  Και τα δύο επέστρεψαν `ttlConfig.state: ACTIVE`· επαληθεύτηκε με `ttls list` → **2 items** (πριν:
  `Listed 0 items`). **Η σειρά ήταν σωστή**: πρώτα το code fix (§0.α), μετά το enable — το αντίστροφο θα
  ήταν θεατρικό, αφού το Firestore θα έψαχνε πεδίο που δεν υπήρχε πουθενά.
  ⚠️ **Δεν είναι retroactive**: τα ~88 documents της περιόδου του bug γράφτηκαν χωρίς `expiresAt` και το
  TTL **αγνοεί** έγγραφα χωρίς το πεδίο — δεν θα διαγραφούν ποτέ αυτόματα. Δοκιμαστικά δεδομένα
