...
| 2026-07-02 | **Column ghost preview ≡ commit (Giorgio: «το φάντασμα μεγαλύτερο από την κολόνα»).** Το WYSIWYG φάντασμα κολόνας χτιζόταν στο default 400×400, αλλά μόλις γινόταν commit ο proactive auto-sizer (`useProactiveMemberSizing`→`AutoSizeMembersCommand`→`buildColumnSizePatch`) το «μίκραινε» two-way στα 250×250 (EC8 `MIN_COLUMN_DIMENSION_MM`) → ορατή ασυμφωνία μεγέθους. FIX: NEW pure `autoSizeGhostColumnParams` στο `column-preview-helpers.ts` (reuse ΤΟΥ ΙΔΙΟΥ SSoT `suggestColumnSection`+`isColumnAutoSized`, `femMoment` absent→nominal e₀) εφαρμόζεται στους ghost-build κλάδους (awaitingPosition/rotation + slanted) ⇒ ghost size == placed size by construction. Locked (`autoSized:false`)/μη-ορθογώνια → no-op. +1 jest (11/11 `column-preview-helpers`). UNCOMMITTED. |
