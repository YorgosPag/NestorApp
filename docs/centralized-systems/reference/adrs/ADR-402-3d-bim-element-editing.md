...
## Changelog
- **2026-07-22 (Opus 4.8) §rotate-ring-depth — 🐛 FIX: το plan-rotate δαχτυλίδι (rotate-y) ΑΟΡΑΤΟ σε generic solid («Στερεό») (Giorgio, UNCOMMITTED, 🔴 browser verify).** Σύμπτωμα: επιλογή generic solid (ADR-684 box primitive) 3Δ → βέλη + plane handle φαίνονται, αλλά το **κίτρινο δαχτυλίδι περιστροφής ΛΕΙΠΕΙ** (πάντα, όχι μόνο στο drag)· τοίχοι/κολώνες το δείχνουν κανονικά. **Root cause (ΟΧΙ σχετικό με §hide-drag-overlays — ανεξάρτητο, latent):** στο `gizmo-geometry.ts makeRotateRing()` τα rotate rings (x/y/z) χτίζονταν με inline `MeshBasicMaterial({ depthTest: true, depthWrite: true })` — **τα ΜΟΝΑ** gizmo visuals που δεν τηρούσαν το «always-on-top manipulator» contract (όλα τα άλλα handles περνούν από `makeMaterial` = `depthTest:false/depthWrite:false`, `gizmo-builders.ts:38-39`). Το gizmo σχεδιάζεται σε post-FX overlay pass **πάνω** από το ήδη-γραμμένο depth buffer· με `depthTest:true` τα rings γίνονταν occlude από τη scene geometry. **Γιατί μόνο στο generic solid:** το gizmo anchor = **κέντρο bbox** (μέσα στο στοιχείο)· το ring radius είναι screen-constant (~1-2m). Σε **λεπτό** τοίχο/κολώνα το ring προβάλλεται γύρω από το στοιχείο (κενός χώρος → ορατό). Σε **ογκώδες** generic solid το ring πέφτει ΜΕΣΑ στον κύβο → η opaque μπροστινή όψη (γραμμένη νωρίτερα, nearer depth) το έκρυβε ολόκληρο. Ίδιο latent bug θα έκρυβε και τα tilt rings (x/z) σε αρκετά ογκώδη column/wall/beam/slab. **Fix (1 αρχείο):** τα ring materials → `depthTest:false, depthWrite:false` (ευθυγράμμιση με το contract· always-on-top όπως όλα τα άλλα handles). **Tests:** 189/189 gizmo GREEN, tsc SKIP (N.17). Αρχείο: `gizmo-geometry.ts` (+ ADR-402). 1 domain (3D gizmo). ✅ Google-level: YES — ένα SSoT depth contract για ΟΛΑ τα gizmo visuals (μηδέν εξαίρεση), διορθώνει και το latent tilt-ring occlusion. Stage ADR-402 (CHECK 6B/6D). 🔴 browser-verify (επίλεξε generic solid 3Δ → φαίνεται το κίτρινο δαχτυλίδι περιστροφής).
