...
## Changelog
- **2026-07-31** — **Το ghost ζωγράφιζε πάνω στους χάρακες — λανθάνον από την αρχή αυτού του ADR (βλ. ADR-741).** Ο `PreviewCanvas` είναι ΑΛΛΟΣ καμβάς (`z15`) πάνω από τον κύριο (`z10`), όπου οι χάρακες ζωγραφίζονται ως pass **μετά** τις οντότητες· ό,τι γράψει ένα ghost στη ζώνη των 30 px κάθεται **από πάνω τους**. Δεν φαινόταν επειδή τα 8 προϋπάρχοντα ghosts είναι **σύμβολα λίγων εκατοστών** και δεν έφταναν ποτέ την άκρη· το ghost της εικόνας (ADR-736) πιάνει **1/3 του ορατού πλάτους** και τα ακούμπησε αμέσως. 🔑 **Η διόρθωση ΔΕΝ μπήκε στα 9 delegates αλλά στον κοινό harness** (`useCanvasGhostPreview`: `save → clipToDrawingArea → draw → restore`) — ένα σημείο, ισχύει αυτόματα για **και τα 9**, και για κάθε επόμενο. Το ορθογώνιο έρχεται από το νέο SSoT `rendering/core/drawing-area.ts`· **δεν κλωνοποιήθηκε** το clip του `LayerRenderer` (N.18) — απορροφήθηκε. Το `try/finally` γύρω από το delegate προστατεύει και από ασύμμετρη στοίβα `save/restore`. Mutation-verified: αφαίρεση του clip ⇒ 3 κόκκινα.
- **2026-07-31** — **Τρίτο χτύπημα του ίδιου bug — και ο άξονας που το προκάλεσε.** Το `attach-image` (ADR-736 §5.4 «Εικόνα») ζωγράφιζε ghost και ο `processDrawingHover` το έσβηνε στο **ίδιο** `mousemove` — μετρημένο με `getImageData` στον PreviewCanvas: **3 `drawImage`, 0 ορατά pixels**. Το εργαλείο δεν περνούσε το `toolOwnsPreviewCanvas()` γιατί **δεν υπήρχε καθόλου** στο `TOOL_CREATES_ENTITY`. 🔑 **Το μάθημα δεν είναι «ξεχάστηκε μια εγγραφή» — είναι ότι ο άξονας του predicate δεν καλύπτει όλη την οικογένεια:** το `toolOwnsPlacementGhost` παράγεται από τον **τύπο οντότητας**, σωστό για τα 16 `mep-*` που μοιράζονται μηχανή, αλλά **αδύνατο** όταν εργαλεία **με** ghost και εργαλεία **χωρίς** ghost δίνουν τον ίδιο τύπο — `'image'`: το `attach-image` έχει mount, τα 4 catalog entourage (έπιπλα/άνθρωποι/οχήματα/φυτά) **δεν** έχουν (ADR-654). Προστέθηκε λοιπόν `PLACEMENT_GHOST_OWNER_TOOLS` **ανά tool id**, σε OR με τον τύπο-άξονα. +2 anchor tests στο `tool-owns-preview-canvas.test.ts` (mutation-verified), εκ των οποίων ένα κλειδώνει ρητά ότι τα 4 αδέλφια **δεν** είναι ιδιοκτήτες — ώστε όταν αποκτήσουν ghost, το test να κοκκινίσει και να μπουν στο set αντί να ξαναγραφτεί το bug. Λεπτομέρειες: ADR-736 §5.4.1.
