...
- ✅ **CSV + Excel** (D6=B) via `xlsx@0.18` (Apache 2.0, +1 dep license-safe)
- Endpoint `GET /api/procurement/spend-analytics/export?format=csv|xlsx&{...filters}`
- Excel multi-sheet: Overview · By Vendor · By Category · By Project
...
| Phase B1 — Aggregator + API + indexes | ✅ DONE 2026-05-04 | 6 (5 NEW + 1 MODIFY ADR) | TBD | `spendAnalyticsAggregator.ts` (288 LOC) · `quarter-helpers.ts` (85 LOC, incl. Athens TZ D25) · `spend-analytics.ts` RBAC (D10) · `route.ts` API GET (78 LOC, withHeavyRateLimit D15) · test suite 17 pure-helper assertions (D26). No new Firestore indexes needed — existing `companyId+isDeleted+dateCreated` index covers range query. Vendor names resolved via `db.getAll()` batch. accountant project-role support deferred → Phase H. |
| Phase B2 — Export endpoint | ⏸️ READY | — | — | After B1 |
| Phase C — Hook + URL serialization + event bus | ⏸️ READY | — | — | After B1 |
