...
| 2026-06-24 | Phase 6 | Opus 4.8 | **`mergeDocsIntoScene` SSoT rollout — κεντρικοποίηση ΟΛΩΝ των «Firestore snapshot → scene diff-merge» loops.** Πριν, ΚΑΘΕ per-entity persistence hook (~26) είχε **copy-pasted** το ΙΔΙΟ διφφ-merge loop (selective-skip dirty/pending/grace, add/update/drop, ADR-397 baseline seed, `'remote-echo'` write). Επεκτάθηκε ο γενικός `hooks/data/merge-docs-into-scene.ts` SSoT (που column+hatch χρησιμοποιούσαν ήδη) με **optional callbacks**, byte-equivalent defaults: `docToEntity(doc, existing, ctx)` (+`null`-skip), `prepareContext`, `differs` (build-once `getCandidate` memo), `seedExtraBaseline`, `shouldDropOrphan`. **Tier 1 (9, params-only):** beam, slabOpening, railing, foundation, furniture, floorplanSymbol, thermalSpace, spaceSeparator, wallCovering → inline call (mirror hatch). **Tier 3 (7 MEP connector-projection, ADR-408):** fixture, boiler, radiator, waterHeater, underfloor, manifold, electricalPanel → NEW SSoT `hooks/data/mep-connector-projection-merge.ts` (`projectMepConnectorsOntoFresh`, reuse `projectConnectorSystemIds`) ως `docToEntity(doc, existing)` + `differs` στο projected candidate (anti-ping-pong). **Tier 2 (4 type-resolution, ADR-412/421/440):** wall+opening (thin adapters `mergeWall/OpeningDocsIntoScene` delegate) + slab+roof (inline) → `differs` = `wallEntityDiffersFromDoc`/`openingEntityDiffersFromDoc`/`slabEntityDiffersFromDoc`/`roofEntityDiffersFromDoc` (reuse, ΟΧΙ re-implement) + `seedExtraBaseline` (δεύτερο type-link map) + opening `prepareContext` host-wall lookup + `docToEntity→null` (ADR-440 host-missing skip). **Tier 4:** mepSegment → `shouldDropOrphan` (κρατά un-persisted DXF segments). **Stair = semi-special** (διαφορετικό return contract `{entities,mutated}` + ADR-402 dirty-gated seed-before-existing + editingBy compare· ρητό SSoT-note στο `stair-snapshot-merge.ts`, ΟΧΙ migrated). **ΕΥΘΥΓΡΑΜΜΙΣΕΙΣ (drift fix, ΟΧΙ regression):** slabOpening+slab seed-άρουν πλέον το ADR-397/412 baseline στο snapshot loop (έλειπε → loaded entities ήταν treated as new στο πρώτο edit)· wall/opening deleted-doc handling → canonical top-skip (delete-wins, unreachable στην πράξη αφού deleted = no live doc). Generic test: 12→19 jest· NEW mep helper: 4 jest· 113/113 hooks/data GREEN. tsc SKIP (N.17 OOM). UNCOMMITTED. 🔴 browser-verify (draw/edit/reload κάθε τύπου· MEP no idle-ping-pong· opening host-missing retry) + commit (σταδιακά ανά Tier· CHECK 6B/6D→stage ADR-040+390). |
...
| 2026-06-24 | Phase 7 | Opus 4.8 | **Ενοποίηση delete-path (ribbon «Διαγραφή» ↔ keyboard Delete) σε ΕΝΑ command-based SSoT — ΦΑΣΗ 1.** Πριν: το ribbon «Διαγραφή» 6 δομικών bridges (column/beam/wall/slab/roof/opening) raw-emit-άρε `bim:X-delete-requested` → **ΟΧΙ undoable, ΚΑΝΕΝΑ cascade** (orphan openings, ADR-401 host-detach, MEP) → παράλληλο μονοπάτι divergent από το keyboard Delete (`useSmartDelete` PRIORITY 3). Το race του «παγωμένου πεδίλου» (optimistic-update band-aid σε `useColumnPersistence`/`useWallPersistence`) ήταν σύμπτωμα αυτού του διπλότυπου. **FIX (SSoT, Giorgio audit — εξαγωγή ΟΧΙ copy):** NEW `hooks/canvas/delete-entities-core.ts` (`deleteEntitiesById(ids, {adapter, sceneEntities, executeCommand})`) — ο πυρήνας του PRIORITY 3 εξήχθη αυτούσιος (cascade prompt `requestWallCascadeDelete` + `collectBimDeleteIds` + `DeleteEntity/MultipleEntitiesCommand` + ADR-408 MEP `CompoundCommand` + `executeCommand` + `emitBimDeleteEvents`)· `useSmartDelete` PRIORITY 3 → καλεί τον πυρήνα (μηδέν regression, selection-clear μένει στον caller). NEW `ui/ribbon/hooks/useRibbonEntityDelete.ts` — ΕΝΑ shared hook για τα 6 bridges (φτιάχνει adapter+scene, καλεί τον πυρήνα, `clearByType('dxf-entity')` κλείνει το contextual tab)· κάθε bridge ήδη είχε `{levelManager, universalSelection}` + `useCommandHistory().execute` (μηδέν νέα plumbing — μόνο widened `clearByType` στο Pick). **Αποτέλεσμα:** ribbon delete = undoable (Ctrl+Z) + cascades + zero race (το command αφαιρεί σύγχρονα τη σκηνή ΠΡΙΝ το `emitBimDeleteEvents` → οι coalesced structural reactions διαβάζουν φρέσκια σκηνή → το race εξαλείφεται **δομικά**, το band-aid #1 γίνεται redundant). Το wall ribbon delete πλέον προτρέπει για orphan openings (ήταν bug). Τα `deleteX` persistence hooks ΑΜΕΤΑΒΛΗΤΑ (η scene-removal τους = idempotent redundant no-op). 8 jest νέα (delete-entities-core) + 20 column-bridge + 27 opening/cascade GREEN, μηδέν regression. tsc SKIP (N.17 OOM)· type-safety verified χειροκίνητα (caller `useDxfViewerRibbon` περνά πλήρες `UniversalSelectionHook`). UNCOMMITTED. 🔴 browser-verify [(α) ribbon delete → Ctrl+Z επαναφέρει· (β) wall delete σβήνει openings· (γ) column delete από combined → πέδιλο reconcile· «ΔΕΣ ΒΑΣΗ»] + commit. **ΦΑΣΗ 2 (endgame, ΧΩΡΙΣΤΑ):** `deleteX` → Firestore-only (αφαίρεση scene filter + band-aid #1). |
