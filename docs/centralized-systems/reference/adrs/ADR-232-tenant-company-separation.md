...
- 🔗 **[ADR-787](./ADR-787-multi-organization-platform.md) — η πολυ-οργανισμική πλατφόρμα.**
  Ο διαχωρισμός αυτού του ADR **είναι** ο μηχανισμός «**πελάτης εκτός πλατφόρμας**»
  (ADR-787, αρχή **Α4**): το `linkedCompanyId` δείχνει σε **επαφή**, άρα ένα έργο μπορεί να
  δηλώνει εντολέα εταιρεία που **δεν έχει λογαριασμό και δεν θα αποκτήσει ποτέ**.
  ⚠️ **ΜΗΝ ξανα-ενοποιήσεις τα δύο πεδία** και **μην** θεωρήσεις ότι το `linkedCompanyId`
  προϋποθέτει μισθωτή — αυτό θα ακύρωνε την αρχή Α4 σιωπηλά.
- **ADR-210** — enterprise ID standardization· εκεί ζει το `bootstrap-company`, το μοτίβο
  «φάντασμα → πραγματικό» που το ADR-787 §5 (Κ-5) ονομάζει **προαγωγή επαφής σε χώρο**.
