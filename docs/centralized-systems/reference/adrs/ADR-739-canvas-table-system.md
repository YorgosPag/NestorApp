...
- 🔴 **ΑΝΕΠΑΛΗΘΕΥΤΟ ΖΩΝΤΑΝΑ**: το CDP κόλλησε (`connectOverCDP: Timeout 180000ms`) και raw
  probe έδειξε **μία καρτέλα viewer που δεν απαντά** (`Runtime.evaluate` timeout) και **μία
  χωρίς τον πίνακα**. Καμία συνθετική χειρονομία δεν έγινε — με ανοιχτές λαβές θα ρίσκαρε
  αλλοίωση. **Ο πρώτος έλεγχος είναι του Giorgio.**
