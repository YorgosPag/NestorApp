...
- **Status**: ✅ **ΥΠΟ ΥΛΟΠΟΙΗΣΗ — Φ.Α/Β/Γ ΚΛΕΙΣΤΕΣ, Φ.Δ ΒΗΜΑΤΑ 1-9 ΚΛΕΙΣΤΑ** (2026-08-02).
  **Φ.Δ βήμα 9 ✅** **εισαγωγή/διαγραφή γραμμών & στηλών από τις ζώνες δείκτη** (§27): δεξί
  ...
  στην ίδια διαδρομή commit. 🔴 **Ανεπαλήθευτο ζωντανά** (§27.9).
  **Φ.Δ βήμα 8 ✅** **επιλογή περιοχής + αντιγραφή/επικόλληση** (§26): `Shift+βέλος/κλικ`,
  ...
  🔴 **Ανεπαλήθευτο ζωντανά ΟΛΟΚΛΗΡΟ** (§26.13): η επέκταση Chrome δεν ήταν συνδεδεμένη.
...
### 26.13 🔴 Τι ΔΕΝ επαληθεύτηκε ζωντανά (δηλωμένο, όχι κρυμμένο)
**Τίποτα από το βήμα 8 δεν περπατήθηκε στον browser.** Ο dev server έτρεχε, αλλά η επέκταση
Chrome **δεν ήταν συνδεδεμένη** («Browser extension is not connected»), οπότε καμία από τις
μετρημένες συνταγές του handoff δεν μπόρεσε να εκτελεστεί. Εκκρεμούν, με αυτή τη σειρά:
1. `Shift+βέλος` / `Ctrl+A` / `Shift+κλικ` — ότι η περιοχή **ζωγραφίζεται** και ότι οι ζώνες
   `A B C` / `1 2 3` ανάβουν **ολόκληρες**·
2. `Ctrl+C` → επικόλληση σε **πραγματικό Excel** και αντίστροφα (η μόνη απόδειξη του TSV
   end-to-end· τα tests καλύπτουν τη σειριοποίηση, **όχι** το πρόχειρο του λειτουργικού)·
3. Επικόλληση που **δεν χωράει** — ότι το μήνυμα εμφανίζεται με σωστούς πληθυντικούς·
4. **ΕΝΑ** `Ctrl+Z` μετά από επικόλληση πολλών κελιών·
5. Το μέγεθος στη **γραμμή κατάστασης** υπό **συγχώνευση** (η αφελής αφαίρεση θα έλεγε λάθος)·
6. Και τα τρία σκέλη **υπό περιστροφή**, μαζί με το κείμενο του σκέλους Β΄.
...
| `table-header-menu.test.tsx` (15) | θύρα δηλώνεται/αποσύρεται· **screen → world → frame → ζώνη** βρίσκει τη σωστή στήλη/γραμμή· **μία εντολή** ανά ενέργεια με **πραγματική** `ICommand`· δρομέας σε κελί που υπάρχει· αριστερό κλικ ⇒ όλη η στήλη/γραμμή· 🔴 το **ανοιχτό** μενού φέρει το σημάδι συνεδρίας |
...
- 🔴 **ΑΝΕΠΑΛΗΘΕΥΤΟ ΖΩΝΤΑΝΑ**: όπως και το βήμα 8, δεν έγινε έλεγχος στον browser. Εκκρεμούν
  ονομαστικά: (α) το μενού ανοίγει στο σωστό σημείο πάνω σε **στραμμένο** πίνακα· (β) οι ζώνες
  **μένουν** ορατές όσο το μενού είναι ανοιχτό· (γ) η εστίαση επιστρέφει στο κελί μετά το
  κλείσιμο· (δ) `Ctrl+Z` επαναφέρει με **ένα** βήμα· (ε) ο τίτλος καλύπτει όλο το πλάτος μετά
  από «εισαγωγή στήλης δεξιά»· (στ) επιβίωση σε reload.
