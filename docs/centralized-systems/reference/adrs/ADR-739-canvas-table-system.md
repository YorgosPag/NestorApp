...
- **Status**: 🚧 **ΥΠΟ ΥΛΟΠΟΙΗΣΗ** — τελευταία ενότητα **§70** (2026-08-29).
  *(⚠️ Η γραμμή έγραφε **§67.10 / 2026-08-10** ενώ το σώμα είχε ήδη φτάσει στο **§69** (28/08) —
  μπαγιάτικη κατά μία ενότητα. Ο δείκτης «τελευταία ενότητα» είναι χειρόγραφος και **καμία πύλη
  δεν τον επαληθεύει**: αν ξαναπαλιώσει, δεν θα το πει κανείς.)*
...
🔧 **2026-09-11 — ψευδώς θετικό του εσωτερικού περάσματος, διορθωμένο.** Μέσα σε αρχείο
οικογένειας, το σύμβολο ενότητας ενός δείκτη σε ADR **εκτός** οικογένειας (το «ADR-787 §5.3 ζ»
μέσα στο ADR-777) διαβαζόταν ως αυτο-αναφορά του αρχείου ⇒ ψευδές `prose-only` που μπλόκαρε
**κάθε** commit του δέντρου, αφού η πύλη διαβάζει τα tracked αρχεία από τον **δίσκο**, όχι το
index. Πλέον το `scanReferences` αγνοεί στο εσωτερικό πέρασμα κάθε `ADR-NNN` ακολουθούμενο από
σύμβολο ενότητας. Άγκυρα **Κ13** (`check-adr-section-refs.test.js`)· μετάλλαξη χωρίς τη διόρθωση
⇒ Κ13 κόκκινη. ⚠️ Το παράθυρο 24 χαρακτήρων μετά από ρητό δείκτη **μένει** ως έχει: η αλλαγή του
μετακινεί μετρήσεις σε όλο το δέντρο και δεν χρειάστηκε.
