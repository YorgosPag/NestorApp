...
### 20.8 ΑΝΟΙΧΤΟ — «ο πίνακας γεννιέται τεράστιος»: **δεν φταίει ο πίνακας**
...
**Άμεση ανακούφιση, χωρίς κώδικα**: το widget κλίμακας της κορδέλας (`DrawingScaleWidget` ->
`setDrawingScale`) θέτει `drawingScaleUserSet = true`, που **μπλοκάρει** το auto-fit από εκεί και
πέρα. Ορίζοντας π.χ. 1:100 πριν τη δημιουργία, ο πίνακας γεννιέται 12 m — κανονικός δίπλα σε
κτίριο.

**Οι τρεις υποψήφιες μόνιμες λύσεις** (απόφαση σχεδίασης, ΔΕΝ υλοποιήθηκε):

- (α) **Κλίμακα ανά πίνακα** — `TableEntity.annotationScale?`, με προεπιλογή στη δημιουργία που
  δίνει αναγνώσιμο μέγεθος στην τρέχουσα προβολή, και πεδίο στις Ιδιότητες. Μικρή αλλαγή, αλλά
  **προσθέτει πεδίο μοντέλου** (σειριοποίηση + migration).
- (β) **Δημιουργία σε μοντελικό μέγεθος** (fit-to-view) αντί sheet-mm × drawingScale, με την
  υπονοούμενη κλίμακα να αποθηκεύεται. Καμία αλλαγή στη γέφυρα, αλλά ο πίνακας παύει να είναι
  αυστηρά annotative.
- (γ) **Οι πίνακες ζουν σε φύλλο/layout** — η απάντηση του AutoCAD, του Revit και του ArchiCAD,
  και το **ήδη σχεδιασμένο τέταρτο backend του §3**. Η σωστή μακροπρόθεσμη λύση· η ακριβότερη.
...
κείμενο και κάθε άλλη σημείωση.
...
- **2026-08-01** — **Φ.Δ βήμα 2 (συνέχεια): ΠΛΟΗΓΗΣΗ ΚΕΛΙΩΝ ΣΑΝ EXCEL** (νέο §20). Ο πίνακας
  ...
  **ΑΝΟΙΧΤΟ** (§20.8): «ο πίνακας γεννιέται τεράστιος» — η αλυσίδα μετρήθηκε ολόκληρη και είναι
  **αριθμητικά σωστή** (σκηνή 1915 m -> auto fit-to-A3 -> `drawingScale = 5000` -> 120 sheet-mm =
  ...
  λύσεις καταγράφηκαν· **καμία δεν υλοποιήθηκε** (απαιτεί απόφαση σχεδίασης).
