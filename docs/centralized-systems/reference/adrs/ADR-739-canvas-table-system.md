...
- **Status**: ✅ **ΥΠΟ ΥΛΟΠΟΙΗΣΗ — Φ.Α/Β/Γ ΚΛΕΙΣΤΕΣ, Φ.Δ ΒΗΜΑΤΑ 1-9 ΚΛΕΙΣΤΑ** (2026-08-02).
  ...
  **Φ.Δ βήμα 8 ✅** **επιλογή περιοχής + αντιγραφή/επικόλληση** (§26): `Shift+βέλος/κλικ`,
  ...
  undo, μέγεθος υπό συγχώνευση, κείμενο που **γέρνει** με τον πίνακα. 🔴 **ΜΕ ΕΝΑ ΕΥΡΗΜΑ**
  (§26.15): **οποιοδήποτε κλικ στον καμβά κλείνει τη συνεδρία** ⇒ «απλό κλικ μετακινεί κελί»
  και «`Shift+κλικ` = δεύτερη γωνία» **δεν λειτουργούν**. Η κούρσα του §4.1 έχασε.
...
| 2 | Περιοχή + ζώνες | `Shift+βέλος` ✅ (μεγαλώνει **και** συρρικνώνεται)· `Ctrl+A` ✅ · ζώνες `A B C`/`1..5` ανάβουν **ολόκληρες**, ενεργό κελί με ξεχωριστό περίγραμμα (screenshot). 🔴 `Shift+κλικ` **ΟΧΙ** — δες §26.15 |
...
### 26.15 🔴 ΕΥΡΗΜΑ — **οποιοδήποτε κλικ στον καμβά κλείνει τη συνεδρία**
