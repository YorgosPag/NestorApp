...
|---|---|---|
| **Σειριοποίηση** (`model.cells` = `Map`, δεν επιβιώνει `JSON.stringify`) | **Φ.Δ** | Δεν σειριοποιείς ό,τι δεν μπορείς ακόμη να **δημιουργήσεις**. ⚠️ Μέχρι τη Φ.Δ **μην υποθέσεις ότι ένας πίνακας επιβιώνει reload** |
| Εργαλείο δημιουργίας / επεξεργαστής κελιού / καρτέλα ribbon | Φ.Δ | §13 — η Φ.Γ κλείνει αυτοτελώς |
