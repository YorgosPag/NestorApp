...
- **Βήμα 4 (εξαγωγή)**: `italic` → oblique/1071 στο STYLE· `underline` → `%%u`· `fontFamily` →
  απαιτεί το κατηγόρημα «SHX ή TrueType;» που η Φ1 άφησε ρητά ως χρέος (γυμνή οικογένεια
  εκτός Arial διαβάζεται ως ανύπαρκτο `.shx`). **Το `RunStyle` του text-engine έχει ήδη
  `italic`/`underline`** και το `extendedFont` του DXF writer έχει ήδη υποδοχή `italic` —
  άρα η διαδρομή υπάρχει, δεν εφευρίσκεται.
