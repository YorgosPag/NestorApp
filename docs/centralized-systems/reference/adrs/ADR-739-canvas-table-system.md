...
- **Ζωντανή επαλήθευση: ΜΕΡΙΚΗ.** Επαληθεύτηκαν στην οθόνη (μετρημένα από το DOM, όχι από
  στιγμιότυπο): η γραμμή αποδίδει **10** χειριστήρια με σωστά `aria-label` · το μενού
  γεμίσματος ανοίγει με **πέντε** ζώνες · **και οι δύο** γραμμές `menuitemradio` υπάρχουν και
  σε μεικτό άξονα καμία δεν είναι `aria-checked` · η ζώνη εγγράφου προσφέρει `#ededed`,
  `#ff00ff` **και `#ffffff`** (η ασυμμετρία του λευκού, ζωντανά) · το πλέγμα έχει **78** κελιά
  με πρώτο το «Ουδέτερο ανοιχτό · **ACI 255**» (δηλαδή το πλέγμα **γεμίσματος**) · μια στήλη
  ήταν όντως βαμμένη στον καμβά, άρα η διαδρομή εφαρμογής δουλεύει άκρη-σε-άκρη.
  **ΔΕΝ επαληθεύτηκαν**: το «Κανένα γέμισμα» ⇒ διαφανές **στον καμβά** · η ίδια η διόρθωση του
  §35.8 στην οθόνη · `Ctrl+Z` · επιβίωση σε reload · εξαγωγή DXF. Το working tree μοιραζόταν με
  την ADR-750, το Fast Refresh ξαναέχτιζε συνεχώς και το εργαλείο του browser σταμάτησε να
  κρατά καρτέλα. Δηλωμένο, όχι κρυμμένο.
...
- **2026-08-03 (ιστ)** — **§35: Φ.Ε/Φ4β — ΧΡΩΜΑ ΓΕΜΙΣΜΑΤΟΣ + «ΚΑΝΕΝΑ ΓΕΜΙΣΜΑ»**, το δεύτερο
  ...
  **είναι** ισχυρισμός). Κόστος: το component πήγε στις **501** ⇒ **δεύτερη εξαγωγή, ποτέ trim**
  ...
  ✅ · ssot-imports ✅ · i18n-types ✅ · όλα < 500. ⚠️ **Ζωντανή επαλήθευση ΜΕΡΙΚΗ** — τι
  μετρήθηκε και τι όχι: §35.10.
