...
### 28.12.7 🔴 ΕΚΚΡΕΜΕΙ Η ΖΩΝΤΑΝΗ ΕΠΑΛΗΘΕΥΣΗ

Ο κώδικας είναι κλειστός και δοκιμασμένος· **η οθόνη δεν το έχει δει**. Το §28.9.8 και το
§31.8 είναι η υπενθύμιση γιατί αυτό δεν είναι διατύπωση ευγενείας: εκεί τα δεδομένα ήταν
**όλα σωστά μέσα στο αρχείο** και **τίποτα δεν φαινόταν**, με 2.642 tests πράσινα.

Τι πρέπει να δει ο ιδιοκτήτης, με αυτή τη σειρά:
1. Δεξί κλικ σε **B** ⇒ **δύο** επιφάνειες, με κενό ανάμεσα, το toolbar **πάνω**.
2. Πάτημα «Β» ⇒ η στήλη γίνεται έντονη **και το μενού μένει ανοιχτό** (το ρίσκο 1).
3. Δεύτερο πάτημα, χωρίς νέο δεξί κλικ ⇒ δουλεύει (η εστίαση δεν κλάπηκε).
4. `A↑` σε στήλη με τίτλο ⇒ **τίποτα δεν μικραίνει**.
5. `←/→` ⇒ κινείται μέσα στο toolbar· η **επιλεγμένη οντότητα δεν μετακινείται**.
6. Δεξί κλικ κοντά στην **κορυφή** της οθόνης ⇒ το toolbar δεν βγαίνει εκτός.
7. `Ctrl+Z` **μία** φορά ⇒ αναιρεί ένα πάτημα.
8. Επαναφόρτωση ⇒ η μορφοποίηση επιβιώνει· εξαγωγή DXF ⇒ **τυπώνεται** (βήματα 1-4).
