...
- **Δύο κοινά αρχεία είναι οριακά στο όριο των 500** (`TableHeaderContextMenu.tsx` **498**,
  `use-table-header-menu.ts` **481**). Και τα δύο τα γράφει ταυτόχρονα η ADR-750· **η εξαγωγή
  δεν έγινε επίτηδες** — δομική αναδιάταξη σε αρχείο που γράφουν δύο agents είναι σύγκρουση, όχι
  βελτίωση. Ανήκει σε δικό της βήμα μόλις κλείσει η ADR-750: το φυσικό σημείο τομής είναι τα
  `Καθαροί βοηθοί` του hook (`resolveAxisFormatSnapshot` / `textColorStateOf` / `toggleStateOf` /
  `resolveHeaderState`) προς `table-header-format-snapshot.ts`.
