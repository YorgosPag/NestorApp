...
| 2026-07-17 | **Drift καταγράφηκε (N.0.1 — ο κώδικας κερδίζει).** Αυτό το ADR σιωπούσε για δύο αλλαγές που είχαν ήδη γίνει: (α) το `RotateEntityCommand` **extends `SnapshotTransformCommand`** από το **ADR-507 §8** (in-place spine: snapshot/restore/follower-cascade) — το ίδιο το αρχείο το επικαλείται στο docblock του, το ADR-188 δεν το ανέφερε ποτέ· (β) το `copyMode` ctor flag γεννήθηκε στο **ADR-357 Φ12** και επεκτάθηκε στο **ADR-561 EXT** (Ctrl-endpoint hinge) — ποτέ δεν καταγράφηκε εδώ. |
| 2026-07-17 | **`copyMode` ΑΦΑΙΡΕΘΗΚΕ (ADR-507 §8).** Το rotate-with-copy δεν είναι πια flag του command: είναι `CloneWithTransformCommand` (Revit `ElementTransformUtils.CopyElements` — clone με το transform ψημένο μέσα), μέσω `createRotateCommand({copy: true})`. Το `RotateEntityCommand` είναι πλέον **καθαρά in-place** (χωρίς `execute/undo/redo` overrides — κληρονομεί τη base) και το `computeUpdates` delegate-άρει στο `buildRotatePatch` SSoT. Διορθώθηκαν 3 bugs που έφερνε το flag: το copy δεν έκανε BIM identity/broadcast (clone εξαφανιζόταν στο Firestore), το redo mint-αρε φρέσκο id (orphan doc), και το `mergeWith` **σιωπηλά έριχνε το `copyMode`** (5 args, 6ο default `false` → merged rotate-copy γινόταν in-place). Βλ. **ADR-507 §8** (2026-07-17) για πλήρη ανάλυση. |
| 2026-07-21 | **ORTHO (F8) στον ορισμό ΚΑΤΕΥΘΥΝΣΗΣ ΑΝΑΦΟΡΑΣ (η πορτοκαλί/χρυσή διακεκομμένη γραμμή).** Ο Giorgio: «ο ορισμός της πορτοκαλί διακεκομμένης γραμμής δεν σέβεται το ORTHO όταν επιλέγω Περιστροφή». **Root cause:** η φάση `awaiting-reference` ήταν **RAW** και στα δύο chokepoints — το commit (`useRotationTool.handleRotationClick`) έθετε `referencePoint`/`startAngleRef` από τον ωμό cursor, και το preview (`useRotationPreview.draw`) ζωγράφιζε τη rubber-band (`drawRubberBandLine`, `#FFD700` dashed) από τον ωμό `effectiveCursor` — το `if (isReferencePhase) return` έκοβε **πριν** το ORTHO/POLAR tracking που έτρεχε **μόνο** στη φάση γωνίας. Έτσι η κατεύθυνση 0° οριζόταν διαγώνια ακόμη κι με ORTHO ON, μολύνοντας κάθε επόμενη γωνία (`angle = rawAngle − startAngleRef`). **Fix (reuse SSoT, μηδέν διπλότυπο):** και τα δύο σημεία περνούν πλέον τον cursor από το **ήδη υπάρχον** `resolveOrthoPolarStep(point, pivot, {ortho, polar})` (το ΙΔΙΟ SSoT που χρησιμοποιεί η φάση γωνίας + το hot-grip rotation· `hardOrtho` dominant-axis lock, no-op όταν και τα δύο toggles OFF) → η χρυσή γραμμή, η δεσμευμένη κατεύθυνση αναφοράς και το περιστρεφόμενο ghost συμφωνούν (WYSIWYG). Η rubber-band κλειδώνει τώρα και στις ΔΥΟ φάσεις (στη γωνία συμπίπτει με την POLAR tracking γραμμή — τέλεια επικάλυψη). Tests: `useRotationTool.activation.test.ts` **+2** (ORTHO ON → H-lock `{10,0}`· OFF → raw `{10,3}`), 7/7 GREEN. Αρχεία: `hooks/tools/useRotationTool.ts`, `hooks/tools/useRotationPreview.ts`. |
