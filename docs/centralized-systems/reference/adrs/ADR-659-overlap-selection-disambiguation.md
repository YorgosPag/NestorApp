...
| Imported-mesh όνομα (user `label` / source `nodeName`) (2026-07-24) | `bim/entities/imported-mesh/imported-mesh-param-access.ts` → `readImportedMeshField()` (ADR-683 SSoT reader — reuse, ΜΗΝ ξαναγραφτεί) |
...
| Named-entity identity (imported-mesh/block instance name) + `#n` duplicate ordinals (2026-07-24, Phase 3) | `systems/selection/candidate-named-identity.ts` |
...
| 2026-07-24 | **Phase 3 — named-entity rows (imported-mesh / block)** | **Πρόβλημα:** τα επικαλυπτόμενα εισαγόμενα πλέγματα (ADR-683, π.χ. τα 5 μέρη μιας καρέκλας) έδειχναν όλα `imported-mesh   default` (raw τύπος + layer), αγνοώντας το **όνομα** — ενώ οι μεγάλοι παίκτες (Revit/ArchiCAD/MAXON C4D/Figma) δείχνουν το **instance name**, όχι την κλάση. Επιπλέον `entityTypeLabel('imported-mesh')` επέστρεφε `''` (το `normalizeEntityType` δεν γνώριζε τον τύπο) → ωμό string. **Αποφάσεις Giorgio:** (κανόνας ονόματος) χωρίς μετονομασία → όνομα πηγής (`nodeName`)· με custom `label`≠`nodeName` → **και τα δύο** (όνομα χρήστη primary + όνομα πηγής secondary). (tiebreaker) αυτόματο `#n` **μόνο** σε γνήσια διπλότυπα (ίδια rendered γραμμή) — C4D `.1/.2`. (εύρος) γενικός/επεκτάσιμος μηχανισμός· καλωδιωμένα **imported-mesh** (πλήρες) + **block** (`name`)· furniture (catalog `assetId`) & generic-solid (shape `kind`) **δεν** έχουν instance name → μένουν στο generic fallback (χωρίς μαντεψιά, ADR-683 §3). **Design (two-stage, ADR-040):** νέο pure SSoT `systems/selection/candidate-named-identity.ts` — `buildNamedIdentitySemantics(entity)` (dispatcher imported-mesh/block· **reuse** `readImportedMeshField` SSoT reader, `displayName=label ?? nodeName`, `sourceName=nodeName` όταν διαφέρει· `unwrapDxfSubEntity` no-op για direct shape) + `assignDuplicateOrdinals(candidates)` (build-time post-pass στο `buildCandidatesFromHits`, ομαδοποίηση κατά `entityType␟displayName␟sourceName`, `#n` σε ομάδες ≥2, immutable). `candidate-label.ts`: `CandidateSemantics.named?` + `CandidateLabelInput.duplicateOrdinal?` + νέο render branch (πριν το generic fallback) με `t('selectionCycling.duplicateOrdinal',{index})`. `SelectionCyclingStore.ts`: `CyclingCandidate.duplicateOrdinal?` + post-pass. `status-bar-text-generator.ts`: `normalizeEntityType`+`EntityTypeKey` γνωρίζουν πλέον `imported-mesh`→`importedMesh` (διορθώνει ΚΑΙ status bar/ARIA/paramless generic fallback). i18n: `bim3d.entityTypes.importedMesh` + `selectionCycling.duplicateOrdinal` (`"#{index}"`, ICU single-brace· el+en). +22 tests (νέο `candidate-named-identity.test.ts` 16 + `candidate-label.test.ts` 31→37· 47/47 πράσινα). jscpd:diff καθαρό (0 clones / 4 files). |
