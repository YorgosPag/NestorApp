...
| Πεδίο | Τιμή |
|---|---|
| **Status** | 🟢 **Φ0 + Φ1 IMPLEMENTED** 2026-07-28 — αγκυρωμένη αριστερά παλέτα με αλλαγή πλάτους· οι δύο καμβάδες ακολουθούν. **Φ2 (δεξιά) / Φ3 (αιωρούμενη) / Φ4 (πάνω από τον κλάδο): NOT IMPLEMENTED.** Εκκρεμεί **ζωντανή μέτρηση FPS** (§4.3 / §11.1) |
| **Date** | 2026-07-28 |
...
1. **Απόδοση §4.3 — ΠΑΡΑΜΕΝΕΙ ΑΝΟΙΧΤΟ μετά τη Φ1.** Δεν έχει μετρηθεί FPS κατά το σύρσιμο με
   3.107 οντότητες, και **δεν μπορεί να μετρηθεί από πράκτορα** (απαιτεί ζωντανό browser με
   πραγματικό φορτίο). Η Φ1 απέστειλε τη στρατηγική **(1) live + rAF coalescing** — δηλαδή
   *καμία* ειδική μεταχείριση: ο `ResizeObserver` παραδίδει ήδη μία φορά ανά frame, και το
   `useViewportManager` έχει ήδη debounce 100ms στα React state updates. Αυτό είναι **συνειδητή
   μη-βελτιστοποίηση**, όχι παράλειψη: το ADR απαγορεύει ρητά την επιλογή (2)/(3) χωρίς μέτρηση.
   ➜ **Ενέργεια Giorgio**: άνοιξε το `47_ergasia.dxf`, σύρε το διαχωριστικό, δες αν η αίσθηση
   είναι ρευστή. Αν όχι ⇒ στρατηγική (2) *cheap-path κατά τη χειρονομία*.
