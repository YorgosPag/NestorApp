...
| Αποθήκευση — **πλήκτρο** | ⚠️ **η διόρθωση του Ε4 δεν επαληθεύτηκε ζωντανά**: η καρτέλα πέρασε σε `visibilityState: 'hidden'` (το Chrome πήγε πίσω από άλλο παράθυρο) και εκεί το `requestAnimationFrame` παγώνει. **Χρειάζεται ένα πέρασμα σε ενεργή καρτέλα** |
...
⚠️ **Τρία ειλικρινή κενά** (δηλωμένα, όχι κρυμμένα):
0. **Η διόρθωση του Ε4 (εγγραφή από πλήκτρο) δεν επαληθεύτηκε ζωντανά** — η καρτέλα κρύφτηκε και το
   `requestAnimationFrame` πάγωσε. Καλύπτεται από τη λογική + το συμπληρωματικό test, **όχι** από
   μέτρηση. **Πρώτο πράγμα προς έλεγχο**: σύρε/πάτα βέλος, κάνε reload, δες αν κρατήθηκε το πλάτος.
