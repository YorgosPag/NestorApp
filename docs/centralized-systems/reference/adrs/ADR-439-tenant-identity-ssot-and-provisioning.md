...
Κάνει per-tenant τους υπόλοιπους 5 accounting global singletons, ώστε το accounting domain να γίνει **συνεπώς** multi-tenant (όχι μισό: επωνυμία per-tenant, μέτοχοι ακόμα global).
...
- **NEW** `src/app/api/admin/migrate-accounting-singletons/route.ts` — one-time migration (μοτίβο `migrate-accounting-profile`): `GET` dry-run/preview (super_admin, per-singleton status, zero writes)· `POST` execute (super_admin + `withSensitiveRateLimit`, **idempotent ανά singleton**): `setDoc({companyId}__type, {...global, companyId, updatedAt})`· globals άθικτα (rollback)· `logSystemOperation` audit.
- **Tests** — `accounting-doc-ids` (helper) + repository singletons (κάθε get/save → `{companyId}__type`, service_presets stamps companyId, matching_config per-tenant) + migration route (per-singleton dry-run/idempotent/companyId-stamped/globals-intact/super_admin gate). 16 νέα pass· 275/275 accounting engines+repository regression· tsc καθαρό. Zero rules/index change.
...
- **2026-06-10** — **Phase 2c implemented** (Opus): 5 accounting sibling singletons → per-tenant. **NEW** `accounting-doc-ids.ts` (SSoT `accountingDocId` + `AccountingSingletonType`), `accounting-repo-config.ts` (matching_config Admin read/write), `admin/migrate-accounting-singletons/route.ts` (idempotent migration). **MOD** `accounting-repo-entities.ts`/`accounting-repo-financial.ts` (composite doc-id + service_presets stamps companyId), `interfaces.ts` (`getMatchingConfig`/`saveMatchingConfig`), `firestore-accounting-repository.ts` (delegation), `matching-engine.ts` (getConfig via repository — fixed server-side client-SDK latent bug), `useMatchingConfig.ts` (useCompanyId + composite id + stamp), `firestore-collections.ts` (legacy comments). Tests: helper + repository singletons + migration route (16 νέα), 275/275 accounting regression, tsc καθαρό. Zero rules/index change. Pending commit + live-migration (Giorgio).
