...
- **2026-09-15** — **ADR-841 §7 Α23 Γ3β — το «Zero rules change» της Phase 2c ΔΕΝ ισχύει πια για τις εγγραφές.** Το `accounting_settings/{docId}` έκρινε create/update **μόνο** με το `companyId` του σώματος ⇒ (α) ένα `setDoc` από browser έγραφε το προφίλ `{companyId}` **παρακάμπτοντας** μάσκα πεδίων + ίχνος (ADR-256/440 2026-09-15)· (β) admin της Α μπορούσε να δημιουργήσει πρώτος το `{B}__matching_config` με companyId Α και να κλειδώσει έξω τη Β (το docId δεν δενόταν). **Απόφαση** (Stripe/Google: νομικά στοιχεία μόνο μέσω API): **λίστα επιτρεπόμενων**, όχι λίστα απαγορεύσεων — ο browser γράφει **μόνο** `{companyId}__<type>` για τους τύπους της νέας `CLIENT_WRITABLE_ACCOUNTING_SINGLETONS` (σήμερα `matching_config`), με το docId **δεμένο** στο δικό του companyId· όλα τα άλλα (προφίλ, legacy globals, `partners`/`members`/`shareholders`/`service_presets`) = server-only (Admin SDK). Οι **αναγνώσεις δεν άλλαξαν**. `accounting-doc-ids.ts`: +`ACCOUNTING_SINGLETON_TYPES` (runtime SSoT, ο τύπος παράγεται από αυτό) · +`CLIENT_WRITABLE_ACCOUNTING_SINGLETONS` · το σχόλιο «rules never read the doc id» διορθώθηκε. `firestore.rules`: τοπικός βοηθός `isClientWritableAccountingSetting` στο μπλοκ. Σουίτα κανόνων: διατρέχει **και τις δύο** σταθερές στον εξομοιωτή (42/42) · μεταλλάξεις 5/5 σε αντίγραφο κανόνων. ⚠️ Ισχύει στην παραγωγή μόνο μετά από deploy κανόνων (απόφαση Giorgio).

- **2026-09-14** — **ADR-841 §7 Α21.19** — ο αναγνώστης `company-legal-identity.ts` απέκτησε **μία** εσωτερική ανάγνωση `readProfileDocument` με **τρεις** καταστάσεις (`present`/`absent`/`unavailable`) και δεύτερο καταναλωτή `readCompanyContactDeclaration` — **μόνο** 7 πεδία επικοινωνίας (διεύθυνση · πόλη · Τ.Κ. · τηλέφωνο · κινητό · email · ιστοσελίδα), ελαχιστοποίηση **με τύπο**, για την «Εισαγωγή από τα στοιχεία της εταιρείας» στην επαγγελματική κάρτα. Συμβόλαιο `readCompanyLegalIdentity` **αμετάβλητο** (αστοχία ⇒ `null`). Άγκυρες Δ1-Δ4. Στην ίδια ανάγνωση στηρίζεται και το `readCompanyRegistryDeclaration` της ADR-841 Α23 (ΓΕΜΗ). ⛔ Self-contact εξακολουθεί να **μην** υπάρχει.
