...
**Status:** 🟢 Slices 0-5 + 7 IMPLEMENTED 2026-06-16 (Opus) — UNCOMMITTED (🔴 tsc[μπλοκαρισμένος από άλλον agent· Giorgio/επόμενη συνεδρία] + browser-verify + commit). **Slice 6 (PDF detail-sheet) DEFER** → handoff `HANDOFFS/HANDOFF_2026-06-16_ADR463-slice6-pdf-detail-sheet.md`. jest: foundation-structural-param 8/8, footing-rebar-3d+foundation-to-three 9/9, foundation-preset 5/5, +183 schedule/bridge GREEN (μηδέν regression).
...
- **DEFER Slice 6 (PDF detail-sheet):** region builders (plan/elevation/schedule/titleblock/perspective) + offscreen 3D capture + host/dialog + ribbon «Λεπτομέρεια Οπλισμού». i18n keys ήδη μπήκαν (`foundationStructural.reinforcementDetail*`). Handoff: `HANDOFFS/HANDOFF_2026-06-16_ADR463-slice6-pdf-detail-sheet.md`.
