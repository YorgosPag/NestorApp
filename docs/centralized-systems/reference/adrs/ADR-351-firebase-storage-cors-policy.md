...
| Date | Change |
|------|--------|
| 2026-09-26 | **ADR-884 Κ3α — resumable uploads + quarantine lifecycle.** `responseHeader` += `Range` · `Content-Range` · `X-Goog-Resumable`: a browser sends a 360° panorama in 8 MiB chunks straight to a server-opened resumable session and **resumes** after a network drop (`PUT` with `Content-Range: bytes */N` → `308` + `Range`); GCS uses `responseHeader` for both allowed and exposed headers. New `infrastructure/firebase-storage/lifecycle.json` (delete `tour-ingest/` after 1 day) + `firebase-storage:lifecycle:{apply,verify}`. ⚠️ `--lifecycle-file` **replaces** the whole lifecycle config — verified empty on the live bucket before the first apply. **Apply both by hand** (`pnpm firebase-storage:cors:apply` · `pnpm firebase-storage:lifecycle:apply`). |
