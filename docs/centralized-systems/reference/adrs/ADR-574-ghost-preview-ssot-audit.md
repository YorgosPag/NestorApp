...
### Νησίδα 2 — MEP + openings PLACEMENT ghosts (Pipeline C) · **Severity: MEDIUM**
...
**Διευκρίνιση (επιβεβαιώθηκε με χειροκίνητο read):** ισχύει **μόνο** για το gesture **placement/
γέννηση**. Στο **edit** (grip/move) των ίδιων οντοτήτων, η διαδρομή είναι η **ενοποιημένη**
`apply-parametric-box-preview.ts` → `computeMep*Geometry` → πραγματικός renderer (✅ WYSIWYG). Επίσης,
το placement ghost **ναι μεν** έχει ξεχωριστό paint, αλλά **επαναχρησιμοποιεί** sub-SSoT helpers
(`resolveManifoldPalette`, `buildDrainageGratingStrokes`, `hexToRgba` — ADR-571), οπότε η απόκλιση
είναι περιορισμένη. Το footprint έρχεται από `getGhostFootprint()` του tool (όχι από committed
geometry service). Είναι το κλασικό «preview ≠ commit by construction» ρίσκο — σε στυλ, όχι σε σχήμα.
...
| Σ2 | Δρομολόγηση MEP/opening **placement** ghosts μέσω committed renderers (ή shared symbol-draw helper), ώστε placement ghost == commit paint όπως ήδη το edit | 2 | MEDIUM |
