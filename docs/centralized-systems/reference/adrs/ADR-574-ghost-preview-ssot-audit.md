...
### Νησίδα 2 — MEP + openings PLACEMENT ghosts (Pipeline C) · **Status: ✅ IMPLEMENTED (2026-07-05)**
...
- **Deferred/blocked (2):** `MepSegmentGhostRenderer.ts` **δεν** διαγράφηκε — παραμένει sole-non-test
  ...
  διαφορετικό feature, εκτός scope). `slab-opening-ghost-renderer.ts` (slab openings) παραμένει bespoke
  **σκόπιμα**: ο commit builder `buildSlabOpeningEntity` απαιτεί live host `SlabEntity` +
  `validateAgainstHost` (hard error `outlineOutsideSlab`) — αν το ghost γινόταν identity-with-commit, θα
  **εξαφανιζόταν** κάθε φορά που το ορθογώνιο άνοιγμα πλησιάζει/διασχίζει άκρη πλάκας, δηλαδή
  regression στο placement UX. Χρειάζεται νέο host-slab bridge store (εκτός hard-constraints αυτού του
  island) — μένει ως follow-up.
