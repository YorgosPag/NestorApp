...
### 2026-07-15 — ΔΥΟ κουμπιά διαστάσεων εικόνας: «Επαναφορά Διαστάσεων» + «Κλείδωμα Αναλογιών»
- **Αίτημα (Giorgio)**: ο χρήστης μπορεί αθελά του να παραμορφώσει ένα entourage sprite (οι **μεσοπλευρικές** λαβές E/S/W
  είναι εξ ορισμού μη-ομοιόμορφες → σπάει ο λόγος πλευρών με το fill render). Θέλει **ΔΥΟ ξεχωριστά κουμπιά** (big-player):
  1. **«Επαναφορά Διαστάσεων»** (PowerPoint «Reset Size») = επαναφορά στο **ΑΠΟΛΥΤΟ αρχικό μέγεθος**, όπως στην πρώτη
     τοποθέτηση (απορρίπτει κάθε resize/scale).
  2. **«Κλείδωμα Αναλογιών»** (ArchiCAD «fit to original proportions») = **un-deform ΧΩΡΙΣ αλλαγή κλίμακας** — κρατά τη
     μεγαλύτερη τρέχουσα πλευρά, διορθώνει μόνο τον λόγο πλευρών (ο χρήστης κρατά όποιο μέγεθος διάλεξε).
- **Πηγή αρχικού μεγέθους**: το `ImageEntity` ΔΕΝ κρατούσε original μέγεθος/catalog id (μόνο `url`). Προστέθηκαν optional
  `intrinsicWidth`/`intrinsicHeight` (`types/image.ts`), που **γεμίζει το `place-entourage.ts` `assemble()`** = το catalog
  μέγεθος τη στιγμή της τοποθέτησης (μία εγγραφή, δεν αλλάζει ποτέ από resize/scale). Legacy/μη-entourage εικόνες
  (detail-sheet, AI title-block) δεν το έχουν → fallback: aspect από **decoded pixel-aspect**.
- **Pure SSoT (μηδέν διπλότυπο, N.18)**: **NEW** `bim/image/reset-image-dimensions.ts` — `resolveResetToOriginalSize`
  (απόλυτο· fallback aspect) + `resolveLockAspect` (πάντα keep-larger-side από «αληθινό» λόγο = intrinsic ή pixel) +
  `resetImageToOriginalSize`/`lockImageAspect` (patch μέσω ΚΟΙΝΟΥ `buildResizePatch`: **σταθερό κέντρο**, **ανέγγιχτη
  rotation**, idempotent) + `hasStoredIntrinsicSize` (decode-gate). Κέντρο/γωνία από το ΚΟΙΝΟ `imageRectFrame` +
  `rectCornerWorld` (ίδιο SSoT με τις λαβές → reposition ≡ grip semantics). Μηδέν DOM/store/decode μέσα.
- **Wiring (mirror `useExplodeRibbonAction`, ΕΝΑΣ hook για 2 actions — N.18)**: **NEW**
  `ui/ribbon/hooks/useImageDimensionRibbonAction.ts` — interceptor που πιάνει `image-reset-size` + `image-lock-aspect`
  (map action→pure patch fn), resolve τις επιλεγμένες εικόνες, decode-άρει ΜΟΝΟ όσες δεν έχουν intrinsic
  (`decodeImageWithTimeout` + `imageIntrinsicSize`, reuse), και εκτελεί `UpdateEntityCommand` (πολλές = `CompositeCommand`,
  **ένα atomic undo**). Κρίκος στην αλυσίδα (`useDxfViewerRibbon.ts`): image-dimension → block-edit → group → join →
  explode → array → base. Δύο `actionBtn` στο `contextual-image-tab.ts` (panel «Εικόνα», big-player θέση = PowerPoint).
- **Icon/i18n**: νέα inline-SVG icons `image-reset-size` (πλαίσιο + διαγώνιο βέλος) & `image-lock-aspect` (πλαίσιο +
  λουκέτο)· `ribbon.commands.imageResetSize` (el «Επαναφορά Διαστάσεων» / en «Reset Size») + `imageLockAspect`
  (el «Κλείδωμα Αναλογιών» / en «Lock Proportions»).
- **Tests**: **NEW** `reset-image-dimensions.test.ts` (13 πράσινα: reset=απόλυτο vs lock=keep-larger-side· intrinsic vs
  decoded· σταθερό κέντρο με & χωρίς rotation· idempotent no-op· null guards). `jscpd:diff` καθαρό. **ΟΧΙ tsc.**
