...
### 2026-07-15 — Κουμπί «Επαναφορά Διαστάσεων» εικόνας (PowerPoint «Reset Size» / ArchiCAD «fit to proportions»)
- **Αίτημα (Giorgio)**: ένα κουμπί που επαναφέρει τις αρχικές διαστάσεις ενός entourage sprite — ο χρήστης μπορεί αθελά
  του να το παραμορφώσει (οι **μεσοπλευρικές** λαβές E/S/W είναι εξ ορισμού μη-ομοιόμορφες → σπάει ο λόγος πλευρών με το
  fill render). Big-player consensus: PowerPoint «Reset Size» (Picture Format ribbon) = επαναφορά **απόλυτου** μεγέθους·
  ArchiCAD «fit to original proportions» = un-deform. Απόφαση Giorgio: **πλήρης επαναφορά μεγέθους** (A + fallback C).
- **Κρίσιμο (§4 handoff)**: το `ImageEntity` ΔΕΝ κρατούσε ούτε original μέγεθος ούτε catalog id (μόνο `url`). **Δρόμος A**:
  προστέθηκαν optional `intrinsicWidth`/`intrinsicHeight` (`types/image.ts`), που **γεμίζει το `place-entourage.ts`
  `assemble()`** = το catalog μέγεθος τη στιγμή της τοποθέτησης (μία εγγραφή, δεν αλλάζει ποτέ από resize/scale). Legacy/
  μη-entourage εικόνες (detail-sheet, AI title-block) δεν το έχουν → **Δρόμος C fallback**: aspect-only από decoded
  pixel-aspect (κράτα μεγαλύτερη πλευρά, διόρθωσε την άλλη).
- **Pure SSoT (μηδέν διπλότυπο, N.18)**: **NEW** `bim/image/reset-image-dimensions.ts` — `resolveImageResetSize` (A>C
  decision) + `resetImageDimensions` (patch με **σταθερό κέντρο**, **ανέγγιχτη rotation**) + `hasStoredIntrinsicSize`
  (decode-gate). Το κέντρο/γωνία διαβάζονται από το ΚΟΙΝΟ `imageRectFrame` + `rectCornerWorld` (ίδιο SSoT με τις λαβές →
  reposition ≡ grip semantics). Μηδέν DOM/store/decode μέσα — testable ως γεωμετρία.
- **Wiring (mirror `useExplodeRibbonAction`)**: **NEW** `ui/ribbon/hooks/useImageResetSizeRibbonAction.ts` — action
  interceptor (`image-reset-size`) που resolve τις επιλεγμένες εικόνες, decode-άρει ΜΟΝΟ όσες δεν έχουν intrinsic
  (`decodeImageWithTimeout` + `imageIntrinsicSize`, reuse), και εκτελεί `UpdateEntityCommand` (πολλές = `CompositeCommand`,
  **ένα atomic undo**). Κρίκος στην αλυσίδα interceptors (`useDxfViewerRibbon.ts`): image-reset-size → block-edit → group →
  join → explode → array → base. Κουμπί = `actionBtn` στο `contextual-image-tab.ts` (panel «Εικόνα», big-player θέση).
- **Icon/i18n**: νέο inline-SVG icon key `image-reset-size` (πλαίσιο + διαγώνιο βέλος)· `ribbon.commands.imageResetSize`
  (el «Επαναφορά Διαστάσεων» / en «Reset Size»).
- **Tests**: **NEW** `reset-image-dimensions.test.ts` (10 πράσινα: A>C priority· keep-larger-side· σταθερό κέντρο με &
  χωρίς rotation· idempotent no-op· null guards). `jscpd:diff` καθαρό. **ΟΧΙ tsc.**
