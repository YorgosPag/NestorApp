...
| **13B** *(opt)* | Cross-project Templates Firestore (`dxf_layer_state_templates/{templateId}` companyId-scoped + permission `company_admin+` + LayerStateStore.saveAsTemplate/importTemplate/searchTemplates + tests) — **split 13B.1/13B.2/13B.3 per Phase-per-session §7.1** | ~15 | L (sub-split) | Q12 | 🟡 13B.1 ✅ v2.18-pre0-13B.1 / 13B.2 PENDING / 13B.3 PENDING |
...
| **13B.2** *(opt)* | LayerStateStore extension `saveAsTemplate(stateId)` + `importTemplate(templateId)` + `searchTemplates(query)` + `useLayerStateTemplates` orchestrator hook + store/hook tests + ratchet allowlist extension for new store-side function names | ~3 | M | Q12 | PENDING |
