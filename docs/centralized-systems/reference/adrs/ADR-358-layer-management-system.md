...
| **9E-3** | Tools: `useTrimTool`, `useStretchTool`, `useScaleTool`, `useExtendTool` — `scene.layers[name]` → id-keyed | 4 | M | Q2 | ⏳ |
| **9E-4** | UI: `LayerItem`, `useColorGroups`, `ColorGroupItem`, `useLayerOperations` — `scene.layers[name]` → id-keyed | 4 | M | Q2 | ⏳ |
| **9E-5** | Services: `LayerOperationsService`, `layer-operation-utils` — maintain `layersById` on every mutation; `useDxfSceneConversion` bridge switch to `scene.layersById` | 2 | M | Q2 | ⏳ |
| **9E-6** | Cleanup: drop `SceneModel.layers` (name-keyed), make `layersById` required, drop `BaseEntity.layer?`, enforce `LayerId` | 3 | M | Q2 | ⏳ |
| **Q17-scaffold** | Floor-stair link scaffold: `StairMultiStoryConfig.linkedToFloor?` flag (tracks whether `storyHeight` auto-tracks building floor height) + `StairFloorLinkInput` interface + `buildDefaultStairParams` optional `floorLink` param (auto-seeds `multiStoryConfig.storyHeight = floorLink.height × 1000mm`, `linkedToFloor: true`) + `useFloorMetadata` hook (Firestore `FLOORS/{floorId}` subscription via ADR-361 equality guard) + `patchStoryHeight` in `useRibbonStairBridge` sets `linkedToFloor: false` on manual user edit | 4+1new | S | Q2 | ✅ v2.14-pre28 |
