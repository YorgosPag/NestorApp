...
| **9D** | `BaseEntity.layer` → `layerId` schema break + ~55 callsite migration (Grep-driven, split 9D-1…9D-5b per Phase-per-session §7.1 + Google LSC playbook 4-step) | ~55 | XL | Q2 | 9D-1 ✅ v2.14-pre1 / 9D-2 ✅ v2.14-pre2 / 9D-3a ✅ v2.14-pre3 / 9D-3b ✅ v2.14-pre4 / 9D-4 ✅ v2.14-pre5 / 9D-5a ✅ v2.14-pre6 / 9D-5b-i ✅ v2.14-pre7 / 9D-5b-ii ⏳ / 9D-5b-iii ⏳ / 9D-5b-iv ⏳ |
...
| **9D-5b-ii** | **Type-chain consumer migration** (Google LSC step C complete). Drop `layer: string` field from downstream type definitions: `SceneEntity` (`core/commands/interfaces.ts`), `PreviewPoint`, `DxfTextSceneEntity`, `EntityModel`, `DxfStair`-related types. Cascade fix ~40-50 file: `systems/trim/*` (8 file), `systems/extend/*`, `systems/array/*` (3 file), `core/commands/text/*` (10 file), `utils/dxf-entity-converters.ts` (8 occ), `utils/dxf-dimension-converter.ts` (4 occ), `ai-assistant/dxf-ai-tool-executor.ts` (4 occ), `hooks/drawing/drawing-preview-*` (PreviewPoint), `useUnifiedDrawing.tsx`, `useFloatingPanelHandle.ts`, `snap-adapter.ts`, `useCentralizedMouseHandlers.ts`, `useStairPersistence.ts`, `stair-completion.ts`, `useCircleTTT/Parallel/Perpendicular`, `mouse-handler-up.ts`, `array-source-extraction.ts`, `canvas-v2/dxf-canvas/DxfRenderer.ts` (`layerId: string | undefined`→`string` resolution), `useEntityCreationManager.ts` (SceneEntity layer requirement). Plus 3 file outside subapp: `app/api/floorplans/process/floorplan-process.service.ts`, `components/shared/files/media/useFloorplanSceneLoader.ts`, `services/floorplans/FloorplanProcessor.ts`. Probable sub-split per-domain (trim/extend/array vs text-commands vs utils-converters vs outside-subapp). Each sub-phase TS green. | ~50 | L+ (likely sub-split) | Q2 | ⏳ |
...
| **9D-5b-iv** | Test fixture migration ~27 test file / ~68 occ `layer: 'X'` literal → `layerId: getLayer('X')?.id` (setup-time stable id capture via `setLayers([createSceneLayer({...})])`). ADR-358 §7.2 row 9D ✅ v2.14 final + §10 changelog v2.14 final entry. | ~30 | L | Q2 | ⏳ |
