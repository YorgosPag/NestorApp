...
| **9** | Layer rename con backref completi — **split in 7 sub-fasi 9A-G per Phase-per-session §7.1** | ~56 | XL | Q9 | 9A ✅ v2.11 / 9B ✅ v2.12 / 9C-G ⏳ |
...
| **9C** | `SceneLayer.id` required + `createSceneLayer()` enforce id-gen via `enterprise-id.service` prefix `lyr` + DXF parser id-gen | 4 | M | Q2 | ⏳ |
| **9D** | `BaseEntity.layer` → `layerId` schema break + 32 callsite migration (Grep-driven) | ~32 | XL | Q2 | ⏳ |
