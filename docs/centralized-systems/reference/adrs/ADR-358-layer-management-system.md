...
| **9D-5b-iv** | Test fixture migration ~27 test file / ~68 occ `layer: 'X'` literal → `layerId: getLayer('X')?.id` (setup-time stable id capture via `setLayers([createSceneLayer({...})])`). ADR-358 §7.2 row 9D ✅ v2.14 final + §10 changelog v2.14 final entry. | ~30 | L | Q2 | 🟡 Sub-iv-a ✅ v2.14-pre13 / Sub-iv-b ✅ v2.14-pre14 / Sub-iv-c ⏳ |
