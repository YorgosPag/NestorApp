...
| **9D** | `BaseEntity.layer` → `layerId` schema break + ~55 callsite migration (Grep-driven, split 9D-1…9D-5b per Phase-per-session §7.1) | ~55 | XL | Q2 | 9D-1 ✅ v2.14-pre1 / 9D-2 ✅ v2.14-pre2 / 9D-3a ✅ v2.14-pre3 / 9D-3b ✅ v2.14-pre4 / 9D-4 ✅ v2.14-pre5 / 9D-5a ✅ v2.14-pre6 / 9D-5b ⏳ |
...
| **9D-5b** | Final flip: `BaseEntity.layerId` REQUIRED, `BaseEntity.layer` REMOVED. `LayerStore.resolveEntityLayerName` simplify id-only (drop legacy fallback). `CreateEntityCommand`+`CreateEntityOptions` drop `layer` field. Test fixture migration ~27 file / ~68 occ `layer: 'X'` → `layerId: getLayer('X')?.id` (setup-time stable id capture). SSoT registry `entity-layer-id-canonical` drop transitional allowlist (canonical pure). ADR-358 §7.2 row 9D ✅ v2.14 final. | ~28 | L | Q2 | ⏳ |
