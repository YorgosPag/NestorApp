...
| **9D** | `BaseEntity.layer` → `layerId` schema break + ~55 callsite migration (Grep-driven, split 9D-1…9D-5 per Phase-per-session §7.1) | ~55 | XL | Q2 | 9D-1 ✅ v2.14-pre1 / 9D-2 ✅ v2.14-pre2 / 9D-3a ✅ v2.14-pre3 / 9D-3b…9D-5 ⏳ |
...
| **9D-3b** | Reader migration **batch B (commands + systems + UI + rendering)**: text commands `core/commands/text/*` 11 file (`assertCanEditLayer({layerName: entity.layer, ...})` callsites) + systems (`auto-area-hit` 10 occ layer-by-name push, `stretch-entity-transform`, `trim-boundary-resolver` locked+visible guard, `stairs/stair-validator`, `selection/utils` get-layer-by-name) + `entity-creation/useEntityCreationManager` + UI (`text-toolbar/useTextToolbarSelectionSync`, `layer-manager/useLayerManagerState`, `layers/hooks/useColorGroups`+`useLayersCallbacks`) + rendering (`HitTester` + `hit-tester-utils`, `EntityPass` batchKey, `canvas-v2/dxf-canvas/DxfRenderer` `layersById[entity.layer]` line 469 — kept name-keyed pending Phase 9E re-key) + `ai-assistant/dxf-ai-tool-executor` filter-by-name. | 17 | L | Q2 | ⏳ |
| **9D-4** | Literal `layer: 'X'` construction sweep: `drawing-entity-builders.ts` (25), `drawing-preview-generator.ts` (10), `drawing-preview-partial.ts` (10), `useDynamicInputHandler.ts` (4), test fixtures (~30) + region/overlay construction sites. Add parallel `layerId` field everywhere. | ~25 | L | Q2 | ⏳ |
| **9D-5** | Final flip: `BaseEntity.layerId` REQUIRED, `BaseEntity.layer` REMOVED. `LayerOperationsService.renameLayer` drop `entity.layer` backref update. SSoT registry: new module `entity-layer-id-canonical` (Tier 3) forbid `entity\.layer\b`. ADR-358 §7.2 row 9D ✅ v2.14. | 2 | S | Q2 | ⏳ |
