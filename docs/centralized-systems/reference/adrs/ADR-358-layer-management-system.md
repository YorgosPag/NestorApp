...
| **13B** *(opt)* | Cross-project Templates Firestore (`dxf_layer_state_templates/{templateId}` companyId-scoped + permission `company_admin+` + LayerStateStore.saveAsTemplate/importTemplate/searchTemplates + tests) — **split 13B.1/13B.2/13B.3 per Phase-per-session §7.1** | ~15 | L (sub-split) | Q12 | 🟡 13B.1 ✅ v2.18-pre0-13B.1 / 13B.2 ✅ v2.18-pre0-13B.2 / 13B.3 PENDING |
...
| **13B.3** *(opt)* | UI: `LayerStateTemplateBrowser` modal + popover/dropdown wiring ("Browse Templates…", "Save as Template…") + i18n el+en (`layerState.templates.*` Greek-pure) + UI tests + ADR-358 §10 v2.18-pre0 FINAL + §7.2 row 13B → ✅ DONE | ~5 | M | Q12 | PENDING |
