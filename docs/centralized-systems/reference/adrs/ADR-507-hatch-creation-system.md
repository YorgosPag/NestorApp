...
- **2026-06-21** — **§8 item α / Level 3 (Scope B): `MoveEntityCommand` extends `SnapshotTransformCommand` — η τελευταία ασυμμετρία της οικογένειας έκλεισε (47 jest GREEN· UNCOMMITTED).** Το Move ήταν το ΜΟΝΟ command με δικό του in-place spine (`move-entity-cascade.ts`) + undo με `reverseDelta` recompute. Audit (διάβασα 6 αρχεία) απέδειξε ΟΛΕΣ τις ισοδυναμίες πριν το refactor: (i) host geometry `computeUpdates = calculateMovedGeometry` (ίδιο)· (ii) pipe-follow ισοδύναμο γιατί `calculateMovedGeometry` **delegate-άρει** στο `calculateBimMovedGeometry` (move-entity-geometry.ts:55) → `nextParamsFromTransformPatch(calculateMovedGeometry(host,δ)) === movedParams(host,δ)`· (iii) slab-opening ίδιο delegate, engine reads own coords (timing-independent)· (iv) followers επιστρέφουν snapshots → `undoInPlace` τα επαναφέρει· (v) merge re-execute (CommandHistory:46-53) διατηρεί execute() contract.
  ...
  - **Tests**: `move-entity-3d-delta.test.ts` — NEW spine describe (execute moves· undo=snapshot-restore exact· identity preserved· redo· multi-entity)· αφαίρεση `reverseDelta` block (dead). 47 jest GREEN (base+followers+copy-mode+move). tsc 0 στο core/commands. **DEFER:** `undoInPlaceWith` πλέον αχρησιμοποίητο σε production (μένει ως seam για future delta-based subclass — base method άλλου agent· candidate αφαίρεσης αν αποφασιστεί).
