...
- **2026-06-21** — **Φ2 Predefined Patterns υλοποιήθηκε (PAT catalog 32 μοτίβα + SSoT geometry + canvas + DXF + UI/i18n· 50 jest GREEN· UNCOMMITTED).** Ενεργοποίηση των αχρησιμοποίητων `patternName/patternScale/patternAngle` πεδίων + `fillType:'predefined'`. **SSoT καρδιά αμετάβλητη** — ο canvas renderer ΚΑΙ ο DXF writer καταναλώνουν την ΙΔΙΑ pattern-segment έξοδο, μηδέν δεύτερη pattern math.
  ...
  - **EXTEND `bim/geometry/shared/hatch-pattern-geometry.ts`**: NEW `buildPredefinedHatchLines(boundaryPaths, pattern, {scale,angleDeg,origin,islandStyle})`. Ανά `PatternLine`: perpendicular loop (mirror `buildAxisAlignedHatch`) → chord μέσω **`clipLineToBbox` (REUSE)** → dash subdivision (η ΜΟΝΗ νέα math: dashes >0/<0/0=dot, phase=i·deltaΧ για brick-alignment) → clip στο όριο μέσω **`clipSegmentToRegion` (REUSE)** even-odd. Έξοδος `HatchLineSegment[]` — ίδιος τύπος με user-defined → canvas+DXF αυτούσιο.
  - **EXTEND `rendering/entities/HatchRenderer.ts`**: branch `fillType==='predefined'` → `getHatchPattern`+`buildPredefinedHatchLines`· refactor σε `userDefinedSegments`/`predefinedSegments`/`drawPatternSegments` (κοινό draw). Thin consumer, μηδέν pattern math. ⚠️ CHECK 6D → stage ADR-507.
  - **EXTEND `export/core/dxf-ascii-writer.ts emitHatch`**: NEW `hatchPatternSegments` (explode mode → predefined LINEs, SSoT με canvas) + `emitPredefinedPattern` (native HATCH: `76=1`, `78=N` pattern lines, ανά line `53/43/44/45/46/79/49` με scale). Αντικατέστησε το hardcoded `78=1`. Άγνωστο pattern → fallback 78/1 (έγκυρο hatch).
