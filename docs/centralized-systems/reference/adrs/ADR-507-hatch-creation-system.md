...
- **2026-06-22** — **Φ2 follow-up: Thumbnail preview + Pattern search + Hatch lineweight (§5β.6-7, §5γ.5, §5γ.6· 88 hatch jest GREEN· UNCOMMITTED).** Τα 3 UX deliverables που έφεραν τον pattern picker σε Revit/AutoCAD-grade. **FULL SSoT — μηδέν νέα pattern math, μηδέν νέο lineweight field.**
  ...
  - **boy-scout (N.0.2):** το `LINEWEIGHT_OPTIONS` (ByLayer + ISO subset) ήταν local στο `contextual-line-tool-tab.ts` → extract σε NEW shared `ui/ribbon/data/lineweight-ribbon-options.ts` (SSoT)· line-tool ΚΑΙ hatch το καταναλώνουν.
