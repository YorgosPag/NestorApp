...
- **2026-06-22** — **Φ5 gap A4 — Gradient ANGLE με draggable βραχίονα (real-time περιστροφή· UNCOMMITTED· 38 jest GREEN).** Δεύτερη λαβή (μετά το origin A3) σε απόσταση `R = ½·διαγώνιος bbox` κατά τη φορά του gradient· σέρνοντάς την → `gradient.angleDeg = atan2(cursor − origin)` → η διαβάθμιση **περιστρέφεται live**. **FULL SSoT — πιστό mirror της A3 υποδομής, μηδέν νέος μηχανισμός:**
  ...
  - **WYSIWYG ghost:** το ίδιο full-opacity `fillHatchGradient` (A3b) ζωγραφίζει το rotated gradient (preview===commit). `useGripGhostPreview`: live βραχίονας (dashed `drawDashedSegment` origin→handle) + `drawGradientOriginMarker` στο άκρο → «περιστροφή» ορατή.
