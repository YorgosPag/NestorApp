...
- **2026-06-22** — **Φ5 gap A3 — Gradient origin/seed με draggable λαβή (UNCOMMITTED· 32 jest GREEN).** Το gradient ξεκινούσε ΠΑΝΤΑ από το κέντρο του bbox· τώρα ο χρήστης σέρνει λαβή για να ορίσει το «κέντρο φωτισμού». **FULL SSoT — reuse του υπάρχοντος `patternOrigin` (μηδέν νέο πεδίο· ήδη plumbed σε 2 converters + DxfHatch + HATCH_SCALAR_KEYS + DXF round-trip), όπως το AutoCAD μοιράζεται το hatch origin μεταξύ pattern ΚΑΙ gradient.**
  ...
  - **(4) Grip pipeline** — `grip-computation.ts` (case 'hatch'): εκπέμπει την origin λαβή **ΜΟΝΟ όταν `fillType==='gradient'`** (θέση = `patternOrigin ?? hatchBoundsCenter`). `apply-entity-preview.ts`: origin kind → live preview ενημερώνει `patternOrigin` (όχι όριο). `grip-parametric-footprint-commits.ts` (`commitHatchGripDrag`): origin kind → `UpdateHatchOriginCommand`· αλλιώς το υπάρχον boundary path. **`HatchRenderer.getGrips` ΑΘΙΚΤΟ** — δεν είναι το interactive source (το `computeDxfEntityGrips` είναι, αυτό σετάρει `hatchGripKind`).
