...
| Ημ/νία | Αλλαγή |
|---|---|
| 2026-09-12 | 🔴🔴 **ΤΟ ΔΟΓΜΑ ΤΗΣ ΛΙΣΤΑΣ ΑΓΝΟΟΥΣΕ ΤΟΝ ΧΩΡΟ ΤΗΣ ΔΙΕΥΘΥΝΣΗΣ** *(ADR-787 §5.3 ζ, όριο 1)*. Το `resolveTenantListScope` διάβαζε **μόνο** το `?companyId=` — και ο πελάτης το στέλνει σε **ΕΝΑ** σημείο όλου του `src/` (μετρημένο: `SimpleProjectDialog.tsx:106`). Άρα κάθε αίτημα super-admin χωρίς εκείνο το ερώτημα έπαιρνε **`all-tenants`**, ακόμα και μέσα στο `/o/<ΡΟΗ>/buildings`: **λάθος δεδομένα κάτω από διεύθυνση που ονομάζει μία εταιρεία** — η κλάση (α) του ADR-787 §9. Το πλήρωσαν 5 διαδρομές (`buildings-list.handler` · `properties/route` · `floors.shared` · `contacts/owner-property-stats` · `admin/migrate-address-labels`). **Θεραπεία**: όταν δεν υπάρχει `?companyId=` αλλά το αίτημα **δήλωσε** χώρο (`ctx.requestedWorkspace?.kind === 'org'`, δήλωση **κριμένη** από τον `decideMembership` στο σύνορο), η λίστα φιλτράρει στο `ctx.companyId`. Το `?companyId=` **προηγείται επίτηδες**: είναι όρισμα της πράξης, ενώ ο χώρος είναι το περιβάλλον της. Το `all-tenants` μένει **μόνο** για αίτημα που δεν ονόμασε **τίποτα** — *«make any cross-tenant administrative path explicit»* (OWASP Multi-Tenant Cheat Sheet). **+Νέος κοινός φρουρός** `assertQueryableWorkspace(ctx)`: δηλωμένος **ιδιωτικός** χώρος **πετά** `TenantIsolationError` αντί να γίνει `all-tenants` — δεύτερη γραμμή άμυνας, γιατί οι δύο τύποι εμβέλειας μπορούν να εκφράσουν «μία εταιρεία» και «όλες», **όχι** «καμία». Τον ρωτά **και** το δόγμα του ADR-356 (ένας φρουρός, δύο δόγματα — N.18). Άγκυρες: Δ1-Δ7 στο `tenant-scope.test.ts`· μεταλλάξεις Μ6/Μ7 σκοτωμένες. |
