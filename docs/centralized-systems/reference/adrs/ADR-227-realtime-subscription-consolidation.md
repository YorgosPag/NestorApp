...
| Field | Value |
|-------|-------|
| **Status** | 🟢 FULLY IMPLEMENTED — All phases complete, 10/10 hooks migrated, FLOORPLAN_UPDATED gap closed (2026-05-24) |
| **Date** | 2026-03-13 |
...
| 2026-05-24 | **`FLOORPLAN_UPDATED` subscriber added to `useFloorFloorplans`** — Hook already had `FLOORPLAN_CREATED` + `FLOORPLAN_DELETED` event bus subscribers. Gap closed: `FLOORPLAN_UPDATED` now triggers `fetchFloorplans()` (refetch via FloorFloorplanService). Note: `getDocs()` in `findFloor()` remains as one-time floor resolution lookup (buildingId+floorNumber → floorId) — not a subscription target. `FloorplanUpdatedPayload` imported from `@/services/realtime`. Last open item from §4.1 inventory matrix resolved. | Claude Sonnet 4.6 |
| 2026-06-11 | **`useFirestoreBuildings` listener dedup (per-call-site → shared module singleton)** — PERF fix for the DXF-viewer freeze investigation (root-cause #3). Each of the ~11 simultaneous call sites (NavigationContext, BuildingsPageContent, ThermalEnvelopeHost, useBimScheduleLookups, useSiteNeighbourMasses, useHeatLoadInputs, useBuildingFloors3DSync, useExecutiveReport, IfcExportHost, PO selectors…) opened its OWN `firestoreQueryService.subscribe('BUILDINGS')` onSnapshot → 9× "Buildings updated" + 9× map/sort per change. Rewrote the hook over a **single module-level, reference-counted subscription** exposed via `useSyncExternalStore`: first consumer opens the listener, last unmount closes it, all consumers share one store + one map/sort. **Zero call-site changes** (identical `{buildings, loading, error, refetch}` API) — chosen over a Context Provider precisely because consumers span unrelated route trees. ADR-300 stale cache still seeds zero-flash remounts; ADR-361 equality guard unaffected. Secondary: `useRealtimeBuildings` refetch trigger converted `useRef`→`useState` (a ref read in the effect dep array was a non-reactive anti-pattern risking spurious re-subscribes on auth flip). Tests: `useFirestoreBuildings.dedup.test.ts` (3 PASS — single-subscription / fan-out / last-unmount-closes). | Claude Opus 4.8 |
