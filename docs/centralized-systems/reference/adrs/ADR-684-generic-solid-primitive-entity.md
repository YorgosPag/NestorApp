...
- **2026-07-23 (Opus 4.8) §delete-undo-persist — 🐛 FIX: συμμετρικός καθρέφτης του §delete-persist (UNCOMMITTED, 🔴 browser verify).** Μετά το forward-delete fix, το Ctrl+Z ξανάφερνε το στερεό στην οθόνη αλλά **όχι** στο Firestore → χανόταν στο επόμενο reload. **Root cause:** το `DeleteEntityCommand` (γενικό delete path — το generic-solid ΔΕΝ έχει dedicated command) εκπέμπει `bim:entity-restore-requested` στο undo μόνο για τύπους στη λίστα `BIM_RESTORE_VIA_DELETE_COMMAND`, που **δεν είχε `generic-solid`** (ούτε `imported-mesh`). Το canonical union (`drawing-event-map-bim.ts`) είχε ήδη `generic-solid` (μπήκε στο Φ2) — απλά η λίστα του command έμεινε πίσω. **Fix (1 src + 1 test):** +`'generic-solid'` (+`'imported-mesh'`) στο `BIM_RESTORE_VIA_DELETE_COMMAND`. Subscriber (`useBimEntityRestoredPersistEffect`) + `persistRestore` + tombstone-clear ήδη πλήρη (κοινός hook). **Tests:** +2 cases στο `DeleteEntityCommand.test.ts` (delete+undo → εκπέμπεται restore ανά τύπο), **11/11 GREEN**, jscpd clean, tsc SKIP (N.17). Αρχεία: `core/commands/entity-commands/DeleteEntityCommand.ts`, `__tests__/DeleteEntityCommand.test.ts` (+ ADR-683). ✅ Google-level: YES — συμμετρία create/delete/restore κλεισμένη, μηδέν sibling path, test κλειδώνει το gap.
- **2026-07-22 (Opus 4.8) §delete-persist — 🐛 FIX: το delete του στερεού δεν persist-άρει (κοινό με imported-mesh, UNCOMMITTED, 🔴 browser verify).** Σύμπτωμα: Delete σε generic-solid → φεύγει από τον καμβά αλλά **ξαναεμφανίζεται στο reload** (το `deleteDoc` δεν έτρεχε ποτέ). **Root cause (ατελής rollout — ΟΧΙ νέος μηχανισμός):** το bulk smart-delete SSoT `smart-delete-bim-events.ts` (`collectBimDeleteIds` + `emitBimDeleteEvents`) είχε πίνακα 21 τύπων που **δεν περιλάμβανε** `generic-solid` (ούτε `imported-mesh`) → το `bim:generic-solid-delete-requested` δεν εκπεμπόταν ποτέ → ο subscriber (`useGenericSolidPersistence`, deleteTrigger γρ.106) δεν καλούσε `deleteGenericSolid` → το reload subscription το ξανάφερνε. Όλο το υπόλοιπο wiring ήταν ήδη πλήρες: event type (`drawing-event-map-bim.ts:123`), emitter mapper (`bim-entity-lifecycle-events.ts:60`), subscriber + **tombstone** (κοινός hook `create-bim-entity-persistence-hook.ts`: `deletedIdsRef.add(id)` γρ.492 → merge γρ.218 → re-add prevention). **Fix (1 src + 1 test):** 3 γραμμές στο `smart-delete-bim-events.ts` (`genericSolidIds` στο interface + `byType('generic-solid')` στο collect + group στο emit) — ίδιο pattern με τους άλλους 21 τύπους. **Tests:** νέο `smart-delete-bim-events.test.ts` (collect partition + emit ανά τύπο, mock `emitBimEntityDeleteRequested`), **13/13 GREEN** (μαζί με `delete-entities-core.test.ts`), tsc SKIP (N.17), jscpd clean. Αρχεία: `hooks/canvas/smart-delete-bim-events.ts`, `hooks/canvas/__tests__/smart-delete-bim-events.test.ts` (+ ADR-683/684). 1 domain (BIM delete). ✅ Google-level: YES — το generic-solid μπήκε στο ΥΠΑΡΧΟΝ delete SSoT (μηδέν sibling path), tombstone/undo κληρονομούνται από τον κοινό hook, test κλειδώνει το gap. 🔴 browser-verify (delete στερεό → refresh → ΔΕΝ ξαναεμφανίζεται· Ctrl+Z → ξαναεμφανίζεται).
- **2026-07-22 (Opus 4.8) §rotate-persist — 🐛 FIX: η 3Δ περιστροφή του στερεού δεν persist-άρει (Giorgio, UNCOMMITTED, 🔴 browser verify).** Σύμπτωμα: gizmo rotate ring σε generic solid → live preview στρέφεται σωστά, αλλά στο release **επανέρχεται** στην αρχική γωνία. **Root cause (ατελής rollout ADR-684 — ΟΧΙ ο Firestore optimistic-revert):** το rotate SSoT `calculateBimRotatedGeometry()` (`bim/transforms/bim-rotate-geometry.ts`) **δεν είχε `case 'generic-solid'`** (ενώ το move `bim-move-geometry.ts` το είχε — γι' αυτό η μετακίνηση δούλευε). Χωρίς case → `default: return null` → fallback `rotateEntity()` (`utils/rotation-math.ts`, non-BIM path, επίσης χωρίς generic-solid handler) → **κενό patch `{}`**. Το `RotateEntityCommand`/`SnapshotTransformCommand` «πετύχαινε» (validate ελέγχει μόνο angle≠0 + id-count, όχι αν το patch άλλαξε κάτι) → pushed στο undo → αλλά το `params.rotationDeg` **ποτέ** δεν γραφόταν στο store → το commit re-sync ξανάχτιζε το mesh από το αρχικό `rotationDeg` (ο converter `generic-solid-to-three.ts:69` το διαβάζει σωστά — απλά δεν είχε αλλάξει) → φαινόταν σαν «επαναφορά». Persistence (`useGenericSolidPersistence`) ήταν σωστά wired — απλά τίποτα δεν άλλαζε για να σωθεί. **Fix (1 src + 1 test):** νέος `rotateGenericSolid` handler (mirror `rotateColumn` + `moveGenericSolid`) — `position` rotate γύρω από pivot + `rotationDeg += angleDeg` (normalized) + `computeGenericSolidGeometry` re-run· `case 'generic-solid'` στον dispatcher. **Tests:** +2 στο `bim-rotate-geometry.test.ts` (position rotate + rotationDeg accumulate + normalize >360°), **19/19 GREEN**, tsc SKIP (N.17). Αρχεία: `bim/transforms/bim-rotate-geometry.ts`, `bim/transforms/__tests__/bim-rotate-geometry.test.ts` (+ ADR-684). 1 domain (BIM rotate). ✅ Google-level: YES — το generic-solid μπήκε στο ΥΠΑΡΧΟΝ rotate SSoT (μηδέν sibling path), coverage test κλειδώνει το gap. 🔴 browser-verify (στρέψε στερεό 3Δ → μένει στραμμένο μετά το release + persist σε reload).
