...
- **Status**: ACCEPTED (Φ2 + Φ3 + Φ4-A/B πλήρη — geometry + citizen integration + authoring UX + per-shape reshape grips + dual-mode per-selection editor· Φ4-C metadata PARTIAL — structuralRole + BOQ classification έτοιμα, per-face υλικό + BOQ quantity auto-feed αναβάλλονται)
...
- **Φ4 — grips + per-selection editor + metadata**:
  ...
  - **Φ4-C ⚠️ PARTIAL** — metadata: `structuralRole` flag (§4.3, δομικό/διακοσμητικό) + Zod schema + BOQ classification SSoT (`resolveGenericSolidMapping`: δομικό → RC OIK-2.03 m³· διακοσμητικό/απόν → null, mirror imported-mesh §10.2) + editor selector. **Αναβάλλονται (scoped follow-ups):** (α) **per-face υλικό** (reuse ADR-539/679) — 3D appearance subsystem, όχι απλό metadata πεδίο· (β) **auto-feed BOQ ποσότητας** — απαιτεί save-pipeline hook (`upsertBoqItemForBim`) + exact `volumeM3` payload· η ταξινόμηση είναι έτοιμη, η εκπομπή γραμμής όχι.
