...
- **2026-07-18 (§opening-width fix 1/2 — όχι ΔΥΝ-gate)** — Ο Giorgio το θέλει να δουλεύει με **σκέτο κλικ στη λαβή**,
  χωρίς τον διακόπτη ΔΥΝ (σε αντίθεση με το άκρο γραμμής). Αφαιρέθηκε το `cadToggleState.isDynInputOn()` gate από το
  opening-corner hot-grip entry (`grip-mouse-handlers`) + από το ring mount (`DynamicInputSubscriber`). Το press-drag
  εξακολουθεί να δουλεύει (moved-release commit)· απλώς πλέον εμφανίζεται πάντα το λάστιχο + length ring στο κλικ.
- **2026-07-18 (§opening-width fix 2/2 — wall-axial frame, ΟΧΙ opening rotation)** 🔴 **Root-cause runtime bug** (console
  diagnosis): το delta υπολογιζόταν κατά τον άξονα `opening.geometry.rotation`, που μπορεί να είναι **αντιπαράλληλος**
  με τη φορά `wall.start→wall.end` (π.χ. κούφωμα σε τοίχο δεξιά→αριστερά· rotation=0 ενώ +axial=−x). Έτσι το delta
  `(+1000,0)` πήγαινε ΑΝΤΙΘΕΤΑ στο πλαίσιο που μετρά το `resizeJamb` (`projectPointToWallOffsetMm`) → η παρειά ξεπερνούσε
  την αντίθετη → clamp (700→100 αντί 700→800). **Fix:** ο resolver παίρνει πλέον τον **host τοίχο** και υπολογίζει την
  +axial world κατεύθυνση με **gradient probe** του `projectPointToWallOffsetMm` (δουλεύει για κεκλιμένους/καμπύλους,
  ανεξαρτήτως μονάδων). Υπογραφή: `resolveOpeningWidthLockedDelta(hostWall, kind, gripPos, cursor)`· και τα δύο seams
  (ghost + commit) αναλύουν τον host τοίχο (commit από scene manager, ghost από level scene). Νέο test «αντιπαράλληλος
  τοίχος» πράσινο.
- **2026-07-18 (§opening-width fix — mm unit)** — Μετά το wall-axis fix, η φορά/μεγέθυνση δούλεψε, αλλά η τιμή
  εφαρμοζόταν **×10** (πληκτρολόγηση `10` → +100mm). Αιτία: ο κοινός `lengthRingField` ερμηνεύει την είσοδο στη
  **global display μονάδα** (εδώ cm) → `10`=10cm=100mm. Το πλάτος κουφώματος όμως είναι **mm-native** (ribbon
  combobox «Πλάτος» = 700/800/900… mm). **Fix:** το `OPENING_WIDTH_RING_CONFIG` χρησιμοποιεί πλέον αποκλειστικό
  **mm-native field** (`openingWidthMmField`) — η είσοδος = mm (`10`→10mm), lock σε scene units (`mm×mmToScene`)
  στο ΙΔΙΟ `DynamicInputLockStore`· ο resolver το διαβάζει και μετακινεί την παρειά τόσα mm. Νέο test
  `opening-width-ring-config` (4) πράσινο· jscpd καθαρό. (Ο κοινός `lengthRingField` τοίχου/γραμμής αμετάβλητος.)
...
- **2026-07-18 (Opus 4.8) — §grip-parity-hotgrip fix: click σε λαβή reshape = ΚΟΚΚΙΝΗ ΑΝΕΞΑΡΤΗΤΑ από τη ΔΥΝ
  (ungate + `!isShift`).** 🔴 **Root-cause του «μένει ΠΟΡΤΟΚΑΛΙ»:** το line-endpoint ΚΑΙ το vertex/edge reshape
  entry (`grip-mouse-handlers.ts`) ήταν gated στο `cadToggleState.isDynInputOn()`. Με **Δυναμική Εισαγωγή OFF**
  (default) το `resolveVertexReshapeHotGrip`/`resolveLineEndpointHotGrip` ΔΕΝ καλούνταν → η λαβή έπεφτε στο
  press-drag path → mouseup <4px → `applyGripArmClick` (ADR-501) → **ΠΟΡΤΟΚΑΛΙ arm** αντί κόκκινο reshape.
  **Big-player audit (AutoCAD/Revit/Figma):** κλικ σε λαβή = HOT + άμεσο edit **ανεξάρτητα** από τον διακόπτη
  dynamic-input· ο διακόπτης ελέγχει ΜΟΝΟ αν φαίνεται το πληκτρολογημένο input tooltip — ΟΧΙ το κόκκινο/λάστιχο.
  **Fix:** και τα δύο gates `isDynInputOn()` → `!isShift`. Το click-move-click δουλεύει και ΧΩΡΙΣ ring γιατί το
  grab-click mouseup παίρνει `'stay'` (`!movedSinceArm`) στο `wall-hot-grip-fsm` (μένει hot· το 1ο moved click
  κάνει commit)· το «Δαχτυλίδι Εντολών» Μήκος/Γωνία εξακολουθεί να mount-άρεται ΜΟΝΟ με ΔΥΝ ON
  (`DynamicInputSubscriber` gate αμετάβλητο). **ADR-501 ΑΜΕΤΑΒΛΗΤΟ + διορθωμένο:** πλέον **Shift+click** →
  press-drag → ΠΟΡΤΟΚΑΛΙ multi-grip arm (πριν, με ΔΥΝ ON, το Shift έμπαινε ΛΑΘΟΣ σε reshape· τώρα σωστά arm).
  **MOD (1):** `grip-mouse-handlers.ts` (2 gates + αφαίρεση αχρησιμοποίητου `cadToggleState` import). Μηδέν νέος
  κώδικας/FSM/store — reuse όλου του υπάρχοντος endpoint-stretch. **Tests:** `vertex-reshape-hotgrip` (11) +
  `line-endpoint-hotgrip` + `GripArmedStore` + `grip-marquee-arm` **37/37 GREEN**. ΟΧΙ tsc (N.17). 🔴 browser-verify
  (ΔΥΝ **OFF** → ορθογώνιο → κλικ γωνία/μέσο-πλευράς → **ΚΟΚΚΙΝΗ** + ακολουθεί κέρσορα → κλικ = commit· Shift+click
  → ΠΟΡΤΟΚΑΛΙ multi-select ΑΜΕΤΑΒΛΗΤΟ) + commit (Giorgio).
- **2026-07-18 (Opus 4.8) — §grip-parity: ο ΠΑΡΑΓΩΓΟΣ του `isVertexReshapeDragInfo` (επιδιόρθωση σπασμένου
  build).** Το `a0cfcbce` πέρασε **μόνο τους καταναλωτές** — `DynamicInputSubscriber.tsx` (import + `vertexReshapeDrag`
  gate) και `grip-mouse-handlers.ts` (`vertexReshape: true` στο hot-grip enter) — ενώ ο παραγωγός έμεινε uncommitted
  στο working tree: το HEAD **δεν εξήγαγε καθόλου** τη συνάρτηση, άρα το subapp δεν έχτιζε. **MOD (1):**
  `systems/cursor/GripDragStore.ts` — προαιρετικό `vertexReshape?: boolean` στο `ActiveDragGripInfo` + καθαρό
  predicate `isVertexReshapeDragInfo`, **ακριβές mirror του `isLineEndpointDragInfo`** (ίδιο σχήμα, ίδιο σχόλιο-
  σύμβαση). Το flag τίθεται **ΜΟΝΟ** στο vertex-reshape hot-grip enter, οπότε ταυτοποιεί μοναδικά το drag που
  mount-άρει το Μήκος/Γωνία δαχτυλίδι. Μηδέν νέο store, μηδέν νέα κατάσταση — το `GripDragStore` παραμένει
  zero-React SSoT. **Μάθημα:** ο διαχωρισμός παραγωγού/καταναλωτή σε ξεχωριστά commits περνά αθόρυβα από τον hook
  (το `tsc` του DXF subapp τρέχει **μόνο** στο CI — ADR-663 CHECK 3.29)· stage-άρε το export μαζί με το import.
- **2026-07-18 (Opus 4.8) — §grip-parity Φάση Α+Β: το 5ο σκαλί (typed value σε λαβή ΜΕΤΑΚΙΝΗΣΗΣ) + η
  ΛΕΥΚΗ ένδειξη μήκους/γωνίας στη ροή armed λαβής.** Ζητούμενο Giorgio: «κλικ σε λαβή → κόκκινη → κόκκινος
  σταυρός + κίτρινη διακεκομμένη + **ένδειξη απόστασης** → δεύτερο κλικ Ή πληκτρολογημένη τιμή, ΚΑΘΟΛΙΚΑ,
  με σεβασμό σε ORTHO (F8) και βηματισμό (F9+Q)».
  **SSoT audit πριν τον κώδικα — τι ΥΠΗΡΧΕ ΗΔΗ (μηδέν νέο):** η FSM (`wall-hot-grip-fsm`), ο ΕΝΑΣ commit
  dispatcher (`commitDxfGripDragModeAware`), ο κόκκινος σταυρός (`drawMoveBasePointMarker`), η κίτρινη
  διακεκομμένη (`drawRubberBandLine`) — και τα δύο ήδη ΚΟΙΝΑ με το εργαλείο Μετακίνηση του Ribbon — καθώς και
  τα ORTHO/βήμα constraints (`applyMoveConstraints`). **Καμία νέα FSM, κανένα νέο commit path, κανένα νέο store.**
  **ΚΕΝΟ 1 — typed value σε `movesEntity: true`:** καμία από τις 4 βαθμίδες της σκάλας κλειδωμάτων δεν έπιανε τη
  λαβή ΜΕΤΑΚΙΝΗΣΗΣ (το `vertex-reshape-lock` την απορρίπτει ΡΗΤΑ), άρα ο move-σταυρός **κάθε** οντότητας έδειχνε
  λάστιχο αλλά **δεν δεχόταν τιμή** — νεκρό ακριβώς στην πιο βασική χειρονομία. **NEW (2):**
  `systems/dynamic-input/displacement-lock-core.ts` (ο ΕΝΑΣ πυρήνας ORTHO/POLAR→typed-length) +
  `move-displacement-lock.ts` (μόνο eligibility). **MOD:** `vertex-reshape-lock.ts` → delegate στον πυρήνα·
  `grip-ghost-locked-delta.ts` → 5ο σκαλί (ΤΕΛΕΥΤΑΙΟ: τα 1-4 είναι όλα `movesEntity !== true`, άρα αμοιβαία
  αποκλειόμενα — η σειρά δεν κλέβει λαβή)· `grip-mouseup-handler.ts` → ΙΔΙΟΣ resolver στα **δύο** commit seams
  (hot-grip + press-drag) πριν τα ORTHO/βήμα constraints → «ρητή είσοδος = τελική γεωμετρία» + preview ≡ commit +
  **διπλή επιλογή** (drag-and-drop ΚΑΙ click+τιμή) που δεν μπορεί να αποκλίνει· `GripDragStore.ts` →
  `isMoveDisplacementDragInfo` (gate ΚΑΙ στο `dragAnchor`: το move hot-grip είναι 3-click και το δαχτυλίδι δεν
  πρέπει να εμφανιστεί στο βήμα «διάλεξε σημείο βάσης», όπου ένα μήκος δεν έχει αρχή να μετρήσει).
  ⚠️ **N.18 — γιατί ΤΡΙΤΟ module αντί για δίδυμο:** το `move-displacement-lock` θα ήταν token-ταυτόσημο με το
  `vertex-reshape-lock` πλην eligibility → structural clone που το name-based `ssot:discover` **δεν βλέπει**.
  Ένας πυρήνας, N eligibility gates. Anchor: test «ισοδυναμία με τον αδελφό» — κοκκινίζει αν κάποιος ξαναγράψει
  δεύτερη ORTHO μαθηματική.
  **ΚΕΝΟ 2 — η ένδειξη απόστασης (σύγκρουση με ADR-560, ΕΠΙΛΥΘΗΚΕ ΩΣ ΜΗ-ΣΥΓΚΡΟΥΣΗ):** ο Giorgio διευκρίνισε με
  στιγμιότυπο (210337) ότι εννοεί τις **λευκές ενδείξεις της σχεδίασης γραμμής** (aligned διάσταση μήκους +
  `∠ γωνία`) — **ΟΧΙ** πινακίδα: «ΔΕΝ θέλω πινακίδα». Άρα το ADR-560 (κατάργηση `drawDimPill`) **μένει ακέραιο**·
  πρόκειται για άλλη ένδειξη. Το grip ghost preview **δεν ζωγράφιζε καθόλου** το HUD (μηδέν χρήσεις
  `drawWallHud`/`liveDimHud` σε όλο το `hooks/tools/`), ενώ το ίδιο το `length-angle-hud-gate.ts` ορίζει ρητά ως
  πεδίο εφαρμογής «κατά τη ΣΧΕΔΙΑΣΗ **και κατά το GRIP-DRAG**» → κλειστό τεκμηριωμένο κενό, όχι επέκταση σκοπού.
  **MOD (2):** `grip-ghost-preview-overlay-helpers.ts` → `paintGripArmedDistanceHud` (ΙΔΙΟΣ `paintWallHud` +
  ΙΔΙΟ `isLengthAngleHudVisible()` gate με τη σχεδίαση — μηδέν νέος painter/toggle· διαστασιολογεί ΤΟ ΛΑΣΤΙΧΟ,
  δηλαδή `anchorPos → anchorPos+delta`, άρα το νούμερο είναι το ήδη ORTHO/βήμα/typed constrained delta = WYSIWYG)·
  `useGripGhostPreview.ts` → κλήση μαζί με τα λάστιχα, **ΠΡΙΝ** το early-return του `drawGroupGhost` ώστε να τη
  δείχνει και η μετακίνηση GROUP/BLOCK. Εξαιρείται η ΠΕΡΙΣΤΡΟΦΗ (εκεί η ρητή ένδειξη είναι το χρωματιστό τόξο
  φοράς με τις μοίρες).
  **ΒΗΜΑΤΙΣΜΟΣ — καμία αλλαγή (Giorgio 2026-07-18):** το handoff το χαρακτήριζε «ασάφεια»· δεν είναι. «F9
  ενεργοποιεί το SNAP, το Q τον βηματισμό» = ακριβώς το `isGripStepActive()` (`isSnapOn() && QKeyTracker`).
  Το μοντέλο επιβεβαιώθηκε ως εσκεμμένο· καλείται το υπάρχον `applyMoveConstraints()`.
  **Boy Scout (N.0.2):** τα 3 σχεδόν ταυτόσημα `<RadialCommandRing>` blocks του `DynamicInputSubscriber.tsx`
  (διέφεραν ΜΟΝΟ σε predicate/config/startKey) συμπτύχθηκαν σε πίνακα `GRIP_RING_VARIANTS` — αλλιώς η προσθήκη
  του 4ου θα ήταν sibling clone. Σειρά ειδικό→γενικό, η μετακίνηση τελευταία → μηδέν regression στα υπάρχοντα.
  **Tests:** νέο `move-displacement-lock.test.ts` (10) + regression sweep grips/tools/dynamic-input/cursor
  **740/740 GREEN**. `jscpd:diff` 9 αρχεία **καθαρό**. ΟΧΙ tsc (N.17).
  🔴 **ΕΚΚΡΕΜΕΙ browser-verify (Giorgio):** κλικ σε move-σταυρό → κόκκινη + σταυρός + κίτρινη διακεκομμένη +
  **λευκό μήκος/γωνία**· ΔΥΝ ON → πληκτρολόγηση τιμής + Enter = μετακίνηση κατά την τιμή· ORTHO/F9+Q σεβαστά·
  drag-and-drop ΑΜΕΤΑΒΛΗΤΟ· τα υπάρχοντα δαχτυλίδια (άκρο γραμμής / κορυφή / πλάτος κουφώματος) ΑΘΙΚΤΑ.
  **ΕΚΚΡΕΜΕΙ Φάση Γ** (καθολικότητα στις λαβές ΑΛΛΑΓΗΣ ΜΕΓΕΘΟΥΣ — απαιτεί το axis-box engine να δεχτεί
  διακριτό commit αντί συνεχούς delta· ο Giorgio ενέκρινε «ΟΛΕΣ»).
- **2026-07-18 (Opus 4.8) — §grip-parity Φάση Γ: ΚΑΘΟΛΙΚΟΤΗΤΑ — οι λαβές ΑΛΛΑΓΗΣ ΜΕΓΕΘΟΥΣ έγιναν click-armed.**
  **Repro Giorgio:** επιλογή κολόνας → κλικ σε **γωνία/μεσοπλευρική** → έμενε **ΠΟΡΤΟΚΑΛΙ** (ADR-501 arm)
  αντί για κόκκινη reshape με σταυρό + διακεκομμένη.
  🔴 **ΔΙΟΡΘΩΣΗ ΠΡΟΗΓΟΥΜΕΝΗΣ ΕΚΤΙΜΗΣΗΣ (ήταν ΛΑΘΟΣ):** η ίδια η Φάση Α+Β εγγραφή είχε δηλώσει ότι η Φάση Γ
  «απαιτεί το axis-box engine να δεχτεί διακριτό commit αντί συνεχούς delta». **ΔΕΝ ΙΣΧΥΕΙ.** Grep-verified:
  το hot-grip `'corner'` και το press-drag καταλήγουν στην **ΙΔΙΑ γραμμή** — `applyResizeConstraints(rawDelta)`
  → `commitDxfGripDragModeAware` (`grip-mouseup-handler.ts`, δύο branches, ίδιο ζεύγος κλήσεων). Μηδέν αλλαγή
  σε engine. Προϋπάρχουσα απόδειξη που το επιβεβαιώνει: τα `mep-fixture-corner-*` ήταν **ήδη** στο registry ως
  `'corner'` και έκαναν resize κανονικά. Ο αρχικός αποκλεισμός τεκμηριωνόταν ως «resize is a plain drag» —
  **απόφαση προϊόντος, όχι τεχνικό εμπόδιο**. **Μάθημα: μη μεταφέρεις σχόλιο-αιτιολογία ως τεχνικό φράγμα
  χωρίς να ιχνηλατήσεις το commit path.**
  **MOD (1 production αρχείο):** `wall-hot-grip-fsm.ts` → **+40 γραμμές στο `HOT_GRIP_OP_REGISTRY`** (66→106
  kinds), όλες `'corner'` (η λαβή είναι η άγκυρα, `tracking` τερματικό): κολόνα 4 γωνίες + `width`/`depth`/
  `edge-w`/`edge-s`· πέδιλο (καθρέφτης κολόνας, 8)· κείμενο 4 γωνίες + 4 μεσοπλευρικές· σκάλα 4 γωνίες +
  `width`/`length`/`landing-depth`· δοκός `start`/`end`/`width`· τοίχος `start`/`end`/`thickness`·
  `mep-fixture-diameter`. **Καμία νέα FSM/commit path/store** — μόνο γραμμές πίνακα, ο τεκμηριωμένος
  μηχανισμός επέκτασης. **Το press-drag ΠΑΡΑΜΕΝΕΙ** ως εναλλακτική χειρονομία (η «διπλή επιλογή»).
  **ΤΑ ΟΡΙΑ ΠΟΥ ΚΡΑΤΗΘΗΚΑΝ ΣΚΟΠΙΜΑ** (τεκμηριωμένα σε tests, όχι σιωπηλά):
  · **Λαβές καμπυλότητας** (`wall-curve`, `beam-curve`, `wall-arc-apex`) — βέλος/bulge, όχι μετατόπιση
    σημείου· ένα 2-click «άφησέ το εδώ» έχει ασαφή σημασιολογία. Press-drag.
  · **Παράμετροι διατομής** (`column-i-web-thickness`, `column-arm-length`, `column-flange-length` κ.λπ.) —
    δεν είναι γεωμετρία περιγράμματος. Press-drag μέχρι να ζητηθούν ρητά.
  · **Δυναμικά index-templated kinds** (`slab-vertex-N`, `roof-vertex-N`, `column-poly-vertex-N`,
    `*-edge-midpoint-N`) — **δεν μπορούν** να ζήσουν σε στατικό registry· θέλουν predicate, όπως το
    υπάρχον bespoke `vertex-reshape-hotgrip`. **Επόμενο βήμα αν ζητηθεί.**
  **Tests:** `wall-hot-grip-fsm.test.ts` — 6 assertions «stays press-drag (null op)» **χαρακτηρίστηκαν
  εκ νέου** ώστε να δηλώνουν τη νέα συμπεριφορά + να φυλάνε τα παραπάνω όρια (58/58). Το `NON_CORNER_KINDS`
  filter δένεται πλέον στο `isWallHotGripKind` αντί σε χειρόγραφη λίστα → δεν ξανα-σπάει σε επόμενη επέκταση.
  Στοχευμένο sweep grips/systems.grip/bim.columns/bim.grips **1232/1232 GREEN**· πλήρες sweep πριν τη
  διόρθωση **11898/11905** (μόνο αυτό το suite κόκκινο). `jscpd:diff` καθαρό. ΟΧΙ tsc (N.17).
  🔴 **ΕΚΚΡΕΜΕΙ browser-verify (Giorgio):** κολόνα → κλικ σε **γωνία** → **ΚΟΚΚΙΝΗ** + ακολουθεί τον κέρσορα
  + λευκό μήκος/γωνία → κλικ = commit. Ίδιο σε πέδιλο/κείμενο/σκάλα/δοκό/**τοίχο** (⚠️ ο τοίχος είναι το
  υψηλότερου ρίσκου: `wall-start`/`wall-end` είναι οι πιο δοκιμασμένες λαβές του συστήματος — **έλεγξέ τον
  πρώτο**). Shift+click → ΠΟΡΤΟΚΑΛΙ multi-select ΑΜΕΤΑΒΛΗΤΟ. Press-drag ΑΜΕΤΑΒΛΗΤΟ παντού.
- **2026-07-18 (Opus 4.8) — §grip-parity Φάση Δ: ΕΝΑ σύστημα affordance + typed value στις λαβές ΑΛΛΑΓΗΣ ΜΕΓΕΘΟΥΣ.**
  **Αίτημα Giorgio (αυτούσιο νόημα):** «Υπάρχουν **δύο** συστήματα. Θέλω **το ένα**. Κλικ σε λαβή — ακριανή ή
  μεσαία — **κόκκινος σταυρός** στο σημείο του κλικ + **ΚΙΤΡΙΝΗ** διακεκομμένη ως τον κέρσορα, ακριβώς όπως η
  εντολή **Μετακίνηση** της καρτέλας **Αρχική**.» Και: «όταν έχω κάνει κλικ σε λαβή, θέλω **δύο** δυνατότητες —
  πληκτρολογώ τιμή + Enter, **ή** κλικ στον καμβά.»

  **Δ1 — Η ΡΙΖΑ ΤΩΝ «ΔΥΟ ΣΥΣΤΗΜΑΤΩΝ» ΗΤΑΝ ΕΝΑ `if/else-if`.** `useGripGhostPreview.ts` — το διαχωριστικό ήταν
  το `dp.movesEntity === true`: λαβές που μετακινούν ΟΛΗ την οντότητα → κόκκινος σταυρός + κίτρινη· **κάθε**
  λαβή reshape/resize → ΚΥΑΝΗ **χωρίς** σταυρό, με ρητό σχόλιο *«Kept neutral (not gold) — these are not
  whole-entity base-point moves»*. Ήταν **σκόπιμη σχεδιαστική επιλογή**, την οποία ο Giorgio ανέτρεψε ρητά:
  ο χρήστης δεν διακρίνει «μετακινώ» από «αναμορφώνω» — και στις δύο περιπτώσεις έπιασε ένα σημείο και το πάει
  αλλού, άρα βλέπει την ΙΔΙΑ ένδειξη. **Οι δύο branches συγχωνεύθηκαν σε έναν** (+5 γραμμές, 488→493).
  **Μηδέν νέος painter** — οι `drawMoveBasePointMarker` + `drawMoveRubberBand` ήταν ήδη οι κοινοί SSoT με το
  Ribbon MOVE tool. **ΜΟΝΗ ΕΞΑΙΡΕΣΗ Η ΠΕΡΙΣΤΡΟΦΗ** (`dp.rotatePivot` set): κρατά τον κυανό leader από το ΚΕΝΤΡΟ
  — δεν είναι μετατόπιση σημείου, και έχει ήδη τη δική της πλήρη ένδειξη (pivot ⊙ + τόξο φοράς 🟢/🔴 + μοίρες).
  ⚠️ Η **πορτοκαλί** γραμμή που ανέφερε ο Giorgio είναι **ΤΡΙΤΟ, ΑΣΧΕΤΟ** πράγμα — `paintPolarTrackingLine` =
  ένδειξη **κλειδώματος γωνίας** POLAR, όχι rubber band. Επιβεβαιώθηκε, **δεν αγγίχθηκε**.

  **Δ2 — ΤΟ HANDOFF ΥΠΟΤΙΜΟΥΣΕ ΤΟ ΚΕΝΟ (διόρθωση εκτίμησης).** Το handoff περιέγραφε «νέα γραμμή στον πίνακα
  + σκαλί». **Grep-verified: οι λαβές της Φάσης Γ δεν πιάνονταν από ΚΑΝΕΝΑ από τα 5 σκαλιά** — το
  `isVertexReshapeGrip` δέχεται **μόνο** `arc | polyline | lwpolyline | rectangle`, οπότε μια `column-width` /
  `wall-thickness` / `stair-width` έπεφτε έξω από ΟΛΗ τη σκάλα. Έλειπε **eligibility gate**, όχι γραμμή πίνακα.

  **🎯 ΣΗΜΑΣΙΟΛΟΓΙΚΗ ΑΠΟΦΑΣΗ (Giorgio, ρητή ερώτηση-απάντηση): ΜΕΤΑΤΟΠΙΣΗ, ΟΧΙ ΑΠΟΛΥΤΗ ΔΙΑΣΤΑΣΗ.**
  Κολόνα πλάτος 300 → πιάνω λαβή παρειάς → γράφω 500 → **πλάτος 800** (η λαβή μετακινήθηκε 500). Δύο λόγοι:
  · **Ομοιομορφία** — ίδια σημασιολογία με ΚΑΙ ΤΑ 5 προηγούμενα σκαλιά (παρειά κουφώματος, κορυφή πολυγραμμής,
    άκρο τόξου, λαβή μετακίνησης) και με την εντολή «Μετακίνηση». ΕΝΑ νοητικό μοντέλο για όλες τις λαβές.
  · **Το engine ΔΕΝ είναι ενιαίο** (μετρημένο): ορθογώνια κολόνα/πέδιλο/τοίχος = **1:1** με απέναντι πλευρά
    σταθερή (`rect-grip-engine.ts:93-127`)· **πολυγωνική** κολόνα = **2:1 συμμετρικά** γύρω από το κέντρο
    (`column-grips.ts:443-479`). Απόλυτη διάσταση θα απαιτούσε μητρώο «λαβή→παράμετρος→αναλογία» με 40+
    εγγραφές που μπορούν να αποκλίνουν **σιωπηλά** από το engine. Η μετατόπιση δουλεύει ΑΝΕΞΑΡΤΗΤΑ αναλογίας.
  **Big-player σημείωση (τιμιότητα):** το AutoCAD grip-stretch με Dynamic Input δείχνει **και τα δύο** πεδία
  (μετατόπιση + διάσταση)· το Revit δείχνει την **απόλυτη** προσωρινή διάσταση. Δηλαδή η επιλογή (α) είναι
  **υποσύνολο** της πρακτικής τους, όχι απόκλιση — η 2η φέτα «Πλάτος» μπορεί να προστεθεί αργότερα χωρίς να
  αναιρέσει τίποτα από αυτή τη φάση. Τέθηκε ρητά στον Giorgio ως επιλογή (γ)· επέλεξε (α).

  **ΤΟ ΕΥΡΟΣ ΔΙΑΒΑΖΕΤΑΙ ΑΠΟ ΤΟ ΥΠΑΡΧΟΝ ΜΗΤΡΩΟ, ΟΧΙ ΑΠΟ ΔΕΥΤΕΡΗ ΛΙΣΤΑ:** επιλέξιμη είναι κάθε λαβή που το
  `HOT_GRIP_OP_REGISTRY` χαρακτηρίζει `'corner'` (= «η ίδια η λαβή είναι η άγκυρα, tracking τερματικό» =
  αλλαγή μεγέθους). Οι `'move'`/`'rotate'` έχουν δικά τους σκαλιά· το `'endpoint-stretch'` σερβίρεται από τα
  σκαλιά 2-4. **Μηδέν επικάλυψη, μηδέν δεύτερη λίστα προς συντήρηση** — νέα λαβή resize μπαίνει στο μητρώο
  ΜΙΑ φορά και κληρονομεί αυτόματα το typed value.

  **NEW (1):** `systems/dynamic-input/resize-grip-lock.ts` (77 γρ.) — **ΜΟΝΟ eligibility**· τα μαθηματικά
  καταναλώνονται από τον υπάρχοντα κοινό πυρήνα `displacement-lock-core.ts` (N.18: γραμμένο ως αδελφός των
  `vertex-reshape-lock`/`move-displacement-lock` θα ήταν structural clone).
  **MOD (5):** `useGripGhostPreview.ts` (Δ1 συγχώνευση) · `grip-ghost-locked-delta.ts` (**σκαλί 5**, το
  `movesEntity` γίνεται 6ο) · `grip-mouseup-handler.ts` (**commit seam**, ΙΔΙΟΣ resolver → preview ≡ commit) ·
  `GripDragStore.ts` (`isResizeGripDragInfo`) · `DynamicInputSubscriber.tsx` (**γραμμή** στον `GRIP_RING_VARIANTS`,
  **όχι** 5ο `if`-block). **Καμία νέα FSM, κανένα νέο commit path, κανένας νέος painter, κανένα νέο ring config.**

  **ΤΟ ΚΕΝΟ ΠΟΥ ΚΛΕΙΝΕΙ ΤΟ ΚΟΙΝΟ PREDICATE:** το `isResizeGripKind` πυλωρεί **ΚΑΙ** το mount του δαχτυλιδιού
  **ΚΑΙ** την αποδοχή της τιμής. Ένα predicate → **αδύνατον** να εμφανιστεί δαχτυλίδι σε λαβή που δεν δέχεται
  τιμή (ή το αντίστροφο) — το κλασικό «το ring φαίνεται αλλά το Enter δεν κάνει τίποτα».

  **Tests:** νέο `__tests__/resize-grip-lock.test.ts` — eligibility από το μητρώο, μη-επικάλυψη με σκαλιά 1-4/6,
  ORTHO+typed σύνθεση, **συμβόλαιο ισοδυναμίας** με τους δύο αδελφούς (αν κάποιος γράψει δεύτερη ORTHO
  μαθηματική, κοκκινίζει), **και δίχτυ πλήθους** στο μητρώο (≥40 `'corner'`) ώστε το anchor να μη μείνει
  πράσινο πάνω σε κενό σύνολο. Sweep dynamic-input/grips/cursor/tools: **80 suites, 791/791 GREEN**.
  `jscpd:diff` καθαρό. Όλα τα αρχεία <500 (N.7.1). ΟΧΙ tsc (N.17).

  🔴 **ΕΚΚΡΕΜΕΙ browser-verify (Giorgio):** (1) κλικ σε **γωνία/μεσοπλευρική** κολόνας → **κόκκινος σταυρός** +
  **ΚΙΤΡΙΝΗ** διακεκομμένη (όχι κυανή, όχι «χωρίς σταυρό»). (2) Ίδια εικόνα σε τοίχο/πέδιλο/κείμενο/σκάλα/δοκό.
  (3) **Περιστροφή αμετάβλητη** — κυανός leader από το κέντρο + τόξο φοράς. (4) Με ΔΥΝ ON: κλικ σε λαβή
  πλάτους → δαχτυλίδι → «500» + Enter → **η λαβή μετακινείται 500** (πλάτος 300→800). (5) Χωρίς πληκτρολόγηση,
  σκέτο κλικ στον καμβά → commit εκεί (η «δεύτερη δυνατότητα»). (6) Press-drag ΑΜΕΤΑΒΛΗΤΟ παντού.
- **2026-07-18 (Opus 4.8) — §grip-parity Φάση Δ ΔΙΟΡΘΩΣΗ: κύκλος αρχικοποίησης τύφλωνε το mount του δαχτυλιδιού.**
  **Repro Giorgio:** κλικ σε λαβή resize → κόκκινος σταυρός + κίτρινη + ghost **ΟΛΑ ΣΩΣΤΑ**, αλλά με ΔΥΝ ON
  «πληκτρολογώ τιμή + Enter → **δεν συμβαίνει τίποτα**». (Το άκρο γραμμής — ίδιο ring config — δούλευε.)
  **Ρίζα (self-inflicted, εντοπίστηκε με `madge --circular`):** η 1η απόπειρα της Φάσης Δ όρισε το eligibility
  predicate `isResizeGripKind` **μέσα** στο `resize-grip-lock.ts` (μαζί με τα μαθηματικά) και το `GripDragStore`
  το εισήγαγε από εκεί. Έτσι το **low-level** `GripDragStore` (φορτώνεται νωρίς μέσω `QKeyTracker`/
  `grip-step-quantize`) άρχισε να σέρνει ΟΛΟ το βαρύ graph `displacement-lock-core → drawing-handler-utils →
  preview-canvas → useUnifiedDrawing` στο import graph του → **κύκλος αρχικοποίησης**. Το `DynamicInputSubscriber`
  φόρτωνε με το `isResizeGripDragInfo`/`isResizeGripKind` σε μη-έγκυρη κατάσταση → ο πίνακας `GRIP_RING_VARIANTS`
  δεν έβρισκε τη variant → **το δαχτυλίδι δεν mount-άριζε** → το heads-up keydown listener του ring δεν
  υπήρχε → η πληκτρολόγηση/Enter δεν πιανόταν πουθενά → «τίποτα». Ο κόκκινος σταυρός/κίτρινη/ghost ζουν σε
  ΑΛΛΟ component (`useGripGhostPreview`), γι' αυτό έμεναν σωστά — ακριβώς η εικόνα που περιέγραψε ο Giorgio.
  **Γιατί το άκρο γραμμής δούλευε:** το `isLineEndpointDragInfo` ήταν ήδη στο `GripDragStore` **πριν** τη
  Φάση Δ, όταν το store ήταν ακόμα ελαφρύ (μηδέν εξάρτηση μαθηματικών). Η Φάση Δ το «βάρυνε».
  **FIX:** το `isResizeGripKind` (καθαρό registry-lookup `hotGripOpForKind(kind) === 'corner'`, ΜΗΔΕΝ
  εξάρτηση μαθηματικών) μετακινήθηκε **δίπλα στο registry**, στο `wall-hot-grip-fsm.ts` (type-only deps).
  Και τα δύο (`GripDragStore` + `resize-grip-lock`) το εισάγουν πλέον από εκεί. Το `GripDragStore` **δεν
  εισάγει πλέον** `resize-grip-lock`/`displacement-lock-core` → κύκλος έφυγε (`madge` επιβεβαίωσε). Μηδέν
  αλλαγή συμπεριφοράς/API — καθαρά τοπολογία imports.
  **ΜΑΘΗΜΑ (γενικό):** ένα low-level imperative store (φορτώνεται από keyboard trackers/geometry helpers)
  **δεν** πρέπει ΠΟΤΕ να εισάγει module που σέρνει React/preview-canvas graph. Predicate που το χρειάζεται
  ένα τέτοιο store → βάλ' το δίπλα στα δεδομένα του (registry), όχι δίπλα στους καταναλωτές των μαθηματικών.
  **Tests:** dynamic-input+grips+cursor+tools **65 suites / 721 GREEN**· `resize-grip-lock` re-export καλύπτεται.
  `madge --circular` καθαρό στο νέο path. `jscpd:diff` καθαρό. ΟΧΙ tsc (N.17).
  🔴 **ΕΚΚΡΕΜΕΙ browser-verify (Giorgio):** κλικ σε λαβή πλάτους (ΔΥΝ ON) → **«500» + Enter → η λαβή
  μετακινείται 500** (πλάτος 300→800). Χωρίς πληκτρολόγηση, σκέτο κλικ καμβά → commit εκεί (η 2η δυνατότητα).
