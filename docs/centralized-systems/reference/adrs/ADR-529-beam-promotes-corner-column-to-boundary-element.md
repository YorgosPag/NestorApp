...
- **2026-06-27** — **§reframe: οι λαβές του δοκαριού (+αναλυτικό μήκος) έμεναν stale στην παλιά παρειά μετά την
  προαγωγή (UNCOMMITTED).** Giorgio (screenshot): επιλέγοντας το δοκάρι, οι λαβές στο ανατολικό άκρο εμφανίζονταν
  στη **δυτική παρειά της αρχικής (προ-προαγωγής) κολόνας — «σημείο 1»** (= nominal `endPoint.x=6951.39`,
  Firestore-verified) αντί στο **ορατό άκρο μετά την προαγωγή — «σημείο 2»** (= δυτική παρειά του ποδιού,
  6551.39). **ΡΙΖΑ:** οι beam grips υπολογίζονται από τα `params` (`getBeamGrips`), ενώ το ορατό δοκάρι (2D κάτοψη
  + 3D mesh) σχεδιάζει το **DERIVED ADR-458 cutback** (`displayOutline`, «η κολόνα νικάει» → κόβεται στη δυτική
  παρειά του ποδιού). Το `endPoint` (6951.39) **δεν είχε reframe-αριστεί** στη νέα παρειά → λαβές+αναλυτικό μήκος
  ≠ ορατό. **Γιατί δεν reframe-άρισε:** ο reframe cascade (ADR-492 `cascadeBeamReframe`) τρέχει **ΜΟΝΟ command-time
  μέσα στα transform commands** (Move/Rotate/Scale/Mirror), **ΟΧΙ** reactive στο `bim:column-params-updated`
  (ADR-492 §4: reactive reframe σε proactive analysis cycle → freeze). Η προαγωγή κάνει `UpdateColumnParamsCommand`
  (όχι transform) → ο cascade δεν κλήθηκε ΠΟΤΕ· το doc comment του `useColumnBeamPromote` («→ ο reframe cascade
  ξανα-πλαισιώνει») ήταν **λανθασμένο** (η σύνδεση έλειπε). Επαληθεύτηκε αριθμητικά ότι το `reframeBeamEndpointsTo-
  Columns` δίνει σωστά `endPoint.x = proj(4958.47) − columnSupportAlong(325) = 4633.47 → 6551.39` (= σημείο 2).
  **Fix (Επιλογή 1, Giorgio-approved «reframe στη νέα παρειά»):** στο `useColumnBeamPromote`, αμέσως μετά τα
  promotion commands, **command-time** (όχι reactive — σέβεται ADR-492 §4) reframe το προαχθέν δοκάρι reusing τον
  ΙΔΙΟ pure SSoT `reframeBeamEndpointsToColumns` (ADR-492) → undoable `UpdateBeamParamsCommand` + emit
  `bim:beam-params-updated`. Idempotent (από τη θέση κολόνας) → αμετάβλητο = null = μηδέν churn· το `bim:beam-params-
  updated` triggάρει το ήδη-υπάρχον `resyncPromotedBoundaryArmsForBeam` που κάνει no-op (armLength==width →
  convergence guard, μηδέν loop). Έτσι structural endpoint + λαβές (2D+3D, κοινό `getBeamGrips`) + ορατό cut άκρο
  **όλα συμπίπτουν στο σημείο 2**. **ΣΗΜΕΙΩΣΗ ειλικρίνειας:** το mismatch **δεν** ήταν bug του Φ9 (3D grips) —
  υπήρχε ήδη στο 2D (ίδιο `getBeamGrips` vs `displayOutline`)· το Φ9 το mirror-άρει πιστά· η προαγωγή σε Γ (πόδι
  400mm) το έκανε ορατό. **2 code (`useColumnBeamPromote.ts` glue + διόρθωση λανθασμένου doc) + test:** +2 ADR-529
  cases στο `beam-column-reframe.test.ts` (προαγωγή σε Γ → endpoint 3600 = νέα δυτική παρειά ποδιού· idempotent μετά
  reframe → null). **41/41 GREEN** (reframe + reframe-cascade + promote-junction, μηδέν regression). tsc SKIP (N.17)
  — glue + pure reuse, καλυμμένο από colocated jest. **Flagged (minor):** η προαγωγή παράγει N×UpdateColumn +
  1×UpdateBeam = πολλαπλά undo steps (όπως ήδη ο resync)· grouping σε ΕΝΑ undo = future polish. 🔴 browser-verify
  (δοκάρι→προαγωγή Γ→επίλεξε δοκάρι→λαβές ανατ. άκρου στο σημείο 2, ταυτισμένες με το ορατό άκρο) + commit=Giorgio
  (stage `useColumnBeamPromote.ts` + `beam-column-reframe.test.ts` + ADR-529).
- **2026-06-27** — **§reframe → SSoT (ADR-540):** το ad-hoc reframe block (ξεχωριστό `UpdateBeamParamsCommand` μέσα
  στο `useColumnBeamPromote`) **αφαιρέθηκε**. Το reframe γίνεται πλέον **αυτόματα**: το universal
  `reconcileAssociativeGeometry` (ADR-540) τρέχει μέσα στο `MergeableUpdateCommand` base → κάθε
  `UpdateColumnParamsCommand` (= η προαγωγή) καλεί `cascadeBeamReframe` command-time → το δοκάρι ξανα-κόβεται στη
  νέα παρειά + `bim:entities-moved`. Αποτέλεσμα: **ΕΝΑ undo step αντί δύο**, μηδέν σκόρπιος κώδικας (το προηγούμενο
  flagged «πολλαπλά undo steps» λύθηκε για το beam-reframe κομμάτι). Βλ. **ADR-540**. UNCOMMITTED.
