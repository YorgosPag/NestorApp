...
| **Τελευταίο μίλι — ΟΛΟΚΛΗΡΩΘΗΚΕ (Giorgio: «κλείσε και τα δύο»)** | (α) `renderInfoLabel` (preview) delegate-άρει πλέον per-line στο `paintMeasurementText(gate:true)` → **ΕΝΑΣ** painter atom παντού (κρατά το preview below-anchor layout + gate· τώρα τιμά και super/sub = συνέπεια με committed). (β) **Κοινή σειρά εμβαδόν→περίμετρος ΠΑΝΤΟΥ**: preview circle/arc-sector → area line πρώτα· preview **rectangle** → `buildAreaPerimeterLabelLines` (ίδιο content+σειρά+i18n με committed). |
| **Τελική κατάσταση** | ΕΝΑ atom (`paintMeasurementText`), ΕΝΑΣ content builder area/perimeter, ΕΝΑ metrics SSoT — committed + preview + hover + drag. Μηδέν διπλός painter/content για το area+perimeter label. |
