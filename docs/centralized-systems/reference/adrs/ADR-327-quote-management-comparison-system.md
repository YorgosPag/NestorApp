...
| Date | Changes |
|------|---------|
| 2026-09-24 | 🔴 **§7 — ΜΙΑ ΠΡΟΣΚΛΗΣΗ = ΜΙΑ ΑΠΑΝΤΗΣΗ ([ADR-876 §5.8](./ADR-876-credential-link-surfaces.md) Σ19).** Επαληθευμένο στον browser (emulator): η προσφορά της πύλης βρισκόταν με ερώτημα `(rfqId, vendorContactId)`· με χειροκίνητο email το `vendorContactId` είναι `''`, άρα ο **δεύτερος** προμηθευτής του ίδιου RFQ **έγραφε πάνω** στην προσφορά του πρώτου (1178 € → 620 €) και ο πρώτος θα έβλεπε τις τιμές του δεύτερου. Τώρα: `VendorInvite.quoteId` γράφεται στην πρώτη υποβολή και η προσφορά διαβάζεται **με ταυτότητα** (φράχτης εταιρείας + RFQ, κανένας δείκτης)· εφεδρεία ερωτήματος μόνο με πραγματική επαφή (Φ7). Επίσης (Σ17): το παράθυρο επεξεργασίας των 72ω (Q8) ήταν γραμμένο **3 φορές** με 3 `Date.now()` → `VENDOR_QUOTE_EDIT_WINDOW_HOURS` + μία στιγμή ανά υποβολή σε προσφορά + πρόσκληση. |
| 2026-09-24 | 🔐 **§7/§11 — «ΕΝΑΣ ΣΥΝΔΕΣΜΟΣ = ΕΝΑ ΔΙΑΠΙΣΤΕΥΤΗΡΙΟ» ([ADR-876 §5](./ADR-876-credential-link-surfaces.md)).** Το `vendor_invites.token` (ωμό, αναγνώσιμο από κάθε μέλος, χτιζόμενο σε URL **μέσα στον browser**) καταργήθηκε: κάθε σύνδεσμος είναι έγγραφο στο server-only `vendor_invite_credentials` με μόνο `nonceHash` (υπογραφή HMAC **και** hash έκδοσης ⇒ διαρροή μυστικού δεν αρκεί). Token στο **fragment** (`/vendor/quote#t=…`) + `Authorization: Bearer` στο API — ποτέ σε διαδρομή/log. ΕΝΑΣ αναλυτής (σκοπός × κατάσταση) πίσω από ΜΙΑ πόρτα-σύνορο ιδεμποτίας για GET/POST/decline/renew· κατάσταση `revoked` (≠ λήξη, που είναι πλέον παράγωγη)· αντιγραφή/επαναποστολή = νέος σύνδεσμος, ανάκληση ανά σύνδεσμο, ίχνος `vendor_invite`· «στείλε μου νέο σύνδεσμο» μόνο στο καταχωρημένο email. `vendor-portal-token-service.ts` + `vendor_invite_tokens` (μετά τη Φ7) αποσύρονται. Migration expand/contract (μηδέν διακοπή). |
| 2026-09-23 | 🔴 **§7 ΠΥΛΗ ΠΡΟΜΗΘΕΥΤΗ — ΑΠΡΟΣΙΤΗ ΑΠΟ ΤΟ `5ff0baa2`, ΘΕΡΑΠΕΥΤΗΚΕ ([ADR-876](./ADR-876-credential-link-surfaces.md)).** Η σελίδα ζούσε στο `/o/[workspace]` ⇒ κάθε προμηθευτής → `/login`· μετακινήθηκε στο `(auth)`. Ο σύνδεσμος άρνησης (`…/decline`) οδηγούσε σε διαδρομή **χωρίς σελίδα** ⇒ πλέον `?intent=decline` (GET = διάλογος, POST = πράξη, ασφαλές απέναντι σε Safe Links). Metadata από το SSoT `credential-link-page` · `decodeRouteParam`. Εκκρεμεί Βήμα 4: `tokenHash` αντί ωμού token, ανάκληση και στη σελίδα, «νέος σύνδεσμος». |
| 2026-07-17 | 🔁 **§6 `extractQuote` — το malformed/κενό JSON μπήκε στο ίδιο retry μονοπάτι με τα «λάθος νούμερα»** (Opus 4.8, UNCOMMITTED· απόφαση Giorgio 2026-07-16). **Η ασυνέπεια:** αριθμητικές ασυνέπειες → escalation retry με feedback (ακριβό, σωστό)· σκουπίδι-JSON ή κενή απάντηση → σιωπηλό `break` στο attempt 0. Δηλαδή **η πιο ανακτήσιμη αστοχία έπαιρνε τη ΛΙΓΟΤΕΡΗ προσπάθεια** — ανάποδα απ' ό,τι επιβάλλει το self-consistency pattern του §6.5 (Google Document AI). **Τι άλλαξε:** `break` → `continue` με feedback prompt· το escalation model ενεργοποιείται κανονικά στο attempt 1+, το `maxValidationRetries` παραμένει το φράγμα (καμία νέα διαδρομή κόστους — το ίδιο ταβάνι κλήσεων που ίσχυε ήδη για τα «λάθος νούμερα»). **Το μη-προφανές:** το `buildRetryFeedback` έλεγε *«Η προηγούμενη εξαγωγή είχε αριθμητικές ασυνέπειες… ξανακοίταξε την ευθυγράμμιση των στηλών»* — **λάθος καθοδήγηση** για αποτυχία parse· θα έστελνε το μοντέλο να κυνηγήσει ανύπαρκτο πρόβλημα. Έγινε **table-driven** (`RetryReason = 'numeric' | 'parse'` → `RETRY_HEADLINE` / `RETRY_CLOSING`) αντί για δεύτερη σχεδόν-ίδια συνάρτηση, που θα χτυπούσε το jscpd (N.18). Default `'numeric'` → το υπάρχον call site αμετάβλητο. Νέα exports: `EMPTY_OUTPUT_ISSUE`, `UNPARSEABLE_ISSUE` (prompt-facing, όχι UI → εκτός N.11, όπως τα υπόλοιπα issues του module). **Παγίδα που πιάστηκε:** το test mock αντικαθιστούσε **όλο** το `quote-analyzer.validation` module → τα νέα constants θα έφταναν `undefined` στον analyzer· το factory χρησιμοποιεί πλέον `jest.requireActual` και μοκάρει μόνο τις 2 συναρτήσεις. **Σκόπιμη αλλαγή συμπεριφοράς** → 2 characterization tests ενημερώθηκαν ρητά· **+1 νέο** (escalation μετά από unparseable). **Verification (❌ όχι tsc, N.17):** 36/36 procurement GREEN · `test:ai-pipeline:all` 1179 GREEN · `jscpd:diff` καθαρό. |
| 2026-05-04 | 🪟 **Hub master-detail panel per Quotes** — `/procurement/quotes/page.tsx` riscritta con split-panel Contacts-pattern. Stato selezione URL-persistent (`?quoteId=`). `onSelectQuote` wired su `QuoteList`. Desktop: `QuoteList` sinistra + `QuoteRightPane` destra (con `buildQuoteHeaderActions(rfq:null)` + status mutation via `PATCH /api/quotes/{id}` + delete). Mobile: lista `md:hidden` + `MobileDetailsSlideIn` overlay. Empty state: `DetailsContainer`. Action-required quotes pinned at top (`submitted|under_review|expired`). `onCreatePo`/`onViewPo`/`onEdit`/`onDuplicate` → coming-soon stub (da implementare in sessione separata). Files: `src/app/procurement/quotes/page.tsx` (REWRITE). **N.7.2 GOL: ✅ YES** — URL-state proattivo, no race (useQuotes real-time subscription auto-update), idempotente, belt-and-suspenders (buildQuoteHeaderActions + status FSM in API), SSoT (QuoteRightPane/QuoteDetailsHeader/MobileDetailsSlideIn riusati), await status mutations, lifecycle owner = quotes hub page. |
...
| 2026-07-16 | 🔌 **§6 — `openai-quote-analyzer` μετακόμισε στον Responses-API SSoT (ADR-294/373). UNCOMMITTED.** Το αρχείο ξανάγραφε ολόκληρο τον OpenAI client: τους 5 wire-protocol τύπους, το `extractOutputText`, τον βρόχο fetch+retry, και το κατέβασμα από το admin bucket — **όλα ταυτόσημα** με τον accounting document analyzer (ADR-584 jscpd: 664 tokens / 5 clones μεταξύ των δύο). Καταναλώνει πλέον `executeResponsesRequest` / `extractOutputText` / `beginVisionContent` από το `@/services/ai/openai-responses` και `downloadAdminObjectByPublicUrl` από το `@/lib/firebaseAdmin-storage`. **343 → ~245 γραμμές.** **Τι ΔΕΝ κεντρικοποιήθηκε (σκόπιμα, είναι domain policy όχι μηχανική):** ο βρόχος validation-retry με escalation model, το rasterize-PDF→PNG μονοπάτι (§6.7), τα prompts/schemas, οι normalizers, το factory (δικά του env vars: `OPENAI_QUOTE_VISION_MODEL` → `OPENAI_VISION_MODEL` → `gpt-4o`). Το `beginVisionContent()` κρατά **μόνο** το κοινό «prompt + input_image από URL» και επιστρέφει `pdfAttachmentPending` ώστε ο analyzer να κρατήσει ακέραιη τη δική του πολιτική PDF. **ΜΗΔΕΝ αλλαγή συμπεριφοράς — αποδεδειγμένη:** το αρχείο είχε **μηδέν** direct tests· γράφτηκαν **36 characterization tests ΠΡΙΝ** το refactor (wire protocol, τα 5 μονοπάτια `buildVisionContent`, ο πλήρης validation-retry loop, το factory) και έμειναν **36 GREEN ΑΜΕΤΑΒΛΗΤΑ** μετά. **🔴 Εύρημα που κλειδώθηκε ΩΣ ΕΧΕΙ (ΔΕΝ διορθώθηκε):** στο `extractQuote`, αν το `safeJsonParse` αποτύχει, ο βρόχος κάνει `break` **χωρίς retry** — ένα malformed-JSON response (πιθανό transient glitch) δεν παίρνει ποτέ δεύτερη ευκαιρία, ενώ ένα «συνεπές αλλά αριθμητικά λάθος» παίρνει escalation. Ασυνέπεια, όχι crash· χαρακτηρίστηκε με test, θέλει απόφαση Giorgio. **Verification (❌ όχι tsc, N.17):** 36/36 GREEN · `jscpd:diff` καθαρό (η πρώτη εκτέλεση έπιασε επιζών 62-token δίδυμο `buildVisionContent` → λύθηκε με το `beginVisionContent()`, ΟΧΙ με SKIP). |
