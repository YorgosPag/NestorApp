...
| 2026-04-27 | 🔓 **Storage public-upload SSoT (UBLA root-cause fix)** — Logo `<img>` rendeva 403 perché `bucket.file().makePublic()` no-op silenzioso su bucket UBLA (`pagonis-87766.firebasestorage.app`, default Firebase Storage moderno). Diagnosi: la curl di prova all'URL `storage.googleapis.com/{bucket}/{path}` ritornava 403 anche con `makePublic()` apparentemente riuscito; nessuna eccezione, nessun log di errore. Fix Google-level + SSoT: nuovo servizio `src/services/storage-admin/public-upload.service.ts::uploadPublicFile()` usa **Firebase Storage download token** (`firebaseStorageDownloadTokens` in metadata + URL `firebasestorage.googleapis.com/v0/b/.../o/...?alt=media&token=UUID`) — bypassa UBLA, non scade, funziona su bucket legacy E nuovi. Migrati 3 consumer rotti: `logo-extractor.ts`, `quotes/scan/route.ts` (PDF della quote), `email-inbound-attachments.ts` (Mailgun webhook). Nuovo modulo `storage-public-upload` aggiunto a `.ssot-registry.json` (tier 2, forbidden patterns: `\.makePublic\(\)` + `https://storage\.googleapis\.com/${`); 4 file legacy in allowlist (`upload/photo`, `telegram/media-download-storage`, `vendor/quote/upload`, `attendance-server-service`) — usano `adminStorage.bucket()` senza arg = bucket vecchio `.appspot.com` no-UBLA, dove `makePublic()` ancora funziona. |
