...
|---|---|---|
| 2026-06-09 | **Phase 1G.5 — 2Δ «Move-from-Characteristic-Point» (Alt + σύρσιμο από χαρακτηριστικό σημείο = μετακίνηση ΟΛΟΚΛΗΡΗΣ οντότητας, AutoCAD «move from base point»)**. Πρόβλημα (Giorgio): στο 2Δ τα BIM grips κάνουν ΠΑΝΤΑ παραμετρική επεξεργασία (ο τοίχος «φαρδαίνει» αντί να μετακινείται) — το `commitDxfGripDragModeAware` έχει ~20 `*GripKind` early-returns ΠΡΙΝ το `mode==='move'`, άρα για κάθε BIM οντότητα η μετακίνηση-από-σημείο ΔΕΝ υπάρχει· μόνο το κεντρικό σημάδι μετακινεί ολόκληρη την οντότητα. **Λύση (FULL SSOT, reuse μηδέν νέο σύστημα):** όταν κρατιέται **Alt** σε grip-drag → bypass ΟΛΩΝ των παραμετρικών paths → whole-entity move με βάση το grip (το `delta` ΗΔΗ μετριέται από `grip.position` στο `runGripMouseUp`, άρα το χαρακτηριστικό σημείο = base point δωρεάν). **Modifier=Alt**: το μόνο ελεύθερο στο 2Δ (Ctrl=COPY ADR-397, Shift=ortho, Space=pan/cycle). NEW vanilla singleton `keyboard/AltKeyTracker.ts` (πιστός αδελφός `CtrlKeyTracker`/`ShiftKeyTracker` — το mouseup χάνει το native event by design → read στο commit). **Σύνθεση Alt+Ctrl (ή right-click Copy toggle)** = clone-with-base via `CopyEntityCommand`. **SSoT helper** `commitWholeEntityMove` εξήχθη από το `mode==='move'` branch (κοινό με την Alt-bypass, αφαιρεί duplication). **🔴 Latent bug κλειστό (Boy-Scout N.0.2):** το `bim-move-geometry.calculateBimMovedGeometry` (το translate SSoT — χρησιμοποιείται ΚΑΙ από bulk-move + 3D gizmo) **δεν κάλυπτε** τις νεότερες οντότητες `mep-radiator`/`mep-boiler`/`mep-water-heater`/`roof`/`mep-underfloor` → bulk/3D move τους σιωπηλά no-op· προστέθηκαν 5 move functions (position-shift τα 3 equipment mirror fixture· outline/footprint-shift τα 2 polygon mirror slab/floor-finish). **Files: 1 NEW (`AltKeyTracker.ts`) + 2 MOD (`grip-commit-adapters.ts` Alt-bypass+helper, `bim-move-geometry.ts` +5 entities) + 2 NEW test (`AltKeyTracker.test.ts` 6, `grip-commit-alt-bypass.test.ts` 5) + `bim-move-geometry.test.ts` +5**. 27/27 νέα PASS· 31/32 γειτονικά suites (1 άσχετη προϋπάρχουσα αποτυχία `AssignWallTypeCommand`, ADR-412 shared tree). ΕΚΤΟΣ ADR-040 (commit adapter + pure geometry, όχι renderer/orchestrator). **DEFER Slice 2 (declutter):** κρύψιμο κεντρικού σημαδιού + wiring body-drag default (`useEntityDrag`/`useMovementOperations` σήμερα dead/unwired — factual blocker) + ⊙ discoverability glyph/hint. ✅ Google-level: YES — reuse upstream anchor (μηδέν νέα math), SSoT helper, additive gap-fix (zero regression), modifier via established tracker pattern. ⚠️ Browser verify: επίλεξε τοίχο → Alt+σύρσιμο από γωνία/μέσο → ΜΕΤΑΚΙΝΕΙΤΑΙ ολόκληρος· χωρίς Alt = stretch/πάχος ως πριν. 🔴 pending commit (Giorgio· shared tree). | Claude Opus 4.8 |
