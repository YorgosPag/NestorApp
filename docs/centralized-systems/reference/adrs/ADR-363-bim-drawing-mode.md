...
- [ ] Multi-storey stack group UI ("Copy to all floors") — cross-level Firestore writes, requires levels access in persistence hook. Dedicated session.
...
- [ ] Snap-to-edge-midpoint preview ghost — needs grip hover state in `RenderOptions` or ADR-040 micro-leaf. Dedicated session.
