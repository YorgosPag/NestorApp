...
- [ ] Edge-midpoint vertex insertion (`slab-edge-midpoint-N` grip → adds a new vertex at the edge midpoint).
- [ ] Slab-opening separate entity (§11.Q3) + boolean cutout on slab fill (mirrors wall's `OpeningsByWall` pattern from Phase 2.5).
- [ ] Rectilinear constraint (Shift toggle clamps grip drag to 0/90/180/270°).
- [ ] Hatch patterns per `reinforcement` (one-way / two-way / waffle / flat).
