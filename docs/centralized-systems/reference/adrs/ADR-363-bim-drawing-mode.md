...
**Phase 1B — Tool State Machine + Persistence (pending)**

- [ ] `useWallTool.ts` — state machine (awaitingStart → awaitingEnd → confirming) με Dynamic Input integration.
- [ ] `bim/tools/wall-tool.ts` — orchestrator που γεφυρώνει `useWallTool` με canvas pointer events + snap engine.
- [ ] Firestore CRUD via `BimEntityService` (collection `floorplan_walls` ήδη Phase 0). `useWallPersistence` hook.
- [ ] `EntityAuditService.recordChange()` integration για WallEntity writes.
- [ ] Hotkey: `W` (Wall).
- [ ] `useRibbonWallBridge` listener — `UpdateWallParamsCommand` dispatch on combobox change.
- [ ] Variant kinds: curved (quadratic Bezier control) + polyline (3+ vertices με mitred joins) με grip wiring.

**Phase 1C — Editing affordances**

- [ ] `wall-grips.ts` — endpoint + thickness + flip + axis-midpoint grips.
- [ ] Floating advanced panel "Σύνθεση Στρώσεων" (WallDna editor).
- [ ] `wall-tool.ts` snap interactions (perpendicular walls auto-trim using `computeWallTrims` port).
- [ ] `WallDnaService` με material catalog (Phase 6+ material library).

**Phase 1D — BOQ Auto-Feed (depends on Phase 6)**
