...
- [ ] **Deferred Phase 4.5**: position/rotation/dimension grips, hatch patterns per material category, anchor cycling visual preview (ghost at all 9 positions), `UpdateColumnParamsCommand` full undo/redo, snap-to-wall-corners + grid-intersections integration.
...
- [ ] **Deferred Phase 5.5**: start/end/midpoint/curveControl grips, width/depth dimension grips, hatch patterns per material (RC/steel/glulam), `UpdateBeamParamsCommand` full undo/redo, auto-connect to columns (beam ends snap to column anchors), snap-to-wall-axis / column-center integration, beam-supports-slab analytical link (Phase 6 BOQ dependency).
...
| 2026-05-18 | **Phase 5.5a IMPLEMENTED — Beam parametric grips + UpdateBeamParamsCommand**. Closes part of the Phase 5.5 deferred list (start/end/midpoint/curveControl grips + atomic command). Files created (4): (1) `bim/beams/beam-grips.ts` — pure handlers (zero React / DOM / Firestore deps). `getBeamGrips(entity)` returns 3 grips for straight beams (`beam-start` / `beam-end` axis-midpoint anchored `beam-midpoint`) and 4 for curved (`+ beam-curve` quadratic Bezier control). `applyBeamGripDrag(entity, kind, drag, opts)` returns patched `BeamParams`: `beam-start`/`beam-end` translate single endpoint; `beam-midpoint` translates the whole axis (both endpoints + `curveControl` when present, axis-midpoint anchor — invariant: drag origin stays at the visual centroid); `beam-curve` moves the Bezier control point and seeds from the axis midpoint when undefined; `drag` is delta in scene-units (mm-internal); `opts.shift` Shift-quantization (deferred to Phase 5.5b alongside snap integration). (2) `bim/beams/__tests__/beam-grips.test.ts` — grip layout per kind + applyDrag transforms + curveControl seed + axis-midpoint invariance + undefined-curve fallback. (3) `core/commands/entity-commands/UpdateBeamParamsCommand.ts` — atomic patch + recomputed `geometry` + `bimValidation` via `computeBeamGeometry()` + `validateBeamParams()` SSoT; merge-on-drag scaffold (`isDragging=true` collapses drag samples into a single undo entry, mirrors `UpdateWallParamsCommand`); fire-and-forget audit (`recordBeamChange('updated')`). (4) `core/commands/entity-commands/__tests__/UpdateBeamParamsCommand.test.ts` — execute/undo/redo round-trip, merge-on-drag window, geometry/validation re-derivation, hardError abort path. Files modified (5): (a) `hooks/grip-types.ts` — `BeamGripKind` union (`'beam-start' \| 'beam-end' \| 'beam-midpoint' \| 'beam-curve'`) + `GripInfo.beamGripKind?` discriminator. (b) `hooks/useGripMovement.ts` — re-export `BeamGripKind`. (c) `hooks/grips/unified-grip-types.ts` — `UnifiedGripInfo.beamGripKind?` forward. (d) `hooks/grips/grip-registry.ts` — `wrapDxfGrip` forwards `beamGripKind`. (e) `hooks/grips/grip-parametric-commits.ts` — imports `UpdateBeamParamsCommand` + `applyBeamGripDrag`, ready for the click-finalize wiring of Phase 5.5b. **Deferred to Phase 5.5b**: width/depth dimension grips, hatch patterns per material, auto-connect to columns (axis snap), beam-supports-slab analytical link (Phase 6 BOQ dependency). | Claude Opus 4.7 |
