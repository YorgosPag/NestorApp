...
- `color-config.getContrastColor`: **ΔΙΑΓΡΑΦΗΚΕ** (dead code — μηδέν runtime consumers· ήταν naive `.includes('fff')` που το «γυαλίσαμε» άσκοπα σε WCAG· ο κανόνας «όχι νεκρός κώδικας» υπερισχύει)· `withOpacity` → `channelToHex` για το alpha byte.
...
- **3× Ruler settings** (Major/Minor/Background) rgba-regex → `parseColor`. Επιπλέον, τα πανομοιότυπα `getBaseColor`/`getOpacityFromColor` (duplicated verbatim σε RulerMajor+RulerMinor) ενοποιήθηκαν σε ΕΝΑ SSoT: `stripAlphaToBaseHex` + `extractColorOpacity` στο `ui/color/utils.ts` (και τα 3 αρχεία delegateάρουν).
...
| `getContrastColor` | **Διαγράφηκε** (dead code, μηδέν runtime consumers). |
