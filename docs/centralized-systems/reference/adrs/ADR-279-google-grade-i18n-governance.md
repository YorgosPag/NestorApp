...
| 2026-07-25 | **Παραβίαση N.11 σε επίπεδο persisted data — raw i18n key ως δεδομένο στη Firestore.** Live έλεγχος αποκάλυψε αποθηκευμένο `addresses[0].label = "contacts.address.primary"`: **i18n key γραμμένο στη βάση**, όχι σε UI string. Διαχεόταν και στο `entity_audit_trail`. Αν μετονομαστεί το κλειδί στα locales, σπάνε **ήδη αποθηκευμένα δεδομένα** — κατηγορία που κανένα από τα υπάρχοντα gates δεν πιάνει: το CHECK 3.8 ελέγχει `t('key')` χωρίς αντίστοιχο locale, το `i18n:audit` ψάχνει `defaultValue:`/`toast(` σε κώδικα, και το pseudo locale (ADR-666) βλέπει μόνο την οθόνη. **Κανένα δεν κοιτά τι γράφεται στη Firestore.** Ρίζα: `EnterpriseContactSaver.convertToEnterpriseStructure` (κλάδος φυσικού προσώπου) είχε hardcoded το key — ενώ ο κλάδος εταιρειών του **ίδιου αρχείου** έκανε ήδη το σωστό (ADR-319: σημασιολογικό slug, `other` → custom label). **Διόρθωση:** ο κλάδος individual καθρεφτίζει τον κλάδο company — `label` = slug από `getPrimaryAddressType()` / `formData.primaryAddressType`· η μετάφραση γίνεται στο render. **Render-side:** νέος SSoT `renderStoredAddressLabel()` στο `contactAddressLabel.ts` — μεταφράζει slugs, περνά ελεύθερο κείμενο, και επιστρέφει `null` για legacy raw keys ώστε να μη φτάσουν ποτέ ωμά στην οθόνη· καταναλώνεται από `AddressCard` και `AddressMap` (τα δύο σημεία που τύπωναν το `label` αυτούσιο). **Migration:** `POST /api/admin/migrate-address-labels` (super-admin, `GET` = dry-run preview, προαιρετικό `?companyId=`, idempotent — δεύτερη εκτέλεση βρίσκει 0). **Tests:** 6 tests για τον resolver. **Δίδαγμα για το ADR-279:** η διακυβέρνηση i18n καλύπτει σήμερα κώδικα και οθόνη, **όχι** persisted data· ένα i18n key σε πεδίο Firestore είναι αόρατο σε όλο το tooling. | Claude Agent |
| 2026-07-25 | **Phase 4 (Remove Fallback-First Rendering) — ωμά κλειδιά σε ήδη προσαρτημένες επιφάνειες· ενοποίηση της preload λίστας σε ΕΝΑ SSoT.** Live: με το άνοιγμα της καρτέλας «Διευθύνσεις» εμφανίστηκαν `locations.newAddress` (κουμπί) και `types.home` (κάρτα) — **απαγόρευση §9** («passing raw i18n keys as end-user strings to toasts, dialogs, and UI cards»). Τα κλειδιά υπήρχαν και το ns ήταν registered· η αιτία ήταν ότι το `addresses` **δεν ήταν σε καμία preload λίστα** και render-άρεται μέσα σε tab που ανοίγει **χωρίς route change** — άρα χωρίς loading boundary, το i18next επιστρέφει το ίδιο το κλειδί για ένα frame. **Η προφανής λύση απορρίφθηκε τεκμηριωμένα:** ready-gated render (`if (!isNamespaceReady) return …`) **μπλοκάρεται από το CHECK 3.25** (ADR-267/ADR-300) γιατί ανταλλάσσει το flash του κλειδιού με **blank flash σε κάθε remount**. Ισχύον δόγμα = **proactive preload** (N.7.2 Q1), ίδιο με το προηγούμενο περιστατικό ADR-547 (`dxf-schedule`/`tool-hints`). **Ρίζα δεύτερου επιπέδου:** υπήρχαν **δύο διπλότυπες** hardcoded λίστες critical namespaces — `config.ts` (boot, 64) και `preloadCriticalNamespaces()` (language switch, 63) — **αποκλίνουσες κατά 5**: `admin`/`procurement`/`settings` έλειπαν από το μονοπάτι αλλαγής γλώσσας, άρα **μετά από αλλαγή γλώσσας** αυτές οι επιφάνειες έδειχναν ωμά κλειδιά. Ήταν ήδη σημειωμένο ως Boy-Scout flag στο ADR-547 (2026-06-28) και έμενε ανοιχτό. **Διόρθωση:** νέο SSoT `CRITICAL_NAMESPACES` στο `lazy-config.ts` (72 entries, ένωση των δύο χωρίς απώλεια)· το `config.ts` το **καταναλώνει** αντί να το επαναδηλώνει· η `preloadCriticalNamespaces()` οδηγείται από το ίδιο. **Κανόνας εισδοχής (τεκμηριωμένος στο docblock):** ένα ns μπαίνει όταν οι συμβολοσειρές του εμφανίζονται σε επιφάνεια που είναι **ήδη προσαρτημένη** (tab/card/dialog χωρίς route change)· τα route-scoped μένουν lazy, αλλιώς καταργείται το code splitting. Με το κριτήριο προστέθηκαν **6** με τεκμήρια: `addresses` (contacts/projects/buildings/POs, 24 αρχεία), `quotes` (`ContactQuotesSection` — tab επαφής), `showcase` (`BuildingDetailsHeader` + storage/parking details), `trash` (soft-delete dialogs), `toolbars` (`ActionButtons`/`ToolbarButtons`), `banking` (`ContactBankingTab` + sales dialogs). Τα υπόλοιπα 16 lazy είναι route-scoped και μένουν ως έχουν. **Boy Scout:** το `BuildingDetailsHeader.tsx` είχε **7 hardcoded αγγλικά** (`'Save'`, `'Cancel'`, `'Showcase'`…) ως χειροκίνητο workaround του ίδιου flash — έδειχναν **αγγλικά σε Έλληνα χρήστη** και παραβίαζαν το N.11· αφαιρέθηκαν, το preload τα κάνει περιττά. **Gate:** `src/i18n/__tests__/critical-namespaces.test.ts` (11 tests) — subset του `SUPPORTED_NAMESPACES`, μηδέν διπλότυπα, γνήσιο υποσύνολο, τα 6 embedded ns καρφωμένα ονομαστικά, και **textual assertion ότι το `config.ts` δεν ξαναδηλώνει τη λίστα**. Χωρίς αυτό το τελευταίο, η απόκλιση επανέρχεται αθόρυβα — ακριβώς όπως έγινε. | Claude (Opus 5) |
