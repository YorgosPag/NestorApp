...
5. **Client DXF levels list re-scoping entry point** — `useLevelsFirestoreSync` in `src/subapps/dxf-viewer/systems/levels/hooks/useLevelsFirestoreSync.ts`. This hook originally used `RealtimeService.subscribeToCollection` (not `firestoreQueryService.subscribe`), so it did not inherit entry point #3. Phase B added a manual `superAdminCompanyTick` re-subscription and an explicit `where('companyId', '==', effectiveSuperAdminCompanyId)` constraint. **Superseded by ADR-355 (2026-05-16)**: the hook now subscribes via `firestoreQueryService.subscribe('DXF_VIEWER_LEVELS', ...)` and inherits both the tenant filter and the switcher rebuild from entry point #3. The manual plumbing has been removed. This entry point is kept here for historical context — the bug it documents (orphan-path Storage loads after switch) is permanently solved by routing every Firestore real-time subscription through the single SSOT service.
...
| `src/subapps/dxf-viewer/systems/levels/hooks/useLevelsFirestoreSync.ts` | **Phase B** — adds `where('companyId', '==', effectiveSuperAdminCompanyId)` to constraints when super admin has a selection; `superAdminCompanyTick` triggers re-subscription on switcher change. **Superseded by ADR-355**: migrated to `firestoreQueryService.subscribe('DXF_VIEWER_LEVELS')`; manual switcher plumbing removed, hook ~⅓ shorter |
...
4. **Reset**: clear switcher (or sign in as non-super) → `setSuperAdminActiveCompanyId(null)` → header no longer sent (outside `/o/`), Firestore subscriptions go back to JWT-claim filter → behavior identical to pre-switcher era. *(2026-09-11: `apiClient.setSuperAdminCompanyId` removed — the header now reads the store through `lib/api/company-scope-source.ts`, see Changelog.)*
...
- **2026-09-11 (ADR-849 Β1 / ADR-787 §5.3 ζ) — THE URL OUTRANKS THE SWITCHER.** Live bug: an email deep link to `/o/<PAGONIS>/properties/<id>` rendered "property not found" because the client scoped every query to the switcher's `localStorage` company (another tenant), ignoring the workspace in the URL that the server had already verified. Changes: (1) the registry now holds two inputs — `url` (written by `WorkspaceScopeBridge` from the `o/[workspace]` guard's verdict) and `switcher` — and exposes ONE answer, `requestedWorkspace` (URL ▸ switcher, the latter only outside `/o/` ▸ claim); `TenantContext.effectiveCompanyId` replaced by `requested`; (2) personal workspace ⇒ `MissingTenantError`, never the global view; (3) the HTTP header is **pulled** at request time via `lib/api/company-scope-source.ts` (the context no longer pushes a copy — its effect ran after children's effects and lagged behind the URL); `apiClient.setSuperAdminCompanyId` removed; (4) `SuperAdminCompanyContext.activeCompanyId` is **derived** (URL company when present); `localStorage` becomes "last company" for landing; (5) inside `/o/`, `CompanySwitcher` **navigates** to `/o/<new>/<section>` (Linear/Vercel/Slack) instead of flipping hidden state; (6) `firestoreQueryService.subscribe` gained a rebuild generation counter (two notifications in one commit used to leak the first `onSnapshot`). Supersedes the "URL query param (rejected)" trade-off above in its intent: the workspace lives in the **path**, set by the guard, not in a query string.
- **2026-05-16 (ADR-355 supersession)** — Entry point #5 (`useLevelsFirestoreSync` manual switcher plumbing) superseded by ADR-355. The dual subscription service (`RealtimeService.subscribeToCollection` / `subscribeToDocument`) was the only reason this manual fix existed. After migrating the hook to `firestoreQueryService.subscribe`, the entry point's behavior is delivered automatically by entry point #3. Manual code removed; ADR description retained for historical context.
