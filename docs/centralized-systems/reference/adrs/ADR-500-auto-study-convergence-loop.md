...
### 2.4 FEM frame-action (Slice 2 — πρώην DEFER B)

Ο hook `useStructuralAutoStudy` **force-engage-άρει** την ανάλυση (`setAnalysisLive(true)`) ΜΟΝΟ για τη διάρκεια της μελέτης (try/finally restore), ώστε το FEM solve + ο gate `resolveEngagedAnalysisResult` (που διαβάζουν reinforce/feasibility) να μετρούν **ακόμη κι όταν τα διαγράμματα είναι κλειστά** — πλήρης frame-action σύγκλιση (column M-N από τον φορέα). Ο latch `analysisLive` δεν δείχνει overlay → **καμία μόνιμη αλλαγή UI**. Μέσα στον βρόχο το `maybeSolveFem` (engaged-gated) τρέχει κάθε γύρο, reuse του SSoT solver `runStructuralAnalysis`, silent.

### 2.5 Per-kind unique report (Slice 2 — πρώην DEFER A)

Τα `runMemberAutoSize`/`runOrganismAutoReinforce` επιστρέφουν πλέον τα **ids** (ΟΧΙ count)· το `runAutoFoundationDesign` επιστρέφει `footingIds`. Ο βρόχος τα ταξινομεί ανά τύπο (`classifyChangedMembers` έναντι της σκηνής) σε **unique Sets** → το report λέει «άλλαξα X κολώνες, Y δοκάρια, Z πλάκες, W πέδιλα» (§7.2), με κάθε μέλος μετρημένο **μία φορά** ακόμη κι αν άλλαξε σε πολλούς γύρους ή και διαστασιολογήθηκε ΚΑΙ οπλίστηκε. Οι callers (proactive/ribbon hooks) αγνοούσαν ήδη το return → μηδέν churn.
...
- ~~Per-kind unique report~~ → **ΕΓΙΝΕ Slice 2** (§2.5).
- ~~Force-FEM στον βρόχο~~ → **ΕΓΙΝΕ Slice 2** (§2.4).
- **Footings reinforced εκτός foundation design:** αν ο reinforce αγγίξει πέδιλο του ορόφου Θεμελίωσης (όχι active), δεν ταξινομείται από τη σκηνή του ενεργού ορόφου (μετριέται μόνο μέσω `footingIds` του foundation core). Edge case· τα active-level footings ταξινομούνται κανονικά.
...
| 2026-06-19 | **Slice 2.** (DEFER A) Per-kind unique report — τα member cores επιστρέφουν ids (όχι count)· foundation core `footingIds`· `classifyChangedMembers` → unique Sets ανά τύπο· report «X κολώνες, Y δοκάρια, Z πλάκες, W πέδιλα». (DEFER B) Force-FEM frame-action — ο hook engage-άρει `analysisLive` για τη διάρκεια της μελέτης (try/finally restore· καμία μόνιμη UI αλλαγή) ώστε column M-N από τον φορέα να μετρά με κλειστά διαγράμματα. 8 study + 7 reinforce jest GREEN. |
