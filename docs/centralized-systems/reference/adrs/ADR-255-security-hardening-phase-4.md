...
| 2026-04-11 | **Ratchet Phase 10C.9 — contact-link.service.ts + file-link.service.ts legacy schema annotations (COMPLETE 🎯)**: Final 14 violations cleared across two symmetric services. `contact-link.service.ts` (7) + `file-link.service.ts` (7), both implementing `ADR-032` Linking Model. Both `ContactLink` + `FileLink` types (`src/types/associations.ts`) have no `companyId` field on their interfaces and existing Firestore documents do not carry one — identical legacy-schema debt to Phase 10C.8 (`FirestoreRelationshipAdapter`). Firestore rules (`firestore.rules:151-186`) on `contact_links` enforce tenant isolation via a two-path model: modern docs match `belongsToCompany(resource.data.companyId)`, legacy docs fall through to `createdBy == request.auth.uid` ownership. `file_links` has no dedicated rules block (default-deny + auth fallback). Adding `where('companyId')` would silently match zero documents across both collections. Violations are concentrated in `listContactLinks()` + `listFileLinks()` where a base `query()` is progressively extended with `query(q, where(...))` chained conditional filters (6–7 calls each). The scanner's 12-line forward window counts each chained `query()` as a separate violation. Resolution: inline `🔒 companyId: N/A — legacy schema` trailing markers on every chained `query()` line + a block comment above the base query explaining the legacy-schema rationale + deferred-migration link to Phase 10C.8. Same annotation pattern as Phase 10C.8 (`FirestoreRelationshipAdapter`) + Phase 9 (`useProjectWorkers`). Baseline: 14 → **0 violations** (2 → 0 files). ✅ **Firestore companyId ratchet cleanup COMPLETE** — from an initial baseline of 74 violations (Phase 7 start) down to **zero** across 10 phases over April 10–11. | Claude |
| 2026-04-11 | **Ratchet Phase 10C.8 — FirestoreRelationshipAdapter legacy schema annotations**: `FirestoreRelationshipAdapter.ts` (4 violations) cleared via inline false-positive annotations. `CONTACT_RELATIONSHIPS` collection is legacy schema — `ContactRelationship` type has no `companyId` field, and existing documents do not carry one. Firestore rules (`firestore.rules:103-115`) enforce tenant isolation via a two-path model: modern docs match `belongsToCompany(resource.data.companyId)`, legacy docs fall through to `createdBy == request.auth.uid` ownership. Since the type + data have no companyId, adding a `where('companyId')` clause would silently match zero documents. A real fix requires data migration across all existing `contact_relationships` (previously deferred in Phase 9 notes for `useProjectWorkers.ts` + Phase 7 for `useContactProjectRoles`). **Scope deferred** — refactor is 7 files × 18 call sites (adapter + `RelationshipSearchService`, `OrganizationHierarchyService`, `RelationshipCRUDService`, `relationship-reciprocal-ops`, `relationship-helpers`, `ContactRelationshipExtendedService`), which exceeds the N.8 single-phase budget. Inline `🔒 companyId: N/A` markers with rule references added to all 4 `query()` blocks: `getContactRelationships` (sourceQuery + targetQuery), `getOrganizationEmployees`, `getSpecificRelationship`. Same annotation pattern as `useProjectWorkers.ts` (Phase 9). Baseline: 18 → **14 violations** (3 → 2 files). | Claude |
| 2026-04-11 | **Ratchet Phase 10C.7 — EnterpriseTeamsService DELETE (dead-code elimination)**: `EnterpriseTeamsService.ts` (6 violations) eliminated by **deleting the service entirely** (Google-level recommendation from Phase 10B.1). Service had: own `initializeApp()` (duplicate Firestore instance on client), dual schema debt (`organizationId` field vs modern `companyId`), 4 caches with custom TTLs, singleton wrapping stateless class, fallback data factories (placebo data when Firebase fails), and only 2 methods actually used by production code (`getTeams` + `getTeamMembers`). **Files deleted (3, ~1079 lines)**: `src/services/teams/EnterpriseTeamsService.ts` (473), `src/services/teams/enterprise-teams-types.ts` (317), `src/services/teams/enterprise-teams-defaults.ts` (289), plus empty `src/services/teams/` directory. **Caller refactor**: `CrmTeamsPageContent.tsx` (single consumer) inlines both queries directly using `@/lib/firebase` + `useCompanyId()`. Teams query: `where('companyId') + where('isActive') + orderBy('name')`. Members query (per team, Promise.all parallel): `CONTACTS` filtered by `companyId + type='employee' + department=teamId + isActive + orderBy('displayName')`. Local minimal `TeamMember` + `DisplayTeam` interfaces replace the 60+ fields of `EnterpriseTeamMember` — only `id`, `displayName`, `role`, `position.title` were ever rendered. Firestore rules (`firestore.rules:2635`) already enforce `belongsToCompany(resource.data.companyId)` on `teams/{teamId}`, matching the new query shape. Baseline: 24 → **18 violations** (4 → 3 files). | Claude |
| 2026-04-11 | **Ratchet Phase 10C.6 — BrokerageService tenant scoping (real fix + query chaining)**: `brokerage.service.ts` (6 violations) cleared. **Real fixes (3 methods)**: `getAgreements(projectId, companyId, propertyId?, status?)` — promoted to required `companyId` parameter; base `query()` now prepends `where('companyId', '==', companyId)` before `projectId`. Chained extensions (`propertyId`, `status`) inherit via query-extension pattern — inline `🔒 companyId: inherited` annotations added (same pattern as Phase 10C.4). `getCommissions(propertyId, companyId)` — added required `companyId` parameter; query prepends `where('companyId', '==', companyId)`. `hasActiveRecords(agentContactId, companyId)` — added required `companyId` parameter; both queries (agreements + commissions) prepend `where('companyId', '==', companyId)`. `validateExclusivity(input, companyId)` — added 2nd param, threads to internal `getAgreements` call (mirrors server-side `validateExclusivityServer` signature). **Caller updates (3 files)**: `SellDialog.tsx` — component now consumes `useCompanyId()?.companyId`; `useEffect` guards `BrokerageService.getAgreements()` call until `companyId` resolved; dep array updated. `useBrokerageAgreements.ts` — already had `useCompanyId()`; validation `useEffect` guard extended with `!companyId` early-return; `validateExclusivity()` call passes `companyId` as 2nd arg; dep array updated. `persona-removal-guards.ts` — `brokerageGuard` now uses the `companyId` parameter (previously `_companyId` placeholder from 10C.5) and threads it to `hasActiveRecords()`. Baseline: 30 → **24 violations** (5 → 4 files). | Claude |
| 2026-04-10 | **Ratchet Phase 10C.5 — ClientService.hasActiveUnits tenant scoping (real fix)**: `client.service.ts` (3 violations) cleared. `ClientService.hasActiveUnits(contactId, companyId)` — promoted to required `companyId` parameter; all three parallel queries (`properties`, `parking_spots`, `storage_units`) now prepend `where('companyId', '==', companyId)` before the existing `array-contains` on `commercial.ownerContactIds`. Downstream plumbing updated: `PersonaRemovalGuardFn` signature + `clientGuard` + `brokerageGuard` (companyId threaded through, unused by brokerage adapter) + public API `checkPersonaRemovalGuard(personaType, contactId, companyId)` — currently no live callers, so no further caller updates needed. Baseline: 33 → **30 violations** (6 → 5 files). | Claude |
| 2026-04-10 | **Ratchet Phase 10C.4 — Assignment policy repository query-chaining false positives**: `AssignmentPolicyRepository.ts` (3 violations) fully annotated. All three violations come from the query-extension pattern in `getAssignmentPolicies()`: the base query at `query(policiesRef, where('companyId', '==', queryParams.companyId))` is tenant-scoped, and subsequent calls `q = query(q, where('projectId'|'status'|'status', ...))` extend it — Firestore preserves all constraints from the parent query. The detector scans 12-line windows around each `query(` call and does not understand query chaining. Inline comments added to each chained extension: `🔒 companyId: inherited from base query above (query extension pattern)`. Baseline: 36 → **33 violations** (7 → 6 files). | Claude |
| 2026-04-10 | **Ratchet Phase 10C.3 — Building spaces service false-positive annotations**: `building-spaces.service.ts` (3 violations) fully annotated as false positives. All three queries in `getBuildingSpaces()` filter by `buildingId` on collections that have **no `companyId` field**. Firestore rules enforce tenant isolation via parent entity lookup: `properties` → `belongsToProjectCompany(project)` (rules line ~613), `parking_spots` → `belongsToBuildingCompany(buildingId)` (rules line ~691), `storage_units` → `belongsToBuildingCompany(buildingId)` (rules line ~660). Since this service receives pre-resolved building IDs (already tenant-scoped upstream by the caller), filtering by `buildingId` IS the tenant filter. Inline `🔒 companyId: N/A` markers added inside all three `query(...)` blocks. Baseline: 39 → **36 violations** (8 → 7 files). | Claude |
