# ADR-740: Χρονοπρογραμματισμός εργασιών στο Netcup — «Coolify = ρολόι, git = πρόγραμμα»
...
> ⚠️ **Ήταν ADR-739 μέχρι τις 2026-07-31.** Δύο ADR γράφτηκαν την ίδια μέρα με τον
> ίδιο αριθμό: αυτό και το `ADR-739-canvas-table-system.md`. **Το table κράτησε το
> 739** — το επεξεργαζόταν ενεργά άλλος πράκτορας και **πέντε commits το ονομάζουν
> ρητά** (`feat(dxf): ADR-739 table …`)· μετονομασία του θα έκανε το ιστορικό git
> να λέει ψέματα. Ο χρονοπρογραμματισμός πήρε το επόμενο ελεύθερο, το **740**.
> Παλιά αναφορά «ADR-739» σε **cron** συμφραζόμενα (handoffs πριν από αυτή την
> ημερομηνία) εννοεί **αυτό** το αρχείο.
