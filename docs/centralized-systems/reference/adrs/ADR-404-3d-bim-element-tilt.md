...
- **Μονάδα εμφάνισης = επιλογή χρήστη**: %/μοίρες/λόγος (1:N). Stored ΠΑΝΤΑ %· η μονάδα είναι ribbon **display
  pref** (NEW `slab-slope-unit.ts`)· numeric-safe (ratio εκθέτει το N). **SSoT reuse (Giorgio audit):** το
  deg↔percent math **delegate** στο υπάρχον `bim/geometry/roof-slope-units.ts` (`roofSlopeToRatio`/`FromRatio`,
  ήδη σε χρήση από roof bridge) με pivot rise/run — εδώ ζει ΜΟΝΟ η 3η μονάδα «1:N» + το pref store.
...
## Changelog
- **2026-07-05 (Opus 4.8) — SSoT: `createToolBridgeStore<THandle>()` factory + 16-store migration + registry guard (pending commit):** Τα `*-tool-bridge-store.ts` (module-level `THandle|null` cell που δημοσιεύει ένα tool-hook μέσα στο `CanvasSection` και διαβάζει ribbon-bridge/3D reader via `get()`/`use()`) επαναλάμβαναν **ταυτόσημο** boilerplate (`let handle` + `Set<Listener>` + `emit`/`subscribe`/`getSnapshot`/`getServerSnapshot` + `set`(identity-guard)/`get`/`use`). **grep-audit: undercount 15→16** — εκτός των 15 στο `ui/ribbon/hooks/bridge/` βρέθηκε και το `bim/beams/beam-tool-bridge-store.ts` (degenerate `{set,get}`, single-reader). **NEW SSoT** `stores/createToolBridgeStore.ts` (sibling του `createExternalStore`/`createConfirmStore`): **big-player layering** — δεν ξαναγράφει notify plumbing, **delegate στο `createExternalStore<THandle|null>(null,{equals:Object.is})`** (το `Object.is` ≡ το χειροκίνητο `if(next===handle)return`· handles=objects/null) + bundled React `use()` binding (`useSyncExternalStore(subscribe, get, ()=>null)`). Public API = **superset** και των 3 legacy shapes (`{set,get,use}`×14, `{set,get,subscribe}`×1 slab, `{set,get}`×1 beam) → **μηδέν consumer αλλαγή**. Και τα 16 → one-liner `createToolBridgeStore<XHandle>()` (interface + `export type` re-exports μένουν). **`stair-status-store` (η πηγή του pattern, `string` state) ΕΞΩ** — άλλο domain (status text) → WAVE 2.5 `createExternalStore`. **2D registry guard** module `create-tool-bridge-store` (tier 3· forbid `let handle: \w+ToolBridgeHandle`· allowlist=factory· **0-violation** επιβεβαιωμένο· `test:registry-golden` 56 GREEN). **Verification:** factory 9 jest + 19 bridge suites/155 GREEN (incl. consumers wall-draw-mode/column-draw-kind/slab-slope-param) + BeamFromWallGhost 8 GREEN. 🔴 ΟΧΙ tsc (N.17)· commit + `ssot:baseline` = Giorgio.
- **2026-06-22 (Opus 4.8) — Phase 5c: ribbon UX ΚΕΚΛΙΜΕΝΗ/ΡΥΣΗ ΠΛΑΚΑΣ (full-flex)** (pending commit, 🔴 browser verify). Τρίτο αδελφό μετά κολώνα (Phase 5) + τοίχο (Phase 5b). Recognition (SSoT grep audit): όλη η γεωμετρία/3D/2D/undo υπήρχε (`SlabSlope`, `applySlabSlope`, `slabSlopeOffsetZmm`, `computeSlabTiltParams`, `UpdateSlabParamsCommand`, Zod superRefine)· μόνο ribbon wiring έλειπε. **Δύο invariants (ADR-369 §9 Q7):** (1) `geometryType:'box'|'tilted'` ↔ `slope` coupling — **εξήχθη** το private gizmo logic σε ΕΝΑ SSoT `withSlabSlope(base, slope|null)` (`slab-slope.ts`, generic `SlabSlopeBearing`), reuse σε gizmo (delegate, byte-equiv) + ribbon + drawing-mode (μάθημα τοίχου `isWallTiltAngleActive`)· (2) `angle`=%. **Giorgio choices = πλήρης ευελιξία:** μονάδα εμφάνισης χρήστη (%/μοίρες/λόγος· stored % πάντα· NEW `slab-slope-unit.ts` — **deg↔percent delegate στο υπάρχον `roof-slope-units.ts` SSoT** μετά από Giorgio SSoT audit· εδώ μόνο 1:N + pref store· `useSyncExternalStore` re-format) + ελεύθερη φορά (`direction°` 0..360 + `pivotEdge`) + drawing-mode born-sloped (NEW minimal `slab-tool-bridge-store.ts`· `useSlabTool` publish· overrides ήδη φέρουν geometryType+slope). NEW `slab-slope-param.ts` (resolver: enabled/unit/angle/direction/pivot· selected→`dispatchParams` + drawing→store). `slab-command-keys` += `slope` group + `isSlabSlopeKey`· `useRibbonSlabBridge` += slope branch (ΠΡΙΝ null-check)· **`useRibbonCommands` += `isSlabSlopeKey` στα 2 guards (το μάθημα τοίχου)**· NEW panel «Κλίση» (5 combobox)· i18n el+en. **Μηδέν διπλότυπη γεωμετρία/εντολή** (Giorgio SSoT audit ✅). `slab-slope.test.ts` += withSlabSlope (4) + NEW `slab-slope-param.test.ts` (20) → 55/55 PASS (gizmo regression byte-equiv). 🔴 tsc deferred (N.17, άλλος agent έτρεχε tsc) + browser-verify + commit.
