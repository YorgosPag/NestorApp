...
| Αρχείο | Αλλαγή |
|---|---|
| `bim/columns/column-grip-utils.ts` | `columnCenterMoveGrip(entity, position?)` — προαιρετική θέση (default centroid)· το SSoT helper κρατά τη δομή, οι callers δίνουν θέση μόνο όταν χρειάζεται. **+ ΝΕΟ `columnRotationHandleMidwayWorld(center,dimY,rot,scale,clearance?)`** — η ΜΙΑ world-placement της «midway» λαβής περιστροφής· `rotationHandleWorld` (rect+polygon) ΚΑΙ `freeReshapeRotationWorld` την καλούν |
| `bim/grips/rotation-handle-policy.ts` | **ΝΕΟ `rotationHandleMidwayOffset(dimY, clearanceMm?)`** + `ROTATION_HANDLE_INSIDE_SAFETY` — ο κανόνας `−dimY/4` (clearance-bounded) σε ΕΝΑ σημείο (πριν: γραμμένος 3 φορές inline σε rect/polygon/free-reshape) |
...
- **2026-06-25 (follow-up, SSoT audit Giorgio)** — **(α)** `columnCenterMoveGrip` επεκτάθηκε με optional
  `position` → εξαλείφθηκε το spread-override στο call site (η δομή του grip ζει 100% στο SSoT helper).
  **(β) Ενοποίηση rotation-handle placement** (εντολή Giorgio): ο κανόνας «midway `−dimY/4`,
  clearance-bounded» ήταν γραμμένος **3 φορές** (rect inline, polygon inline, free-reshape inline) →
  τώρα ΜΙΑ πηγή: `rotationHandleMidwayOffset` (`rotation-handle-policy.ts`, ο αριθμητικός κανόνας) +
  `columnRotationHandleMidwayWorld` (`column-grip-utils.ts`, η world-placement). rect/polygon/free-reshape
  την καλούν όλα. **Καθαρό refactor — μηδέν αλλαγή συμπεριφοράς** (οι τιμές ταυτίζονται), 110/110 jest GREEN.
