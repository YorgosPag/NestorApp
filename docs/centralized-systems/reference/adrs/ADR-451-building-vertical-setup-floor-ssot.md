...
**Status:** 🟢 Slices 0-3 + Slice 4 (rename-propagation + **height-edit from DXF: «Ύψος Ορόφου» ribbon + column «Ύψος» read-only/derived, 2026-06-15**) Implemented — pending browser-verify + commit
...
## 7. Changelog
- **2026-06-15** — **Slice 4 (height-edit from DXF): «Ύψος Ορόφου» ribbon command — δεύτερο entry point στο ΙΔΙΟ SSoT** (Giorgio: «είναι σωστό να ορίζω το ύψος ορόφου και από το ribbon; να καλώ την εντολή και από το ribbon και από Κτίρια→Όροφοι;» → ΝΑΙ, με όρο ίδια πηγή). **Item A:** combobox «Ύψος Ορόφου» στο contextual column tab (δίπλα στο read-only column «Ύψος») → γράφει `floor.height` του **ΕΝΕΡΓΟΥ** ορόφου μέσω του υπάρχοντος `updateFloorWithPolicy` → `handleUpdateFloor` → `reconcileFloorStackAfterEdit` (entity re-stretch + upper FFLs). 3Δ re-sync **αυτόματο** (onSnapshot→active storey→`resyncBimScene`). ΜΗΔΕΝ διπλή λογική. `_v` undefined = force-write (single-user ribbon edit). NEW `storey-command-keys.ts` (zero-dep keys+guard) + `storey-height-bridge.ts` (pure `resolveStoreyHeightState`/`parseStoreyHeightMetres` + store-backed `getStoreyComboboxState`/`applyStoreyComboboxChange`)· routed στο `useRibbonCommands` (πριν τον columnBridge — storey key ≠ column key)· reuse `useActiveStoreyStore.context.floorId/storeyHeightMm`· mirror του `column.structural.code` pattern (ADR-456: column-tab key που γράφει non-column SSoT). **Item B (ADR-449/451 cross-ref): column «Ύψος» read-only/derived** όταν `topBinding≠'unconnected'` (storey/level-bound = παράγωγο)· NEW pure `column-height-display.ts` (`isColumnHeightDerived` + `deriveStoreyBoundHeightMm` = resolved height μέσω ΙΔΙΑΣ SSoT με πυρήνα `resolveColumnVerticalExtentMm` → greyed πεδίο δείχνει 3000 storey ceiling, ΟΧΙ stale 2700)· `disabled:true` flag (υπάρχον στο `RibbonComboboxState`). i18n el+en `columnEditor.storeyHeight`. 15 jest (storey-bridge 10 + column-height-display 5). **ΜΑΘΗΜΑ: πολλά UI entry points → ΕΝΑ SSoT = σωστό (Revit Level edit από πολλά σημεία)· διαφορετικές πηγές = SSoT violation. column height ≠ floor height (δύο διαφορετικά πεδία/SSoT).** **DEFER:** «Ύψος Ορόφου» και σε permanent tab (νυν μόνο column-contextual)· create/delete/elevation-edit από DXF. 🔴 tsc(Giorgio)+browser-verify (επίλεξε storey-ceiling κολόνα→«Ύψος» greyed=3000· άλλαξε «Ύψος Ορόφου»→3300→πυρήνας+σοβάς+δοκάρια τεντώνονται μαζί, ίδιο με Κτίρια→Όροφοι)+commit (git add ΜΟΝΟ δικά μου). | Opus
