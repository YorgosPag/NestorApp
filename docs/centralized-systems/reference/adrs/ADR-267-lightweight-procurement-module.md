...
| Date | Changes |
|------|---------|
| 2026-05-05 | 🎨 **Phase K — Full SSoT Alignment + Quick Filters + Grid View + Eye Toggle** for the 5 procurement pages (vendors / materials / agreements / quotes / purchase-orders). Allineamento 1:1 al pattern Buildings/Properties/Contacts: `PageHeader` (variant `sticky-rounded`, layout `compact`) sopra ogni split-panel, con eye/list/grid toggle (`viewMode`) + dashboard toggle (`showDashboard`) + `UnifiedDashboard` collapsible (4-5 KPIs per dominio). **Lists riscritte SSoT** (`VendorList.tsx`, `MaterialSlimList.tsx`, `AgreementSlimList.tsx`): `EntityListColumn` + `GenericListHeader` + `CompactToolbar` (configs nuovi `vendorsConfig`/`materialsConfig`/`agreementsConfig` esportati da `@/components/core/CompactToolbar`) + status quick-filter chips (`VendorStatusQuickFilters`/`MaterialStatusQuickFilters`/`AgreementStatusQuickFilters` da `TypeQuickFilters.tsx`) + `ScrollArea` con render condizionale `viewMode==='grid'` → `GridCard` tile (`VendorGridCard`/`MaterialGridCard`/`AgreementGridCard`) altrimenti `ListCard` (`VendorListCard`/`MaterialListCard`/`AgreementListCard`). Search ora vive nel toolbar (rimosse search bar custom inline + `MaterialFilters`/`FrameworkAgreementFilters` redundant). **Detail headers SSoT** (`VendorDetail.tsx`, `MaterialDetail.tsx`, `AgreementDetail.tsx`): rimpiazzato `<header>` custom con `EntityDetailsHeader` (`@/core/entity-headers`) + `createEntityAction`. Vendor: action `view` con icon `ExternalLink` → `/contacts/{id}`. Material: `edit` + `delete`. Agreement: `edit` + `delete` + `titleAdornment` con status badge interattivo (`getStatusColor` semantic mapping `pending/available/reserved/cancelled`). **Predicates SSoT** (`src/subapps/procurement/utils/quick-filter-predicates.ts`): pure functions `makeVendorPredicate(filter, allVendors)`, `makeMaterialPredicate(filter)`, `makeAgreementPredicate(filter)` con thresholds centralizzati (12mo active, 30d new, 90d recent, 180d inactive, 30d expiring, 20% top-spend = preferred). **Grid view per Quotes/POs**: aggiunto prop `viewMode?: 'list'\|'grid'` a `QuoteList.tsx` + `PurchaseOrderList.tsx` con render condizionale `QuoteGridCard`/`PurchaseOrderGridCard` (CSS-grid 1/2/3 col responsive). **CompactToolbar configs.ts** esteso: `ToolbarType` union (in `filter-definitions.ts`) + 3 nuovi factory targets (`vendors`/`materials`/`agreements`) con filter categories status, sort options name/number/value/date, label maps NEW_ITEM/EDIT/DELETE/SHARE_TOOLTIP_BY_TYPE estesi, `*ToolbarConfig` esportati con `availableActions` overrides (vendors no editItem/deleteItems perché FK→Contacts; materials/agreements full CRUD). **Pages riscritti**: `/procurement/vendors`/`/materials`/`/agreements`/`/quotes`/`/purchase-orders` con `PageHeader` + `useState<viewMode>('list')` + `useState<showDashboard>(false)` + dashboard stats memo per dominio. Vendors KPI: total/active/totalSpend/avgOnTime/totalOrders. Materials KPI: total/recently/inactive/no_supplier. Agreements KPI: total/active/expiring/expired/drafts. Quotes KPI: total/submitted/under_review/accepted/expired. POs KPI: total/drafts/approved/ordered/overdue. **GOL checklist**: ✅ proattivo (status derivato da timestamps/relations, no flag manuali); ✅ idempotente (predicate puri, no side effects); ✅ SSoT (tutti i building block centralizzati: `EntityDetailsHeader`/`CompactToolbar`/`PageHeader`/`UnifiedDashboard`/`*ListCard`/`*GridCard`/`*StatusQuickFilters`); ✅ <500 LOC tutti i file; ✅ <40 LOC tutte le funzioni; ✅ no `any`/`as any`/inline styles; ✅ i18n compliant zero hardcoded strings (chiavi pre-esistenti `procurement.json` `hub.{vendorMaster,materialCatalog,frameworkAgreements}.*` + `filters.{vendor,material,agreement}Status.*`). Files: `quick-filter-predicates.ts` (NEW), `VendorList.tsx`/`MaterialSlimList.tsx`/`AgreementSlimList.tsx` (REWRITE), `VendorDetail.tsx`/`MaterialDetail.tsx`/`AgreementDetail.tsx` (REWRITE — EntityDetailsHeader), `vendors/page.tsx`/`materials/page.tsx`/`agreements/page.tsx`/`quotes/page.tsx`/`purchase-orders/page.tsx` (PageHeader + viewMode + dashboard wire), `QuoteList.tsx`/`PurchaseOrderList.tsx` (viewMode prop + grid render branch), `CompactToolbar/configs.ts` + `filter-definitions.ts` (EXTEND vendors/materials/agreements types), `CompactToolbar/index.ts` (+3 exports). |
...
| 2026-04-28 | 🎨 **Phase G+1 — PO List Panel SSoT alignment con Επαφές**: la lista in `/procurement` (sinistra del split) usava ancora un layout custom (`<Card>` "Requires Action" + `<Card>` "All Orders" con `<Table>` desktop / `<article>` mobile), divergente dal pattern unificato di `/contacts`. Riscritta `PurchaseOrderList.tsx` (~250 LOC, era 358) con la composizione SSoT identica a `ContactsList`: **`GenericListHeader`** (icon Package + "Παραγγελίες (N)") → **`CompactToolbar`** (config `procurementConfig`, desktop sempre, mobile toggleable) → **`POStatusQuickFilters`** (chips: Όλες/Πρόχειρες/Εγκεκριμένες/Παραγγελμένες/Μερική Παραλαβή/Παραδοθείσες/Ολοκληρωμένες/Ακυρωμένες) → **`ScrollArea`** con **`PurchaseOrderListCard`** items. La sezione "Requires Action" rimane (POs pinnati in cima con divider + counter), ma dentro la stessa scroll list anziché `<Card>` separata. Selected styling 1:1 con `ContactListCard` (info border + bg + shadow via `ListCard` SSoT). Card single-line: title=`po.poNumber`, `inlineBadges` con status badge, subtitle truncated `"Supplier · Total · Date"`. Drop search/hideSearchBar props (lista possiede ora la sua searchTerm interna, AdvancedFiltersPanel a livello pagina coesiste invariato — stesso pattern di Επαφές). **SSoT extensions** (puramente additive, zero impatto su consumer esistenti): (1) `createToolbarConfig` factory esteso a `'procurement'` con i suoi `*_BY_TYPE` map entries + filter categories status + sort options (date/number/status/value); search placeholder via override inline (`procurement:list.searchPlaceholder`) per evitare edit cross-subapp di `dxf-viewer/config/modal-select`. `procurementToolbarConfig` esportato come `procurementConfig` da `@/components/core/CompactToolbar`. `availableActions` overrides: favorites/import/preview/copy/refresh/reports/settings/help/deleteItems/archive disabled (POs gestiscono cancellazione via lifecycle, non delete). (2) `PO_STATUS_OPTIONS` + componente pre-configurato `POStatusQuickFilters` aggiunti a `src/components/shared/TypeQuickFilters.tsx`, in pattern con `ContactTypeQuickFilters`. (3) **NEW** `PurchaseOrderListCard` (`src/domain/cards/po/PurchaseOrderListCard.tsx`, ~110 LOC) wrapper di `ListCard` con default PO-specific (Package icon + status badge mapping `secondary/info/warning/success/destructive`); barrel `src/domain/cards/po/index.ts` + re-export in `src/domain/cards/index.ts` + `src/domain/index.ts` allineati al pattern degli altri domini. **i18n**: 19 nuove chiavi `procurement.json` (el+en) — `list.entityName/ariaLabel/searchPlaceholder/cardAriaLabel`, `filters.poStatusAriaLabel`, `filters.poStatus.{all,draft,approved,ordered,partially_delivered,delivered,closed,cancelled}` + matching `*Tooltip`. `ProcurementPageContent.tsx` aggiornato: rimossi `searchValue`/`onSearchChange`/`hideSearchBar` da entrambi i mount (desktop+mobile), aggiunto `onEditPO={() => selectedPO && setEditMode(true)}`. Files: `PurchaseOrderList.tsx` (REWRITE), `configs.ts`+`index.ts` (CompactToolbar, EXTEND), `TypeQuickFilters.tsx` (EXTEND), `PurchaseOrderListCard.tsx` (NEW), `domain/cards/po/index.ts` (NEW), `domain/cards/index.ts` (+1 export), `domain/index.ts` (+1 export), `procurement.json` el+en (+19 keys), `ProcurementPageContent.tsx` (prop cleanup). |
| 2026-04-28 | 📄 **Phase H — Quotes List Panel + Detail Header SSoT alignment con Παραγγελίες/Επαφές**: la pagina `/procurement/quotes` usava ancora un layout custom (`<Card>` + `<Table>` con search inline) per la lista e un `QuoteDetailSummary` con header proprio (displayNumber+status+vendor+total) duplicato — niente toolbar SSoT, niente status quick filter chips, niente SSoT card, niente "+Νέα/Επεξεργασία/Αρχειοθέτηση" header sempre visibili. Allineato 1:1 al pattern Phase G/G+1 di POs. **Composizione lista (REWRITE `QuoteList.tsx`, ~225 LOC, era 199 con layout completamente diverso)**: `GenericListHeader` (icon `FileText` + "Προσφορές (N)") → `CompactToolbar` (config `quotesConfig`, desktop sempre, mobile toggleable) → `QuoteStatusQuickFilters` (chips: Όλες/Πρόχειρες/Απεσταλμένες/Υποβληθείσες/Υπό Εξέταση/Εγκεκριμένες/Απορριφθείσες/Ληγμένες/Αρχειοθετημένες) → `ScrollArea` con `QuoteListCard` items. **Sezione "Απαιτούν Ενέργεια"** introdotta (parità con POs): quotes con status `submitted` (νέα υποβολή — needs review), `under_review` (in valutazione — decision pending) o `expired` (scaduta — renew/archive) sono pinnate in cima con divider + counter. Derivazione in `useQuotesPageState` via `useMemo` su `displayedQuotes` + esposizione `actionRequired` nel return. **Detail header SSoT (NEW `QuoteDetailsHeader.tsx`, ~80 LOC)** wrapper di `EntityDetailsHeader` (`@/core/entity-headers`) — stesso pattern di `ContactDetailsHeader`/`PurchaseOrderDetailsHeader`: icon=`FileText`, title=`quote.displayNumber`, `titleAdornment` = status badge interattiva (semantic mapping gray/blue/yellow/orange/green/red/purple → pending/planned/construction/reserved/available/cancelled/completed), `actions` derivate via `createEntityAction`: `new` (sempre visibile → `t('list.createQuote')` → `handleScanNew` route `/procurement/quotes/scan` poiché creazione = scan flow, no inline form), `edit` (solo se non `archived` → router push `/procurement/quotes/{id}/review`), `delete` semantically=archive (solo se non `archived` → `archiveWithUndo` con toast undo 5s pre-esistente). `QuoteDetailSummary` aggiornato: rimosso `<header>` interno (displayNumber+status+vendor+total — ora vive nell'header SSoT); rimosse anche prop `onArchive` + footer button "Επεξεργασία" + import `useRouter`/`Pencil`/`Archive`/`Button`/`QuoteStatusBadge` (azioni ora nell'header). Mantiene vendor info section + lines preview + totals + terms. **SSoT extensions** (puramente additive): (1) `createToolbarConfig` factory esteso a `'quotes'` con `*_BY_TYPE` map entries (NEW_ITEM_LABELS/TOOLTIP, EDIT/DELETE/SHARE_TOOLTIP), filter categories (8 status), sort options (date/number/status/value condivise con `'procurement'`), search placeholder via `quotes:list.searchPlaceholder` namespace. `quotesToolbarConfig` esportato come `quotesConfig` da `@/components/core/CompactToolbar`. `availableActions` overrides: favorites/import/preview/copy/refresh/reports/settings/help/deleteItems/archive/share disabled (quote archive=lifecycle status triggered da header, non da toolbar — toolbar archive button è cosmetic-only across all entities, allineato a PO behavior). (2) `QUOTE_STATUS_OPTIONS` + componente pre-configurato `QuoteStatusQuickFilters` aggiunti a `src/components/shared/TypeQuickFilters.tsx` (icone: FileEdit/Send/Inbox/Eye/CheckCircle/XCircle/Clock/Archive — 4 nuovi import lucide). (3) **NEW** `QuoteListCard` (`src/domain/cards/quote/QuoteListCard.tsx`, ~125 LOC) wrapper di `ListCard` con default Quote-specific: icon `FileText` ambra (allineato a sub-nav Phase F), title=`displayNumber`, `inlineBadges` con status badge mapping `secondary/info/warning/success/destructive`, subtitle truncated `"Vendor · Total · ValidUntil/Date"` con vendor resolution priority `extractedData.vendorName.value` → `useContactById(vendorContactId)` → raw id; barrel `src/domain/cards/quote/index.ts` + re-export in `src/domain/cards/index.ts` + `src/domain/index.ts` allineati al pattern. **i18n**: 22 nuove chiavi `quotes.json` (el+en) — `list.entityName/ariaLabel/searchPlaceholder/cardAriaLabel/requiresAction/allQuotes/emptyTitle/emptyDescription/createQuote`, `filters.quoteStatusAriaLabel`, `filters.quoteStatus.{all,draft,sent_to_vendor,submitted,under_review,accepted,rejected,expired,archived}` + matching `*Tooltip`, `detail.editQuote/archiveQuote`. `QuotesPageContent.tsx` aggiornato: aggiunto `useRouter`+`handleEditQuote(id) → router.push(/quotes/{id}/review)`, montato `QuoteDetailsHeader` sopra `QuoteDetailSummary` su entrambi i mount (desktop split + mobile slide-in), `QuoteList` cabla `actionRequired/onCreateNew=handleScanNew/onSelectQuote/selectedQuoteId/onEditQuote`. View archiviata (showArchived) refactorata in inline `ArchivedQuotesPanel` (~50 LOC, ScrollArea + restore button per item) — il vecchio `QuoteList` faceva double-duty per main+archived, ora separazione chiara di responsabilità. **GOL checklist**: ✅ proattivo (action-required derivato da status, no flag stato); ✅ idempotente; ✅ SSoT (tutti i building block centralizzati); ✅ <500 LOC tutti i file; ✅ <40 LOC tutte le funzioni; ✅ no `any`/`as any`/inline styles; ✅ i18n compliant zero hardcoded strings. Files: `QuoteList.tsx` (REWRITE), `QuoteDetailSummary.tsx` (REFACTOR — header drop), `QuotesPageContent.tsx` (REWRITE — wiring SSoT + ArchivedQuotesPanel inline), `QuoteDetailsHeader.tsx` (NEW), `QuoteListCard.tsx` (NEW), `domain/cards/quote/index.ts` (NEW), `useQuotesPageState.ts` (+`actionRequired` memo), `configs.ts`+`index.ts` (CompactToolbar, EXTEND `'quotes'`), `TypeQuickFilters.tsx` (EXTEND `QUOTE_STATUS_OPTIONS`+`QuoteStatusQuickFilters`+4 lucide icons), `domain/cards/index.ts` (+1 export), `domain/index.ts` (+1 export), `quotes.json` el+en (+22 keys ciascuno). |
| 2026-04-28 | ⚡ **Phase H+2 — Quotes namespace-ready guard removal (sister-page parity)**: dopo il fix Phase H+1 il flash residuo persisteva perché `QuotesPageContent` aveva un guard early-return `if (!isNamespaceReady) return <PageContainer><Loader2 .../></PageContainer>`. Su prima visita orders→quotes, il namespace `quotes` (lazy-loaded da `useTranslation('quotes')` via `loadNamespace()` ADR-280) si carica in ~50-150ms → durante quella finestra il guard mostrava `<Loader2>` → flash. Le sister page (`ProcurementPageContent`/orders, `ContactsPageContent`, `BuildingsPageContent`) NON usano questo guard — e infatti non flashano. Rimosso il guard da `QuotesPageContent`: drop import `Loader2`, rimosso `isNamespaceReady` dal destructuring di `useQuotesPageState`, rimosso il blocco early-return. La cache stale ADR-300 (Phase H+1) garantisce dati istantanei; il render iniziale può mostrare per ~50ms chiavi raw nelle dashboard stats labels (se utente apre dashboard subito), tradeoff accettabile e identico alle sister pages. `useQuotesPageState` continua a esporre `isNamespaceReady` (zero impatto consumer), solo non viene più consumato. Files: `QuotesPageContent.tsx`. |
| 2026-04-28 | ⚡ **Phase H+1 — Quotes navigation flash fix (ADR-300 parity)**: navigando da `/procurement` (Παραγγελίες) a `/procurement/quotes` (Προσφορές) lo schermo lampeggiava (loader → list), mentre quotes→orders era fluido. Root cause: asimmetria tra `usePurchaseOrders` (già ADR-300 stale-cache via `createStaleCache<PurchaseOrder[]>('procurement')` + `silentInitialFetch: cache.hasLoaded()`) e `useQuotes` (`initialData: []` hardcoded, no cache → `loading=true` su remount → flash). Allineato `useQuotes` al pattern centralizzato di `usePurchaseOrders`: import `createStaleCache` da `@/lib/stale-cache`, istanza modulo `quotesCache = createStaleCache<Quote[]>('quotes')`, calcolo `isDefaultFilters` (no projectId/rfqId/trade/status/vendorContactId), `fetcher` chiama `quotesCache.set(result)` se default, `initialData: isDefaultFilters ? quotesCache.get() ?? [] : []`, `silentInitialFetch: isDefaultFilters && quotesCache.hasLoaded()`. Risultato: seconda visita Προσφορές con default filters mostra dati istantaneamente da cache, refetch silente in background → zero flash, parità completa con orders. Files: `useQuotes.ts` (~30→44 LOC). |
| 2026-04-28 | 🧩 **Phase I — Tabs SSoT Consolidation** ([ADR-328](./ADR-328-tabs-ssot-consolidation.md)). Estratta primitiva pura `BaseTabs` (~130 LOC, 15 test, 95% coverage, jest-axe a11y) + 2 wrapper sottili `StateTabs` (controlled/uncontrolled + selection banner + `fillHeight`, 14 test) e `RouteTabs` (pathname/router-aware, 12 test). `TabsComponents.tsx` ridotto da 229→50 LOC di alias `@deprecated` (`TabsContainer`/`ToolbarTabs`/`TabsOnlyTriggers` → `StateTabs`). `TabsNav.tsx` rimosso (zero consumer remaining post-L4 — `ProcurementSubNav` migrato a `RouteTabs` diretto, nessuno usava `variant='link'`). `ProcurementSubNav` migrato a consumer diretto di `RouteTabs` come pattern canonico per nuove sub-nav. Type SSoT in `tabs-types.ts` (BaseTabDef/TabDefinition/TabsNavTab + TABS_STYLES). **Enforcement**: CHECK 3.24 AST-based ratchet (`scripts/check-tabs-import-ratchet.js`, baseline `.tabs-import-baseline.json` 25 file/25 violazioni — tutti legacy) blocca nuovi import deprecati + nuovi `<TabsNav variant="radix">`. `.ssot-registry.json` +modulo `tabs-primitive` Tier 2 (defense-in-depth grep). Zero call-site churn sui 26 consumer esistenti. Files: `tabs-types.ts` (NEW), `base-tabs.tsx` (NEW), `state-tabs.tsx` (NEW), `route-tabs.tsx` (NEW), `__tests__/{base,state,route}-tabs.test.tsx` (NEW, 41 test totali), `TabsComponents.tsx` (REWRITE — alias only), `TabsNav.tsx` (DELETED — zero consumer post-L4), `ProcurementSubNav.tsx` (REFACTOR — direct RouteTabs), `scripts/check-tabs-import-ratchet.js` (NEW), `.tabs-import-baseline.json` (NEW), `package.json` (+2 scripts), `scripts/git-hooks/pre-commit` (+CHECK 3.24), `.ssot-registry.json` (+`tabs-primitive` Tier 2), `ADR-328-tabs-ssot-consolidation.md` (NEW), `adr-index.md` (+ADR-328 entry), `precommit-checks.md` (+CHECK 3.24 entry). |
| 2026-04-29 | 🐛 **Phase J — Contact tab Quotes Section bugfix (parità con PO/RFQ Sections)**: dentro `/contacts → tab "Προσφορές & Παραγγελίες"`, la sub-sezione "Προσφορές" montava erroneamente l'intero `<QuoteList />` global (CompactToolbar + search + status quick filter chips + `QuoteListCard` verticali) → UX rotta vs sister-sections "Παραγγελίες Αγοράς" / "Προσκλήσεις σε RFQ" che usano il pattern compatto `Card + Table` inline. Riscritto `ContactQuotesSection.tsx` (~110 LOC, era 37) con composizione 1:1 delle sister-sections: `Card` + `CardHeader` (title + optional `+Νέα Προσφορά` button se `!archived`) + `CardContent` con loading/empty state e `Table` 4-col (Quote# / Status / Date / Total + arrow). Status badge via `QuoteStatusBadge` SSoT (riusa semantic mapping). Date via `formatDate` su `createdAt.seconds * 1000`. Total via `formatCurrency(quote.totals.total)`. Click row → `router.push(/procurement/quotes/{id}/review)` (rotta detail pre-esistente, invariata). Vendor column omessa (scope già contact-specific, allineato a `ContactPurchaseOrdersSection` che è già 4-col senza vendor). **Robust `timestampToDate` helper**: gestisce 4 shape che `quote.createdAt` può assumere a runtime (Firestore client SDK `{ seconds, nanoseconds }`, Admin SDK serialized `{ _seconds, _nanoseconds }`, `Date` instance, ISO string/epoch number) + fallback `'—'` per null/undefined/invalid. Bug repro: durante render iniziale, `formatDate(undefined * 1000)` → `RangeError: Invalid time value` se `createdAt.seconds` non è il path canonico. Test suite NEW `__tests__/ContactQuotesSection.test.tsx` (12 test, ~2.5s): empty/loading state, N rows render, row click navigation, status badge per row, create button visibility per `archived`, formatted total, **+4 timestamp-shape edge cases** (Date instance, ISO string, Admin SDK `{ _seconds }`, null → em-dash). Container globali (`/procurement`, `/procurement/quotes`, `QuoteList.tsx`, `QuoteListCard.tsx`, `QuotesPageContent.tsx`) **invariati**. Sister sections (`ContactPurchaseOrdersSection`, `ContactRfqInvitesSection`) **invariate**. Mount order in `ProcurementContactTab.tsx` **invariato**. **GOL checklist**: ✅ proattivo (pattern già stabilito da PO/RFQ Sections, allineato 1:1); ✅ idempotente (pure render); ✅ SSoT (riusa `QuoteStatusBadge`, `formatDate`, `formatCurrency`, i18n keys esistenti `quotes:quotes.{loading,empty,number,status,createdAt,total,create}`, `contacts:procurementTab.sections.quotes`); ✅ <500 LOC; ✅ <40 LOC funzioni; ✅ no `any`/`as any`/inline styles; ✅ i18n compliant zero hardcoded strings; ✅ no nuove i18n keys (zero locale changes). Files: `ContactQuotesSection.tsx` (REWRITE), `__tests__/ContactQuotesSection.test.tsx` (NEW). |
| 2026-05-03 | 📋 **§"Αποφάσεις" #1 SUPERSEDED by [ADR-330](./ADR-330-procurement-hub-scoped-split.md)** — la decisione "Standalone top-level (displayOrder: 55)" del 2026-03-28 era corretta per la fase A (solo PO management), ma con l'aggiunta di Quote/RFQ/Sourcing Events (ADR-327) e le richieste future (Vendor Master, Material Catalog, Framework Agreements), serve modello ibrido SAP-style: top-level `/procurement` = Hub company-wide (Vendors, Catalog, Agreements, Cross-project Spend), project tab `Προμήθειες` (order 12.5) = transazionale (RFQ, Quote, PO scoped per progetto). Sidebar entry resta a displayOrder 55 con label tooltip aggiornato. Phase 1 (project tab enrichment) additive zero-risk; Phase 2 (top-level redesign) posticipata fino a Phase 3-4. ADR-330 è 📋 PROPOSED, pending Giorgio answers a §6 D1-D7. |
| 2026-05-03 | 🎨 **Empty State SSoT alignment con Έργα — Quotes** — stesso intervento applicato a `/procurement/quotes` (Προσφορές): sostituito custom `EmptyDetailState` locale (FileText in rounded `bg-muted`, no CTA button) con `<DetailsContainer selectedItem={null} onCreateAction={handleScanNew} emptyStateProps={{ icon: NAVIGATION_ENTITIES.quote.icon, title, description }} />`. Aggiunto entity `quote` (icon `FileText`, color `text-amber-600` allineato sub-nav Phase F) a `NAVIGATION_ENTITIES` + tipo `'quote'` a `NavigationEntityType`. Nuove i18n keys `quotes.emptyState.{title,description}` (el+en) parallele a `projects.emptyState.*`/`procurement.emptyState.*`. Click sul Plus button → `handleScanNew` (route `/procurement/quotes/scan`, scan-flow per quote creation). Refactor branch logic identico al sibling PO: outer wrapper div solo per `selectedQuote`; empty branch delega interamente a DetailsContainer. Eliminata funzione `EmptyDetailState` locale (~20 LOC duplicate). Files: `QuotesPageContent.tsx` (refactor + cleanup), `navigation-entities/types.ts` (+quote type), `navigation-entities/entity-config.ts` (+quote entry), `quotes.json` el+en (+emptyState block). |
| 2026-05-03 | 🎨 **Empty State SSoT alignment con Έργα** — pannello dx di `/procurement` (no PO selezionato) usava custom `EmptyDetailState` locale (rounded `bg-muted` icon container, bare `<button>`, `<p>` titolo) divergente dal pattern canonico di `/projects` che usa `DetailsContainer` SSoT (h2 titolo, `<Button variant="outline">` + Plus icon, layout uniforme). Sostituito con `<DetailsContainer selectedItem={null} onCreateAction={handleCreateNew} emptyStateProps={{ icon, title, description }} />` — pixel parity garantita perché stesso componente sotto. Aggiunto entity `procurement` (icon `Package`, color `text-blue-600`) a `NAVIGATION_ENTITIES` (SSoT registry) + tipo `'procurement'` a `NavigationEntityType`. Nuove i18n keys `procurement.emptyState.{title,description}` (el+en) parallele a `projects.emptyState.*`. Refactor branch logic: outer wrapper div solo per editMode/selectedPO; empty branch delega interamente a DetailsContainer (evita doppio `bg-card border`). Eliminata funzione `EmptyDetailState` locale (~30 LOC duplicate). Files: `ProcurementPageContent.tsx` (refactor + cleanup), `navigation-entities/types.ts` (+procurement type), `navigation-entities/entity-config.ts` (+procurement entry), `procurement.json` el+en (+emptyState block). |
| 2026-05-04 | 🎨 **Master-Detail Split Panel — Vendors, Materials, Agreements**: applicato pattern master-detail uniforme (URL-persistent `?vendorId=` / `?materialId=` / `?agreementId=`) alle 3 pagine procurement restanti, in parità con Quotes (Phase H) e POs (Phase E). **Vendors** (`/procurement/vendors`): nuovi `VendorList.tsx` (scroll list slim, search integrata, `EntityListColumn` SSoT) + `VendorDetail.tsx` (header contact + KPI grid 2×2 via `useSupplierMetrics` + recent POs via `useVendorPurchaseOrders` + CRM link → `/contacts/{id}`); `vendors/page.tsx` riscritto con `ListContainer` + desktop `hidden md:flex` split + mobile `MobileDetailsSlideIn`. **Materials** (`/procurement/materials`): nuovo `MaterialSlimList.tsx` (slim list, click-to-select, parità codice/categoria/prezzo) + `MaterialDetail.tsx` (header name/code/category/unit + price history + preferred suppliers risolti via `usePOSupplierContacts` + Edit/Delete callbacks → dialog esistenti); `materials/page.tsx` riscritto con filters bar + `ListContainer` split. **Agreements** (`/procurement/agreements`): nuovo `AgreementSlimList.tsx` (slim list con status badge color-coded) + `AgreementDetail.tsx` (header agreementNumber/title/status + vendor risolto + validity range + days remaining + discount terms: flat % o volume breakpoints table + Edit/Delete → dialog esistenti); `agreements/page.tsx` riscritto con filters bar + `ListContainer` split. **i18n**: 30 nuove chiavi `procurement.json` (el+en) nei namespace `hub.vendorMaster.detail.*`, `hub.materialCatalog.detail.*`, `hub.frameworkAgreements.detail.*`. Files: `VendorList.tsx` (NEW), `VendorDetail.tsx` (NEW), `MaterialSlimList.tsx` (NEW), `MaterialDetail.tsx` (NEW), `AgreementSlimList.tsx` (NEW), `AgreementDetail.tsx` (NEW), `vendors/page.tsx` (REWRITE), `materials/page.tsx` (REWRITE), `agreements/page.tsx` (REWRITE), `procurement.json` el+en (+keys). |
| 2026-04-29 | 🛡️ **Phase H+3 — CHECK 3.25 No-Navigation-Flash Ratchet enforcement** (Beyoncé Rule per Phase H+1/H+2). I fix navigation flash di Phase H+1 (`useQuotes` → ADR-300 stale-cache) e H+2 (rimozione guard `!isNamespaceReady`) erano **non protetti**: qualsiasi futuro hook list-fetch o nuova `*PageContent.tsx` poteva reintrodurre flash senza nessun blocco al commit. Aggiunto **`scripts/check-no-flash-ratchet.js`** (~270 LOC, regex-based, smoke <500ms) che rileva tre pattern: (A) `useAsyncData<T[]>` in `src/hooks/**/use*.ts` o `src/subapps/*/hooks/**/use*.ts` senza import `@/lib/stale-cache` E senza opzione `silentInitialFetch:` (ADR-300 violation), (B) `if (!isNamespaceReady)` early-return in `*PageContent.tsx` (namespace lazy-load ~50-150ms = blank flash), (C) bare `if (loading)` con `<Loader2>` body in `*PageContent.tsx` (canonico SSoT è `if (loading && data.length === 0) return <PageLoadingState />` ADR-229). **Ratchet semantics** identiche a CHECK 3.23/3.24: new file = zero tolerance, existing file = count solo decrescente. **Baseline** `.no-flash-baseline.json` cattura **4 violazioni pre-esistenti** (3 hook vendor portal/RFQ + 1 EditObligationPageContent), ratchet-down via Boy Scout. **Golden test suite** `scripts/__tests__/check-no-flash-ratchet.test.js` — **53 test** (8 group: pattern A/B/C semantics, file scope hooks/PageContent, allowlist, baseline ratchet new/equal/lower/higher, parseArgs CLI), runtime ~2s. **Pre-commit integration**: CHECK 3.25 dopo CHECK 3.24, env `SKIP_NO_FLASH=1` per emergency. **Registry**: `.ssot-registry.json` +modulo `no-navigation-flash` Tier 2 (defense-in-depth grep ERE su `if[[:space:]]*\([[:space:]]*![[:space:]]*isNamespaceReady`, allowlist `src/lib/stale-cache.ts`+`src/hooks/useAsyncData.ts`+`src/subapps/dxf-viewer/`). **Scripts** package.json: `no-flash:audit`/`:baseline`/`test:no-flash`. **GOL checklist**: ✅ proattivo (presubmit prima di commit non a runtime); ✅ no race (smoke deterministica, pure functions); ✅ idempotente (regex su file content, stesso input = stesso output); ✅ belt-and-suspenders (regex scanner + registry grep + golden tests + ADR doc); ✅ SSoT (`@/lib/stale-cache` riferimento canonico, `<PageLoadingState>` ADR-229); ✅ await-free (sync I/O scanner ok per smoke target); ✅ lifecycle owner = `scripts/check-no-flash-ratchet.js` + baseline. **Comandi**: `npm run no-flash:audit` (full scan), `npm run no-flash:baseline` (refresh dopo cleanup), `npm run test:no-flash` (golden), `npm run test:ssot-suite` (include CHECK 3.25). Files: `scripts/check-no-flash-ratchet.js` (NEW), `scripts/__tests__/check-no-flash-ratchet.test.js` (NEW, 53 test), `.no-flash-baseline.json` (NEW, 4 baseline violations), `.ssot-registry.json` (+`no-navigation-flash` Tier 2), `scripts/git-hooks/pre-commit` (+CHECK 3.25 block), `package.json` (+3 scripts), `precommit-checks.md` (+CHECK 3.25 entry), `ADR-294-ssot-ratchet-enforcement.md` (modulo count update). | (Beyoncé Rule per Phase H+1/H+2). I fix navigation flash di Phase H+1 (`useQuotes` → ADR-300 stale-cache) e H+2 (rimozione guard `!isNamespaceReady`) erano **non protetti**: qualsiasi futuro hook list-fetch o nuova `*PageContent.tsx` poteva reintrodurre flash senza nessun blocco al commit. Aggiunto **`scripts/check-no-flash-ratchet.js`** (~270 LOC, regex-based, smoke <500ms) che rileva tre pattern: (A) `useAsyncData<T[]>` in `src/hooks/**/use*.ts` o `src/subapps/*/hooks/**/use*.ts` senza import `@/lib/stale-cache` E senza opzione `silentInitialFetch:` (ADR-300 violation), (B) `if (!isNamespaceReady)` early-return in `*PageContent.tsx` (namespace lazy-load ~50-150ms = blank flash), (C) bare `if (loading)` con `<Loader2>` body in `*PageContent.tsx` (canonico SSoT è `if (loading && data.length === 0) return <PageLoadingState />` ADR-229). **Ratchet semantics** identiche a CHECK 3.23/3.24: new file = zero tolerance, existing file = count solo decrescente. **Baseline** `.no-flash-baseline.json` cattura **4 violazioni pre-esistenti** (3 hook vendor portal/RFQ + 1 EditObligationPageContent), ratchet-down via Boy Scout. **Golden test suite** `scripts/__tests__/check-no-flash-ratchet.test.js` — **53 test** (8 group: pattern A/B/C semantics, file scope hooks/PageContent, allowlist, baseline ratchet new/equal/lower/higher, parseArgs CLI), runtime ~2s. **Pre-commit integration**: CHECK 3.25 dopo CHECK 3.24, env `SKIP_NO_FLASH=1` per emergency. **Registry**: `.ssot-registry.json` +modulo `no-navigation-flash` Tier 2 (defense-in-depth grep ERE su `if[[:space:]]*\([[:space:]]*![[:space:]]*isNamespaceReady`, allowlist `src/lib/stale-cache.ts`+`src/hooks/useAsyncData.ts`+`src/subapps/dxf-viewer/`). **Scripts** package.json: `no-flash:audit`/`:baseline`/`test:no-flash`. **GOL checklist**: ✅ proattivo (presubmit prima di commit non a runtime); ✅ no race (smoke deterministica, pure functions); ✅ idempotente (regex su file content, stesso input = stesso output); ✅ belt-and-suspenders (regex scanner + registry grep + golden tests + ADR doc); ✅ SSoT (`@/lib/stale-cache` riferimento canonico, `<PageLoadingState>` ADR-229); ✅ await-free (sync I/O scanner ok per smoke target); ✅ lifecycle owner = `scripts/check-no-flash-ratchet.js` + baseline. **Comandi**: `npm run no-flash:audit` (full scan), `npm run no-flash:baseline` (refresh dopo cleanup), `npm run test:no-flash` (golden), `npm run test:ssot-suite` (include CHECK 3.25). Files: `scripts/check-no-flash-ratchet.js` (NEW), `scripts/__tests__/check-no-flash-ratchet.test.js` (NEW, 53 test), `.no-flash-baseline.json` (NEW, 4 baseline violations), `.ssot-registry.json` (+`no-navigation-flash` Tier 2), `scripts/git-hooks/pre-commit` (+CHECK 3.25 block), `package.json` (+3 scripts), `precommit-checks.md` (+CHECK 3.25 entry), `ADR-294-ssot-ratchet-enforcement.md` (modulo count update). |
...
## ΣΗΜΕΙΩΣΗ (2026-08-09, ADR-781) — ΤΙ ΔΕΝ ΦΤΑΝΕΙ ΤΟ CHECK 3.25

Το CHECK 3.25 (No-Navigation-Flash Ratchet, ADR-267/300) απαγορεύει **ήδη** την αρχή:
*μην κλειδώνεις το render στην ετοιμότητα ενός namespace*. Η αρχή είναι σωστή· **η επιβολή δεν
έφτασε**, και το κόστος μετρήθηκε: **17 ωμά i18n κλειδιά σε 141 διαδρομές**, μόνιμα, στην παραγωγή.

**Δύο ανεξάρτητες τυφλότητες**, καμία από τις οποίες είναι ελάττωμα του 3.25 — είναι **εμβέλεια**:

1. Η εμβέλειά του είναι **μόνο** `*PageContent.tsx`. Ο φρουρός εδώ ζούσε σε
   `src/components/sidebar/sidebar-menu-item.tsx`, δηλαδή στο **root layout** — που ζωγραφίζεται
   σε **κάθε** διαδρομή, όχι σε μία σελίδα.
2. Ψάχνει το **literal** `isNamespaceReady`. Εδώ η ίδια σημασιολογία λεγόταν `isLoading` και
   γεννιόταν σε **helper hook** (`useTranslationLazy`), όχι στο σημείο του φρουρού.

Η κλάση καλύπτεται πλέον από το **CHECK 3.51 (ADR-781)**, που ρωτά το ερώτημα **σε επίπεδο αιτίας**
(«παραδίδει module το `t` μαζί με ετοιμότητα που μόνο `useEffect` διορθώνει;») και **σε επίπεδο
αποτελέσματος** («περιέχει ωμά κλειδιά το HTML που στέλνει ο server;»).

⚠️ Τα δύο **δεν** ενοποιούνται: το 3.25 είναι **ratchet** πάνω σε αναγνωστικό flash πλοήγησης· το
3.51 είναι **zero-tol** πάνω στη δομή της ετοιμότητας + **ratchet** πάνω στην πραγματική έξοδο του
server. Η συγχώνευσή τους θα ήταν το λάθος του ADR-749.
