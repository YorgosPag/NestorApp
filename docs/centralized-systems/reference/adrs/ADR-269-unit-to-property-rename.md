...
| 2026-04-01 | Phase B6: Records/configs/services cleanup — Fixed 30 TS errors from stale `unit`/`units` keys in Record literals and config objects. Files: PropertyHierarchyCard (BreadcrumbSyncParams), CustomerActionButtons + CustomerInfoTypes (CustomerInfoContext), smart-dialog-config (DialogEntityType — merged duplicate unit/property entries), OverlayListCard (KIND_TO_ENTITY), useEntityAssociations (GroupedContactEntityLinks.units→.properties), entity-linking config + service (EntityType), ContextualNavigationService (NavigableEntityType), sales-accounting-helpers (SaleLineItem.assetType), DXF toolbar-colors + DraggableOverlayToolbar + OverlayToolbar + KindSelector + FullscreenView + WizardStepsUnit + useFloorEntitiesForLinking (removed dead unitName fallback). 17 files total. | Claude Code (Opus 4.6) |
