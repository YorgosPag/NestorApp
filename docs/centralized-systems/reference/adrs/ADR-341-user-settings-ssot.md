...
| Date | Changes |
|------|---------|
| 2026-07-16 | 🧹 **`SelectionBoxSettings` — ένα shape αντί για τέσσερα αντίγραφα + `migrateSelectionBox` βγήκε από το `loadSettings`** (Opus 4.8, N.18 + ADR-584 de-duplication· εντοπίστηκε από full `jscpd` scan). **Root:** το selection-box shape (`fillColor`/`fillOpacity`/`borderColor`/`borderOpacity`/`borderStyle`/`borderWidth`) ήταν γραμμένο **αυτούσιο 4×** — `config.ts` `selection.window`, `config.ts` `selection.crossing`, `canvas-v2/layer-canvas/layer-types.ts` `SelectionSettings.window` + `.crossing` — ενώ το `user-settings-schema.ts` το είχε **ήδη σωστά** ως ένα κοινό `selectionBoxFaceSchema`. Το persistence layer δηλαδή ήξερε ότι είναι ένα shape· ο κώδικας όχι. **NEW exports στο `systems/cursor/config.ts`**: `SelectionMode` (`'window' \| 'crossing'`) + `SelectionBoxSettings` → `CursorSettings.selection` έγινε `Record<SelectionMode, SelectionBoxSettings>`. `layer-types.ts` καταναλώνει πλέον τον ίδιο τύπο (`import type` = σβήνεται στο build → **καμία** runtime εξάρτηση canvas→cursor· ο σχολιασμός «✅ REMOVED DUPLICATE» της γρ. 12 έλεγε ψέματα, τώρα ισχύει). `useCanvasSettings.ts`: το 14γραμμο **identity-copy** `useMemo` έγινε pass-through — **referentially ισοδύναμο**, το memo dep ήταν ήδη `[cursorSettings.selection]` και το `updateSettings` ανακατασκευάζει το `selection` σε κάθε κλήση, άρα το downstream churn είναι **ακριβώς** το ίδιο (ADR-040-safe). **NEW exported pure `migrateSelectionBox(mode, parsedBox)`** — το migration block του `loadSettings` έγραφε το ίδιο 8γραμμο 2× (defaults fill + legacy `opacity`→`fillOpacity` rename)· τώρα 1×, με τα defaults να επιλέγονται από το `mode`. **Tests: NEW `__tests__/migrate-selection-box.test.ts` (10)** — ο δρόμος **δεν είχε κανένα test** παρότι διαβάζει αποθηκευμένες ρυθμίσεις χρήστη (σφάλμα εκεί = σιωπηλό reset χρωμάτων). Καρφώνουν ανά πεδίο: `??` (κρατά `0` = πλήρως διάφανο) vs `\|\|` (κενό `borderStyle` = λείπει), legacy `opacity` precedence, per-mode defaults (window=solid/μπλε, crossing=dashed/πράσινο), και ότι δεν επιστρέφεται το defaults object by reference. **15 suites / 70 tests GREEN** σε `systems/cursor`. Καμία αλλαγή συμπεριφοράς/σχήματος → **καμία migration ανάγκη** για ήδη αποθηκευμένα settings. UI πλευρά: βλ. `SelectionSettings` στο ADR-584. **ΟΧΙ tsc (N.17).** |
...
| 2026-06-28 | ⚡ **Perf — split volatile autosave-status out of the God settings context** (Opus 4.8, GOL). Root (React-DevTools Profiler, 2026-06-28): `EnterpriseDxfSettingsProvider` sits high in the DXF-viewer tree and its **monolithic** `contextValue` baked `saveStatus`/`lastSaved`/`isSaving`/`lastError` into the `settings` spread + the memo deps. Autosave is debounced but fires `SAVE_START → SAVE_SUCCESS` (2 dispatches) per edit → each rebuilt the whole context → re-rendered **all ~28 consumer subtrees (~1980 fibers / ~270-320ms per commit)**; a single ribbon-combobox tweak ≈ 3 full-app re-renders (~800ms). FIX: NEW dedicated `SettingsSaveStatusContext` (`SettingsSaveStatusValue`: saveStatus/lastSaved/isSaving/isAutoSaving/hasUnsavedChanges/lastError) + `useSettingsSaveStatus`/`useSettingsSaveStatusOptional`, provided by a second `.Provider` wrapping the main one. Save-status fields REMOVED from `EnterpriseSettingsWithMetadata` (now `{ mode }` only) + the main interface + the `contextValue` memo deps → the main value rebuilds **only on a real settings edit**, so autosave re-renders ONLY its single consumer. `CentralizedAutoSaveStatus` reads settings content (line/text/grip dots) from the main context + volatile status from the new context (behaviour-identical: `isAutoSaving`/`hasUnsavedChanges` keep the prior `saveStatus`-derived semantics). Blast radius = 3 files (`EnterpriseDxfSettingsProvider.tsx`, `settings-provider/index.tsx`, `ui/components/CentralizedAutoSaveStatus.tsx`); only consumer of the moved fields was `CentralizedAutoSaveStatus`. Part of the ADR-532 click-rerender-cascade investigation (the `EnterpriseDxfSettingsProvider` full-tree cascade was the single heaviest pattern in the profile). 🔴 browser-verify (Profiler: autosave commit re-renders only `CentralizedAutoSaveStatus`, not ~1980 fibers; autosave status icon/text/last-saved still correct) + commit. |
| 2026-06-16 | 🐛 **Fix false-positive "INFINITE LOOP DETECTED"** (Opus 4.8, GOL). Root cause: `EnterpriseDxfSettingsProvider` render-counter was **cumulative across the whole component lifetime and never reset** → after ~100 legitimate re-renders (StrictMode 2× + floor/settings/hydration transitions) it tripped `RENDER_LOOP_THRESHOLD` and `derr()` (which always prints to `console.error`, even in prod) logged a scary loop warning. No actual loop existed: the repo-sync `LOAD_SUCCESS` reducer stores `action.payload` verbatim (no normalization) so `stableHash(state.settings) === remoteHash === lastWrittenHashRef`, making the mirror effect early-return. FIX: converted the detector to **rate-based (sliding burst window)** — counter resets when `RENDER_LOOP_WINDOW_MS` (2000ms) elapses without crossing the threshold; only a genuine tight loop (>100 renders inside one window) now fires. EDIT `settings-provider/constants.ts` (+RENDER_LOOP_WINDOW_MS) + `settings-provider/EnterpriseDxfSettingsProvider.tsx` (window logic + self-reset after firing). Behaviour-only; no architectural change. **NOTE:** the separately-reported "lost foundation formwork after hard refresh" is unrelated to this provider (settings ≠ scene geometry) — tracked as a distinct scene-persistence investigation. |
