...
**Status:** 🟢 Slices 1-6 + Slice 8 implemented (data model + resolver + BOQ + 3D/2D skin + δοκάρια + toggle/override + beam↔column junction + **Slice 8 height-aware wall coverage**). **Slice 7 (merged structural silhouette) ΑΝΑΙΡΕΘΗΚΕ → dormant** (boundary-dependent σε ανοιχτή τοπολογία· ενεργό = per-element Slice 6). Corner-join-only merged-miter = 🔵 DEFER. — pending browser-verify + commit (2026-06-13)
