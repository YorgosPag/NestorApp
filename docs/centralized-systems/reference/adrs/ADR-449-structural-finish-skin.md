...
**Status:** 🟢 Slices 1-6 + Slice 8/8b + **Slice 9** implemented (data model + resolver + BOQ + 3D/2D skin + δοκάρια + toggle/override + beam↔column junction + height-aware wall coverage + attached-top exclusion + **Slice 9 συνεπής γωνιακή ΕΝΩΣΗ σοβά δοκαριού↔κολόνας, browser-verified «τέλεια ένωση»**). **Slice 7 (merged structural silhouette) ΑΝΑΙΡΕΘΗΚΕ → dormant** (boundary-dependent σε ανοιχτή τοπολογία· ενεργό = per-element Slice 6/9). Corner-join-only merged-miter = 🔵 DEFER. — Slice 9 pending commit (2026-06-13)
