...
**NEW:**
- `bim/finishes/structural-finish-silhouette.ts` *(NEW, pure)* — `computeStructuralSilhouetteBands(input)`: height-band decomposition (sorted unique z-breakpoints) + ανά band `safeUnion` cores → outer ring(s) → `resolveStructuralFinishFaces` (τοίχοι **un-dilated** obstacles) → merged faces → **`alignSegmentsToWallFaces`** (A-fix: collinear-με-τοίχο ακμές → outer σοβά στην όψη τοίχου, coplanar+ορατός). `footprintToPolygon` **CCW-normalise** (winding fix: το CW beam outline αλλιώς = τρύπα στο `safeUnion`). REUSE-only (`safeUnion` + resolver), μηδέν νέα boolean/offset. + `__tests__/structural-finish-silhouette.test.ts` (Β union/μηδέν seam, Α wall-obstacle αφαίρεση, band decomposition, regression μεμονωμένου, disabled/empty).
