...
- **2026-09-19 (ADR-777 §8.60.20)**: η επαναφορά παρακολουθήματος (`revert`) μηδενίζει **και** το `commercial.rentPrice` — ως τότε έμενε κρυμμένο κάτω από «Μη διαθέσιμο» και ξαναεμφανιζόταν στην επόμενη διάθεση. Το πεδίο μπήκε στον τύπο `AppurtenanceUpdate` και στο κλειστό σύνολο της άγκυρας Α4. Η λίστα/κάρτες/πωλήσεις δείχνουν πλέον τη διάθεση του χώρου από το `commercialStatus` (τον ΕΝΑ αναγνώστη `resolveSpaceStatuses`), άρα μια θέση που πούλησε αυτή η διαδρομή φαίνεται «Πωλήθηκε» — όχι «Διαθέσιμη» από το παλιό `status`.
