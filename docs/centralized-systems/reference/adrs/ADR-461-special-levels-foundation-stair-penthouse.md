...
## 6. Changelog
- **2026-06-23 (Opus 4.8) — Απόκρυψη/μη-εκλογή του ασύνδετου default «Επίπεδο 1» όταν υπάρχει δομή κτιρίου (data-loss root, ADR-420· 16 jest+tsc, UNCOMMITTED):** Το bootstrap default level (`isDefault`, `floorId=null`, χωρίς `buildingId`· `LevelOperations.createDefaultLevels`) παρέμενε ορατό + επιλέξιμο ΚΑΙ μετά τον wizard import — landmine: ό,τι σχεδιαζόταν πάνω του γραφόταν χωρίς `floorId` (ADR-420 `bimScopeWriteFields`) και **εξαφανιζόταν** σε `floorId`-scoped reload (κολόνες/BIM χαμένα μόνιμα· Firestore-verified `lvl_01f065a1` orphan δίπλα σε 3 floor-linked). **FIX (Revit-grade «μία επιφάνεια ανά όροφο, σταθερή ταυτότητα floorId»):** NEW pure SSoT `systems/levels/level-visibility.ts` — `isUnlinkedDefaultLevel` / `hasFloorLinkedLevel` / `selectVisibleLevels` / `pickActiveLevel`. (α) `orderLevelsForPanel` φιλτράρει μέσω `selectVisibleLevels` → ο orphan κρύβεται από το «Στάθμες» panel **μόλις υπάρχει ≥1 floor-linked level** (χωρίς δομή→παραμένει, είναι η μόνη επιφάνεια). (β) `useLevelsFirestoreSync` auto-εκλογή μέσω `pickActiveLevel` + re-elect αν ο ΕΝΕΡΓΟΣ είναι ο orphan ενώ υπάρχει δομή → ο χρήστης δεν ξεκινά/μένει ποτέ στην orphan επιφάνεια. `useFloorTabs` ήδη σωστό (tabs=floors, default χωρίς buildingId δεν εμφανίζεται — αμετάβλητο). Wizard/linking (`ensureLevelsForBuilding`/`findOrCreateLevelForFloor`) αμετάβλητα (δουλεύουν σωστά — REUSE). Το προηγούμενο spec «Επίπεδο 1 ALWAYS top» (§ panel ordering) **αναθεωρήθηκε**: το pin ισχύει μόνο όσο ΔΕΝ υπάρχει δομή. Δευτερεύον belt-and-suspenders (persistence-side defer floorId-less first-save) **DEFERRED**. 🔴 browser-verify (μετά wizard import: το «Επίπεδο 1» δεν φαίνεται στο panel ούτε επιλέγεται· σχεδίαση κολόνας→επιμένει σε reload) + commit. [[ADR-420]]
- **2026-06-17 (Opus 4.8) — C4++ διαβαθμισμένο foundation gating (ADR-467, Giorgio· UNCOMMITTED, 42 jest GREEN):** Ερώτηση Giorgio «είναι σωστό να δημιουργώ θεμελίωση σε υπόγεια/ισόγειο/όροφο/απόληξη;» → **εν μέρει**: η θεμελίωση έχει στατικό νόημα **μόνο** σε θεμελίωση + υπόγεια (+ κατώτατο ισόγειο χωρίς υπόγειο), ΟΧΙ σε υπέργειους ορόφους/απόληξη/στέγη. Το προηγούμενο C4 gating συνέστηνε **όλα** τα tools σε κάθε counted storey (mode `'all'`) → τα θεμελιακά εμφανίζονταν κανονικά (μη-dimmed) και στον 2ο όροφο/απόληξη. **FIX:** NEW SSoT `isFoundationDisciplineInContext(storey)` στο `storey-creation-defaults.ts` (foundation/basement→✅· ground→✅ μόνο αν `isLowestOccupiedStorey`· standard/mezzanine/roof/stair-penthouse→❌). `isCommandRecommendedForStorey` πλέον δέχεται **`ActiveStoreyContext`** (όχι μόνο `kind` — χρειάζεται `isLowestOccupiedStorey`) και κάνει τη `foundation` category **διαβαθμισμένη** ανεξάρτητα από το αν ο όροφος είναι counted· οι υπόλοιπες disciplines αμετάβλητες. `shouldWarnFoundationOnStorey` = `!isFoundationDisciplineInContext` (ΕΝΑ SSoT για dim + warning· basement πλέον ΠΟΤΕ warn). `useRibbonCommands` περνά `activeStorey` context. Toast `storeyGating.foundationUpperStorey` εντονότερο («δεν έχει στατικό νόημα σε υπέργειο όροφο»). Παραμένει **warn-don't-block** (Revit). Τα υπάρχοντα `shouldWarnFoundationOnStorey` jest αμετάβλητα (στο FULL_STACK το basement είναι ήδη lowest). 🔴 browser-verify (Πέδιλο/Πεδιλοδοκός/Συνδετήριος/Εδαφόπλακα dimmed σε όροφο/απόληξη, κανονικά σε θεμελίωση/υπόγειο) + commit. [[ADR-467]]
