...
**Status:** ✅ Phase 1 + Phase 2 + Phase 3 + Phase 4 + Phase 5 (G5 live fence preview + tests) COMPLETE. Phase 5 adds: `buildEntityPreviewPath()` in `trim-fence-hit-detector.ts`, `FencePreviewFn` registry in `TrimToolStore` (`registerFencePreviewFn`/`execFencePreview`), `computeFencePreview` in `useTrimTool`, throttled `execFencePreview` in `useTrimDragCapture` (80ms), `dragPreview` cleared on mouseup, test suite `__tests__/trim-fence-hit-detector.test.ts` (10 test cases).
**Date:** 2026-05-15
