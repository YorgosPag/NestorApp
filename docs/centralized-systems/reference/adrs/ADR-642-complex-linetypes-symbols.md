...
| **Edit-in-place** ✅ | **Επεξεργασία υπάρχοντος user-created τύπου** (Revit/AutoCAD parity): κουμπί «✎ Επεξεργασία» στο αριστερό Properties tab (μόνο `origin==='user-created'`) → διάλογος pre-loaded (name locked) → `upsertUserLinetype` δέχεται/διατηρεί `complex` (δεν ισοπεδώνει compound) → όλες οι γραμμές που τον χρησιμοποιούν repaint (registry sub). Imported/ISO = read-only (duplicate & edit = fast-follow) | #9 | Μεσαίο — **DONE** |
| — | *Εκτός scope προς το παρόν:* Art-brush stretch, raster line styles, `.shx` shape import, «Διπλότυπο & επεξεργασία» για imported/ISO | — | — |
...
- **2026-07-13 (Edit-in-place IMPLEMENTED — επεξεργασία υπάρχοντος user-created τύπου)** — Η **τελευταία**
  ...
  - `ui/line-advanced-panel/LinePropertiesTab.tsx` — κουμπί «✎ Επεξεργασία» δίπλα στο `compoundHint`, ενεργό
    **μόνο** `linetypeDef?.origin === 'user-created'` → ανοίγει τον διάλογο με `editName={linetypeDef.name}`. Re-render:
    same-name upsert notify-άρει μέσω `subscribeLinetypeRegistry` (μηδέν νέο wiring).
  - i18n: `linePatternEditor.editTitle` + `linePatternEditor.edit` (el+en, N.11 keys-first).
  - Tests (jest, N.17): `LinetypeRegistry.upsert.test.ts` — upsert create/edit διατηρεί `complex` (symbol layer,
    ...
    pre-load→Save→registry κρατά το compound/symbol def. 27/27 πράσινα. jscpd καθαρό (N.18).
