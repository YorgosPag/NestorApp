...
### 6.7 Graphical grip editor — 2D bounding-box + per-layer handles (Φ6-A ✅ DONE· Φ6-B/C 📋 PROPOSED)

**Αίτημα Giorgio (2026-07-13):** «Οι σύνθετοι τύποι είναι δισδιάστατοι — μπορούμε να βάλουμε λαβές σε
4 γωνίες + μέση των πλευρών ώστε ο χρήστης να τους χειρίζεται από τις λαβές και να αλλάζει τις σχέσεις
των στρωμάτων;» **Ναι** — είναι πρακτική μεγάλων (**MicroStation Line Style Editor** = γραφικός drag-editor·
**Illustrator** bounding box + Width Tool). Η διαίσθηση είναι σωστή: ο compound/complex τύπος έχει
πραγματικό **2D εύρος** (band) κάθετα στη διαδρομή (offsets στρωμάτων, ύψος συμβόλου/κειμένου, πλάτος),
άρα ένα 8-handle bounding box τον χειρίζεται φυσικά.

```
        ◻────────────◻────────────◻   ← πάνω side-grip = spread άνω στρωμάτων (offset +)
        │  ──┃──┃──┃──┃──┃──┃──┃──  │
  ◻ ────┤  ────────────────────────  ├──── ◻   ← corner = scale ΚΑΙ στους 2 άξονες
        │  ──┃──┃──┃──┃──┃──┃──┃──  │
        ◻────────────◻────────────◻   ← κάτω side-grip = spread κάτω στρωμάτων (offset −)
        ▲            ▲            ▲
   αριστ/δεξ side-grip = κλίμακα μήκους μοτίβου (dash/gap κατά τη διαδρομή)
   • (per-layer dot σε κάθε ράγα) = μετακίνηση ΜΟΝΟ αυτού του στρώματος (offsetMm)
```

**6.7.1 ΠΟΥ ζουν οι λαβές (κρίσιμη απόφαση):** στο **preview swatch του editor** (`CompoundPatternPreview`
μέσα στον `LinePatternLayersEditor`), **ΟΧΙ** στη γραμμή του καμβά. Ο τύπος είναι **ονομασμένο κοινό style**
(edit-in-place → όλες οι γραμμές του ενημερώνονται). Λαβές στην καμβά-γραμμή θα μετέτρεπαν **σιωπηλά όλες**
τις γραμμές που τον χρησιμοποιούν από ένα drag — αντι-πρακτική που το §10 changelog ήδη τεκμηριώνει ρητά ως
λάθος (κανείς μεγάλος δεν «σπάει» κοινό style από property tweak). Το authoring surface = ο διάλογος.

**6.7.2 Χαρτογράφηση λαβών → μοντέλο (μηδέν νέο data — direct manipulation του `ComplexLinetypeDef`, §6.1):**

| Λαβή | Ενέργεια | Πεδίο SSoT |
|---|---|---|
| **πάνω/κάτω μέση** | uniform scale του band width | όλα τα `layers[].offsetMm` × factor γύρω από το band centre |
| **αριστερά/δεξιά μέση** | κλίμακα μοτίβου κατά τη διαδρομή | όλα τα geometry `elements[].lengthMm` (dash/gap/dot) × factor |
| **4 γωνίες** | και τα δύο μαζί (2-άξονας scale) | offset-spread + length μαζί |
| **per-layer dot** (Φ6-B) | μετακίνηση ΕΝΟΣ στρώματος | `layers[i].offsetMm` (drag κατά τη left-normal) |

**6.7.3 Προτεινόμενες αποφάσεις (Revit-grade defaults — προσαρμόσιμα):**
- **Spread = κατοπτρικό γύρω από τον άξονα (0) = το οπτικό μέσο του swatch** (`scaleLayerSpread`).
  ⚠️ **Αναθεώρηση 2026-07-13 (UX Giorgio, 2 iterations):** (1) αρχικά γύρω από *band centre* → σε ασύμμετρο
  compound ξέφευγε από το μέσο· (2) μετά `offset × factor` γύρω από 0 → συμμετρικά ok αλλά ασύμμετρες ράγες
  κινούνταν με **διαφορετικό βήμα** (η μακρινή πιο πολύ). **Τελικό = κατοπτρικό**: κάθε πλευρά οδηγείται στο
  ΙΔΙΟ half-height `H` (η εξωτερική ράγα κάθε πλευράς → `±H`, τυχόν εσωτερικές κρατούν αναλογία μέσα στην
  πλευρά· on-axis layer μένει στη μέση). Έτσι οι ράγες **πάντα ισαπέχουν** από το μέσο με ίδιο βήμα, και η
  τυχόν ασυμμετρία διορθώνεται στο drag — big-player «resize gauge» συμπεριφορά.
- **Corner = ανεξάρτητο 2-άξονα scale** (ΟΧΙ aspect-lock): οι side-grips ήδη δίνουν per-axis, οπότε η γωνία
  απλώς συνδυάζει τα δύο· `Shift` = lock aspect (uniform) ως opt-in (Illustrator-style).
- **Snap 0.5 mm** στο drag (τα linetype dims είναι mm, στρογγυλά νούμερα μετράνε)· `Alt` = free/continuous.
- **Min-guard**: `offsetMm`/`lengthMm` ≥ ε (0.05 mm) — καμία κατάρρευση/αναστροφή προσήμου κατά λάθος.
- **Reversible**: το drag ενημερώνει το editor state (`setLayers`), commit στο **Save** (όχι magic
  auto-mutation)· Cancel = τίποτα — Google-level, όπως το «Κεντράρισμα» της Φ5-A.

**6.7.4 SSoT & υλοποίηση (proposed):**
- **Pure mapping helpers** στο `config/line-pattern-segments.ts` (δίπλα στο `centerOffsetForLayer`):
  `scaleLayerSpread(layers, factor)` (offset × factor γύρω από centre) + `scalePatternLength(layers, factor)`
  (geometry element lengths × factor, text/symbol slots ανέγγιχτα). Testable χωρίς UI (jest).
- **Grip overlay** πάνω από το `StrokePreviewCanvas`: local AABB hit-test 8 handles + N per-layer dots σε
  pixel space του swatch (**ΔΕΝ** χρειάζεται ο βαρύς canvas grip subsystem — είναι μικρό local overlay·
  optional reuse των grip visual tokens για ομοιομορφία). Drag → pixel delta → mm factor → helper → `setLayers`.
- **Live feedback**: ο `CompoundPatternPreview` ήδη re-render-άρει σε κάθε αλλαγή model (μηδέν νέο wiring).
- **ADR-040**: το swatch είναι **editor preview**, όχι κύριος καμβάς → **εκτός** ADR-040 (μηδέν cache key /
  hover / `useSyncExternalStore` concern). Καμία αλλαγή στον stroker/render pipeline (τα helpers είναι
  authoring-side· ο render διαβάζει το ήδη-υπάρχον `layers[]`).
- **i18n**: κανένα νέο user-facing string στα ίδια τα grips (είναι direct-manipulation)· tooltips/aria μέσω `t()`.

**6.7.5 Φάσεις εντός Φ6:** **Φ6-A** = 8-handle global box (spread + length scale)· **Φ6-B** = per-layer
offset dots (μετακίνηση μεμονωμένης ράγας)· *(optional Φ6-C = rotation handle / per-symbol scale grip)*.
...
| **Edit-in-place** ✅ | **Επεξεργασία υπάρχοντος user-created τύπου — απλού ΚΑΙ σύνθετου** (Revit/ArchiCAD/Maxon/Figma parity: edit named type → όλες οι γραμμές ενημερώνονται): κουμπί «✎ Επεξεργασία» στο αριστερό Properties tab (μόνο `origin==='user-created'`) → διάλογος pre-loaded (name locked) → `upsertUserLinetype` δέχεται/διατηρεί `complex` (δεν ισοπεδώνει compound) → repaint (registry sub). ISO/built-in = read-only → inline COW editor (per-line copy) | #9 | Μεσαίο — **DONE** |
...
- **2026-07-13 (Φ6-A IMPLEMENTED — γραφικός grip editor, 8-handle bounding box)** — Υλοποίηση του §6.7
  ...
  4 μέσα πλευρών) που χειρίζεται direct-manipulation τις σχέσεις των στρωμάτων — big-player parity
  (MicroStation Line Style Editor / Illustrator bounding box). **ΠΟΥ:** μόνο στο swatch, ΟΧΙ στην καμβά-
  γραμμή (κοινό named style· §6.7.1). **Mapping** (§6.7.2): πάνω/κάτω μέση → `scaleLayerSpread` (offset ×
  factor γύρω από band centre)· αριστ/δεξ μέση → `scalePatternLength` (dash/gap/dot × factor· text/symbol
  ανέγγιχτα)· γωνίες → 2-άξονα (Shift = aspect-lock). **UX** (§6.7.3): snap 0.5mm (Alt = free), min-guard
  0.05mm (καμία κατάρρευση/αναστροφή), reversible → `setLayers`, commit στο **Save** (Cancel = τίποτα).
  **SSoT** (§6.7.4): το scale math είναι pure model helpers, η layout/hit/factor geometry pure module — μηδέν
  clone (jscpd:diff clean). Το overlay mirror-άρει το `BezierCurveEditor` (καθιερωμένο SVG-editor pattern:
  `setPointerCapture`, `role="slider"`). **ADR-040**: εκτός (editor preview, όχι κύριος καμβάς).
  - `config/line-pattern-segments.ts` — νέοι pure helpers: `LINE_PATTERN_MIN_MM`, `bandCenterOffset`,
    `bandHalfExtentMm`, `patternTotalLengthMm`, `scaleLayerSpread`, `scalePatternLength`.
  - `ui/panels/dimensions/line-pattern-grip-geometry.ts` — **NEW** pure overlay math (`HANDLE_IDS`,
    `layoutHandles`, `hitTestHandle`, `outwardDeltaPx`, `computeDragFactor`).
  - `ui/panels/dimensions/LinePatternGripOverlay.tsx` — **NEW** SVG 8-handle overlay (pointer-capture +
    keyboard a11y· Alt=free, Shift=aspect-lock). Χρώμα λαβών = **grip-identity SSoT** (`config/color-config`):
    cold azure `#007FFF` → warm magenta (hover) → hot red (drag) + μαύρο contour → ξεχωρίζουν σε κάθε θέμα
    (Giorgio 2026-07-13: οι default theme-token λαβές ήταν σκούρες/δυσδιάκριτες).
...
- **2026-07-13 (Duplicate & edit + ribbon parity IMPLEMENTED — big-player follow-up)** — Ο Giorgio ζήτησε
  να ολοκληρωθούν τα δύο fast-follow «όπως οι μεγάλοι». **(1) Duplicate & edit** για read-only τύπους
  (ISO/built-in/imported): στο Properties tab, όταν ο τύπος **δεν** είναι user-created, κουμπί «⧉ Διπλότυπο»
  → ο διάλογος ανοίγει σε νέο `duplicateFrom` mode (seed layers από τον source· **editable** όνομα
  pre-filled με `suggestCopyName` = μικρότερο ελεύθερο numeric suffix, `Dashed`→`Dashed 2`) → `save()`
  κάνει `registerUserLinetype` νέου τύπου → `onCreated` το **assign**-άρει στη γραμμή (undoable). Big-player:
  Revit/ArchiCAD «Duplicate» — ποτέ σιωπηλό fork κοινού τύπου. **(2) Ribbon parity**: νέο
  `LineEditLinePatternWidget` δίπλα στο «Νέος τύπος», contextual στον ΤΡΕΧΟΝΤΑ τύπο (διαβάζει το live
  `linetype` value μέσω `useRibbonComboboxState` — ίδιο slice με το «Τύπος» combobox): user-created →
  «✎ Επεξεργασία» (edit-in-place), ISO/built-in → «⧉ Διπλότυπο» (assign μέσω του ΙΔΙΟΥ
  `onComboboxChange(linetype,…)`), solid/ByLayer → renders nothing.
  - `config/line-pattern-segments.ts` — νέος SSoT helper `suggestCopyName(base, existing)` (locale-neutral
    numeric suffix· τα linetype names δεν είναι i18n prose).
  - `ui/panels/dimensions/LinePatternEditorDialog.tsx` — νέο `duplicateFrom?` mode (seed όπως edit αλλά
    editable pre-suggested name· `duplicateTitle`)· `save()` register σε create/duplicate, upsert σε edit.
  - `ui/line-advanced-panel/LinePropertiesTab.tsx` — `typeActionButton` SSoT (edit για user-created,
    duplicate για κάθε άλλο resolvable def, none για solid)· `assignLinetype` (patchEntity) για το duplicate.
  - `ui/ribbon/components/LinePatternLauncherButton.tsx` — **γενικεύτηκε** (optional `editName`/`duplicateFrom`
    + icon ανά mode Plus/Pencil/Copy)· ΕΝΑ button+dialog SSoT για create/edit/duplicate (μηδέν clone, N.18).
  - `ui/ribbon/components/LineEditLinePatternWidget.tsx` (ΝΕΟ) + registration στο `RibbonPanel.tsx` +
    button στο `contextual-line-tool-tab.ts` (`editLineType` key) + bridge early-return (mirror `newLineType`).
  - i18n: `linePatternEditor.{duplicateTitle,duplicate}` + `inlineTab.editHint` (panels)· `ribbon.commands.
    {lineEditLineType,lineDuplicateLineType}` (shell) — el+en, N.11 keys-first.
  - Tests (jest, N.17): `suggestCopyName` (config)· dialog duplicate-mode (ISO→νέος user copy + assign)·
    `LineEditLinePatternWidget` (edit/duplicate/none ανά origin + re-dispatch)· ribbon-tab widget presence.
    **116 σχετικά πράσινα.** jscpd καθαρό (N.18).

- **2026-07-13 (Φ6 PROPOSED — Graphical grip editor· spec-only, ΜΗΔΕΝ κώδικας)** — Ο Giorgio ρώτησε αν,
  επειδή οι σύνθετοι τύποι είναι **δισδιάστατοι**, μπορούμε να βάλουμε λαβές (4 γωνίες + μέση πλευρών) ώστε
  ο χρήστης να αλλάζει τις σχέσεις των στρωμάτων από τις λαβές. Απάντηση: **ναι**, big-player pattern
  (MicroStation Line Style Editor / Illustrator bounding box). Προδιαγράφηκε ως **§6.7** + roadmap row Φ6:
  8-handle bounding box στο **editor preview swatch** (ΟΧΙ στην καμβά-γραμμή — κοινό named style, θα μετέτρεπε
  σιωπηλά όλες τις γραμμές) → πάνω/κάτω = spread `offsetMm` γύρω από band centre (reuse `centerOffsetForLayer`),
  αριστ/δεξ = κλίμακα `lengthMm`, γωνίες = 2-άξονα (Shift=aspect-lock)· per-layer dot = μετακίνηση ενός
  στρώματος (Φ6-B). Direct manipulation του `ComplexLinetypeDef` (§6.1) — **μηδέν νέο data**· pure helpers
  `scaleLayerSpread`/`scalePatternLength` στο `config/line-pattern-segments.ts`· overlay πάνω από το
  `StrokePreviewCanvas` (local AABB hit-test, ΟΧΙ ο βαρύς canvas grip subsystem)· snap 0.5mm, min-guard,
  reversible (commit στο Save). **Εκτός ADR-040** (editor swatch, όχι κύριος καμβάς). **Καμία υλοποίηση —
  αναμονή έγκρισης Giorgio (N.8).**

- **2026-07-13 (Edit-in-place IMPLEMENTED — επεξεργασία υπάρχοντος user-created τύπου)** — Η **τελευταία**
  φάση του ADR-642. Πρόσωπο «όπως οι μεγάλοι» (Revit «Line Patterns» / AutoCAD LTYPE / Figma styles): ο
  χρήστης επιλέγει μια γραμμή με user-created τύπο → «✎ Επεξεργασία» → διάλογος pre-loaded με τα layers του →
  Save in place → **όλες** οι γραμμές που τον χρησιμοποιούν ενημερώνονται (same-name upsert). Λύνει το
  grep-verified core gap: το `upsertUserLinetype` **ΔΕΝ** δεχόταν `complex` → μια in-place επεξεργασία compound
  θα τον **ισοπέδωνε** (χάνονταν ράγες/ties/σύμβολα/κείμενο).
  - `stores/LinetypeRegistry.ts` — **core fix**: `upsertUserLinetype(name, pattern, description, complex?)`
    (mirror `registerUserLinetype`): `...(complex ? { complex } : {})` στο replace-def **και** πέρασμα του
    `complex` στο early-return `registerUserLinetype(...)` (create-via-upsert path). Simple τύποι → `undefined`
    (byte-identical με πριν, μηδέν regression). Persistence (`persistUserLinetypes`) & hydrate ήδη κρατούσαν `complex`.
  - `ui/panels/dimensions/LinePatternEditorDialog.tsx` — νέο prop `editName?`: seed effect (`resolveLinetypeDef`
    → `complexToLayers` για compound/text/symbol, αλλιώς `singleLayer(dashPatternToSegments(pattern))`)· **name
    locked** (read-only Input)· εξαίρεση του `editName` από τα taken-names (όχι false `name.taken`)· title
    `editTitle`· `save()` → `upsertUserLinetype(editName, …, complex)` σε edit mode, αλλιώς `registerUserLinetype`.
  - `ui/line-advanced-panel/LinePropertiesTab.tsx` — κουμπί «✎ Επεξεργασία» για **κάθε** user-created τύπο
    (απλό **ή** σύνθετο): compound → `CompoundPatternPreview` + `compoundHint` + κουμπί· απλός →
    `PatternPreview` (SVG dash) + `editHint` + κουμπί· ISO/built-in → μένει ο inline COW editor. Ενεργό
    **μόνο** `linetypeDef?.origin === 'user-created'` → ανοίγει τον διάλογο με `editName={linetypeDef.name}`.
    Το κουμπί εξάγεται ως ΕΝΑ `EditLinetypeButton` (SSoT, μηδέν clone στα δύο branches). Re-render:
    same-name upsert notify-άρει μέσω `subscribeLinetypeRegistry` (μηδέν νέο wiring).
  - **Design (big-player-faithful, Giorgio 2026-07-13):** ένας **ονομασμένος** user-created τύπος επεξεργάζεται
    σε ΕΝΑ σημείο → ενημερώνονται ΟΛΕΣ οι γραμμές του (Revit «Edit Type» / ArchiCAD attributes / Cinema 4D
    materials / Figma «Edit style»). Το προηγούμενο inline COW editor **σιωπηλά** έκανε fork τον απλό τύπο
    σε per-line copy (`LTP-<entityId>`) — **ΟΧΙ** πρακτική των μεγάλων (κανείς δεν «σπάει» σιωπηλά κοινό τύπο
    από ένα property tweak). Πλέον ο inline editor μένει **μόνο** για ISO/built-in (δεν επεξεργάζονται → το
    fork = «φτιάξε δικό σου αντίγραφο», θεμιτό, Figma-detach-style). Explicit «Διπλότυπο & επεξεργασία» για
    per-line differentiation = fast-follow.
  - i18n: `linePatternEditor.editTitle` + `linePatternEditor.edit` + `linePatternEditor.inlineTab.editHint`
    (el+en, N.11 keys-first).
  - Tests (jest, N.17): `LinetypeRegistry.upsert.test.ts` — upsert create/edit διατηρεί `complex` (symbol layer,
    δεν flatten-άρει) + simple = `undefined`· `LinePatternEditorDialog.editInPlace.test.tsx` — edit-mode
    pre-load→Save→registry κρατά compound/symbol def **ΚΑΙ** απλό pattern (χωρίς complex). 12 σχετικά + 34
    adjacent πράσινα. jscpd καθαρό (N.18).
  - **Επόμενο (fast-follow, εκτός session):** «Διπλότυπο & επεξεργασία» για imported/ISO (φτιάχνει user copy
    pre-loaded) + optional ribbon sibling κουμπί.
