...
> **Status:** 🟢 **Φάση A IMPLEMENTED** (streaming build — freeze fix) · Φάση B → C εκκρεμεί · Q&A ανοιχτό (Giorgio 2026-07-12)
> **Date:** 2026-07-12
...
### Φάση B — Shared glyph atlas text (SSoT· το κρίσιμο για multi-floor scale)
Αντί per-text `CanvasTexture`: rasterize **κάθε ΜΟΝΑΔΙΚΟ glyph μία φορά** από την ADR-557 glyph engine
σε **ΕΝΑ atlas texture**· όλο το DXF text → **instanced quads** (μία BufferGeometry, ένα draw call) που
δειγματίζουν το atlas με per-instance UV + transform + χρώμα.
- 468 text × N ορόφους → **1 atlas + 1 draw call** αντί για χιλιάδες textures.
...
### Φάση C — View-dependent culling + LOD (Revit/Navisworks)
