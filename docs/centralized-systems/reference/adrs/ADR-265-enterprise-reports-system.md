...
> 🔴 **ΥΠΕΡΚΕΡΑΣΤΗΚΕ 2026-07-27 — ADR-710 §10. Η παλέτα αυτής της ενότητας ΔΕΝ είναι πλέον
> ενεργή και ΔΕΝ ήταν ποτέ μετρημένη σε αυτή την εφαρμογή.**
>
> Τα `--report-chart-1..8` είναι πλέον **aliases** των `--chart-1..8` στο `globals.css`.
> Όταν το Okabe-Ito μετρήθηκε επιτέλους στις πραγματικές επιφάνειες του ΝΕΣΤΩΡ, **απέτυχε και
> στα δύο θέματα**: το κίτρινο `#F0E442` έβγαλε **1.11:1** contrast σε ανοιχτό φόντο (πρακτικά
> αόρατο), το slot 8 είναι **καθαρό μαύρο** (χρώμα κειμένου, όχι σειράς, με μηδενικό κορεσμό),
> και στο σκοτεινό θέμα δύο γειτονικές θέσεις βρέθηκαν σε ΔE 11.8 — **κάτω από το κατώφλι για
> πλήρη χρωματική όραση**, όχι μόνο για αχρωματοψία.
>
> **Το Okabe-Ito δεν είναι λάθος — είναι λάθος ΕΔΩ.** Είναι επικυρωμένο σετ για επιστημονικά
> διαγράμματα σε **λευκό χαρτί**: δεν έχει ξεχωριστά βήματα ανά θέμα, δεν εγγυάται contrast σε
> έγχρωμη επιφάνεια UI, και το όγδοο «χρώμα» του είναι μελάνι. Η αντιγραφή του σε CSS tokens
> κράτησε το **όνομα** της εγγύησης και πέταξε τη **συνθήκη** της. Το «color-blind safe»
> παρακάτω ίσχυε ως αναφορά βιβλιογραφίας, ποτέ ως μέτρηση αυτού του κώδικα.
>
> Η ενεργή παλέτα φυλάσσεται πλέον από το **CHECK 3.32** (`npm run validate:chart-palette`).
> Αν χρειαστείς χρώματα σειρών, χρησιμοποίησε `--chart-1..8`· μην προσθέτεις νέες χρήσεις
> `--report-chart-*`.

#### Okabe-Ito Palette (8 Colors — Industry Standard, Nature Methods) — ΙΣΤΟΡΙΚΟ
```
