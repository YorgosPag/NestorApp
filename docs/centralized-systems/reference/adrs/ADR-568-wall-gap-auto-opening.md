...
- **2026-07-07 (flagged item E, Opus 4.8)** — Ο dual-flow pick FSM + οι scene helpers που ήταν byte-identical με το `useWallMergeTool` εξήχθησαν σε **NEW SSoT** `hooks/tools/useWallPickScaffold.ts`· το `useWallGapOpeningTool` κρατά μόνο το `executeBridge` (δέχεται `getSceneManager`/`getScene`/`levelManager`/`setHint` μέσω execute context, ώστε το `buildOpeningResolvers(ctx.levelManager)` finalization να μένει αμετάβλητο). Public API αμετάβλητο. Λεπτομέρειες: **ADR-566 §9 Changelog (2026-07-07)**. ΟΧΙ tsc (N.17). 🔴 browser-verify + commit.
