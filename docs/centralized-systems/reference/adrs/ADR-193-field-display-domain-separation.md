...
### 2026-09-18 — Η διάθεση επιστρέφει στη Γενική καρτέλα, ως ΧΩΡΙΣΤΗ ομάδα (ADR-777 §8.60.18)

**Ο κώδικας είχε ξεπεράσει την απόφαση §6** (N.0.1: ο κώδικας κερδίζει): η «μελλοντική Phase 2» των
Πωλήσεων **δεν ήρθε ποτέ** — καμία οθόνη πωλήσεων δεν γράφει διάθεση ή τιμή χώρου. Αποτέλεσμα, μετρημένο:
θέση ή αποθήκη **δεν μπορούσε να δηλωθεί προς ενοικίαση από την εφαρμογή**, και το μόνο πεδίο τιμής που
γραφόταν ήταν το @deprecated `price` της γρήγορης επεξεργασίας (που ο επιλυτής διαβάζει πάντα ως πώληση).

**Η νέα απόφαση κρατά την αρχή του §3.1 και αλλάζει μόνο τη μορφή της**: η διάθεση **δεν** ανακατεύεται με
τα φυσικά πεδία — ζει σε **δική της κάρτα** «Διάθεση & τιμή» (`SpaceCommercialCard`), όπως οι ομάδες
παραμέτρων της παλέτας Properties του Revit. Ίδιο πρωτότυπο με το ακίνητο (που το §3 ορίζει ως πρότυπο
των χώρων): ίδιος επιλογέας διάθεσης, ίδια πεδία τιμής **με μονάδα**, ίδιος έλεγχος εύλογου. Νέα θέση ή
αποθήκη γεννιέται **εκτός αγοράς** — η κάρτα δεν εμφανίζεται στη δημιουργία.

Κράτηση · πώληση · μίσθωση μένουν **αποκλειστικά** στους διαλόγους πωλήσεων: ο επιλογέας τις δείχνει
κλειδωμένες και ο server τις αρνείται από φόρμα (409). Λεπτομέρειες: ADR-777 §8.60.18.
