...
- **2026-07-27 (Opus 5) — ΑΝΑΘΕΩΡΗΣΗ ΦΥΣΗΣ: αυτό είναι μηχανισμός **PARSE-TIME**· το render-time σκέλος καταργήθηκε (ADR-716 Φ5–Φ7)**
  - **Τι ίσχυε:** η ρητή επιλογή του μηχανικού ταξίδευε ως **ετικέτα** μέχρι το render (`useDxfSceneConversion` → `CanvasSection`), όπου η σκηνή **ξανα-ερμηνευόταν** ανά καρέ απόδοσης. Δύο τόποι έκριναν την ίδια ερώτηση: ο parser (τι μονάδα;) και ο renderer (και πάλι — τι μονάδα;).
  - **Τι ισχύει τώρα:** η μονάδα **οδηγεί το parse** και δεν ξανα-ερμηνεύεται στην απόδοση. Η επιλογή γράφεται στο `FileRecord.userDrawingUnits` κατά τη **δημιουργία του FileRecord** και φτάνει ως `unitsOverride` στο `applyCanonicalMmScale` (ADR-462) — σκαλί 0 «explicit» της σκάλας τεκμηρίων ADR-716 §8. Το αποτέλεσμα είναι **canonical mm** από την πρώτη στιγμή.
  - **Το ΕΝΑ σημείο απόφασης μονάδας ΑΠΟΔΟΣΗΣ** είναι πλέον το `resolveRenderUnits(scene, userDrawingUnits)` (`utils/scene-units.ts`): **σκηνή που ΔΗΛΩΝΕΙ μονάδα είναι ήδη canonical ⇒ καμία δεύτερη κλιμάκωση.** Η ετικέτα ADR-368 επιβιώνει **μόνο** για legacy **αδήλωτες** σκηνές (αποθηκευμένες πριν από αυτή την αλλαγή) — εκεί, και μόνο εκεί, εξακολουθεί να είναι η σωστή απάντηση.
  - **Γιατί έχει σημασία:** όσο η μονάδα ήταν render-time ετικέτα, το ×1000 μπορούσε να εφαρμοστεί **δύο φορές** (μία στο parse, μία στην απόδοση) ή **καμία**, ανάλογα με τη σειρά φόρτωσης — κλασικό race. Η μετακίνηση στο parse-time το κάνει δομικά αδύνατο.
  - **Ζωντανή επαλήθευση (2026-07-27, `47_ergasia.dxf`, ΕΓΣΑ87 μέτρα, ρητή επιλογή «Μέτρα (m)»):** σωστή γεωμετρία (586.800 × 487.940 mm), **ύψη κειμένων 150–2.936 mm σε 1.193 κείμενα** — δηλαδή κλιμακωμένα **μία** φορά μαζί με τη γεωμετρία, όχι ×1000 ξανά· title-block και ετικέτες αναγνώσιμες στις σωστές αναλογίες· **μετά από F5 ταυτόσημα νούμερα**. Κλείδωμα σε τεστ: `utils/__tests__/render-units-resolution.test.ts`.
  - ⚠️ **ΜΗΝ** επαναφέρεις render-time κλιμάκωση για canonical-mm σκηνές «για ασφάλεια» — θα είναι διπλή. Αν μια σκηνή δείχνει λάθος μέγεθος, το λάθος έγινε στο **parse**, όχι στην απόδοση.
  - ✅ Google-level: YES — ένας ιδιοκτήτης της απόφασης, μηδέν race, legacy μονοπάτι διατηρημένο ρητά αντί για σιωπηλή συμβατότητα.
