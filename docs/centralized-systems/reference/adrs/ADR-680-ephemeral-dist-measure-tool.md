...
| 3D click bridge | `bim-3d/measure/use-bim3d-dist-measure.ts` | Mirror του `use-bim3d-column-placement`: `usePlacementInteractionEffect({tools:['dist']})` (ADR-618). Κλικ → `raycastWorldPointOrPlane` (ΕΠΙΦΑΝΕΙΑ→**DXF floor**→camera-plane) → OSNAP με τον **ΙΔΙΟ SSoT** `computeSnap3DHover` (ADR-542, viewport-synced tolerance, ίδια engine/glyph με 2D+3D hover) ταϊσμένο με ΤΟ ΔΙΚΟ μας world point ώστε να πιάνει άκρα/μέσα **DXF+BIM** (`marker.view.point`+`elevMm`) → scene-unit `DistPoint` → `addDistPoint`. `subscribeDist` re-render για Enter/Backspace/Esc. |
| Snap glyph owner | `bim-3d/viewport/snap/bim3d-pointer-scheduler.ts` | Ο scheduler **παραχωρεί** το glyph στο `dist` (όπως column/wall): ο BIM-only raycast του θα έχανε τα DXF-floor άκρα → clobber. Έτσι ο dist hook είναι ο ΜΟΝΟΣ writer του `Snap3DOverlayStore` όσο μετράμε (μηδέν race). **Επίσης** ο scheduler **σβήνει** το hover silhouette + DXF glow όσο `dist` ενεργό (Giorgio 2026-07-22): στη μέτρηση καθαρή εικόνα με ΜΟΝΟ διακριτά snap σημεία-γωνίες — όχι κίτρινο περίγραμμα σε όλο το μήκος της ακμής. |
...
| 2026-07-22 | **Καθαρή εικόνα μέτρησης** (Giorgio report): ο `bim3d-pointer-scheduler` σβήνει hover silhouette + DXF glow όσο `dist` ενεργό → μόνο διακριτά snap σημεία-γωνίες, όχι κίτρινο περίγραμμα σε όλη την ακμή BIM. |
| 2026-07-22 | **Fix OSNAP glyphs στο 3D dist** (Giorgio report). Το snap SSoT = `computeSnap3DHover` (ίδιο με 2D+3D hover, viewport-synced tolerance) ταϊσμένο με το floor-plane world point → glyphs ┘/▲/⊕ ΚΑΙ στα άκρα/μέσα DXF (όχι μόνο BIM). Ο `bim3d-pointer-scheduler` παραχωρεί το glyph στο `dist` → εξάλειψη double-writer race που έκρυβε το marker. Καταργήθηκε το `resolvePlacementSnapWithView` (2D-tolerance) από τον dist path. |
