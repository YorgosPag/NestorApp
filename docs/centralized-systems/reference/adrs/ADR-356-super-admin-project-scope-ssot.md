...
- **2026-09-12 (ADR-787 §5.3 ζ, όριο 1)** — 🔴 **Η ΕΡΩΤΗΣΗ ΤΟΥ ΒΟΗΘΟΥ ΗΤΑΝ ΛΑΘΟΣ, ΚΑΙ ΤΟ ΟΝΟΜΑ ΤΗΣ ΚΑΤΑΣΤΑΣΗΣ ΤΟ ΕΚΡΥΒΕ.** Ο βοηθός ρωτούσε `ctx.superAdminOverride`, που είναι αληθές **μόνο** όταν ο δηλωμένος χώρος **διαφέρει** από το claim — δηλαδή απαντά *«άλλαξε εταιρεία;»*. Η ερώτηση που χρειάζεται είναι ***«ονόμασε κάποιος εταιρεία;»***: ένας super-admin μέσα στο **δικό του** `/o/<εταιρεία>` έδινε `overridden: false` ⇒ `filterCompanyId: null` ⇒ **ΟΛΗ η συλλογή**, κάτω από διεύθυνση που ονομάζει **μία**. Ζωντανό σύμπτωμα που το γέννησε: `/o/me/projects` ⇒ «Έργα (7)» (ADR-787 §9, γραμμή 2026-09-11 (γ)). **Θεραπεία**: ο βοηθός ρωτά πλέον `namesOneCompany(ctx)` = `superAdminOverride === true || ctx.requestedWorkspace?.kind === 'org'` — η δήλωση του πελάτη ταξιδεύει στο `AuthContext` από το σύνορο (`buildRequestContext`), **κριμένη** από τον `decideMembership`. Η παλιά συνθήκη **μένει μέσα στη διάζευξη** (αιτήματα με την αποσυρόμενη κεφαλίδα από ανοιχτή καρτέλα, και contexts που δεν γεννιούνται από HTTP). **Μετονομασία**: `mode: 'super-admin-impersonate'` → **`'super-admin-scoped'`** — η κατάσταση δεν είναι «υποδύεται άλλον», είναι «**το αίτημα ονόμασε εταιρεία**», και το παλιό όνομα έκρυβε ακριβώς το κενό. Καταναλωτής: `bootstrap-queries.ts`. **+Δεύτερη γραμμή άμυνας**: `assertQueryableWorkspace(ctx)` — δηλωμένος **ιδιωτικός** χώρος **πετά** αντί να δώσει `filterCompanyId: null` (που εδώ σημαίνει «όλη η συλλογή»). Άγκυρες: `lib/auth/__tests__/super-admin-scope.test.ts` (7 tests, μεταλλάξεις Μ5/Μ7 σκοτωμένες).
