...
| 2026-05-16 | 3.0.1 | **Fix**: `GET /users` excludes synthetic `development-bypass` users (e.g. `dev-admin`) from the unassigned list. They have no Firebase Auth identity and cannot be promoted to `companyMembers`, so listing them caused 404 "User not found in this company" on PATCH role/status/permission-sets. Filter applied in-memory after the `companyId == null` query (Firestore can't combine `!=` with another `where` efficiently). |
| 2026-09-21 | 2.x | **`POST /api/admin/role-management/bootstrap` ΔΙΑΓΡΑΦΗΚΕ (ADR-867 Β9(β) Ε1).** Κανένας καλών στο UI· ρωτούσε τον **καθρέφτη** `users.companyId` αντί για τα claims (που ανοίγουν την πόρτα) και έγραφε **δικό του** σχήμα μέλους — ο πέμπτος γραφέας του «τι είναι μέλος». Αντικαταστάθηκε από `npm run backfill:workspace-membership` (ξηρό = αναφορά απόκλισης, `--apply --by=<uid>`), που γράφει με τον ΕΝΑ γραφέα `grantWorkspaceMembership`. Η αλλαγή ρόλου (`users/[uid]/role`) ήταν ήδη σωστή: θέση πρώτα, claim μετά. |
