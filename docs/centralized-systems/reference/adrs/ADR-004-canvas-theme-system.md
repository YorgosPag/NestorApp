...
| `--canvas-grid-major` / `-minor` | grid line colours | 2D grid (`resolveGridMajorColor/MinorColor` in `useCanvasSettings`; explicit panel setting still wins) |
...
- **2026-06-30** — NEW **Cinema 4D canvas theme** (exact copy of the C4D viewport scheme:
  ...
  3D exact gradient via ADR-446 §2.1 explicit stops. Grid theme-aware via
  `resolveGridMajorColor/MinorColor` (explicit user setting still wins). New theme is
  non-destructive (AutoCAD black stays default). Files: design-tokens.json (+regen
  variables.css/tokens.ts), color-config.ts, CanvasLayerStack.tsx, useCanvasSettings.ts,
  BackgroundCategory.tsx (+ el/en i18n), studio-background-texture.ts, envmap-generator.ts.
  9 jest. ⚠️ Live-refresh note: 2D background updates instantly (pure CSS); grid + 3D backdrop
  repaint on the next canvas interaction (pan/zoom) after a theme switch. 🔴 browser-verify + commit.
