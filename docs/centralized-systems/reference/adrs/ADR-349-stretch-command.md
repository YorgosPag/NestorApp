...
**Status:** ✅ FULLY IMPLEMENTED — Phase 1a + 1b.1 + 1b.2 + 1c-A + 1c-B1 + 1c-B2 + 1c-B3 + 1d (2026-05-15).
**Date:** 2026-05-15
...
| Q11 | Audit trail (ADR-195) granularity — per command, per entity, per vertex? | ✅ **1 entry per command** (compact, atomic with Undo). Audit payload: `{ op: 'stretch', displacement: {x,y}, affectedEntityIds: [...], affectedEntityCount: N }`. Pattern: `IDxfTextAuditRecorder` injected dependency (same as `CreateTextCommand`/`DeleteTextCommand`) — `noopAuditRecorder` default until Phase 7 persists DXF entities to Firestore. `EntityAuditService.recordChange()` is server-only (Admin SDK) and cannot be called from client commands. Execute/undo/redo each emit one entry. |
...
| Date       | Version | Change |
|------------|---------|--------|
| 2026-07-06 | 1.4     | **LINE endpoint hover-menu «Μήκος» removed (Giorgio: «εξαφάνισέ την»).** Με το ADR-513 hot-grip + «Δαχτυλίδι Εντολών», το σύρσιμο άκρου γραμμής δίνει πλέον το ΑΚΡΙΒΕΣ μήκος μέσα στο ίδιο το drag (πληκτρολόγηση) → το `gripMenu.lengthen` («Μήκος») hover entry έγινε πλεονάζον. `grip-menu-resolver.ts`: `case 'line'` → `[]` (αφαιρέθηκε ο αχρησιμοποίητος `isLineEndpoint`)· ΤΟ ΑΡΚ κρατά το δικό του `lengthen` (δεν έχει hot-grip/ring). Ίδια Revit-grade λογική «το drag ΕΙΝΑΙ το stretch» με το v1.3. Test `grip-menu-resolver.test.ts`: line endpoint → `[]` (17 GREEN). i18n `gripMenu.lengthen` παραμένει (χρησιμοποιείται από το arc). |
| 2026-06-17 | 1.3     | **Revit-grade cleanup — «Stretch» removed from the hover menu (Giorgio).** Dragging a grip IS already the stretch, so Revit/AutoCAD never surface a redundant «Έλξη» entry on hover. `grip-menu-resolver.ts`: `'stretch'` dropped from the `GripMenuActionId` union + `META` + every `resolveMenuActions` output → genuine multifunctional actions only (`lengthen`/`addVertex`/`removeVertex`/`radius`). Anchors (column/BIM, circle/ellipse/text/point), line midpoints, arc centers and whole-entity MOVE glyphs now resolve to `[]` → no menu pops (the controller's `options.length===0` short-circuit, unchanged). `grip-menu-actions.ts`: dead `case 'stretch'` removed. Dead i18n key `gripMenu.stretch` deleted (el+en); `gripMode.stretch` (Spacebar mode cycle, distinct concept) untouched. New coverage `systems/grip/__tests__/grip-menu-resolver.test.ts` (17 tests) locks "Stretch never surfaces". Continuation of the ADR-397 `movesEntity → []` suppression. |
...
| 2026-05-15 | 0.10    | Q10 confirmed: multi-grip stretch (Shift+click multiple grips, drag → all move) in Phase 1. Q11 confirmed: audit trail 1 entry per STRETCH command via `IDxfTextAuditRecorder` pattern (noopAuditRecorder until Phase 7 Firestore persist; `EntityAuditService` is server-only and cannot be called from client commands). |
...
| 2026-05-15 | 1.7     | **Phase 1d IMPLEMENTED — Audit trail wiring (Q11).** Modified: `core/commands/entity-commands/StretchEntityCommand.ts` — injected `IDxfTextAuditRecorder` as optional third constructor param (default `noopAuditRecorder`); `execute()` emits one audit entry after all mutations (action `'updated'`, payload: `op:'stretch'`, `displacement:{x,y}`, `affectedEntityIds`, `affectedEntityCount`); `undo()` emits `op:'stretch-undo'` with negated displacement; `redo()` emits same payload as execute. Architecture note: `EntityAuditService.recordChange()` (ADR-195) is **server-only** (Admin SDK, `import 'server-only'`) and cannot be called from client commands. The established DXF pattern (`CreateTextCommand`, `DeleteTextCommand`) uses `IDxfTextAuditRecorder` — injected dependency, `noopAuditRecorder` until Phase 7 persists DXF entities to Firestore. Q11 updated to reflect this. ADR status: ✅ FULLY IMPLEMENTED. CHECK 3.17 not triggered (StretchEntityCommand writes to in-memory SceneManager, not tracked Firestore collections). 2 existing call sites backward-compatible (optional param). |
| 2026-05-15 | 1.6     | **Phase 1c-B3 IMPLEMENTED — Legacy fragmented commit replaced with `StretchEntityCommand` path + arc partial-capture math + rectangle → polyline coercion.** Modified: `systems/stretch/stretch-entity-transform.ts` — `applyVertexDisplacement` return type changed from `Partial<SceneEntity>` to discriminated `StretchUpdate = {kind:'noop'} \| {kind:'update', updates} \| {kind:'replace', entity}`; new `stretchArcSingleEndpoint` (bulge-preserving math: derives new center via L = chord, r' = L/(2 sin\|θ/2\|), distance from chord midpoint = √(r'²−(L/2)²), side selected by sign of original chord×center cross product so the bulge stays on the same geometric side regardless of CW/CCW or reflex sweep, signed sweep preserved by atan2 + 2π wrap when sign flips); new `stretchArcMidpoint` (3-point circumcircle through old start, new midpoint, old end via determinant formula — collinear→null no-op); new `stretchRectangle` (1–3 corners captured → coerce to closed `polyline` with 4 vertices preserving the entity id so selection/refs stay valid; 4 corners → rigid x/y translate; 0 → noop). `systems/stretch/stretch-vertex-classifier.ts` — added `arc-mid` to `VertexKind` + `enumerateVertices` (start, mid, end) + `getVertexPosition` (midpoint at (startAngle+endAngle)/2 — derivable even when not captured by the crossing window). `core/commands/entity-commands/StretchEntityCommand.ts` — `execute` / `undo` / `redo` adapt to `StretchUpdate`: snapshots both update and replace entries; for replacements, undo `removeEntity(newId)` + `addEntity(oldEntity)` to reverse the type change atomically (single undo step covers both vertex moves and rectangle coercion); orphan `hasUpdates` helper removed. `hooks/tools/useStretchPreview.ts` — `buildVertexGhost` switches on `result.kind` (noop→null, update→`{...entity, ...updates}`, replace→`result.entity` so the ghost shows the coerced polyline live). **Feature-parity additions (no regressions vs legacy commit):** `stretch-vertex-classifier.ts` adds `circle-quadrant` to `VertexKind` + `enumerateVertices(circle)` returning 4 quadrants (right, top, left, bottom) + `getVertexPosition(circle, 'circle-quadrant', 0..3)`; `stretch-entity-transform.ts` adds `stretchCircle` (any captured quadrant → new radius = distance from center to translated quadrant; multi-capture → average resulting radii so window-stretch over a circle remains stable) and extends `translateEntityByAnchor` to cover `arc` (via `center` translation) + `rectangle` / `rect` (via `x/y`); `grip-to-vertex-refs.ts` adds `refsForCircle` (gripIndex 1–4 → quadrant index 0–3, gripIndex 0 falls through to anchor-translate). `hooks/grips/grip-commit-adapters.ts` — legacy `commitDxfGripDrag` (manual edge mutation per entity type / `MoveVertexCommand` / `moveEntities`) deleted; new `commitDxfGripDragViaStretchCommand` resolves grip via `gripToVertexRefs` SSoT → builds `StretchEntityCommand` with `vertexMoves` (refs found) or `anchorMoves` (movesEntity fallback for circle center / ellipse / text / etc.) → `command.validate()` guard → `deps.execute(command)`; `commitDxfGripDragModeAware` stretch branch routes through the new helper; orphan imports removed (`i18next`, `MoveVertexCommand`, `toolHintOverrideStore`); `createSceneManagerAdapter` retained — needed by the command. `hooks/grips/index.ts` — re-export updated (`commitDxfGripDrag` removed, `commitDxfGripDragViaStretchCommand` + `commitDxfGripDragModeAware` exported). Behavior: every stretch grip drag now flows through one command — undo/redo is a single step regardless of entity type or capture configuration; the same math drives the live preview (`useStretchPreview`) and the commit (`StretchEntityCommand.execute`) so they cannot diverge by construction. Edge cases: arc with both endpoints captured → rigid translate (unchanged); arc with degenerate sweep (≈0 or ≥2π) or chord collapse → no-op (no Update); rectangle with 0 corners → noop. |
