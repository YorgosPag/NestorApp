...
### 5.7 ✅ ΤΡΙΤΗ ΕΡΩΤΗΣΗ (2026-07-28, ADR-724) — το ίδιο αρχέτυπο, ένα επίπεδο πιο κάτω

Το §5.6 έκλεισε λέγοντας ότι μία λέξη («editable») κουβαλούσε **δύο** ερωτήσεις. Ένα εικοσιτετράωρο
αργότερα φάνηκε ότι η λέξη «yield» κουβαλούσε **δύο** ακόμη:

| Ερώτηση | Τι κλέβει ο accelerator | `role="separator"` |
|---|---|---|
| 2. «Θα καταναλώσει τον **εκτυπώσιμο χαρακτήρα**;» | ένα γράμμα (π.χ. «Z» = zoom) | **ΟΧΙ** |
| 3. «Θα καταναλώσει το **πλήκτρο πλοήγησης**;» | ένα βέλος | **ΝΑΙ** |

**Ο πληθυσμός ήταν ήδη γραμμένος εδώ — ως προειδοποίηση, χωρίς στέγη.** Το σχόλιο «ΤΙ ΕΞΑΙΡΕΙΤΑΙ
ΕΠΙΤΗΔΕΣ» του `TYPEAHEAD_ROLES` έλεγε ρητά ότι `slider`/`radio`/`tab`/`grid` «πλοηγούνται με
**βέλη**, ΟΧΙ με εκτυπώσιμο χαρακτήρα» — και τους **εξαιρούσε**, σωστά, γιατί δεν υπήρχε ερώτηση να
τους στεγάσει. Η τρίτη ερώτηση **είναι** αυτή η στέγη· δεν είναι νέα ιδέα, είναι η ολοκλήρωση του
§5.6. (Η εξαίρεση στην ερώτηση 2 **παραμένει** σωστή και προστατεύεται από test.)

**Το μετρημένο ελάττωμα (ADR-724 §5.2.1):** με εστίαση στο διαχωριστικό του χώρου εργασίας, τα βέλη
**δεν** άλλαζαν το πλάτος της παλέτας (670 → 670) και αντ' αυτού **πάναραν το σχέδιο** ~80px ανά
πάτημα. Ο handler του splitter (`react-resizable-panels`) είναι **element-level, φάση bubble**, και
ξεκινά με `if (e.defaultPrevented) return;`· ο accelerator του viewer τρέχει σε **window capture**,
δηλαδή πρώτος, και έκανε `preventDefault()`. **Ένα** σφάλμα, **δύο** συμπτώματα.

**Η προσθήκη**: `ARROW_NAVIGATION_ROLES` + `DIRECTIONAL_KEYS` + `isDirectionalKey` +
`consumesDirectionalKeys` + `focusConsumesDirectionalKeys`. Το `shouldGlobalShortcutYield` δέχεται
πλέον **και** το `key`, γιατί **το πλήκτρο επιλέγει την ερώτηση**: πλοηγικό ⇒ 3, αλλιώς ⇒ 2.

⚠️ **Γιατί ΟΧΙ ένα ενιαίο, πλατύτερο predicate** (ο πειρασμός): θα σκότωνε τους accelerators
**γραμμάτων** με εστίαση σε `slider`/`tab` — δηλαδή ακριβώς η παλινδρόμηση για την οποία
προειδοποιεί το §5.6. Και τα δύο όρια είναι κλειδωμένα με tests: «παραιτείται σε `ArrowLeft` πάνω
στον splitter» **και** «ΔΕΝ παραιτείται σε γράμμα πάνω στον splitter» **και** «ΔΕΝ παραιτείται σε
`ArrowLeft` πάνω στον καμβά» (αλλιώς δεν θα υπήρχε ποτέ pan με πληκτρολόγιο).

📌 **Το `toolbar` έμεινε ρητά έξω**: κατά APG έχει roving tabindex — το focus κάθεται στο *κουμπί*,
ποτέ στο toolbar — και οι **12** `role="toolbar"` του repo είναι `<nav>` χωρίς `tabIndex`. Θα ήταν
εγγραφή που δεν πυροδοτείται ποτέ, δηλαδή ψεύτικη αίσθηση κάλυψης.

**Tests**: `keyboard-scope.test.ts` **40 → 81**.
...
### 7.2 Ζωντανή επαλήθευση του §5.6 — 2026-07-27 (Μ1/Μ2/Μ3 ✅)

| # | Σενάριο | Αποτέλεσμα |
|---|---|---|
| **Μ2** | Focus σε Radix Select trigger (`<button role="combobox">`) → `m` | **ΠΡΙΝ τη διόρθωση**: type-ahead νεκρό (`cm`→`cm`), γραμμή εντολών `"M"`. Αυτή είναι η μέτρηση που **προκάλεσε** το §5.6 |
| **Μ1** | Focus σε native `<select>` → `m` | `defaultPrevented` = **false**, έφτασε **ανέπαφο** στο `<select>`, γραμμή εντολών **κενή** ⇒ ο φύλακας παραιτείται |
| **Μ3** | Dialog «Γεωαναφορά» ανοιχτός, focus σε `<select>` **μέσα** του → `Escape` | Dialog **έκλεισε με ΕΝΑ πάτημα** (ESC ανέπαφο στο window-capture → καταναλώθηκε σε document-bubble/Radix)· focus επέστρεψε στον trigger ⇒ **και** το Ε2 επιβεβαιώθηκε ξανά |

**Μ3 = ο regression guard του «δρόμου (β)»** (προσθήκη `SELECT` στο predicate του escape bus). Πράσινο
⇒ η ερώτηση 1 έμεινε όντως αμετάβλητη· ο `<select>` **δεν** θεωρείται πεδίο κειμένου.

**🔴 ΤΟ ΟΡΓΑΝΟ — δύο παγίδες που ακύρωσαν την πρώτη προσπάθεια. Διάβασέ τις πριν ξαναμετρήσεις:**

1. **Τα CDP synthetic keys ΔΕΝ οδηγούν το native type-ahead του `<select>`** (ζει στο UI layer του
   browser, όχι στο DOM). Άρα «η τιμή του select δεν άλλαξε» **δεν** είναι ένδειξη σφάλματος. Ο
   μετρήσιμος όρος είναι αυτός που ελέγχει ο **δικός μας** κώδικας: `e.defaultPrevented` σε
   window-capture listener εγγεγραμμένο **ΤΕΛΕΥΤΑΙΟ**. `true` ⇒ η εφαρμογή έκλεψε τον χαρακτήρα·
   `false` ⇒ παραιτήθηκε.
2. **ΧΩΡΙΣ αρνητικό control η μέτρηση λέει ψέματα.** Πρώτη προσπάθεια: «καμία γραμμή εντολών» έμοιαζε
   επιτυχία — αλλά το control (ίδιο πλήκτρο, focus σε `<body>`) ήταν **επίσης** σιωπηλό, δηλαδή το
   μονοπάτι ενεργοποίησης δεν ήταν καν προσβάσιμο σε εκείνη την κατάσταση. **Απόδειξε πρώτα ότι το
   control ανοίγει τη γραμμή εντολών με `"M"`**· μόνο μετά μέτρα.

⚠️ **Η καρτέλα πρέπει να είναι ΕΝΕΡΓΗ.** Σε hidden tab (`visibilityState: 'hidden'`) ο viewer κολλάει
στη φόρτωση (RAF throttling), το `element.focus()` δεν πιάνει και το layout δίνει rect `0×0` — τρία
συμπτώματα που διαβάζονται λανθασμένα ως σφάλματα εφαρμογής. Το `navigate` σε **νέα** καρτέλα
(`tabs_create_mcp`) την ενεργοποιεί· το `navigate` σε υπάρχουσα **δεν** αρκεί πάντα.

📌 **Το «Μ3 απέτυχε» της πρώτης εκτέλεσης ήταν στάσιμη κατάσταση, όχι παλινδρόμηση** — *συμπέρασμα,
όχι μέτρηση*: είχε μείνει ανοιχτό layer γραμμής εντολών από προηγούμενο βήμα και κατανάλωσε **νόμιμα**
το πρώτο `Escape` (ένα ESC = μία ενέργεια, κατά προτεραιότητα — ADR-364). Η καθαρή επανεκτέλεση με
recorders οπλισμένους από την αρχή έκλεισε τον dialog με ένα πάτημα.
...
### 2026-07-28 — **τρίτη ερώτηση**: «θα καταναλώσει το ΒΕΛΟΣ;» (§5.7, οδηγός ADR-724)

**Τι έγινε**: `ARROW_NAVIGATION_ROLES` + `DIRECTIONAL_KEYS` + `isDirectionalKey` +
`consumesDirectionalKeys` + `focusConsumesDirectionalKeys`. Το `shouldGlobalShortcutYield` δέχεται
πλέον **και** το `key` — **το πλήκτρο επιλέγει την ερώτηση** (πλοηγικό ⇒ 3, αλλιώς ⇒ 2).
Tests **40 → 81**. Πλήρες σκεπτικό: **§5.7**.

**Ο οδηγός** (ADR-724 §5.2.1): το εστιάσιμο `role="separator"` του νέου χώρου εργασίας έπαιρνε την
εστίαση, **δεν** έκανε τίποτα με αυτήν, και **διέρρεε τα βέλη στον καμβά** (~80px pan/πάτημα). Ο
accelerator (window **capture**) προλάβαινε τον handler του splitter (element **bubble**, που
παραιτείται σε `defaultPrevented`).

**Γιατί δεν ήταν νέος μηχανισμός**: ο πληθυσμός (`slider`/`radio`/`tab`/`grid`) ήταν **ήδη
γραμμένος** στο σχόλιο του `TYPEAHEAD_ROLES` ως «πλοηγούνται με βέλη» — εξαιρεμένος επειδή δεν
υπήρχε ερώτηση να τον στεγάσει. Ίδιο ακριβώς αρχέτυπο με το §5.6: **μία λέξη, δύο ερωτήσεις**· η
λύση δεν ήταν να νικήσει η μία, αλλά να **ονομαστούν και οι δύο**.

### 2026-07-27 — §5.6 έκλεισε **και** επαληθεύτηκε ζωντανά (Μ1/Μ2/Μ3)

**Τι έγινε**: δύο ονομασμένα predicates (`isTextEntryTarget` / `consumesTypedCharacters`) + δύο
`activeElement` readers· ο έλεγχος έγινε **ρόλου** αντί `tagName`· allowlist **3 → 1**· νέος ratchet
για τα **12** ανώνυμα inline αντίγραφα (§6). Πλήρες σκεπτικό: **§5.6** και **§3.1**.

**Γιατί δεν ήταν «απλή ενοποίηση»**: το Μ2 έδειξε ότι ο θύτης **δεν** ήταν το δαχτυλίδι (όπως
υπέθετε η καταγραφή του χρέους) αλλά το `shouldGlobalShortcutYield` → η **γραμμή εντολών**. Το
δαχτυλίδι είχε το ίδιο κενό **καλυμμένο**: η γραμμή εντολών αρπάζει το focus μέσα στη φάση capture,
οπότε ο έλεγχός του πάνω στο `document.activeElement` έβλεπε το `<input>` της γραμμής και παραιτούνταν
**κατά τύχη**. ⇒ Διόρθωση μόνο του ενός θα **αποκάλυπτε** το άλλο· γι' αυτό άλλαξαν μαζί.
📌 Μετρημένο παράδοξο προς μελλοντική χρήση: `event.target` = `button[role=combobox]` ενώ
`document.activeElement` = `input[aria-label="Γραμμή εντολών"]` **στο ίδιο event**.

**Έρευνα κλάδου που άλλαξε τη λύση**: Mousetrap (`stopCallback`) και hotkeys-js (`filter`) κάνουν
σκέτο `INPUT|SELECT|TEXTAREA` — το σχήμα του παλιού δαχτυλιδιού. Το **react-hotkeys-hook** κάνει
tags **+ ρόλους** (`FORM_TAGS_AND_ROLES`) πάνω στο `event.target` ⇒ επικυρώνει την κατεύθυνση, αλλά
του λείπει το `combobox`/`listbox`, δηλαδή **ακριβώς** η περίπτωσή μας. Το VS Code δεν εξετάζει DOM
καθόλου (declarative context keys) — καταγεγραμμένο στο header του `keyboard-scope.ts` ως το τέρμα
της διαδρομής, όχι ως εκκρεμότητα.

**Επαλήθευση**: **1877/1877** jest σε 191 suites· CHECK 3.7 `rc=0`· registry golden **96/96**·
`jscpd:diff` καθαρό· **Μ1/Μ2/Μ3 πράσινα ζωντανά** (§7.2, μαζί με τις δύο παγίδες του οργάνου).
**ΟΧΙ tsc (N.17).**
