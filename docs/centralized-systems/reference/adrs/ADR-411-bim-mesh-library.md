...
|---|---|---|---|
| v1.6 | 2026-07-24 | Claude (Opus 5) | **Το bbox placeholder έγινε «φορτώνει» ghost — βλ. ADR-693 Άξονας Β.** Το `meshToObject3D::buildPlaceholder` **δεν καλεί πια** `getMaterial3D(matId)`: ζωγράφιζε το κουτί φόρτωσης με υλικό καταλόγου, και για το `elem-imported-mesh` (που δεν υπήρχε στο `MATERIAL_DEFS`) κατέληγε στο `DEFAULT_MATERIAL_KEY` → **φωτογραφία σκυροδέματος με σκιές** σε κάθε εισαγόμενο μοντέλο. Πλέον καλεί το `buildLoadingGhostBox` (NEW `bim-3d/materials/loading-placeholder-material.ts`): ημιδιάφανο ουδέτερο σώμα + περίγραμμα ακμών, **καμία υφή, καμία σκιά**, στις μετρημένες διαστάσεις. Το `tagObject` σέβεται τη σημαία `bimLoadingGhost` αντί να ξαναγράφει `castShadow=true`. Ισχύει για **όλες** τις mesh-based οντότητες (εισαγόμενα/έπιπλα/Η-Μ) — ένα call site, καμία εξαίρεση, όπως το `attachMeshWireframe`. Η υπογραφή του `buildPlaceholder` έχασε το `matId` (το `userData.matId` σφραγίζεται ως είχε από το `tagObject`). Tests: +5 στο `mesh-to-object3d.test.ts`, +12 στο νέο `loading-placeholder-material.test.ts`. |
| v1.5 | 2026-06-08 | Claude (Opus 4.8) | **Per-kind φιλτράρισμα picker «3Δ Όψη» (Revit family filtering) + 1ο WC μοντέλο (Plan Mode).** Έκλεισε το v1.4 minor follow-up: το dropdown έδειχνε ΟΛΑ τα sanitary presets ανεξ. kind. **ENTERPRISE/SSOT fix (μηδέν fork, μηδέν renderer change):** ο `useRibbonMepFixtureBridge.getComboboxState(assetId)` επιστρέφει πλέον δυναμικά `[Παραμετρικό, ...sanitaryMeshPresetsForKind(fixture.kind)]` (υπάρχον SSoT helper)· ο `RibbonCombobox` ήδη προτιμά τα bridge-provided options όταν `length>0` (αλλιώς static). Το στατικό `THREE_D_VIEW_OPTIONS` απλοποιήθηκε σε parametric-only (kind-blind fallback· λύνει και το stale-build του handoff αφού το tab module δεν εξαρτάται πια από τον catalog). Labels `t()` non-literal (N.11). Αποτέλεσμα: λεκάνη WC→μόνο WC models· ντουζιέρα→μόνο shower· kind χωρίς mesh→μόνο Παραμετρικό. **+WC μοντέλο** `wc_realistic_01` («Curved Modern WC CC Open» by **attilakozma**, **CC-BY** ✅ license-checked live 2360 faces): download (Sketchfab API token)→μέτρηση bbox (three node: X=805/Y=1052/Z=450mm, origin ~centred X/Z· base anchor=bbox min.y→μηδέν catalog Y correction)→upload `bim-mesh-library/sanitary/wc_realistic_01.glb` (gcloud, 115KB)→catalog entry (kind `wc`, μετρημένες dims)+i18n el/en (`wcRealistic01`)+**Credits αυτόματο**. **+2ο WC** `wc_realistic_02` («Homested VorMax Collection» by **Livspace**, **CC-BY** ✅· measured 410×729×893mm = πρότυπη λεκάνη, καλύτερες dims από wc_01· «Collection»=1 λεκάνη/5 sub-meshes ΟΧΙ πολλαπλά· upload 4.16MB· i18n `wcRealistic02`)· ⚠️ **149.664 faces** (~3× soft budget ~50k· αποδεκτό για μεμονωμένο premium fixture, flagged στον Giorgio). NEW bridge tests (per-kind options isolation: shower↮wc, non-sanitary→parametric-only, labels non-literal) + catalog WC test· 29/29 PASS· tsc 0. ΕΚΤΟΣ ADR-040. 🔴 browser-verify (μόνο WC models σε WC fixture) + commit (Giorgio). Επόμενα: νιπτήρας/μπανιέρα/μπιντές μοντέλα (license-check ΠΡΙΝ) όταν στείλει links ο Giorgio. |
