...
**Status:** ✅ **Φ1 ΥΛΟΠΟΙΗΜΕΝΗ + ΜΕΤΡΗΜΕΝΗ ΣΤΟΝ BROWSER (2026-07-29 στ)** — η **τέταρτη** μέτρηση
(§2.3) την ανέδειξε **και** ως θεραπεία απόδοσης, όχι μόνο parity: με το bitmap cache κλεισμένο
(ADR-726 Φ3), το snap ήταν το **75-88%** του καρέ υπό pan (avg 24,2ms / min 22,1ms)· η Φ1 το
κατέβασε σε **0,5-0,9ms** και το `frame:TOTAL` έπεσε στο επίπεδο του control group (27,2→4,7-7,8ms).
**Φ0.1 (attribution) ΥΛΟΠΟΙΗΜΕΝΗ** · **Φ3 (cost-based backoff) ΥΛΟΠΟΙΗΜΕΝΗ (2026-07-30)** —
η μέτρηση του ADR-732 §7.1 έδειξε `frame:snap-detection` p95 34,9ms / max 205-455ms σε hover
υπό φόρτο (2.909 οντότητες)· ένα σταθερού-χρόνου throttle (32ms) ξανάτρεχε πάνω σε ήδη
κορεσμένο νήμα. **Φ2 (broad phase) ΥΛΟΠΟΙΗΜΕΝΗ (2026-07-30)** — η ίδια μέτρηση την
επανα-δικαιολόγησε: η υποβίβαση της (δ) αφορούσε **pan** (όπου η Φ1 ήδη αναστέλλει)· το hover
ΕΚΤΟΣ χειρονομίας δεν καλύπτεται από τη Φ1 και εκεί το `snap:ENTITIES` = 2.909 ανά αναζήτηση.
Ισοδυναμία 100% αποδεδειγμένη διαφορικά (§5 Φ2 «Τι υλοποιήθηκε»).
✅ **Η production μέτρηση-απόδειξη ΕΓΙΝΕ (2026-07-30 βράδυ)** — `snap:ENTITIES` **2.909 → p50 15 /
max 76**, `snap:BROAD` **1,1%** του snap (καμία μετακόμιση κόστους), καρέ >33ms **15,9% → 4,5%**,
Φ3 backoff **38/612 κλήσεις** έτρεξαν όντως. 🔴 **ΑΛΛΑ** η ίδια μέτρηση έδειξε ότι **4 engines με
ιδιωτικό ευρετήριο = 98% του χρόνου** (16-19ms **ανά κλήση**) και ότι **η Φ2 δεν μπορούσε εκ
κατασκευής να τις αγγίξει** (δεν διαβάζουν `context.entities`) — η αιτία και η θεραπεία στο
**ADR-735**. **Φ4/Φ5 ΠΑΡΑΜΕΝΟΥΝ ΑΝΕΝΕΡΓΕΣ** — το «⛔ ΥΠΟΒΙΒΑΣΜΕΝΟ (δ)» ισχύει πλέον **μόνο** για αυτές.
...
> # ⛔ ΟΡΙΣΤΙΚΟ (2026-07-29 δ): ΤΟ SNAP **ΔΕΝ** ΕΙΝΑΙ Ο ΕΝΟΧΟΣ. ΑΥΤΟ ΤΟ ADR ΥΠΟΒΙΒΑΖΕΤΑΙ.
>
> Τρίτη μέτρηση, **απομονωμένη χειρονομία pan** (`pan-pending` 60 σε 31 καρέ):
>
> | stage | avg | **min** | p95 | total | **%** |
> |---|---|---|---|---|---|
> | `frame:TOTAL` | 48,1 | 0,5 | 114,1 | 1.492,1 | 100% |
> | **`frame:dxf-canvas`** | **48,9** | **32,7** | 113,6 | **1.466,7** | 🔴 **98,3%** |
> | `frame:snap-detection` | **0,53** | 0,3 | 1,3 | 15,8 | **1,06%** |
>
> **Το αποφασιστικό νούμερο είναι το `min = 32,7ms`:** κανένα από τα 30 καρέ pan δεν κόστισε
> λιγότερο. Δεν είναι ουρά κατανομής — είναι **δάπεδο**, σε κάθε καρέ. `frame:INTERVAL` avg
> **81,2ms ⇒ ~12 FPS**.
>
> Το snap είναι **0,53ms** — τρεις τάξεις μεγέθους κάτω από το δάπεδο. Η μεγαλύτερη engine
> (`snap:text`) είναι **0,233ms**. **Τα 50,2ms του §1 ήταν παθολογία** (κρύο JIT μετά από
> `Fast Refresh` 12.176ms + πλήρες zoom-out), όχι η κανονική κατάσταση.
>
> ⇒ **Ο ένοχος του pan/zoom είναι το bitmap cache. Πήγαινε στο ADR-726 §6 Φ3.**
> ⚠️ **(2026-07-29 στ):** αυτό ίσχυε **όσο το bitmap cache κυριαρχούσε**. Μόλις εκείνο έκλεισε
>    (ADR-726 Φ3 «raster με άγκυρα»), η **τέταρτη** μέτρηση (§2.3) έδειξε ότι το snap ήταν το
>    επόμενο δάπεδο (min 22,1ms/καρέ υπό pan) — η Φ1 αναστήθηκε, μετρήθηκε, και έκλεισε.
>    Δίδαγμα: **«δεν είναι ο ένοχος» σημαίνει «δεν είναι ο ΜΕΓΑΛΥΤΕΡΟΣ όρος ΤΩΡΑ»** — οι όροι
>    ενός αθροίσματος δεν εξαφανίζονται όταν φύγει ο πρώτος· απλώς προάγονται.
> ⇒ **Τι επιβιώνει από αυτό το ADR:** το `snap:ENTITIES` (μετρημένο **2910→2996**, σταθερά σε κάθε
>    αναζήτηση) — το διπλότυπο του §3.3 είναι **πραγματικό** και άξιο καθαρισμού κάποια στιγμή
>    (N.0.2), αλλά **δεν έχει μετρήσιμη επίπτωση στην απόδοση**. Και το **Φ0.1 (το όργανο)**, που
>    είναι υλοποιημένο, δοκιμασμένο, και **αυτό ακριβώς που αναίρεσε τη διάγνωση αυτού του ADR**.
> ⇒ **ΜΗΝ υλοποιήσεις τις Φ1-Φ5.** Δεν υπάρχει μέτρηση που να τις δικαιολογεί.
> ⚠️ **(2026-07-30):** και αυτό το «οριστικό» είχε ημερομηνία λήξης — για **τρίτη** φορά σε αυτό
>    το ADR, από **μέτρηση**: το Batch 4 του ADR-732 (§7.1, production) έδειξε ότι ο ένοχος του
>    **hover ΕΚΤΟΣ χειρονομίας** είναι το snap (`frame:snap-detection` p95 34,9ms, max 205-455ms,
>    `snap:ENTITIES` 2.909) — σενάριο που η Φ1 **δεν** καλύπτει (αναστέλλει μόνο ΜΕΣΑ στη
>    χειρονομία πλοήγησης). **Φ2+Φ3 υλοποιήθηκαν 2026-07-30** (βλ. §5 + changelog)· Φ4/Φ5
>    παραμένουν ανενεργές.

> 🔴🔴 **Η ΔΕΥΤΕΡΗ ΜΕΤΡΗΣΗ — ΔΙΑΒΑΣΕ ΤΟ §2.2.**
...
### 2.3 🟢 Η ΤΕΤΑΡΤΗ ΜΕΤΡΗΣΗ (2026-07-29 στ) — το snap αναστήθηκε ως ένοχος, η Φ1 το έκλεισε

Μόλις το bitmap cache έκλεισε (ADR-726 Φ3), η ίδια χειρονομία pan ξαναμετρήθηκε — και το snap,
που η (δ) είχε μετρήσει **1,06%**, έγινε **75-88%**. Καμία από τις δύο μετρήσεις δεν ήταν λάθος:
η (δ) μέτρησε ένα καρέ με δάπεδο 32,7ms από το cache· όταν το δάπεδο έφυγε, ο επόμενος όρος του
αθροίσματος προάχθηκε σε πρώτο. **Τρεις ομάδες, 3 τρεξίματα η καθεμία, όλα με πύλες εγκυρότητας**
(`hasFocus() === true`, `visibilityState === 'visible'`, `pan-pending` 60/60, συνθετική 60-καρέ
middle-drag χειρονομία, `47_ergasia.dxf` 2.910 οντότητες):

| Ομάδα | Κατάσταση | `frame:snap-detection` avg (min) | `frame:TOTAL` avg |
|---|---|---|---|
| **Α** πριν | OSNAP ON, χωρίς Φ1 (1:1361) | **24,2ms (22,1)** — 75-88% του TOTAL | 22,7-36,2ms |
| **Β** control | OSNAP OFF (1:1361) | — (η γραμμή απουσιάζει) | **6,3ms** |
| **Γ** μετά | OSNAP ON + Φ1 (1:2177, δυσμενέστερη) | **0,52-0,87ms** | **4,7-7,8ms** |

Το `frame:TOTAL` με OSNAP ενεργό έπεσε **στο επίπεδο του control group** — δηλαδή το snap έπαψε
να είναι μετρήσιμος όρος της χειρονομίας. `snap:ENTITIES` count ανά χειρονομία: 55 → **1**
(μόνο το πρώτο καρέ, πριν από την πρώτη αλλαγή transform — σωστό: εκείνη τη στιγμή δεν
πλοηγούμασταν ακόμη).

**Συμπεριφορική επαλήθευση στον browser (όχι μόνο jest):** σε ηρεμία το snap τρέχει κανονικά
(6/6 probes)· μέσα σε wheel burst **0** (το κενό του §3.5 έκλεισε και για wheel, όπου δεν υπάρχει
mouseup)· 250ms μετά τη ριπή ξανά 6/6 (αυτόματη λήξη)· αμέσως μετά από mouseup pan 9/9 (το ρητό
`end` δουλεύει — μηδέν trailing αναμονή)· 800ms πλήρους ησυχίας: **κανένα** transform write, άρα
η αναστολή δεν μπορεί να μείνει οπλισμένη από θόρυβο.

**Δύο παγίδες μέτρησης που κόστισαν χρόνο, καταγεγραμμένες για τον επόμενο:**
1. **Το F9 ΔΕΝ είναι ο διακόπτης του snap detection.** Το F9 = `cad-toggle-snap` (grid step)·
   ο master διακόπτης του `snapEnabled` είναι το **OSNAP (F11)** = `cad-toggle-osnap`
   (`SnapContext.tsx`: «snapEnabled is synced with cadToggles.osnap»). Control group με F9
   βγάζει **ψευδώς αμετάβλητα** νούμερα — ο έλεγχος είναι το `snap:ENTITIES` (2910 = on, 0 = off).
2. **`document.elementFromPoint` στο κέντρο του καμβά μπορεί να επιστρέψει overlay SPAN** —
   συνθετικά events προς αυτό δεν φτάνουν ποτέ στους React handlers του canvas και τα probes
   δείχνουν ψευδώς «νεκρό» snap. Dispatch **απευθείας στο canvas element**.
...
#### Τι υλοποιήθηκε (2026-07-30) ✅

**Νέο module `snapping/orchestrator/snap-broad-phase.ts`** (311 γρ., pure): AABB ανά οντότητα από
το SSoT `resolveEntityBounds` (ADR-587 Φ9 — ο ίδιος resolver του marquee, μηδέν δίδυμο bounds
μαθηματικών), μέσα στον ΥΠΑΡΧΟΝΤΑ `ISpatialIndex` SSoT. Wiring: `SnapOrchestrator.initialize`
χτίζει (ο κύκλος ζωής μένει στο idle-deferred + fingerprint-guarded `useGlobalSnapSceneSync` —
κανένας νέος)· `findSnapPoint` φιλτράρει **μία** φορά πριν το `createEngineContext`. Αποφάσεις
που αποκλίνουν ή συγκεκριμενοποιούν το σχέδιο, όλες με τεκμήριο:

1. **GRID με προσαρμοστικό `gridSize = extent / clamp(⌈√N⌉, 8, 128)`** (~1 οντότητα/κελί), ΟΧΙ το
   προφίλ `forSnapping` (πάγιο 50 — tuned για *σημεία* σε γνωστή κλίμακα: σχέδιο σε μέτρα ⇒ 1 κελί
   = γραμμική σάρωση· σε mm ⇒ 36M κελιά), ΟΧΙ QuadTree (το `getChildIndex` κατεβάζει item μόνο με
   **πλήρη** εγκλεισμό ⇒ AABB πάνω σε όρια κόμβων συσσωρεύονται στη ρίζα).
2. **Fail-open παντού** (§Φ2.1 «υπερ-εκτίμηση ασφαλής»): τύπος χωρίς bounds provider, exception,
   μη-πεπερασμένα bounds, οντότητα >64 κελιά (πλαίσια/hatch), αποτυχία index/ερωτήματος ⇒ η
   οντότητα (ή όλη η σκηνή) περνά. Χειρότερη απόδοση, ποτέ άλλο αποτέλεσμα.
3. **Η σειρά είναι ορθότητα**: το φιλτραρισμένο σύνολο παράγεται με `filter` πάνω στον **αρχικό**
   πίνακα (Set ids), όχι με τη σειρά του ερωτήματος — οι engines κόβουν στο `maxCandidates` και ο
   βρόχος έχει sub-pixel early-exit· άλλη σειρά ⇒ άλλος νικητής. Mutation-verified (σειρά
   ερωτήματος ⇒ 3 κόκκινα).
4. **Aperture = max ενεργό `worldRadiusForType` × `SNAP_RADIUS_MULTIPLIERS.EXTENDED` (2×)** — όχι
   σκέτο max ανοχής: `Perpendicular`/`Extension`/`OrthoTrack` πολλαπλασιάζουν 1,5× και `Parallel`
   2× πριν ψάξουν. Mutation-verified σε 3 κλίμακες υπο-εκτίμησης (×0,5 / ×0,15 / ×0,05 ⇒ 4-5
   κόκκινα).
5. **🔴 ΠΕΜΠΤΗ μη-τοπική engine: `Perpendicular`.** Η απαρίθμηση του §Φ2.3 (4 engines) ήταν
   **ελλιπής** — βρέθηκε από **κόκκινο διαφορικό test**, όχι από ανάγνωση: πόδι καθέτου με
   `clampToSegment=false` προσγειώνεται στον κέρσορα από οντότητα 550 μονάδες μακριά. Πρόσβαση
   στο πλήρες σύνολο ΜΟΝΟ μέσω του νέου SSoT accessor `resolveNonLocalEntities()` (ένα σύμβολο,
   greppable, κανόνας «κάθε νέα χρήση απαιτεί γραπτή αιτιολόγηση» — 5 καταναλωτές σήμερα).
6. **Κανένα rebuild στο `updateSettings`**: το index εξαρτάται ΜΟΝΟ από entities· η εξάρτηση από
   `enabledTypes` ζει στο aperture, που υπολογίζεται ανά ερώτημα.
7. **`snap:BROAD` stage** (withPerf, μόνο όταν υπάρχει index): το κόστος του ίδιου του φίλτρου
   χρεώνεται δίπλα στις engines — αλλιώς η Φ2 θα «κέρδιζε» μετακινώντας χρόνο σε αμέτρητη θέση
   (το λάθος της Φ11/ADR-040, §3.6). Το `snap:ENTITIES` οφείλει πλέον να δείχνει ~5-50 αντί 2.909.

**⚠️ Διόρθωση της απογραφής του §3.2 (code = truth, μετρημένο 2026-07-30):** από τους **22
default-enabled** τύπους, ο **ΜΟΝΟΣ** που σαρώνει `context.entities` ανά αναζήτηση είναι ο
`BIM_WALL_FACE`. Οι `INTERSECTION`/`TEXT` είναι index-less αλλά **προϋπολογίζουν στο
`initialize()`** — δεν σαρώνουν ανά κίνηση· το rectangle-fallback του `CENTER` είναι νεκρός
κώδικας (το ιδιωτικό του index περιέχει ήδη κάθε κέντρο). Άρα το **άμεσο** κέρδος της Φ2
εντοπίζεται στο `snap:BIM_WALL_FACE` (σκηνές με τοίχους) και στις 6 σβηστές-by-default σαρωτικές
engines — **σε καθαρά DXF σκηνή χωρίς BIM η Φ2 μπορεί να μην μετρηθεί καθόλου**· τα max 205-455ms
του hover μπορεί να κατοικούν και σε index-έχουσες engines (προφίλ «ανακατασκευής index» του
§2.2, ακόμη αμέτρητο ανά engine). Γι' αυτό η production μέτρηση-απόδειξη είναι **εκκρεμής
προϋπόθεση**, όχι τυπικότητα — και γι' αυτό η Φ3 (budget) υλοποιήθηκε ΜΑΖΙ: καλύπτει και ό,τι
η Φ2 δεν αγγίζει.

**Tests:** 22 unit (module) + 5 διαφορικά ισοδυναμίας (396 θέσεις × 3 διαμορφώσεις, pass-through
vs φίλτρο ⇒ πανομοιότυπα) + 11 behavioral anchors (γράφτηκαν ΠΡΙΝ τη Φ2 πάνω στη ζωντανή
συμπεριφορά, έμειναν πράσινα ΧΩΡΙΣ αλλαγή) — σύνολο snapping+cursor: **46 suites / 394 πράσινα**.
Mutations 5/5 κόκκινες στο σωστό test. `jscpd:diff` καθαρό (9 πηγαία). Engines: **μηδέν αλλαγή
γραμμής** πλην των 5 μη-τοπικών (πηγή πίνακα μόνο).

#### ✅ Η production μέτρηση-απόδειξη ΕΓΙΝΕ (2026-07-30 βράδυ)

**Συνθήκες:** production BUILD_ID `eYjId2a564HxVpkhX49jh` · `47_ergasia.dxf` 2.910 στοιχεία ·
**20,0s ακριβώς** (auto-cut recorder) · 979 καρέ · κλίμακα **σταθερή 1:2352** · OSNAP **ON** ·
tab ορατό+focused όλη τη διάρκεια (`hiddenDuringRun=false`) · DOM canvases **4**.

**Το καρέ:**

| Δείκτης | Baseline (ADR-732 §7.1, Batch 4) | **Τώρα** |
|---|---|---|
| Καρέ **>33ms** | **15,9%** | **4,5%** (44/979) |
| Καρέ >70ms | — | 3,2% (31) |
| p50 / p90 / p95 | — | **16,9** / 23,8 / 28,1 ms |
| p99 / max | — | 114 / 178,8 ms |

**Φ2 broad phase — αποδεδειγμένο ΚΑΙ φθηνό:**

| | Baseline | Τώρα |
|---|---|---|
| `snap:ENTITIES` | **2.909 σταθερά** (γραμμική σάρωση) | p50 **15** · p90 62 · **max 76** |
| `snap:BROAD` (το ίδιο το φίλτρο) | δεν υπήρχε | p50 0,6ms · max 1,7ms · **29ms / 1,1%** όλου του snap |

Το κρίσιμο: πριν ήταν 2.909 **ό,τι κι αν έκανες**. Ο κίνδυνος της Φ2 ήταν να **μετακομίσει** το
κόστος στο φίλτρο (το λάθος της Φ11/ADR-040, §3.6) — **δεν έγινε**: 1,1%.

**Φ3 backoff — δουλεύει επιθετικά:** `frame:snap-detection` **n=612 κλήσεις, p50=0, p90=0** —
μόνο **38 από τις 612** έτρεξαν όντως ανίχνευση· οι υπόλοιπες βγήκαν αμέσως από το throttle/backoff.

#### 🔴 …και η προειδοποίηση του ίδιου αυτού §Φ2 επιβεβαιώθηκε: **η Φ2 δεν άγγιξε το 98%**

Η παράγραφος «⚠️ Διόρθωση της απογραφής» παραπάνω είχε γράψει ότι *«τα max 205-455ms του hover
μπορεί να κατοικούν και σε **index-έχουσες engines**»*. **Ακριβώς αυτό συνέβη.** Από τα 2.556ms
συνολικού `frame:snap-detection`:

| Μηχανή | Άθροισμα | **p50 ΑΝΑ ΕΚΤΕΛΕΣΗ** | n |
|---|---|---|---|
| `snap:endpoint` | **824ms** | **18,8ms** | 38 |
| `snap:midpoint` | **744ms** | **18,0ms** | 37 |
| `snap:bim_corner` | **506ms** | **16,2ms** | 38 |
| `snap:bim_midpoint` | **430ms** | **16,0ms** | 38 |
| *οι άλλες 10 μαζί* | **13ms** | ~0 | — |

**4 μηχανές = 98% του χρόνου, πάνω σε 15-76 οντότητες.** Και οι τέσσερις **χτίζουν ιδιωτικό
`spatialIndex` και δεν διαβάζουν ποτέ το `context.entities`** (grep-verified) — δηλαδή το μονοπάτι
που η Φ2 φιλτράρει. ⇒ **Η Φ2 δεν μπορούσε, εκ κατασκευής, να τις αγγίξει.**

Αυτό **δεν ακυρώνει** τη Φ2: παραμένει σωστή για τις engines που σαρώνουν τη σκηνή, το φίλτρο
κοστίζει 1,1%, και ο κίνδυνος μετακόμισης κόστους αποφεύχθηκε. Απλώς οι engines που ωφελείται
κοστίζουν συνολικά **13ms** — ενώ ο χρόνος ήταν αλλού.

> **Το μάθημα, ρητά:** το `snap:ENTITIES` είναι **ΠΛΗΘΟΣ, όχι χρόνος** — αυτό το ADR το γράφει
> ήδη, και η ίδια η παγίδα το ξαναπρόλαβε. Μια θεραπεία που στοχεύει μονοπάτι Α αποδεικνύεται από
> μέτρηση **του χρόνου**, ποτέ από μέτρηση του μεγέθους του Α.

**Η αιτία των 16-19ms δεν είναι σάρωση, είναι η πλευρά κελιού των ιδιωτικών ευρετηρίων:
→ ADR-735.** (Ειρωνεία που αξίζει να καταγραφεί: η απόφαση #1 παραπάνω — «GRID με προσαρμοστικό
`gridSize`, ΟΧΙ το προφίλ `forSnapping` (πάγιο 50)» — είχε **ήδη διαγνώσει το πρόβλημα σωστά**,
αλλά εφαρμόστηκε μόνο στο δέκατο ευρετήριο και ποτέ πίσω στα εννέα.)
...
### 7.3 Ο αριθμός «1-5ms» πρέπει να φύγει από τη κεφαλίδα του `snap-scheduler.ts` ✅ ΕΓΙΝΕ (στ)

Είναι μετρημένα **50,2ms**. Ένας αριθμός σε σχόλιο χωρίς ημερομηνία και χωρίς όργανο είναι
παγίδα για τον επόμενο (§2.1). **Διορθώθηκε με τη Φ1**: η κεφαλίδα φέρει πλέον τη μετρημένη τιμή
**με ημερομηνία και όργανο** (avg 24,2ms / min 22,1ms, `__dxfPerf`, 2026-07-29) — και τα τρία
που απαιτεί αυτός ο κανόνας.
...
| Ημ/νία | Αλλαγή |
|---|---|
| 2026-07-30 (γ) | ✅ **Η ΕΚΚΡΕΜΗΣ PRODUCTION ΜΕΤΡΗΣΗ-ΑΠΟΔΕΙΞΗ ΕΓΙΝΕ** (νέο §5 Φ2 «Η production μέτρηση-απόδειξη»). Production BUILD_ID `eYjId2a564HxVpkhX49jh`, 20,0s auto-cut, 979 καρέ, 1:2352 σταθερή, OSNAP ON, tab ορατό+focused, DOM canvases 4. **Φ2 αποδεδειγμένη ΚΑΙ φθηνή:** `snap:ENTITIES` 2.909 σταθερά → **p50 15 / p90 62 / max 76**· `snap:BROAD` p50 0,6ms / max 1,7ms = **29ms, 1,1% όλου του snap** ⇒ ο κίνδυνος «μετακόμισης κόστους στο φίλτρο» (το λάθος της Φ11/ADR-040, §3.6) **δεν υλοποιήθηκε**. **Φ3 backoff επιθετικό:** `frame:snap-detection` n=612, p50=0, p90=0 — μόνο **38/612** έτρεξαν ανίχνευση. **Καρέ:** >33ms **15,9% → 4,5%** (44/979)· p50 16,9 / p90 23,8 / p95 28,1ms· p99 114 / max 178,8ms. 🔴 **ΚΑΙ Η ΠΡΟΕΙΔΟΠΟΙΗΣΗ ΤΟΥ ΙΔΙΟΥ ΤΟΥ §Φ2 ΕΠΙΒΕΒΑΙΩΘΗΚΕ**: η παράγραφος «Διόρθωση της απογραφής» είχε γράψει ότι *«τα max 205-455ms μπορεί να κατοικούν και σε **index-έχουσες** engines»* — ακριβώς αυτό. Από 2.556ms συνολικού `frame:snap-detection`, **4 μηχανές = 98%**: `snap:endpoint` 824ms (**p50 18,8ms ανά εκτέλεση**), `snap:midpoint` 744ms (18,0), `snap:bim_corner` 506ms (16,2), `snap:bim_midpoint` 430ms (16,0)· **οι άλλες 10 μαζί = 13ms**. Και οι τέσσερις **χτίζουν ιδιωτικό `spatialIndex` και δεν διαβάζουν ΠΟΤΕ το `context.entities`** (grep-verified) ⇒ **η Φ2 δεν μπορούσε, εκ κατασκευής, να τις αγγίξει**· το `snap:ENTITIES` 2.909→15 είναι αληθινό αλλά μετρά μονοπάτι που δεν χρησιμοποιούν. **Το μάθημα:** το `snap:ENTITIES` είναι ΠΛΗΘΟΣ, όχι χρόνος — μια θεραπεία μονοπατιού Α αποδεικνύεται από μέτρηση **του χρόνου**, όχι του μεγέθους του Α. Η αιτία των 16-19ms (πάγιο `cellSize` 50 έναντι aperture που μεγαλώνει με το zoom-out ⇒ Ο(zoom²)) και η θεραπεία → **ADR-735**. **Μηδέν αλλαγή κώδικα σε αυτό το ADR — μόνο μέτρηση + τεκμηρίωση.** |
| 2026-07-30 (β) | ✅ **Φ2 — broad phase στον orchestrator ΥΛΟΠΟΙΗΘΗΚΕ** (νέο §5 Φ2 «Τι υλοποιήθηκε»). Δικαιολόγηση: η μέτρηση Batch 4 (ADR-732 §7.1, production) έδειξε ότι ο ένοχος του **hover εκτός χειρονομίας** — σενάριο που η Φ1 δεν καλύπτει — είναι το snap (p95 34,9ms, max 205-455ms, `snap:ENTITIES` 2.909)· η υποβίβαση της (δ) αφορούσε pan. Νέο pure module `snap-broad-phase.ts` πάνω στους ΥΠΑΡΧΟΝΤΕΣ SSoT (`resolveEntityBounds` ADR-587 + `ISpatialIndex`/factory): GRID με προσαρμοστικό gridSize (extent/⌈√N⌉, clamp 8-128 — ΟΧΙ το πάγιο 50 του `forSnapping`, ΟΧΙ QuadTree: αιτιολόγηση στο §5), fail-open σε ΚΑΘΕ αστοχία, φιλτράρισμα με διατήρηση **σειράς σκηνής** (η σειρά είναι ορθότητα: `maxCandidates` cut + sub-pixel early-exit), aperture = max ενεργή ανοχή × `SNAP_RADIUS_MULTIPLIERS.EXTENDED`. **Εύρημα από κόκκινο test, όχι ανάγνωση: η `Perpendicular` είναι ΠΕΜΠΤΗ μη-τοπική engine** (`clampToSegment=false` ⇒ πόδι καθέτου από οντότητα 550 units μακριά) — η λίστα του §Φ2.3 ήταν ελλιπής· νέος SSoT accessor `resolveNonLocalEntities()` = η μοναδική greppable πόρτα στο πλήρες σύνολο (5 καταναλωτές). **Διόρθωση απογραφής §3.2 (code=truth):** από τους 22 default τύπους ΜΟΝΟ ο `BIM_WALL_FACE` σαρώνει `context.entities` ανά αναζήτηση (INTERSECTION/TEXT προϋπολογίζουν στο initialize) ⇒ το άμεσο κέρδος ζει σε σκηνές με τοίχους + στις 6 σβηστές-by-default engines· τα max 205-455ms μπορεί να κατοικούν και σε index-έχουσες engines (§2.2 «προφίλ ανακατασκευής», αμέτρητο). Tests: 22 unit + 5 διαφορικά (396 θέσεις ×3, pass-through vs φίλτρο πανομοιότυπα) + 11 anchors άθικτα· mutations 5/5 κόκκινες· snapping+cursor 46 suites/394 πράσινα· jscpd καθαρό. **ΕΚΚΡΕΜΕΙ: production μέτρηση-απόδειξη** (`snap:ENTITIES` 2.909→~5-50, νέα γραμμή `snap:BROAD`, ουρά >33ms του hover). |
| 2026-07-30 | ✅ **Φ3 — cost-based backoff για το snap detection throttle ΥΛΟΠΟΙΗΘΗΚΕ.** Η μέτρηση παραγωγής του ADR-732 §7.1 (hover σε 2.909 οντότητες, ίδιο δείγμα με τη μέτρηση του Μοχλού Δ) έδειξε `frame:snap-detection` **p95 34,9ms, max 205-455ms** — πολύ πάνω από το σταθερό `SNAP_DETECTION` throttle (32ms), που ξανάτρεχε την ίδια ακριβή εργασία στα ίδια ~32ms κάτω από φόρτο, χωρίς να λαμβάνει υπόψη το πραγματικό κόστος της τελευταίας εκτέλεσης. Νέα σταθερά `SNAP_BUDGET_BACKOFF_FACTOR` (2×) στο `dxf-timing.ts`: η επόμενη εκτέλεση περιμένει τουλάχιστον 2× το μετρημένο κόστος της τελευταίας. Άνω φράγμα `SNAP_BUDGET_MAX_GAP_MS` (250ms) ώστε μια ακραία ακριβή εκτέλεση (μετρημένο max 455ms) να μην κρατήσει το OSNAP αισθητά «νεκρό» για τον χρήστη. Αδύναμα καρέ (λίγα ms) παραμένουν αμετάβλητα — το υπάρχον σταθερό throttle κυριαρχεί εκεί. 16 νέα tests (`snap-scheduler.frame-budget.test.ts` + `SnapOrchestrator.behavioral-anchors.test.ts`), πράσινα. `jscpd:diff` καθαρό. |
| 2026-07-29 (ζ) | 🛡️ **Finite-point πύλη στο snap pipeline** — περιστατικό στον browser (Giorgio): «`NaN` is an invalid value for the `left` css style property» στο `SnapIndicatorGlyph`. Κλάση σφάλματος: **καμία πύλη δεν απαιτούσε πεπερασμένο σημείο** από engine → store → glyph/commit· εκφυλισμένη γεωμετρία (π.χ. τομή σχεδόν παράλληλων: ορίζουσα→0 ⇒ NaN/∞) περνούσε ατόφια. Και δεν είναι μόνο glyph: το `ImmediateSnap` είναι το **σημείο commit** (κολώνα/δοκάρι/τοίχος μέσω `resolveEffectivePreviewCursor`) ⇒ NaN εκεί = γεωμετρία τοποθετημένη στο πουθενά, σιωπηλά. Διόρθωση σε **δύο SSoT πύλες** με τον **υπάρχοντα** `isFinitePoint` (ADR-510 Φ5, `config/geometry-constants.ts` — μηδέν νέος helper): (α) `snap-scheduler.runSnapDetection` πριν τη δημοσίευση (μη-πεπερασμένο ⇒ «δεν βρέθηκε» ⇒ clear) — προστατεύει marker ΚΑΙ commit anchor· (β) `isSnapMarkerVisible` (extended-types) — προστατεύει **όλους** τους render καταναλωτές (2D overlay, 3D wrapper, crosshair pickbox) από **όλους** τους writers (και το σύγχρονο grip-drag path που δεν περνά από τον scheduler). Ίδιο σχήμα με το «NaN bbox poisoning» του import (commit `32169bde`). Tests: +2 (1 στο `visible-snap-mode.test.ts`, 1 στο suspension suite με πραγματικό scheduler)· **mutation-verified 2/2** (αφαίρεση κάθε πύλης ⇒ κόκκινο το δικό της test). Επαληθεύτηκε επίσης στον browser ότι ο stale-during-pan δείκτης (Φ1) προβάλλεται σωστά world-locked σε pan/wheel/level-switch — το NaN ΔΕΝ προερχόταν από τη Φ1 (η δημοσίευση σημείων δεν άλλαξε· μόνο το πότε τρέχει). |
| 2026-07-29 (στ) | 🟢 **Η ΤΕΤΑΡΤΗ ΜΕΤΡΗΣΗ: η Φ1 είναι ΚΑΙ θεραπεία απόδοσης — μετρημένη πριν/μετά στον browser με control group** (νέα §2.3). Με το bitmap cache κλεισμένο (ADR-726 Φ3), το snap που η (δ) μέτρησε 1,06% έγινε **75-88% του καρέ υπό pan** (avg 24,2ms / **min 22,1ms**) — οι όροι του αθροίσματος δεν εξαφανίζονται όταν φύγει ο πρώτος, **προάγονται**. Μετά τη Φ1: `frame:snap-detection` **0,52-0,87ms**, `frame:TOTAL` **27,2 → 4,7-7,8ms** = επίπεδο control group (OSNAP OFF: 6,3ms). `snap:ENTITIES`/χειρονομία 55 → **1**. Συμπεριφορική επαλήθευση: αναστολή και σε **wheel burst** (0 runs — το κενό του §3.5 όπου δεν υπάρχει mouseup)· αυτόματη λήξη 250ms μετά· ρητό `end` στο mouseup = **άμεση** επαναφορά (9 runs στο θα-ήταν-παράθυρο)· 800ms ησυχία = μηδέν transform writes (η αναστολή δεν κολλάει από θόρυβο). Η διατύπωση του (ε) «όχι θεραπεία απόδοσης» ήταν σωστή **με τα δεδομένα της στιγμής της** — γράφτηκε πριν από την τέταρτη μέτρηση. Καταγράφηκαν 2 παγίδες μέτρησης (§2.3): **F9 ≠ snap detection** (F9=grid step· master = OSNAP/F11· έλεγχος = `snap:ENTITIES` 2910 vs 0) και **`elementFromPoint` στο κέντρο καμβά → overlay SPAN** (συνθετικά events χάνονται· dispatch στο canvas element). Ανεξάρτητη διασταύρωση από Chrome DevTools trace (Giorgio): `FireAnimationFrame` avg 22,3ms/193 καρέ ταιριάζει με το όργανο· στο CPU profile κυριαρχούν React fiber walks (~9.200 δείγματα `updateFiberRecursively`/`updateVirtualChildrenRecursively`) ενώ Layout+Paint+Commit ≈ **160ms σε όλο το trace** ⇒ ο εκτός-rAF χρόνος (Μοχλός Β, ADR-040 TransformBridge) είναι **React re-renders, όχι ζωγραφική**. **Μηδέν αλλαγή κώδικα — μόνο μέτρηση + τεκμηρίωση.** |
| 2026-07-29 (ε) | ✅ **Φ1 ΥΛΟΠΟΙΗΘΗΚΕ**, παρά την υποβίβαση της (δ) — γιατί η (δ) υποβίβασε τη **Φ2** (broad phase) ως θεραπεία απόδοσης χωρίς μέτρηση, ενώ η **Φ1** δεν ήταν ποτέ θεραπεία απόδοσης· είναι **ορθότητα/parity** (§Φ1: «ο δείκτης OSNAP σβήνει όσο σέρνεις — ταυτόσημο με AutoCAD/Revit», «Ρίσκο: χαμηλό — δεν αλλάζει καμία γεωμετρία, αλλάζει πότε τρέχει»), ανεξάρτητη από το ποιος είναι ο ένοχος του frame budget. Νέο module `systems/navigation/NavigationGestureStore.ts` — module singleton, μηδέν React, ίδιο ιδίωμα με `ImmediateTransformStore`/`HoverStore` (ίδια σύσταση με το §Φ1): συνάγει «είμαστε σε πλοήγηση;» από αλλαγές του transform SSoT (8 διαδρομές πλοήγησης → 1 σημείο εγγραφής, όχι N write-sites), με **ρητό κανάλι που ΜΟΝΟ τερματίζει** (`endNavigationGesture`, καλείται στο `mouse-handler-up.ts` + `useCentralizedMouseHandlers.ts` mouseup/mouseleave, ΜΕΤΑ την τελευταία εγγραφή transform) — ώστε χαμένο `end` να μη μπορεί ποτέ να κολλήσει την κατάσταση (μόνο η ασφαλής προεπιλογή, λήξη μετά το idle παράθυρο· CesiumGS/cesium#11889 vs #2950). Ο μοναδικός καταναλωτής σήμερα, `snap-scheduler.onSnapFrame`, μπαίνει σε bail **χωρίς να καθαρίζει** το `ImmediateSnapStore` (ίδιο σχήμα με τον grip guard) — κρίσιμο γιατί το store είναι σημείο **commit** γεωμετρίας, όχι απλός δείκτης. Reuse του `DXF_TIMING.gesture.WHEEL_IDLE` ως idle window (ADR-516 — ίδια φυσική ερώτηση με το bim-3d wheel-idle, καμία νέα σταθερά). 18 νέα tests (2 suites) πράσινα: `NavigationGestureStore.test.ts` (11) + `snap-scheduler.navigation-suspension.test.ts` (7). **Μηδέν αλλαγή στο snap-detection cost per se** — αυτό παραμένει ό,τι μέτρησε η (δ)· αλλάζει μόνο πότε τρέχει η δουλειά. |
| 2026-07-29 (δ) | ⛔ **ΟΡΙΣΤΙΚΟ: ΤΟ SNAP ΔΕΝ ΕΙΝΑΙ Ο ΕΝΟΧΟΣ. ΤΟ ADR ΥΠΟΒΙΒΑΖΕΤΑΙ.** Τρίτη μέτρηση με **απομονωμένη χειρονομία pan** (`pan-pending` 60 / 31 καρέ): `frame:dxf-canvas` **98,3%** (avg **48,9ms**, **min 32,7ms**, p95 113,6) έναντι `frame:snap-detection` **1,06%** (avg **0,53ms**). `frame:INTERVAL` avg **81,2ms ⇒ ~12 FPS**. **Το αποφασιστικό νούμερο είναι το `min`:** κανένα από τα 30 καρέ pan δεν κόστισε λιγότερο από 32,7ms — **δάπεδο**, όχι ουρά κατανομής. Το snap είναι **τρεις τάξεις μεγέθους** κάτω· η μεγαλύτερη engine (`snap:text`) **0,233ms**. **Τα 50,2ms του §1 ήταν παθολογία** (κρύο JIT μετά από `Fast Refresh` 12.176ms + πλήρες zoom-out), όχι κανονική κατάσταση — η υπόθεση του §2.2 για το επίπεδο μεγέθυνσης επιβεβαιώθηκε έμμεσα. ⇒ **Ο ένοχος του pan/zoom είναι το bitmap cache· η δουλειά μεταφέρεται στο ADR-726 §6 Φ3.** **Επιβιώνουν δύο πράγματα:** (α) το **Φ0.1** — το όργανο που **αναίρεσε τη διάγνωση αυτού του ίδιου του ADR**, υλοποιημένο και δοκιμασμένο· (β) το `snap:ENTITIES` = **2910→2996 σταθερά**, δηλαδή το διπλότυπο του §3.3 είναι **πραγματικό** και άξιο καθαρισμού (N.0.2) αλλά **χωρίς μετρήσιμη επίπτωση**. **Φ1-Φ5: ΔΕΝ ΥΛΟΠΟΙΟΥΝΤΑΙ.** Τρεις διαδοχικές ανατροπές διάγνωσης σε μία μέρα, **και οι τρεις από μέτρηση, καμία από επιχείρημα** — αυτό είναι το προοριζόμενο αποτέλεσμα του κανόνα «μέτρα πριν διορθώσεις». **Μηδέν κώδικας, μηδέν commit.** |
