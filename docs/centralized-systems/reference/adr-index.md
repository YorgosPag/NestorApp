...
**📊 Stats**: 411 ADRs | Last Updated: 2026-06-16
...
| 📄 **Uncategorized** | 201 | [View](#uncategorized) |
...
| **ADR-476** | Unified Slab Reinforcement (οπλισμός ΟΛΩΝ των πλακών: εδαφόπλακα + δάπεδο/οροφή· kind-aware suggester EC2 §9.3.1/§9.8.2· auto re-study· 2Δ polygon-clipped σχάρες + 3Δ κλωβός + Properties panel & ribbon + PDF detail sheet — full SSoT με κολόνα/δοκό/πέδιλο) | 🟡 IN PROGRESS (Slices 0-5 DONE, UNCOMMITTED· 🔴 browser-verify+commit) | 2026-06-18 | Uncategorized | [📄](./adrs/ADR-476-unified-slab-reinforcement.md) |
| **ADR-477** | Tie-Beam Reinforcement Unification (συνδετήρια δοκός θεμελίωσης: live auto re-study + ενοποίηση render με beam pipeline + PDF longitudinal + EC8 §5.4.1.2 σεισμική δύναμη σύνδεσης + store layering fix) | 🟡 PARTIAL (Slice 1 COMMITTED· Slices 2/2b/3 + Task A DONE UNCOMMITTED) | 2026-06-18 | Uncategorized | [📄](./adrs/ADR-477-tie-beam-reinforcement-unification.md) |
| **ADR-478** | Wall Line-Loads (T1 gap analysis — τοιχοποιία πληρώσεως → γραμμικό μόνιμο φορτίο στη φέρουσα δοκό: g_face από DNA layers+density· spatial/explicit-FK resolve· fold σε αξονικό→smear σε UDL→οπλισμός/ύψος· proactive re-study· shear-wall exclusion) | 🟢 DONE (UNCOMMITTED) | 2026-06-18 | Uncategorized | [📄](./adrs/ADR-478-wall-line-loads.md) |
| **ADR-479** | Structural Project Presets (Revit-grade templates — named presets `greek-rc-ec8`/`greek-rc-legacy`/`blank` αρχικοποιούν building `StructuralSettings` από canonical defaults + πραγματική μελέτη STATICS 2025 Θέρμη 288/08· `THERMI_288_08` machine SSoT· `applyStructuralPreset` store action· `StructuralPresetSelector` UI [ui/select, ADR-001] στο FloorManagementDialog· `resolveActivePresetKind` active-state detection· `useStructuralSettingsRecompute` κεντρικός subscriber [settings change→`bim:compute-loads-requested`→οπλισμός/σχέδια]· confirm-before-replace [reuse `BuildingSpaceConfirmDialog`]· cross-check regression test engine↔μελέτη + docs JSON sync guard) | 🟢 Slices 1-2-2b-2c DONE (UNCOMMITTED· 44 jest GREEN· 🔴 tsc+browser-verify+commit· DEFER persisted presets Slice 3) | 2026-06-18 | Uncategorized | [📄](./adrs/ADR-479-structural-project-presets.md) |
| **ADR-480** | Analytical Model SSoT (T2 — DERIVED αναλυτικός φορέας από τον organism graph ADR-459: κόμβοι/μέλη/στηρίξεις/άκαμπτο διάφραγμα/load-cases EN1990· Revit physical↔analytical split· priority-aware node merge· grid-anchored κόμβοι· seismic-combination hook για T4· προκαταρκτικά diagnostics ευστάθειας· proactive single-writer στο useStructuralOrganism· θεμέλιο για FEM T3/σεισμό T4/ελέγχους T5) | 🟢 DONE (UNCOMMITTED· 27 jest GREEN· 🔴 tsc+browser-verify+commit) | 2026-06-18 | Uncategorized | [📄](./adrs/ADR-480-analytical-model-ssot.md) |
| **ADR-481** | Static Linear FEM Solver (T3 — δικός μας 3D space-frame direct-stiffness solver πάνω στο analytical model ADR-480: in-house dense LDLᵀ· στοιχειακό 12×12 [αξονικό+στρέψη+2 κάμψεις] + local→global· assembly + BC + penalty διάφραγμα· consistent UDL load-vector από δοκάρια ανά συνδυασμό· K·u=F → M/V/N άκρων + διαγράμματα + envelope· singular-K detection· `AnalysisResultsStore` + diagnostics· explicit «Ανάλυση» trigger· ζει ΠΑΡΑΛΛΗΛΑ με tributary ADR-467· extensible για σεισμικούς συνδυασμούς T4) | 🟢 DONE (UNCOMMITTED· 13 jest GREEN [γνωστές αναλυτικές λύσεις]· 🔴 tsc+browser-verify+commit) | 2026-06-18 | Uncategorized | [📄](./adrs/ADR-481-static-fem-solver.md) |
| **ADR-482** | Static Analysis UI Surface (T3-UI — κάνει ορατό τον αόρατο FEM solver ADR-481: κουμπί «Ανάλυση» στο ribbon → emit `bim:run-structural-analysis` → dormant hook ξυπνά· `useStructuralAnalysisNotification` toast· read-only M/V/N envelope readout [`AnalysisForcesSection` + `useEntityAnalysisForces`] στα column/beam panels· diagnostics surfacing [κοινός `diagnostics-index` Boy-Scout + `AnalysisDiagnosticsStore` + reader-side union στο `EntityWarningsSection`]· engine read-only μηδέν τροποποίηση· M/V/N πληροφοριακά [ΠΑΡΑΛΛΗΛΑ tributary ADR-467]· canvas diagrams DEFER) | 🟢 DONE (UNCOMMITTED· 7 jest GREEN +122 γειτονικά· 🔴 tsc+browser-verify+commit) | 2026-06-18 | Uncategorized | [📄](./adrs/ADR-482-static-analysis-ui-surface.md) |
| **ADR-483** | Static Analysis Canvas Diagrams (T3-UI / Slice 4+4b — διαγράμματα M/V/N κατά μήκος δοκαριών στον καμβά, Revit→Robot: read-only ADR-040 micro-leaf overlay πάνω στα `DiagramStation[]` του solver ADR-481· pure `member-diagram-geometry` [ULS-first· κυρίαρχος άξονας· `f=xM/L`· +`reliable`] + pure `member-diagram-draw` [smooth καμπύλη· **4b**: ζώνες T/C sign-split + caution dashed + tension labels] + **4b** NEW `member-load-arrows` [UDL βέλη `kN/m` από designLineLoadKnM]· **model-space κλίμακα** zoom-stable· **4b** selector M/V/N [`DiagramComponentSelect` canonical select· Μ κόκκινο/V πράσινο/N μπλε]· **4b+** end values M_i/M_j (interior span max) + σύμβολα στηρίξεων [`support-glyphs` 11px+halo] + caption συνδυασμού + σημεία M=0· **T/C calibration: sagging=ΑΡΝΗΤΙΚΟ** (verified)· βέλη φορτίου από **ανάκτηση q=`|d²M/dx²|`** (όχι scene tributary)· transient `analysis-diagram-view-store` toggle· κολόνες/3Δ DEFER· engine read-only) | 🟢 DONE (UNCOMMITTED· Slice 4+4b+· **browser-verified ✅**· 14 jest GREEN· 🔴 tsc+commit) | 2026-06-18 | Uncategorized | [📄](./adrs/ADR-483-static-analysis-canvas-diagrams.md) |
| **ADR-485** | Reinforcement-Utilization Overlay (T3-UI / Slice 4c — χρωματισμός επάρκειας As,req/As,prov στον καμβά, Robot/SAP2000 stress map: NEW ADR-040 micro-leaf `StructuralUtilizationOverlay` [mirror HeatLoadOverlay] βάφει footprint κάθε φέροντος μέλους [δοκάρι **και** κολόνα] πράσινο ≤0.85/πορτοκαλί 0.85–1.0/κόκκινο >1.0· pure `member-utilization` [As,req=asStrength{Beam,Column} ADR-472 exported· As,prov=ενεργός οπλισμός×barArea· ΟΧΙ νέος M_Rd engine] + pure `utilization-color` [διακριτά bands]· `ShowUtilizationToggle` [default OFF] + `showUtilization` στο κοινό view-store· υπόμνημα· repaint σε codeId· πέδιλα/per-combination DEFER) | 🟢 DONE (UNCOMMITTED· utilization-color jest GREEN· 🔴 tsc+browser-verify+commit) | 2026-06-18 | Uncategorized | [📄](./adrs/ADR-485-utilization-overlay.md) |
| **ADR-484** | Cross-level Foundation Properties + Revit-canonical level assignment (**Slice 1:** cross-level πέδιλο [σε άλλο επίπεδο από τον ενεργό] έδειχνε άδειο panel + κανένα contextual tab — ρίζα: όλοι οι consumers έψαχναν ΜΟΝΟ στο `currentScene.entities`, ενώ τα footings ζουν cross-level [`floorplan_foundations`/foundation level]· NEW pure `resolveSelectedEntityFrom` [active scene → fallback `useFoundationLevelStore` low-freq ADR-040-safe] + hook· reused από BimPropertiesShell/Router/FoundationPropertiesTab/contextual resolver· cross-level-aware dispatcher [active→undoable command, cross-level→`foundation-cross-level-writer` + ΙΔΙΕΣ pure recompute]· **Slice 2 (Giorgio «όπως η Revit»):** foundation placement → ΠΑΝΤΑ foundation level [SSoT routing στο `add-foundation-to-scene` + cross-level writer reuse· σωστά υψόμετρα από kind-default· auth scope στο useSpecialTools· αφαίρεση misleading warning]· κανένα migration [δοκιμαστικά δεδομένα]· **Slice 3:** file-level cross-floor guard `resolveFloorScopedScene` στους 3 all-floors aggregators· **Slice 4:** foundation υψόμετρο από FFL ορόφου Θεμελίωσης [`resolveFoundationTopElevationMm`+`resolveActiveFoundationLevelElevationMm`]· **Slice 5:** render isolation [foundations ΜΟΝΟ στον foundation level· `stripAllFoundations`· +ενεργός μη-foundation όροφος]· **Slice 6:** grid+tie-beam reconcile δρομολογείται στον foundation level [NEW `ReconcileCrossLevelFoundationsCommand` atomic undo· reuse `FoundationCrossLevelWriter`· existing από foundation-level-store· auto-trigger gate cross-level-aware] — η ρίζα του «πεδιλοδοκοί στο Ισόγειο») | 🟢 DONE (UNCOMMITTED· +9 jest Slice6 GREEN +71 foundation regression· 🔴 tsc+browser-verify+commit) | 2026-06-18 | Uncategorized | [📄](./adrs/ADR-484-cross-level-foundation-properties.md) |
| **ADR-486** | Topology-aware Beam Support Condition (DERIVED-from-connectivity SSoT — λύνει τη **διπλή αλήθεια** στήριξης: μετακίνηση/αποκόλληση κολώνας → δοκάρι σε 1 στήριξη = πρόβολος, αλλά ο tributary οπλισμός [ADR-471/472] διάβαζε stored `supportType='simple'` → `wL²/8` αντί `wL²/2`, ενώ το FEM [ADR-481] ήταν σωστό → τα δύο **απέκλιναν**· NEW pure `derive-beam-support` [`resolveBeamSupportCondition`/`buildBeamSupportTypeMap` reuse `beamSupportColumnIds` ADR-467· κανόνας count===1→cantilever, 2+→stored, 0→stored+unstable] + transient `beam-support-condition-store` [render-path transport ADR-040 safe]· **Phase A** optional `supportType` override [pure `buildBeamSectionContext`/`resolveActiveBeam*`/checks/render 2Δ+3Δ/utilization/detail-sheet· graphless=stored fallback]· **Phase B** threading `reinforce-patch`+`AutoReinforceOrganismCommand`+`structural-auto-reinforce-core` [πρόβολος re-study→`materiallyDiffers`→patch→count>0→toast `autoReinforced` αυτόματα σωστό]· στήριξη=ΜΟΝΟ κολώνες· κολώνα-ροπή-προβόλου/FEM-convergence DEFER) | 🟢 DONE (Phase A+B· UNCOMMITTED· 13 jest GREEN· 🔴 tsc+browser-verify+commit) | 2026-06-18 | Uncategorized | [📄](./adrs/ADR-486-topology-aware-beam-support.md) |
| **ADR-487** | 🧭 Το ΟΡΑΜΑ: Δυναμικός Στατικός Οργανισμός (Living Structural Organism — north-star κείμενο για όλους τους πράκτορες: η εφαρμογή=επιβλέπων στατικός, ο χρήστης=αρχιτέκτονας που δεν ξέρει στατικά· σε ΚΑΘΕ κίνηση [add/connect/remove] → recompute τάσεων εδάφους/μεγεθών θεμελίωσης/οπλισμού+τύπου+διατομών/επάρκειας ΟΛΟΥ του οργανισμού + ενημέρωση/καθοδήγηση· συνδεδεμένα μέλη=ΕΝΑΣ ενιαίος οργανισμός, ΟΧΙ αυθαίρετα τμήματα [κολώνα+πέδιλο στέκεται μόνο → δοκάρι ενώνει 2 ζεύγη → νέος οργανισμός → re-analysis]· δυναμική σειρά: κολώνες+πέδιλα→δοκάρια→συνδετήρια→εδαφόπλακα→πλάκα δαπέδου→πλάκα οροφής→τοιχοποιία-φορτία· ΑΝΟΙΧΤΑ ΚΕΝΑ: §6.1 κενό κολώνας-πεδίλου [στατική συνέχεια], §6.2 βάθος θεμελίωσης πρέπει να είναι DERIVED-δυναμικό ΟΧΙ χειροκίνητη δήλωση· mapping σε ADR-459/467/471/472/474/475/476/477/480/481/483/486) | 🧭 ΟΡΑΜΑ / North-Star (ζωντανό κείμενο, ΟΧΙ υλοποίηση) | 2026-06-18 | Uncategorized | [📄](./adrs/ADR-487-living-structural-organism-vision.md) |
| **ADR-488** | Ζωντανός Στατικός Οργανισμός — Proactive FEM re-analysis (ADR-487 §4: ο solver ADR-481 έμενε dormant/explicit → το διάγραμμα M/V/N δεν ακολουθούσε τη μετακίνηση/αποσύνδεση κολώνας [έμενε αμφιέρειστο αντί πρόβολος]· το analytical model ADR-480 & ο tributary οπλισμός ADR-471/472/486 ΗΤΑΝ ήδη proactive — μόνο ο FEM όχι· λύση **engaged latch** Revit «analytical results enabled»: ΕΝΑ SSoT predicate `isAnalysisEngaged = analysisLive(latch κουμπιού «Ανάλυση») ‖ showAnalysisDiagrams ‖ showUtilization` → proactive re-solve σε ΚΑΘΕ κίνηση μόνο όταν ο μηχανικός παρατηρεί στατικά, dormant αλλιώς [διατηρεί απόφαση κόστους ADR-481]· **DERIVED triggers** `bim:structural-organism-updated`+`bim:structural-loads-computed` [όχι raw geometry → μηδέν intra-tick race] + flip→engaged immediate solve· coalesced/microtask ADR-040 safe· **silent** proactive [`silent` flag στο `bim:analysis-solved` → toast μόνο στη ρητή «Ανάλυση»]· ΕΝΑ predicate→ομοιόμορφο gating 3 consumers [diagram+readouts+utilization]· boy-scout shared geometry-events DEFER) | 🟢 DONE (UNCOMMITTED· 5 jest GREEN· 🔴 tsc+browser-verify+commit) | 2026-06-18 | Uncategorized | [📄](./adrs/ADR-488-living-structural-organism-proactive-fem.md) |
| **ADR-489** | Στατική συνέχεια κολώνα→πέδιλο + δυναμικό βάθος θεμελίωσης (υλοποιεί ADR-487 §6· §6.1 concrete bug: κολώνες ισογείου αιωρούνταν 1m πάνω από τα πέδιλα [κολώνα baseZmm=0 floor-relative ↔ πέδιλο topElevationMm=−1000 absolute, κανείς δεν τα συμφιλίωνε] → **derived organism continuity**: η βάση κάθε κολώνας με πέδιλο γίνεται DERIVED=άνω παρειά πεδίλου [ΠΟΤΕ persisted, ΠΟΤΕ αλλάζει baseBinding· κορυφή σταθερή, κολώνα επιμηκύνεται κάτω]· reuse footing-bearing ακμών του graph [μηδέν re-pairing]· NEW pure `derive-column-base-continuity`+transient `column-base-continuity-store` [mirror ADR-486]· `columnToMesh` baseDropMm flat path· βαθύτερο πέδιλο νικά [pad −1000 αντί tie-beam −500]· §6.2 «βάθος θεμελίωσης» χειροκίνητη σταθερά → **Derived+override "Auto"**: NEW shared pure `derived-foundation-depth` engine [depth=max(footingThk+tieRise+cover, slab+cover, frostMin), μη-κυκλικό type-level· seed 1200mm bootstrap]· dialog Auto badge+override toggle· `foundationDepthAuto` building field· απορρίφθηκε attach-base επέκταση [same-level/beam-slab only]· DEFER: live viewer-reconciler [engine→building.foundationDepth από πραγματικά πέδιλα]+analytical baseZmm+attached-prism) | 🟢 DONE (§6.1 πλήρες· §6.2 engine+dialog· UNCOMMITTED· 18 jest GREEN· 🔴 tsc+browser-verify+commit) | 2026-06-18 | Uncategorized | [📄](./adrs/ADR-489-column-footing-continuity-dynamic-foundation-depth.md) |
| **ADR-490** | Structural Warning Overlay (Revit/Robot «unstable member» highlight — οπτική επισήμανση στην κάτοψη κάθε φέροντος μέλους με στατικό σφάλμα/προειδοποίηση, π.χ. δοκάρι στον αέρα=μηχανισμός: **always-on** [όχι toggle — τα σφάλματα δεν κρύβονται· Robot/SAP unstable=always red] κόκκινο halo+badge ⚠ vector για error / διακριτικό amber halo για warning / info μένει μόνο στο panel· **ξεχωριστό overlay ΟΧΙ βαφή μέλους** [fill=σημασιολογικό ADR-445/utilization ADR-485]· καθαρά οπτική προβολή ΥΠΑΡΧΟΝΤΩΝ diagnostics [organism ADR-459 + FEM ADR-481 getAll union]→μηδέν νέα λογική «τι είναι πρόβλημα»· actionable=υπάρχον flow [badge→επιλογή μέλους→EntityWarningsSection αιτία+Διαγραφή/Σύνδεση]· 3 NEW SSoT: `collectEntityHighlights` [worst severity/entity] + `severityStyle` [halo χρώμα] + `resolveMemberFootprintVertices` [footprint κολόνα/δοκάρι· migrate & utilization boy-scout]· ADR-040 micro-leaf pointer-events-none topmost· inline actions/hover-tooltip/3Δ DEFER· ⚠ 489 πιάστηκε από άλλον agent [column-footing continuity]→πήρα 490) | 🟢 DONE (UNCOMMITTED· 5 jest GREEN· 🔴 tsc+browser-verify+commit) | 2026-06-18 | Uncategorized | [📄](./adrs/ADR-490-structural-warning-overlay.md) |
...
| **ADR-476** | Unified Slab Reinforcement (οπλισμός πλακών: εδαφόπλακα/δάπεδο/οροφή — kind-aware suggester + auto re-study + 2Δ/3Δ render + Properties panel & ribbon + PDF detail sheet, full SSoT) | 🟡 IN PROGRESS (Slices 0-5 DONE, UNCOMMITTED) | [View](./adrs/ADR-476-unified-slab-reinforcement.md) |
| **ADR-477** | Tie-Beam Reinforcement Unification (συνδετήρια δοκός — live auto re-study + ενοποίηση render + PDF longitudinal + EC8 §5.4.1.2 σεισμική δύναμη σύνδεσης + store layering fix) | 🟡 PARTIAL (Slice 1 COMMITTED· Slices 2/2b/3 + Task A DONE UNCOMMITTED) | [View](./adrs/ADR-477-tie-beam-reinforcement-unification.md) |
| **ADR-478** | Wall Line-Loads (T1 — τοιχοποιία πληρώσεως → γραμμικό μόνιμο φορτίο στη φέρουσα δοκό· auto από DNA+density· spatial/explicit-FK· fold σε αξονικό→UDL→οπλισμός/ύψος· proactive· shear-wall exclusion) | 🟢 DONE (UNCOMMITTED) | [View](./adrs/ADR-478-wall-line-loads.md) |
| **ADR-479** | Structural Project Presets (Revit-grade templates — `greek-rc-ec8`/`greek-rc-legacy`/`blank` presets αρχικοποιούν building settings από canonical defaults + πραγματική μελέτη Θέρμη 288/08· machine SSoT `THERMI_288_08`· `applyStructuralPreset` + `StructuralPresetSelector` UI + `useStructuralSettingsRecompute` (settings→recompute) + confirm-before-replace· cross-check test engine↔μελέτη) | 🟢 Slices 1-2-2b-2c DONE (UNCOMMITTED· 44 jest· DEFER persisted Slice 3) | [View](./adrs/ADR-479-structural-project-presets.md) |
| **ADR-480** | Analytical Model SSoT (T2 — DERIVED αναλυτικός φορέας από organism graph: κόμβοι/μέλη/στηρίξεις/διάφραγμα/load-cases EN1990· physical↔analytical split· node merge· seismic hook T4· diagnostics ευστάθειας· proactive single-writer· θεμέλιο FEM/σεισμός/έλεγχοι) | 🟢 DONE (UNCOMMITTED· 27 jest GREEN) | [View](./adrs/ADR-480-analytical-model-ssot.md) |
| **ADR-481** | Static Linear FEM Solver (T3 — δικός μας 3D space-frame direct-stiffness solver: in-house LDLᵀ· 12×12 element + local→global· assembly+BC+penalty διάφραγμα· consistent UDL load-vector ανά συνδυασμό· K·u=F → M/V/N+διαγράμματα+envelope· singular detection· AnalysisResultsStore· explicit trigger· ΠΑΡΑΛΛΗΛΑ με tributary· extensible T4) | 🟢 DONE (UNCOMMITTED· 13 jest GREEN) | [View](./adrs/ADR-481-static-fem-solver.md) |
| **ADR-482** | Static Analysis UI Surface (T3-UI — ορατός ο FEM solver: κουμπί «Ανάλυση»→emit→dormant hook· toast· read-only M/V/N readout στα column/beam panels· diagnostics surfacing [κοινός indexer + AnalysisDiagnosticsStore + union στο EntityWarningsSection]· engine read-only· M/V/N πληροφοριακά· canvas diagrams DEFER) | 🟢 DONE (UNCOMMITTED· 7 jest GREEN) | [View](./adrs/ADR-482-static-analysis-ui-surface.md) |
| **ADR-483** | Static Analysis Canvas Diagrams (T3-UI / Slice 4+4b+ — διαγράμματα M/V/N κατά μήκος δοκαριών στον καμβά· read-only ADR-040 micro-leaf overlay· pure geometry+draw· auto-fit zoom-stable· **4b** selector M/V/N + ζώνες T/C + βέλη φορτίου UDL + caution· **4b+** end values M_i/M_j + σύμβολα στηρίξεων + caption συνδυασμού + σημεία M=0· T/C calibration sagging=αρνητικό· βέλη από ανάκτηση q· transient toggle default OFF· κολόνες/3Δ DEFER) | 🟢 DONE (UNCOMMITTED· Slice 4+4b+· browser-verified ✅· 14 jest GREEN) | [View](./adrs/ADR-483-static-analysis-canvas-diagrams.md) |
| **ADR-485** | Reinforcement-Utilization Overlay (T3-UI / Slice 4c — χρωματισμός επάρκειας As,req/As,prov στον καμβά· NEW ADR-040 micro-leaf overlay βάφει footprint δοκαριού+κολόνας πράσινο/πορτοκαλί/κόκκινο· pure member-utilization [reuse asStrength + ενεργός οπλισμός, ΟΧΙ νέος M_Rd] + utilization-color· toggle default OFF· υπόμνημα· πέδιλα DEFER) | 🟢 DONE (UNCOMMITTED· jest GREEN) | [View](./adrs/ADR-485-utilization-overlay.md) |
| **ADR-484** | Cross-level Foundation Properties + Revit-canonical level assignment (S1: cross-level πέδιλο→άδειο panel+κανένα tab· NEW `resolveSelectedEntityFrom` [active→fallback foundation-level store]+hook· 4 consumers· cross-level-aware dispatcher· S2 «όπως η Revit»: foundation placement→ΠΑΝΤΑ foundation level [SSoT routing add-foundation-to-scene+cross-level writer· σωστά υψόμετρα· κανένα migration]) | 🟢 DONE (UNCOMMITTED· 9 jest GREEN) | [View](./adrs/ADR-484-cross-level-foundation-properties.md) |
| **ADR-486** | Topology-aware Beam Support Condition (DERIVED-from-connectivity SSoT — διπλή αλήθεια: κολώνα μετακινείται→δοκάρι σε 1 στήριξη=πρόβολος, αλλά tributary οπλισμός διάβαζε stored `simple`→`wL²/8` αντί `wL²/2`· FEM ήταν σωστό→απέκλιναν· NEW pure `derive-beam-support`+transient store· optional `supportType` override σε section-context/checks/render/utilization/detail [pure]· Phase B threading reinforce-patch+command+core→πρόβολος re-study→patch→toast σωστό· στήριξη=ΜΟΝΟ κολώνες· FEM-convergence DEFER) | 🟢 DONE (Phase A+B· UNCOMMITTED· 13 jest GREEN) | [View](./adrs/ADR-486-topology-aware-beam-support.md) |
| **ADR-487** | 🧭 Το ΟΡΑΜΑ: Δυναμικός Στατικός Οργανισμός (Living Structural Organism — north-star για όλους τους πράκτορες: εφαρμογή=επιβλέπων στατικός, χρήστης=αρχιτέκτονας· σε ΚΑΘΕ κίνηση→recompute εδάφους/θεμελίωσης/οπλισμού/επάρκειας ΟΛΟΥ του οργανισμού + καθοδήγηση· συνδεδεμένα μέλη=ΕΝΑΣ οργανισμός ΟΧΙ τμήματα· δυναμική σειρά κολώνες→δοκάρια→συνδετήρια→εδαφόπλακα→πλάκες→τοιχοποιία· ΑΝΟΙΧΤΑ: κενό κολώνας-πεδίλου, δυναμικό βάθος θεμελίωσης) | 🧭 ΟΡΑΜΑ / North-Star | [View](./adrs/ADR-487-living-structural-organism-vision.md) |
| **ADR-489** | Στατική συνέχεια κολώνα→πέδιλο + δυναμικό βάθος θεμελίωσης (ADR-487 §6· §6.1 κολώνες αιωρούνταν 1m→derived organism continuity [βάση=άνω παρειά πεδίλου, reuse footing-bearing ακμών, κορυφή σταθερή]· §6.2 Derived+override "Auto" [shared pure engine, μη-κυκλικό]) | 🟢 DONE (UNCOMMITTED· 18 jest GREEN) | [View](./adrs/ADR-489-column-footing-continuity-dynamic-foundation-depth.md) |
