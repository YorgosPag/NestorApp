...
| **ADR-479** | Structural Project Presets (Revit-grade templates — `greek-rc-ec8`/`greek-rc-legacy`/`blank` presets αρχικοποιούν building settings από canonical defaults + πραγματική μελέτη Θέρμη 288/08· machine SSoT `THERMI_288_08`· `applyStructuralPreset` + `StructuralPresetSelector` UI· cross-check test engine↔μελέτη) | 🟢 Slices 1-2 DONE (UNCOMMITTED· 39 jest· DEFER persisted Slice 3) | [View](./adrs/ADR-479-structural-project-presets.md) |
