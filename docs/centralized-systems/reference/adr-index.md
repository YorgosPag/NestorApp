...
| **ADR-512** | Εξαγωγή Tekton `.TEK` (αρχιτεκτονικά, ADR-505 επέκταση): decode ότι το σημερινό `.TEK` v9.1 = απλό **XML** (vs παλιό v5.1 brace/token) → νέα 4η μορφή εξαγωγής. **Template-based**: sanitized σκελετός (head/global/building + άδειο floor, μηδέν usid/SID/runtime) + injection records στους markers. **xmatrix DECODED** (τοίχος (0,0)→(5,0): `(x00,x10)=E−S`, `(x01,x11)=n̂·thickness`, `(x20,x21)=παρειά`, μέτρα). **Φάση 1=τοίχοι**: 7 NEW `export/core/tek/*`+adapter (template/record auto-gen· geometry reuse `mmToSceneUnits`· pure xml-writer· `collectTekWalls` straight) + wiring 5 σημεία (`ExportFormat`+='tek'· `runTekExport`· dialog· i18n el/en). 19 jest, 102 export-suite GREEN, tsc clean. | 🟡 Φ1 τοίχοι IMPLEMENTED (UNCOMMITTED) — 🔴 browser-verify Τέκτων (centerline-vs-παρειά + Y-flip calibration) + commit· Φ2 κουφώματα/έπιπλα· Φ3 στατικά DEFER | [View](./adrs/ADR-512-tekton-tek-export.md) |
