...
**📊 Stats**: 350 ADRs | Last Updated: 2026-05-31
...
| 📄 **Uncategorized** | 141 | [View](#uncategorized) |
...
| **ADR-363** | BIM Drawing Mode (Parametric Building Elements) | ✅ 🟢 **FULLY IMPLEMENTED** 2026-05-21 — Phases 0-8 complete. Wall/Opening/Slab/Column/Beam tools, Phase 5.6 Wall Split, Phase 6 BOQ auto-feed, Phase 7.1-7.2 multi-select + bulk edit, Phase 8 schedule export. **+ Phase «Από περίγραμμα» Φ0+Φ1 2026-06-01 (pending commit, 🔴 verify):** «Τοίχος από περίγραμμα» — box-select παρειών (ορθογώνιο/Γ/Τ/Π) → κοινό SSoT `perimeter-from-faces.ts` (classify + slab decomposition) → αλυσίδα WallEntity ανά σκέλος + miter. **+ Φ2 2026-06-01 (pending commit, 🔴 verify):** ColumnKind 7→9 (`U-shape` + `composite`, polygon-backed «όπως ETABS/Revit/Tekla» — ακριβές πολύγωνο = SSoT για στατικές ιδιότητες). U-shape=explicit polygon ή παραμετρικό Π· composite=αυθαίρετο polygon. 3D/BOQ/hatch kind-agnostic μηδέν αλλαγή· +Boy-Scout schema-gap fix (polygon/shear-wall/I-shape έλειπαν από Zod enum). 15/15 + 1002 regression, tsc clean. **+ Φ3 2026-06-01 (pending commit, 🔴 verify):** «Τοιχίο από περίγραμμα» tool — box-select παρειών → **ΕΝΑ ColumnEntity ανά κλειστή περίμετρο** (ΠΟΤΕ αποσύνθεση). NEW `bim/columns/column-from-faces.ts` (reuse `perimeterFacesToRects`+`completeColumnFromClick`)· map shape→ColumnKind (ορθογώνιο→rectangular/shear-wall παραμετρικό· Γ/Τ/σύνθετο→composite· Π→U-shape· polygon-backed exact-polygon). +tool `'column-from-perimeter'` (mirror «Τοίχος από περίγραμμα»: tool-def/ribbon/contextual/4 box-select gates/EventBus/toast/i18n)· +`ushape`/`composite` override path (έλειπε από Φ2). 14/14 + 578 column regression, tsc clean. **+ Φ2b 2026-06-01 (pending commit, 🔴 verify):** επεξεργασία polygon-backed τοιχίων — per-vertex grips (`column-poly-vertex-${n}`, mirror slab vertex SSoT, re-center+position compensation WYSIWYG) + manual παραμετρικό Π grips (leg/base thickness)· mirror polygon-backed (`reflectLocalPolygon` negate-y+reverse vs flipY)· dropdown +U-shape μόνο (industry: channel ναι/composite όχι)· section symbols U/composite (`resolveColumnSectionOutline` SSoT, ColumnRenderer 499→488)· Boy-Scout `computeCentroidWorld` polygon-backed bbox alignment. 3 NEW + ~12 MOD, 345 tests PASS, tsc clean. **+ Φ3b 2026-06-01 (pending commit, 🔴 verify):** auto-union γειτονικών πλαισίων στο «Τοιχίο από περίγραμμα» — 3 ορθογώνια Π → ΕΝΑ τοιχίο U-shape (Eurocode 8 σύνθετη στατική λειτουργία, Giorgio). NEW `unionTouchingPolygons` (reuse `safeUnion`/polygon-clipping) + optional `{unionTouching}` στο κοινό `perimeterFacesToRects` (default off→τοίχοι αμετάβλητοι· on στο column path). 17/17+18/18+41/41, tsc 0. **+ Φ3c 2026-06-02 (pending commit, 🔴 verify):** «Κολώνα από περίγραμμα» (NEW tool `column-discrete-from-perimeter`) — box-select παρειών **ΧΩΡΙΣ ένωση** (κάθε περίγραμμα ξεχωριστό)· **αυτόματη στατικά τίμια ταξινόμηση** ανά αναλογία πλευρών (aspect<4→κολώνα `rectangular`· ≥4 ή μη-ορθογωνικό→τοιχίο `shear-wall`/`composite`/`U-shape`)· ενημερωτικό confirm όταν ≥1 τοιχίο («N τοιχία + M κολώνες», [Δημιουργία]/[Άκυρο]=όλα-ή-τίποτα)· όλα κολώνες→δημιουργία κατευθείαν. NEW `column-perimeter-confirm-store.ts` (mirror `wall-cascade-delete-store`) + `ColumnPerimeterConfirmDialog.tsx`· `column-from-faces.ts` += `classifyPerimeterFacesToColumns`/`perimeterColumnKind` (reuse· `perimeterFacesToColumns` ΑΜΕΤΑΒΛΗΤΟ). 26/26, tsc 0. **+ «Δοκάρι από τοίχο» 2Δ+3Δ 2026-06-02 (pending commit, 🔴 verify):** tool `beam-from-wall` — 1 κλικ σε τοίχο → δοκάρι στον άξονά του (πλάτος=πάχος, top 3000/depth 500) + αυτόματο κόντεμα τοίχου στα 2.5m (ADR-401 D). 2Δ: NEW SSoT `bim/beams/beam-from-wall.ts` (reuse `completeBeamFromTwoClicks`). **3Δ (Opus 4.8):** ΙΔΙΑ συμπεριφορά + WYSIWYG ghost — NEW `use-bim3d-beam-from-wall-pick.ts` (raycast→emit `bim:beam-from-wall-picked-3d {wallId}`) + `BeamFromWallGhost.ts` (`buildBeamFromWall`→`beamToMesh`) + `beam-tool-bridge-store.ts`· `useBeamTool` refactor core `commitForWall` κοινό 2Δ/3Δ. Mirror attach-pick + column-placement, μηδέν geometry/commit dup. pick 7/7 + ghost 8/8 + commit 2 + 127 regression, tsc 0. **Deferred: vertex add/remove (μόνο move)· holes σε union· browser verify Φ1/Φ3/Φ2b/Φ3b/Φ3c· 3Δ δοκάρι-από-τοίχο.** | 2026-05-17 | DXF Viewer — BIM / Parametric Building Modeling | [📄](./adrs/ADR-363-bim-drawing-mode.md) |
...
| **ADR-405** | BIM Discipline Taxonomy & MEP Foundation (Step 1) | ✅ ACCEPTED | 2026-06-02 | Uncategorized | [📄](./adrs/ADR-405-bim-discipline-taxonomy-and-mep-foundation.md) |
...
| **ADR-405** | BIM Discipline Taxonomy & MEP Foundation (Step 1) | ✅ ACCEPTED | [View](./adrs/ADR-405-bim-discipline-taxonomy-and-mep-foundation.md) |
