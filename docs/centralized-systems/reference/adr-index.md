...
| **ADR-455** | Vertical Section Cuts (X/Y «L» slider + βελάκι φοράς· 2 νέες κάθετες τομές κατά DXF X/Y στη βάση + αριστερά του καμβά· ΕΝΑ SSoT `x/yAxisCut{active,position,sign}`· 3Δ = γενίκευση `SectionSceneController` single→≤3 axis cuts [ίδιο clip+caps+fast-path]· 2Δ = cut-away πλευρά **φάντασμα** [reuse Isolate dim-alpha] + γραμμή τομής· flip βελάκι αλλάζει πλευρά θέασης) | 🟢 v1 Implemented — 26 jest + tsc clean — pending browser-verify + commit | [View](./adrs/ADR-455-vertical-section-cuts.md) |
