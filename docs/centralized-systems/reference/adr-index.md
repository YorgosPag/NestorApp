...
| **ADR-501** | DXF Grip Multi-Arm + Group Move (AutoCAD/Revit hot-grips) — κλικ cold λαβή→πορτοκαλί 'armed' (GripArmedStore SSoT + 'armed' temperature)· shift+click multi· click-vs-drag threshold (no drag regression)· Esc/click-away clear· render thread ADR-040-safe (useSyncExternalStore low-freq) | 🟡 Slice 1 (Slice 2 marquee + Slice 3 group-move+numeric DEFER) | [View](./adrs/ADR-501-dxf-grip-multi-arm-group-move.md) |
