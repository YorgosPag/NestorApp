...
| **ADR-550** | Unified Entity Render Contract — ΕΝΑ συμβόλαιο οντότητας, N backends (2D/3D)· Φ-Ghost = type-keyed 3D placement-ghost registry (`PLACEMENT_GHOST_3D_FACTORIES`) + `placementGhost3D` contract field + coverage binding (2D ghost εκτός — όχι introspectable seam) | 🟢 Φ0+Φ2+Φ3+Φ4+Φ-Ghost IMPLEMENTED (UNCOMMITTED) | 2026-06-29 | Canvas & Rendering | [📄](./adrs/ADR-550-unified-entity-render-contract.md) |
...
| **ADR-557** | Text/MText λαβές με ΠΛΗΡΗ rect-box parity (τοίχος/κολόνα SSoT) — ΝΕΟ `bim/text/text-grips.ts` καλεί `rect-grip-engine` (10 grips: 4 γωνίες+4 μέσα+move+rotation)· κοινό emission 2D-interaction/2D-render(`TextRenderer.getGrips`)/3D· minimal `UpdateTextTransformCommand` (top-level)· TEXT `widthFactor` X-scale + MTEXT `width`· 2 audit-fixes (2 grip συστήματα· lower-left bbox άγκυρα). 54 jest, browser-verified 2D+3D | 🟢 IMPLEMENTED (UNCOMMITTED) | 2026-06-30 | Canvas & Rendering | [📄](./adrs/ADR-557-text-mtext-rect-box-grip-parity.md) |
| **ADR-558** | Cinema-4D-style ground grid στην 3D προβολή — shader grid σε bounded XZ quad (fwidth AA, 1 draw-call)· adaptive cascade + minor-fade REUSE του 2D `computeAdaptiveLevels`/`lerpOpacityTowards`· radial distance-fade → ορίζοντας (ΟΧΙ άπειρο, ανά screenshot C4D)· major/minor χρώματα token-live (`resolveCssVarColor`, #414141/#4B4B4B)· world άξονες (X κόκκινο/Z μπλε) + horizon· post-FX `'underlay'` (AO-immune, occluded από κτίριο)· 5 νέα αρχεία + wiring `ThreeJsSceneManager`· 7/7 jest | 🟡 IMPLEMENTED (UNCOMMITTED) | 2026-06-30 | Canvas & Rendering | [📄](./adrs/ADR-558-cinema4d-3d-viewport-grid.md) |
...
| **ADR-558** | Cinema-4D-style 3D ground grid — shader grid (fwidth AA), adaptive cascade REUSE 2D SSoT, radial fade→ορίζοντας, token-live χρώματα, post-FX `'underlay'` | 🟡 IMPLEMENTED (UNCOMMITTED) | [View](./adrs/ADR-558-cinema4d-3d-viewport-grid.md) |
