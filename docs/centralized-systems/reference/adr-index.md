...
| **ADR-378** | Snap System Master Architecture & SSoT | 🟡 ACTIVE 2026-05-27 — Master ADR consolidates 5 parallel snap subsystems. Documents Orchestrator→Registry→26 engines pattern, resurrects phantom ADR-149 priority table, defines 6-phase cleanup roadmap. **Phases 1+2+3+5 DONE 2026-05-27** (P1: AISnappingEngine + useProSnapShortcuts deleted; P2: pro-snap-engine.ts ghost deleted + index.ts re-export removed; P3: TextSnapEngine ADR-344 6.C completion — 177 LOC engine + 226 LOC test (14 cases PASS) + Registry wire + SnapIndicatorOverlay ▣ nested-square + i18n el/en snapModes.labels.text+textPoints + ProSnapToolbar TEXT key + type system commits 7f788d8d/aabfb04f; P5: 12 stale doc/code references updated across 11 files incl. ADR-370/153/359/034/092/362 + 3 DXF docs + storage-utils orphan const + snap-engine-utils stale comment). Supersedes phantom ADR-149 reference. Phases 4 (geo-canvas unification), 6 (SSoT registry) pending. | 2026-05-27 | DXF Viewer — Snapping (Master) | [📄](./adrs/ADR-378-snap-system-master-architecture.md) |
