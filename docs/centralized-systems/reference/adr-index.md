...
| **ADR-376** | Opening Tags (Revit-Faithful Pattern) — instance Mark + per-kind prefix + floor-prefix hundreds | ✅ Phase A + B.1 IMPLEMENTED 2026-05-25 — Phase A: OpeningMarkService + OpeningTagRenderer + Layer SSoT + ribbon Mark field + AnnotationsSection toggle + migration script. Phase B.1 (v6): Renumber Openings command (IMAGINiT-style) + ICommand undo + Dialog modal + Annotate ribbon tab + contextual quick-action + `markIsManual` flag (industry preserve-by-default, 5/5 convergence: IMAGINiT/ArchiCAD/Tekla/Bentley/Vectorworks). 50 tests (38 Phase A + 12 Phase B.1). Phase B.2 (BOQ Schedule signature group-by Mode C) + Phase C (draggable tag) pending. | 2026-05-25 | DXF Viewer — BIM / Annotation | [📄](./adrs/ADR-376-opening-tags.md) |
