...
| **ADR-375** | BIM Entity Line Weight Semantic System (Revit-Equivalent) | ✅ Phase A + B.1 + B.2 + B.3 IMPLEMENTED 2026-05-25 — Phase A: 4 SSoT + 7 renderers + 35 tests. Phase B.1: Drawing Scale Selector. Phase B.2: `bim-render-settings-store` SSoT (Level doc persistence), ViewRangePanel + ObjectStylesPanel. Phase B.3: View Templates library (`dxf_viewer_view_templates` collection, snapshot-copy model, Apply/Update/Detach + client-side fan-out propagation), enterprise ID `vtmpl_`. **92 tests PASS**. Phase C deferred. | 2026-05-25 | Canvas & Rendering / BIM Drawing | [📄](./adrs/ADR-375-bim-entity-line-weight-semantic-system.md) |
...
| **ADR-375** | BIM Entity Line Weight Semantic System | ✅ Phase A + B.1 + B.2 + B.3 IMPLEMENTED | [View](./adrs/ADR-375-bim-entity-line-weight-semantic-system.md) |
