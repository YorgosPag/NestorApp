...
| **ADR-483** | Static Analysis Canvas Diagrams (T3-UI / Slice 4 — διαγράμματα ροπής Μ κατά μήκος των δοκαριών στον καμβά, Revit→Robot moment diagrams: read-only ADR-040 micro-leaf overlay πάνω στα έτοιμα `DiagramStation[]` του solver ADR-481· pure `member-diagram-geometry` [συνδυασμός ULS-first· κυρίαρχος άξονας ροπής· `f=xM/L`· μέτρα→canvas] + pure `member-diagram-draw` [screen-space offset καμπύλη + ribbon fill + pill ακραίας, reuse canvas-pill]· **model-space κλίμακα** [μέγιστη ροπή→35% μέσου μήκους μέλους × transform.scale → κλιμακώνεται με το zoom, Robot-grade· ΟΧΙ σταθερό pixel]· transient `analysis-diagram-view-store` toggle [default OFF]· κολόνες/V/N DEFER· engine read-only) | 🟢 DONE (UNCOMMITTED· 5 jest GREEN· 🔴 tsc+browser-verify+commit) | 2026-06-18 | Uncategorized | [📄](./adrs/ADR-483-static-analysis-canvas-diagrams.md) |
| **ADR-484** | Cross-level Foundation Properties (κοινός SSoT selection resolver: cross-level πέδιλο [σε άλλο επίπεδο από τον ενεργό] έδειχνε άδειο panel + κανένα contextual tab — ρίζα: όλοι οι consumers έψαχναν ΜΟΝΟ στο `currentScene.entities`, ενώ τα footings ζουν cross-level [`floorplan_foundations`/foundation level]· NEW pure `resolveSelectedEntityFrom` [active scene → fallback `useFoundationLevelStore` low-freq ADR-040-safe, μηδέν νέο subscription] + hook· reused από BimPropertiesShell/Router/FoundationPropertiesTab/contextual resolver· cross-level-aware dispatcher [active→undoable command, cross-level→`foundation-cross-level-writer` + ΙΔΙΕΣ pure recompute]· **(B) διασαφήνιση: ΟΧΙ per-kind level — ΕΝΑΣ foundation level/κτίριο· «διαφορετικά επίπεδα»=provenance write-path, χειροκίνητο→active vs cross-level writer→foundation· DEFER redirect**) | 🟢 DONE (UNCOMMITTED· 6 jest GREEN +202 regression· 🔴 tsc+browser-verify+commit) | 2026-06-18 | Uncategorized | [📄](./adrs/ADR-484-cross-level-foundation-properties.md) |
...
| **ADR-483** | Static Analysis Canvas Diagrams (T3-UI / Slice 4 — διαγράμματα ροπής Μ κατά μήκος δοκαριών στον καμβά· read-only ADR-040 micro-leaf overlay πάνω στα `DiagramStation[]` του solver· pure geometry+draw· auto-fit zoom-stable· transient toggle default OFF· κολόνες/V/N DEFER) | 🟢 DONE (UNCOMMITTED· 5 jest GREEN) | [View](./adrs/ADR-483-static-analysis-canvas-diagrams.md) |
| **ADR-484** | Cross-level Foundation Properties (κοινός SSoT selection resolver: cross-level πέδιλο→άδειο panel+κανένα tab· NEW pure `resolveSelectedEntityFrom` [active scene→fallback foundation-level store low-freq]+hook· reused 4 consumers· cross-level-aware dispatcher· (B) ΟΧΙ per-kind level — provenance write-path· DEFER redirect) | 🟢 DONE (UNCOMMITTED· 6 jest GREEN) | [View](./adrs/ADR-484-cross-level-foundation-properties.md) |
