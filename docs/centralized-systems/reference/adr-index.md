...
| **ADR-398** | Column placement snap — corner projection + **beam-axis snap** (κέντρο κολώνας ≡ άξονας δοκαριού) + **ghost coloring** (🟢 δοκάρι / 🔴 overlap) + **bugfix** snap-indicator εξαφάνιση στο εργαλείο «Κολώνα» + **§3.3 Smart beam ghost** (φάντασμα δοκαριού στις παρειές κολόνας, 12 θέσεις, mirror) + **§3.4 beam ghost +Y offset fix** (canonical viewport SSoT στο WYSIWYG preview — `setViewportOverride`) + **§3.5 preview-render unification** (canonical preview-frame + `useCanvasGhostPreview` harness· 19 ghost hooks migrated· +transform-lag fix) + **§3.13 Polar Magnet** (κολώνα ΜΕΣΑ στον δίσκο κύκλου = πολικό πλέγμα κέντρο/δακτύλιοι/ακτίνες· snap κέντρο/δακτύλιο∩ακτίνα· R/θ dims· Shift→κλάσματα) + **§3.14 symmetry detector** (n-fold gcd auto-detect + scroll override· interaction defer) + **§3.15 Cartesian Magnet** (κολώνα ΜΕΣΑ σε ΟΡΘΟΓΩΝΙΟ = καρτεσιανό πλέγμα 9-point/grid· 4 dx/dy dims· Shift→κλάσματα· λοξά+ίσια ΕΝΑ κώδικα) + **§3.16 auto-grid-fill** (πλέγμα m×n ίσες αποστάσεις· interaction defer) + **§3.17 Rectangle size adoption** (κολόνα υιοθετεί μέγεθος+κέντρο+γωνία ορθογωνίου DXF στο 1ο κλικ· opt-in confirm· Φ1 rectangle/closed-polyline + Φ2 4-γραμμές· reuse findRectContaining+getCachedRegionPerimeters+rectFrameFromCorners· 17 jest) | 🟢 DONE (snap+beam-ghost browser-verified ✅· §3.13 browser-verified ✅· §3.15/§3.17 impl+jest 🔴 browser-verify· §3.14/§3.16 detector pure 🔴) (UNCOMMITTED) | 2026-06-23 | DXF/BIM | [📄](./adrs/ADR-398-column-placement-snap.md) |
...
| **ADR-510** | Line Creation System (δημιουργία+επεξεργασία γραμμών στο DXF Viewer, «ανώτεροι από AutoCAD»): πλήρης έρευνα μεγάλων παικτών (AutoCAD/BricsCAD/Fusion/Onshape) + Q&A 15 ερωτήσεων (απλά λόγια/παραδείγματα) + αρχιτεκτονική + 10 φάσεις. Καλύπτει: τύπους γραμμών (γραμμή/πολυγραμμή-με-τόξα-bulge/άπειρη/ακτίνα/spline-fit+CV/πολλαπλή-MLINE), input (Direct Distance κύριο + dynamic fields + συντεταγμένες), polar 15°, πλήρες OSNAP, πλήρη linetypes (+σύνθετα κείμενο/σύμβολο), υβριδικοί έξυπνοι δεσμοί (Shift-override), πλήρες live ghost (μήκος+γωνία+σχέση+snap), multifunctional grips, πλήρες modify suite (offset/trim/extend/join/break/fillet/chamfer), 4 magic (command preview/rollover/guided copy-move/selection cycling), «✨ Έξυπνη Γραμμή», μέτρα-με-δεκαδικά, συνεχής σχεδίαση. Renumber 508→510 (collision). | 🟡 Φ2 CORE COMPLETE (UNCOMMITTED) — Φ1 core committed 8ab4143a. Φ2 audit γύρος-2: ~80% ήδη υπάρχει (ADR-358 layer linetype/lineweight + ADR-357 Quick Style)· αληθινό κενό = ο DxfRenderer δεν περνούσε το resolved dash pattern στο setLineDash → γραμμές διακεκομμένων linetypes φαίνονταν συνεχείς. NEW rendering/linetype-dash-resolver.ts (mm→px zoom+LTSCALE) + stores/LinetypeScaleStore.ts + wiring σε line-batch + BaseEntityRenderer.setupStyle (ADR-040-safe, zero regression). 20 jest GREEN. DEFER: σύνθετα/custom-UI/DXF LTYPE round-trip (Φ9). 🔴 browser-verify+commit. Επόμενο Φ3 (bulge+grips) | [View](./adrs/ADR-510-line-creation-system.md) |
| **ADR-511** | Wall Finish per Room/Face (beyond-Revit): ΕΝΑΣ δομικός τοίχος (SSoT, άθικτος) + N περιοχές `wall-covering` (IfcCovering CLADDING/INTERIOR) ανά δωμάτιο/παρειά, όρια από IfcSpace (ΟΧΙ κολώνες, ΧΩΡΙΣ split). Compound layered assembly (μπογιά surface 0πάχος + σοβάς/knauf/πλακίδια body). Magic: auto-πρόταση φινιρίσματος ανά χρήση δωματίου (useType). Mirror floor-finish ADR-419. | 🟡 Slices A+B+C DONE — UNCOMMITTED· A=data+persistence (indexes DEPLOYED)· B=2D live face-strip render (per-frame setWallsById, ⚠️ADR-040 6B/6D)+manual tool+ribbon+i18n· C=room-auto-extent (coveredIntervals/IfcSpace→region/δωμάτιο + useType auto + batch ένα undo). 26 jest GREEN. 🔴 grips/D(3D)/E(BOQ) DEFER· tsc+browser-verify+commit (stage ADR-040) | [View](./adrs/ADR-511-wall-finish-per-room.md) |
| **ADR-512** | Εξαγωγή Tekton `.TEK` (αρχιτεκτονικά, ADR-505 επέκταση): decode ότι το σημερινό `.TEK` v9.1 = απλό **XML** (vs παλιό v5.1 brace/token) → νέα 4η μορφή. **Template-based**: sanitized σκελετός + injection records στους markers. **xmatrix DECODED column-major** (u=(x00,x01)=E−S, v=(x10,x11)=n̂·thickness, (x20,x21)=παρειά, μέτρα). **Φ1=τοίχοι** (browser-verified): 7 NEW `export/core/tek/*`+adapter (geometry reuse `sceneUnitsToMeters`· pure xml-writer· `collectTekWalls`). **Φ2=κουφώματα** (nested `<open>`): `OPEN_RECORD_TEMPLATE`/`TekOpening`/`buildXMatrix`(γενικό SSoT)/`buildOpeningXMatrix`/`buildOpenRecordXml`/`buildOpenXml`· group ανά `wallId`· **θέση/άξονας reuse SSoT `computeOpeningGeometry`** (μηδέν re-derive· straight+curved+polyline). **Φ2b=έπιπλα→`<plane>` κουτιά**: object route (library-based, σταθερές διαστάσεις) απορρίφθηκε· `<plane>`=footprint (world μέτρα)+`<width>`=εξώθηση=ύψος. NEW `PLANE_RECORD_TEMPLATE`/`TekPlane`/`footprintRingToMeters`/`buildPlaneRecordXml`/`collectTekPlanes`+`<!--TEK_PLANE_RECORDS-->`· **reuse γενικών export extractors `extractEntityFootprintRing`+`extractHeightMm`** (ίδιοι με DXF/IFC → ΕΝΑΣ extractor όλα τα formats· γενικό για Φ3 slabs). **Φ-A=στέγη→native `<autoroof>` (κεκλιμένη)**: decoded από `ΔΙΑΦΟΡΑ.tek` (type 8): `<point>` footprint+**`<angle>` κλίση ακμής σε rad** + `<v3list>` «νερά» (faces, per-vertex z). FULL SSoT: footprint/κλίση από `params.outline`/`edges` μέσω `roofSlopeToRatio`→`atan`· «νερά» από ήδη-υπολογισμένο `geometry.faces[].outline` (μηδέν re-derive). NEW `AUTOROOF_*_TEMPLATE`/`TekRoof`/`roofFaceRingToMeters`/`buildAutoroofRecordXml`/`collectTekRoofs`+`<!--TEK_AUTOROOF_RECORDS-->`. (Το ΙΔΙΟ δείγμα ξεκλείδωσε `<line>` type4/`<arc>` type5 για Φ-D· `<hatch>` ΚΕΝΟ→Φ-B blocked.) 46 jest, 142 export-suite GREEN. | 🟢 Φ1+Φ2 BROWSER-VERIFIED Τέκτων 2Δ+3Δ· 🟡 Φ2b έπιπλα + 🟡 Φ-A στέγη(autoroof) κώδικας έτοιμος (UNCOMMITTED) — 🔴 browser-verify+commit· Φ-D primitives (decoded) + Φ3 στατικά DEFER | [View](./adrs/ADR-512-tekton-tek-export.md) |
| **ADR-513** | «Δαχτυλίδι Εντολών» (Radial Command Ring) — in-canvas dynamic input στη σχεδίαση τοίχου: ραδιακό (σταυρωτό) δαχτυλίδι 4 επεξεργάσιμων πεδίων κοντά στον κέρσορα (Μήκος πάνω/Γωνία δεξιά/Πάχος αριστερά/Ύψος κάτω) — πληκτρολόγηση χωρίς ribbon (Revit/AutoCAD Dynamic-Input-grade). **FULL SSoT, μηδέν νέο store**: reuse `DynamicInputField`+`evalExpr`+`DynamicInputLockStore`(Μήκος/Γωνία)+`wallToolBridgeStore.setParamOverrides`(Πάχος/Ύψος). **Κενό που έκλεισε**: το length/angle lock ήταν preview-only & `line`-only → εξαγωγή SSoT `applyLengthAngleLock(point,ref)` (no-op χωρίς lock) που τρέχει **και** στο preview (gate `line\|wall`) **και** στο wall click-commit → preview≡committed. NEW `length-angle-lock.ts`+`radial-ring-logic.ts`(pure)+`RadialCommandRing.tsx`· mount isolated micro-leaf (awaitingEnd) στο `DynamicInputSubscriber` (αντί γραμμικού overlay)· i18n el+en. 24 jest GREEN. | 🟡 IMPLEMENTED (UNCOMMITTED) — 🔴 tsc + browser-verify + commit· DEFER keyboard-only commit + επέκταση σε δοκάρι/κολώνα | [View](./adrs/ADR-513-radial-command-ring.md) |
| **ADR-517** | Πλήρης παρ-ομοιότητα λαβών pad πέδιλο ↔ ορθογώνια κολόνα: το pad έδειχνε 7 grips, η κολόνα 10. Κενό **εκπομπής** (όχι υποδομής — όλα τα SSoT ήδη συνδεδεμένα ADR-436 Slice 1b): εκπομπή `foundation-center` (σταυρός μετακίνησης **4 αυτόνομων βελακιών** + directional move-by-value) + 2 νέες λαβές μέσου πλευράς (`foundation-edge-w`/`-edge-s`, καθρέφτης `column-edge-w/-s`) + σήμα περιστροφής στο `−length/4` (midway, όπως column rect — αποφεύγει σύμπτωση με edge-s). FULL SSoT reuse (`rect-grip-engine`/`grip-glyph-registry`/hot-grip FSM/`move-glyph-frame`), μηδέν νέο μηχανισμό· DRY edge map mirror `COLUMN_EDGE_MAP`. Εύρος = μόνο pad (Giorgio)· strip/tie-beam parity τοίχου/δοκού αμετάβλητο. 42/42 + 27/27 jest GREEN. | 🟡 DONE (UNCOMMITTED) — ΕΚΤΟΣ ADR-040· CHECK 6D → stage ADR-436+517· 🔴 browser-verify + commit | [View](./adrs/ADR-517-foundation-pad-column-grip-parity.md) |
| **ADR-518** | Polygon Column Grips — πλήρες set λαβών πολυγωνικής κολόνας με parity ορθογώνιας (Giorgio 2026-06-24): λαβή ανά **κορυφή** (`column-poly-vertex-i`) + λαβή ανά **μέσο πλευράς** (`column-poly-edge-i`) + **center MOVE 4-βελάκια** (`column-center`, glyph 'move' = 4 ξεχωριστές ζώνες) + **rotation** (`column-rotation`). **FULL SSoT reuse, μηδέν νέο μηχανισμό**: το per-vertex/edge free-reshape υπήρχε ήδη (`freeCornerReshapeGrips`, L/T/U/composite) — απλώς η `kind==='polygon'` έβγαινε νωρίς. ΚΡΙΣΙΜΟ: για **convex** polygon το `interiorAnchorPoint ≈ centroid` → rotation θα έπεφτε πάνω στο move· λύση = polygon rotation στο `−dimY/4` (mirror rectangular, ανάμεσα κέντρο↔κάτω). Νέο `columnCenterMoveGrip` SSoT (dedup inline `rectColumnGrips`)· extract `perVertexAndEdgeGrips`. Downstream (glyph/zones/hit-test/render/commit) = ΜΗΔΕΝ αλλαγή (keyed σε υπάρχοντα grip-kinds). Renumber 517→518 (collision με foundation-pad). | 🟡 IMPLEMENTED (UNCOMMITTED) — 🔴 tsc + browser-verify + commit (CHECK 6B/6D → stage ADR-518) | [View](./adrs/ADR-518-polygon-column-grips.md) |
| **ADR-519** | Circular Column Grips — πλήρες set λαβών στρόγγυλης κολόνας με parity ορθογώνιας (Giorgio 2026-06-24): **center MOVE 4-βελάκια** (`column-center`, glyph 'move' = 4 αυτόνομες ζώνες) + **4 λαβές στα τεταρτημόρια** Β/Α/Ν/Δ (`column-{width,depth,edge-w,edge-s}` — ΙΔΙΑ kinds με τα μέσα-πλευρών ορθογώνιας) που **μεγαλώνουν την ακτίνα**. **ΧΩΡΙΣ rotation** (κύκλος = rotationally symmetric, απόφαση Giorgio). Πριν: 1 grip μόνο. **FULL SSoT reuse, μηδέν νέο registry**: reuse `columnCenterMoveGrip` (ADR-518) + grip-glyph/move-glyph zones· ΝΕΟ focused `column-circular-adapter.ts` (αδελφός `column-rect-adapter.ts`) με συμμετρικό diameter resize (factor 2, κέντρο σταθερό). Boy-Scout: αφαιρέθηκε dead circular branch στο `resizeWidth` (μία πηγή). Downstream = ΜΗΔΕΝ αλλαγή. 104/104 column-grips jest GREEN. | 🟡 DONE (UNCOMMITTED) — ΕΚΤΟΣ ADR-040· CHECK 6D → stage ADR-519· 🔴 browser-verify + commit | [View](./adrs/ADR-519-circular-column-grips.md) |
| **ADR-520** | Center MOVE grip σε free-reshape / composite κολόνες (Giorgio 2026-06-25): μια **composite** στήλη (από συγχώνευση 2 επικαλυπτόμενων στηλών) δεν έδειχνε τον **σταυρό μετακίνησης 4-βελακιών** στο εσωτερικό της. Τώρα το `freeCornerReshapeGrips` (composite + Γ/Τ/Π-polygon) εκπέμπει center MOVE (`column-center`) στο **body-`interiorAnchorPoint`** (όχι bbox-centre — για κοίλα πέφτει στην εγκοπή) + περιστροφή **μετατοπισμένη** προς τα κάτω (`min(depth/4, clearance·0.85)`, φραγμένη ώστε να μένει μέσα στο σώμα) για να μη συμπίπτουν. **FULL SSoT reuse**: `columnCenterMoveGrip` (ADR-518, επεκτάθηκε με optional position) + move-glyph zones + `interiorAnchorPoint`· νέο `interiorAnchorPointWithClearance`. Υλοποιεί την «μελλοντική φάση» που flag-άρε το ADR-518. Downstream = ΜΗΔΕΝ αλλαγή. 110/110 jest GREEN. | 🟡 DONE (UNCOMMITTED) — ΕΚΤΟΣ ADR-040· CHECK 6D → stage ADR-520· 🔴 browser-verify + commit | [View](./adrs/ADR-520-free-reshape-column-center-move-grip.md) |
| **ADR-521** | Column Type «Τύποι» Ribbon Dropdown — το πλήκτρο «Στήλη» (καρτέλα Δομικά) γίνεται καθαρό dropdown «Τύποι» για επιλογή τύπου κολόνας πριν τη σχεδίαση (Giorgio 2026-06-25). Πριν: μόνο rectangular (default FSM). **FULL SSoT reuse**: νέο `'dropdown'` button type (ήδη στο union, ΔΕΝ render-αρε) → `RibbonDropdownButton` που reuse-άρει `RibbonSplitDropdown` (portal/escape)· variants = υπάρχοντα `COLUMN_KIND_OPTIONS` (8 τύποι + i18n)· `columnToolBridgeStore.setKind`. Activate+setKind ΧΩΡΙΣ race: `setKind` (state.kind=νέο) + `handleToolChange('column')` (activate κρατά prev.kind) στο `onAction`. Νέα `COLUMN_DRAW_KIND_ACTIONS` + parser. Renumber 520→521 (collision). | 🟡 IMPLEMENTED (UNCOMMITTED) — 🔴 tsc + browser-verify + commit | [View](./adrs/ADR-521-column-type-ribbon-dropdown.md) |
