...
| **ADR-483** | Static Analysis Canvas Diagrams (T3-UI / Slice 4 — διαγράμματα ροπής Μ κατά μήκος των δοκαριών στον καμβά, Revit→Robot moment diagrams: read-only ADR-040 micro-leaf overlay πάνω στα έτοιμα `DiagramStation[]` του solver ADR-481· pure `member-diagram-geometry` [συνδυασμός ULS-first· κυρίαρχος άξονας ροπής· `f=xM/L`· μέτρα→canvas] + pure `member-diagram-draw` [screen-space offset καμπύλη + ribbon fill + pill ακραίας, reuse canvas-pill]· auto-fit pxScale [μέγιστη ροπή→60px, zoom-stable]· transient `analysis-diagram-view-store` toggle [default OFF]· κολόνες/V/N DEFER· engine read-only) | 🟢 DONE (UNCOMMITTED· 5 jest GREEN· 🔴 tsc+browser-verify+commit) | 2026-06-18 | Uncategorized | [📄](./adrs/ADR-483-static-analysis-canvas-diagrams.md) |
