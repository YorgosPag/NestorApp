...
| **ADR-375** | BIM Entity Line Weight Semantic System (Revit-Equivalent) | ✅ Phase A + B.1 IMPLEMENTED 2026-05-25 — Phase A: 4 SSoT files + 7 renderers + 35 tests. Phase B.1: Drawing Scale Selector (Zustand store + ribbon widget + i18n + 7 renderer wirings + 9 tests). Phase B.2/B.3 + Phase C deferred. | 2026-05-25 | Canvas & Rendering / BIM Drawing | [📄](./adrs/ADR-375-bim-entity-line-weight-semantic-system.md) |
| **ADR-376** | Opening Tags (Revit-Faithful Pattern) — instance Mark + per-kind prefix + floor-prefix hundreds | ✅ Phase A IMPLEMENTED 2026-05-25 — OpeningMarkService + OpeningTagRenderer (canvas-pill SSoT) + Layer SSoT (`__system_opening_tags__`) + i18n (el/en) + ribbon Mark field + migration script. v5.1 — AnnotationsSection global toggle. v5.2 — cross-floor allocator bug fix. v5.3 — blank-canvas Option D defer (§8 Out-of-scope). 38 tests. Phase B (BOQ + Renumber) + Phase C (draggable tag) deferred. | 2026-05-25 | DXF Viewer — BIM / Annotation | [📄](./adrs/ADR-376-opening-tags.md) |
...
| **ADR-375** | BIM Entity Line Weight Semantic System | ✅ Phase A + B.1 IMPLEMENTED | [View](./adrs/ADR-375-bim-entity-line-weight-semantic-system.md) |
