...
| **ADR-611** | Stair geometry generators + run computers SSoT (`stair-geometry-generators.ts` + `stair-geometry-runs.ts`· `computeRadialStair`/`computeWalklineStair` + flight/assembler generators· 12 kinds → thin thunks· −47 jscpd) | ✅ ACTIVE | 2026-07-09 | Uncategorized | [📄](./adrs/ADR-611-stair-geometry-generators-ssot.md) |
| **ADR-613** | Guide command SSoT (`core/commands/base-command.ts` generic `BaseCommand` + `CreatedGuidesCommand`/`BatchRotateGuidesCommand` guide bases + `guide-command-geometry.ts`· 18 guide `ICommand` classes → thin subclasses· −30 jscpd) | ✅ ACTIVE | 2026-07-09 | Uncategorized | [📄](./adrs/ADR-613-guide-command-ssot.md) |
| **ADR-614** | Text command SSoT (`core/commands/text/dxf-text-command-base.ts`· `DxfTextCommandBase` + `DxfTextNodeMutationCommand` + `resolveEditableTextEntity`/`recordTextAudit`/`mergePatchInputs` helpers· adopts ADR-613 `BaseCommand`· 13 text `ICommand` classes → thin subclasses· −26 jscpd) | ✅ ACTIVE | 2026-07-09 | Uncategorized | [📄](./adrs/ADR-614-text-command-ssot.md) |
| **ADR-616** | Layer command SSoT (`core/commands/layer/layer-command-base.ts`· `LayerCommandBase` + `SingleLayerFlagCommand`/`MutateAllLayersCommand`/`IsolateEffectsCommand`/`DelegatingLayerCommand`· `BaseCommand` +optional-id· 12 layer `ICommand` classes → thin subclasses· −18 jscpd) | ✅ ACTIVE | 2026-07-09 | Uncategorized | [📄](./adrs/ADR-616-layer-command-ssot.md) |
| **ADR-617** | Entity command SSoT (`core/commands/entity-commands/`· `BatchEntityPatchCommand`/`EntityIdsBatchPatchCommand` + `AssignTypeCommandBase` + `EntityFieldOverrideCommand`/`FaceAppearanceFieldCommand` + `writeParamsOnly`/`applyResolvedState`/`faceKeyOverride*` helpers· adopts ADR-613 `BaseCommand`· 12 `ICommand` classes across 3 families → thin subclasses· −22 jscpd) | ✅ ACTIVE | 2026-07-09 | Uncategorized | [📄](./adrs/ADR-617-entity-command-ssot.md) |
| **ADR-618** | BIM-3D placement SSoT (`bim-3d/placement/`· `createPlacementGhostClass` (7 ghosts) + `usePlacementInteractionEffect`/`createSnapMarkerPlacementController`/`resolveWorkPlaneHit` (interaction primitive under the ADR-605 factory + column/wall/mep-segment hooks) + `TempDim3DOverlayBase` (2 temp-dim overlays)· all thin subclasses/leaves· −30 jscpd) | ✅ ACTIVE | 2026-07-10 | Uncategorized | [📄](./adrs/ADR-618-bim3d-placement-ssot.md) |
| **ADR-611** | Opening Frame Profile (Διατομή Κάσας) — editable per-manufacturer catalog, CONSTANT cross-section decoupled from opening width/height | 🔵 RECOGNITION+DESIGN | 2026-07-09 | DXF Viewer — BIM / Parametric Building Modeling | [📄](./adrs/ADR-611-opening-frame-profile-catalog.md) |
| **ADR-615** | Free-standing / self-hosted opening (host-less parametric κούφωμα· `OpeningHost` dual-provider SSoT· optional `wallId` + `selfHost`· placement πάνω σε ADR-600) | 🔵 RECOGNITION+DESIGN | 2026-07-09 | DXF Viewer — BIM / Parametric Building Modeling | [📄](./adrs/ADR-615-free-standing-self-hosted-opening.md) |
| **ADR-619** | «Σκάλα από περιοχή» (Stair-from-Region) — free-polygon capture (`usePolygonSketchChain`, ADR-363 pattern) + shape classifier (rectangle→straight, L→l-shape/winder, U→u-shape, circle→spiral) → `buildStairParamsFromRegion`/`buildStairEntity` (ADR-358/611, unchanged) | 🔵 RECOGNITION+DESIGN | 2026-07-10 | DXF Viewer — BIM / Parametric Building Modeling | [📄](./adrs/ADR-619-stair-from-region.md) |
| **ADR-620** | Rotated-rectangle geometry — canonical model (corner1/corner2 τοπικά + `rotation` γύρω από corner1)· rotation-aware `createRectangleVertices` + entity-level `rectangleEntityVertices` SSoT (−3 duplicate copies)· `rotateRectangleLike` double-rotation fix· καταναλωτής = ADR-513 §rectangle dynamic input | 🟢 IMPLEMENTED | 2026-07-10 | DXF Viewer — Rendering / Geometry | [📄](./adrs/ADR-620-rotated-rectangle-geometry.md) |
| **ADR-621** | Section stencil-cap SSoT (`bim-3d/systems/section/`· `createParityStencilMaterial` + `createCutCapMaterial` (4 parity + 3 cap + hatch/colour caches) + `createHatchCanvas` (5 texture builders) + `hideNonParityMeshes`/`restoreHidden` + `withSectionCapRenderState` (new `section-parity-scene`) + `PlaneCapPass`/`runMaskedParity` (secondary passes)· identical public API· renderer 494→423 L· section clones 256→0· full-scan −47) | ✅ ACTIVE | 2026-07-10 | DXF Viewer — BIM / 3D Section | [📄](./adrs/ADR-621-section-stencil-cap-ssot.md) |
| **ADR-622** | Structural detail-sheet SSoT (`bim/structural/detail-sheet/`· `buildScheduleTable`/`buildReinforcementSchedule` (4 schedules) + `buildFieldBlock` (4 title blocks) + `assembleDetailSheet`/`standardSheetRegions` (4 orchestrators) + `captureDetail3d`/`bboxDimSpecs`/`projectDims`/`projectMarks` (5 captures· column inline-copy migration 254→90 L)· identical public API· dir 58→18 clone pairs· full-scan −80 window) | ✅ ACTIVE | 2026-07-10 | DXF Viewer — BIM / Structural Detail Sheets | [📄](./adrs/ADR-622-structural-detail-sheet-ssot.md) |
| **ADR-623** | Ribbon entity-bridge SSoT (`ui/ribbon/hooks/`· 4 shared primitives in `ribbon-entity-bridge-shared.ts`: `useResolveSelectedEntity` adopted ×21 + new `useNoopToggles`/`useViolationBadgeState`/`useStableBridge`· 27 bridges + stair migrated to thin bindings + `extends RibbonEntityBridgeCore`/`RibbonBridgeCore`· identical public API· dir 1116→931 cloned lines / 86→62 pairs· full-scan −130· 308 jest pass +smoke) | ✅ ACTIVE | 2026-07-10 | DXF Viewer — UI / Ribbon Bridges | [📄](./adrs/ADR-623-ribbon-entity-bridge-ssot.md) |
| **ADR-624** | WYSIWYG placement-ghost SSoT (`hooks/tools/`· `use-wysiwyg-placement-ghost.ts`: generic `useWysiwygPlacementGhost<TBuild>` build→paint(+overlay) primitive over `useCanvasGhostPreview` + `createBridgeStorePlacementGhostHook` factory· 6 single-point MEP/electrical-panel ghosts → 3-line bindings + 3 variants (opening/segment/slab-opening) onto the primitive· identical public API· leaves untouched· jscpd:diff clean (no SKIP)· full-scan 3364→3339· 27 jest incl. 9-hook smoke) | ✅ ACTIVE | 2026-07-10 | DXF Viewer — Tools / Ghost Preview | [📄](./adrs/ADR-624-wysiwyg-placement-ghost-ssot.md) |
| **ADR-625** | Edit/transform preview draw-skeleton SSoT (`hooks/tools/`· 4-layer stack: `useGhostOverlay` base (subscribe+toScreen idiom) + `overlay-draw-primitives` (`tracePolyline`) + 3 paint primitives `useEditFencePreview` (EXTEND/TRIM colour-config) / `useCornerToolPreview` (CHAMFER/FILLET Template-Method· `resolveCornerStrokes` hoists identical polyline branches, two-line stays bespoke) / `useTransformGhostPreview` (SCALE/STRETCH crosshair+rubber-band+tooltip+WYSIWYG loop· owns bim-preview wiring)· 6 hooks→thin bindings· identical public API· leaves untouched· jscpd:diff clean (no SKIP)· full-scan 3339→3299· 22 jest) | ✅ ACTIVE | 2026-07-10 | DXF Viewer — Tools / Edit & Transform Preview | [📄](./adrs/ADR-625-edit-transform-preview-ssot.md) |
| **ADR-626** | Closed-area drawing-tool SSoT (`hooks/drawing/`· `usePolygonAreaTool<TOverrides,TEntity>` domain-wrapper πάνω στο canonical `usePolygonSketchChain` FSM (ADR-363)· Roof/FloorFinish/MepUnderfloor σταματούν να inline-άρουν το ίδιο vertex-chain FSM → thin bindings (295/262/253→~65γρ)· **face-snap parity** για FloorFinish/Underfloor (Revit «by boundary»)· orchestrator getSceneUnits/getSceneEntities/autoClose resolver hoist· identical public API· full-scan 3299→3281· 20 jest· jscpd:diff clean no SKIP) | ✅ ACTIVE | 2026-07-10 | DXF Viewer — Drawing / Area Tools | [📄](./adrs/ADR-626-polygon-area-tool-ssot.md) |
| **ADR-628** | Wall boolean-op persistence primitive + BIM BOQ/audit lifecycle SSoT (`hooks/data`· cluster #18)· `useWallBooleanOpPersistence(params,event,run)` generic EventBus scaffold (service refs / scope-init / live BOQ scope / reference-stable subscribe + `upsertMergedWallBoq`/`deleteWallBoq`/`applyOpeningUpdates`) → merge/split twins thin· `createBimBoqAuditLifecycle<TEntity>` → `{onPersisted,onDeleted,onRestored}` audit+Η-Μ BOQ triplet adopted by 5 MEP hooks (boiler/manifold/radiator/water-heater/underfloor)· `BimEntityPersistencePublicScope` param-surface base· NOT scene-aware structural feeds (N.1)· identical public API· full-scan 3494→3262· 60 jest· jscpd:diff clean no SKIP | ✅ ACTIVE | 2026-07-10 | DXF Viewer — Persistence / BOQ & Audit | [📄](./adrs/ADR-628-wall-boolean-op-persistence-ssot.md) |
| **ADR-629** | Point-placement completion builder SSoT (`hooks/drawing`· cluster #19)· `point-completion-builders.ts`: `buildBimPointEntity<>` (identical validate→geometry→create→`PlacementBuildResult` skeleton adopted ×8) + `resolveBodyPlacement` (footprint/pose prologue ×5, `*ParamOverrides extends BodyPlacementParamOverrides`) + `assembleMepApplianceBodyParams` (boiler/radiator/water-heater base-assembly twin)· manifold/fixture/furniture/floorplan bespoke `buildDefault` (N.1)· identical public API (`Build*EntityResult`=aliases)· hooks/drawing 516→478 cloned L / 41→40 pairs· full-scan 3494→3260· jest parity green· jscpd:diff clean no SKIP | ✅ ACTIVE | 2026-07-10 | DXF Viewer — Drawing / Point Placement | [📄](./adrs/ADR-629-point-completion-builders-ssot.md) |
| **ADR-631** | Vertex/overlay command base SSoT (`core/commands`· cluster #20 Stage 1)· vertex+overlay command families adopt υπάρχον `BaseCommand` (id/timestamp/serialize-envelope/redo/canMergeWith) + 3 νέα SSoTs: `DragVertexEditCommand<TSelf>` (drag-merge Template-Method· MoveOverlay/Multi) + `EntityVertexCommand`/`OverlayVertexCommand<TStore>` family bases (target surface + getAffectedEntityIds + validate prologue) + `vertex-command-validation.ts`· MoveVertex/MoveOverlayVertex = family-base + inline drag (diamond)· dead-getter Boy-Scout· identical public API· full-scan 3494→3256· 628 jest· jscpd:diff clean no SKIP· Stage 2 (entity-command pairs) pending | ✅ ACTIVE | 2026-07-10 | DXF Viewer — Core / Commands | [📄](./adrs/ADR-631-vertex-command-base-ssot.md) |
| **ADR-634** | BIM BOQ base-row SSoT (`bim/services`· cluster #21)· `boq-base-row.ts`: `buildBoqBaseRow` (~30 default BOQItem fields, moved από boq-multi-layer-builder `buildBaseRow`· `BoqBaseRowContext` minimal· `entityType: BoqSourceEntityType`=`BOQItem['sourceEntityType']` wider than BimEntityType→'envelope' no cast) + `buildSingleEntityBoqRow` (base+ATOE mapping+quantity+null-tail, stripUndefinedDeep)· 6 writers adopt (bridge/multi-layer/structural-finish/envelope/opening/stair)· divergent rows override explicit (opening group-id null, envelope scope/material)· 1:1 values· full-scan 3494→3248· 273 jest· payload jscpd clean· **+follow-up**: `boq-firestore-sync.ts` (`syncManagedBoqRow`/`deleteManagedBoqRow`) + `buildGroupParentBoqRow` + bridge-local `resolveEntityAtoeMapping`/`upsertRowGroup` → 6 co-staged sync clones eliminated· 132 BOQ jest· jscpd:diff clean no SKIP | ✅ ACTIVE | 2026-07-10 | DXF Viewer — BIM / BOQ | [📄](./adrs/ADR-634-boq-base-row-ssot.md) |
| **ADR-635** | AutoCAD DXF import entity coverage (Φ1)· root-cause: `buildScene` empty-line `filter` έσπαγε το (code,value) βήμα-2 → alignment corrupt → ~90% entities silent-drop (R12 sample 4483→467)· fix=μην φιλτράρεις κενές γραμμές· +old-style `POLYLINE` (POLYLINE+VERTEX+SEQEND compound via `parsePolylineGroup`, header elevation excluded)· +`parseVerticesFromPairs` ordered-pairs SSoT (ADR-507 pattern) fixes LWPOLYLINE data-map overwrite bug· closed=`(70&1)` bitmask (129=closed)· real import 467→4467 entities· 260 jest· **Φ2 DONE**: INSERT/BLOCK expansion (section-aware parse· `parseBlockDefinitions`+`instantiateInsert` reuse scaleEntity/rotateEntity/translateEntityByAnchor· nested+MINSERT arrays· block-def geometry ΔΕΝ standalone)· fixes ~360m-away block geom (authored+363619 ⊕ INSERT−346494=17125): bbox 363m→68.7m, 0 strays, 4467→556 (417 orphan defs σωστά κρύβονται)· 266 jest jscpd-clean· **Φ3-text DONE**: imported text «no height» — mm-scale σκάλωνε flat `height` αλλά ο `resolveTextHeight` διαβάζει textNode run-height FIRST (shadowed)· fix=`scaleText`/`scaleMText` σκαλάρουν textNode μέσω υπάρχοντος `scaleTextNodeRunHeights`· Ελληνικά ΗΤΑΝ ΗΔΗ σωστά (Win-1253 auto-detect)· `+95.00` legit stray αφέθηκε (AutoCAD)· **Φ3 skipped-warning pending** | ✅ ACTIVE | 2026-07-11 | DXF Viewer — Import/Parser | [📄](./adrs/ADR-635-autocad-dxf-import-entity-coverage.md) |
| **ADR-636** | Professional DXF export (HEADER + version-driven encoding· AutoCAD-style, staged)· root-cause: writer έβγαζε bare envelope (χωρίς HEADER/$ACADVER/$DWGCODEPAGE)· Ελληνικά UTF-8 → ο AutoCAD υποθέτει ANSI → garbled («ÊËÉÌÁÊÁ» στο export)· **Στάδιο 1 DONE**: gated HEADER στο `dxf-ascii-writer` (`acadVer`/`insunits`/`codepage` options, ΠΡΙΝ TABLES) + `dxf-export-adapter` resolvers (`resolveUnicodeSafeAcadVer`: pre-Unicode+utf-8→AC1021· `encodingToCodepage`· `DXF_UNIT_VALUES[unit]` για $INSUNITS)· UTF-8 ανοίγει σωστά σε AutoCAD 2007+ + καθαρές μονάδες round-trip· bare calls (Tekton) αμετάβλητα· 65 jest jscpd-clean· **Στάδιο 2 pending**: version selector UI + Win-1253 bytes/\U+ escapes για R12-2004 + LAYER table + MTEXT/alignment round-trip | ✅ ACTIVE | 2026-07-11 | DXF Viewer — Export | [📄](./adrs/ADR-636-professional-dxf-export-header-encoding.md) |
...
| **ADR-597** | BIM Corner Snap System (Revit/ArchiCAD-grade Face-Corner Snapping) | ✅ IMPLEMENTED | [View](./adrs/ADR-597-bim-corner-snap-system.md) |
