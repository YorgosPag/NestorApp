...
**📊 Stats**: 345 ADRs | Last Updated: 2026-05-28
...
| 📄 **Uncategorized** | 136 | [View](#uncategorized) |
...
| **ADR-363** | BIM Drawing Mode (Parametric Building Elements) | ✅ 🟢 **FULLY IMPLEMENTED** 2026-05-21 — Phases 0-8 complete. Wall/Opening/Slab/Column/Beam tools, Phase 5.6 Wall Split, Phase 6 BOQ auto-feed, Phase 7.1-7.2 multi-select + bulk edit, Phase 8 schedule export. Wall grip refinements: 1C-bis/1C-ter asymmetric corners, 1F 3-click lateral alignment, 1G hot-grip move/rotate/copy, **1H ESC incremental-back (2026-05-29)**, **§5.4 hosted-opening cascade unified SSoT — κουφώματα ακολουθούν τον τοίχο σε move/rotate/mirror/scale/grip/length-edit (2026-05-29)**, **1J «Τοίχος πάνω σε οντότητα 2Δ» — pick line/rectangle → wall(s), 2-click pick-entity/pick-side (2026-05-30)**, **1K «Τοίχος σε περιοχή (4 γραμμές)» — επιλογή 4 γραμμών που κλείνουν ορθογώνιο (ή κλικ μέσα) → ΕΝΑΣ γεμάτος τοίχος (μήκος=μεγάλη, πάχος=μικρή πλευρά)· corner-graph detection SSoT `wall-in-region.ts`· **modes A+B+C — Mode C box-select (drag window/crossing → ΟΛΑ τα εσώκλειστα ορθογώνια, decoupled EventBus `bim:wall-region-box-select`, απλό κλικ ανέπαφο pick), 15/15 tests (2026-05-30)**, **Wall-miter diagonal-corner fix — `cornerMiter` swap-aware pairing για end+end/start+start γωνίες (φυσικά σχεδιασμένο ορθογώνιο → 4/4 σωστές γωνίες), 24/24 tests (2026-05-30)**, **§5.4 Opening `kind` SSoT — `params.kind` μοναδική αλήθεια (top-level `kind`+`ifcType` καθαρά παράγωγα: hydration/save/command/persist), auto-Mark Θ↔Π στην αλλαγή τύπου (μόνο αυτόματα), + cut-plane gating (Revit parity: εκτός-τομής κούφωμα→συμπαγής τοίχος + «<Beyond>» dash)· Bug Θ.101 self-heal στο φόρτωμα· 5 test suites (2026-05-30)**. | 2026-05-17 | DXF Viewer — BIM / Parametric Building Modeling | [📄](./adrs/ADR-363-bim-drawing-mode.md) |
...
| **ADR-371** | BIM Corner Snap System (Revit/ArchiCAD-grade Face-Corner Snapping) | ✅ ✅ **IMPLEMENTED** 2026-05-22 — Phases 2A-2F complete. 27 files created/edited. Integration test (S1-S5) added 2026-05-22. Bugfix: SnapContext.ALL_MODES missing BIM types fixed 2026-05-22. Extension: Wall Face Corner Projection Snap (Revit-style) added 2026-05-22. Extension (ADR-398): Column Body Corner Projection Snap — move/resize/draw parity, added 2026-05-29 (§17). | 2026-05-22 | DXF Viewer — Snapping / BIM Precision | [📄](./adrs/ADR-370-bim-corner-snap-system.md) |
...
| **ADR-394** | Fit to View to Selected Entities (Z key) | ✅ 🟢 **IMPLEMENTED** 2026-05-28 — `Z` = fit-to-selection (DXF + BIM + mixed). Home unchanged (fit-all). SSoT `calculateCombinedEntityBounds`. Command-line precedence + empty-selection fallthrough. Hotfix 2026-05-29: layout-independent `event.code` fallback → `Z` δουλεύει και σε ελληνικό πληκτρολόγιο. Pending commit. | 2026-05-29 | Canvas & Rendering / DXF Viewer — Tools & Keyboard | [📄](./adrs/ADR-394-fit-to-view-selected-entities.md) |
| **ADR-395** | BIM Quantities → Building Measurements (Επιμετρήσεις) | 🟢 **PHASE 2 + G6 + G2 + WAIST PARAM + G5 IMPLEMENTED** 2026-05-29 (pending commit) — **G5**: `qto` field = νεκρός κώδικας (επαλήθευση: 0 writers· η premise «το διαβάζει το Schedule» ήταν λάθος — ο μόνος reader, Schedule combined default view, έβγαζε 3 κενές στήλες = λανθάνον bug). Fix (Option A): νέο `deriveAtoeQuantity` geometry SSoT (bridge delegate-άρει) + `mapCombined` geometry-derived + delete `qto`/`BimQuantityTakeoff`/`StairQTO`/`makeBimQto` + 5 factory inputs + Firestore/hydration passthroughs. 151/151 tests, tsc clean. — G1 σκάλες: 3 BOQ rows/σκάλα (concrete OIK-2.05 m³ / cladding OIK-5.05 m² / handrail OIK-12.01 m), geometry-derived (`StairQTO` ποτέ δεν γεμίζει → §4.6/G5), per-row detach + delete-when-zero. **Waist param**: πάχος πλάκας σκάλας 150mm σταθερά → επεξεργάσιμο `StairParams.waistThickness` + ribbon combobox «Πάχος Πλάκας» (default 150, `DEFAULT_WAIST_SLAB_THICKNESS_MM` SSoT). **G6**: καθαρό m² τοίχου = gross − ανοίγματα (computeWallGeometry +openings, bim:opening-persisted live re-feed). **G2**: καθαρό m³ πλάκας = gross − slab-openings (premise §4.5 ήταν ψευδής — useSlabPersistence περνούσε undefined· fix: collectSlabOpenings + slabBoqGeometry SSoT + bim:slab-opening-persisted re-feed). Scene/3D geometry μένει gross, μόνο BOQ payload net. ΝΕΑ stair-boq-quantities.ts + stair-boq-sync.ts· wiring use-stair-persistence + StairPersistenceHost/DxfViewerTopBar (+buildingId/floorId). Boy Scout: column I-shape MET-1.01→OIK-12.10. 95/95 tests (slab) + 46/46 (wall) + 43/43 (stair) + 14/14 (stair-boq waist), tsc clean. Phase 1 (G3+G7) → commit `ecea20e1`. | 2026-05-29 | BIM / Quantity Take-Off ↔ Buildings | [📄](./adrs/ADR-395-bim-quantities-building-measurements.md) |
| **ADR-396** | Ενιαία Εξωτερική Θερμοπρόσοψη (ETICS) για BIM | 🟢 **P1+P2+P3 COMMITTED** (a736012e) + **P4 2D + P5 3D + P6 UI COMMAND DONE** 2026-05-29 (pending commit) — Υβριδικό μοντέλο (per-element exterior layer + 1 command auto-apply + unified offset perimeter). 4 ζώνες. **P1** foundations · **P2** per-element layer · **P3** geometry SSoT (`envelope-perimeter.ts` Option 1 offset/mitre + `exposed-slab-classifier.ts` Z2/Z3). **P4** 2D render — dedicated floor-overlay (`EnvelopeOverlay` micro-leaf ADR-040) → συνεχές offset περίγραμμα + insulation hatch band (reuse `computeWallHatchPlan`) + V/G κατηγορία «Θερμοπρόσοψη» (ADR-382/375) + minimal spec store. **P5** 3D render (parity ADR-370) — extruded κέλυφος **Z1 κατακόρυφο** (`EnvelopeToThree.envelopeChainToMesh` + `envelope-material-resolver` + `MaterialCatalog3D` `elem-envelope` + `BimSceneLayer.syncEnvelope`)· παράγωγο floor-shell mirror του 2D overlay· ίδιο `computeEnvelopePerimeter` SSoT· 2D⟷3D parity. **P6** UI command «Εφαρμογή Θερμοπρόσοψης» (Analyze tab) — `ThermalEnvelopeDialog`+`ThermalEnvelopeHost` (Radix Dialog+EventBus mirror ADR-376 C.2)· material Neopor/XPS + πάχη mm + Z1-Z4 toggles + ΚΕΝΑΚ soft-warn → `setEnvelopeSpec` ανά όροφο/«σε όλους»· **auto-seed scaffold ΚΑΤΑΡΓΗΘΗΚΕ** (εμφάνιση μόνο μετά apply)· 3D resync via `use-bim3d-vg-resync`. P6 = authoring spec ΜΟΝΟ (per-element layers → P7). ⚠️ P5 = μόνο Z1· Z2/Z3 flat + Z4 reveals = ξεχωριστή φάση. P4 6/6 + P5 6/6 + P6 11/11 tests PASS, tsc clean. OQ-1/2/3 RESOLVED, OQ-6 by-design. **P7 DONE (A+B)** (2026-05-29, pending commit) — Part A: spec persistence στο level doc (`thermalEnvelopeSpec` mirror `bimRenderSettings` + `useThermalEnvelopeSync` quiet-window) + Zod unblock (`thermal-envelope.schemas.ts` SSoT· 4 entity schemas +envelopeLayer/revealInsulation πριν `.strict()`) → **κέλυφος επιβιώνει reload**. **Part B**: per-element layers — `envelope-element-applicator.ts` (Z1 columns/beams proximity `EXTERIOR_PROXIMITY_M`· Z2/Z3 `filterExposedSlabs`· Z4 exterior openings) + audit tracked-fields (envelopeLayer/revealInsulation, ride existing audit-clients) + `envelope-boq-sync.ts` (χωριστές γραμμές `boq_env_<floorId>_<zone>` OIK-10.05 m², D5· orphan cleanup) + persist via `bim:envelope-applied` (ride existing hooks, μηδέν νέο host) + `.ssot-registry.json` module `thermal-envelope`. 18/18 + 58/58 regression PASS. **P-RENDER DONE** (2026-05-29, pending commit) — Z2/Z3/Z4 ορατότητα 2D+3D (πριν: μόνο Z1). Render = pure read per-element data. Z2/Z3 = slab footprint hatch (2D) + flat layer πάνω/κάτω (3D)· Z4 = inset frame γύρω από άνοιγμα (2D) + lining frame extruded (3D). `+insetClosedPolygon` SSoT (`polygon-utils`)· `buildSlabHatchPlan`/`buildRevealBandPlan` (render-plan)· `slabFlatLayerToMesh`/`revealLiningToMesh` (`EnvelopeToThree`)· `syncEnvelope` split addEnvelopeShell/addFlatLayers/addRevealLinings. 7 mod, 23/23 + 116/116 regression PASS, tsc clean, ADR-040 staged (CHECK 6B/6D). **P8 DONE** (2026-05-29, pending commit) — θερμική απόδοση U-value/ΚΕΝΑΚ (OQ-7 RESOLVED): νέο `bim/thermal/` SSoT (`assembly-u-value.ts` `computeAssemblyUValue` 1/(Rsi+Σd/λ+Rse) ISO 6946 + `kenak-thermal-config.ts` ΤΟΤΕΕ `KENAK_MAX_U_WALL` Α0.55/Β0.45/Γ0.40/Δ0.35 + `REFERENCE_BARE_WALL_LAYERS` τυπικός τοίχος + `CLIMATE_ZONE_OPTIONS` + `isAboveKenakUMax` soft-warn) + `wall-material-catalog` `+WALL_MATERIAL_LAMBDA`/`getThermalConductivityLambda` (λ ανά υλικό SSoT, τρέφει P9) + U-value/pass-warn panel στον `ThermalEnvelopeDialog` + **`Building.climateZone`** ρύθμιση κτιρίου (passthrough API PATCH, μηδέν firestore.rules/zod/allowlist → no CHECK 3.16). 3 new + 5 mod, 13/13 tests PASS, tsc P8-scope clean. **P9 DONE** (2026-05-29, Opus 4.8, pending commit) — **ADR-396 ΟΛΟΚΛΗΡΩΜΕΝΟ 9/9, FULL enterprise parity Revit/ArchiCAD/Bentley.** IFC interoperability: 6ος serializer `ifc-covering-serializer.ts` → ETICS κέλυφος ως `IfcCovering(INSULATION)` **per-element** + `IfcRelCoversBldgElements` + `IfcMaterial`/`IfcMaterialLayerSet` + `Pset_MaterialThermal`(λ από P8). **semantic-only** (χωρίς γεωμετρία· `IfcMaterialLayerSetUsage` παραλείπεται)· **walls Z1** από per-floor spec (`ifc-envelope-spec-loader.ts`, αφού τοίχοι δεν κουβαλούν per-element layer). `+typed` inline-measure value (graph+writer, ADR-369) + `+envelopeSpecs` param + IfcExportHost wiring. 2 new + 5 mod, 10/10 + roundtrip 8/8 PASS, tsc IFC-clean. **BUGFIX insulation-line diagonal jog (2026-05-30, Opus 4.8)** — γραμμή μόνωσης «έσπαγε» λοξά σε γωνίες· 2 αιτίες: (α) cascade από ADR-363 wall-miter fix → καθαρό exteriorFaceLoop· (β) closed-ring offset bug — `offsetPolyline`/`insetClosedPolygon` έκαναν offset κλειστό loop ως open polyline → seam vertex diagonal jog. SSoT fix: closed-awareness στο `offsetPolyline` (`polygon-utils`, νέα param `closed`, default false→walls/beams ανέπαφα) + `stripClosingDuplicate`· διορθώνει insulation loop + Z4 reveal frames από 1 σημείο. Νέος `naturalRect` test builder (τα παλιά CCW-only tests δεν έπιαναν inconsistent γωνίες). 30/30 envelope PASS. **BUGFIX 2Δ reveal (2026-05-30, Opus 4.8, pending commit)** — (BUG 1) λοξή παρειά: το 2D Z4 ζωγραφιζόταν ως inset frame 4 πλευρών (45° miter) → πλέον **2 παραστάδες** κάθετες στον άξονα μέσω νέου SSoT `reveal-lining-geometry.ts:computeRevealJambQuads` (2D `buildRevealJambPlans`+`renderSlabHatch`· 3D `revealLiningToMesh` reuse· `buildRevealBandPlan` καταργήθηκε). (BUG 2) αμόνωτο κενό γωνίας: `computeEnvelopeOpeningCuts` στενεύει το Z1 cut κατά `revealThk` σε κάθε άκρο → η μόνωση πρόσοψης τυλίγει+σκεπάζει το εξωτ. άκρο παραστάδας (2D⟷3D parity). **Follow-up:** (α) παραστάδες ξανα-ορατές (`buildRevealJambPlans`→`EnvelopeRenderPlan[]` με orange περίγραμμα)· (β) **πραγματικό root τραπεζοειδούς παρειάς = `opening-geometry.ts`** (όχι Z4): το outline χτιζόταν `άξονας±πάχος/2` αλλά οι ακμές τοίχου σε mitered γωνίες είναι λοξές → fix: προβολή στις πραγματικές `outerEdge`/`innerEdge` (βλ. ADR-363). 507/507 broad regression PASS, tsc clean, ADR-040 staged (CHECK 6D). **BUGFIX Z1 cut ↔ wall ↔ Z4 collinear (2026-05-30, Opus 4.8, pending commit)** — wall punch + Z4 ήδη flush (κοινό outline)· το Z1 cut (`computeEnvelopeOpeningCuts`) απέκλινε γιατί προέβαλλε axis-midpoints στο face loop → λοξό face πλευρική μετατόπιση. Fix: προβολή των exterior-face γωνιών του outline (`projectExteriorCorner`) → 3 παρειές collinear· κανένα change σε reveal/3D (κοινό SSoT). 403/403 bim/geometry + 16/16 3D PASS, διαγνωστικό-first. 🔴 browser verify. **BUGFIX 3D τραπεζοειδής τοίχος (2026-05-30, Opus 4.8, pending commit)** — Giorgio (3D view): παρειά τοίχου λοξή στο άνοιγμα (μαζεύεται πρόσοψη/επεκτείνεται μέσα). Root: `buildStraightWallWithOpenings` (3D) όριζε παρειά με `lerp(outerEdge,sF)`+`lerp(innerEdge,sF)` κοινό fraction → σε miters λοξή. Fix: NEW `wall-opening-pieces.ts:computeWallOpeningPieces` — παρειά από κάθετες γωνίες outline (κοινό SSoT με 2D punch + Z4 + Z1) → 3D wall↔Z4↔Z1 collinear. 70/70 bim-3d PASS, διαγνωστικό-first. 🔴 browser verify. **ΣΗΜΑΣΙΟΛΟΓΙΚΗ ΑΛΛΑΓΗ — reveal «τρώει τον τοίχο» (Free vs Structural, 2026-05-30, Opus 4.8, Plan Mode, pending commit)** — Giorgio (step-by-step): η Z4 μόνωση έμπαινε ΜΕΣΑ στο άνοιγμα (μείωνε το φως). Σωστό ETICS: `width/height`=ΕΛΕΥΘΕΡΟ κούφωμα (σταθερό)· μόνωση ΕΞΩ τρώγοντας τον τοίχο (δομικό κενό +2t· τοίχος 2.00→1.95). SSoT `computeOpeningGeometry +revealOutline` + `structuralRevealHeightRangeMm`· wall punch 2D+3D + Z4 (flip+lintel/sill)=structural· Z1=free (no wrap)· φύλλο/hit-test/BOQ=free. **ΡΙΖΑ που «επέστρεφε»: σημασιολογικό, όχι γεωμετρικό.** 9 αρχεία + 5 test suites, 1047/1047 affected PASS, tsc clean. 🔴 browser verify· ⚠️ Z1 facade πιθανό follow-up. **v2 Phase 1 (2026-05-30, pending commit)** — `enclosesRegion` gate (ανοιχτές αλυσίδες → καμία ETICS· 3 consumers). **v2 Phase 2 (2026-05-30, Opus 4.8, pending commit)** — NEW `building-footprint.ts` boolean union (ΤΟΙΧΟΙ+ΚΟΛΩΝΕΣ+ΔΟΚΑΡΙΑ → outer rings + holes/αίθρια + per-edge entity attribution, lib `polygon-clipping` MIT· πιάνει επικαλύψεις π.χ. κολώνα-σε-τοίχο)· αυτόνομο (μηδέν wiring — Φάση 5· compat σχήμα → μελλοντική αντικατάσταση `envelope-perimeter`)· reuse computeWall/Beam geometry+prepareColumns· 11/11+410/410 PASS, tsc clean. §3.1.1. **v2 Phase 3 (2026-05-30, Opus 4.8, pending commit)** — NEW `footprint-region-classifier.ts` auto ταξινόμηση 3 στρώσεων: Στρ.1 εξώτατο όριο→ΠΑΝΤΑ μόνωση (`exterior`)· Στρ.2 κάθε τρύπα→**αίθριο** (χωρίς πλάκα από πάνω→μόνωση) ή **κλειστό δωμάτιο** (με πλάκα→όχι)· Στρ.3 per-element override→Φάση 4+6. Έλεγχος «πλάκα από πάνω;» = **γεωμετρική τομή** (`polygon-clipping` τομή/ένωση· coverage≥`ATRIUM_COVERAGE_THRESHOLD` 0.5 configurable→δωμάτιο). Resolver `selectSlabsAboveFloor` reuse ΙΔΙΟ elevation SSoT με `classifyExposedSlab` (`resolveSlabTopMm`→`getEntityAbsoluteElevation`, εξήχθη ως export). Αυτόνομο (μηδέν wiring — Φάση 5). 1 new+1 test+2 mod, 12/12+436/436 PASS, tsc clean. §3.1.2. **v2 Phase 4 (2026-05-31, Opus 4.8, pending commit)** — per-element override data model: NEW `envelopeFunction?: 'exterior'|'interior'` (undefined=auto) σε τοίχο+κολώνα+δοκάρι = χειροκίνητη παράκαμψη (Revit-style, Στρ.3) της auto ταξινόμησης. **FULL SSOT** (Giorgio): ξεχωριστό από το δομικό `wall.category`· per-element (όχι per-region — derived geometry χωρίς σταθερό ID)· undefined=auto (όχι 'auto' literal). Mirror P7A: type SSoT (`thermal-envelope-types`) + Zod SSoT (`thermal-envelope.schemas` `EnvelopeFunctionSchema` πριν `.strict()`) + audit scalar (μηδέν νέο AuditEntityType). 9 mod+1 test ext, 21/21+112/112 PASS, tsc clean (μηδέν any). §3.1.3. ΟΧΙ slabs/openings (διαφορετικός άξονας ζώνης). **v2 Phase 5A (2026-05-31, Opus 4.8, pending commit)** — NEW engine `envelope-shell.ts` `computeEnvelopeShell` ενώνει Φ2+Φ3+Φ4 και **αντικαθιστά** το `computeEnvelopePerimeter` ως πηγή της ορατής μόνωσης (Giorgio: «νέο σύστημα οδηγεί» + «πλήρες override τώρα»). Μοντέλο ring→runs→offset· έξοδος `EnvelopeChain[]` verbatim (drop-in). Override: `interior`=αφαιρετικό (κενό στη γραμμή)· `exterior`=orphan-wrap only (δικό του τύλιγμα, μηδέν double-wrap). Offset: try-both-signs + pick-by-centroid (sign αναξιόπιστο σε notched γωνίες). `+EnvelopeChain.beamIds?` + extract `polylinePerimeterMeters`→`polygon-utils` (SSoT). Αυτόνομο (wiring 3 consumers=Φάση 5B). 1 new+1 test(18)+2 mod, 18/18+472/472 PASS, tsc clean. §3.1.4. **v2 Phase 5B (2026-05-31, Opus 4.8, pending commit)** — wired 3 consumers + BOQ στο `computeEnvelopeShell` (αντικαθιστά `computeEnvelopePerimeter` πλην IFC P9). 2 gates στον classifier (Giorgio Plan Mode): hole-gate (component με ≥1 τρύπα μονώνεται· αλλιώς νέος ρόλος `open-structure`) + no-slab-data→δωμάτιο default (μηδέν regression· αίθρια=Φ5C). Engine `computeEnvelopeShell` **άθικτος** (οδηγείται από `classified.insulated`). Applicator proximity→`chain.columnIds/beamIds` (open point #2). Consumers χωρίς `enclosesRegion` filter (engine authoritative· τα ανοιχτά runs `interior` override ζωγραφίζονται). +`collectEnvelopeOverrides` SSoT helper. 6 mod+4 test mod, 213/213 envelope+footprint + 32/32 BimSceneLayer PASS, tsc clean, ADR-040 staged. §3.1.5. | 2026-05-29 | BIM / Building Envelope — Thermal Insulation | [📄](./adrs/ADR-396-bim-external-thermal-envelope-etics.md) |
| **ADR-397** | BIM Grip & Glyph Behavior System (FULL SSoT) — Wall reference → Column parity | 🟢 **IMPLEMENTED P1+P2+P3** 2026-05-29 (pending commit) — Βαθιά μελέτη wall grip/glyph/hot-grip (4 parallel explorers) → γενίκευση σε FULL SSoT, μετά column parity. **P1** shared infra: NEW `grip-glyph-registry.ts` (`GRIP_GLYPH_REGISTRY`+`gripGlyphShape`, collapse `wall/stairGripGlyphShape`) + `grip-math.ts` (`project2D`/`perpUnit`/`unitVector`/`rotatePointAround`) + `bim-rotate-hotgrip-store.ts` (`BimRotateHotGripStore`, ex-Wall) + generalized `wall-hot-grip-fsm.ts` (`HOT_GRIP_OP_REGISTRY`+`hotGripOpForKind(string)`+`hotGripKindOf`). **P2** κολώνα: `ColumnRenderer.getGrips` +`shape` (move/rotation glyphs) + entity-agnostic `grip-mouse-handlers` (`hotGripKindOf`) + `rotateAroundPivot` 6-click orbit + `commitColumnGripDrag` rotate-store read. **P3** NEW `add-column-to-scene.ts` SSoT (DRAW+COPY share) + `commitColumnCopy` + `commitHotGripCopy` dispatcher + Ctrl-COPY block generalized. Κολώνα = πλήρης parity (move 3-click, rotate 6-click reference, Ctrl-COPY). **SSoT corrections (§12c):** `rotatePointAround` ήταν διπλότυπο→χρησιμοποιεί `rotatePoint` (ADR-188 SSoT)· beam project2D/perpUnit dedup· `append-entity-to-scene.ts` SSoT (addColumn+appendAndBroadcast delegate). **4 runtime fixes (§12d, Giorgio test — κολώνα δεν επιλεγόταν/μόνο move glyph):** (1) `computeDxfEntityGrips` +`case 'column'` (ΗΤΑΝ κενό→μηδέν interactive grips)· (2) mmScaleFor scaling grip positions (off-screen σε metre scene)· (3) HitTestingService column geometry fallback (selection)· (4) resizeWidth/Depth ÷s. **5 preview-pipeline fixes (§12e, «column rotation δεν συμπεριφέρεται σωστά» — commit δούλευε αλλά ΜΗΔΕΝ live ghost):** η κολώνα έλειπε από ΟΛΟ το ghost-preview SSoT — `apply-entity-preview.ts` (+column branch move/rotate/resize)· `EntityPreviewTransform` (+columnGripKind)· `buildRotateReferencePreview` (+columnGripKind)· `useGripGhostPreview` (+pass-through)· `draw-ghost-entity.ts` (+case 'column' footprint). Τωρα ghost===commit. **2 persistence fixes (§12f, «περιστροφή→snap-back στην αρχική»):** το `useColumnPersistence` ΗΤΑΝ partial copy του wall — έλειπε (1) `lastSavedParamsRef` seed στο subscribe (Firestore-loaded column ποτέ «known»→auto-save skip→snapshot revert) + (2) `persist()` πάντα setDoc αντί isNew?setDoc:updateDoc (createdAt reject). Fixed κι οι δύο + ΙΔΙΟ bug στο `useBeamPersistence` (Boy Scout). pivot preview/commit = μία πηγή (το bug ήταν 100% persistence). tsc clean. 531/531 tests PASS (+3 column-ghost). **Deferred:** variant L/T/I resize-drag scaling· `calculatePriority` column case· `BimPersistenceStateStore.setColumn`· column/beam persistence Firestore-mock tests. | 2026-05-29 | Drawing System / DXF Viewer / BIM | [📄](./adrs/ADR-397-bim-grip-glyph-behavior-ssot.md) |
| **ADR-399** | DXF Viewer: Building Floor Navigation Tabs + Auto-Provisioned Levels | 🟢 **IMPLEMENTED** 2026-05-30 (pending commit) — Οριζόντια μπάρα καρτελών ορόφων κάτω από το status bar· μία καρτέλα/όροφο κτιρίου (SSoT `useFloorsByBuilding`), ενεργή ο εισαχθείς όροφος, υπόλοιπες κενές+πλοηγήσιμες. Lazy provisioning level ανά όροφο μέσω ADR-286 gateway (idempotent, in-flight guard)· visibility gate μόνο για `floorplanType='floor'`+buildingId· ετικέτες ADR-369 (`generateAutoLongName`, σειρά κάτω→πάνω). NEW `useFloorTabs`+`FloorTabBar`, MOD `NormalView`+`useFloorsByBuilding`(additive longName/kind)+i18n. 10/10 tests PASS, tsc clean. **+Phase B+C 3Δ multi-floor (2026-05-30, pending commit):** καρτέλα «Όλοι οι όροφοι» (first-left) → `floor3DScope='all'` → στοιβαγμένο κτίριο κατά elevation (`BimSceneLayer.syncMultiFloor`, reuse converters `floorElevationMm`)· per-tab checkbox ορατότητας (κοινό SSoT `floorVisibilityModes`). NEW `multi-floor-3d-source`+`bim3d-resync`(scope-aware SSoT, 6 call sites centralized)+`useFloors3DAggregator`+`use-bim3d-multifloor-sync`. 42/42 tests, tsc clean. 🔴 browser verify. **+Phase D 2Δ underlay (2026-05-30, pending commit):** «Όλοι» σε 2Δ → overlay όλων των κατόψεων DXF (AutoCAD xref): ενεργός όροφος editable, άλλοι ξεθωριασμένοι+read-only· per-floor checkbox κοινό SSoT `floorVisibilityModes`. NEW read-only micro-leaf `FloorUnderlayOverlay`(z-[5], `DxfRenderer`+`destination-out` fade, selection-isolated)+`useFloors2DUnderlay`(mirror 3D aggregator)· `convertSceneToDxf` SSoT export· `onSelectAllFloors` δεν enter-raster σε 2Δ. ADR-040-safe (καμία αλλαγή bitmap cache key/orchestrator sub). 5/5+16/16 tests. 🔴 browser verify. ⚠️ pending-ratchet: dedup sourcing με 3D aggregator. | 2026-05-30 | DXF Viewer — Levels & Navigation | [📄](./adrs/ADR-399-dxf-floor-navigation-tabs.md) |
| **ADR-400** | DXF Viewer: Viewport State Persistence (URL deep-link + localStorage) | 🟢 **IMPLEMENTED** 2026-05-30 (pending commit) — Page refresh / shared link διατηρεί pan/zoom + ενεργό όροφο αντί για πάντα fit-to-view. URL schema `?s&ox&oy&lvl` (`history.replaceState`, μηδέν re-render, 60fps) + per-`fileRecordId` localStorage fallback. NEW SSoT `services/viewport-persistence.ts` + `hooks/canvas/useViewportUrlSync.ts` (debounced 400ms `TransformStore.subscribe`, ADR-040-safe — όχι `useSyncExternalStore`)· restore-vs-fit στο `useAutoFitOnFileChange`· `canvas-restore-viewport` event στο `useFitToView`· one-shot URL level restore στο `LevelsSystem`. 18/18 tests, tsc clean. 🔴 browser verify. | 2026-05-30 | DXF Viewer — Navigation & Persistence | [📄](./adrs/ADR-400-viewport-state-persistence.md) |
