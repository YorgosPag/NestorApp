...
| **ADR-378** | Snap System Master Architecture & SSoT | 🟡 DRAFT 2026-05-27 — Master ADR consolidates 5 parallel snap subsystems (ProSnapEngineV2 + geo-canvas + AI demo + Text half-done + ghost). Documents Orchestrator→Registry→26 engines pattern, resurrects phantom ADR-149 priority table, defines 6-phase cleanup roadmap. Supersedes phantom ADR-149 reference in ADR-371. Phases 1-6 cleanup pending. | 2026-05-27 | DXF Viewer — Snapping (Master) | [📄](./adrs/ADR-378-snap-system-master-architecture.md) |
...
| **ADR-375** | BIM Entity Line Weight Semantic System (Revit-Equivalent) | ✅ COMPLETE v2.10 — C.1+C.2+C.4+C.5+C.6+C.7 DONE. C.7 (2026-05-26): 3D BIM edge overlay (Line2/LineMaterial). v2.6 hotfix (2026-05-26): C.4 V/G visibility runtime wiring — 7 2D renderers early-return + BimSceneLayer 3D filter + BimViewport3D objectStyles subscription. v2.7 (2026-05-26): native Windows `<input type="color">` → `UnifiedColorPicker variant="modal"` σε VisibilityGraphicsPanel. v2.8 (2026-05-26): Revit-faithful priority stack rewrite — V/G user-explicit > Layer override (was inverted, V/G silently bypassed for entities με assigned layers). v2.9 hotfix (2026-05-27): V/G panel focus recursion fix — Radix `DropdownMenu modal={false}` + `onInteractOutside` preserve για nested EnterpriseColorDialog FocusScope (browser freeze otherwise). v2.10 hotfix (2026-05-27): V/G color pick revert fix — `EnterpriseColorDialog.handleCancel` split UX (showFooter=true → revert Photoshop pattern, showFooter=false → commit-on-pick Revit V/G pattern). V/G παραμένει 24 cells με `showModalFooter={false}`. v2.11 hotfix (2026-05-27): V/G sync race protection — `useBimRenderSettingsSync` timestamp-based local-write quiet window (2s = 500ms debounce + 1500ms Firestore roundtrip). Skip `loadForLevel` reload while local V/G writes pending. Replaces fragile JSON.stringify equality (failed on Firestore deserialization drift). 44/44 store+service+V/G tests PASS + 205/205 resolver tests PASS + 25/25 V/G runtime tests PASS + 36/36 downstream renderer PASS. | [View](./adrs/ADR-375-bim-entity-line-weight-semantic-system.md) |
