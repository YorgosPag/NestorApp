...
| **ADR-358** | DXF Stair Tool: Google-Level Parametric Staircase System | ✅ APPROVED | 2026-05-25 | DXF Viewer / Buildings | [📄](./adrs/ADR-358-dxf-stair-tool-google-level.md) |
...
| **ADR-363** | BIM Drawing Mode (Parametric Building Elements) | ✅ 🟢 **FULLY IMPLEMENTED** 2026-05-21 — Phases 0-8 complete. Wall/Opening/Slab/Column/Beam tools, Phase 5.6 Wall Split, Phase 6 BOQ auto-feed, Phase 7.1-7.2 multi-select + bulk edit, Phase 8 schedule export. **2026-05-25 Phase 8 Column Shapes A+B+C+D**: 3 νέα column kinds (polygon / shear-wall / I-shape) — types/geometry/validator/renderer/hatch + anchors+grips+snap parity + ribbon kind selector with conditional polygon/I-shape numeric inputs (drawing-mode + selected-entity bridge via columnToolBridgeStore). **2026-05-25 Phase 8E Section Catalog Presets**: section-catalog.ts (5 Eurocode 2 RC + 10 EN 10025-2 IPE/HEA), ColumnParams.catalogProfile persist, Revit-style Custom sentinel, 2 visibility-gated catalog ribbon panels, TSC PASS, 23/23 tests. Commit chain A→B→C→D→E pending. **2026-05-25 Phase 2 canvas-wiring follow-up**: opening tool silent-failure fix — click pipeline routing + ghost preview hook + renderer + leaf + CanvasSection wiring + scene-units thread through buildDefaultOpeningParams/buildOpeningEntity/computeOpeningGeometry/useOpeningTool. 8 modified + 3 new files. TSC PASS. **2026-05-25 (later) Phase 2 carry-over CLOSED**: scene-units thread στα 4 opening edit-path callers (`bim-readonly-hydration:132`, `useOpeningPersistence:109`, `UpdateOpeningParamsCommand:71`, `WallSplitCommand:149`) via `hostWall.params.sceneUnits ?? 'mm'` frozen-context. Pending-list είχε 11 callers — 7 false positives + 1 missing real caller. 3 new tests στο opening-geometry.test.ts (27/27 PASS). **2026-05-25 (even later) Bug 1 + Bug 2 fix**: opening selectability (hit-test polygon containment + child-over-parent priority — 5 files: hit-test-entity-tests, hit-tester-utils, OpeningRenderer, WallRenderer) + opening 3D cutouts (per-segment front-face re-extrude — 6 modified + 1 new utility: wall-opening-extrude.ts, BimToThreeConverter, Bim3DEntitiesStore, BimSceneLayer, BimViewport3D, OpeningPersistenceHost, Bim3DReadOnlyOverlay). IFC IfcRelVoidsElement pattern, zero new deps, zero three upgrade. 15/15 new tests PASS. TSC clean. **2026-05-25 (final) Bug 1 v2**: opening wrapped ως DxfOpening αλλά HitTestingService grouped το με direct entities (wall/column/beam) → BoundsCalculator έπαιρνε wrapper χωρίς geometry → opening εξαφανιζόταν από spatial index → wall πάντα κέρδιζε. Fix: case 'opening' σε ξεχωριστό branch με unwrap (mirror slab/slab-opening). 1 file. **2026-05-25 (final+1) Bug 3 — Door leaf line**: η πόρτα στην κάτοψη έδειχνε μόνο dashed swing arc — λείπε η solid leaf line (door panel σε 90°-open). OpeningGeometry +hingeAnchor/hingeAnchor2 fields, buildHingeArc returns full structure, OpeningRenderer.drawHingeArc draws arc (dashed) + leaf line(s) (solid). Industry-standard AutoCAD/Revit pattern. 3 files. 27/27 tests still PASS. **2026-05-25 (even later+1) Phase 8F Permanent Dimension Labels**: Revit-style centred pill label on column footprint when selected OR hovered. `formatColumnDimLabels` SSoT (7 kinds + catalogProfile prefix), `drawColumnDimPill` centred pill, `COLUMN_LABEL_MIN_FOOTPRINT_PX` threshold. canvas-pill.ts SSOT extracted (N.0.2: pillPath dedup). 18/18 tests PASS. Commit chain A→B→C→D→E+F pending. **2026-05-25 (final+2) Bug 4 — Hit-test opening leaf line + swing arc**: hover στο leaf line ή swing arc → opening ΔΕΝ αντιλαμβανόταν. `hitTestOpening` 1-branch → 3-branch (outline + leaf line + arc chord iteration). 1 file, 5 new tests (14/14 PASS). **2026-05-25 (final+3) Bug 4b — Opening bbox expansion**: spatial pre-filter (Bounds.ts `calculateBimEntityBounds`) χρησιμοποιούσε ΜΟΝΟ `geometry.bbox` που κάλυπτε ΜΟΝΟ outline vertices (y:-125..125) → cursor στο arc (y≈900) → bbox miss → `hitTestOpening` δεν καλείται. Fix: `opening-geometry.ts` `bboxPoints=[...outline,...arc.points]`. 38/38 PASS, TSC exit 0. | 2026-05-17 | DXF Viewer — BIM / Parametric Building Modeling | [📄](./adrs/ADR-363-bim-drawing-mode.md) |
...
| **ADR-366** | 3D BIM Viewer & Photorealistic Rendering | ✅ 🟢 **PHASES 0-8.1 FULLY IMPLEMENTED** 2026-05-21 + 🔵 **GROUP C RESEARCH CLOSED 7/7** 2026-05-22 + 🟢 **PHASE 9 — C.7 COMPLETE 5/5** (C.4 ✅ 2026-05-22, C.5 ✅ 2026-05-22, C.3 ✅ 2026-05-22, C.6 ✅ 2026-05-22, C.2 ✅ 2026-05-24, **C.7 Q1+Q2+Q3+Q4+Q5 ✅ 2026-05-24**) — **C.7 Q2 ✅ Session 4 2026-05-24** (super-admin `/admin/bim-diagnostics` route + FSM triage + Recharts aggregates + CSV export + audit; `PATCH /[id]` status+assignee + `PUT /[id]/notes` GitHub-Issues pattern; performance_diagnostics schema extension + restricted Firestore update rule). **C.1 split σε 3 sub-phases 2026-05-25**: **C.1.a Logic Foundation ✅ 2026-05-25** (animation-types + AnimationStore + TurntablePathBuilder + WaypointPathBuilder + keyframe-interpolator + 8 easing presets registry + bim-animations.service + SSoT extensions anm_bim_/rnj_bim_ + RBAC + audit type + Firestore rules + 5 test suites), **C.1.b UX/Timeline ✅ 2026-05-25** (TimelineEditor.tsx + TimelineWaypointForm.tsx + WaypointDragHandle.ts + contextual-animation-tab.ts + AnimationStore +toolActive (SSoT mirror BimDimensions3DStore, ΟΧΙ ViewMode3DStore) + Floating3DPanel 7th tab + ribbon-contextual-config trigger + ThreeJsSceneManager renderer lifecycle + useDxfViewerCallbacks 5 animation actions + i18n el/en bim3d.animation.* ~50 keys × 2 locales. **drag interaction ✅ 2026-05-25** (waypoint-drag-controller pure FSM + camera-aligned drag plane + use-waypoint-drag-interaction React hook με DOM pointer listeners gated on toolActive + WaypointDragHandle setHoverState + 17 new tests). **§C.1.Q4 Bezier 4-point editor ✅ 2026-05-25** FULL ENTERPRISE — 5 new + 4 modified + 23 new tests. viewport/bezier-easing.ts pure cubic-bezier Newton-Raphson + De Casteljau, presets/preset-bezier-defaults.ts CSS-standard mapping 8 presets, BezierCurveEditor.tsx Chrome-DevTools-style SVG editor (drag handles role="slider" ARIA + Tab focusable + Arrow ±0.01 / Shift+Arrow ±0.1 + 4 numeric inputs P1xy/P2xy + 8 preset chips quick-swap + reset + live preview ball RAF 2s loop + overshoot Y∈[-1,2] για back/elastic). Waypoint +customBezier?: BezierControlPoints optional (no migration). getEasingFunction(id, customBezier?) overload. interpolator passes through. Native `<details>` semantic expander. i18n +12 keys el/en. **+real scene-bbox turntable ✅ 2026-05-25** (SceneBboxProvider register/getter bridge bim-3d↔app subsystems + ThreeJsSceneManager.getSceneFramingBounds() SSoT REUSE του computeSceneFramingBounds + BimViewport3D mount/unmount lifecycle + useDxfViewerCallbacks resolveTurntableBbox helper με graceful camera fallback. 7 new tests). **+TimelineEditor/TimelineWaypointForm/WaypointDragHandle React component tests ✅ 2026-05-25** (3 suites + 25 tests όλα PASS — 13 για TimelineEditor + 6 για WaypointForm + 6 για DragHandle. Boy Scout production bugfix στο TimelineEditor.tsx: αντικατάσταση `useAnimationStore(selectAnimationConfig)` με ατομικά selectors → απέτρεψε infinite re-render loop)), **C.1.c Rendering/Queue ✅ 2026-05-25** (MP4Exporter WebCodecs H.264+VP9 fallback + mp4-muxer@5.2.2 MIT + RenderQueueStore FIFO + RenderQueuePanel 8th tab + animation-queue-processor glue + render-checkpoint + animation-action-handlers + auto-save flow + Save+Export ribbon activation + Storage rules block + 3 test suites + 4 ADR drift notes resolved). **Animation Phase 9 FULLY CLOSED** + **Group B Custom HDRI Upload ✅ 2026-05-25** (HdriUploader + hdri-upload.service + EnvironmentStore.customHdri* + storage.rules bim_environments + i18n custom.* — closes Group B deferred research item, zero rendering code changes, full GOL 7/7). **+Snap-to-Grid Waypoints ✅ 2026-05-25** (snap-quantizer.ts + AnimationStore snapEnabled/snapStepUnits + writeWaypointPosition quantize + ribbon toggle+combobox panel + useRibbonCommands bridge + i18n el+en — closes C.1.b deferred items 2/2). **+Axis-constrained drag gizmo ✅ 2026-05-25** (axis-constraint-projector.ts + AnimationStore.dragAxisLock + WaypointDragHandle gizmo 3 arrows + keyboard X/Y/Z + arrow click + i18n el+en + 10 tests — closes last deferred C.1.b item, Phase 9 zero deferred). ADR-366 total estimate: ~254-303h Phases 0-9. **Bugfix SSAO black-screen ✅ 2026-05-25**: BimSSAOPass subclass hides LineSegments during normal-buffer pass (SSAOPass.overrideMaterial=MeshNormalMaterial on LineSegments → garbage AO → black). _hasMesh cache in BimSceneLayer. All [3D-DEBUG] logs removed (6 files). | 2026-05-19 | DXF Viewer — 3D Rendering / Photorealistic Output | [📄](./adrs/ADR-366-3d-bim-viewer-photorealistic-rendering.md) |
...
| **ADR-370** | BIM Read-Only Visualization in Properties Floorplan Tab | ✅ **IMPLEMENTED** 2026-05-20 + Phase 4 stairs 2026-05-25 + Phase 5 3D stair parity 2026-05-25 + Phase 6 slab-opening 2D 2026-05-25 + **Phase 7 3D slab-opening parity 2026-05-25** (THREE.Shape.holes + ExtrudeGeometry, IFC IfcOpeningElement pattern) | 2026-05-20 | DXF Viewer — Read-Only Visualization / 3D Parity | [📄](./adrs/ADR-370-bim-readonly-visualization.md) |
...
| **ADR-375** | BIM Entity Line Weight Semantic System (Revit-Equivalent) | ✅ Phase A + B.1 + B.2 + B.3 IMPLEMENTED 2026-05-25 + 🩹 RUNTIME VERIFIED 2026-05-26 (6 hotfixes v1.6-v1.9: bitmap cache invalidation, 6 renderers ViewRange wiring, Firestore undefined rejection, **v1.9 StairRenderer + OpeningRenderer secondary-line coverage** — StairRenderer.render() μία φορά cutState+baseLineWidth + προωθεί σε StairStyleContext +baseLineWidth field + 8 hardcoded weights σε stair-render-structure-style.ts + 4 OpeningRenderer secondary cues hingeArc/leaf/sliding/glazing). Phase A: 4 SSoT + 7 renderers + 35 tests. B.1: Drawing Scale Selector. B.2: `bim-render-settings-store` SSoT + ViewRange/ObjectStyles panels. B.3: View Templates library (`dxf_viewer_view_templates`, snapshot-copy, vtmpl_ ID). **92 tests PASS** + 16/16 stair-render-structure-style PASS (test factory updated). ⏸️ Phase C DEFERRED (~25-40h, 7 sub-phases C.1-C.7: Pen Table editor / Pen Sets presets / Subcategories / per-view+per-element overrides / Layer integration / 3D parity ADR-370). | 2026-05-26 | Canvas & Rendering / BIM Drawing | [📄](./adrs/ADR-375-bim-entity-line-weight-semantic-system.md) |
| **ADR-376** | Opening Tags (Revit-Faithful Pattern) — instance Mark + per-kind prefix + floor-prefix hundreds + BOQ signature aggregation | ✅ Phase A + B.1 + B.2 IMPLEMENTED 2026-05-26 — Phase A: OpeningMarkService + OpeningTagRenderer + Layer SSoT + ribbon Mark field + AnnotationsSection toggle + migration script. Phase B.1 (v6): Renumber Openings command (IMAGINiT-style) + ICommand undo + Dialog modal + Annotate ribbon tab + contextual quick-action + `markIsManual` flag (5/5 convergence). Phase B.2 (v7): single aggregated BOQ row per Mode C signature group (kind+width+height+sillHeight+openDirection), 6/6 industry convergence (Revit Schedule pattern), per-floorplan scope, mark range compaction in description, drift fix (originally "mirror walls" — corrected: openings are atomic). 79 tests (38 Phase A + 12 Phase B.1 + 29 Phase B.2). Phase C split (v8 2026-05-26 Giorgio scope decision) → C.1 (draggable tag + leader, ~3-5h) + C.2 (per-project styling, ~2-3h) + C.3 (PDF schedule export, ~2-3h) — separate ~2h sessions each. Phase C.1 ✅ DONE 2026-05-26 (v9): γωνιακή/elbow leader Revit 2027 + ArchiCAD, tag πάντα horizontal (3/3 convergence), Reset Position ribbon + right-click (Vectorworks UX), pure FSM controller + React leaf hook + RAF-coalesced scene patches, 28/28 tests PASS. Phase C.2 ✅ DONE 2026-05-26 (v10): per-project Tag Style override — 6 fields (fontSize/borderWidth/leaderStyle/pillBgColor/leaderColor/leaderVisible), 3/3 industry convergence (Revit shared params + ArchiCAD .PRF + AutoCAD DIMSTYLE), Annotate ribbon "Tag Style" button + Radix modal με Slider/Select/Color/Switch controls, debounced 200ms live preview (Figma/Photoshop pattern), `Project.openingTagStyle` Firestore field, pure singleton service + leaf Host, 22+9=31 tests PASS. Phase C.3 ✅ DONE 2026-05-26 (v11): combined PDF schedule export (Πίνακας Θυρών + Παραθύρων, 1 PDF 2 sections), jsPDF+autoTable A4 landscape SSoT mirror ADR-363 Phase 8, mark column added to door+window presets (industry standard), ribbon "Πίνακας PDF" button + FileDown icon + EventBus, OpeningSchedulePdfHost headless leaf, 6 new tests PASS (104 total schedule suite). **ADR-376 COMPLETE — ALL PHASES SHIPPED.** | 2026-05-26 | DXF Viewer — BIM / Annotation / BOQ | [📄](./adrs/ADR-376-opening-tags.md) |
...
| **ADR-375** | BIM Entity Line Weight Semantic System | ✅ Phase A + B IMPLEMENTED + RUNTIME VERIFIED 2026-05-26 · ⏸️ Phase C DEFERRED | [View](./adrs/ADR-375-bim-entity-line-weight-semantic-system.md) |
