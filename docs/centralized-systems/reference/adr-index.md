...
| **ADR-511** | Wall Finish per Room/Face (beyond-Revit): ΕΝΑΣ δομικός τοίχος (SSoT, άθικτος) + N περιοχές `wall-covering` (IfcCovering CLADDING/INTERIOR) ανά δωμάτιο/παρειά, όρια από IfcSpace (ΟΧΙ κολώνες, ΧΩΡΙΣ split). Compound layered assembly (μπογιά surface 0πάχος + σοβάς/knauf/πλακίδια body). Magic: auto-πρόταση φινιρίσματος ανά χρήση δωματίου (useType). Mirror floor-finish ADR-419. | 🟡 Slice A (data+persistence) DONE — 10 jest GREEN, indexes DEPLOYED, UNCOMMITTED· Slices B(render+tool+ribbon)/C(room-auto-extent)/D(3D)/E(BOQ) PENDING. 🔴 tsc+browser-verify+commit | [View](./adrs/ADR-511-wall-finish-per-room.md) |
