...
**📊 Stats**: 334 ADRs | Last Updated: 2026-05-26
...
| 📄 **Uncategorized** | 132 | [View](#uncategorized) |
...
| **ADR-363** | BIM Drawing Mode (Parametric Building Elements) | ✅ 🟢 **FULLY IMPLEMENTED** 2026-05-26 — Phases 0-8 complete + Phase 1F strict 3-click lateral alignment for straight walls (`awaitingAlignment` phase, `computeWallAlignmentOffset` SSoT, preview shifts toward cursor, wall-trims unit-mismatch hotfix) + v12.2 bugfix (Firestore UPDATE permission error). | 2026-05-26 | DXF Viewer — BIM / Parametric Building Modeling | [📄](./adrs/ADR-363-bim-drawing-mode.md) |
...
| **ADR-375** | BIM Entity Line Weight Semantic System (Revit-Equivalent) | ✅ COMPLETE v2.5 — Phase A+B+C ALL DONE (C.1+C.2+C.4+C.5+C.6+C.7). C.7 (2026-05-26): 3D BIM edge overlay parity. Line2/LineMaterial industry stack (Forge/Three.js Editor). 4 NEW edge module files + 5 MODIFIED converters/scene-manager. EdgesGeometry 30° (Revit silhouette). 38/38 edge tests PASS + 76/76 2D regression PASS. | 2026-05-26 | DXF Viewer — BIM / Rendering | [📄](./adrs/ADR-375-bim-entity-line-weight-semantic-system.md) |
...
| **ADR-377** | BIM Subcategories System | 🟡 ACTIVE v0.7 — Phase A+B+C.1+C.2+C.3 DONE (Wall/Slab/SlabOpening+Beam/Column+Opening/Stair wired, 45 tests). v0.7 (2026-05-27) cross-ref: latent persistence gap (subcategories silently stripped από `/api/dxf-levels` schema) closed via ADR-375 v2.13 (`SubcategoryStyleSchema` mirror). D+E+F pending. | 2026-05-27 | DXF Viewer — BIM / Rendering | [📄](./adrs/ADR-377-bim-subcategories-system.md) |
| **ADR-379** | BIM Entity Audit Coverage Fix | ✅ **DONE** 2026-05-27 — Δύο root bugs fixed: (A) `/api/audit-trail/record` ownership check έτρεχε `entityDoc.exists` ΜΕΤΑ το client-side delete → 404 → silent swallow → **κανένα BIM delete audit row**. Fix: action='deleted' bypass με defense-in-depth caller-companyId tagging. (B) 5 BIM audit-clients (wall/column/slab/beam/opening) δέχονταν μόνο `Pick<_, 'id'\|'kind'>` και έγραφαν `[{ field: 'kind' }]` placeholders αντί για πραγματικό field-level diff. Refactor: νέο SSoT `bim-audit-helpers.ts` + 5 νέα tracked-fields registries (WALL/COLUMN/SLAB/BEAM/OPENING) + persistence hooks πλέον περνούν `prevParams` + full snapshot σε delete. 27/27 tests PASS. 2 new + 12 modified files. Phase F (ADR write + cross-refs) pending commit. | 2026-05-27 | DXF Viewer — Persistence / Audit | [📄](./adrs/ADR-379-bim-entity-audit-coverage.md) |
| **ADR-380** | Stair + Slab-Opening Audit Coverage | ✅ **DONE** 2026-05-27 — Δύο BIM entity types εκτός ADR-379: (A) `stair` — ZERO audit coverage (no client + missing από `AuditEntityType` union + missing από route `VALID_ENTITY_TYPES`). (B) `slab-opening` — partial coverage με `[{ field: 'kind' }]` placeholder pattern. Fix: type union + route enablement + 2 νέα tracked-fields registries (STAIR/SLAB_OPENING) + 1 νέο audit-client (stair) + 1 refactor (slab-opening) + persistence hooks wiring. Total 4 new + 6 modified files. SSoT registry `bim-audit-helpers` επεκτάθηκε με +2 forbidden patterns (buildStairChanges + buildSlabOpeningChanges). 7/7 BIM entities πλέον audit-covered. Pending commit bundled με ADR-379. | 2026-05-27 | DXF Viewer — Persistence / Audit | [📄](./adrs/ADR-380-stair-slab-opening-audit-coverage.md) |
| **ADR-381** | DXF Viewer Subsystem Duplication Audit (Master) | 🟢 **RESEARCH COMPLETE** 2026-05-27 — 7-parallel-domain audit (Opus orchestrator + 7× Haiku Explore) σε `/dxf/viewer`. 2 clean (stores, transforms), 2 medium (commands/geometry), 3 critical (visibility, BIM lifecycle, preferences). 🔴 **Suspected production bug**: `layer.visible` ΔΕΝ ελέγχεται στο 2D render path + ignored στο 3D → 2D/3D visibility divergence. 🔴 **50% του BIM codebase είναι copy-paste** (audit-clients consolidated ADR-379/380, υπόλοιπα 5 pillars εκκρεμούν). 🔴 **7 παράλληλα preference services + 3 race-protection patterns** + triple-source για Line/Text General. Roadmap: 9 sub-ADRs (ADR-382→390) με priority C1-C3 critical + M1-M4 medium + L1-L3 low. Total ROI ~1500+ LOC + 1 production bug fix + 4 νέα SSoT establishments. Implementation pending επόμενες συνεδρίες. | 2026-05-27 | DXF Viewer — Architecture Audit (Master) | [📄](./adrs/ADR-381-dxf-viewer-duplication-audit-master.md) |
| **ADR-378** | Snap System Master Architecture & SSoT | ✅ **COMPLETE** 2026-05-27 — Master ADR consolidates 5 parallel snap subsystems → 1 production singleton. Documents Orchestrator→Registry→26 engines pattern, resurrects phantom ADR-149 priority table, defines 6-phase cleanup roadmap. **ALL Phases 1+2+3+4+5+6 DONE 2026-05-27** (P1: AISnappingEngine + useProSnapShortcuts deleted; P2: pro-snap-engine.ts ghost deleted + index.ts re-export removed; P3: TextSnapEngine ADR-344 6.C completion — 177 LOC engine + 226 LOC test (14 cases PASS) + Registry wire + SnapIndicatorOverlay ▣ nested-square + i18n el/en + ProSnapToolbar TEXT key + type system commits 7f788d8d/aabfb04f; **P4: geo-canvas unification — NEW parser-result-to-entities.ts adapter (108 LOC, GeoJSON→Entity[]) + REFACTORED useSnapEngine.ts (~190 LOC façade over getGlobalSnapEngine with EXTENDED_TO_GEO_MODE 6-mode mapper, public API surface preserved) + DELETED 5 legacy files (SnapEngine.ts 275 LOC + endpoint-detector 202 + snap-distance 139 + engine/index + useSnapPoints zero-callers), property floorplan tab gains 26 modes vs 6, tsc zero new errors, jest 188 pass / 2 pre-existing unrelated fails**; P5: 12 stale doc/code references updated across 11 files incl. ADR-370/153/359/034/092/362 + 3 DXF docs + storage-utils orphan const + snap-engine-utils stale comment). Supersedes phantom ADR-149 reference. **Phase 6 (SSoT registry hardening): `snap-engine` module +2 forbidden patterns (`new SnapEngine(` + `class AISnappingEngine`) + description rewrite + addedByAdr→ADR-378 + allowlist __tests__ glob + baseline refreshed. ADR-378 fully shipped.** | 2026-05-27 | DXF Viewer — Snapping (Master) | [📄](./adrs/ADR-378-snap-system-master-architecture.md) |
...
| **ADR-375** | BIM Entity Line Weight Semantic System (Revit-Equivalent) | ✅ COMPLETE v2.13 — C.1+C.2+C.4+C.5+C.6+C.7 DONE. **v2.13 ROOT-CAUSE FIX (2026-05-27)**: server-side schema strip — `UpdateDxfLevelSchema.objectStyles[category]` validated μόνο `projectionPen/cutPen` και Zod `.strip()` διέγραφε σιωπηρά ΟΛΟ το V/G surface (visible/projection&cutColor/projection&cutPattern + ADR-377 subcategories) από κάθε Firestore write. Snapshot listener delivered stripped doc → store wipe μετά το quiet-window. Fix: full reusable named sub-schemas (PenIndexSchema/HexColorSchema/LinePatternSchema enum+custom_regex/SubcategoryStyleSchema/ObjectStyleSchema/BimRenderSettingsSchema) με enterprise validation (6-digit hex regex, 29 built-in patterns + `custom_*` prefix, PenIndex 1-16). Επίσης fix latent template apply/propagate path (ίδιο gateway). 13 νέα schema tests + 44/44 BIM regression = 57/57 PASS. Diagnostic logs αφαιρέθηκαν, quiet-window guard διατηρήθηκε ως defense-in-depth. C.7 (2026-05-26): 3D BIM edge overlay (Line2/LineMaterial). v2.6 hotfix: C.4 V/G visibility runtime wiring. v2.7: UnifiedColorPicker swap. v2.8: Revit-faithful priority stack (V/G > Layer). v2.9: focus recursion fix (Radix modal=false + onInteractOutside). v2.11: V/G sync race quiet-window guard. v2.12: V/G body fill tint SSoT 6-renderer sweep. | [View](./adrs/ADR-375-bim-entity-line-weight-semantic-system.md) |
| **ADR-377** | BIM Subcategories System | 🟡 ACTIVE v0.7 — C.1+C.2+C.3 DONE (45 tests). v0.7 (2026-05-27) cross-ref: persistence gap closed via ADR-375 v2.13 (`SubcategoryStyleSchema` mirror — subcategories πλέον φτάνουν στο Firestore). D+E+F pending | [View](./adrs/ADR-377-bim-subcategories-system.md) |
