...
| **ADR-378** | Snap System Master Architecture & SSoT | 🟡 ACTIVE 2026-05-27 — Master ADR consolidates 5 parallel snap subsystems. Documents Orchestrator→Registry→26 engines pattern, resurrects phantom ADR-149 priority table, defines 6-phase cleanup roadmap. **Phases 1+2+5 DONE 2026-05-27** (P1: AISnappingEngine + useProSnapShortcuts deleted; P2: pro-snap-engine.ts ghost deleted + index.ts re-export removed; P5: 12 stale doc/code references updated across 11 files incl. ADR-370/153/359/034/092/362 + 3 DXF docs + storage-utils orphan const + snap-engine-utils stale comment). Supersedes phantom ADR-149 reference. Phases 3 (TextSnapEngine), 4 (geo-canvas unification), 6 (SSoT registry) pending. | 2026-05-27 | DXF Viewer — Snapping (Master) | [📄](./adrs/ADR-378-snap-system-master-architecture.md) |
...
| **ADR-375** | BIM Entity Line Weight Semantic System (Revit-Equivalent) | ✅ COMPLETE v2.12 — C.1+C.2+C.4+C.5+C.6+C.7 DONE. C.7 (2026-05-26): 3D BIM edge overlay (Line2/LineMaterial). v2.6 hotfix (2026-05-26): C.4 V/G visibility runtime wiring — 7 2D renderers early-return + BimSceneLayer 3D filter + BimViewport3D objectStyles subscription. v2.7 (2026-05-26): native Windows `<input type="color">` → `UnifiedColorPicker variant="modal"` σε VisibilityGraphicsPanel. v2.8 (2026-05-26): Revit-faithful priority stack rewrite — V/G user-explicit > Layer override. v2.9 hotfix (2026-05-27): V/G panel focus recursion fix — Radix `DropdownMenu modal={false}` + `onInteractOutside` preserve για nested EnterpriseColorDialog FocusScope. v2.11 hotfix (2026-05-27): V/G sync race protection — `useBimRenderSettingsSync` timestamp-based local-write quiet window (2s) skips `loadForLevel` reload while local V/G writes pending. v2.12 hotfix (2026-05-27): V/G body fill tint SSoT + 6-renderer sweep — νέο `bim/utils/bim-vg-fill-tint.ts` (`VG_FILL_ALPHA=0.2`, `hexToRgba`, `resolveVgFillTint`). Wall + Column + Slab + Beam + SlabOpening (canonical body fill) + Stair (tread fill via `StairStyleContext.vgFillTint`) όλοι tint με V/G category color (Revit cut-pattern background convention). 2D plan-footprint adapted priority: cutState-matching column → opposite column → hardcoded fallback. 395/395 tests PASS. | [View](./adrs/ADR-375-bim-entity-line-weight-semantic-system.md) |
