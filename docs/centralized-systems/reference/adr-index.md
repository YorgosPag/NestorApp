...
**📊 Stats**: 357 ADRs | Last Updated: 2026-06-02
...
| 📄 **Uncategorized** | 148 | [View](#uncategorized) |
...
| **ADR-363** | BIM Drawing Mode (Parametric Building Elements) | ✅ 🟢 **FULLY IMPLEMENTED** 2026-05-21 — Phases 0-8 complete. Wall/Opening/Slab/Column/Beam tools, Phase 5.6 Wall Split, Phase 6 BOQ auto-feed, Phase 7.1-7.2 multi-select + bulk edit, Phase 8 schedule export. **+ EN 10365 steel I/H sections (Φ1 columns + Φ2 beams, 75 διατομές, shared SSoT, swept 3D + BOQ kg) 2026-06-02 (🔴 browser verify).** | 2026-05-17 | DXF Viewer — BIM / Parametric Building Modeling | [📄](./adrs/ADR-363-bim-drawing-mode.md) |
| **ADR-364** | Escape Command Bus (Centralized ESC Dispatcher) | ✅ 🟢 **APPROVED** — Group 1+2+3 implemented; Boy-Scout Group 4 (10 secondary components) migrated 2026-06-03 (🔴 browser verify) | 2026-05-18 | DXF Viewer — Tools & Keyboard | [📄](./adrs/ADR-364-escape-command-bus.md) |
...
| **ADR-390** | Symmetric BIM Entity Delete/Undo Persistence | ✅ **DONE** 2026-05-27 (Φ1-3) · **Φ4 active-floor SSoT load 2026-06-14** (snapshot=derived cache → drop stale BIM στο load, per-entity docs authoritative· save unchanged για multi-floor· + beam/column param-edit persist-gap fix) UNCOMMITTED | 2026-06-14 | DXF Viewer — Commands / Persistence / Audit Trail | [📄](./adrs/ADR-390-symmetric-bim-delete-undo.md) |
...
| **ADR-449** | Structural Finish Skin (σοβάς κολόνας/δοκαριού· per-face adjacency-driven· immutable πυρήνας + derived σοβάς + BOQ ξεχωριστές γραμμές) | 🟢 Slices 1-7 (κολόνες+δοκάρια+toggle+beam↔column junction+ενιαία σιλουέτα στις συμβολές) — pending browser-verify | [View](./adrs/ADR-449-structural-finish-skin.md) |
...
| **ADR-453** | Print/Export Engine (2Δ & 3Δ → PDF / εκτυπωτή / plotter· ΕΝΑ SSoT `print/` module· 2Δ re-render offscreen [Option A] + 3Δ offscreen WebGLRenderer → κοινός jsPDF assembler → `triggerExportDownload`/`openBlobInNewTab+print`· A4-A0, fit-to-page & 1:N, title block· ribbon Ανάλυση→«Εκτύπωση») | 🟢 Slices 0-5 Implemented — 30 jest + tsc clean — pending browser-verify + commit | [View](./adrs/ADR-453-dxf-print-export-engine.md) |
| **ADR-454** | Print Plot Style (AutoCAD CTB / Revit print, **2Δ-only**· νέο SSoT `config/print-color-policy.ts` + 2 chokepoints [`DxfRenderer.resolveStyleForRender` + `resolveSubcategoryStyle`]· module singleton mirror `_activePenTable`· white-safe colour remap [λευκό/ACI7/null→μαύρο] + ISO πάχη στο πραγματικό print DPI· modes colour/monochrome/grayscale/by-pen· μηδέν leaf-renderer αλλαγές) | 🟢 Slices 0-4 Implemented — 43 print jest — pending browser-verify + commit | [View](./adrs/ADR-454-print-plot-style.md) |
| **ADR-455** | Vertical Section Cuts (X/Y)· 2 κάθετες τομές κατά DXF X/Y· ΕΝΑ SSoT `x/yAxisCut{active,position,sign}`· 3Δ = γενίκευση `SectionSceneController` single→≤3 axis cuts [ίδιο clip+caps+fast-path]· **v2 redesign**: 2Δ drag-**λαβή πάνω στη γραμμή τομής** (transform-synced· `axis-cut-grip`+`axis-cut-drag-store`+claims στους 3 mouse handlers· αντικατέστησε το Radix slider X/Y) + cut-away πλευρά = ΕΝΑ **fade-rect** [αντικατέστησε per-entity ghost· straddling πλέον σβήνει σωστά]· compact corner widget toggle/flip/readout· flip βελάκι αλλάζει πλευρά | 🟢 v2 Implemented — axis-cut jest + tsc clean — pending browser-verify + commit | [View](./adrs/ADR-455-vertical-section-cuts.md) |
| **ADR-456** | Στατικά: Ποσότητες Σκυροδέματος & Οπλισμός· SSoT `bim/structural/` [concrete-grades + rebar-catalog B500C + code providers Eurocode/ΕΚΩΣ-ΕΑΚ + reinforcement compute]· +`concreteGrade`/`reinforcement` στο ColumnParams· Slice 1 ποσότητες ορθογ. κολώνας· Slice 2 UI panel + building-level κανονισμός SSoT· **Slice 3 2Δ/3Δ σχεδίαση οπλισμού** (`column-rebar-layout` geometry SSoT → 2Δ κουκκίδες+στεφάνι scene-pass & 3Δ rebar cage· toggle `showReinforcement`)· **Slice 3b καμπύλες γωνίες στεφανιών** (EC2 rounded bends r_cl=2.5·dbw· `buildRoundedStirrupPath`/`stirrupPathMm` κοινό 2Δ/3Δ· `stirrupCenterlinePerimeterMm` geometry-is-SSoT μήκος)· geometry-is-SSoT (θέσεις re-derived) | 🟢 Slice 1+2+3+3b Implemented — pending browser-verify + commit | [View](./adrs/ADR-456-structural-quantities-reinforcement.md) |
| **ADR-457** | Column Reinforcement Detail Sheet (φύλλο σχεδίου οπλισμού κολώνας, Revit/Tekla-grade)· εντολή «Λεπτομέρεια Οπλισμού» στο contextual tab → παράθυρο 5 ενοτήτων (κάτοψη/όψη/3Δ/schedule/title-block) με πλήρη διαστασιολόγηση· **κεντρική αρχή: ΕΝΑ pure `DetailSheetModel` (sheet-mm) → ΔΥΟ backends [Canvas live-preview + jsPDF export]** ώστε preview===PDF· reuse rebar SSoT (ADR-456) + print engine (ADR-453/454)· NEW `bim/structural/detail-sheet/`· Slice 0 shell (3 στήλες: αρ ΟΨΗ/ΚΑΤΟΨΗ· μέση 3Δ full-height· δεξ schedule/title-block)· Slice 1 ΚΑΤΟΨΗ (footprint+οπλισμός geometry-SSoT+διαστάσεις `detail-sheet-dim`+κλίμακα)· Slice 2 ΟΨΗ (περίγραμμα+διαμήκεις+στεφάνια ανά τύπο hooked/welded/spiral στις στάθμες `computeStirrupLevelsMm`+lcr+height dim)· Slice 3 3Δ ΠΡΟΟΠΤΙΚΟ (self-contained offscreen WebGL: αχνό concrete prism από footprint-extrude + `buildColumnRebarCage` geometry-SSoT → isometric PNG raster· sync one-shot [unlit MeshBasicMaterial]· contain-fit SSoT canvas+PDF· progressive paint· tight projected-bbox fit· 3D W/D/H dims + bar marks #1…#N ως **2Δ overlay primitives** [camera-projected → ΙΔΙΟ `resolveDimGeometry`/βελάκια/text με ΟΨΗ/ΚΑΤΟΨΗ = FULL SSOT· raster=μόνο κολώνα+κλωβός])· ADR-040-safe (offscreen) | 🟢 Slice 0+1+2+3 Implemented (0-2 verified· 3 🔴 verify) — 39 jest — pending commit | [View](./adrs/ADR-457-column-reinforcement-detail-sheet.md) |
| **ADR-459** | **Structural Organism / Analytical Connectivity Model** (Revit-grade· συνδεδεμένες οντότητες πέδιλο→κολόνα→δοκάρι = ΕΝΑΣ στατικός οργανισμός)· **DERIVED connectivity graph** (nodes=μέλη footing/column/beam, edges=footing-bearing/column-bearing/top-attachment) re-derived σε κάθε structural μεταβολή, ΠΟΤΕ persisted (φιλοσοφία ADR-458)· REUSE `findColumnsFramedByBeam`/`resolveColumnBaseZmm`/`beamHostInput`/`attachTopToIds` (μηδέν duplicate)· Phase 1 cross-entity checks `runOrganismChecks` → **`columnMissingFooting` («λείπει το πέδιλο»)** + `beamUnsupportedEnd` + `memberIsolated`· surfacing: low-freq `StructuralDiagnosticsStore` (ADR-040-safe) + shell `useStructuralOrganism` (mirror auto-attach· coalesced recompute· emit `bim:structural-organism-updated`) + generic `EntityWarningsSection` mounted στο `ColumnAdvancedPanel`· NEW `bim/structural/organism/`· i18n el+en· **Phase 2 explicit FK κολόνα↔πέδιλο**: `ColumnParams.footingId` (αναλυτικό FK, geometry-neutral· δείχνει σε foundation pad/strip/tie-beam ή εδαφόπλακα)· 2 SSoT helpers `footing-column-coverage` (bearing κριτήριο, εξαγωγή από graph) + `footing-element-summary` (recognition+summary)· `foundation-column-attach-coordinator` (αμφίδρομος)· undoable geometry-neutral `AttachColumnFootingCommand`· αμφίδρομο auto-attach (νέο πέδιλο↔νέα κολόνα) στο `useStructuralAutoAttach`· graph **explicit-FK-wins** (+spatial fallback)· event `bim:column-footing-attached`· **Phase 4a beam reinforcement**: `BeamReinforcement` (κάτω/άνω διαμήκης+συνδετήρες+cover) στο `BeamParams.reinforcement` (+Zod strict)· provider επέκταση `beamReinforcementLimits`/`suggestBeamReinforcement` (EC2/EC8+ΕΚΩΣ/ΕΑΚ)· κοινό `resolveBarSet` (κολόνα+δοκάρι SSoT)· `beam-reinforcement-compute` derived ποσότητες· **Phase 4b footing reinforcement**: `FootingReinforcement` discriminated (pad δι-διευθυντική σχάρα / strip ανεστραμμένη δοκός / **tie-beam reuse `BeamReinforcement`**) στο `FoundationParams.reinforcement` per-kind (+Zod strict)· provider επέκταση `footingReinforcementLimits`/`suggestFootingReinforcement` (EC2/EC8+ΕΚΩΣ/ΕΑΚ)· NEW κοινό `resolveMatMesh` (spacing-based SSoT)· `footing-reinforcement-compute` (tie-beam delegate σε beam compute)· export beam schemas για reuse· **Phase 4c organism reinforcement continuity** (η ΚΑΡΔΙΑ): provider lap/anchorage SSoT `lapLengthMm`/`anchorageLengthMm` (EC2 §8.4/§8.7· eurocode 50/40·Ø, legacy 55/50· αντικαθιστά flat 50·Ø· modifiers SSoT `rebar-catalog.developmentLengthMm`)· NEW pure DERIVED `organism/reinforcement-continuity.ts` `computeOrganismReinforcementContinuity` (ανά edge **αμφίδρομα**: footing-bearing→dowels/αναμονές, column-bearing→αγκύρωση δοκαριού στον κόμβο EC8 §5.6, top-attachment→μάτισμα ορόφου· item στο byMember ΚΑΙ των 2 μελών)· continuity-aware compute (optional param, flat fallback back-compat)· **Phase 4d reinforcement διαγνωστικά + auto-apply**: NEW `organism/reinforcement-checks.ts` `runReinforcementChecks(graph,entities,provider)` (ξεχωριστή signature από geometry-only `runOrganismChecks`· `memberMissingReinforcement` info / `ratioOutOfRange` warning [ρ vs provider limits] / `barMismatchAtJoint` warning [μάτισμα κολόνας διαφορετικό πλήθος/Ø, ανεπαρκές μήκος])· types +`info` severity +3 codes +`messageParams`· surfacing wire στο `useStructuralOrganism` (active code + settings subscribe) + `EntityWarningsSection` info tone· NEW undoable `AutoReinforceOrganismCommand` (batch/idempotent/geometry-neutral· per-kind `suggest*`)· NEW `useStructuralAutoReinforce` hook (scope selection→selected/κενή→όλος ο όροφος)· ribbon «Αυτόματος Οπλισμός» (global «Ανάλυση» + contextual δοκάρι/πέδιλο parity με κολόνα· action→bridge onAction→emit→command)· boy-scout SSoT NEW `bim/structural/section-context.ts` (entity→SectionContext + `buildReinforcePatch`· column builder εξήχθη από bridge→delegate)· **Phase 4e (E1+E3)**: E1 top-attachment host μη-κολόνα → anchorage (`lbd`) αντί `null` + NEW warning `columnTopAnchorageUnverified`· E3 NEW `SlabFoundationReinforcement` (raft/εδαφόπλακα top+bottom δι-διευθυντική σχάρα· reuse `RebarMesh`/`resolveMatMesh`/mesh SSoT) + provider slab-foundation limits/suggester (EC2/ΕΚΩΣ) + `SlabParams.structuralReinforcement` (διακριτό από hint) + `buildReinforcePatch`/`reinforcement-checks` raft coverage (E2 footing-pad own-bar = SKIP)· **Phase 4f manual connectivity UX**: NEW `DetachColumnFootingCommand` (key-removal Firestore-safe, undoable) + foundation-ribbon «Συνδεσιμότητα» panel (attach selected / detach all· αποφυγή MIXED `column-command-keys.ts`) + events `bim:column-footing-attached-manual`/`-detached` + toasts + organism re-derive· DEFER Phase 3 (section adequacy advisory)/5 (loads/M-N — ξεχωριστό ADR)/footing pad own-bar continuity/full EC2 §8.4.2 lb,rqd | 🟢 Phase 0+1+2+4a+4b+4c+4d+4e+4f Implemented — 290 structural/bridge jest, tsc clean — pending browser-verify + commit (⚠️ codes/* MIXED με ADR-460) | [View](./adrs/ADR-459-structural-organism-connectivity.md) |
| **ADR-458** | Beam-to-Column **Cutback** (Revit join-geometry, «η κολόνα νικάει»)· ο πυρήνας δοκαριού κόβεται στην παρειά κολόνας που το τέμνει (net volume, overlap μετριέται μία φορά)· **DERIVED, ΠΟΤΕ persisted** (re-derive on column change· ίδια σύμβαση με σοβά ADR-449 + foundation net ADR-441 Slice 4)· ΕΝΑ pure SSoT `bim/geometry/beam-column-cutback.ts` [`computeBeamCutbackOutline`/`computeBeamCutbackNetAreaM2` via `safeDifference`+`multiPolygonArea`· bbox fast-path identity· λοξές/composite κολόνες ήδη rotated]· ρέει 2Δ+3Δ+BOQ· B2 3Δ `beamToMesh` per-piece `extrudeShapesAndRotate`· B3 BOQ net core (mirror foundation net· I-shape DEFER)· B4 2Δ scene post-pass `applyBeamColumnCutback2D`→`BeamGeometry.displayOutline`→`BeamRenderer` multi-piece [fill/stroke/hover/hatch/hit-test· ADR-040-safe CHECK 6D]· σοβάς silhouette αμετάβλητος | 🟢 v1 (B1-B4) Implemented — 13 jest — pending browser-verify + commit | [View](./adrs/ADR-458-beam-column-cutback.md) |
| **ADR-460** | **Multi-shape Column & Wall Reinforcement** (επεκτείνει ADR-456/457· όλη η αυτοματοποίηση οπλισμού από μόνο-ορθογωνική → **ΟΛΟΥΣ τους 9 τύπους διατομής**)· SSoT classifier `resolveColumnReinforcementSection` (outline+mode `perimeter`/`circular`/`wall`+min πάχος/περίμετρος/Ac shape-correct) πάνω στο `materializeColumnLocalPolygonMm`· ΕΝΑΣ dispatcher `resolveColumnRebarLayout`/`resolveColumnCrossTies` τρέφει 2Δ/3Δ/ποσότητες/detail-sheet (rectangular fast-path → μηδέν regression)· NEW `column-section-outline`/`-perimeter-layout`/`-circular-layout`/`-wall-reinforcement`/`-layout-resolve`· **wall = boundary elements (κρυφοκολώνες) + κατανεμημένος κορμός** (EC8 §5.4.3.4)· SSoT `insetPolygonMiter` (winding-aware concave-safe miter inset)· concave-aware `buildRoundedStirrupPath`· `ColumnReinforcement` +`wall`/`spiralPitchMm` (+Zod· **fix κενό `crossTiePattern`** στο .strict schema)· providers/suggest perimeter-count+`minThicknessMm`+wall/circular· validator gate removal (shape-correct ρ)· 7 detail-sheet builders + 2Δ/3Δ render gates removed· settings panel ορατό σε όλα + shape-aware ctx/readouts | 🟢 Slices 1-10 Implemented — 540+ structural jest, tsc clean — pending browser-verify + commit | [View](./adrs/ADR-460-multi-shape-column-reinforcement.md) |
| **ADR-461** | **Special Levels: Foundation + Stair Penthouse** (επεκτείνει ADR-451· θεμελίωση & απόληξη κλιμακοστασίου = drawable special levels, ΟΧΙ μετρημένοι όροφοι — Revit «Building Story» OFF)· **REUSE υπάρχον `FloorKind` SSoT** (`floor-naming.ts`) αντί νέου `role` discriminator (N.0.2/N.12)· `FloorKind += 'stair-penthouse'` (διακριτό από `'roof'`· short "SP"/long "Απόληξη Κλιμακοστασίου") + NEW `SPECIAL_LEVEL_KINDS`/`isBuildingStorey`/`countBuildingStoreys` (το «Όροφοι: N» = παράγωγο)· building-level `hasStairPenthouse`/`stairPenthouseHeight` (καθρέφτης foundation· default 2.40m· Zod)· `generateFloorStack` εκπέμπει foundation spec (κάτω) + penthouse spec (πάνω) όταν toggles ON, κάθε storey παίρνει inferred `kind`· back-compat (toggles OFF → ίδιες στάθμες)· numbering foundation=lowest−1/penthouse=top+1 (elevation=SSoT· edge-case 0-υπογείων→Phase D)· **Phase B** «Όροφοι» UI+server: toggle «Έχει απόληξη»+ύψος στο `BuildingVerticalSetupForm`· `handleCreateFloor` boy-scout fix threadάρει `kind` (πριν χανόταν)· πίνακας special badge (κεντρικό `Badge`) + count=`countBuildingStoreys`· building PATCH & list handler passthrough (καμία αλλαγή)· SSoT cleanup checkboxes→κεντρικό `Checkbox`· **Phase C** DXF levels (pipeline ήδη kind-agnostic→αυτόματα ορατά· C1 NEW `resolveStoreyDefaultEntityTypes`+`BimToolCategory`· C2 boy-scout `SPECIAL_LEVEL_KINDS`· C3 badge στο level switcher· C4 advisory ribbon tool-gating «warn don't block» NEW `storey-tool-gating`+`getCommandRecommendation`)· **Phase D** kind-aware elevation cascade (**satellite model**: foundation re-anchors κάτω/penthouse rides· R5 datum εξαιρεί specials→3D δεν ανεβαίνει· R2 top-height kind-aware· R3 intermediate-delete μόνο counted→foundation δεν μπλοκάρει ισόγειο/422· R4 thermal counted-only· R6 kind-aware duplicate-check→409 co-existence) | 🟢 Phase A+B+C+D Implemented — jest+tsc clean — pending browser-verify + commit | [View](./adrs/ADR-461-special-levels-foundation-stair-penthouse.md) |
| **ADR-462** | **Canonical-mm Units** (ΕΝΑ unit system — ΟΛΗ η γεωμετρία πάντα σε χιλιοστά, όπως Revit σταθερή εσωτερική μονάδα· υπερισχύει ADR-368 render-label→import-scale)· **import scaling SSoT** στο `DxfSceneBuilder.buildScene(content, unitsOverride?)` (scale source→mm via ADR-348 `scaleEntity`, stamp `units:'mm'`)· NEW `resolveImportSourceUnits` (trust $INSUNITS εκτός lying-mm Greek→bounds-magnitude wins)· `resolveSceneUnits` trust declared (μηδέν heuristic re-guess — ρίζα «μικροσκοπική κολώνα στο Ισόγειο»)· override threading client/worker· server path auto-detect· ~30 consumers+~25 entity types αμετάβλητοι (παίρνουν 'mm' αυτόματα). Incident: Ισόγειο meter-DXF→κολώνες sceneUnits:'m'→1000× mismatch. 10 jest. DEFER: display-unit UI, server explicit units param | 🟢 Phase 1 core Implemented — 10 jest, tsc clean — pending browser-verify + commit | [View](./adrs/ADR-462-canonical-mm-units.md) |
| **ADR-463** | **Foundation Reinforcement UX** (mirror κολώνας: η ίδια ακριβώς εμπειρία οπλισμού — Ribbon → αριστερό «Ιδιότητες» panel → 2Δ/3Δ render → PDF detail-sheet → «Αυτόματος Οπλισμός» — για πέδιλα `pad`/συνεχή `strip`/πεδιλοδοκούς `tie-beam`)· **compute/model/persist/providers/auto-reinforce/organism ΥΠΑΡΧΟΥΝ ΗΔΗ ΠΛΗΡΗ** (ADR-459 Φ4b/4d + ADR-456)· κενό = **μόνο UX/render/PDF**· REUSE `footing-reinforcement-compute`/`section-context`/`buildFootingSectionContext`/detail-sheet generic framework/`DxfRenderer` pass pattern/`REBAR_MATERIAL`/`scalePoints`· NEW kind-aware (pad σχάρα / strip εγκάρσιες+διαμήκεις+συνδετήρες / tie-beam beam-like) `foundation-structural-bridge`/`-param`/`foundation-property-fields`/`FoundationAdvancedPanel`/`footing-rebar-2d`/`footing-rebar-3d`/`footing-detail-*` (Slice6: `DetailSheetDialog` SSoT + `detail-3d-capture-core` + `FoundationDetailHost`)· browser-fix: «Οπλισμός» toggle στο foundation ribbon + `stripUndefinedDeep` στο foundation persist· DEFER: real-time auto re-derive (χρειάζεται `auto` flag shared model), stepped/sloped pad rebar | 🟢 Slices 0-7 IMPLEMENTED — UNCOMMITTED (🔴 browser-verify+commit) | [View](./adrs/ADR-463-foundation-reinforcement-ux.md) |
