...
νέα** δήλωση σταθερού hex σε **σημασιολογικό ρόλο** μέσα στα design-token modules —
**ακόμα κι αν σήμερα περνά** και στα δύο θέματα.
Baseline: **`.theme-pairing-baseline.json`** — 35 παραβιάσεις / 43 δηλώσεις (2026-08-07).
...
### Τέσσερις ομάδες, εννέα ρητές καταστάσεις
...
2. **ΚΛΕΙΣΤΟ ΣΥΝΟΛΟ** των 43 σημασιολογικών δηλώσεων — **κάθε ΝΕΑ** μπλοκάρει, **ακόμα κι
...
### Δηλωμένα όρια (tests `Κ5`–`Κ7`, όχι ελπίδες)

- Κρίνονται **μόνο literal hex** — `rgba(…)`, `var(--x)` και υπολογισμένες τιμές **όχι**.
  Το `style={μεταβλητή}` (**216** σημεία) παραμένει **runtime** ερώτημα.
- Επιφάνεια που υπάρχει σε **ένα** θέμα δεν μπορεί να απαντήσει «αλλάζει;».
- **Fail-closed**: ελλιπές δέντρο ή χαλασμένη baseline ⇒ **σφάλμα**, ποτέ «καθαρό».
...
npm run test:theme-pairing       # 29 tests (Μ0·Μ1-Μ9·Ρ1-Ρ5·Π1-Π3·Κ1-Κ8)
```
...
ταξινόμηση ρόλων, η αναγνώριση ζεύγους και οι **εννέα** καταστάσεις ζουν **μία φορά**, στα
...
Γι' αυτό **κλείνει** τα ρητά δηλωμένα όρια **Κ5–Κ7** του 3.39 αντί να τα ξανα-δηλώνει:
`rgba()`, `hsl(var(--x))`, `color-mix()`, indirection μέσω `var()`.
...
| `translucent-invisible` | **0** (12 `translucent-ok`) | Ημιδιαφανής δήλωση αόρατη σε **κάθε** επιφάνεια. Το άλφα είναι ο λόγος που το 3.39 δεν μπορούσε: το χρώμα **δεν είναι ένα**, είναι ένα ανά φόντο. |
