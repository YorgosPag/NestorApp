/**
 * ============================================================================
 * ESCO Skills Import Script (ADR-132)
 * ============================================================================
 *
 * Downloads ESCO skills from the EU API and imports them into Firestore.
 *
 * Strategy: Uses `isInScheme` parameter to browse ALL skills in the
 * concept scheme. Search results already contain `preferredLabel` in ALL
 * languages, so only ~28 paginated requests are needed (500/page × 13,485 total).
 *
 * Usage:
 *   tsx scripts/import-esco-skills.ts
 *
 * What it does:
 * 1. Paginates through all ESCO skills via concept-scheme browsing
 ...
 * 4. Batch writes to Firestore: system/esco_cache/skills
 *
 * Requirements:
 ...
 *
 * Performance: ~28 API calls + ~34 Firestore batch writes ≈ 30-60 seconds
 *
...
const ESCO_SKILLS_SCHEME = 'http://data.europa.eu/esco/concept-scheme/skills';
const FIRESTORE_COLLECTION = 'system/esco_cache/skills';
 ...
 *
 * Same algorithm as occupations import — generates all prefixes of each word
 * (min 2 chars) to enable autocomplete search.
 *
 * Example: "μαθηματικά" → ["μα", "μαθ", "μαθη", "μαθημ", "μαθημα", "μαθηματ",
 *   "μαθηματι", "μαθηματικ", "μαθηματικα"]
 */
 ...
 * "http://data.europa.eu/esco/skill/fbceeac6-798b-4307-a825-626707a753ad"
 */
function uriToDocId(uri: string): string {
  ...
  return uri
    .replace('http://data.europa.eu/esco/skill/', '')
...
/**
 * Fetch ALL ESCO skills by browsing the concept scheme.
 *
 * Uses `isInScheme` parameter to list all skills without needing
 * a search term. Each result already contains `preferredLabel` in ALL
 * EU languages — no per-skill detail requests needed.
 */
async function fetchAllSkills(): Promise<EscoSearchResult[]> {
  ...
  console.log('\n📥 Fetching ALL ESCO skills via concept-scheme browsing...');

  while (page < totalPages()) {
    const url = `${ESCO_API_BASE}/search?type=skill&language=en&offset=${page}&limit=${API_PAGE_SIZE}&isInScheme=${encodeURIComponent(ESCO_SKILLS_SCHEME)}`;
    ...
    try {
      ...
      console.log(`  📊 Page ${page}/${totalPages()}: ${allResults.length}/${totalItems} skills (${pct}%)`);
  ...
  console.log(`  ✅ Fetched ${allResults.length} skills total`);
  return allResults;
}
 ...
 */
function transformToDocuments(results: EscoSearchResult[]): SkillDocument[] {
  console.log('\n🔀 Transforming skills to Firestore documents...');
  ...
  console.log(`  ✅ Transformed ${documents.length} skills`);
  if (missingEl > 0) console.log(`  ⚠️  ${missingEl} skills missing Greek label`);
  if (missingEn > 0) console.log(`  ⚠️  ${missingEn} skills missing English label`);
...
/**
 * Write skill documents to Firestore in batches.
 */
async function writeToFirestore(
  db: FirebaseFirestore.Firestore,
  documents: SkillDocument[]
): Promise<void> {
  console.log(`\n📤 Writing ${documents.length} skills to Firestore...`);
  ...
  console.log(`  ✅ All ${written} skills written to Firestore`);
}
...
async function main(): Promise<void> {
  console.log('====================================================');
  console.log('🇪🇺 ESCO Skills Import (ADR-132)');
  console.log('====================================================');
  console.log(`📅 Date: ${new Date().toISOString()}`);
  console.log(`📍 Target: ${FIRESTORE_COLLECTION}`);
  console.log(`📡 Strategy: Concept-scheme browsing (fast, ~28 API calls)`);
  ...
  // Step 1: Fetch all skills (single paginated request series)
  const startFetch = Date.now();
  const rawResults = await fetchAllSkills();
  ...
  console.log('====================================================');
  console.log(`📊 Total skills: ${documents.length}`);
  console.log(`📍 Collection: ${FIRESTORE_COLLECTION}`);
  console.log(`⏱️  Total duration: ${totalDuration}s`);
  console.log(`📡 API calls: ~${Math.ceil(rawResults.length / API_PAGE_SIZE)}`);
  console.log('');
  console.log('Next steps:');
  console.log('  1. Deploy Firestore indexes: firebase deploy --only firestore:indexes --project pagonis-87766');
  console.log('  2. Verify in Firebase Console: Firestore → system → esco_cache → skills');
