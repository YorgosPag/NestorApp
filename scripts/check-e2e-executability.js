 ...
 * λένε. Τα projects `firefox`·`webkit`·`Mobile *` **δεν είχαν testMatch**, άρα έτρεχαν κι αυτά
 * τα 43 visual tests και συγκρίνονταν με τα **ίδια** αρχεία ⇒ **172 βέβαιες αποτυχίες**. Και σε
 * Linux runner αποτυγχάνει **ακόμα και το chromium**, γιατί λείπει και το `{platform}`.
 *
 * ✅ **09/09/2026 — ΤΟ ΔΕΥΤΕΡΟ ΣΚΕΛΟΣ ΕΚΛΕΙΣΕ** (ADR-775 §11): τα γενικά projects φέρουν πλέον
 * `testIgnore: GENERIC_TEST_IGNORE` και **δεν σηκώνουν καθόλου** spec που ανήκει σε ειδικό
 * project (μετρημένο `--list`: **405 → 155**, ανά spec **6 → 1**). ⚠️ Η ομάδα **Β** παραμένει
 * ακέραιη και **δεν χαλαρώνει**: το `{platform}` λείπει ακόμη ως πρόβλημα Linux, και το
 * `testIgnore` **δεν φυλάγεται από καμία πύλη** — το ρητό πρότυπο είναι το μόνο που φυλάγεται.
