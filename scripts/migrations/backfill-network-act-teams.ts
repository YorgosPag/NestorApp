...
async function main(): Promise<void> {
  ...
  for (const doc of properties.docs) {
    ...
    for (const mandate of mandatesOf(property)) {
      ...
      if (found === null || !APPLY) continue;

      const outcome = await ensureActTeam(
        db,
        {
          actKind: 'mandate',
          actSeed: mandateActSeed(doc.id, agencyCompanyId),
          hostCompanyId: agencyCompanyId,
          responsibleUid: found.uid,
        },
        nowISO,
      );
