#!/bin/bash
# =============================================================================
# ENTERPRISE: UI Hardcoded Strings Audit Report
# =============================================================================
# Shows current status vs. baseline + top offenders.
# Run: npm run ui-strings:audit
# =============================================================================
...
TMP_RAW="/tmp/ui-strings-audit-raw.txt"

if [[ ! -f "$BASELINE_FILE" ]]; then
    echo -e "${RED}❌ Baseline not found: $BASELINE_FILE${NC}"
    echo -e "${YELLOW}   Run: npm run ui-strings:baseline${NC}"
    exit 1
fi

# Current scan (same logic as generator)
{
    grep -rnP ">[^<>{}]*\p{Greek}[^<>{}]*<" src --include="*.tsx" 2>/dev/null
    grep -rnP "(placeholder|title|aria-label|alt|label)=\"[^\"]*\p{Greek}[^\"]*\"" src --include="*.tsx" 2>/dev/null
    grep -rnP "(throw new Error|alert|confirm|prompt)\(\s*[\"'\`][^\"'\`]*\p{Greek}" src --include="*.ts" --include="*.tsx" 2>/dev/null
    grep -rnP "toast\.[a-z]+\(\s*[\"'\`][^\"'\`]*\p{Greek}" src --include="*.ts" --include="*.tsx" 2>/dev/null
} \
...
echo ""
echo -e "${BOLD}${CYAN}═══════════════════════════════════════════════════════════════${NC}"
echo -e "${BOLD}${CYAN}  📊 UI Hardcoded Greek Strings — Audit Report${NC}"
echo -e "${BOLD}${CYAN}═══════════════════════════════════════════════════════════════${NC}"
echo ""
...
if [[ $DIFF -gt 0 ]]; then
    ...
    echo -e "  ${CYAN}   Run: npm run ui-strings:baseline (to update baseline)${NC}"
elif [[ $DIFF -lt 0 ]]; then
...
echo ""
echo -e "${BOLD}Top 15 offenders (current):${NC}"
echo ""
head -15 "$TMP_RAW" | while read count file; do
    printf "  %4d  %s\n" "$count" "$file"
done
echo ""
