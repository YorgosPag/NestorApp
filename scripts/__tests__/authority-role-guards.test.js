...
function mutate(source, from, to) {
  if (!source.includes(from)) {
    throw new Error(`η μετάλλαξη δεν βρήκε το «${from}» — δεν θα άλλαζε τίποτα`);
  }
  const out = source.replace(from, to);
  if (out === source) throw new Error('η μετάλλαξη ΔΕΝ άλλαξε τίποτα');
  return out;
}
...
describe('Μ — επαναφορά του πραγματικού φρουρού ⇒ ⛔', () => {
  ...
  it('Μ7 — ο μεταλλάκτης ΟΥΡΛΙΑΖΕΙ αν δεν άλλαξε τίποτα', () => {
    expect(() => mutate('abc', 'δεν υπάρχει', 'x')).toThrow(/δεν θα άλλαζε τίποτα/);
  });
});
