...
describe('🔴 Γ — Η ΤΑΥΤΟΤΗΤΑ ΕΙΝΑΙ ΕΓΚΥΡΟ ΚΛΕΙΔΙ (ADR-657 §3.3)', () => {
   ...
   * `resource != null` σε `update`/`delete` και **αφαιρεί** το `assignedTo` — δηλαδή είναι
   * **αυστηροποίηση που δεν ισχύει ποτέ**, γιατί το πρώτο *(PR-1B)* τα επιτρέπει ήδη.
   *
   * ⚠️ **ΔΕΝ διορθώνεται εδώ**: είναι αλλαγή σε **κανόνες παραγωγής**, με δικό της εύρος
   * και δική της απόφαση. Κλειδώνεται ως **γεγονός** — ratchet, ώστε να μη γίνουν τέσσερα.
   */
  ...
  it('🔴 RATCHET: καμία ΝΕΑ διπλή διαδρομή — το allow είναι η ΕΝΩΣΗ των μπλοκ', () => {
    const duplicates = findDuplicateMatchPaths(parseFirestoreRules(read('firestore.rules')));

    // 🔑 Υποσύνολο, όχι ισότητα: η **διόρθωση** ενός από τα τρία δεν πρέπει να κοκκινίζει.
    for (const path of duplicates) {
      expect(KNOWN_DUPLICATE_PATHS).toContain(path);
    }
  });
