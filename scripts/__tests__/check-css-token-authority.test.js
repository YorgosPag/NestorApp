...
describe('Θ — οι υποσχέσεις της θεραπείας Π1 (ό,τι η πύλη ΔΕΝ βλέπει)', () => {
  ...
  test('Θ11 — το μελάνι της προειδοποίησης πιάνει 4,5:1 στο δοχείο της, και στα δύο', () => {
    ...
    // 🔑 ΚΑΙ ΤΟ ΣΧΗΜΑ ΤΗΣ ΑΠΟΤΥΧΙΑΣ ΗΤΑΝ ΤΟ ΜΑΘΗΜΑ: το προφανές ζεύγος δεν ήταν «λίγο
    // χειρότερο», ήταν σπασμένο ΣΤΟ ΕΝΑ θέμα και μια χαρά στο άλλο — γι' αυτό η μηχανική
    // μετονομασία *φαινόταν* να δουλεύει σε όποιο θέμα τύχαινε να κοιτάς.
    // ✅ ΑΝΤΙΣΤΡΑΦΗΚΕ, ΔΕΝ ΣΒΗΣΤΗΚΕ (ADR-770 §18, 2026-09-17): η ρίζα διορθώθηκε στο token —
    // το φωτεινό `--text-warning` ήταν ΚΙΤΡΙΝΟ (`43 96% 56%`) = 1,38–1,61:1 σε ΚΑΘΕ ανοιχτή
    // επιφάνεια, και βαθμονομήθηκε σε κεχριμπάρι `32 95% 28%`. Το ζεύγος πλέον ΕΙΝΑΙ ζεύγος και
    // στα δύο θέματα. Αν ξαναπέσει κάτω από 4,5 στο ένα, το μάθημα από πάνω εξηγεί γιατί.
    expect(ratio('light', '--text-warning', '--bg-warning')).toBeGreaterThanOrEqual(4.5); // ήταν 1,61 → 6,39
