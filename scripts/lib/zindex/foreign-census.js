 ...
 *
 * ⚠️ ΔΗΛΩΜΕΝΟ ΟΡΙΟ, ΤΩΡΑ **ΜΕΤΡΗΜΕΝΟ ΚΑΙ ΜΕ ΟΝΟΜΑΤΑ** (ADR-780 Φάση Γ, 2026-08-09).
 * Ανοίγονται τα **άμεσα** runtime dependencies (και τα ένθετα `node_modules` τους). Ένα
 * **έμμεσο** πακέτο που δεν το φέρνει κανένα άμεσο δεν σαρώνεται.
 *
 * Πλήρης σάρωση του `node_modules/.pnpm` εκτελέστηκε **μία φορά** για να απαντηθεί το
 * ερώτημα «είναι το κενό θεωρητικό;»: **125,0s · 206.235 αρχεία** ⇒ **15** πακέτα ≥1000
 * συνολικά, έναντι **11** που βλέπει η απογραφή. Τα **τέσσερα** που λείπουν:
 *
 *   2147483647  playwright-core            → **devDependency** — δεν μπαίνει ποτέ σε bundle browser
 ...
 *
 * ⇒ **Κανένα από τα τέσσερα δεν φτάνει σήμερα στην οθόνη**, άρα το κενό παραμένει
 * **γραμμένο** αντί να λυθεί με κόστος 125s ανά έλεγχο — αυτό είναι το κριτήριο, όχι η
 * ευκολία. 🔴 ΑΛΛΑ ΤΟ ΣΗΜΑΝΤΙΚΟ ΕΙΝΑΙ ΤΟ **ΠΩΣ ΘΑ ΕΠΑΥΕ**: το Sentry feedback widget
 * απέχει **μία γραμμή** (`integrations: [feedbackIntegration()]`) από το να θάψει
 * ολόκληρη την εφαρμογή στο **100000** — και **καμία** πύλη δεν θα το έλεγε, γιατί δεν
 * είναι άμεσο dependency. Αν προστεθεί ποτέ, το πακέτο πρέπει να μπει στο
 * `.zindex-foreign.json` **και** να δηλωθεί άμεσο, αλλιώς η απογραφή μένει τυφλή.
 * Γράφεται εδώ ώστε το «11» να μη διαβαστεί ως «όλα».
 */
