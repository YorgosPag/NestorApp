 ...
 *
 * 8. 🔴 **ΤΟ ΝΑ ΑΚΟΛΟΥΘΕΙΣ ΑΝΑΚΑΤΕΥΘΥΝΣΗ ΕΙΝΑΙ ΝΑ ΚΡΙΝΕΙΣ ΑΛΛΗ ΣΕΛΙΔΑ** (ADR-781
 *    §13). Ο χρησμός χτυπά **ανώνυμα**, και το `o/[workspace]/layout.tsx` κάνει
 *    `redirect(AUTH_ROUTES.login)` σε κάθε ανώνυμο. Με `redirect: 'follow'`,
 *    **110** διαδρομές έδιναν το πόρισμα της **σελίδας σύνδεσης** — με 110
 *    διαφορετικά ονόματα διαδρομών. Και επειδή είναι όλες δυναμικές, το πόρισμα
 *    απορροφιόταν στο 🔶 `surface-synthetic-id`, που **δεν απαριθμείται ποτέ**:
 *    το σφάλμα δεν αποτύγχανε δυνατά, **εξατμιζόταν**. Πλέον `redirect: 'manual'`
 *    και ρητή κατάσταση `route-redirected` (βλ. `probe.js`, `states.js`).
 *
