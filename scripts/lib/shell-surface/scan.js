...
/**
 * Συμβολοσειρές **πρώτα**, μετά σχόλια — σε **ΕΝΑ** πέρασμα.
 *
 * 🔴 ΤΙ ΕΣΠΑΣΕ, ΜΕ ΑΡΙΘΜΟΥΣ (ADR-861 §6.4 · ADR-797 §Δ)
 * Η προηγούμενη γραφή ήταν **δύο** `replace` με **ανάποδη** σειρά: έσβηνε πρώτα
 * τα μπλοκ (`/\*…*\/`) και **μετά** τις γραμμές `//`. Ένα **σχόλιο γραμμής** που
 * περιέχει την ακολουθία `/*` — και ο πιο αθώος τρόπος να συμβεί είναι να γράψεις
 * το glob `src/app/**` μέσα σε σχόλιο — διαβαζόταν ως **άνοιγμα μπλοκ** και
 * ζευγάρωνε με το **επόμενο** `*\/` του αρχείου, καταπίνοντας ό,τι ήταν ανάμεσα.
 *
 * Μετρημένο στο `src/app/(app)/test-harness/listing-shapes/page.tsx`: **7.994 →
 * 2.234** χαρακτήρες· καταπόθηκαν οι γραμμές **78–133** **μαζί με το
 * `export default` της γρ. 118** (**4** ανοίγματα `/*` έναντι **3** κλεισιμάτων).
 * ⇒ `exportedRootOf` = `null` ⇒ ο σαρωτής έπεφτε στο **ένα άλμα** και έκρινε
 * **άλλο αρχείο**: η σελίδα μετριόταν «καθαρή» **χωρίς να την έχει δει κανείς**,
 * ενώ η ρίζα της γράφει `<main className="flex h-screen flex-col">`. Ο μετρητής
 * της πύλης έπεσε **28 → 27** και η **πράσινη** γραμμή πρότεινε να σφραγιστεί.
 * **Λανθάνων πυροδότης**: **31** αρχεία `src/**` + **14** `scripts/**`.
 *
 * ⚠️ **ΓΙΑΤΙ ΟΧΙ ΣΚΕΤΗ ΑΝΤΙΣΤΡΟΦΗ ΤΗΣ ΣΕΙΡΑΣ.** Θα θεράπευε το δείγμα και θα
 * γεννούσε το συμμετρικό: το `//` **μέσα σε μπλοκ σχόλιο** (`* … http://…`) θα
 * έσβηνε τη γραμμή πριν προλάβει να κλείσει το μπλοκ. Το παλιό `[^:]` ήταν
 * ακριβώς αυτό το μπάλωμα — φυλούσε το `https://` **μόνο** χάρη στην άνω τελεία.
 *
 * 🔑 **ΤΟ ΣΩΣΤΟ ΟΡΙΟ ΕΙΝΑΙ Η ΣΥΜΒΟΛΟΣΕΙΡΑ, ΟΧΙ Ο ΧΑΡΑΚΤΗΡΑΣ ΠΡΙΝ.** Με τις
 * συμβολοσειρές **πρώτες στην εναλλαγή**, ένα `'src/app/**'` σε **κώδικα** (όχι
 * σχόλιο) δεν ανοίγει μπλοκ, και ένα `"https://x"` δεν σβήνει γραμμή — χωρίς
 * κανέναν ειδικό κανόνα για την άνω τελεία. Το `(?<!\\)` καλύπτει την άλλη
 * κατεύθυνση: το `//` ενός regex literal (`.replace(/\/\//g, '')`).
 *
 * ⚠️ **ΑΦΑΙΡΕΙ, ΔΕΝ ΠΡΟΣΘΕΤΕΙ** — το συμβόλαιο μένει ακέραιο: κάθε ταίριασμα που
 * επιβιώνει υπήρχε και στο πρωτότυπο. Τα σχόλια γίνονται **κενά ίδιου μήκους**
 * και οι **νέες γραμμές επιβιώνουν**, σε αντίθεση με πριν που ένα μπλοκ γινόταν
 * **ένα** κενό: το `bodyEnd` αγκυρώνει με `^…` σε **πολυγραμμική** σημαία, άρα η
 * κατάρρευση γραμμών του έκρυβε ορίγματα δηλώσεων.
 *
 * 🔶 **ΔΕΝ είναι tokenizer, και είναι δηλωμένο**: apostrophe μέσα σε κείμενο JSX
 * (`γι' αυτό`) μπορεί να ανοίξει ψευδή συμβολοσειρά — αλλά **μόνο** αν κλείσει
 * στην **ίδια** γραμμή, και το χειρότερο αποτέλεσμα είναι να **διατηρηθεί**
 * κείμενο, ποτέ να σβηστεί κώδικας. Η ασφαλής κατεύθυνση, επίτηδες.
 *
 * 🔴 **ΤΟ ΔΙΔΥΜΟ ΕΛΑΤΤΩΜΑ ΖΕΙ ΑΚΟΜΑ, ΚΑΙ ΔΕΝ ΔΙΟΡΘΩΝΕΤΑΙ ΕΔΩ**: το
 * `scripts/lib/source-text.js` (δηλωμένο SSoT των πυλών) κρατά **τις ίδιες δύο
 * γραμμές**, και το καταναλώνουν τα CHECK 3.56 · 3.58 · 3.59 · 3.69. Η
 * ενοποίηση απαιτεί **μέτρηση ανά πύλη** και είναι δηλωμένη ως **δική της
 * συνεδρία** (N.8) στο `.claude-rules/pending-ratchet-work.md`.
 */
const LITERALS_THEN_COMMENTS =
  /'(?:[^'\\\n]|\\.)*'|"(?:[^"\\\n]|\\.)*"|`(?:[^`\\]|\\.)*`|\/\*[\s\S]*?\*\/|(?<!\\)\/\/[^\n]*/g;

function stripComments(source) {
  return source.replace(LITERALS_THEN_COMMENTS, (match) =>
