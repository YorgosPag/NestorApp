...
// ═════════════════════════════════════════════════════════════════════════════
// ADR-875 — ΟΙ ΕΚΠΡΟΣΩΠΟΙ ΤΟΥ ΧΡΗΣΜΟΥ 3.51 (golden tenant)
// ═════════════════════════════════════════════════════════════════════════════

/**
 * **Η κλάση ταυτότητας** ενός ανθρώπου — `(εμβέλεια, ρόλος)`, ό,τι ακριβώς ξεχωρίζει
 * ο `classifyIdentityClaims` (`src/lib/auth/identity-claims.ts`).
 *
 * ⚠️ **ΑΝΤΙΓΡΑΦΟ ΤΗΣ ΑΥΘΕΝΤΙΑΣ, ΟΧΙ ΚΛΗΣΗ ΤΗΣ** — για τον ίδιο λόγο με το
 * `COMPANY_ALIAS_KEY`: ο σπορέας τρέχει σε σκέτο `tsx`. Η συμφωνία **εκτελείται**
 * στην άγκυρα Τ2 (`scripts/__tests__/i18n-ssr-identity.test.ts`) πάνω σε ΟΛΟ το
 * `PERSONAS` — αν αποκλίνουν, ο χρησμός θα έκρινε «κλάση» που η εφαρμογή δεν ξέρει.
 */
export function oracleClassOf(person: SeedIdentity): string {
  return `${person.companyId ? 'organization' : 'personal'}:${person.globalRole}`;
}

/**
 * **Ένας εκπρόσωπος ανά κλάση ΟΡΓΑΝΙΣΜΟΥ** — με αυτούς ο χρησμός κρίνει τις
 * `/o/[workspace]/**` (ADR-875 §5).
 *
 * 🔑 **ΚΑΛΥΨΗ ΕΚ ΚΑΤΑΣΚΕΥΗΣ, ΟΧΙ ΧΕΙΡΟΔΙΑΛΕΓΜΕΝΟΙ ΛΟΓΑΡΙΑΣΜΟΙ**: η άγκυρα Τ3 απαιτεί
 * κάθε κλάση οργανισμού του `PERSONAS` να έχει **ακριβώς έναν** εκπρόσωπο εδώ. Νέος
 * ρόλος στον κατάλογο χωρίς εκπρόσωπο ⇒ κόκκινο — όχι σιωπηλά άκριτος.
 *
 * ⚠️ **Ο ΙΔΙΩΤΙΚΟΣ ΧΩΡΟΣ ΔΕΝ ΕΙΝΑΙ ΕΔΩ, ΚΑΙ ΔΕΝ ΕΙΝΑΙ ΠΑΡΑΛΕΙΨΗ**: το
 * `PERSONAL_WORKSPACE_SURFACE` είναι **κενό** ⇒ το `/o/me/**` δεν προσφέρει σήμερα
 * **καμία** σελίδα (ανακατευθύνει στο `PRIVATE_SPACE_HOME`). Η Τ3 κοκκινίζει τη
 * στιγμή που αποκτήσει έστω μία — τότε ο προσωπικός άνθρωπος **οφείλει** εκπρόσωπο.
 */
export const ORACLE_REPRESENTATIVES: readonly string[] = [
  'int.architect@alpha.local',
  'admin.civil@alpha.local',
];
