...
export interface SeedOccupation {
  ...
  /**
   * 🔴 **ΚΑΙ ΤΑ ΤΕΣΣΕΡΑ ΥΠΟΧΡΕΩΤΙΚΑ — ΤΟ ΕΠΕΒΑΛΕ ΜΕΤΡΗΣΗ (ADR-798 §19)**.
   *
   * Μέχρι 2026-08-26 τα τρία ήταν προαιρετικά και **κανένας** από τους οκτώ δεν
   * έγραφε `escoUri`. Ο `useDeclaredOccupation` όμως **αρνείται** να εκθέσει
   * `iscoCode` χωρίς `escoUri` *(άγκυρα `Ρ-4`: «ορφανός κωδικός δεν οδηγεί την
   * πρόταση δουλειάς»)* ⇒ **8 στους 8** έφταναν στην οθόνη ως **σιωπή**, ενώ η
   * άγκυρα `Κ2` — που ρωτά τον **πίνακα** και όχι τη **διαδρομή** — έμενε πράσινη.
   *
   * 🔑 Ο **μοναδικός** γραφέας της παραγωγής (`EscoOccupationPicker`) εκπέμπει
   * **και τα τέσσερα ή κανένα**. Προαιρετικά πεδία εδώ επέτρεπαν στον σπορέα να
   * φτιάξει άνθρωπο που ο picker **δεν θα μπορούσε ΠΟΤΕ να δημιουργήσει** — και
   * το rig δοκίμαζε ακριβώς αυτόν. Υποχρεωτικά, η απόκλιση γίνεται **σφάλμα
   * μεταγλώττισης**, όχι εύρημα άγκυρας.
   */
  readonly escoLabel: string;
  readonly iscoCode: string;
  /** Η **αυθεντία** της ταξινόμησης. Επαληθευμένο ζωντανά στο ESCO API. */
