...
/**
 * **ΑΠΟ ΠΟΥ ήρθε η επιβαλλόμενη βαθμίδα — από ΓΡΑΜΜΗ του πίνακα ή από την ΠΡΟΕΠΙΛΟΓΗ;**
 *
 * ─────────────────────────────────────────────────────────────────────────────
 * 🔴 ΓΙΑΤΙ ΞΕΧΩΡΙΣΤΗ ΣΥΝΑΡΤΗΣΗ ΚΑΙ ΟΧΙ ΕΠΙΣΤΡΟΦΗ ΖΕΥΓΟΥΣ ΑΠΟ ΤΗΝ ΠΑΡΑΠΑΝΩ
 * ─────────────────────────────────────────────────────────────────────────────
 * Το {@link enforcedCategory} υπόσχεται *«ακριβώς όπως ο `getEndpointCategory` της
 * παραγωγής»* — και αυτή η υπόσχεση **επαληθεύεται διαβάζοντας τις δύο δίπλα-δίπλα**.
 * Αλλαγή της υπογραφής της σε `{category, source}` θα έσπαγε την ομοιότητα *(η παραγωγή
 * επιστρέφει σκέτη κατηγορία)* και θα ανάγκαζε κάθε καλούντα — τις **19 άγκυρες**
 * περιλαμβανομένων — να ξαναγραφτεί για ένα ερώτημα που **δεν έκανε**.
 *
 * ⇒ Δύο ερωτήματα, δύο συναρτήσεις. Η σειρά `startsWith` είναι **ταυτόσημη** επίτηδες: αν
 *   αποκλίνει, η πηγή θα λέει «από γραμμή» για βαθμίδα που ήρθε από αλλού.
 *
 * ─────────────────────────────────────────────────────────────────────────────
 * 🔴 ΤΟ ΠΕΡΙΣΤΑΤΙΚΟ ΠΟΥ ΤΗ ΓΕΝΝΗΣΕ — 57 ΑΠΟ 84 ΨΕΥΔΩΣ ΘΕΤΙΚΑ, ΜΕΤΡΗΜΕΝΑ
 * ─────────────────────────────────────────────────────────────────────────────
 * Ο `judge` έκρινε `declared !== enforced ⇒ SHADOWED` *(«νεκρή γραμμή πίνακα»)*. Αλλά το
 * `enforcedCategory` επιστρέφει την **προεπιλογή** όταν **καμία** γραμμή δεν ταιριάζει —
 * και τότε **δεν υπάρχει γραμμή για να νεκρώσει**. Δηλαδή κάθε νέα διαδρομή που δηλώνει
 * σωστά οτιδήποτε ≠ `STANDARD` καταγγελλόταν ως παραβίαση.
 *
 * **Μετρημένο 2026-09-12 πάνω στη baseline**: **57 από 84** «SHADOWED» πέφτουν στην
 * προεπιλογή *(κάτω φράγμα 54: τα 3 υπολογισμένα προθέματα δεν λύθηκαν στη μέτρηση)* —
 * δηλαδή **δύο στα τρία** του κριτηρίου ήταν θόρυβος, πολύ πάνω από τον πήχη <10% που
 * αυτό το δέντρο επιβάλλει (3.61 · 3.68 · 3.76).
 *
 * ⚠️ Το έπιασε η **ίδια η πύλη πάνω στον εαυτό της**, δεύτερη φορά: μια σωστά δηλωμένη
 *    `withHeavyRateLimit` σε δημόσια διαδρομή token (ADR-853 Φ4) μπλόκαρε commit **χωρίς
 *    να υπάρχει τίποτα λάθος**.
 */
function enforcementSource(url, table) {
...
function takeInventory(root, override = {}) {
  ...
  const routes = files.map((entry) => {
    ...
    return {
      ...
      // 🔑 Χωρίς αυτό, «η δήλωση διαφέρει από τον πίνακα» και «η δήλωση διαφέρει από την
      //    προεπιλογή» είναι **αδιάκριτα** — και το δεύτερο δεν είναι παραβίαση.
      enforcedFrom: enforcementSource(url, table),
    };
  });
