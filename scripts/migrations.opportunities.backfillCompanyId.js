/**
 * =============================================================================
 * CANONICAL SCRIPT: Backfill Opportunities companyId
 * =============================================================================
 *
 * Enterprise-grade migration script for backfilling missing companyId to opportunities.
 * Derives companyId from: 1) leadId lookup, 2) projectId lookup, 3) createdBy user's company.
 *
 * @module scripts/migrations.opportunities.backfillCompanyId
 ...
 * COMPANY_ID=<ID> COLLECTION_OPPORTUNITIES=opportunities COLLECTION_LEADS=leads COLLECTION_PROJECTS=projects COLLECTION_USERS=users node scripts/migrations.opportunities.backfillCompanyId.js
 *
 * # EXECUTE - Backfill opportunities with missing companyId
 * COMPANY_ID=<ID> COLLECTION_OPPORTUNITIES=opportunities COLLECTION_LEADS=leads COLLECTION_PROJECTS=projects COLLECTION_USERS=users DRY_RUN=false node scripts/migrations.opportunities.backfillCompanyId.js
 * ```
 *
 * COMPLIANCE:
...
const SCRIPT_NAME = 'migrations.opportunities.backfillCompanyId.js';
...
const COLLECTION_OPPORTUNITIES = process.env.COLLECTION_OPPORTUNITIES;
if (!COLLECTION_OPPORTUNITIES) {
  console.error(`[${SCRIPT_NAME}] ERROR: COLLECTION_OPPORTUNITIES is required`);
  process.exit(1);
}
...
async function backfillOpportunitiesCompanyId() {
  const startTime = Date.now();
  const report = createReportWriter(COLLECTION_OPPORTUNITIES, { outputPath: process.env.REPORT_OUTPUT_PATH });

  printHeader('OPPORTUNITIES COMPANYID BACKFILL MIGRATION', {
    'Target Company': COMPANY_ID,
    'Mode': DRY_RUN ? 'DRY-RUN' : 'EXECUTE',
    'Opportunities Collection': COLLECTION_OPPORTUNITIES,
    'Page/Batch Size': `${PAGE_SIZE}/${BATCH_SIZE}`,
    'Report File': report.getFilePath()
  });

  try {
    console.log('Scanning opportunities collection (streaming mode)...\n');
    let lastDoc = null, hasMore = true;

    while (hasMore) {
      stats.pages++;
      let query = db.collection(COLLECTION_OPPORTUNITIES).limit(PAGE_SIZE);
      ...
      for (const doc of snapshot.docs) {
        ...
        // Try to derive companyId: 1) leadId lookup, 2) projectId lookup, 3) createdBy lookup
        let derivedCompanyId = null;
        let derivedFrom = null;

        if (data.leadId) {
          derivedCompanyId = await getLeadCompanyId(data.leadId);
          if (derivedCompanyId) derivedFrom = 'lead';
        }

        if (!derivedCompanyId && data.projectId) {
        ...
        if (!derivedCompanyId) {
          stats.cannotDerive++;
          report.recordSkip({ id: doc.id, reason: 'cannot_derive_companyId', details: `leadId=${data.leadId || 'N/A'}, projectId=${data.projectId || 'N/A'}, createdBy=${data.createdBy || 'N/A'}` });
          continue;
        }
...
backfillOpportunitiesCompanyId().then(s => process.exit(s ? 0 : 1)).catch(() => process.exit(1));
