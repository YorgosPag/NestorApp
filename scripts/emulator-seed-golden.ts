...
/** `copy_link`: κανένα email — το token το κόβει ο ΠΡΑΓΜΑΤΙΚΟΣ `generateVendorPortalToken`. */
async function createVendorToken(session: ApiSession, rfqId: string): Promise<string> {
  ...
  return decodeURIComponent(new URL(portalUrl).pathname.split('/').filter(Boolean).pop() ?? '');
}
