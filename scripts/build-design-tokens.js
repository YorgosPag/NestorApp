...
const { emitFluidLayout } = require('./lib/design-tokens/fluid-layout');
...
// ============================================================================
// ADR-797 — ΡΕΥΣΤΗ ΚΛΙΜΑΚΑ ΚΕΛΥΦΟΥΣ (fluid layout)
// ============================================================================
...
:root {\n`;
  ...
  // 🔴 ADR-797 — ΤΑ ΠΑΡΑΓΩΓΑ ΤΗΣ ΡΕΥΣΤΗΣ ΚΛΙΜΑΚΑΣ, ΠΑΡΑΓΟΜΕΝΑ ΚΑΙ ΟΧΙ ΓΡΑΜΜΕΝΑ.
  css += emitFluidLayout(tokens);

  css += `}\n\n`;

  // 🔴 ADR-811 — Ο ΜΟΧΛΟΣ ΤΗΣ ΠΡΟΤΙΜΗΣΗΣ ΠΥΚΝΟΤΗΤΑΣ.
  //    Εκπέμπεται ΕΞΩ από το `:root {}` επειδή είναι κανόνες με επιλογέα
  //    (`:root[data-density="…"]`), όχι δηλώσεις μέσα σε μπλοκ — γι' αυτό δεν
  //    μπορεί να ζήσει μέσα στο `emitFluidLayout`, που επιστρέφει γραμμές.
  css += emitAppearanceCss(tokens);
...
function main() {
  ...
  try {
    ...
    // 🔴 ADR-811 — Ο ΚΑΤΑΛΟΓΟΣ ΤΩΝ ΡΟΛΩΝ ΠΥΚΝΟΤΗΤΑΣ, ΓΙΑ ΤΟΝ ΠΕΛΑΤΗ.
    //    Χωρίς αυτό ο πελάτης θα έγραφε χειρόγραφη ένωση `'comfortable' | 'compact'`
    //    δίπλα στο JSON — δεύτερη λίστα, το σχήμα που το αποθετήριο έχει πληρώσει
    //    μετρημένα σε 3.34 / 3.37 / 3.49 / 3.57.
    console.log('🎚️  Generating appearance axes...');
    const appearance = emitAppearanceTypeScript(tokensData);
    fs.writeFileSync(path.join(CONFIG.output.typescript, 'appearance.ts'), appearance);
    ...
    console.log(`   - ${path.relative(process.cwd(), path.join(CONFIG.output.typescript, 'appearance.ts'))}`);
