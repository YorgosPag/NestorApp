/**
 * =============================================================================
 * CANONICAL SCRIPT: Backfill Activities companyId
 * =============================================================================
 *
 * Enterprise-grade migration script for backfilling missing companyId to activities.
 * Derives companyId from: 1) relatedEntityId lookup (contact/lead/project), 2) createdBy user's company.
 *
 * @module scripts/migrations.activities.backfillCompanyId
 ...
 * COMPANY_ID=<ID> COLLECTION_ACTIVITIES=activities COLLECTION_CONTACTS=contacts COLLECTION_LEADS=leads COLLECTION_PROJECTS=projects COLLECTION_USERS=users node scripts/migrations.activities.backfillCompanyId.js
 *
 * # EXECUTE - Backfill activities with missing companyId
 * COMPANY_ID=<ID> COLLECTION_ACTIVITIES=activities COLLECTION_CONTACTS=contacts COLLECTION_LEADS=leads COLLECTION_PROJECTS=projects COLLECTION_USERS=users DRY_RUN=false node scripts/migrations.activities.backfillCompanyId.js
 * ```
 *
 * COMPLIANCE:
...
const SCRIPT_NAME = 'migrations.activities.backfillCompanyId.js';
...
const COLLECTION_ACTIVITIES = process.env.COLLECTION_ACTIVITIES;
if (!COLLECTION_ACTIVITIES) {
  console.error(`[${SCRIPT_NAME}] ERROR: COLLECTION_ACTIVITIES is required`);
  process.exit(1);
}
...
if (!COLLECTION_LEADS) {
  console.error(`[${SCRIPT_NAME}] ERROR: COLLECTION_LEADS is required`);
  process.exit(1);
}
...
const contactCompanyIdCache = new Map();
const leadCompanyIdCache = new Map();
...
async function getContactCompanyId(contactId) {
  if (contactCompanyIdCache.has(contactId)) { stats.cacheHits++; return contactCompanyIdCache.get(contactId); }
  stats.cacheMisses++;
  try {
    const doc = await db.collection(COLLECTION_CONTACTS).doc(contactId).get();
    const companyId = doc.exists ? (doc.data().companyId || null) : null;
    contactCompanyIdCache.set(contactId, companyId);
    return companyId;
  } catch (e) { contactCompanyIdCache.set(contactId, null); return null; }
}

async function getLeadCompanyId(leadId) {
  if (leadCompanyIdCache.has(leadId)) { stats.cacheHits++; return leadCompanyIdCache.get(leadId); }
  stats.cacheMisses++;
  try {
    const doc = await db.collection(COLLECTION_LEADS).doc(leadId).get();
    const companyId = doc.exists ? (doc.data().companyId || null) : null;
    leadCompanyIdCache.set(leadId, companyId);
    return companyId;
  } catch (e) { leadCompanyIdCache.set(leadId, null); return null; }
}
...
// =============================================================================
// DERIVE COMPANYID FROM RELATED ENTITY
// =============================================================================

async function deriveFromRelatedEntity(data) {
  // Activities may have relatedEntityType + relatedEntityId
  // Or specific fields like contactId, leadId, projectId

  // Try relatedEntityId first (polymorphic reference)
  if (data.relatedEntityId && data.relatedEntityType) {
    switch (data.relatedEntityType) {
      case 'contact':
        const contactCompanyId = await getContactCompanyId(data.relatedEntityId);
        if (contactCompanyId) return { companyId: contactCompanyId, from: 'relatedEntity:contact' };
        break;
      case 'lead':
        const leadCompanyId = await getLeadCompanyId(data.relatedEntityId);
        if (leadCompanyId) return { companyId: leadCompanyId, from: 'relatedEntity:lead' };
        break;
      case 'project':
        const projectCompanyId = await getProjectCompanyId(data.relatedEntityId);
        if (projectCompanyId) return { companyId: projectCompanyId, from: 'relatedEntity:project' };
        break;
    }
  }
  ...
  if (data.contactId) {
    const companyId = await getContactCompanyId(data.contactId);
    if (companyId) return { companyId, from: 'contactId' };
  }

  if (data.leadId) {
    const companyId = await getLeadCompanyId(data.leadId);
    if (companyId) return { companyId, from: 'leadId' };
  }

  if (data.projectId) {
    const companyId = await getProjectCompanyId(data.projectId);
    if (companyId) return { companyId, from: 'projectId' };
  }
...
async function backfillActivitiesCompanyId() {
  const startTime = Date.now();
  const report = createReportWriter(COLLECTION_ACTIVITIES, { outputPath: process.env.REPORT_OUTPUT_PATH });

  printHeader('ACTIVITIES COMPANYID BACKFILL MIGRATION', {
    'Target Company': COMPANY_ID,
    'Mode': DRY_RUN ? 'DRY-RUN' : 'EXECUTE',
    'Activities Collection': COLLECTION_ACTIVITIES,
    'Page/Batch Size': `${PAGE_SIZE}/${BATCH_SIZE}`,
    'Report File': report.getFilePath()
  });

  try {
    console.log('Scanning activities collection (streaming mode)...\n');
    let lastDoc = null, hasMore = true;

    while (hasMore) {
      stats.pages++;
      let query = db.collection(COLLECTION_ACTIVITIES).limit(PAGE_SIZE);
      ...
      for (const doc of snapshot.docs) {
        ...
        // Try to derive companyId: 1) relatedEntity lookup, 2) createdBy lookup
        let derivedCompanyId = null;
        let derivedFrom = null;

        // Try related entity first
        const relatedResult = await deriveFromRelatedEntity(data);
        if (relatedResult) {
          derivedCompanyId = relatedResult.companyId;
          derivedFrom = relatedResult.from;
        }
        ...
        if (!derivedCompanyId) {
          ...
          report.recordSkip({
            id: doc.id,
            reason: 'cannot_derive_companyId',
            details: `relatedEntityType=${data.relatedEntityType || 'N/A'}, relatedEntityId=${data.relatedEntityId || 'N/A'}, createdBy=${data.createdBy || 'N/A'}`
          });
          continue;
        }
...
backfillActivitiesCompanyId().then(s => process.exit(s ? 0 : 1)).catch(() => process.exit(1));
