...
export function PropertyQuickView({ property }: PropertyQuickViewProps) {
  ...
  const { areas, layout, orientations, linkedSpaces, price } = property;
  ...
  const pricePerSqm = price && property.area && property.area > 0
    ? Math.round(price / property.area)
    : null;
          ...
          <QuickViewRow
            ...
            value={
              <>
                ...
                {pricePerSqm && (
                  <span className="text-muted-foreground font-normal"> ({pricePerSqm.toLocaleString('el-GR')}€/m²)</span>
                )}
              </>
            }
          />
