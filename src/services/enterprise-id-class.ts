/**
 * ENTERPRISE ID GENERATION — CORE CLASS
 *
 * Cryptographically secure, collision-resistant ID generation. Extracted from
 * `enterprise-id.service.ts` (2026-05-06) to break a circular import:
 *
 *   service.ts re-exports convenience.ts
 *     ↓
 *   convenience.ts imports the singleton from enterprise-id-singleton.ts
 *     ↓
 *   singleton.ts imports THIS class file (linear, no cycle)
 *
 * The class has a single dependency (`./enterprise-id-prefixes`), so this file
 * is a leaf in the import graph. The previous co-location of the class with
 * the convenience-helper re-exports created a webpack TDZ at runtime where
 * `const P = ENTERPRISE_ID_PREFIXES` was sometimes evaluated after a method
 * referenced it via the cycle.
 *
 * Public API exposed from `./enterprise-id.service` (facade) is unchanged —
 * consumers continue to import `EnterpriseIdService` / `enterpriseIdService` /
 * `generate*Id` from there.
 *
 * @module services/enterprise-id-class
...
export class EnterpriseIdService {
  ...
  generateFrameworkAgreementId(): string { return this.generateId(P.FRAMEWORK_AGREEMENT).id; }
