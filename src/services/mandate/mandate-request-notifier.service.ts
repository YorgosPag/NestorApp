...
export async function announceMandateRequestAnswer(
  ...
  try {
    ...
    const result = await dispatchNotification({
      ...
      //
      // 🔑 **Ο ΠΡΟΟΡΙΣΜΟΣ ΕΙΝΑΙ Ο ΙΔΙΩΤΙΚΟΣ ΧΩΡΟΣ — ΑΝΤΙΘΕΤΟΣ ΑΠΟ ΤΟΝ ΑΔΕΛΦΟ ΑΓΩΓΟ.**
      ...
      //    δίνει `read`, άρα το `/offers/<id>` ανοίγει. Ο `mandate-decision-notifier`
      //    ειδοποιεί τον **υπάλληλο** και οδηγεί στην **εντολή** (`mandateDetailHref`,
      //    ADR-841 Α18.12) — «συνεπής» αλλαγή και στους δύο = bug (το φυλά το Μ4).
      //
