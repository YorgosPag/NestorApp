/**
 * @fileoverview **ΓΙΑΤΙ ΔΕΝ ΓΡΑΦΤΗΚΕ ΤΟ ΑΙΤΗΜΑ** — το λεξιλόγιο, ποτέ η πράξη.
 * @related services/mandate/mandate-request.service.ts (ο ΓΡΑΦΕΑΣ) · ADR-827 §9.17 · ADR-834 §6.2
 * @module services/mandate/mandate-request-vocabulary
 *
 * ────────────────────────────────────────────────────────────────────────────
 * 🔴 ΞΕΧΩΡΙΣΤΟ ΑΡΧΕΙΟ — ΙΔΙΟΣ ΛΟΓΟΣ ΜΕ ΤΑ ΔΥΟ ΑΔΕΛΦΙΑ ΤΟΥ, ΚΑΙ ΕΝΑΣ ΑΚΟΜΗ
 * ────────────────────────────────────────────────────────────────────────────
 *
 * Το `mandate-acceptance-types.ts` και το `mandate-decision-vocabulary.ts` έχουν ήδη
 * γράψει το γιατί: **τους ίδιους τύπους τους χρειάζονται πολλοί**, και γραμμένοι μέσα
 * στον γραφέα η εισαγωγή γίνεται βαρύτερη από την πληροφορία.
 *
 * 🔴 **ΚΑΙ ΕΔΩ ΥΠΑΡΧΕΙ ΤΡΙΤΟΣ ΛΟΓΟΣ, ΜΕΤΡΗΜΕΝΟΣ**: ο γραφέας είναι `server-only`, ενώ
 * το λεξιλόγιο το ζητούν **πελατικά** αρχεία — `mandate-request-form-labels.ts`
 * (`'use client'`) και οι άγκυρές του. Όσο ζητούσαν **τύπο** (`import type`, που
 * σβήνεται στη μεταγλώττιση) δούλευε κατά τύχη· η άγκυρα
 * `mandate-request-labels.test.ts` όμως ζητά την **ΤΙΜΗ** `MANDATE_REQUEST_REJECTIONS`,
 * και στέκεται μόνο επειδή το jest **μοκάρει** το `server-only`. Δηλαδή ένα πελατικό
 * αρχείο απείχε **μία εισαγωγή τιμής** από εξαίρεση στον φυλλομετρητή.
 *
 * ⇒ Το λεξιλόγιο είναι **leaf**: μόνο τύποι και μία κυριολεκτική λίστα, καμία
 * ανάγνωση, κανένα ρολόι, **κανένα `server-only`**. Ο γραφέας μένει κλειστός.
 *
 * **Layering**: leaf.
 */

import type {
  MandateRequest,
  MandateRequestInvariant,
  ProposedMandateTerms,
} from '@/types/mandate-request';

/**
 * **Γιατί δεν γράφτηκε το αίτημα** — κλειστό σύνολο, κάθε κωδικός γίνεται κλειδί i18n.
 *
 * 🔴 **ΤΟ `agency-absent` ΣΚΕΠΑΖΕΙ ΔΥΟ ΑΛΗΘΕΙΕΣ, ΚΑΙ ΕΙΝΑΙ ΤΟ ΙΔΙΟ ΤΟ §9.4.** Γραφείο
 * που **δεν δημοσιεύεται** και ψευδώνυμο που **δεν υπάρχει** απαντούν **ταυτόσημα**.
 * Αν τα ξεχωρίζαμε, η πόρτα θα γινόταν **μαντείο** *«υπάρχει τέτοιος οργανισμός;»* —
 * δηλαδή απαρίθμηση ένα ερώτημα τη φορά, ακριβώς ό,τι απαγορεύει το ADR-787 Ε-5 §4 #1.
 *
 * ⚠️ **Η ΒΛΑΒΗ ΔΕΝ ΕΙΝΑΙ ΕΔΩ.** *«Δεν μπόρεσα να ρωτήσω»* είναι `kind: 'unavailable'`
 * και φεύγει ως **503** — ποτέ ως άρνηση (N.12: *άγνωστο ≠ κενό*). Ένα
 * `agency-absent` σε βλάβη θα έστελνε τον άνθρωπο μακριά από γραφείο που **υπάρχει**.
 */
export const MANDATE_REQUEST_REJECTIONS = [
  /** Η αγγελία δεν υπάρχει — **ή δεν είναι δική σου**. Ποτέ χωριστά (§9.4). */
  'listing-absent',
  /** Αποσυρμένη αγγελία **δεν έχει δημόσια προβολή** ⇒ αίτημα για το τίποτα. */
  'listing-not-live',
  /**
   * Έχει **ήδη** εντολή. Για την προεπιλεγμένη `EXCLUSIVE_AGENCY` μια δεύτερη
   * ανάθεση θα ήταν **παράβαση της ίδιας της σύμβασης** που το σύστημα συνέταξε.
   */
  'listing-already-brokered',
  /** Το γραφείο δεν δημοσιεύεται — **ταυτόσημο** με «δεν υπάρχει». */
  'agency-absent',
  /**
   * 🔴 **Το ΓΕΜΗ λέει ότι η επιχείρηση ΔΕΝ είναι ενεργή** (ADR-841 §7 Α23 Φ3.2).
   *
   * ⚠️ **ΔΕΝ είναι το `agency-absent`**: η βιτρίνα **υπάρχει και φαίνεται**, με την ετικέτα
   * «σύμφωνα με το ΓΕΜΗ δεν είναι ενεργή» — άρα η άρνηση **δεν αποκαλύπτει** τίποτα. Νέα εντολή σε
   * διαγραμμένη επιχείρηση θα ήταν σύμβαση μεσιτείας με αριθμό ΓΕΜΗ που δεν ισχύει (ν. 4072/2012 άρθ.
   * 200)· Google Actions Center: οριστικά κλειστή δεν δέχεται κράτηση. Ο κριτής: `registryClosureOf`.
   */
  'agency-closed',
  /**
   * 🔴 **ΤΟ ΓΡΑΦΕΙΟ ΥΠΑΡΧΕΙ, ΦΑΙΝΕΤΑΙ — ΚΑΙ ΔΕΝ ΑΣΚΕΙ ΜΕΣΙΤΕΙΑ** (ADR-841 §7 Α5).
   *
   * 🔴 **Γεννήθηκε από μετρημένο εύρημα οθόνης (2026-09-06)**: η βιτρίνα ενός
   * γραφείου **εγκαταστάσεων φυσικού αερίου** *(ISCO `7126`)* πρόσφερε «ζητήστε
   * του να αναλάβει το ακίνητό σας», και **κανένας** από τους τέσσερις κρίκους
   * της διαδρομής δεν ρωτούσε αν ασκεί μεσιτεία — άρα το αίτημα **γραφόταν**.
   *
   * ⚠️ **ΔΕΝ είναι το `agency-absent`, και η διαφορά είναι όλη η ουσία.** Εκείνο
   * λέει *«δεν υπάρχει τέτοια βιτρίνα»* και **οφείλει** να σκεπάζει δύο αλήθειες,
   * γιατί το αντίθετο θα έκανε την πόρτα μαντείο *«υπάρχει τέτοιος οργανισμός;»*.
   * Αυτό λέει *«η βιτρίνα **υπάρχει και τη βλέπεις** — απλώς αυτή η πράξη δεν
   * είναι δική της»*, δηλαδή **δεν αποκαλύπτει τίποτα** που ο άνθρωπος δεν έχει
   * ήδη μπροστά του. Ένας κοινός κωδικός θα τον έστελνε στον κατάλογο να ψάξει
   * γραφείο που **μόλις επισκεπτόταν**.
   *
   * ⚠️ **Καμία διέξοδος στο `REJECTION_REMEDY`, και είναι σωστό**: δεν υπάρχει
   * πεδίο να διορθώσει ο άνθρωπος — δεν έκανε λάθος. Ο μόνος «άλλος δρόμος» είναι
   * **άλλο γραφείο**, και αυτόν τον δείχνει η ίδια η βιτρίνα με την πράξη που
   * όντως δέχεται.
   *
   * 🔑 Ο κριτής είναι **ένας** (`lib/professional/showcase-acts.ts`), και τον
   * ρωτούν και οι τρεις οθόνες της διαδρομής.
   */
  'agency-not-brokerage',
  /** Εκκρεμεί ήδη αίτημα με **άλλους** όρους. Απόσυρε το πρώτο. */
  'request-already-pending',
  /**
   * 🔴 Το γραφείο είπε **τελικά όχι** για αυτή την αγγελία (`declined-final`).
   *
   * ⛔ **ΑΝΤΙΚΑΤΕΣΤΗΣΕ το `request-terms-unchanged`, ΔΕΝ ΜΠΗΚΕ ΔΙΠΛΑ ΤΟΥ** (ADR-749:
   * δύο κριτές στο ίδιο ερώτημα). Ο παλιός ρωτούσε *«άλλαξες κάτι;»* — υπολογισμένη
   * διαφορά όρων, δηλαδή **εικασία** για το τι θα ξαναρωτούσε ο κριτής. Ο νέος ρωτά
   * *«σου έδωσε δικαίωμα;»* — **ρητή πράξη** εκείνου που έκρινε.
   *
   * 🔑 Και η αλλαγή έχει **λειτουργικό** αποτέλεσμα, όχι μόνο σημασιολογικό: μετά από
   * `declined-revisable` ο ιδιώτης μπορεί να ξαναστείλει **τους ίδιους ακριβώς όρους**
   * — που είναι ό,τι θέλει ένας μεσίτης που είπε *«στείλε το ξανά, μου ξέφυγε»*, και
   * ό,τι ο παλιός κανόνας **απαγόρευε**.
   */
  'request-declined-final',
  /**
   * 🔴 **ΣΥΓΚΡΟΥΕΤΑΙ ΜΕ ΖΩΝΤΑΝΗ ΕΝΤΟΛΗ ΑΛΛΟΥ ΓΡΑΦΕΙΟΥ** (ADR-832).
   *
   * ⚠️ **ΔΕΝ είναι το `listing-already-brokered`, και η διαφορά δεν είναι λεπτολογία.**
   * Εκείνο σημαίνει *«αυτό το ζεύγος έχει ήδη κλείσει»* — το ίδιο γραφείο, ήδη
   * αποδεκτό. Αυτό σημαίνει *«**άλλος** κρατά δικαίωμα που δεν χωρά δίπλα στο δικό
   * σου»* — και έχει **διέξοδο**: άλλη πράξη, άλλη περίοδος, ή απλή αντί για
   * αποκλειστική. Ένας κοινός κωδικός θα έκρυβε και τους τρεις δρόμους.
   */
  'listing-conflicting-mandate',
  /**
   * 🔴 **ΤΟ ΠΡΟΦΙΛ ΤΟΥ ΙΔΙΟΥ ΤΟΥ ΑΙΤΟΥΝΤΟΣ ΔΕΝ ΟΝΟΜΑΖΕΙ ΣΥΜΒΑΛΛΟΜΕΝΟ** (ADR-834 §6.2).
   *
   * ⚠️ **Είναι ο ΜΟΝΟΣ κωδικός αυτού του συνόλου που δεν αφορά την αγγελία ή το
   * γραφείο — αφορά ΕΚΕΙΝΟΝ ΠΟΥ ΠΑΤΗΣΕ.** Και γι' αυτό είναι ο μόνος με **ενεργή
   * διέξοδο μέσα στην ίδια την οθόνη**: το μήνυμά του κουβαλά σύνδεσμο προς το
   * `/profile` (`REJECTION_REMEDY`), γιατί η διόρθωση είναι **δύο πεδία μακριά**.
   *
   * 🔑 **Ίδιο όνομα με την άρνηση της αποδοχής, και είναι σκόπιμο**: ένα ερώτημα,
   * ένας κριτής, ένα όνομα. Τα δύο κλειστά σύνολα μένουν χωριστά (άλλο ακροατήριο,
   * άλλο κείμενο) — αλλά όποιος διαβάσει το ένα αναγνωρίζει το άλλο.
   *
   * ⚠️ **ΔΕΝ σπάει σε τρεις κωδικούς** (όνομα · email · ΑΦΜ): και τα τρία λύνονται
   * στην **ίδια** οθόνη, με την ίδια πράξη. Τρεις αρνήσεις θα ήταν τρία ταξίδια για
   * μία διόρθωση — ίδιο σκεπτικό με το `agency-absent`, που σκεπάζει δύο αλήθειες
   * επειδή η **θεραπεία** είναι μία.
   */
  'identity-incomplete',
] as const;

export type MandateRequestRejection = (typeof MANDATE_REQUEST_REJECTIONS)[number];

/**
 * **Τι απέγινε η προσπάθεια** — κλειστό σύνολο, ποτέ `boolean` + μήνυμα.
 *
 * 🔑 Το `unchanged` **δεν είναι σφάλμα**: είναι η **επιτυχία** της ιδεμποτησίας. Ο
 * άνθρωπος που πάτησε δύο φορές πρέπει να δει το **αίτημά του**, όχι μια άρνηση για
 * κάτι που πέτυχε.
 */
export type MandateRequestWriteResult =
  | { readonly kind: 'created'; readonly request: MandateRequest }
  | { readonly kind: 'unchanged'; readonly request: MandateRequest }
  | { readonly kind: 'rejected'; readonly reason: MandateRequestRejection }
  | { readonly kind: 'invalid'; readonly violations: readonly MandateRequestInvariant[] }
  /** 🔴 **Δεν μάθαμε** — ποτέ ίδιο με άρνηση. */
  | { readonly kind: 'unavailable' }
  | { readonly kind: 'failed' };

/** Ό,τι δηλώνει ο ιδιώτης. **Καμία ταυτότητα** — εκείνη έρχεται από την απόδειξη. */
export interface MandateRequestDeclaration {
  readonly ownerPropertyId: string;
  readonly agencyCompanyId: string;
  readonly terms: ProposedMandateTerms;
}
