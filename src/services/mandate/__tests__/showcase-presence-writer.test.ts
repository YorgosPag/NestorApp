...
describe('Γ — ο ΕΝΑΣ γραφέας της αποδεδειγμένης παρουσίας', () => {
  ...
  it('🔴 Γ5 — ΒΙΤΡΙΝΑ ΠΟΥ ΔΕΝ ΥΠΑΡΧΕΙ ⇒ ΣΙΩΠΗ, ΚΑΙ ΟΥΤΕ ΚΑΝ ΕΡΩΤΗΜΑ ΑΓΓΕΛΙΩΝ', async () => {
    // 🔴 **Η ΚΑΝΟΝΙΚΗ ΠΕΡΙΠΤΩΣΗ, ΟΧΙ Η ΕΞΑΙΡΕΣΗ.** Η βιτρίνα είναι **opt-in** (§4.2): οι
    //    περισσότεροι οργανισμοί δημοσιεύουν **αγγελίες** χωρίς να δημοσιεύσουν ποτέ
    //    **βιτρίνα**. Μετρήθηκε ζωντανά στο `brokered-listing`: η πρώτη γραφή έβγαζε
    //    `warn` σε **κάθε** τέτοια δημοσίευση — δηλαδή ημερολόγιο που μαθαίνει τον
    //    αναγνώστη **να μην το διαβάζει**.
    //
    // ⚠️ Και μια **εξαίρεση** εδώ θα ακύρωνε τη δημοσίευση της **αγγελίας**: το μάρκετινγκ
    //    θα έριχνε τη **δέσμευση προς τον ιδιοκτήτη** (θεματοφυλακή, CHECK 3.56).
    const { fake, admin } = db();
    fake.seed(COLLECTIONS.PUBLIC_LISTINGS, 'prop_a', listingDoc('prop_a', HOME));
    ...
    // Καμία γραφή — και ο έλεγχος ύπαρξης **προηγείται**, οπότε ούτε το ερώτημα αγγελιών
    // εκτελέστηκε. Ο παρονομαστής ζει στο Γ1, όπου η βιτρίνα **υπάρχει** και γράφεται.
