...
export type ConsentRejection =
  ...
  | 'superseded'
  /**
   * 🔴 **ΔΙΚΟ ΜΑΣ ΣΦΑΛΜΑ, ΟΧΙ ΔΙΚΟ ΤΟΥ** (ADR-834 §6.5.στ).
   *
   * Δεν μπορέσαμε να **ελέγξουμε** τον σύνδεσμο — λείπει το μυστικό υπογραφής από
   * τον διακομιστή. Ο σύνδεσμος του ανθρώπου μπορεί να είναι **απολύτως έγκυρος**.
   *
   * ⚠️ **Μετρημένο ζωντανά 2026-08-31**: το `MANDATE_CONSENT_SECRET` λείπει από την
   * παραγωγή (Netcup), και ο ιδιοκτήτης που πάτησε τον σύνδεσμό του διάβασε
   * *«Ο σύνδεσμος δεν είναι έγκυρος»* — δηλαδή η οθόνη τον κατηγόρησε για **δική μας**
   * παράλειψη ρύθμισης, και η φυσική του αντίδραση («ο μεσίτης μού έστειλε σπασμένο
   * σύνδεσμο») ήταν **λάθος** και μη διορθώσιμη από εκείνον.
   *
   * 🔑 **Η διάκριση ΥΠΗΡΧΕ ΗΔΗ ένα στρώμα πιο κάτω και ισοπεδωνόταν εδώ**: το
   * `SignedTokenRejection` ονομάζει το `'server-config'` και το σχόλιό του λέει ρητά
   * *«δεν είναι σφάλμα του παραλήπτη»* — και μετά **τέσσερις** λόγοι έπεφταν σε
   * **ένα** `'link-invalid'`. Ίδιο σχήμα με το `clientNameFrom` (§6.5.δ) και το
   * `cause.data` (§6.5.ε): η πληροφορία υπήρχε, ο μεταφραστής την πετούσε.
   *
   * ⛔ **ΜΗΝ το ενώσεις ξανά με το `link-invalid` «για απλότητα»**: οι δύο ζητούν
   * **αντίθετη** ενέργεια από τον άνθρωπο — *υποψιάσου τον αποστολέα* ⇄ *ξαναδοκίμασε,
   * δεν φταις εσύ*. Και ο κωδικός HTTP διαφέρει για τον ίδιο λόγο (503, όχι 400):
   * 4xx λέει «το αίτημά σου», 5xx λέει «εμείς».
   */
  | 'service-unavailable';
...
function readToken(
  ...
  try {
    ...
    // ⚠️ **Η ΔΕΥΤΕΡΗ ΠΟΡΤΑ ΓΙΑ ΤΗΝ ΙΔΙΑ ΠΑΡΑΛΕΙΨΗ.** Το μυστικό μπορεί να λείπει
    // **πριν** καν κληθεί ο αποκωδικοποιητής (εδώ) **ή** μέσα του (`'server-config'`).
    // Δύο διαδρομές, **ένας** υπαίτιος: εμείς. Αν αυτή έμενε `'link-invalid'`, η
    // διόρθωση θα ήταν πράσινη στα test και **ψεύτικη στην παραγωγή** — γιατί εκεί
    // λείπει ολόκληρη η μεταβλητή, δηλαδή χτυπά **αυτή** η διαδρομή.
