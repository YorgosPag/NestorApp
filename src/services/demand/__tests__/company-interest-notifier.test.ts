...
describe('Υ — ο παραλήπτης είναι αυτός που καταχώρησε το ακίνητο', () => {
  ...
  /**
   * 🔴 **ΤΟ ΠΕΔΙΟ ΠΟΥ ΑΠΟΦΑΣΙΖΕΙ ΠΟΥ ΟΔΗΓΕΙ ΤΟ ΚΟΥΜΠΙ** (ADR-841 §7 Α18.9).
   *
   * Ο κοινός πυρήνας (`announcement-pass`) είναι **ένας** για τους δύο σαρωτές, και
   * μέχρι το Α18.9 έστελνε **και τους δύο** στην οθόνη του ιδιώτη. Ο σαρωτής
   * γραφείου είναι ο **μόνος** που ξέρει ότι διάβασε τη συλλογή `properties` — αν
   * σιωπήσει εδώ, ο επαγγελματίας ξαναβρίσκει το *«δεν είναι δικό σου»*.
   *
   * ⚠️ Η άγκυρα ρωτά την **τιμή**, όχι την παρουσία: ένα `source: 'owner-property'`
   * θα ήταν παρόν, τυπολογικά έγκυρο — και **ακριβώς το ελάττωμα**.
   */
  it('Υ4 🔴 — ο σαρωτής ΔΗΛΩΝΕΙ κατοχή γραφείου, αλλιώς κληρονομεί την πόρτα του ιδιώτη', async () => {
    withSeekers(3);
    await announceInterestToCompanyStaff(dbReturning([property()]) as never);
