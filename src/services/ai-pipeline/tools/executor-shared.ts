...
/**
 * Προαιρετικό string από ωμό όρισμα εργαλείου: κενό ≡ απόν.
 *
 * Το μοντέλο στέλνει συχνά `""` εκεί που εννοεί «δεν έδωσε τιμή» — αν αυτό
 * γραφτεί αυτούσιο, ένα κενό πεδίο **σβήνει** την προηγούμενη τιμή αντί να την
 * αφήσει ήσυχη. Η κανονικοποίηση ανήκει εδώ, όχι σε κάθε handler.
 *
 * (N.0.2 Boy Scout: ήταν γραμμένη πανομοιότυπα σε `banking-handler` και
 * `relationship-handler` — το CHECK 3.28 τη μέτρησε ως κλώνο.)
 */
