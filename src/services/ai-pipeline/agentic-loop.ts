...
NESTED DATA — ΟΛΟΚΛΗΡΩΜΕΝΑ DOCUMENTS:
- Τα units documents περιέχουν ΟΛΕΣ τις πληροφορίες μέσα τους (nested objects):
  - commercial.askingPrice, commercial.finalPrice, commercial.buyerContactId, commercial.buyerName
  - commercial.paymentSummary.totalAmount, .paidAmount, .remainingAmount, .paidPercentage
  - commercial.paymentSummary.totalInstallments, .paidInstallments, .overdueInstallments
  - commercial.paymentSummary.nextInstallmentAmount, .nextInstallmentDate
  - areas.gross, areas.net, areas.balcony, areas.terrace, areas.garden
- ΜΗΝ ψάχνεις ξεχωριστό collection για πληρωμές/τιμές — ΟΛΕΣ οι πληροφορίες είναι ΜΕΣΑ στο unit document
- Για ερωτήσεις "πόσα χρωστάει", "τι τιμή έχει", "πόσες δόσεις" → κάνε firestore_query("units") ΧΩΡΙΣ filters σε nested πεδία → διάβασε τα nested πεδία από τα αποτελέσματα
- ΚΡΙΣΙΜΟ: ΜΗΝ κάνεις WHERE σε nested πεδία (commercial.*, areas.*) — θα αποτύχει λόγω missing index! Αντί αυτού κάνε BROAD query (π.χ. χωρίς filter ή μόνο με status) και φιλτράρεσε τα αποτελέσματα εσύ
- Παράδειγμα: "ληξιπρόθεσμες δόσεις" → firestore_query("units") → φίλτραρε στα αποτελέσματα αυτά με commercial.paymentSummary.overdueInstallments > 0
...
export async function executeAgenticLoop(
  ...
  try {
    ...
    const summaryResponse = await callOpenAI(messages, [], cfg.perCallTimeoutMs);
    const summaryContent = summaryResponse.choices?.[0]?.message?.content;

    if (summaryContent) {
      const { answer: summaryAnswer, suggestions: summarySuggestions } = parseSuggestions(summaryContent);
      return {
