 ...
 *
 * ✅ **Η παγίδα που περιέγραφε αυτό το σχόλιο ΔΕΝ ΥΠΑΡΧΕΙ ΠΙΑ** (ADR-801 Φάση 3):
 * το `isAdminRole()` του `security-policy.ts` — λίστα `['admin','broker',
 * 'builder']`, **άλλο λεξιλόγιο** από παλαιότερο σύστημα ρόλων, που επέστρεφε
 * `false` για `'company_admin'` — **διαγράφηκε** μαζί με το `ADMIN_ROLES` του,
 * αφού μετρήθηκε ότι είχε **μηδέν** καταναλωτές. *Ένα σχόλιο δεν είναι
 * φρουρός* (CHECK 3.36): η αφαίρεση του δολώματος είναι η θεραπεία.
 *
 * 🔶 **Αυτό το σύνολο μένει ρητό, και είναι απόφαση**: απαντά *«ποιον θεωρεί
 * διαχειριστή το επίπεδο ικανοτήτων του AI agent;»*. Ο κριτής του ADR-801 κρίνει
 * **ικανότητα** (`PermissionId`), όχι *«είναι διαχειριστής;»*. Μετακινείται στο
 * `decideCapability` όταν αποκτήσει τη δική του ικανότητα στο `PERMISSIONS` —
 * όχι νωρίτερα, γιατί μια ικανότητα χωρίς όνομα δεν είναι κριτήριο.
 */
