...
async function supersedePreviousInvitations(
  ...
  // 🔑 **Ο ΑΞΟΝΑΣ ΕΙΝΑΙ ΤΟ ΚΑΝΑΛΙ, ΚΑΙ ΕΙΝΑΙ Ο ΜΟΝΟΣ ΥΠΟΨΗΦΙΟΣ** — ADR-843 · ADR-747 §13.7.
  //
  // Το ADR-747 §13.7 #2 κατέγραψε εδώ *«συγκρίνει μόνο τον στόχο, ποτέ τον εκδότη ⇒
  // άλλη εταιρεία σβήνει τον ζωντανό σύνδεσμο της πρώτης»*. **Μετρήθηκε 2026-09-21 και
  // δεν στέκει**, για τρεις ανεξάρτητους λόγους:
  //
  //   1. **Δεν υπάρχει «εκδότης εταιρεία» να συγκριθεί.** Η πρόσκληση γεννιέται στο
  //      `POST /api/first-contacts/guest` — **ανώνυμη** διαδρομή, χωρίς `ctx`. Εκδότης
  //      είναι ο **ίδιος ο ζητών**, και το έγγραφο δεν φέρει (ούτε πρέπει να φέρει)
  //      `companyId`.
  //   2. **Η αγκύρωση ΕΙΝΑΙ ήδη ο άξονας.** Το `channelEmail` είναι ο ζητών· άρα το
  //      ερώτημα δεν βλέπει ποτέ προσκλήσεις **άλλου ανθρώπου**. Δύο γραφεία που
  //      προσεγγίζει ο **ίδιος** άνθρωπος δίνουν δύο **διαφορετικούς στόχους**, και το
  //      `sameFirstContactTarget` τους ξεχωρίζει: καμία δεν παύει την άλλη.
  //   3. **Το `agencyCompanyId` ΑΠΑΓΟΡΕΥΕΤΑΙ ρητά ως άξονας** — `tenant-config.ts`
  //      (FIRST_CONTACTS): *«υπάρχει σε ΜΕΡΙΚΑ μόνο έγγραφα· άξονας που λείπει από
  //      κάποια έγγραφα δεν είναι άξονας, είναι σιωπηλή εξαίρεση»*. Ο στόχος είναι
  //      **διακριτή ένωση** (αγγελία ή επαγγελματίας) — ο ζητών υπάρχει ΠΑΝΤΑ.
  //
  // ⛔ Άρα η «προφανής διόρθωση» `where(companyId)` δεν είναι απλώς περιττή: θα
  //    φιλτράριζε σε πεδίο **που δεν υπάρχει σε κανένα έγγραφο** ⇒ μηδέν αποτελέσματα
  //    ⇒ **καμία** παλιά πρόσκληση δεν θα έπαυε ⇒ δύο ζωντανοί σύνδεσμοι για τον ίδιο
  //    στόχο, δηλαδή ακριβώς το σφάλμα που αυτή η συνάρτηση υπάρχει για να αποτρέψει.
  // tenant-scope-exempt: άξονας = channelEmail (ο ζητών), ίδιο σχήμα με FIRST_CONTACTS/seekerUserId
