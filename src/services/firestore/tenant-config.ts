...
const TENANT_OVERRIDES: Partial<Record<CollectionKey, TenantFieldConfig>> = {
  ...
  // --- ADR-777 §8.74: Η ΑΠΟΘΗΚΕΥΣΗ ΑΓΓΕΛΙΑΣ ----------------------------------
  // 🔴 ΔΕΝ διαβάζεται από πελάτη (read/write: false). Ίδιο δόγμα με το FIRST_CONTACTS: άξονας
  // είναι αυτός που κάνει την πράξη, και υπάρχει σε ΚΑΘΕ έγγραφο. Η πλευρά του ΚΑΤΟΧΟΥ
  // («πόσοι κράτησαν ΑΥΤΗ την αγγελία») ΔΕΝ είναι ερώτηση απομόνωσης αλλά ΓΟΝΕΑ — μετά τον
  // `mayAdminister` (CHECK 3.56)· δηλώνεται στο σημείο κλήσης με `tenant-scope-exempt` + λόγο.
  SAVED_LISTINGS: { mode: 'userId', fieldName: 'saverUserId' },

  // --- ADR-882 Φάση 2: ΤΟ ΙΣΤΟΡΙΚΟ ΑΝΑΖΗΤΗΣΕΩΝ ΤΟΠΟΥ ----------------------------
  // Ένα έγγραφο ανά άνθρωπο (`uplsrch_{uid}`), ανάγνωση ΜΟΝΟ by-id. Ο άξονας είναι ο
  // ιδιοκτήτης — ένας ιδιώτης δεν έχει εταιρεία, και το ιστορικό του δεν ανήκει σε καμία.
  USER_PLACE_SEARCHES: { mode: 'userId', fieldName: 'userId' },
  ...
  // ✏️ ADR-867 Β7 — Η ΑΝΑΘΕΩΡΗΣΗ: ίδια φύση με την ανάκληση (αντίγραφο συμμόρφωσης που κανείς
  // πελάτης δεν διαβάζει), άρα ίδιος άξονας — καμία δήλωση χώρου που θα υποσχόταν ανάγνωση.
  NETWORK_MESSAGE_REVISIONS: {
    mode: 'none',
    fieldName: '',
    unscopedCategory: 'cross-space-thread',
    unscopedReason:
      'ADR-867 Β7 — το κείμενο ενός μηνύματος ΠΡΙΝ από επεξεργασία (αντίγραφο συμμόρφωσης). Ακολουθεί το νήμα του, που ανήκει σε ΔΥΟ πλευρές διαφορετικών χώρων (και, στο νήμα σχέσης, σε πρόσωπο ΧΩΡΙΣ χώρο). ΚΑΜΙΑ ανάγνωση πελάτη — ούτε του αποστολέα: αλλιώς η επεξεργασία θα έσβηνε ίχνος που ο άλλος δεν βλέπει. Ανάγνωση μόνο από διακομιστή για ΓΚΠΔ άρθρο 17 §3(ε)· γράφει ΜΟΝΟ ο `services/network-messaging/thread-messages.ts` (CHECK 3.89).',
  },
