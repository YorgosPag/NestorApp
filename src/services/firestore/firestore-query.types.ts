...
export type UnscopedCategory =
  ...
  | 'public-world'
  /**
   * **Δημοσιευμένη προβολή** (ADR-777 Α3/Α5): παράγωγο δεδομένου **μισθωτή**, που ο
   * κάτοχός του επέλεξε ρητά να δημοσιεύσει.
   *
   * 🔴 **Δεν είναι `public-world`, και η διάκριση δεν είναι ταξινομική — είναι
   * ΚΥΚΛΟΣ ΖΩΗΣ.** Ένα `public-world` έγγραφο υπάρχει **ανεξάρτητα** από κάθε
   * λογαριασμό και **δεν σβήνεται** όταν φύγει ένας πελάτης· μια δημοσιευμένη προβολή
   * **οφείλει να πάψει να υπάρχει** τη στιγμή που η αγγελία παύει να δημοσιεύεται.
   * Ένα «κοινό φυσικό γεγονός» που εξαφανίζεται επειδή κάποιος πάτησε «απόσυρση» δεν
   * ήταν ποτέ φυσικό γεγονός.
   *
   * ⚠️ **Το να το βάφαμε `public-world` θα ήταν ακριβώς το σφάλμα που διόρθωσε το Β1**:
   * ένα `mode: 'none'` που έκρυβε **δύο** διαφορετικά πράγματα κάτω από μία λέξη. Ο
   * τύπος υπάρχει για να το κάνει αδύνατο — να τον παρακάμψουμε στην πρώτη ευκαιρία
   * θα ακύρωνε τον λόγο ύπαρξής του.
   *
   * Ισχύει το ίδιο συμβόλαιο εγγραφής: ο πελάτης **διαβάζει**· γράφει **μόνο ο
   * διακομιστής** — εδώ ως **παραγωγός προβολής**, όχι ως επαληθευτής πηγής.
   */
  | 'published-projection'
  /**
   * **Καθολικό ευρετήριο αντίστροφης αναζήτησης** (ADR-787 §5.3 δ).
   *
   * 🔴 **ΔΕΝ είναι `system`, και η διάκριση έχει ασφαλιστική συνέπεια.** Το `system`
   * λέει *«δεν ανήκει σε μισθωτή»*· εδώ **κάθε εγγραφή ανήκει** — φέρει `companyId`
   * στο σώμα της. Αυτό που λείπει δεν είναι η ιδιοκτησία, είναι η **δυνατότητα να
   * φιλτράρεις με αυτήν**: η ερώτηση είναι **αντίστροφη** *(«ποιος χώρος έχει αυτό
   * το ψευδώνυμο;»)*, άρα ένα `where('companyId', …)` θα απαιτούσε να ξέρεις ήδη την
   * απάντηση.
   *
   * ⚠️ Το να το βάφαμε `system` θα έλεγε στον επόμενο *«κοινό, δεν ανήκει σε
   * κανέναν»* — και θα ήταν εύλογο να συμπεράνει ότι μπορεί να το **απαριθμήσει**
   * ελεύθερα. Είναι ακριβώς η **απαρίθμηση** που απαγορεύει το ADR-787 Ε-5 §4 #1:
   * ένας κατάλογος ψευδωνύμων λέει ποια γραφεία υπάρχουν.
   *
   * ⇒ Συμβόλαιο: **σημειακή ανάγνωση κατά κλειδί, ποτέ σάρωση**· γράφει **μόνο ο
   * διακομιστής**· τα `firestore.rules` επιβάλλουν και τα δύο.
   */
  | 'global-index'
  /**
   * **Νήμα ανάμεσα σε δύο πλευρές που ΔΕΝ μοιράζονται χώρο** (ADR-867 §4.1).
   *
   * 🔴 **ΔΕΝ είναι `system`, και η διάκριση έχει ασφαλιστική συνέπεια** — ο ίδιος λόγος
   * με το `global-index` από πάνω, με **αντίστροφο** σχήμα. Το `system` λέει *«κοινό,
   * δεν ανήκει σε κανέναν»*· εδώ το έγγραφο ανήκει σε **ΔΥΟ** πλευρές ταυτόχρονα, και
   * η μία από αυτές (νήμα **σχέσης**, ADR-834 (γ) ②) είναι **πρόσωπο χωρίς χώρο**. Να το
   * βάφαμε `system` θα έλεγε στον επόμενο ότι μπορεί να το **απαριθμήσει** — δηλαδή να
   * διαβάσει ιδιωτικές συνομιλίες.
   *
   * 🔑 **Η εμβέλεια ΥΠΑΡΧΕΙ — απλώς δεν είναι πεδίο: είναι το ΑΚΡΟΑΤΗΡΙΟ** (υποσυλλογή
   * `network_audience/{uid}`), και το επιβάλλουν τα `firestore.rules`. Ένα `companyId`
   * πάνω στο έγγραφο θα υπήρχε στα **μισά** έγγραφα (πράξη ναι, σχέση όχι) — ακριβώς ο
   * «άξονας που λείπει από μισή συλλογή» που απαγορεύει το CHECK 3.35.
   *
   * ⇒ Συμβόλαιο: **σημειακή ανάγνωση κατά κλειδί, ποτέ σάρωση**· ο κατάλογος νημάτων
   * σερβίρεται από τον **διακομιστή**· γράφει **μόνο** ο ένας γραφέας (CHECK 3.89).
   */
  | 'cross-space-thread';
