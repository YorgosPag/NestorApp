...
export interface FirstContactVerificationEmailData {
  ...
  targetLabel: string;
  /**
   * **Πλήρες URL της δημόσιας σελίδας που πλησίασε**, ή `null` όταν δεν υπάρχει.
   *
   * 🔴 **ΘΕΡΑΠΕΥΕΙ ΤΟ ΔΗΛΩΜΕΝΟ ΟΡΙΟ #1 ΤΟΥ ADR-844** *(εγκρίθηκε 2026-09-05)*: η
   * ετικέτα είναι **γενική** *(«την αγγελία που είδατε»)*, γιατί ο πραγματικός τίτλος
   * θα απαιτούσε ανάγνωση δημόσιας αγγελίας μέσα από το σύνορο του ADR-839 (CHECK
   * 3.74). Άρα ο άνθρωπος που πλησίασε **δύο** αγγελίες έπαιρνε δύο **όμοια** email με
   * διαφορετικά κλειδιά: δούλευαν και τα δύο, αλλά **δεν ξεχώριζαν με το μάτι** — και
   * αν πατούσε λάθος, έστελνε το μήνυμά του αλλού απ' ό,τι νόμιζε.
   *
   * 🔑 **Κοστίζει ΜΗΔΕΝ αναγνώσεις**: το `listingId` ζει ήδη μέσα στη δήλωση. Δεν
   * μαθαίνουμε τίποτα για την αγγελία — απλώς **δείχνουμε πού ήταν ο άνθρωπος**.
   *
   * ⚠️ **`null` για τον επαγγελματία, και είναι δηλωμένο κενό**: ο στόχος
   * `professional` κρατά `agencyCompanyId`, που **δεν είναι** δημόσια διεύθυνση — η
   * βιτρίνα ζει σε `alias` που θα απαιτούσε ανάγνωση. Το κενό δεν καλύπτεται με μαντεψιά.
   */
...
export function buildFirstContactVerificationEmail(
  ...
  const href = data.targetHref ?? null;

  // 🔑 **Ο ΣΥΝΔΕΣΜΟΣ ΤΥΛΙΓΕΙ ΤΗΝ ΕΤΙΚΕΤΑ, ΔΕΝ ΜΠΑΙΝΕΙ ΔΙΠΛΑ ΤΗΣ.** Μια δεύτερη γραμμή
  //    *«δείτε την αγγελία»* θα ήταν **δεύτερο κουμπί** σε email που έχει ήδη ένα, και το
  //    μάτι θα διάλεγε λάθος: ο σκοπός του μηνύματος είναι η **επιβεβαίωση**, όχι η
  //    επιστροφή στην αγγελία.
  // ⚠️ **Χωρίς `<strong>` όταν είναι σύνδεσμος** — η υπογράμμιση ήδη το ξεχωρίζει· και
  //    τα δύο μαζί θα το έκαναν να μοιάζει με το κύριο κουμπί.
  const targetHtml = href === null
    ? `<strong>${escapeHtml(data.targetLabel)}</strong>`
  ...
  const textLines = [
    ...
    // ⚠️ **Δική του γραμμή στο απλό κείμενο, όχι τυλιγμένος** — εκεί δεν υπάρχει
    //    υπερσύνδεσμος να τυλίξει τίποτα, και μια γυμνή διεύθυνση μέσα σε πρόταση
    //    σπάει στο τύλιγμα γραμμής των περισσότερων πελατών αλληλογραφίας.
    ...(href === null ? [] : [`Η σελίδα της: ${href}`]),
