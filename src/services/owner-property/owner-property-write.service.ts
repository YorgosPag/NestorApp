 ...
 *
 * ────────────────────────────────────────────────────────────────────────────
 * 🔴 ΤΑ INVARIANTS ΤΗΣ **ΕΝΤΟΛΗΣ** ΟΜΩΣ ΚΡΙΝΟΝΤΑΙ — ΟΤΑΝ ΑΛΛΑΖΕΙ Η **ΣΥΜΦΩΝΙΑ**
 * ────────────────────────────────────────────────────────────────────────────
 *
 * Μέχρι τις 2026-08-29 αυτή η πόρτα **δεν έκρινε τίποτα**: ήταν ο δηλωμένος
 * «μοναδικός γραφέας του `mandate`» και έγραφε **ό,τι του δώσουν**. Η κρίση ζούσε
 * μόνο στο {@link createOwnerProperty} — δηλαδή **μόνο στη γέννηση αγγελίας**.
 *
 * ⚠️ **Και η Φάση Β του ADR-827 θα περνούσε από ΕΔΩ** (μετάβαση `self → brokered` σε
 * **υπάρχουσα** αγγελία), δηλαδή θα γεννούσε εντολή από πόρτα που **δεν κρίνει**. Το
 * νέο `mandate-term-exceeds-statute` θα ήταν, κατά γράμμα, **αδρανής φρουρός**
 * (ADR-749 §5): κανόνας γραμμένος στη μία από τις δύο διαδρομές.
 *
 * 🔑 **ΚΡΙΝΕΤΑΙ Ο,ΤΙ ΑΛΛΑΖΕΙ, ΟΧΙ Ο,ΤΙ ΓΡΑΦΕΤΑΙ — και η διαφορά είναι λειτουργική.**
 * Ο σαρωτής λήξης (`mandate-expiry.service.ts`) ξαναγράφει την **ίδια** εντολή για να
 * ανανεώσει τη δημόσια προβολή μιας **ληγμένης** αγγελίας. Αν κρίναμε κάθε γραφή, ο
 * σαρωτής θα κοκκίνιζε στο `mandate-expiry-past` και οι ληγμένες αγγελίες θα **έμεναν
