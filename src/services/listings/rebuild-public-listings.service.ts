 ...
 *
 * ────────────────────────────────────────────────────────────────────────────
 * ✅ Η ΑΠΟΔΕΔΕΙΓΜΕΝΗ ΠΑΡΟΥΣΙΑ ΞΑΝΑΧΤΙΖΕΤΑΙ **ΜΕΤΑΒΑΤΙΚΑ** — ΜΗΝ ΤΗΝ ΞΑΝΑΚΑΛΕΣΕΙΣ
 * ────────────────────────────────────────────────────────────────────────────
 *
 * Το `presence` του `agency_profiles` *(ADR-846 Φ5δ)* ενημερώνεται μέσα από το
 * {@link writeListingProjection}, απ' όπου περνά **κάθε** αγγελία αυτής της σάρωσης. Άρα
 * αυτή η πράξη είναι **ήδη** το δίχτυ ασφαλείας του πεδίου — **χωρίς καμία γραμμή εδώ**.
 *
 * ⚠️ **Το κόστος είναι γνωστό και μετρημένο**: **μία** επιπλέον ανάγνωση + **μία** γραφή
 * **ανά αγγελία**, αντί για μία ανά **γραφείο**. Σε μαζική επανασύνθεση 2.000 αγγελιών
 * αυτό είναι 2.000 περιττά ζεύγη.
 *
 * ⛔ **ΚΑΙ ΟΜΩΣ ΔΕΝ ΜΠΑΙΝΕΙ ΣΗΜΑΙΑ `deferPresence` ΣΤΟΝ ΓΡΑΦΕΑ.** Μια παρακάμψιμη
 * παράμετρος σε **χαμηλού επιπέδου** γραφέα είναι **ακριβώς** ο μηχανισμός που γέννησε
 * το περιστατικό της επικεφαλίδας: μια οικογένεια εγγράφων που *«δεν καλυπτόταν»* και
 * ανέφερε **`balanced: true`**. Η ιδεμποτής ορθότητα σε **κάθε** διαδρομή αξίζει
 * περισσότερο από ένα φραγμένο κόστος σε **χειροκίνητη** πράξη διαχειριστή.
 */
