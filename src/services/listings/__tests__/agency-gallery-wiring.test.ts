...
describe('Κ4 — ΚΗΔΕΜΟΝΙΑ: το ερώτημα ΔΕΝ γίνεται ποτέ πλατύτερο από τον μισθωτή', () => {
  it('🔴 ρωτά με `companyId` + οντότητα + ΤΟΥΣ ΔΥΟ ΚΑΔΟΥΣ — ποτέ σάρωση όλης της `files`', async () => {
    filesInCollection = [fileDoc({ id: 'file_a' })];

    await republishListing(adminDb, LISTING, PROPERTY, resolveAgency);

    // 🔴 **ΑΛΛΑΞΕ ΣΤΗΝ Α17.7 (2026-09-06), ΚΑΙ Η ΑΓΚΥΡΑ ΕΙΧΕ ΔΙΚΙΟ ΝΑ ΡΩΤΑ.** Μέχρι τότε
    //    το τέταρτο φίλτρο ήταν `['category','==','photos']` — δηλαδή **καμία κάτοψη δεν
    //    έφτανε ποτέ** στον κανόνα, όσο κι αν κάποιος τη δήλωνε. Αυτό ήταν η **πραγματική**
    //    αιτία του Ο-21, όχι η λευκή λίστα που το Ο-21 κατηγορούσε *(Α17.7.1)*.
    // ⛔ **ΜΗΝ το χαλαρώσεις σε «κανένα φίλτρο κατηγορίας»**: θα κατέβαζε συμβόλαια,
    //    τιμολόγια και άδειες για να τα πετάξει — περισσότερα reads και περιττή έκθεση.
    // 🔴 **ΤΡΙΤΟΣ ΚΑΔΟΣ ΣΤΟ ΒΗΜΑ Γ (ADR-845, 2026-09-08)**: το `models` μπήκε γιατί ένα 3D
    //    μοντέλο **δεν κατέβαινε καν** — δηλαδή οι δύο φρουροί του `agency-media-publication`
    //    δεν εκτελούνταν ποτέ γι' αυτό, **το ίδιο σχήμα με το Ο-21**. Η λίστα μένει
    //    **κυριολεκτική** επίτηδες: σύγκριση με τη σταθερά θα ήταν ταυτολογία και δεν θα
    //    έπιανε ποτέ διεύρυνση. Ο κανόνας «κάθε τιμή εδώ ΜΠΟΡΕΙ να φύγει» φυλάγεται
    //    εκτελεστά στο `agency-model-publication.test` (Κ4).
