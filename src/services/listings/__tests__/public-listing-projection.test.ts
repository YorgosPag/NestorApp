...
describe('Κ4 — η προβολή δεν κουβαλά ΚΑΜΙΑ ταυτότητα πελάτη', () => {
  ...
  it('το σχήμα εξόδου είναι ΚΛΕΙΣΤΟ — ακριβώς τα δηλωμένα κλειδιά', () => {
    ...
    expect(Object.keys(listing).sort()).toEqual([
      ...
      // 🔴 **ΕΒΔΟΜΗ ΦΟΡΑ ΠΟΥ ΑΥΤΗ Η ΑΓΚΥΡΑ ΚΟΚΚΙΝΙΣΕ (ADR-841 §7 Α17, 2026-09-03)** — οι
      // **κατόψεις**, **δίπλα** στη συλλογή και ποτέ μέσα της. Πέρασε από γραμμένη
      // απόφαση: το `alt` μιας κάτοψης λέει **άλλο πράγμα** από «Φωτογραφία N από M»,
      // και ένας πίνακας με **δύο σημασιολογίες** θα ξαναγεννούσε το Ο-18.
      //
      // ⛔ **ΚΑΙ ΤΟ ΠΙΟ ΣΗΜΑΝΤΙΚΟ ΕΙΝΑΙ ΤΙ ΤΑΞΙΔΕΥΕΙ ΜΕΣΑ**: `SourcedAttribute<ListingImage>`
      // — εικόνα **και προέλευση** *(δηλωμένη ή μετρημένη)*, ποτέ μονοπάτι, ποτέ ταυτότητα
      // αρχείου. Ο έλεγχος διαρροής (Κ4, από πάνω) το επιβεβαιώνει ανεξάρτητα.
      'floorplans',
      'gallery',
      ...
      'legality',
      // 🔴 **ΠΕΜΠΤΗ ΦΟΡΑ ΠΟΥ ΑΥΤΗ Η ΑΓΚΥΡΑ ΚΟΚΚΙΝΙΣΕ (ADR-777 §8.61, 2026-09-06)** — και
      // πάλι έκανε τη δουλειά της. Το `listedAt` πέρασε από **γραμμένη** απόφαση: η οθόνη 2
      // ταξινομούσε κατά `documentId`, δηλαδή **κατά τάξη συντάκτη** (`ownp_` πριν από
      // `prop_`), και καμία τίμια σειρά δεν ήταν δυνατή χωρίς πεδίο.
      //
      // ⛔ **ΚΑΙ ΤΟ ΠΙΟ ΣΗΜΑΝΤΙΚΟ ΕΙΝΑΙ ΤΙ *ΔΕΝ* ΤΑΞΙΔΕΥΕΙ**: καμία ημερομηνία που δεν
      // ξέρουμε. Η τιμή είναι ένωση `{kind:'known'} | {kind:'unknown', reason}` — ποτέ
      // «σήμερα» ως προεπιλογή, ποτέ το `projectedAt` μεταμφιεσμένο σε ημερομηνία αγοράς.
      'levels',
      'listedAt',
      // 🔴 **ΟΓΔΟΗ ΦΟΡΑ ΠΟΥ ΑΥΤΗ Η ΑΓΚΥΡΑ ΚΟΚΚΙΝΙΣΕ (ADR-845 Φ4.1, 2026-09-08)** — και
      // πάλι έκανε τη δουλειά της, **μέσα στο ίδιο tool call** που πρόσθεσε το πεδίο.
      // Το `models` πέρασε από **γραμμένη** απόφαση (Giorgio): το δημόσιο σχήμα είχε
      // **τρία** κουτιά, **όλα raster**, και ένα τρισδιάστατο μοντέλο δεν είχε πού να
      // καθίσει — φράγμα ④ του ADR-845 §2.3.
      //
      // ⛔ **ΚΑΙ ΤΟ ΠΙΟ ΣΗΜΑΝΤΙΚΟ ΕΙΝΑΙ ΓΙΑΤΙ ΕΙΝΑΙ **ΠΙΝΑΚΑΣ****: η **Α11** του ADR-841
      // δημοσιεύει **ΚΑΙ** `as-built` **ΚΑΙ** `proposal` *(«με έτοιμη μελέτη» — το ADR το
      // λέει **«αξία, όχι κίνδυνος»**)*, και οι παραγωγοί είναι **δύο ανεξάρτητοι**
      // (εργολάβος Φ4.2 · αρχιτέκτονας ιδιώτη Φ4β). Το «ένα μοντέλο = **ένα αρχείο**»
      // (αληθές — τα LOD ζουν μέσα στο GLB) απαντά **άλλη ερώτηση**, και η φράση
      // «το δημοσιευμένο GLB, στον ενικό» που κυκλοφόρησε ως επιχείρημα **δεν υπάρχει**
      // στο ADR-841 (μετρημένο σε 7.120 γρ.).
      //
      // ⚠️ **Σήμερα είναι ΠΑΝΤΑ κενό**: ο ψήστης γεννιέται στη Φ4.2. Το κουτί μπαίνει
      // μόνο του, ώστε αν σπάσει κάτι να φαίνεται **ποιο** από τα τρία έσπασε.
      'models',
      'netAreaSqm',
      'offerKinds', 'orientations', 'place', 'position',
      // 🔴 **ΚΟΚΚΙΝΙΣΕ ΞΑΝΑ (ADR-777 §8.69, 2026-09-15)** — και πάλι έκανε τη δουλειά της.
      // Το `priceReduction` πέρασε από **γραμμένη** απόφαση (Giorgio): σήμανση «↓%» στην
      // κάρτα, με αναφορά τη **χαμηλότερη τιμή των 30 ημερών**.
      //
      // ⛔ **ΚΑΙ ΤΟ ΠΙΟ ΣΗΜΑΝΤΙΚΟ ΕΙΝΑΙ ΤΙ ΔΕΝ ΤΑΞΙΔΕΥΕΙ**: το **ιστορικό** τιμών μένει στο
      // έγγραφο πηγής. Φεύγει **ένα** συμπέρασμα (`from` · `to` · `since`), ποτέ η διαδρομή.
      'priceReduction',
      'projectedAt',
