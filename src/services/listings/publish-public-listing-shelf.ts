 ...
 * `reconcileShelfSafely(listingId, [])`, καλεσμένη από **δύο** σημεία — και **άδειαζε μόνο το
 ...
 *
 * ⚠️ **ΕΞΑΓΕΤΑΙ ΕΠΙΤΗΔΕΣ, ΚΑΙ Ο ΛΟΓΟΣ ΕΙΝΑΙ ΕΙΛΙΚΡΙΝΕΙΑ ΓΙΑ ΤΟ ΤΙ ΜΠΟΡΕΙ ΝΑ ΔΟΚΙΜΑΣΤΕΙ** —
 * ίδιο ιδίωμα με το γραμμένο σκεπτικό του `ceilingOrThrow` στον ψήστη. Η **κρίση** *(«σε ποιο
 * ράφι;»)* ελέγχεται **χωρίς κάδο, χωρίς Firestore, χωρίς WASM**· η **καλωδίωσή** της
 * *(«έφτασε όντως εκεί;»)* ελέγχεται χωριστά, μέσω του {@link writeWithShelf} με μοκαρισμένα
 * κεφάλια. Δύο ερωτήσεις, δύο άγκυρες — αντί για μία που θα ήταν αργή και θολή.
 */
