...
export interface ProjectableProperty {
  ...
  /**
   * **Το κοινό της καταχώρησης** (ADR-864 §5.1) — ωμό, όπως κάθεται στο έγγραφο.
   *
   * ⚠️ **`unknown` και όχι `MarketingAudience`, επίτηδες**: το `properties` φτάνει εδώ
   * ως απλωμένο έγγραφο χωρίς σύνορο, άρα ο τύπος θα υποσχόταν κάτι που η βάση **δεν
   * χρωστά**. Η **μία** ερμηνεία (απουσία ⇒ `public`, Α3) ζει στο `marketingAudienceOf`,
   * που το ρωτά η πύλη — ποτέ ο καλών.
   */
  readonly marketingAudience?: unknown;
  ...
  /**
   * **Το ιστορικό τιμής του ακινήτου** (ADR-777 §8.69) — ό,τι επέστρεψε η σφραγίδα
   * (`resolvePriceHistory`), ή το ωμό πεδίο του εγγράφου.
   *
   * ⚠️ **`unknown`, επίτηδες** — ίδιο σκεπτικό με τα χαρακτηριστικά της Φ3 πιο πάνω: η
   * πηγή είναι ωμό έγγραφο Firestore, και η **μόνη** ανάγνωση είναι το
   * `readPriceHistory`. `null` = η σφραγίδα απέτυχε ⇒ **καμία** μείωση στην αγγελία.
   */
