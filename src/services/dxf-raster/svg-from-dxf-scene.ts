 ...
 * `functions/src/shared/svg-from-dxf-scene.ts` so the Cloud Function bundle
 * stays self-contained (Firebase deploys `functions/lib/**`, not the Next.js
 * tree). Change one → change both → commit together.
 *
 * @module services/dxf-raster/svg-from-dxf-scene
