 ...
 *
 * ⛔ **ΜΗΝ ξαναφτιάξεις `'use server'` ανάγνωση με ταυτότητα από τον client**: το
 * παλιό `getStorageUnitById` (`services/storage.service.ts`, **διαγράφηκε**
 * 2026-09-19, ADR-777 §8.60.20) ήταν server action — **καλέσιμη από κάθε client
 * με κάθε id** — με Admin SDK και σκέτο `doc(id).get()`, **κανέναν** έλεγχο
 * `companyId` ⇒ διαρροή μεταξύ εταιρειών (CHECK 3.35). Η σελίδα λεπτομέρειας
 * αποθήκης διαβάζει πλέον **από εδώ**. Φρουρός: άγκυρα `Α7`.
 *
