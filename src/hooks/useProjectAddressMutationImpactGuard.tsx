...
interface UseProjectAddressMutationImpactGuardReturn {
  checking: boolean;
  previewBeforeMutate: (req: ProjectAddressMutationRequest, action: () => Promise<void>) => Promise<boolean>;
  reset: () => void;
  ImpactDialog: ReactNode;
}

interface UseProjectAddressMutationImpactGuardOptions {
  onBlockDismiss?: () => void;
}
...
export function useProjectAddressMutationImpactGuard(
  projectId: string,
  options: UseProjectAddressMutationImpactGuardOptions = {},
): UseProjectAddressMutationImpactGuardReturn {
    ...
    async (req: ProjectAddressMutationRequest, action: () => Promise<void>) => {
      ...
      try {
        const impactPreview = await apiClient.post<ProjectMutationImpactPreview>(
          API_ROUTES.PROJECTS.ADDRESS_IMPACT_PREVIEW(projectId),
          req,
        );
        ...
        if (ApiClientError.isApiClientError(error)) {
          console.error(`[useProjectAddressMutationImpactGuard] Preview failed (${error.statusCode}):`, error.message);
        } else {
          console.error('[useProjectAddressMutationImpactGuard] Preview failed:', error);
        }
        deferredActionRef.current = null;
