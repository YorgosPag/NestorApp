...
export function useBuildingStats(buildings: Building[]) {
  const stats = useMemo(() => {
    return {
      ...
      totalArea: buildings.reduce((sum, b) => sum + (b.totalArea ?? 0), 0),
      averageProgress: buildings.length > 0 ? Math.round(buildings.reduce((sum, b) => sum + (b.progress ?? 0), 0) / buildings.length) : 0,
