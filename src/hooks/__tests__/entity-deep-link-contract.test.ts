...
describe('ADR-777 §8.31 — το κοινό εξάρτημα ρωτά, και κανείς δεν το παρακάμπτει', () => {
  ...
  // ==========================================================================
  // Α12 — 🔴 ΤΟ ΠΑΝΟ ΔΕΝ ΚΑΘΕΤΑΙ ΠΑΝΩ ΑΠΟ ΤΗΝ ΠΡΟΗΓΟΥΜΕΝΗ ΕΓΓΡΑΦΗ
  // ==========================================================================

  it('Α12 🔴 το κοινό εξάρτημα σβήνει την παλιά επιλογή όταν αλλάζει η ταυτότητα', () => {
    /*
     * 🔴 Βρέθηκε **ζωντανά** (στιγμιότυπο 2026-08-20) με **24 άγκυρες και 13
     * πύλες πράσινες**: το πανό «δεν οδηγεί σε εγγραφή» εμφανιζόταν σωστά, και
     * από κάτω έμενε ορατό το **προηγούμενο** κτίριο. Αλλαγή μόνο του ερωτήματος
     * δεν ξαναστήνει το component ⇒ η παλιά επιλογή επιβιώνει.
     */
    const source = read(GENERIC_HOOK);
    expect(source).toContain('shouldClearStaleSelection(selection, selectedItem?.id)');
    expect(source).toMatch(/shouldClearStaleSelection[\s\S]{0,120}setSelectedItemRaw\(null\)/);
  });

  it('Α13 🔴 ένα «δεν υπάρχει» δεν καταγράφεται ως ΣΦΑΛΜΑ', () => {
    /*
     * Η πρώτη γραφή άφηνε το 404 να ανέβει ⇒ `logger.error` ⇒ **κόκκινο overlay
     * του Next.js στην οθόνη** για προβλεπόμενη έκβαση (μετρημένο ζωντανά).
     * ⚠️ Το 403 μπαίνει στην ίδια κατηγορία: ο φύλακας εταιρείας απαντά ταυτόσημα
     * για «δεν υπάρχει» και «δεν είναι δικό σου».
     */
    const sources = read('src/hooks/entity-deep-link-sources.ts');
    expect(sources).toContain('ApiClientError');
    expect(sources).toMatch(/ANSWERED_NO\s*=\s*new Set\(\[403, 404\]\)/);
    // Ό,τι ΔΕΝ είναι αυτά ανεβαίνει — «δεν βρέθηκε» για σφάλμα δικτύου θα ήταν ψέμα.
    expect(sources).toMatch(/throw error;/);
  });
