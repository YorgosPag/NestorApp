...
describe('🔴 Η δεύτερη λίστα παρατηρήσεων — «ΕΜΒΑΔΟΝ ΓΕΩΤΕΜΑΧΙΟΥ» (ADR-759 §4.11β)', () => {
  const areaNote = (): DocumentBodyReading => {
    const found = g753DocumentReadings().find((r) => r.kind === 'parcel-area-note');
    if (!found) throw new Error('δεν βρέθηκε το «ΕΜΒΑΔΟΝ ΓΕΩΤΕΜΑΧΙΟΥ»');
    return found;
  };

  const notes = (): readonly DocumentBodyListRow[] =>
    areaNote().rows.filter((row) => row.list === 'remarks');

  it('τέσσερις παρατηρήσεις, όσες γράφει το σχέδιο', () => {
    expect(notes()).toHaveLength(4);
  });

  it('🔴 η ΑΡΙΘΜΗΣΗ ΤΟΥ ΕΝΤΥΠΟΥ δεν γίνεται κείμενο του μηχανικού', () => {
    // Η παγίδα του `takeRest`: το `LABEL_VALUE_SEPARATOR` περιέχει την τελεία, αλλά η γραμμή
    // αρχίζει με **ψηφίο** (`1.⇥…`), οπότε τίποτα δεν προσπερνιέται. Με `whole`+`rest` η τιμή
    // θα ήταν «1.⇥Το διάγραμμα…» — και η ίδια πρόταση, πληκτρολογημένη από άνθρωπο, θα
    // εμφανιζόταν ως **δεύτερη** παρατήρηση (η ταυτοδυναμία της Φ4γ συγκρίνει κείμενο).
    for (const row of notes()) {
      const text = row.parts.find((p) => p.part === 'remark')?.rawValue;
  ...
  it('και οι τέσσερις προτάσεις, αυτούσιες — διπλά κενά και ορθογραφικά του σχεδίου', () => {
    // ⚠️ Τα διπλά κενά είναι **όρια format runs** του MTEXT, και μένουν: ίδιο δόγμα με το
    // «οικόπεδο  δεν» της ενότητας Η. Το ίδιο και το `oρίζεται` με **λατινικό o** στην
    // τέταρτη — λάθος του σχεδίου, όχι του αναγνώστη.
    expect(notes().map((row) => row.parts.find((p) => p.part === 'remark')?.rawValue)).toEqual([
      "Το διάγραμμα είναι ενταγμένο στο κρατικό σύστημα συντεταγμένων  ΕΓΣΑ'87",
      'Οι διαστάσεις και τα εμβαδά υπολογίστηκαν αναλυτικά από τις  συν/νες των κορυφών',
      "Η εξάρτηση από το κρατικό δίκτυο συντεταγμένων ΕΓΣΑ '87 έγινε με GPS δύο συχνοτήτων  (ακρίβειας εκατοστού) με τη χρήση του Ελληνικού Συστήματος Εντοπισμού - Hepos",
      'Υπάρχει ρυμοτομική γραμμή και η οικοδομική γραμμή oρίζεται στα 5μ. από αυτή.',
    ]);
  });

  it('🔴 ο ΤΡΙΤΟΣ ΜΑΡΤΥΡΑΣ του εμβαδού επέζησε των νέων ενοτήτων', () => {
    // Η αφελής κίνηση — μόνο ενότητα «ΠΑΡΑΤΗΡΗΣΕΙΣ» — θα έσβηνε αυτό το πεδίο σιωπηλά: το
    // `splitIntoSections` πετά ό,τι προηγείται της πρώτης επικεφαλίδας, και το εμβαδόν είναι
    // στην **πρώτη γραμμή**. Χωρίς αυτόν τον έλεγχο η απώλεια θα φαινόταν μόνο ως
    // `corroboration` 3 → 2, δηλαδή ως αριθμός σε άλλο αρχείο.
    expect(areaNote().fields.map((f) => f.key)).toContain('plotArea');
    expect(areaNote().fields.find((f) => f.key === 'plotArea')?.rawValue).toBe('1.364,05');
  });

  it('η επικεφαλίδα ΠΑΡΑΤΗΡΗΣΕΙΣ δεν γίνεται η ίδια παρατήρηση', () => {
    for (const row of notes()) expect(row.rawText).not.toContain('ΠΑΡΑΤΗΡΗΣΕΙΣ');
  });
});
...
describe('🔴 κάθε δηλωμένο λεξιλόγιο γραμμών ΠΑΡΑΓΕΤΑΙ από το πραγματικό σχέδιο', () => {
  ...
  it('ΔΥΟ έγγραφα φέρουν λίστες — τα υπόλοιπα δεν επινοούν καμία', () => {
    // ⚠️ Ο έλεγχος έλεγε «**τα υπόλοιπα** έγγραφα δεν φέρουν λίστες» και ήταν αληθής όσο
    // μόνο η φόρμα Α–Ι είχε `listRules`. Το ADR-759 §4.11 μέτρησε **δεύτερο** μπλοκ
    // «ΠΑΡΑΤΗΡΗΣΕΙΣ» μέσα στο «ΕΜΒΑΔΟΝ ΓΕΩΤΕΜΑΧΙΟΥ» — τέσσερις αριθμημένες δηλώσεις που
    // προσγειώνονται στην ίδια λίστα της καρτέλας. Η άγκυρα μένει, με τον **αριθμό**
    // ονομαστικό: ένα τρίτο έγγραφο που αρχίζει ξαφνικά να παράγει γραμμές είναι ακριβώς
    // αυτό που πρέπει να σταματήσει εδώ.
    const withRows = g753DocumentReadings().filter((r) => r.rows.length > 0);

    expect(withRows.map((r) => r.kind).sort()).toEqual(['parcel-area-note', 'private-engineer-survey']);
    expect(withRows.every((r) => r.rows.every((row) => row.rawText.length > 0))).toBe(true);
  });
