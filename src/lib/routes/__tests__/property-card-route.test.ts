...
describe('ADR-777 §8.30 — η καρτέλα ακινήτου έχει διεύθυνση που οδηγεί κάπου', () => {
  ...
  it('Α9 🔴 το πρώτο καρέ δεν καλεί `t()` — ωμά κλειδιά σε δυναμική διαδρομή', () => {
    /*
     * 🔴 **ΜΕΤΡΗΜΕΝΟ ΣΤΟ ΣΕΡΒΙΡΙΣΜΕΝΟ HTML.** Το `/properties/[id]` έχει δυναμικό
     * τμήμα ⇒ **δεν προαποδίδεται** ⇒ το όριο αναστολής δεν πέφτει στο `fallback`
     * και ο διακομιστής αποδίδει το περιεχόμενο. Τα `properties` /
     * `properties-detail` όμως ζουν **κομμένα** στο i18n shell slice (το δεύτερο
     * με **μηδέν** κλειδιά), άρα κάθε `t()` εκείνου του καρέ φεύγει **ωμό**.
     *
     * 🔑 Η αδελφή δυναμική διαδρομή `/listing/[id]` είναι καθαρή επειδή το
     * `search-results` είναι **ΟΛΟΚΛΗΡΟ** στο slice. Η ίδια θεραπεία εδώ
     * κοστίζει **+75.482 bytes / +33%** στο slice **κάθε** διαδρομής ⇒
     * απορρίφθηκε με μέτρηση.
     *
     * Άρα: όσο η κατάσταση είναι `loading`, η σελίδα δείχνει το **ίδιο**
     * `StaticPageLoading` του ορίου αναστολής — μηδέν `t()`.
     */
    const content = read('src/components/properties/detail/PropertyDetailPageContent.tsx');

    expect(content).toContain('StaticPageLoading');
    expect(content).toMatch(/state\.kind === 'loading' && <StaticPageLoading/);

    // Το `PageLoadingState` παίρνει **υποχρεωτικό** μεταφρασμένο `message` — δηλαδή
    // είναι δομικά αδύνατο να το αποδώσεις εδώ χωρίς να καλέσεις `t()`.
    expect(content).not.toContain('PageLoadingState');

    // Ο σύνδεσμος επιστροφής ζει **εκτός** του καρέ άγνοιας.
