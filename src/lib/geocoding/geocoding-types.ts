...
export interface ResolvedAddressFields {
  ...
  country?: string;
  /**
   * **Η διοικητική ιεραρχία που ΑΠΟΔΕΙΧΘΗΚΕ** — ADR-332 D27 Φάση Β′.
   *
   * 🔑 **Γιατί εδώ**: αυτός ο τύπος είναι το κανάλι του **κοινού** `AddressEditor`, και τον
   * διασχίζουν **δύο** από τις τέσσερις διαδρομές συρσίματος *(έδρα επαφής · κτίρια)*. Χωρίς
   * αυτό το πεδίο, ό,τι απέδειξε ο διακομιστής **σταματά** στο σύνορο του editor και οι δύο
   * διαδρομές μηδενίζουν την ιεραρχία όπως πριν — δηλαδή η διόρθωση θα έπιανε **τις μισές**.
   *
   * ⚠️ **ΔΕΝ είναι πεδίο σύγκρισης.** Το `COMPARABLE_FIELDS` του `diffAddressFields` είναι
   * **ρητή λίστα** και δεν το περιλαμβάνει — η «Σύνοψη Αλλαγών» μένει ακριβώς όπως ήταν.
   * *(Το αν ο άνθρωπος πρέπει να **βλέπει** τις ταυτότητες είναι χωριστό βήμα, απόφαση
   * Giorgio 2026-09-12.)*
   * ⚠️ **Απών ≠ κενός** *(N.12)*: απών ⇒ «δεν ρωτήθηκε» *(π.χ. εμπρός γεωκωδικοποίηση, που
   * δεν παράγει αλυσίδα)* ⇒ ο γραφέας **δεν αγγίζει** ταυτότητες.
   */
...
export interface ReverseGeocodingResult {
   ...
   *
   * Ο διακομιστής κρατά την **ωμή** αλυσίδα του Nominatim *(«Δήμος Αθηναίων», «Περιφέρεια
   * Αττικής»)*, όπου το πρόθεμα **δηλώνει τη βαθμίδα** — ο πελάτης δεν τη βλέπει ποτέ, γιατί
   * το `cleanPlaceName` τη σβήνει. Άρα η ταυτοποίηση γίνεται **εκεί**, μία φορά, και ο
   * πελάτης δεν κατεβάζει **0,75 MB** μητρώου για να ξαναρωτήσει το ήδη γνωστό.
   *
   * ⚠️ **ΤΡΕΙΣ ΚΑΤΑΣΤΑΣΕΙΣ, ΟΧΙ ΔΥΟ** *(N.12)*:
   * - **απών** ⇒ «δεν ρωτήθηκε» *(η ιεραρχία δεν διαβάστηκε)* — ο γραφέας **ΔΕΝ αγγίζει**
   *   τα πεδία ταυτότητας, αλλιώς βλάβη δική μας σβήνει επιλογή ανθρώπου.
   * - **κενός πίνακας** ⇒ «ρωτήθηκε, τίποτα δεν αποδείχθηκε» — ο γραφέας **καθαρίζει**.
   * - **με στοιχεία** ⇒ γράφονται **αυτά** και καθαρίζονται τα υπόλοιπα.
   */
  admin?: readonly ProvedAdminLevel[];
}

/**
 * Μία **αποδεδειγμένη** διοικητική βαθμίδα: αριθμός βαθμίδας, ταυτότητα ΕΛΣΤΑΤ, όνομα μητρώου.
 *
 * 🔑 Ταξιδεύει ως **αριθμός βαθμίδας** επίτηδες: ο γραφέας το μεταφράζει σε πεδία μέσω του
 * **υπάρχοντος** `PATH_TO_VALUE`, που δηλώνει ήδη «βαθμίδα → πεδίο id + πεδίο ονόματος».
 * Ένα δικό του λεξιλόγιο θα ήταν ο πέμπτος χειρόγραφος μετατροπέας του ADR-772.
 */
export interface ProvedAdminLevel {
