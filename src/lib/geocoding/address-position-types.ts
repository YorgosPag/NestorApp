...
export interface AddressGeocodingMetadata {
  ...
  /**
   * **ΤΟ ΚΕΙΜΕΝΟ ΓΙΑ ΤΟ ΟΠΟΙΟ ΛΥΘΗΚΕ ΑΥΤΗ Η ΘΕΣΗ** — ADR-332 D27 Ζ6.
   *
   * 🔴 **Χωρίς αυτό, ο ισχυρισμός ακρίβειας ήταν ΜΗ ΕΛΕΓΞΙΜΟΣ.** Μετρημένο ζωντανά
   * 2026-09-12 στην έδρα της ALFA: το έγγραφο έγραφε «Εγνατία **102**» με
   * `accuracy: 'exact'`, ενώ η **αντίστροφη** γεωκωδικοποίηση του αποθηκευμένου σημείου
   * απαντά «Εγνατία **100**» — **22 μέτρα** αλλού. Το `exact` ήταν αληθές για την 100 και
   * ψευδές για ό,τι έβλεπε ο άνθρωπος, και **κανένας αναγνώστης δεν μπορούσε να ρωτήσει**.
   *
   * 🔑 **Είναι το ερώτημα που τέθηκε, αυτούσιο** (`toQuery`) — όχι νέα δομή, όχι σύνοψη,
   * όχι κατακερματισμός. Άρα η ερώτηση *«ισχύει για το τρέχον κείμενο;»* απαντιέται από τον
   * **ίδιο** κριτή που ήδη κρίνει τις αλλαγές ({@link addressIdentityChanged}) — **ποτέ**
   * δεύτερος κριτής (το μάθημα του Β10, όπου ο ανιχνευτής «Παλιές συντεταγμένες» έκρινε
   * ξανά στον πελάτη ό,τι ο διακομιστής είχε ήδη κρίνει).
   *
   * ⚠️ **Απουσία = άγνοια, ΟΧΙ «άλλαξε».** Κάθε εγγραφή πριν από το Ζ6 δεν το έχει· η
   * ετυμηγορία εκεί είναι `unverifiable` και **καμία** ειδοποίηση δεν εκδίδεται. Ίδιο
   * ιδίωμα με το `unknown` του `ProvedAdminLevel` (Φάση Β′).
   *
   * 🔴 **ΔΕΝ ΞΑΝΑΓΡΑΦΕΤΑΙ ΑΠΟ ΤΟ `keepStored`.** Όταν η μηχανή δεν απαντά, η θέση κρατιέται
   * **μαζί με την παλιά της απόδειξη** — αλλιώς η ψευδής πρόταση θα γινόταν αυτο-επικυρωμένη.
   */
  readonly resolvedFor?: GeocoderQuery;
  /**
   * **Ο ΠΑΡΟΧΟΣ ΔΗΛΩΣΕ ΟΤΙ ΔΕΝ ΤΑΙΡΙΑΞΑΝ ΟΛΑ** — ADR-332 D27 Ζ6 (σκέλος Β).
   *
   * 🔴 **Δεν είναι το ίδιο με το `accuracy`, και γι' αυτό χανόταν.** Η ακρίβεια περιγράφει
   * **το αποτέλεσμα** (πόσο καλά εντοπίστηκε αυτό που βρέθηκε)· αυτό περιγράφει **πόσο από
   * ό,τι ζητήθηκε ταίριαξε**. Μετρημένο: «Εγνατία 102» με Τ.Κ. 54002 επιστρέφει
   * `accuracy: 'exact'` **και** `partialMatch: true` (`fieldMatches.postalCode: 'mismatch'`).
   * Ο πάροχος το έλεγε ήδη — το `GeocodeHit` δεν το κουβαλούσε, άρα **χανόταν στη μεταφορά**:
   * κατά λέξη το σχήμα του ελαττώματος που γέννησε αυτό το module.
   *
   * ⚠️ **Γράφεται μόνο όταν είναι `true`.** Η απουσία σημαίνει «καμία επιφύλαξη» — έτσι
   * κανένα παλιό έγγραφο δεν αποκτά αναδρομικά ισχυρισμό που κανείς δεν μέτρησε.
   */
  readonly partialMatch?: boolean;
}

/**
 * **ΙΣΧΥΕΙ Η ΑΠΟΘΗΚΕΥΜΕΝΗ ΘΕΣΗ ΓΙΑ ΤΟ ΚΕΙΜΕΝΟ ΠΟΥ ΒΛΕΠΕΙ Ο ΑΝΘΡΩΠΟΣ;** — ADR-332 D27 Ζ6.
 *
 * Τέσσερις καταστάσεις, και οι δύο τελευταίες **δεν** είναι η ίδια σιωπή:
 *
 * - `matches` — η θέση λύθηκε **γι' αυτό ακριβώς** το κείμενο.
 * - `differs` — λύθηκε για **άλλο**. Ο ισχυρισμός ακρίβειας δεν ισχύει εδώ.
 * - `unverifiable` — υπάρχει ισχυρισμός αλλά **καμία απόδειξη** (εγγραφή πριν το Ζ6).
 *   **Άγνοια, όχι κατηγορία**: τίποτα δεν λέγεται στον άνθρωπο.
 * - `not-geocoded` — δεν υπάρχει ισχυρισμός καθόλου (πινέζα ανθρώπου ή καμία θέση). Η
 *   ερώτηση **δεν έχει νόημα**, και μια απάντηση «εντάξει» εδώ θα ήταν θόρυβος.
 */
export type PositionTextVerdict = 'matches' | 'differs' | 'unverifiable' | 'not-geocoded';
...
export interface GeocodeHit {
  ...
  /**
   * **Ο πάροχος δήλωσε ότι δεν ταίριαξαν όλα τα πεδία που ζητήθηκαν** (ADR-332 D27 Ζ6, σκέλος Β).
   *
   * 🔴 Το `GeocodingApiResponse.partialMatch` **υπήρχε ήδη** και το καταναλώνει ο editor
   * ζωντανά (`computeSuggestionTriggers`, `transitions`) — αλλά **σταματούσε εδώ**, στη
   * μετατροπή προς τον γραφέα, οπότε δεν έφτανε ποτέ στο έγγραφο. Δες
   * {@link AddressGeocodingMetadata.partialMatch} για το γιατί δεν ισοδυναμεί με `accuracy`.
   */
  readonly partialMatch?: boolean;
}
