...
function isContentEditableEl(el: Element): boolean {
  ...
  //
  // 🔴 ADR-753 §28 — **και το `plaintext-only`**, που είναι εξίσου επεξεργάσιμο και το
  // χρησιμοποιεί ο επεξεργαστής κελιού πίνακα. Χωρίς αυτή τη γραμμή ο φύλακας απαντούσε
  // `false` για ένα πεδίο που **δέχεται κάθε χαρακτήρα**, δηλαδή οι 43 window listeners του
  // viewer θα ξυπνούσαν πάνω στο κελί και το πρώτο γράμμα θα άνοιγε τη **γραμμή εντολών**.
  //
  // ⚠️ Σε πραγματικό browser το `isContentEditable` παραπάνω το πιάνει ήδη· σε **jsdom όχι**
  // (δες το σχόλιό του), οπότε χωρίς αυτό κάθε jest test θα έτρεχε τον λάθος κλάδο — και η
  // απόδειξη ιδιοκτησίας πλήκτρων θα ήταν πράσινη για λάθος λόγο. Baseline σε όλες τις
  // μηχανές από 03/2025.
  const attr = el.getAttribute('contenteditable');
  if (attr === 'true' || attr === '' || attr === 'plaintext-only') return true;
  // 🔴 ADR-753 §28 — **και η IDL ιδιότητα**, ως τρίτη διαδρομή.
  //
  // Κατά προδιαγραφή το `contentEditable` αντανακλά το γνώρισμα, οπότε σε πραγματικό browser
  // αυτή η γραμμή δεν προσθέτει τίποτα. Το **jsdom δεν το αντανακλά**: ένα
  // `el.contentEditable = 'true'` αφήνει το γνώρισμα `null` **και** το `isContentEditable`
  // `false`, δηλαδή και οι δύο προηγούμενες διαδρομές αστοχούν.
  //
  // ⚠️ Δεν είναι ευκολία για τα tests. Όταν οι έξι ad-hoc φύλακες του §28.7 ενοποιήθηκαν εδώ,
  // εκείνοι ρωτούσαν **ακριβώς** αυτή την ιδιότητα — χωρίς αυτή τη γραμμή η ενοποίηση θα ήταν
  // σιωπηλή **αποδυνάμωση**: ο κοινός φύλακας οφείλει να είναι γνήσιο **υπερσύνολο** καθενός
  // που αντικαθιστά, αλλιώς η «κεντρικοποίηση» αφαιρεί κάλυψη που κανείς δεν μέτρησε ότι
  // έφυγε. Το έπιασε άγκυρα (`table-clipboard-key-ownership`), όχι η ανάγνωση.
