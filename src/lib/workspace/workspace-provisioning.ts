...
export interface ProvisioningInput {
  ...
  readonly uid: string;
  /**
   * Ο χώρος που δηλώνει **σήμερα** το token του — `null` αν δεν ανήκει πουθενά.
   *
   * ⚠️ Είναι ο **φρουρός**, όχι πληροφορία: όποιος έχει ήδη χώρο δεν περνά.
   *
   * 🔑 **`null`, ΟΧΙ ΚΕΝΗ ΣΥΜΒΟΛΟΣΕΙΡΑ** (ADR-817 · διορθώθηκε 2026-08-27). Ο τύπος
   * είναι η **έξοδος** του `actorWorkspace()`, που είναι η **μία** νόμιμη μετάφραση
   * της απουσίας οργανισμού. Με `string` ο καλών αναγκαζόταν σε `?? ''` — και κενό
   * `companyId` δεν είναι «κανένας», είναι «εταιρεία με κενό όνομα»: δεν ταιριάζει
   * ούτε με τον εαυτό της (`hasTenant`) και είναι ακριβώς αυτό που κυνηγά η
   * **CHECK 3.35**. Ο κύριος πληθυσμός αυτής της συνάρτησης **είναι** το `null`.
   */
...
export async function provisionWorkspace(
  ...
  //
  // ⚠️ Ο έλεγχος είναι **παρουσίας**, όχι μήκους: `null` (πολίτης) και `''`
  //    (κάποιος που πέρασε κενή τιμή) οφείλουν να απαντηθούν **το ίδιο** —
  //    «δεν έχει χώρο, προχώρα».
