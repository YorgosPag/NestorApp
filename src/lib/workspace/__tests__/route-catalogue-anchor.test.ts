...
describe('Φρουρός #4 — ο κατάλογος διαδρομών και ο κριτής χώρου', () => {
   ...
   *
   * 🔑 **ΤΟ `third-party` ΕΙΝΑΙ Η ΙΔΙΑ ΚΑΤΗΓΟΡΙΑ, ΟΧΙ ΕΞΑΙΡΕΣΗ** (ADR-863 Φ3). Η σελίδα
   * `/open-source` συνδέει με τα **παραγόμενα αρχεία** `public/third-party/` — το πλήρες
   * κείμενο των αδειών και το SBOM. Είναι **αρχεία**, όπως τα `fonts/` και τα `images/`:
   * δεν έχουν `page.tsx`, δεν παίρνουν ποτέ πρόθεμα χώρου, και τα σερβίρει ο ίδιος
   * μηχανισμός (`Dockerfile`: `COPY public ./public`· ο matcher του middleware εξαιρεί
   * ρητά `.txt`/`.json`). Μια δήλωση στο {@link DECLARED_UNRESOLVED} θα ήταν **λάθος
   * φάρμακο**: εκείνο λέει *«σελίδα που ΔΕΝ υπάρχει»*, ενώ αυτά **υπάρχουν** — απλώς δεν
   * είναι σελίδες.
   */
  const NOT_A_PAGE = /^\/(api|images|fonts|static|third-party|_next|\.well-known)\//;
