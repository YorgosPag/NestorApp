...
export const CRITERION_PARAM: Record<CriterionKey, string> = {
  // ── αριθμητικοί ───────────────────────────────────────────────────────────
  /**
   * 🔴 **ΤΡΙΑ ΟΝΟΜΑΤΑ, ΕΝΑ ΑΝΑ ΜΟΝΑΔΑ** (§8.60.14 Φάση 2) — και η μονάδα ζει **μέσα
   * στο όνομα της παραμέτρου**, ώστε ένας κοινοποιημένος σύνδεσμος να μην μπορεί να
   * είναι διφορούμενος.
   *
   * ⚠️ **Το παλιό `p` ΔΕΝ ανακυκλώνεται σε κανέναν από τους τρεις, και είναι απόφαση.**
   * Σήμαινε *«ποσό, όποιου ρόλου»* — ερώτηση που πλέον ξέρουμε ότι **δεν απαντιέται**.
   * Ανακύκλωσή του σε έναν ρόλο θα ήταν **εικασία με στολή συμβατότητας**: ο άνθρωπος
   * που έγραψε «έως 1.000» μπορεί να εννοούσε ενοίκιο. Δες {@link RETIRED_RANGE_PARAMS}.
   */
  priceSale: 'psale',
  priceRent: 'prent',
  priceNightly: 'pnight',
...
// ADR-885: η σχεδιασμένη περιοχή (`draw`) — ίδιο ιδίωμα.
export const RESERVED_SEARCH_PARAMS = [
  'lat', 'lng', 'r', 'box', SEARCH_REGION_PARAM, SEARCH_DRAWN_PARAM,
...
/**
 * 🔴 **ΠΑΡΑΜΕΤΡΟΙ ΠΟΥ ΑΠΟΣΥΡΘΗΚΑΝ ΕΠΕΙΔΗ Η ΕΡΩΤΗΣΗ ΤΟΥΣ ΔΕΝ ΑΠΑΝΤΙΕΤΑΙ** — ποτέ
 * ψευδώνυμα, γιατί **δεν υπάρχει** κανονικό όνομα να δείξουν.
 *
 * Το `pmin`/`pmax` ζητούσε *«τιμή»* χωρίς μονάδα. Από το §8.60.14 αυτή η ερώτηση είναι
 * αναγνωρισμένα **διφορούμενη**: «έως 1.000 €» δέχεται ενοίκιο 900 €/μήνα **και**
 * διανυκτέρευση 50 €/νύχτα. Δεν μπορούμε ούτε να τη μεταφράσουμε *(θα ήταν εικασία)*
 * ούτε να τη σβήσουμε *(σιωπηλά χαμένο φίλτρο)*.
 *
 * 🏆 **Η τρίτη επιλογή, και είναι πάνω από τον κλάδο**: ο παλιός σύνδεσμος
 * **αναγνωρίζεται** και η οθόνη **ρωτά σε ποια μονάδα** — με τους αριθμούς του ήδη
 * γραμμένους. Είναι το αναγνωρισμένο ελάττωμα *«the URL implies a filter that is
 * silently ignored»*, λυμένο αντί για αγνοημένο. Τα portal απλώς το πετούν.
 *
 * ⚠️ **Ο κατάλογος διαβάζεται από τη διεύθυνση, ΔΕΝ γράφεται ποτέ σε αυτήν.**
 */
export const RETIRED_RANGE_PARAMS = { price: { min: 'pmin', max: 'pmax' } } as const;

/** Οι αριθμοί ενός αποσυρμένου εύρους, ή `null` όταν ο σύνδεσμος δεν το ζητά. */
export function readRetiredPriceRange(params: URLSearchParams): CriterionRange | null {
  const min = readFiniteNumber(params, RETIRED_RANGE_PARAMS.price.min);
  const max = readFiniteNumber(params, RETIRED_RANGE_PARAMS.price.max);
