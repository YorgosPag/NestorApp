...
export function showcaseFixture(overrides: Partial<PublicShowcase> = {}): PublicShowcase {
  return {
    ...
    position: null,
    coverage: null,
    // ⚠️ **Και η προεπιλογή της απόδειξης είναι ΚΕΝΗ, γιατί είναι η ΑΛΗΘΕΙΑ σήμερα**
    //    (ADR-846 Φ5δ): μετρημένο **6 ακίνητα · 0 στον χάρτη** (§8.8.5 α). Ένα γεμάτο
    //    `presence` εδώ θα έκανε **κάθε** δοκιμή να μετρά τη σπάνια περίπτωση, και θα
    //    έκρυβε ότι ο κατάλογος οφείλει να είναι σωστός **πρώτα** με κενή απόδειξη.
    presence: [],
    // ⚠️ **ΚΑΙ ΤΟ ΔΙΟΙΚΗΤΙΚΟ ΚΑΤΟΠΤΡΟ ΞΕΚΙΝΑ ΚΕΝΟ, ΓΙΑ ΤΟΝ ΙΔΙΟ ΛΟΓΟ** *(§9 #13)*:
    //    είναι η κατάσταση **κάθε** εγγράφου που γράφτηκε πριν το πεδίο υπάρξει, άρα η
    //    προεπιλογή μετρά ότι ο κατάλογος είναι σωστός **χωρίς** αυτό — δηλαδή ότι η
    //    ένωση **συμπληρώνει** και δεν αντικαθιστά.
    presenceAdminIds: [],
    ...
    mark: null,
    // ⚠️ **Κενή κάρτα ως προεπιλογή** (ADR-841 §7 Α21.16): είναι η κατάσταση κάθε βιτρίνας
    //    που γράφτηκε πριν την κάρτα — η σελίδα οφείλει να είναι σωστή **πρώτα** χωρίς αυτήν.
    locations: [],
