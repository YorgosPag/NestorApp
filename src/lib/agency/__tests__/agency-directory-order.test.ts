...
describe('Α. Antitrust — καμία εμπορική είσοδος (ADR-827 §9.9 α, NAR $418M)', () => {
  it('Α1 🔑 — ο συγκριτής της ισοπαλίας διαβάζει ΜΟΝΟ `companyId`', () => {
    ...
    //
    // 🔴 **ΤΟ `coverage` ΜΠΗΚΕ ΜΕ ΤΟ ADR-846, ΚΑΙ ΕΙΝΑΙ ΤΟ ΠΙΟ ΕΠΙΚΙΝΔΥΝΟ ΤΗΣ ΛΙΣΤΑΣ.**
    //    Είναι **ανεξέλεγκτη δήλωση του ίδιου**, και ο πειρασμός να ταξινομήσει είναι
    //    διατυπωμένος σε **εύλογα** ελληνικά: *«όποιος καλύπτει ΟΛΟΚΛΗΡΗ την περιοχή
    //    σου, πριν από όποιον καλύπτει μέρος της»*. Αν περάσει, η ίδια η δικαιολόγηση
    //    που επέτρεψε στο πεδίο να υπάρξει *(«φιλτράρει, ΔΕΝ κατατάσσει»)* παύει να
    //    ισχύει **αναδρομικά** — και ο κατάλογος αποκτά κατάταξη που ο επαγγελματίας
    //    ρυθμίζει μόνος του, δηλαδή ακριβώς το NAR σχήμα με άλλο όνομα.
    //
    // 🔴🔴 **ΚΑΙ ΤΟ `presence` ΜΠΗΚΕ ΜΕ ΤΗ Φ5δ — ΕΙΝΑΙ ΤΟ ΠΙΟ ΕΠΙΚΙΝΔΥΝΟ ΟΛΩΝ.**
    //    Το `coverage` είναι **ανεξέλεγκτη δήλωση**, άρα ο πειρασμός να ταξινομήσει
    //    ακούγεται τουλάχιστον **αμφισβητήσιμος**. Το `presence` είναι **αποδεδειγμένο
    //    γεγονός**, οπότε ο ίδιος πειρασμός ακούγεται **αξιοκρατικός**: *«όποιος έχει
    //    αποδεδειγμένα ακίνητα εκεί, πριν από όποιον απλώς το δηλώνει»*.
    //
    //    Αυτό είναι **κατάταξη με βάση τον όγκο αποθέματος** — δηλαδή το NAR σχήμα
    //    *($418M, 17/08/2024)* **με το πιο πειστικό δυνατό πρόσωπο**, και ακριβώς αυτό
    //    που κάνει σήμερα το Zillow Agent Finder *(«most active agents within that
    //    geographical search area»)*.
    //
    // 🔑 **Ο δεύτερος φρουρός είναι ΔΟΜΙΚΟΣ, και ζει στο σχήμα**: οι κύκλοι του
    //    `presence` είναι **απορροφημένοι**, άρα το πεδίο λέει **πού** και **δεν μπορεί**
    //    να πει **πόσα**. Ακόμη κι αν αυτή η άγκυρα παρακαμφθεί, δεν υπάρχει αριθμός
    //    αποθέματος να ταξινομηθεί. Δες `lib/agency/showcase-presence.ts`.
    const forbidden: (keyof PublicShowcase)[] = [
      ...
      'position',
      'presence',
      // 🔴 **ΚΑΙ ΤΟ ΔΙΟΙΚΗΤΙΚΟ ΤΟΥ ΚΑΤΟΠΤΡΟ** *(ADR-846 §9 #13)* — ο πειρασμός είναι ο
      //    **ίδιος**, και εδώ πιο εύκολος: ένας πίνακας ταυτοτήτων μοιάζει με «εμβέλεια
      //    δραστηριότητας» και ταξινομείται με ένα `.length`. Ο δομικός φρουρός μένει ο
      //    ίδιος — οι ταυτότητες είναι **απορροφημένες**, δύο αγγελίες στον ίδιο δήμο
      //    δίνουν **μία** — αλλά ο φρουρός της πρόθεσης γράφεται **εδώ**.
      'presenceAdminIds',
      'publishedAt',
    ];
