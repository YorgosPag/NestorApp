...
describe('🔴 Ζ — ΟΙ ΤΡΕΙΣ ΖΩΝΕΣ: κάθε μία λέει ΚΑΤΙ ΑΛΛΟ, και όλες λένε ΑΛΗΘΕΙΑ', () => {
  ...
  it('🔴 128-255 ΚΑΛΥΠΤΕΙ ΚΑΙ ΤΙΣ ΔΥΟ επιφάνειες — «θολό στη σελίδα» εκεί είναι ΨΕΜΑ', () => {
    for (const edge of [128, 180, 200, 255]) {
      expect(zoneOf(edge)).toBe('card/page');
    }
  });

  it('🔑 88-127 καλύπτει ΜΟΝΟ την κάρτα — εδώ ΙΣΧΥΕΙ το «θολό στη σελίδα»', () => {
    for (const edge of [88, 100, 127]) {
  ...
  it('🔴 ΟΙ ΤΡΕΙΣ ΖΩΝΕΣ ΕΙΝΑΙ ΔΙΑΚΡΙΤΕΣ — αλλιώς ένα μήνυμα θα ήταν περιττό ή ψεύτικο', () => {
    // ⚠️ Αν δύο ζώνες έδιναν το ίδιο ζεύγος, η οθόνη **δεν θα μπορούσε** να τις
    //    ξεχωρίσει — και το τρίτο μήνυμα θα ήταν ή αδύνατο να εμφανιστεί ή λάθος.
    const zones = new Set([zoneOf(70), zoneOf(100), zoneOf(180)]);
