...
describe('Λ2 — καθαρότητα του γραφήματος εισαγωγών', () => {
  ...
  it('🔴 η ΠΡΟΣΥΜΠΛΗΡΩΣΗ μένει ΕΞΩ από τον Λ2 (ADR-759 Φ1)', () => {
    // Το `contact-prefill.ts` κάθεται στον **ίδιο φάκελο** και απαντά σε άλλη ερώτηση: όχι
    // «ποια επαφή είναι αυτός;» αλλά «τι θα έγραφε ο άνθρωπος στη φόρμα;». Γι' αυτό εισάγει
    // `EnterpriseContactSaver` — καθαρό σήμερα, αλλά ζει στον τομέα των επαφών και θα μεγαλώσει
    // εκεί. Την ημέρα που κάποιος «απλοποιήσει» καλώντας το από τον resolver, ο Λ2 κληρονομεί
    // ολόκληρο εκείνο το γράφημα και η άγκυρα καθαρότητας από πάνω παύει να λέει την αλήθεια —
    // **σιωπηλά**, γιατί τίποτα σήμερα δεν είναι απαγορευμένο. Το σύνορο δηλώνεται εδώ.
    const prefill = [...graph.files].filter((f) => f.includes('contact-prefill'));
