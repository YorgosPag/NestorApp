...
/**
 * **Το ανώτατο πλήθος ατόμων** σε ερώτηση, αίτημα ή κράτηση διαμονής.
 *
 * ⚠️ Ζει **εδώ**, δίπλα στο {@link STAY_PETS_CEILING}, και όχι στον αναλυτή εντολών του ημερολογίου,
 * επειδή τον ρωτούν και η διεύθυνση της αναζήτησης (`listing-stay-url`) και η δημόσια ερώτηση
 * διαθεσιμότητας (`stay-public-request`). Ένας αναγνώστης διεύθυνσης δεν πρέπει να εισάγει αναλυτή
 * εντολών για να μάθει έναν αριθμό.
 */
export const STAY_BOOKING_MAX_GUESTS = 50;

/**
 * **Ο ΕΝΑΣ κριτής πλήθους ατόμων**: ακέραιος στο `[1, STAY_BOOKING_MAX_GUESTS]` (ADR-777 §8.60.21.7).
 *
 * 🔴 **Γιατί υπάρχει** (μετρημένο 2026-09-22): ο ίδιος έλεγχος ήταν γραμμένος inline **έξι** φορές, με
 * **διαφορετικά** όρια. Ο αναγνώστης της διεύθυνσης δεν είχε ταβάνι, ενώ η δημόσια ερώτηση απέρριπτε το
 * `> 50` ⇒ με `?guests=51` η αναζήτηση έστελνε ερώτηση που ο διακομιστής απαντούσε `400`, και **κάθε**
 * κάρτα έδειχνε «αποτυχία». Όποιος διαβάζει ή γράφει πλήθος ατόμων ρωτά **αυτόν**.
 */
export function isWholeGuestCount(value: unknown): value is number {
  return typeof value === 'number' && isWholeStayCount(value) && value <= STAY_BOOKING_MAX_GUESTS;
}
