 ...
 *
 * ⚠️ **Το `percentage` της αντιπαροχής ΔΕΝ χαρτογραφείται ΕΔΩ** — τρίτη μορφή του
 * {@link KINDS_WITHOUT_LEGACY_PROJECTION}: το παλιό σχήμα έχει **πεδία ευρώ** και **κανένα
 * ποσοστού**. Ένα ποσοστό γραμμένο σε `askingPrice` θα εμφανιζόταν ως **«50 €»** στην κάρτα —
 * ψέμα με νούμερο. ✅ **Δεν είναι πια απώλεια** (ADR-777 §8.60.17): φτάνει στη δημόσια αγγελία
 * από **δικό του** κουτί, `deriveExchangeTerms` (`derive-exchange-terms.ts`) → `PublicListing.exchange`
 * — το πρότυπο του `stay`.
 *
