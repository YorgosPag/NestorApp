/**
 * 🛡️ UNIT FIELD LOCKING — Shared Utility (ADR-249 P0-2)
 *
 * Validates that locked fields on sold/rented/reserved units cannot be modified.
 ...
 *
 * @module lib/firestore/unit-field-locking
...
/**
 * Fields locked when a unit is sold or rented.
...
/**
 * Fields locked when a unit is reserved.
...
/**
 * Validates that no locked fields are being modified on a unit with a given commercialStatus.
 *
 * @param commercialStatus - Current commercialStatus of the unit document
 ...
 */
export function validateUnitFieldLocking(
  ...
  if (commercialStatus === 'sold' || commercialStatus === 'rented') {
    ...
    if (attempted.length > 0) {
      throw new ApiError(
        403,
        `Cannot modify locked fields on a ${commercialStatus} unit: ${attempted.join(', ')}`
      );
    }
    ...
    if (attempted.length > 0) {
      throw new ApiError(
        403,
        `Cannot modify locked fields on a reserved unit: ${attempted.join(', ')}`
      );
    }
  }
}
