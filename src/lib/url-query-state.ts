 ...
 *
 * ## Γιατί ζει εδώ και όχι στον καταναλωτή
 * Είναι η **ίδια** ερώτηση που απαντούν οι δύο από πάνω — *«ποιος γράφει στο native
 * History API, και τι διατηρεί όταν γράφει;»* — απλώς σε άλλον άξονα: εκεί διατηρείται
 * το **query string**, εδώ η **διεύθυνση ολόκληρη**. Και στις δύο περιπτώσεις το
 * `history.state` περνά αυτούσιο, γιατί ο App Router κρατά **εκεί** τη δική του
 * κατάσταση δρομολόγησης και ένα σκέτο `pushState({δικό μου: true})` θα την έσβηνε
 * σιωπηλά — με τη βλάβη να εμφανίζεται στην **επόμενη** πλοήγηση, μακριά από την αιτία.
 * Ένα δεύτερο σημείο που καλεί `pushState` θα ήταν δεύτερος ορισμός του «τι διατηρώ»,
 * έτοιμος να αποκλίνει σε ακριβώς αυτή τη λεπτομέρεια.
 *
 * ## Πότε είναι το σωστό εργαλείο
 * Όταν η κατάσταση είναι **στιγμή περιήγησης** και όχι ερώτημα: μια επιφάνεια που ο
 * άνθρωπος διαβάζει ως σελίδα, άρα το πίσω κουμπί οφείλει να την κλείνει, αλλά που
 * **δεν** έχει νόημα να κοινοποιηθεί ή να επιβιώσει reload. Αν η κατάσταση αξίζει να
 * μοιραστεί, ανήκει στο query string — δηλαδή στο {@link replaceUrlSearchParams}.
 *
 * @param marker - Τα κλειδιά του καταναλωτή. Απλώνονται πάνω στο ζωντανό `history.state`.
 */
export function pushHistoryMarker(marker: Readonly<Record<string, true>>): void {
  if (typeof window === 'undefined') return;
