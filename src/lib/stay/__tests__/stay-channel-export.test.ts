...
describe('stayExportEvents — η γραμμή: φυσική κατάληψη ΝΑΙ, πολιτική κράτησης ΟΧΙ', () => {
  it('κράτηση + block ιδιοκτήτη + block άλλου καναλιού, με τα ημι-ανοιχτά διαστήματα αυτούσια', () => {
    ...
    expect(stayExportEvents(entries, rules(), { kind: 'all' })).toEqual([
  ...
  it('🔴 οι νύχτες προετοιμασίας ΕΞΑΓΟΝΤΑΙ (όπως η Airbnb) — δύο γεγονότα γύρω από την κράτηση', () => {
    const events = stayExportEvents([bookingEntry('stay_1', '2026-10-10', '2026-10-14')], rules(1), { kind: 'all' });
  ...
  it('η προετοιμασία ΔΕΝ μπαίνει γύρω από block του ιδιοκτήτη (δεν είναι επισκέπτης)', () => {
    const events = stayExportEvents([blockEntry('sblk_own', '2026-11-01', '2026-11-03')], rules(2), { kind: 'all' });
  ...
  it('🏆 ο σύνδεσμος ΕΝΟΣ καναλιού δεν περιέχει ό,τι ήρθε από ΕΚΕΙΝΟ (ούτε την προετοιμασία του)', () => {
    ...
    const uids = stayExportEvents(entries, rules(1), { kind: 'feed', feedId: FEED }).map((e) => e.uid);
  ...
  it('🔑 εξάγεται ΑΚΡΙΒΩΣ ό,τι καταλαμβάνει — η απάντηση είναι του STAY_LIFECYCLE_OCCUPIES', () => {
    for (const lifecycle of STAY_BOOKING_LIFECYCLES) {
      const entry = withLifecycle(bookingEntry('stay_1', '2026-10-10', '2026-10-14'), lifecycle);
      const events = stayExportEvents([entry], rules(), { kind: 'all' });
      // Όταν το Στάδιο Δ κάνει το `requested` να καταλαμβάνει όσο ζει η προθεσμία, τα holds
      // εξάγονται ΜΟΝΑ ΤΟΥΣ — αυτή η άγκυρα γίνεται η απόδειξη, χωρίς αλλαγή στον εξαγωγέα.
      expect(events).toHaveLength(STAY_LIFECYCLE_OCCUPIES[lifecycle] ? 1 : 0);
    }
  });
  ...
  it('🔴 ΚΑΝΕΝΑ ΠΡΟΣΩΠΟ: ούτε όνομα, ούτε σημείωση, ούτε άτομα σε κανένα byte', () => {
    ...
    const noted: StayCalendarEntry = block.kind === 'block'
      ...
      : block;
    const output = stayExportCalendar([named, noted], rules(1), { kind: 'all' }, 'Κατάλυμα');
  ...
  it('η πολιτική κράτησης (ειδοποίηση · παράθυρο · CTA/CTD · ελάχ. νύχτες) ΔΕΝ ταξιδεύει', () => {
    ...
    const events = stayExportEvents([bookingEntry('stay_1', '2026-10-10', '2026-10-12')], strict, { kind: 'all' });
    expect(events).toHaveLength(1);
  });
});
