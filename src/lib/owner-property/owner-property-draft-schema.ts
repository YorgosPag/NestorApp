...
const media = z.object({
  ...
  uploadedAt: z.string(),
  /**
   * 🔴 **Η ΕΠΙΛΟΓΗ ΔΗΜΟΣΙΕΥΣΗΣ (ADR-841 §7 Α2.7) — ΚΑΙ ΤΟ ΚΕΝΟ ΤΗΣ ΗΤΑΝ ΑΟΡΑΤΟ.**
   *
   * Το `zod` **κόβει σιωπηλά** ό,τι δεν δηλώνεται. Χωρίς αυτή τη γραμμή, ο άνθρωπος
   * τσέκαρε «Δημοσίευση», η οθόνη έγραφε «Δημοσιεύονται 1 από 24», και το πεδίο
   * **δεν έφτανε ποτέ στη Firestore** — η αγγελία έμενε χωρίς εικόνες και **τίποτα
   * δεν το έλεγε**. Μετρημένο περπατώντας τη Φ3 (2026-09-01).
   *
   * ⚠️ **Και ο μεταγλωττιστής ΔΕΝ μπορούσε να το πιάσει**, παρότι αυτό το αρχείο
   * δηλώνει ρητό τύπο επιστροφής ακριβώς γι' αυτό: το πεδίο είναι **προαιρετικό**
   * στην οντότητα, οπότε ένα αντικείμενο **χωρίς** αυτό ικανοποιεί τον τύπο. Ο
   * φρουρός της «απόκλισης σχήματος ⇄ οντότητας» πιάνει τα **υποχρεωτικά**· τα
   * προαιρετικά τα φυλάει **μόνο** άγκυρα που περνά τιμή από το σύνορο.
   */
...
export const ownerPropertyDraftSchema = z.object({
   ...
   *
   * ✅ **ΤΟ ΔΕΥΤΕΡΟ `as` ΕΦΥΓΕ 2026-09-06** (ADR-842 §7.6.12 / §8 #11): η προβολή
   * **κανονικοποιεί**, και η πύλη δημοσίευσης αρνείται να δημοσιεύσει ακίνητο χωρίς
   * λυμένο είδος — ποτέ ξανά «διαμέρισμα» για ό,τι δεν αναγνωρίστηκε. Αυτό εδώ το
   * σύνορο **μένει ακέραιο**: είναι η **πύλη γραφής**, και η γραφή οφείλει να είναι
   * αυστηρότερη από την ανάγνωση.
   *
   * 🔑 **Το κλειστό σύνολο είναι το ΚΑΝΟΝΙΚΟ** — και από τις 2026-09-06 αυτό είναι
