 ...
 * | `consentNonce` · `decidedAt` · `notifiedAt` · `notifyOutcome` | Είναι **γεγονότα του συστήματος**, όχι δηλώσεις. Ένα `decidedAt` από το δίκτυο θα έλεγε «μίλησε στις…» για συνομιλία που δεν έγινε· ένα `notifyOutcome: 'sent'` θα έλεγε «του το είπαμε» **χωρίς να έχει φύγει μήνυμα** (ADR-834 §6.5.δ) |
