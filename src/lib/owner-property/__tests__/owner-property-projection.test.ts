...
describe('🔴 Ι — το ιδιωτικό ΔΕΝ ταξιδεύει στη δημόσια αγγελία', () => {
  ...
  it('🔑 Ι4 — με ρητή επιλογή, ΚΑΙ ΤΟΤΕ ΜΟΝΟ, το μονοπάτι φτάνει στο ράφι', () => {
    ...
    expect(projectableFromOwnerProperty(chosen, AT).publishedMedia).toEqual([
      { privateStoragePath: property.media[0].storagePath, material: { kind: 'photo' }, focalPoint: null },
    ]);
  });
   ...
   * διαδρομή που έφτανε εδώ ξεκινούσε από `snap.data() as OwnerProperty` (μετρημένο
   * 2026-09-02: **~20** ωμά cast στο `src/`, κανένα δεν επικύρωνε· 2026-09-06:
   * **19 σε 14 αρχεία**, και **όλα** πέρασαν από το σύνορο — ADR-842 §7.6.12 / §8 #11).
   *
   * 🔑 **Η ΑΓΚΥΡΑ ΜΕΝΕΙ ΚΑΙ ΓΙΝΕΤΑΙ ΑΥΣΤΗΡΟΤΕΡΗ**: αυτή η συνάρτηση δέχεται **δομικό**
   * τύπο, άρα την καλούν και διαδρομές που **δεν** περνούν από το σύνορο (φόρμα,
   * δοκιμές, μελλοντικοί καταναλωτές). Το ότι το σύνορο υπάρχει **δεν** την απαλλάσσει
   * — απλώς σημαίνει ότι η **παραγωγική** διαδρομή έχει πλέον δύο δίχτυα αντί για ένα.
   * Ο τύπος δηλώνει το
