...
describe('Ε2 — ο κρίκος 6 αλλάζει σχήμα χωρίς να εφευρίσκει γνώση', () => {
  ...
  it('η έκδοση ανέβηκε ΜΑΖΙ με κρίκο (ADR-839 §5)', () => {
    ...
    // 🔴 **ΔΙΟΡΘΩΘΗΚΕ 2026-09-03 (ADR-841 Α17): ΡΩΤΑ ΤΟΝ ΚΡΙΚΟ 6 ΟΝΟΜΑΣΤΙΚΑ, ΟΧΙ
    //    «ΤΟΝ ΤΕΛΕΥΤΑΙΟ».** Η προηγούμενη μορφή ήταν `LISTING_MIGRATIONS.at(-1)` — που
    //    λέει *«ο κρίκος 6 είναι ο τελευταίος»*, πρόταση **προσωρινή εξ ορισμού** σε μια
    //    αλυσίδα που μεγαλώνει. Έγινε ψευδής με τον **κρίκο 7** (Α15) και **η άγκυρα
    //    έμεινε κόκκινη**· ο κρίκος 8 απλώς τη βρήκε έτσι.
    //
    // 🔑 Το ερώτημα αυτής της ομάδας είναι *«ο **κρίκος 6** προσθέτει τα `levels`;»* —
    //    ιδιότητα **αμετάβλητη**, γιατί «οι κρίκοι είναι αμετάβλητοι». Η θέση του στη
    //    λίστα **δεν** είναι μέρος του ερωτήματος.
    const link6 = LISTING_MIGRATIONS.find((migration) => migration.to === 6);
    expect(link6).toBeDefined();
    expect(link6?.adds).toEqual(['levels']);
  });
