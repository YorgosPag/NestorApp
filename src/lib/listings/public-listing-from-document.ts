 ...
 *
 * ────────────────────────────────────────────────────────────────────────────
 * 🔴 ΔΥΟ ΔΟΥΛΕΙΕΣ, ΔΥΟ ΜΗΧΑΝΙΣΜΟΙ — ΚΑΙ ΤΟ ΛΕΞΙΛΟΓΙΟ **ΔΕΝ** ΕΙΝΑΙ ΚΡΙΚΟΣ
 * ────────────────────────────────────────────────────────────────────────────
 *
 * Η **αλυσίδα** ({@link upgradeListingDocument}) απαντά *«τι **λείπει** από την έκδοση
 * N ώστε να γίνει N+1;»* — ερώτημα **σχήματος**. Η κανονικοποίηση του `type` απαντά
 * *«η **τιμή** είναι στην κανονική μορφή;»* — ερώτημα **λεξιλογίου** (ADR-842 §7.6.12).
 *
 * ⛔ **Και γι' αυτό ΔΕΝ μπήκε ως κρίκος, παρότι θα ήταν η προφανής θέση.** Το
 * `upgradeListingDocument` επιστρέφει **αυτούσιο** κάθε έγγραφο που φέρει ήδη τη
 * σημερινή σφραγίδα· άρα ένα έγγραφο **τρέχουσας έκδοσης** με ωμή τιμή —ακριβώς αυτό
 * που παρήγαγε το παλιό `(property.type ?? 'apartment') as PropertyType` του γραφέα—
 * **δεν θα περνούσε ποτέ** από τον κρίκο. Ορθότητα δεμένη σε αριθμό έκδοσης δεν είναι
 * ορθότητα· η κανονικοποίηση γίνεται **άνευ όρων**, σε κάθε ανάγνωση.
 *
 * 🔑 **Ιδιοδύναμη, άρα δωρεάν για τα υγιή έγγραφα**: κάθε κανονική τιμή απεικονίζεται
 * στον εαυτό της (φυλάσσεται από την άγκυρα `property-type-classes.test.ts`).
 */
...
export interface StoredListingRead {
   ...
   *
   * 🔑 **Ίδιο δόγμα με το {@link storedVersion} από πάνω**: ο επισκέπτης βλέπει ήδη
   * σωστά (η μετάφραση έγινε στη μνήμη), οπότε **τίποτα δεν πονάει** και τίποτα δεν θα
   * το φανερώσει αλλιώς. Είναι ο **μετρητής** που κάνει την αφαίρεση του παλαιού
   * λεξιλογίου απόφαση **με αριθμό** και όχι τόλμη (ADR-842 §7.6.12).
   *
   * ⚠️ **Η απουσία δεν είναι ολίσθηση**, και το ζεύγος τα ξεχωρίζει χωρίς τρίτο πεδίο:
   * `type: null` **με** ολίσθηση ⇒ *ήρθε τιμή που δεν αναγνωρίζεται*· **χωρίς** ⇒ *δεν
   * ήρθε τιμή καθόλου*.
   */
...
export function readStoredListing(raw: unknown, id: string): StoredListingRead | null {
  ...
  // 🔴 Η δουλειά **λεξιλογίου**, άνευ όρων και μετά την αλυσίδα — δες την κεφαλίδα.
  //    Μετά την αλυσίδα επίτηδες: ένας μελλοντικός κρίκος μπορεί να **γεννήσει** ή να
  //    **μετακινήσει** το πεδίο, και η κανονικοποίηση οφείλει να κρίνει ό,τι θα δει ο
  //    καταναλωτής — όχι ό,τι υπήρχε πριν.
  const { type, drift } = readPropertyType(upgraded.type);
  ...
  return {
    ...
    listing: { ...upgraded, id, type } as PublicListing,
    storedVersion,
