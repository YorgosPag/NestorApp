 ...
 *
 * @returns `true` αν μοιράζονται **έστω μία στιγμή**· `null` αν κάποιο άκρο δεν
 *   διαβάζεται ως στιγμή, ή αν κάποιο διάστημα είναι **ανάποδο** (λήξη πριν την
 *   έναρξη) — δεδομένο που δεν περιγράφει διάστημα και δεν πρέπει να κριθεί σιωπηλά.
 */
export function intervalsOverlap(
  ...
  const aFrom = normalizeToMillisOrNull(aStart);
  const bFrom = normalizeToMillisOrNull(bStart);
  if (aFrom === null || bFrom === null) return null;

  // 🔑 `null`/`undefined` = «δεν λήγει» ⇒ `+Infinity`. Κάθε **άλλη** μη αναγνώσιμη
  //    τιμή είναι **βλάβη δεδομένου** και επιστρέφει «δεν ξέρω» — τα δύο δεν
  //    ισοπεδώνονται: «ανοιχτή σύμβαση» και «χαλασμένη ημερομηνία» έχουν εντελώς
  //    διαφορετική θεραπεία.
  const aTo = aEnd === null || aEnd === undefined ? Infinity : normalizeToMillisOrNull(aEnd);
  const bTo = bEnd === null || bEnd === undefined ? Infinity : normalizeToMillisOrNull(bEnd);
  if (aTo === null || bTo === null) return null;

  if (aTo < aFrom || bTo < bFrom) return null;
