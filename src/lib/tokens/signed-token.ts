...
/**
 * Σύγκριση **σταθερού χρόνου** δύο **δεκαεξαδικών** μυστικών — και οι τρεις
 * έλεγχοι μήκους είναι απαραίτητοι.
 *
 ...
 *
 * 🔑 **ΕΞΑΓΕΤΑΙ (ADR-844)**, γιατί η ερώτηση *«είναι αυτά τα δύο μυστικά ίδια,
 * χωρίς να προδώσω πόσο μοιάζουν;»* είναι **η ίδια** για την υπογραφή ενός
 * συνδέσμου και για το αποτύπωμα ενός εξαψήφιου κωδικού. Δεύτερη υλοποίηση θα
 * ήταν δεύτερη ευκαιρία να ξεχαστεί ένας από τους τρεις ελέγχους — και η
 * παράλειψη **δεν φαίνεται σε καμία δοκιμή**, μόνο σε επίθεση.
 *
 * ⚠️ **Δέχεται ΜΟΝΟ δεκαεξαδικό.** Δεν είναι γενικός συγκριτής συμβολοσειρών.
 */
export function equalsInConstantTime(actual: string, expected: string): boolean {
