/**
 * geometry-fingerprint — ADR-683 Φ2. **ΕΝΑ SSoT** για την ερώτηση «τι σχήμα έχει αυτή η γεωμετρία;»,
 * που τρέχει και στα **δύο άκρα** του round-trip: στο export γράφεται στο manifest (`.nestor.json`),
 * στο import ξαναϋπολογίζεται από το επιστρεφόμενο αρχείο. Χωρίς αυτό ο reconciler δεν ξεχωρίζει την
 * **κατάσταση A** (ADR-683 §5 — «άλλαξε μόνο χρώμα» → αυτόματη εφαρμογή) από την **κατάσταση C**
 * («άλλαξε το σχήμα» → ρώτα τον χρήστη).
 *
 * ⚠️ **ΜΕΤΑΦΕΡΘΗΚΕ ΑΥΤΟΥΣΙΟ** από `subapps/dxf-viewer/io/mesh3d-roundtrip/geometry-hash.ts`
 * *(ADR-845 Φ4.2)* — σκεπτικό στο `./triangle-soup`.
 *
 * 🏆 **ΚΑΙ ΑΠΕΚΤΗΣΕ ΔΕΥΤΕΡΟ ΚΑΤΑΝΑΛΩΤΗ ΤΗΝ ΙΔΙΑ ΜΕΡΑ**: η **Α-6** του ADR-845 §8 ρωτά *«διαφωνούν τα
 * μετρημένα μεγέθη με τη δήλωση του πελάτη;»* — που είναι **η ίδια ερώτηση** με το «άλλαξε το σχήμα
 * ανάμεσα στην εξαγωγή και την επιστροφή;», σε νέο πεδίο. ⇒ Η κλειστή λογιστική **δεν γεννά δικό της
 * λεξιλόγιο ανοχών**: καλεί το {@link compareGeometry}. Ένα δεύτερο κατώφλι εδώ θα ήταν δεύτερη
 * αλήθεια για το πότε δύο σχήματα είναι «το ίδιο».
 *
 ...
 *      συγκρίνεται **με ανοχή**. Τρέχει ΜΟΝΟ όταν το hash διαφέρει.
 *
 * ## Χώρος αναφοράς (τι ΠΙΑΝΕΙ και τι ΟΧΙ — 100% ειλικρίνεια)
 *
 * Οι κορυφές έρχονται σε **world space** και μετά **μεταφέρονται ώστε το bbox min να πέσει στο
 * origin**. Δηλαδή ο περιγραφέας είναι **ανεξάρτητος θέσης**:
 ...
 *
 * Μονάδα: **μέτρα** — ο three κόσμος είναι σε μέτρα (ADR-462) και το glTF είναι spec-locked σε μέτρα.
 *
 * @see ./triangle-soup — η κοινή ανάγνωση γεωμετρίας
 * @see docs/centralized-systems/reference/adrs/ADR-683-bim-collaboration-roundtrip.md §5
 */

import { forEachTriangle, triangleArea, type TriangleSoup } from './triangle-soup';
...
/** Συνολικό εμβαδόν + πλήθος τριγώνων. Η διέλευση ζει στο `./triangle-soup` (κοινή με το solid measure). */
function measureTriangles(
  soup: TriangleSoup,
  points: Float64Array,
): { areaM2: number; triangleCount: number } {
  let areaM2 = 0;
  const triangleCount = forEachTriangle(soup.index, points.length / 3, (ia, ib, ic) => {
...
/**
 * Το fingerprint μιας γεωμετρίας. `null` όταν δεν υπάρχουν αξιοποιήσιμες κορυφές (κενή γεωμετρία)
 * — ο caller το αντιμετωπίζει ως «άγνωστο σχήμα», ποτέ ως «ίδιο».
 *
 * 🔴 **ΔΕΝ ΜΕΤΑΛΛΑΣΣΕΙ ΤΗΝ ΕΙΣΟΔΟ, ΚΑΙ ΕΙΝΑΙ ΑΠΟΦΑΣΗ ΤΗΣ ΜΕΤΑΚΙΝΗΣΗΣ** *(ADR-845 Φ4.2)*. Ως τη Φ4.2
 * η `rebaseToBoundingBox` μετέφερε τις κορυφές **επί τόπου** — αβλαβές, γιατί κάθε καλών έπαιρνε
 * **φρέσκο** πίνακα από τη `readWorldPositions`. Από τη στιγμή που η είσοδος έγινε {@link TriangleSoup},
 * δηλαδή τιμή που **μοιράζεται** *(ο ίδιος πολτός ταΐζει fingerprint **και** όγκο)*, η επί τόπου
 * μετακίνηση θα ήταν σιωπηλή παρενέργεια σε δεδομένα του καλούντος. Το αντίγραφο κοστίζει έναν
 * `Float64Array` — αμελητέο δίπλα στην ταξινόμηση που ακολουθεί — και κάνει τη σειρά των κλήσεων
 * **να μη μετράει**. ⚠️ Το ίδιο το γεγονός ότι κανένα test δεν θα το έπιανε είναι ο λόγος που γράφεται.
 */
export function computeFingerprint(soup: TriangleSoup | null): GeometryFingerprint | null {
  if (soup === null || soup.positions.length === 0) return null;

  const points = Float64Array.from(soup.positions);
  const vertexCount = points.length / 3;
  const { sizeM, centroidM } = rebaseToBoundingBox(points);
  const { areaM2, triangleCount } = measureTriangles(soup, points);
