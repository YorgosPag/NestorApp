 ...
 *
 * ⚠️ **ΜΕΤΑΦΕΡΘΗΚΕ ΑΥΤΟΥΣΙΟ** από `subapps/dxf-viewer/io/mesh3d-roundtrip/` *(ADR-845 Φ4.2)* ώστε να
 * το βλέπει **και ο διακομιστής** — το σκεπτικό της μετακίνησης ζει στο `./triangle-soup`. **Καμία**
 * αλλαγή συμπεριφοράς: μόνο η είσοδος έπαψε να είναι `THREE.Mesh` και έγινε {@link TriangleSoup}.
 *
 * ## Γιατί υπάρχει
 *
 * Η προμέτρηση χρειάζεται ποσότητα ανά μονάδα. Μήκος και εμβαδόν βγαίνουν ήδη από το
 * `GeometrySignature` (`./geometry-fingerprint` — `sizeM`, `areaM2`). Ο **όγκος** όμως δεν βγαίνει από
 * το κουτί οριοθέτησης: ένα κάγκελο 10×1×0,05 m δίνει 0,5 m³ ως κουτί ενώ πραγματικά είναι ~0,02 m³
 ...
 *
 * @see ./triangle-soup — η κοινή ανάγνωση γεωμετρίας (SSoT, μοιράζεται με το fingerprint)
 * @see ./geometry-fingerprint — από εκεί έρχονται εμβαδόν & διαστάσεις (δεν ξαναϋπολογίζονται εδώ)
 * @see docs/centralized-systems/reference/adrs/ADR-683-bim-collaboration-roundtrip.md §10.2
 */

import { forEachSoupTriangle, soupVertexCount, type TriangleSoup } from './triangle-soup';
 ...
 */
function isOrientedManifold(soup: TriangleSoup, welded: Int32Array): boolean {
  ...
  forEachSoupTriangle(soup, (ia, ib, ic) => {
    const a = welded[ia];
    const b = welded[ib];
    const c = welded[ic];
 ...
 */
function signedVolume(soup: TriangleSoup): number {
  const points = soup.positions;
  const n = soupVertexCount(soup);
  const origin = [0, 0, 0];
...
/**
 * Ο όγκος μιας γεωμετρίας, ή `null` όταν δεν στηρίζει την ερώτηση.
 * Καλείται **μία φορά ανά κόμβο κατά την εισαγωγή** — τα τρίγωνα είναι ήδη στη μνήμη.
 */
export function measureSolid(soup: TriangleSoup | null): MeshSolidMeasure {
  if (soup === null || soup.positions.length === 0) return NOT_SOLID;

  if (!isOrientedManifold(soup, buildWeldMap(soup.positions))) return NOT_SOLID;

  const volumeM3 = Math.abs(signedVolume(soup));
