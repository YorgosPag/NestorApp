...
/**
 * **Πού τέμνονται γνησίως** τα ευθύγραμμα τμήματα `p1p2` και `p3p4` — ή `null`.
 *
 * 🔑 **Ένα κατηγόρημα, δύο ερωτήσεις** *(ADR-885)*: ο κριτής απλότητας
 * (`isSimpleGeoOutline`) ρωτά *«τέμνονται;»*, ο διασπαστής της ελεύθερης χειρονομίας
 * (`geo-freehand.ts`) ρωτά *«πού;»*. Αν ζούσαν χωριστά, μια μέρα ο διασπαστής θα
 * έβλεπε τομή που ο κριτής δεν βλέπει — και θα παρέδιδε σχήμα που ο κριτής απορρίπτει.
 * Ζούσε ιδιωτικό στο `geo-ring.ts`· μετακόμισε εδώ, δίπλα στον {@link LocalPoint}.
 */
export function localSegmentCrossing(
  p1: LocalPoint,
  p2: LocalPoint,
  p3: LocalPoint,
  p4: LocalPoint,
