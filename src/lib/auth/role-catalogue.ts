...
export const PREDEFINED_ROLES: Record<string, RoleDefinition> = {
  ...
  company_admin: {
    ...
    permissions: [
      ...
      // ═══════════════════════════════════════════════════════════════════════
      //
      // Το `admin_access` το έχει **ΑΚΡΙΒΩΣ ΕΝΑΣ** ρόλος — αυτός. Ο `AdminGuard`
      // του `/o/[workspace]/dxf/viewer` δεν δέχεται κανέναν άλλο. Και το
      // `dxf:layers:view`, που ζητά **κάθε** κλήση του προβολέα, το είχαν οι
      // `project_manager` · `architect` · `engineer` · `viewer` — **κανένας από
      // τους οποίους δεν μπορεί να ΜΠΕΙ**.
      //
      // 🔑 Δηλαδή: {ποιος ανοίγει τον προβολέα} ∩ {ποιος μπορεί να τον
      //    χρησιμοποιήσει} = **∅**. Καμία «σπάνια περίπτωση» — **δομικά
      //    αδύνατη επιφάνεια**, για κάθε άνθρωπο, πάντα.
      //
      // Ζωντανή μέτρηση (emulator, `admin.civil@alpha.local`, σκέτο φόρτωμα):
      //   GET /api/dxf-levels → 403 {"requiredPermission":"dxf:layers:view"}
      // Το ορατό σύμπτωμα ήταν **αλλού**: «Δεν έχει φορτωθεί σκηνή» + κενός
      // καμβάς μετά από φαινομενικά επιτυχή εισαγωγή DXF. Ο λόγος είναι ότι
      // `useDxfViewerCallbacks.handleFileImportWithEncoding` δεν βρίσκει όροφο,
      // ζητά `addLevel()` → **403** → `console.error` + `return`: η εισαγωγή
      // **αμβλώνεται σιωπηλά**, χωρίς μήνυμα στην οθόνη. «Δεν υπάρχουν επίπεδα»
      // ΔΕΝ ήταν κενή βάση — ήταν 403.
      //
      // ⚠️ ΔΕΝ είναι «προσθήκη για να περάσει». Η ΑΠΟΔΕΙΞΗ ότι είναι παράλειψη
      //    και όχι πολιτική είναι **δομική**, και υπάρχει σε δύο ανεξάρτητα
      //    αντίγραφα μέσα στον ίδιο αυτόν κατάλογο:
      //      • Είχε `dxf:layers:unlock` — «ξεκλείδωσε κλειδωμένο επίπεδο» —
      //        και είναι ο **ΜΟΝΟΣ** κάτοχός του. Δικαίωμα **αυστηρότερο** του
      //        `view`, δοσμένο **χωρίς** το `view`. Δεν ξεκλειδώνεις ό,τι δεν
      //        επιτρέπεσαι να δεις.
      //      • Είχε `projects:floors:delete` **χωρίς** `projects:floors:view` —
      //        «μπορείς να σβήσεις όροφο, αλλά όχι να τον δεις».
      //    Δύο φορές το ίδιο σχήμα ⇒ ο κατάλογος του `company_admin` γράφτηκε
      //    για **διοικητικά** ερωτήματα και ποτέ δεν ρωτήθηκε τι απαιτούν οι
      //    **λειτουργικές** οθόνες που η ίδια η εφαρμογή του ανοίγει.
      //
      // ⛔ ΜΗΝ «στρογγυλέψεις» εδώ. Κάθε γραμμή παρακάτω αντιστοιχεί σε
      //    διαδρομή που ο προβολέας **ΚΑΛΕΙ**, μετρημένη με grep στο
      //    `src/app/api/**/route.ts` — όχι σε κατηγορία «θα χρειαστεί».
      //
      // 🛡️ Το φυλάει πλέον άγκυρα: `__tests__/surface-authority-reachability.test.ts`
      //    ρωτά την **ΚΛΑΣΗ** («ανοίγει η εφαρμογή επιφάνεια που ο server
      //    αρνείται;»), όχι αυτό το δείγμα.
      //
      // @see ADR-829 · ADR-823 §13 (το ίδιο σχήμα, δύο μέρες πριν, σε `units`)
      // ═══════════════════════════════════════════════════════════════════════
      // /api/dxf-levels (GET·POST·PATCH) + /api/floorplan-overlays (×4)
      "dxf:layers:view",
      // /api/dxf-levels DELETE + /api/floorplan-overlays DELETE. Ήταν **ορφανό**:
      // απαιτούμενο από δύο διαδρομές, κατεχόμενο από **κανέναν** ρόλο ⇒ η
      // διαγραφή επιπέδου ήταν ανεκτέλεστη για κάθε άνθρωπο. Πηγαίνει εδώ και
      // πουθενά αλλού, γιατί ο κατάλογος **ήδη** δίνει σε αυτόν τον ρόλο τα
      // `delete` κάθε άλλου πόρου (floors · properties · units · contacts).
      "dxf:layers:manage",
      // /api/floors (GET·POST·PATCH) — το ταίρι του `projects:floors:delete` που
      // ήδη κατείχε δέκα γραμμές παρακάτω.
      "projects:floors:view",
      // /api/dxf/revisions GET (ADR-745 — ιστορικό αναθεωρήσεων σχεδίου).
      "dxf:files:view",
      // ADR-344 text engine
      "dxf:layers:unlock",
      "dxf:text:create",
