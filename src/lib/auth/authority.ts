 ...
 * ταυτότητα θα ήταν «ναι» που κανείς δεν μπορεί να εξηγήσει.
 *
 * 📏 **ΤΟ ΙΣΤΟΡΙΚΟ ΔΕΙΓΜΑ**: το `users/dev-admin` έφερε `globalRole: 'admin'` —
 * τιμή εκτός λεξιλογίου. ⚠️ **ΔΕΝ ΤΟ ΦΕΡΕΙ ΠΙΑ**: το commit `c8b374a0`
 * (2026-08-25) το ανέβασε σε `'super_admin'`, και δέκα ώρες αργότερα η αλλαγή
 * έφτασε στο **ζωντανό** έγγραφο (ADR-822 §2.5). Το `'admin'` παραμένει
 * **άγνωστος** ρόλος και η περίπτωση παραμένει έγκυρη — αλλά είναι πλέον
 * **ιστορικό**, όχι τρέχουσα μέτρηση.
 *
