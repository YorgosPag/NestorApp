...
/**
 * Κάθε ανάθεση σε μεταβλητή-κλειδί μνήμης, **ολόκληρη** (μπορεί να είναι
 * πολυγραμμική).
 *
 * 🔴 Το `\r?` **δεν είναι λεπτομέρεια**: η πρώτη γραφή τερμάτιζε σε `;\n` και
 * το repo είναι **CRLF** (`core.autocrlf=true` σε Windows) ⇒ ο ανιχνευτής
 * έβρισκε **μηδέν** εκφράσεις στα πραγματικά αρχεία και ο έλεγχος περνούσε
 * **κενός**. Το πιάσαμε μόνο επειδή η μετάλλαξη «κλειδί χωρίς μισθωτή»
 * **επέζησε** — πράσινο test που δεν πεθαίνει δεν φυλάει τίποτα. Ίδιο σχήμα με
 * το CHECK 3.33 του CLAUDE.md (ωμή σύγκριση bytes σε Windows).
 */
const CACHE_KEY_ASSIGNMENT = /const\s+\w*[cC]ache[kK]ey\w*\s*(?::\s*string\s*)?=\s*([\s\S]*?);\r?\n/g;
...
describe('⚓ ADR-742 §7decies.2 — καμία μνήμη API δεν μοιράζεται θέση μεταξύ εταιρειών', () => {
  ...
  /**
   * 🔴 **Η φρουρά που έλειπε.** «Κανένας παραβάτης» και «καμία μέτρηση» δίνουν
   * το **ίδιο** πράσινο. Ο ανιχνευτής όντως μετρούσε μηδέν εκφράσεις (CRLF) και
   * ο έλεγχος από κάτω περνούσε **κενός** — φάνηκε μόνο από επιζήσασα μετάλλαξη.
   * Ο αριθμός είναι **κάτω όριο**, όχι ισότητα: νέες μνήμες επιτρέπονται.
   */
  it('🔴 ο ανιχνευτής βρίσκει ΟΝΤΩΣ κλειδιά στο πραγματικό δέντρο', () => {
    const found = API_FILES.flatMap((f) => cacheKeyExpressions(f.source));
