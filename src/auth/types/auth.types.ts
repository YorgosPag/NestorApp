...
export interface UserProfileDocument extends DeclaredOccupation {
  ...
  authProvider: string;
  /**
   * **ΑΦΜ** — ελληνικός αριθμός φορολογικού μητρώου, **9 ψηφία**, επαληθευμένος
   * με τον mod-11 ελεγκτή του `lib/validation/vat-validation.ts` (ADR-827 §9.20).
   *
   * 🔑 **ΕΙΝΑΙ Η ΠΗΓΗ, ΟΧΙ ΤΟ ΑΠΟΤΥΠΩΜΑ.** Τη στιγμή που ένα γραφείο δέχεται
   * αίτημα ανάθεσης, ο **διακομιστής** αντιγράφει αυτή την τιμή στην επαφή που
   * γεννιέται (§8.3) — και το αντίγραφο **δεν αλλάζει** αν ο άνθρωπος διορθώσει
   * αργότερα το προφίλ του. Ίδιο δόγμα με τη διεύθυνση χρέωσης ενός τιμολογίου:
   * το παραστατικό κρατά τι ίσχυε **τότε**, ποτέ ξένο κλειδί.
   *
   * ⛔ **Ο πελάτης ΔΕΝ το γράφει απευθείας**: είναι `serverOwnedUserFields()` στο
   * `firestore.rules`, γιατί ο ελεγκτής ψηφίου δεν εκφράζεται σε κανόνα. Η μόνη
   * πόρτα είναι το `PATCH /api/account/vat-number`.
   *
   * `null` = δεν δηλώθηκε. Δεν είναι σφάλμα — είναι η κατάσταση κάθε ανθρώπου που
   * δεν έχει ακόμη αναθέσει εντολή (§9.20 β: just-in-time, ποτέ στην εγγραφή).
   */
