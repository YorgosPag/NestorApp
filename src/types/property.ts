...
export interface PropertyCommercialData {
  ...
  /**
   * Τιμή **ανά διανυκτέρευση** — βραχυχρόνια μίσθωση (ADR-835 §4.4).
   *
   * 🔴 **Χωρίς αυτό το πεδίο το μοντέλο θα είχε ΔΥΟ ΑΛΗΘΕΙΕΣ ανάλογα με το ποιος
   * καταχωρεί.** Η αγγελία **ιδιώτη** κουβαλά ήδη την τιμή βραδιάς (μέσω του
   * `deriveCommercialAmounts`, που προβάλλει τις διαθέσεις σε αυτό ακριβώς το σχήμα)·
   * αν το `PropertyCommercialData` δεν την είχε, μια μονάδα **επαγγελματία** θα ήταν
   * **δομικά ανίκανη** να την εκφράσει — δηλαδή η ίδια πράξη θα υπήρχε στη μία πλευρά
   * και θα ήταν αδύνατη στην άλλη.
   *
   * ⚠️ **Ξεχωριστό πεδίο, ΠΟΤΕ ξαναχρησιμοποίηση του `rentPrice`**: «800» ως μηνιαίο
   * και «65» ως ημερήσιο δεν ξεχωρίζουν από τον αριθμό — τα ξεχωρίζει **μόνο** το
   * πεδίο. Ο ρόλος τους ονομάζεται στον `price-resolver` (`PriceRole: 'nightly'`).
   */
...
export interface Property extends PropertySpecificationFields {
   ...
   *
   * ⚠️ **ΔΕΝ είναι νέο πεδίο: υπήρχε ΠΑΝΤΑ στα δεδομένα και έλειπε από τον τύπο.**
   * Το γράφει το `buildCommonFields` του `lib/firestore/entity-creation.service.ts`
   * σε **κάθε** οντότητα, και η διαδρομή `/api/properties/create` περνά από εκεί —
   * δηλαδή είναι συστηματικό, όχι τυχαίο. Επαληθεύτηκε στη ζωντανή βάση
   * (2026-08-19): **8 στα 8** ακίνητα το έχουν συμπληρωμένο.
   *
   * 🔴 **Η απουσία του από εδώ ήταν πραγματικό κόστος**: ρωτώντας τον *τύπο* αντί
   * για τα *δεδομένα*, το §8.23 συμπέρανε αρχικά ότι «τα ακίνητα δεν κρατούν ποιος
   * τα καταχώρησε» και παραλίγο να προτείνει μετανάστευση που **δεν χρειαζόταν**.
   * Ένα πεδίο που υπάρχει στον δίσκο αλλά όχι στον τύπο είναι αόρατο σε κάθε
   * μελλοντικό αναγνώστη.
   *
   * Είναι το `uid` του χρήστη — παραλήπτης ειδοποίησης, όχι όνομα προς εμφάνιση
   * (για εμφάνιση υπάρχει το `_lastModifiedByName`).
   *
   * Προαιρετικό στον τύπο επειδή έγγραφα γραμμένα από παλαιότερα μονοπάτια (ή από
   * seed scripts) μπορεί να μην το έχουν· ο καταναλωτής **οφείλει** να χειριστεί
   * την απουσία ρητά αντί να υποθέσει παραλήπτη.
   */
