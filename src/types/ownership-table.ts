...
/**
 * Στάδιο απόκτησης του μεριδίου ενός οικοπεδούχου (ADR-745 Φ3α).
 *
 * Ισχύει **και** στην αγορά **και** στην αντιπαροχή:
 * - Αγορά: η κυριότητα μεταβιβάζεται στο `secured`.
 * - Αντιπαροχή: η κυριότητα **δεν μεταβιβάζεται ποτέ** — ο οικοπεδούχος παραμένει
 *   ιδιοκτήτης ενώ εμείς ανεγείρουμε. Γι' αυτό το τελικό στάδιο λέγεται `secured`
 *   («κλείδωσε») και **όχι** `owned`: το «δικό μας» είναι διαδρομή που εκεί δεν υπάρχει.
 *
 * Χάρτης προς το σύστημα συμβολαίων ADR-230 (`legal-contracts.ts`), για όταν/αν
 * χρειαστεί γέφυρα — εκείνο μοντελοποιεί την **αντίθετη** κατεύθυνση (εμείς πουλάμε):
 * - `under_contract` ↔ `phase:'preliminary'` + `status:'signed'`
 * - `secured`        ↔ `phase:'final'`       + `status:'signed'`
 * - `withdrawn`      ↔ (δεν υπάρχει εκεί — το FSM συμβολαίων είναι forward-only)
 */
export type AcquisitionStatus =
  | 'prospective'     // Σε συζήτηση — τίποτα υπογεγραμμένο
  | 'under_contract'  // Προσύμφωνο ή εργολαβικό αντιπαροχής
  | 'secured'         // Οριστικό συμβόλαιο + μεταγραφή
  | 'withdrawn';      // Ναυάγησε — κρατάμε το ιστορικό

/**
 * Επιτρεπτές μεταβάσεις. Πρότυπο: `CONTRACT_STATUS_TRANSITIONS` (ADR-230).
 *
 * Σε αντίθεση με εκείνο, **δεν** είναι forward-only — και αυτό είναι σκόπιμο σε δύο σημεία:
 * - `withdrawn → prospective`: υπόθεση που ναυάγησε μπορεί να ξανανοίξει.
 * - `secured → withdrawn`: **καμία κατάσταση δεν είναι παγίδα**. Το ADR-230 έχει τερματικό
 *   `completed: []`, αλλά εκεί το λάθος διορθώνεται από άλλη διαδρομή· εδώ η μοναδική
 *   διαδρομή είναι αυτός ο επιλογέας, οπότε τερματικό `secured` θα σήμαινε ότι μια λάθος
 *   καταχώρηση **δεν παίρνεται ποτέ πίσω**. Είναι και νομικά αληθές: υπάρχει ακυρωτικό.
 */
export const ACQUISITION_STATUS_TRANSITIONS: Record<AcquisitionStatus, AcquisitionStatus[]> = {
  prospective: ['under_contract', 'withdrawn'],
  under_contract: ['secured', 'withdrawn'],
  secured: ['withdrawn'],
  withdrawn: ['prospective'],
};

/** Όλες οι τιμές, με τη σειρά που εμφανίζονται στο UI */
export const ACQUISITION_STATUSES: readonly AcquisitionStatus[] = [
  'prospective',
  'under_contract',
  'secured',
  'withdrawn',
...
/**
 * Οικοπεδούχος έργου (ADR-244).
 *
 * ⚠️ Ισχύει σε **δύο** σενάρια, όχι μόνο στην αντιπαροχή:
 * - **Αντιπαροχή**: ο οικοπεδούχος παραμένει ιδιοκτήτης, εμείς ανεγείρουμε.
 * - **Αγορά**: ο πωλητής είναι ιδιοκτήτης μέχρι το οριστικό συμβόλαιο.
 * (Το προηγούμενο docblock έλεγε μόνο «σενάριο αντιπαροχής» — ήταν ήδη ανακριβές:
 * το δείγμα Γ753 του ADR-745 είναι αγορά και χρησιμοποιεί την ίδια δομή.)
 */
export interface LandownerEntry {
  ...
  /**
   * Στάδιο απόκτησης (ADR-745 Φ3α) — **προαιρετικό επίτηδες**.
   *
   * Απουσία σημαίνει **«δεν δηλώθηκε»**, ΟΧΙ `prospective`: τα υπάρχοντα έργα
   * γράφτηκαν πριν υπάρξει το πεδίο, και σιωπηλό default θα δήλωνε «σε συζήτηση»
   * για οικόπεδο που ίσως χτίζεται ήδη. Πρβλ. ADR-287, όπου τιμή enum που «ποτέ
   * δεν ταίριαζε με πραγματικά δεδομένα» χρειάστηκε να αφαιρεθεί.
   */
