...
import type { AliasRejection } from './workspace-alias';
...
// ============================================================================
// ΔΗΜΙΟΥΡΓΙΑ ΧΩΡΟΥ — ΤΟ ΛΕΞΙΛΟΓΙΟ ΤΗΣ ΑΠΟΤΥΧΙΑΣ (ADR-787 Κ-1)
// ============================================================================

/**
 * Γιατί **δεν** φτιάχτηκε ο χώρος.
 *
 * 🔑 **Ζει εδώ, στο client-safe στρώμα, και είναι απόφαση.** Την ετυμηγορία τη
 * **γεννά** ο διακομιστής (`lib/workspace/workspace-provisioning.ts`, που είναι
 * `server-only`) και τη **διαβάζει** η οθόνη, για να διαλέξει διατύπωση από τα
 * locale αρχεία. Αν ο τύπος ζούσε στον διακομιστή, ο πελάτης θα τον εισήγαγε με
 * `import type` από `server-only` module — δουλεύει (τα type imports σβήνονται),
 * αλλά **σπάει σιωπηλά** την ημέρα που κάποιος αφαιρέσει τη λέξη `type`.
 *
 * ⚠️ Το `AliasRejection` **δεν αντιγράφεται** — συντίθεται. Οι λόγοι απόρριψης
 * ψευδωνύμου έχουν **έναν** ιδιοκτήτη (`types/workspace-alias.ts`), και μια
 * δεύτερη απαρίθμησή τους εδώ θα ήταν σύνολο που αποκλίνει σιωπηλά (ADR-749).
 */
