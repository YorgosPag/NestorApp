...
/**
 * 🔴 **ΤΑ ΟΝΟΜΑΤΑ ΤΩΝ ΟΡΩΝ, ΣΕ ΧΡΟΝΟ ΕΚΤΕΛΕΣΗΣ** — και γεννήθηκε από **ζωντανή βλάβη**.
 *
 * ────────────────────────────────────────────────────────────────────────────
 * ΤΟ ΠΕΡΙΣΤΑΤΙΚΟ, ΜΕΤΡΗΜΕΝΟ 2026-08-30
 * ────────────────────────────────────────────────────────────────────────────
 *
 * Το ADR-832 πρόσθεσε **δύο** πεδία σε αυτόν τον τύπο (`scope`, `startsAt`) και η
 * φόρμα άρχισε να τα στέλνει. Το **σχήμα zod της πόρτας** (`/api/mandate-requests`)
 * έμεινε στα **τρία** — και το zod **αφαιρεί σιωπηλά** ό,τι δεν δηλώνεται.
 *
 * ⇒ Από τότε, **κάθε** αίτημα ανάθεσης έφτανε στον γραφέα με `scope: undefined`:
 *
 * | Κατάσταση αγγελίας | Τι γινόταν |
 * |---|---|
 * | με υπάρχουσα δεσμευτική εντολή | 🔴 **HTTP 500** — `a.scope.filter` σε `undefined` |
 * | χωρίς εντολή | γραφόταν αίτημα **χωρίς πράξεις και χωρίς έναρξη**, σιωπηλά |
 *
 * 🔴 **ΚΑΙ ΚΑΝΕΝΑ TEST ΔΕΝ ΤΟ ΕΙΔΕ**: όλες οι σουίτες φτιάχνουν `ProposedMandateTerms`
 * σε **TypeScript**, δηλαδή **παρακάμπτουν το σύνορο** όπου ζει το ελάττωμα. Ο
 * μεταγλωττιστής δεν βλέπει ποτέ το zod, και το zod δεν βλέπει ποτέ τον τύπο.
 *
 * 🔑 **ΑΥΤΗ Η ΛΙΣΤΑ ΕΙΝΑΙ Η ΓΕΦΥΡΑ**: το `satisfies` αναγκάζει τον **μεταγλωττιστή**
 * να επιβεβαιώσει ότι κάθε όνομα υπάρχει στον τύπο, και η άγκυρα του συνόρου
 * διατρέχει **αυτήν** για να απαιτήσει ότι κάθε όνομα **επιβιώνει** του zod. Έκτο
 * πεδίο αύριο κληρονομεί τον έλεγχο χωρίς να το θυμηθεί κανείς.
 *
 * ⚠️ **ΔΕΝ αρκεί το `satisfies` μόνο του** — εγγυάται ότι τα ονόματα **υπάρχουν**,
 * ποτέ ότι είναι **όλα**. Την πληρότητα την κλειδώνει η άγκυρα, με βρόχο πάνω στα
 * κλειδιά ενός πλήρους δείγματος (ίδιο ιδίωμα με τους «παρονομαστές» της CHECK 3.54).
 */
export const PROPOSED_MANDATE_TERM_FIELDS = [
  'agreement',
  'compensation',
  'expiresAt',
  'scope',
  'startsAt',
] as const satisfies readonly (keyof ProposedMandateTerms)[];
