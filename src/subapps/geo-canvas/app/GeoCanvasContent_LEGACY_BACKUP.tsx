...
export function GeoCanvasContent(props: GeoCanvasAppProps) {
  ...
  // ============================================================================
  const geoCanvasState = useGeoCanvasState({
    ui: {
      sidebarWidth: 320,
  ...
  // ✅ ENTERPRISE: All state now managed by domain modules
  const { state, actions, selectors } = geoCanvasState;

  // ✅ ENTERPRISE: All map data managed by domain state
  // Search markers, boundaries, layers -> state.mapView
  // Administrative data -> geoCanvasState centralized management
  ...
  // ============================================================================
  // 🏢 ENTERPRISE DOMAIN SYSTEM INITIALIZATION
  // ============================================================================
  ...
  React.useEffect(() => {
    ...
    const unsubscribeToolActivated = globalGeoEventBus.subscribe('tool-activated', (event) => {
      ...
      geoCanvasState.actions.setActiveTool(event.data.toolId);
    });
  ...
  }, [geoCanvasState.actions]);

  // Initialize enterprise state
  React.useEffect(() => {
    geoCanvasState.actions.initialize({
  ...
  // ============================================================================
  // 🏢 ENTERPRISE PANEL SYSTEM CONFIGURATION
  // ============================================================================

  // Generate dynamic panel data based on user type and active components
  const enterprisePanels: InfoPanelData[] = React.useMemo(() => {
    const panels: InfoPanelData[] = [];

    // Floor Plan Control Point Picker για Professional/Technical
    if (floorPlanUpload.result && floorPlanUpload.result.success && (isProfessional || isTechnical)) {
      panels.push({
        ...
        width: 350,
        height: 400,
        content: <FloorPlanControlPointPicker controlPoints={controlPoints} />
      });
    }
    ...
    if (isCitizen) {
      panels.push({
        id: 'citizen-interface',
        title: 'Citizen Tools',
        position: { x: 16, y: isCitizen && panels.length === 0 ? 16 : 200 },
        width: 350,
        height: 500,
        content: (
    ...
    if (isProfessional) {
      panels.push({
        ...
        position: { x: 16, y: isProfessional && panels.length === 0 ? 16 : 200 },
        width: 350,
        height: 500,
        content: (
    ...
    if (isTechnical) {
      panels.push({
        ...
        position: { x: 16, y: isTechnical && panels.length === 0 ? 16 : 200 },
        width: 350,
        height: 500,
        content: (
  ...
  // ✅ ENTERPRISE: Panel dragging now handled by DraggableInfoPanels domain module
  ...
  // ✅ ENTERPRISE: All drag handling moved to DraggableInfoPanels domain module
    ...
    <PolygonSystemProvider initialRole="citizen">
      ...
      <main className="flex-1 flex">
        ...
        <div className="flex-1 flex flex-col">
          {/* 🏢 ENTERPRISE TOOLBAR - DOMAIN MODULE */}
          <div className={`${colors.bg.primary} ${quick.separatorH}`}>
            <GeoToolbar
              mode={geoCanvasState.state.mode.current}
              actions={configManager.getConfig().tools.map(tool => ({
                id: tool.id,
                type: 'button' as const,
                label: tool.name,
                icon: tool.icon,
                shortcut: tool.keyboard,
                isActive: geoCanvasState.state.tools.activeTool === tool.id,
                isDisabled: !tool.isEnabled,
                tooltip: tool.description,
                onClick: () => {
                  // Emit enterprise event
                  globalGeoEventBus.emit(GeoEventFactory.createToolEvent('activated', tool.id));
                  geoCanvasState.actions.setActiveTool(tool.id);
                }
              }))}
              orientation="horizontal"
              position="top"
              onModeChange={(mode) => {
                geoCanvasState.actions.changeMode(mode);
              }}
              onActionClick={(actionId) => {
                console.log('🔧 Enterprise Toolbar Action:', actionId);
              }}
            />

            {/* Legacy View Controls - To be migrated */}
            <div className="px-3 py-1 flex items-center space-x-4">
              <div className="flex items-center space-x-2">
                <span className="text-gray-400 text-sm">{t('toolbar.view')}</span>
                <select
                  value={activeView}
                  onChange={(e) => {
                  ...
                  className={`${colors.bg.hover} ${quick.input} px-2 py-1 text-xs`}
                >
          ...
          <div className={`flex-1 ${colors.bg.backgroundSecondary} relative`}>
            ...
            {activeView === 'georeferencing' && (
              ...
              <div className="absolute inset-0">
                ...
                {/* 🏢 ENTERPRISE DRAGGABLE INFO PANELS - DOMAIN MODULE */}
                <DraggableInfoPanels
                  panels={enterprisePanels}
                  canvasBounds={{ width: window.innerWidth, height: window.innerHeight }}
                  onPanelMove={(panelId, position) => {
                    console.log(`📊 Enterprise Panel ${panelId} moved to:`, position);
                    // Emit enterprise event
                    globalGeoEventBus.emit(GeoEventFactory.createMapEvent(
                      'panel-opened',
                      { panelId, position },
                      'user'
                    ));
                  ...
                  onPanelClose={(panelId) => {
                    console.log(`📊 Enterprise Panel ${panelId} closed`);
                    globalGeoEventBus.emit(GeoEventFactory.createMapEvent(
                      'panel-closed',
                      { panelId },
                      'user'
                  ...
                  onPanelMinimize={(panelId, isMinimized) => {
                    console.log(`📊 Enterprise Panel ${panelId} minimized:`, isMinimized);
                  }}
                />
              </div>
            )}
