...
export const UI_COLORS_BASE = {
  ...
  /**
   * 🔴 ADR-739 §48 — **tint του `INDICATOR_BLUE`, όχι τέταρτο ανεξάρτητο μπλε.**
   *
   * Γεννήθηκε επειδή το `INDICATOR_BLUE` είναι ήδη πιασμένο **τρεις** φορές γύρω από τα ίδια
   * κελιά (δρομέας · περίγραμμα λειτουργίας · επιλογή) και το δεύτερο από αυτά είναι ήδη
   * **διακεκομμένο**. Ο ιδιοκτήτης ζήτησε ρητά να μείνουμε σε μπλε αποχρώσεις (2026-08-05),
   * άρα η διάκριση χτίζεται σε τέσσερα κανάλια αντί για νέο hue: **απόχρωση** (εδώ), **μοτίβο**
   * (`[7,2]` έναντι `[6,4]`), **πάχος** (1 px έναντι 2) και **κίνηση**.
   *
   * `#7FD0FF` = ο ίδιος τόνος με ~50% λεύκανση: αναγνωρίζεται ως «το μπλε της εφαρμογής,
   * ανοιχτότερο» και όχι ως ξένο χρώμα, ενώ πάνω στο σκούρο φόντο του viewer ξεχωρίζει
   * καθαρά από το `#0099ff` που κάθεται δίπλα του.
   */
  INDICATOR_BLUE_LIGHT: '#7FD0FF',
  /**
   * 🔴 ADR-739 §48 — **βαθύ κορεσμένο μπλε δείκτη** (υπό δοκιμή, 2026-08-05).
   *
   * Γεννήθηκε από παρατήρηση του ιδιοκτήτη ότι το {@link UI_COLORS.INDICATOR_BLUE_LIGHT}
   * διαβάζεται «αχνό σιέλ». Η αιτία **δεν είναι η φωτεινότητα** — είναι ο **κορεσμός**: το
   * `#7FD0FF` είναι το μπλε ξεπλυμένο ~50% προς το λευκό, άρα φωτεινό αλλά χωρίς ένταση.
   *
   * ── 🔴 Γιατί ΑΥΤΟ και όχι πραγματικά σκούρο μπλε ──
   * Ο καμβάς **δεν είναι ένα χρώμα**: το `--canvas-background-dxf` έχει **9 θέματα + custom**
   * (`config/canvas-theme.ts`), με προεπιλογή `nestorApp1 #1d283a`. Μετρημένες αντιθέσεις:
   *
   * ```
   *   #0563C1 (κλασικό «σκούρο μπλε»)  →  2,53:1 σε #1d283a   ❌ κάτω από MIN_ENTITY_CONTRAST
   *   #0A6BFF (εδώ)                    →  3,20:1 σε #1d283a   ✅
   *                                       4,60:1 σε #000000   ✅
   * ```
   *
   * Είναι, δηλαδή, **το βαθύτερο μπλε που περνά ακόμα το όριο 3:1 στο προεπιλεγμένο θέμα**.
   * Πιο σκούρο ⇒ ο δείκτης αρχίζει να χάνεται μέσα στο ίδιο του το φόντο — το ακριβώς ίδιο
   * σφάλμα που τεκμηριώνουν τα `table-ink.ts` / `table-style-presets.ts` για το μελάνι `#111111`
   * (μετρήθηκε **1,27:1** στο ίδιο θέμα, «αόρατο σε 6 από τα 9»).
   *
   * ⚠️ Το όριο ισχύει για το **προεπιλεγμένο** θέμα. Αν κάποτε χρειαστεί πραγματικά σκούρο μπλε,
   * η λύση **υπάρχει ήδη κεντρικά** και δεν είναι νέα σταθερά: αυτόματη επιλογή έναντι του
   * ζωντανού φόντου, όπως κάνει το `table-ink.ts`.
   */
  INDICATOR_BLUE_DARK: '#0A6BFF',
 ...
 *
 * ── Γιατί ΔΕΝ είναι το `INDICATOR_BLUE` ──
 * Θα ήταν το **τέταρτο** μπλε γύρω από τα ίδια κελιά, και το ένα από τα τρία
 * ({@link TABLE_MODE_OUTLINE}) είναι ήδη **διακεκομμένο**. Δύο διακεκομμένα μπλε ορθογώνια που
 * συνυπάρχουν πάντα — το ένα γύρω από τον πίνακα, το άλλο γύρω από την περιοχή — θα ήταν
 * αδιάκριτα σε κάθε γωνία που εφάπτονται, και ο χρήστης θα έπρεπε να μαντέψει ποιο σημαίνει
 * «είμαι μέσα στον πίνακα» και ποιο «αυτό είναι στο πρόχειρο».
 *
 ...
 * δόγμα του αρχείου — αντιγράφουμε τον **κανόνα** («η αντιγραφή έχει δικό της χρώμα,
 * διαφορετικό από τον δρομέα»), όχι τον **κατάλογο**.
 *
 * ── Πώς ξεχωρίζει, τότε: τέσσερα κανάλια ──
 * **απόχρωση** ({@link UI_COLORS.INDICATOR_BLUE_LIGHT}) · **μοτίβο** (`[7,2]` έναντι `[6,4]`,
 * δηλαδή 78% έναντι 60% πυκνότητα μελανιού) · **πάχος** (1 px έναντι 2) · **κίνηση**. Το
 * τελευταίο είναι το ισχυρότερο και είναι μοναδικό στον καμβά: **τίποτε άλλο δεν κινείται**.
 * Γι' αυτό το μοτίβο και η ταχύτητα ζουν στο `bim/table/table-marching-ants.ts` — εκεί όπου η
 * κίνηση είναι **αριθμητική** και όχι χρώμα.
 */
export const TABLE_COPY_MARQUEE = {
  /** ⚠️ ΥΠΟ ΔΟΚΙΜΗ (2026-08-05): ήταν `INDICATOR_BLUE_LIGHT`. Δες το σκεπτικό στο token. */
  colorHex: UI_COLORS.INDICATOR_BLUE_DARK,
  /**
