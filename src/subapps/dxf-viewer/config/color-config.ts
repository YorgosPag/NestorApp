...
export const UI_COLORS_BASE = {
  ...
  // Indicators
  /**
   * 🔴 ADR-739 §48.13 — **ΤΟ ΕΝΑ μπλε δείκτη των κελιών**: δρομέας, περίγραμμα επιλογής,
   * περίγραμμα λειτουργίας **και** τα μυρμήγκια της αντιγραφής δείχνουν όλα εδώ.
   *
   * ⚠️ **Δύο tint (`INDICATOR_BLUE_LIGHT #7FD0FF`, `INDICATOR_BLUE_DARK #0A6BFF`) διαγράφηκαν
   * στις 2026-08-06.** Γεννήθηκαν για να ξεχωρίζουν τα μυρμήγκια από την επιλογή όταν οι δύο
   * κάθονταν στην **ίδια διαδρομή** — σύγκρουση που το §48.12 έλυσε στη ρίζα της (το συμπαγές
   * περίγραμμα αποσύρεται). Ο ιδιοκτήτης ζήτησε ενοποίηση (2026-08-06) και η απόχρωση έπαψε να
   * έχει δουλειά: έμεινε **νεκρός κώδικας με τεκμηρίωση**, που είναι το χειρότερο είδος —
   * διαβάζεται ως ενεργός κανόνας. Το ιστορικό των μετρήσεων ζει στο ADR §48, όχι εδώ.
   */
 ...
 *
 * ── 🔴 ΕΝΑ ΧΡΩΜΑ ΜΕ ΤΟ ΠΕΡΙΓΡΑΜΜΑ ΠΟΥ ΦΕΡΕΙ ΤΗ ΛΑΒΗ (ιδιοκτήτης, 2026-08-06) ──
 * Τα μυρμήγκια βάφονται με το **ίδιο** `INDICATOR_BLUE` που βάφει το περίγραμμα της επιλογής
 * ({@link TABLE_CELL_SELECTION}) και τον δρομέα ({@link TABLE_CELL_CURSOR}). Ρητή απόφαση, και
 * **αντιστροφή** της προηγούμενης: μέχρι τις 2026-08-05 ήταν ξεχωριστή απόχρωση.
 *
 * 🔑 Ο λόγος που η απόχρωση **έπαψε να χρειάζεται** είναι το §48.12. Το επιχείρημα υπέρ της
 * ήταν «δύο δείκτες πάνω στα ίδια κελιά πρέπει να ξεχωρίζουν» — αλλά από τη στιγμή που το
 * συμπαγές περίγραμμα **αποσύρεται** μπροστά στα μυρμήγκια, οι δύο **δεν συνυπάρχουν ποτέ πάνω
 * στην ίδια διαδρομή**. Και όπου συνυπάρχουν (πρόχειρο εδώ, επιλογή αλλού) είναι σε
 * **διαφορετικές θέσεις**, όπου η θέση είναι ήδη η απάντηση. Η απόχρωση έλυνε σύγκρουση που
 * δεν υπάρχει πια· κρατώντας την, ο χρήστης πλήρωνε ένα τέταρτο μπλε για το τίποτα.
 *
 * ⚠️ **Το τίμημα, ρητά**: το {@link TABLE_MODE_OUTLINE} είναι κι αυτό διακεκομμένο μπλε και
 * **συνυπάρχει πάντα**. Η διάκριση στηρίζεται πλέον σε **τρία** κανάλια αντί για τέσσερα (δες
 * παρακάτω) — από τα οποία το ισχυρότερο, η **κίνηση**, δεν το έχει κανένας άλλος δείκτης. Το
 * περίγραμμα λειτουργίας είναι επιπλέον **η περίμετρος ολόκληρου του πίνακα**, δηλαδή σχεδόν
 * ποτέ ομόκεντρο με μια περιοχή κελιών.
 *
 * ── Γιατί ΔΕΝ είναι το πράσινο `#217346` του Excel ──
 * Ο ιδιοκτήτης το απέρριψε ρητά (2026-08-05): «μένουμε σε μπλε αποχρώσεις». Ισχύει και το
 * δόγμα του αρχείου — αντιγράφουμε τον **κανόνα**, όχι τον **κατάλογο**. Σημείωση: το ίδιο το
 * Excel βάφει **και** τα μυρμήγκια **και** την επιλογή στο ίδιο accent, οπότε η ενοποίηση από
 * πάνω είναι *πιο* κοντά στο πρότυπο απ' ό,τι ήταν η ξεχωριστή απόχρωση.
 *
 * ── Πώς ξεχωρίζει, τότε: τρία κανάλια ──
 * **μοτίβο** (`[7,2]×3` έναντι `[6,4]`, δηλαδή 78% έναντι 60% πυκνότητα μελανιού) · **πάχος**
 * (3 px έναντι 2) · **κίνηση**. Το τελευταίο είναι το ισχυρότερο και είναι μοναδικό στον
 * καμβά: **τίποτε άλλο δεν κινείται**. Γι' αυτό το μοτίβο και η ταχύτητα ζουν στο
 * `bim/table/table-marching-ants.ts` — εκεί όπου η κίνηση είναι **αριθμητική** και όχι χρώμα.
 */
export const TABLE_COPY_MARQUEE = {
  /**
   * 🔴 §48.13 — **ΤΟ ΙΔΙΟ token με το περίγραμμα που φέρει τη λαβή**, ποτέ αντίγραφο της τιμής.
   *
   * Και τα τρία περιγράμματα των κελιών (δρομέας · επιλογή · μυρμήγκια) δείχνουν στο **ένα**
   * `UI_COLORS.INDICATOR_BLUE`. Μια μελλοντική αλλαγή του μπλε δείκτη ταξιδεύει και στα τρία
   * χωρίς να χρειάζεται να το θυμηθεί κανείς — και υπάρχει anchor που κοκκινίζει αν αποκλίνουν
   * (`table-marching-ants.test.ts`).
   */
  colorHex: UI_COLORS.INDICATOR_BLUE,
  /**
   * 🔴 ADR-739 §48.11 — **ΤΟ ΠΑΧΟΣ ΔΕΝ ΖΕΙ ΠΙΑ ΕΔΩ**: είναι το
   * `MARCHING_ANTS_LINE_WIDTH_PX` στο `bim/table/table-marching-ants.ts`.
   *
   * Δεν είναι μετακόμιση για την τάξη — είναι **αλλαγή ιδιοκτησίας**. Όσο το marquee ήταν
   * «γραμμή με χρώμα και πάχος», το πάχος ανήκε στο λεξιλόγιο δεικτών αυτού του αρχείου. Από τη
   * στιγμή που έγινε **«το marquee του Excel σε κλίμακα N»**, το πάχος είναι σκέλος της
   * αναλογίας 7:2:1 — δηλαδή **γεωμετρία του μοτίβου**, όχι απόφαση χρώματος. Αν έμενε εδώ ως
   * ανεξάρτητος αριθμός, θα επέτρεπε συνδυασμούς που δεν είναι το Excel σε καμία κλίμακα.
   *
   * Το ιστορικό της παρέκκλισης από τα 2 px (WCAG 2.2 SC 2.4.13 — που δεσμεύει τον δείκτη
   * **εστίασης**, δηλαδή τον {@link TABLE_CELL_CURSOR}, ο οποίος μένει συμπαγής στα 2 px)
   * τεκμηριώνεται στο ADR §48.6 και §48.11. Για ακέραια κλίμακα ≥ 2 η παρέκκλιση **παύει να
   * υφίσταται**.
   */
} as const;
...
export const TABLE_CELL_SELECTION = {
   ...
   * ακουμπά τη σκίαση του ίδιου χρώματος.
   *
   * 🔴 **ΥΠΟ ΔΟΚΙΜΗ (2026-08-05): έπεσε σε 1 px και ΕΠΑΨΕ να είναι παράγωγο του δρομέα.**
   * Αίτημα του ιδιοκτήτη: αμέσως μετά το `Ctrl+C` η ίδια περιοχή είναι **ταυτόχρονα**
   * επιλεγμένη και αντιγραμμένη, οπότε αυτό το συμπαγές περίγραμμα και τα «μυρμήγκια» του
   * §48 κάθονται στην **ίδια ακριβώς διαδρομή** — το συμπαγές πνίγει το διακεκομμένο και η
   * κίνηση χάνεται. Λεπταίνει, ώστε να μείνει το marquee η κυρίαρχη ανάγνωση.
   *
   * ⚠️ Το **1 είναι πάτωμα, όχι προτίμηση**: κάθε μη ακέραιη τιμή (`0.5`, `1.5`) μοιράζεται σε
   * ημιδιαφανείς σειρές pixel και δίνει **θόλωμα αντί για λεπτή γραμμή** — δες παραπάνω, το
   * ίδιο σφάλμα έχει ήδη συμβεί μία φορά εδώ.
   *
   * ⚠️ **Τι έσπασε συνειδητά**: ήταν `TABLE_CELL_CURSOR.lineWidthPx` επειδή τα δύο ορθογώνια
   * συμπίπτουν σε επιλογή 1×1 και «οφείλουν να φαίνονται ένα». Πλέον σε επιλογή 1×1 ο δρομέας
   * (2 px) και αυτό (1 px) **διαφέρουν**. Αν φανεί διπλή γραμμή εκεί, η αιτία είναι αυτή.
   */
  outlineWidthPx: 1,
} as const;
...
/**
 * 🔴 ADR-739 §41 — **ΤΟ ⊖ ΤΗΣ ΔΙΑΓΡΑΦΗΣ**: ο δίσκος μέσα στη ζώνη, και το κόκκινο πλύσιμο που
 * δείχνει **ποιοι άξονες φεύγουν**.
 *
 * ## Το κόκκινο ΔΕΝ επιλέγεται εδώ — είναι ήδη διαλεγμένο σε αυτό το αρχείο
 * Το `UI_COLORS.ERROR` σημαίνει ήδη **άρνηση/καταστροφή** εδώ (δες `refusedOutlineHex` του
 * φαντάσματος μεταφοράς). Ένα νέο hex θα ήταν δεύτερο κόκκινο με την ίδια σημασία.
 *
 * ## 🔴 Η ΔΟΜΗ ΤΟΥ ΔΙΣΚΟΥ ΕΙΝΑΙ Η ΙΔΙΑ ΜΕ ΤΟΥ ⊕ — και το περίγραμμα είναι που τη σηκώνει
 * Το σχήμα του δίσκου το δηλώνει **πάντα** το σκούρο περίγραμμα, ποτέ το γέμισμα: είναι το
 * ίδιο μελάνι (`RULER_DARK_GRAY`) με τις γραμμές της ζώνης, δηλαδή ο δίσκος «ανήκει» οπτικά
 * εκεί που κάθεται. Το γέμισμα αλλάζει με τη **φάση** και είναι η δήλωση κατάστασης.
 *
 * ## ⚠️ ΜΕΤΡΗΜΕΝΕΣ ΑΝΤΙΘΕΣΕΙΣ (§38.11 — «μη διαλέξεις κόκκινο με το μάτι»)
 * Ο δίσκος κάθεται σε **γνωστή, φραγμένη** επιφάνεια (τη ζώνη), όχι πάνω σε αυθαίρετο σχέδιο
 * όπως το ⊕ — άρα οι υποψήφιοι είναι **δύο**, όχι δέκα:
 *
 * | ζεύγος | λόγος | κατώφλι 3:1 (WCAG 1.4.11, γραφικά) |
 * |---|---|---|
 * | περίγραμμα `#666` ↔ ουδέτερη ζώνη `#f0f0f0` | **5,04:1** | ✅ |
 * | γέμισμα `#ef4444` ↔ ουδέτερη ζώνη `#f0f0f0` | **3,30:1** | ✅ |
 * | λευκό `−` ↔ γέμισμα `#ef4444` | **3,76:1** | ✅ |
 * | κόκκινο `−` ↔ λευκό γέμισμα (φάση `nearby`) | **3,76:1** | ✅ |
 *
 * 🔴 **Η ΜΙΑ ΠΕΡΙΠΤΩΣΗ ΠΟΥ ΔΕΝ ΠΕΡΝΑΕΙ, ΔΗΛΩΜΕΝΗ ΑΝΤΙ ΓΙΑ ΣΙΩΠΗΛΗ**: πάνω σε **ενεργή** ζώνη
 * (`INDICATOR_BLUE #0099ff`) το κόκκινο δίνει **1,26:1** και το σκούρο περίγραμμα **1,91:1**.
 * Το κόκκινο και αυτό το μπλε έχουν σχεδόν **ίδια φωτεινότητα** — καμία απόχρωση κόκκινου δεν
 * το λύνει (το `DARK_RED` δίνει 1,61:1). Δεν χαλαρώνεται κατώφλι και δεν εφευρίσκεται τρίτο
 * χρώμα· η κατάσταση δηλώνεται από **επιφάνεια**, όχι από λεπτή αντίθεση: το `armed` βάφει
 * **ολόκληρο τον άξονα** ({@link washRgba}), σήμα ασύγκριτα μεγαλύτερο από έναν δίσκο 14 px.
 * ⚠️ Το 1,91:1 του περιγράμματος **προϋπάρχει**: είναι το ίδιο μελάνι που η ζώνη ζωγραφίζει
 * ήδη πάνω στην ενεργή στήλη (`TABLE_INDICATOR.lineHex`). Δεν το εισάγει το ⊖.
 *
 * @see bim/table/table-delete-control.ts — ΠΟΥ κάθεται (η γεωμετρία που τα καταναλώνει)
 */
export const TABLE_DELETE_CONTROL = {
  /** Γέμισμα δίσκου στη φάση `nearby` — συμπαγές λευκό, ίδια αρχή με το ⊕. */
  fillHex: UI_COLORS.WHITE,
  /** Περίγραμμα δίσκου, **και στις δύο φάσεις** — δες την κεφαλίδα: αυτό σηκώνει το σχήμα. */
  lineHex: UI_COLORS.RULER_DARK_GRAY,
  /** Το `−` στη φάση `nearby`: **κόκκινο πάνω σε λευκό** — η πράξη δηλώνεται πριν οπλιστεί. */
  glyphHex: UI_COLORS.ERROR,
  /** Γέμισμα δίσκου στη φάση `armed` — αντιστροφή: το κόκκινο γίνεται επιφάνεια. */
  armedFillHex: UI_COLORS.ERROR,
  /** Το `−` πάνω στο συμπαγές κόκκινο. */
  armedGlyphHex: UI_COLORS.WHITE,
  /**
   * 🔴 **Η ΠΡΟΕΠΙΣΚΟΠΗΣΗ ΤΗΣ ΑΠΩΛΕΙΑΣ**: ό,τι θα φύγει, βαμμένο — μόνο στη φάση `armed`.
   *
   * Αυτό είναι που κάνει τον διάλογο επιβεβαίωσης **περιττό** (πρβλ. ADR-755): εκεί ο διάλογος
   * υπάρχει επειδή η συγχώνευση δεν **μπορεί** να δείξει τι χάνεται· εδώ φαίνεται ολόκληρο,
   * πριν το κλικ. Η διαφάνεια είναι χαμηλή επίτηδες — ο άξονας πρέπει να παραμείνει
   * **αναγνώσιμος** όσο βάφεται, αλλιώς ο χρήστης χάνει ακριβώς την πληροφορία που κρίνει.
   */
  washRgba: hexToRgba(UI_COLORS.ERROR, 0.18),
  /** Πάχος περιγράμματος δίσκου, σε px οθόνης — **το ίδιο** με το ⊕. */
  lineWidthPx: 1,
  /** Πάχος του `−`, σε px οθόνης — **το ίδιο** με τον σταυρό του ⊕. */
  glyphWidthPx: 1.5,
} as const;
 ...
 *
 * ## Γιατί το ΙΔΙΟ μπλε με τον δρομέα, ενώ οι αναφορές τύπου πήραν δική τους παλέτα
 * Η αντίθετη απόφαση (§11.5) ήταν σωστή **εκεί**: μια αναφορά τύπου είναι **άλλο πράγμα** από
 * τον δρομέα και το μπλε ήταν ήδη κατειλημμένο τρεις φορές γύρω από τα ίδια κελιά. Εδώ ισχύει
 * το αντίστροφο — η λαβή **ανήκει** στην επιλογή: είναι το χερούλι της, κάθεται πάνω στην
 * κορυφή της, και εξαφανίζεται μαζί της. Ένα τέταρτο χρώμα θα έλεγε «κάτι άλλο ζει εδώ», ενώ
 * δεν ζει: είναι η ίδια οντότητα με λαβή. Το Excel κάνει ακριβώς το ίδιο (πράσινο δρομέας,
 * πράσινη λαβή).
 */
export const TABLE_FILL_HANDLE = {
  /** Γέμισμα — συμπαγές, ώστε να έχει σχήμα πάνω σε κάθε φόντο. */
  fillHex: UI_COLORS.INDICATOR_BLUE,
  /**
   * Λευκό περίγραμμα, **πάντα**: η λαβή κάθεται πάνω στη γραμμή της επιλογής, που είναι κι
   * αυτή μπλε. Χωρίς αυτό, το τετράγωνο θα χανόταν μέσα στην ίδια του τη γραμμή και ο χρήστης
   * δεν θα έβλεπε **ότι υπάρχει κάτι να πιάσει** — η ακριβής αστοχία ανακάλυψης του §31.8.
   */
  outlineHex: UI_COLORS.WHITE,
  outlineWidthPx: 1,
  /** Το διακεκομμένο περίγραμμα της προεπισκόπησης όσο σέρνεις. */
  previewHex: UI_COLORS.INDICATOR_BLUE,
  previewWidthPx: 1,
  /** Το μοτίβο της προεπισκόπησης, σε px οθόνης. */
  previewDashPx: [3, 3] as readonly number[],
} as const;

/**
 * 🔴 ADR-828 Φ4α — **ΤΟ ΚΟΥΜΠΙ «ΕΠΙΛΟΓΕΣ ΑΥΤΟΜΑΤΗΣ ΣΥΜΠΛΗΡΩΣΗΣ»** (Excel smart tag).
 *
 * ## Γιατί ΑΝΤΙΣΤΡΟΦΟ από τη λαβή, και όχι ίδιο
 * Η λαβή είναι **μπλε γεμάτη με λευκό περίγραμμα**: κάθεται πάνω στη γραμμή της επιλογής και
 * πρέπει να ξεχωρίσει από **αυτήν**. Το κουμπί κάθεται στο **κενό**, κάτω από την περιοχή, και
 * η ερώτησή του είναι άλλη: «*είμαι πράγμα που πατιέται*». Ένα δεύτερο μπλε τετράγωνο 8 px πιο
 * κάτω θα διαβαζόταν ως **δεύτερη λαβή** — δηλαδή θα υποσχόταν σύρση εκεί που υπάρχει κλικ.
 *
 * Άρα: **λευκή πλάκα με μπλε περίγραμμα και μπλε σύμβολο** — η οικογένεια «κουμπί πάνω από
 * φύλλο» που ο χρήστης έχει ήδη μάθει από το ίδιο το Excel. Το μπλε είναι το **ίδιο** token με
 * τη λαβή και τα τρία περιγράμματα κελιών (`INDICATOR_BLUE`), ποτέ αντίγραφο της τιμής: μια
 * μελλοντική αλλαγή του μπλε δείκτη ταξιδεύει και εδώ χωρίς να το θυμηθεί κανείς.
 */
export const TABLE_FILL_BADGE = {
  /** Η πλάκα — συμπαγής, ώστε να διαβάζεται πάνω σε κάθε φόντο σχεδίου. */
  fillHex: UI_COLORS.WHITE,
  outlineHex: UI_COLORS.INDICATOR_BLUE,
  outlineWidthPx: 1,
  /** Το σύμβολο μέσα στην πλάκα (το βελάκι «άνοιξε μενού»). */
  glyphHex: UI_COLORS.INDICATOR_BLUE,
  glyphWidthPx: 1.5,
} as const;

/**
 * 🔴 ADR-767 Δ4 — **Ο ΔΕΣΜΟΣ ΤΟΥ ΠΙΝΑΚΑ ΣΤΗΝ ΟΘΟΝΗ** (και **ποτέ** στο χαρτί).
 *
 * ── Ο διαχωρισμός που αντιγράφουμε από το AutoCAD, ως **ρητή αρχή** ──
 * Το AutoCAD ξεχωρίζει δύο πράγματα με τρεις άσχετες μεταβλητές (`FIELDDISPLAY` /
 * `DATALINKNOTIFY` / `DXEVAL`). Εμείς τα λέμε μία φορά:
 *
 * | τι δηλώνεται | τι είναι | πού φαίνεται |
 * |---|---|---|
 * | «η στήλη τρέφεται από πηγή» · «κάποιος παρέκαμψε» | **διακόσμηση** | **μόνο οθόνη** (`FIELDDISPLAY`: *«not plotted»*) |
 * | «τα νούμερα είναι μπαγιάτικα» | **ισχυρισμός ορθότητας** | οθόνη **+ φραγμός** στην εξαγωγή (`DXEVAL`) |
 *
 * ── 🔴 ΤΡΙΑ ΧΡΩΜΑΤΑ, ΤΡΕΙΣ ΔΙΑΦΟΡΕΤΙΚΕΣ ΕΡΩΤΗΣΕΙΣ ──
 * Δεν είναι διαβάθμιση έντασης· είναι **ξένες** ερωτήσεις, και η κοκκομετρία τους διαφέρει:
 *
 * - `columnHex` — **ανά στήλη** (Δ8: ο δεσμός *είναι* η στήλη). Πληροφορία, όχι προειδοποίηση
 *   ⇒ το ίδιο `INDICATOR_BLUE` της υπόλοιπης διεπαφής πίνακα, ημιδιαφανές. Ένα σημάδι ανά
 *   κελί θα ήταν **800** σε τοπογραφικό 200 κορυφών — το Δ5 ονομάζει αυτή την αστοχία.
 * - `overriddenHex` — **ανά κελί**. «Διαφωνώ»: ανθρώπινη απόφαση, όχι σφάλμα ⇒ **κεχριμπάρι**.
 * - `conflictHex` — **ανά κελί**. «Η πηγή κουνήθηκε **κάτω από** τη διαφωνία σου»: η μόνη
 *   κατάσταση που απαιτεί απόφαση ⇒ το ίδιο `ERROR` που ήδη σημαίνει «σταμάτα» στο ⊖.
 *
 * ── Γιατί ΟΧΙ βάψιμο του κειμένου (η σύμβαση της ArchiCAD) ──
 * Η ArchiCAD δηλώνει «μη επεξεργάσιμο» με **μαύρο μελάνι** και «επεξεργάσιμο» με μπλε. Είναι
 * η μόνη τεκμηριωμένη σύμβαση της αγοράς — και **δεν** μεταφέρεται εδώ: το μελάνι του κελιού
 * ανήκει στον χρήστη (`textColorHex`, §38) και ταξιδεύει στο **χαρτί**. Βάφοντάς το, ή θα
 * καταπίναμε την επιλογή του χρήστη, ή θα τυπώναμε την κατάσταση δεσμού — δηλαδή θα σπάγαμε
 * το Δ4 από την πίσω πόρτα. Το σημάδι κάθεται **δίπλα** στο περιεχόμενο, ποτέ πάνω του.
 *
 * ── 🔴 ΓΙΑΤΙ ΔΕΝ ΥΠΑΡΧΕΙ `boundCellHex` ──
 * Μέσα σε δεμένο πίνακα, «δεμένο» είναι ο **κανόνας** — δεν είναι είδηση. Είδηση είναι η
 * **εξαίρεση**. Ένα τέταρτο χρώμα για την κανονικότητα θα ήταν ακριβώς ο θόρυβος που ο
 * χρήστης μαθαίνει να αγνοεί, και τότε θα έχανε **και** τα δύο σημάδια που μετράνε.
 *
 * @see bim/table/binding/table-bound-marks.ts — ΤΙ σημαδεύεται (η κρίση)
 ...
 */
/**
 * 🔴 ADR-771 Φ.1 — οι δύο **ελεύθερες** γωνίες κελιού, δηλαδή το δεύτερο κανάλι ταυτότητας.
 *
 * Κάτω-δεξιά **δεν** είναι επιλογή: εκεί κάθεται η λαβή συμπλήρωσης (ADR-754 Γ4). Κάτω-
 * αριστερά είναι τεχνικά ελεύθερη αλλά αποκλείεται σκόπιμα — το μάτι σαρώνει το κελί από
 * πάνω, και ένα σημάδι στη βάση θα ανακαλυπτόταν τελευταίο.
 */
export type TableExceptionMarkCorner = 'top-left' | 'top-right';

export const TABLE_BOUND_STATE = {
  /** Η λωρίδα της **δεμένης στήλης**, στην πάνω ακμή του πίνακα. */
  columnHex: UI_COLORS.INDICATOR_BLUE,
  /** Πάχος λωρίδας σε **px οθόνης** — δείκτης διεπαφής, όχι γεωμετρία σχεδίου. */
  columnThicknessPx: 3,
  /**
   * Ημιδιαφανής: η λωρίδα κάθεται πάνω στην περίμετρο του πίνακα και δεν επιτρέπεται να
   * διαβαστεί ως **γραμμή του σχεδίου**. Δηλώνει προέλευση, δεν χαράζει.
   */
  columnAlpha: 0.55,
  /**
   * 🔴 ADR-769 Δ7 — **η δεμένη στήλη που ΔΕΝ γράφεται**: ίδιο χρώμα, μισή ένταση.
   *
   * ## Γιατί ένταση και όχι τέταρτο χρώμα
   * Το ArchiCAD δηλώνει τη γραψιμότητα με **χρώμα** (*«non-editable in black, editable in
   * blue»*) επειδή εκεί το κείμενο του κελιού είναι ο φορέας. Εδώ ο φορέας είναι η λωρίδα
   * που ήδη λέει «τρέφομαι από πηγή», και το μπλε της είναι **πιασμένο**: είναι το
   * `INDICATOR_BLUE`, το χρώμα «δείκτη» όλου του viewer (δες `TABLE_CELL_CURSOR`). Ένα
   * τέταρτο χρώμα στην ίδια λωρίδα θα ζητούσε από τον χρήστη να θυμάται **τέσσερα** μπλε.
   *
   * Η ένταση λέει το ίδιο πράγμα με **λιγότερο λεξιλόγιο**, και είναι η λέξη που χρησιμοποιεί
   * το ίδιο το Revit για τα calculated values: *«read-only… greyed out»*. Έντονη λωρίδα =
   * «γράψε μου και θα το πω στην οντότητα»· ξεθωριασμένη = «μόνο διαβάζεις».
   *
   * ⚠️ **Οθόνη μόνο**, όπως όλη η οικογένεια: ο φρουρός είναι το `getPrintColorPolicy()`
   * στο `stamp-table-bound-state.ts` — δες τη δική του κεφαλίδα για το γιατί το
   * `skipInteractive` **δεν** τον αντικαθιστά.
   */
  readOnlyColumnAlpha: 0.22,
  /**
   * 🔴 ADR-771 Φ.1 — **τα δύο σημάδια εξαίρεσης: χρώμα ΚΑΙ γωνία, ποτέ μόνο χρώμα.**
   *
   * ## Τι άλλαξε και γιατί (07/08)
   * Και τα δύο τρίγωνα κάθονταν **πάνω-αριστερά** και διέφεραν **μόνο στην απόχρωση**. Η
   * ταυτότητα της κατάστασης ήταν δηλαδή κωδικοποιημένη σε **ένα** κανάλι — το ακριβώς
   * απαγορευμένο από το **WCAG 1.4.1** («Use of Color»): το χρώμα δεν επιτρέπεται να είναι
   * ο *μόνος* φορέας πληροφορίας.
   *
   * ⚠️ **Η δικαιολόγηση ΔΕΝ είναι «τα χρώματα μοιάζουν»** — μετρήθηκε και **δεν** μοιάζουν:
   * `#f59e0b` ↔ `#ef4444` δίνουν ΔE **13,9** στη χειρότερη προσομοίωση CVD (Machado 2009),
   * δηλαδή **πάνω** από το κατώφλι 8 του CHECK 3.32. Τρεις άλλοι λόγοι στέκουν:
   *
   *  1. **Ταυτότητα ≠ διάκριση.** Το ΔE απαντά «ξεχωρίζουν;»· το 1.4.1 ρωτά «ξέρω **ποιο
   *     είναι ποιο**;». Δύο τρίγωνα στην ίδια γωνία δεν το απαντούν σε καμία τιμή ΔE.
   *  2. **Το κατώφλι 8 είναι βαθμονομημένο για μπάρες γραφημάτων**, όχι για σχήμα **6px**·
   *     η χρωματική διάκριση σε μικρό οπτικό πεδίο είναι σημαντικά χειρότερη.
   *  3. **Συνέπεια.** Η ίδια οικογένεια ήδη κωδικοποιεί με μη-χρωματικά κανάλια παντού
   *     αλλού: `stale` ↔ `overridden` έχουν **ταυτόσημο** hex (ΔE **0**) και διακρίνονται
   *     αποκλειστικά από τη γεωμετρία· η γραψιμότητα διακρίνεται από **ένταση**. Μόνο αυτό
   *     το ζεύγος στηριζόταν στην απόχρωση.
   *
   * ## Γιατί η γωνία είναι το σωστό δεύτερο κανάλι
   * Είναι η σύμβαση που **ήδη ξέρει ο χρήστης**: το Excel χρησιμοποιεί τη γωνία ως διάσταση
   * (δείκτης σφάλματος πάνω-αριστερά · σχόλιο πάνω-δεξιά). Και οι δύο πάνω γωνίες του κελιού
   * είναι ελεύθερες — η λαβή συμπλήρωσης κάθεται **κάτω**-δεξιά (§Γ4).
   *
   * 🔑 **Ο ζωγράφος διαβάζει ΑΥΤΟ το πεδίο, και η πύλη διαβάζει ΤΟ ΙΔΙΟ.** Δεν υπάρχει
   * ξεχωριστό «μεταδεδομένο καναλιού» που θα μπορούσε να ψευτίσει: αν κάποιος επαναφέρει τις
   * δύο γωνίες στην ίδια τιμή, ο έλεγχος διακριτότητας το βλέπει **επειδή** είναι το πεδίο
   * που ζωγραφίζεται. Δες `scripts/check-state-channel-distinctness.js`.
   */
  exceptionMarks: {
    /** Παράκαμψη — **ανθρώπινη απόφαση**, όχι σφάλμα. Η θέση του δείκτη σφάλματος του Excel. */
    overridden: { hex: UI_COLORS.WARNING, corner: 'top-left' },
    /** Σύγκρουση — η μόνη κατάσταση που **απαιτεί** απόφαση. Αντίθετη γωνία, αντίθετο βάρος. */
    conflict: { hex: UI_COLORS.ERROR, corner: 'top-right' },
  },
  /**
   * Η κάθετη πλευρά του τριγώνου στη γωνία του κελιού, σε **px οθόνης**.
   *
   * Κοινή και για τις δύο καταστάσεις **επίτηδες**: το μέγεθος δεν είναι διακριτικό κανάλι
   * εδώ — ένα μεγαλύτερο τρίγωνο θα διάβαζε ως «πιο επείγον», δηλαδή θα πρόσθετε τρίτη
   * σημασία που το σχήμα δεν έχει.
   */
  markSizePx: 6,
  /**
   * 🔴 **Ο ΜΠΑΓΙΑΤΙΚΟΣ ΠΙΝΑΚΑΣ** — το ίδιο μοτίβο λωρίδας, σε κεχριμπάρι και **διακεκομμένο**.
   *
   * Δεν είναι τέταρτο χρώμα σε νέα θέση: είναι η **ίδια** λωρίδα που ήδη λέει «αυτή η στήλη
   * τρέφεται από πηγή», να λέει τώρα «…και δεν συμφωνεί μαζί της». Ένα ξεχωριστό σημάδι θα
   * ήταν δεύτερο λεξιλόγιο για την ίδια οικογένεια πληροφορίας.
   *
   * ⚠️ Ζωγραφίζεται **μόνο** όταν ένας ρητός έλεγχος έχει τρέξει (`table-binding-freshness-store`).
   * «Κανείς δεν κοίταξε» δεν είναι «ενημερωμένος» — αλλά δεν είναι ούτε «μπαγιάτικος»: είναι
   * **σιωπή**.
   */
  staleHex: UI_COLORS.WARNING,
  /** Το μοτίβο της μπαγιάτικης λωρίδας, σε px οθόνης — «σπασμένος» δεσμός. */
  staleDashPx: [4, 3] as readonly number[],
} as const;

// ============================================================================
// 🔴 ADR-782 §27.4 — ΤΑ ΣΗΜΑΔΙΑ ΤΗΣ ΑΝΤΙΣΤΟΙΧΙΣΗΣ ΥΠΟΒΑΘΡΟΥ (2026-08-11)
// ============================================================================

/**
 * 🔴 ADR-782 §27.4 — **η δεύτερη οικογένεια καταστάσεων** που κρίνει το CHECK 3.41.
 *
 * ## Τι ζωγραφίζει
 * Το εργαλείο «Αντιστοίχιση» ζητά από τον χρήστη να δείξει **το ίδιο σημείο δύο φορές**: μια
 * φορά πάνω στο σχέδιο, μια φορά πάνω στον χάρτη. Μέχρι σήμερα δεν ζωγραφιζόταν **τίποτα** —
 * το πάνελ δήλωνε **πλήθος** («Αντιστοιχίες: 1 από 2»), ποτέ **πού**. Πάνω σε ράστερ χάρτη λίγα
 * εικονοστοιχεία είναι **μέτρα**: εργαλείο ακριβείας που δεν δείχνει τι του είπες δεν είναι
 * ανακριβές, είναι **αδοκίμαστο από τον ίδιο τον χρήστη**.
 *
 * ## 🔑 ΜΙΑ ΑΠΟΧΡΩΣΗ, ΤΡΙΑ ΣΧΗΜΑΤΑ — πάμε ΠΑΡΑΠΕΡΑ από την πρώτη οικογένεια
 * Το `TABLE_BOUND_STATE.exceptionMarks` παραπάνω λύνει το WCAG 1.4.1 δίνοντας **και** γωνία
 * **και** χρώμα: το χρώμα υπάρχει, απλώς δεν είναι μόνο του. Εδώ το χρώμα **δεν υπάρχει ως
 * κανάλι καθόλου** — και οι τρεις καταστάσεις μοιράζονται **ταυτόσημο** hex. Έτσι η ερώτηση
 * «μήπως κάποιος στηρίχτηκε στην απόχρωση;» δεν έχει απλώς αρνητική απάντηση: είναι **δομικά
 * αδύνατη**, σε κάθε τιμή ΔE, χωρίς να χρειάζεται μέτρηση.
 *
 * Ο λόγος δεν είναι καθαρότητα — είναι ότι εδώ τα δύο σημάδια **επικαλύπτονται εκ σχεδιασμού**:
 * όταν η προσαρμογή είναι τέλεια, το σημείο σχεδίου και το σημείο χάρτη πέφτουν στο **ίδιο**
 * εικονοστοιχείο. Δύο σημάδια που διαφέρουν μόνο σε απόχρωση και κάθονται το ένα πάνω στο
 * άλλο δεν είναι δυσανάγνωστα — είναι **ένα** σημάδι.
 *
 * ## Γιατί ΑΥΤΑ τα σχήματα
 * | Κατάσταση | Σχήμα | Από πού |
 * |---|---|---|
 * | `drawingPending` | τετράγωνο **διάστικτο** | AutoCAD «rubber»: δείχτηκε, περιμένει το ταίρι του |
 * | `drawingSettled` | τετράγωνο **συμπαγές** | το τετράγωνο του σημείου βάσης/άκρου κάθε CAD — σημείο **του σχεδίου** |
 * | `mapSettled` | κύκλος **συμπαγής** | η γλώσσα της χαρτογραφίας (GCP: QGIS Georeferencer, ArcGIS) — σημείο **της Γης** |
 *
 * **ΔΥΟ ανεξάρτητοι άξονες, ποτέ ένας με «ή»** (το μάθημα του CHECK 3.41): το **σχήμα** απαντά
 * «σχέδιο ή χάρτης;», το **μοτίβο γραμμής** απαντά «ολοκληρωμένο ή εκκρεμές;». Γι' αυτό το
 * `variant` που διαβάζει η πύλη είναι `<σχήμα>:<μοτίβο>` και **δεν** περιέχει μέγεθος: αν το
 * περιείχε, δύο ταυτόσημα σχήματα θα περνούσαν την πύλη επειδή το ένα είναι μεγαλύτερο — και
 * το μέγεθος **δεν είναι** διακριτικό κανάλι (ίδιος λόγος με το `markSizePx` παραπάνω).
 *
 * ## Γιατί το τετράγωνο μπαίνει ΜΕΣΑ στον κύκλο χωρίς να τον αγγίζει
 * Η διαγώνιος του τετραγώνου (`squareHalfPx`·√2 ≈ 8,5) είναι **μικρότερη** από την ακτίνα
 * (`circleRadiusPx` = 12) ⇒ στην τέλεια προσαρμογή βλέπεις «τετράγωνο μέσα σε κύκλο», δύο
 * σημάδια ευκρινώς, **μηδέν κοινά σημεία**. Δεν είναι αισθητική: ένα σημείο επαφής θα ήταν
 * ακριβώς η κατάσταση όπου ο χρήστης δεν ξέρει αν βλέπει ένα σχήμα ή δύο.
 *
 * ## Γιατί ΜΑΤΖΕΝΤΑ
 * 1. Είναι **πρωτογενές** της παλέτας (`UI_COLORS_BASE.MAGENTA`), όχι σημασιολογικό: δεν
 *    δανείζεται τη σημασία κανενός `SUCCESS`/`WARNING`/`ERROR`, άρα το σημάδι **δεν μπορεί**
 *    να διαβαστεί ως σφάλμα ή προειδοποίηση — δεν είναι ούτε το ένα ούτε το άλλο.
 * 2. Είναι η **μόνη** απόχρωση που δεν εμφανίζεται σε χαρτογραφικό ράστερ (μπεζ · πράσινο ·
 *    μπλε · γκρι · λευκό) — η ίδια σύμβαση της φωτογραμμετρίας για σημεία ελέγχου.
 * 3. Η σύγκρουση με το `GRIP_WARM_COLOR` (ίδιο hex, hover λαβής) είναι **δομικά αδύνατη στην
 *    οθόνη**: όσο τρέχει η συνεδρία τοποθέτησης, η αδιαφανής στον δείκτη επιφάνεια κατέχει
 *    **κάθε** γεγονός δείκτη ⇒ καμία λαβή δεν μπορεί να είναι σε hover ταυτόχρονα.
 *
 * ⚠️ Η **αναγνωσιμότητα** δεν λύνεται εδώ και **δεν λύνεται με απόχρωση**: το σημάδι κάθεται
 * πάνω σε ράστερ που δεν ελέγχουμε. Λύνεται με **casing** στον ζωγράφο — δες
 * `basemap-correspondence-marks.ts`, που παίρνει το χρώμα του από το ίδιο SSoT (`maxContrastInk`)
 * με το `bim-contrast-casing.ts` (ADR-771 Φ.3).
 *
 * 🔑 **Ο ζωγράφος διαβάζει ΑΥΤΑ τα πεδία και η πύλη διαβάζει ΤΑ ΙΔΙΑ** — δες
 * `scripts/lib/contrast/state-marks.js`. Ένα ξεχωριστό «μεταδεδομένο καναλιού» θα μπορούσε να
 * λέει «κύκλος» ενώ το DOM βάφει τετράγωνο, και η πύλη θα έμενε πράσινη.
 *
 * @see docs/centralized-systems/reference/adrs/ADR-782-basemap-under-canvas.md §27.4
 */
export const BASEMAP_CORRESPONDENCE_MARKS = {
  /**
   * Ταυτόσημο για **και τις τρεις** καταστάσεις — δες την κεφαλίδα. Είναι **ένα** πεδίο και
   * όχι τρία επίτηδες: τρία πεδία με την ίδια τιμή είναι πρόσκληση να αποκλίνουν.
   */
  hex: UI_COLORS.MAGENTA,
  /** Μισή πλευρά του τετραγώνου «σημείο σχεδίου», σε px οθόνης. */
  squareHalfPx: 6,
  /** Ακτίνα του κύκλου «σημείο χάρτη», σε px οθόνης — δες γιατί δεν αγγίζονται. */
  circleRadiusPx: 12,
  /** Πάχος πένας των σημαδιών, σε px οθόνης. */
  strokeWidthPx: 2,
  /** Το μοτίβο του **εκκρεμούς** σημείου, σε px οθόνης — «δείχτηκε, δεν έκλεισε». */
  pendingDashPx: [4, 3] as readonly number[],
  /**
   * Οι τρεις καταστάσεις, ως **δεδομένα**. `shape` + `dash` είναι τα δύο ανεξάρτητα κανάλια
   * και μαζί φτιάχνουν το `variant` που κρίνει το Κ1 της πύλης.
   */
  states: {
    /** Το σημείο σχεδίου που δείχτηκε και περιμένει το ταίρι του πάνω στον χάρτη. */
    drawingPending: { shape: 'square', dash: 'dashed' },
    /** Το σημείο σχεδίου μιας **ολοκληρωμένης** αντιστοιχίας. */
    drawingSettled: { shape: 'square', dash: 'solid' },
    /** Το σημείο **της Γης** της ίδιας αντιστοιχίας, προβαλλόμενο στο χαρτί. */
