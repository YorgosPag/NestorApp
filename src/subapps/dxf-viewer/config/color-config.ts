...
export const TABLE_COPY_MARQUEE = {
  colorHex: UI_COLORS.INDICATOR_BLUE_LIGHT,
  /**
   * 🔴 ADR-739 §48 — **1 px, ΚΑΙ ΕΙΝΑΙ ΡΗΤΗ ΠΑΡΕΚΚΛΙΣΗ ΑΠΟ ΤΟΝ ΚΑΝΟΝΑ ΤΩΝ 2 px.**
   *
   * Ήταν `TABLE_CELL_CURSOR.lineWidthPx` (= 2) μέχρι τις 2026-08-05, με σχόλιο που παρέπεμπε
   * στο WCAG σκεπτικό του {@link TABLE_MODE_OUTLINE}. Η παραπομπή ήταν **λανθασμένη επέκταση**:
   *
   * ── Γιατί το SC 2.4.13 δεν δεσμεύει ΑΥΤΟΝ τον δείκτη ──
   * Το WCAG 2.2 SC 2.4.13 λέγεται *Focus Appearance* και δεσμεύει τον δείκτη **εστίασης
   * πληκτρολογίου** — «πού θα πάει το επόμενο πλήκτρο». Εδώ αυτός ο δείκτης είναι ο
   * {@link TABLE_CELL_CURSOR}, που μένει **συμπαγής στα 2 px** και δεν αγγίζεται. Το marquee
   * απαντά σε **άλλη** ερώτηση — «τι υπάρχει στο πρόχειρο» — είναι **προσωρινό**, σβήνει με
   * `Escape` ή με το πρώτο γράψιμο, και **δεν** μετακινείται με το πληκτρολόγιο. Ένας κανόνας
   * για δείκτες εστίασης δεν γίνεται κανόνας για κάθε γραμμή της εφαρμογής επειδή έτυχε να
   * γραφτεί δίπλα της.
   *
   * ── Γιατί ΔΕΝ ήταν επιλογή να κρατηθούν τα 2 px ──
   * Το πάχος και το μοτίβο **είναι δεμένα**: το μετρημένο κενό του Excel είναι 2 px (δες
   * `MARCHING_ANTS_DASH_PX`). Κενό 2 px πάνω σε γραμμή πάχους 2 px δίνει τετράγωνες εγκοπές
   * που το μάτι διαβάζει ως **συμπαγή** γραμμή — δηλαδή τα 2 px θα κατέστρεφαν ακριβώς αυτό
   * που ζητήθηκε (ότι φαίνεται διακεκομμένο σαν του Excel) και θα έκαναν το marquee αδιάκριτο
   * από τον συμπαγή δρομέα. Η εναλλακτική «ίδιες αναλογίες στο διπλάσιο» (`[14,4]` @2px)
   * παρουσιάστηκε στον ιδιοκτήτη με τα νούμερα και **απορρίφθηκε** υπέρ της πιστότητας.
   *
   * ── Τι ΔΕΝ χαλαρώνει ──
   * Το SC **1.4.11** (αντίθεση μη-κειμένου, 3:1) δεν εξαρτάται από το πάχος και εξακολουθεί να
   * ισχύει — το `#7FD0FF` πάνω στο σκούρο φόντο του viewer το περνά άνετα. Και το 1 px δεν
   * είναι πρωτοφανές λεξιλόγιο εδώ: {@link TABLE_FILL_HANDLE}`.previewWidthPx` και
   * {@link TABLE_INSERT_AFFORDANCE}`.lineWidthPx` είναι ήδη 1 px, για τον ίδιο λόγο —
   * παροδικές ενδείξεις, όχι μόνιμοι δείκτες.
   */
  lineWidthPx: 1,
} as const;
