...
import { channelToHex, hexToRgba } from './color-math';
...
export const TABLE_CELL_SELECTION = {
  /**
   * Γέμισμα των επιλεγμένων κελιών — **παράγωγο** του χρώματος δρομέα μέσω του `hexToRgba`
   * SSoT (ADR-571), ποτέ γραμμένο ως δεύτερο κυριολεκτικό `rgba(...)`: αλλιώς μια αλλαγή
   * του `INDICATOR_BLUE` θα άφηνε το γέμισμα στο παλιό μπλε, σιωπηλά.
   */
  fillRgba: hexToRgba(UI_COLORS.INDICATOR_BLUE, 0.18),
  /** Περίγραμμα ολόκληρης της περιοχής — συμπαγές, ίδιο χρώμα με τον δρομέα. */
  outlineHex: UI_COLORS.INDICATOR_BLUE,
  /** Πάχος περιγράμματος περιοχής, σε px οθόνης. */
  outlineWidthPx: 1.5,
} as const;
...
export const TABLE_INDICATOR = {
  ...
  /** Ύψος της πάνω ζώνης (γράμματα), σε px οθόνης. */
  columnBandPx: 18,
  /** Πλάτος της αριστερής ζώνης (αριθμοί), σε px οθόνης — πλατύτερη: χωρά τετραψήφια. */
  rowBandPx: 28,
