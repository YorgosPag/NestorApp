...
/**
 * Γενικά placeholders επιλογής (ADR-804 §4).
 *
 * 🔶 **ΔΗΛΩΜΕΝΟ ΧΡΕΟΣ i18n**: οι τιμές είναι **σκληρά ελληνικά**, όχι κλειδιά — σε
 * αντίθεση με το υπόλοιπο αρχείο. Μετακινήθηκαν **ως έχουν** από το `modal-select.ts`
 * ώστε να πάψει να υπάρχει δεύτερο σπίτι· η **μετάφρασή** τους είναι ξεχωριστή δουλειά
 * και ΔΕΝ έγινε εδώ, γιατί δεν υπάρχει πλήρες αντίστοιχο σύνολο κλειδιών (μετρημένο:
 * `placeholders.select` δεν υπάρχει σε κανένα από τα 103 namespaces).
