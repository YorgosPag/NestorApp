...
/**
 * ADR-739 Φ.Ε/Φ4 — **θα επιβιώσει αυτό το χρώμα ως μελάνι σε λευκό χαρτί;**
 *
 * `false` για ό,τι το {@link applyPlotColor} θα ισοπέδωνε σε μαύρο στον ρόλο `'ink'`: σχεδόν
 * λευκά, και το ACI 7 (η λευκή/μαύρη πένα). Ένας επιλογέας **χρώματος κειμένου** το χρειάζεται
 * *πριν* την εκτύπωση — για να μην προσφέρει καν δείγμα που θα κατέληγε αλλού απ' ό,τι δείχνει.
 *
 * 🔴 Εκτίθεται **αντί** να αντιγραφεί το κατώφλι: ένα δεύτερο `0.92` σε άλλο αρχείο θα ήταν δύο
 * πηγές για την ίδια απόφαση, που αποκλίνουν στην πρώτη ρύθμιση. Ο επιλογέας και ο εκτυπωτής
 * **οφείλουν** να απαντούν το ίδιο πράγμα, αλλιώς η παλέτα λέει ψέματα.
 *
 * @param colorHex χρώμα οθόνης· άκυρο/άγνωστο ⇒ `false` (δεν υπάρχει χρώμα να κρατηθεί).
 * @param colorAci δείκτης ACI όταν είναι γνωστός, αλλιώς `null`.
 */
export function survivesAsInk(colorHex: string, colorAci: number | null = null): boolean {
  if (colorAci === ACI_WHITE) return false;
  const rgb = parseHex(colorHex);
  return rgb !== null && !isNearWhite(rgb);
}
