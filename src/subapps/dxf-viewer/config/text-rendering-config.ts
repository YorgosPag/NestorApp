...
/**
 * 🔴 ADR-739 Φ.Ε/Φ2 βήμα 4 — **η γωνία κλίσης που ΕΠΙΒΑΛΛΟΥΜΕ όταν δεν υπάρχει πλάγια όψη.**
 *
 * Δύο καταναλωτές, ένας αριθμός:
 * - **DXF, γραμματοσειρά SHX**: τα σχήματα SHX δεν έχουν πλάγια παραλλαγή — το AutoCAD τα
 *   γέρνει γεωμετρικά μέσω του `obliqueAngle` του STYLE (group 50). (Οι TrueType **δεν**
 *   περνούν από εδώ: εκεί δηλώνεται η σημαία italic στο XDATA 1071 και το AutoCAD φορτώνει
 *   την **πραγματική** πλάγια όψη — καλύτερο αποτέλεσμα από κάθε γεωμετρική κλίση.)
 * - **PDF**: ο `greek-font-loader` δηλώνει το ίδιο `Roboto-Regular.ttf` για κάθε ύφος, οπότε
 *   ένα `setFont(…, 'italic')` δεν γέρνει τίποτα — ίδιο δομικό αδιέξοδο με το έντονο
 *   (ADR-739 §28.9.4). Η κλίση γίνεται με **συνθετική διάτμηση** του text matrix.
 *
 * **15°** επειδή αυτό ορίζει το **ISO 3098** για την κεκλιμένη γραφή τεχνικού σχεδίου — το
 * ίδιο πρότυπο απ' όπου έρχεται και το ελάχιστο ύψος 2,5mm αυτού του αρχείου. Είναι επίσης η
 * γωνία που χρησιμοποιεί το ίδιο το AutoCAD για τα «italic» text styles του.
 */
export const TEXT_OBLIQUE_ITALIC_DEG = 15;
