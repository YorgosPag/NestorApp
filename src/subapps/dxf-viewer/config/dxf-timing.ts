...
export const DXF_TIMING = {
  ...
  animation: {
    ...
    MESH_REVEAL: 260,
    /** «Μυρμήγκια» αντιγραφής πίνακα — ρυθμός αίτησης καρέ (~12Hz, ADR-739 §48).
     *  Το ζητούμενο είναι **κίνηση**, όχι ρευστότητα: με 20 px/s το βήμα είναι ~1,7 px,
     *  ήδη κάτω από το κατώφλι όπου το μάτι βλέπει «σκαλοπάτια». Στα 60Hz θα ζητούσαμε
     *  πενταπλάσια καρέ για ταυτόσημη εικόνα — ο ADR-040 δεν απαγορεύει τα καρέ,
     *  απαγορεύει το να ζητούνται χωρίς λόγο. */
    MARCHING_ANTS_TICK: 80,
  },
  ...
  lifecycle: {
    ...
    /** ADR-833 Φ5Β — χρονικός προϋπολογισμός **μιας** κατασκευής διάταξης πίνακα.
     *  Είναι **διαιρετέος**, όχι καθυστέρηση: το `MAX_TABLE_GRID_CELLS` **παράγεται** από αυτό
     *  δια του μετρημένου κόστους ανά πυκνό κελί — άρα ζει στην κατηγορία του **quota**.
     *  Το «όριο ροής της σκέψης» (~1s), **όχι** το «στιγμιαίο» (~100ms): η διάταξη
     *  ξαναχτίζεται ανά **δεσμευμένη** αλλαγή (Enter/Tab/επικόλληση), όχι ανά πλήκτρο. */
    TABLE_LAYOUT_BUDGET: 1000,
  },
