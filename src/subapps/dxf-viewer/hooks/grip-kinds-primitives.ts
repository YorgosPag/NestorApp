 ...
 *   - `table-row-edge`    → το **κάτοπτρό** της: μία λαβή ανά εσωτερικό όριο γραμμών, πάνω
 *                           στην ΑΡΙΣΤΕΡΗ ακμή· δίνει ρητό `heightMm` στη γραμμή ΠΑΝΩ από το
 *                           όριο. Ίδιο χρώμα με την προηγούμενη — ίδια **εμβέλεια**.
 *
 * ⚠️ **Φράγμα πλήθους, όχι απαγόρευση άξονα** (Giorgio 2026-08-04). Μέχρι τότε αυτό το σχόλιο
 * απαγόρευε λαβές ανά γραμμή, με σωστό επιχείρημα: το πλήθος λαβών δεν επιτρέπεται να είναι
 * ανάλογο των δεδομένων (500 γραμμές ⇒ 500 λαβές ζωγραφισμένες + hit-tested ανά καρέ — το
 * σχήμα που πλήρωσε ο ADR-735). Λάθος ήταν το **συμπέρασμα**: το επιχείρημα αφορά το πλήθος,
 * και ίσχυε ήδη για τις στήλες (`MAX_TABLE_COLUMN_COUNT` = **256**, όχι «μονοψήφιες»). Πλέον
 * υπάρχει ένα `MAX_AXIS_EDGE_GRIPS` που ισχύει **και στους δύο** άξονες, και πάνω του οι
 * λαβές του άξονα παραλείπονται συνολικά. Το ύψος **όλων** των γραμμών κλιμακώνεται πάντα
 * αναλογικά από τις μεσοπλευρικές/γωνιακές παρακάτω — **8 σταθερές** λαβές, εκτός φράγματος.
 *
 * ## Το λεξιλόγιο κουτιού (Giorgio 2026-08-03 — «οι λαβές όπως σου τις δείχνω»)
 * Ο πίνακας απέκτησε τις ΙΔΙΕΣ 8 περιμετρικές λαβές με την εικόνα (ADR-654) και το block
 * (ADR-641), μέσω του **κοινού** `rect-frame` + `rect-grip-engine` (μηδέν νέα γεωμετρία, N.18):
 *   - `table-corner-*` → 4 γωνιακές: 2-DOF resize με την **ΑΝΤΙΘΕΤΗ γωνία σταθερή**. Ο λόγος
 *                        πλευρών είναι **ΕΛΕΥΘΕΡΟΣ** και το **Shift τον κλειδώνει** — η
 *                        **αντίστροφη** πολικότητα από την εικόνα, κατ' εντολή Giorgio: ο πίνακας
 *                        είναι Excel/AutoCAD, όχι φωτογραφία που παραμορφώνεται.
 *   - `table-edge-*`   → 4 μεσοπλευρικές: 1-άξονα stretch με την **ΑΝΤΙΘΕΤΗ ακμή σταθερή**.
 * Στο πλαίσιο του πίνακα (+u δεξιά, +v **κάτω**, άγκυρα πάνω-αριστερά) τα local πρόσημα του
 * `RectFrame` δίνουν: **ne = πάνω-δεξιά, nw = πάνω-αριστερά (η ΑΓΚΥΡΑ), sw = κάτω-αριστερά,
 * se = κάτω-δεξιά**· `table-edge-n` = η **πάνω** ακμή.
 *
 * ⚠️ Το `table-move` **δεν κάθεται πια στην άγκυρα** και το `table-rotation` **δεν κάθεται πια
 * στο μέσο της πάνω ακμής**: εκεί κάθονται πλέον οι `table-corner-nw` και `table-edge-n`. Και τα
 * δύο μετακόμισαν σε **ξεχωριστή λωρίδα** λίγο έξω από την πάνω ακμή — βλ. `table-entity-grips.ts`
 * για το γιατί το standoff **δεν** είναι καλλωπισμός.
 */
...
/**
 * Το υπο-σύνολο των kinds που περιγράφουν **κουτί** (γωνία ή ακμή) — αυτά που δρομολογούνται
 * στον κοινό `rect-grip-engine` μέσω του `TABLE_BOX_TARGETS`. Δηλωμένο **εδώ**, δίπλα στον γονικό
 * union, ώστε ο compiler να επιβάλλει ότι κάθε νέο γωνιακό/μεσοπλευρικό kind αποκτά **και**
 * αντιστοιχία στόχου: `Record<TableBoxGripKind, …>` = εξαντλητικός πίνακας, όχι ευχή.
 */
export type TableBoxGripKind = Extract<TableGripKind, `table-corner-${string}` | `table-edge-${string}`>;
