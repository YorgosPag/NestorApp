/**
 * @module rotation-direction-arc.test
 * @description ADR-397 §15 — τόξο φοράς περιστροφής: πρόσημο→χρώμα (🟢/🔴) + screen-space γεωμετρία
 * (φορά Y-flip safe, αιχμή στον κέρσορα, ακτίνα/clamp). Pure unit tests — μηδέν canvas/DOM.
 */

import {
  resolveRotationArcColor,
  resolveRotationDirectionArc,
  ROTATION_ARC_MIN_SWEEP_DEG,
} from '../rotation-direction-arc';

const GREEN = '#2e9e44';
const RED = '#d23b3b';

describe('resolveRotationArcColor — πρόσημο sweep → χρώμα (SSoT ghost-status)', () => {
  it('θετικό sweep (CCW) → πράσινο', () => {
    expect(resolveRotationArcColor(30)).toBe(GREEN);
    expect(resolveRotationArcColor(0.5)).toBe(GREEN);
  });

  it('αρνητικό sweep (CW) → κόκκινο', () => {
    expect(resolveRotationArcColor(-30)).toBe(RED);
    expect(resolveRotationArcColor(-0.5)).toBe(RED);
  });

  it('μηδέν → πράσινο (boundary ≥ 0)', () => {
    expect(resolveRotationArcColor(0)).toBe(GREEN);
  });
});

describe('resolveRotationDirectionArc — γεωμετρία screen-space', () => {
  ...
  it('αμελητέο sweep → null', () => {
    ...
    expect(
      resolveRotationDirectionArc(pivot, anchorRight, cursor, ROTATION_ARC_MIN_SWEEP_DEG / 2),
    ).toBeNull();
  });

  it('NaN sweep → null', () => {
    expect(resolveRotationDirectionArc(pivot, anchorRight, { x: 100, y: 0 }, NaN)).toBeNull();
  });
  ...
  it('κέρσορας ΠΑΝΩ (screen up), world sweep θετικό → sign +1, αιχμή στον κέρσορα, ακτίνα = απόσταση', () => {
    ...
    const arc = resolveRotationDirectionArc(pivot, anchorRight, cursor, 90);
  ...
  it('baseline 0° πάντα σε ακτινική απόσταση = radius κατά τον άξονα αναφοράς', () => {
    ...
    const arc = resolveRotationDirectionArc(pivot, anchorDiag, { x: 60, y: 180 }, -40);
    expect(arc).not.toBeNull();
    if (!arc) return;
  ...
  it('ακτίνα clamp σε ελάχιστο όταν ο κέρσορας είναι κοντά στο pivot', () => {
    ...
    const arcNear = resolveRotationDirectionArc(pivot, anchorRight, cursor, 45);
  ...
  it('αιχμή πάντα σε ακτινική απόσταση = radius από το pivot', () => {
    ...
    const arc = resolveRotationDirectionArc(pivot, anchorRight, cursor, -60);
    expect(arc).not.toBeNull();
    if (!arc) return;
