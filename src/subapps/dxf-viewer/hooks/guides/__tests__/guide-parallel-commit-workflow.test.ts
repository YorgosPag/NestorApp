...
// ADR-677 — CanvasNumericInputStore ερμηνεύει πλέον τον πληκτρολογημένο buffer στη
// μονάδα εμφάνισης (display → mm → scene) πριν φτάσει εδώ. Χωρίς ρητό setUnit('mm')
// θα έπεφτε στο DEFAULT_DISPLAY_UNIT ('m') και κάθε πληκτρολογημένη τιμή σε αυτό το
// αρχείο θα πολλαπλασιαζόταν ×1000 πριν συγκριθεί με τα expected (25, -9 κλπ).
import { displayUnitState } from '../../../config/display-unit-state';
// Sole writer σε production: useDxfSceneConversion. Εδώ κλειδώνουμε 1 (1 scene unit =
// 1 mm) ώστε η μετατροπή display→scene να περνά ΜΟΝΟ από fromDisplay — επαληθεύτηκε
// πως το module-level default είναι ήδη 1 σε αυτό το αρχείο (κανένα άλλο test το
// αλλάζει), αλλά το δηλώνουμε ρητά για να μην είναι σιωπηρή προϋπόθεση.
...
const ORIGINAL_DISPLAY_UNIT = displayUnitState.getUnit();

beforeEach(() => {
  CanvasNumericInputStore.cancel();
  cadToggleState.set(false, false);
  cadToggleState.setSnap(false, 0);
  // ADR-677: μονάδα εμφάνισης = 'mm' ⇒ fromDisplay(τιμή, 'mm') = τιμή (ταυτοτική
  // μετατροπή) — έτσι οι πληκτρολογημένες τιμές αυτού του αρχείου (25, 9, …)
  // συνεχίζουν να αντιστοιχούν 1:1 στα scene units που ελέγχουν τα assertions.
  displayUnitState.setUnit('mm');
  immediateSceneScale.set(1);
});

afterEach(() => {
  displayUnitState.setUnit(ORIGINAL_DISPLAY_UNIT);
});
...
describe('handleParallelDistanceCommitted — η ΠΛΗΚΤΡΟΛΟΓΗΜΕΝΗ τιμή νικά το κλικ', () => {
  it('buffer με τιμή → μέγεθος από την πληκτρολόγηση, πλευρά από το κλικ (μονάδα mm — ταυτοτική)', () => {
    const h = mount([GUIDE_X]);
    selectRef(h, 'guide-x-1', { x: 100, y: 250 });
    act(() => {
  ...
  it('η πληκτρολόγηση δίνει ΜΟΝΟ μέγεθος — η πλευρά μένει του κέρσορα (μονάδα mm — ταυτοτική)', () => {
    const h = mount([GUIDE_X]);
    selectRef(h, 'guide-x-1', { x: 100, y: 250 });
  ...
  it('ADR-677: μονάδα εμφάνισης "m" → η πληκτρολόγηση "25" γίνεται 25000 (mm) πριν κερδίσει τον κέρσορα', () => {
    ...
    // Το φάντασμα και το commit ΠΡΕΠΕΙ να συμφωνούν — ήταν το επαναλαμβανόμενο bug
    // αυτής της ροής (WYSIWYG): ελέγχουμε ρητά ότι το ghost value == το committed value.
    const ghostValue = CanvasNumericInputStore.getPendingDistance();
    commitAt(h, 'guide-x-1', { x: 160, y: 250 });

    expect(ghostValue).toBe(25000);
    expect(h.addParallelGuide.mock.calls[0][1]).toBeCloseTo(25000, 6);
  });
});
