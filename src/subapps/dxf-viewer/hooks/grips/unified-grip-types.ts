...
export interface UnifiedGripInfo {
   ...
   * per-entity grip discriminator (forwarded from `GripInfo.gripKind` in
   * `grip-registry.wrapDxfGrip`): the 31 legacy `xxxGripKind?` optionals were
   * removed (Wave 2). Read via `gripKindOf(grip, '<entity.type>')`.
   */
