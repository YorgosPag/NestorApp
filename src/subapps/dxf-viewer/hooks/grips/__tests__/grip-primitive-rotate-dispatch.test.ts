...
describe('ADR-561 — primitive move/rotation dispatch', () => {
  ...
  it('arc-rotation → commitArcGripDrag', () => {
    commitDxfGripDragModeAware(grip({ type: 'vertex', arcGripKind: 'arc-rotation', movesEntity: false }), DELTA, makeDeps(), 'stretch');
  ...
  it('polyline-rotation → commitPolylineRotationGripDrag', () => {
    commitDxfGripDragModeAware(grip({ type: 'vertex', polylineGripKind: 'polyline-rotation', movesEntity: false }), DELTA, makeDeps(), 'stretch');
  ...
  it('arc-move → neither rotation handler (whole-entity translate falls through)', () => {
    commitDxfGripDragModeAware(grip({ arcGripKind: 'arc-move' }), DELTA, makeDeps(), 'stretch');
    expect(commitArcGripDrag).not.toHaveBeenCalled();
    expect(commitPolylineRotationGripDrag).not.toHaveBeenCalled();
  });

  it('circle-move → neither rotation handler', () => {
    commitDxfGripDragModeAware(grip({ circleGripKind: 'circle-move' }), DELTA, makeDeps(), 'stretch');
    expect(commitArcGripDrag).not.toHaveBeenCalled();
    expect(commitPolylineRotationGripDrag).not.toHaveBeenCalled();
  });

  it('polyline-move → neither rotation handler', () => {
    commitDxfGripDragModeAware(grip({ polylineGripKind: 'polyline-move' }), DELTA, makeDeps(), 'stretch');
    expect(commitArcGripDrag).not.toHaveBeenCalled();
    expect(commitPolylineRotationGripDrag).not.toHaveBeenCalled();
  });
});
