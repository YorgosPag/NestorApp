/**
 * ADR-412 Φ5 / ADR-604 — `UpdateFamilyTypeCommand<TP>` (generic) tests.
 *
 * The generic replaces the former per-entity `UpdateWallFamilyTypeCommand` (and
 * its slab/roof twins). Exercised here with a concrete `WallTypeParams` payload:
 * execute replaces the type's params in the (injected) store + runs
 * persist/audit/notify; undo restores the previous params + re-runs side effects
 * with reversed diff; idempotent re-apply; the command NEVER mutates scene
 * entities directly (instances re-resolve via the store-version mechanism, not
 * this command); validate guards the type id.
 *
 * NOTE: the retired `UpdateWallFamilyTypeCommand` carried an extra
 * `if (next.thickness <= 0)` validate branch. It was DEAD at runtime —
 * `CommandHistory.execute()` never calls `validate()` — so the generic drops it
 * (domain validation, if ever wanted, belongs at the edit-dialog input boundary,
 * not in an undoable command replaying already-valid history).
 */
...
interface Harness {
  ...
  deps: FamilyTypeMutationDeps<WallTypeParams>;
...
function makeHarness(initialThickness = 100): Harness {
  const h: Harness = {
    ...
    audits: [],
    notifyCount: 0,
    deps: {} as FamilyTypeMutationDeps<WallTypeParams>,
  };
  h.deps = {
    getTypes: () => h.catalog,
...
describe('UpdateFamilyTypeCommand<TP> (ADR-604 generic, wall payload)', () => {
  it('1. execute: optimistic store replace + persist + audit + notify', () => {
    const h = makeHarness();
    const cmd = new UpdateFamilyTypeCommand<WallTypeParams>(TYPE_ID, NEXT, PREV, h.deps);
    cmd.execute();
  ...
  it('5. leaves OTHER types untouched (replaces only the target id)', () => {
    ...
    new UpdateFamilyTypeCommand<WallTypeParams>(TYPE_ID, NEXT, PREV, h.deps).execute();
    expect((h.catalog.find((t) => t.id === 'bimftype-wall-2') as BimFamilyType<'wall'> | undefined)?.typeParams.thickness).toBe(300);
  });

  it('6. validate rejects empty id, accepts a valid id', () => {
    const h = makeHarness();
    expect(new UpdateFamilyTypeCommand<WallTypeParams>('', NEXT, PREV, h.deps).validate()).toMatch(/ID/);
    expect(new UpdateFamilyTypeCommand<WallTypeParams>(TYPE_ID, NEXT, PREV, h.deps).validate()).toBeNull();
  });
  ...
  it('7. serialize round-trips + no scene entities affected', () => {
    ...
    expect(s.type).toBe('update-family-type');
