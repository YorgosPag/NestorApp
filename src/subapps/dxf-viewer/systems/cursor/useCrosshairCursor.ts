...
function tableIndicatorCursorValue(role: TableIndicatorCursorRole): string {
  switch (role) {
    ...
    case 'delete-control':
      return 'pointer';
    // 🔴 ADR-833 Φάση 3 — **η καρτέλα φύλλου**. Ο ίδιος δείκτης με τα ⊕/⊖ και για τον ίδιο
    // λόγο: ένα πάτημα, μία πράξη, καμία σύρση. Η λέξη-κλειδί που κάθε χρήστης έχει ήδη μάθει
    // από τον ιστό — και από την ίδια τη λωρίδα καρτελών κάθε φύλλου υπολογισμού.
    //
    // ⚠️ Καμία φάση να ελεγχθεί εδώ, σε αντίθεση με τα ⊕/⊖: η καρτέλα **δεν έχει** φάση
    // `nearby` — το ορθογώνιό της είναι ακριβώς ο στόχος της (δες `TablePointerHit`).
    case 'worksheet-tab':
      return 'pointer';
    ...
    // 🔴 ADR-828 Φ4α — **το κουμπί «Επιλογές Αυτόματης Συμπλήρωσης»**, γείτονας της λαβής και
    // αντίθετός της σε είδος: εκείνη είναι χερούλι, αυτό είναι **κουμπί** — ένα πάτημα, ένα
    // μενού, καμία σύρση. `pointer`, όπως τα τρία άλλα κουμπιά του πίνακα (⊕, ⊖, γωνία «όλα»).
    //
    // ⚠️ Εδώ ο κανόνας «native όπου υπάρχει» ισχύει **ακέραιος**, σε αντίθεση με τη γραμμή από
    // πάνω: η λέξη-κλειδί δεν είναι πιασμένη από τίποτα, και αποδίδει ακριβώς την πράξη. Ένα
    // δεύτερο ράστερ εδώ θα ήταν εικόνα για μηδέν επιπλέον πληροφορία — και θα έκανε τα δύο
    // χειριστήρια της γωνίας να μοιάζουν, εκεί ακριβώς που πρέπει να ξεχωρίζουν.
    case 'fill-badge':
      return 'pointer';
