/**
 * 🔴 ADR-739 §29.9 — **ΤΟ ΛΑΣΟ ΚΑΙ ΤΑ ΣΗΜΑΔΙΑ ΕΛΞΗΣ ΣΩΠΑΙΝΟΥΝ ΟΣΟ Ο ΠΙΝΑΚΑΣ ΕΙΝΑΙ ΑΝΟΙΧΤΟΣ.**
 *
...
import {
  shouldArmSelectDrag,
  shouldCanvasActOnRelease,
  shouldDetectSnap,
} from '../select-gesture-gates';
...
describe('🔴 ADR-739 §29.9 — η σύρση επιλογής δεν armάρει όσο ζει η λειτουργία πίνακα', () => {
...
describe('🔴 ADR-739 §29.9 — τα σημάδια έλξης σβήνουν όσο ζει η λειτουργία πίνακα', () => {
...
/**
 * 🔴 ADR-739 §29.11 — **ΤΡΙΤΗ ΚΑΤΗΓΟΡΙΑ: η ΑΠΕΛΕΥΘΕΡΩΣΗ.**
 *
 * Δύο ελαττώματα του ιδιοκτήτη (2026-08-03, στιγμιότυπα `000155` / `000430`) που είναι **το
 * ίδιο** ελάττωμα, στο **ίδιο** μπλοκ του `mouse-handler-up`:
 *
 *  - **Δ1** κλικ στη **ζώνη δείκτη** ⇒ καμία οντότητα από κάτω ⇒ `cursor.startSelection()`
 *    (πλαίσιο δύο κλικ, AutoCAD) ⇒ πράσινο διακεκομμένο *crossing* πάνω από τον καμβά, ενώ ο
 *    πίνακας είχε ήδη επιλέξει σωστά τρεις στήλες·
 *  - **Δ2** κλικ **μέσα σε κελί** πάνω από την οντότητα κειμένου «ΤΕΣΤ» ⇒ `onEntitySelect` ⇒
 *    λαβές της οντότητας **και η κορδέλα γύρισε σε «Επεξεργαστής Κειμένου»**.
 *
 * ## 🔑 Γιατί το §29.9 ΔΕΝ τα έπιασε — και πάλι δεν είναι κενό του
 * Το §29.9 έκλεισε το **arm** της σύρσης (`lassoDownRef`, κουμπί κάτω). Το πλαίσιο δύο κλικ
 * είναι **τρίτος** μηχανισμός: γεννιέται στο `mouseup`, από `cursor.startSelection()`, **χωρίς
 * κρατημένο κουμπί** — δεν περνά ποτέ από το `lassoDownRef`. Και η επιλογή οντότητας του Δ2
 * ζει στο **ίδιο** μπλοκ, τρεις γραμμές πιο πάνω. Μία πύλη, δύο ελαττώματα.
 */
const UNLOCKED_RELEASE = { lockedByTableSession: false } as const;

describe('🔴 ADR-739 §29.11 — ο καμβάς δεν ερμηνεύει την απελευθέρωση όσο ζει η λειτουργία πίνακα', () => {
  it('βάση: χωρίς κλείδωμα ο καμβάς ερμηνεύει κανονικά την απελευθέρωση', () => {
    // Χωρίς αυτό, ένα «return false» παντού θα ήταν πράσινο στα υπόλοιπα.
    expect(shouldCanvasActOnRelease(UNLOCKED_RELEASE)).toBe(true);
  });

  it('🔴 Δ1+Δ2 — ΚΛΕΙΔΩΜΕΝΟΣ ΚΑΜΒΑΣ ⇒ ούτε πλαίσιο δύο κλικ, ούτε επιλογή από κάτω', () => {
    expect(shouldCanvasActOnRelease({ lockedByTableSession: true })).toBe(false);
  });
});
