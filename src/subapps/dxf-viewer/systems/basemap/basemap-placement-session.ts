 ...
 *
 * 🔴 **`set`, ΠΟΤΕ `reset`** (ADR-782 §26). Το `reset` του `createExternalStore` είναι **teardown
 * δοκιμών**: αλλάζει την τιμή **χωρίς ειδοποίηση** *και* κάνει `listeners.clear()`. Εδώ αυτό
 * σήμαινε ότι το «Τέλος» (α) δεν έκρυβε το πάνελ και (β) **αποσύνδεε για πάντα** τον
 * `BasemapPlacementLeaf` και τον `BasemapPlacementPanel` από το store — άρα **κανένα** από τα
 * τέσσερα κουμπιά δεν ξαναδούλευε, και η αδιαφανής στον δείκτη επιφάνεια έμενε προσαρτημένη
 * κρατώντας τον καμβά **όμηρο μέχρι την ανανέωση σελίδας**. Ο ίδιος κανόνας είναι γραμμένος στο
 * `ToolStateStore.reset()`: *«runtime/public API reset … must notify still-attached subscribers
 * and must NOT drop them»*. Άγκυρες: `Ω1`-`Ω4`.
 */
