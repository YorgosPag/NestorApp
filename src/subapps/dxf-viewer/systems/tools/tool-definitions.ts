...
export const TOOL_DEFINITIONS: Record<ToolType, ToolInfo> = {
  ...
  // ADR-633 1b-ii: Stair Add-Turn (pick a parieta of the selected stair → angle → commit, exits on click/ESC)
  'stair-add-turn': { id: 'stair-add-turn', category: 'editing', requiresCanvas: true, canInterrupt: true, allowsContinuous: false, preservesOverlayMode: false },
  ...
  'vehicles-plan': { id: 'vehicles-plan', category: 'drawing', requiresCanvas: true, canInterrupt: true, allowsContinuous: true, preservesOverlayMode: false }, // ADR-654 M6 — single-click placement of a top-view vehicle entourage image (ImageEntity, non-BIM), shared entourage engine
  'plants-plan': { id: 'plants-plan', category: 'drawing', requiresCanvas: true, canInterrupt: true, allowsContinuous: true, preservesOverlayMode: false }, // ADR-654 M7 — single-click placement of a top-view plant entourage image (ImageEntity, non-BIM), shared entourage engine
  'attach-image': { id: 'attach-image', category: 'drawing', requiresCanvas: true, canInterrupt: true, allowsContinuous: true, preservesOverlayMode: false }, // ADR-736 §6 — single-click (or Enter = viewport centre) placement of a user-uploaded raster (ImageEntity, non-BIM), shared entourage engine; the ribbon «Εικόνα» command arms it after the upload
  'title-block': { id: 'title-block', category: 'drawing', requiresCanvas: true, canInterrupt: true, allowsContinuous: true, preservesOverlayMode: false }, // ADR-651 Φάση Β — single-click πινακίδα σχεδίου (resolved template → BLOCK, ίδιο render path με το block library)
  ...
  // ADR-656 M12 — Βέλος Βορρά (bake). Ίδιο συμβόλαιο: panel-driven, εκπέμπει entities (lwpolyline +
  // text) via completeEntities → σκόπιμη απουσία από TOOL_CREATES_ENTITY.
  'topo-north':        { id: 'topo-north',        category: 'utility', requiresCanvas: false, canInterrupt: false, allowsContinuous: false, preservesOverlayMode: false },
...
export const TOOL_CREATES_ENTITY: Partial<Record<ToolType, RenderableEntityType>> = {
  ...
  // ADR-649 / ADR-662 §12 — «Ετικέτα Εμβαδού» δημιουργεί κανονικό TEXT entity.
