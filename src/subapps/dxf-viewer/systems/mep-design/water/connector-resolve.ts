/**
 * ADR-426 — resolve a host connector's WORLD point (routing helper).
 *
 * Reuses the SSoT `connectorWorldPosition` (point-host transform) + `getEntityConnectors`
 * accessor — zero new geometry. Point hosts (fixture / manifold / boiler) carry
 * `position` + `rotation` in their params; segments use a different SSoT and are not
 * routing endpoints here, so they are intentionally not handled.
 *
