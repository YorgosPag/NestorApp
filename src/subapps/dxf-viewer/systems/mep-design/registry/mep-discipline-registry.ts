...
export const MEP_DISCIPLINE_REGISTRY: Readonly<Record<MepDisciplineId, MepDisciplineMeta>> = {
  ...
  'electrical-strong': {
    ...
    // Non-pipe discipline: it routes logical circuits, not plumbing classifications (ADR-430).
    classifications: [],
    demandStandardId: 'HD384/VA-per-point',
    sizingStandardId: 'HD384/voltage-drop',
    status: 'active',
  },
  hvac: {
    disciplineId: 'hvac',
    ...
    flowModel: 'air',
    // Non-pipe discipline: it routes duct air classifications (supply-air / return-air),
    // not plumbing ones — so `classifications` (typed to plumbing) stays empty (ADR-432).
    classifications: [],
    demandStandardId: 'constant/150cmh-per-terminal',
    sizingStandardId: 'ASHRAE/equal-friction(~0.9Pa/m)',
    status: 'active',
  },
  'electrical-weak': {
    ...
    flowModel: 'electrical',
    // Non-pipe discipline: it routes logical structured-cabling channels (ADR-431).
    classifications: [],
    demandStandardId: 'ISO11801/ports-per-outlet',
    sizingStandardId: 'ISO11801/channel-90m',
    status: 'active',
  },
  'fire-protection': {
