...
describe('detectFenceHits', () => {
  ...
  it('skips non-trimmable entity types (hatch)', () => {
    const scene = { entities: [{ id: 'h1', type: 'hatch', layerId: 'lyr_test_default' }], layers: {} } as unknown as SceneModel;
    const hits = detectFenceHits({ fenceStart: { x: 0, y: 0 }, fenceEnd: { x: 10, y: 0 }, scene, ...DEFAULT_ARGS });
    expect(hits).toHaveLength(0);
  });
  ...
  it('pick point is closest intersection to fenceStart', () => {
    ...
    const circle = { id: 'c1', type: 'circle', center: { x: 5, y: 0 }, radius: 3, layerId: 'lyr_test_default' } as CircleEntity;
...
describe('buildEntityPreviewPath', () => {
  it('LINE → 2 points [start, end]', () => {
    const line: LineEntity = { id: 'l1', type: 'line', start: { x: 0, y: 0 }, end: { x: 10, y: 0 }, layerId: 'lyr_test_default' };
  ...
  it('CIRCLE → closed loop of points', () => {
    const circle: CircleEntity = { id: 'ci1', type: 'circle', center: { x: 0, y: 0 }, radius: 3, layerId: 'lyr_test_default' };
  ...
  it('ELLIPSE → tessellated points', () => {
    const ell: EllipseEntity = { id: 'ell1', type: 'ellipse', center: { x: 0, y: 0 }, majorAxis: 4, minorAxis: 2, layerId: 'lyr_test_default' };
  ...
  it('LWPOLYLINE → vertex array', () => {
    const poly: LWPolylineEntity = {
      ...
      closed: false, layerId: 'lyr_test_default',
    };
