...
export const ESC_PRIORITY = {
  /**
   * P1050 — **Blocking confirm που διακόπτει χειρονομία ήδη σε εξέλιξη** (ADR-739 §36.22).
   *
   * Το μοναδικό σκαλί **πάνω** από το {@link MODAL_DIALOG}, και υπάρχει επειδή εκεί κάθονται
   * **δύο** ταυτόχρονοι ιδιοκτήτες του `Esc`: ο διάλογος «υπάρχουν ήδη δεδομένα εδώ» της
   * μεταφοράς περιοχής, και ο **inline editor** του ίδιου κελιού (`use-inline-editor-keys`,
   * P1000), που είναι προσαρτημένος όσο ζει η λειτουργία πίνακα. Ο bus σπάει την ισοπαλία με
   * **σειρά εγγραφής** — δηλαδή με το ποιος προσαρτήθηκε πρώτος, που εδώ είναι σύμπτωση του
   * lazy loading και όχι απόφαση.
   *
   * Η κακή έκβαση δεν είναι θεωρητική: αν κέρδιζε ο editor, ένα `Esc` θα **έκλεινε τη
   * συνεδρία πίνακα** αφήνοντας τον διάλογο ανοιχτό πάνω από πεθαμένο δρομέα — και η ερώτηση
   * που θα απέμενε είναι ακριβώς αυτή που φυλάει δεδομένα από σβήσιμο.
   *
   * ⚠️ Κάθε εγγραφή εδώ ΟΦΕΙΛΕΙ να είναι inert (`canHandle: () => state.open`) όσο ο διάλογος
   * είναι κλειστός. Ένα σκαλί πάνω από τα modal που απαντά **πάντα** θα κατανάλωνε κάθε `Esc`
  ...
  /**
   * P675 — **Συνεδρία χειροκίνητης τοποθέτησης υποβάθρου** (ADR-782 §23).
   *
   * Όσο το εργαλείο είναι ανοιχτό, ο καμβάς ανήκει σε αυτό: μια ημιδιαφανής επιφάνεια πάνω από
   * τα σχέδια δέχεται τον δείκτη, οπότε κανένα εργαλείο σχεδίασης δεν μπορεί να τρέχει
   * ταυτόχρονα. Άρα πρέπει να κερδίζει κάθε σκαλί ≤650: ένα `Esc` που ακύρωνε πρώτα ένα κόψιμο ή
   * μια εντολή τροποποίησης θα ακύρωνε κάτι που **δεν είναι σε εξέλιξη**, αφήνοντας τον χρήστη
   * μέσα σε λειτουργία που νόμιζε ότι έκλεισε.
   *
   * ⚠️ Γιατί **κάτω** από το {@link SELECTION_CYCLING} (700): εκείνο είναι εφήμερο αναδυόμενο που
   * ζει κλάσματα του δευτερολέπτου πάνω από τα πάντα. Η τοποθέτηση είναι **λειτουργία**, όχι
   * αναδυόμενο· περιμένει τη σειρά της πίσω από ό,τι μόλις άνοιξε.
   *
   * ⚠️ **ΜΙΑ** εγγραφή, τρία σκαλιά **μέσα** της (χειρονομία → μισή αντιστοιχία → έξοδος). Δες
   * `escapeBasemapPlacement` για το γιατί όχι τρεις εγγραφές.
   */
  BASEMAP_PLACEMENT: 675,
