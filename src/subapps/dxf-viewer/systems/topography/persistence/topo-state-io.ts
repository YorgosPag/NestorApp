 ...
 *
 * The persistence layer must read from and write back to SIX independent vanilla
 * stores (survey definition, contour config, contour display style, 3D prefs, cut/fill
 * question, ADR-650 §M10g baked-frame stamps). Centralising the collect/apply here keeps
 * `useTopoPersistence` focused on the Firestore lifecycle and gives the round-trip a
...
import { getBakedFrames, restoreBakedFrames } from '../topo-baked-frame-store';
...
export function applyTopoState(state: TopoPersistedState): void {
  ...
  // 3) ADR-650 §M10g — οι σφραγίδες πλαισίου. ΠΡΙΝ τον reconciler (τον καλεί ο καταναλωτής
  // αμέσως μετά το apply), αλλιώς εκείνος θα έβλεπε τις σφραγίδες της ΠΡΟΗΓΟΥΜΕΝΗΣ σκηνής
  // και θα μετακινούσε με λάθος delta.
  restoreBakedFrames(state.bakedFrames);
}
