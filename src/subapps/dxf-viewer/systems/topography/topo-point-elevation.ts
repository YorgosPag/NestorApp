...
export function surfacePointsOf(points: readonly TopoPoint[]): readonly TopoPoint3D[] {
  const elevated = points.filter(hasElevation);
  return elevated.length === points.length ? (points as readonly TopoPoint3D[]) : elevated;
}
...
export interface ElevationCoverage {
  ...
  /** Points that carry a measured Ζ. **These, and only these, define the surface.** */
...
export function describeElevationCoverage(points: readonly TopoPoint[]): ElevationCoverage {
  let withElevation = 0;
  for (const point of points) if (hasElevation(point)) withElevation++;
  return {
    total: points.length,
    withElevation,
