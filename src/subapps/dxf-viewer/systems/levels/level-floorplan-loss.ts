 ...
 *
 * **Άρα η πρόθεση ταξιδεύει ΜΕ το γεγονός**, όχι δίπλα του: event provenance, το ίδιο μοτίβο
 * με το echo-suppression των Figma/Google Docs και με το `hasPendingWrites` του Firestore.
 *
 * 🔑 **ADR-862 Φ0 Β10 — η πρόθεση έγινε ΟΝΟΜΑ ΓΕΓΟΝΟΤΟΣ.** Πρώτα ταξίδευε ως σημαία
 * (`FILE_TRASHED` + `supersededByFileId`) και αυτή η συνάρτηση έπρεπε να τη ρωτά πρώτη. Πλέον
 * η αντικατάσταση είναι `FILE_SUPERSEDED` (αρχειοθέτηση, ποτέ κάδος), που ο συνδρομητής της
 * απώλειας **δεν ακούει** — η σημαία και ο έλεγχός της αφαιρέθηκαν ως νεκρός δρόμος.
 *
 ...
 *
 * Συντηρητικό εξ ορισμού — απαντά `false` (μην καθαρίσεις) όταν το αρχείο δεν είναι ούτε το
 * αρχείο του επιπέδου, ούτε η κάτοψη του ορόφου του.
 *
 * 🔑 **Η ΑΝΤΙΚΑΤΑΣΤΑΣΗ ΔΕΝ ΦΤΑΝΕΙ ΠΟΤΕ ΕΔΩ** (ADR-862 Φ0 Β10). Μέχρι τότε ερχόταν ως
 * `FILE_TRASHED` + `supersededByFileId`, και αυτή η συνάρτηση έπρεπε να θυμάται να ρωτήσει τη
 * σημαία **πρώτη**. Πλέον η αντικατάσταση είναι **άλλο γεγονός** (`FILE_SUPERSEDED`) που ο
 * `useLevelFloorplanSync` δεν ακούει: η προστασία του Ο-16 είναι **δομική**, όχι έλεγχος.
 *
