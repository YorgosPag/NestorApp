...
describe('buildDimensionSymbol (ADR-531 Φ5b.1++)', () => {
  it('γραμμή με κενό → 2 τμήματα + 8 end markers (2×3 τρίγωνα βέλους + 2 extension) + 1 κείμενο', () => {
    const g = buildDimensionSymbol(dim([]));
    expect(g.lines).toHaveLength(2);
    expect(g.ticks).toHaveLength(8); // arrowHead(end0)=3 + arrowHead(end1)=3 + 2 perpTick
  ...
  it('οι extension παύλες είναι κάθετες στη γραμμή (κατακόρυφες για οριζόντια διάσταση)', () => {
    ...
    const ext = ticks[6]; // 7ο = perpTick(end0) (μετά 3+3 πτερύγια/βάσεις βελών)
