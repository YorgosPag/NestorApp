...
describe('matchExternalReferences — ΤΟ ΠΡΑΓΜΑΤΙΚΟ ΤΟΠΟΓΡΑΦΙΚΟ (Ι. Νικολάου)', () => {
  ...
  const DECLARED: ReadonlyArray<[string, { x: number; y: number }]> = [
    ...
    ['2.jpg', { x: 800, y: 600 }],
    ['3.jpg', { x: 1024, y: 768 }],
    ['dianomi_1.JPG', { x: 690, y: 500 }],
    ['dianomi_2.JPG', { x: 700, y: 510 }],
    ['diatagma_1993.JPG', { x: 1200, y: 900 }],
    ['diatagma_1994_47.JPG', { x: 1240, y: 930 }],
    ['google_47.JPG', { x: 1600, y: 1200 }],
    ['gps_47.JPG', { x: 1920, y: 1080 }],
    ['ΣΦΡΑΓΙΔΑ ΜΑΥΡΟΜΙΧΑΛΗΣ.jpg', { x: 400, y: 300 }],
  ];
  ...
  it('βρίσκει 9 από 10 ΧΩΡΙΣ καμία ενέργεια χρήστη — και όλα μέσω διαστάσεων', async () => {
    const files = DELIVERED.map((d) => file(d.name));
    const sizes = Object.fromEntries(DELIVERED.map((d) => [d.name, d.size]));
    const r = await matchExternalReferences(refs, files, depsWithSizes(sizes));

    expect(r.matched).toHaveLength(9);
    expect(r.matched.every((m) => m.reason === 'pixel-size')).toBe(true);
    expect(r.ambiguous).toEqual([]);
    // Το 10ο (η σφραγίδα, από `Z:\Jobs\arxeio\`) δεν παραδόθηκε — μένει «λείπει».
  ...
  it('κάθε αναφορά πήρε ΤΟ ΔΙΚΟ της αρχείο — καμία διπλή ανάθεση', async () => {
    ...
    const sizes = Object.fromEntries(DELIVERED.map((d) => [d.name, d.size]));
    const r = await matchExternalReferences(refs, files, depsWithSizes(sizes));
    expect(new Set(r.matched.map((m) => m.file)).size).toBe(9);
  });
});
