 ...
 * (`stampTableWorksheetStrip`) — δηλαδή η **πύλη μεγέθους (CHECK 4)** θα μπλόκαρε το commit
...
export function stampTableChromeControls(
  ...
  // 🔴 ADR-833 Φάση 3 — **Η ΛΩΡΙΔΑ ΠΡΩΤΗ**, και η σειρά δεν λύνει επικάλυψη.
  //
  // ⚠️ **Ενημερώθηκε (ADR-833 §5.4.11 #3, 2026-09-05)**: εδώ έγραφε «η λωρίδα ζει στην **κάτω**
  // ακμή» — δεν ισχύει πια. Μετακόμισε στην **πάνω**, γιατί κρεμασμένη από το `heightMm`
  // έφευγε κάτω από το χέρι μόλις άλλαζε το ενεργό φύλλο. Πλέον και τα τρία χειριστήρια ζουν
  // στην ίδια ακμή — αλλά **σε ξένες ζώνες**, από κατασκευή: το ⊖ μέσα στη ζώνη γραμμάτων, το
  // ⊕ αμέσως έξω της, και η λωρίδα ένα ολόκληρο κενό (`TABLE_INDICATOR_GRIP_CLEARANCE_PX`)
  // πιο έξω από το ⊕ — δες `tableWorksheetStripOuterPx`, που **διαβάζει** τον από κάτω του.
  //
  // Η σειρά δηλώνει προτεραιότητα **στοίβαξης**: τα `⊖`/`⊕` είναι τα μόνα στοιχεία του πίνακα
  // που λειτουργούν ως **κουμπιά**, και τίποτα δεν επιτρέπεται να τα σκεπάσει — ούτε καν
  // χρώμιο πλοήγησης που σήμερα δεν τα ακουμπά.
  //
