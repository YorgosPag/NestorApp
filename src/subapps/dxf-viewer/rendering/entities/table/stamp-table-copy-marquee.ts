 ...
 *
 * ⇒ Όταν ταυτίζονται, το συμπαγές περίγραμμα **αποσύρεται εντελώς** και μένει μόνο η **λαβή
 ...
 *
 * ⚠️ **«Το συμπαγές περίγραμμα» είναι ΔΥΟ ζωγράφοι, όχι ένας** — και η παράλειψη του δεύτερου
 * ήταν το σφάλμα που έφτασε στην οθόνη: με επιλεγμένη περιοχή το γράφει το `stampTableSelection`,
 * με σκέτο ενεργό κελί το γράφει ο **δρομέας** (`stampTableCellCursor`). Και οι δύο αποσύρονται,
 * από την **ίδια** απάντηση — δες `tableFrameEffectiveRange`.
 *
 * ℹ️ **Excel parity, μετρημένο**: στο στιγμιότυπο `2026-08-05 212519.jpg` (ένα κελί μετά από
 * `Ctrl+C`) φαίνονται **μόνο** τα μυρμήγκια και η λαβή στην κάτω δεξιά γωνία — **καμία** συμπαγής
 * γραμμή. Η προδιαγραφή δεν είναι γούστο: είναι ανάγνωση από την ίδια εικόνα που έδωσε το `[7,2]`.
 *
 ...
 *
 * ## 🔴 ΓΙΑΤΙ «Range» ΚΑΙ ΟΧΙ «Selection» ΣΤΟ ΟΝΟΜΑ (2026-08-06)
 * Λεγόταν `…CoversSelection` και **το όνομα έλεγε ψέματα κατά το ήμισυ**: ο πραγματικός
 * καταναλωτής δεν είναι «η επιλογή» αλλά η **ενεργή περιοχή** — που υπάρχει και *χωρίς*
 * επιλογή (σκέτο ενεργό κελί). Όσο το όνομα μιλούσε για επιλογή, ήταν «φυσικό» να ρωτηθεί
 * μόνο ο ζωγράφος της επιλογής· και ο δρομέας —που ζωγραφίζει το **ίδιο** περίγραμμα στην
 * **ίδια** διαδρομή όταν επιλογή δεν υπάρχει— δεν ρωτήθηκε ποτέ. Το σφάλμα του §48.12 ήταν
 * ακριβώς αυτή η παράλειψη, και το όνομα ήταν ο συνεργός της.
 */
export function tableCopyMarqueeCoversRange(
  entity: TableEntity,
  marquee: TableCopyMarqueeState | null,
  rangeBounds: TableRectBounds | null | undefined,
): boolean {
