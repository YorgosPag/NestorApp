...
describe('stampTableSelection — το περίγραμμα της περιοχής', () => {
  ...
  it('🔴 πάχος 2 px — μετρημένο στο Excel· ήταν 1,5 και ο καμβάς το μοίραζε σε δύο σειρές pixel', () => {
    const log = createPaintLog();
    stampTableSelection(createRc(log), RANGE, ACTIVE);

    expect(log.strokes[0].lineWidth).toBe(2);
    // Παράγωγο του δρομέα, όχι δεύτερη τιμή: σε επιλογή 1×1 τα δύο ορθογώνια συμπίπτουν και
    // **οφείλουν** να φαίνονται ένα.
    expect(TABLE_CELL_SELECTION.outlineWidthPx).toBe(TABLE_CELL_CURSOR.lineWidthPx);
  });
