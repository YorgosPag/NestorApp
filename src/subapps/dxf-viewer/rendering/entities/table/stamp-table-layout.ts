...
export interface StampTableContext {
  ...
  /**
   * 🔴 ADR-771 Φ.2 — **ζωγραφίζει ο πίνακας ο ίδιος το {@link surfaceHex};**
   *
   * Έρχεται αυτούσιο από το `TableEntityGeometry.surfacePaint`, δηλαδή από την **ίδια**
   * απόφαση που έδωσε το χρώμα. Το διαβάζει **μόνο** ο `stampTableSheet`, ως δική του πρώτη
   * γραμμή: ο καλών τον καλεί πάντα και δεν έχει συνθήκη να ξεχάσει.
   */
...
export function stampTableSelection(
   ...
   * (`tableCopyMarqueeCoversRange`) — εδώ φτάνει έτοιμη απάντηση, γιατί αυτός ο ζωγράφος
