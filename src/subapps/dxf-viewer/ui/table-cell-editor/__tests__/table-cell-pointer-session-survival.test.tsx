...
describe('🔴 ADR-739 §26.15 — το κλικ στον καμβά και η συνεδρία πίνακα', () => {
  ...
  describe('🔴 §27.14 + §68 — ΤΟ ΔΕΞΙ ΚΛΙΚ ΕΙΝΑΙ ΧΕΙΡΟΝΟΜΙΑ ΤΗΣ ΣΥΝΕΔΡΙΑΣ, ΚΑΙ ΕΓΚΑΘΙΣΤΑ ΤΟΝ ΣΤΟΧΟ ΤΟΥ', () => {
    /**
    ...
    /**
     * 🔴 **§68 (20/08) — ΑΥΤΗ Η ΑΓΚΥΡΑ ΕΛΕΓΕ ΤΟ ΑΝΤΙΘΕΤΟ, ΚΑΙ ΗΤΑΝ ΣΩΣΤΗ ΤΟΤΕ.**
     *
     * Έγραφε «*ΜΟΝΟ δηλώνει: ο δρομέας ΔΕΝ μετακινείται*», με επιχείρημα ότι «*μια μετακίνηση
     * δρομέα εδώ θα άλλαζε το αντικείμενο της εντολής κάτω από το μενού που μόλις άνοιξε*».
     * Το επιχείρημα ήταν σωστό για τη **σειρά** και λάθος για τον **στόχο**: το μενού ανοίγει
     * στο `contextmenu`, δηλαδή **μετά**, οπότε δεν βρίσκει ποτέ κάτι να αλλάζει από κάτω του —
     * βρίσκει την επιλογή ήδη εκεί που πάτησε ο χρήστης, που είναι όλο το ζητούμενο.
     *
     * Ο ιδιοκτήτης ζήτησε **full parity με το Excel** (20/08): επιλεγμένο `A1`, δεξί κλικ στο
     * `B2` ⇒ μαρκάρεται το `B2`.
     */
    it('🔴 §68 δεξί κλικ ΕΞΩ από ό,τι ισχύει ⇒ ο δρομέας ΜΕΤΑΚΙΝΕΙΤΑΙ εκεί (Excel parity)', () => {
      // Ενεργό το A3 (`beforeEach`)· το πάτημα πέφτει στο C4 — άλλο κελί, άρα «έξω».
      rightPressOn(canvas, cellScreenPoint(entity, 3, 2));
      nextFrame();
      ...
      // ⚠️ Καμία **επέκταση**: το δεξί μετακινεί, δεν σύρει και δεν μαρκάρει περιοχή.
      expect(onSelectTo).not.toHaveBeenCalled();
      expect(getTableCellCursor()?.selection).toBeFalsy();
    });

    /**
     * 🔴 **Η άλλη μισή προδιαγραφή**, και ο λόγος που το κριτήριο ρωτά **δύο** πεδία.
     *
     * Χωρίς το σκέλος του ενεργού κελιού στο `tableContextMenuMovesSelection`, αυτό εδώ θα
     * ξανάγραφε τον δρομέα στο **ίδιο** κελί — αβλαβές στα μάτια, αλλά το `tableCursorAt`
     * γεννά **νέα στήλη αγκύρωσης**, δηλαδή θα άλλαζε σιωπηλά πού επιστρέφει το επόμενο
     * `Enter`. Ένα «no-op» που δεν είναι no-op.
     */
    it('🔴 §68 δεξί κλικ ΠΑΝΩ στο ήδη ενεργό κελί ⇒ ΤΙΠΟΤΑ δεν γράφεται', () => {
      const before = getTableCellCursor()?.position;
      rightPressOn(canvas, cellScreenPoint(entity, 2, 0));
      nextFrame();
