...
export function useTableIndicatorHover(params: UseTableIndicatorHoverParams): void {
  ...
  const handleMouseMove = useEventCallback((
    ...
    const { hit, cursor, insert, remove, selectAll, worksheetStrip } = tableIndicatorProbeAtWorld(
      target,
      world,
      transform.scale,
      selection,
    ...
    setTableIndicatorHover(
      hit
        ...
        : selectAll
          ...
          // 🔴 ADR-833 Φάση 4 — **και το ⊕ της προσθήκης**, από την ίδια απάντηση: η λωρίδα
          // ρωτήθηκε **μία** φορά και ξέρει ήδη ποιο από τα δύο είναι κάτω από το χέρι.
          : worksheetStrip?.kind === 'tab'
            ? { entityId: target.id, target: { kind: 'worksheet-tab', worksheetId: worksheetStrip.tab.id } }
            : worksheetStrip?.kind === 'add'
              ? { entityId: target.id, target: { kind: 'worksheet-add' } }
              : null,
    );
