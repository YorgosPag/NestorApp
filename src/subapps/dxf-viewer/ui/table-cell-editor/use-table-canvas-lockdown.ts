...
// 🔴 ADR-768 Βήμα 5 — ο **δεύτερος** αναγνώστης του στόχου βαψίματος. Το ίδιο store που διαβάζει
// το πάτημα: δύο υπολογισμοί θα ήταν pixel όπου το κλείδωμα αφήνει το συμβάν να περάσει και το
// πινέλο δεν βάφει — ή το ανάποδο, που **αποεπιλέγει** τον πίνακα.
import { getTableFormatPaintTarget } from '../../state/table-format-paint-target-store';
...
function shouldBlock(
  ...
  // 5β. 🔴 **ADR-833 Φάση 3 — Η ΚΑΡΤΕΛΑ ΦΥΛΛΟΥ ΕΙΝΑΙ ΚΟΥΜΠΙ, ΜΕ ΤΗΝ ΙΔΙΑ ΑΣΥΜΜΕΤΡΙΑ.**
  //
  //    Ταυτόσημη περίπτωση με το ⊕ από πάνω, γραμμένη **πριν** εμφανιστεί το σύμπτωμα: η
  //    λωρίδα κάθεται **έξω** από το πλέγμα (θετικό `v`, πέρα από την οπή λαβής), άρα χωρίς
  //    αυτή τη γραμμή ο φύλακας θα έκοβε το `mousedown` σε **σύλληψη στο `document`** —
  //    δηλαδή δομικά **πριν** από τον ακροατή του δοχείου που εκτελεί την αλλαγή φύλλου. Η
  //    λωρίδα θα ήταν ορατή, φωτισμένη στο hover, και **άφταστη** (§40.8).
  //
  //    Και το `mouseup` μένει κομμένο, για τον ίδιο λόγο: εκεί γεννιέται η **επιλογή
  //    οντότητας** (`mouse-handler-up.ts` → `onCanvasClick`), και ο καμβάς την κάνει ακόμα κι
  //    όταν δεν είδε ποτέ το `mousedown` — δηλαδή μια αλλαγή φύλλου θα **αποεπέλεγε** τον
  //    πίνακα με την ίδια κίνηση.
  //
  //    🔴 **ADR-833 Φάση 4 — και το ⊕ της προσθήκης, στην ΙΔΙΑ γραμμή.** Ζει στην ίδια λωρίδα,
  //    στο ίδιο θετικό `v`, και το πάτημά του εκτελείται από τον **ίδιο** ακροατή οπλισμένων
  //    χειριστηρίων. Δύο γραμμές θα ήταν δύο ευκαιρίες να μάθει ο φύλακας τη μία περίπτωση και
  //    όχι την άλλη — δηλαδή ⊕ ορατό και άφταστο, ακριβώς το περιστατικό του §40.8.
  //
  //    🔴 **ΚΑΙ ΤΟ `dblclick` ΠΕΡΝΑ — Η ΛΩΡΙΔΑ ΕΙΝΑΙ Η ΜΟΝΗ ΠΕΡΙΠΤΩΣΗ ΠΟΥ ΤΟ ΧΡΕΙΑΖΕΤΑΙ.**
  //    Από τη Φάση 4 το διπλό κλικ σε καρτέλα ανοίγει **μετονομασία** (Excel/Sheets parity).
  //    Χωρίς αυτή τη λέξη, η χειρονομία θα δούλευε σε **απλή επιλογή** (όπου ο φύλακας δεν
  //    τρέχει καθόλου) και θα ήταν **νεκρή μέσα** στη λειτουργία πίνακα — δηλαδή ακριβώς το
  //    σύμπτωμα που ο ιδιοκτήτης μέτρησε στο §40.8 («δουλεύει έξω από το edit mode, μέσα όχι»).
  //
  //    ⚠️ Το `mouseup` **μένει κομμένο**: η ασυμμετρία δεν χαλαρώνει, απλώς μαθαίνει ένα
  //    δεύτερο συμβάν που έχει πραγματική δουλειά εδώ.
  if (hit?.where === 'worksheet-tab' || hit?.where === 'worksheet-add') {
    return event.type !== 'mousedown' && event.type !== 'dblclick';
  }
  // 6. 🔴 **ADR-768 Βήμα 5 — ΤΟ ΠΙΝΕΛΟ ΒΑΦΕΙ ΚΑΙ ΣΕ ΑΛΛΟΝ ΠΙΝΑΚΑ.**
  //
  //    Ο φρουρός από κάτω ρωτά τη γεωμετρία **ΤΟΥ ΠΙΝΑΚΑ ΤΟΥ ΔΡΟΜΕΑ** και μόνο. Άρα ένα κλικ σε
  ...
  //    `stopPropagation()` εδώ σημαίνει ότι ο ακροατής του πινέλου **δεν καλείται ποτέ**:
  //    πρόγονος ⇒ πάντα πρώτος. Το cross-table βάψιμο ήταν δομικά αδύνατο, όχι απλώς άγραφο.
  //
  //    🔑 Η ασυμμετρία είναι **ταυτόσημη** με του ⊕ από πάνω, και για τον ίδιο λόγο: το
  //    `mousedown` **οφείλει** να περάσει (ο ακροατής του πινέλου το καταναλώνει επί τόπου), αλλά
  //    το `mouseup` **οφείλει** να μείνει κομμένο — εκεί γεννιέται η επιλογή οντότητας
  //    (`mouse-handler-up.ts` → `onCanvasClick`), και ο καμβάς την κάνει **ακόμα κι όταν δεν είδε
  //    ποτέ το `mousedown`**. Συμμετρικό «άσ' τα όλα να περάσουν» θα έβαφε το κελί και θα
  //    **αποεπέλεγε** τον πίνακα με την ίδια κίνηση.
  //
  //    ⚠️ **Η ΤΡΥΠΑ ΕΙΝΑΙ ΣΤΕΝΗ ΕΠΙΤΗΔΕΣ**: απαιτεί **γραμμένο στόχο**, όχι σκέτο «το πινέλο
  //    είναι οπλισμένο». Ο στόχος γράφεται από τον ΕΝΑ γραφέα μόνο εκεί που ο δείκτης υπόσχεται
  //    βάψιμο — άρα πάνω σε κενό σχέδιο είναι `null` και το κλείδωμα κρατά. Ένα σκέτο
  //    `isTableFormatPainterArmed()` θα άφηνε το `mousedown` να φτάσει στον καμβά έξω από κάθε
  //    πίνακα ⇒ lasso ⇒ `blur` ⇒ **θάνατος της συνεδρίας**: έξοδος κατά λάθος, δηλαδή ακριβώς
  //    αυτό που το §29 ήρθε να καταργήσει.
  //
  //    Κόστος: μία ανάγνωση store ανά συμβάν. Καμία γεωμετρία — την πλήρωσε ήδη ο hover.
  if (getTableFormatPaintTarget()) return event.type !== 'mousedown';
  return hit === null;
}
