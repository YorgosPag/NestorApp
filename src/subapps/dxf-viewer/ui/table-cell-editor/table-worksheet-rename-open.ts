 ...
 *
 * ## Γιατί η θέση ξαναϋπολογίζεται εδώ
 * Ο πρώτος καλών ήταν το **μενού**: ανάμεσα στο δεξί κλικ και την επιλογή «Μετονομασία»
 ...
 *
 * ## 🔴 ADR-833 §5.4.11 #3 — ΚΑΙ ΤΟ ΔΙΠΛΟ ΚΛΙΚ ΜΠΑΙΝΕΙ ΠΛΕΟΝ ΑΠΟ ΕΔΩ (2026-09-05)
 * Εδώ έγραφε *«ενώ το διπλό κλικ την έχει ήδη»* — και **αυτό ακριβώς ήταν το ελάττωμα**. Το
 * διπλό κλικ έδινε το `slot` που βρήκε από το pixel, δηλαδή απάντηση που **λήγει μέσα στην
 * ίδια χειρονομία**: το πρώτο του κλικ ενεργοποιεί το φύλλο, το παράθυρο είναι κεντραρισμένο
 * στην ενεργή καρτέλα, άρα η λωρίδα κυλά και το pixel αποκτά **άλλον ένοικο**.
 *
 * **Μετρημένο**: με 40 φύλλα, διπλό κλικ στο `ws_23` άνοιγε μετονομασία του **`ws_35`**.
 *
 * 🔑 Η φράση που δικαιολογούσε αυτή τη συνάρτηση για το μενού — *«η λωρίδα κυλά, το φύλλο
 * όχι»* — ίσχυε **από την πρώτη μέρα** και για το διπλό κλικ· απλώς κανείς δεν είχε μετρήσει
 * ότι η λωρίδα κυλά και **μέσα** σε μία χειρονομία. Πλέον υπάρχει **μία** θύρα και για τους
 * δύο, και το {@link openWorksheetRename} δεν έχει άλλον καλούντα εκτός από αυτήν.
 *
