...
export type TableCellKeyIntent =
  ...
  /**
   * 🔴 ADR-828 Φ4α — `Alt+↓` σε **πλοήγηση**: άνοιξε το μενού του κουμπιού «Επιλογές
   * Αυτόματης Συμπλήρωσης», αν το κουμπί ζει αυτή τη στιγμή.
   *
   * ## Γιατί ΑΚΡΙΒΩΣ αυτό το πλήκτρο
   * Είναι η σύμβαση των **Windows** και του **WAI-ARIA APG** για «*άνοιξε το dropdown αυτού
   * που είναι εστιασμένο*», και το **ίδιο** πλήκτρο που χρησιμοποιεί ήδη το Excel για το
   * dropdown επικύρωσης δεδομένων και για το μενού AutoFilter — δηλαδή ο χρήστης το ξέρει
   * από το ίδιο το φύλλο, δεν το μαθαίνει από εμάς.
   *
   * ## 🔴 ΕΔΩ ΞΕΠΕΡΝΑΜΕ ΤΟ EXCEL, ΚΑΙ ΤΟ ΚΕΝΟ ΕΙΝΑΙ ΜΕΤΡΗΜΕΝΟ
   * Το Excel **δεν έχει καμία** συντόμευση προς αυτό το κουμπί (επιβεβαιωμένο σε δύο
   * ανεξάρτητες αναζητήσεις, 2026-08-29): έχει `Ctrl+D`/`Ctrl+R` για fill down/right και
   * `Ctrl+E` για Flash Fill, αλλά οι **επιλογές** της συμπλήρωσης — μαζί τους η μόνη πόρτα
   * προς τη «Συμπλήρωση καθημερινών» — είναι απρόσιτες χωρίς ποντίκι.
   *
   * Το κόστος για εμάς είναι σχεδόν μηδέν, και γι' αυτό δεν υπάρχει δικαιολογία να μην
   * υπάρχει: το μενού μας είναι ήδη Radix, δηλαδή **πλήρως πλοηγήσιμο με πληκτρολόγιο** μόλις
   * ανοίξει. Έλειπε μόνο ένα πλήκτρο που το ανοίγει.
   *
   * ## Γιατί ΔΕΝ συγκρούεται με τίποτα
   * Δύο λόγοι, και ο δεύτερος είναι ο ουσιώδης:
   *  1. ο κλάδος `altKey` παρακάτω δεχόταν **μόνο** `Alt+Enter`· κάθε άλλο `Alt+*` έφευγε ως
   *     `passthrough`, δηλαδή το πλήκτρο ήταν ελεύθερο·
   *  2. η δέσμευση ζει **μόνο όσο ζει το κουμπί** — μια μεταβατική κατάσταση που γεννιέται από
   *     συμπλήρωση και πεθαίνει στην επόμενη πράξη. Δεν μπορεί να συγκρουστεί με μόνιμη
   *     συντόμευση, γιατί τις περισσότερες στιγμές **δεν υπάρχει**.
   *
   * ⚠️ Μόνο σε `nav`: σε γραφή το `Alt+↓` δεν έχει τι να ανοίξει (το κουμπί δεν ζωγραφίζεται
   * εκεί, §13.5 parity), και ο ίδιος φρουρός που έχουν ο ζωγράφος, ο δείκτης και το πάτημα
   * οφείλει να ισχύει και για το πληκτρολόγιο — τέταρτος φρουρός με διαφορετική άποψη θα
   * έδινε «μενού για κουμπί που δεν υπάρχει».
   */
  | { readonly kind: 'fillOptions' }
  /**
...
const FILL_OPTIONS: TableCellKeyIntent = { kind: 'fillOptions' };
...
export function resolveTableCellKeyIntent(
  ...
  if (mod.altKey) {
    // 🔴 ADR-828 Φ4α — `Alt+↓`: το μενού του κουμπιού συμπλήρωσης. Δες το `fillOptions` του
    // union για το γιατί ακριβώς αυτό το πλήκτρο, και γιατί δεν συγκρούεται με τίποτα.
    if (key === 'ArrowDown' && mode === 'nav') return FILL_OPTIONS;
