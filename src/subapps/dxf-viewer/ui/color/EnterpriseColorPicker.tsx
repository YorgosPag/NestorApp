...
function ContrastRow({ label, ratio, passAA, passAAA, textColor, bgColor }: ContrastRowProps) {
  ...
  return (
    <div className={`flex items-center ${PANEL_LAYOUT.GAP.SM}`}>
      ...
      <span className={`${PANEL_LAYOUT.TYPOGRAPHY.XS} ${PANEL_LAYOUT.FONT_WEIGHT.MEDIUM} ${passAA ? 'text-[hsl(var(--text-success))]' : 'text-destructive'}`}>AA</span>
      <span className={`${PANEL_LAYOUT.TYPOGRAPHY.XS} ${PANEL_LAYOUT.FONT_WEIGHT.MEDIUM} ${passAAA ? 'text-[hsl(var(--text-success))]' : 'text-destructive'}`}>AAA</span>
    </div>
  );
}
