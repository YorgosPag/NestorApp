  ...
  return (
    <div className={`flex items-center ${PANEL_LAYOUT.GAP.XS} ${colors.bg.primary} rounded ${PANEL_LAYOUT.SPACING.COMPACT}`}>
      ...
      <input
        ...
        className={`${PANEL_LAYOUT.WIDTH.MD} ${PANEL_LAYOUT.INPUT.PADDING_X} ${PANEL_LAYOUT.PADDING.VERTICAL_NONE} ${colors.bg.tertiary} ${getStatusBorder('muted')} rounded ${colors.text.inverted} ${PANEL_LAYOUT.TYPOGRAPHY.XS} text-center ${getFocusBorder('input')} focus:outline-none ${PANEL_LAYOUT.TRANSITION.COLORS} ${PANEL_LAYOUT.SELECT.ALL}`}
