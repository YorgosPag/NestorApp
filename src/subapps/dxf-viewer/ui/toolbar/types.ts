...
export type ToolType =
  ...
  | 'title-block'
  // ADR-654: single-click placement of a floorplan-entourage furniture image (ImageEntity)
  | 'furniture-plan'
  // ADR-654 M6: single-click placement of a top-view people entourage image (ImageEntity)
  | 'people-plan'
  ...
  | 'plants-plan'
  // ADR-736 §6: single-click (or Enter = viewport centre) placement of a user-uploaded raster (ImageEntity)
  | 'attach-image'
  ...
  | 'hatch-area-label'    // AREALABEL — 2-click → TextEntity (εμβαδόν γραμμοσκίασης Ή τοπογραφικής
                          // επιφάνειας· το id είναι ΙΣΤΟΡΙΚΟ, βλ. ADR-662 §12)
