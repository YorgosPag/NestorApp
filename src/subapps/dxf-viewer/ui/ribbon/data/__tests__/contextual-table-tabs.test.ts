...
describe('Καλωδίωση εντολών — καμία εντολή χωρίς παραλήπτη', () => {
  ...
  it('🔴 ΚΑΘΕ `widgetId` υπάρχει στο ΠΡΑΓΜΑΤΙΚΟ μητρώο — άγνωστο id = αόρατο κουμπί', async () => {
    ...
    // Τα **οκτώ** σύνθετα χειριστήρια: δύο χρώματα, συγχώνευση, περιγράμματα, το **πινέλο
    // μορφοποίησης** (ADR-768 Βήμα 5), η **«Επικόλληση»** (ADR-739 §57) και τα **δύο βελάκια
    // γωνίας** που ανοίγουν τον διάλογο «Μορφοποίηση κελιών» (ADR-739 §60). Αν αυτό γίνει 0, το
    // test θα περνούσε κενό ενώ η καρτέλα θα είχε χάσει τα widgets της.
    //
    // ⚠️ §60 — τα δύο βελάκια είναι `widget` και όχι `type: 'simple'` με `action`, παρότι
    // μοιάζουν με απλά κουμπιά: κρατούν **κατάσταση** (ποιος διάλογος είναι ανοιχτός) και
    // αποδίδουν τον διάλογο μέσα τους. Ένα `action` θα χρειαζόταν τον κύκλο ζωής του διαλόγου
    // κάπου αλλού — δηλαδή δεύτερο ιδιοκτήτη για ένα παράθυρο.
    //
    ...
    expect(widgetIds).toHaveLength(8);
