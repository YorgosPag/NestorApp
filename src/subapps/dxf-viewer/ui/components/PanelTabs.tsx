...
import { BarChart, Box, Frame, PackageOpen, Palette, Ruler, Settings, Sliders } from 'lucide-react';
...
export function PanelTabs({ activePanel, onTabClick, disabledPanels, isCollapsed }: PanelTabsProps) {
  ...
  const tabs: TabDefinition[] = [
    ...
    {
      id: 'properties',
      label: isCollapsed ? '' : t('panels.properties.title'),
      icon: Sliders,
      content: null,
      disabled: disabledPanels['properties'],
    },
    {
      id: 'dimensions',
      label: isCollapsed ? '' : t('panels.dimensions.title'),
      icon: Ruler,
      content: null,
      disabled: disabledPanels['dimensions'],
    },
    {
      id: 'materials',
      label: isCollapsed ? '' : t('panels.materials.title'),
      icon: Palette,
      content: null,
    ...
    {
      // ADR-683 Φ3.1γ — «Εισαγόμενα»: λίστα πλεγμάτων συνεργάτη + πλήθος ανανάθετων.
      id: 'imported-meshes',
      label: isCollapsed ? '' : t('panels.importedMeshes.title'),
      icon: PackageOpen,
      content: null,
      disabled: disabledPanels['imported-meshes'],
    },
