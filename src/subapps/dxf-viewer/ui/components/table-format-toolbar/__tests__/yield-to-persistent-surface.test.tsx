...
describe('Γ — καμία επιφάνεια με mini toolbar δεν μένει έξω από τον κανόνα', () => {
  const HOSTS = ['TableHeaderContextMenu.tsx', 'TableRangeContextMenu.tsx'] as const;
   ...
   * το «ποιος αποδίδει `TableFormatToolbar`». Νέος ξενιστής ⇒ πέφτει εδώ **μόνος του**.
   */
  const dir = path.join(__dirname, '..', '..');

  it('Γ1 — ο κατάλογος ξενιστών είναι ΑΚΡΙΒΩΣ όσοι αποδίδουν το TableFormatToolbar', () => {
    const rendersToolbar = fs.readdirSync(dir)
      .filter((f) => f.endsWith('.tsx'))
      .filter((f) => /<TableFormatToolbar\b/.test(fs.readFileSync(path.join(dir, f), 'utf8')));
