...
describe('κλειδιά i18n', () => {
  ...
  /**
   * 🔴 **ADR-759 Φ6 — το κουμπί και η παλέτα λένε ΤΟ ΙΔΙΟ όνομα.**
   *
   * Το όνομα ζει σε **δύο** κλειδιά: την εντολή της κορδέλας που ανοίγει την παλέτα και τον
   * τίτλο της ίδιας της παλέτας. Δύο τόποι για ένα όνομα αποκλίνουν — και η απόκλιση εδώ δεν
   * βγάζει σφάλμα, βγάζει **χρήστη που δεν αναγνωρίζει το παράθυρο που μόλις άνοιξε**.
   *
   * ⚠️ Η πύλη δεν καρφώνει το **κείμενο** (θα ήταν test της διατύπωσης, όχι της συμπεριφοράς):
   * καρφώνει την **ισότητα**. Μετονομασία σε οποιοδήποτε από τα δύο απαιτεί και το άλλο.
   */
  it.each(['el', 'en'])('🔴 Φ6: η εντολή και ο τίτλος της παλέτας ταυτίζονται στο %s', (lang) => {
    const tree = lang === 'el' ? el : en;
    const command = at(tree, 'ribbon.commands.titleBlockBinding');
    expect(typeof command).toBe('string');
    expect(at(tree, 'titleBlockBinding.title')).toBe(command);
  });
