...
export const GridSettings: React.FC<GridSettingsProps> = ({ className = '' }) => {
  ...
  // ─── 🌊 Adaptive grid (smooth fade) — null-safe view + handlers ─────────
  ...
  const handleSmoothFadeDurationChange = (ms: number) => {
    updateGridSettings({
      behavior: { ...gridSettings.behavior, ...adaptiveFade, smoothFadeDurationMs: ms },
    });
  };
