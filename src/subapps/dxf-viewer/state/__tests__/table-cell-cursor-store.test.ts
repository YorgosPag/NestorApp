...
describe('κύκλος ζωής', () => {
  ...
  it('τοποθέτηση κρατά ταυτότητα, θέση και κατάσταση', () => {
    ...
    expect(getTableCellCursor()).toEqual({
      ...
      selection: null,
      sessionId: 0,
      // ADR-763 Φ2.4.1 — κανένα αίτημα δέσμευσης· αύξων για όλη τη ζωή του store, όπως τα δύο
      // αδέλφια του. Η **πλήρης** σύγκριση σχήματος εδώ είναι ο λόγος που ένα νέο πεδίο δεν
      // μπορεί να προστεθεί σιωπηλά.
      commitRequest: 0,
      // ADR-754 §4 — ο αριθμός τοποθέτησης κέρσορα, αύξων για όλη τη ζωή του store.
      caretRevision: 0,
    });
  });
...
describe('αριθμός συνεδρίας — ο φρουρός της χαμένης πληκτρολόγησης', () => {
  ...
  it('η ΑΚΥΡΩΣΗ γραφής τον αυξάνει και επιστρέφει σε πλοήγηση, ΣΤΟ ΙΔΙΟ κελί', () => {
    ...
    expect(getTableCellCursor()).toEqual({
      ...
      selection: null,
      sessionId: 1,
      // ADR-763 Φ2.4.1 — η ακύρωση **δεν** τον αυξάνει: δεν είναι αίτημα δέσμευσης, είναι το
      // αντίθετό της.
      commitRequest: 0,
      // ADR-754 §4 — η ακύρωση **δεν** τον αυξάνει: το πεδίο ξαναστήνεται ούτως ή άλλως από
      // τον αριθμό συνεδρίας, και εκεί ο κέρσορας γεννιέται από την αρχή.
      caretRevision: 0,
    });
  });
