...
/**
 * 🏢 ENTERPRISE CROSSHAIR COMPOSITOR — shared 2D/3D core (ADR-040 / ADR-545)
 *
 * The CAD crosshair render code, ONE source of truth for BOTH the 2D canvas
 * (`CrosshairOverlay`) and the 3D BIM viewport (`BimCrosshairOverlay3D`). It owns
 * the promoted DOM (4 arm segments + centre aperture + «+/−» badge), the static
 * styling (colours/size/gap from `getCursorSettings`), the hover badge decision
 * (`resolveHoverBadge` + `HoverStore` + Shift) and the per-move `translate3d`.
 *
 ...
 *     `ImmediateSnapStore`.
 *   - 3D feeds a camera-PROJECTED canvas-local position each RAF frame (the snap
 *     point reprojects as the camera orbits), and snap-active from `Snap3DOverlayStore`.
 *
 ...
 *
 * Geometry / gap / aperture / badge logic is unchanged from the previous monolithic
 * `CrosshairOverlay` (v4) — see `crosshair-compositor-layout` + ADR-515.
 *
 * @module CrosshairCompositor
 * @version 1.0.0 — Extracted shared core (2026-06-27, ADR-545)
 */
...
/** Imperative driver surface exposed to each host (2D screen-store / 3D camera-RAF). */
...
export const CrosshairCompositor = forwardRef<CrosshairCompositorHandle, CrosshairCompositorProps>(
  ...
  // 2026-06-24). Γράφει μόνο `display`. Είναι ο ΜΟΝΟΣ snap-hide μηχανισμός.
  // ============================================================================
  ...
  // Runs on settings / size / pick-box change (rare), NEVER per mouse move.
  // ============================================================================
  const applyStaticStyles = useCallback(() => {
    ...
    // Sizes/χρώμα εδώ· η ορατότητα (incl. snap-hide) καθορίζεται από updateApertureVisibility().
    const ap = apertureRef.current;
  ...
  // ============================================================================
  // PER-MOVE TRANSFORM — the ONLY thing called on every move. Writes a SINGLE
  // `translate3d` to the one promoted layer (+ container visibility). Stores the
  // applied position so settings/suppression/resize can re-apply without the host.
  // ============================================================================
  ...
  // Snap-active setter (host drives it from its snap store). Writes DOM only on change.
  ...
  const recompute = useCallback(() => {
    ...
    applyStaticStyles();
    applyBadge();
    applyTransform(lastPosRef.current);
  ...
  useEffect(
    () => subscribeCrosshairSuppression(() => applyTransform(lastPosRef.current)),
    [applyTransform],
  );

  // Selection badge: hover changes + Shift key (low-frequency). The badge rides
  // the moving layer, so only its text/colour/visibility change here.
  useEffect(() => {
