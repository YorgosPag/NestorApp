...
/**
 * ADR-408 DHW — useBim3DMepWaterHeaterPlacement hook.
 *
 * Lets the user place a point-based domestic hot water heater (θερμοσίφωνας) directly
 * on the 3D canvas. Mirror of `useBim3DMepBoilerPlacement` (ADR-408 Εύρος Β #2): one
 ...
 *
 * Armed only while the water heater tool is active AND the viewport is in 3D — the
 * SAME `activeTool === 'mep-water-heater'` the 2D pipeline uses, so the existing water
 * heater FSM stays the single source of truth. On click the screen point is projected
 * onto the active mounting-elevation work-plane, converted to scene units, and
 * handed to the 2D `useMepWaterHeaterTool.onCanvasClick` via the
 * `bim:place-mep-water-heater-3d` EventBus bridge — reusing the whole commit path.
 *
...
export interface UseBim3DMepWaterHeaterPlacementParams {
  readonly managerRef: MutableRefObject<ThreeJsSceneManager | null>;
  readonly canvasEl: HTMLCanvasElement | null;
}

export function useBim3DMepWaterHeaterPlacement(
  { managerRef, canvasEl }: UseBim3DMepWaterHeaterPlacementParams,
): void {
  useEffect(() => {
    ...
    const unitsNow = (): ReturnType<NonNullable<ReturnType<typeof mepWaterHeaterToolBridgeStore.get>>['getSceneUnits']> =>
      mepWaterHeaterToolBridgeStore.get()?.getSceneUnits() ?? 'mm';

    // A water heater is floor/wall-mounted: the box is centred on `mountingElevationMm`, so
    // the cursor projects onto that work-plane (mirror of the boiler work-plane).
    // The SAME elevation feeds the raycast and `waterHeaterToMesh` (FFL + mounting) so
    // ghost == cursor (WYSIWYG).
    const mountingElevationMmNow = (): number =>
      mepWaterHeaterToolBridgeStore.get()?.overrides.mountingElevationMm ?? DEFAULT_WATER_HEATER_MOUNTING_ELEVATION_MM;
    ...
    const onMove = (e: PointerEvent): void => {
      ...
      // Pass the FLOOR elevation — `waterHeaterToMesh` re-adds `mountingElevationMm` and
    ...
    const onClick = (e: MouseEvent): void => {
      ...
      e.preventDefault();
      e.stopPropagation();
      EventBus.emit('bim:place-mep-water-heater-3d', { point: planMmToScenePoint(hit.planMm, unitsNow()) });
    };
    ...
    const apply = (): void => {
      ...
      const active = tool === 'mep-water-heater' && selectIs3D(useViewMode3DStore.getState());
      if (active) setup();
      else teardown();
    };
