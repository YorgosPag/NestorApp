...
/**
 * ADR-408 Φ3 — useBim3DElectricalPanelPlacement hook.
 *
 * Lets the user place a point-based electrical panel directly on the 3D canvas.
 ...
 *
 * Armed only while the panel tool is active AND the viewport is in 3D — the SAME
 * `activeTool === 'electrical-panel'` the 2D pipeline uses, so the existing panel
 ...
 * handed to the 2D `useElectricalPanelTool.onCanvasClick` via the
 * `bim:place-electrical-panel-3d` EventBus bridge — reusing the whole commit path.
 *
...
export interface UseBim3DElectricalPanelPlacementParams {
  readonly managerRef: MutableRefObject<ThreeJsSceneManager | null>;
  readonly canvasEl: HTMLCanvasElement | null;
}

export function useBim3DElectricalPanelPlacement(
  { managerRef, canvasEl }: UseBim3DElectricalPanelPlacementParams,
): void {
  useEffect(() => {
    ...
    const unitsNow = (): ReturnType<NonNullable<ReturnType<typeof electricalPanelToolBridgeStore.get>>['getSceneUnits']> =>
      electricalPanelToolBridgeStore.get()?.getSceneUnits() ?? 'mm';

    // A panel is wall-mounted: the box is centred on `mountingElevationMm`, so the
    // cursor projects onto that work-plane (mirror of the fixture ceiling plane).
    // The SAME elevation feeds the raycast and `panelToMesh` (FFL + mounting) so
    // ghost == cursor (WYSIWYG).
    const mountingElevationMmNow = (): number =>
      electricalPanelToolBridgeStore.get()?.overrides.mountingElevationMm ?? DEFAULT_PANEL_MOUNTING_ELEVATION_MM;
    ...
    const onMove = (e: PointerEvent): void => {
      ...
      // Pass the FLOOR elevation — `panelToMesh` re-adds `mountingElevationMm` and
    ...
    const onClick = (e: MouseEvent): void => {
      ...
      e.preventDefault();
      e.stopPropagation();
      EventBus.emit('bim:place-electrical-panel-3d', { point: planMmToScenePoint(hit.planMm, unitsNow()) });
    };
    ...
    const apply = (): void => {
      const active =
        toolStateStore.get().activeTool === 'electrical-panel' &&
