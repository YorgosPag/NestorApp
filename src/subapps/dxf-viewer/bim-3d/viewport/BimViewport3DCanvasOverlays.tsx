...
// ADR-549 Phase 8 — the CAD crosshair is now the OS HARDWARE cursor (see `useCrosshairCursor` in
// BimViewport3D); the old canvas crosshair (`BimCrosshairOverlay3D`) is retired.
...
export function BimViewport3DCanvasOverlays({ managerRef }: BimViewport3DCanvasOverlaysProps) {
  return (
    <>
      ...
      {/* ADR-549 Phase 8 — the CAD crosshair is now the OS HARDWARE cursor (CSS `cursor: url(png)`),
          set on the viewport container by `useCrosshairCursor` (perfect 1:1 tracking). The old canvas
          crosshair (`BimCrosshairOverlay3D` + shared `CrosshairCompositor`) is retired. */}
