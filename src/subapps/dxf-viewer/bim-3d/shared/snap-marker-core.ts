...
import { SNAP_MARKER_BASE_COLOR } from '../../rendering/ui/snap/snap-visual-config';
 ...
 * `THREE.Color().getHex()` = το standard pattern του codebase (αντί custom hex→0x helper),
 * κρατά το export σε `number` για backward-compat με τους gizmo re-exporters.
 */
export const SNAP_MARKER_COLOR = new THREE.Color(SNAP_MARKER_BASE_COLOR).getHex();
