...
describe('faceted taxonomy — category + style + series (M5)', () => {
  it('κάθε def έχει έγκυρο στυλ από το controlled λεξιλόγιο', () => {
    for (const def of listFurniturePlanDefs()) {
      expect(STYLES.has(def.style)).toBe(true);
    }
  });
  ...
  it('το series είναι μοναδικό μέσα σε κάθε ζεύγος (category, style)', () => {
    const seen = new Set<string>();
    for (const def of listFurniturePlanDefs()) {
      const key = `${def.category}/${def.style}/${def.series}`;
  ...
  it('getFurniturePlanLabelParts δίνει i18n κλειδιά facets + series (μηδέν per-item strings)', () => {
    ...
    expect(parts.styleKey).toBe(`furniturePlan.styles.${def.style}`);
