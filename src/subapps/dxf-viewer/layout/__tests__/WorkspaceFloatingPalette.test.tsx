...
describe('ADR-724 Φ3 — WorkspaceFloatingPalette', () => {
  describe('η παλέτα υπάρχει και είναι μία', () => {
    ...
    it('🔴 κάθεται στο στρώμα ΤΗΣ ΠΑΛΕΤΑΣ, όχι στο στρώμα των ειδοποιήσεων', () => {
      /*
        Η προεπιλογή του `FloatingPanel` είναι `zIndex.toast` — **πάνω** από τα μενού και τους
        διαλόγους. Με αυτή, το μενού της ίδιας της παλέτας άνοιγε πίσω της και ο χρήστης έβλεπε
        ένα κουμπί που «δεν ακούει» (αναφορά Giorgio, §14.9).
        Ο έλεγχος είναι **σχεσιακός**, όχι μαγικός αριθμός: κάτω από το στρώμα των μενού.
        ⚠️ Και το **σχόλιο** είναι σχεσιακό, από το ADR-780 Φ.Γ: έγραφε «(1700)»·«(1000)»·«(1400)»
        και η συμπίεση της κλίμακας (κορυφή 10400 → 1310) τα έκανε **ψέματα**, ενώ ο έλεγχος από
        κάτω έμεινε σωστός. Ένας αριθμός σε σχόλιο είναι χειρόγραφος καθρέφτης που **κανείς δεν
        επιβάλλει** — το ίδιο σχήμα που έκρυβε την έκτη κλίμακα στο `portal-overlay.ts`.
      */
      render(<Harness />);
