...
export function emitEntityStyle(pair: Pair, style: EntityStyleCodes): void {
  ...
  // γραμμής κεφαλίδας απέχει 18 μονάδες ανά κανάλι από το καθαρό λευκό και 47 από το πιο
  // κοντινό γκρι (ACI 254) — άρα κέρδιζε το λευκό, και συγκεκριμένα το **ACI 7**, την
  // κανονική λευκή/μαύρη πένα (μετρημένο στο `settings/standards/aci.ts`). Δηλαδή **η
  // γκρίζα κεφαλίδα έβγαινε λευκή** — και χειρότερα: το ACI 7 είναι ακριβώς η τιμή που το
  // `applyPlotColor` μετατρέπει σε **μαύρο μελάνι** στην εκτύπωση. Το ελάττωμα φαινόταν σαν
  // «η εξαγωγή χάνει το γέμισμα» ενώ ήταν κβάντιση χρώματος.
  //
