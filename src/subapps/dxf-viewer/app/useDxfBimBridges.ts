...
import { useRibbonImportedMeshBridge, type UseRibbonImportedMeshBridgeProps } from '../bim/hooks/use-ribbon-imported-mesh-bridge';
...
import { useRibbonGenericSolidBridge } from '../ui/ribbon/hooks/useRibbonGenericSolidBridge';
import { useRibbonBlockLibraryBridge } from '../ui/ribbon/hooks/useRibbonBlockLibraryBridge';
import { useRibbonTitleBlockBridge } from '../ui/ribbon/hooks/useRibbonTitleBlockBridge';
...
import { useRibbonWallCoveringBridge } from '../ui/ribbon/hooks/useRibbonWallCoveringBridge';
import { useRibbonHatchBridge } from '../ui/ribbon/hooks/useRibbonHatchBridge';
...
export type UseDxfBimBridgesProps =
  ...
  & UseRibbonImportedMeshBridgeProps
...
export function useDxfBimBridges(p: UseDxfBimBridgesProps) {
  ...
  // ADR-683 Φ3.1γ B2 — imported-mesh (εισαγόμενο πλέγμα) contextual properties bridge
  // (position/elevation/rotation — the only legal edits, §10.1).
  const importedMeshBridge = useRibbonImportedMeshBridge(p);
  ...
  // ADR-684 — generic-solid (παραμετρικά στερεά) dual-mode bridge (Φ4-B): επιλεγμένο στερεό
  // (edit μέσω UpdateGenericSolidParamsCommand) ↔ tool-active defaults (shape selector + per-shape dims).
  const genericSolidBridge = useRibbonGenericSolidBridge(p);
  // ADR-652 M1.5 — Block Library contextual bridge (tool-active, NUMERIC-ONLY: rotation/scale·
  // το «ποιο block» το κατέχει το palette, όχι το ribbon).
  const blockLibraryBridge = useRibbonBlockLibraryBridge();
  // ADR-651 Φάση Γ — «Πινακίδα Σχεδίου»: preset/χαρτί/κορνίζα (options store) + rotation/scale
  // (tool handle). Καμία εξάρτηση από selection/levelManager — drawing-mode only.
  const titleBlockBridge = useRibbonTitleBlockBridge();
  ...
  // ADR-511 — wall covering (IfcCovering CLADDING/INTERIOR) contextual properties bridge.
  const wallCoveringBridge = useRibbonWallCoveringBridge(p);
  // ADR-507 S2 — γραμμοσκίαση (hatch) contextual properties bridge (dual mode:
  // selected entity ↔ draw-defaults).
  const hatchBridge = useRibbonHatchBridge(p);
