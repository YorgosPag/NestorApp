...
// ADR-652 M6 — «Δημιουργία Block» action interceptor (opens the create dialog for the selection).
import { useCreateBlockRibbonAction } from '../ui/ribbon/hooks/useCreateBlockRibbonAction';
...
// 🔴 ADR-739 §52 — οι δύο contextual καρτέλες πίνακα. Self-contained όπως το scale tool: όλη
// η κατάσταση ζει στη θύρα `table-format-port` (που δημοσιεύει ο `CanvasSection`), εδώ μπαίνει
// μόνο η **μία** συνδρομή στον αριθμό έκδοσής της.
import { useRibbonTableFormatBridge } from '../ui/ribbon/hooks/useRibbonTableFormatBridge';
...
export function useDxfViewerRibbon(params: DxfViewerRibbonParams): DxfViewerRibbonReturn {
  ...
  // ADR-652 M6 — «Δημιουργία Block» wraps the image-dimension interceptor (top of chain). Καταγράφει
  // την επιλογή και ανοίγει τον διάλογο· το build/replace/save ζει στο CreateBlockDialogHost.
  const createBlockActionInterceptor = useCreateBlockRibbonAction({
    universalSelection,
    fallback: imageDimensionActionInterceptor,
  });
  ...
  // ADR-739 §52 — «Ιδιότητες Πίνακα» + «Μορφοποίηση» ↔ `table-format-port`.
  const tableFormatBridge = useRibbonTableFormatBridge();
  ...
  const ribbonCommands = useRibbonCommands({
    ...
    wrappedHandleAction: createBlockActionInterceptor,
    closeContextualTab,
    canUndo, canRedo,
    textEditorBridge, arrayBridge, stairBridge, railingBridge, importedMeshBridge, wallBridge, openingBridge, slabBridge, roofBridge, floorFinishBridge, wallCoveringBridge, hatchBridge, thermalSpaceBridge, columnBridge, beamBridge, foundationBridge,
