...
export interface TextPrimitive {
  ...
  /**
   * 🔴 ADR-739 §59 Δ1 — **γωνία του κειμένου** σε μοίρες, θετικές αριστερόστροφα. Απούσα ⇒
   * οριζόντιο, δηλαδή **κάθε** primitive που παράγει σήμερα το έργο.
   *
   * ## Γιατί εδώ και όχι στην οντότητα
   * Ίδιος λόγος με τα `bold`/`italic`/`fontFamily`: το `DetailPrimitive` είναι το **κοινό**
   * λεξιλόγιο των τεσσάρων backends (καμβάς · PDF · DXF/σκηνή · ΤΕΚ), και ό,τι δεν δηλώνεται
   * εδώ πρέπει να ξαναγεννηθεί χωριστά σε καθένα — δηλαδή τέσσερις ευκαιρίες να διαφωνήσουν.
   * Ο μοναδικός σημερινός παραγωγός είναι ο πίνακας (`table-layout-to-primitives`), αλλά η
   * γωνία είναι ερώτηση **κειμένου**, όχι πίνακα: το επόμενο σύμβολο σημείωσης που θα τη
   * χρειαστεί δεν πρέπει να προσθέσει δεύτερο πεδίο.
   *
   * ⚠️ **Προστίθεται** σε όποια γωνία έχει ήδη η οντότητα-ξενιστής (π.χ. `TableEntity.angleRad`)·
   * δεν την αντικαθιστά. Η πρόσθεση γίνεται στον μεταφραστή (`mapTablePrimitive`), όχι εδώ.
   */
