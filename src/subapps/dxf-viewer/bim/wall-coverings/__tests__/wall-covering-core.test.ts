...
describe('material catalog', () => {
  ...
  // ADR-534 Φ4 — νέα υλικά οροφής (κίτρινο + σπατουλαριστό) στο shared catalog.
  it('ADR-534 Φ4: paint-yellow + plaster-spackle resolve χρώμα/hatch + labelKey', () => {
    expect(getWallCoveringColor('paint-yellow')).toBe('#F4D03F');
    expect(getWallCoveringMaterial('paint-yellow')?.labelKeySuffix).toBe('paintYellow');
    expect(getWallCoveringColor('plaster-spackle')).toBe('#E8DFD0');
    expect(getWallCoveringMaterial('plaster-spackle')?.hatch).toBe('plaster');
    expect(getWallCoveringDefaultThicknessMm('plaster-spackle')).toBe(2);
  });

  it('ADR-534 Φ4: σπατουλαριστό μετρά ως plaster στο resolveWallCoveringKind', () => {
    expect(resolveWallCoveringKind(layers({ materialId: 'plaster-spackle', thicknessMm: 2, function: 'surface' }))).toBe('plaster');
  });
