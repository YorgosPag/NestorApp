...
/**
 * ADR-408 Φ12 — Fire-and-forget audit client for plumbing manifold create /
...
import { MEP_MANIFOLD_TRACKED_FIELDS } from '@/config/audit-tracked-fields';
...
export type MepManifoldAuditAction = 'created' | 'updated' | 'deleted' | 'restored';

export type MepManifoldAuditSnapshot = Pick<MepManifoldEntity, 'id' | 'kind'> & {
  readonly layerId?: string;
  readonly params?: Partial<MepManifoldEntity['params']>;
};

export interface RecordMepManifoldChangeOptions {
  readonly entityName?: string | null;
  readonly prevParams?: Partial<MepManifoldEntity['params']> | null;
}

export function recordMepManifoldChange(
  action: MepManifoldAuditAction,
  entity: MepManifoldAuditSnapshot,
  options?: RecordMepManifoldChangeOptions,
): void {
...
function buildChanges(
  action: MepManifoldAuditAction,
  entity: MepManifoldAuditSnapshot,
  prevParams: Partial<MepManifoldEntity['params']> | null,
  ...
  if (action === 'created' || action === 'restored') {
    return ensureNonEmptyChanges(
      buildBimCreationChanges(snapshot, MEP_MANIFOLD_TRACKED_FIELDS),
  ...
  if (action === 'deleted') {
    return ensureNonEmptyChanges(
      buildBimDeletionChanges(snapshot, MEP_MANIFOLD_TRACKED_FIELDS),
  ...
  const changes = buildBimUpdateChanges(prevSnapshot, snapshot, MEP_MANIFOLD_TRACKED_FIELDS);
