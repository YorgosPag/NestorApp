...
/**
 * Point-in-polygon test — **crossing-number ray casting** (XY plane, z αγνοείται).
 *
 * ⚠️ **ΣΗΜΑΣΙΟΛΟΓΙΑ — διάβασέ την πριν την καταναλώσεις**:
 * - Σημείο **γνήσια εντός** → `true`. Σημείο **γνήσια εκτός** → `false`.
 * - Σημείο **ΠΑΝΩ στην ακμή ή σε κορυφή** → **ΑΠΡΟΣΔΙΟΡΙΣΤΟ**. Το αποτέλεσμα
 *   εξαρτάται από τη φορά της ακμής, από το ποια πλευρά της ανισότητας πέφτει η
 *   κορυφή (`yi > point.y !== yj > point.y` = half-open κανόνας) και από τον φρουρό
 *   `|| 1e-12` του παρονομαστή σε οριζόντιες ακμές. **ΔΕΝ υπάρχει ανοχή (tolerance)**
 *   και **ΔΕΝ υπάρχει έλεγχος ακμής** — μην υποθέσεις ότι το σύνορο μετράει ως «μέσα».
 * - Αντιστοιχεί περίπου στο OGC `within` (γνήσια εντός), **ΟΧΙ** στο `covers`
 *   (εντός ή επί του συνόρου).
 *
 * 📌 Καταναλωτής που **χρειάζεται** ντετερμινιστική απάντηση για σημείο επί του
 * συνόρου (π.χ. τοπογραφική βολή πάνω στη γραμμή του οικοπέδου) **ΔΕΝ** πρέπει να
 * χρησιμοποιήσει αυτή τη συνάρτηση χωρίς ρητό έλεγχο απόστασης από τις ακμές.
 *
