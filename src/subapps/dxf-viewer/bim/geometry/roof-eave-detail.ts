...
export function buildRoofEaveDetail(cfg: RoofEaveDetailInput): RoofEaveDetail {
  ...
  // Ανά ακμή: frame (κορυφές + εξωτερικό κάθετο + κυρίαρχο νερό για z/quads).
  const frames: EdgeFrame[] = [];
  for (let i = 0; i < n; i++) {
    frames.push(buildEdgeFrame(verts, i, sign, planes));
  }

  // Mitered εξωτερικό δαχτυλίδι — SSoT (`roofEaveOuterRing`, κοινό με τις έλξεις
  // γείσου). Ίδιος split/order με τα `frames` → `miter[i]` αντιστοιχεί στο frame `i`.
  const miter = roofEaveOuterRing(baseVerts, cfg.edges, cfg.s, cfg.ridges ?? []);
