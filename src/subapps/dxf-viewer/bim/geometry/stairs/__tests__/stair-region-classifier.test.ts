...
describe('buildStairParamsFromRegion — walkline-driven sketch StairParams (STEP 5)', () => {
  ...
  it('ίσιος (χωράει) → variant STRAIGHT (Revit: ορθογώνιο = κανονική σκάλα), stepCount = N_risers, full parametric', () => {
    const c = classifyStairRegion(straightCorridor(6000, 1200));
    const params = buildStairParamsFromRegion(c, 'mm');
    // ADR-619 §straight-region — ένα ορθογώνιο αποτύπωμα βγαίνει ΚΑΝΟΝΙΚΗ 'straight' σκάλα
    // (8 λαβές + move + rotation + πλήρες resize), ΟΧΙ παγωμένο walkline sketch.
    expect(params.variant.kind).toBe('straight');
    expect(params.stepCount).toBe(17); // N_risers (straight: treads = stepCount-1 = N_goings)
    expect(params.totalRun).toBeCloseTo(280 * 16, 3); // tread·(stepCount-1) = going·N_goings
    ...
    // Integration: το straight geometry παράγεται χωρίς throw.
  ...
  it('κοντός ίσιος διάδρομος → tread συμπιεσμένο (< default), straight variant, geometry OK', () => {
    const c = classifyStairRegion(straightCorridor(3000, 1200));
    const params = buildStairParamsFromRegion(c, 'mm');
    expect(params.variant.kind).toBe('straight');
    expect(params.stepCount).toBe(17); // N_risers
    expect(params.tread).toBeCloseTo(3000 / 16, 3); // 187.5 συμπιεσμένο (going = L / N_goings)
...
describe('buildStairParamsFromRegion — multi-flight fill + πλατύσκαλα (bug #2, ADR-619)', () => {
  ...
  it('μακρύς ίσιος διάδρομος → going παραμένει ~280 (όχι απλωμένο σε όλο το μήκος), straight variant', () => {
    ...
    expect(params.variant.kind).toBe('straight');
    // Η σκάλα καταλαμβάνει ~4.48μ (16×280), όχι τα 20μ — totalRun = tread·(stepCount-1).
    expect(params.totalRun).toBeCloseTo(280 * 16, 1);
    expect(params.totalRun).toBeLessThan(6000);
  });
});
