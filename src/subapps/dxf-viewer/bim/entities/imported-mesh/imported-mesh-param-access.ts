...
const K = { ...IMPORTED_MESH_NUMBER_KEYS, ...IMPORTED_MESH_READONLY_KEYS };
 ...
 * of the four writable numeric keys (`posX`/`posY`/`elevation`/`rotation`); returns the SAME
 * reference (no-op) for every read-only key (`name`/`width`/`depth`/`height`/`level`/
 * `currentMaterial`/`materialSlots`/`boq*`), an unknown key, or non-finite numeric input.
 *
 ...
 * distinguish "rejected" from "applied" should gate on `isImportedMeshRibbonKey` themselves before
 * calling (exactly as `use-ribbon-railing-bridge.ts` gates on `isRailingRibbonKey` /
 * `isRailingRibbonStringKey` before calling `patchRailingField`).
 */
export function patchImportedMeshField(
  ...
  if (!isImportedMeshRibbonKey(key)) return p;
  return patchNumberField(key, p, Number.parseFloat(value));
}
