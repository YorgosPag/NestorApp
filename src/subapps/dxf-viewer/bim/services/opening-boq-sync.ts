...
async function writeSignatureGroup(
  ...
  // Serialize the read-modify-write per group id. On a MASS opening delete the
  // hook fires N parallel `deleteOpeningFromGroup` for the same signature group;
  // without this all N read `exists() === true` and all N `deleteDoc` → the
  // losers hit a now-missing doc, which the Firestore delete rule surfaces as a
  // false "Missing or insufficient permissions" (it reads `resource.data` on a
  // null resource). Same race on upsert → wrong `estimatedQuantity` (ADR-634).
  return boqRowWriteQueue.run(groupId, async () => {
    ...
    if (members.length === 0) {
      ...
      if (existing && existing.exists()) {
        await deleteBoqRowIdempotent(doc(db, COLLECTIONS.BOQ_ITEMS, groupId), 'OpeningBoqSync: signature-group', { groupId });
      }
      return;
    }
