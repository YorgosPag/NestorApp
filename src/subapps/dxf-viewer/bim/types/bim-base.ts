...
/**
 * **Ίχνος κάτοψης** — άξονο-ευθυγραμμισμένο κουτί στο XY, **ΧΩΡΙΣ z**, και αυτό
 * *είναι* το νόημα: το ίχνος δεν έχει γνώμη για ύψος.
 *
 * 🔑 **Γιατί υπάρχει** (ADR-793): μέχρι 2026-08-22 υπήρχε **ένας** τύπος `BimBounds`
 * που σήμαινε **δύο** πράγματα. Μετρημένο σε 44 αρχεία: **7** παραγωγοί έγραφαν
 * γνήσιο z σε **μέτρα**, **ένας** (railing) σε **χιλιοστά**, και **11** σημεία
 * έγραφαν `z: 0` επειδή το ίχνος δεν έχει ύψος — με το πραγματικό ύψος να ζει στο
 * **αδελφό πεδίο του ίδιου αντικειμένου** (`RectangularBodyGeometry.height`). Τη
 * διαφορά τη γνώριζε **ένα σχόλιο** και μια χειρόγραφη αλυσίδα `if`
 * (`entity-world-aabb.ts`) — το σχήμα που σε αυτό το repo έχει αποτύχει μετρημένα
 * στα CHECK 3.34 (63) · 3.37 (18 vs 26) · 3.49 (60).
 *
 * 🏆 **Ο κανόνας είναι ομόφωνος: 2Δ και 3Δ κουτί έχουν ΠΑΝΤΑ άλλο όνομα.**
 * Revit `BoundingBoxUV`/`BoundingBoxXYZ` · ArchiCAD `BoundingBox2D`/`API_Box3D` ·
 * three.js `Box2`/`Box3` · JTS `Envelope` (**μόνο** 2Δ — 3Δ δεν υπάρχει στον πυρήνα).
 * **Κανείς** δεν έχει τύπο του οποίου η τρίτη διάσταση *μπορεί* να σημαίνει κάτι.
 *
 * ⚠️ Το σχήμα `{min,max}` με **σημεία** (αντί για βαθμωτά, όπως το {@link Bbox} του
 * `xy-bounds`) διατηρείται **επίτηδες**: είναι το σχήμα που **ΕΙΝΑΙ ΗΔΗ ΓΡΑΜΜΕΝΟ ΣΤΟ
 * FIRESTORE** για `floor-finish` · `thermal-space` · `wall-covering` — τους **μόνους
 * τρεις** τύπους που επιμένουν το `geometry` (οι υπόλοιποι το ξαναϋπολογίζουν στο
 * hydrate). Η αφαίρεση του `z` είναι **μηδενική μετανάστευση**: τα υπάρχοντα έγγραφα
 * κουβαλούν ένα `z: 0` που πλέον **κανείς δεν μπορεί να διαβάσει**.
 *
 * @see docs/centralized-systems/reference/adrs/ADR-793-bounds-dimension-vocabulary.md
 */
export interface PlanBounds {
  readonly min: PlanarPoint;
  readonly max: PlanarPoint;
}

/**
 * **Στερεό κουτί** — ίχνος κάτοψης **συν** κατακόρυφο εύρος, με τη **μονάδα στο όνομα**.
 *
 * 🔑 **Η μονάδα ΕΙΝΑΙ μέρος του τύπου, και εκεί ξεπερνάμε τους μεγάλους.** Όλοι
 * ονοματίζουν τη *διάσταση*· **κανείς** δεν ονοματίζει τη *μονάδα* — και η σύγχυση
 * μονάδων είναι η #1 κατηγορία σφαλμάτων του Revit API (εσωτερικά πάντα πόδια).
 * Εδώ ήταν **ζωντανή**: το `railing-geometry` έγραφε `min.z = baseElevationMm` (ωμά
 * **χιλιοστά**) στον **ίδιο** τύπο όπου τοίχος/πλάκα/δοκός γράφουν **μέτρα** — σφάλμα
 * **1000×**, αόρατο μόνο επειδή κανείς δεν διάβαζε το z του railing. Το `minZm`/`maxZm`
 * κάνει την επανάληψή του **αδύνατη**: ο μεταγλωττιστής απαιτεί το όνομα με το `m`.
 *
 * ⚠️ Τα `min`/`max` μένουν σε **μονάδες καμβά** (scene units), όπως παντού στο ίχνος —
 * το z **μόνο** είναι σε μέτρα. Αυτή η ασυμμετρία **υπήρχε ήδη**· ήταν γραμμένη σε
 * πρόζα σε **ένα** αρχείο (`entity-world-aabb.ts:5`) και τώρα τη λέει ο τύπος.
 *
 * ⚠️ Ένα `SolidBounds` **είναι** και `PlanBounds` (κάθε στερεό έχει ίχνος)· το
 * αντίστροφο **όχι** — και αυτό είναι ο φρουρός: ένα ίχνος δεν μπορεί να περάσει
 * εκεί που ζητείται ύψος.
 */
export interface SolidBounds extends PlanBounds {
  /** m. Κάτω παρειά. */
