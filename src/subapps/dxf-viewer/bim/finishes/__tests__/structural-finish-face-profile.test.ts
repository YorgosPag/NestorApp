...
describe('ADR-449/534 Φ7 — buildFaceProfile: unified όψη με τρύπα στο άνοιγμα', () => {
  ...
  it('CORNER-JOIN: L-γωνία (Ν+Α όψεις) → κάθε profile επεκτείνεται κατά το πάχος στη γωνία (overlap)', () => {
    ...
    // Χωρίς corner-join κάθε όψη = 0.1m κατά μήκος· με join το junction άκρο +25mm → 0.125m.
    for (const p of profiles) {
      expect(tSpan(p.polygons[0].outer)).toBeCloseTo(0.125, 6);
    }
  });
