...
describe('ADR-449/534 Φ7 — buildFaceProfile: unified όψη με τρύπα στο άνοιγμα', () => {
  ...
  it('CORNER-MITER (Φ7c): L-γωνία (Ν+Α όψεις) → body στο core-length· το miter είναι back-cap delta', () => {
    ...
    // Το (t,z) profile polygon ΜΕΝΕΙ στο core-length 0.1m (το miter δεν αλλάζει το 2Δ profile —
    // εφαρμόζεται ως back-cap vertex shift στο extrude· βλ. structural-finish-corner-overlap-diag).
    for (const p of profiles) {
      expect(tSpan(p.polygons[0].outer)).toBeCloseTo(0.1, 6);
    }
    // Κάθε όψη ΦΕΡΕΙ το ενσωματωμένο miter του: ΕΝΑ shift (η γωνία, ±25mm) — το ελεύθερο άκρο = μηδέν shift.
    for (const p of profiles) {
      expect(p.miter).toHaveLength(1);
      expect(Math.abs(p.miter[0].deltaM)).toBeCloseTo(0.025, 6);
    }
  });

  it('CONCAVE CORNER (Φ7d): reflex L-γωνία → κάθε όψη ΜΙΤΕΡΑΡΕΙ (πριν το Φ7d: 0 shifts → επικάλυψη «X»)', () => {
    // faceA x[0,100] (dir +x)· faceB x=100 y[0,−100] (dir −y)· μοιράζονται τη γωνία (100,0). Η στροφή
    // είναι ΔΕΞΙΑ → **REFLEX/concave** γωνία: το `computeMiteredOuter` TRIM-άρει το outer προς τα μέσα
    // (mPt = (100−t,−t)), αντί να το επεκτείνει. Πριν το Φ7d το gate (`beyond*`) το απέρριπτε → 0 shifts
    // → οι δύο λωρίδες επικαλύπτονταν στη γωνία (το «X» του Giorgio στο C4D).
    const a = mkSeg({ x: 0, y: 0 }, { x: 100, y: 0 });
    const b = mkSeg({ x: 100, y: 0 }, { x: 100, y: -100 });
    const bands: SilhouetteBand[] = [mkBand([a, b], 0, 3000)];
    const profiles = buildFaceProfiles(mergeSilhouetteBandsToStripGroups(bands, 'mm'), SCENE_TO_M);
    expect(profiles).toHaveLength(2);
    // Φ7d: η concave γωνία ΜΙΤΕΡΑΡΕΙ πλέον — κάθε όψη φέρει 1 shift, |delta| = πάχος (25mm), αρνητικό (μέσα).
    for (const p of profiles) {
      expect(p.miter).toHaveLength(1);
      expect(Math.abs(p.miter[0].deltaM)).toBeCloseTo(0.025, 6);
    }
