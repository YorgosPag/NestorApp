...
describe('ADR-449/534 Φ7 — buildFaceProfile: unified όψη με τρύπα στο άνοιγμα', () => {
  ...
  it('CORNER-MITER (Φ7c): L-γωνία (Ν+Α όψεις) → body στο core-length· το miter είναι back-cap delta', () => {
    ...
    // Κάθε όψη ΦΕΡΕΙ το ενσωματωμένο miter του: μία γωνία (±25mm έξω) + ένα ελεύθερο άκρο (0).
    for (const p of profiles) {
      const nonZero = [p.miter.deltaLoM, p.miter.deltaHiM].filter((d) => Math.abs(d) > 1e-9);
      expect(nonZero).toHaveLength(1);
      expect(Math.abs(nonZero[0])).toBeCloseTo(0.025, 6);
    }
  });
