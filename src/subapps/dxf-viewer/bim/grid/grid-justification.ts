/**
 * ADR-441 — Κανόνας αυτόματης έδρασης άξονα (justification) «Εσχάρας από κάναβο».
 *
 * **Ενιαία πηγή αλήθειας (SSoT)** για το ΠΟΥ κάθεται κάθε grid-managed στοιχείο ως προς
 * τον άξονά του κατά τη γένεση από τον κάναβο (Revit managed grid). Κοινό για ΟΛΕΣ τις
 * δομικές οντότητες: πεδιλοδοκοί, **δοκάρια, τοίχοι** (γραμμικά → perpendicular offset)
 * και **κολόνες** (σημειακά → anchor, μέσω `grid-column-justification.ts`).
 *
 * Ζούσε στο `foundations/foundation-grid-justification.ts` (foundation-only)· μεταφέρθηκε
 * εδώ (neutral `bim/grid/`) ώστε col/beam/wall να μην εξαρτώνται από το foundations module
 * (ADR-441 3-mode generalization, Slice 0). Το παλιό path παραμένει ως re-export (back-compat).
 *
 * Δομικά σωστό default:
 ...
 * + CCW normal στο `canonicalAxisNormal`):
 ...
 *
 * @see ./grid-segment-justification.ts — linear perpendicular offset (beam/wall)
 * @see ./grid-column-justification.ts — point anchor mapping (column)
