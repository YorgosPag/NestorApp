...
import type { MepElectricalSystemParams, MepSystemEntity } from '../../types/mep-system-types';

const SEG = buildSegmentKey('a', 'b');

function makeSystem(params: Partial<MepElectricalSystemParams> = {}): MepSystemEntity {
  const electricalParams: MepElectricalSystemParams = {
  ...
  return { id: 's1', params: electricalParams };
}

describe('applyWaypointGesture', () => {
  it('inserts a new vertex into an empty segment', () => {
    const system = makeSystem();
    const gesture: WaypointGesture = { mode: 'insert', system, startParams: system.params as MepElectricalSystemParams, keyA: 'a', keyB: 'b', orientedIndex: 0 };
  ...
  it('moves an existing vertex without touching others', () => {
    ...
    const gesture: WaypointGesture = { mode: 'move', system, startParams: system.params as MepElectricalSystemParams, keyA: 'a', keyB: 'b', orientedIndex: 1 };
