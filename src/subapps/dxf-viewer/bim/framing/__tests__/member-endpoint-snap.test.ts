/**
 * ADR-508 — `resolveMemberEndpointSnap`: το ΑΚΡΟ ενός γραμμικού μέλους (τοίχος/δοκάρι) κουμπώνει flush
 * σε παρειά μέλους/κολώνας (point snap, reuse του κοινού dispatcher). Scene units = mm. Η precedence vs
 * length/angle lock ζει στον caller (`useWallTool`) — εδώ ελέγχουμε μόνο το pure snap/no-snap + fine step.
 * Κοινός κώδικας τοίχου ↔ δοκαριού (Giorgio 2026-06-24).
 */

import { resolveMemberEndpointSnap, resolveMemberEndpointWithFineStep } from '../member-endpoint-snap';
...
describe('resolveMemberEndpointSnap', () => {
  it('Άκρο κοντά σε ΚΟΛΩΝΑ → κουμπώνει flush στην παρειά (point = snap.start) + faceFrame', () => {
    const r = resolveMemberEndpointSnap({ x: 500, y: 200 }, [COLUMN_FP], [], 200, 'mm');
  ...
  it('Άκρο κοντά σε ΜΕΛΟΣ → κουμπώνει flush στην παρειά (ολίσθηση κατά μήκος)', () => {
    const r = resolveMemberEndpointSnap({ x: 5000, y: 150 }, [], [MEMBER], 200, 'mm');
  ...
  it('Άκρο ΕΚΤΟΣ capture → επιστρέφει το rawEnd αυτούσιο, χωρίς faceFrame', () => {
    ...
    const r = resolveMemberEndpointSnap(raw, [COLUMN_FP], [MEMBER], 200, 'mm');
  ...
  it('Κολόνα → proportional fine βήμα στο endpoint (όχι 12-jump)', () => {
    ...
    const r = resolveMemberEndpointSnap({ x: 500, y: 252 }, [COLUMN_FP], [], 200, 'mm');
...
describe('resolveMemberEndpointWithFineStep — Shift 1cm βήμα στο ελεύθερο (face-snap νικά)', () => {
  ...
  it('face-snap υπάρχει → επιστρέφει το flush σημείο αυτούσιο ακόμη και με Shift', () => {
    ...
    immediateSceneScale.set(1);
    const out = resolveMemberEndpointWithFineStep({ point: { x: 1043, y: 0 }, faceFrame: FACE }, START);
    expect(out).toEqual({ x: 1043, y: 0 }); // face νικά → ΟΧΙ 1cm
  });

  it('ελεύθερο + Shift κάτω → no-op (raw endpoint)', () => {
    ShiftKeyTracker._setForTest(false);
    const out = resolveMemberEndpointWithFineStep({ point: { x: 1047, y: -23 } }, START);
  ...
  it('ελεύθερο + Shift πάνω → μετατροπή 1cm σε scene units (metre-scale)', () => {
    ...
    ShiftKeyTracker._setForTest(true);
    const out = resolveMemberEndpointWithFineStep({ point: { x: 1.047, y: 0 } }, { x: 1.0, y: 0 });
