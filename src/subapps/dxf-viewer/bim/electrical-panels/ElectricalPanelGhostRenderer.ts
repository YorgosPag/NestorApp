/**
 * ElectricalPanelGhostRenderer — ADR-408 Φ3 (2D placement ghost).
 *
 * Paints the translucent panel footprint preview directly onto the preview
 * canvas while the electrical panel tool awaits a position — the 2D counterpart
 * of the 3D `ElectricalPanelPlacementGhost`, mirror of `MepFixtureGhostRenderer`.
 ...
 *
 * Palette mirrors `ElectricalPanelRenderer` (teal) so the preview reads as
...
export interface ElectricalPanelGhostRenderInput {
...
export class ElectricalPanelGhostRenderer {
  constructor(private readonly ctx: CanvasRenderingContext2D) {}

  render(input: Readonly<ElectricalPanelGhostRenderInput>): void {
