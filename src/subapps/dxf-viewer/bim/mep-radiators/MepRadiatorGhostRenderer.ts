/**
 * MepRadiatorGhostRenderer — ADR-408 Εύρος Β #1 (2D placement ghost).
 *
 * Paints the translucent radiator footprint preview directly onto the preview
 * canvas while the heating radiator tool awaits a position — the 2D counterpart of
 * the 3D `MepRadiatorPlacementGhost`, mirror of `MepManifoldGhostRenderer`. Pure
 ...
 *
 * Palette mirrors `MepRadiatorRenderer` (warm-red) so the preview reads as "exactly
...
/** Warm-red radiator palette — mirror of `MepRadiatorRenderer`. */
const RADIATOR_STROKE = '#dc2626';
const RADIATOR_FILL = 'rgba(220, 38, 38, 0.28)';
const GHOST_LINE_WIDTH = 2;
const ANCHOR_MARKER_SIZE_PX = 5;

export interface MepRadiatorGhostRenderInput {
...
export class MepRadiatorGhostRenderer {
  constructor(private readonly ctx: CanvasRenderingContext2D) {}

  render(input: Readonly<MepRadiatorGhostRenderInput>): void {
  ...
  private drawOutline(
    ...
    const ctx = this.ctx;
    ctx.save();
    ctx.strokeStyle = RADIATOR_STROKE;
