...
/**
 * `true` όταν το κελί έχει **πραγματικά** μορφοποίηση ανά χαρακτήρα — δηλαδή όταν τα τμήματα
 * λένε κάτι που το `TableTextRun` δεν λέει ήδη μόνο του.
 *
 ...
 */
export function hasStyledSpans(spans: readonly TableTextStyleSpan[]): boolean {
 ...
 *
 * ⚠️ Ποτέ `JSON.stringify`: το `fontFamily` είναι προαιρετικό, οπότε δύο ίσες τυπογραφίες
 * μπορούν να έχουν άλλη σειρά ή άλλο πλήθος κλειδιών ανάλογα με το ποιο επίπεδο απάντησε.
 * Είναι η **ίδια** παγίδα που η Φ1 ανακάλυψε ότι δεν ελεγχόταν από κανένα test (ADR-753 §8)
 * — και ο λόγος που εκείνη η μετάλλαξη επέζησε ήταν ακριβώς ότι κάθε δείγμα γεννιόταν από
 * τον ίδιο δρόμο, άρα με ίδια σειρά κλειδιών.
 */
