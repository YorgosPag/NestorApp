...
/**
 * 🔴 ADR-753 §25 — **δείκτες ΜΑΖΙ με το κείμενο πάνω στο οποίο μετρήθηκαν.**
 *
 * ## Γιατί υπάρχει ως τύπος και δεν είναι δύο ορίσματα
 * Δύο αριθμοί δεν μπορούν να πουν αν εξακολουθούν να δείχνουν στα ίδια γράμματα: το `[2,4)`
 * είναι **εξίσου έγκυρο** πάνω σε «ΝΕΣΤ» και πάνω σε «ΝΕΣΤΩΡ», και δείχνει στα ίδια γράμματα
 * μόνο κατά σύμπτωση. Η μόνη απόδειξη είναι το κείμενο πάνω στο οποίο μετρήθηκαν — άρα οφείλει
 * να **ταξιδεύει μαζί τους**, όχι να ζητιέται χωριστά από κάθε καλούντα (και ο δεύτερος θα το
 * ζητούσε από **άλλη** πηγή· αυτό ακριβώς έγινε, δες §25).
 *
 * Ίδια αρχή με το compare-and-swap του ADR-769: **η βάση σύγκρισης ταξιδεύει με το αίτημα.**
 *
 * ⚠️ Ο τύπος δεν επιβάλλει από μόνος του τίποτα — επιβάλλει το ότι **κανείς δεν μπορεί να
 * κατασκευάσει στόχο-γράμματα χωρίς να δηλώσει τη βάση**. Την επαλήθευση την κάνει ο ΕΝΑΣ
 * γραφέας ({@link import('./table-chars-style-ops')}), εκεί όπου η βάση συγκρίνεται με το
 * **πραγματικό** κείμενο του κελιού.
 */
export interface TableTextAnchoredRange extends TableTextRange {
  /** Το κείμενο του πεδίου τη στιγμή που διαβάστηκαν τα {@link TableTextRange.start}/`end`. */
