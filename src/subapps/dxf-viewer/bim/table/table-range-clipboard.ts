...
function writeGridCells(
  ...
  for (let r = 0; r < fitted.rows; r++) {
    for (let c = 0; c < fitted.columns; c++) {
      ...
      //
      // ⚠️ **Δηλωμένη ισοδύναμη μετάλλαξη (M38, Φ5Β)**: το να δοθεί στο `writeCellInput` το
      // **ακοπο** `value` αντί για το `clipped.text` **δεν αλλάζει τίποτα** — ο γραφέας κόβει
      // ούτως ή άλλως. Και αυτό είναι ακριβώς το ζητούμενο: η **επιβολή** έχει ένα σπίτι
      // (M23 τη σκοτώνει εκεί), εδώ γίνεται μόνο η **μέτρηση**. Το `clipped.text` περνά
      // παρ' όλα αυτά, ώστε ο αναγνώστης να μη χρειάζεται να ξέρει τι κάνει ο γραφέας για να
