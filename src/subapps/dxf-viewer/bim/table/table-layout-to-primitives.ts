...
export function tableLayoutToPrimitives(
  ...
  for (const row of layout.rows) {
    ...
    for (const cell of byRow.get(row.id) ?? []) {
      // 🔴 ADR-739 §58 Γ2 — **όλες** οι οπτικές γραμμές. Ο εξωτερικός βρόχος είναι η μόνη
      // αλλαγή που χρειάστηκε η εξαγωγή: κάθε γραμμή είναι ήδη έτοιμο `TableTextRun` με δική
      // της θέση, άρα το PDF και το DXF παίρνουν N οντότητες TEXT **ήδη σπασμένες**.
      //
      // 🔑 Αυτό είναι **σωστότερο** από ένα MTEXT με πλάτος στήλης (κωδ. 41): η τεκμηρίωση του
      // μορφότυπου δηλώνει ρητά ότι η αναδίπλωση MTEXT **αποδίδεται διαφορετικά ανά εφαρμογή**
      // — δηλαδή το ίδιο αρχείο θα έσπαγε αλλού στο AutoCAD και αλλού στην οθόνη μας, και η
      // αρχή «οθόνη === PDF === DXF === σκηνή» (§3) θα έπαυε να είναι δομή.
