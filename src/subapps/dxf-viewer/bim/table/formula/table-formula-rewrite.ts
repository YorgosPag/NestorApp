...
export function rewriteTableFormulaRefs(
  ...
  switch (node.kind) {
    ...
    case 'boolean':
    case 'error':
    // 🔴 ADR-765 — ούτε το γυμνό όνομα ούτε το παραλειπόμενο όρισμα περιέχουν **διεύθυνση**,
    // άρα καμία μετακόμιση και καμία αντιγραφή δεν έχει τι να τους αλλάξει. Επιστρέφονται
    // αυτούσια, που είναι και η σύμβαση ταυτότητας αυτού του αρχείου.
    case 'name':
    case 'blank':
      return node;
  }
...
/**
 * 🔴 ADR-833 Φάση 7 — **η ΑΝΑΓΝΩΣΗ των φύλλων με διεύθυνση**, με την ίδια κάθοδο.
 *
 * ## Γιατί δεν είναι δεύτερη αναδρομή — και γιατί ΗΤΑΝ, μέχρι τώρα
 * Το `table-formula-recalc.ts` κρατούσε **δικό του** `walk` με πανομοιότυπο `switch` επτά
 * περιπτώσεων, γραμμένο πριν υπάρξει αυτό το module. Δηλαδή η κεφαλίδα από πάνω («είκοσι
 * γραμμές πανομοιότυπης αναδρομής σε δύο αρχεία είναι ο structural clone που πιάνει το
 * **CHECK 3.28**») περιέγραφε **υπαρκτό** δίδυμο, όχι υποθετικό — και η Φάση 7, που θέλει
 * **τρίτη** ανάγνωση (ποια φύλλα διαβάζει αυτό το πλέγμα;), θα το έκανε τριπλό.
 *
 * Η υλοποίηση είναι το {@link rewriteTableFormulaRefs} με **ταυτοτική** ξαναγραφή: το φύλλο
 * επιστρέφεται αυτούσιο, άρα η εγγύηση ταυτότητας κρατά σε **κάθε** κόμβο και δεν
 * κατασκευάζεται τίποτα. Μία αναδρομή, δύο ονόματα προθέσεων.
 */
export function forEachTableFormulaRefLeaf(
  node: TableFormulaNode,
  visit: (leaf: TableFormulaRefLeaf) => void,
): void {
  rewriteTableFormulaRefs(node, (leaf) => {
    visit(leaf);
    return leaf;
  });
}
