...
export function resolveVisibleCellText(input: CellTextFitInput): VisibleCellText {
  ...
  const spans = cellStyledSpansOf(input, input.text, input.text.length);
...
function clipToFit(
  ...
  return {
    text: cut.text,
    clipped: true,
    spans: cellStyledSpansOf(input, cut.text, cut.runsLimit),
    heightScale: 1,
  };
}
 ...
 *
 * ⚠️ Εξαγόμενο **μόνο** για το `table-cell-content.ts` (§58 Γ2): η αναδίπλωση χρειάζεται τα
 * τμήματα του **ακέραιου** κειμένου για να μετρήσει εύρη. Μια δεύτερη γραφή του ίδιου
 * τριγράμμου εκεί θα ήταν structural clone (N.18 / CHECK 3.28) — και, χειρότερα, δεύτερη
 * απάντηση στο «ποιο στυλ έχει κάθε χαρακτήρας».
 */
export function cellStyledSpansOf(
  input: CellTextFitInput,
  text: string,
  runsLimit: number,
