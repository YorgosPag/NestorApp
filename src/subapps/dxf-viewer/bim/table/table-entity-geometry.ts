...
/**
 * **Px οθόνης ανά sheet-mm** — η κλίμακα με την οποία ο πίνακας φτάνει στα μάτια.
 *
 * Μία γραμμή, αλλά με **δύο** καταναλωτές που ΠΡΕΠΕΙ να συμφωνούν απόλυτα: ο ζωγράφος του
 * καμβά (`TableRenderer` → `StampTableContext.pxPerMm`) και ο in-cell επεξεργαστής της Φ.Δ
 * βήμα 3, που στήνει DOM κουτί πάνω από το ίδιο κελί. Αν οι δύο εκφράσεις αποκλίνουν έστω
 * κατά έναν παράγοντα, το `<input>` δεν κάθεται πάνω στο κελί — και αυτό είναι ακριβώς το
 * ελάττωμα που το βήμα 3 διορθώνει. Γι' αυτό η έκφραση ζει **εδώ**, δίπλα στο `mmToWorld`
 * που την τροφοδοτεί, και όχι σε δύο αρχεία.
 */
export function tablePxPerMm(mmToWorld: number, viewScale: number): number {
  return mmToWorld * viewScale;
}
