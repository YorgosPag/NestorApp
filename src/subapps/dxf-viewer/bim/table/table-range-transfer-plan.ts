...
/**
 * 🔴 §36 ΦΑΣΗ 3 — **ΤΟ ΟΡΘΟΓΩΝΙΟ ΠΟΥ ΔΕΙΧΝΕΙ ΤΟ ΠΟΝΤΙΚΙ**, ή `null` όταν δεν είναι σχεδιάσιμο.
 *
 * Το χρειάζεται **μόνο η άρνηση**. Όταν το σχέδιο πετυχαίνει, το φάντασμα ζωγραφίζει το
 * `plan.destination` και τίποτε άλλο (§36.5)· όταν απορρίπτεται, δεν υπάρχει `destination` —
 * και το να μη φανεί τίποτα θα διαβαζόταν ως «δεν καταλαβαίνει πού είμαι», ενώ η αλήθεια είναι
 * «σε καταλαβαίνω, **δεν γίνεται εδώ**».
 *
 * Είναι εξαγωγή των **ίδιων δύο** ιδιωτικών βημάτων που ήδη εκτελεί το
 * {@link planTableRangeTransfer}, όχι δεύτερη αριθμητική: μια χειρόγραφη «μετατόπιση + έλεγχος
 * ορίων» στη χειρονομία θα ήταν το sibling clone που πιάνει το CHECK 3.28 (N.18) — και θα
 * μπορούσε να αποκλίνει την πρώτη φορά που θα αλλάξει ο ορισμός του «χωράει».
 *
 * ⚠️ `null` και για **εκτός πλέγματος**, επίτηδες: το `tableRangeRectMm` **περιορίζει** τους
 * δείκτες στα όρια της διάταξης, οπότε ένα ορθογώνιο με αρνητική γραμμή θα ζωγραφιζόταν
 * κολλημένο στην πρώτη — δηλαδή θα έδειχνε προσγείωση που κανείς δεν ζήτησε.
 */
export function tableRangeDroppedRect(
  model: PersistedTableModel,
  source: TableCellRangeBounds,
  to: TableCellRef,
): TableCellRangeBounds | null {
  const dropped = droppedRectOf(model, source, to);
  return dropped && isInsideGrid(model, dropped) ? dropped : null;
}
