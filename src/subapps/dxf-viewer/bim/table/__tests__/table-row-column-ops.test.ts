...
describe('insertTableRow', () => {
  it('βάζει γραμμή δεδομένων στη ζητούμενη θέση', () => {
    const next = insertTableRow(bookOf(model()),model(), 3);
  ...
  it('η νέα γραμμή είναι ΠΑΝΤΑ `data`, ακόμα και πάνω από τον τίτλο', () => {
    const next = insertTableRow(bookOf(model()),model(), 0);
  ...
  it('δεν αγγίζει κανένα κελί — ο αραιός χάρτης δείχνει σε ταυτότητες', () => {
    const before = model();
    const next = insertTableRow(bookOf(before),before, 2);
  ...
  it('μεγαλώνει συγχώνευση που τη διαπερνά, αφήνει ήσυχη όποια είναι εκτός', () => {
    ...
    expect(insertTableRow(bookOf(base),base, 3).merges[0].rowSpan).toBe(3); // μέσα στο [r2..r3]
    expect(insertTableRow(bookOf(base),base, 1).merges[0].rowSpan).toBe(2); // πάνω από αυτό
    expect(insertTableRow(bookOf(base),base, 4).merges[0].rowSpan).toBe(2); // ακριβώς μετά — Excel: όχι
  });
  ...
  it('🔴 ADR-833 Φ5Β — στο πλήρες ΓΙΝΟΜΕΝΟ επιστρέφει το ΙΔΙΟ μοντέλο by-reference', () => {
    ...
    expect(insertTableRow(bookOf(full),full, 0)).toBe(full);
  });
...
describe('insertTableColumn', () => {
  it('βάζει στήλη και κληρονομεί το σχήμα της στήλης αναφοράς', () => {
    ...
    const next = insertTableColumn(bookOf(wide),wide, 0);
  ...
  it('🔴 ΠΟΤΕ δεν αντιγράφει `sourceKey` — δύο στήλες στην ίδια πηγή είναι σφάλμα δεσμού', () => {
    ...
    expect(insertTableColumn(bookOf(bound),bound, 1).columns[1].sourceKey).toBeUndefined();
  });

  it('🔴 ο τίτλος πλήρους πλάτους ΜΕΝΕΙ πλήρους πλάτους σε εισαγωγή δεξιά', () => {
    const next = insertTableColumn(bookOf(model()),model(), 3);
    expect(next.columns).toHaveLength(4);
    expect(titleSpan(next).colSpan).toBe(4);
  });

  it('εισαγωγή στη μέση μεγαλώνει επίσης τον τίτλο', () => {
    expect(titleSpan(insertTableColumn(bookOf(model()),model(), 1)).colSpan).toBe(4);
  });
  ...
  it('συγχώνευση που ΔΕΝ καλύπτει όλο τον άξονα δεν μεγαλώνει από δίπλα (κανόνας Excel)', () => {
    ...
    expect(insertTableColumn(bookOf(base),base, 2).merges[0].colSpan).toBe(2);
    expect(insertTableColumn(bookOf(base),base, 1).merges[0].colSpan).toBe(3);
  });
  ...
  it('🔴 ADR-833 Φ5Β — στο πλήρες ΓΙΝΟΜΕΝΟ επιστρέφει το ΙΔΙΟ μοντέλο by-reference', () => {
    ...
    expect(insertTableColumn(bookOf(full),full, 0)).toBe(full);
  });
...
describe('deleteTableRow', () => {
  it('σβήνει τη γραμμή ΚΑΙ τα κελιά της', () => {
    const next = deleteTableRow(bookOf(model()),model(), 'r3');
  ...
  it('άγνωστη ταυτότητα ⇒ το ΙΔΙΟ μοντέλο by-reference (κανένα βήμα undo για το τίποτα)', () => {
    const before = model();
    expect(deleteTableRow(bookOf(before),before, 'r99')).toBe(before);
  });
  ...
  it('η τελευταία γραμμή δεν διαγράφεται — πίνακας χωρίς ύψος είναι αόρατη οντότητα', () => {
    ...
    expect(deleteTableRow(bookOf(single),single, 'r0')).toBe(single);
  });
  ...
  it('μικραίνει συγχώνευση που περνά από τη σβησμένη γραμμή', () => {
    ...
    expect(deleteTableRow(bookOf(base),base, 'r3').merges[0].rowSpan).toBe(2);
  });
});
...
describe('deleteTableColumn', () => {
  ...
  it('🔴 διαγραφή της ΑΓΚΥΡΑΣ ξανα-αγκυρώνει τη συγχώνευση ΚΑΙ κουβαλά το περιεχόμενο', () => {
    const next = deleteTableColumn(bookOf(model()),model(), 'c0');
  ...
  it('συγχώνευση που εκφυλίζεται σε 1×1 παύει να υπάρχει', () => {
    ...
    expect(deleteTableColumn(bookOf(base),base, 'c0').merges).toHaveLength(0);
  });
  ...
  it('η τελευταία στήλη δεν διαγράφεται', () => {
    ...
    expect(deleteTableColumn(bookOf(single),single, 'c0')).toBe(single);
  });
});
