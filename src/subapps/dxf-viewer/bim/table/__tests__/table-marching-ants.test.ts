...
describe('🔴 ADR-739 §48 — ξεχωρίζει από το περίγραμμα λειτουργίας, που ζει ΤΑΥΤΟΧΡΟΝΑ', () => {
  it('🔴 ΔΕΝ είναι το μοτίβο του `TABLE_MODE_OUTLINE`', () => {
    // Τα δύο διακεκομμένα ζουν **ταυτόχρονα** γύρω από τα ίδια κελιά.
    //
    // 🔴 §48.13 — αυτό το test **ανέβηκε σε σημασία** στις 06/08: μέχρι τότε τα δύο μοτίβα τα
    // ξεχώριζε **και** η απόχρωση, οπότε ίδιο μοτίβο θα ήταν άσχημο αλλά όχι μοιραίο. Τώρα που
    // το χρώμα είναι κοινό (ρητή απόφαση ιδιοκτήτη), το μοτίβο είναι ένα από **τρία** κανάλια
    // που έμειναν — και το μόνο που επιβιώνει σε ακίνητο στιγμιότυπο μαζί με το πάχος.
    //
  ...
  it('🔴 §48.13 ΕΝΑ ΜΠΛΕ ΓΙΑ ΤΑ ΤΡΙΑ ΠΕΡΙΓΡΑΜΜΑΤΑ — δρομέας ≡ επιλογή ≡ μυρμήγκια', () => {
    // Ρητή απόφαση του ιδιοκτήτη (2026-08-06): τα μυρμήγκια παίρνουν το χρώμα **του
    // περιγράμματος που φέρει τη λαβή**. Έγινε εφικτό από το §48.12 — από τη στιγμή που το
    // συμπαγές περίγραμμα αποσύρεται μπροστά στα μυρμήγκια, οι δύο δεν μοιράζονται ποτέ
    // διαδρομή, άρα η ξεχωριστή απόχρωση έλυνε σύγκρουση που έπαψε να υπάρχει.
    //
    // Το test δεν γράφει `'#0099ff'`: συγκρίνει τα **ζωντανά** tokens μεταξύ τους. Ένα literal
    // θα έμενε πράσινο αν κάποιος άλλαζε το μπλε δείκτη στα δύο από τα τρία σημεία — δηλαδή θα
    // φύλαγε την τιμή αντί για την **ταυτότητα**, που είναι το ζητούμενο.
    expect(TABLE_COPY_MARQUEE.colorHex).toBe(TABLE_CELL_SELECTION.outlineHex);
    expect(TABLE_COPY_MARQUEE.colorHex).toBe(TABLE_CELL_CURSOR.colorHex);
  });

  it('🔴 §48.13 ΧΩΡΙΣ ΑΠΟΧΡΩΣΗ, Η ΔΙΑΚΡΙΣΗ ΚΡΕΜΕΤΑΙ ΑΠΟ ΤΟ ΜΟΤΙΒΟ — και το ξέρει', () => {
    // Το τίμημα της ενοποίησης, γραμμένο ως έλεγχος: το περίγραμμα λειτουργίας είναι κι αυτό
    // διακεκομμένο μπλε και **συνυπάρχει πάντα**. Πριν τις 06/08 τα ξεχώριζαν τέσσερα κανάλια·
    // τώρα τρία. Αν κάποιος ευθυγραμμίσει και το μοτίβο ή το πάχος, η διάκριση πέφτει στη
    // **μόνη** κίνηση — και αυτό πρέπει να χτυπήσει εδώ, όχι στην οθόνη του χρήστη.
    expect(TABLE_COPY_MARQUEE.colorHex).toBe(TABLE_MODE_OUTLINE.colorHex);
    expect([...MARCHING_ANTS_DASH_PX]).not.toEqual([...TABLE_MODE_OUTLINE.dashPx]);
    expect(MARCHING_ANTS_LINE_WIDTH_PX).not.toBe(TABLE_MODE_OUTLINE.lineWidthPx);
  });
});
