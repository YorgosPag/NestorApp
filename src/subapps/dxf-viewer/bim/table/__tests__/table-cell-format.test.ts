...
describe('🔴 η αποθηκευμένη τιμή δεν αλλάζει ΠΟΤΕ από τη μορφή', () => {
  ...
  // ⚠️ `,` και όχι `;`: ο πίνακας μιλά **κανονική** μορφή (`.` δεκαδικό, `,` διαχωριστής)
  // — τεκμηριωμένη απόφαση του ADR-739 §6, γιατί αυτό γράφει το `ACAD_TABLE` στο DXF. Το `;`
  // είναι η σύμβαση του **ελληνικού Excel**, και ένα `=…` που δεν αναλύεται μένει ορατό
  // κείμενο ώστε ο χρήστης να το διορθώσει (§Φ.Ζ) — δεν χάνεται σιωπηλά.
