...
describe('σειρά προτεραιότητας — η ρητή ακμή νικά ΚΑΘΕ άλλο επίπεδο', () => {
  ...
  it('η ρητή ακμή που ΣΥΜΦΩΝΕΙ με το κληρονομημένο μολύβι δεν σπάει το τμήμα', () => {
    ...
    // 🔑 Το μολύβι διαβάζεται **από το στυλ**, δεν ξαναγράφεται εδώ. Ήταν καρφωμένο
    // `{#666666, 0.25}` και έσπασε τη στιγμή που ο Giorgio ισοπέδωσε τα πάχη του `standard`
    // (2026-08-04) — ένα test που ρωτά «συμφωνεί με το κληρονομημένο;» ενώ κρατά δικό του
    // αντίγραφο του κληρονομημένου ρωτά στην πραγματικότητα «συμφωνεί με ό,τι έγραψα εδώ».
    const grid: TableBorderSpec = STANDARD.rowClasses.data.borders.insideH;
