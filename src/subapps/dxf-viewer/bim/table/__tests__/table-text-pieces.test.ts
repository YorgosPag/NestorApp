...
/**
 * 🔴 ADR-753 Φ4 (§15.5) — **ΚΑΡΦΩΜΕΝΟ ύψος, και το ζητά η μέθοδος αυτού του αρχείου.**
 *
 * Τα tests εδώ συγκρίνουν γεωμετρία **ανάμεσα σε δύο μοντέλα** (με runs και χωρίς) ή ανάμεσα σε
 * δύο διαδρομές (τμήματα και primitives). Από τη Φ4 το **αυτόματο** ύψος γραμμής μεγαλώνει για
 * να χωρέσει το ψηλότερο τμήμα — ακριβώς το ελάττωμα «το κείμενο βγαίνει έξω από τη γραμμή» που
 * έκλεισε — και το `TALL_UNDERLINE` δηλώνει `textHeightMm: 20` σε στυλ που γράφει 10. Δηλαδή τα
 * δύο μοντέλα έπαψαν να έχουν την **ίδια** γραμμή, και η διαφορά τους επέστρεφε τέσσερις γραμμές
 * αντί για μία.
 *
 * Η θεραπεία δεν είναι χαλάρωση του κριτηρίου (θα αντέγραφε τη γνώση του κώδικα υπό δοκιμή):
 * είναι να **μη μετακινείται** η γεωμετρία. Ρητό ύψος ⇒ καρφωμένη γραμμή (`measureRows`), και τα
 * δύο μοντέλα ξανά συγκρίσιμα. Ότι το **αυτόματο** ύψος όντως μεγαλώνει το φυλάει δικό του test
 * — `table-row-height-runs.test.ts`.
 */
const ROW: TableRow = { id: 'r1', rowClass: 'data', heightMm: 60 };
