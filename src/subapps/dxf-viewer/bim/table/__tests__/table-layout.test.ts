...
// ── ADR-739 Φ.Δ βήμα 5 — το ΚΑΤΑΚΟΡΥΦΟ όριο, ρητά καρφωμένο ────────────────

/**
 * 🔴 **ΓΝΩΣΤΟ ΟΡΙΟ, ΣΥΝΕΙΔΗΤΗ ΑΠΟΦΑΣΗ (Giorgio, 2026-08-01) — όχι παράλειψη.**
 *
 * Το βήμα 5 έκλεισε το **οριζόντιο** ξεχείλισμα. Κατακόρυφα η μηχανή δεν κάνει τίποτα, και ο
 * λόγος είναι μετρημένος: το ύψος γραμμής είναι **πάντα ρητό** (`row.heightMm` ή
 * `style.defaultRowHeightMm`) και το κείμενο **πάντα μία γραμμή** ύψους `style.textHeightMm`.
 * Άρα κατακόρυφο ξεχείλισμα δεν μπορεί να προκληθεί από **δεδομένα του χρήστη** — μόνο από
 * στυλ που δηλώνει κείμενο ψηλότερο από τη γραμμή του. Το οριζόντιο, αντίθετα, το προκαλεί
 * κάθε μακριά λέξη.
 *
 * Το δάπεδο ύψους (συμπεριφορά AutoCAD) **δεν** μπήκε επίτηδες: το preset `detailSheet` έχει
 * ΣΚΟΠΙΜΑ κοντύτερη γραμμή κεφαλίδας (§16.4) και ένα δάπεδο θα μετακινούσε τα 10 snapshots
 * χαρακτηρισμού του ADR-622 — δηλαδή τα φύλλα οπλισμού που ήδη τυπώνονται.
 *
 * Αυτά τα tests **καρφώνουν** τη σημερινή συμπεριφορά: αν κάποιος προσθέσει δάπεδο ή
 * κατακόρυφη περικοπή, γίνονται κόκκινα και η αλλαγή θα είναι **συνειδητή**, όχι σιωπηλή.
 */
describe('layoutTable — κατακόρυφα: το ύψος γραμμής δεν ακολουθεί το κείμενο (γνωστό όριο)', () => {
  /** Στυλ με κείμενο ΨΗΛΟΤΕΡΟ από τη γραμμή του — η μόνη διαμόρφωση που ξεχειλίζει κατακόρυφα. */
  const TALL_TEXT: TableStyle = {
    ...STANDARD,
    defaultRowHeightMm: 4,
    rowClasses: {
      ...STANDARD.rowClasses,
      data: { ...STANDARD.rowClasses.data, textHeightMm: 9 },
    },
  };

  const model = createTableModel({
    columns: [col('a', { kind: 'fixed', widthMm: 100 })],
    rows: [row('r1')],
    cells: [['r1', 'a', text('Χ')]],
  });

  it('η γραμμή ΔΕΝ μεγαλώνει για να χωρέσει ψηλότερο κείμενο', () => {
    expect(layoutTable(model, TALL_TEXT, { measureText }).rows[0].heightMm).toBe(4);
  });
  ...
  it('το κείμενο ζωγραφίζεται ολόκληρο — καμία κατακόρυφη περικοπή', () => {
    ...
    expect(run?.clipped).toBeUndefined();
  });
});
