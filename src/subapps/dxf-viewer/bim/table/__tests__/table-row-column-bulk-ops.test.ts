...
describe('deleteTableColumns', () => {
  it('🔴 σβήνει ΟΛΕΣ τις επιλεγμένες — το ελάττωμα που ζήτησε ο ιδιοκτήτης (04/08)', () => {
    const next = deleteTableColumns(bookOf(model()),model(), ['c1', 'c2']);
  ...
  it('οι εγγυήσεις της μίας υλοποίησης επιβιώνουν: ο τίτλος μικραίνει ΟΣΟ πρέπει', () => {
    expect(deleteTableColumns(bookOf(model()),model(), ['c1', 'c2']).merges[0].colSpan).toBe(2);
  });

  it('🔴 διαγραφή που περιλαμβάνει την ΑΓΚΥΡΑ κουβαλά το περιεχόμενο στην πρώτη επιζώσα', () => {
    const next = deleteTableColumns(bookOf(model()),model(), ['c0', 'c1']);
  ...
  it('🔴 ΟΛΑ Ή ΤΙΠΟΤΑ: 4 από 4 στήλες ⇒ ΚΑΜΙΑ διαγραφή, ίδιο μοντέλο by-reference', () => {
    const before = model();
    expect(deleteTableColumns(bookOf(before),before, ['c0', 'c1', 'c2', 'c3'])).toBe(before);
  });

  it('3 από 4 επιτρέπονται — ο πίνακας μένει με μία στήλη', () => {
    expect(ids(deleteTableColumns(bookOf(model()),model(), ['c0', 'c1', 'c2']).columns)).toEqual(['c3']);
  });

  it('κενή λίστα ⇒ το ίδιο μοντέλο by-reference (κανένα βήμα undo για το τίποτα)', () => {
    const before = model();
    expect(deleteTableColumns(bookOf(before),before, [])).toBe(before);
  });

  it('η σειρά των ταυτοτήτων δεν αλλάζει το αποτέλεσμα — id, όχι index', () => {
    expect(ids(deleteTableColumns(bookOf(model()),model(), ['c2', 'c0']).columns))
      .toEqual(ids(deleteTableColumns(bookOf(model()),model(), ['c0', 'c2']).columns));
  });
});

describe('deleteTableRows', () => {
  it('σβήνει όλες τις γραμμές και τα κελιά τους', () => {
    const next = deleteTableRows(bookOf(model()),model(), ['r1', 'r3']);
  ...
  it('🔴 ΟΛΑ Ή ΤΙΠΟΤΑ: 5 από 5 γραμμές ⇒ καμία (πίνακας χωρίς ύψος = αόρατη οντότητα)', () => {
    const before = model();
    expect(deleteTableRows(bookOf(before),before, ['r0', 'r1', 'r2', 'r3', 'r4'])).toBe(before);
  });
});

describe('insertTableColumns / insertTableRows', () => {
  it('🔴 με 3 μαρκαρισμένες βάζει 3 — ο κανόνας του Excel', () => {
    const next = insertTableColumns(bookOf(model()),model(), 1, 3);
  ...
  it('οι νέες στήλες μπαίνουν ΟΛΕΣ στη ζητούμενη θέση, όχι διάσπαρτα', () => {
    const next = insertTableColumns(bookOf(model()),model(), 2, 2);
    expect(ids(next.columns)).toEqual(['c0', 'c1', 'c5', 'c4', 'c2', 'c3']);
  });

  it('ο τίτλος πλήρους πλάτους μεγαλώνει ΚΑΤΑ ΤΟ ΠΛΗΘΟΣ', () => {
    expect(insertTableColumns(bookOf(model()),model(), 4, 3).merges[0].colSpan).toBe(7);
  });
  ...
  it('🔴 ΟΛΑ Ή ΤΙΠΟΤΑ στο φράγμα: πλήθος που δεν χωρά ⇒ ΚΑΜΙΑ εισαγωγή', () => {
    ...
    expect(insertTableColumns(bookOf(nearFull),nearFull, 0, 3)).toBe(nearFull);
    expect(insertTableColumns(bookOf(nearFull),nearFull, 0, 1).columns).toHaveLength(columnCount + 1);
  });

  it('πλήθος 0 ή αρνητικό ⇒ το ίδιο μοντέλο by-reference', () => {
    const before = model();
    expect(insertTableColumns(bookOf(before),before, 1, 0)).toBe(before);
    expect(insertTableRows(bookOf(before),before, 1, -2)).toBe(before);
  });

  it('γραμμές: 2 μαρκαρισμένες ⇒ 2 νέες, όλες `data`', () => {
    const next = insertTableRows(bookOf(model()),model(), 2, 2);
