...
describe('LIVE_REF — αναφορά χωρίς τελεστή μετά, το κλικ ΑΝΤΙΚΑΘΙΣΤΑ', () => {
  it.each([
    ...
    ['=B2+C3|', 4, 6],
    ['=SUM(A1|', 5, 7],
    // 🔴 ADR-761 — **η αναφορά ΜΕΤΑ τον διαχωριστή**. Ήταν το μοναδικό μονοπάτι όπου το
    // `expectsOperand` φτάνει στο `grammar.argumentSeparator`: για κάθε άλλη στίξη (`(`, `:`)
    // η διάζευξη κόβεται νωρίτερα. Χωρίς αυτή τη γραμμή, μια κλήση που είχε **χάσει** το
    // όρισμα γραμματικής έμενε αόρατη σε 47 tests — και έσκαγε σε `=SUM(A1;B2` + κλικ.
    ['=SUM(A1;B2|', 8, 10],
    ['=SUM(A1;B2:C3|', 8, 13],
