...
// ─── Parse back to store units (ADR-769 Δ4) ──────────────────────────────────

/**
 * 🔄 **Ο ΑΝΤΙΣΤΡΟΦΟΣ** του {@link formatCellForXlsx}: τιμή σε μονάδες **ΟΘΟΝΗΣ** → τιμή σε
 * μονάδες **ΑΠΟΘΗΚΗΣ**. Χρειάζεται μόνο όταν ο πίνακας γίνεται **αμφίδρομος** (ADR-769 —
 * ο χρήστης γράφει σε κελί και αλλάζει η οντότητα).
 *
 * ## 🔴 Γιατί ζει ΕΔΩ και όχι σε δικό του αρχείο
 * Είναι το **ζεύγος** του formatter από πάνω: ίδιο `switch`, ίδια ένωση, αντίστροφη πράξη.
 * Σε ξεχωριστό αρχείο οι δύο κατευθύνσεις θα ήταν **δύο χειρόγραφες λίστες** — και αυτό το
 * repo το πλήρωσε τέσσερις φορές (CHECK 3.34: δύο λίστες namespace που απέκλιναν κατά 63·
 * CHECK 3.37: συγκεντρωτής με 18 ενώ το δέντρο είχε 26). Δίπλα-δίπλα, ο ίδιος αναγνώστης
 * βλέπει και τις δύο σε μία οθόνη, και ο μεταγλωττιστής απαιτεί απάντηση για κάθε κλάδο.
 *
 * ## 🔑 Γιατί ΣΤΡΟΓΓΥΛΕΥΕΙ στην ακρίβεια της οθόνης
 * Η οθόνη δείχνει `dimension-mm-to-m` με **3 δεκαδικά μέτρου** = 1 mm. Ο χρήστης πληκτρολογεί
 * **ό,τι βλέπει**, άρα διαφορά κάτω από το χιλιοστό είναι **αόρατη** — και μια εντολή γι' αυτήν
 * θα ήταν κενή εγγραφή στο ιστορικό (N.7.2 #3). Η κβάντωση είναι **ακριβώς** η ίδια με του
 * `formatCellForDisplay`, ώστε «γράφω αυτό που βλέπω» να είναι ταυτότητα, όχι προσέγγιση.
 *
 * @returns `null` όταν η τιμή **δεν** είναι αριθμητική ή ο τύπος στήλης δεν έχει αριθμητική
 *   αποθήκη (`text`). 🔴 **Ποτέ `0` ως υποκατάστατο** — ADR-720: κατασκευασμένο νούμερο σε
 *   παραδοτέο είναι μέτρηση που κανείς δεν πήρε. Ο καλών μετατρέπει το `null` σε **ρητή άρνηση**.
 */
export function parseCellToStore(
  value: ScheduleCellValue,
  valueType: ScheduleColumnValueType,
): number | null {
