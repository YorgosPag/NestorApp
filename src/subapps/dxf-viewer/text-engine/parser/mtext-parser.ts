...
function applyParagraphCode(
  ...
  //
  // 🐛 ADR-635 Φ C.21 — Ο ΕΛΕΓΧΟΣ ΗΤΑΝ ΠΟΛΥ ΣΤΕΝΟΣ: `state.paragraphs.length === 0 && …`, δηλαδή
  // «μόνο η ΠΡΩΤΗ παράγραφος γλιτώνει». Το AutoCAD όμως γράφει ρουτίνα `…\P\pxsm1.5;Κείμενο` —
  // πρώτα κλείνει την παράγραφο με `\P`, μετά δηλώνει τις ιδιότητες της ΕΠΟΜΕΝΗΣ. Ο δεύτερος
  // κωδικός έβρισκε `paragraphs.length > 0` και έκλεινε ΞΑΝΑ, παράγοντας μια φανταστική κενή
  // παράγραφο (= μια κενή γραμμή) πριν από **κάθε** παράγραφο με ιδιότητες. Η σωστή ερώτηση δεν
  // είναι «είναι η πρώτη;» αλλά «έχει μαζευτεί περιεχόμενο σε ΑΥΤΗΝ;» — το `flushRun` από πάνω
  // έχει ήδη αδειάσει το `currentText`, οπότε το `runs` είναι η πλήρης απάντηση.
  const atStart = state.runs.length === 0;
