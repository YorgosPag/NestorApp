...
function escapeText(text: string): string {
  return text
    ...
    .replace(/\{/g, '\\{')
    .replace(/\}/g, '\\}')
    // 🔴 Η αλλαγή γραμμής ΤΕΛΕΥΤΑΙΑ, και ο λόγος είναι η σειρά των δύο από πάνω: το `\N`
    // εισάγει **νέα** ανάστροφη κάθετο, που δεν πρέπει να διπλασιαστεί.
    //
    // Ο parser γράφει το `\N` ως **κυριολεκτικό** `'\n'` μέσα στο run (`mtext-parser.ts`,
    // `case 'lineBreak'`) — σε αντίθεση με το `\P`, που γίνεται νέα παράγραφος. Χωρίς αυτή τη
    // γραμμή, η αλλαγή γραμμής **εξαφανιζόταν στη σειριοποίηση** και δύο γραμμές γίνονταν μία:
    // (α) στην **εξαγωγή DXF** ένας ωμός χαρακτήρας νέας γραμμής μέσα στο group 1 δεν είναι
    // έγκυρο MTEXT· (β) στην **ανάγνωση πινακίδας** (ADR-745 Φ3β) η δεύτερη γραμμή έχανε το
    // ύψος της, άρα και τη σημασία της.
    .replace(/\n/g, '\\N');
}
