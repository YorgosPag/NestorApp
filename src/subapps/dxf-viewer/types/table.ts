 ...
 *
 * ## Τι ΔΕΝ είναι εδώ (και γιατί)
 * Το `TableEntity` (§4 του ADR) **δεν** γράφεται σε αυτή τη φάση: απαιτεί `'table'` στο
 * `EntityType` union, που πυροδοτεί και τα 9 gates του §12 (render contract, type
 * descriptor, export coverage, ~20 capability anchors). Το roadmap τα τοποθετεί ρητά στη
 * **Φάση Γ**· ένα entity χωρίς απαντημένα gates θα άφηνε το repo κόκκινο (ADR-587 §6.1:
 * *«anchor χωρίς gate δεν είναι anchor — είναι σχόλιο»*). Εδώ ζει μόνο ό,τι η μηχανή
 * διάταξης χρειάζεται για να τρέξει ως καθαρή συνάρτηση.
 *
