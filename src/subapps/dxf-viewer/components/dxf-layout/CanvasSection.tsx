...
// ADR-040 — τα ghost-preview payloads χτίζονται εκτός orchestrator (καθαρή συνάρτηση).
import { buildCanvasSectionGhostPreviews } from './canvas-section-ghost-previews';
...
export const CanvasSection: React.FC<DXFViewerLayoutProps & { overlayMode: OverlayEditorMode, currentStatus: Status, currentKind: OverlayKind }> = (props) => {
  ...
  const specialTools = useSpecialTools({ activeTool, levelManager });
  const { circleTTT, lineParallel, angleEntityMeasurement, stairTool, stairRegionTool, wallTool, slabTool, roofTool, floorFinishTool, wallCoveringTool, columnTool, foundationTool, mepFixtureTool, furnitureTool, genericSolidTool, blockLibraryTool, titleBlockTool, furniturePlanTool, peoplePlanTool, vehiclesPlanTool, plantsPlanTool, attachImageTool, floorplanSymbolTool, electricalPanelTool, mepManifoldTool, mepRadiatorTool, mepBoilerTool, mepWaterHeaterTool, mepUnderfloorTool, thermalSpaceTool, bathroomAutoArrangeTool, spaceSeparatorTool, mepSegmentTool, mepRiserTool, railingTool, beamTool, beamBetweenMembersTool, slabOpeningTool, openingTool, selfOpeningTool } = specialTools;
  ...
  const { handleCanvasClick } = useCanvasClickHandler({
    ...
    foundationTool,
    mepFixtureTool,
    furnitureTool, genericSolidTool, blockLibraryTool, titleBlockTool, furniturePlanTool, peoplePlanTool, vehiclesPlanTool, plantsPlanTool, attachImageTool,
    floorplanSymbolTool,
    electricalPanelTool,
    mepManifoldTool,
    mepRadiatorTool,
    ...
    mepWaterHeaterTool,
    mepUnderfloorTool,
    thermalSpaceTool, bathroomAutoArrangeTool, spaceSeparatorTool,
    mepSegmentTool,
    mepRiserTool,
    railingTool,
    beamTool,
    ...
    extendIsActive: extendTool.isActive, handleExtendClick: extendTool.handleExtendClick, wallSplitIsActive: wallSplitTool.isActive, handleWallSplitClick: wallSplitTool.handleWallSplitClick, wallAttachIsActive: wallAttachTool.isActive, handleWallAttachClick: wallAttachTool.handleWallAttachClick, stairAddTurnIsActive: stairAddTurnTool.isActive, handleStairAddTurnClick: stairAddTurnTool.handleStairAddTurnClick, wallMergeIsActive: wallMergeTool.isActive, handleWallMergeClick: wallMergeTool.handleWallMergeClick, wallGapOpeningIsActive: wallGapOpeningTool.isActive, handleWallGapOpeningClick: wallGapOpeningTool.handleWallGapOpeningClick, copyIsActive: copyTool.isCollectingInput, handleCopyClick: copyTool.handleCopyClick,
    ...
    parallelRefGuideId: guideWorkflows.parallelRefGuideId, onParallelRefSelected: guideWorkflows.handleParallelRefSelected, onParallelDistanceCommitted: guideWorkflows.handleParallelDistanceCommitted,
  ...
  useCanvasKeyboardShortcuts({
    ...
    handleWallSplitEscape: wallSplitTool.handleWallSplitEscape, wallSplitIsActive: wallSplitTool.isActive, handleWallAttachEscape: wallAttachTool.handleWallAttachEscape, wallAttachIsActive: wallAttachTool.isActive, handleStairAddTurnEscape: stairAddTurnTool.handleStairAddTurnEscape, stairAddTurnIsActive: stairAddTurnTool.isActive, handleWallMergeEscape: wallMergeTool.handleWallMergeEscape, wallMergeIsActive: wallMergeTool.isActive, handleWallGapOpeningEscape: wallGapOpeningTool.handleWallGapOpeningEscape, wallGapOpeningIsActive: wallGapOpeningTool.isActive, handleCopyEscape: copyTool.handleCopyEscape, copyIsActive: copyTool.isActive,
  ...
  const { textEditor, tableCellEditor, tableHeaderMenu, tableRangeMenu, tableLinkMenu, handleDoubleClick, handleMouseMoveWithAutoArea } = useCanvasSectionUI({
  ...
  return (
    <>
      ...
      <CanvasLayerStack
        ...
        entityPickingActive={angleEntityMeasurement.isActive || rotationTool.phase === 'awaiting-entity' || moveTool.phase === 'awaiting-entity' || mirrorTool.phase === 'awaiting-entity' || activeTool === 'wall-on-entity' || isWallRegionTool(activeTool) || activeTool === 'beam-from-wall' || activeTool === 'beam-between-members' || wallAttachTool.isActive || stairAddTurnTool.isActive || wallMergeTool.isActive || activeTool === 'guide-arc-segments' || activeTool === 'guide-arc-distance' || activeTool === 'guide-arc-line-intersect' || activeTool === 'guide-circle-intersect' || activeTool === 'guide-line-midpoint' || activeTool === 'guide-circle-center' || isDimTool(activeTool) || activeTool === 'match-properties' || activeTool === 'array-path'}
        ...
        scalePreview={{}}
        stretchPreview={{}}
        {...buildCanvasSectionGhostPreviews({ tools: specialTools, levelManager, hoveredGrip: unified.hoveredGrip })}
      ...
      <CanvasSectionOverlays
        ...
        entityMenuHost={{ entityMenuRef, currentScene: props.currentScene ?? null, dxfScene, entityJoinHook, handleSmartDelete, handleReorderEntity, onToolChange: props.onToolChange as ((tool: string) => void) | undefined, replaceEntitySelection: (ids) => universalSelectionRef.current.replaceEntitySelection(ids), executeCommand, levelManager, t }}
