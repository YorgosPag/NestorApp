...
export const CanvasSection: React.FC<DXFViewerLayoutProps & { overlayMode: OverlayEditorMode, currentStatus: Status, currentKind: OverlayKind }> = (props) => {
  ...
  const { circleTTT, linePerpendicular, lineParallel, angleEntityMeasurement, stairTool, wallTool, slabTool, roofTool, floorFinishTool, columnTool, foundationTool, mepFixtureTool, furnitureTool, floorplanSymbolTool, electricalPanelTool, mepManifoldTool, mepRadiatorTool, mepBoilerTool, mepWaterHeaterTool, mepUnderfloorTool, thermalSpaceTool, spaceSeparatorTool, mepSegmentTool, mepRiserTool, railingTool, beamTool, slabOpeningTool, openingTool } = useSpecialTools({ activeTool, levelManager });
  ...
  return (
    <>
      ...
      <CanvasSectionOverlays
        ...
        entityMenu={{ selectedCount: selectedEntityIds.length, canJoin: entityJoinState.canJoin, joinResultLabel: entityJoinState.joinResultLabel, onJoin: () => entityJoinHook.joinEntities(selectedEntityIds), onDelete: () => handleSmartDelete(), onCancel: () => entityMenuRef.current?.close(), canSplit: selectedEntityIds.length === 1 && !!props.currentScene?.entities.find((x) => x.id === selectedEntityIds[0] && isWallEntity(x)), onSplit: () => { entityMenuRef.current?.close(); props.onToolChange?.('wall-split' as ToolType); }, canSelectSimilar: selectedEntityIds.length >= 1, onSelectSimilar: () => { entityMenuRef.current?.close(); const scene = props.currentScene; if (!scene || selectedEntityIds.length === 0) return; const ids = findEntitiesWithSimilarColor(selectedEntityIds[0], scene.entities, scene.layersById); if (ids.length > 0) universalSelection.replaceEntitySelection(ids); }, canIsolateEntity: selectedEntityIds.length >= 1, onIsolateEntity: () => { entityMenuRef.current?.close(); if (selectedEntityIds.length === 0) return; executeCommand(new EntityIsolateCommand({ targetEntityIds: selectedEntityIds, category: t('layer.isolate.statusBadge.entityCount', { count: selectedEntityIds.length }) })); }, canIsolateCategory: collectBimCategories(selectedEntityIds, dxfScene?.entities).length > 0, onIsolateCategory: () => { entityMenuRef.current?.close(); const cats = collectBimCategories(selectedEntityIds, dxfScene?.entities); if (cats.length === 0) return; executeCommand(new CategoryIsolateCommand({ targetCategories: cats, category: t('layer.isolate.statusBadge.categoryCount', { count: cats.length }) })); }, ...entityLayerCommands }}
