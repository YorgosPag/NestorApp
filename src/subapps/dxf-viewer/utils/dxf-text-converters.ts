...
function buildTextSceneEntity(params: {
  ...
  return {
    ...
    fontSize: height,
    height,
    // 🔑 «ΤΟ ΑΡΧΕΙΟ ΕΧΕΙ ΜΙΛΗΣΕΙ» — το group 40 ενός ξένου DXF είναι ύψος σε **μονάδες
    // κόσμου**, όχι πρόθεση σε mm χαρτιού. Δηλώνεται **ρητά** ώστε ένα εισαγόμενο σχέδιο να
    // ΜΗΝ αλλάξει ποτέ επειδή ο χρήστης γύρισε την Κλίμακα σχεδίου — η ίδια αρχή που ήδη
    // τηρεί το ADR-362 για το εισαγόμενο DIMSTYLE («δηλωμένο = ήδη σωστό»).
    //
    // ⚠️ Το `value` είναι **προέλευση**: το ύψος που ΔΗΛΩΣΕ το αρχείο, αμετάβλητο από τις
    // δικές μας επεξεργασίες. Η γεωμετρία **δεν** το διαβάζει ποτέ — ο `resolveTextSize`
    // ξαναπαράγει την τιμή από την αλυσίδα ύψους, ώστε ένα grip-resize να μη σκιάζεται.
    // Χρήσιμο σημασιολογικά: ξεχωρίζει «εισαγόμενο, μοντέλου εκ προθέσεως» από «δικό μας,
    // γραμμένο πριν υπάρξει το λεξιλόγιο» — τη διάκριση που θα χρειαστεί η ρητή ενέργεια
    // «Μετατροπή σε μέγεθος χαρτιού» για να μη ρωτά ό,τι δεν πρέπει.
    annotationSize: { basis: 'model' as const, value: height },
    rotation,
    alignment,
    textNode,
    ...(color && { color }),
