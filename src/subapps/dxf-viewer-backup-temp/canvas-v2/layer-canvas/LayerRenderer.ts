...
export class LayerRenderer {
  ...
  /**
   * ✅ ΦΑΣΗ 6: Centralized UI rendering method (DEPRECATED - Use renderUnified instead)
   * Alternative render που χρησιμοποιεί το UIRendererComposite
   */
  renderWithUnifiedUI(
    layers: ColorLayer[],
    transform: ViewTransform,
    ...
    // 1. Render color layers (entity content) με clipping για rulers
    // 🚀 CLIP στο "content area" χωρίς rulers (συνέπεια με legacy rendering)
    this.ctx.save();
    const left = rulerSettings.width || 0;
    const bottom = rulerSettings.height || 0;
    ...
    // 2. ✅ ΦΑΣΗ 6: Render ALL UI elements using centralized system
    const uiSettings = this.createUISettings({
      crosshairSettings,
      cursorSettings,
      snapSettings,
    ...
    // For now, use individual rendering through UIComposite
    // TODO: Enhance UIRendererComposite to accept custom context data
    const extendedViewport = {
      x: 0,
      y: 0,
      width: viewport.width,
  ...
  private renderPolygon(
    ...
    console.log('🚨 MARGINS IMPACT DEBUG:', {
      ...
      marginsOffset: { left: 80, bottom: 30 }, // From COORDINATE_LAYOUT.MARGINS
      isOnScreen: screenVertices.some(v => v.x >= 0 && v.x <= viewport.width && v.y >= 0 && v.y <= viewport.height),
      isInRenderArea: screenVertices.some(v => v.x >= 80 && v.x <= viewport.width && v.y >= 0 && v.y <= viewport.height - 30)
    });
