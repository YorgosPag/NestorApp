...
export const LayerCanvas = React.forwardRef<HTMLCanvasElement, LayerCanvasProps>(({
  ...
  layersVisible = false, // ✅ LAYER PERSISTENCE: Default false
  crosshairSettings,
  cursorSettings,
  snapSettings,
  gridSettings,
    ...
    const handleOriginMarkersToggle = (event: CustomEvent) => {
      ...
      if (rendererRef.current) {
        requestAnimationFrame(() => {
          ...
          // Trigger re-render with current props
          if (useUnifiedUIRendering) {
            renderer.renderWithUnifiedUI(
  ...
  const renderLayers = useCallback(() => {
    ...
    try {
      ...
      // ✅ ΦΑΣΗ 6: Conditional rendering based on feature flag
      if (useUnifiedUIRendering) {
        renderer.renderWithUnifiedUI(
        ...
        // Legacy rendering για backward compatibility
        renderer.render(
          filteredLayers, // ✅ FILTERED: Κενά layers αν δεν είναι layering active
          transform,
          viewport,
          ...
          cursorSettings,
          snapSettings,
          gridSettings, // 🎯 ΕΜΦΑΝΙΣΗ: Δείχνουμε grid και rulers στο layering mode
          rulerSettings, // 🎯 ΕΜΦΑΝΙΣΗ: Δείχνουμε grid και rulers στο layering mode
          selectionSettings,
          finalRenderOptions
        );
      }
  ...
  return (
    <canvas
      ...
      style={{
        ...
        width: '100%',
        height: '100%',
        zIndex: 10, // 🎯 ΚΡΙΣΙΜΟ: Inline z-index για LayerCanvas πάνω
        pointerEvents: 'auto',
        cursor: activeTool === 'pan' ? 'grab' : (crosshairSettings.enabled ? 'none' : 'crosshair'), // 🔥 Pan tool has priority over crosshair
        backgroundColor: 'transparent', // 🎯 ΔΙΑΦΑΝΕΣ: Δείχνουμε μόνο τα έγχρωμα layers
        border: activeTool === 'layering' ? '2px solid lime' : 'none', // 🔍 DEBUG: Lime border μόνο στο layering
