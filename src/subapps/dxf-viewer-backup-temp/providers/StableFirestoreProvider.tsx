...
interface FirestoreContextType {
  ...
  levelId: string | null;
  subscribe: (key: string, callback: (data: FirestoreData) => void) => () => void;
...
class StableSubscriptionManager {
  private subscriptions = new Map<string, {
    unsubscribe: () => void;
    callbacks: Set<(data: FirestoreData) => void>;
    lastData: FirestoreData | null;
    subscribeCount: number;
  }>();
  ...
  subscribe(key: string, callback: (data: FirestoreData) => void): () => void {
    ...
    let lastData: FirestoreData | null = null;
  ...
  private createFirestoreSubscription(key: string, callback: (data: FirestoreData) => void): () => void {
...
export const StableFirestoreProvider: React.FC<StableFirestoreProviderProps> = ({
  ...
  const contextValue = useMemo<FirestoreContextType>(() => ({
    ...
    subscribe: (key: string, callback: (data: FirestoreData) => void) => {
...
export const useStableFirestoreSubscription = (
  ...
  const [data, setData] = React.useState<FirestoreData | null>(null);
  ...
  // ═══ STABLE CALLBACK ═══
  const handleData = useCallback((newData: FirestoreData) => {
    setData(newData);
    setLoading(false);
    setError(null);
  }, []);
...
if (typeof window !== 'undefined' && process.env.NODE_ENV !== 'production') {
  (window as { debugFirestore?: unknown }).debugFirestore = {
