...
function useLevelsSystemState({
  ...
  }, [levels, currentLevelId, enableFirestore, firestoreCollection, handleError, onLevelChange]); // ✅ FIX: Removed sceneManager from deps
  ...
  const clearAllLevels = useCallback(async (): Promise<void> => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [levels, enableFirestore, firestoreCollection, handleError, onLevelChange]); // ✅ FIX: Removed sceneManager from deps
  ...
  const setLevelScene = useCallback((levelId: string, scene: SceneModel) => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []); // ✅ FIX: Empty deps - sceneManager is accessed via closure to prevent infinite loop
