...
export const CanvasSection: React.FC<DXFViewerLayoutProps & { overlayMode: OverlayEditorMode, currentStatus: Status, currentKind: OverlayKind }> = (props) => {
  ...
  React.useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      // 🐛 DEBUG: Uncomment to debug keyboard issues
      // if (e.shiftKey) {
      //   console.log('🔍 Shift key combo detected:', { key: e.key, code: e.code, keyCode: e.keyCode });
      // }

      // 🎯 ENTERPRISE SHORTCUTS: Shift+1/2/0 (AutoCAD/Figma standard)
      // Support both main keyboard (Digit1) AND numpad (Numpad1) for international compatibility
      if (e.shiftKey && (e.code === 'Digit1' || e.code === 'Numpad1')) { // Shift+1 → fit to view
        e.preventDefault();

        const canvas = dxfCanvasRef.current || overlayCanvasRef.current;
        const actualViewport = canvas
          ? { width: canvas.clientWidth, height: canvas.clientHeight }
          : { width: 800, height: 600 };

        const event = new CustomEvent('canvas-fit-to-view', { detail: { viewport: actualViewport } });
        document.dispatchEvent(event);
        return;
      }

      if (e.shiftKey && (e.code === 'Digit0' || e.code === 'Numpad0')) { // Shift+0 → 100% zoom (1:1)
        e.preventDefault();

        const zoomResult = zoomSystem.zoomTo100(mouseCss || undefined);
        if (zoomResult && zoomResult.transform) {
          setTransform(zoomResult.transform);
        }
        return;
      ...
      switch (e.key) {
        ...
        case '=':
          e.preventDefault();
          // ✅ ΚΕΝΤΡΙΚΟΠΟΙΗΣΗ: Use unified keyboard zoom with cursor position
          // Supports both bare +/- and Ctrl/Cmd+± (works automatically)
          zoomSystem.handleKeyboardZoom('+', mouseCss || undefined);
          break;
        case '-':
          e.preventDefault();
          // ✅ ΚΕΝΤΡΙΚΟΠΟΙΗΣΗ: Use unified keyboard zoom with cursor position
  ...
  }, [draftPolygon, zoomSystem, mouseCss]);
