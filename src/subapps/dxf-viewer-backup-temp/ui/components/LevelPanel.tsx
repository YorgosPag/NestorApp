...
export function LevelPanel({ 
  ...
  // ✅ FIX: Get the scene for the current level instead of using global scene prop
  const currentLevelScene = currentLevelId ? getLevelScene(currentLevelId) : null;

  // 🔍 DEBUG: Log current level scene info (TEMPORARILY DISABLED to fix infinite loop)
  // React.useEffect(() => {
  //   console.log('🔍 LEVEL PANEL DEBUG:', {
  //     currentLevelId,
  //     hasLevelScene: !!currentLevelScene,
  //     levelSceneKeys: currentLevelScene ? Object.keys(currentLevelScene) : [],
  //     hasLayers: currentLevelScene?.layers ? Object.keys(currentLevelScene.layers).length : 0,
  //     layers: currentLevelScene?.layers ? Object.keys(currentLevelScene.layers) : [],
  //     propScene: scene ? 'exists' : 'null',
  //     propSceneLayers: scene?.layers ? Object.keys(scene.layers).length : 0,
  //     propSceneLayerNames: scene?.layers ? Object.keys(scene.layers) : []
  //   });
  //   // eslint-disable-next-line react-hooks/exhaustive-deps
  // }, [currentLevelId]); // ✅ FIX: Only depend on currentLevelId to prevent infinite loop
  ...
  const levelScenes = useMemo(() => {
    ...
    return scenes;
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [levels]); // ✅ FIX: Removed getLevelScene from deps to prevent infinite loop
  ...
  const handleLayeringActivation = React.useCallback(() => {
    setShowToolbox(true);
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []); // ✅ FIX: Empty deps - updateGripSettings is stable
  ...
  React.useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []); // ✅ FIX: Empty deps - handleLayeringActivation is now stable useCallback
  ...
  return (
    <div className="h-full flex flex-col px-1 py-2 space-y-4">
      ...
      {/* ✅ REVERTED: Use global scene prop - layers are NOT in level scenes */}
