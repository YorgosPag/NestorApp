...
// 🎯 CENTRALIZED TEST HELPERS - Import από __tests__/helpers/
import {
  createTestScene,
  publishHighlight,
  TEST_EVENTS,
...
// 🎯 TYPE DEFINITIONS - Import production types
import type { Point2D, Entity } from '../../rendering/types/Types';

// 🎯 ENTERPRISE EVENT CONTRACT - Using centralized constant
const HILITE_EVENT = TEST_EVENTS.HILITE; // ✅ From centralized helpers
  ...
  test('✅ Should only select entities on visible layers', () => {
    ...
    const getSelectableEntities = (ids: string[], scene: Scene): string[] => {
      ...
      return ids.filter((id: string) => {
        ...
        return entity && entity.layer && visibleLayers.has(entity.layer);
      });
    };
