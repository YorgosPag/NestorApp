...
// ✅ ΚΕΝΤΡΙΚΟΠΟΙΗΣΗ: Unified entity selection service (screen-space hit testing)
import { unifiedEntitySelectionService } from '../../services/UnifiedEntitySelectionService';
// ✅ ΚΕΝΤΡΙΚΟΠΟΙΗΣΗ: Selection event bus
...
export function useCentralizedMouseHandlers({
  ...
  // ✅ ΚΕΝΤΡΙΚΟΠΟΙΗΣΗ: Update hit testing services με το scene
  useEffect(() => {
    hitTestingService.updateScene(scene);
    unifiedEntitySelectionService.updateScene(scene); // ✅ Screen-space hit testing
  }, [scene]);
  ...
  const handleMouseUp = useCallback((e: React.MouseEvent) => {
    ...
    // ✅ FIX (2025-10-04): Διάκριση simple click από drag selection με movement threshold
    if (cursor.isSelecting && cursor.selectionStart && cursor.position) {
      // Calculate movement distance (pixels)
      const dx = cursor.position.x - cursor.selectionStart.x;
      const dy = cursor.position.y - cursor.selectionStart.y;
      const moveDistance = Math.sqrt(dx * dx + dy * dy);
      const CLICK_THRESHOLD = 5; // CAD standard: < 5px = click, >= 5px = drag

      // ✅ FIX: Simple click (no drag) → Entity selection με screen-space hit-test
      if (moveDistance < CLICK_THRESHOLD && onEntitySelect) {
        console.log('🎯 SIMPLE CLICK DETECTED - Performing screen-space entity hit-test');

        // ✅ SCREEN-SPACE HIT TESTING: Using unified entity selection service
        // This was the working solution from backups (19-08-2025)
        const hitResult = unifiedEntitySelectionService.findEntityAtPoint(
          cursor.position,  // Screen position
          transform,
          viewport,
          8  // CAD-standard tolerance (8 pixels)
        );

        // ✅ CRITICAL: Call onEntitySelect callback to update canvasContext.selectedEntityIds
        // This triggers DxfCanvas re-render with grips (see DxfCanvas.tsx:92, 282)
        const entityId = hitResult?.entityId || null;
        onEntitySelect(entityId);
        console.log('🎯 Entity Click:', entityId ? `Selected ${entityId}` : 'No entity at cursor');

        cursor.endSelection();
      } else {
        // ✅ Drag selection → Marquee selection για layers
      ...
      // ✅ FALLBACK: Αν δεν ήταν σε selection mode αλλά έχει position (edge case)
      // Χρήση screen-space hit testing
      const hitResult = unifiedEntitySelectionService.findEntityAtPoint(
        cursor.position,
        transform,
        viewport,
      ...
      const entityId = hitResult?.entityId || null;
      onEntitySelect(entityId);
      console.log('🎯 Entity Click (fallback):', entityId ? `Selected ${entityId}` : 'No entity at cursor');
    } else {
