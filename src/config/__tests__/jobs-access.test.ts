...
describe('Μ-5 — Υ-4: κάθε στοιχείο πλοήγησης ανήκει κάπου', () => {
  ...
  it('η μετρημένη τιμή των «Οικονομικών» είναι 9 — ο αριθμός του στιγμιότυπου, συν τον κατάλογο', () => {
    ...
    //
    // 🔴 ΑΛΛΑΞΕ 2026-08-10, ΚΑΙ ΤΟ ΣΗΜΑ ΛΕΙΤΟΥΡΓΗΣΕ: **9 → 8**. Αιτία: ADR-777 §Α4 —
    // έκλεισε η πόρτα `/geo/canvas` (διαδρομή + καταχώρηση μενού «Εργαλεία»). Το στοιχείο
    // ανήκε στο `design`, άρα **κρυβόταν** στις άλλες πέντε δουλειές και μετριόταν εκεί.
    // Απογραφή **και των έξι**, εκτελεσμένη πριν το κλείδωμα (ADR-748 §14.6.4):
    //   Σχέδιο 7→**7** (ορατό εκεί, δεν μετριόταν) · Εργοτάξιο 8→**7** · Πελάτες 6→**5**
    //   Οικονομικά 9→**8** · Προμήθειες 9→**8** · Διαχείριση 10→**9**
    // ⚠️ Το **8** είναι **υπολογισμένο**, όχι ιδωμένο σε στιγμιότυπο· η ταύτιση κώδικα ⇄
    // ανθρώπου του §14.6.3 θέλει νέο γύρο στην οθόνη για να ξαναποκτήσει δύο μάρτυρες.
    const total = LIVE_MENUS.reduce(
      (sum, buildMenu) =>
        sum + filterItemsByJob(buildMenu('production', ALL_PERMISSIONS), 'finance').hiddenCount,
      0,
    );
    // 🔴 ΑΛΛΑΞΕ 2026-08-21, ΚΑΙ ΤΟ ΣΗΜΑ ΞΑΝΑΛΕΙΤΟΥΡΓΗΣΕ: **8 → 9**. Αιτία: ADR-777
    // §8.34 — άνοιξε ο **κατάλογος εντολών** (`/listings/mandates`, διαδρομή +
    // καταχώρηση κύριου μενού). Ανήκει στους **Πελάτες**, άρα κρύβεται στις άλλες
    // πέντε δουλειές και μετριέται εκεί. Απογραφή **και των έξι**, εκτελεσμένη πριν
    // το κλείδωμα:
    //   Σχέδιο 7→**8** · Εργοτάξιο 7→**8** · Πελάτες 5→**5** (ορατό εκεί)
    //   Οικονομικά 8→**9** · Προμήθειες 8→**9** · Διαχείριση 9→**10**
    // ⚠️ Το **9** είναι **υπολογισμένο**, όχι ιδωμένο σε στιγμιότυπο.
    expect(total).toBe(9);
  });
});
