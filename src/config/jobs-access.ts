...
export interface JobFilterResult<T> {
   ...
   *
   * ⚠️ Μονάδα μέτρησης = **στοιχείο του ίδιου επιπέδου με την είσοδο** (Ε14.ιβ),
   * όχι φύλλα του δέντρου. Ένας κλάδος που φεύγει με 11 παιδιά μετρά **1**, και
   * ένας ορατός γονιός που έχασε 7 παιδιά μετρά **0** — γιατί ο χρήστης βλέπει
   * τον γονιό στη θέση του. Ο δείκτης απαντά «τι λείπει από την οθόνη μου», όχι
   * «πόσες διαδρομές φιλτραρίστηκαν».
   */
...
export function filterItemsByJob<T extends JobFilterableItem & { readonly subItems?: readonly T[] }>(
  ...
  for (const item of items) {
    ...
    // 🔑 Ε14.ιβ — ΤΟ ΙΔΙΟ ΜΕΤΡΟ ΚΑΙ ΣΤΑ ΔΥΟ ΕΠΙΠΕΔΑ. Ο μετρητής **δεν**
    // αυξάνεται από τα κλαδεμένα παιδιά ενός ΟΡΑΤΟΥ γονιού. Μετρημένο ζωντανά
    // (2026-08-02, «Οικονομικά»): ο δείκτης έλεγε **22** ενώ από το μενού
    // έλειπαν **9** κλάδοι — τα υπόλοιπα 13 ήταν κλειστά υπο-στοιχεία μέσα στο
    // ορατό `/reports` (7) και `/settings` (6). Ο Γιώργος ρώτησε «είναι σωστό;»:
    // ο δείκτης, αντί να πληροφορεί, γέννησε αμφιβολία — δηλαδή απέτυχε στον
    // σκοπό του. Ο κανόνας «ένα ανά κλάδο» γράφτηκε παραπάνω αλλά εφαρμοζόταν
    // **μόνο** στους κλάδους που φεύγουν· εδώ γίνεται συμμετρικός.
