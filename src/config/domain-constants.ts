...
export const API_ROUTES = {
  ...
  // ⚠️ «Σε ποιους χώρους ανήκω;» απαντιέται ΜΟΝΟ από τον διακομιστή — ποτέ με
  //    ερώτημα του πελάτη πάνω στα μέλη: μια σάρωση όλων των γραφείων είναι
  ...
  // ⛔ **ΟΧΙ μέσα στο `ADMIN`**, παρότι τις καλεί η κονσόλα διαχείρισης: οι διαδρομές
  //    είναι `/api/workspace-invitations/*`, **όχι** `/api/admin/*`. Η ομαδοποίηση εδώ
  //    ακολουθεί τη **διεύθυνση**, όχι τον καλούντα — αλλιώς η σταθερά ονομάζει δρόμο
  //    διαφορετικό από τον φάκελο που τον εξυπηρετεί, και ο επόμενος τον ψάχνει αλλού.
  //
  // 🔑 **ΚΑΜΙΑ `RESEND`, ΚΑΙ ΕΙΝΑΙ ΑΠΟΦΑΣΗ** (§7.1 · §7.3): η επαναποστολή είναι **το ίδιο
  //    `ISSUE` ξανά** με το ίδιο email — γεννά **νέο** token με νέα λήξη και ανακαλεί την
  //    προηγούμενη στην **ίδια** συναλλαγή (`supersededCount: 1`). Δεύτερη διαδρομή θα
  //    άφηνε ζωντανό τον παλιό σύνδεσμο, δηλαδή **δύο** κλειδιά για μία πόρτα.
  WORKSPACE_INVITATIONS: {
    /** Έκδοση **και** επαναποστολή — δες παραπάνω γιατί είναι η ίδια πράξη. */
    ISSUE: '/api/workspace-invitations',
    REVOKE: (invitationId: string) =>
      `/api/workspace-invitations/${encodeURIComponent(invitationId)}/revoke` as const,
    /** Η πράξη του προσκεκλημένου· το token ταξιδεύει στο **σώμα** (RFC 6819 §5.1.5). */
    REDEEM: '/api/workspace-invitations/redeem',
    /**
     * Η όψη **πριν** την απόφαση — δημόσια, μόνο με token.
     *
     * ⚠️ **Κωδικοποιείται**, και το μάθημα είναι μετρημένο (άγκυρα Σ2 της Φ5): το ωμό
     * `a+b/c=d` **δεν επιβιώνει** σε διεύθυνση. Ο κατασκευαστής δεν επιτρέπεται να
     * εξαρτάται από τη σημερινή αλφάβητο του `signed-token`.
     */
