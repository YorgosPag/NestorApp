 ...
 *
 * ## Συμβόλαιο
 * - **ΕΝΑΣ** ιδιοκτήτης κύκλου ζωής: το `useSelectedContactPersistence`. Κάθε
 *   σκόπιμος μηδενισμός (νέα επαφή, καθαρισμός φίλτρου URL, διαγραφή, αρχειοθέτηση,
 *   κάδος) περνά από το ίδιο state ⇒ **κανένας handler δεν χρειάζεται αλλαγή**. Χωρίς
 *   αυτό, μια διαγραμμένη επαφή θα «επανερχόταν» — χειρότερο από το αρχικό ελάττωμα.
 *   Οι συναρτήσεις εδώ είναι **καθαρές αποφάσεις**, ελέγξιμες χωρίς React.
 ...
 * @returns Η επαφή που αντιστοιχεί στο αποθηκευμένο id, αλλιώς `null`. ⚠️ Το `null`
 *          **δεν ξεχωρίζει** «σκουπίδι» από «η λίστα δεν φόρτωσε ακόμη». Γι' αυτό ο
 *          καλών δεν επιτρέπεται να σβήσει το κλειδί με βάση αυτό μόνο — η διάκριση
 *          παίρνεται από το {@link resolveLateSelectionRestore} όταν υπάρχει λίστα.
 */
