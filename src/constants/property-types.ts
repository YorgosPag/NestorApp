 ...
 *
 * 🔴 **ΑΛΛΑΞΕ Ο ΛΟΓΟΣ ΥΠΑΡΞΗΣ ΤΟΥ, ΚΑΙ ΤΟ ΓΡΑΦΟΥΜΕ ΑΝΤΙ ΝΑ ΤΟ ΑΦΗΣΟΥΜΕ ΝΑ ΠΑΛΙΩΣΕΙ**
 * (ADR-842 §7.6.12 / §8 #11). Εδώ έγραφε *«χρειάζεται επειδή το `PropertyType` **ΔΕΝ**
 * είναι μόνο οι κανονικές τιμές»* — **έπαψε να ισχύει** στις 2026-09-06: με τη φάση
 * contract η ένωση **έγινε ίση** με τις κανονικές, και το είδος κανονικοποιείται στο
 * σύνορο ανάγνωσης.
 *
 * 🔑 **Και όμως ΔΕΝ είναι νεκρός — ο πραγματικός του καταναλωτής ήταν πάντα άλλος**:
 * επτά μονάδες ευλογοφάνειας (`area-` · `floor-type-` · `layout-` · `orientation-` ·
 * `systems-` · `interior-features-plausibility`, και το `propertyClassOf`) δέχονται
 * **`unknown`** — τιμή φόρμας, ωμή είσοδο, `undefined` — και χρειάζονται **στένεμα
 * τύπου** πριν δεικτοδοτήσουν τους πίνακές τους. Αυτό είναι στένεμα από το `unknown`,
 * όχι από μια φαρδιά ένωση: η ερώτηση επιβίωσε, η **αιτιολογία** της άλλαξε.
 *
 * ⚠️ Το {@link PROPERTY_TYPE_I18N_KEYS} καλύπτει **μόνο** τις κανονικές· μια οθόνη που
 * το δεικτοδοτεί με ανεπίβεβαιη τιμή βάφει `undefined` ⇒ **ωμό κλειδί** ή κενό κελί.
 *
...
/**
 * **Το ΚΛΕΙΣΤΟ ιστορικό λεξιλόγιο** — ελληνικές τιμές γραμμένες πριν τις 2026-01-24.
 *
 * 🔴 **ΔΕΝ ΕΙΝΑΙ ΠΙΑ ΤΥΠΟΣ, ΚΑΙ ΕΙΝΑΙ ΑΠΟΦΑΣΗ** (ADR-842 §7.6.12 / §8 #11): μέχρι τις
 * 2026-09-06 ο τύπος `LegacyGreekPropertyType` έμπαινε στην ένωση `PropertyType`, άρα
 * ο μεταγλωττιστής **επέτρεπε ρητά** αυτές τις τιμές να ταξιδεύουν σε όλο τον τομέα.
 * Με τη φάση **contract** η ένωση στένεψε στις κανονικές, και η μετάφραση συμβαίνει
 * **μία φορά**, στο σύνορο ανάγνωσης.
 *
 * 🔑 **Ο πίνακας μένει, γιατί άλλαξε ΡΟΛΟΣ, όχι σημασία**: είναι πλέον **δεδομένα** —
 * η δηλωμένη, **κλειστή** απογραφή του τι μπορεί να κρύβει η βάση. Το
 * `PROPERTY_TYPE_ALIASES` ξέρει να τα μεταφράσει· η άγκυρα
 * `property-type-classes.test.ts` **εκτελεί** αυτόν τον πίνακα για να το αποδείξει
 * τιμή-τιμή, ώστε να μην είναι λίστα που πάλιωσε.
 *
 * ⚠️ **ΚΑΜΙΑ ΓΗ ΕΔΩ, ΚΑΙ ΕΧΕΙ ΣΗΜΑΣΙΑ**: τα `'Οικόπεδο'`/`'Αγροτεμάχιο'` **ποτέ** δεν
 * ήταν στην ένωση (η γη μπήκε 2026-08-20, πολύ μετά το πάγωμα αυτής της λίστας) —
 * έφταναν μόνο από ωμές αναγνώσεις, δηλαδή σε κανάλι που ο μεταγλωττιστής **δεν
 * έβλεπε**. Αυτό ακριβώς έκλεισε το σύνορο.
 */
