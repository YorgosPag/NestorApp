...
/**
 * Η κατάσταση στην οποία προσγειώνεται ένα **παρακολούθημα** (θέση στάθμευσης /
 * αποθήκη) όταν ακυρώνεται η κράτηση ή η πώληση του ακινήτου που το συνοδεύει.
 *
 * 🔴 **ΔΙΑΦΕΡΕΙ ΕΠΙΤΗΔΕΣ ΑΠΟ ΤΟ ΚΥΡΙΟ ΑΚΙΝΗΤΟ** — και αυτή η ασυμμετρία είναι η
 * ίδια η απόφαση, όχι παράλειψη. Στο **ίδιο** γεγονός ακύρωσης:
 *   - το **ακίνητο** πάει `for-sale` (το `revertPropertySaleWithPolicy` **πετά**
 *     αν ο στόχος είναι οτιδήποτε άλλο) — ξαναμπαίνει στην αγορά·
 *   - το **παρακολούθημα** πάει εδώ — **βγαίνει** από την αγορά.
 *
 * Ο λόγος είναι εμπορικός: ένα παρακολούθημα **δεν πωλείται αυτόνομα**. Θα πάει
 * ξανά *μαζί* με το ακίνητο στον επόμενο αγοραστή. Αν έμπαινε σε `for-sale` θα
 * γεννιόταν **φάντασμα προσφοράς** — η ίδια θέση προσφερόμενη και μόνη της και
 * ως παρακολούθημα. *(Απόφαση Giorgio 2026-08-11· ADR-777 §8.5α (δ).)*
 *
 * ⚠️ **Η ΤΙΜΗ ΑΥΤΗ ΔΕΝ ΑΛΛΑΖΕΙ ΤΙ ΒΛΕΠΕΙ Ο ΧΡΗΣΤΗΣ ΣΗΜΕΡΑ** — τη διορθώνει ως
 * *δήλωση*. Μέχρι τις 2026-08-11 η διαδρομή `revert` έγραφε `null`, τιμή **εκτός**
 * του κλειστού συνόλου, και ο μοναδικός ζωντανός αναγνώστης
 * (`useEntityStatusResolver`) την **μάντευε** με `?? 'unavailable'`. Ο γράφων
 * έγραφε άκυρα και ο αναγνώστης διόρθωνε σιωπηλά — τώρα η αλήθεια ζει στον
 * γράφοντα και ο μεταγλωττιστής μπορεί να την ελέγξει.
 *
 * 🔑 Είναι **ονομασμένη σταθερά και όχι σκέτο `DEFAULT_COMMERCIAL_STATUS`** παρότι
 * η τιμή συμπίπτει: εκείνο απαντά «τι είναι μια **νεοδημιουργημένη** μονάδα», αυτό
 * απαντά «πού **προσγειώνεται μια ακύρωση**». Ίδια τιμή, άλλο ερώτημα — και όταν
 * η μία απάντηση αλλάξει, δεν πρέπει να σύρει την άλλη μαζί της.
 */
export const APPURTENANCE_REVERTED_STATUS: CommercialStatus = 'unavailable';
