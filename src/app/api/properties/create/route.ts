...
/**
 * Property creation payload from client.
 * All fields except name are optional.
 */
interface PropertyLevelPayload {
  floorId: string;
  floorNumber: number;
  name: string;
  isPrimary: boolean;
...
// =============================================================================
// POST — Create Property via Admin SDK
// =============================================================================

/**
 * 🏢 ENTERPRISE: Create new property via Admin SDK
 *
 ...
 * @permission properties:properties:create
 * @see ADR-078
 */
export const POST = withStandardRateLimit(
  withAuth<ApiSuccessResponse<PropertyCreateResponse>>(
    async (request: NextRequest, ctx: AuthContext, _cache: PermissionCache) => {
      try {
        const body: PropertyCreatePayload = await request.json();
        ...
        const result = await createEntity('property', {
          auth: ctx,
        ...
        logger.error('[Properties] Error creating property', { error: getErrorMessage(error) });
        throw new ApiError(500, getErrorMessage(error, 'Failed to create property'));
      }
    },
    { permissions: 'properties:properties:create' }
  )
);
