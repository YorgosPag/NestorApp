...
export interface UnitLevelPayload {
  floorId: string;
  floorNumber: number;
  name: string;
  isPrimary: boolean;
...
export interface UnitPatchPayload extends Record<string, unknown> {
  name?: string;
  type?: string;
  status?: string;
  floor?: string | number;
 ...
 */
export function applyMultiLevelDefaults(body: UnitPatchPayload): void {
...
export function applyLevelDataAggregation(
  body: UnitPatchPayload,
  existingLevelsFromDoc: UnitLevelPayload[] | undefined,
): void {
  if (!body.levelData || typeof body.levelData !== 'object') return;

  const ld = body.levelData as Record<string, LevelData>;
  const activeLevels = (body.levels ?? existingLevelsFromDoc) as UnitLevelPayload[] | undefined;
