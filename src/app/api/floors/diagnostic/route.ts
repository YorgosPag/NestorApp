/**
 * 🔍 FIRESTORE CONNECTIVITY DIAGNOSTIC
 *
 * ΣΚΟΠΟΣ: Root cause analysis για floors timeout issues
 * ΣΤΑΝΤΑΡ: Enterprise-grade infrastructure diagnostics
 *
 * ΔΙΑΓΝΩΣΕΙΣ:
 * 1. Firestore connection health
 * 2. Environment variables validation
 * 3. Collection accessibility tests
 * 4. Query performance analysis
 * 5. Authentication & permissions check
 *
 * @author Claude Enterprise Infrastructure Diagnostics
 * @date 2025-12-17
 * @priority MAXIMUM - INFRASTRUCTURE FOUNDATION
 */
...
export async function GET(): Promise<NextResponse<FirestoreDiagnosticResult>> {
  const diagnosticStart = Date.now();
  console.log('🔍 FIRESTORE DIAGNOSTIC: Starting comprehensive connectivity analysis...');
  ...
  try {
    ...
    try {
      // Try to access a simple collection that should exist
      const testSnapshot = await getDocs(query(collection(db, COLLECTIONS.PROJECTS), limit(1)));
    ...
    // TEST 3: Collection Accessibility Tests
    ...
    for (const collectionName of collectionsToTest) {
      ...
      try {
        const testSnapshot = await getDocs(query(collection(db, collectionPath), limit(5)));
    ...
    console.log(`✅ DIAGNOSTIC COMPLETE: ${result.summary.overallHealth} (${totalDiagnosticTime}ms)`);
