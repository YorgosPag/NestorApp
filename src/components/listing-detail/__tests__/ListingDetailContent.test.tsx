...
function listing(over: Partial<PublicListing> = {}): PublicListing {
  return {
    ...
    floorplans: [],
    // 🔴 **ΑΠΑΙΤΕΙΤΑΙ ΑΠΟ ΤΟ ΣΧΗΜΑ ΑΠΟ ΤΗΝ ΕΚΔΟΣΗ 10** (ADR-845 Φ4.1) — και η απουσία του
    //    εδώ ΕΣΚΑΣΕ στη Φ4.3 με `Cannot read properties of undefined (reading 'filter')`,
    //    δηλαδή **ζωντανή επιβεβαίωση του Ο-6**: το `tsconfig` αποκλείει ρητά τα `__tests__`
    //    και το jest τρέχει με `@swc/jest` ⇒ **κανένα fixture δεν τυποελέγχεται ΠΟΤΕ**, άρα
    //    κάθε νέο πεδίο του δημόσιου σχήματος αφήνει πίσω του fixtures που **λένε ψέματα για
    //    το σχήμα** χωρίς να κοκκινίζουν — μέχρι να τα διαβάσει κάποιος.
    //    ⛔ Η θεραπεία είναι ΕΔΩ, ποτέ `?? []` στην οθόνη: ένα `??` θα έκρυβε το ψέμα και θα
    //    έκανε το «λείπει το πεδίο» να μοιάζει με «δεν υπάρχει μοντέλο».
    models: [],
    type: 'apartment',
    areaSqm: 95,
    offerKinds: ['sell'],
