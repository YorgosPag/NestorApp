            ...
            <tbody>
              {sorted.map((s, i) => (
                <tr key={s.supplierId} className="border-b last:border-0 hover:bg-muted/30">
                  ...
                  <td className="px-4 py-3 text-center">
                    <span className={cn(s.cancellationRate > 20 && 'text-[hsl(var(--text-warning))]')}>
                      {s.cancellationRate}%
                    </span>
                  </td>
                </tr>
...
function OnTimeBadge({ rate }: { rate: number }) {
  if (rate >= 80) {
    return <Badge variant="default" className="bg-[hsl(var(--bg-success))]/40 text-[hsl(var(--text-success))]">{rate}%</Badge>;
  }
  if (rate >= 50) {
...
function getRankColor(index: number): string {
  switch (index) {
    case 0: return 'text-[hsl(var(--text-warning))]';
    case 1: return 'text-muted-foreground';
    case 2: return 'text-[hsl(var(--text-warning))]';
