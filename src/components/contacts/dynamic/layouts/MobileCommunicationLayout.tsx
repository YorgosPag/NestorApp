...
function MobileCommunicationItem({
  ...
  return (
    <article
      className={`w-full max-w-none min-w-full p-2 ${quick.card}`}
      aria-label={`${config.title} item ${index + 1}`}
    >
      {/* 🎯 SEMANTIC FIELDSET: Groups related form controls */}
      <fieldset
        className="w-full max-w-none min-w-full space-y-2"
      ...
      >
        ...
        {config.type === 'social' && (
          <div className="w-full max-w-none min-w-full">
            <Label>{t('communication.tableHeaders.url')} ({t('common:autoGenerated')})</Label>
            <Input
              value={item.url || ''}
