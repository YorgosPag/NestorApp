 ...
 *
 * 🔑 **Το ίδιο κέλυφος με τις θέσεις στάθμευσης και τις αποθήκες** (`EntityMediaFilesTab`) — διαφέρουν η **σύνδεση**
 * (`propertyDossierMediaBinding`, κάτοχος-άνθρωπος) και το **εύρος** (`propertyDossierMediaTab`: τύποι εγγράφου από την
 * όψη του ενός καταλόγου, ADR-866 §2.10 Β1). Τα αρχεία γράφονται στο `people/{uid}/entities/property_dossier/{pdos}/…`
 * από τη μηχανή της Φ0, χωρίς δεύτερο αγωγό.
 *
...
import { propertyDossierMediaTab } from './property-dossier-media';
...
/** Μία καρτέλα αρχείων = σύνδεση φακέλου + ρύθμιση καρτέλας (ανά είδος ακινήτου). Ένα σημείο, τέσσερις χρήσεις. */
function DossierFilesTab({ dossier, tab }: DossierTabProps & { readonly tab: PropertyDossierFileTab }) {
  return <EntityMediaFilesTab binding={propertyDossierMediaBinding(dossier)} media={propertyDossierMediaTab(tab, dossier.type)} />;
}

export function PropertyDossierFloorplanTab({ dossier }: DossierTabProps) {
  return <DossierFilesTab dossier={dossier} tab="floorplan" />;
}

export function PropertyDossierDocumentsTab({ dossier }: DossierTabProps) {
  return <DossierFilesTab dossier={dossier} tab="documents" />;
}

export function PropertyDossierPhotosTab({ dossier }: DossierTabProps) {
  return <DossierFilesTab dossier={dossier} tab="photos" />;
}

export function PropertyDossierVideosTab({ dossier }: DossierTabProps) {
  return <DossierFilesTab dossier={dossier} tab="videos" />;
}
