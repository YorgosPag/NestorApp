  ...
  const goTo = useCallback((next: number) => {
    ...
    */
    ...
    /*
      ...
      Μετρήθηκε ζωντανά ότι η ομαλή κύλιση μπορεί να **μην κάνει απολύτως τίποτα** σε
      αυτόν τον κύλινδρο: με `scroll-behavior: smooth` το `scrollLeft` έμενε **0**, ενώ
      η **ίδια** κλήση με `auto` πήγαινε στο **361** — και επαληθεύτηκε ότι δεν φταίει
      ο handler (η περιτύλιξη του `scrollTo` κατέγραψε τη σωστή κλήση) ούτε το
      `prefers-reduced-motion` (μετρήθηκε `false`). Δηλαδή υπάρχει περιβάλλον όπου το
      κουμπί θα ήταν **νεκρό, χωρίς κανένα ίχνος**.
      ...
      ⚠️ **Διπλό `rAF`, όχι ένα**: το πρώτο κλείνει το τρέχον καρέ, το δεύτερο τρέχει
      **αφού** ο περιηγητής έχει συνθέσει — ένα μόνο είναι πολύ νωρίς και θα κατήγγελλε
      ως «ακίνητο» ένα smooth scroll που μόλις ξεκίνησε. Το ίδιο ιδίωμα με το §8.56.4.
      Και ο έλεγχος `target !== startedAt` κρατά τον φρουρό **αδρανή** όταν δεν
      ζητήθηκε καν μετακίνηση.
    */
    requestAnimationFrame(() =>
      requestAnimationFrame(() => {
        ...
        /*
          ...
          ΜΕΤΡΗΣΗ.** Ο φρουρός γράφτηκε πρώτα ως σκέτο `scroller.scrollLeft = target`
          και **έμεινε κι αυτός στο 0**: το CSS `scroll-behavior: smooth` δεν αφορά μόνο
          το `scrollTo` — κάνει ομαλή **κάθε** προγραμματιστική κύλιση, **συμπεριλαμβανο-
          μένης της ανάθεσης** `scrollLeft`. Δηλαδή ο φρουρός έπεφτε στην ίδια ακριβώς
          παγίδα που έφτιαχτε να πιάσει. Η στιγμιαία απενεργοποίηση είναι ο **μόνος**
          τρόπος να εγγυηθεί κανείς τη μετάβαση.
        */
