...
/**
 * 🔴 **ΤΟ ΣΠΑΣΙΜΟ ΠΟΥ ΚΑΝΕΙΣ ΔΕΝ ΕΙΔΕ, ΚΑΙ Ο ΛΟΓΟΣ ΠΟΥ ΔΕΝ ΤΟ ΕΙΔΕ** *(ADR-846, 2026-09-09)*.
 *
 * Η **Φάση 5β** πρόσθεσε **μία** γραμμή στον `AgencyShowcaseContent` — τον
 * `CoverageAgreementNotice`, που διαβάζει τις αγγελίες του γραφείου. Το `useAuth` εδώ
 * δίνει **υπαρκτό** `companyId`, οπότε ο αναγνώστης άνοιγε **πραγματικό**
 * `collection(db, …)` μέσα σε jsdom ⇒ `FirebaseError` **σε κάθε render**, και **9** από
 * τις άγκυρες αυτού του αρχείου κοκκίνιζαν για λόγο **άσχετο** με ό,τι φυλάνε.
 *
 * ⚠️ **Δεν ήταν σιωπηλό επειδή ήταν δύσκολο — ήταν σιωπηλό επειδή Η ΣΟΥΙΤΑ ΔΕΝ ΕΤΡΕΞΕ:**
 * το φρένο βαρέων εργασιών ήταν κατειλημμένο σε **~12** συνεχόμενες απόπειρες εκείνης της
 * συνεδρίας *(ίδιος λόγος που οι άγκυρες `Η1`–`Η6` έμειναν «γραμμένες, όχι
 * αποδεδειγμένες»)*. **Πύλη που δεν εκτελείται δεν είναι πύλη** — το ίδιο μάθημα με το
 * CHECK 3.54, μια βαθμίδα πιο πάνω.
 *
 * 🔑 **Η θεραπεία είναι ΔΗΛΩΣΗ ΓΕΙΤΟΝΑ, όχι αλλαγή του component**: η ανάγνωση είναι
 * σωστή στην παραγωγή· αυτό το αρχείο απλώς **δεν την είχε δηλώσει** — όπως δηλώνει ήδη
 * το `firestore-query.service` για τη βιτρίνα, δύο mocks πιο πάνω.
 */
jest.mock('@/services/realtime/hooks/usePublicListings', () => ({
  usePublicAgencyListings: () => ({ listings: [], loading: false, error: null }),
}));
