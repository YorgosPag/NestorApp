...
import { MARK_CARD_SLOT, markBox, type ShowcaseMarkSize } from '../showcase-mark-box';
...
describe('Κ3 — 🔴 Α21.15: Η ΚΑΡΤΑ ΔΙΝΕΙ ΖΩΝΗ, ΚΑΙ Ο ΡΥΘΜΟΣ ΜΕΝΕΙ — ΜΕ ΣΤΑΘΕΡΗ ΣΤΗΛΗ', () => {
  it('🔴 στην κάρτα, ο wordmark παίρνει ΑΛΛΟ κουτί από το τετράγωνο', () => {
    // 🔴 **Η παλιά υποχώρηση («ίδιο κουτί») μετρήθηκε στην παραγωγή**: 512×122 ⇒ 36×9
    //    μελάνι. Η Φάση Β δεν το έλυσε, γιατί το λογότυπο ήταν ήδη τριμμένο.
    expect(markBox('card', logo(4))).not.toEqual(markBox('card', logo(1)));
    expect(markBox('card', logo(4)).className).toBe('h-12 w-24');
  });

  it('🔴 η ζώνη της κάρτας έχει ΣΤΑΘΕΡΟ πλάτος — ποτέ `w-auto`, αλλιώς σπάει ο ρυθμός', () => {
    // 🔴 **Η ΜΕΤΑΛΛΑΞΗ**: δώσε στο `BAND_BOX.card` ελεύθερο πλάτος ⇒ κοκκινίζει.
    expect(markBox('card', logo(4)).className).not.toMatch(/w-auto/);
  });

  it('🏆 η στήλη της κάρτας έχει ΑΚΡΙΒΩΣ το πλάτος της ζώνης — ο τίτλος στο ίδιο `x`', () => {
    // 🔑 Αν η στήλη ήταν στενότερη, η ζώνη θα ξεχείλιζε· αν φαρδύτερη, κενό χωρίς λόγο.
    const bandWidth = /\bw-(\d+)\b/.exec(markBox('card', logo(4)).className)?.[1];

    expect(MARK_CARD_SLOT).toContain(`sm:w-${bandWidth}`);
    // ⚠️ Στο κινητό η κάρτα στοιβάζεται — η στήλη ΔΕΝ πρέπει να δεσμεύει πλάτος εκεί.
    expect(MARK_CARD_SLOT).not.toMatch(/(^|\s)w-\d/);
  });
