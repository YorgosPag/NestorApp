  ...
  return (
    ...
    >
      ...
      {/* ═══════════════════════════════════════════════════════════════════════
          ...
          🔴 **ΕΤΙΚΕΤΑ, ΟΧΙ ΓΡΑΜΜΑΤΙΚΟ ΥΠΟΚΕΙΜΕΝΟ — ΚΑΙ ΤΟ ΕΠΙΒΑΛΕ Η ΟΘΟΝΗ**
          *(ζωντανό περπάτημα 2026-09-10, ADR-846 §8.9)*.

          Η πρώτη γραφή έλεγε ``Ο {area} δεν είναι…`` και **παρήγαγε ΛΑΘΟΣ ΓΕΝΟΣ**: η
          οθόνη έγραψε **«Ο ΔΗΜΟΤΙΚΗ ΕΝΟΤΗΤΑ ΘΕΣΣΑΛΟΝΙΚΗΣ»**. Το άρθρο ήταν σωστό για
          **έναν** τύπο και λάθος για **πέντε** — μετρημένο στα ίδια τα δεδομένα:
          `municipality` **αρσενικό** *(333/333 «ΔΗΜΟΣ…»)* · `municipal_unit` *(948/948)* ·
          `community` *(6.062/6.062)* · `region` · `regional_unit` ·
          `decentralized_administration` **θηλυκά** — και το **ΑΓΙΟ ΟΡΟΣ** *(που εμφανίζεται
          σε **ΔΥΟ** τύπους)* **ουδέτερο**.

          ⛔ **Δεν λύθηκε με ICU `select` πάνω στον τύπο**, που ήταν το προφανές: θα
          δούλευε για τους έξι τύπους και θα κουβαλούσε το **«ΑΓΙΟ ΟΡΟΣ» ως εξαίρεση
          ονόματος μέσα στον κώδικα** — δηλαδή γραμματική εξαρτημένη από δεδομένα, που
          σπάει σιωπηλά την επόμενη φορά που το λεξιλόγιο αποκτά όνομα άλλου γένους.

          🔑 **Η θεραπεία αφαιρεί το πρόβλημα αντί να το κωδικοποιεί**: το όνομα βγαίνει
          **έξω** από την πρόταση και γίνεται **ετικέτα**. Κερδίζουμε τρία πράγματα μαζί —
          *(α)* καμία γλώσσα δεν χρειάζεται πια γένος, *(β)* το όνομα **ξεχωρίζει οπτικά**
          αντί να χάνεται μέσα σε πρόταση, *(γ)* ένα μακρύ όνομα *(«ΑΠΟΚΕΝΤΡΩΜΕΝΗ ΔΙΟΙΚΗΣΗ
          ΜΑΚΕΔΟΝΙΑΣ - ΘΡΑΚΗΣ»)* διαβάζεται, αντί να σπάει τη ροή της πρότασης.
          ═══════════════════════════════════════════════════════════════════════ */}
      {areaName !== null && (
        <p className="m-0 text-sm font-semibold text-foreground" data-testid="coverage-reach-area">
          {areaName}
        </p>
      )}

      <p className="m-0 text-sm text-foreground">
        {t(areaName === null ? REACH_KEYS.unnamed : REACH_KEYS.named)}
      </p>
