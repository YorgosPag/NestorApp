...
export const SHOWCASE_MARK_KEYS = {
  ...
  /**
   * 🔴 **ΤΡΙΑ ΜΗΝΥΜΑΤΑ, ΟΧΙ ΔΥΟ — ΚΑΙ ΤΟ ΤΡΙΤΟ ΤΟ ΓΕΝΝΗΣΕ ΖΩΝΤΑΝΗ ΜΕΤΡΗΣΗ.**
   *
   * Η πρώτη γραφή είχε **δύο**, και διάλεγε με `coversCard ? blurryPage : blurryEverywhere`.
   * Ήταν **ψέμα σε ολόκληρη ζώνη**: για `shortest` ανάμεσα σε **128 και 255** ισχύουν
   * *και τα δύο* `coversCard` **και** `coversPage`, οπότε η οθόνη ανακοίνωνε *«θολό στη
   * σελίδα»* για εικόνα που τη **σελίδα την καλύπτει μια χαρά**.
   *
   * ⚠️ **Και το παράδειγμα της τεκμηρίωσης ήταν ΜΕΣΑ στη ζώνη**: το ADR έγραφε *«180px —
   * καθαρό στην κάρτα, θολό στη σελίδα»*, και τα 180 καλύπτουν τη σελίδα στο 2×. Δηλαδή
   * **το ίδιο το παράδειγμα ήταν το ψέμα** — ίδιο σχήμα με το Ο-18 *(«το alt που λέει
   * ψέμα»)*: ισχυρισμός αληθοφανής, λανθασμένος, σε γραμμή που κανείς δεν επαληθεύει.
   *
   * ⇒ Οι ζώνες είναι **τρεις**, γιατί οι επιφάνειες είναι **δύο**:
   *
   * | `shortest` | κάρτα | σελίδα | μήνυμα |
   * |---|---|---|---|
   * | 64-87 | ✗ | ✗ | {@link blurryEverywhere} |
   * | 88-127 | ✓ | ✗ | {@link blurryPage} |
   * | 128-255 | ✓ | ✓ | {@link blurryDense} — *«καθαρό και στα δύο· θολό μόνο σε πολύ πυκνή οθόνη»* |
   */
  blurryPage: `${K}.mark.blurryPage`,
  /** Δες {@link blurryPage} — η ζώνη που **έλειπε** και έλεγε ψέματα. */
  blurryDense: `${K}.mark.blurryDense`,
