...
type SocialPlatform = 'facebook' | 'instagram' | 'linkedin' | 'twitter' | 'youtube' | 'github' | 'other';
type SocialItem = { platform: SocialPlatform; url: string; label?: string | null };
...
export function ProjectShowcaseHeader({ company, projectName }: ProjectShowcaseHeaderProps) {
  const { t } = useTranslation('showcase');
  const logoSrc =
    company.logoUrl && company.logoUrl.trim().length > 0 ? company.logoUrl : FALLBACK_LOGO_URL;
  ...
  const socials = (company.socialMedia ?? []) as SocialItem[];

  const hasContacts =
    addresses.length > 0 || phones.length > 0 || emails.length > 0 || websites.length > 0 || socials.length > 0;
    ...
    <header className="bg-[hsl(var(--showcase-surface))] text-[hsl(var(--showcase-fg))] px-6 py-5 rounded-xl shadow-md border border-[hsl(var(--showcase-border))] grid gap-4 md:grid-cols-[auto_1fr] md:items-start">
      ...
      <div className="min-w-0">
        ...
        <p className="text-sm text-[hsl(var(--showcase-muted-fg))] text-center md:text-left">
          {company.name} · {t('projectShowcase.header.subtitle')}
        </p>
