...
export function ScheduleVarianceTable({
  rows,
  loading,
  baselineData,
  onClearBaseline,
      ...
      if (row.type === "phase") {
        ...
        if (expandedPhases.has(row.id)) {
          const children = rows.filter(
            (r) => r.type === "task" && r.parentId === row.id,
          );
  ...
  if (!loading && isEmpty) {
    return (
      ...
      >
        <ReportEmptyState title={t("tabs.timeline.dashboard.empty.noPhases")} />
      </ReportSection>
    );
  }
      ...
      {isComparing && baselineData && (
        <div className="flex items-center gap-2 mb-3 rounded-md bg-primary/10 px-3 py-1.5 text-sm">
          ...
          <span className="font-medium">
            {t("tabs.timeline.dashboard.baseline.variance.comparingTo", {
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className={cn("border-b", colors.text.muted)}>
              ...
              <th
                scope="col"
                className="text-left py-2 px-2 font-medium hidden md:table-cell"
              >
              ...
              {isComparing && (
                <>
                  <th
                    scope="col"
                    className="text-left py-2 px-2 font-medium hidden lg:table-cell text-primary/80"
                  >
                    {t(
                      "tabs.timeline.dashboard.baseline.variance.colBaselineStart",
                    )}
                  </th>
                  <th
                    scope="col"
                    className="text-left py-2 px-2 font-medium hidden md:table-cell text-primary/80"
                  >
                    {t(
                      "tabs.timeline.dashboard.baseline.variance.colBaselineEnd",
                    )}
                  </th>
                  <th
                    scope="col"
                    className="text-right py-2 px-2 font-medium text-primary/80"
                  >
                    {t(
                      "tabs.timeline.dashboard.baseline.variance.colBaselineVariance",
                    )}
                  </th>
                </>
              )}
                      ...
                      {isPhase ? (
                        <button
                          ...
                          aria-label={
                            isExpanded
                              ? t(
                                  "tabs.timeline.dashboard.variance.collapsePhase",
                                  { name: row.name },
                                )
                              : t(
                                  "tabs.timeline.dashboard.variance.expandPhase",
                                  { name: row.name },
                                )
                          }
                        >
                  ...
                  {isComparing &&
                    (() => {
                      ...
                      const bVar =
                        bEnd && row.plannedEnd
                          ? Math.round(
                              (new Date(row.plannedEnd).getTime() -
