...
export function FloorInlineCreateForm({
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      const parsedElevation = createElevation ? parseFloat(createElevation) : undefined;
      const parsedHeight = createHeight ? parseFloat(createHeight) : undefined;

      // Use the ADR-369 factory to derive kind, longName, nameAutoGenerated,
      // longNameAutoGenerated, and finishThickness from the form values.
      const floor = createFloor({
        number: num,
        name: createName.trim(),
        buildingId,
        ...(parsedElevation !== undefined && !isNaN(parsedElevation) ? { elevation: parsedElevation } : {}),
        ...(parsedHeight !== undefined && !isNaN(parsedHeight) ? { height: parsedHeight } : {}),
        ...(projectId ? { projectId } : {}),
      });
