 ...
 *
 * ── 🔴 ΤΡΕΙΣ ΟΙΚΟΓΕΝΕΙΕΣ ΚΑΡΤΩΝ, ΤΡΕΙΣ ΑΡΙΘΜΟΙ — ΜΕΤΡΗΜΕΝΟ, ΟΧΙ ΠΡΟΤΙΜΗΣΗ ──
 *
 ...
 *
 * Η **τρίτη** οικογένεια γεννήθηκε από τη μετανάστευση του ADR-784 §10 και **επίσης από
 * μέτρηση, όχι από γούστο**: το `EntryCard` των επιλογέων μεταφόρτωσης είναι **εικονίδιο και μία
 * ετικέτα** — τίποτα άλλο. Δοκιμάστηκε το πλακίδιο και **απορρίφθηκε με αριθμούς**: στο κινητό
 * το δοχείο του είναι ~344 px, οπότε ελάχιστο 240 px δίνει **μία** στήλη εκεί που ο σχεδιαστής
 * έστειλε **δύο** — δηλαδή ένα τσιπάκι με εικονίδιο 40 px θα καταλάμβανε ολόκληρη τη γραμμή.
 *
 * **Ένας μηχανισμός, τρία δηλωμένα ελάχιστα** — αυτό είναι κεντρικοποίηση· ένας αριθμός για
...
/**
 * **Τσιπάκι εικονιδίου-και-ετικέτας** (`EntryCard` των επιλογέων μεταφόρτωσης) — η **τρίτη**
 * οικογένεια, και ο μόνος αριθμός που **αναπαράγει ακριβώς τη σκάλα που έστειλε ο σχεδιαστής**.
 *
 * **Δάπεδο από τις ίδιες του τις δηλώσεις:** περίγραμμα 2+2 = 4 · εσωτερικό περιθώριο 8+8 = 16 ·
 * κύκλος εικονιδίου 40 ⇒ **60 px** μόνο για το κέλυφος. Η ελληνική ετικέτα στα 12 px θέλει
 * ~100 px για να μη σπάσει η μακρύτερη συνηθισμένη λέξη («Αρχιτεκτονική» ≈ 81 px) ⇒ **~120 px**
 * απόλυτο δάπεδο.
 *
 * **Γιατί ΑΚΡΙΒΩΣ 10rem και όχι 9 ή 11:** με το κενό των 8 px που ήδη χρησιμοποιούν αυτά τα
 * πλέγματα, το βήμα κελιού γίνεται 168 px και αναπαράγει **και τα τρία** σκαλιά της υπάρχουσας
 * σκάλας, τώρα όμως μετρώντας το **δοχείο** αντί για το παράθυρο:
 *
 *   δοχείο ~344 px (κινητό) → 2 στήλες   ·   ~600 px → 3 στήλες   ·   ~800 px → 4 στήλες
 *
 * Δηλαδή **μηδέν οπτική αλλαγή** εκεί που η σκάλα έλεγε την αλήθεια, και διόρθωση εκεί που
 * έλεγε ψέματα: σε παράθυρο 1400 px με πλαϊνό μενού και στενό ταμπλό 400 px, η σκάλα έδινε
 * **τέσσερα** τσιπάκια των 91 px — στριμωγμένα κάτω από το δάπεδο των 120 px.
 *
 * ⚠️ **ΜΗΝ το χρησιμοποιήσεις για πλακίδιο με μεταδεδομένα.** Το κριτήριο δεν είναι «μικρό
 * πράγμα» — είναι «**ένα γλυφικό και μία ετικέτα, χωρίς σειρές μεταδεδομένων**». Το `GroupCard`
 * του ίδιου αρχείου έχει τίτλο **και** υπότιτλο πλήθους, άρα είναι πλακίδιο.
 */
const CATALOG_COLUMNS_CHIP = 'grid-cols-[repeat(auto-fill,minmax(min(100%,10rem),1fr))]';
...
export const gridPatterns = {
  ...
  cards: {
    ...
    /** Κατάλογος από **τσιπάκια εικονιδίου-και-ετικέτας**. Σκεπτικό πάνω από το `CATALOG_COLUMNS_CHIP`. */
    chip: CATALOG_COLUMNS_CHIP,
    gap: 'gap-4',
